---
name: tianapi-saylove-get
description: 随机获取一句土味情话。当用户需要情话、撩人语录或幽默表白时使用此技能。
homepage: https://www.tianapi.com/apiview/80
metadata: {"openclaw": {"emoji": "💖", "requires": {"bins": ["python3"], "env": ["TIANAPI_SAYLOVE_KEY"]}, "primaryEnv": "TIANAPI_SAYLOVE_KEY"}}
---
...
### 获取土味情话
    ...
    ```bash
    # 获取一句土味情话
    python3 scripts/fetch_saylove.py <API_KEY>
...
### 直接调用 API（无需脚本）
    ...
    ```bash
    # 获取一句土味情话
    GET https://apis.tianapi.com/saylove/index?key=YOUR_API_KEY
...
当用户需要情话、撩人语录或幽默表白时，按以下步骤操作：

1.  **识别意图**：用户想获取一句土味情话。
2.  **调用脚本**：执行 `python3 scripts/fetch_saylove.py` 命令，传入API Key。
3.  **展示结果**：解析返回的JSON数据，直接展示情话内容。

### 返回字段说明

| 字段 | 含义 | 示例 |
| :--- | :--- | :--- |
| content | 情话内容 | 你有打火机吗？没有啊，那你是怎么点燃我的心的？ |
...
`scripts/fetch_saylove.py` - 封装了土味情话获取功能。
