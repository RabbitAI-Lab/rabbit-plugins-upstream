...
async function runOrThrow(file, args, label) {
...
async function acquireBootstrapLock() {
    while (true) {
        try {
            ...
            await writeFile(path.join(bootstrapLockDir, 'owner.json'), JSON.stringify({ pid: process.pid, createdAt: Date.now() }), 'utf8');
            return async () => {
    ...
    bootstrapPromise = (async () => {
        ...
        try {
            ...
            if (installedDigest !== digest) {
                ...
                await runOrThrow(pythonBinPath(), ['-m', 'pip', 'install', '-r', requirementsPath], 'python dependency install');
...
export async function callPythonHelper(command, payload = {}) {
    ...
    const { code, stdout, stderr } = await execFileNoThrow(pythonBinPath(), [helperPath, command, '--payload', JSON.stringify(payload)], { useCwd: false });
