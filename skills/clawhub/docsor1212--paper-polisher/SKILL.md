...
version: 2.0.0
description: >
  AI Detection · 4-Layer DeAI Gate · De-AI Rewriting · Terminology Check · Metaphor Audit · Quality Report.
  Bilingual (CN/EN), 100% local, zero upload. 9-layer word engine with 1002 patterns and
  model-specific fingerprinting for Chinese LLMs, PLUS stylometric style analysis that catches
  AI-paraphrased drafts which dodge word blacklists (EVAL gap 49.2), translation-smell layer,
  and a weighted-fusion composite verdict from one command.
tags: [ai-detection, deai, terminology, academic-writing, paper-polish]
---

# paper-polisher-pro — AI Detection & Academic Paper Polishing

> 🎯 **4-Layer DeAI Gate · AI Detection · De-AI Rewriting · Paraphrase · Terminology Check · Metaphor Audit · Quality Report**
...
### 4-Layer DeAI Gate — one command, composite verdict (new in v2.0.0)
...
## Extended: Complete Article Review Workflow

For full pre-publication review (terminology + AI detection + metaphor/analogy audit), see [`references/article-review-workflow.md`](references/article-review-workflow.md).

Key additions beyond the core 5-step workflow:
- **Metaphor/Analogy Audit**: Extract all figurative language, assess scientific accuracy, check imagery-system consistency, flag over-explanation
- **Parallel execution**: Run `term_check.py` and `ai_detector.py` simultaneously, then do metaphor audit while reading
- **Fix priority**: Model fingerprints (DeepSeek/GLM) > AI openers > RLHF patterns > markdown formatting (ignore last)
- **Presentation rule**: Consolidate findings into ONE actionable table, get user confirmation once, then execute all patches. Don't write essays.
...
## Article Review Delivery Workflow

When reviewing articles that live on 归墟 Hub (or any web note platform), follow this sequence:

### ⚠️ Critical: Edit the web note, not just the local file

The user's article lives on 归墟 Hub at `http://192.168.3.82:8765`. After running automated checks and confirming changes:

1. **Fetch current note** via API (`GET /api/notes/{id}`) — note the `version` number
2. **Apply the same edits to the归墟 content** (not a different copy) — images, tables, and figure descriptions must be preserved
3. **PUT the updated content** with `version=current_version` (see归墟 API rules below)
4. **Generate PDF from rendered HTML** and send to user

**Do NOT** just edit the local markdown file and send it as-is. The user expects the归墟 web note to be the source of truth, and a PDF from the rendered content.

### PDF Generation Pipeline (归墟 SPA → PDF)
...
1. Fetch note content via API
2. Build standalone HTML: inline `marked.min.js` (no CDN — SSL may fail in headless env), embed SVG figures as `data:image/svg+xml;base64,...` `<img>` tags
3. Run `google-chrome --headless --disable-gpu --no-sandbox --virtual-time-budget=5000 --print-to-pdf=output.pdf --no-margins file:///tmp/article.html`
4. Verify PDF with PyMuPDF (`fitz.open` → check page count + first-page text)

See `references/guiwu-pdf-pipeline.md` for full details.
...
- **v1.2.0** — Added `content_only_score` field (excludes markdown_format false positives, reduces 92%+ noise in high-risk paragraph lists); ai_patterns_zh.json v3.3.0 adds `translationese` category (14 patterns) + 5 medical metaphor patterns to `ai_cliches` (89→94)
