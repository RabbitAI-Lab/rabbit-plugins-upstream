...
> 🎯 **四层去AI化门禁 · AI痕迹检测 · 去AI味改写 · 智能降重 · 术语标准化 · 综合质量报告**
...
"润色论文", "降AI检测", "查AI写作", "论文润色", "改写论文", "去AI味", "AI论文检测", "学术写作助手", "AI写作检测", "去除AI痕迹", "人化AI文本", "毕业论文润色", "学位论文降重", "SCI论文编辑", "手稿润色", "AI写作评分", "去AI化", "AI改写检测", "文风对标顶刊", "这篇文章像不像AI"
...
### 四层去AI化门禁——一条命令出融合综合分（v2.0.0 新增）

```bash
python3 {{SKILL_DIR}}/scripts/deai_gate.py your_paper.txt        # 人类可读
python3 {{SKILL_DIR}}/scripts/deai_gate.py your_paper.txt --json # 管线集成
```
...
词级黑名单抓不住"AI改写稿"：把"全景"拉黑，AI 就改用"图景"——同义词搬运，AI 腔纹丝不动。门禁新增与主题无关的文体特征层，这类稿件躲不掉（实测正负样本差距 49.2，同类样本词级分接近失效）。

```
综合分 = 0.35×词级分 + 0.45×(100−文体分) + 0.20×翻译腔分
<35 通过 · 35–55 需复核 · ≥55 疑似AI   （术语层为建议性）
```

| 层 | 脚本 | 抓什么 | 盲区 |
|---|---|---|---|
| 层1 术语 | `term_check.py` | 非标准术语（2308条） | 与AI无关（质量项） |
| 层2 词级 | `ai_detector.py` | 已知AI套话、模型指纹、段落均匀度 | 新造变体词 |
| 层3 翻译腔 | `translation_smell_check.py` | 翻译腔语法+术语盲区新词 | 中文原生问题 |
| 层4 文体 | `style_distance.py` | AI改写稿句法：句长CV/标点单调/虚词密度/长句占比（中英分别建模，绝对阈值，不依赖语料库） | 刻意模仿人类节奏的深度改写 |

诚实边界：所有分数是概率性判断非定罪；层4 排除参考文献/表格/引用块，医学专业长复合句是人类特征不会被罚。推荐闭环：`deai_gate.py` 检测 → 改写标记段落（长短句交错、标点多样化）→ 复测。
...
- **v2.0.0** — 四层去AI化门禁：新增文体特征层（抓AI改写稿，正负差距49.2）、翻译腔层、加权融合综合分（deai_gate.py 一条命令）；term_check 升级至2308条术语；血统：ImmunoLens 4层门禁系统并入
