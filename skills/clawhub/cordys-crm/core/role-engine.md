...
└─ 不存在 →
    ...
    ├─ 将结果写入 user-role.md         持久化用户身份
    └─ 匹配角色，加载 profiles/{role}.md
```

**换账号 / 刷新身份**：用户说"刷新身份"或"换账号" → 重新执行上述流程，覆盖 user-role.md。

> `user-role.md` 位于 skill 根目录，由系统自动管理，请勿手动编辑。
...
|------|------|
| 用户说"刷新身份" | 重新执行初始化 |
| 用户说"换账号" | 清除 user-role.md + 重新初始化 |
| 连续 3 次 API 调用返回 401/403 | 提示用户检查密钥，建议刷新 |
| 从创建起超过 7 天 | 后台静默刷新（不打扰用户） |

### 约束
- `user-role.md` 是运行时产物，**不提交版本控制**
- AI 每次对话第一件事：确认 user-role.md 就绪且有效
- 如果 user-role.md 存在但解析失败（格式损坏），视为不存在
