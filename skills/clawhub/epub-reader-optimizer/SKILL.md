...
## Aligning Requirements with User（建议优先询问）
...
### Issue 2: No explicit class for standalone formula paragraphs
...
依赖：`fonttools`、`brotli`。不要自动运行 `pip install`；报告缺失依赖并等待用户手动安装。
