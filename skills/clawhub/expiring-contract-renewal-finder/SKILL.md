---
name: expiring-contract-renewal-finder
description: 临期项目与续约商机发现助手。当用户想查询即将到期的项目合同、寻找续约/替换机会时，必须使用此SKILL：临期项目查询（合同到期前0-180天）、现供应商（在位者）识别、到期紧急度排序、按采购单位类型（政府/学校/医院/国企等）筛选、以及配套的拟建项目与采购意向商机扫描。给一个行业/地区即输出按价值排序的商机清单。即使用户没有提到「临期」，只要涉及合同快到期的项目、续约机会、替换现供应商等需求，都应使用本SKILL。
metadata: { "openclaw": {"requires": {"env":["ZLBX_API_KEY"]},"primaryEnv": "ZLBX_API_KEY"}}
---

# 临期续约商机 · 现供应商合同到期窗口

专挖**存量合同到期**的替换机会：谁家的服务合同快到期、现供应商是谁、还剩多少天，按紧急度排序，替换方案提前准备。配合拟建与意向两路，商机全覆盖。

## 常见使用场景

- 「我们是做物业的，看看江苏有哪些快到期要续约的物业项目」
- 「未来90天到期的食堂承包项目有哪些？现在是谁在做」
- 「政府和学校的安保服务合同快到期的帮我列一下」
