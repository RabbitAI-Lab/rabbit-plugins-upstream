---
name: amazon-review-alerts
display_name: 亚马逊差评预警 · 评论舆情监控提醒
description: >
  亚马逊差评预警 Skill。订阅 ASIN 后服务端持续监控评论，差评突增、
  星级下滑自动生成预警；会话内一条命令读取未读预警并定位问题评论，
  可继续生成差评根因分析与回复建议。Use when the user asks about review alerts,
  negative review spikes, rating drops, 差评预警、口碑监控、星级下滑提醒、舆情监控。
  Requires an ARI API key (ari_live_*).
author: ARI (funewa)
agent_created: true
slug: review-alerts
displayName: 亚马逊差评预警 · 评论舆情监控提醒
version: 1.4.7
summary: 订阅 ASIN 后自动监控差评突增，会话内直接读预警
license: MIT
---
