...
FETCH_TIMEOUT = 15
MAX_AGE_HOURS = 168  # 7天窗口，抓取层宽进；时效性由输出端控制
...
def compute_relevance_score(item, keywords_config, source_layer):
    """计算相关性得分 0-10。得分越高越相关。
    ...
    for kw in keywords_config["global_filter"]["core_keywords"]:
        if _combo_match(kw, full_text):
            core_hits += 1
    score += min(core_hits * 3, 6)

    # AI 关键词命中
    ai_kws = keywords_config["global_filter"]["context_ai"]
    ai_title_hits = sum(1 for kw in ai_kws if _word_match(kw, title))
    ai_summary_hits = sum(1 for kw in ai_kws if _word_match(kw, summary))
    # 标题命中权重2x，摘要1x
    ai_score = ai_title_hits * 2 + ai_summary_hits * 0.5
    score += min(ai_score, 4)

    # 游戏关键词命中
    game_kws = keywords_config["global_filter"]["context_game"]
    game_title_hits = sum(1 for kw in game_kws if _word_match(kw, title))
    game_summary_hits = sum(1 for kw in game_kws if _word_match(kw, summary))
    game_score = game_title_hits * 2 + game_summary_hits * 0.5
    
    # Layer 1 源本身是游戏媒体，游戏维度给固定基础分（不叠加关键词）
    # 但 AI 维度必须有实质命中才能拿到及格分
    if source_layer == 1:
        game_score = 2  # 固定基础分
        # 核心检查：AI 维度必须至少命中 1 个非泛词，或命中 2 个任意 AI 词
        if ai_title_hits == 0 and ai_summary_hits <= 1:
            # 标题无 AI 词且摘要最多 1 个，基本不是 AI 相关
            score = min(score, 2)  # 压到不及格
    score += min(game_score, 4)

    # 扣分项：如果 AI 关键词只命中了泛词（"智能"、"自动"、"算法"），扣分
    vague_ai_words = {"智能", "自动", "算法", "自动化"}
    if ai_title_hits + ai_summary_hits > 0:
        specific_ai_hits = sum(1 for kw in ai_kws if _word_match(kw, full_text) and kw not in vague_ai_words)
        if specific_ai_hits == 0:
            score -= 2

    # Layer 2/3 约束：标题里必须有至少 1 个 AI 词或核心词命中，否则压低分数
    if source_layer >= 2 and core_hits == 0 and ai_title_hits == 0:
        score = min(score, 3)  # 标题无 AI 元素，压到不及格
...
def is_relevant(item, keywords_config, source_layer):
    """判断是否相关。基于评分系统，>=4分才保留。"""
    score = compute_relevance_score(item, keywords_config, source_layer)
...
def classify_item(item, keywords_config):
    ...
    额外约束：industry 分类必须同时含 AI 元素，纯行业新闻不收。
    """
    ...
    # industry 分类额外约束：必须同时含 AI 元素
    if primary == "industry":
        ai_kws = ["AI", "人工智能", "大模型", "LLM", "AIGC", "生成式",
                  "GPT", "Claude", "神经网络", "深度学习", "机器学习",
                  "machine learning", "deep learning", "artificial intelligence"]
        has_ai = any(kw.lower() in text for kw in ai_kws)
        if not has_ai:
            # 没有 AI 元素，降级为不相关（返回 None 表示应被过滤）
            return None, None
...
# 二手转载源列表——这些源的内容通常是翻译/整合
SECONDARY_SOURCES = [
    "IT之家", "IT之家（RSS）", "量子位", "新智元", "CSDN",
    "机器之心",
]

# 有独立分析/点评能力的媒体——即使是二手源，如果内容含评论/分析则保留原文
ANALYSIS_INDICATORS = [
    "分析", "解读", "点评", "盘点", "周报", "深度", "观察",
    "评论", "看法", "认为", "表示", "战略", "布局",
    "市场研究", "报告", "洞察", "趋势",
    "analysis", "insight", "review", "opinion", "deep dive",
    "commentary", "perspective", "roundup", "weekly",
]
...
def has_original_analysis(item):
    """判断二手源的内容是否含有独立分析/点评（而非纯搬运）"""
    title = item.get('title', '').lower()
    summary = item.get('summary', '').lower()
    text = f"{title} {summary}"
    hits = sum(1 for indicator in ANALYSIS_INDICATORS if indicator.lower() in text)
    return hits >= 2  # 至少命中2个分析指示词才算有独立分析
...
def main():
    ...
    for source in sources_config.get("sources", []):
        ...
        # 跳过已知损坏的源
        if source.get("status", "").startswith("broken"):
            print(f"  ⚠️  [{source.get('tier','?')}] {name} (跳过: {source.get('status')})", file=sys.stderr)
            continue
    ...
    for item in deduped:
        ...
        # classify_item 返回 None 表示不相关（如 industry 但无 AI 元素）
        if primary is None:
            continue
        ...
        if is_secondary_source(item.get("source", "")):
            if has_original_analysis(item):
                # 有独立分析/点评，保留原文，不需追溯
                item["source_type"] = "secondary_with_analysis"
            else:
                # 纯搬运，建议追溯
    ...
    print(f"  🎯 分类过滤后: {len(final_items)}", file=sys.stderr)
    
    # 按时间排序
    final_items.sort(key=lambda x: x.get("published_at") or "", reverse=True)
    ...
    output = {
        ...
        "stats": {
            ...
            "after_classify_filter": len(final_items),
            "by_category": {}
        }
    }
    ...
    save_pushed_history(history, final_items)

    print(f"\n  ✅ 完成！共 {len(final_items)} 条，写入 {output_path}", file=sys.stderr)
