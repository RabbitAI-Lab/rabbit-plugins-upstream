...
describe("auth module", () => {
  ...
  describe("checkAuth()", () => {
    ...
    it("allows localhost in none mode", () => {
      const result = checkAuth(mockReq("127.0.0.1"), { mode: "none" });
      assert.strictEqual(result.authorized, true);
      assert.strictEqual(result.user.type, "localhost");
    });

    it("requires auth for localhost in token mode", () => {
      const result = checkAuth(mockReq("127.0.0.1"), { mode: "token", token: "secret" });
      assert.strictEqual(result.authorized, false);
    });

    it("requires auth for localhost (::1) in tailscale mode", () => {
      const result = checkAuth(mockReq("::1"), { mode: "tailscale", allowedUsers: [] });
      assert.strictEqual(result.authorized, false);
    });

    it("requires auth for localhost (::ffff:127.0.0.1) in token mode", () => {
