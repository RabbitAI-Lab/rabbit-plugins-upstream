  ...
  "src/utils.js"(exports2, module2) {
    ...
    async function runCmd(cmd, options = {}) {
      ...
      const opts = {
        ...
        env: {
          ...process.env,
          PATH: envPath.includes("/usr/sbin")
            ? envPath
            : `${systemPath}:${envPath}`,
        },
        ...options,
  ...
  "src/config.js"(exports2, module2) {
    ...
    function loadConfig() {
      ...
      const config = {
        ...
        server: {
          port: parseInt(
            process.env.PORT || fileConfig.server?.port || "3333",
            10,
          ),
        ...
        paths: {
          ...
          memory:
            expandPath(
              process.env.OPENCLAW_MEMORY_DIR || fileConfig.paths?.memory,
            ) || path2.join(workspace, "memory"),
          state:
            expandPath(
              process.env.OPENCLAW_STATE_DIR || fileConfig.paths?.state,
            ) || path2.join(workspace, "state"),
          cerebro:
            expandPath(
              process.env.OPENCLAW_CEREBRO_DIR || fileConfig.paths?.cerebro,
            ) || path2.join(workspace, "cerebro"),
          skills:
            expandPath(
              process.env.OPENCLAW_SKILLS_DIR || fileConfig.paths?.skills,
            ) || path2.join(workspace, "skills"),
          jobs:
            expandPath(
              process.env.OPENCLAW_JOBS_DIR || fileConfig.paths?.jobs,
            ) || path2.join(workspace, "jobs"),
          logs:
            expandPath(
              process.env.OPENCLAW_LOGS_DIR || fileConfig.paths?.logs,
            ) || path2.join(HOME, ".openclaw-command-center", "logs"),
        },
        // Auth settings
        auth: {
          mode:
            process.env.DASHBOARD_AUTH_MODE || fileConfig.auth?.mode || "none",
        ...
        integrations: {
          linear: {
            enabled: !!(
              ...
              fileConfig.integrations?.linear?.apiKey
            ),
            apiKey:
              process.env.LINEAR_API_KEY ||
              fileConfig.integrations?.linear?.apiKey,
            teamId:
              ...
              fileConfig.integrations?.linear?.teamId,
          },
        },
        // Billing - for cost savings calculation
        billing: {
          claudePlanCost: parseFloat(
            process.env.CLAUDE_PLAN_COST ||
              fileConfig.billing?.claudePlanCost ||
              "200",
          ),
          claudePlanName:
            process.env.CLAUDE_PLAN_NAME ||
            fileConfig.billing?.claudePlanName ||
            "Claude Code Max",
        },
      };
      return config;
...
var require_jobs = __commonJS({
  "src/jobs.js"(exports2, module2) {
    ...
    async function getAPI() {
      ...
      try {
        const { createJobsAPI } = await import(
          path2.join(JOBS_DIR, "lib/api.js")
        );
    ...
    async function handleJobsRequest2(req, res, pathname, query, method) {
      ...
      try {
        ...
        if (pathname === "/api/jobs" && method === "GET") {
          ...
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({ jobs: enhanced, timestamp: Date.now() }, null, 2),
          );
          return;
        }
        ...
        if (historyMatch && method === "GET") {
          ...
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({ runs: enhanced, timestamp: Date.now() }, null, 2),
          );
          return;
        }
...
var require_vitals = __commonJS({
  "src/vitals.js"(exports2, module2) {
    ...
    async function refreshVitalsAsync() {
      ...
      try {
        ...
        if (isLinux) {
          if (mpstatOutput) {
            ...
            if (!Number.isNaN(idle)) {
              ...
              vitals.cpu.usage = Math.max(
                0,
                Math.min(100, Math.round(100 - idle)),
              );
            }
          }
          if (
            topOutput &&
            (vitals.cpu.idlePercent === null ||
              vitals.cpu.idlePercent === void 0)
          ) {
            const userMatch = topOutput.match(/([\d.]+)\s*us/);
            const sysMatch = topOutput.match(/([\d.]+)\s*sy/);
            const idleMatch = topOutput.match(/([\d.]+)\s*id/);
            vitals.cpu.userPercent = userMatch
              ? parseFloat(userMatch[1])
              : null;
            vitals.cpu.sysPercent = sysMatch ? parseFloat(sysMatch[1]) : null;
            vitals.cpu.idlePercent = idleMatch
              ? parseFloat(idleMatch[1])
              : null;
            if (
              vitals.cpu.userPercent !== null &&
              vitals.cpu.sysPercent !== null
            ) {
              vitals.cpu.usage = Math.round(
                vitals.cpu.userPercent + vitals.cpu.sysPercent,
              );
            }
          }
        } else if (topOutput) {
        ...
        if (isLinux) {
          ...
          const memAvailable =
            parseInt(memAvailableMatch?.[1] || memFreeMatch?.[1] || 0, 10) *
            1024;
          ...
          const compressedPages = parseInt(
            (memInfoRaw.match(/Pages occupied by compressor:\s+(\d+)/) ||
              [])[1] || 0,
            10,
          );
        ...
        const [perfCores, effCores, chip, iostatRaw] = await Promise.all([
          isMacOS
            ? runCmd(
                "sysctl -n hw.perflevel0.logicalcpu 2>/dev/null || echo 0",
                { fallback: "0" },
              )
            : Promise.resolve("0"),
          isMacOS
        ...
        if (isLinux) {
          ...
          const lastLine =
            iostatLines.length > 0 ? iostatLines[iostatLines.length - 1] : "";
        ...
        const isAppleSilicon =
          vitals.cpu.chip && /apple/i.test(vitals.cpu.chip);
        if (isAppleSilicon) {
          ...
          if (!vitals.temperature) {
            try {
              ...
              if (tempMatch) {
                vitals.temperature = Math.round(
                  parseInt(tempMatch[1], 10) / 100,
                );
              }
            } catch (e) {}
          }
    ...
    async function checkOptionalDeps2() {
      ...
      if (isMacOS) {
        const chip = await runCmd("sysctl -n machdep.cpu.brand_string", {
          fallback: "",
        });
      ...
      for (const dep of deps) {
        ...
        let installed = false;
        const hasBinary = await runCmd(`which ${dep.binary} 2>/dev/null`, {
          fallback: "",
        });
        if (hasBinary) {
          installed = true;
        } else if (isMacOS && dep.binary === "osx-cpu-temp") {
          const homebin = await runCmd(
            `test -x ${home}/bin/osx-cpu-temp && echo ok`,
            {
              fallback: "",
            },
          );
      ...
      if (missing.length > 0) {
        ...
        for (const dep of missing) {
          ...
          console.log(
            `   \u{1F4A1} ${dep.name} \u2014 ${dep.purpose}: ${action}`,
          );
        }
      }
      return results;
    ...
    function checkAuth2(req, authConfig) {
      ...
      if (mode === "none") {
        return isLocalhost
          ? {
              authorized: true,
              user: { type: "localhost", login: "localhost" },
            }
          : { authorized: true, user: null };
      }
      if (mode === "token") {
      ...
      if (mode === "tailscale") {
        const login = (
          req.headers[AUTH_HEADERS.tailscale.login] || ""
        ).toLowerCase();
        ...
        if (!login) {
          return {
            authorized: false,
            reason: "Not accessed via Tailscale Serve",
          };
        }
        ...
        if (isAllowed) {
          return {
            authorized: true,
            user: { type: "tailscale", login, name, pic },
          };
        }
        return {
          authorized: false,
          reason: `User ${login} not in allowlist`,
          user: { login },
        };
      }
      if (mode === "cloudflare") {
        const email = (
          req.headers[AUTH_HEADERS.cloudflare.email] || ""
        ).toLowerCase();
        if (!email) {
          return {
            authorized: false,
            reason: "Not accessed via Cloudflare Access",
          };
        }
        const isAllowed = authConfig.allowedUsers.some((allowed) => {
          if (allowed === "*") return true;
        ...
        return {
          authorized: false,
          reason: `User ${email} not in allowlist`,
          user: { email },
        };
      }
      if (mode === "allowlist") {
...
var require_operators = __commonJS({
  "src/operators.js"(exports2, module2) {
    ...
    function getOperatorBySlackId2(dataDir, slackId) {
      ...
      return data.operators.find(
        (op) => op.id === slackId || op.metadata?.slackId === slackId,
      );
    }
    ...
    async function refreshOperatorsAsync(dataDir, getOpenClawDir2) {
      ...
      try {
              ...
              try {
                ...
                if (slackMatch) {
                  ...
                  if (!operatorsMap.has(userId)) {
                    ...
                    op.lastSeen = Math.max(
                      op.lastSeen,
                      toMs(entry.timestamp, stat.mtimeMs),
                    );
                    op.sessionCount++;
                  }
                  break;
                ...
                const discordUsernameMatch = text.match(
                  /"username":\s*"([^"]+)"/,
                );
                if (discordSenderMatch) {
                  const userId = discordSenderMatch[1];
                  const label = discordLabelMatch
                    ? discordLabelMatch[1]
                    : userId;
                  const username = discordUsernameMatch
                    ? discordUsernameMatch[1]
                    : label;
        ...
        const existingMap = new Map(
          existing.operators.map((op) => [op.id, op]),
        );
        for (const [id, autoOp] of operatorsMap) {
          if (existingMap.has(id)) {
            ...
            manual.sessionCount =
              (manual.sessionCount || 0) + autoOp.sessionCount;
          } else {
        ...
        console.log(
          `[Operators] Refreshed: ${merged.operators.length} operators detected`,
        );
      } catch (e) {
    ...
    function startOperatorsRefresh2(dataDir, getOpenClawDir2) {
      ...
      setInterval(
        () => refreshOperatorsAsync(dataDir, getOpenClawDir2),
        5 * 60 * 1e3,
      );
    }
    module2.exports = {
  ...
  "src/topics.js"(exports2, module2) {
    ...
    function detectTopics(text) {
      ...
      return Object.entries(scores)
        .filter(
          ([_, score]) => score >= threshold || (score >= 1 && bestScore <= 2),
        )
        .sort((a, b) => b[1] - a[1])
        .map(([topic, _]) => topic);
    }
...
var require_sessions = __commonJS({
  "src/sessions.js"(exports2, module2) {
        ...
        try {
          ...
          for (let i = 0; i < Math.min(lines.length, 10); i++) {
            try {
              ...
              if (!text) continue;
              const slackUserMatch = text.match(
                /\]\s*([\w.-]+)\s*\(([A-Z0-9]+)\):/,
              );
              if (slackUserMatch) {
        ...
        return {
          ...
          metrics: {
            ...
            minutesActive: Math.max(
              1,
              Math.min(Math.round(minutesAgo), 24 * 60),
            ),
          },
        };
      }
      async function refreshSessionsCache() {
        ...
        try {
          ...
          if (jsonStr) {
            ...
            console.log(
              `[Sessions Cache] Refreshed: ${mapped.length} sessions`,
            );
          }
        } catch (e) {
        ...
        try {
          ...
          transcript.forEach((entry) => {
            ...
            if (msg.usage) {
              ...
              totalCacheRead +=
                msg.usage.cacheRead || msg.usage.cacheReadTokens || 0;
              totalCacheWrite +=
                msg.usage.cacheWrite || msg.usage.cacheWriteTokens || 0;
            ...
            if (msg.role === "user" && text) {
              ...
              if (
                ...
                if (match)
                  needsAttention.push(`\u{1F4CB} ${match[0].slice(0, 100)}`);
              }
            }
    ...
    module2.exports = {
      createSessionsModule: createSessionsModule2,
      CHANNEL_MAP,
    };
  },
});
  ...
  "src/cron.js"(exports2, module2) {
    ...
    function cronToHuman(expr) {
      ...
      function formatTime(h, m) {
        ...
        return mNum === 0
          ? `${h12}${ampm}`
          : `${h12}:${mNum.toString().padStart(2, "0")}${ampm}`;
      }
      if (
        minute === "*" &&
        hour === "*" &&
          ...
          day === 1 || day === 21 || day === 31
            ...
            : day === 2 || day === 22
              ...
              : day === 3 || day === 23
                ...
                : "th";
        return timeStr
          ? `${day}${suffix} of month at ${timeStr}`
    ...
    function getCerebroTopics2(cerebroDir, options = {}) {
      ...
      try {
        ...
        topicNames.forEach((name) => {
          ...
          try {
            ...
            if (frontmatterMatch) {
              ...
              if (statusMatch)
                topicStatus = statusMatch[1].trim().toLowerCase();
        ...
        if (fs2.existsSync(orphansDir)) {
          try {
            result.orphans = fs2
              .readdirSync(orphansDir)
              .filter((f) => f.endsWith(".md")).length;
          } catch (e) {}
        }
        result.lastUpdated = latestModified
  ...
  "src/tokens.js"(exports2, module2) {
    ...
    function getCostBreakdown2(config, getSessions, getOpenClawDir2) {
      ...
      for (const [key, windowConfig] of Object.entries(windowConfigs)) {
        ...
        windows[key] = {
          ...
          savingsPercent:
            monthlySavings > 0
              ? Math.round((monthlySavings / monthlyProjected) * 100)
              : 0,
    ...
    function getTokenStats2(sessions2, capacity, config = {}) {
      ...
      for (const [key, windowConfig] of Object.entries(windowConfigs)) {
        ...
        const bucket =
          usage.windows?.[bucketKey === "24h" ? "24h" : bucketKey] || usage;
        const bucketCosts = calculateCostForBucket(bucket);
        const dailyAvg = bucketCosts.totalCost / windowConfig.days;
        const monthlyProjected = dailyAvg * 30;
        ...
        savingsWindows[key] = {
          ...
          estSavings: windowSavingsPositive
            ? `$${formatNumber(windowSavings)}/mo`
            : null,
      ...
      return {
        ...
        estSavings: savingsPositive
          ? `$${formatNumber(monthlySavings)}/mo`
          : null,
        savingsPercent: savingsPositive
...
var require_llm_usage = __commonJS({
  "src/llm-usage.js"(exports2, module2) {
    ...
    function transformLiveUsageData(usage) {
      const anthropic = usage.providers?.find(
    ...
    function getLlmUsage2(statePath) {
      ...
      try {
        if (fs2.existsSync(stateFile)) {
          ...
          const sessionValid =
            data.claude?.session?.resets_in &&
            data.claude.session.resets_in !== "unknown";
          const weeklyValid =
          ...
          if (sessionValid || weeklyValid) {
            return {
              ...
              claude: {
                session: {
                  usedPct: Math.round(
                    (data.claude?.session?.used_pct || 0) * 100,
                  ),
                  remainingPct: Math.round(
                    (data.claude?.session?.remaining_pct || 1) * 100,
                  ),
                  resetsIn: data.claude?.session?.resets_in || "?",
                },
                weekly: {
                  usedPct: Math.round(
                    (data.claude?.weekly_all_models?.used_pct || 0) * 100,
                  ),
                  remainingPct: Math.round(
                    (data.claude?.weekly_all_models?.remaining_pct || 1) * 100,
                  ),
                  resets: data.claude?.weekly_all_models?.resets || "?",
                },
                sonnet: {
                  usedPct: Math.round(
                    (data.claude?.weekly_sonnet?.used_pct || 0) * 100,
                  ),
                  remainingPct: Math.round(
                    (data.claude?.weekly_sonnet?.remaining_pct || 1) * 100,
                  ),
              ...
              routing: {
                ...
                claudePct:
                  data.routing?.total_tasks > 0
                    ? Math.round(
                        (data.routing.claude_tasks / data.routing.total_tasks) *
                          100,
                      )
                    : 0,
                codexPct:
                  data.routing?.total_tasks > 0
                    ? Math.round(
                        (data.routing.codex_tasks / data.routing.total_tasks) *
                          100,
                      )
                    : 0,
                codexFloor: Math.round(
                  (data.routing?.codex_floor_pct || 0.2) * 100,
                ),
              },
            };
          }
        ...
        try {
          ...
          if (stats.total_requests > 0) {
            stats.success_rate = Math.round(
              (successes / stats.total_requests) * 100,
            );
          }
          return stats;
        } catch (e2) {
      ...
      try {
        switch (action) {
          ...
          case "gateway-restart":
            results.output =
              "To restart gateway, run: openclaw gateway restart";
            results.success = true;
          ...
          case "clear-stale-sessions": {
            ...
            try {
              ...
              if (staleJson) {
                ...
                staleCount = (data.sessions || []).filter(
                  (s) => s.ageMs > 24 * 60 * 60 * 1e3,
                ).length;
              }
            } catch (e) {}
    ...
    function createStateModule2(deps) {
      ...
      function getCapacity() {
        ...
        try {
          ...
          console.error(
            "Failed to get capacity from sessions, falling back to filesystem:",
            e.message,
          );
        }
        try {
            ...
            for (const file of files) {
              try {
                ...
                try {
                  ...
                  isSubagent =
                    key.includes(":subagent:") || key.includes(":cron:");
                } catch (parseErr) {
          ...
          console.error(
            "Failed to count active sessions from filesystem:",
            e.message,
          );
        }
        return result;
      ...
      function getData() {
        ...
        const statusCounts = {
          ...
          recent: allSessions.filter((s) => !s.active && s.recentlyActive)
            .length,
          idle: allSessions.filter((s) => !s.active && !s.recentlyActive)
            .length,
        };
      ...
      function getFullState() {
        ...
        try {
          ...
          const operatorsWithStats = operatorData.operators.map((op) => {
            const userSessions = allSessions.filter(
              (s) =>
                s.originator?.userId === op.id ||
                s.originator?.userId === op.metadata?.slackId,
            );
            return {
              ...op,
              stats: {
                ...
                lastSeen:
                  userSessions.length > 0
                    ? new Date(
                        Date.now() -
                          Math.min(...userSessions.map((s) => s.minutesAgo)) *
                            6e4,
                      ).toISOString()
                    : op.lastSeen,
              },
        ...
        try {
          const retentionHours = parseInt(
            process.env.SUBAGENT_RETENTION_HOURS || "12",
            10,
          );
      ...
      function startStateRefresh(broadcastSSE2, intervalMs = 3e4) {
        ...
        console.log(
          `[State] Background refresh started (${intervalMs}ms interval)`,
        );
      }
            ...
            for (const s of subagentSessions) {
              ...
              for (const entry of transcript.slice(0, 15)) {
                if (
                  ...
                  let taskMatch = text.match(
                    /You were created to handle:\s*\*\*([^*]+)\*\*/i,
                  );
                  if (taskMatch) {
...
var { getLlmUsage, getRoutingStats, startLlmUsageRefresh } =
  require_llm_usage();
...
var server = http.createServer((req, res) => {
  ...
  if (!isPublicPath && AUTH_CONFIG.mode !== "none") {
    ...
    if (!authResult.authorized) {
      ...
      res.end(
        getUnauthorizedPage(authResult.reason, authResult.user, AUTH_CONFIG),
      );
      return;
    }
    ...
    if (authResult.user?.login || authResult.user?.email) {
      ...
      console.log(
        `[AUTH] Allowed: ${req.socket?.remoteAddress} (path: ${pathname})`,
      );
    }
  }
  if (pathname === "/api/status") {
    ...
    const data = getCerebroTopics(PATHS.cerebro, {
      offset,
      limit,
      status: statusFilter,
    });
    ...
    req.on("end", () => {
      try {
        ...
        if (
          !newStatus ||
          !["active", "resolved", "parked"].includes(newStatus)
        ) {
          res.writeHead(400, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({
              error: "Invalid status. Must be: active, resolved, or parked",
            }),
          );
          return;
        }
    ...
    const data = getCostBreakdown(
      CONFIG,
      (opts) => sessions.getSessions(opts),
      getOpenClawDir,
    );
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(data, null, 2));
    ...
    if (method === "GET") {
          ...
          stats: {
            ...
            lastSeen:
              userSessions.length > 0
                ? new Date(
                    Date.now() -
                      Math.min(...userSessions.map((s) => s.minutesAgo)) * 6e4,
                  ).toISOString()
                : op.lastSeen,
          },
        };
      ...
      req.on("end", () => {
        try {
          ...
          const existingIdx = data.operators.findIndex(
            (op) => op.id === newOp.id,
          );
          if (existingIdx >= 0) {
      ...
      req.on("end", () => {
        try {
          ...
          const merged = {
            ...
            hiddenSessions:
              updates.hiddenSessions ?? current.hiddenSessions ?? [],
          ...
          if (savePrivacySettings(DATA_DIR, merged)) {
            ...
            res.writeHead(500, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({ error: "Failed to save privacy settings" }),
            );
          }
        } catch (e) {
          res.writeHead(400, { "Content-Type": "application/json" });
...
server.listen(PORT, () => {
  const profile = process.env.OPENCLAW_PROFILE;
  console.log(
    `\u{1F99E} OpenClaw Command Center running at http://localhost:${PORT}`,
  );
  if (profile) {
