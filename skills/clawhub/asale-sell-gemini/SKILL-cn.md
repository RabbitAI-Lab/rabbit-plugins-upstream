...
把 Gemini CLI 订阅挂到 asale 市场上：导入账号、设定价格底价与并发上限，并查看它的哪些模型真的在卖。
...
凭证来自 macOS 钥匙串条目 `gemini-cli-oauth`（账号 `main-account`）或
`~/.gemini/oauth_creds.json`。扁平文件格式和 keytar 的嵌套格式都能读，所以 CLI 写的是
哪一种都无所谓。
## 使用方法
...
Gemini 订阅在 `gemini-cli-oauth` 钥匙串条目或 `~/.gemini/oauth_creds.json` 里。
文件里带 `id_token` 时，账号邮箱从它的 claim 里解出来。

挑 `provider` 是 `gemini` 的那一行，它的 `account_id` 就是后面每个调用用来指代它的名字。
