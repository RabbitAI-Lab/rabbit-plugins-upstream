...
Put a Gemini CLI subscription on the asale market: import it, set a price floor and a concurrency cap, and see which of its models are actually selling.

## Trigger Keywords

- sell my Gemini subscription
- put Gemini CLI on the market
- rent out Gemini quota
- asale sell gemini
...
A Gemini subscription is found in the `gemini-cli-oauth` keychain entry or in
`~/.gemini/oauth_creds.json`. The account email comes out of the `id_token`
claims when the file carries one.

Pick the row whose `provider` is `gemini`. Its `account_id` is what every later call
names it by.

### 2. Put it on the market

```bash
call set_account_sell '{"provider":"gemini","accountId":"<id>","enabled":true,"minRatio":10,"concurrency":5,"dailyLimit":0}'
```
...
```bash
call list_lanes '{"provider":"gemini","accountId":"<id>"}'
```
...
```bash
call resume_lane '{"provider":"gemini","accountId":"<id>","model":"<model>"}'
```

### 4. Take it off the market

```bash
call set_account_sell '{"provider":"gemini","accountId":"<id>","enabled":false}'
```
