...
1. **链接原样完整输出**：凡展示平台地址（充值、报告、公司页、公告详情页），一律输出完整 URL（含 `sk` 参数），不要用「官网」「点此」等文字代替。
...
5. **凭证不进对话**：不索要、不在回复中输出 API Key；充值/账户操作只给平台链接。
6. **风险舆情只陈述不定性**：WebSearch 到的涉诉、行政处罚、失信等信息，固定用「公开信息显示…（来源：{链接}）」句式逐条陈述，**必须附来源链接**；不下「该公司有问题/不靠谱/有法律风险」类定性结论，是否影响合作由用户自行判断。搜不到风险信息时写「未检索到相关公开风险信息（不代表不存在）」。
...
8. 调用预算默认单查 ≤18 次、对比 ≤25 次；将超出时暂停并告知用户，征得同意后继续。
9. **隐私最小化注册 + 显式征询 gate**：自动注册前必须先征得用户同意（见上方「API Key 获取」），仅采集 3 项非身份设备特征，同意前不得发起任何注册请求。
...
11. **报告全文只输出一遍**，输出完成后立即停止，不得重复任何章节。
12. **绝不编造数据**。查不到的信息在报告中明确标注「数据缺口」；**查不到 ≠ 没有**，报告必须写明数据边界（本库覆盖范围、时间范围）。
13. **结论必须可追溯**：每个判断（如"中标实力强"）后面给出支撑数据（如"近 2 年中标 86 个、总额 3.2 亿"）。
14. 报告末尾必须附带免责声明（见 report-template.md）。
