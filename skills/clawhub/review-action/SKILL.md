---
name: amazon-review-to-action
description: >
  Amazon 评论转行动 Skill：结合商品详情，把评论问题整理成产品、包装、说明和供应商任务。
  仅用于证据驱动的产品行动，不用于自动联系买家、直接改页面或采购下单。Use when
  the user asks to turn reviews into actions, 评论转行动、产品改进任务。
  Requires an ARI API key (ari_live_*).
slug: review-action
displayName: Amazon 评论转行动
version: 1.4.7
summary: 把评论问题转成产品、包装与供应商任务
license: MIT
---
...
- 本入口只执行 `product/action`，输出模板固定为 `ops_product`。
