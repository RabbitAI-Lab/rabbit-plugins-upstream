...
description: "Collect comments from a known Instagram post URL. Do not use for profile data or Reel details."
---

# Dataify Instagram Comment By Post URL

Submit Instagram post comment collection jobs through Dataify Builder by post URL. After submission, continue monitoring the returned `task_id` and return the final result by default.
...
```powershell
python3 ".\scripts\submit_dataify_instagram_comment_by_posturl.py" --posturl "https://www.instagram.com/cats_of_instagram/reel/C4GLo_eLO2e/" --file-name "{{TasksID}}"
```
...
```bash
python3 scripts/submit_dataify_instagram_comment_by_posturl.py --help
```
