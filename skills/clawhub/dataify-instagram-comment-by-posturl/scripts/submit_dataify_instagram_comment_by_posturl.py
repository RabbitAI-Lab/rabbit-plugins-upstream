...
DEFAULT_POSTURL = "https://www.instagram.com/cats_of_instagram/reel/C4GLo_eLO2e/"
...
def ensure_python_version():
    if sys.version_info < MIN_PYTHON:
        print(
            "Python {}.{} or newer is required. Run this script with a Python 3 interpreter, for example: python3 scripts/submit_dataify_instagram_comment_by_posturl.py --posturl \"{}\"".format(
                MIN_PYTHON[0],
                MIN_PYTHON[1],
                DEFAULT_POSTURL,
            ),
...
def normalize_posturl(value):
    clean = str(value).strip()
    if not clean:
        raise ValueError("posturl cannot be empty")
    if not clean.startswith("https://www.instagram.com/"):
        raise ValueError("posturl must start with https://www.instagram.com/")
    return clean
...
def normalize_group(group):
    return {
        "posturl": normalize_posturl(group.get("posturl", DEFAULT_POSTURL)),
    }
...
def build_groups(args):
    ...
    posturls = args.posturl or [DEFAULT_POSTURL]
    return [normalize_group({"posturl": posturl}) for posturl in posturls]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Submit a Dataify Instagram post comment by post URL Builder task.")
    parser.add_argument("--posturl", action="append", help="Instagram post URL. Repeat for multiple post URLs.")
    parser.add_argument("--file-name", default=DEFAULT_FILE_NAME, help="Builder file_name field. Default: {{TasksID}}.")
    parser.add_argument("--params-json", help="JSON array of posturl parameter objects.")
