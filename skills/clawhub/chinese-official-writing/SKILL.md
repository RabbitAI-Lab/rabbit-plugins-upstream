...
仅在用户任务属于中文公文、中文正式工作材料，或明确要求新闻稿、新闻消息、快讯、活动报道、活动新闻稿、新闻通稿、新闻评论、时评、评论员文章时启用本技能，包括起草、改写、压缩、润色修改、复核、去口语化、降 AI 味、文种校验、办理要素核对和 Word 文稿正文处理。
...
用户明确要求简短正文、正文不超过300字，或给出200字左右且没有硬性下限时，在读取 `references/information-selection.md` 后直接读取 `references/short-draft-naturalness.md`。用户给出明确字数下限或区间时仍先按原任务成稿，不用短稿规则代替篇幅不足处理。
...
- 审稿或复核：用户只要求检查、审查、格式核验或语气检查且未要求代改时，输出问题位置、风险层级和修改建议，不重写全文；用户要求代改时才输出改后正文。只对既有可研摘要做用户点名的完整性核对，且未要求语言、格式或 AI 味综合审稿时，直达 `references/genre-checklist-feasibility-review.md`，由该页完成定位、分级和建议，不自动叠加通用审稿页或去 AI 味页；其他审稿的具体定位、分级和格式按 `references/review-checklist.md` 执行，去 AI 味检查按 `references/anti-ai-patterns.md` 执行。
...
- 正式正文的标题、小标题、段落标签直接用普通文本承接。纯文本主标题行末不加句号，标题后空一行；`一、申请事项`等层级标题不加句号，编号内容本身是完整正文句时仍用正常句末标点；Word 或正式排版稿用段落样式形成标题间距，用户固定模板优先。正文不用 Markdown `**` 加粗、`###`、代码块或 `---` 横线包装；只有用户明确要求 Markdown 文档格式时，才按其格式要求处理。
