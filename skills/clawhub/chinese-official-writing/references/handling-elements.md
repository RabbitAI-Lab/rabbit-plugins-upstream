...
本文件只识别候选办理要素；信息进入正文、保持状态、省略或作为实质缺口短列，统一按 `information-selection.md` 处理。要素缺失不停止写作，交付正文不得残留 `〔签发日期〕`、`〔会议时间〕`、`〔待补充〕`、`[具体项目名称]`、`XXXX万元`、`YYYY年MM月DD日`、`（签发日期）` 等未完成占位，除非用户明确要求输出占位模板。
...
- 默认不外搜补缺项。用户明确要求搜索或核验公开来源，或任务含“最新”“当前”“今日”“现行政策”“近期数据”等时效事实时，才可联网核验；搜索结果只作事实核查和来源参考，来源冲突、无法核验或工具不可用时保持其当前状态，不写成已确认事实。
- 不因出现单位名称就搜索单位公开样文、固定格式或写作风格；用户未给单位模板时，使用中性称谓和通用文种格式。
- 不把 `〔签发日期〕`、`〔会议时间〕`、`〔维护开始时间〕`、`[具体项目名称]`、`XXXX万元`、`YYYY年MM月DD日`、`（签发日期）`、`根据用户提供材料填写` 等未完成字段留在交付正文中。
- 用户要求完整落款但未说明成文日期缺失或待确认时，可使用当前日期作为草稿日期；用户明示成文日期缺失、待确认或需另行确认时，不使用当前日期补落款。正式签发日期、文号、印章和签发人仍不得编造。
- 当前日期只可用于草稿落款，不可替代会议时间、维护时间、实施期限、政策依据或业务数据。
- 缺项影响文种成立时，例如请示没有请批事项、批复没有来文依据，将其识别为实质缺口；请示、报告、上报申请缺主送机关、发文或申请单位、成文日期时，不编造“上级单位”“申请单位”等泛称。
