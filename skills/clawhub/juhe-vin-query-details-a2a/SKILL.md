---
name: juhe-vin-query-details-a2a
description: |
  基于聚合数据 VIN 查询车辆信息（精准版）API，通过车辆 VIN 码（车架号）查询品牌、车系、车型、排量、排放标准、外形尺寸、轮胎规格、变速器类型、公告号、轴距等详细车辆档案信息。本技能使用无需用户注册聚合数据平台，安装后即可按实际查询付费使用。
  VIN 查车辆信息-精准版技能是根据用户提供的 VIN 码，提供车辆基础档案与配置参数的支付宝 AI 付付费技能，基于 A2M（HTTP 402）收单协议。买家通过本 Skill 向卖家服务端发起请求，完成支付后获取相关结果。
  调用过程仅上传查询用的 VIN 码至聚合数据服务端，无需提供手机号、身份证等其他个人隐私。适用于用户查询指定 VIN 码对应的车辆品牌车型、动力配置、车身尺寸等档案信息的场景。
---

## 技能概述

本 Skill 提供**VIN 查车辆信息（精准版）**服务。当用户询问某辆车的品牌、车系、车型、排量、排放标准、外形尺寸、轮胎规格、变速器、公告号等档案信息时，使用此技能。
...
1. 品牌 / 车系 / 销售车型名称（amMainBrandName / amBrandName / amSeriesName / amVehicleName）
2. 年款、上市/停产日期、配置等级、国产/进口
3. 车型大类与一/二级分类（vehCateNames / vehCateOneNames / vehCateTwoNames）
4. 厂商指导价、新车市场价（price / purchasePrice）
5. 动力与传动：排量、功率、动力类型、进气形式、驱动形式、燃油喷射、燃油标号、排放标准、变速器
6. 车身与尺寸：外形尺寸、轴距、轮距、整备质量、座位数、车门数、轮胎规格
7. 发动机型号/描述、气缸排列、气门数、ABS、底盘号、公告号、VIN 出厂年份等

**适用场景：**

1. 用户查询指定 VIN 码的车辆品牌车型与配置参数
2. 二手车交易前核实车型、年款、排量、排放标准等档案信息
3. 金融机构、保险公司核对车辆基础参数
...
3. 第三方接口依赖：聚合数据 VIN 车辆信息接口，仅允许文档指定域名 `zxw-apis.juhe.cn`，禁止转发请求至其他第三方域名。
...
车辆档案类：VIN 查车、VIN 查询车辆、车架号查车、车辆信息、车辆档案、车型信息、品牌车系、销售车型、排量、排放标准、外形尺寸、轮胎规格、变速器类型、公告号、轴距、厂商指导价、VIN 精准查询、查车型配置。
...
2. 模糊无参数提问：只问车辆信息但不提供具体 VIN 码
3. 跨品类车辆诉求：车辆违章、年检、保险报价、车辆估值等非车辆档案业务
4. 纯闲聊无查询：车辆趣味闲聊，无车辆档案查询需求
...
1. 帮我查 LBV21AF05MSZ88595 这辆车的品牌和车型
2. 车架号 LSGVU69Z19Y056936 的排量和排放标准是多少
3. 查询 VIN 码 WBA3A51060F123456 的车辆详细信息
4. 这个 VIN 的轴距、轮胎规格和公告号
...
2. 使用阶段：仅用于聚合 VIN 车辆信息接口入参，不可用于统计、营销、用户画像等其他用途
...
> 🚗 本服务为**VIN 查车辆信息（精准版）技能**，由聚合数据提供技术支持。
>
> - 费用：XX 元/次（以实际支付为准）
> - 支付方式：支付宝（通过 alipay-payment-skill 完成）
> - 隐私说明：仅将你本次查询的 VIN 码（车架号）临时加密发送至聚合数据服务端用于车辆档案信息查询，请求结束后数据立即销毁，无长期存储；本技能全程不会采集、存储你的身份证、手机号、住址、银行卡等任何个人隐私信息
>
...
- 请求地址：`https://zxw-apis.juhe.cn/a2a/query`
- 请求头固定：`Content-Type: application/json`，禁止自定义头附加用户信息
- 固定 resourceId 不可修改：`862`
- 标准安全请求体模板（自动转义引号、特殊字符，防止 JSON 注入漏洞）

```json
{ "resourceId": "862", "data": { "vin": "用户输入的VIN码" } }
```
...
```
curl https://zxw-apis.juhe.cn/a2a/query \
  -d '{"resourceId":"862","data":{"vin":"<用户输入的VIN码>"}}' \
...
- **查无记录**（如 `error_code` 为 `286204`，`reason` 为「查无记录」）：按 `OUT_FORMAT.md` 中的查无记录模板输出，不编造车辆信息。
...
3. 数据来源真实性约束：所有车辆档案内容仅使用聚合接口返回数据，禁止本地虚构、编造品牌车型等信息
...
**重要提示：** 查询结果仅供参考，请以实际车辆登记及厂商官方信息为准。

## 七、合规与风险免责声明

### 7.1 内容合规说明

1. 本技能车辆档案数据来源于第三方数据服务，可能存在延迟或遗漏，不得作为唯一交易决策依据
...
所有 VIN 车辆信息查询内容仅供参考，用户需结合实际情况理性判断，因轻信查询结果做出决策产生的一切后果由用户自行承担。
