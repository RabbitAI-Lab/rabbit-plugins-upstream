...
Pandoc转换工具是针对文档转换领域的专业化AI辅助工具。免费版面向个人用户，提供核心功能与基础用法，适合快速上手和日常使用.
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：通用文档格式转换、Markdown、HTML、Word、PDF、等多种格式互转、转换工具、免费版等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | Pandoc转换工具（免费版）处理的输入数据或指令 |
...
def convert(input_file, output_format, output_file=None):
    ...
    return output_file
# ...
convert("document.md", "docx")
convert("document.md", "html")
```
...
```json
{
  "success": true,
  "data": {
    "result": "Pandoc转换工具（免费版）处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "pandoc convert"
    }
  },
