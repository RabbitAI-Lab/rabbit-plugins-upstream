...
def add_paragraph(doc, text, font_size=12, font_name='宋体', bold=False, alignment=None):
    """添加段落并设置格式（无额外空行）"""
...
def add_heading_text(doc, text, level=1, font_size=18):
    """添加标题（无额外空行）"""
    # 不使用 doc.add_heading，直接添加段落作为标题
    para = doc.add_paragraph(text)
    for run in para.runs:
        set_heading_font(run, '黑体', font_size)
        run.font.bold = True
    set_para_spacing(para, space_before=Pt(12), space_after=Pt(6))
...
def create_cover_document(output_path, info):
    ...
    # 添加标题
    title = doc.add_paragraph(info.get("项目名称", "项目名称"))
    for run in title.runs:
        set_heading_font(run, '黑体', 22)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(title, space_before=Pt(20), space_after=Pt(12))
    ...
    for i, (label, value) in enumerate(rows_data):
        ...
        for para in info_table.rows[i].cells[1].paragraphs:
            for run in para.runs:
                set_font(run, '宋体', 12)
        set_para_spacing(info_table.rows[i].cells[0].paragraphs[0])
        set_para_spacing(info_table.rows[i].cells[1].paragraphs[0])
    
    # 添加底部说明
    bottom_para = doc.add_paragraph("（此页 intentionally 空白）")
    bottom_para.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    for run in bottom_para.runs:
        set_font(run, '宋体', 10)
    set_para_spacing(bottom_para, space_before=Pt(20))
...
def create_business_license_doc(output_path, info):
    ...
    # 标题
    title = doc.add_paragraph("营业执照信息")
    for run in title.runs:
        set_heading_font(run, '黑体', 20)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(title, space_before=Pt(20), space_after=Pt(12))
    ...
    for run in note_para.runs:
        set_font(run, '宋体', 10)
    set_para_spacing(note_para, space_before=Pt(12))
...
def create_quote_document(output_path, info):
    ...
    # 项目信息
    info_text = f"""项目名称：{info.get("项目名称", "")}
项目编号：{info.get("项目编号", "")}
投标人名称：{info.get("公司全称", "")}
报价日期：2026 年    月    日"""
    ...
    for run in info_para.runs:
        set_font(run, '宋体', 12)
    set_para_spacing(info_para, space_before=Pt(12), space_after=Pt(12))
...
def create_deviation_document(output_path, info):
    ...
    # 标题
    title = doc.add_paragraph("技术/商务偏离表")
    for run in title.runs:
        set_heading_font(run, '黑体', 20)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(title, space_before=Pt(20), space_after=Pt(12))
...
投标人名称：{info.get("公司全称", "")}"""
    ...
    # 偏离表内容
    content = f"""我方对招标文件（项目编号：{info.get("项目编号", "")}）的响应情况如下：
...
def create_qualification_document(output_path, info):
    ...
    # 标题
    title = doc.add_paragraph("资格能力证明文件")
    for run in title.runs:
        set_heading_font(run, '黑体', 20)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(title, space_before=Pt(20), space_after=Pt(12))
    ...
    # 一、基本情况
    section1 = doc.add_paragraph("一、基本情况")
    for run in section1.runs:
        set_heading_font(run, '黑体', 16)
    set_para_spacing(section1, space_before=Pt(12), space_after=Pt(6))
    
    content1 = f"""1.1 公司名称：{info.get("公司全称", "")}
...
我方是一家依法注册成立的合法企业，具备独立承担民事责任的能力。""".replace('\n\n', '\n')
    
    para1 = doc.add_paragraph(content1)
    for run in para1.runs:
        set_font(run, '宋体', 12)
    set_para_spacing(para1)
    
    # 二、银行信息
    section2 = doc.add_paragraph("二、银行信息")
    for run in section2.runs:
        set_heading_font(run, '黑体', 16)
    set_para_spacing(section2, space_before=Pt(12), space_after=Pt(6))
    
    content2 = f"""2.1 开户银行：{info.get("开户银行", "")}

2.2 银行账号：{info.get("银行账号", "")}

2.3 联行号：{info.get("开户银行联行号", "")}

我方账户信息真实有效，具备履行合同所需的资金结算能力。""".replace('\n\n', '\n')
    
    para2 = doc.add_paragraph(content2)
    for run in para2.runs:
        set_font(run, '宋体', 12)
    set_para_spacing(para2)
    
    # 三、授权代表信息
    section3 = doc.add_paragraph("三、授权代表信息")
    for run in section3.runs:
        set_heading_font(run, '黑体', 16)
    set_para_spacing(section3, space_before=Pt(12), space_after=Pt(6))
    
    content3 = f"""3.1 授权代表姓名：{info.get("授权代表", "")}

3.2 联系电话：{info.get("授权代表电话", "")}

我方授权代表有权代表我方处理本次投标及后续合同履行的相关事宜。""".replace('\n\n', '\n')
    
    para3 = doc.add_paragraph(content3)
    for run in para3.runs:
        set_font(run, '宋体', 12)
    set_para_spacing(para3)
    
    # 四、资质证明
    section4 = doc.add_paragraph("四、资质证明")
    for run in section4.runs:
        set_heading_font(run, '黑体', 16)
    set_para_spacing(section4, space_before=Pt(12), space_after=Pt(6))
    
    content4 = """我方具备履行本合同所需的全部资质，包括但不限于：
...
以上资质证书均在有效期内，真实有效。""".replace('\n\n', '\n')
    
    para4 = doc.add_paragraph(content4)
    for run in para4.runs:
        set_font(run, '宋体', 12)
    set_para_spacing(para4)
    
    # 五、业绩材料
    section5 = doc.add_paragraph("五、业绩材料")
    for run in section5.runs:
        set_heading_font(run, '黑体', 16)
    set_para_spacing(section5, space_before=Pt(12), space_after=Pt(6))
    
    content5 = """我方具有类似项目的实施经验，主要业绩包括：
...
def create_technical_document(output_path, info, tma_content):
    ...
    # 封面
    title = doc.add_paragraph("技术实施方案")
    for run in title.runs:
        set_heading_font(run, '黑体', 22)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(title, space_before=Pt(20), space_after=Pt(12))
    ...
    # 第一章 项目概述
    chapter1 = doc.add_paragraph("第一章 项目概述")
    for run in chapter1.runs:
        set_heading_font(run, '黑体', 18)
    set_para_spacing(chapter1, space_before=Pt(12), space_after=Pt(6))
...
（4）我方技术积累和经验"""
    ...
    # 第二章 技术方案
    chapter2 = doc.add_paragraph("第二章 技术方案")
    for run in chapter2.runs:
        set_heading_font(run, '黑体', 18)
    set_para_spacing(chapter2, space_before=Pt(12), space_after=Pt(6))
...
（4）经济性""".replace('\n\n', '\n')
    ...
    # 第三章 实施计划
    chapter3 = doc.add_paragraph("第三章 实施计划")
    for run in chapter3.runs:
        set_heading_font(run, '黑体', 18)
    set_para_spacing(chapter3, space_before=Pt(12), space_after=Pt(6))
...
（5）安全员：负责安全管理""".replace('\n\n', '\n')
    ...
    # 第四章 质量保证
    chapter4 = doc.add_paragraph("第四章 质量保证")
    for run in chapter4.runs:
        set_heading_font(run, '黑体', 18)
    set_para_spacing(chapter4, space_before=Pt(12), space_after=Pt(6))
...
（3）及时处理质量问题""".replace('\n\n', '\n')
    ...
    # 第五章 售后服务
    chapter5 = doc.add_paragraph("第五章 售后服务")
    for run in chapter5.runs:
        set_heading_font(run, '黑体', 18)
    set_para_spacing(chapter5, space_before=Pt(12), space_after=Pt(6))
...
我方将组建专门的售后服务团队，提供全天候服务。""".replace('\n\n', '\n')
...
def create_power_of_attorney(output_path, info):
    ...
    # 标题
    title = doc.add_paragraph("授  权  委  托  书")
    for run in title.runs:
        set_heading_font(run, '黑体', 20)
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_para_spacing(title, space_before=Pt(20), space_after=Pt(12))
    
    content = f"""{info.get("招标人", "")}：
...
def main():
    parser = argparse.ArgumentParser(description="标书自动生成脚本 - V2 版（修复空格问题）")
