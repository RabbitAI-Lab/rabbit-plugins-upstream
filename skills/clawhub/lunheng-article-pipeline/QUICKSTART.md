...
- **⚠️ 文件写入**（**运行即会写盘**）：主控与各子代理会在 workspace 创建/修改约 15-25 个文件——`status.md` 状态机、（检查）（任务简报/文献卡/数据卡/案例卡/草稿/审计报告/定稿）、各子代理心跳文件；**仅写 workspace 内**，不写 workspace 外
...
1. **Phase 0 两个「4 选 1」同意关卡**（v2.5.6 P1-4 修订，两个别混淆）：
...
**答：不能。** 论衡是 LLM 推理 + 文件读写 + Web 检索的纯推理流水线，没有任何 shell 执行能力。算法文档中的 （检查）/（检查）/（检查）/（检查） 等命令**仅供人类主人手动验证**。

---

### Q2：数据来源可以是二手转引吗？

**答：可以但严格限制。** 详见 [`glossary.md § 三 数据信任级别`](references/glossary.md#三数据信任级别v221-核心机制)。**🟢🟡🔴 是「信任级别」标识**（v2.2.1 起），**与「时效评级」不共用**——时效评级用文字描述（≤2 年 / 2-5 年 / >5 年，详见 [`audit-checklist-quickref.md § G11`](references/_shared/audit-checklist-quickref.md)）：
- 🟢 已发布公开数据（最高信任）
- 🟡 主人投喂数据（中信任）
