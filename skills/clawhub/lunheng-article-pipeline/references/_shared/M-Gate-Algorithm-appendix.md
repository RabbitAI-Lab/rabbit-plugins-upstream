...
- **「M 门是论衡的「返璞归真」** —— 不引入 Python 脚本，不扩大 exec 白名单，由 LLM 推理执行规则化校验」（v2.2.0）
...
- **「**阶段闸门是论衡主控纪律**」** —— 不引入新 agent，主控用 progress_card + read 实现；ARS Stage 2.5/4.5 论衡化（v2.2.1）
...
- **「修订也需独立角色**」** —— 论衡角色独立性（现 10 张角色卡）的核心理念应贯穿到修订轮，主控只协调不代笔（v2.2.4；v2.7.1 起 10 张）
