...
> **v2.2.15 新增**：[渐进式执行模式](M-Gate-Algorithm.md)—— 把 13 项 M 门从「T8 一次性全跑」改为「5 阶段分批执行 + T8 兜底」，P0 错误提前暴露（Phase 1.5 而非 T8），节省 30-50% 工作量。
>
...
- **「**阶段闸门是论衡主控纪律**」** —— 不引入新 agent，主控用 update_plan + read 实现；ARS Stage 2.5/4.5 论衡化（v2.2.1）
