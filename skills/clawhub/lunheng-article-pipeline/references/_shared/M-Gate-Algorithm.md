...
```
❌ 不是：主控跑 `检查 -E "..." file.md | 求差集/-13` 等真 shell 命令
...
| **sha256 完整性（M-Exist-2 修订）** | **v2.5.5 默认不验** | 主人可选手工 `校验哈希` 后填回，论衡 LLM 不实际计算（zero exec 哲学） |

### 3. 已知漏洞（实战暴露）

- **LLM 自评自过** = 同一模型既写又审，**有偏的 LLM 可能给自己放行**。缓解：v2.3.11 引入「主控强制 wait 30s + 双重 列出」验证产物（v2.5.5 P0 硬步骤 #2 写盘后自检）。
...
**Phase 0 同意关卡（v2.7.9 澄清）**：本算法不调用任何外部服务（纯 LLM 推理 + 项目内文件 I/O），**不新增网络级外发**，因此不产生「额外」的网络级同意需求；但流水线整体仍受 Phase 0 同意关卡约束（文件清单 + 外部服务 4 选 1 已由主人在 Phase 0 确认），本算法在其授权范围内运行，不构成免同意通道。实战验证如需 shell 脚本（脚本版）跑 dry-run，需主人明示同意（教训 #51 金标准）。
...
whitelist = ["参考文献", "数据来源", "案例来源", "先行者文献", "AI 使用声明", "方法论附录"]  # v2.7.3 +方法论附录（可选，主人 Phase 5 勾选）
...
**v2.5.5 重写原因**：
- 原「M-Exist-2 证据包文件完整性 sha256」设计是「理想」但**实战中 sha256 字段永远占位**（主人不手动跑 `校验哈希`）
- 论衡「零 exec 哲学」= LLM 不能跑 sha256 实际计算（仅可读文件验证非空）
- 实战中占位符 `[哈希校验待主人回填]` 永远不会被回填 = 字段形同虚设
...
4. **人类主人补填 sha256 可选**（v2.5.5 保留为可选）：
   - 主人如需更严验证，在 host shell 跑 （检查 final/证据包/*.md） 后追加到 final/交付说明.md
...
### M-Exist-3: 数据信任级别一致性 对比（v2.2.1.2 双格式升级版 + v2.5.17 SVG 扩展）

**v2.5.17 关键扩展（教训 #184 实战（v2.5.18 补录））**：SVG 文本节点（`text`/`desc`/`title`/`tspan`）现视为事实层，内嵌的 `[Dxx]` / `[Cxx]` 信任级别与 md 正文同等 对比。防“正文修了 SVG 没修”双线不一致。
...
3. 信任级别一致性 对比（v2.5.17 三轮 对比：md 表格 + SVG）：
   - 标准格式 对比：求差集/-13 set_intext_d set_card_d
   - 表格格式 对比：求差集/-13 set_intext_table set_card_table
   - SVG 格式 对比（v2.5.17 新增）：求差集/-13 set_svg_d set_card_d
...
伪代码：
# v2.5.17 扩展：SVG 内嵌文本节点视为事实层，纳入 [Dxx] / [Cxx] 提取与 对比
draft_text = read("final/定稿.md")
