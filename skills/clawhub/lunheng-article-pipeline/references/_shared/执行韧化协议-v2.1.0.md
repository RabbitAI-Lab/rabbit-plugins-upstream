...
> **v2.2.11 范围调整**：原 § 4.5 中的 spawn 日志诊断方法（读取 （检查））原本在主流程中提及，但**该路径属于 OpenClaw runtime 内部诊断**（不在论衡主流程职责范围内）。
> **论衡主流程只负责**：「spawn 后立即验证落地」（§ 4.1）+ 「yield watchdog」（§ 4.2）+ 「完成事件幂等」（§ 4.3）+ 「三段式 spawn」（§ 4.4）
> **人类主人排查时的额外诊断**（仅人工使用，不在主流程）：
> ```bash
> # 主流程外的诊断，主人手动执行（不在 OpenClaw runtime 主流程职责范围）
> # 仅供参考，论衡 agent 不调 exec、不读 ~/.openclaw 路径
> cd ~/.openclaw/agents/<agent>/sessions/
> F=$(列出 -t *.trajectory.jsonl | head -1)
> 计数 '"sessions_spawn"' "$F"   # spawn 次数
> 计数 '"sessions_yield"' "$F"   # yield 次数
> 匹配 'session_key: agent:[a-z]+:subagent:[a-f0-9-]+' "$F" | sort | uniq -c | sort -rn | head -5
> ```
...
**论衡主流程实际能跑的诊断**（用 read 工具推理/用 update_plan 打钩）：
...
- `sessions_history(sessionKey=...)` —— 看某个子代理会话的产物
- `update_plan` 打钩「§ 4.1 spawn 验证」「§ 4.2 watchdog 自查」「§ 4.3 幂等检查」

**论衡 agent 不调用的**：exec / process / 读 ~/.openclaw 路径 / sha256 直接计算（这些是**人类主人**的能力，不在 agent 范围内）。
