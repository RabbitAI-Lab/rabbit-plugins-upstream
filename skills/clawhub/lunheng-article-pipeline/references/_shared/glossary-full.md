...
> **v2.3.0 重构 + v2.4.0 加 T9**：原 8 角色 → v2.3.0 改为 9 张角色卡 → v2.7.0 T8 终检独立角色卡 → 共 10 张角色卡（T0-T7 + T8 终检 + T9 同行评审）。编号 = 流水线 Phase 顺序：T1-T3 检索 / T4-T5 加工 / T6-T8 防御 / T9 同行评审。教训 #116。
...
- **七维清单**（v2.7.3 公开，详见角色卡 §批判维度）：C1 核心论点可攻击性 / C2 论证链薄弱 / C3 理论假设审查 / C4 立场中立性 / C5 反方观点完整性 / C6 元叙事清理 / C7 一处两用识别（主人可对照报告核验，不再盲信）
...
- **触发阶段**：**Phase 4.5**（终稿前；T6 ∥ G14 已在 Phase 3.6 同批完成，v2.7.2 时序统一）
...
- **见 T8 独立角色卡** [`08-终检-final-inspector.md`](agents/08-终检-final-inspector.md)（v2.7.0 起独立，此前散落在主控卡 + 扩展职责）
...
- **执行模型**（v2.2.12 澄清）：
  ...
  - ❌ **不执行 shell**：算法中的 `校验哈希` 等命令仅供人类手动验证
...
### 判定标准速查（v2.7.3 文档化，ECS 实战：D14 头条聚合判 🟢 与 D15 媒体转引判 🟡 口径不一）
...
- **禁止场景**：核心论据、统计数据、精确数字
- **终稿导出（v2.7.3）**：emoji 标记仅限工作稿；导出 latex/docx/pdf 前按 [`_shared/format-export.md`](_shared/format-export.md) §四·五 清洗为正文括注，禁止 emoji 进交付稿（学术严肃性）
...
## 四、关键协议（v2.5.6 拆分，详见 [`_shared/关键协议.md`](_shared/关键协议.md)）

> **v2.5.6 拆分**（第三方独立审查建议 #3）：关键协议段（原 L192-L282，共 91 行）拆到独立文件 [`_shared/关键协议.md`](_shared/关键协议.md)，按需加载。glossary.md 仅保留协议索引。

### 关键协议索引

- **Phase 0 同意关卡**（v2.1.7 金标准）→ [`_shared/关键协议.md`](_shared/关键协议.md)
- **0 条空卡协议**（v2.1.8）→ [`_shared/关键协议.md`](_shared/关键协议.md)
- **三检索员并行独立运行**（v2.1.8 核心架构）→ [`_shared/关键协议.md`](_shared/关键协议.md)
- **修订回环 ≤2 轮降级**（v2.2.0，v2.3.1 定义细化，教训 #64 + #120）→ [`_shared/关键协议.md`](_shared/关键协议.md)
- **完成验证铁律**（v2.1.3，教训 #46）→ [`_shared/关键协议.md`](_shared/关键协议.md)
- **执行韧化协议**（v2.1.0）→ [`_shared/关键协议.md`](_shared/关键协议.md)

## 五、工具能力边界（v2.5.6 拆分，详见 [`_shared/工具能力边界.md`](_shared/工具能力边界.md)）

> **v2.5.6 拆分**：工具能力边界段拆到独立文件，按需加载。glossary.md 仅保留工具索引。
