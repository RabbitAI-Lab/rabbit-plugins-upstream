...
**处理**: 解析添加任务到待办列表的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回添加任务到待办列表的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 列出所有当前事项
列出所有当前事项

**处理**: 解析列出所有当前事项的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回列出所有当前事项的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 标记事项为已完成
标记事项为已完成

**处理**: 解析标记事项为已完成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回标记事项为已完成的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 设置事项优先级
设置事项优先级

**处理**: 解析设置事项优先级的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回设置事项优先级的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 查看今日与本周事项
查看今日与本周事项

**处理**: 解析查看今日与本周事项的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回查看今日与本周事项的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：命令行任务与笔记、管理工具、支持添加、优先级与提醒功能、任务管理领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 笔记记录器基础版处理的输入数据或指令 |
