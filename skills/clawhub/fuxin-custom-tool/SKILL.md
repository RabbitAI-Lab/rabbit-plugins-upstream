---
version: "1.00.03.195"
name: fuxin-custom-tool
description: >
  福昕 Office 企业自定义文档操作示例技能。演示企业如何组合 MCP 网关的 Word 场景工具
  （write_report / unify_terminology / highlight_and_comment / checklist_review 等），
  封装出"企业专属操作流程"，形成可复用的自定义工具。
  示例：一键生成"合同评审报告"——写报告 + 统一术语 + 高亮批注 + 清单审查的组合流程。
  Trigger: "自定义", "custom tool", "自定义工具", "企业操作", "合同评审",
  "合同检查", "一键生成评审报告", "自定义流程".
---

# fuxin-custom-tool — 福昕 Office 企业自定义文档操作示例

> **定位**: 示例技能。演示如何把多个 Word 场景工具组合成**企业自定义操作流程**，
> 以便企业参照套改为自己公司的专属文档操作。**不新增任何网关工具**，仅做组合编排。
> **产品分类**: `Word`
> **依赖**: FuxinAiService（端口见 `fuxin-office-bridge`「运行端口」，由 `MCPServerPort.ini` 读取）+ Word 场景工具 + `fuxin-word` / `fuxin-office-bridge`
...
本技能提供**一组演示用的企业自定义批量处理流程**示例（以"合同评审"为例），
展示如何组合 `fuxin-word` 的 4 个场景工具，把一句企业指令展开为一条完整的处理管线。
...
> 管线内部各步骤统属同一 MCP会话，逐步骤调用 `fuxin-word` 的场景工具，
> 由网关合并为单事务，用户一次 Ctrl+Z 即可整组撤销（见 `fuxin-batch-undo`）。
...
```
用户 → fuxin-custom-tool（本技能：定义"企业自定义操作流程"）
    │ 依次路由到
    ▼
fuxin-word 场景工具（write_report / unify_terminology / highlight_and_comment / checklist_review）
    ▼
FuxinAiService（端口见 `fuxin-office-bridge`「运行端口」） → FuxinOfficeWord App
```
...
预检链路统一走 `fuxin-office-bridge` 的 5 层预检，通过后再进入管线。
...
**步骤编排**（按顺序依次调用 `fuxin-word` 场景工具）：

1. **语义检查**：先调用 `fuxin-office-bridge` 预检 → 链路就绪
...
5. **补充预检与撤销**：入口跑 `fuxin-office-bridge` 预检，末尾给 `fuxin-batch-undo` 撤销引导
...
- **单点场景工具**：`fuxin-word` / `fuxin-excel` / `fuxin-ppt` 的完整参数表
- **预检**：`fuxin-office-bridge` 5 层链路
- **批次撤销**：管线为一事务，`fuxin-batch-undo` 提供撤销引导
- **路由入口**：`fuxin-office` 汇总层承载所有编排入口，本技能作为其自定义扩展示例
