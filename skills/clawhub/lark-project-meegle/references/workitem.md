...
查询工作项类型、字段、角色配置的辅助命令。在 `npx @lark-project/meegle@latest workitem create` / `npx @lark-project/meegle@latest workitem update` / `npx @lark-project/meegle@latest workitem query` 之前用来确认合法 key。
...
## npx @lark-project/meegle@latest workitem meta-create-fields
查看创建工作项时可用的字段及类型。
