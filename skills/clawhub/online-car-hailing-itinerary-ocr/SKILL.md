---
name: online_car_hailing_itinerary_ocr
description: 支持网约车行程单的票据识别。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 网约车行程单 OCR 技能

本技能封装了网约车行程单识别的 OCR 服务，通过单一接口即可调用 1种识别能力，高效提取票据内容。

---

## 功能特性

- **网约车行程单识别**：支持识别网约车行程单的票据信息。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• RIDE_HAILING_ITINERARY（网约车行程单） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/online_car_hailing_itinerary_ocr/scripts/main.py RIDE_HAILING_ITINERARY /path/to/test.jpg
```
...
- “帮我识别这张图像中的网约车行程单票据信息，图片在 /Users/name/Downloads/test.jpg”
