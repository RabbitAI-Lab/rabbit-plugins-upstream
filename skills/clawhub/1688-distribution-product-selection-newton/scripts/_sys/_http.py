...
def _read_proxy_from_etc_environment() -> str | None:
    """从 /etc/environment 文件中读取 https_proxy 配置"""
    etc_env_path = "/etc/environment"
    if not os.path.isfile(etc_env_path):
        return None
    try:
        with open(etc_env_path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
def _get_proxies() -> dict | None:
    """从 /etc/environment 读取 https_proxy，有则返回代理配置，无则返回 None（直连）"""
    https_proxy = _read_proxy_from_etc_environment()
    if https_proxy:
        logger.info("从 /etc/environment 检测到 https_proxy: %s，将使用代理", https_proxy)
...
def api_post(tool_name: str, body: dict = None, timeout: int = 30):
    ...
    Args:
        tool_name: 工具名称，如 fx_query_order
        body:    请求体 dict（会 json.dumps）
        timeout: 超时秒数

    Returns:
        API 响应中的 model 字段（类型取决于具体接口）
    ...
    try:
        resp = requests.post(
            ...
            timeout=timeout, proxies=_get_proxies(), verify=False,
        )
    ...
    if "success" in data and data.get("success") is False:
        msg = data.get("message") or data.get("msg") or "业务处理失败"
