...
description: Create trip-specific packing and pre-departure checklists. Use for 行李清单、出门检查、packing lists, family, road, outdoor, or international trips.
---
...
1. Extract trip facts from the request:
   ...
   - Transport and lodging: international flight, domestic flight, train, self-driving, hotel, camping, cruise; bag capacity, checked/carry-on constraints, laundry access, and drying time.
   - Activity profile: city sightseeing, hiking, theme park, photography, shopping, business, skiing, swimming.

2. If key facts are missing, make conservative assumptions instead of blocking. Ask at most one concise follow-up only when the checklist would be materially wrong without it, such as unknown child age, winter/summer ambiguity, or whether travel is international. For a high-risk outdoor trip with unknown route, season, experience, or team, do not infer route-specific equipment or go/no-go advice: give only universal risk-preparation items, state the missing factor, and ask exactly one question about the highest-impact gap; do not combine unrelated gaps in that question.
...
Use the same language for the context line, any assumptions, section headings, checklist items, optional labels, cautions, and "can buy locally" notes. Keep proper nouns, app names, document names, and destination names in their common form when clearer.

Start with a short context line. State assumptions only when a material fact was inferred:
...
When no material fact was inferred, acknowledge the supplied details instead:

`按你提供的「2名成人 / 日本 / 7天 / 酒店住宿 / 常规城市观光」整理如下。`

`Using your details — "2 adults / Japan / 7 days / hotel stay / regular city sightseeing" — here is the checklist.`
...
For policy-sensitive questions:
- Do not decide whether an item, document, medicine, battery, or bag is allowed by an airline, border authority, cruise line, venue, or attraction.
- If official live sources are available, verify only the relevant authority's current source before stating a rule. Otherwise say the rule is unverified and direct the user to that authority; provide only non-rule packing precautions.
- Read `references/rule-sensitive-checks.md` before answering a policy-sensitive question or adding an airline-specific packing instruction.
...
- For hikes that leave developed areas, include an official closure/alert check, a turnaround trigger, and a Plan B when conditions are uncertain.
...
When the route or conditions are not sufficiently known, keep the answer to risk controls and general preparation. Do not present a route-specific kit, technical system, or departure recommendation as safe to follow.
...
Climate and season:
- Cold or snow: thermal layers, gloves, hat, scarf, snow boots or anti-slip covers, hand warmers, moisturizer, lip balm. For cold or wet hiking, use moisture-managing layers, keep a dry base layer and socks in waterproof storage, and do not rely on cotton as the only insulating layer.
...
Transport:
- Flight: ID/passport, boarding documents, airline luggage limits, liquids in carry-on, power bank in an easy-access carry-on item, neck pillow or earplugs for long flights. For the operating carrier's battery, baggage, or in-flight-use rules, read `references/rule-sensitive-checks.md` and verify live official sources.
...
Luggage strategy:
- Carry-on only: use the actual bag capacity and carrier constraints, reduce liquids, prioritize quick-dry clothing, use solid toiletries, avoid bulky backups, and plan laundry and drying rather than packing speculative duplicates for longer trips.
...
- For multi-day carry-on-only travel, make the clothing quantity consistent with a stated laundry and drying plan.
...
- [ ] The response starts with a concise context line in the user's language and names material assumptions only when facts were inferred.
...
| --- | --- |
| 中文普通旅行："去成都玩4天要带什么？" | Chinese output; inferred season is stated; city-weather and pre-departure checks are present |
| Fully specified city trip | No invented assumptions; start by acknowledging supplied details |
| English carry-on only: "What should I pack for 10 days in Italy with carry-on only?" | English output; carry-on constraints and can-buy-locally split |
| Mixed language: "去新加坡 5 days with toddler, packing list please" | Chinese by default; toddler and international items are present |
| Parent/child beach trip | Child safety, sun, water, spare clothes, cleanup, and pre-departure checks precede optional items |
| Cold or wet day hike | Moisture-managing layers and dry backup items are included; closure/alert check, turnaround trigger, and Plan B are present |
| High-risk outdoor route with unknown season or team | Read `references/high-risk-outdoor.md`; give risk controls, avoid route-specific kit, and ask one single-fact question about the highest-impact gap |
| High-altitude or remote outdoor trip | Include acclimatization time margin, descent/evacuation plan, and activity-specific insurance checks; do not prescribe medication or diagnose symptoms |
| Multi-day carry-on-only trip | Clothing quantity matches bag capacity plus a laundry/drying plan, not speculative duplicate items |
| Rule-sensitive request: "Can I bring this medicine into Japan?" | Do not decide legality; use only a verified official source or state that the rule is unverified |
| Airline power bank request | Read `references/rule-sensitive-checks.md`; verify the operating carrier rather than quoting fixed capacity or use rules |
