...
VERDICT_STYLE = {
    "investable": {"color": "#16a34a", "bg": "#f0fdf4", "label": "Can Invest", "icon": "check-circle"},
    "not_investable": {"color": "#dc2626", "bg": "#fef2f2", "label": "Cannot Invest", "icon": "x-circle"},
    "needs_review": {"color": "#d97706", "bg": "#fffbeb", "label": "Needs Review", "icon": "alert-triangle"},
    "skip": {"color": "#6b7280", "bg": "#f9fafb", "label": "Skipped", "icon": "minus-circle"},
}
...
def generate_html(results, date_str):
    ...
    for item in sorted(active, key=lambda x: {"investable": 0, "needs_review": 1, "not_investable": 2}.get(x.get("verdict", ""), 3)):
        ...
        link_html = f'<a href="{url}" target="_blank" class="detail-link">View Details</a>' if url else ""
        ...
        <div class="item-card" style="border-left-color: {style['color']}; background: {style['bg']}">
            <div class="item-header">
                ...
                {priority_badge}{region_badge}{source_badge}{entity_badge}
            </div>
            ...
            <div class="item-meta">
                <span>ID: {rid}</span>
                {f'<span>Published: {publish_time}</span>' if publish_time else ''}
            </div>
            {link_html}
        </div>"""

    if total == 0:
        items_html = '<div class="empty">No new announcements today.</div>'
...
<head>
    ...
    <title>Bid Briefing - {date_str}</title>
    <style>
...
<body>
    <div class="header">
        <h1>Bid Briefing Report</h1>
        <div class="date">{date_str} | Total: {total}</div>
    </div>
