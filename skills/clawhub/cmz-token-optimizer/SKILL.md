...
description: 智能模型路由 - 根据任务是否需要工具调用自动选择模型。需要工具(搜索/浏览器/文件/天气/股票等)使用百炼付费模型，纯聊天使用OpenRouter免费模型。V3.1.0新增route_by_tool_required()函数。
version: 3.1.0
...
根据任务是否需要工具调用自动选择模型，节省成本。

### 路由规则

| 任务类型 | 判断依据 | 使用模型 |
|---------|---------|---------|
| 需要工具 | 包含 search/搜索/browser/exec/文件/天气/股票/日报 等 | bailian/qwen3.5-plus |
| 纯聊天 | 不需要上述工具 | openrouter/nvidia/nemotron-3-nano-30b-a3b:free |

### Python 使用

```python
from model_router import route_by_tool_required

# 自动路由
result = route_by_tool_required("帮我搜索天气")
print(result)
# {'needs_tool': True, 'model': 'bailian/qwen3.5-plus', ...}

result = route_by_tool_required("你好")
print(result)
# {'needs_tool': False, 'model': 'openrouter/nvidia/nemotron-3-nano-30b-a3b:free', ...}
```

### 启用自动路由

在 AGENTS.md 中添加规则，收到任务时调用 `route_by_tool_required()` 函数判断。
