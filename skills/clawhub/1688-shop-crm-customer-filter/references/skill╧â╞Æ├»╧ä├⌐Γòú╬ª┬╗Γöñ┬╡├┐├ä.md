...
本文描述 **1688-shop-crm-customer-filter** 中 Skill 调用埋点的上报时机、请求内容与失败策略，便于对接网关统计与二次开发对齐行为。
...
| 场景 | 是否上报 | 说明 |
|------|----------|------|
| 已识别子命令（如 `alibaba.1688.customer.list`、`alibaba.1688.customer.attr.field.config`），且子命令 `main()` **正常执行完毕**（无未捕获异常、未中途 `sys.exit`） | **是** | 无论业务 JSON 中 `success` 为 `true` 或 `false`（例如 AK 未注入、参数校验失败等只要进程未崩），均在子命令返回后上报一次。 |
...
| 鉴权 | 与能力调用一致：通过 `x-csk-ak` 头注入 AK；**未注入 AK 时** `api_post` 会抛鉴权类异常，由埋点模块捕获（见下文），**不会**向网关发出 HTTP 请求。 |
...
| `skillsName` | string | Skill 名称，来自环境变量 `SKILL_NAME`，缺省为 `1688-shop-crm-customer-filter`。 |
