        ...
        return new Promise((resolve, reject) => {
            setTimeout(function () {
                getTx(txid).then(tx => {
                    ...
                    if (tx.confirmations || tx.blockheight > 0)
                        return resolve(tx);
        ...
        return new Promise((resolve, reject) => {
            if (Number.isInteger(options.ignoreOld)) //backward support: data from floBlockchainAPI < v2.5.6
                readAllTxs_oldSupport(addr, options, options.ignoreOld).then(txs => {
                    let last_tx = txs.find(t => (t.confirmations > 0 || t.blockheight > 0));
        ...
        return new Promise((resolve, reject) => {
            ...
            readAllTxs(addr, query_options).then(response => {
                ...
                const filteredData = response.items.filter(tx => {
                    ...
                    if (!(tx.confirmations || tx.blockheight > 0))  //unconfirmed transactions: this should not happen as we send mempool=false in API query
                        return false;
    ...
    const getLatestData = floBlockchainAPI.getLatestData = function (addr, caseFn, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            readTxs(addr, query_options).then(response => {
                ...
                if (!new_lastItem) {
                    let last_tx = response.items.find(t => (t.confirmations > 0 || t.blockheight > 0));
                    if (last_tx)
