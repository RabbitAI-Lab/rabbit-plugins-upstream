...
| 模特身材对比（三版） | 需要"偏瘦 / 标准 / 偏胖"三位目标受众商业模特并排、各带身材数据的三联画尺码图 |
...
- 默认比例 1:1。尺码图模板**不消费** `point`（与卖点无关）、`customSetting`（画面固定为尺码示意）、`salesRegion` / `platform`（模特恒定为符合目标受众的自然面孔，由模板内置规则决定）。
...
- [ ] 三版：三位模特为偏瘦 / 标准 / 偏胖、外貌各异、年龄段与性别统一的目标受众商业模特；普通：尺码表 + 模特参考 + 测量指南线稿 + 图标化提示四模块齐全
...
- 模特恒定为符合目标受众的自然面孔（模板内置规则），不随 `salesRegion` 变化。
