#!/usr/bin/env python3
"""
TianAPI VIN Query Fetcher
-------------------------
通过17位车架号（VIN码）查询车辆信息，包括品牌、车型年款、排量、发动机类型及指导价等。
零外部依赖，仅使用 Python 标准库。

Usage:
    # 查询指定车架号的车辆信息
    python3 fetch_vin-query.py --key YOUR_KEY --vincode LSVAX60E8K2018698

Env Config:
    export TIANAPI_VIN_KEY=your_api_key
    # 或在脚本同级目录创建 .env 文件: TIANAPI_VIN_KEY=your_api_key
"""
...
# --- 配置常量 ---
API_ENDPOINT = "https://apis.tianapi.com/chavin/index"
ENV_KEY_NAME = "TIANAPI_VIN_KEY"
DEFAULT_TIMEOUT = 10
...
class Config:
    ...
    def _exit_with_help(self, msg: str):
        ...
        print(f"💡 提示: 申请 Key -> https://www.tianapi.com/apiview/260", file=sys.stderr)
        sys.exit(1)
...
class TianAPIClient:
    ...
    def fetch_vin(self, vincode: str) -> Dict[str, Any]:
        """
        执行 API 请求
...
def main():
    parser = argparse.ArgumentParser(description="TianAPI VIN Query Fetcher")
    parser.add_argument("--key", help="API Key (可选，优先使用环境变量)")
    parser.add_argument("--vincode", required=True, help="17位车架号 (如: LSVAX60E8K2018698)")
    ...
    # 2. 请求
    result = client.fetch_vin(vincode=args.vincode)
