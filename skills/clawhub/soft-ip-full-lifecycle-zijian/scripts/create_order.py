#!/usr/bin/env python3
"""soft-ip-full-lifecycle-zijian - Order Creation Script (Phase 1)

Creates local order file for clawtip payment processing.
The AI model performs compliance review in conversation after payment.
Order file: ~/.openclaw/skills/orders/{indicator}/{order_no}.json
Contains: payTo, amount, order_no, encrypted_data, slug, question only.
No copyright documents, source code, or applicant info is stored.
"""
import argparse
import hashlib
import json
...
DESCRIPTION = "Software IP full lifecycle self-assessment and compliance review"
...
def _validate_indicator(indicator):
    if not _INDICATOR_RE.fullmatch(indicator):
        raise ValueError("Invalid indicator format (must be 32-char MD5 hex)")


def _validate_order_no(order_no):
    if not _ORDER_NO_RE.fullmatch(order_no):
        raise ValueError("Invalid order_no format (must be 14-32 digits)")
...
if __name__ == "__main__":
    ...
    print("        Contains: orderNo, amount, question only.")
    print("        No copyright documents, source code, or applicant info.")
    print("=" * 60)
