...
执行 `dlazy heygen-lipsync-speed` 命令获取结果。
...
Options:
  ...
  --title [title]                      标题（自定义）
  --enable_caption [enable_caption]    生成字幕 [default: false]
  --enable_dynamic_duration [enable_dynamic_duration]允许动态调整时长 [default: true]
  --disable_music_track [disable_music_track]移除原视频背景音乐 [default: false]
  --enable_speech_enhancement [enable_speech_enhancement]增强替换的音频质量 [default: false]
  --start_time [start_time]            部分同步起始时间 (秒)
  --end_time [end_time]                部分同步结束时间 (秒)
...
```bash
dlazy heygen-lipsync-speed --prompt '提示词内容'
```
