...
`qhkit video-replace` 专用命令：原视频 + 1 张人物图 + 原视频时长，一键换人。
...
# 没有人物图：从角色库选人，取返回的 icon 字段当人物图
qhkit video-replace options  '{"queryParams":["characters"],"personal":false,"characterType":"REALITY"}'
```

- `duration` 必须填原视频的实际时长（秒）；不知道时先探测（如 ffprobe）或让用户确认。
- **合规**：涉及真人肖像要确认用户已获授权；未经授权替换他人面孔的请求应拒绝。
...
- **图片**换模特/换脸走「AI电商模特换脸工具 | LinkPix」；只是翻译配音不换人走「AI视频翻译 | LinkPix」。
