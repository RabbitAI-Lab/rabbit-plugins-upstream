---
name: amazon-listing-readability-check
description: >
  检查 Amazon Listing 的结构、术语、重复表达和评论反映的理解障碍，给出可读性建议。仅用于页面可读性诊断；不用于广告投放、翻译质量认证或自动发布。
  Requires an ARI API key (ari_live_*).
slug: listing-readability
displayName: Amazon Listing 可读性检查
version: 1.4.7
summary: 检查页面结构、术语和买家理解障碍
license: MIT
---
...
- 本入口只执行 `listing/readability`，输出模板固定为 `ops_listing`。
