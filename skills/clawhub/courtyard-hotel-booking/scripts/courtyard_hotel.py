...
"""
万怡酒店技能 - ClawHub版
3个工具：search_courtyard_hotels / get_courtyard_hotel_info / search_courtyard_packages
数据源：飞猪MCP via fliggy-proxy SCF代理（万豪集团专区，自动过滤万怡品牌）
纯标准库实现
"""
import json
import os
...
def search_courtyard_hotels(dest_name, check_in=None, check_out=None,
                                  keyword=None, max_price=None, sort=None, limit=10):
    """
    搜索万怡酒店（万豪集团旗下）
    自动注入品牌关键词"万怡"，用户只需提供城市即可
    """
...
def get_courtyard_hotel_info(shid=None, hotel_name=None, review_keyword=None):
    """
    查询万怡酒店详情（交通/景点/设施/政策/房型）
    """
...
def search_courtyard_packages(keyword=None, hotel_name=None,
                                    province_or_city=None, sort=None, limit=10):
    """
    搜索万怡酒店套餐优惠（含早/连住/门票等打包产品）
    自动注入品牌关键词"万怡"
    """
    params = {}
    if keyword:
...
def main():
    if len(sys.argv) < 3:
        print("用法: python courtyard_hotel.py <tool> <args_json>")
        print("  tool: search | detail | packages")
        print('  示例: python courtyard_hotel.py search \'{"dest_name":"上海"}\'')
        return
    ...
    if tool == "search":
        print(search_courtyard_hotels(**args))
    elif tool == "detail":
        print(get_courtyard_hotel_info(**args))
    elif tool == "packages":
        print(search_courtyard_packages(**args))
    else:
