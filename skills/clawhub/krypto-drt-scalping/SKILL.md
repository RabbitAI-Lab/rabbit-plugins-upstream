...
description: "Crypto DRT scalping strategy + backtest reference on 12 crypto pairs (TRX, BNB, BTC, LINK, AVAX, DOGE, SOL, NEAR, XRP, LDO, ADA, ETH), all 7 days. Backtested 2 years (~400+ trades). Core skill is FREE and runs locally. ⚠️ OPTIONAL premium live signals use the paid x402 API (X402_API_KEY, USDC on Ethereum) — PAID external call. Backtest results are historical and not a guarantee of future performance. Separate play-money account rules."
metadata: {"clawbot": {"requires": {"python3": true, "network": ["https://186.240.156.169:8791"], "env": ["X402_API_KEY"]}, "permissions": {"exec": ["python3"], "network": ["https://186.240.156.169:8791"], "env": ["X402_API_KEY"], "notes": "The core skill is a free backtest/strategy reference — runs locally with no network. OPTIONAL premium live signals call the x402 API (X402_API_KEY, USDC on Ethereum) — PAID external call, HTTP blocked unless X402_ALLOW_HTTP=1. Read the premium section before using."}}}
---
