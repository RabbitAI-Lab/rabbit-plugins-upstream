...
from repo_reader import prepare_context
from task_io import print_json, read_payload, write_result
from utils import read_json_file, write_json_file


def main():
    parser = argparse.ArgumentParser(description="Research KB Git/Gitea repository ingest helper")
    subparsers = parser.add_subparsers(dest="command", required=True)

    prepare = subparsers.add_parser("prepare", help="Inspect the remote repository and build an incremental analysis context")
    ...
    prepare.add_argument("--result-output", help="Optional result JSON path for unchanged repositories")

    apply = subparsers.add_parser("apply", help="Write generated entity pages to the Gitea KB")
...
def run_prepare(args):
    ...
    try:
        ...
        if context.get("mode") == "skip":
            ...
            if result_path:
                write_result(result_path, context.get("skipResult") or {})
        print_json(context)
