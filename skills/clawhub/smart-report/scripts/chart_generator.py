...
class ChartGenerator(ChartRenderersMixin, HTMLTemplateMixin):
    ...
    def __init__(self, output_dir: str = './smart_charts_output', theme: str = 'default'):
        ...
        # 渲染器静态派发表：用字面量属性访问替代 getattr 动态派发，
        # 只暴露 ChartType 枚举到具体渲染方法的显式映射，杜绝任意属性访问。
        # spreadsheet 在 generate_chart 中提前分流，不在此表内。
        self._renderers = {
            ...
            ChartType.RADAR: self._radar,
            ChartType.HEATMAP: self._heatmap,
            ChartType.TREEMAP: self._treemap,
            ChartType.GRAPH: self._graph,
            ChartType.BOXPLOT: self._boxplot,
            ChartType.WATERFALL: self._waterfall,
            ChartType.GAUGE: self._gauge,
            ChartType.SANKEY: self._sankey,
            ChartType.FUNNEL: self._funnel,
            ChartType.SUNBURST: self._sunburst,
            ChartType.WORDCLOUD: self._wordcloud,
            ChartType.HISTOGRAM: self._histogram,
            ChartType.STACKED_BAR: self._stacked_bar,
            ChartType.BUBBLE: self._bubble,
            ChartType.PARETO: self._pareto,
            ChartType.COMBO: self._combo,
            ChartType.VENN: self._venn,
            ChartType.MINDMAP: self._mindmap,
            ChartType.ORGCHART: self._orgchart,
            ChartType.LIQUID: self._liquid,
        }
