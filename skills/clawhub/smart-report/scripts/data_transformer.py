...
安全机制（三层校验 + 子进程隔离，无需用户确认）：
1. 关键字黑名单校验 — 在执行前扫描代码中的危险关键字，阻止恶意代码运行
2. AST 白名单校验 — 解析代码的抽象语法树，仅允许安全的 AST 节点类型
3. 沙箱执行 — 在资源受限的子进程中执行，仅暴露安全内置函数，禁止 open/exec/eval/__import__ 等
"""
...
class _DangerousAttributeVisitor(ast.NodeVisitor):
    """遍历 AST，命中下划线开头属性或 DANGEROUS_ATTRIBUTES 中的属性访问即记录违规。

    下划线拦截覆盖 dunder（__class__/__reduce__/__getattribute__ 等）与私有属性
    （_typ/_metadata 等），封堵对象内省逃逸路径；正常 pandas/numpy 链式调用
    （groupby/fillna/iloc/str/dt 等）不含下划线前缀，不受影响。
    """
    ...
    def visit_Attribute(self, node):
        if node.attr.startswith('_'):
            self.violations.append(f"不允许访问下划线开头的属性: .{node.attr}")
        elif node.attr.lower() in DANGEROUS_ATTRIBUTES:
...
class DataTransformer:
    ...
    def _execute_in_sandbox(self, df: pd.DataFrame, code: str) -> pd.DataFrame:
        """在受限子进程中执行已通过黑名单/AST 校验的代码。

        安全措施：
        - 子进程边界 + 进程级资源限制（CPU/内存），与父进程隔离
        - 子进程仅暴露安全 builtins（open/exec/eval/__import__ 等被移除）
        - subprocess 墙钟超时跨平台生效（替代原 SIGALRM 仅 Unix 的方案）
        错误码与 suggestions 与旧实现保持一致，CLI 结构化输出契约不变。
        """
        runner = Path(__file__).with_name('_transform_runner.py')
        with tempfile.TemporaryDirectory(prefix='sr_transform_') as td:
            td_path = Path(td)
            df_path = td_path / 'df.pkl'
            code_path = td_path / 'code.txt'
            result_path = td_path / 'result.pkl'
            ...
            try:
                proc = subprocess.run(
                    [sys.executable, str(runner), str(df_path), str(code_path),
                     str(result_path), str(status_path)],
            ...
            if proc.returncode != 0:
                # 子进程异常崩溃（如 OOM、段错误）：透传 stderr 尾部便于排查
                raise TransformError(
                    f"转换代码执行失败（沙箱子进程退出码 {proc.returncode}）",
                    ErrorCode.TRANSFORM_EXEC_ERROR,
                    details={'code': code, 'stderr': (proc.stderr or '')[-500:],
            ...
            try:
                ...
                raise TransformError(
                    ...
                    ErrorCode.TRANSFORM_EXEC_ERROR,
                    details={'code': code,
                             'suggestion': '沙箱子进程异常退出，请重试；如反复出现请反馈给维护者'},
                )

            if not status.get('ok'):
                err_value = status.get('code', ErrorCode.TRANSFORM_EXEC_ERROR.value)
                try:
                    err_code = ErrorCode(err_value)
                except ValueError:
                    err_code = ErrorCode.TRANSFORM_EXEC_ERROR
                message = status.get('message') or '转换代码执行失败'
                details = {'code': code}
                if err_code is ErrorCode.TRANSFORM_INVALID_RESULT and status.get('result_type'):
                    details['result_type'] = status['result_type']
                if err_code is ErrorCode.TRANSFORM_EXEC_ERROR:
                    details['error'] = message
                    details['suggestion'] = '根据报错信息修正转换代码；可先用 print(df.columns) 或 print(df.head()) 检查列名与数据类型'
                raise TransformError(message, err_code, details=details)

            result = pd.read_pickle(result_path)

        # 双保险：即使子进程状态异常也不放行非法结果
