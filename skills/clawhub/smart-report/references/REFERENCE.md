...
Optional deps (for `--format docx/pptx`): `python-docx`, `python-pptx`, `resvg-py`（SVG→PNG 光栅化，纯 wheel 无系统依赖）+ Node.js ≥ 16（图表 SSR 静态化）。缺任一时该格式降级报 5007，HTML 交付不受影响。

Self-test (developer): 无独立回归脚本，用两条命令自检——`python {skill_base}/scripts/cli.py <file> <chart_type> --dry-run`（校验渲染配置并输出 `plot_stats`/`data_preview`，不写 HTML）与 `python {skill_base}/scripts/report_assembler.py --spec <spec.json> --charts-dir <dir> --ledger <ledger.json>`（校验 spec 组装 + 台账溯源）。

---

## Capability Boundaries

**Supported:** CSV (.csv=comma / .tsv=tab / .txt=auto-detect delimiter), Excel (.xlsx/.xls), JSON (.json，支持 1 层嵌套对象，自动展开为「父.子」点分列，列名规范化后为「父_子」); 26 chart types (see SKILL.md); 3 themes (`--theme default|classic|dark`); multi-file auto-merge (recommended ≤ 10 files); single file ≤ 100 MB (≤ 50 MB recommended); auto-detects UTF-8/GBK/GB2312/UTF-16/Latin-1. 交付格式：单文件交互式 HTML（默认）+ 静态 DOCX/PPTX（可选导出，图表内嵌 PNG，台账附录自动附加）。

**Not supported:** Databases (export to CSV first), real-time/streaming data, geo maps, >100 MB files, nested JSON >1 level, non-tabular data (images/audio/video), Word/PPT 之外的私有文档格式. Auto-merge requires ≥50% column overlap. docx/pptx 为静态快照（无交互），需相应可选依赖。
...
**Security:** transform 代码经三层校验（关键字黑名单 + AST 白名单 + 安全 builtins）后，在资源受限的子进程沙箱中执行（CPU 30s / 内存 2GB / 递归深度 500 / 墙钟 10s），违规返回带 `suggestion` 的结构化错误，按提示修正重试即可，无需用户确认。机制细节见下方 Transform Code Generation。
...
| 5001 | REPORT_SPEC_INVALID | report_spec.json 非法（缺字段/结构错） |
| 5002 | CHART_HTML_INVALID | 图表 HTML 缺失或非法 |
| 5003 | REPORT_ASSEMBLE_ERROR | 报告组装失败（含未捕获异常兜底） |
| 5004 | LEDGER_MISMATCH | 台账未溯源/占位符 id 未注册/精度不符 |
| 5005 | RAW_NUMBER_IN_PLACEHOLDER_MODE | placeholder 模式下存在必溯源形态的裸数字 |
| 5006 | EXPORT_ERROR | docx/pptx/svg 导出任一步失败（SSR 静态化失败） |
| 5007 | EXPORT_DEPENDENCY_MISSING | 缺 python-docx/python-pptx/resvg-py/node |
...
When raw data doesn't match the target chart's input format, the LLM should generate pandas code following the template in [SKILL.md](../SKILL.md). The code is validated in-process (keyword blacklist + AST whitelist + safe builtins) and executed in a resource-limited subprocess sandbox before chart rendering.
...
- Wall-clock timeout: 10 seconds (subprocess); subprocess CPU quota 30s / memory quota 2GB
