...
> **核心功能**: 本技能提供、格式互转、内容提取时使用、提升工作效率等能力。
...
3. **通用方法**：
   ...
   - **优势**：虽然命令行工具可以处理文件操作，但对于非技术用户来说，它们的学习曲线较陡峭。file-browser-tool提供了更为友好的Markdown+execute()接口，降低了使用门槛。
### 独特功能
...
## 功能清单
- **自动化执行**: SkillHub工作区只读文件浏览与读取,安全查看。Read-only file browsing and readin
