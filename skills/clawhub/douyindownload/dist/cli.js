...
async function cmdParse(url) {
    // 先检查额度
    const check = (0, license_js_1.checkAndRecord)();
    if (!check.allowed) {
        log(`\n⚠️  免费次数已用完\n`, 'yellow');
...
async function cmdActivate(key) {
    const result = await (0, license_js_1.activateKey)('', key);
    if (result.isPaid) {
        log(`\n✅ ${result.reason}\n`, 'green');
    }
    else {
...
function cmdStatus() {
    const result = (0, license_js_1.getStatus)();
