...
// 检测平台
function detectPlatform(url: string): 'douyin' | 'unknown' {
...
async function parseDouyin(url: string): Promise<ParseResult> {
  ...
  try {
    ...
    const redirectResp = await axios.get(realUrl, {
      headers: {
  ...
  if (!videoData) {
    try {
      ...
      videoData = parseHtmlVideoData(pageResp.data as string, videoId);
    } catch (pageErr: any) {
...
// 从 HTML 中解析视频数据（备用）
function parseHtmlVideoData(html: string, videoId: string): any {
  try {
