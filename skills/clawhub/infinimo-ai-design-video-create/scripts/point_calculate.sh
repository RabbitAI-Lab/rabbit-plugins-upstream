...
[[ -n "$MODE" && -n "$MODEL" && -n "$RATIO" && -n "$SIZE" ]] || {
  echo "Usage: point_calculate.sh --mode N --model ID --ratio ID --size ID [--length N]" >&2; exit 1; }
TOKEN="${INFINIMO_TOKEN:-${INFINIMO_API_KEY:?Set INFINIMO_TOKEN or INFINIMO_API_KEY}}"
PAYLOAD=$(python3 -c 'import json,sys; m,md,r,s,l,p=sys.argv[1:8]; b={"create_mode":int(m),"model":md,"ratio":r,"size":s,"prompt":p,"platform":1,"terminal":4,"language":"en"};
ln=l.strip();
