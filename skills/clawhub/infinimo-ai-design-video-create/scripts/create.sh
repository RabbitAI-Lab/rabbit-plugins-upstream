...
[[ -n "$MODE" && -n "$PROMPT" && -n "$MODEL" && -n "$RATIO" && -n "$SIZE" ]] || {
  echo "Usage: create.sh --mode N --prompt TEXT --model ID --ratio ID --size ID [--length N] [--attaches JSON]" >&2; exit 1; }
...
b={"create_mode":int(mode),"prompt":prompt,"model":model,"ratio":ratio,"size":size,"platform":1,"terminal":4,"language":"en"}
ln=length.strip()
