...
description: "Bridge to MCP servers - IPFS, GitHub, filesystem"
version: 2.1.0
capabilities:
...
> NEXUS Agent-as-a-Service on Cardano + Stellar + XRPL | Price: $0.25/request | Accepts: ADA, DJED, iUSD, USDCx, USDM (Cardano) + USDC, XLM (Stellar) + XRP, RLUSD (XRPL)

## When to use

Use when you need to bridge to mcp servers - ipfs, github, filesystem.
...
curl -X POST https://ai-service-hub-15.emergent.host/api/original-services/mcp-bridge \
  ...
  -d '{
  "input": "Example input for MCP Bridge Agent"
}'
```
