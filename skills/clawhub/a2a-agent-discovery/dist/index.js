...
server.registerTool("get_hub_agent_card", {
...
}, async ({ agent_card_url, endpoint_url, service_request, payload, dry_run, confirm_external_request }) => {
...
server.registerTool("normalize_manifest", {
...
}, async ({ agent_card_url, manifest, dry_run, confirm_external_submission }) => {
    const payload = normalizeManifest({ ...manifest, ...(agent_card_url ? { agent_card_url } : {}) });
    if (dry_run) {
...
async function getRuntimeEndpointFromAgentCard(agentCardUrl) {
    if (!agentCardUrl) {
...
function extractRuntimeEndpoint(card) {
    if (!isObject(card)) {
        return "";
    }
    const candidates = [card.url];
...
function buildServiceRequestPayload(serviceRequest) {
    return {
        jsonrpc: "2.0",
        id: `request-${Date.now()}`,
        method: "tasks/send",
...
function assertPayloadHasNoObviousSecrets(payload) {
...
function stringifyCompact(data) {
    try {
