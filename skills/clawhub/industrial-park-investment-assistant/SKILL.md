...
> **适用场景：** 需要向客户发送正式的办公选址建议书，用于客户内部决策参考。
> **输出格式：** Markdown文档，可直接复制粘贴到邮件/微信，或导出为PDF。

**核心逻辑：** 生成专业、完整、可直接发给客户的选址建议书，帮助客户进行内部决策。
...
1. **读取客户档案** — 从 `customers/客户档案.json` 读取企业信息
   - 企业名称、行业、需求面积、预算、入驻时间
   - 若信息不全，询问用户补充
2. **读取房源信息** — 从知识库读取可租房源，筛选匹配房源
   - 面积匹配（需求面积 × 0.8 ~ 需求面积 × 1.2）
   - 租金匹配（预算范围内）
   - 楼层/装修状态匹配（如有要求）
3. **读取配套资源** — 从知识库读取园区配套信息
   - 交通配套（地铁/公交/停车）
   - 商业配套（食堂/餐饮/商业）
   - 产业生态（已入驻企业/产业链）
4. **读取政策信息** — 从知识库读取地方产业政策
   - 科技创新政策
   - 人才支持政策
   - 产业发展政策
   - 专项扶持政策
5. **生成选址建议书** — 按照 `templates/选址方案模板.md` 生成完整文档
6. **输出** — Markdown格式，可直接发给客户

---

**输出格式：** 按照 `templates/选址方案模板.md` 模板生成完整文档
...
| 数据项 | 来源 | 说明 |
|--------|------|------|
| 企业需求 | `customers/客户档案.json` | 企业基本信息 |
| 房源信息 | 知识库（房源销控表） | 可租房源清单 |
| 租金报价 | 知识库（租金报价表） | 租金/物业费标准 |
| 配套资源 | 知识库（配套资源表） | 交通/商业/产业配套 |
| 政策信息 | 知识库（地方产业政策） | 省/市/区各级政策 |
...
1. **禁止编造数据** — 所有信息必须基于知识库，禁止编造
2. **政策风险提示** — 政策收益需标注"以政府最新政策及审核结果为准"
3. **成本展示规范** — 仅展示实际费用构成，不做政策抵扣（避免给客户不切实际的预期）
4. **配套信息核实** — 若知识库中配套信息为`{TODO}`，需标注"待核实"
...
| 功能 | 关系 | 说明 |
|------|------|------|
| 智能推荐（6.1.C） | 被整合 | 选址方案生成器包含智能推荐功能 |
| TCO成本对比（6.2） | 被整合 | TCO分析作为内部决策工具，不展示给客户 |
| 竞品对比 | 可选功能 | 若客户需要，可在方案中加入竞品对比（作为附件） |

---

### 8. 📄 合同生成（自动化流程）

> **适用场景：报价方案确认后，自动生成标准合同文件。**
> **支持项目：上海美兰中心（模板已就位）**

**一句话：** 谈判确认后说「给XX公司生成合同」→ AI 自动匹配项目模板并填充客户信息。
...
**能力说明：**
- 按项目管理模板（`contract_templates/{项目名}/`）
- 支持模板：主合同、附件4（交付确认书）、附件11（廉洁协议）
- 自动匹配项目模板 → 填充客户信息 → 生成 .docx 文件
- 未来扩展：生成后自动上传腾讯文档

**注意：** 模板文件放在项目知识库（IMA）中管理，AI 自动拉取使用。
...
- ✕ **企查查敏感数据未脱敏**：企查查返回的联系方式、财务/年报、自然人身份证号等敏感字段，落档(客户档案)与对外发送(报价/选址建议)时必须脱敏，不得明文写入文件或消息
...
> - **企查查MCP顺藤摸瓜（已连接时）** → 锚定标杆企业后，调用 `get_external_investments`(对外投资)/`get_branches`(分支)/股东关联，挖上下游关联企业名单（比WebSearch更准，是企查查最强能力）

**招商不是"等客户"，是"找客户"。**

Agent 帮你做：
1. **产业链招商** — 根据已入驻企业/主导产业，找上下游配套企业（用企查查MCP锚定标杆→查对外投资/分支→顺藤摸瓜生成关联名单）
