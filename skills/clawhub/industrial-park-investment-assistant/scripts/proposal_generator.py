...
# 添加 skills 目录到路径（data_factory.py 在 skills/ 下）
sys.path.append(str(Path(__file__).parent.parent.parent))

try:
    from data_factory import ParkDataInterface, get_data_adapter
except ImportError:
...
class ProposalGenerator:
    ...
    def __init__(self, data_interface=None, workspace_dir=None):
        ...
        Args:
            ...
            workspace_dir: 工作区目录
        """
        self.data = data_interface or (get_data_adapter() if get_data_adapter else ParkDataInterface())
        ...
        # 通过 data_interface 获取数据(不直接加载知识库)
        pass

        # 数据通过 data_interface 获取
        pass
    ...
    def _generate_section1_tco(self, customer_info, matched_products, tco_data):
        ...
        # 使用真实租金数据
        rent_info = self.data.get_rent_info() if self.data else {'property_fee': 16, 'avg_price': 1.9}
...
"""
    ...
    def _generate_section2_ecosystem(self, customer_info, matched_products):
        ...
        # 使用真实企业名录
        enterprises_content = ""
        if self.data.get_enterprises():
            # 从真实数据提取企业列表
            enterprises_list = '\n'.join([f"- **{e['name']}** ({e['industry']})" for e in self.data.get_enterprises() if e.get('status') == '已签约'][:10])
...
**招商负责人:** 张颖异 / 蓝永琴
**联系电话:** 138-xxxx-xxxx / 139-xxxx-xxxx
**邮箱:** {TODO: 填写邮箱} / {TODO: 填写邮箱}
...
"""
    ...
    def _find_matching_enterprises(self, industry, enterprises_content=None):
        ...
        if not self.data.get_enterprises():
        ...
        for ent in self.data.get_enterprises():
