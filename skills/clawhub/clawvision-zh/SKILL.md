---
name: "clawvision-zh"
description: "ClawVision 中文版 1.0.7 — 本地导出工具。读取你选择的 OpenClaw 会话历史，使用本地 LLM 总结，并写入 HTML、Markdown、PowerPoint 和 PNG 导出文件到本地磁盘。需要权限：读取会话历史、列出会话、本地文件读写、执行本地 Python/Playwright 脚本、本地 LLM 推理。数据不会发送到外部 API。"
metadata:
  version: 1.0.7
  author: Maximius
  tags: [visualization, summary, html, sessions, codex, markdown, powerpoint, aesthetic, presets, chinese, 中文]
  homepage: https://github.com/monaxamo/clawvision-zh
  license: MIT
