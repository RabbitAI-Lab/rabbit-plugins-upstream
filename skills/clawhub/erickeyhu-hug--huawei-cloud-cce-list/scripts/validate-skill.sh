...
check "Critical" "No credential hardcoding" "$(grep -RqiE '(AK=|SK=|access.key=|secret.key=|password=)' "$SKILL_PATH" --include='*.md' --include='*.json' --include='*.sh' --exclude='validate-skill.sh' && echo FAIL || echo PASS)"
