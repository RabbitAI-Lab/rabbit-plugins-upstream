...
- **Full Abstracts**: All sources return complete abstracts (PubMed uses `efetch` for full XML records).
...
This skill uses a local Python script (`lit_search.py`) that you need in your workspace. Place it at a known path and adjust the commands below accordingly. The script requires Python 3 and the `requests` library.

```bash
# Install dependencies
pip install requests

# Run a search (adjust path to your local script location)
python3 /path/to/lit_search.py search "your query" --limit 5 --source all
```
...
```bash
python3 /path/to/lit_search.py search "impact of glycyrrhiza on bifidobacterium" --limit 5 --source all
```
...
```bash
python3 /path/to/lit_search.py search "prebiotic effects of liquorice" --source pm
```

### 3. Comparing Sources
Search both S2 and OA simultaneously to ensure nothing is missed. Deduplicated by default.
```bash
python3 /path/to/lit_search.py search "Bifidobacterium infantis growth" --source both
```

### 4. Getting Full Details (S2)
Retrieve detailed metadata including TL;DR summaries.
```bash
python3 /path/to/lit_search.py details "DOI:10.1016/j.foodchem.2023.136000"
```
