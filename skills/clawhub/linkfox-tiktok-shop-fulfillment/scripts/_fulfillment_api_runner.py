"""Run registered TikTok Shop ERP fulfillment APIs via /tiktokShop/developerProxy."""
...
from _fulfillment_endpoints import FULFILLMENT_ENDPOINTS, RESERVED_PARAM_KEYS
from _shop_fulfillment_common import (
...
def resolve_shop_cipher(params: dict, open_id: str) -> str:
    ...
    print(
        ...
        '  Example: {"api":"get_order_split_attributes","openId":"...","order_ids":["..."],'
        '"shop_cipher":"GCP_..."}',
        file=sys.stderr,
    )
    sys.exit(1)
...
def run_fulfillment_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in FULFILLMENT_ENDPOINTS:
        print(
            f"Unknown api: {api_name}. Valid: {', '.join(sorted(FULFILLMENT_ENDPOINTS))}",
            file=sys.stderr,
        )
        sys.exit(1)

    spec = FULFILLMENT_ENDPOINTS[api_name]
...
def run_fulfillment_proxy(params: dict, caller: str = "fulfillment_proxy.py") -> dict:
    ...
    shop_cipher = None
    needs_cipher = str(path).lstrip("/").startswith("fulfillment/")
    if needs_cipher:
        shop_cipher = resolve_shop_cipher(params, open_id)
