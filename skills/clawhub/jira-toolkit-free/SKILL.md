...
**处理**: 解析查看、创建与更新 Jira 事的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回查看、创建与更新 Jira 事的响应数据,包含状态码、结果和日志.
...
**处理**: 解析变更事务状态与分配负责人的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回变更事务状态与分配负责人的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 添加评论与查看当前 Sprin
添加评论与查看当前 Sprint

**处理**: 解析添加评论与查看当前 Sprin的输入参数,完成核心逻辑,返回结构化响应.
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | Jira 事务工具包基础版处理的输入数据或指令 |
