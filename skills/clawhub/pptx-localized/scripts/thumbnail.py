...
# soffice import adapted for Windows: just use subprocess directly;
# the localized soffice.py wrapper is also available in scripts/office/
try:
...
# Fixed: was LABEL_PADDING_RATIO in original (typo), corrected below
...
def _convert_to_images(pptx_path: Path, temp_dir: Path) -> list[Path]:
    ...
    try:
        ...
        if result.returncode != 0 or not pdf_path.exists():
            print(f"Warning: PDF conversion with soffice failed: {result.stderr}", file=sys.stderr)
            return []
    except (FileNotFoundError, subprocess.TimeoutExpired) as e:
        print(f"Warning: soffice not available or timed out: {e}", file=sys.stderr)
        return []
    ...
    try:
        ...
        print(f"Warning: pdftoppm not found (install Poppler): {e}", file=sys.stderr)
        return []
...
def _create_grid(
    ...
    # Fixed: was LABEL_PATIO (typo), now using correct name
