...
## 12-Stage Complete Product Lifecycle
...
1. **Industry Analysis Report** (15-25 page PPT, use `ppt-generator`)
...
```
Total budget: 200K–1M tokens (per current leading models; plan conservatively at 200K; corrected 2026-07)
...
#### 5.3 AI Native Capability Enhancement (2025–2026 Routine Iteration, Added 2026-07)

> This section supplements the "10 Modules" above with frontier paradigms. Existing modules (RAG/Agent/Prompt/Evaluation, etc.) remain unchanged.

##### Supplement 1: RAG 2.0 — GraphRAG & Agentic RAG

- **GraphRAG**: Beyond vector retrieval, introduces "knowledge graph + entity/relationship extraction." First uses LLM to extract entities and relationships from documents to build a graph, then performs "graph retrieval + vector retrieval" hybrid answering. Suitable for strongly relational, high-reasoning scenarios (contract clause correlation, organizational structure, permission inheritance, supply chain traceability).
- **Agentic RAG**: Uses an Agent to replace "one-shot retrieval," autonomously deciding "what to retrieve / how many rounds / whether to switch data sources / whether to invoke tools for verification." Typical pattern: Router-Retriever → Critique → Re-retrieve. Suitable for answers requiring multi-source cross-validation and long-chain reasoning in B2B scenarios.
- **Implementation Guidance**: FAQ / single-document Q&A use basic RAG; cross-document reasoning, compliance review, audit traceability prioritize GraphRAG / Agentic RAG.

##### Supplement 2: MCP (Model Context Protocol) as the Standard for Agent Tool Integration

- **MCP** has become the open integration standard for Agents connecting to external systems (databases, ERP, CRM, file systems, internal APIs). B2B AI products should prioritize exposing enterprise capabilities via MCP Servers rather than writing one-off function-calling adapters for each system.
- **PM Perspective**: Abstract "internal enterprise system capabilities" into an MCP tool catalog (read/write/approve/query); Agents invoke based on permission and HITL matrix. During technology selection, prioritize the target system's MCP / open API maturity as an evaluation dimension.

##### Supplement 3: Computer Use for Handling API-less Legacy Systems

- For legacy B2B systems without open APIs (legacy ERP / in-house intranet systems), **Computer Use (models directly operating GUI/browsers)** can serve as the "last mile" integration: letting Agents click, fill forms, and read pages like a human.
- **Risk Controls**: Limited to low-risk read-only / data-entry scenarios; must include HITL confirmation + operation replay auditing. Direct execution of high-risk actions (payments, permission changes, etc.) is strictly prohibited (aligned with the HITL matrix in Module 3).

##### Supplement 4: Reasoning Model Routing

- Not every task needs the largest model. Use **tiered routing** based on task complexity:
  - **Lightweight Tier** (classification / extraction / rewriting) → Small models / low-cost APIs: GPT-4o-mini / Claude Haiku / Gemini Flash
  - **Mid-Tier** (general Q&A, summarization, standard generation) → Standard large models: GPT-4.1 / Claude Sonnet 4 / Gemini 2.5 Pro
  - **Heavy Reasoning Tier** (multi-step planning, code, math, compliance argumentation) → Reasoning models: OpenAI o1 / o3, Claude Opus 4.5, DeepSeek-R1, GPT-5
- **Cost Governance**: Add "routing strategy + budget cap + degradation fallback" at the Agent orchestration layer to prevent all requests from routing to the most expensive model.

##### Supplement 5: Chinese Open-Source Models & Xinchuang LLM Adaptation (China Subsidiary / Global Expansion Perspective)

- **Top Choices for Private Deployment**: DeepSeek (V2/R1 series — cost paradigm representative for reasoning), Qwen (Tongyi Qianwen — full-size open-source), GLM (Zhipu — ChatGLM/GLM series) are the primary base models for Chinese B2B private deployment and Xinchuang deployment scenarios.
- **Xinchuang LLM Adaptation**: In finance, government, central SOEs, and other China-market scenarios, adaptation to Xinchuang software/hardware stacks (domestic CPU / OS / database) is required. Prioritize open-source / domestic LLMs that have completed Xinchuang compatibility mutual certifications. Add a "Xinchuang Compatibility" dimension in the model selection matrix.
- **Hybrid Deployment**: Sensitive data routes to privately deployed open-source models; general capabilities route to closed-source APIs via a unified gateway with routing and data masking.
- **Global Expansion Perspective**: For Chinese SaaS companies expanding overseas, evaluate local open-source alternatives (e.g., Llama, Mistral) alongside Chinese models to meet data sovereignty requirements in target markets (EU/GDPR, Middle East, Southeast Asia).

##### Supplement 6: Embedding Model Recommendation Update

- Additional recommendations: **bge-m3** (Chinese-English multilingual, supports dense/sparse/multi-vector — top choice for Chinese B2B), **Voyage AI** (long context, strong on code/enterprise corpora), **Jina Embeddings** (multilingual, long-document friendly).
- Existing text-embedding-3 / bge-large-zh recommendations remain valid. For new scenarios, prioritize evaluating bge-m3 / Voyage / Jina, and select based on three dimensions: "language coverage + context length + retrieval accuracy."
...
**Method 3: JsDesign (Domestic Figma alternative)** [China Subsidiary / Global Expansion Perspective]
...
To PPT: use ppt-generator skill (extract key points, auto layout)
```
...
#### AI-Assisted Development (Coding Agent) (Added 2026-07)

> Beyond bi-weekly iterations, AI coding has become the core lever for B2B R&D efficiency. PMs don't need to write code, but must understand how to collaborate with Coding Agents, set boundaries, and conduct acceptance.

**Coding Agent Collaboration Modes:**

| Mode | Applicable | Example | PM's Focus |
|------|------|------|------|
| **Code Completion / Generation** | Well-defined small changes, boilerplate | GitHub Copilot / Tongyi Lingma / Zhipu CodeGeeX | Are requirements described clearly enough? |
| **End-to-End Implementation** | Independent modules/scripts with clear boundaries | Cursor / Claude Code / Devin-class agents | Are ACs machine-verifiable? |
| **Test / Documentation Generation** | Unit tests, API docs, SQL | Various agents | Coverage and accuracy |
| **Code Review / Refactoring** | Legacy code optimization, code smell cleanup | AI Review | Don't break existing contracts |

**PM Collaboration Essentials:**
1. **Requirements as Specs**: Feed ACs (Given/When/Then) directly as Coding Agent input — machine-verifiable requirements beat natural language descriptions.
2. **Small Commits + Continuous Verification**: Require agents to make small PRs with accompanying tests; avoid large single changes that are hard to review.
3. **Boundaries and No-Go Zones**: Agents must not directly connect to production databases, alter authentication/permission models without authorization, or delete migration scripts (aligned with HITL high-risk checklist).
4. **On-Premise / Air-Gapped Stacks**: In data-sensitive or compliance-constrained environments, prioritize privately deployable Coding Agents (e.g., local coding models based on Qwen/DeepSeek + enterprise codebase RAG). For China-subsidiary use cases, evaluate domestic alternatives (Tongyi Lingma, Zhipu CodeGeeX) against global tools (GitHub Copilot, Cursor).
