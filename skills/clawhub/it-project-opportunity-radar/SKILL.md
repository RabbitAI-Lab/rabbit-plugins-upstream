---
name: it-project-opportunity-radar
description: IT信息化商机雷达。当用户想挖掘IT行业早期商机（信创/国产化/软件开发/系统集成/云/数据中心/网络安全/智慧城市等）时，必须使用此SKILL：数字化拟建项目查询（智慧城市/数字政府立项阶段，提前6-18个月）、IT采购意向（发标前1-3个月）、运维与软件服务临期续约（IT服务续约窗口价值高）、按预算与紧急度排序、甲方单位跟进建议。给一个技术领域/地区即输出按价值排序的商机清单。即使用户没有提到「IT」，只要涉及信息化项目线索、信创商机、数字化项目早期发现等需求，都应使用本SKILL。
metadata: { "openclaw": {"requires": {"env":["ZLBX_API_KEY"]},"primaryEnv": "ZLBX_API_KEY"}}
---

# IT 信息化商机雷达 · 信创与数字化项目早期发现

面向**信创、软件开发、系统集成、云与网络安全**的早期商机：智慧城市/数字政府类拟建项目提前捕获、IT 采购意向提前对接、运维与服务合同临期续约锁定（IT 服务续约率高、窗口价值大）。

## 常见使用场景

- 「帮我找找山东的智慧城市和数字政府早期商机」
- 「最近有哪些单位发布了信创改造的采购意向？」
- 「软件运维服务合同快到期的项目帮我列一下，看看谁在做」
...
| 路 | 工具 | 提前量 | 商机含义 |
|----|------|--------|----------|
| ① 拟建项目 | search_proposed_projects | 最早（审批期，6-18 个月） | 项目还在发改立项阶段（如「XX市城市大脑二期建设项目」这类），抢跑窗口最大 |
| ② 采购意向 | search_bids（bid_process=[1]） | 次早（发标前 1-3 个月） | 大数据局等单位公开的采购计划，发标前即可上门 |
