---
name: skill-vitals
homepage: https://github.com/gold3bear/skill-vitals
description: 体检本机安装的 Agent Skills —— 查出为什么某个 skill 明明装了却不触发（描述预算静默溢出、跨层级被覆盖、语义重叠导致误选），统计上下文占用，找出从不触发的僵尸 skill，并做供应链安全扫描（提示注入、可疑脚本、明文密钥）。只要用户提到 skill 太多、装了一堆不知道哪些在用、上下文被挤占、某个 skill 该触发却没触发、想清理或精简技能库、想知道哪些 skill 真的在被触发，就使用本技能，即使他们没有说"体检"或"审计"这两个词。也适用于用户问"我装了多少个 skill""哪些 skill 可以删""为什么我的 skill 没生效""这个 skill 安全吗""skill 昨天还好用今天不见了"。
---
...
```bash
python3 scripts/scan.py --json /tmp/skill-scan.json
```

脚本会尝试常见宿主目录（Claude Code、Codex、OpenClaw、WorkBuddy、cc-switch、Cursor、Gemini CLI、OpenCode 及项目本地）。

> **宿主能力差异，报告里要先讲清。** 结构、上下文成本、语义重叠、安全这四项对任何宿主都有效；但**描述预算（2.1）、加载判定（`enabledPlugins`）、触发数据（2.5）三项依赖 Claude Code 的 `~/.claude.json`**。在其他宿主上这三项会缺失或降级——如实标注为「本次未获取」，不要用别的字段凑数。
...
- **未启用的插件副本** —— 脚本读 `~/.claude.json` 的 `enabledPlugins` 判断。同一个插件往往在 `plugins/cache/` 和 `plugins/marketplaces/` 各有一份，都不算数
...
> 早期版本走的是 `~/.claude/projects/**/*.jsonl`（动辄上百个文件、上百 MB，且只能靠正则猜）。那条路已废弃，`scripts/probe_logs.py` 仅在宿主**没有** `skillUsage` 时才有用（非 Claude Code 宿主、或极旧版本）。默认流程不要调用它。
...
- 触发数据来自 `~/.claude.json` → `skillUsage`，是**终身累计**而非近 30 天；非 Claude Code 宿主多数不提供
