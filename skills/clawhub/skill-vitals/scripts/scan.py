...
LEVEL_RANK = {"workspace": 0, "project": 1, "personal": 2, "managed": 3,
              "enterprise": 0, "plugin": 3, "unknown": 9}
    ...
    for plugin in data.get("plugins", []):
        ...
        if candidate.is_dir():
            roots.append((str(candidate), {
                "root_kind": ("builtin-skill-cache" if candidate == cached
...
def main():
    ...
    if os.environ.get("SKILL_VITALS_DISABLE_WORKBUDDY_BUILTINS") != "1":
        ...
        for p, meta in read_workbuddy_builtin_roots():
    ...
    for host in ("claude-code", "codex", "openclaw", "hermes", "workbuddy"):
        ...
        for s in summary_scope:
            host_groups.setdefault((s["instance_id"] or "", s["namespace"] or "",
                                    s["name"]), []).append(s)
        host_unique = [sorted(group, key=lambda s: LEVEL_RANK.get(s["level"], 9))[0]
                       for group in host_groups.values()]
        host_summaries[host] = {
            "skills_on_disk": len(host_skills),
            "discoverable_skills": len(host_discoverable),
            "runtime_verified_loaded_skills": sum(1 for s in host_skills
                                                  if s["runtime_verified"] and s["loaded"]),
            "runtime_verified_unique_skills": sum(
                len(r.get("skills", [])) for r in openclaw_runtime
                if r.get("available")) if host == "openclaw" else
                len({(s["instance_id"], s["namespace"] or "", s["name"])
                     for s in host_loaded if s["runtime_verified"]}),
    ...
    out = {
        ...
        "workbuddy_discovery": {
            ...
            "excluded_roots": ["~/.workbuddy/connectors-marketplace (catalog only)",
                               "builtin-plugin internal skills (not top-level installed Skills)"],
            "welcome_mode": read_workbuddy_welcome_mode(),
            "top_level_manifest_packages": len(read_workbuddy_builtin_roots()),
