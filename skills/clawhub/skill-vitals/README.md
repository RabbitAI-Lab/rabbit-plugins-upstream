...
The scanner auto-detects Claude Code, Codex, OpenClaw, WorkBuddy, cc-switch,
Cursor, Gemini CLI, OpenCode, and project-local `.claude/skills`. But the checks
are not equally available everywhere:

| Check | Any host | Claude Code only |
|---|---|---|
| Context cost, structure, semantic overlap, security | ✅ | |
| **Description budget** (2.1) | | ✅ needs `~/.claude.json` |
| **Loaded-vs-disk determination** (`enabledPlugins`) | | ✅ |
| **Trigger data** (2.5, `skillUsage`) | | ✅ |

On other hosts those three degrade to "not available" and are reported as such —
the tool will not substitute install time or mtime to fake the missing column.
...
Python 3.8+, standard library only. No dependencies, no network calls.
...
```bash
python3 scripts/scan.py --json scan.json
```
...
扫描器会自动识别 Claude Code、Codex、OpenClaw、WorkBuddy、cc-switch、Cursor、
Gemini CLI、OpenCode，以及项目本地 `.claude/skills`。但**七项检查并非在所有宿主上
都可用**：

| 检查项 | 任意宿主 | 仅 Claude Code |
|---|---|---|
| 上下文成本、结构、语义重叠、安全 | ✅ | |
| **描述预算**（2.1） | | ✅ 需要 `~/.claude.json` |
| **加载 vs 磁盘判定**（`enabledPlugins`） | | ✅ |
| **触发数据**（2.5，`skillUsage`） | | ✅ |

在其他宿主上，这三项会降级为「本次未获取」并如实标注——**工具不会用安装时间或
