#!/usr/bin/env python3
"""
TianAPI Company Info Fetcher
----------------------------
查询企业工商注册全量信息，支持通过企业名称或统一社会信用代码进行检索。
零外部依赖，仅使用 Python 标准库。

Usage:
    # 查询指定企业的工商信息
    python3 fetch_company-info.py --key YOUR_KEY --com 深圳市腾讯计算机系统有限公司

Env Config:
    export TIANAPI_COMPANY_KEY=your_api_key
    # 或在脚本同级目录创建 .env 文件: TIANAPI_COMPANY_KEY=your_api_key
"""
...
# --- 配置常量 ---
API_ENDPOINT = "https://apis.tianapi.com/companyinfo/index"
ENV_KEY_NAME = "TIANAPI_COMPANY_KEY"
DEFAULT_TIMEOUT = 10
...
class Config:
    ...
    def _exit_with_help(self, msg: str):
        ...
        print(f"💡 提示: 申请 Key -> https://www.tianapi.com/apiview/272", file=sys.stderr)
        sys.exit(1)
...
class TianAPIClient:
    ...
    def fetch_company(self, com: str) -> Dict[str, Any]:
        """
        执行 API 请求
...
def main():
    parser = argparse.ArgumentParser(description="TianAPI Company Info Fetcher")
    parser.add_argument("--key", help="API Key (可选，优先使用环境变量)")
    parser.add_argument("--com", required=True, help="企业名称或统一社会信用代码 (如: 深圳市腾讯计算机系统有限公司)")
    ...
    # 2. 请求
    result = client.fetch_company(com=args.com)
