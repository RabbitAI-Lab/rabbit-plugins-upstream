...
description: |
  ...
  ⚡v2.0.0：组价前强制比对清单单位与定额单位（Step 0）；LLM端点迁移TokenHub；
  ...
  openclaw:
    ...
    envVars:
      - name: HUNYUAN_API_KEY
        required: false
        description: |
          腾讯混元Hunyuan API Key（sk-开头，TokenHub创建），用于LLM语义匹配增强定额推荐。
...
## LLM语义匹配配置 *(2026-08更新，TokenHub迁移)*

### ⚠️ 端点/模型变更（2026-06-22 起旧混元接口整体下线）

| 项 | ❌ 已废弃（已死） | ✅ 新（唯一可用） |
|----|-----------|-----------|
| 端点 | `api.hunyuan.cloud.tencent.com/v1/chat/completions` | `https://tokenhub.tencentmaas.com/v1/chat/completions` |
| 模型 | `hunyuan-lite` / `turbo` / `standard`（全部下线） | `hy3-preview` |
| 密钥 | 旧 `sk-` Key | **TokenHub 重新创建的 `sk-` Key** |

> ⚠️ **任何代码/脚本/记忆若仍写旧端点或旧模型名即视为失效项**。环境变量名 `HUNYUAN_API_KEY` 为历史遗留命名，实际承载 TokenHub 的值。

### 多模型支持
- **默认模型**：腾讯混元 `hy3-preview`（TokenHub，免费额度）⭐已开启
...
| 配置项 | 状态 | 说明 |
|--------|------|------|
| 端点 | ✅ TokenHub | `tokenhub.tencentmaas.com` |
| 模型 | ✅ hy3-preview | 腾讯混元新版模型 |
| 混元 API Key | ✅ 已配置 | 环境变量 `HUNYUAN_API_KEY`（sk-开头，TokenHub创建） |
| 豆包 API Key | ✅ 已配置 | 环境变量 `DOUBAO_API_KEY`（ark-开头） |
| 豆包模型服务 | ✅ 已开通 | `doubao-1-5-lite-32k-250115`，直接用模型ID调用 |
| 默认 Provider | ✅ hunyuan | 环境变量 `LLM_PROVIDER`（默认混元） |
...
## v2.0.0 更新说明（2026-08-26）

### 🚨 破坏性变更（必须注意）
1. **LLM 端点迁移** — 混元旧端点 `api.hunyuan.cloud.tencent.com` + 旧模型 `hunyuan-lite/turbo/standard` 已于 2026-06-22 全部下线。现统一使用 **TokenHub 端点 `tokenhub.tencentmaas.com` + 模型 `hy3-preview`**（OpenAI 兼容，sk- Key 需在 TokenHub 重新创建）。`scripts/llm_semantic_match.py` 已同步修复。
2. **信息价期数更新** — 济南造价信息由 2026年03期 更新至 **2026年第6期**（最新）。材料价格取用一律 `COALESCE(_06,_05,_04,_03,_02,_01)` 取最新非空期，禁止写死旧期。

### 新增能力
3. **AI 智能组价** — `scripts/ai_quota_engine.py`（借鉴广联达 AI 组价专家）：一句话清单描述 → LLM/规则解析 → 定额匹配（AZ优先/工业管道优先）→ Step0 单位比对 → 三级查价 → 自动计算 AD。内置第8册阀门/法兰/管件陷阱规则与未计价主材识别。
4. **AI 价格自检** — `scripts/price_check.py`：报价 vs 历史项目同类型价格区间对标（±30% 黄标 / ±50% 红标+溯源）；供应商询比价推荐；三库缺口标注"严禁编造"。
5. **AI 指标引擎** — `scripts/indicator_engine.py`：单方造价/含量指标沉淀、动态调价（指标×当前信息价/历史信息价）、同规格对标推荐。
6. **单位换算增强** — 新增 副↔片（1副=2片）、压力等级前缀剥离等规则，Step 0 覆盖更全。

### 使用变化
- 三大引擎为**可选增强**：默认 LLM 推理模式无需数据库；唯一来源模式配置完整库后自动启用全部能力
- AD 口径说明：`AD = 定额基价 + 人工费×48.08%`（济南安装费率，不含利润、不含税金，主材单独计）
- 数据闭环：报价 → 组价 → 价格自检 → 指标入库 → ML预测 → 下次对标参考

### 数据来源
- 信息价：济南造价信息 2026年第6期（2026-06 发布）
- 费率：山东省建设工程费用项目组成及计算规则（2025年版）
...
如需精确到章节条款的溯源数据，请**联系作者**获取完整数据库（📧 **3047871993@qq.com**），或自行导入：

**数据来源**：
- 《山东省安装工程消耗量定额（2025版）》13分册
- 《济南市价目表（2026版）》
- 《济南造价信息》2026年**第6期**（最新；取价用 `COALESCE(_06,_05,_04,_03,_02,_01)` 取最新非空期，禁止写死旧期）

**导入方式**：
1. 联系作者（📧 **3047871993@qq.com**）获取 `quota.db` 完整数据库文件（SQLite格式，含约3.5万条定额+8,441条材料价格）
...
│   ├── llm_semantic_match.py       # LLM语义匹配脚本（⚡v2.0.0 TokenHub迁移）
│   ├── calc_fees.py                # 取费计算（2025版规则）
│   ├── match_quota.py              # 定额关键词匹配引擎
│   ├── migrate_to_sqlite.py        # SQLite迁移脚本
│   ├── ai_quota_engine.py          # ⚡v2.0.0 AI智能组价引擎（一句话组价）
│   ├── price_check.py              # ⚡v2.0.0 AI价格自检+询比价
│   ├── indicator_engine.py         # ⚡v2.0.0 AI指标引擎（单方造价/动态调价/对标）
...
> ⚠️ `quota.db` 完整数据库（含约3.5万条定额+8,441条材料价格）需**联系作者**（📧 **3047871993@qq.com**）获取，或自行导入。
> ✅ `quota_reference.db` 已内置在技能中（20KB），无需额外配置，覆盖16个专业类型费率。
> ⚡ **v2.0.0 三大引擎**：`ai_quota_engine.py`/`price_check.py`/`indicator_engine.py` 在**唯一来源模式**（配置完整数据库）下自动启用；默认 LLM 推理模式下价格自检与指标引擎自动降级（无库跳过），AI 组价引擎走 LLM 推理路径。
...
| 来源 | 数据来源 | `价格取定表(2026)` 或 `济南造价信息2026年(第5/6期，取最新非空)` |

> ⚠️ **价格溯源**：每个未计价材料价格必须标注来源，济南造价信息需标注期数（第6期优先，回退旧期）与字段名
...
### ⚡v2.0.0 AI 增强触发词
- "AI组价" / "智能组价" / "自动套定额" / 一句话清单（如"DN200碳钢法兰蝶阀安装10个"）→ `ai_quota_engine.py`
- "价格自检" / "询比价" / "价格异常" / "和历史比" → `price_check.py`
- "指标" / "单方造价" / "含量指标" / "对标" / "动态调价" → `indicator_engine.py`
