...
export async function cmdDoctor() {
    ...
    const envNote = apiEnv === 'custom' ? 'using a custom base from SIOBAC_API_BASE/OVOCLAW_API_BASE — NOT the default production server'
        : undefined;
    try {
...
export async function cmdSetup(_flags) {
    ...
    if ((!auth || !auth.agentId) && (process.env.SIOBAC_TOKEN || process.env.OVOCLAW_TOKEN)) {
        try {
            auth = (await requireBoundAgent()).auth;
        }
        catch { /* still not authed → fall through */ }
    }
