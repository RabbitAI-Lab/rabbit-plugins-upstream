...
export async function claimLoginLock(ttlMs = 60_000): Promise<boolean> {
  const f = loginLockFile()
  await fs.mkdir(dirname(f), { recursive: true, mode: 0o700 })
  try {
    const fh = await fs.open(f, 'wx', 0o600)   // O_EXCL — fails if it already exists
    try { await fh.writeFile(String(Date.now())) } finally { await fh.close() }
    return true
  } catch (e) {
...
export async function releaseLoginLock(): Promise<void> {
  try { await fs.unlink(loginLockFile()) } catch (e) {
    if ((e as NodeJS.ErrnoException).code !== 'ENOENT') throw e
  }
}
