...
[[ -n "$PROMPT" && -n "$MODEL" && -n "$RATIO" && -n "$SIZE" ]] || {
  echo "Usage: video_create.sh --prompt TEXT --model ID --ratio ID --size ID [--length N] [--attaches JSON]" >&2; exit 1; }
...
b={"create_mode":1,"prompt":prompt,"model":model,"ratio":ratio,"size":size,"platform":1,"terminal":4,"language":"en"}
ln=length.strip()
