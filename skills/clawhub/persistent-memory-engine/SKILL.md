...
**处理**: 按照skill规范执行无限分层结构化存储操作,遵循单一意图原则。
### 2. 三层索引体系
...
**处理**: 按照skill规范执行三层索引体系操作,遵循单一意图原则。
...
小规模直接 grep，大规模走索引导航，超大规模建议接入向量数据库做语义回退。系统按当前分类文件数自动推荐最优策略。

- 执行`混合检索策略`操作，处理输入数据并返回结果
- 验证执行结果，确认输出符合预期格式
- 参考`混合检索策略`相关配置参数进行设置
### 4. 记忆生命周期管理
...
本skill还覆盖以下能力场景: 无限分层持久记忆、解决跨会话遗忘、检索不准、记忆膨胀与冲突问、Agent、的无限分层持久记、忆系统、在内置记忆之上构、建并行、可扩展、可检索的结构化本、地存储、直击跨会话遗忘、新旧冲突四大痛点、核心能力涵盖无限、分类自动分裂、回收站恢复机制、完全位于、与内置、记忆并行运作、永不修改内置、适用于长周期项目、人脉网络管理、决策归档、领域知识库构建、多项目并行记忆等。这些能力在上述核心功能中均有对应处理逻辑。

**输出**: 返回冲突检测与版本化的执行结果,包含操作状态和输出数据。
...
- `-stack-decision`: 命令参数,用于指定操作选项
- `-preferences`: 命令参数,用于指定操作选项
