...
- It does **not** delete shots or profiles — destructive operations (`DELETE /api/shots/*`, `DELETE /api/profile-select/*`) are intentionally unsupported; leave those to the machine's Web UI.
- It does **not** trigger firmware/OTA updates — the `/api/firmware/*` endpoints are out of scope by design.
...
- full profile definition / profile export / 某条曲线的完整定义 / 曲线配置详情  
  → `scripts/gaggiuino.sh profiles` first to resolve id, then `scripts/gaggiuino.sh get-profile <id>`
- maintenance history / last descale / backflush / 维护记录 / 上次除垢 / 反冲历史 / 除垢后多少杯  
  → `scripts/gaggiuino.sh maintenance`
- connectivity probe / machine health check / 连通性诊断 / 机器健康检查  
  → `scripts/gaggiuino.sh health` (mostly for diagnosing connection issues)
...
- In this local context, **超萃** usually refers to the local `HyperEx` / `HyperEx 2.0` profiles, while `HyperEx 2.0` is a more soup-like profile.
- When users say **超萃** here, default to checking the exact profile name in profile description first.
...
All visual assets generated by the skill are captured in a defined standard directory with automatic naming under `~/.openclaw/workspace/gaggiuino-output`:
...
> [!TIP]
> On this host, please prepend `PYTHONNOUSERSITE=1` when manually running Python rendering scripts to avoid NumPy 2.x ABI conflicts (e.g. `PYTHONNOUSERSITE=1 python3 ...`).

##### Graph Renderer
`scripts/render_shot_graph.py`
```bash
# Generate static PNG (shot<id>_static.png)
python3 scripts/render_shot_graph.py --shot-id <id> --mode png

# Generate animated MP4 (shot<id>_animated.mp4)
python3 scripts/render_shot_graph.py --shot-id <id> --mode mp4
```
...
- If `--offset` is omitted, the renderer automatically detects the earliest credible **machine-start onset** from the video audio. **Note:** If the video has no audio track, or synchronization fails due to heavy background noise, please fallback to manual `--offset` alignment.
...
python3 scripts/render_shot_video_overlay.py --shot-id <id> --video landscape.mp4 --offset 1.4

# Auto sync from machine-start onset
python3 scripts/render_shot_video_overlay.py --shot-id <id> --video landscape.mp4
```
...
python3 scripts/render_shot_video_overlay.py --shot-id <id> --video portrait.mp4 --alpha 0.7 --position bottom
```
...
- if the image is clearly from Gaggiuino or includes Gaggiuino temperature data, also apply the feed-forward / Brew Delta temperature rules from [references/analysis-protocol.md](references/analysis-protocol.md)
- if the user supplies a named profile, profile description, or phase details, also consult [references/profile-mapping.md](references/profile-mapping.md) and [references/profile-descriptions.md](references/profile-descriptions.md) when an entry exists
...
3. **Upgrade immediately if the user supplies profile context**
   ...
   - if the named profile is not described in [references/profile-descriptions.md](references/profile-descriptions.md), use [references/profile-mapping.md](references/profile-mapping.md) plus any embedded or user-supplied phase structure, and say that profile-specific description support is limited
...
- read [references/profile-descriptions.md](references/profile-descriptions.md) only when a named profile has an entry there; if no entry exists, do not invent profile-specific behavior beyond the mapping and any available phase structure
...
- Gaggiuino active-shot temperature control is predictive feed-forward / Brew Delta, not a traditional reactive PID; do **not** diagnose normal intra-shot recorded temperature spikes as PID overshoot or PID tuning failure
