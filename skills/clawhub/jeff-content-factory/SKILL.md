...
- Review the draft for:
  ...
  - **精炼检查** ⚠️ MANDATORY:
    ...
    - 确保每个段落都有新信息，不是前文的重复
    - 删除所有不影响理解的冗余句子
  - **去AI味检查** ⚠️ MANDATORY（全文通过8项检查）：
    - 有没有「值得注意的是」「总的来说」「此外」等套话？→ 直接删
...
| **去AI味** | 15 | 全文通过8项去AI味检查，无套话，无整齐三段式，有强烈个人观点 |
| **语言质量** | 0 | （已合并到精炼度和去AI味中，不再单独计分） |
...
**⚠️ 去AI味8项检查清单（Quality Review 强制执行）：**
```
...
□ 删掉最后一段，文章更好？→ 直接删
```
> 💡 **快速检验法**：直接删掉最后一个总结段，如果文章反而更有力，说明原结尾是AI味的废话。
...
- Jeff 有修改意见 → 根据意见修改后重新写入飞书，循环确认

**⚠️ 绝对禁止：Jeff 确认选题后直接生成4种格式和发布！必须先走飞书母版确认流程！**
...
**Format 2: HTML (.html) - 完整专业版（公众号发布用）**
- Save to `/root/.openclaw/workspace/output/YYYY-MM-DD-[article-title-slug].html`
- **🚨 CRITICAL - 微信排版规则**：所有样式必须用内联 `style=""`，禁止 `<style>` 块和 CSS class！微信会强制剥离所有 class 和 style 块，导致样式全部丢失。详见下方「微信公众号排版规范」章节。
...
- **内联样式示例**：
  ...
  - CTA结尾：`<p style="margin:0;padding:20px 15px;text-align:center;color:#333;line-height:1.8;font-size:15px;"><strong>如果你觉得文章对你有所帮助，请关注公众号。</strong></p>`
...
**规格：** 900x386 像素（21:9），PNG 格式

**使用方式：**
1. 直接复制固定封面图片到输出目录，或上传到微信公众号获取 media_id
2. 如需压缩：
   ```bash
...
2. **Generate WeChat cover photo** using Seedream 5.0 API (see section 8)
