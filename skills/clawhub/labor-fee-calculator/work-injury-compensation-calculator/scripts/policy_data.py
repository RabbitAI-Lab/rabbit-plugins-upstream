...
RESOURCE_TYPE = "work-injury-compensation-calculator"
DEFAULT_POLICY_API_URL = "https://platform.delilegal.com/api/v1/skill/calculator/resource"
PLACEHOLDER_VALS = {"YOUR_API_KEY", "YOUR_APP_KEY", "Bearer YOUR_API_KEY", "", None}
...
def policy_api_url() -> str:
    return os.getenv("WORK_INJURY_POLICY_API_URL", DEFAULT_POLICY_API_URL).strip()
...
def int_year(raw: Any, field_name: str) -> int:
    try:
        year = int(raw)
    except Exception as exc:
        raise PolicyDataError(f"{field_name} 必须是有效年份") from exc
    if year < 1900 or year > 2100:
        raise PolicyDataError(f"{field_name} 超出支持范围：1900-2100")
    return year


def request_payload_from_input(payload: dict[str, Any]) -> dict[str, Any]:
    action = str(payload.get("action") or "").strip()
    region = str(first_payload_value(payload, REGION_KEYS) or "全国").strip()
    ...
    social_avg_year = payload.get("social_avg_year")
    if social_avg_year:
        request_payload["social_avg_year"] = int_year(social_avg_year, "social_avg_year")
    elif payload.get("year") and action not in {"work_death_subsidy_lookup", "death_benefits"}:
        request_payload["social_avg_year"] = int_year(payload["year"], "year")

    if payload.get("work_death_data_year"):
        request_payload["work_death_data_year"] = int_year(payload["work_death_data_year"], "work_death_data_year")
    elif payload.get("death_year"):
        request_payload["work_death_data_year"] = int_year(payload["death_year"], "death_year") - 1
    elif payload.get("year") and action in {"work_death_subsidy_lookup", "death_benefits"}:
        request_payload["work_death_data_year"] = int_year(payload["year"], "year") - 1
...
def normalized_region_for_cache(region: Any) -> str:
    ...
    if not text:
        return "全国"
    if text in {"全国", "国家", "national", "National"}:
        return "全国"
    if "深圳" in text:
        return "深圳"
    normalized = text
...
def cache_path_for_request(request_payload: dict[str, Any]) -> Path:
    ...
    work_death_year = safe_cache_part(request_payload.get("work_death_data_year") or CACHE_YEAR_IF_MISSING)
    return POLICY_CACHE_DIR / f"{region}__{query_date}__{social_avg_year}__{wage_caliber}__death_{work_death_year}.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(json.dumps(fail("api_snapshot", str(exc)), ensure_ascii=False, indent=2), file=sys.stderr)
        return 1
