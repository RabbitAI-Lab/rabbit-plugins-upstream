...
RESOURCE_TYPE = "overtime-pay-calculator"
DEFAULT_POLICY_API_URL = "https://platform.delilegal.com/api/v1/skill/calculator/resource"
PLACEHOLDER_VALS = {"YOUR_API_KEY", "Bearer YOUR_API_KEY", "", None}
...
REQUIRED_POLICY_FIELDS = (
    "min_wage",
    "worktime_params",
    "local_wage_base_rule",
    "statutory_holiday_rules",
    "law_basis",
    "law_articles_text",
)
...
def policy_api_url() -> str:
    return os.getenv("OVERTIME_POLICY_API_URL", DEFAULT_POLICY_API_URL).strip()
...
def request_payload_from_input(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not region:
        raise PolicyDataError("缺少 region，无法请求加班工资政策数据 API")

    request_payload: dict[str, Any] = {"region": region}
...
def cache_path_for_request(request_payload: dict[str, Any]) -> Path:
    ...
    return POLICY_CACHE_DIR / f"{region}__{query_date}.json"
...
def dispatch(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not has_direct_policy_data and not first_payload_value(payload, REGION_KEYS):
        return fail(action, "缺少 region，无法请求加班工资政策数据 API")
