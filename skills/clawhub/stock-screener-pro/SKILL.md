...
日常行情、选股、回测、组合记录、AI4Trade 与多智能体研究的可复制示例见 [references/usage-examples.md](references/usage-examples.md)。使用涉及外部 AI、AI4Trade 状态变更或本地记录写入的示例前，先确认对应影响范围。
...
- `screen_stocks`：五种策略：`bull_momentum`、`low_valuation_value`、`volume_breakout`、`growth_tech`、`oversold_rebound`。
- `ask_stock`：单股实时行情、均线、RSI、支撑阻力。
- `backtest_strategy`：个股轻量历史回测，策略为 `sma_cross`、`momentum_breakout`、`rsi_mean_reversion`。
