...
4. 如需 QuantaAlpha，先调用 `run_quantaalpha_research(action="status")`。正式挖掘要求 `runtime.ready_for_full_mining=true`；官方因子回测要求 `runtime.ready_for_backtest=true`。
...
- `run_tencent_factor_research`：不依赖 Tushare 的价量因子横评。支持 `momentum_20`、`reversal_5`、`low_volatility_20`、`volume_ratio_20`、`ma_gap_20`；从显式提供的 20–120 只 A 股股票池下载腾讯复权日线，按训练期确定方向，再报告独立样本外期的 Rank IC、最高分组收益、分组价差、回撤与随机对照。它不使用 LLM，不接券商。
