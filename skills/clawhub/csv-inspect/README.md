...
Then run the executable from the installed skill directory:

```bash
scripts/csv-inspect data.csv
```

### From this folder

```bash
scripts/csv-inspect /path/to/file.csv --head 10
```

## Usage

The script is executable (`#!/usr/bin/env python3`). Invoke it as a shell
command — do not prefix with `python3`.

```bash
scripts/csv-inspect FILE
scripts/csv-inspect FILE --head 10 --tail 3
scripts/csv-inspect FILE --scan 500 --json
scripts/csv-inspect FILE --no-header
```
