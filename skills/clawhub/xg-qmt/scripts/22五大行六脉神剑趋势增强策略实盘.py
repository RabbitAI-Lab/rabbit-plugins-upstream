...
text={
	...
	'自定义股票池':"自定义股票池设置",
	"股票池设置":"持有限制10的股票池设置",
	"股票池":['601328.SH',"601939.SH",
		'601988.SH',"601288.SH",'601998.SH'],
	"股票池名称":['交通银行','建设银行',
		'中国银行','农业银行','工商银行']
}
class A():
			...
			for stock,amount,trader_type,maker,oder_id,name in zip(trader_log['证券代码'].tolist(),trader_log['未成交数量'].tolist(),
				...
				if trader_type==49:
					...
					if check_trader.shape[0]>0:
						...
						check_trader=check_trader[check_trader['撤单']=='是']
						if check_trader.shape[0]>0:
						...
						check_trader_maker_list=[]
					if maker not in check_trader_maker_list:
