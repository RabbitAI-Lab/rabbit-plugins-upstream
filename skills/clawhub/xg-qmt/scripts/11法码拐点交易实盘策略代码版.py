...
def init(c):
	...
	"是否开启策略隔离":"是",
	"策略运行周期秒":3,
	"记录文件路径":"C:/Users/lxg123456/Desktop/拐点记录.xlsx",
	"买入价格编码":4,
	"卖出价格编码":6,
	"买入拐点设置":"买入拐点设置下跌反弹买入*********",
	"是否开启拐点买入":"是",
	...
	"监控股票池说明":"持股/自定义",
	"监控股票池":"自定义",
	"股票池":"159509.SZ,518800.SH,160719.SZ",
	"股票池名称":"纳斯达克科技ETF,黄金ETF,黄金lof",
	"交易时间段":4,
	"交易开始时间":9,
	"交易结束时间":14,
	"是否参加集合竞价":"否",
...
def trade(c):
	...
	'''
	#第一次循环不启动交易函数
	is_buy=c.text['是否开启拐点买入']
	is_sell=c.text['是否开启拐点卖出']
	if is_buy=='是':
		try:
			run_buy_trade(c)
		except Exception as e:
			...
			for stock,name in zip(df['证券代码'],df['名称']):
				try:
					...
					if tick.shape[0]>0:
						...
						#最新值
						min_value=min(lastClose_list)
						#p1值
						p_1=base_price*(1-down_ratio/100)
						p_1=round(p_1,3)
						#目前涨跌幅
						last_zdf=((last_price-base_price)/base_price)*100
						last_zdf=round(last_zdf,2)
						#最低点涨跌幅
						last_min_zdf=((min_value-base_price)/base_price)*100
						last_min_zdf=round(last_min_zdf,2)
						#p2最低价
						p_2=min_value
						p_2=round(p_2,3)
						#反弹比例计算p3
						p_3=p_2*(1+ft_ratio/100)
						p_3=round(p_3,3)
						#目前反弹比例
						last_ft_ratio=((last_price-p_2)/p_2)*100
						last_ft_ratio=round(last_ft_ratio,2)
						#最新价小于P1,开始监控
						if last_price<=p_1:
							if p_3<p_1:
								if last_price>=p_3:
									print('触发拐点买入,时间:{},股票:{},名称:{},基准价:{},p1值:{},p2值:{},p3:值{},最新价:{},目前涨跌幅:{},最低点涨跌幅:{},目前反弹比例:{} '.format(str(datetime.now())[:-7:],stock,name,base_price,p_1,p_2,p_3,last_price,last_zdf,last_min_zdf,last_ft_ratio))
									if trader_type=='数量':
										amount=value
									elif trader_type=='金额':
										amount=value/price
									...
									if check_is_buy(c,c.account,c.account_type,stock,amount,price):
										maker='{},拐点买入,{},{},{},{}'.format(c.st_name,stock,'buy',amount,price)
										...
										print(stock,'买入失败可能没有资金')
								else:
									print('没有拐点买入,最新价小于P3,时间:{},股票:{},名称:{},基准价:{},p1值:{},p2值:{},p3:值{},最新价:{},目前涨跌幅:{},最低点涨跌幅:{},目前反弹比例:{} '.format(str(datetime.now())[:-7:],stock,name,base_price,p_1,p_2,p_3,last_price,last_zdf,last_min_zdf,last_ft_ratio))
							else:
								print('没有拐点买入,P3大于P1,时间:{},股票:{},名称:{},基准价:{},p1值:{},p2值:{},p3:值{},最新价:{},目前涨跌幅:{},最低点涨跌幅:{},目前反弹比例:{} '.format(str(datetime.now())[:-7:],stock,name,base_price,p_1,p_2,p_3,last_price,last_zdf,last_min_zdf,last_ft_ratio))
						else:
							print('没有拐点买入,最新价大于P1,时间:{},股票:{},名称:{},基准价:{},p1值:{},p2值:{},p3:值{},最新价:{},目前涨跌幅:{},最低点涨跌幅:{},目前反弹比例:{} '.format(str(datetime.now())[:-7:],stock,name,base_price,p_1,p_2,p_3,last_price,last_zdf,last_min_zdf,last_ft_ratio))
					else:
						print(str(datetime.now())[:-7],stock,'拐点买入：拐点触发后的时间没有数据,拐点触发交易时间大于15:00')
				except Exception as e:
					print(e,stock,'拐点买入有问题*********')
		else:
			print('拐点买入没有自定义交易股票池*********')
	else:
		print('{}拐点买入目前不是交易时间*********'.format(datetime.now()))
