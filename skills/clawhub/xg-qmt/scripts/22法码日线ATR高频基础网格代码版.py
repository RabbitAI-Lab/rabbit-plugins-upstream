...
def init(c):
	...
	"是否开启策略隔离":"是",
	"策略运行周期秒":3,
	"记录文件路径":"C:/Users/lxg123456/Desktop/日线ATR网格交易.xlsx",
	"买入价格编码":4,
	"卖出价格编码":6,
	"ATR网格设置":"ATR网格设置",
	"ATR周期":14,
...
def ATR(CLOSE,HIGH,LOW,N=14):
	...
	ATR=MA(MTR,N)
	return MTR,ATR
def cacal_atr(c,stock='513100.SH',N=14):
	'''
	计算atr
	'''
		...
		if df.shape[0]>0:
			for stock,name in zip(df['证券代码'],df['名称']):
				try:
					...
					#atr涨跌幅
					atr=cacal_atr(c,stock=stock,N=N)
					atr=round(atr*ratio,3)
					down_ratio=round(-1*(atr/last_price)*100,3)
					if last_zdf<=down_ratio:
						print('触发日线ATR买入网格{} 最新价{},基础价{},目前跌幅{}大于目标跌幅{},atr{},调整系数{}'.format(stock,last_price,base_price,last_zdf,down_ratio,atr,ratio))
						if trader_type=='数量':
							amount=value
						elif trader_type=='金额':
							amount=value/price
						...
						if  check_is_buy(c,c.account,c.account_type,stock=stock,amount=amount,price=price) and amount>=10:
							...
							print(stock,'日线ATR网格买入失败可能没有金额')
					else:
						print('没有触发日线ATR买入网格{} 最新价{},基础价{},目前跌幅{}小于目前跌幅{},atr{},调整系数{}'.format(stock,last_price,base_price,last_zdf,down_ratio,atr,ratio))
				except Exception as e:
					print(e,stock,'买入ATR网格有问题*************')
		else:
			print('日线网格买入没有自定义交易股票池*********')
	else:
		print('{}日线ATR网格买入目前不是交易时间*********'.format(datetime.now()))
		...
		if df.shape[0]>0:
			for stock,name in zip(df['证券代码'],df['名称']):
				try:
					...
					atr=cacal_atr(c,stock=stock,N=N)
					atr=round(atr*ratio,3)
					up_ratio=round(1*(atr/last_price)*100,3)
					#涨幅大于目前涨跌幅
					if last_zdf>=up_ratio:
						print('触发日线ATR网格卖出{} 最新价{},基准价{},涨跌幅{}大于目标涨跌幅{},atr{},调整系数{}'.format(stock,last_price,base_price,last_zdf,up_ratio,atr,ratio))
						if trader_type=='数量':
							amount=value
						elif trader_type=='金额':
							amount=value/price
						...
						if check_is_sell(c,c.account,c.account_type,stock=stock,amount=amount) and amount>=10:
							...
							print(stock,'ATR卖出失败可能没有持股')
					else:
						print('ATR没有触发日线网格卖出{} 最新价{},基准价{},涨跌幅{}小于目标涨跌幅{},atr{},调整系数{}'.format(stock,last_price,base_price,last_zdf,up_ratio,atr,ratio))
				except Exception as e:
					print(e,stock,'日线ATR网格卖出有问题*******************')
		else:
			print('ATR日线网格卖出没有自定义交易股票池*********')
	else:
		print('{}ATR日线网格卖出目前不是交易时间*********'.format(datetime.now()))
