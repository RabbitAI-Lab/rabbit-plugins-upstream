...
text={
	...
	"股票池设置":"持有限制10的股票池设置",
	"股票池":[
			"513100.SH", "159869.SZ", "512480.SH", "159915.SZ",
			"510300.SH", "159531.SZ", "159509.SZ", "159937.SZ",
			"159680.SZ", "513400.SH", "159351.SZ", "512010.SH", 
			"516160.SH", "511090.SH", "510880.SH", "512880.SH",
			"159866.SZ", "512800.SH", "159655.SZ", "511380.SH", 
			"512670.SH", "513030.SH", "513220.SH", "513090.SH",
			"511130.SH"],
	"股票池名称":[
			"纳斯达克ETF", "游戏ETF", "半导体ETF", "创业板ETF", 
			"300ETF", "中证2000", "纳斯达克科技", "黄金ETF", 
			"1000ETF", "道琼斯ETF", "A500ETF", "医药ETF", 
			"新能源ETF", "30年国债ETF", "红利ETF", "证券ETF", 
			"日本ETF", "银行ETF", "标普ETF", "可转债ETF", 
			"军工ETF", "德国ETF", "互联网ETF", "香港证券ETF", 
			"30年国债ETF"],
...
def init(c):
	...
	c.run_time("run_tarder_func","1nDay","2024-07-25 14:30:00")
	#10分钟不成交撤单
	c.run_time("run_order_trader_func","600nSecond","2024-07-25 13:20:00")
	#30分钟一次
			...
			for stock,amount,trader_type,maker,oder_id,name in zip(trader_log['证券代码'].tolist(),trader_log['未成交数量'].tolist(),
				...
				if trader_type==49:
					...
					if check_trader.shape[0]>0:
						check_trader['不成交']=check_trader['委托状态'].apply(lambda x: '是' if x in not_list else '不是')
						check_trader=check_trader[check_trader['不成交']=='是']
						if check_trader.shape[0]>0:
							check_trader_maker_list=check_trader['投资备注'].tolist()
						else:
							check_trader_maker_list=[]
					else:
						check_trader_maker_list=[]
					if maker in check_trader_maker_list:
						print(maker,'没有撤单成功不委托')
					else:
