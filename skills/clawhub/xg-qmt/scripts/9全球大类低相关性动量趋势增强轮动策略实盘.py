...
def init(c):
	...
	c.windows=text['动量周期']
	#最小的周期数量
	c.min_amount=text['最小的周期数量']
	#当前的周期
	c.last_amount=text['当前的周期']
	#5日均线
	c.mean_line=text['均线']
	#最低分
	c.min_score=text['分数']
	...
	if df.shape[0]>0:
		...
		for stock in df['证券代码'].tolist():
			try:
				...
				subscribe=True)
				hist=hist[stock]
				df1=six_pulse_excalibur_hist(c,hist)
				df1['均线']=df1['close'].rolling(c.mean_line).mean()
				df1['站上均线']=df1['close']>df1['均线']
				score=mean_line_models(c,close_list=df1['close'].tolist())
				df1=df1[-c.windows:]
				up=df1['站上均线'].tolist()[-1]
				total_amount=sum(df1['signal'].tolist())
				last_amount=df1['signal'].tolist()[-1]
				total_amount_list.append(total_amount)
				last_amount_list.append(last_amount)
				score_list.append(score)
				up_line_list.append(up)
			except Exception as e:
				print(e,stock,'有问题')
				total_amount=None
				last_amount=None
				...
				last_amount_list.append(last_amount)
				score_list.append(score)
				up_line_list.append(None)
			
		df['数量']=total_amount_list
		df['最新数量']=last_amount_list
		df['分数']=score_list
		df['站上均线']=up_line_list
		#选择数据
		df=df[df['数量']>=c.min_amount]
		df=df[df['最新数量']>=c.last_amount]
		df=df[df['分数']>=c.min_score]
		df=df[df['站上均线']==True]
		#安总数量排序
		df=df.sort_values(by=['数量','分数'], ascending=False)
		print('分析数据***********************')
		print(df)
		rank_stock_list=df['证券代码'][:c.rank].tolist()
		print('交易股票池***************')
		print(df[:c.rank])
		buy_list=[]
		sell_list=[]
		#买入
		for stock in rank_stock_list:
			if stock in hold_stock_list:
				print(stock,'买入分析在持有排名股票池继续持有')
			else:
				print(stock,'买入分析持有股票池排名没有在持股买入')
				buy_list.append(stock)
		#卖出
		for stock in hold_stock_list:
			if stock in rank_stock_list:
				print(stock,'卖出分析在持有排名股票池继续持有')
			else:
				print(stock,'卖出分析不在持有排名股票池卖出')
				sell_list.append(stock)
		buy_df=pd.DataFrame()
		buy_df['证券代码']=buy_list
		if buy_df.shape[0]>0:
			buy_df['名称']=buy_df['证券代码'].apply(lambda x:c.name_dict.get(x,x) )
		if len(hold_stock_list)>0:
			hold_stock['卖出']=hold_stock['证券代码'].apply(lambda x: '是' if x in sell_list else '不是')
...
def run_tarder_func(c):
	...
	if check_is_trader_date_1():
		#先卖在买入
		buy_df,sell_df=get_stock_analysis(c)
		print(sell_df)
