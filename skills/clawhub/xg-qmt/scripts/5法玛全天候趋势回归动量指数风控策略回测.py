...
def init(c):
	...
	c.line=10
	#指数风险控制
	c.index_stock='000001.SH'
	#指数风险线
	c.index_line=10
	c.stock_list=['518880.SH', '513100.SH', '159915.SZ']
	c.name_list=['黄金ETF', '纳指ETF', '创业板ETF']
	#指数风险生效的股票池,注意这个股票池,用上证指数去控制纳斯达克,黄金是不合理的,主要针对A股标的
	c.index_trader_stock=['159915.SZ']
	c.name_dict=dict(zip(c.stock_list,c.name_list))
	#设置股票池
	c.trader_stock=[]
	for stock in c.stock_list:
		c.trader_stock.append(stock)
	#添加指数
	c.trader_stock.append(c.index_stock)
	c.set_universe(c.trader_stock)  
	#下载个股数据
	for stock in c.trader_stock:
...
def handlebar(c):
	...
	index_df['close']=hist[c.index_stock]
	score_list=[]
	for stock in c.stock_list:
		df=pd.DataFrame()
		df['close']=hist[stock]
		score=MOM(c,date,df,index_df,stock)
