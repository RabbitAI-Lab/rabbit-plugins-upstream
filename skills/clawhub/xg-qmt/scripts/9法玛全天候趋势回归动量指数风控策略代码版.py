#encoding:gbk
'''
9法玛全天候趋势回归动量指数风控策略代码版,策略模板,只做学习研究
...
def init(c):
	...
	"是否参加集合竞价":"否",
	"开始交易分钟":0,
	"股票池":"518880.SH,513100.SH,159915.SZ",
...
def MOM(c,etf='513100.SH'):
	...
	n=c.text['动量天数']
	r_line=c.text['风险控制线']
	index_stock=c.text['指数代码']
	index_line=c.text['指数风控线']
	index_trader_stock=c.text['指数风控标的'].split(',')
	stock=etf
	...
	#指数数据
	index_df=c.get_market_data_ex([],stock_code=[index_stock], period = "1d",count =-1,
		start_time='20210101',
		end_time='20500101',
		dividend_type='front')
	index_df=index_df[index_stock]
...
def get_rank(c,etf_pool=["518880.SH","513100.SH",
	"159915.SZ","510180.SH"]):
	score_list = []
	min_value=c.text['最小动量']
	max_value=c.text['最大动量']
	for etf in etf_pool:
		score = MOM(c,etf)
