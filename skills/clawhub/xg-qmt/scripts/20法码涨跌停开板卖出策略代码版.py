...
def trade(c):
	...
	'''
	#第一次循环不启动交易函数
	is_up=c.text['是否开启涨停板开板卖出']
	is_down=c.text['是否开启跌停板开板卖出']
	if is_up=='是':
		try:
			run_up_limit_open_sell(c)
		except Exception as e:
			print(e,'涨停板开板卖出运行有问题*******')
	else:
		print(datetime.now(),'不开启涨停板开板卖出***************')
	if is_down=='是':
		try:
			run_down_limit_open_sell(c)
		except Exception as e:
			print(e,'跌停板开板卖出有问题*******')
	else:
		print(datetime.now(),'不开启跌停板开板卖出**************')
def check_open_pool_sell_func(c,stock='300332.SZ'):
	'''
	检查涨停板开板卖出函数
	'''
	...
	if hist.shape[0]>0:
		...
		if amount>0:
			...
			print(stock,'可能停牌没有交易数据')
			return False
	else:
		print('开板卖出目前没有数据不交易')
		return False
def run_up_limit_open_sell(c):
	'''
	运行涨停开板卖出函数
	'''
	...
	if check_is_trader_date_1(c):
		...
		if df.shape[0]>0:
			for stock,name in zip(df['证券代码'],df['名称']):
				try:
					print('***************{} {} 涨停板开板卖出分析**************'.format(stock,name))
					...
					if stats==True:
						...
						if check_is_sell(c,c.account,c.account_type,stock=stock,amount=amount) and amount>0:
							maker='{},涨停开板卖出,{},{},{},{}'.format(c.st_name,stock,'sell',amount,price)
							passorder(c.sell_code, 1101,c.account, stock, c.sell_price_code, 0, amount, maker,1,maker,c)
							print(maker)
						else:
							print(stock,'涨停开板卖出可能没有持股')
					else:
						print(stock,'涨停开板卖出没有触发')
				except Exception as e:
					print(stock,e,'涨停开板卖出有问题')
		else:
			print('涨停开板卖出没有股票池')
	else:
		print('涨停开板卖出目前不是交易时间')
def check_down_break_func(c,stock='000680.SZ'):
	'''
	检查跌停开板函数
	基于盘口买盘挂单量判断：买盘五档挂单量之和为0 视为跌停封板
	返回 True 表示跌停开板，False 表示未开板或从未跌停
	'''
	date = ''.join(str(datetime.now())[:10].split('-'))
	n = 10  # 观察的tick周期
	# 订阅并获取tick数据
	hist = c.get_market_data_ex([], stock_code=[stock], period="tick", count=-1,
			start_time=date, end_time=date, dividend_type='front')
	hist = hist[stock]
	if hist.shape[0] > 0:
		amount = hist['amount'].tolist()[-1]
		if amount > 0:
			# 处理时间，只保留交易时段（9:28-14:59）
			hist['date'] = hist.index.tolist()
			hist['date'] = hist['date'].apply(lambda x: int(str(x).split('.')[0][8:]))
			hist = hist[hist['date'] >= 92800]
			hist = hist[hist['date'] <= 145659]
			# 跌停封板标记：买盘挂单量为0
			hist['down_stats'] = hist['bidVol'].apply(lambda x: 1 if sum(x) == 0 else 0)
			# 取最后n个tick
			hist = hist[-n:]
			stats_list = hist['down_stats'].tolist()
			last_stats = stats_list[-1]
			cum_stats = sum(stats_list)
			# 判断跌停开板（逻辑与原涨停函数一致）
			if cum_stats == n:
				print(stock, '跌停一直封板未开')
				return False
			elif cum_stats != n and cum_stats >= 1 and last_stats == 0:
				print(stock, '跌停开板')
				return True
			elif cum_stats != n and cum_stats >= 1 and last_stats == 1:
				print(stock, '跌停开板后再次封板')
				return False
			else:
				print(stock, '未出现跌停')
				return False
		...
		print('跌停开板检测目前没有数据')
		return False
def run_down_limit_open_sell(c):
	'''
	运行跌停板开板卖出函数
	'''
	trader_type=c.text['交易模式']
	...
	if check_is_trader_date_1(c):
		...
		if df.shape[0]>0:
			for stock,name in zip(df['证券代码'],df['名称']):
				try:
					print('***************{} {} 跌停板开板卖出分析**************'.format(stock,name))
					stats=check_down_break_func(c,stock)
					if stats==True:
						if trader_type=='数量':
							amount=value
						elif trader_type=='金额':
						...
						if av_amount>=amount:
							...
							amount=av_amount
						if check_is_sell(c,c.account,c.account_type,stock=stock,amount=amount) and amount>0:
							maker='{},跌停开板卖出,{},{},{},{}'.format(c.st_name,stock,'sell',amount,price)
							passorder(c.sell_code, 1101,c.account, stock, c.sell_price_code, 0, amount, maker,1,maker,c)
							print(maker)
						else:
							print(stock,'跌停开板卖出可能没有持股')
					else:
						print(stock,'跌停开板卖出没有触发')
				except Exception as e:
					print(stock,e,'跌停开板卖出有问题')
		else:
			print('跌停开板卖出没有股票池')
	else:
		print('跌停开板卖出目前不是交易时间')
