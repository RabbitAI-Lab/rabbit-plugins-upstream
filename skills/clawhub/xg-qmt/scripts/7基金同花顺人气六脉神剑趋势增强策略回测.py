...
def init(c):
	...
	#开始时间
	c.start='20241101 00:00:00'
	#结束时间
	c.end='20500101 00:00:00'
	#测试资金
	c.capital=300000
	df=get_trader_stock(c)
	c.stock_list=df['证券代码'].tolist()
	c.name_list=df['名称'].tolist()
	#持有限制
	c.hold_limit=10
	#买入比率
	c.buy_ratio=0.1
...
def adjust_stock(stock='600031.SH'):
	...
	if stock[-2:]=='SH' or stock[-2:]=='SZ' or stock[-2:]=='sh' or stock[-2:]=='sz':
		...
		if stock[:3] in ['600','601','603','605','688','689',
			] or stock[:2] in ['11','51','58'] or str(stock)[0]=='5':
			stock=stock+'.SH'
		else:
			stock=stock+'.SZ'
	return stock
...
def get_trader_stock(c):
	...
	try:
		...
		df['标记1']=df['名称'].apply(lambda x: str(x)[:2])
		df['标记2']=df['名称'].apply(lambda x: str(x).split('ETF')[0][-2:])
