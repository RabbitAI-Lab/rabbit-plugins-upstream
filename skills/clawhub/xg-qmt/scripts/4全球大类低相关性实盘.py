...
text={
	...
	"固定交易金额":1000,
	"固定交易数量":100,
	"表格路径":"C:/Users/lxg123456/Desktop/code.xlsx",
	"特殊交易标的设置":"特殊交易标的设置",
...
def init(c):
	...
	if df.shape[0]>0:
		a.trade_code_list=df['证券代码'].tolist()
		a.trade_code_name=df['名称'].tolist()
	else:
		a.trade_code_list=[]
		a.trade_code_name=[]
	#定时
...
def cacal_select_stock_factor(c):
	...
	if df.shape[0]>0:
		...
		for stock in stock_list:
			print(stock,'买入因子计算完成***************')
			hist=c.get_market_data_ex([],stock_code=[stock], period = "1m",count =-1,
			start_time='20250101',
			end_time='20500101',
			dividend_type='front')
			tick=c.get_full_tick(stock_code=[stock])
			...
			for name in  factor_name:
				try:
					...
					hist1=resample_stock_data_func(hist1)
					tick1=tick[stock]
					models=user_def_factor(hist1,tick1)
