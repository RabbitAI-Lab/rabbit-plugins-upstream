...
text={
	...
	'策略名称':'全市场可转债势增强实盘策略6',
	"是否测试模型设置":"是否测试模型设置,实盘改成否",
	"是否测试模型":"否",
	"账户":"770",
	"账户类型":"STOCK",
	"可转债路径":'C:/Users/lxg123456/Desktop/QBKZZ.blk',
	"买入价格编码":5,
	"卖出价格编码":5,
	"是否隔离策略":"是",
	"买入黑名单":['600031.SH'],
...
def init(c):
	...
	if stock_df.shape[0]>0:
		a.trade_code_list=stock_df['证券代码'].tolist()
		a.trade_code_name=stock_df['名称'].tolist()
	else:
		a.trade_code_list=[]
		a.trade_code_name=[]
	...
	c.run_time("run_tarder_func","1nDay","2024-07-25 14:10:00")
	#30分钟一次
	...
	#1分钟补单,下单没有成功的重新提交,买入，卖出失败的重新检查委托就/、
	#c.run_time("run_trader_check_fun","600nSecond","2024-07-25 13:20:00")
	#账户个股止盈止损，安成本价计算
	...
	test=text['是否测试模型']
	if test=='是':
		print('开启测试模型*************实盘关闭，是否测试参数改成否***************')
		run_tarder_func(c)
	else:
		print('不开启测试模型*************开启实盘交易***************')
...
def get_del_qzsh_data(c):
	...
	models=xms_quant_bond_user_factor_trader(
		...
		date=date,
		redeem_n=redeem_n)
	df=models.get_del_qzsh_data()
	if df.shape[0]>0:
		df['转债代码']=df['转债代码'].apply(lambda x: adjust_stock(x))
		qzsh_list=df['转债代码'].tolist()
	else:
		qzsh_list=[]
	return qzsh_list
...
def get_trader_stock(c):
	...
	if df.shape[0]>0:
		...
		df['买入黑名单']=df['证券代码'].apply(lambda x: '是' if x in text['买入黑名单'] else '不是')
		df=df[df['买入黑名单']=='不是']
		qzsh_list=get_del_qzsh_data(c)
		df['强制赎回']=df['证券代码'].apply(lambda x: '是' if x in qzsh_list else '不是')
		df=df[df['强制赎回']=='不是']
		print('强制赎回的标的***************')
		print(qzsh_list)
	return df
		...
		for stock  in stock_list:
			try:
				...
				for name in  factor_name:
					models=user_def_factor(stock,hist,tick)
					try:
...
def get_buy_sell_stock_data(c):
	...
	if sell_df.shape[0]>0:
		sell_df=sell_df[['证券代码','持仓量','可用数量','投资备注']]
	qzsh_list=get_del_qzsh_data(c)
	if len(hold_stock_list)>0:
		hold_stock['强制赎回']=hold_stock['证券代码'].apply(lambda x:'是' if x in qzsh_list else '不是')
		hold_stock['投资备注']=c.st_name+','+'sell'+','+hold_stock['证券代码']
		qzsh_df=hold_stock[hold_stock['强制赎回']=='是']
		qzsh_df=qzsh_df[['证券代码','持仓量','可用数量','投资备注']]
	else:
		qzsh_df=pd.DataFrame()
	print('强制赎回卖出标的******************')
	print(qzsh_df)
	sell_df=pd.concat([sell_df,qzsh_df],ignore_index=True)
	if sell_df.shape[0]>0:
			...
			for stock,maker in zip(buy_df['证券代码'].tolist(),buy_df['投资备注'].tolist()):
				...
				if trader_models=='数量':
					...
					if stock  in sep_stock_list:
						...
						amount=adjust_amount(stock,amount)
				if maker not in a.log_id or maker not in trader_maker_list:
	...
	if check_is_trader_date_1():
		...
		if trader_log.shape[0]>0:
			...
			if trader_log.shape[0]>0:
				#委托没有成交
				not_list=[57]
				trader_log['废单']=trader_log['委托状态'].apply(lambda x: '是' if x in not_list else '不是')
				trader_log=trader_log[trader_log['废单']=='不是']
				if trader_log.shape[0]>0:
					trader_log=trader_log.drop_duplicates(subset=['投资备注'])
		...
		position=get_position(c,c.account,c.account_type)
		if position.shape[0]>0:
			av_amount_dict=dict(zip(position['证券代码'].tolist(),position['可用数量'].tolist()))
		else:
			av_amount_dict={}
		if len(a.log_id)>0:
			for maker in a.log_id:
				if maker not in maker_list:
					print(maker,'没有委托成功重新委托')
					#df['投资备注']=c.st_name+','+'buy'+','+df['证券代码']
					name,trader_type,stock=maker.split(',')[:3]
					if trader_type=='buy':
						maker_1=maker+',补单'
						price=get_price(c,stock)
						if trader_models=='数量':
						...
						if maker not in maker_list:
							if check_is_buy(c,c.account,c.account_type,stock=stock ,amount=amount,price=price) and amount>=10:
								...
								print(maker,'没有委托重新委托提交完成')
							else:
								print('{}金额交易买入不了*******'.format(stock))
						else:
							print(maker,'委托了没有成交不补单')
					...
					print(maker,'在委托下单记录不重新委托')
		else:
			print('临时委托id记录没有数据')
				
	else:
		print(datetime.now(),'补单目前不是交易时间')
...
def get_position(c,accountid,datatype):
	...
	if len(positions)>0:
		...
		try:
			...
			df=df[df['隔离策略']=='是']
			data=df
			data['卖出黑名单']=data['证券代码'].apply(lambda x: '是' if x in text['卖出黑名单'] else '不是')
...
class user_def_factor:
	...
	对于真和假的判断用1，0替代
	'''
	def __init__(self,stock,hist,spot):
		self.stock=stock
		self.hist=hist
		self.df=hist
		self.tick=spot
...
class xms_quant_bond_user_factor_trader:
	...
	def get_del_qzsh_data(self):
		...
		'''
		try:
			url='{}:{}/data/强制赎回/{}.json?t=1752251108452'.format(self.url,self.port,self.date)
			res=requests.get(url=url)
			res=res.json()
			df=pd.DataFrame(res)
			...
			print(e,'获取强制赎回数据表有问题')
			df=pd.DataFrame()
		df1=df[df['已公告强赎处理']=='是']
		df2=df[df['赎回需要时间']<=self.redeem_n]
