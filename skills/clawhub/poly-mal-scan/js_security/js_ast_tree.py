#-*- coding:utf-8 -*-
"""
js_ast_tree.py —— JS 语法树建立脚本(最初步,对称于 php_ast_tree.py)

用途:用 tree-sitter(javascript 语言)对 JS 源码建立语法树,并打印遍历结构,
验证解析器可用、观察 node 类型/文本/行列位置。
这是 JS 侧后续(AST → 行为模型 → 复用正则组件)的第一步,暂不做任何恶意判定。

依赖:tree_sitter_language_pack(mainwork3_12_13 环境已安装)
用法:
	<mainwork3_12_13 python> js_ast_tree.py <file.js> [--depth N] [--no-text]
运行示例:
	~/.pyenv/versions/3.12.13/envs/mainwork3_12_13/bin/python js_ast_tree.py <file.js>
"""
import sys
import tree_sitter_language_pack as ts_pack
JS_LANG='javascript'#tree_sitter_language_pack里JS的语言名为javascript
def build_tree(file_path):#读取文件并用tree-sitter解析,返回(bytes源码,tree)
	with open(file_path,'rb') as f:
		source=f.read()
	parser=ts_pack.get_parser(JS_LANG)
...
def main():
	args=sys.argv[1:]
	if not args:print('用法: <py> js_ast_tree.py <file.js> [--depth N] [--no-text]');sys.exit(1)
	file_path=args[0]
