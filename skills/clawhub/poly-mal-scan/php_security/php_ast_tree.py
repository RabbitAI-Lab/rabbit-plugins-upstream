#-*- coding:utf-8 -*-
"""
php_ast_tree.py —— PHP 语法树建立脚本(最初步)

用途:用 tree-sitter 对 PHP 源码建立语法树,并打印树的遍历结构,
验证解析器可用、观察 node 类型/文本/行列位置。
这是后续(AST → 行为模型 → 复用正则组件)的第一步,暂不做任何恶意判定。

依赖:tree_sitter_language_pack(pyenv 的 mainwork3_12_13 环境已安装)
用法:
	python php_ast_tree.py <file.php> [--depth N] [--no-text]
本文件由 mainwork3_12_13 的 python 运行:
	~/.pyenv/versions/3.12.13/envs/mainwork3_12_13/bin/python php_ast_tree.py <file.php>
"""
import sys
...
def main():
	args=sys.argv[1:]
	if not args:print('用法: python php_ast_tree.py <file.php> [--depth N] [--no-text]');sys.exit(1)
	file_path=args[0]
