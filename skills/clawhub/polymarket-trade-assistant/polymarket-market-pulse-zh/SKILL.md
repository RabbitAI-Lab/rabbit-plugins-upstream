...
metadata: {"openclaw": {"emoji": "📊", "requires": {"bins": ["python3", "node", "npx"]}}}
---
...
```bash
cd {baseDir}/../api-trade-polymarket/scripts && npx tsx scrape-market.ts --slug <event_slug> --sections context,rules
```
...
- 对每个市场执行 2-3 个定向网络搜索
- 记录每个信息源的 URL 和关键发现
- 优先级：官方数据 > 权威媒体 > 专家分析 > 社交情绪

**推理与预测（A2 模块）：**
- 从基准概率出发（历史频率或以市场价格作为先验）
- 根据每条证据进行更新（强证据：10-20% 偏移；中等：5-10%；弱：1-5%）
- 标注置信度：高 / 中 / 低
...
```bash
cd {baseDir}/../api-trade-polymarket/scripts && npx tsx scrape-market.ts --slug <event_slug> --sections comments --comment-limit 20 --comment-sort likes
```
...
6. `position_pct = position / bankroll`（实际分配的资金比例）

显示：完整凯利 %、1/4 凯利 %、凯利金额（% → $）、流动性上限（% → $）、**建议仓位以资金百分比为主**（附 $ 等值）。
...
4. 四维分析（信息流、结算清晰度、流动性、波动性）— 各 1-5 分并附简要说明
5. 仓位建议（方向、规模范围、限价、下单策略）
6. 完整的信息源列表（URL + 关键要点）
...
- **scrape-market.ts**：位于 `{baseDir}/../api-trade-polymarket/scripts/`，用于获取结算规则和评论。需要 `npx tsx`。
