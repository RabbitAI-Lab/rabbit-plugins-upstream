...
from surgery_sufficiency_review import DEFAULT_LLM_BASE, DEFAULT_LLM_MODEL, review_surgery_sufficiency_payload
...
def save_prepared(payload: dict[str, Any], output_json: str, input_path: Path) -> None:
    save_dir = Path(output_json).parent if output_json else ICD_DRG_DIR / "runs" / "surgery-sufficiency-review"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="手术/操作依据充分性审核统一入口：支持 pdf/doc/docx/xls/xlsx/csv/txt/json 输入。")
...
def main() -> int:
    ...
    try:
        ...
        response = review_surgery_sufficiency_payload(payload)
