---
name: amazon-conversion-copywriter
description: >
  结合 Amazon 商品详情和评论证据，诊断买家决策障碍并提出页面转化表达建议。仅用于证据驱动的转化表达诊断；不用于转化率预测、广告投放或自动发布。
  Requires an ARI API key (ari_live_*).
slug: conversion-copy
displayName: Amazon 转化表达优化
version: 1.4.7
summary: 依据评论信号改善页面转化表达
license: MIT
---
...
- 本入口只执行 `listing/conversion`，输出模板固定为 `ops_listing`。
