...
length $(z_1, ..., z_n)$, with $x_i, z_i in RR^d$, such as a hidden layer in a
typical sequence transduction encoder or decoder. Motivating our use of
...
The third is the path length between long-range
dependencies in the network. Learning long-range dependencies is a key challenge in
many sequence transduction tasks. One key factor affecting the ability to learn
...
dependencies @hochreiter2001gradient. Hence we also compare the maximum path length
between any two input and output positions in networks composed of the different
layer types.
...
#figure(
  ...
  table(
    ...
    [Self-Attention],              [$O(n^2 dot.c d)$],       [$O(1)$], [$O(1)$],
    [Recurrent],                   [$O(n dot.c d^2)$],       [$O(n)$], [$O(n)$],
    [Convolutional],               [$O(k dot.c n dot.c d^2)$], [$O(1)$], [$O(log_k (n))$],
    [Self-Attention (restricted)], [$O(r dot.c n dot.c d)$], [$O(1)$], [$O(n \/ r)$],
    table.hline(stroke: 0.8pt),
  ),
  caption: [
    Maximum path lengths, per-layer complexity and minimum number of sequential
    operations for different layer types. $n$ is the sequence length, $d$ is the
    representation dimension, $k$ is the kernel size of convolutions and $r$ the
    size of the neighborhood in restricted self-attention.
  ],
) <tab-complexity>

As noted in @tab-complexity, a self-attention layer connects all positions with a
...
translations, such as word-piece @wu2016google and byte-pair @sennrich2015neural
representations. To improve computational performance for tasks involving very long
sequences, self-attention could be restricted to considering only a neighborhood of
size $r$ in the input sequence centered around the respective output position. This
would increase the maximum path length to $O(n \/ r)$. We plan to investigate this
approach further in future work.

A single convolutional layer with kernel width $k < n$ does not connect all pairs
of input and output positions. Doing so requires a stack of $O(n \/ k)$
convolutional layers in the case of contiguous kernels, or $O(log_k (n))$ in the
case of dilated convolutions @kalchbrenner2017neural, increasing the length of the
...
convolutions @chollet2016xception, however, decrease the complexity considerably,
to $O(k dot.c n dot.c d + n dot.c d^2)$. Even with $k = n$, however, the complexity
