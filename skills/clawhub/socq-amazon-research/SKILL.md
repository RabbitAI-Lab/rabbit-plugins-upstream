---
name: socq-amazon-research
description: Research public Amazon content, accounts, keywords, and performance data with SocQ. Use when an agent needs Amazon-specific discovery, collection, endpoint selection, credit estimates, asynchronous task execution, pagination, or raw exports through the SocQ CLI, MCP, or REST API.
metadata:
  openclaw:
...
- API documentation: <https://docs.socq.ai/api-manual/amazon>
...
Use this Skill to select and run SocQ endpoints for public Amazon research. SocQ requests are asynchronous and credit-metered.

## Use When

- The user asks to discover, collect, compare, or analyze public Amazon data.
- The workflow needs Amazon-specific endpoint selection, input validation, pagination, or raw exports.
...
1. Prefer an already configured SocQ MCP server at `https://api.socq.ai/mcp?platforms=amazon`; use the typed tool listed in [platform.md](references/platform.md), or compact `socq_execute` when needed.
