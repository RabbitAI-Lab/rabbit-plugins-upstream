"""CLI argument parsing tests for time-series-classification."""
import subprocess
import sys
import os
import json
...
class TestCLIBasics:
    ...
    def test_bbox_only_runs(self, tmp_path):
        ...
        assert os.path.exists(os.path.join(out, "feature_importance.json"))
        assert os.path.exists(os.path.join(out, "typical_curves.json"))

    def test_synthetic_mode_runs(self, tmp_path):
        out = str(tmp_path / "out")
        r = run_cli(["--bbox", "116", "39", "117", "40", "--synthetic",
                     "--n-dates", "10", "--output-dir", out, "--quiet"])
        assert r.returncode == 0, r.stderr
