...
**外应权重**：
- 体用生克指向明确时，外应作为辅助验证，权重较低
- 体用比和或吉凶难断时，外应权重提高，可作重要参考
- 外应与卦象明显矛盾时，以卦象为主，外应供参考
...
**⏸️ 输出完以上内容后必须暂停，等待用户回复外应，再进行阶段二。**

### 阶段二：推断（用户回复外应后，**逐节输出，每节需引用依据**）
...
├── learning_notes.py   # 学习笔记管理
├── SKILL.md           # 说明文档
├── _meta.json         # 元信息
└── data/
    ├── divination_records.db   # 卦例记录（SQLite）
    ├── iching.json     # 易经六十四卦数据
    ├── 万物类象.md     # 八卦象意参考
    ├── 三要十应.md     # 心法参考（必读）
    └── 解卦技巧.md     # 解卦方法论
```
