# Spatial ETL Pipeline (geoskill-spatial-etl-pipeline)

> Config-driven extract-transform-load pipeline with per-step logging and a quality report

---

## 1. Overview

Config-driven spatial ETL (Extract-Transform-Load) pipeline: - **Extract**: extracts a GeoDataFrame from synthetic data or a local vector file. - **Transform**: executes composable operators in configuration order — `filter_bbox` (window filtering), `filter_attribute` (attribute comparison filtering, supporting 6 comparison operators such as >/</==), `reproject` (reprojection), `add_field` (derived columns: area / perimeter / centroid / sequence number), `rename` (field renaming), `buffer` (buffering). - **Load**: writes GeoJSON or GeoPackage. Each step records structured logs (step name, elapsed time, input/output feature counts, status), and a quality report is aggregated at the end (feature gain/loss, retention rate, null-value ratio, invalid geometry count, CRS). Any pipeline can be customized by supplying a JSON config via `--config`. The `--synthetic` mode generates random polygons and runs the default pipeline.
...
python geoskill-spatial-etl-pipeline.py --bbox 116 39 117 40 --synthetic --output-dir ./out
```
