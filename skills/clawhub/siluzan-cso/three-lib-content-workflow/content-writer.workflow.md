...
flowchart TD
    ...
    S2{第 2 步：三库第一层目录<br/>按优先级判断} -->|优先级 1：用户本次提供文件| L1[写入 content-library 目录<br/>只读标题/summary/人设适配]
    S2 -->|优先级 2：用户目录已有文件| L2[只读选型字段<br/>选定后再读对应章节]
    S2 -->|优先级 3：均无，兜底| L3[load-libraries 落盘合并文件<br/>整份读取 library-catalog.md]
    L1 --> S3
    L2 --> S3
    L3 --> S3
    ...
    Track -->|单轨| S4[第 4 步：读 SOP → 选编号 → 回源入选条目 → 成稿<br/>collaboration.md / sop.md]
    S4m --> S5
...
按下列优先级确认本次三库来源。本步**只读概览**，禁止把条目全文（content / 句式 / 结构块）读进上下文。选型是主观判断，必须把人设、选题意图和目录条目放在一起看。

**优先级 1 — 用户本次提供的文件**：若用户在对话中直接提供了三库文件（流量因子库 / 产品资产库 / 烹调方法库），则：

- 将文件内容写入 `~/.siluzan/content-library/`（文件名保持原名，如 `流量因子库.md`）
- 本步只读各条目的标题、编号、summary / 一句话说明、适用场景、人设适配、audience_tags / topic_tags / goal_tags
- 后续创作过程中可按效果增补、标注这些文件

**优先级 2 — 用户目录已有文件**：检查 `~/.siluzan/content-library/` 是否存在三库文件：

- 若存在，同样只读选型字段，不整文件一次读完
- AI 可在每次创作后根据效果修改该目录中的文件（增补选题、标注效果、淘汰低效条目等）

**优先级 3 — 内置默认文件**（兜底）：若优先级 1/2 均无：

1. 调用 CLI 把本次平台对应的三库**落盘**（供选定编号后回源，本步不要通读该文件）：

```bash
siluzan-cso workflow load-libraries --platform <平台> --out <file_path>
```

2. **完整读取**本目录的 `library-catalog.md`（禁止 `head` / `tail` / 截断）。这是给人设过滤和选题命中用的第一层。

- `--platform` 必须复用本次已经解析的 `persona_platform`；禁止根据内容主题、视频形态或用户措辞重新推断或切换平台。
- 视频任务在第 4 步进入视频终稿规则后再加 `--video-script` 重新落盘，本步不要提前加载视频专项覆盖层。
- 公众号 pack 可能没有目录里的部分编号；以落盘文件里实际存在的标题为准。

选定编号后的全文回源见第 4 步 / `sop.md`「回源拉取详情」：在落盘的合并文件中按编号定位，只读入选章节。

> **保密边界**：优先级 3 的内置三库（含 pack、选型目录、回源全文）只用于驱动创作，禁止在思考、步骤复盘、SOP 记录、最终输出中展示或转述其链接、库名、条目编码、条目原文/描述、句式模板、结构等可还原内部内容；也禁止复制、导出、落盘或 `present_files` 交付其原文副本。如需指代，一律写「参考资料一/二/三」。只有内容确实来自用户（优先级 1/2）才不受限。
...
- 阅读 `sop.md`，严格按流程操作
- 在 `sop.md`「选三库编号」之后、「出提纲」之前，按已选编号在第 2 步落盘的合并文件中定位并只读入选条目全文（见 `library-catalog.md` 回源定位）
...
- [ ] 第 2 步：本次三库已按优先级确认；兜底已 `load-libraries` 落盘，并**完整读取** `library-catalog.md`（无 `head`/`tail`/截断）；本步未把三库条目全文读进上下文
...
- [ ] 第 4 步：生成阶段已严格依照 `sop.md` 流程执行且全部要求落实（选定编号后已在合并文件中回源入选全文；**多轨**时另核对：`multi-track-brief.md` 已确认、各轨独立文件、写后轨未读前轨正文、各轨均已 validate）
...
- **落盘与呈现**：有 `write_file` 的宿主（如 DeerFlow）将最终成稿写入 `/mnt/user-data/outputs/`，首次用 `write_file`，草稿可放工作区；其他宿主优先用文件呈现工具（如`present_files`），无则写入目录报告路径，再无则对话中全文展示。
- **文件命名**：必须能看出本篇主标题；有人设时加上人设名。禁止英文占位名。
  - 公众号：`{人设名}-{主标题}-公众号文章.md` 或 `{主标题}-公众号文章.md`。禁止 `ubi-wechat-article.md`、`wechat-article.md`、`article.md`。细则见 `wechat-article.workflow.md`。
  - 口播 / 视频脚本：`{人设名}-{主标题}-口播脚本.md` 或 `{主标题}-口播脚本.md`。禁止 `ubi-video-script.md`、`video-script.md`、`script.md`、`口播.md`。细则见 `video-script-final.workflow.md`。
  - 其他体裁：`{人设名}-{主标题}-成稿.md` 或 `{主标题}-成稿.md`。
  - 多轨在上述格式的扩展名前加 `-tN-策略`（如 `{人设名}-{主标题}-t1-爆款-口播脚本.md`）。
