...
2. 从流量因子库判断「写什么能被看见」，但**先整份读 `library-catalog.md`**，按「目标受众 / 禁忌」过滤，再按已确认选题命中编号；选定后再到合并文件回源全文（见 `sop.md` 选三库编号 / 回源拉取详情）。
3. 从产品资产库找落点：选题命中题材表则用对应模板；未命中才从「内容 DNA / 一句话定位」推断风格倾向过滤模板。
4. 从烹调方法库选结构：题材表指定的结构优先；否则从「内容 DNA · 开场惯用方式 / 节奏」推断，无则通用默认。
