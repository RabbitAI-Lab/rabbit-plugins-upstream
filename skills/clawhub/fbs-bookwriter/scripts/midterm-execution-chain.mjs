...
async function main() {
  try {
    ...
    if (args.json) {
      ...
      // 明确区分无数据（all KPIs=0）与正常通过，避免无数据时静默误导
      const kpi = out.gate?.checks || [];
      const hasData = kpi.some((c) => typeof c.rate === 'number' && c.rate > 0);
      if (!hasData) {
        console.log('[midterm-execution-chain] no-data: 当前 book-root 尚无 KPI 信号，gate 结果仅供参考，不代表真实质量');
      }
