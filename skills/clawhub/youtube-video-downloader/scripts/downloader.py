...
Uses redfox.hk API to parse YouTube video links and return watermark-free download URLs.
...
def main():
    parser = argparse.ArgumentParser(
        description="YouTube Video Downloader - parse videos via redfox.hk API and return download links",
...
"""
    ...
    try:
        resp = session.post(API_URL, json={"url": url, "source": "youtube视频下载-ClawHub"}, timeout=30)
        result = resp.json()
    ...
    # Resources: display every resource's full info (type / duration / download URL / cover URL)
