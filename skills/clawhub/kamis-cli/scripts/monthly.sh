#!/usr/bin/env bash
# 월별 평균 가격 (action=monthlySalesList).
#
# Required: --year YYYY --category <code>
# Optional: --period "YYYY,YYYY"   compare years (e.g. 2024,2025)
#           --item <code> --kind <code>
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/_common.sh"

year=""; period=""; category=""; item=""; kind=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --year) year="$2"; shift 2;;
...
[[ -n "$period" ]] && args+=("p_period=${period}")
[[ -n "$item" ]] && args+=("p_itemcode=${item}")
[[ -n "$kind" ]] && args+=("p_kindcode=${kind}")

kamis_call monthlySalesList "${args[@]}"
