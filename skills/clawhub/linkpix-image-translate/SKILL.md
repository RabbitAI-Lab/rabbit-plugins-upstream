...
description: 批量翻译商品图片中的文字内容，自动保持原有版式与设计风格，帮助跨境卖家完成多语言商品图本地化。当用户要求翻译图片文字、图片多语言化、主图/详情图翻译成英语/西语等语言时必须触发。关键词：LinkPix、qhkit、译图、图片翻译、批量译图、批量翻译、图片本地化、多语言主图、跨境电商、翻译详情图、图片改语言。
user-invocable: true
...
走官方「批量译图」通道（`image-batch`，图片翻译专用 SKU，与 LinkPix 工作台「图片翻译」同链路），一次 1–10 张：

```bash
# 目标语言必须写在 prompt 里
qhkit image-batch generate '{"mode":"批量译图","prompt":"翻译成英语，保持原有版式与设计风格","uploadedImages":["./主图1.jpg","./主图2.jpg"]}'
# 报价（按张计费，同参数换 estimate）
qhkit image-batch estimate '{"mode":"批量译图","prompt":"翻译成英语","uploadedImages":["./主图1.jpg","./主图2.jpg"]}'
```

- **`prompt` 必须写明目标语言**（如「翻译成英语」「翻译成日语」），否则模型不知道译成什么；可追加「保持原有版式与设计风格」。
- **没有张数参数**：出图张数 = 传入图片张数，**单批上限 10 张**（服务端超过 10 张会静默截断，CLI 提前拦下），更多图分批提交后按原文件名汇总交付。
- **按张计费**，模型与尺寸固定（`modelLabel` / `sizePreset` / `quality` 在这个 mode 下不生效），提交前用 estimate 报总价。
- **必须目检**：AI 渲染文字可能出错，出图后逐张核对译文拼写与关键信息（价格、规格、品牌名），价格数字建议在 prompt 里明确「数字保持不变」。
- 要在翻译之外顺带改版式/改配色，那是改图需求，走「批量改图」或 `qhkit image generate`。
