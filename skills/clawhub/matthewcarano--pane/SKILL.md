...
4. Complete every step through this session: reply with results, use Pane's tools, schedule follow-ups with the cron recipe above. `exec` is permitted only to run `openclaw cron add`. Never use it to do the user's task, and never touch files or databases outside the session.
