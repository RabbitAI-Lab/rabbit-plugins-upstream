...
description: Generate 3D rendered art and icons. Use when the user asks for 3D graphics, claymorphism, octane render, or blender 3d style.
version: 0.5.0
...
# 3D Image Generator (`3d-image-generator`)
...
As the 3D Image Generator agent, you MUST always enforce this specific style.
1. Always append `--style 3d-render` to your `3d-image-generator` CLI command.
2. When building the prompt, seamlessly blend these keywords into the visual description: "3d render, octane render, unreal engine, claymorphism, soft lighting, ray tracing".
Do not ask the user which style they want, because this skill is strictly dedicated to 3D Image Generator.
...
- **Files written**: Output images and optional local config under `.image-skills/3d-image-generator/` (project) and/or `~/.image-skills/3d-image-generator/` (home).
...
```bash
test -f .image-skills/3d-image-generator/EXTEND.md && echo project
test -f "$HOME/.image-skills/3d-image-generator/EXTEND.md" && echo user
```
