...
```powershell
# 1. 找到网关进程（两个 node 进程，命令行含 "openclaw-gateway"）
Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -match "openclaw" } | Select-Object ProcessId, CreationDate
# 2. 杀旧进程（PID 取上面结果）
# ⚠️ 此操作会中断所有活跃会话/定时任务/进行中的工作，确认无重要任务再执行
Stop-Process -Id <PID1>, <PID2> -Force
# 3. 等待新进程自动拉起（约 1-3 分钟）
Get-NetTCPConnection -LocalPort <网关端口> -State Listen | Select-Object OwningProcess
# 4. 确认新进程创建时间是"刚刚"，且监听网关端口（以实际环境为准）
```
...
> 若网关无法自动拉起，手动启动：`openclaw gateway run`（前台）。
...
- **想跨 agent 发送** → 需 `tools.agentToAgent`，且 visibility=`all`（风险高，慎用）
