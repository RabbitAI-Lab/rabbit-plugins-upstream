...
var setUploader = function(path){
	...
	$('#newfolder').unbind().click(function(){
		...
		var getFolderName = function(v, m){
			...
			if(fname != ''){
				...
				if(foldername.length != fname.length) {
					alert('名称只能包含英文字母、阿拉伯数字和下划线');
					return;
				}
				var d = new Date(); // to prevent IE cache issues
				$.getJSON(fileConnector + '?mode=addfolder&path=' + $('#currentpath').val() + '&name=' + foldername + '&time=' + d.getMilliseconds() + '&spaceId=' + spaceId, function(result){
					if(result['Code'] == 0){
	...
	$('#createfile').unbind().click(function(){
		...
		var getFileName = function(v, m){
			...
			if(fname != ''){
				//filename = cleanString(fname);
				filename = fname;
				var d = new Date(); // to prevent IE cache issues
				$.getJSON(fileConnector + '?mode=addfile&path=' + $('#currentpath').val() + '&name=' + filename + '&time=' + d.getMilliseconds() + '&spaceId=' + spaceId, function(result){
					if(result['Code'] == 0){
...
var bindToolbar = function(data){	
	...
	$('#fileinfo').find('button').wrapInner('<span style="font-weight:500;"></span>');
	...
	if (!has_capability(data, 'download')) {
		...
		$('#fileinfo').find('button#download').click(function(){
			window.location = fileConnector + '?mode=download&path=' + encodeURIComponent(data['Path'] + '&spaceId=' + spaceId);
		}).show();
	}
	$('#edit').click(function(){		
		if($.browser.msie)
			window.open('edit.jsp?spaceId=' + spaceId + '&websiteId=' + websiteId + '&filePath=' + encodeURIComponent($('#uploader h1').text().substring($('#uploader h1').text().indexOf('/') + 1) + $('#preview h1').text()), '', 'channelmode=yes');
		else
			window.open('edit.jsp?spaceId=' + spaceId + '&websiteId=' + websiteId + '&filePath=' + encodeURIComponent($('#uploader h1').text().substring($('#uploader h1').text().indexOf('/') + 1) + $('#preview h1').text()));
	});
	var fileName = $('h1').text();	
	if(fileName.indexOf('.html') > 0 || fileName.indexOf('.htm') > 0 || fileName.indexOf('.xhtml') > 0 || fileName.indexOf('.shtml') > 0) {
		$('#edit').show();
	}
	else {
		$('#edit').hide();
	}
    setFileProtected(data.protect == 'true');
}
...
var renameItem = function(data){
	...
	var getNewName = function(v, m){
		...
		if(rname != ''){
			...
			/*if(givenName.length - suffix.length != rname.length) {
				alert('名称只能包含英文字母、阿拉伯数字和下划线');
				return;
			}*/
			var oldPath = data['Path'];	
			var connectString = fileConnector + '?mode=rename&old=' + data['Path'] + '&new=' + givenName + '&spaceId=' + spaceId;
...
var deleteItem = function(data){
	...
	var doDelete = function(v, m){
		if(v != 1) return false;	
		var connectString = fileConnector + '?mode=delete&path=' + encodeURIComponent(data['Path'] + '&spaceId=' + spaceId),
...
var setMenus = function(action, path){
	var d = new Date(); // to prevent IE cache issues
	$.getJSON(fileConnector + '?mode=getinfo&path=' + path + '&time=' + d.getMilliseconds() + '&spaceId=' + spaceId, function(data){
		if($('#fileinfo').data('view') == 'grid'){
		...
		switch(action){
			...
			case 'download':
				window.location = fileConnector + '?mode=download&path=' + data['Path'] + '&spaceId=' + spaceId;
				break;
...
function setFileProtected(isProtected){
	if(isProtected){
		$('#download,#edit,#rename').hide();
	}
}
...
var getFileInfo = function(file){
	...
	if((window.opener || window.tinyMCEPopup) && CKEditorFuncNum) template += '<button id="select" name="select" type="button" value="Select">' + lg.select + '</button>';
	template += '<button id="download" name="download" type="button" value="Download">' + lg.download + '</button>';
	template += '<button id="edit" name="edit" type="button" value="Edit">' + lg.edit + '</button>';
	if(browseOnly != true) template += '<button id="rename" name="rename" type="button" value="Rename">' + lg.rename + '</button>';
	if(browseOnly != true) {
		if(!onlyAdminDo || onlyAdminDo && isSuperAdmin){
	...
	var d = new Date(); // to prevent IE cache issues
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds() + '&spaceId=' + spaceId, function(data){
		if(data['Code'] == 0){
...
var getHtmlFiles = function(path) {
	var url = fileConnector + '?path=' + encodeURIComponent(path) + '&mode=getfolder&time=' + d.getMilliseconds() + '&spaceId=' + spaceId;
...
var getFolderInfo = function(path){
	...
	var d = new Date(); // to prevent IE cache issues
	var url = fileConnector + '?path=' + encodeURIComponent(path) + '&mode=getfolder&showThumbs=' + showThumbs + '&time=' + d.getMilliseconds() + '&spaceId=' + spaceId;
...
$(function(){
	...
	if(autoload == true) {
		...
		$('#createfile').append(lg.create_file);
		$('#compile').append(lg.compile);
		$('#exporte').append(lg.exporte);
		$('#property').append(lg.property);
	...
	$('#exporte').click(function(){
		
	});
	$('#compile').click(function(){
		$('#compilingFrame').attr('src', 'compile.jsp?spaceId=' + spaceId + '&websiteId=' + websiteId + '&encoding=' + website.encoding + '&t=' + new Date().getTime());
		$( "#compiling" ).dialog({
		      height: 330,
		      width: 805,
		      modal: true
	...
	$('#uploader').attr('action', fileConnector + '?spaceId=' + spaceId);
