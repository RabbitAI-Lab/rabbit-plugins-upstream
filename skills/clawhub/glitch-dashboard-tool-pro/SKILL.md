...
summary: "企业级运维仪表盘,支持自定义面板、告警规则、多节点监控与历史数据分析。核心能力: 运维监控领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 运维监控领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要系统监控、日志分析、运维告警、部署管理时使用。不适用于物理硬件维修。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。 功能涵盖: glitch, dashboard。"
...
本工具是 **运维监控** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析自定义仪表盘面板与布局配置的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自定义仪表盘面板与布局配置的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 告警规则
告警规则:阈值触发邮件/短信/Webhook

**处理**: 解析告警规则的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回告警规则的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多节点集群监控与聚合视图
多节点集群监控与聚合视图

**处理**: 解析多节点集群监控与聚合视图的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多节点集群监控与聚合视图的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 历史数据存储与趋势分析图表
历史数据存储与趋势分析图表

**处理**: 解析历史数据存储与趋势分析图表的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回历史数据存储与趋势分析图表的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 权限管理
权限管理:多用户角色与访问控制

**处理**: 解析权限管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回权限管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级运维仪表盘、支持自定义面板、多节点监控与历史、数据分析、核心能力、运维监控领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 统一仪表盘专业版处理的输入数据或指令 |
...
```bash
dashboard status
# 输出: CPU 45% | MEM 62% | Tasks 3 pending
```
...
A: 检查端口是否被占用,尝试更换端口或检查防火墙设置.
### Q: 支持哪些监控指标?

A: CPU/内存/负载/运行时间/任务队列/网络状态,PRO 版可扩展自定义指标.
...
```json
{
  ...
  "data": {
    ...
    "metadata": {
      "version": "1.0",
      "processor": "glitch dashboard pro"
    }
  },
...
- **自动化执行**: 企业级运维仪表盘,支持自定义面板、告警规则、多节点监控与历史数据分析。核心能力: 运维监控领域的专业化 AI 辅助工具,
