...
def mark_duplicates(threshold=0.90):
    ...
    修复（2026-07-06）：修复"链式去重断裂"问题。
    旧逻辑：只查询 is_duplicate=0 的文章互比，导致新来的文章无法和
    前一天已标记重复的文章比较，同一事件的后发报道漏网。
    新逻辑：查询窗口内所有文章（含已标记重复），两两比较；
    同一簇中只保留最早的一批（按 id 升序），后续相似的标为重复。
