...
def clean_summary(summary, max_length=800):
    """
...
def check_article_quality(news_list):
    """质量检查：检测空壳/短内容新闻，返回 (过滤后的列表, 问题列表)"""
    ok_list = []
    issues = []
    for i, news in enumerate(news_list):
        ...
        # 检查摘要是否实质为空（忽略 logo、链接等垃圾内容）
        clean_summary_text_stripped = re.sub(r'<img[^>]*>|原文链接[\uff1a:][^\n]*', '', summary).strip()
        summary_chars = len(clean_summary_text_stripped)
        ...
        if summary_chars < 30:
            issues.append({
                ...
                'source': source,
                'url': url,
                'summary_len': summary_chars,
                'content_len': len(content) if content else 0,
                'reason': '摘要为空或极短(空壳)' if summary_chars < 10 else f'摘要过短({summary_chars}字)'
            })
            # 跳过空壳新闻，不输出
            continue
...
def generate_output(news_list, max_count=35):
    ...
    for i, news in enumerate(filtered_news, 1):
        ...
        # 摘要处理：合并多行为紧凑段落
        if summary:
            # 检测结构化摘要(数据卡片型:以**或-开头的多行格式)
            is_structured = bool(re.match(r'^\*\*|^- ', summary))
