---
name: linkedin-company-search-zh
description: 依托 LinkedIn 数据库，按照企业名称、所属行业、公司规模以及成立年份筛选企业资料，助力外贸从业者开发客户、开展市场调研，完成领英企业画像搭建以及 ABM 账户营销的目标客户筛选工作。
...
使用跨境魔方开放平台API从领英数据搜索公司。

## 概述

本技能通过一个列表搜索脚本提供对跨境魔方领英公司数据的访问。通过`auth.py`脚本提供 API密钥生成、充值；
...
**重要**：始终使用直接脚本调用，如 `python scripts/linkedin_company_search.py`。**不要使用** shell 复合命令如 `cd scripts && python linkedin_company_search.py`

### 领英公司列表搜索 (`linkedin_company_search.py`)
- **返回粒度**：每家公司为一行记录
- **适用场景**：关心的是领英数据中"有哪些公司"
- **示例**：
  - "找生产LED灯的厂家"
  - "找员工规模100-500人的科技公司"
- **参数**：查看参数说明 [领英公司列表](references/linkedin-company-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/linkedin/company/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"领英公司搜索查询失败，服务端异常"}'
```
...
| 用户意图 | 使用API |
|-------------|---------|
| "找生产XXX的公司" | 领英公司列表（keywords / companyNames） |
| "找有邮箱/电话的公司" | 领英公司列表 existEmail=1 / existPhone=1 |
| "找100-500人的公司" | 领英公司列表 companySizes=["51-200","201-500"] |
| "找2015年后成立的公司" | 领英公司列表 minCompanyFounded=2015 |
...
- [领英公司列表 API](references/linkedin-company-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/linkedin-company-list-api.md](references/linkedin-company-list-api.md)
   - 不要猜测参数名称，从文档中获取准确的参数名称和格式

2. **优化查询参数**：
   - 使用 `keywords` / `companyNames` 精确筛选，行业和产品名称需使用**英文**
   - 使用 `existEmail=1` 或 `existValidEmail=1` 筛选有联系方式的公司
   - 使用 `countryCodes` 限定国家范围；使用 `companySizes` 限定公司规模
