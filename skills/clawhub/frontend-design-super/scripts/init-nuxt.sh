#!/bin/bash
# Initialize a Nuxt 3 + TypeScript + Tailwind + @nuxt/ui project
# Usage: bash scripts/init-nuxt.sh <project-name>
...
# Update nuxt.config.ts with all modules
...
# Create composables directory with motion composable
mkdir -p composables
cat > composables/useMotion.ts << 'EOF'
...
# Create a basic page with animations
mkdir -p pages
