#!/bin/bash
# Initialize a Vite + Vue 3 + TypeScript + Tailwind + shadcn-vue project
# Usage: bash scripts/init-vue-vite.sh <project-name>
...
echo "🚀 Creating Vue 3 + Vite project: $PROJECT_NAME"

# Create Vite project with Vue 3
...
# Create Button component for Vue
...
# Update vite.config.ts with path aliases and Vue plugin
...
# Create components.json for shadcn-vue CLI
...
# Create Vue motion plugin setup
mkdir -p src/plugins
cat > src/plugins/motion.ts << 'EOF'
import { MotionPlugin } from '@vueuse/motion'

export function setupMotion(app: any) {
