---
name: organization_code_certificate_ocr
description: 支持组织机构代码证的票据识别。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 组织机构代码证 OCR 技能

本技能封装了组织机构代码证识别的 OCR 服务，通过单一接口即可调用 1种识别能力，高效提取票据内容。

---

## 功能特性

- **组织机构代码证识别**：支持识别组织机构代码证的票据信息。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• ORGANIZATION_CODE_CERT（组织机构代码证） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/organization_code_certificate_ocr/scripts/main.py ORGANIZATION_CODE_CERT /path/to/test.jpg
```
...
- “帮我识别这张图像中的组织机构代码证票据信息，图片在 /Users/name/Downloads/test.jpg”
