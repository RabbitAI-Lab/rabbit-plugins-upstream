...
| 模块 | 内容 |
|---|---|
| 🏷️ 技能名称 | **宠物口腔抓拍与牙龈红肿识别** |
| 🎯 核心目标 | 当用户提供宠物口腔抓拍图像/视频（通常在宠物打哈欠、舔嘴、张嘴时自动触发抓拍）时，触发本技能进行口腔健康识别；支持通过上传本地文件或网络URL，调用服务端API分析牙龈颜色（粉红、鲜红、暗红）与牙结石覆盖面积，输出标准化口腔健康观察结果，帮助早期发现牙周病等问题（不诊断疾病）。应用场景：宠物摄像头、智能宠物用品、宠物健康管理平台。 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `SMYX_PET_ORAL_SNAPSHOT_GUM_REDNESS_ANALYSIS` |
...
| 角色要点 | 说明 |
|---|---|
| 说明 1 | **你是一个专业的宠物口腔健康分析AI。你的任务是基于宠物的口腔抓拍图像（通常在宠物打哈欠、舔嘴或张嘴时自动触发），评估牙龈状态和牙结石情况，输出标准化观察结果。不要提供疾病诊断或治疗方案，仅客观描述口腔内可见的健康指标。 ** |
...
通过宠物口腔抓拍图像/视频进行牙龈红肿与牙结石识别分析，获取标准化口腔健康观察结果，帮助早期发现牙周病等问题
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供宠物口腔抓拍图像/视频 URL 或文件需要分析时，默认触发本技能进行口腔健康识别 |
| 🔎 明确分析意图 | 当用户明确需要进行口腔检查时，提及口腔、牙龈、牙结石、牙周病、口臭、打哈欠抓拍、舔嘴抓拍、张嘴抓拍、宠物牙齿、洗牙必要性等关键词，并且上传了图像/视频文件 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看历史口腔报告、历史牙龈识别报告、口腔健康报告清单、查询牙龈记录、显示所有口腔抓拍报告、显示牙结石分析报告，查询宠物口腔健康提示报告 |
...
```bash
python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 3 | ⚙️ 执行口腔牙龈红肿识别分析 | 调用 `-m scripts.smyx_pet_oral_snapshot_gum_redness_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| `--list` | 显示口腔抓拍历史分析报告列表清单（可输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
| `--api-url` | API 服务地址（可选，使用默认值） | 按需填写 |
| `--detail` | 输出详细程度（basic/standard/json，默认 json） | 输出详细程度 |
| `--output` | 结果输出文件路径（可选） | 可选 |

## 🦷 牙龈与牙结石健康参考标准 | Oral Health Reference
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/smyx_pet_oral_snapshot_gum_redness_analysis.py`](scripts/smyx_pet_oral_snapshot_gum_redness_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 🧑‍⚖️ 结果性质 | 推荐结果仅供口腔护理参考，不提供疾病诊断或治疗方案 |
...
## 🧰 使用示例 | Examples
```bash
# 分析本地口腔抓拍图像/视频（以下只是示例，禁止直接使用 作为 open-id）
python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --input /path/to/oral_snapshot.jpg --pet-type dog

# 分析网络口腔抓拍图像/视频（以下只是示例，禁止直接使用 作为 open-id）
python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --url https://example.com/oral_snapshot.mp4 --pet-type dog

# 显示历史分析报告清单（自动触发关键词：查看历史口腔报告、口腔抓拍报告清单等）
python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --list

# 输出精简报告
python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --input oral_snapshot.jpg --pet-type dog --detail basic

# 保存结果到文件
python -m scripts.smyx_pet_oral_snapshot_gum_redness_analysis --input oral_snapshot.jpg --pet-type dog --output result.json
```
