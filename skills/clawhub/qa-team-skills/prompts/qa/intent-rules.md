...
> 本文件是 `/qa` 的**路由速查表**，仅在意图路由不确定 / 需要子能力匹配时加载（见 SKILL.md「渐进式加载指引」）。完整执行流程（历史加载、自动规划、记忆写入、规范沉淀、索引维护等）统一维护在 `prompts/qa/prompt.md`，此处不重复，避免双处维护导致规则漂移。
...
以下规则已在 `/qa` 入口指令 `prompts/qa/prompt.md` 中完整定义，本文件不再重复，加载时按需指向：
