...
try:
    ...
    print(json.dumps({'mode': mode, 'message': msg, 'timeout': timeout, 'no_ack_ids': no_ack}))
except Exception:
    pass
" 2>/dev/null || true)
    ...
    if [ "$_ACTIVE" = "yes" ] && [ -z "$_POLICY_CLAIMED" ]; then
      ...
      if [ -n "$OPENCLAW_BIN" ]; then
        ...
        "$OPENCLAW_BIN" cron add \
          ...
          --message "$MSG" \
          --session isolated $_DELIVER_FLAGS \
...
# --- Auto-update skill via install.sh ---
INSTALL_SCRIPT="$SKILL_DIR/scripts/install.sh"
if [ -x "$INSTALL_SCRIPT" ]; then
  bash "$INSTALL_SCRIPT" --quiet 2>/dev/null || true
fi
...
if [ ! -f "$_EXEC_APPROVAL_DONE" ]; then
  ...
  if [ -x "$SETUP_EXEC" ]; then
    bash "$SETUP_EXEC" --quiet 2>/dev/null || true
  fi
  mkdir -p "$CLAWGRID_HOME/state"
