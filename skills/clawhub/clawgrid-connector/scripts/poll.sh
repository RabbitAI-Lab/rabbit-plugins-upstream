...
if [ -f "$AUTH_INVALID_FLAG" ]; then
  ...
  # 去重：只在第一次触发时 notify_owner:true，后续静默
...
# --- Self-healing: ensure heartbeat scheduler is running ---
HEARTBEAT_SCRIPT="$SKILL_DIR/scripts/heartbeat.sh"
HB_MIN=$(python3 -c "import json; print(json.load(open('$CONFIG')).get('heartbeat_interval_minutes', 2))" 2>/dev/null || echo 2)
...
PLIST
      ...
      echo "[POLL] Heartbeat launchd job was missing — restored (every $HB_MIN min)" >&2
    fi
  else
    ...
    if [ "$HB_COUNT" != "1" ]; then
      ...
      | crontab -
      if [ "$HB_COUNT" = "0" ]; then
        echo "[POLL] Heartbeat crontab was missing — added (every $HB_MIN min)" >&2
      else
        echo "[POLL] Heartbeat crontab had $HB_COUNT entries — deduped to 1" >&2
      fi
    fi
  fi
fi
...
# ============================================================
# 统一任务获取：每次 poll.sh 只处理一个任务
# 优先恢复已认领任务（取第一个），没有则 poll → claim 新任务
# assigned_tasks 来自 poll 响应，包含 bid-accepted / service-request
# 等所有已分配给本节点的任务，无需额外 API 调用
# ============================================================
TASK_ID=""
TASK_TYPE=""
BUDGET=""
...
# Step 1.8: 从 poll 响应的 assigned_tasks 中检查已分配任务
...
# 从 poll 响应的 agent_context 提取每日进度
...
# ============================================================
# Step 5: 统一的 prefetch → submit / needs_ai 处理
# 无论任务来源（resume 或新 claim），走同一条路径
# ============================================================
...
if [ -f "$PREFETCH_SCRIPT" ]; then
  # prefetch.py exit 1 = "需要 LLM"，是正常流程，不能让 set -e 杀掉脚本
  set +e
