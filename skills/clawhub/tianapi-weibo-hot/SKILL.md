---
name: tianapi-weibo-hot
description: 获取微博实时热搜榜，返回热搜词、热度及话题链接。
homepage: https://www.tianapi.com/apiview/100
metadata: {"openclaw": {"emoji": "🔥", "requires": {"bins": ["python3"], "env": ["TIANAPI_WEIBO_HOT_KEY"]}, "primaryEnv": "TIANAPI_WEIBO_HOT_KEY"}}
---
...
### 获取微博热搜榜
    ...
    ```bash
    # 获取微博热搜榜
    python3 scripts/fetch_weibo_hot.py <API_KEY>
...
**输出示例：**
    ...
    1. 失踪女童确认曾在漳州出现 [热]
       热度: 129940
       链接: https://s.weibo.com/weibo?q=失踪女童确认曾在漳州出现

    2. 沈佳妮给朱亚文备注是大腻乎 [新]
       热度: 101845
       链接: https://s.weibo.com/weibo?q=沈佳妮给朱亚文备注是大腻乎
    ...
...
### 直接调用 API（无需脚本）
    ...
    ```bash
    # 获取微博热搜榜
    GET https://apis.tianapi.com/weibohot/index?key=YOUR_API_KEY
...
当用户询问微博热搜、当前热点话题时，按以下步骤操作：

1.  **识别意图**：用户想查询微博热搜榜。
2.  **调用脚本**：执行 `python3 scripts/fetch_weibo_hot.py` 命令，传入API Key。
3.  **展示结果**：解析返回的JSON数据，以清晰的列表形式展示热搜话题、热度、标签和链接。
...
| :--- | :--- | :--- |
| hottag | 热搜标签 | 热 |
| hotword | 热搜话题 | 失踪女童确认曾在漳州出现 |
| hotwordnum | 热搜指数 | 129940 |
...
`scripts/fetch_weibo_hot.py` - 封装了微博热搜榜查询功能。
