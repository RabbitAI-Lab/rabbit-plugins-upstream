#!/usr/bin/env python3
"""
TianAPI Weibo Hot Fetcher
--------------------------------
获取微博实时热搜榜，返回热搜词、热度及话题链接。
零外部依赖，仅使用 Python 标准库。

Usage:
    # 获取微博热搜榜
    python3 fetch_weibo_hot.py --key YOUR_KEY

    # JSON模式 (适合 Agent 调用)
    python3 fetch_weibo_hot.py --key YOUR_KEY --json

Env Config:
    export TIANAPI_WEIBO_HOT_KEY=your_api_key
    # 或在脚本同级目录创建 .env 文件: TIANAPI_WEIBO_HOT_KEY=your_api_key
"""
...
# --- 配置常量 ---
API_ENDPOINT = "https://apis.tianapi.com/weibohot/index"
ENV_KEY_NAME = "TIANAPI_WEIBO_HOT_KEY"
DEFAULT_TIMEOUT = 10
...
class Config:
    ...
    def _exit_with_help(self, msg: str):
        ...
        print(f"💡 提示: 申请 Key -> https://www.tianapi.com/apiview/100", file=sys.stderr)
        sys.exit(1)
...
class ConsoleRenderer:
    ...
    def render(result: Dict[str, Any]):
        ...
        for idx, item in enumerate(items, 1):
            hotword = item.get("hotword", "无话题")
            hotnum = item.get("hotwordnum", "0")
            hottag = item.get("hottag", "")
            
            tag_str = f" [{hottag}]" if hottag else ""
            print(f"{idx}. {hotword}{tag_str}")
            print(f"   🔥 热度: {hotnum}\n")


def main():
    parser = argparse.ArgumentParser(description="TianAPI Weibo Hot  Fetcher")
