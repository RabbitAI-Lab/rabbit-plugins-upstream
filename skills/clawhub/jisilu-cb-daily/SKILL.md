...
description: 每日从集思录 webapi 抓取可转债基本数据、强赎倒计时、下修倒计时，支持Cookie管理和本地持久化存储
---
...
- Python 3.8+
- 依赖包：`requests`, `pandas`
- 集思录账号（需要登录后的 `kbzw__user_login` 和 `kbzw__Session` Cookie）
...
### 检查逻辑
1. 每次执行任务前，先检查 `references/cookie.json` 是否存在且包含 `kbzw__user_login` 和 `kbzw__Session`
...
- 解析 JSON，直接取 `data` 字段
- 以 `bond_id` 为键，**LEFT JOIN** 到下修主表

### Step 4: 抓取基本数据
- 使用相同 Cookie 访问 `https://www.jisilu.cn/webapi/cb/list/`
- 解析 JSON，直接取 `data` 字段
- 以 `bond_id` 为键，**LEFT JOIN** 到已有数据
