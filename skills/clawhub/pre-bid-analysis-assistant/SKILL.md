---
name: pre-bid-analysis-assistant
description: 标前分析与投标尽调助手。当用户给出一个具体的招标项目并希望在投标前做全面分析时，必须使用此SKILL：标前分析/标前尽调、项目画像拆解（预算/标的物/资质门槛/时间线）、采购方历史采购与偏好供应商分析、竞争对手预测、同类项目历史成交价参考、废标红线与限制性条款排查、投标决策建议。基于全网招中标历史数据输出标前分析报告。即使用户没有提到「标前分析」，只要涉及投标前调查、项目背景排查、该不该投等需求，都应使用本SKILL。
metadata: { "openclaw": {"requires": {"env":["ZLBX_API_KEY"]},"primaryEnv": "ZLBX_API_KEY"}}
---

# 标前分析助手 · 投标之前先查这一份

投标前的**标前尽调**一站式完成：项目画像、采购方历史、竞争格局、价格基准、风险清单，五步产出一份标前分析报告，替你省掉半天人工排查。

## 你可以这样问

- 「投标前帮我做个标前分析（附公告链接）」
- 「这个项目的采购方背景和历史采购情况怎么样」
- 「帮我排查这个标有没有废标红线和限制性条款」
