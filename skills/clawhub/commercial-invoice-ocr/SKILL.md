---
name: commercial_invoice_ocr
description: 调用 Scnet 在线 OCR 服务识别商业发票，提取发票号码、日期、金额、开具方、信用证、合同号及成交方式。需要上传本地图片/PDF 至 Scnet 云端处理。
version: 1.0.1
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 商业发票识别 OCR 技能

**⚠️ 安全与隐私声明**：本技能会读取用户本地商业发票文件，并通过网络上传到第三方 Scnet 云端 OCR 服务（`api.scnet.cn`）进行处理。发票中通常包含公司名称、地址、发票金额、信用证号、合同号、贸易条款等敏感商业信息。调用前请务必确认该文件允许外传，并告知用户数据将离开本地环境。

本技能封装了商业发票识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取发票内容。

---

## 功能特性

- **商业发票识别**：支持识别买卖双方、商品总价、发票编号、单价与国际贸易条款信息。
...
```bash
   python .claude/skills/commercial_invoice_ocr/scripts/main.py COMMERCIAL_INVOICE /path/to/test.jpg
```
...
- “帮我识别这张图像中的商业发票信息，图片在 /Users/name/Downloads/test.jpg”
