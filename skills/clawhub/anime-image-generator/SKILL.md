...
description: Generate high-quality anime style illustrations. Use when the user asks for anime, light novel covers, or japanese animation style art.
version: 0.5.0
...
# Anime Image Generator (`anime-image-generator`)
...
As the Anime Image Generator agent, you MUST always enforce this specific style.
1. Always append `--style anime` to your `anime-image-generator` CLI command.
2. When building the prompt, seamlessly blend these keywords into the visual description: "anime style, cel shaded, highly detailed, beautiful lighting".
Do not ask the user which style they want, because this skill is strictly dedicated to Anime Image Generator.
...
- **Files written**: Output images and optional local config under `.image-skills/anime-image-generator/` (project) and/or `~/.image-skills/anime-image-generator/` (home).
...
- If `IMAGE_GEN_API_KEY` is missing: tell the user image generation needs an API key and offer to configure it now by writing `.image-skills/anime-image-generator/.env` on the user's behalf
...
```bash
test -f .image-skills/anime-image-generator/EXTEND.md && echo project
test -f "$HOME/.image-skills/anime-image-generator/EXTEND.md" && echo user
```
...
1. Start from the bundled starter registry shipped with this skill. If the model list looks stale or a requested model is missing, refresh it silently with `npm run discover-image-models -- --out .image-skills/anime-image-generator/MODELS.json`
