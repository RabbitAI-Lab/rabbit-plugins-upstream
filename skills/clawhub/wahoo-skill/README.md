...
`schema/wahoo_db_schema.sql` defines these tables:
...
- **`records`** — 1 Hz time-series rows per workout: power, cadence, HR, GPS, elevation, speed.
- **`laps`** — lap splits with aggregate power/HR/distance/duration per lap.
- **`device_info`** — sensors and devices detected in the FIT file (manufacturer, product, serial, battery status).
- **`zones`** — power and HR zone thresholds extracted from the FIT session.
...
clawhub publish . \
  ...
  --version 0.1.8 \
  --changelog "v0.1.8: full FIT capture — records, device_info, zones, session/lap gaps"
```
