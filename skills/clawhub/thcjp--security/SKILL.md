...
summary_zh: GoPlus AgentGuard安全扫描，支持定时巡逻、漏洞发现、Webhook通知与结果检查。GoPlus AgentGuard驱动的安全扫描工具，提供代码与依赖安全检查.
  支持定时巡
license: MIT
description: GoPlus Agen。支持自动化配置和灵活的参数设置，适适配多种工作环境，增强工作效率。GoPlus安全扫描工具。支持自动化配置和灵活的参数设置，适用于多种工作场景，提升工作效率和准确性。GoPlus。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。 功能涵盖: scan。
