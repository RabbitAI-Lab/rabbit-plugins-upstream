...
PROFILES_ENDPOINT = f"{API_BASE}/properties/query"
...
# Semantic presets for common soil-property tasks.
PRESETS = {
    "agronomy": {
        "description": "Topsoil chemistry + texture for general agronomy",
        "properties": ["phh2o", "soc", "clay", "sand", "silt", "nitrogen"],
        "depths": ["0-5cm", "5-15cm", "15-30cm"],
    },
    "topsoil": {
        "description": "Topsoil (0-30cm) pH + SOC + texture",
        "properties": ["phh2o", "soc", "clay", "sand", "silt"],
        "depths": ["0-5cm", "5-15cm", "15-30cm"],
    },
    "water-holding": {
        "description": "Volumetric water content at field capacity / wilting point",
        "properties": ["wv0010", "wv0033", "wv1500", "clay", "sand", "soc"],
        "depths": ["0-5cm", "5-15cm", "15-30cm", "30-60cm"],
    },
    "carbon-stock": {
        "description": "Organic carbon stock + supporting properties",
        "properties": ["ocs", "soc", "bdv", "cfvo", "nitrogen"],
        "depths": ["0-5cm", "5-15cm", "15-30cm", "30-60cm"],
    },
}

# Nominatim fallback chain (mirrors osm-data-download / nasa-power-download)
NOMINATIM_ENDPOINTS = [
    ...
    for endpoint in NOMINATIM_ENDPOINTS:
        try:
            r = requests.get(endpoint, params=params,
                headers={"User-Agent": "soilgrids-download/0.2.0",
...
# ── API Functions ──────────────────────────────────────────────────────────────
def fetch_soilgrids_point(properties, lat, lon, depths=None, verbose: bool = False):
    ...
    if verbose:
        print(f"[verbose] GET {PROFILES_ENDPOINT} params={params} (timeout=60s)",
              file=sys.stderr)
    try:
...
def fetch_soilgrids_bbox(properties, bbox, depths=None, verbose: bool = False):
    """Fetch soil data using a single representative point in the bbox.

    The SoilGrids v2.0 API is point-based (lat/lon), not bbox-based.
    For "bbox" queries we use the centre of the bbox. To get a real
    grid, the user should download the SoilGrids VRTs (not in scope
    for this script).
    """
    west, south, east, north = bbox
    params = {
    ...
    if verbose:
        print(f"[verbose] GET {PROFILES_ENDPOINT} params={params} (timeout=120s)",
              file=sys.stderr)
    try:
...
def cmd_query(args):
    ...
    # Expand preset (overrides --property when set)
    if args.preset:
        if args.preset not in PRESETS:
            raise ValueError(
                f"Unknown preset {args.preset!r}; available: {', '.join(sorted(PRESETS))}"
            )
        properties = list(PRESETS[args.preset]["properties"])
        if not args.depth:
            depths = list(PRESETS[args.preset]["depths"])
        else:
            depths = validate_depths(args.depth)
        print(f"Using preset {args.preset!r}: {PRESETS[args.preset]['description']}")
    ...
    _vlog(f"resolved: properties={properties} depths={depths} preset={args.preset}")

    # Resolve spatial input
    place_info = None
    bbox = None
    if args.place:
        ...
        print(f"  Point: ({lat:.4f}, {lon:.4f})  OSM {place_info['osm_type']}/{place_info['osm_id']}")
    elif args.lat is not None and args.lon is not None:
        validate_latlon(args.lat, args.lon)
        lat, lon = args.lat, args.lon
        _vlog(f"using --lat={lat} --lon={lon}")
    elif args.bbox:
        ...
        _vlog(f"bbox centre -> lat={(south+north)/2}, lon={(west+east)/2}")
    else:
        raise ValueError("Provide one of --place / --lat+--lon / --bbox")

    # Fetch
    if bbox:
        if depths:
            print(f"  Depths: {', '.join(depths)}")
        data = fetch_soilgrids_bbox(properties, bbox, depths, verbose=verbose)
        records = parse_point_response(data, properties,
                                       (bbox[1] + bbox[3]) / 2,
                                       (bbox[0] + bbox[2]) / 2)
    else:
        ...
        if depths:
            print(f"  Depths: {', '.join(depths)}")
        data = fetch_soilgrids_point(properties, lat, lon, depths, verbose=verbose)
        records = parse_point_response(data, properties, lat, lon)
    _vlog(f"parsed {len(records)} record(s)")
    ...
    if args.qa:
        qa = {
            ...
            "property_count": len(set(r.get("property") for r in records)),
            "depth_count": len(set(r.get("depth") for r in records)),
            "missing_count": sum(1 for r in records if r.get("mean") is None),
            "output": output_path,
        }
        qa_path = os.path.splitext(output_path)[0] + ".qa.json"
        with open(qa_path, "w", encoding="utf-8") as f:
            json.dump(qa, f, ensure_ascii=False, indent=2)
        print(f"QA: {qa_path}  ({qa['record_count']} rows, {qa['property_count']} properties)")
        _vlog(f"wrote QA summary to {qa_path}")


def cmd_list_presets(args):
    """List available soil-property presets."""
    print("=" * 70)
    print("SoilGrids - Property Presets")
    print("=" * 70)
    for name, info in PRESETS.items():
        print(f"\n  {name}: {info['description']}")
        print(f"    properties: {', '.join(info['properties'])}")
        print(f"    depths: {', '.join(info['depths'])}")
    print("\n" + "=" * 70)
...
Examples:
  ...
  %(prog)s list-depths
    ...
    q.add_argument("--preset", choices=sorted(PRESETS.keys()),
                   help="Use a property preset (agronomy/topsoil/water-holding/carbon-stock)")
    q.add_argument("--property", default="phh2o",
                   help="Comma-separated property names (ignored when --preset set)")
    q.add_argument("--place", help="Place name (e.g. '北京市朝阳区'); resolved via Nominatim")
    ...
    q.add_argument("--depth", type=lambda s: [d.strip() for d in s.split(",")],
                   help="Comma-separated depth layers (default: all / preset default)")
    ...
    q.add_argument("--qa", action="store_true",
                   help="Write a QA summary JSON next to the output")
    q.add_argument("--verbose", "-v", action="store_true",
                   help="Print detailed progress to stderr (URL, response codes, "
                        "byte counts, record counts, etc.)")
    ...
    lpr = subparsers.add_parser("list-presets", help="List available property presets")
    lpr.set_defaults(func=cmd_list_presets)
