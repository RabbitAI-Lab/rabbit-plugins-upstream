const DEFAULT_BASE = process.env.RY_DRINK2_API_BASE || 'http://192.168.0.66:9302';
/** 会话绑定门店 ID（user-system chat.send 写入），优先级最高，LLM 无法覆盖 */
const FORCED_SHOP_ID = String(process.env.RY_DRINK2_FORCED_SHOP_ID || '').trim();
/** 租客 ID（t_merchant.id），/merchant/{id}/info、/menus 路径用此值 */
const DEFAULT_PLATFORM_TENANT_ID = String(process.env.RY_DRINK2_PLATFORM_TENANT_ID || '').trim();
/** 仅本地调试可设 RY_DRINK2_SHOP_ID */
const DEFAULT_SHOP_ID = String(process.env.RY_DRINK2_SHOP_ID || '').trim();
const DEFAULT_SAAS_ID = String(process.env.RY_DRINK2_SAAS_ID || '').trim();
const DEFAULT_TENANT_ID = String(
  process.env.RY_DRINK2_TENANT_ID || DEFAULT_PLATFORM_TENANT_ID || '',
).trim();
const DEFAULT_LINK_PHONE = String(process.env.RY_DRINK2_LINK_PHONE || '').trim();
const DEFAULT_PAYMENT_LINK_BASE = String(
  process.env.RY_DRINK2_PAYMENT_LINK_BASE || 'https://claw.jscitylife.cn:43003',
).trim();
 ...
 * 优先级：纯数字参数 > store-{id}-slug > 环境变量 RY_DRINK2_SHOP_ID（仅调试）
 */
