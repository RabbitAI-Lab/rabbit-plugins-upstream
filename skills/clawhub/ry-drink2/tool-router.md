...
| 环境 | 基址 | 说明 |
|------|------|------|
| 默认（内网直连，推荐） | `http://192.168.0.66:9302` | OpenClaw 与 user-biz 同网段时直连，少一跳、不依赖 Gateway 路由 |
| 经 Gateway（可选） | `http://192.168.0.66:43000/user-biz` | 仅当 9302 不可达或须走网关负载均衡时使用 |

环境变量：`RY_DRINK2_API_BASE` 覆盖默认基址；`RY_DRINK2_SAAS_ID` / `RY_DRINK2_PLATFORM_TENANT_ID` / `RY_DRINK2_FORCED_SHOP_ID` / `RY_DRINK2_LINK_PHONE` 由平台按会话动态注入，**勿在 Nacos 写死 saas-id、tenant-id**。
...
Handler 优先使用 OpenClaw 环境变量 `RY_DRINK2_FORCED_SHOP_ID`（user-system 按会话 merchantId 写入）；禁止 LLM 改用其他 shopId。

## 部署

将整个 `ry-drink2/` 目录同步到 OpenClaw workspace：

```
/home/node/.openclaw/workspace/skills/ry-drink2/
├── SKILL.md
├── tools.json
├── skill.json
├── tool-router.md
...
安装后在商家端「已安装技能」确认 **ry-drink2 已启用**（非「已停用/待部署」）。
