...
**禁止**自行拼接或猜测其他 API 路径；**仅**使用 `buildPaymentLink` Tool 或 `POST /aiemployees/dining/payment/link`。
...
- **saasId** = `t_store_sync.saas_id`，由 user-system 每次 chat.send 按当前门店从库解析写入 `RY_DRINK2_SAAS_ID`，**不在 Nacos 固定配置**。
- 后端在 chat.send 时注入 `RY_DRINK2_PLATFORM_TENANT_ID` 与 `RY_DRINK2_FORCED_SHOP_ID`；Handler 会自动选用正确 ID。

API 基址：`http://192.168.0.66:9302`（内网直连 user-biz，Tool 内部；环境变量 `RY_DRINK2_API_BASE` 可覆盖）
...
| **reduceOrder** | **减餐（单道菜）** |
| cancelOrder | 取消整单 |
| buildPaymentLink | 生成付款链接（买单/付款/结账） |
