---
name: amazon-asin-audit
description: >
  Amazon ASIN 运营体检 Skill：综合商品详情与评论证据，输出问题、证据与运营优先级。
  仅用于单 ASIN 整体诊断，不用于广告投放、库存或利润核算。Use when the user asks
  for an ASIN audit, listing diagnosis, 亚马逊运营体检、ASIN诊断、商品问题排查。
  Requires an ARI API key (ari_live_*).
slug: asin-audit
displayName: Amazon ASIN 运营体检
version: 1.4.7
summary: 商品详情与评论证据驱动的一键运营诊断
license: MIT
---
...
- 本入口只执行 `audit/full`，输出模板固定为 `ops_audit`。
