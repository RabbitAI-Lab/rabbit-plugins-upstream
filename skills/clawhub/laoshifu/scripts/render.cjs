...
function chartToFlat(chart, currentYear) {
    ...
    currentYear = currentYear || chart.calculationContext?.currentYear || new Date().getFullYear();
    const civilSolarDate = bz.calculation?.civilSolarDateTime || `${bi.year}-${String(bi.month).padStart(2, '0')}-${String(bi.day).padStart(2, '0')} ${String(bi.hour).padStart(2, '0')}:${String(bi.minute).padStart(2, '0')}`;
    const birthYear = Number(civilSolarDate.slice(0, 4));
    const virtualAge = calcVirtualAge(birthYear, currentYear);
    // ============ META ============
    out['meta.solar_date'] = civilSolarDate;
    if (zw.lunarDate) {
    ...
    out['ziwei.ming_zhu'] = zw.mingZhu || '-';
    out['ziwei.shen_zhu'] = zw.shenZhu || '-';
...
function analysisToFlat(analysis) {
    ...
    // User-facing judgment strength; keep numeric model scores internal.
    const certainty = analysis.certainty || analysis.confidence;
    if (certainty) {
        for (const k of ['bazi', 'ziwei', 'consistency', 'stability']) {
            out[`certainty.${k}_level`] = certainty[`${k}_level`] || '-';
        }
        out['certainty.note'] = certainty.note || '-';
    }
    return out;
}
...
function main() {
    ...
    if (args.internal !== 'true') {
        throw new Error('旧技术海报仅限内部审计；必须显式传 --internal=true，且不得交付给用户');
    }
    ...
    const chart = JSON.parse(fs.readFileSync(args.chart, 'utf-8').replace(/^\uFEFF/, ''));
    const analysis = args.analysis ? JSON.parse(fs.readFileSync(args.analysis, 'utf-8').replace(/^\uFEFF/, '')) : {};
