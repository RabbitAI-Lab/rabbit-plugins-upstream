---
name: meta-calibrated-autonomy
version: 1.0.0
description: |
  由 model-distillation 从教师技能 calibrated-autonomy 蒸馏并增强的超越型元技能，
...
> 由 `model-distillation` 从教师技能 **calibrated-autonomy** 蒸馏并增强生成。
> 生成时间：2026-07-23 08:25:03 ｜ 蒸馏机制：跨模型蒸馏（见 meta-evolver 北极星策略）
...
## 已知限制（来自教师蒸馏 + 元进化补充）
- 并从结果中学习收紧阈值，让 agent 在"自主"与"可控"之间校准，而非无脑全自主。构成北极星
  "超越一线大模型"的元之终局能力之一（下一代可信自主域）。
agent_created: true
visibility: public
---
...
## 集成（超越无脑自主）
- 接 `super-agent-loop`：每个待执行节点先过校准器再决定 ACT/DEFER。
- 接 `safety-guardrails`：critical 判定与护栏风险分级对齐。
- 接 `agent-society-protocol`：DELEGATE 的目标专家由社会投标选出。
