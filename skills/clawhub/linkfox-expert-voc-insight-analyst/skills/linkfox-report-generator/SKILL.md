...
7. **数据源标注（必须）**：每个含统计/度量数字的 `content-section` 末尾必须有 `<div class="data-source">`，列出所有贡献数据的 skill 短名；若含 Python 预计算的派生值，还须追加 `.ds-computed` 子块写明指标名与计算公式。纯定性分析/建议章节可省略。详见 `analysis-layouts.md § 16`。
...
**产物**：一份 JSON（stdout 或文件均可），包含报告所需的全部派生指标。后续 Step 3 写 HTML 时，统计数字**只从这份 JSON 里取**；来源与公式在 section 末尾的 `data-source` / `.ds-computed` 中标注。
