# Environmental Impact Assessment (geoskill-environmental-impact-assessment)

> Combines normalized, weighted pressure factors into a composite impact index, estimates cumulative effects with an independent-probability model, and classifies impact into 5 grades by thresholds. Outputs impact-index and impact-grade GeoTIFFs.

---

## 1. Overview

Four pressure factors (pollution, land-use change, noise, and habitat fragmentation) are each min-max normalized and combined by sensitivity weights (0.30/0.25/0.25/0.20); cumulative effects use the independent-probability model C = 1 − Π(1−Ii), ensuring the multi-project combination never exceeds 1 and is ≥ any single project; the final index = 0.5 × weighted overlay + 0.5 × cumulative effect, and is classified into five grades — negligible/minor/moderate/significant/severe — at thresholds 0.1/0.3/0.5/0.7. Use cases: EIA of construction projects, plan-level EIA, and cumulative environmental impact screening.
...
python geoskill-environmental-impact-assessment.py --bbox 116 39 117 40 --synthetic --output-dir ./out
```
