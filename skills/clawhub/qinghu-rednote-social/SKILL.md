---
name: qinghu-rednote-social
description: 青虎AI 小红书社媒运营：结合小红书热搜榜、关键词搜索笔记、笔记内容与评论、达人主页与作品数据，完成「爆款笔记拆解 → 种草痛点提取 → 优质 KOC/KOL 筛选」，搭建高效种草与社媒矩阵方案，并可生成小红书种草码。当用户要做小红书运营、拆解爆款笔记、找种草选题、筛 KOC/KOL、看笔记评论痛点、搭内容矩阵时必须触发。关键词：青虎AI、小红书、种草、笔记、KOC、KOL、达人筛选、热搜、痛点、社媒矩阵、种草码。
user-invocable: true
...
小红书种草三步走：**拆爆款笔记 → 提痛点 → 选对的 KOC/KOL。**

小红书和抖音的差别要记住：**这里是「搜索 + 收藏」驱动**，笔记的长尾价值高，所以选题要能被搜到，内容要值得收藏。

## 何时触发

- 「小红书上什么笔记火」「帮我拆解这篇爆款笔记」
- 「找一批适合种草的 KOC/KOL」
- 「用户在小红书上吐槽什么」「痛点是什么」
- 「做一套小红书种草方案」
...
| 工具 | 中文名 | 用在哪一步 |
| --- | --- | --- |
| `hot_rednote_rank` | 热搜榜数据 | 站内热点，选题起点 |
| `rednote_search_notes` | 关键词搜索笔记 | 按行业/产品词找对标笔记（主力） |
| `rednote_note_content` | 查笔记数据 | 单篇笔记的正文与互动数据 |
| `rednote_note_comments` | 查笔记评论 | 评论区痛点与用户语言（种草文案的金矿） |
| `rednote_user_info` | 查达人数据 | 博主的粉丝与互动量级 |
| `rednote_user_notes` | 查主页作品数据 | 博主的内容方向与稳定性 |
| `rednote_publish_code` | 小红书种草码 | 生成种草码（按 type 区分） |

> 小红书工具的入参是**链接**：笔记链接（`rednote_note_content` / `rednote_note_comments`）、用户主页链接（`rednote_user_info` / `rednote_user_notes`）。
> 小红书的互动结构里**收藏量的权重很高**——收藏多说明内容有长期实用价值，是判断优质笔记比点赞更可靠的信号。

## 工作流程

### 1) 选题：热搜 + 关键词双线

`hot_rednote_rank` 看站内热点，`rednote_search_notes` 按产品词/场景词搜对标笔记。
**小红书选题要能被搜到**：优先做「XX 怎么选」「XX 测评」「XX 平替」这类有搜索需求的题，而不是纯追热点。

### 2) 拆爆款笔记：`rednote_note_content`

看结构：
- **标题**：有没有明确的利益点或身份标签；
- **封面**：文字信息量与视觉风格；
- **正文结构**：痛点开场 → 方案 → 效果 → 引导；
- **互动结构**：点赞 vs 收藏 vs 评论的比例——**收藏率高的是干货型，点赞率高的是情绪型**，两种要用不同打法复刻。

### 3) 提痛点：`rednote_note_comments`

评论区的重复提问和吐槽就是种草文案的开场白。**直接用用户原话，比自己编的文案转化高得多。**

### 4) 筛 KOC/KOL：`rednote_user_info` + `rednote_user_notes`

不要只看粉丝数，看三件事：
1. **内容垂直度**：主页作品是不是集中在你的品类；
2. **互动质量**：近期笔记的互动率是否稳定（不是靠一篇爆款撑门面）；
3. **商业化密度**：广告太密的账号，粉丝已经疲劳。

KOC（几千到几万粉）适合铺量做搜索卡位，KOL 适合造声量，**方案里要说清各投多少**。

### 5) 出种草方案

选题清单 + 笔记结构模板 + 达人分层名单 + 投放节奏。需要种草码时用 `rednote_publish_code` 生成。

## 输出

- **选题清单**：能被搜到的选题 + 依据 + 建议内容形式。
- **爆款笔记结构模板**：标题公式、封面要点、正文分段、引导话术。
- **痛点素材库**：来自评论区的用户原话（可直接用作文案）。
- **达人分层名单**：KOC / KOL 分开，附粉丝量、垂直度、互动率、建议合作方式。
- **投放节奏建议**：铺量与造势的比例、节奏安排。
- 名单/明细超过 10 条按「数据交付」导出。
...
- 本技能做**内容与达人策略**，不做商务对接、报价谈判与结算。
- 不生成图文素材与视频，那属于 LinkPix / qhkit 系列技能。
- 小红书对硬广有平台规则限制，方案里要提醒用户按平台规范做报备与标注。
- 笔记与评论抓取有条数上限，结论要说明样本量。

## 相关技能

同系列的其他「青虎AI」技能（按需转交，不要在本技能里硬做）：

- **抖音-社媒运营专家 / B站-社媒运营专家**：跨平台内容矩阵
- **LinkPix 系列技能**：笔记封面与配图制作
