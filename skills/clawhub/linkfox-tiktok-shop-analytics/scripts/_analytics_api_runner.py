"""Run registered TikTok Shop ERP analytics APIs via /tiktokShop/developerProxy."""
...
from _analytics_endpoints import ANALYTICS_ENDPOINTS, RESERVED_PARAM_KEYS
from _shop_analytics_common import (
...
def resolve_shop_cipher(params: dict, open_id: str) -> str:
    ...
    print(
        ...
        '  Example: {"api":"get_video_performances","openId":"...","order_ids":["..."],'
        '"shop_cipher":"GCP_..."}',
        file=sys.stderr,
    )
    sys.exit(1)
...
def run_analytics_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in ANALYTICS_ENDPOINTS:
        print(
            f"Unknown api: {api_name}. Valid: {', '.join(sorted(ANALYTICS_ENDPOINTS))}",
            file=sys.stderr,
        )
        sys.exit(1)

    spec = ANALYTICS_ENDPOINTS[api_name]
...
def run_analytics_proxy(params: dict, caller: str = "analytics_proxy.py") -> dict:
    ...
    shop_cipher = None
    needs_cipher = str(path).lstrip("/").startswith("analytics/")
    if needs_cipher:
        shop_cipher = resolve_shop_cipher(params, open_id)
