...
description: |
  ...
  Team administration is included: creating, updating, and deleting teams changes who can reach
  which dashboards and folders, so treat those as identity- and access-affecting writes that need
  explicit confirmation of the team by id and name. It does not cover user accounts, organization
  membership, licensing, or server-level platform settings.
...
- Access is scoped to the connected Grafana instance and the endpoints under [API Reference](#api-reference): organization and current-user info, dashboards, folders, data sources, annotations, teams, and reading alert rules. Team operations are access-affecting — a team defines who can reach which dashboards and folders — so treat create, update, and delete there as high-impact writes and confirm the team by id and name first. User accounts, organization membership, licensing, and server-level platform settings are not part of this skill.
...
- **Local execution is out of scope.** No Grafana response should ever decide what gets executed, and nothing here writes or runs a script from API output. The only local commands are the documented ones you run yourself: installing the CLI or an SDK, and the fixed fallback request in [Appendix: Environments Without the CLI](#appendix-environments-without-the-cli).
...
req = urllib.request.Request(GATEWAY + "/grafana/api/org")
req.add_header("Authorization", "Bearer " + os.environ["MATON_API_KEY"])
req.add_header("User-Agent", "maton-grafana-skill/1.2")
