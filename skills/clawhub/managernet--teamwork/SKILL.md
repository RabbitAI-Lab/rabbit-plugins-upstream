...
## 数据治理规范（2026-08-09 全智能体通用）

所有智能体/团队/子 agent 必须遵守：

1. **审计即清理**：每次审计/统计/优化时，同步清理过期/无效/证伪内容，移入 `专项研究/归档/`，不得堆积垃圾数据。
2. **去重常态化**：新增采集后执行文件名/标题去重，重复文件保留最新版，旧版归档。
3. **错误/临时文件归档**：`*_错误日志.md`、`*_抓取日报.md`、`*_临时*.md` 等临时文件不留在生产目录，归档处理。
4. **研究成果归档**：完成的研究成果移入对应产品目录 `归档/`；待确认项集中 `待Michael人工确认/`，完成后立即移入 `archive/`。

详细执行流程见 skill：`vault-data-governance`
...
- P1 是侧线规模对齐基准：所有 P2-P8 规模对齐任务以 P1 为前置依赖，P1 完成后才能并行派生其余规模对齐
