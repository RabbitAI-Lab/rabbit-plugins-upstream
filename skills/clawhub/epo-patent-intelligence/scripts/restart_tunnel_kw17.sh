#!/bin/bash
# Restart tunnel for KW17
# Usage: ./restart_tunnel_kw17.sh
...
nohup python3 -m http.server 8080 > /tmp/http_server_kw17.log 2>&1 &
sleep 3
...
nohup cloudflared tunnel --no-autoupdate run --token "$TUNNEL_TOKEN" --url http://localhost:8080 > /tmp/tunnel_kw17.log 2>&1 &
sleep 5

echo ""
echo "═══════════════════════════════════════════════════"
echo "  ✅ PATENT REPORT KW17 DEPLOYMENT COMPLETE"
echo "═══════════════════════════════════════════════════"
echo ""
echo "📊 Local Server:  http://localhost:8080"
echo "🌐 Public URL:   https://hermes.sqncr.ai/Patent_report_kw17"
echo ""
