...
mbs base-table field update --doc-id <DOC_ID> --name orders_large --field amount --formatter "#,##0.00" --font-size 12 --font-family Arial --verify
...
**Base table field style.** For a Base-backed column formatter, color, background, font size/family, or width, use `mbs base-table field list` before `mbs base-table field update`. Prefer native `--table-id --field-id`; `--name --field` remains a compatibility resolver. Width is a field-level property persisted as `field.property.style.width`; font size and family are set with `--font-size` (points, greater than 0) and `--font-family` and persist as `field.property.style.fontSize` / `fontFamily`. Do not use `views[*].field_widths` or the Sheet-only `style columns-width` route for Base targets. Use `--verify` and inspect the returned field property. The legacy `db-table field batch-update` command is compatibility-only.
...
**Styles.** Use `mbs style beautify` for agent-friendly report/table polish. It reads metadata first, classifies columns from Chinese/English headers plus sample values, applies Excel worksheet styles, and writes Base-backed field style metadata through the native field-update route when possible. Use first-class `excel-worksheet style` commands for explicit freeze panes, filters, widths, heights, cell style batches, gridlines, filter values, conditional formats, and worksheet style planning/apply. Cell style payloads (`--style`) support `font_size`/`font_family` alongside `bold`, `font_color`, `bg_color`, `horizontal`, `wrap_text`, and `format`; on Base worksheets they route to per-field style updates through native `/table/field/update`, resolving `field_id` from `read_headers` (with a native table-read fallback when the backend does not yet return `field_id`). Worksheet column and row dimensions should use explicit pixel values such as `--width 140px` and `--height 32px`; bare numeric values remain legacy-compatible but must not be generated for new commands. See [references/charts-formatting.md](references/charts-formatting.md).
