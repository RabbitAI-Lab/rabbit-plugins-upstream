...
Do not infer the model from a worksheet's name, a `db-table` alias, or its
...
**Object model.** Use `excel-worksheet read` and `excel-worksheet range` only for Sheet targets. Use `excel-table` for Sheet-backed tables. Use current `db-table` commands for Base discovery/creation and `base-table` record/field/formula commands for Base mutation. Cell `formula` commands and worksheet calculation are Sheet-only. SQL Config is a distinct raw-SQL producer. Details: [references/base-mode-verification.md](references/base-mode-verification.md)
...
**Write priority.** For a Sheet target, use `sheet update-data-keep-headers` only for a full data-row refresh that must preserve Sheet headers, styles, and formula columns; use `excel-worksheet range write` for exact cells. For a Base target, use `base-table record replace` or `base-table record upsert` with resolved `table_id`, `field_id`, and record identity. `db-table create-from-range`, `db-table create-from-query`, and `db-table create` create Base tables; they are not record-replace/upsert APIs. SQL Config owns raw query materialization.
...
**Verify after every write.** Use `--verify` where available. Verify against the target model: read an A1 range and run `excel-worksheet check-error` for Sheet; inspect record and Formula execution evidence for Base; verify stored raw SQL plus the materialized result for SQL Config. `db-table sample` alone is not Formula execution evidence. For a full refresh reporting `written_unverified` with `error: null`, readback comparison is mandatory before any failure claim. When the user required the original worksheet, run `workbook list-worksheets` after the write and confirm its recorded `gid` is unchanged. See [references/base-mode-verification.md](references/base-mode-verification.md).
