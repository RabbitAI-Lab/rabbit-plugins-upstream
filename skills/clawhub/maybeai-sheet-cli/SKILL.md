...
**Base table field style.** For a Base-backed column formatter, color, background, or width, use `mbs base-table field list` before `mbs base-table field update`. Prefer native `--table-id --field-id`; `--name --field` remains a compatibility resolver. Width is a field-level property persisted as `field.property.style.width`; do not use `views[*].field_widths` or the Sheet-only `style columns-width` route for Base targets. Use `--verify` and inspect the returned field property. The legacy `db-table field batch-update` command is compatibility-only.
