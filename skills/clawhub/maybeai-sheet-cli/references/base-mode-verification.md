...
Current CLI versions can inspect a known Base table:

```bash
mbs db-table metadata --doc-id <DOC_ID> --name <TABLE_NAME> --include-headers --output json
mbs db-table field metadata --doc-id <DOC_ID> --name <TABLE_NAME> --output json
...
Use the P5 record commands with a field-ID JSON array:
...
Use the P5 Base Formula commands:
...
- `excel-worksheet range write/clear/set-formula/calculate`
- `sheet update-data-keep-headers`, `worksheet import --strategy replace`, or
  any row-2 template/preserve-formula behavior
- copy-range, cell Formula, row/column structural, chart, image, or Sheet
  formatting operations
- a legacy `db-table range set-formula` request

P5 defines `BASE_COLUMN_FORMULA_REQUIRED` for the legacy formula route and
`SHEET_ONLY_OPERATION` for a Sheet-only route invoked on Base. Treat those as
the intended guard contract, not as a substitute for checking current CLI/help
availability. Do not retry against the same Base target with a different `gid`,
a different range, or a keep-headers flag.
