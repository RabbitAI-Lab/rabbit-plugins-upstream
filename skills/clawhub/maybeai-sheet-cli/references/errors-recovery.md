...
1. `mbs excel-table schema` or `mbs base-table schema --table-id <TABLE_ID>`
...
5. if the error came from a legacy `=SQL(...)` cell, validate headers with `mbs excel-table schema` or `mbs base-table schema --table-id <TABLE_ID>` and simplify the SQL
