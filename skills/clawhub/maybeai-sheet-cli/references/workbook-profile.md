...
|---|---|---|
| `data_engine: sheet` | Excel-style worksheet | `excel-worksheet`, `excel-table`, A1/range operations |
| `data_engine: base` plus `table_id` | Base-backed table | `base-table` records, fields, Formula operations |
...
`base-table metadata`/`schema` after resolving the target.
...
3. For Sheet, read a bounded range:
   ...
   ```bash
   mbs excel-worksheet read --doc-id <DOC_ID> --worksheet-name Orders --range A1:H20 --output table
   ```
...
- Missing Base `table_id`: stop before a Base mutation and resolve the table by
  `mbs base-table metadata --name <TABLE_NAME>`.
