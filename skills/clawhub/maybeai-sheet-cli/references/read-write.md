...
### `base-table field metadata` / `field update`
...
```bash
mbs base-table field metadata --doc-id <DOC_ID> --name Orders --output json
mbs base-table field update --doc-id <DOC_ID> --name Orders --field amount --formatter "#,##0.00" --verify --output json
```
...
`--verify`, the command returns the updated field property. The legacy
