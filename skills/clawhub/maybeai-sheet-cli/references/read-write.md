...
A worksheet name, a `gid`, a Base-looking table name, and a `table` alias
...
mbs range read --doc-id <DOC_ID> --worksheet-name <SHEET> --range A1:G20 --output table
mbs table schema --doc-id <DOC_ID> --worksheet-name <SHEET> --table-id <PERSISTENT_TABLE_ID>
mbs table inspect --doc-id <DOC_ID> --worksheet-name <SHEET> --table-id <PERSISTENT_TABLE_ID>
mbs table sample --doc-id <DOC_ID> --worksheet-name <SHEET> --table-id <PERSISTENT_TABLE_ID> --limit 50
...
mbs table sample --doc-id <DOC_ID> --table-id <TABLE_ID> --limit 50
...
```bash
mbs table inspect --doc-id <DOC_ID> --name <TABLE_NAME> --include-headers --output json
mbs table schema --doc-id <DOC_ID> --table-id <TABLE_ID> --output json
mbs table sample --doc-id <DOC_ID> --table-id <TABLE_ID> --limit 50 --output table
...
5. Use `range write --gid <RECORDED_GID> --verify`, then read
   back the same footprint plus sentinels, run
   `formula recalculate --gid <RECORDED_GID>`, and run
   `range inspect --gid <RECORDED_GID>` when formula/result cells
...
`range write` for exact RAW text preservation instead of this
full-refresh command.

### `table insert` (Sheet-backed tables only)

Best when:

- appending row objects to a known Sheet-backed Excel Table
- the target table metadata is already known
- you can verify with `table read` or a targeted Sheet range read

For an `table`, resolve the persistent ID and current range through
`table inspect`, then build `rows.json` from the headers returned by
`table schema` or `read`. The input must be a non-empty array of objects,
...
mbs table read --doc-id <DOC_ID> --worksheet-name Orders --table-id <PERSISTENT_TABLE_ID> --limit 20 --output table
```

The JSON example's keys are illustrative: replace them with the target table's
actual headers. `table insert` does not have `--verify`. Do not reuse an
...
### Compatibility `table create`

Prefer canonical `mbs table create` with a workbook target for new Base table
creation. Keep this form when a legacy `--name`/`--columns` response is needed.
...
mbs table inspect --doc-id <DOC_ID> --name Orders --include-headers --output json
mbs table sample --doc-id <DOC_ID> --table-id <TABLE_ID> --limit 20 --output table
```

Use `table inspect --include-headers` when the next step needs exact header
...
confirms the registry adopted a Base-backed worksheet. Use `table create`
only for table-shaped data; use `worksheet create` or `range write` for workbook-layout reports.

### Compatibility `column list` / `field update`

Prefer canonical `mbs column rename` for field/header names,
`mbs column insert --field-type` when creating Base fields, and
`mbs column batch-update --updates` for supported Base field/schema changes.
The current `mbs column config` command is a resource-style alias: it requires
`--spec` and uses `--columns` (Sheet) or `--field` (Base). It is not the old
inline typed metadata editor.

Use this compatibility surface only for Base field style properties that the
canonical batch shape does not expose yet, such as formatter/font/color details.
...
`column batch-update` command is compatibility-only and is not part of
new examples.

### Compatibility `table create-from-range`

Prefer canonical `mbs table create-from-range` with a workbook target for new
workflows; this section documents the legacy flags and response shape.
...
mbs table inspect --doc-id <TARGET_DOC_ID> --name R_OrderLines_Store1 --output json
mbs table sample --doc-id <TARGET_DOC_ID> --table-id <TABLE_ID> --limit 20 --output table
```
...
### Compatibility `table create-from-query`

Prefer canonical `mbs table create-from-query` with a workbook target for new
workflows; this section documents the legacy flags and response shape.
...
mbs table inspect --doc-id <DOC_ID> --name S1_RevenueStructureInput --output json
mbs table schema --doc-id <DOC_ID> --table-id <TABLE_ID> --output json
mbs table sample --doc-id <DOC_ID> --table-id <TABLE_ID> --limit 20 --output table
```
...
Use `range set-formula` for ordinary worksheet formula writes.
Use `formula read`, `formula batch-set`, and `range lineage --range` for the
formula-focused Sheet workflow. For a Base formula, use `formula`
with a resolved `table_id` and `field_id` instead.

For direct one-cell calculation, `mbs formula calculate` and
`mbs range calculate` both call `/api/v1/excel/calc-formula`.
...
```bash
mbs range write --doc-id <DOC_ID> --worksheet-name <SHEET> --range A1:C3 --values values.json --verify
mbs range clear --doc-id <DOC_ID> --worksheet-name <SHEET> --range D1:F10
mbs table inspect --doc-id <DOC_ID> --worksheet-name <SHEET> --table-id <PERSISTENT_TABLE_ID>
mbs table insert --doc-id <DOC_ID> --worksheet-name <SHEET> --table-id <PERSISTENT_TABLE_ID> --rows rows.json
mbs range set-formula --doc-id <DOC_ID> --worksheet-name <SHEET> --cell E2 --formula '=SUM(B2:D2)'
```

Value handling:

- `range write` defaults to `RAW`; numeric-looking strings such as `"5.53%"` and `"9,007,000"` stay strings.
...
Use canonical `table` record operations with field-ID record JSON:
...
Current `table create`, `create-from-range`, and `create-from-query` create
...
result afterward. Use `mbs table create-from-query` only when the desired
...
- Sheet: targeted `range read`, then `range inspect`
...
Reject a route when a Base target receives a range, cell, or A1 row/column position,
