...
mbs db-table sample --doc-id <DOC_ID> --name <TABLE_NAME> --limit 50 --output table
mbs db-table read --doc-id <DOC_ID> --name <TABLE_NAME> --limit 100 --output table
```
...
Current `db-table create`, `create-from-range`, and `create-from-query` create
...
result afterward. Use `mbs db-table create-from-query` only when the desired
