...
```bash
mbs table create --target "https://www.maybe.ai/docs/spreadsheets/d/<DOC_ID>" --table-name cost_monthly --engine base --rows rows.json --verify
mbs table create-from-query --target "https://www.maybe.ai/docs/spreadsheets/d/<DOC_ID>" --table-name cost_monthly --sql-file <GOLD_SQL_FILE> --verify
...
template. Its desired mutation surface is canonical `table
replace-records`/`update`, `row` record operations, and `formula
compile`/`set`/`recalculate` with a Base `--target`. `table` remains a
compatibility fallback; use the leaf `--help` output to confirm flags and
follow the verification runbook.
...
mbs range set-formula --doc-id <DOC_ID> --worksheet-name <REPORT_WORKSHEET> --cell E2 --formula '=<FORMULA>'
