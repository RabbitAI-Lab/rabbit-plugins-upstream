...
```bash
mbs db-table create --doc-id <DOC_ID> --name cost_monthly --rows rows.json --verify
mbs db-table create-from-query --doc-id <DOC_ID> --name cost_monthly --sql-file <GOLD_SQL_FILE> --if-exists adopt --verify
mbs db-table metadata --doc-id <DOC_ID> --name cost_monthly --include-headers --output json
mbs db-table field metadata --doc-id <DOC_ID> --name cost_monthly --output json
mbs db-table sample --doc-id <DOC_ID> --name cost_monthly --limit 20 --output table
```
