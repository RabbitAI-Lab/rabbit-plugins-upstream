...
- If shape confirmation is needed after a batch raw-surface import, resolve one representative table with `base-table metadata`, then sample it by `table_id`.
...
mbs base-table sample --doc-id <TARGET_DOC_ID> --table-id <TABLE_ID> --limit 2 --output table
```
...
7. `mbs workbook list-worksheets --doc-id <DOC_ID>`
8. Sample only when needed for shape confirmation; use at most one representative `excel-table sample` or `base-table sample` after resolving its table ID
...
4. `mbs workbook list-worksheets --doc-id <DOC_ID> --output table`
...
5. If needed, resolve `base-table metadata --name <REPRESENTATIVE_TABLE_NAME>`, then use `mbs base-table sample --doc-id <DOC_ID> --table-id <TABLE_ID> --limit 2 --output table`
