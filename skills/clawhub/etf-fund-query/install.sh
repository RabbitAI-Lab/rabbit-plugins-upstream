...
usage() {
  cat <<'EOF'
Install and initialize the ETF fund query skill (etf-fund-query).
...
require_file "${SCRIPT_DIR}/references/catalog-etf.md" "请重新解压安装包，确保 ETF 参考文件完整。"
printf '安装包检查通过：%s\n' "$SKILL_NAME"
...
req = urllib.request.Request(
    base_url + "/skill/v1/etf/detail",
    data=json.dumps({"fundCodes": ["510300"]}).encode("utf-8"),
    headers={
