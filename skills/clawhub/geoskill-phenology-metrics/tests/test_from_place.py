"""Tests for phenology-metrics from-place (PHASE 1+ REFACTORED)."""
import os
import sys
import subprocess
...
def test_from_place_subcommand_in_help():
    out = subprocess.run(
        [sys.executable, os.path.join(SCRIPTS, "phenology-metrics.py"), "--help"],
...
def test_from_place_resolves_place_then_runs():
    ...
    out = subprocess.run(
        [sys.executable, os.path.join(SCRIPTS, "phenology-metrics.py"),
         "from-place", "--place", "北京市",
         "--start-date", "2024-01-01", "--end-date", "2024-06-30",
         "--output", os.path.join(os.environ.get("TEMP", "/tmp"), "phen_test.tif")],
        capture_output=True, text=True, timeout=120,
    )
    combined = out.stdout + out.stderr
    assert "from-place" in combined
    assert "PHASE 0 DISABLED" not in combined
    # 没有真实数据时应该有失败（任何非 0 退出都可接受）
