...
description: Read-only, normalized public-data reads of App Store applications through the ReplyNodes fetcher domain: app lookup by track id or bundle id, bounded term search, and related-app listings. Bearer workspace-key authentication or x402 v2 payment negotiation at the gateway, transparent bounded pages, normalized errors, and an explicit unsupported-capability matrix. Public reads only: no purchase, review, account, or other mutation exists, and no platform login material is involved.
version: 1.0.4
contract_version: v1
mode: readonly
auth: Bearer workspace key or x402 v2 negotiation at the gateway; the read layer itself carries no credential material
license: Apache-2.0
keywords: [app store, appstore-api, ios, read-only, public data, app metadata, fetcher]
...
Read-only, normalized public-data reads of App Store applications through the ReplyNodes fetcher domain: app lookup by track id or bundle id, bounded term search, and related-app listings. Bearer workspace-key authentication or x402 v2 payment negotiation at the gateway, transparent bounded pages, normalized errors, and an explicit unsupported-capability matrix. Public reads only: no purchase, review, account, or other mutation exists, and no platform login material is involved.
...
| --- | --- | --- | --- | --- |
| `get_app` | `appstore_get_app` | GET `/v1/appstore/apps/{id}` | `app` | `services/appstore-fetcher/internal/providerclient/client.go Client.App` |
...
| `get_similar_apps` | `appstore_get_similar_apps` | GET `/v1/appstore/apps/{id}/similar` | `similar` | `services/appstore-fetcher/internal/providerclient/client.go Client.Similar` |
...
- Hermes: Register each tool below as a function/tool definition; execute by issuing plain HTTPS GET requests with the Bearer key.
...
- This package is published on ClawHub as @replynodes-ai/replynodes-appstore version 1.0.4; moderation is recorded as clean. Public discovery is not claimed without registry inspect evidence.
