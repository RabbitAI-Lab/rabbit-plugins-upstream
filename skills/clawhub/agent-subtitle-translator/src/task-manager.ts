...
export class TaskManager {
  ...
  async compose(id: string, outputPath?: string, overwrite = false): Promise<TaskSnapshot> {
    ...
    try {
      const safeOutputPath = this.resolveOutputPath(task, outputPath);
  ...
  private resolveOutputPath(task: InternalTask, requestedPath?: string): string {
    const outputDirectory = path.join(this.dataDir, task.id, "output");
    const outputExtension = task.outputFormat ?? (task.inputFormat === "vtt" ? "srt" : task.inputFormat ?? "srt");
    const inputStem = path.basename(task.fileName, path.extname(task.fileName));
    const defaultName = `${safeFileName(inputStem)}.${safeFileName(task.targetLanguage)}.${outputExtension}`;
    const candidate = path.resolve(requestedPath ?? path.join(outputDirectory, defaultName));
    const relative = path.relative(outputDirectory, candidate);
    if (!relative || relative.startsWith("..") || path.isAbsolute(relative) || relative.includes(path.sep)) {
      throw new Error("可视化输出只能写入当前任务的 output 目录");
    }
    return candidate;
  }
