...
function portFlag(flags: Flags): number | undefined {
  const port = numberFlag(flags, "port");
  if (port !== undefined && (port < 1 || port > 65535)) {
    throw new Error("参数 --port 必须在 1 到 65535 之间");
  }
  return port;
}
...
function baseUrl(flags: Flags): string {
  if (flag(flags, "url")) {
    throw new Error("桥接只允许访问本机 visualizer，不支持远程 URL");
  }
  return `http://${DEFAULT_HOST}:${portFlag(flags) ?? DEFAULT_PORT}`;
}
...
async function main(): Promise<void> {
  ...
  if (command === "start") {
    if (flags.open === true || flag(flags, "host") || flag(flags, "url")) {
      throw new Error("visualizer 只支持本机监听和显式浏览器打开，不接受 --host、--url 或 --open");
    }
