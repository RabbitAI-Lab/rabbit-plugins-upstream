...
test("TaskManager follows the real Python pipeline and preserves visual state", async () => {
  ...
  try {
    ...
    const outputPath = path.join(dataDir, created.id, "output", "result.srt");
...
test("bridge compose keeps collision safety and supports explicit overwrite", async () => {
  ...
  const running = await startServer({ rootDir: ROOT, port: 0, dataDir });
  try {
    ...
    const outputPath = path.join(dataDir, created.id, "output", "bridge-result.srt");
    const bridgePath = path.join(ROOT, "dist", "src", "agent-bridge.js");
    const bridgeArgs = ["compose", "--port", new URL(running.url).port, "--task", created.id, "--output", outputPath];
...
test("bridge rejects remote visualizer URL overrides", async () => {
  const bridgePath = path.join(ROOT, "dist", "src", "agent-bridge.js");
  let error: { stdout?: string; stderr?: string } | undefined;
  try {
    await execFileAsync(process.execPath, [bridgePath, "status", "--url", "http://example.com"], { cwd: ROOT });
  } catch (caught) {
    error = caught as { stdout?: string; stderr?: string };
  }
  assert.ok(error);
  assert.match(`${error.stdout ?? ""}\n${error.stderr ?? ""}`, /不支持远程 URL/);
});

test("visualizer compose confines output to the task directory", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "subtitle-visualizer-output-boundary-"));
  try {
    const manager = new TaskManager({ skillRoot: ROOT, dataDir });
    await manager.init();
    const created = await manager.createTask({ filePath: FIXTURE, targetLanguage: "zh-Hans" });
    await waitFor(() => manager.getTask(created.id), (task) => task.status === "awaiting_translation");
    await manager.startBatch(created.id, 1);
    const responsePath = path.join(dataDir, "response.txt");
    await writeFile(
      responsePath,
    ...
    await manager.submitResponse(created.id, 1, responsePath, false);
    await assert.rejects(
      () => manager.compose(created.id, path.join(dataDir, "outside.srt")),
      /可视化输出只能写入当前任务的 output 目录/,
    );
  } finally {
    await rm(dataDir, { recursive: true, force: true });
  }
...
test("visualizer web surface is display-only and leaves task input to the Agent", async () => {
  ...
  assert.match(skill, /openclaw:[\s\S]*requires:[\s\S]*node[\s\S]*npm[\s\S]*python3/);
  assert.match(skill, /optional loopback-only visualizer/);
  assert.doesNotMatch(skill, /Mandatory Agent visualizer gate|visualizer:open|SUBTITLE_VISUALIZER_URL/);
...
test("visual state records a failed validation, retry, and ASS degradation", async () => {
  ...
  try {
    ...
    const outputPath = path.join(dataDir, created.id, "output", "karaoke.zh-Hans.ass");
    const completed = await manager.compose(created.id, outputPath);
    assert.equal(completed.status, "completed");
