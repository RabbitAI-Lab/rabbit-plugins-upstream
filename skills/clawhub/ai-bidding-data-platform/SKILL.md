---
name: ai-bidding-data-platform
description: 招投标大数据 AI 分析平台，用自然语言完成市场分析、商机研判与趋势预测：多维聚合统计（月/季/年/省份/行业/品牌）、Top采购单位/中标单位/中标品牌、历史中标价格走势、潜在中标候选预测。当用户提出招投标市场分析、行业商机简报、采购趋势、市场份额、谁在买谁在中标等开放式分析需求时，必须使用此SKILL。
---

# 招投标 AI 数据分析平台 - 自然语言驱动的招中标大数据分析

用自然语言完成招中标大数据分析。自动调度多维聚合、市场统计、趋势预测、潜在中标预测等接口，把“谁在买、谁在中标、价格怎么走、机会在哪”这类开放问题转成结构化分析报告。
...
本平台擅长把开放式分析问题自动拆解成数据接口调度，常见用法：

- **市场全景**："分析国产数据库市场，谁在采购、谁在中标" → `get_top_purchasers` + `get_top_suppliers` + `aggregate_bids_advanced`。
- **趋势研判**："这个行业近三年招标规模和价格趋势如何" → `aggregate_bids_advanced`（group_by:["month"] + compare_with:"yoy"）+ `get_price_trends`。
- **商机简报**："生成一份服务器行业本季度商机简报" → 多接口串联 + `get_top_brands`。
- **中标预测**："预判这个项目最可能的中标候选人" → `find_potential_bidders` + `find_competitors`。
...
| INSUFFICIENT_BALANCE / QUOTA_EXCEEDED | **判断 API Key 来源**：<br>① 来自 `~/.zlbx/config.json` 且 `source == "auto"` → 调用 `POST /web-api/auth/generate-device-sid`（带 `X-API-Key` Header）拿到 `sid`，输出充值链接 `https://ai.zhiliaobiaoxun.com/auto-login?sid=<sid>`，提示文案：「免费额度已用完，点击链接自动登录并充值；首次会引导绑定手机号，绑定即送 100 次」<br>② 来自 `$ZLBX_API_KEY` → 提示访问 `https://ai.zhiliaobiaoxun.com/?ch=s133` 手动登录充值（不输出自动登录链接） |
