...
当用户明确要求**生成或更新模块文档**时激活，必须包含明确的动作意图：
...
**中优先级触发**（需要确认用户意图）:
- "分析 admin-api 模块并生成文档"（必须包含"并生成文档"）
- "扫描模块生成文档"（必须包含"生成文档"）

**不触发的情况**（仅是询问或理解）:
- ❌ "分析这个模块"（没有明确要求生成文档）
- ❌ "理解这个模块的业务逻辑"（只是询问，不是生成文档）
- ❌ "这个模块是做什么的"（只是询问功能）

**激活前确认**: 如果用户的表述模糊，先确认："您是想让我生成这个模块的文档，还是只是想了解这个模块的功能？"
...
**目的**: 检查已存在的文档状态，**仅报告问题，所有修改操作必须获得用户明确确认**
...
### Step 0.6: 识别低质量文档（仅报告，绝不自动删除）

**目的**: 识别不符合要求的文档，**仅生成报告，绝不自动执行任何删除操作**
...
# 2. 生成报告（只报告，不删除）
Write-Host "发现 $($lowQualityDocs.Count) 个低质量文档："
foreach ($doc in $lowQualityDocs) {
    Write-Host "[报告] 低质量文档：$($doc.FullName)"
}
```
...
### 6. 文件读取失败处理
- 如文件读取失败，尝试使用不同的 PowerShell 命令（仅限标准文件操作命令如 `Get-Content`、`Test-Path`）
- 如果仍失败，尝试使用只读的文件读取工具（Python `open()` 或 Read 工具）
- ⚠️ 注意：任何 shell 回退仅限只读文件操作，**绝不执行任意命令或修改文件**
- 如果仍失败，等待 30 秒后重试
- 如果 3 次重试后仍失败，记录该文件到失败列表
- 在最终报告中标注无法访问的文件
...
解决:
1. 尝试使用不同的 PowerShell 只读命令（如 `Get-Content`、`Test-Path`）
2. 尝试使用 Python 的文件读取功能（如果可用）
3. ⚠️ 注意：所有 shell 回退仅限只读文件操作，绝不执行任意命令
4. 如果 3 次重试后仍失败，跳过该文件
5. 在最终报告中标注无法访问的文件
```
...
| 1.0.4 | 2026-07-16 | 安全加固：修复 Step 0.5/0.6 文档处理范围、收紧触发条件、添加 shell 回退安全约束、修复安全声明矛盾 |
