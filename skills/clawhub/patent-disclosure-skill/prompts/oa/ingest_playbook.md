...
```bash
python ${CLAUDE_SKILL_DIR}/tools/oa/ingest_playbook.py peek --path "本地书.pdf或.txt或.md"
```
...
```bash
python ${CLAUDE_SKILL_DIR}/tools/oa/ingest_playbook.py ensure-skill
```

脚本会：探测本机 skill 目录 → 已装则用；未装则请求 GitHub [`virgiliojr94/book-to-skill`](https://github.com/virgiliojr94/book-to-skill) 的最新 README 安装命令并自动安装；失败则降级 `npx --yes skills add virgiliojr94/book-to-skill`，再失败则 `git clone` 到 `~/.agents/skills/book-to-skill`。
...
```bash
python ${CLAUDE_SKILL_DIR}/tools/oa/ingest_playbook.py ingest \
...
告诉用户：手册路径、**未进入案例检索**、写答复时按缺陷 Read cheatsheet。不要把手册条目写成 `case_id`。
