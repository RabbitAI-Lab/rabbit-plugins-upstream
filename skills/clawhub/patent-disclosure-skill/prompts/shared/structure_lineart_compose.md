...
**脚本**：`tools/shared/structure_lineart_compose.py`  
...
crop / image 写的是该件整块子文件，不要在同一件内再拆轮廓、剖面线、紧固件等子层。总图用相对 `href` 引用子文件，禁止把整张总装 PNG 再 base64 进总 SVG、禁止用 clipPath 当图层。
...
|----------|-----------------|------|
| `assembly`（总装/剖视） | 已有轮廓 → **`crop`**，`crop_box` 框在该件在总装图上的大致范围 | `slot` 可与 `crop_box` 相同；允许重叠 |
...
```bash
python ${CLAUDE_SKILL_DIR}/tools/shared/structure_lineart_compose.py \
...
抽查：磁盘上存在 `parts/{视}_{id}.svg`；总 SVG 含 `href="parts/`，**没有** `src-assembly` / `clipPath` / 整图 `data:image`。每个可见件都有 `id="part-…"` / `data-part-id`。  
...
- [ ] 输出为 `parts/{视}_{id}.svg` + 总图相对引用；不是无 `part-*` 的扁图，也不是总图内 clip 整图  
