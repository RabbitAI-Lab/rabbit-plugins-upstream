...
- [ ] **行内分隔符（硬项）**：定稿 md **不得**把含 `\mathrm` / `\,` / `_{` 的式子写成普通括号（如 `(M_{\mathrm{total}})`、`(m_{\mathrm{silica}}=60\,\mathrm{g})`）。须为 `\(…\)` 或 `$…$`。预览里 `\(` 看起来像 `(`，勿删反斜杠。跑 `python tools/shared/latex_delimiters.py -i <定稿.md>`，有 hits **不得出 Word**；改正后对**该时间戳 md**重跑 `mermaid_render.py` / `md_to_docx.py`，禁止用未改正的 `draft.md` 出交付 `.docx`
