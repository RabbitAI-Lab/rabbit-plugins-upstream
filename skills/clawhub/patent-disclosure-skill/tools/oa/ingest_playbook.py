#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""模式 D · 经验手册：预读 / 安装 book-to-skill / 转写入 Obs。

  python tools/oa/ingest_playbook.py peek --path "D:/books/oa.pdf"
  python tools/oa/ingest_playbook.py ensure-skill
  python tools/oa/ingest_playbook.py ingest --from-skill-dir DIR --source-path BOOK --slug slug
...
from tools.oa.book_to_skill_setup import ensure_book_to_skill
from tools.oa.config import load_config
from tools.oa.playbook import ingest_distilled_skill, list_playbook_records, peek_source, reject_if_url
from tools.oa.vault_layout import refresh_oa_vault, resolve_oa_root
