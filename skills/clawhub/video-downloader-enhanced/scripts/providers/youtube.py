...
def _base_ytdlp_command() -> list[str]:
    command = [_require_ytdlp(), "--no-playlist"]
...
def _run_first_successful(commands: list[list[str]], *, timeout: int = 240) -> str:
    ...
    raise RuntimeError("yt-dlp YouTube route failed: " + " | ".join(errors))
...
def _require_ytdlp() -> str:
    ...
    if yt_dlp:
        return yt_dlp
    raise RuntimeError("YouTube provider requires yt-dlp, but yt-dlp is not installed or not on PATH.")
