...
- **⚠️ 入参规则**：
  - 所有接口的入参均来自 `fuzzy-search-org` 的搜索结果，用户确认企业后从中提取对应字段
  - `searchKey`：使用 `orgName`（企业全称）
  - `orgId`：使用 `orgId`（企业ID）
...
| 劳动仲裁送达报告 | `get-labour-arb` | `references/get-labour-arb.md` |
...
| 失信被执行 | `get-dishonest-debtors` | `references/get-dishonest-debtors.md` |
...
| 开庭公告（司法） | `get-open-court` | `references/get-open-court.md` |
...
| 失信被执行人 | `get-dishonest` | `references/get-dishonest.md`（待接入） |
| 税收违法 | `get-tax-violation` | `references/get-tax-violation.md`（待接入） |