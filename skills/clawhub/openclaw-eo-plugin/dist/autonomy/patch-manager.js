...
export class PatchManager {
    patches = new Map();
    reviewQueue = []; // Patches waiting for review
    constructor() {
        this.loadPatches();
    }
    /**
    ...
    async savePatch(patch) {
        ...
        await fs.writeFile(path.join(PATCH_DIR, `${patch.id}.json`), JSON.stringify(patch, null, 2), 'utf-8');
    }
    /**
     ...
     */
    async generatePatchesFromLowScores() {
        const results = [];
