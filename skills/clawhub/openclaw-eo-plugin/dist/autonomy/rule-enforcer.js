 ...
 */
function generateRulesSection(rules, metadata) {
    const ruleLines = [
        SOUL_MARKER_START,
        '',
        `**⚠️ 这些是强制规则，不可忽略**`,
 ...
 */
function insertRulesIntoSoul(soul, rules, metadata) {
 ...
 */
export function applyRulesToAgent(agentId, rules, dryRun = false) {
 ...
 */
export function applyRulesToAllAgents(rules, options = {}) {
    ...
    return {
        ...
        success: results.filter((r) => r.success && !r.dryRun).length,
 ...
 */
export function extractRulesFromSoul(soul) {
 ...
 */
export function generateRulesFromEvolution(evolutionResult, stats) {
    const rules = [];
