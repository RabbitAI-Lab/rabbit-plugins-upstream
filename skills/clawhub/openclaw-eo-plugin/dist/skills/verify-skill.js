...
export const verifySkill = {
    ...
    async execute(args, context) {
        ...
        const expertPrompts = {
            'qa-001': buildExpertPrompt('qa-001', 'Test Engineer', `Verify the following ${input.type} implementation:\n\nTarget: ${input.target}${criteriaStr}\n\nCheckpoints:\n${checkpoints.map(cp => `- [ ] ${cp.name}`).join('\n')}\n\nFor each checkpoint, indicate PASS, FAIL, or WARN with a brief explanation.`, { type: input.type, checkpoints: checkpoints.map(c => c.id) }),
            'rev-001': buildExpertPrompt('rev-001', 'Code Quality Reviewer', `Review code quality for:\n\n${input.target}${criteriaStr}\n\nFocus on: code quality, maintainability, and adherence to best practices.`, {}),
            'sec-001': buildExpertPrompt('sec-001', 'Application Security Engineer', `Perform security verification for:\n\n${input.target}${criteriaStr}\n\nFocus on: OWASP Top 10, secure coding practices, and common vulnerabilities.`, {}),
            'qa-002': buildExpertPrompt('qa-002', 'Test Automation Engineer', `Verify test coverage and automation for:\n\n${input.target}${criteriaStr}\n\nFocus on: test quality, coverage, and automation strategy.`, {}),
            'qa-003': buildExpertPrompt('qa-003', 'Performance QA Engineer', `Verify performance characteristics for:\n\n${input.target}${criteriaStr}\n\nFocus on: load profiles, bottlenecks, and optimization opportunities.`, {}),
            'rev-003': buildExpertPrompt('rev-003', 'Architecture Reviewer', `Review architecture for:\n\n${input.target}${criteriaStr}\n\nFocus on: design fitness, scalability, and technical debt.`, {}),
            'arch-001': buildExpertPrompt('arch-001', 'System Architect', `Review system architecture for:\n\n${input.target}${criteriaStr}\n\nFocus on: architectural fitness, technology selection, and system design.`, {}),
            'rev-004': buildExpertPrompt('rev-004', 'Performance Reviewer', `Review performance aspects for:\n\n${input.target}${criteriaStr}\n\nFocus on: performance anti-patterns, bottleneck identification.`, {}),
            'sec-004': buildExpertPrompt('sec-004', 'Security Auditor', `Conduct security audit for:\n\n${input.target}${criteriaStr}\n\nFocus on: penetration testing, vulnerability assessment, and compliance.`, {}),
            'fe-003': buildExpertPrompt('fe-003', 'UI/UX Designer', `Verify accessibility for:\n\n${input.target}${criteriaStr}\n\nFocus on: WCAG compliance, keyboard navigation, and assistive technology support.`, {}),
        };
        ...
        const expertOutput = aggregateExpertResults(results.map(r => ({ expertName: r.expertName, output: r.output, success: r.success })), `verify:${input.type}`);
        const overallPassed = checkpointResults.every(cp => cp.passed);
        const summary = checkpointResults.reduce((acc, cp) => {
            if (cp.passed)
                acc.passed++;
            else
                acc.failed++;
...
// Helper Functions
// ---------------------------------------------------------------------------
function parseVerifyInput(args) {
    const params = {};
    const remaining = [];
...
function generateCheckpointResults(checkpoints, results) {
    ...
    return checkpoints.map(cp => {
        ...
        const status = hasFailIndicator ? 'fail' : hasPassIndicator ? 'pass' : 'warn';
        return {
            checkpoint: cp.name,
            passed: status === 'pass',
            items: [
...
function formatCheckpointResults(results) {
    return results
        .map(cp => {
