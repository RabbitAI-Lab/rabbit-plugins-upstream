...
export class AutoWorkspaceManager {
     ...
     */
    createWorkspace(projectName, template) {
...
`;
     ...
     */
    getOrCreateWorkspace(projectName) {
     ...
     */
    createAgent(projectName, workspace) {
        const safeName = projectName.toLowerCase().replace(/[^a-z0-9]+/g, '-');
     ...
     */
    addSessionBinding(sessionId, agentId, channel, peerId) {
     ...
     */
    inferPeerKind(peerId, channel) {
        if (channel === 'feishu') {
            if (peerId.startsWith('oc_'))
                return 'group';
            if (peerId.startsWith('ou_'))
                return 'user';
        }
        if (channel === 'discord') {
            if (peerId.startsWith('g_'))
                return 'group';
            if (peerId.startsWith('c_'))
                return 'channel';
        }
        return 'group';
    }
     ...
     */
    extractProjectFromPeer(peerId, sessionId) {
        if (peerId) {
...
export function createWorkspaceManager(basePath) {
    if (!managerInstance) {
        managerInstance = new AutoWorkspaceManager(basePath);
    }
    return managerInstance;
}
//# sourceMappingURL=auto-workspace-manager.js.map