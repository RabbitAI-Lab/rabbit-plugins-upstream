...
class CapabilityRegistry {
     ...
     */
    hasCapability(layer, capability) {
...
export class TrinityOrchestrator {
    ...
    autoRoute(task) {
        ...
        let typeBasedLayer = this.config.defaultLayer;
        switch (task.type) {
            case 'coding':
     ...
     */
    getAlternativeLayers(primary, required) {
        const alternatives = [];
        for (const layer of ['openclaw', 'hermes', 'claude_code']) {
            if (layer === primary)
                continue;
            const hasExtra = required.some(cap => this.capabilityRegistry.hasCapability(layer, cap) &&
                !this.capabilityRegistry.hasCapability(primary, cap));
            if (hasExtra)
                alternatives.push(layer);
        }
    ...
    async execute(task) {
        ...
        if (routing.alternativeLayers.length > 0 && this.config.layerRouting === 'hybrid') {
            ...
            const altPromises = routing.alternativeLayers.map(layer => this.executeOnLayer(layer, task));
     ...
     */
    async executeOnLayer(layer, task) {
        switch (layer) {
            case 'openclaw':
     ...
     */
    calculateConfidence(layerResults, routing) {
        const results = Array.from(layerResults.values());
...
//# sourceMappingURL=trinity-orchestrator.js.map