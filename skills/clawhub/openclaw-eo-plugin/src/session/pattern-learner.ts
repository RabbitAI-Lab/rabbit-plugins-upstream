...
export interface LearnedPattern {
  ...
  taskType: string
  description: string
  pattern: string  // The execution steps as text
  tools: string[]
  successIndicators: string[]
 适用场景: string[]
  示例Task?: string
  confidence: number  // Based on how many times this pattern succeeded
  lastUsed: string
}
...
export class PatternLearner {
  private ragSystem = getSharedRAGSystem()
  private patterns: LearnedPattern[] = []

  /**
   * Analyze a session and extract successful patterns
   */
  async analyzeSession(sessionKey: string, sessionData: any): Promise<LearnedPattern[]> {
   ...
   */
  async learnFromSessions(sessions: Array<{ key: string; data: any }>): Promise<{
...
`.trim()
   ...
   */
  async retrievePatterns(taskQuery: string, topK = 3): Promise<LearnedPattern[]> {
   ...
   */
  private isTaskStart(content: any): boolean {
   ...
   */
  private extractTaskDescription(content: any): string {
    const text = this.extractText(content)
   ...
   */
  private generatePatternName(task: string): string {
   ...
   */
  async getStats(): Promise<{ totalPatterns: number; byType: Record<string, number> }> {
