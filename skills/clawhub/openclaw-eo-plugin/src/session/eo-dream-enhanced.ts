...
import { SessionArchiver, createSessionArchiver } from './session-archiver.js'
import { logger } from '../utils/logger.js'

// For proactive notification integration
interface ProactiveNotifier {
  notify(
    ...
    channels?: string[],
    options?: any
  ): Promise<string | void>  // Returns notification ID or void
}
...
export class DreamModule {
  ...
  shouldDreamOnIdle(): boolean {
    if (!this.config.enabled || !this.config.triggerOnIdle) return false
   ...
   */
  async dream(sessionKey?: string): Promise<DreamResult> {
    ...
    try {
      ...
      if (this.config.enableProactiveReport && this.config.notifier) {
        await this.config.notifier.notify(
          'warning',
          '⚠️ EO 做梦分析异常',
          `Dream Module 分析失败: ${err instanceof Error ? err.message : String(err)}`,
          ['feishu'],
          { feishuChatId: this.config.feishuChatId }
        ).catch(e => console.error('[Dream v2] Failed to send error notification:', e))
      }
   ...
   */
  private async getRecentSessions(currentSessionKey?: string): Promise<string[]> {
  ...
  private analyzePatterns(sessionKeys: string[]): CrossSessionPattern[] {
    const patterns: CrossSessionPattern[] = []
    const archiveIndex = this.archiver.getArchiveIndex()

    // Group patterns by type and find cross-session ones
    const byType = new Map<string, typeof archiveIndex.patterns>()
    for (const pattern of archiveIndex.patterns) {
   ...
   */
  private generateInsights(sessionKeys: string[], patterns: CrossSessionPattern[]): DreamInsight[] {
  ...
  private generateSuggestions(insights: DreamInsight[]): string[] {
   ...
   */
  private async sendProactiveReport(result: {
    sessionsAnalyzed: number
    patternsExtracted: number
    insightsGenerated: number
    suggestions: string[]
   ...
   */
  private async syncToLongTermMemory(insights: DreamInsight[]): Promise<void> {
    try {
...
interface CrossSessionPattern {
  id: string
  type: 'command' | 'workflow' | 'expert' | 'topic'
  description: string
  occurrences: number
  sessions: string[]
  consistency: number
...
class DreamMemory {
  ...
  addInsight(insight: DreamInsight): void {
    // Don't add duplicate insights
    const exists = this.insights.some(
      i => i.category === insight.category && i.content === insight.content
    )
    if (!exists) {
...
let globalDreamModule: DreamModule | null = null

export function getDreamModule(config?: Partial<DreamConfig>): DreamModule {
...
export function createDreamModule(config: Partial<DreamConfig>): DreamModule {
