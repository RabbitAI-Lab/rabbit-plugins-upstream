...
export async function handleDream(params: DreamParams): Promise<any> {
...
async function syncInsightsToMemory(workspaceRoot: string, insights: Insight[]): Promise<void> {
  ...
  try {
    ...
    const insightLines = insights.map(i =>
      `- **[${i.type}]** ${i.content}${i.timestamp ? ` (${new Date(i.timestamp).toLocaleString()})` : ''}`
    ).join('\n')
...
function buildDreamOutput(
  insights: Insight[],
  stats: { trigger: string; durationMs: number; learningsCount: number }
): string {
  const lines = [
