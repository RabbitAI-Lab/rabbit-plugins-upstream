...
export const DECISION_STRATEGIES: Record<StrategyType, StrategyWeights> = {
...
export function calculateScores(option: DecisionOption, weights: StrategyWeights): ScoredOption {
...
export function selectBestOption(options: DecisionOption[], context: DecisionContext): ScoredOption | null {
...
export function getStrategyDescription(type: StrategyType): string {
  const descs: Record<StrategyType, string> = {
