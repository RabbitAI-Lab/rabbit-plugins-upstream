...
export function initContextWorkflowHooks(api: OpenClawPluginApi): void {
...
export async function onMessageHook(api: OpenClawPluginApi, event: any): Promise<void> {
  if (!contextManager || !workflowExecutor) {
    initContextWorkflowHooks(api)
  }

  const now = Date.now()
  const message = (event?.context?.message as string) || (event?.context?.content as string) || ''
  ...
  if (now - lastProcessTime > PROCESS_INTERVAL_MS) {
    ...
    try {
      ...
      if (contextState.level !== 'normal') {
        api.logger.info(
          `[EO ContextWorkflow] Context level: ${contextState.level} ` +
  ...
  if (now - lastWorkflowCheck > WORKFLOW_CHECK_INTERVAL_MS) {
    ...
    const matchingWorkflows = WorkflowTrigger.findMatchingWorkflows(
      PRESET_WORKFLOWS.map(p => createWorkflowFromPreset(p.id, {})!).filter(Boolean) as any[],
 ...
 * 3. Decide next steps
 */
export async function afterToolCallHook(api: OpenClawPluginApi, event: any): Promise<void> {
  if (!contextManager || !workflowExecutor) return
 ...
 */
export function getActiveWorkflows(): Map<string, any> {
