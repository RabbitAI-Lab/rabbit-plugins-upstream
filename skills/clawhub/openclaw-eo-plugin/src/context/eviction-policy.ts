...
export class EvictionPolicyEngine {
  ...
  selectForEviction(items: SummarizableItem[], targetReduction: number): SummarizableItem[] {
    ...
    const preservableIndices = new Set(
      items.slice(-preserveRecent).map((_, i) => items.length - preserveRecent + i)
    )

    const toEvict: SummarizableItem[] = []
    ...
    for (const candidate of candidates) {
      ...
      if (preservableIndices.has(itemIndex)) continue

      toEvict.push(candidate.item)
      currentReduction++

      if (currentReduction >= target) break
    }
