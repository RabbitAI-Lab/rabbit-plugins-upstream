...
import type { ArchitectOptions, EOCommandResult, ExpertResult } from '../types/index.js';
...
export async function executeArchitect(
  ...
  try {
    ...
    const expertPrompts: Record<string, string> = {
      'architect': buildExpertPrompt(
        'architect', 'Solutions Architect',
        `Design a comprehensive system architecture for:\n\n${task}\n\nStyle: ${archStyle}\nLanguage: ${language ?? 'TypeScript'}`, archContext
      ),
      'tech-lead': buildExpertPrompt(
        'tech-lead', 'Tech Lead',
        `Provide technical implementation guidance for:\n\n${task}\n\nArchitecture Style: ${archStyle}`, archContext
      ),
      'dba': buildExpertPrompt(
        'dba', 'Database Administrator',
        `Design database schema and data layer for:\n\n${task}\n\nArchitecture Style: ${archStyle}`, archContext
      ),
      'devops': buildExpertPrompt(
