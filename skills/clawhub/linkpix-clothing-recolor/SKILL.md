...
一件衣服拍一次、全色系出图：底层是 `qhkit image` 自定义生图的定向换色重绘，逐色循环即可铺满 SKU。

## 何时触发

- 「这件衣服出红/蓝/米白色版本」「补全 SKU 色卡图」
- 上新只拍了一个颜色、其他色号要图时。

## 使用配方

```bash
qhkit image generate '{"modelLabel":"图片 5.0 Lite","uploadedImages":["./服装图.jpg"],"prompt":"将图片中的【蓝色连衣裙】变成【莫兰迪灰粉色连衣裙】，版型、材质纹理与光影保持一致"}'
```

官方提示词模板（与 LinkPix 工作台「服装换色」功能同款）：`将图片中的【原颜色+品名】变成【目标颜色+品名】`——可追加「版型、材质纹理与光影保持一致」。

- **多色 SKU**：逐色循环提交（每色一条命令），模型用 `图片 5.0 Lite` 保证各色版本之间细节一致。
- 颜色描述尽量具体（莫兰迪灰粉、藏青、燕麦米白），必要时给参照物。
- 有印花/logo 的服装要注明「印花图案与 logo 颜色保持不变」（或明确要一起换）。
...
- 换整件衣服走「AI电商模特换装工具 | LinkPix」；印花图案的配色裂变走「电商pod印花裂变设计生成器 | LinkPix」。
