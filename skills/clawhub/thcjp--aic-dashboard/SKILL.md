...
| 风险项 | 等级 | 防护措施 | 验证方法 |
| --- | --- | --- | --- |
| Token 泄露 | 高 | 使用 HTTPS 加密通信，定期更换 token | 使用 HTTPS 测试连接，并定期检查日志中的 token 更新记录 |
| 数据访问权限 | 中 | 限制对 `inbox.jsonl` 和 `session.json` 的访问权限 | 使用文件权限设置检查文件权限，确保只有授权用户可以访问 |
| 数据完整性 | 中 | 定期备份数据文件，防止数据损坏或丢失 | 定期检查备份文件，确保数据一致性 |
| 服务可用性 | 中 | 监控服务状态，及时响应故障 | 使用服务监控工具监控服务状态，并在服务异常时及时通知管理员 |
| 端口安全 | 中 | 使用非标准端口，减少端口扫描风险 | 检查服务监听端口是否为标准端口，如果不是，确保端口安全性 |
