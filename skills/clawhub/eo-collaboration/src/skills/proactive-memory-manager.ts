...
export class ProactiveMemoryManager {
   ...
   */
  async distillSessionMemory(sessionId: string, messages: SessionMessage[]): Promise<MemoryDistillResult> {
   ...
   */
  private categorizeMessages(messages: SessionMessage[]): MemoryCandidate[] {
  ...
  private extractTitle(content: string, maxLen: number): string {
  ...
  private generateMemoryFileName(type: MemoryType, title: string): string {
   ...
   */
  private async saveMemoryEntry(filePath: string, entry: MemoryCandidate): Promise<void> {
   ...
   */
  private async updateMemoryIndex(fileName: string, title: string, description: string, type: MemoryType): Promise<void> {
   ...
   */
  async loadRelevantMemories(query?: string): Promise<RelevantMemoryResult> {
    this.ensureMemoryStructure()
   ...
   */
  private filterByRelevance(entries: IndexEntry[], query: string): IndexEntry[] {
...
// ===========================================================================
   ...
   */
  truncateIndex(content: string): { content: string; wasLineTruncated: boolean; wasByteTruncated: boolean } {
...
export function createMemoryManager(workspace: string): ProactiveMemoryManager {
