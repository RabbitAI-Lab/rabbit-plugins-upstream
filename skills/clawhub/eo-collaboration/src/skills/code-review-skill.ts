...
const FOCUS_AREA_EXPERTS: Record<string, string[]> = {
  'quick': ['rev-001'],
...
export const codeReviewSkill = {
  ...
  async execute(
    args: string,
    context: CodeReviewSkillContext
  ): Promise<CodeReviewSkillResult> {
    ...
    const expertPrompts: Record<string, string> = {
      'rev-001': buildExpertPrompt(
        ...
        `Perform a comprehensive code review:\n\nTarget: ${target}${filesStr}${prStr}${commitStr}${branchStr}${repoStr}${focusStr}${depthStr}\n\nFocus on: code quality, maintainability, readability, best practices, and design patterns.`,
        reviewContext
      ),
      'rev-002': buildExpertPrompt(
        'rev-002', 'Security Code Reviewer',
        `Perform a security-focused code review:\n\nTarget: ${target}${filesStr}${prStr}${commitStr}${branchStr}${repoStr}${focusStr}${depthStr}\n\nFocus on: OWASP Top 10, injection vulnerabilities, authentication/authorization issues, data exposure, and secure coding practices.`,
        reviewContext
      ),
      'rev-004': buildExpertPrompt(
        'rev-004', 'Performance Reviewer',
        `Perform a performance-focused code review:\n\nTarget: ${target}${filesStr}${prStr}${commitStr}${branchStr}${repoStr}${focusStr}${depthStr}\n\nFocus on: algorithmic complexity, database query efficiency, memory usage, caching opportunities, and scalability concerns.`,
        reviewContext
      ),
      'sec-001': buildExpertPrompt(
        'sec-001', 'Application Security Engineer',
        `Conduct a deep security assessment:\n\nTarget: ${target}${filesStr}${prStr}${commitStr}${branchStr}${repoStr}${focusStr}${depthStr}\n\nFocus on: threat modeling, attack surface analysis, secure architecture patterns, and remediation recommendations.`,
        reviewContext
      ),
    }
...
function parseCodeReviewInput(args: string): CodeReviewSkillInput {
  ...
  return {
    ...
    depth: params.depth as CodeReviewSkillInput['depth'],
...
function parseFindings(results: ExpertResult[]): CodeReviewFinding[] {
  ...
  for (const line of lines) {
    ...
    let severity: CodeReviewFinding['severity'] = 'info'
    let category: CodeReviewFinding['category'] = 'maintainability'
    ...
    for (const [sev, keywords] of Object.entries(severityKeywords)) {
      if (keywords.some(kw => trimmed.toLowerCase().includes(kw))) {
        ...
        else if (sev === 'medium' && severity !== 'critical' && severity !== 'high') severity = 'medium'
        else if (sev === 'low' && severity === 'info') severity = 'low'
      }
    }

    for (const [cat, keywords] of Object.entries(categoryKeywords)) {
      if (keywords.some(kw => trimmed.toLowerCase().includes(kw))) {
        category = cat as CodeReviewFinding['category']
      }
    }
    ...
    if (findingId > 20) break // Limit findings
  }

  return findings.slice(0, 20)
}

function formatFindings(findings: CodeReviewFinding[]): string {
  if (findings.length === 0) return '_No specific findings extracted. See expert analysis below._'
