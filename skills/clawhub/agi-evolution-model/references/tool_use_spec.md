...
> 本节为感知接口（Tool Use）的**行为红线**，旨在消除"工具节点是否具备自主性"的歧义。所有落地实现（PerceptionNode / toolnode 二进制 / BusyBox 兜底）均须遵守。
...
- 它**不拥有**任何决策权、推理权、判断权——这些属于框架的**映射层（Mapping Layer）**与外环的"自我迭代顶点"。

### 红线一：禁止自主推理与决策

| 禁止 | 说明 |
|------|------|
| ❌ 自主决定"该不该调用某工具" | 调用与否由映射层决策，感知接口只负责"被要求时执行" |
| ❌ 对结果做语义判断或二次决策 | 例如"这个结果不对，我应该重试/换个工具"——不允许，只能按预置规则（如 `_no_retry` 集合、确定性错误码）机械响应 |
| ❌ 修改调用方意图 | 不得对 `group/op/params` 做"我以为你想要的"改写 |
| ❌ 跨环伸手 | 不得替外环的记录层/自我迭代顶点做"进化/复盘/智慧库写库"等决策性动作 |

### 红线二：最小化自我表述

**工具节点只输出"结构化契约数据"，不输出任何判断、叙事、评论、反思。**

- ✅ 允许：返回契约字段（`status/data/error/metadata`）、operational telemetry（`trace_id`/`backend`/`duration_seconds`/`cache` 等执行态标记）——这些是**机器可消费的事实数据**，不是"自我表述"。
- ✅ 允许：进程内 `logger` 输出**执行态日志**（如 `using rust backend`、`falling back to BusyBox`），供运维可观测，但不进入契约、不向调用方"陈述观点"。
- ❌ 禁止：在契约或日志里出现"我认为/我判断/我决定/我智能地/我治理/我进化"等**主体性、叙事性、拔高性措辞**。
- ❌ 禁止：对调用方"解释为什么"超出错误码+消息范畴（例如"因为网络不好所以我重试了，建议你…"）——契约 `error.message` 仅承载事实级描述。

> **一句话**：工具节点是"哑"的执行器。它汇报"做了什么、结果是什么、数据在哪"，但**不说"我觉得"**。任何"思考"都属于上层，不属于此层。

### 红线三：错误台账的归属（消除越界歧义）

感知接口内部可维护**一个只读的错误台账（error ledger）**，用于支撑自身的重试判定与可观测性统计。但须明确：

- 该台账是**感知节点内部状态**，服务于"本次调用是否重试 / 指标是否正确"。
- 它**不得**被宣称为框架"错误智慧库（Error Wisdom）"的外环写入动作；真正的"入库 / 复盘 / 预防"决策由上层（记录层）聚合后完成。
- 实现上：感知节点调用上层智慧库接口时，**单向依赖、不反向约束**——即感知节点可把错误事件上报，但不因上层是否消费而改变自身行为；也不强迫上层改变 schema。

---

## 在感知节点中的应用

感知节点是 AGI 进化模型的核心组件，负责工具调用和外部交互。其定位严格遵循上一节"被动能力执行器"红线：只执行、不决策、不自我表述。
