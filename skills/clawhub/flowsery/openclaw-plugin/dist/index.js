...
import { callApi, readConfig } from "./api.js";
...
const Filters = Object.fromEntries(FILTER_DIMENSIONS.map((dimension) => [
    `filter_${dimension}`,
...
].map((value) => Type.Literal(value)), { description: "The dimension to group visitors by." });
const query = (extra = {}) => Type.Object({ ...WebsiteSelector, ...DateRange, ...Pagination, ...Filters, ...extra });
export default definePluginEntry({
    id: "flowsery",
    name: "Flowsery",
    description: "Query privacy-first web analytics: visitors, trends, 24 breakdown dimensions, live visitors, and the issues AI found in session recordings.",
    register(api) {
        const cfg = () => readConfig(api);
        const get = (path, params, signal) => callApi(cfg(), "GET", path, { query: params, signal });
        api.registerTool({
            name: "flowsery_websites",
            label: "Flowsery: list websites",
            description: "List the websites this workspace token can read, with their ids and domains. Call this first: every other tool needs a websiteId or domain, and a workspace token does not imply one.",
        ...
        api.registerTool({
            ...
            description: "Aggregated totals for one site: visitors, sessions, bounce rate, average session duration, revenue, revenue per visitor and conversion rate. Omit the dates for all time. Every filter_* argument narrows the whole result, so filter_country plus filter_device answers 'mobile visitors from Germany' in one call.",
            parameters: query({
                fields: Type.Optional(Type.String({
                    description: "Comma-separated subset of visitors, sessions, bounce_rate, avg_session_duration, currency, revenue, revenue_per_visitor, conversion_rate. Omit for all.",
                })),
            }),
        ...
        api.registerTool({
            ...
            description: "The same metrics as flowsery_overview, but bucketed by hour, day, week or month. Use this for anything shaped like a trend or a chart. Asking for hourly buckets across a year returns thousands of points, so match the interval to the range.",
            parameters: query({
                interval: Type.Optional(Type.Union([Type.Literal("hour"), Type.Literal("day"), Type.Literal("week"), Type.Literal("month")], { description: "Defaults to day." })),
                fields: Type.Optional(Type.String({ description: "Comma-separated: visitors, sessions, revenue, conversion_rate, name." })),
            }),
            async execute(_toolCallId, params, signal) {
        ...
        api.registerTool({
            ...
            description: "Count the visitors active on the site in the last five minutes. A point-in-time number with no history, so it answers 'is anyone on the site now' and nothing about trends. Use flowsery_timeseries for those.",
        ...
        api.registerTool({
            ...
            description: "List the bugs, broken flows and UX problems the AI found while analyzing session recordings, deduplicated across sessions and ranked by severity. Each issue carries how many sessions hit it, when it was first and last seen, and steps to replicate. Also returns open, in-progress and resolved counts. Suspended issues are excluded unless status asks for them.",
            parameters: Type.Object({
                ...WebsiteSelector,
                ...Pagination,
                status: Type.Optional(Type.Union([Type.Literal("open"), Type.Literal("in_progress"), Type.Literal("resolved"), Type.Literal("suspended")], { description: "Default excludes suspended." })),
                ...
                sort: Type.Optional(Type.Union([Type.Literal("severity"), Type.Literal("recency")], {
