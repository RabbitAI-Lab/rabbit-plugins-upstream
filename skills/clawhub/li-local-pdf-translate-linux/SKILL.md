---
name: li_local_pdf_translate-linux
display_name: "本地模型PDF翻译（Ubuntu）"
display_name_en: "Local Model PDF Translation (Ubuntu)"
description: 使用本地llama.cpp模型（如Hy-MT2-7B）批量翻译PDF文档，结合学术翻译三步法（直译→反思→雅化），支持中/英/日/韩/法/德/西/俄八大主要语言任意互译，逐页提取+翻译，按目标语言输出.md文件（Ubuntu/Linux 平台）
category: Education
description_zh: "本地模型PDF批量翻译（Ubuntu/Linux）——八大语言互译+学术三步法+逐页处理+批量输出"
description_en: "Batch PDF translation using local llama.cpp models on Ubuntu/Linux, supporting 8 major languages with academic three-step method"
version: 1.0.4
...
1. **只读检测**（Agent 每次使用前执行，只检测不改动）：
   ...
   which llama-server
   python3 --version
   python3 -c "import fitz, requests"
   ls models/*.gguf 2>/dev/null || echo model-missing
   ```
2. **缺失项与推荐安装方式（Ubuntu/Linux）**：

   | 缺失项 | 推荐安装方式 |
   |--------|-------------|
   | `llama-server`（核心，llama.cpp） | 备选按序：① `sudo apt install llama.cpp`（发行源/官方 PPA 含包时）；② 官方 GitHub Releases 下载 Linux 预编译包并加入 PATH；③ 源码构建（`cmake -B build -DLLAMA_CURL=ON && cmake --build build --target llama-server`） |
   | `pymupdf` / `requests` | `pip3 install pymupdf requests`（PyPI 官方源） |
   | GGUF 翻译模型（**可选**，通常数 GB） | 用户自备翻译模型放入 `models/`；默认**不自动下载**，仅当用户明确指定来源并要求时，Agent 方可代为下载 |

3. **执行规则**：
   1. Agent 先输出检测结果与安装计划，并提问："是否执行以上安装？（Y/N）"
   2. 得到 Y 后逐条执行（下载解压、源码构建、`pip3 install` 等），执行路径一律相对本 skill 根目录
   3. 任一步失败立即停下，向用户说明并提供备选方案；不得静默重试、绕过或更换安装源
   4. 全部完成后重新检测；仍缺失则中止翻译并告知原因

4. 安装完成后可通过 `bash start_server.sh` 启动 llama-server（相对路径、本机端口 8001），再运行 `python3 scripts/batch_translate.py --check` 验证连通。
...
python3 scripts/config.py set api_url "http://localhost:8001/v1/chat/completions"
