...
# 3. 在 skill 根目录启动本地翻译模型（Ubuntu，相对路径，端口 8001）
...
4. 安装完成后由 `bash start_server.sh` 启动 llama-server（相对路径、端口 8001），再运行 `python3 scripts/batch_translate.py --check` 验证连通。
...
# 2. 翻译单个 PDF：英语→中文（默认）
python3 scripts/batch_translate.py --input "paper.pdf"

# 3. 翻译单个 PDF：中文→日语，指定输出文件
python3 scripts/batch_translate.py --input "paper.pdf" --direction zh2ja --output "paper.ja.md"

# 4. 批量翻译目录：韩语→法语
python3 scripts/batch_translate.py --input-dir "~/docs" --direction ko2fr
```
...
```bash
# 单 Agent：英→俄
python3 scripts/auto_translate.py --input "~/pdfs" --direction en2ru

# 多 Agent：英→俄（4 并发）
python3 scripts/multi_agent_translate.py --input "~/pdfs" --direction en2ru --agents 4
```
...
```bash
python3 scripts/batch_translate.py --input "paper.pdf" --direction ja2en --depth full
```
