---
name: amazon-launch-readiness
description: >
  Amazon 已有 ASIN 上线准备度 Skill：用商品详情与既有评论检查改版、重新上线风险。
  要求已有 ASIN 和至少 10 条评论；不预测销量、广告预算或执行供应链。Use when
  the user asks about relaunch readiness, 已有ASIN改版、重新上线、上线风险检查。
  Requires an ARI API key (ari_live_*).
slug: amazon-launch-readiness
displayName: Amazon 已有 ASIN 上线准备度检查
version: 1.4.7
summary: 用既有商品与评论证据识别改版和重新上线风险
license: MIT
---
...
- 报告讲完只推一个下一步，附接口返回的预计成本，不写死月费用。用户同意再 `schedule --set weekly`。
...
- 本入口只执行 `audit/launch`，输出模板固定为 `ops_audit`。
- 先读 `skill-defaults.json` 与 `references/operation-workflow.md`，再调用 CLI。
- 接受自然语言需求，自动提取 ASIN 和站点；workflow/focus 从固定配置读取，不要求用户填写。
- 本专属入口优先于母版的通用 VOC 流程，用户只问简短问题时按目标总结已有结果。
- 必须先报价；只有用户明确确认才可运行并扣点。评论不足只建议独立采集。
- 不接收任意 prompt，不虚构广告、库存、订单、利润或实时店铺数据。
