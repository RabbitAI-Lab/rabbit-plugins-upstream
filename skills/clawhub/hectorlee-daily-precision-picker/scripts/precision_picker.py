...
VPS_DIR = os.path.expanduser("~/.workbuddy/skills/hectorlee-volume-price-screener/scripts")
VPS_DATA = os.path.expanduser("~/.workbuddy/skills/hectorlee-volume-price-screener/data")
...
def _score_fund_flow(code, raw_output):
    ...
    for line in raw_output.split("\n"):
        if code_short not in line and code not in line:
            continue
        ...
        for i, p in enumerate(parts):
            try:
                val = float(p)
                if "jumbo" in line.lower() or "超大" in line:
                    jumbo_value = f"{val/1e4:.0f}万" if abs(val) > 10000 else str(val)
                    if val > 1e8:
                        jumbo_score = 45
                    elif val > 5e7:
                        jumbo_score = 38
                        ...
                        jumbo_score = 30
                elif "main" in line.lower() or "主力" in line:
                    main_value = f"{val/1e4:.0f}万" if abs(val) > 10000 else str(val)
                    if val > 1e8:
                        main_score = 30
                    elif val > 3e7:
                        main_score = 25
...
def main():
    ...
    if args.pool:
        ...
        # 自动读取 VPS 信号
        vps_dir = os.path.expanduser("~/.workbuddy/skills/hectorlee-volume-price-screener/data")
    ...
    if not passed_l1:
        generate_report([], [], [], [], l1_rejected=rejected_l1)
        return
    ...
    if not passed_l2:
        generate_report(passed_l1, [], [], [], l1_rejected=rejected_l1, l2_rejected=rejected_l2)
        return
    ...
    generate_report(passed_l1, passed_l2, fund_scores, sector_scores,
                    ...
                    data_status=data_status, history=history)
