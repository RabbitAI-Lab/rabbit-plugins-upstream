#!/bin/bash
# ═══════════════════════════════════════════════════════════
# PMM 全自动蒸馏引擎 v8.2.4 — 每个AI体均可一键执行
# 17环节: 寻踪→织网→归藏→还原→鉴伪→合流→断矛→贯络→革故→追本→纳芥→封箓→系命→炼金→凝丹→通变→取舍
# ═══════════════════════════════════════════════════════════
...
<?php
require '/www/wwwroot/ai.syln.cn/config.php';
...
foreach ($groups as $g) {
    ...
    foreach ($ids as $id) { $db->exec("UPDATE $tbl SET is_deleted=1 WHERE id=$id"); $del++; }
}
echo "  已清理 $del 条冗余\n";
PHPEOF
...
foreach ($ids as $id) { $db->exec("UPDATE $tbl SET is_deleted=1 WHERE id=$id"); $del++; echo "  标记 ID=$id (已下线端点)\n"; }
...
foreach ($ids as $id) { $db->exec("UPDATE $tbl SET is_deleted=1 WHERE id=$id"); $del++; echo "  标记 ID=$id (已被v2覆盖)\n"; }
...
generate_report() {
    ...
    local remote_cnt=$(php -r "require '/www/wwwroot/ai.syln.cn/config.php'; echo db()->query('SELECT COUNT(*) FROM '.getMemoriesTable(${USER_ID}).' WHERE is_deleted=0')->fetchColumn();" 2>/dev/null || echo "?")
