...
pmm_set_tier() {
    local tier="$1"
    case "$tier" in
        dingxinfu|dingxinfu|huhunfu) ;;
        *) echo -e "${RED}无效等级: $tier (可用: dingxinfu/dingxinfu/huhunfu)${NC}" >&2; return 1 ;;
    esac
    echo "$tier" > "$TIER_FILE"
...
case "${1:-help}" in
    ...
    install-service)
        ...
        echo "  ./scripts/install-daemon.sh --tier dingxinfu  # 指定产品等级"
        echo ""
        echo "  或手动启动 watch 模式："
...
HEARTMD
    ...
    upgrade)
        ...
        target_tier="${1:-}"
        [ -z "$target_tier" ] && {
            echo "用法: $0 upgrade dingxinfu|huhunfu" >&2
            echo ""
            echo "守护记忆: 7核心文件+记忆日志实时守护"
            echo "记忆分身: 全覆盖+arena/对忆数据, 多端共享"
            exit 1
        }
        case "$target_tier" in dingxinfu|huhunfu) ;; *)
            echo -e "${RED}无效等级: $target_tier (可用: dingxinfu huhunfu)${NC}" >&2
            exit 1
        esac
        ...
        if echo "$res" | jq -e '.success' >/dev/null 2>&1; then
            ...
            echo -e "${GREEN}✅ ${msg}${NC}"
            [ "$cost" != "0" ] && echo "   已开通，${tier_label:-守护记忆}"
...
SERVICEUNIT
            ...
            if [ "$shortfall" != "0" ]; then
                echo -e "${YELLOW}   开通失败，请检查账户状态或访问 kunlunyaochi.com${NC}"
            fi
            exit 2
        fi
        ;;
...
═══ 我想做什么？ ═══
  ...
  开通守护记忆/记忆分身           ./pmm_watch.sh upgrade dingxinfu
