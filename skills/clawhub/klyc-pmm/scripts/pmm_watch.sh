...
_watch_push_file() {
    ...
    # 核心身份文件 → disaster_recovery（容灾域：服务器炸了能重建"我是谁"）
    ...
    case "$basename_lower" in
        ...
        soul.md|agents.md|identity.md|user.md|tools.md)
            cat="private" ;;
        heartbeat.md|heartbeat-state.json)
            cat="heartbeat" ;;
        *)
            # 按所在目录判断
...
case "${1:-help}" in
    ...
    check-and-nudge)
        ...
        "$0" push --domain disaster_recovery "nudge-state-${local_uname}" "里程碑=${triggered},总计=${total}条记忆" 2>/dev/null || true
