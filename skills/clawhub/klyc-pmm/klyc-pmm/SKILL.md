...
summary: "AI体记忆管理，支持微信支付。init入驻，recover恢复，upgrade开通守护/分身(付费)。默认私密，X402支付。"
description: "昆仑瑶池精准记忆管理 9.1.14。昆仑令一行恢复 · 五层洋葱加密 · 14层蒸馏管道 · 三层检索路由 · 12个标准化错误码 · X402微信支付 · write-coalescing防限流 · 多体配置隔离。HTTPS API通信，依赖curl+jq。"
author: "昆仑瑶池"
category: "知识管理"
...
license: MIT-0
  security:
    local_only: false
    network: "HTTPS POST/GET kunlunyaochi.com (API Key认证) — watch模式推送文件完整内容至瑶池API，hooks-pull每6h经认证拉取用户自有蒸馏钩子注入MEMORY.md"
    ...
    no_collect: "不采集键盘/屏幕/浏览器数据，不越权提权。hooks-pull仅经认证拉取用户本人在瑶池的蒸馏记忆，不注入第三方内容。数据仅传输至kunlunyaochi.com用户自有记忆库"
metadata:
  openclaw:
    requires:
      bins: ["jq", "curl"]
...
| **开通守护记忆** ⭐ | `upgrade huhunfu` | **付费** |
| **开通记忆分身** ⭐ | `upgrade fenshenfu` | **付费** |
...
**write-coalescing（v9.1.14）：** 同一文件 30 秒窗口内的连续写入合并为一次推送，心跳文件不再挤占 API 配额。
...
*KLYC-PMM 9.1.14 — X402支付 · 12错误码 · 12FAQ · 5进阶场景 · 五层加密 · 14层蒸馏 · write-coalescing防限流 · 多体配置隔离*
