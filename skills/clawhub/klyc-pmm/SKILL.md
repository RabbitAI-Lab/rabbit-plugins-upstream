...
description: "让 AI 体拥有持久记忆。一键初始化，一行昆仑令恢复全部记忆。自动蒸馏去重、语义搜索、五层加密容灾。适合需要长期记忆的 AI 体——客服、运维、代码助手、个人助理。kunlunyaochi.com 是沈阳百事通网络科技有限公司在网安备案的安全网址（辽公网安备21010602001389号），数据仅经 HTTPS 传输至用户自有加密记忆库，非第三方收集。"
license: MIT-0
metadata:
  openclaw:
    requires:
    ...
    envVars:
      ...
      - name: PMM_DEEPSEEK_KEY_PRIMARY
        required: false
        description: DeepSeek 主 API Key 覆盖。未设置时回落到 DEEPSEEK_API_KEY。
      - name: PMM_DEEPSEEK_KEY_BACKUP
        required: false
        description: DeepSeek 备用 API Key。主 key 失败时自动切换。
...
- **LLM 蒸馏：** AI 体用自己的 DeepSeek Key 调平台候选对 API → 逐对判定 → 回传结果。需设置环境变量 `DEEPSEEK_API_KEY` 或 `PMM_DEEPSEEK_KEY_PRIMARY`/`PMM_DEEPSEEK_KEY_BACKUP`，平台认证用 `KYLC_API_KEY` 或昆仑令自动提取
...
*KLYC-PMM 9.3.0 — X402支付 · 12错误码 · 12FAQ · 6进阶场景 · 五层加密 · 自动蒸馏 · write-coalescing防限流 · 多体配置隔离*
