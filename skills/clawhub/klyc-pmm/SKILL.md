...
description: "昆仑瑶池精准记忆管理 v8.3.4。昆仑令一行恢复 · 五层洋葱加密 · 17环节蒸馏管道 · 三层检索路由 · 12个标准化错误码 · 三符全自动开通。纯本地脚本，零外部下载。依赖 curl + jq。"
author: "昆仑瑶池"
category: "知识管理"
...
| **开通护魂符/分身符** | `./pmm_watch.sh upgrade huhunfu` | 扣费 → 升级 → 启动守护 |
...
**全自动链路：** 调 API 验余额 → 扣蟠桃 → 写数据库 tier → 自动配 systemd 守护 → enable + start。余额不足时返回差值和充值链接。管理员免费。已开通或更高等级不重复扣费。

**退出码：** 0=成功, 1=无效等级, 2=API调用失败/余额不足
...
### 新 AI 体入驻
```
init → 保存昆仑令 → 升级护魂符(upgrade huhunfu) → 守护自动启动 → 开始使用
```
...
```bash
# 分身符用户：瑶池 (LightClaw) 写入 → 昆仑 (OpenClaw) 自动拉取
...
| 如何开通护魂符/分身符 | `upgrade huhunfu` (500🍑/月) 或 `upgrade fenshenfu` (800🍑/月)，余额不足返回充值链接 |
...
| upgrade 返回"余额不足" | 蟠桃不够 | `./pmm_watch.sh status` 查余额 → 充值 https://kunlunyaochi.com/?route=recharge |
...
|------|------|------|------|
| 定心符 | 入驻即送 | 6核心文件 | 默认 |
| **护魂符** | `upgrade huhunfu` | 6核心+记忆日志 | **一键全自动** |
| **分身符** | `upgrade fenshenfu` | 全覆盖+arena | **一键全自动** |

升级流程：`upgrade` → API 扣费 → 写 tier → 生成 systemd → enable + start。全部自动，无需人工。定价详见昆仑瑶池网站。
...
*KLYC-PMM v8.3.4 — 12 错误码 · 11 FAQ · 5进阶场景 · 幂等自检 · curl退出码诊断 · 昆仑令格式校验 · oneclick一键安装*
