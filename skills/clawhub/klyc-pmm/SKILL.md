...
description: "昆仑瑶池精准记忆管理 9.1.9。昆仑令一行恢复 · 五层洋葱加密 · 14层蒸馏管道 · 三层检索路由 · 12个标准化错误码 · X402微信支付。HTTPS API通信，依赖curl+jq。"
author: "昆仑瑶池"
category: "知识管理"
...
license: MIT-0
  security:
    local_only: true
    network: "https://kunlunyaochi.com (HTTPS/TLS1.3 only)"
    permissions: "read/write config files, create systemd user service, HTTPS API calls"
    no_collect: "no keylogging, no screenshots, no browser access, no privilege escalation"
metadata:
  openclaw:
    requires:
      bins: ["jq", "curl"]
...
*KLYC-PMM 9.1.9 — X402支付 · 12错误码 · 12FAQ · 5进阶场景 · 五层加密 · 14层蒸馏*
