...
def main():
    ...
    # 防御性打开：非 docx / 损坏 / PDF 等一律友好报错，不抛 traceback
    try:
        doc = Document(input_path)
    except Exception as e:
        ext = os.path.splitext(input_path)[1].lower()
        if ext == '.pdf':
            reason = "PDF 格式无法自动提取章节结构（PDF 不支持 docx 解析）"
        elif 'PackageNotFound' in type(e).__name__ or 'not a zip' in str(e).lower() or 'bad' in str(e).lower():
            reason = "文件不是合法的 Word 文档（可能损坏、非 docx 格式或扩展名错误）"
        else:
            reason = f"文档打开失败：{type(e).__name__}"
        print(f"[ERROR] {reason}")
        print("[HINT] 建议：① 确认文件能用 Word 正常打开 ② PDF 请转 .docx 后重试 ③ 或手动输入章节列表")
        sys.exit(2)
