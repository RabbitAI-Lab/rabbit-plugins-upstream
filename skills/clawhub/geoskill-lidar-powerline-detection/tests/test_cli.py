"""CLI argument parsing tests for lidar-powerline-detection."""
import subprocess
import sys
import os
...
class TestCLIBasics:
    ...
    def test_input_file_not_found_exit_2(self):
        ...
        r = run_cli(["--input", "nonexistent_cloud.xyz"])
        assert r.returncode == 2
    ...
    def test_synthetic_runs(self):
        """Synthetic mode with bbox → exit 0 + manifest."""
        out = "./_test_cli_out"
        r = run_cli([
    ...
    def test_bbox_only_auto_synthetic(self):
        """--bbox without --synthetic flag → auto synthetic → exit 0."""
        out = "./_test_cli_bboxonly"
        r = run_cli(["--bbox", "116.0", "39.0", "117.0", "40.0",
