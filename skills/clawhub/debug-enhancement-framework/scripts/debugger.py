...
class RetryPolicy:
    ...
    BUG FIXED (v2.1.0): this used plain exponential backoff -
    ``delay *= multiplier`` with no randomisation - so every client that failed
    at the same moment retried at the same moment. Measured before the fix: the
    gap sequence was byte-identical across runs (0.05, 0.10, 0.20 s). That is
    the textbook thundering-herd anti-pattern, and it shipped inside a framework
    whose job is to teach reliable debugging.

    Full jitter (the AWS-recommended default) is::

        sleep = random_uniform(0, min(cap, base * multiplier**attempt))

    It gives the lowest server load and the fastest overall completion of the
    common strategies. Set ``jitter=False`` only to reproduce a legacy timing
    bug; ``jitter_seed`` makes the sequence deterministic for tests.
    """
    ...
    def _seeded_rng(self) -> "random.Random":
        """One RNG per policy instance, created on first use.

        BUG FIXED (v2.1.0, caught in review): this built a fresh
        ``random.Random(seed)`` on EVERY call, so a seeded policy drew the same
        uniform each time - ``compute_delay(1)`` returned an identical value
        forever and the "sequence" was just one draw rescaled by the cap. A seed
        must give a reproducible *sequence*, not a constant. Cache the generator
        so successive calls advance it.
        """
        rng = getattr(self, "_rng_cache", None)
        if rng is None:
            rng = random.Random(self.jitter_seed)
            object.__setattr__(self, "_rng_cache", rng)
        return rng

    def compute_delay(self, attempt: int, _rng=None) -> float:
        """Delay before retry `attempt` (1-based). Exposed so it is testable."""
        capped = min(self.initial_delay * (self.backoff_multiplier ** (attempt - 1)),
                     self.max_delay)
        if not self.jitter:
            return capped
        rng = _rng or (self._seeded_rng() if self.jitter_seed is not None else random)
    ...
    def __call__(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            last_exception = None
            rng = self._seeded_rng() if self.jitter_seed is not None else None
            ...
            for attempt in range(1, self.max_attempts + 1):
                try:
                    ...
                    time.sleep(self.compute_delay(attempt, rng))
...
class CircuitState(Enum):   # NOT a dataclass: @dataclass on an Enum was a no-op
    CLOSED = "closed"
    OPEN = "open"
    HALF_OPEN = "half_open"
...
def diagnose_environment() -> dict:
    ...
    try:
        ...
        # BUG FIXED (v2.1.0): the threshold was a flat 100 GB, so a machine with
        # 17 GB free - a perfectly healthy laptop or container - was permanently
        # reported as "Low disk space". A diagnostics tool that always cries wolf
        # trains its reader to ignore it. Use a proportional threshold with a
        # small absolute floor, and report the rule that fired.
        free_pct = (usage.free / usage.total * 100) if usage.total else 100.0
        diagnosis["disk_space"]["free_percent"] = round(free_pct, 1)
        diagnosis["disk_space"]["low_disk_rule"] = "free < 10% or free < 2 GiB"
        if free_pct < 10.0 or usage.free < 2 * 1024**3:
            diagnosis["issues"].append(
                f"Low disk space: {round(usage.free / 1024**3, 2)} GiB free "
                f"({round(free_pct, 1)}%)")
    except Exception as e:
...
def main():
    ...
    if args.command == "diagnose":
        # BUG FIXED (v2.1.0): the skill name is documented as OPTIONAL, but a
        # bare `diagnose` fell back to the literal name "unknown", reported
        # "Skill not found" and exited 1. Running the documented command on a
        # healthy machine therefore looked like a failure. With no name, report
        # the environment only and succeed; only a NAMED-but-missing skill is
        # an error.
        if args.skill_name:
            results = run_diagnostics(args.skill_name)
            print(json.dumps(results, indent=2))
            if results["checks"].get("skill_exists"):
                print("✅ Skill found and operational", file=sys.stderr)
                return 0
            print(f"❌ Skill not found or misconfigured: {args.skill_name}", file=sys.stderr)
            return 1
        results = {"scope": "environment", "environment": diagnose_environment()}
        print(json.dumps(results, indent=2))
        issues = results["environment"].get("issues", [])
        # stdout stays pure JSON (SKILL.md: "All emit JSON on stdout");
        # the human summary goes to stderr so pipelines can parse stdout directly.
        print(f"{'⚠️' if issues else '✅'} Environment check: {len(issues)} issue(s)",
              file=sys.stderr)
        return 0
        ...
        print(json.dumps({"report": "empty", "reason":
                          "performance metrics are collected in-process; use "
                          "PerformanceMonitor inside your code and read its "
                          "summary there", "metrics": []}, indent=2))
    
    elif args.command == "simulate":
        # BUG FIXED (v2.1.0): this raised the exception uncaught, so the CLI
        # printed a Python traceback and exited 1. The documented contract is
        # that every command emits JSON on stdout - a traceback is neither
        # machine-readable nor a useful simulation result. Classify the error
        # the way the framework itself would, emit JSON, and signal via exit
        # code so a harness can assert on it.
        error_map = {
        ...
        exc = error_map[args.error_type]
        classified = ErrorClassifier.classify(exc)
        print(json.dumps({
            "simulated": True,
            "requested": args.error_type,
            "exception": type(exc).__name__,
            "message": str(exc),
            "classified_as": classified.error_type.value,
            "retryable": classified.error_type not in (ErrorType.VALIDATION,
                                                       ErrorType.PERMISSION),
        }, indent=2))
        return 2
...
if __name__ == "__main__":
    # BUG FIXED (v2.1.0): main() returned meaningful exit codes that were thrown
    # away, so `simulate` reported success (0) while announcing a simulated
    # failure. SKILL.md documents "0 = success, non-zero = failure"; honour it.
