...
async def run_full_scan(keywords: list, config_path: str = None, push: bool = True, api_key: str = None) -> dict:
    """运行完整GEO检测流程"""
    # 默认使用skill目录下的config.json
    if config_path is None:
        config_path = str(Path(__file__).parent.parent / "config.json")

    # Step 0: 配额检查（免费版限制，api_key 优先于环境变量）
    allowed, err_msg, enabled_platforms = geo_quota.check_quota(keywords, api_key=api_key)
    if not allowed:
        print("\n" + "=" * 50)
        print("❌ 配额不足")
        print("=" * 50)
    ...
    geo_quota.show_quota_status(api_key=api_key)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="GEO Monitor - 品牌AI可见性检测",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("keywords", nargs="*", help="要检测的品牌名")
    parser.add_argument("--no-push", action="store_true", help="不推送飞书")
    parser.add_argument("--status", action="store_true", help="查看配额状态")
    parser.add_argument("--upgrade-pro", action="store_true", help="升级到专业版")
    parser.add_argument("--upgrade-ent", action="store_true", help="升级到企业版")
    parser.add_argument("--api-key", dest="api_key", default=None,
                        help="API Key（优先于环境变量 GEO_API_KEY）")

    args = parser.parse_args()

    if args.status:
        geo_quota.show_quota_status(api_key=args.api_key)
        return
...
用法:
  python geo_report.py <品牌名> [品牌B] [品牌C] [--no-push] [--api-key YOUR_KEY]
  python geo_report.py --status [--api-key YOUR_KEY]   # 查看配额状态
...
示例:
  ...
  python geo_report.py 提分引擎AI --api-key YOUR_API_KEY
        """)
        sys.exit(1)
