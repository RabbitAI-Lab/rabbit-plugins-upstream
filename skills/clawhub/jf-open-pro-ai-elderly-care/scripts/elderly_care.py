...
支持平台：JF Tech（杰峰）
用法：
    python elderly_care.py --action status --sn <序列号> --user <用户 ID> --uuid <uuid> --appkey <appKey> --secret <secret> --auth <authorization> --movecard <moveCard>
    python elderly_care.py --action switch --enable true --sn <序列号> --user <用户 ID> --uuid <uuid> --appkey <appKey> --secret <secret> --auth <authorization> --movecard <moveCard>
...
def get_switch_status(sn: str, user: str, uuid: str, appkey: str, 
    ...
    url = "https://api-cn.jftechws.com/aisvr/v3/gateway/api/elderly/ai/analysis/switch/get"
...
def set_switch(sn: str, user: str, enable: bool, uuid: str, appkey: str, 
    ...
    url = "https://api-cn.jftechws.com/aisvr/v3/gateway/api/elderly/ai/analysis/switch/change"
...
def format_switch_result(result: dict, enable: bool) -> str:
    ...
    return f"✅ 老人看护服务已{action}"


def main():
    parser = argparse.ArgumentParser(description="老人看护 - 服务状态查询和设置")
