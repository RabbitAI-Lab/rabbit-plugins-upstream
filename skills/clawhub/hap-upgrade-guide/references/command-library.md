...
**用途**: 将 Markdown 升级指南转换为 HTML 格式

**特点**:
- ✅ 正确处理超长代码块(已测试 4000+ 字符的 JavaScript 脚本)
- ✅ 支持多个连续代码块
- ✅ 处理表格、列表、引用、标题等所有 Markdown 元素
- ✅ 自动生成带复制按钮的代码块
- ✅ 替换所有模板占位符
...
**依赖**: Python 3.x (仅使用标准库,无需额外安装)

**注意事项**:
- 脚本必须在生成的 Markdown 文件所在目录执行
- HTML 模板路径在脚本中已配置,会自动根据部署模式选择对应模板
- 脚本支持单机和集群两种模式的转换

**脚本路径**: `scripts/convert_md_to_html.py`
