...
Usage:
    # 查询指定域名的备案信息
    python3 fetch_icp-query.py --key YOUR_KEY --domain www.baidu.com

Env Config:
    export TIANAPI_ICP_KEY=your_api_key
    # 或在脚本同级目录创建 .env 文件: TIANAPI_ICP_KEY=your_api_key
"""
...
# --- 配置常量 ---
API_ENDPOINT = "https://apis.tianapi.com/icp/index"
ENV_KEY_NAME = "TIANAPI_ICP_KEY"
DEFAULT_TIMEOUT = 10
...
class Config:
    ...
    def _exit_with_help(self, msg: str):
        ...
        print(f"💡 提示: 申请 Key -> https://www.tianapi.com/apiview/118", file=sys.stderr)
        sys.exit(1)
...
class TianAPIClient:
    ...
    def fetch_icp(self, domain: str) -> Dict[str, Any]:
        """
        执行 API 请求
...
def main():
    parser = argparse.ArgumentParser(description="TianAPI ICP Query Fetcher")
    parser.add_argument("--key", help="API Key (可选，优先使用环境变量)")
    parser.add_argument("--domain", required=True, help="要查询的网站域名 (如: www.baidu.com)")
    ...
    # 2. 请求
    result = client.fetch_icp(domain=args.domain)
