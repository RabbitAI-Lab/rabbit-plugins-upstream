...
> **语法高亮支持范围（如实标注）**：`md2share.py` 的词法高亮只内置 **4 种语言**——`python` / `javascript` / `bash` / `sql`（含关键字 + 注释着色）；代码块请用这 4 个规范名标注语言。其它语言（如 `js`、`py`、`sh`、`c++`）**不会高亮**：仍显示大写语言标签 + 一键复制按钮，但内容退化为纯文本渲染（仅字符串/数字做简单着色，无关键字与注释高亮）。这是当前实现的行为，未做任何代码改动。
