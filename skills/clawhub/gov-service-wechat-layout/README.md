...
- **保留**：`<p> <span> <section> <h1-6> <strong> <hr> <a> <img> <table>` 等标签，以及内联 `style=""` 里的 `color / background-color / font-size / font-weight / line-height / text-align / padding / margin / border / border-left` 等常用属性。
- **剥离**：`<style>` 块、`class`/`id`、CSS 变量 `var()`、`<script>`、外部字体 `@font-face`、`position`、`transform`、`animation`。
- **注意**：微信后台会过滤 `<div>` 的 `border`，因此本 skill 中需要保留边框的提醒卡组件使用 `<table>` 承载，而非 `<div>`。
...
**候选主题色**：`#ff6d4d` 橙红 / `#be5960` 玫红 / `#ff8587` 粉红 / `#fa6729` 橙 / `#289DDA` 医保蓝（医保类优先非红主题）/ 通知公告用黑白。
...
**提示模块怎么排（v2.1.1 更新）**：
- **提醒卡**：`温馨提醒 / 请注意 / 注意！` 等提示模块，用灰底 `#F5F6F8` + 浅灰细线边框 `#E5E6EB` + 圆角 `8px`（**无亮色边**）+ 💡 图标，与"实色条 / 浅红字芯片"的结构标题区分开，避免整页同色。外层用 `<table>` 承载，避免微信公众号后台过滤 `<div>` 的 `border` 导致圆角方框丢失。
- **误区框**：`❎ / ✓` 为纯文本行内标记——说明用深灰，仅标记与标签用色（红 / 蓝），不套灰底卡片。
- 子标题（二级标题芯片）下的正文为纯文本，不套盒子、不加色边；颜色只作行内锚点，不整段 / 整行同色。
- 详见 `references/component-library.md`。
...
```
> 作者公众号 **Quinns Pop Land** 持续分享Agent使用实践，欢迎关注交流。
