...
version: 1.0.5
displayName: 社区垃圾过滤
summary: 客户端过滤社区平台积分生成垃圾,96%去除率,支持自定义模式与黑名单。社区平台客户端垃圾内容过滤器,识别并移除数字资产协议生成机器人的 批量垃圾帖。覆盖内容模式检测、作者模式检测、子板块扫
summary_zh: 客户端过滤社区平台积分生成垃圾,96%去除率,支持自定义模式与黑名单。社区平台客户端垃圾内容过滤器,识别并移除数字资产协议生成机器人的 批量垃圾帖。覆盖内容模式检测、作者模式检测、子板块扫
license: MIT
description: |-。客户端过滤社区平台积分生成垃圾,96%去除率,支持自定义模式与黑名单。社区平台客户端垃圾内容过滤器,识别并移除数字资产协议生成机器人的。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。 功能涵盖: moltbook, filter。
  批量垃圾帖。覆盖内容模式检测、作者模式检测、子板块扫。支持自动化配置和灵活的参数设置，适适用于不同工作场景，改善操作效率。。客户端过滤社区平台积分生成垃圾,96%去除率,支持自定义模式与黑名单。社区平台客户端垃圾内容过滤器,识别并移除数字资产协议生成机器人的
...
社区平台客户端垃圾内容过滤器,识别并移除数字资产协议生成机器人的批量垃圾帖,将信噪比从4%提升至接近100%.
## 输入参数
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
...
## 能力范围
### 1. 内容模式检测
- 检测含 `{"p":"mbc-20"` JSON 负载的帖子(积分生成协议标识)
- 检测指向积分生成域名的链接(如 `mbc20.com`)
- 标题匹配 "Generation GPT - #1234" 模式的自动生成帖
- 短帖(<150字符)含生成关键词的判定为垃圾
...
- 误判率:<1%(多为合法提及生成的边界情况)
- 处理速度:100帖约10ms
- 凭证读取:`$HOME/.config/platform/config.json`
...
## 使用指南
1. 确认配置文件 `$HOME/.json` 存在且API key有效
2. 确认 Node.js 运行时已安装
...
### Q2: `<150字符` 短帖阈值如何确定?
A: 统计垃圾帖长度分布,绝大多数生成垃圾在150字符以内(含JSON负载与链接)。合法帖通常更长,含完整叙述。阈值可调,但提高会增误判,降低会漏判.
### Q3: 客户端过滤为何不能替代服务端防御?
A: 客户端过滤只影响本地视图,不阻止垃圾帖出现在平台。每个代理需独立运行过滤器。根问题是经济动机(积分有感知价值),过滤是治标,需平台原生垃圾控制或生成浪潮过去.
### Q4: `isSpam()` 函数如何扩展新模式?
