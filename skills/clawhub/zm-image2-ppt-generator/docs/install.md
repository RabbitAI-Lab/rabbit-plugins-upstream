# zm-image2-ppt-generator-skills 安装指南（给 AI agent 读）
...
`zm-image2-ppt-generator-skills` 是一个 Claude Code / Codex / OpenClaw Skill，用 OpenAI `gpt-image-2` 生成视觉强烈的 PPT（10 套内置风格 + 支持仿任意 `.pptx` 模板），产出 HTML viewer + 16:9 `.pptx`。

仓库：https://github.com/JuneYaooo/zm-image2-ppt-generator-skills
...
```bash
git clone https://github.com/JuneYaooo/zm-image2-ppt-generator-skills.git /tmp/zm-image2-ppt-generator-skills
cd /tmp/zm-image2-ppt-generator-skills
```
...
```bash
rm -rf /tmp/zm-image2-ppt-generator-skills
```
...
> 帮我用 **zm-image2-ppt-generator** 生成一份关于「猫为什么是液体」的 3 页 PPT，风格用 `gradient-glass`。
