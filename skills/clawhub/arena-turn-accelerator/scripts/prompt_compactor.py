...
def verify_preserved(original, compacted):
    """Anti-hallucination gate (v2.0.0): PROVE compaction kept every
    constraint-bearing span of the original. Three checks:
      spans   — every vaulted code/quote span of the original appears verbatim
      numbers — the multiset of number tokens may shrink (duplicates dropped)
                only by repetition, never introduce/lose a distinct value... so
                we require every DISTINCT number token of the original present
      signals — every DISTINCT constraint-signal word (must/never/only/fa-fو…)
                present in the original still appears in the output
    Returns {"ok": bool, "missing": {...}, "checked": {...}} — a machine
    contract: ok==False means the compactor DROPPED constraint content.
    Deliberately protective/conservative: numbers are checked by DISTINCT
    token presence (merging repeated identical constraints is allowed); spans
    must survive verbatim (this compactor's vault guarantees that). Patterns
    containing \\d or quote spans are excluded from word-signals because the
    numbers check and the vault check already cover exactly those.
    """
    from collections import Counter
    if not isinstance(original, str) or not isinstance(compacted, str):
        return {"ok": False, "reason": "non-string input",
                "missing": {"spans": [], "numbers": [], "signals": []},
                "checked": {"spans": 0, "numbers": 0, "signals": 0}}
    _stashed, vault = protect(original)
    miss_spans = [v[:80] for v in vault if v not in compacted]

    def _numtok(t):
        return Counter(re.findall(r"\d+(?:[.,:/-]\d+)*", t))
    miss_numbers = sorted(set(_numtok(original)) - set(_numtok(compacted)))

    word_sigs = [p for p in CONSTRAINT_SIGNALS
                 if not p.startswith(("`", '"', "'")) and "\\d" not in p]
    orig_words = set()
    for p in word_sigs:
        for m in re.finditer(p, original, re.I):
            orig_words.add(m.group(0).lower())
    miss_signals = sorted(w for w in orig_words
                          if not re.search(re.escape(w), compacted, re.I))
    ok = not (miss_spans or miss_numbers or miss_signals)
    return {"ok": ok,
            "missing": {"spans": miss_spans, "numbers": miss_numbers,
                        "signals": miss_signals},
            "checked": {"spans": len(vault), "numbers": len(_numtok(original)),
                        "signals": len(orig_words)}}
...
def main():
    ...
    ap.add_argument("--verify", action="store_true",
                    help="prove all constraints survived compaction; exit 3 on violation")
    a = ap.parse_args()
    ...
    if a.verify:
        v = verify_preserved(src, r["compact"]); r["verification"] = v
    if a.json:
        print(json.dumps(r, indent=2, ensure_ascii=False))
        return 0 if (not a.verify or r["verification"]["ok"]) else 3
    ...
    if a.verify:
        if r["verification"]["ok"]:
            m = r["verification"]["checked"]
            print(f"VERIFIED — constraints preserved (spans={m['spans']} numbers={m['numbers']} signals={m['signals']})")
            return 0
        print("VIOLATED — compactor dropped constraint content:")
        for kind, items in r["verification"]["missing"].items():
            for it in items:
                print(f"  missing {kind}: {it!r}")
        return 3
    return 0
