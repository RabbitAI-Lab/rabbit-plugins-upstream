...
def state_path(basename, agent=None):
    ...
    The directory is created 0700 (owner-only). This state is derived from
    conversation content, so on a shared or multi-tenant host it must not be
    world-readable under the default umask — flagged by the ClawHub security
    scanner in v2.1.0 and hardened in v2.1.1.
    """
    d = state_dir(agent)
    os.makedirs(d, mode=0o700, exist_ok=True)
    # Tighten the directory AND any intermediate dir os.makedirs created with the
    # default umask (e.g. ~/.arena_turn/agents), plus dirs made by older versions.
    _p = d
    while True:
        try:
            os.chmod(_p, 0o700)
            ...
            pass
        if os.path.normpath(_p) == os.path.normpath(_BASE):
            break
        parent = os.path.dirname(_p)
        if not parent or parent == _p or len(parent) < len(_BASE):
            break
        _p = parent
