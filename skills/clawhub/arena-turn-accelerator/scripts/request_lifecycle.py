...
Usage: new <prompt> [--force] | check <gen> | supersede | complete <gen> | resume <gen> | status | diagnose | reset
"""
...
def cmd_new(prompt, force=False):
    ...
    with _Lock():                      # generation bump must be serialized
        ...
        if (not force and cur and cur.get("status") == "running"
            ...
            save(s)
            print(f"DUPLICATE of generation={cur['generation']} "
                  f"(same prompt, {age:.1f}s in flight, retry #{cur['retries']})")
            print(f"ADOPT generation={cur['generation']} — do NOT restart; keep waiting on the "
        ...
        if cur and cur.get("status") == "running":
            ...
            print(f"ABORT generation={cur['generation']} ({why})")
        ...
        save(s)
        print(f"CURRENT generation={s['generation']}")
...
def cmd_check(gen):
    ...
    s = load()
    if gen == s["generation"]:
        print(f"CURRENT generation={gen} -> RENDER"); return 0
    print(f"STALE generation={gen} (current={s['generation']}) -> DISCARD, do not render"); return 1
...
def cmd_status():
    s = load(); print(f"current_generation = {s['generation']}")
    f = s.get("inflight")
    if f and f.get("status") == "running":
        print(f"in-flight: gen={f['generation']} age={time.time()-f['started']:.1f}s prompt={f['prompt'][:70]!r}")
    else: print("in-flight: none")
    stale = sum(1 for h in s["history"] if h.get("status") == "superseded")
    print(f"history: {len(s['history'])} finished, {stale} superseded (stale answers blocked)")
