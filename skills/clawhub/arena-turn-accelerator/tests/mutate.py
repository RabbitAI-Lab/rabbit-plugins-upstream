...
def run_suites(root):
    ...
    # BUG FIXED (v2.1.0): this ran the ORIGINAL tests/test_properties.py while only
    # setting PYTHONPATH at the mutated tree. That file does
    # `sys.path.insert(0, <its own>/../scripts)`, and an explicit sys.path.insert
    # beats PYTHONPATH — so the property suite imported the PRISTINE modules and
    # could never observe the mutation. Two real mutants ("--urgent ignored",
    # "grief no longer blocks comedy") were therefore reported as surviving blind
    # spots when the tests do in fact catch them. Run the COPIED test file so its
    # relative path resolution lands inside the mutated tree.
    copied_test = os.path.join(root, "tests", "test_properties.py")
    if not os.path.exists(copied_test):
        return False, "properties-missing"
    pt = subprocess.run([sys.executable, "-m", "pytest", copied_test,
                         "-q", "-x", "--no-header", "-p", "no:cacheprovider"],
                        capture_output=True, text=True, timeout=900,
                        cwd=root, env=dict(env, PYTHONPATH=root + "/scripts"))
    if pt.returncode == 5:            # no tests collected = harness failure, not a kill
        return False, "properties-empty"
    if pt.returncode != 0:
