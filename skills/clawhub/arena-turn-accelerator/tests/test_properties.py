...
try:
    ...
    # BUG FIXED (v2.1.0): this used to raise, and the runner counted the crash as a
    # pass — a FALSE GREEN that hid every property test. Exit 77 is the widely used
    # "skipped" code (autotools/GNU convention), so a harness can tell SKIPPED apart
    # from PASSED instead of silently believing the suite ran.
    print("SKIP test_properties.py — optional dependency 'hypothesis' is not installed")
    print("     install it to run property tests:  python3 -m pip install hypothesis")
...
# ─────────────────────────────────────────────────────────────────────────────
# BUG FIXED (v2.1.0): this module defined property tests but never ran them.
# `python3 tests/test_properties.py` imported the file, defined the functions and
# exited 0 — a FALSE GREEN that reported success while executing nothing. The
# runner below discovers every test_* function, runs it, and exits non-zero on the
# first failure so a harness can actually trust the result.
# ─────────────────────────────────────────────────────────────────────────────
def _run_all():
    import traceback
    ...
    for name, fn in tests:
        try:
            ...
            print(f"FAIL {name}")
            traceback.print_exc()
    print(f"-------- {passed} passed, {failed} failed ({len(tests)} property tests) --------")
