...
def _child_env(home):
    ...
    BUG FIXED (v2.1.0): this inherited the ambient environment wholesale. If the
    operator had ARENA_AGENT exported, the CLI wrote its state to
    ~/.arena_turn/agents/<name>/lifecycle.json while `state()` below still read the
    legacy shared ~/.arena_turn/lifecycle.json. The checker saw generation 0 forever
    and reported a FALSE safety violation ("current gen 0 refused to render") against
    perfectly correct code. The state namespace must be pinned, not inherited.
    """
    env = dict(os.environ, HOME=home)
    env.pop("ARENA_AGENT", None)
    return env


def run(home, args):
    return subprocess.run([sys.executable, CLI] + args, capture_output=True, text=True,
                          env=_child_env(home), timeout=60)
...
def state(home):
    ...
    with open(p, "r", encoding="utf-8") as fh:   # utf-8 + closed handle
        return json.load(fh)
