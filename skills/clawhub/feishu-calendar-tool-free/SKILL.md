...
**处理**: 解析列出可用日历及 ID的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回列出可用日历及 ID的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 按名称搜索日历
按名称搜索日历

**处理**: 解析按名称搜索日历的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回按名称搜索日历的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查指定人员日历状态
检查指定人员日历状态

**处理**: 解析检查指定人员日历状态的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回检查指定人员日历状态的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 创建日历事件并设置参与人
创建日历事件并设置参与人

**处理**: 解析创建日历事件并设置参与人的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回创建日历事件并设置参与人的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 同步日程到本地状态
同步日程到本地状态

**处理**: 解析同步日程到本地状态的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回同步日程到本地状态的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 飞书日历基础版处理的输入数据或指令 |
