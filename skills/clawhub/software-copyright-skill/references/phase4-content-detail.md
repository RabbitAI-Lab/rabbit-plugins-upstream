...
| 生成架构图 | 🐍 excalidraw | 使用excalidraw skill生成，render_excalidraw.py渲染 |
| 插入图片 | 🐍 python-docx | 在图名段落前插入PNG图片 |
...
┌─────────────────────────────────────────────────────────────┐
│ 7. 生成架构图并插入文档（第一章专用）                          │
│    - 使用 excalidraw skill 生成架构图JSON                     │
│    - 使用 render_excalidraw.py 渲染为PNG（300dpi）            │
│    - 用 python-docx 在图名段落前插入图片                      │
│    - ⚠️ 从后往前插入，避免段落索引变化                        │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
...
**执行者**：excalidraw skill + Python脚本

**时机**：合并章节后，验证内容质量前

**⚠️ 只为第一章生成架构图**，其他章节使用浏览器截图

**流程**：
1. 根据第一章内容，使用excalidraw skill生成架构图JSON
2. 使用 `render_excalidraw.py` 渲染为PNG（300dpi，宽度1000px）
3. 用python-docx在图名段落前插入图片
4. **⚠️ 从后往前插入**，避免段落索引变化
...
# 精确匹配图名（必须完全匹配，避免误匹配）
image_mapping = {
    "图8 系统总体架构图": "diagrams/系统总体架构图.png",
    "图9 前端技术架构": "diagrams/前端技术架构.png",
}

# 找到位置
positions = []
for i, para in enumerate(doc.paragraphs):
    for fig_name, img_path in image_mapping.items():
        if para.text.strip() == fig_name:
            positions.append((i, img_path))

# 从后往前插入（避免索引变化）
for pos, img_path in sorted(positions, reverse=True):
    new_para = doc.paragraphs[pos].insert_paragraph_before("")
...
**⚠️ 图名匹配必须精确**：`text == fig_name`，不要用 `startswith`，否则图80-89也会匹配到图8
...
6. **架构图独立生成**：使用excalidraw skill生成，不依赖浏览器截图
