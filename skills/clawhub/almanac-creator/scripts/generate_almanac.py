...
# -*- coding: utf-8 -*-
"""
黄历生成脚本 V3.0.4 - 3D 立体效果增强版
升级内容：
1. 【V3.0.4 新增】渐变背景 - 径向渐变替代纯色，产生光晕聚焦效果
2. 【V3.0.4 新增】文字阴影 - 多层偏移绘制，产生浮起立体感
3. 【V3.0.4 新增】装饰光晕 - 顶部金色光效，高端大气
4. 【V3.0.4 新增】立体边框 - 多层边框 + 渐变模拟厚度
5. 【V3.0.4 新增】祥云/角花装饰 - 传统纹样增加中国风
6. 【V3.0.1 新增】节气计算通用化（支持 1900-2100 年）
7. 【V3.0.1 新增】批量生成功能（一次生成 7 天/30 天）
8. 【V3.0.1 新增】配置文件支持（config.yaml）
9. 集成 lunar-python 库（lunar-java 的 Python 版本）
10. 干支计算 - 使用 lunar-python 准确计算
11. 宜忌生成 - 使用 lunar-python 传统算法
...
def create_radial_gradient(img, center_x, center_y, radius, color_center, color_edge):
    """【V3.0.4 新增】创建径向渐变背景"""
    pixels = img.load()
    ...
    for y in range(height):
        for x in range(width):
            ...
            # 插值颜色
            color = lerp_color(color_center, color_edge, t_smooth)
            pixels[x, y] = hex_to_rgb(color)
    
    return img


def draw_centered_text_shadow(draw, text, y, font, text_color, shadow_color='#000000', shadow_offset=2):
    """【V3.0.4 新增】居中绘制带阴影的文字（3D 立体效果）"""
    bbox = draw.textbbox((0, 0), text, font=font)
    text_width = bbox[2] - bbox[0]
    x = (WIDTH - text_width) // 2
    ...
    for i in range(3, 0, -1):
        alpha_offset = shadow_offset + i
        draw.text((x + alpha_offset, y + alpha_offset), text, fill=shadow_color, font=font)
...
def draw_gradient_border(draw, width, height, template):
    """【V3.0.4 新增】绘制渐变立体边框"""
    border_outer = template['border_outer']
    border_inner = template['border_inner']
    
    # 外层边框（粗）
    draw.rectangle([8, 8, width-8, height-8], outline=border_outer, width=10)
    
    # 中层边框（渐变效果）
    draw.rectangle([14, 14, width-14, height-14], outline=border_outer, width=3)
    
    # 内层边框（细金色）
    draw.rectangle([22, 22, width-22, height-22], outline=border_inner, width=2)
    
    # 添加金色光效线
    draw.rectangle([26, 26, width-26, height-26], outline=border_inner, width=1)


def draw_cloud_decoration(draw, x, y, size, color, scale=1.0):
    """【V3.0.4 新增】绘制祥云装饰"""
    s = int(size * scale)
    # 祥云主体（多个相连的圆弧）
    draw.arc([x, y, x+s, y+s], 0, 180, fill=color, width=3)
    draw.arc([x+s//2, y-s//4, x+s*1.5, y+s*0.75], 0, 180, fill=color, width=3)
    draw.arc([x+s//4, y-s//8, x+s*1.25, y+s*0.92], 0, 180, fill=color, width=2)
    # 云脚
    draw.line([(x+s//2, y+s//2), (x+s//2, y+s)], fill=color, width=2)
    draw.line([(x+s, y+s//2), (x+s, y+s)], fill=color, width=2)


def draw_corner_flower(draw, x, y, size, color, corner='tl', scale=1.0):
    """【V3.0.4 新增】绘制四角装饰花"""
    s = int(size * scale)
    
    if corner == 'tl':  # 左上角
        draw.arc([x, y, x+s, y+s], 0, 90, fill=color, width=3)
        draw.arc([x, y, x+s//2, y+s//2], 0, 90, fill=color, width=2)
        # 装饰点
        draw.ellipse([x+s//4, y+s//4, x+s//4+6, y+s//4+6], fill=color)
    elif corner == 'tr':  # 右上角
        draw.arc([x-s, y, x, y+s], 90, 180, fill=color, width=3)
        draw.arc([x-s//2, y, x, y+s//2], 90, 180, fill=color, width=2)
        draw.ellipse([x-s//4-6, y+s//4, x-s//4, y+s//4+6], fill=color)
    elif corner == 'bl':  # 左下角
        draw.arc([x, y-s, x+s, y], 270, 360, fill=color, width=3)
        draw.arc([x, y-s//2, x+s//2, y], 270, 360, fill=color, width=2)
        draw.ellipse([x+s//4, y-s//4-6, x+s//4+6, y-s//4], fill=color)
    elif corner == 'br':  # 右下角
        draw.arc([x-s, y-s, x, y], 180, 270, fill=color, width=3)
        draw.arc([x-s//2, y-s//2, x, y], 180, 270, fill=color, width=2)
        draw.ellipse([x-s//4-6, y-s//4-6, x-s//4, y-s//4], fill=color)


def draw_top_glow(draw, width, height, template):
    """【V3.0.4 新增】绘制顶部金色光晕效果"""
    border_inner = template['border_inner']
    
    # 顶部装饰线
    draw.line([(100, 60), (width-100, 60)], fill=border_inner, width=2)
    ...
    for offset in [60, 120, 180]:
        # 左侧
        draw.ellipse([center_x - offset - 4, 58, center_x - offset + 4, 62], fill=border_inner)
        # 右侧
        draw.ellipse([center_x + offset - 4, 58, center_x + offset + 4, 62], fill=border_inner)


def apply_radial_gradient_background(img, template):
    """【V3.0.4 新增】应用径向渐变背景"""
    bg_color = template['bg']
    border_outer = template['border_outer']
    
    # 计算渐变颜色（中心更亮，边缘使用边框色）
    center_color = '#FFFFFF'  # 中心白色光晕
    edge_color = bg_color     # 边缘使用模板背景色
    
    # 创建渐变
    center_x = WIDTH // 2
    center_y = PAGE_HEIGHT // 2
    radius = max(WIDTH, PAGE_HEIGHT) * 0.7
    
    return create_radial_gradient(img, center_x, center_y, radius, center_color, edge_color)
...
def draw_decoration(draw, template_name, template):
    """【V3.0.4 升级】根据模板绘制装饰元素（增强 3D 效果）"""
    decoration_type = template.get('decoration', 'double_border')
    
    # 【V3.0.4 新增】所有模板都使用立体边框
    draw_gradient_border(draw, WIDTH, PAGE_HEIGHT, template)
    
    # 【V3.0.4 新增】顶部光晕装饰
    draw_top_glow(draw, WIDTH, PAGE_HEIGHT, template)
    
    # 【V3.0.4 新增】四角装饰花
    border_inner = template['border_inner']
    corner_size = 50
    # 左上角
    draw_corner_flower(draw, 35, 35, corner_size, border_inner, 'tl')
    # 右上角
    draw_corner_flower(draw, WIDTH-35, 35, corner_size, border_inner, 'tr')
    # 左下角
    draw_corner_flower(draw, 35, PAGE_HEIGHT-35, corner_size, border_inner, 'bl')
    # 右下角
    draw_corner_flower(draw, WIDTH-35, PAGE_HEIGHT-35, corner_size, border_inner, 'br')
    
    # 【V3.0.4 新增】祥云装饰（根据模板类型）
    if decoration_type in ['double_border', 'cloud', 'ornament']:
        # 底部祥云
        cloud_color = template['border_inner']
        draw_cloud_decoration(draw, 150, PAGE_HEIGHT-80, 40, cloud_color, 0.8)
        draw_cloud_decoration(draw, WIDTH-200, PAGE_HEIGHT-80, 40, cloud_color, 0.8)
    
    # 原有模板特定装饰（保留兼容性）
    if decoration_type == 'single_border':
        # 简约风格额外装饰
        pass
...
def get_month_ganzhi_by_jieqi(year, month, day):
    ...
    # 获取年干（使用 Solar 转换公历→农历，避免 Lunar.fromYmd 把公历当农历解析）
...
def get_almanac_data(date_str):
    ...
    # 【V3.0.2 修复】使用 lunar_python.Solar 转换公历→农历（避免 Lunar.fromYmd 把公历当农历解析）
    from lunar_python import Solar as LunarPythonSolar
    lunar = LunarPythonSolar.fromYmd(date_obj.year, date_obj.month, date_obj.day).getLunar()
...
def generate_page1(data, fonts, output_dir, date_str, template_name=None, template=None):
    ...
    # 【V3.0.4 升级】使用模板配色 + 径向渐变背景
    img = Image.new('RGB', (WIDTH, PAGE_HEIGHT), template['bg'])
    
    # 【V3.0.4 新增】应用渐变背景
    img = apply_radial_gradient_background(img, template)
    ...
    y = 85
    # 【V3.0.4 新增】标题使用阴影效果
    y = draw_centered_text_shadow(draw, "每日黄历", y, fonts['title'], template['title'], shadow_offset=3)
...
def generate_page2(data, fonts, output_dir, date_str, template_name=None, template=None):
    ...
    y = 95
    # 【V3.0.4 新增】标题使用阴影效果
    y = draw_centered_text_shadow(draw, "财神方位", y, fonts['section'], template['section'], shadow_offset=2)
    y += 15
...
def generate_page3(data, fonts, output_dir, date_str, template_name=None, template=None):
    ...
    # 【V3.0.4 新增】标题使用阴影效果
    y = draw_centered_text_shadow(draw, "今日穿衣建议", y, fonts['section'], template['section'], shadow_offset=2)
    y += 15
...
def get_default_output():
    """【V3.0.4 修复】固定输出到技能目录下的 output/ 文件夹"""
    # 1. 优先使用环境变量（用户可自定义）
    if 'OPENCLAW_OUTPUT' in os.environ:
        return os.environ['OPENCLAW_OUTPUT']
    
    # 2. 默认输出到技能目录的 output/ 文件夹（绝对路径）
    script_dir = os.path.dirname(os.path.abspath(__file__))
    skill_dir = os.path.dirname(script_dir)
    return os.path.join(skill_dir, 'output')
