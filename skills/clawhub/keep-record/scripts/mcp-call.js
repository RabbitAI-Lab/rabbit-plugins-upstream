 ...
 *
 * 保留这个脚本路径（packages/keep-record/scripts/mcp-call.js）是为了
 * 兼容运行器里既有的 exec 命令模板：
 *   node {baseDir}/scripts/mcp-call.js <tool> '<json_args>'
 *
 * 详细用法、退出码、环境变量见 @keepclaw/skill-sdk 的 src/mcp-cli.js 注释。
 *
 * 埋点：为与 `keep` CLI 区分，exec 桥默认 `KEEP_INVOKE_SOURCE=keep-record`（见 skill-sdk reportInvoke source）。
 * 需自定义时可在外层环境覆盖。
 */

if (!process.env.KEEP_INVOKE_SOURCE) {
  process.env.KEEP_INVOKE_SOURCE = "keep-record";
}
