...
> **术语统一说明**：下文"净利润"默认指**归属于母公司股东的净利润**（归母净利润）。若数据源仅含含少数股东的总净利润，需标注差异。ROE 计算使用归母净利润/平均归母净资产；ROA 计算使用总净利润/平均总资产。财报报表项目命名以 `financial-statements.md` 为准。

逐层拆解：
- **净利率**（归母净利润/营业收入）：盈利能力
- **资产周转率**（营业收入/平均总资产）：运营效率
- **权益乘数**（平均总资产/平均归母净资产）：杠杆水平
...
- 投研分析 `earnings-review.md`：上市公司财报点评和估值分析场景，可引用本分析的杜邦拆解和五维指标结果

## 失败回退机制

| 步骤 | 失败条件 | 回退目标 | 最大重试 | 不可恢复时升级路径 |
|------|---------|---------|---------|------------------|
| 第一步：数据提取与整理 | 财务报表数据缺失（如仅有利润表无资产负债表）或 Notion 读取失败 | 降级为基于已有报表做部分分析，缺失报表对应指标标注"[报表缺失，该指标待补充]" | 2次 | 建议用户补充缺失报表后重新做完整分析 |
| 第二步：杜邦分析 | 缺少期初/期末净资产数据，无法计算平均余额 | 用期末值替代平均值，标注"[使用期末值非平均值，ROE可能有偏差]" | 1次 | 建议用户补充期初数据后重新计算 |
| 第三步：偿债能力分析 | 有息负债明细缺失，无法准确计算有息负债率 | 用"短期借款+长期借款"近似估算，标注"[有息负债为估算值，建议核实应付债券/租赁负债]" | 2次 | 建议用户从报表附注补充完整有息负债明细后重新计算 |
| 第四步：运营能力分析 | 缺少上期应收/存货/应付数据，无法计算平均余额 | 仅用期末余额计算，标注"[使用期末值非平均值，周转率可能有偏差]" | 1次 | 建议用户补充上期数据后重新计算周转率 |
| 第五步：盈利能力分析 | 扣非净利润数据缺失，无法做盈利质量分析 | 跳过扣非分析，标注"[扣非净利润待补充，盈利质量分析待完善]" | 1次 | 建议用户从报表附注获取非经常性损益明细后补充分析 |
| 第六步：现金流质量分析 | 现金流量表缺失，无法做现金流组合分析 | 跳过现金流分析，标注"[现金流量表缺失，现金流质量分析待补充]" | 1次 | 建议用户补充现金流量表后重新分析 |
| 第七步：Z-Score 破产预测模型 | 非上市公司缺少股权市值数据（X4无法计算） | 改用非上市公司修正模型Z'，标注"[使用非上市修正模型Z'，X4用账面值替代]" | 1次 | 建议用户确认企业是否为上市主体后选择正确模型 |
| 第八步：EVA 经济增加值 | WACC参数缺失（无风险利率/Beta/债务成本未知） | 使用行业平均WACC估算，标注"[WACC为行业估算值，EVA结果仅供参考]" | 2次 | 建议用户提供企业实际WACC参数后重新计算EVA |
| 第九步：综合诊断 | 多个维度数据缺失，预警信号清单无法完整评估 | 仅基于已有数据输出诊断，缺失维度标注"[数据不足，该维度诊断待补充]" | 1次 | 建议用户补全关键财务数据后做完整综合诊断 |
