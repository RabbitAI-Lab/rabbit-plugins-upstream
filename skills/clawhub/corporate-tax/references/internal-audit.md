...
|------|---------|---------|---------|------------------|
| 第一步：审计计划 | 审计范围描述过于模糊或可用资料缺失，无法构建风险评估矩阵 | 默认按"全面审计+六大风险领域全扫描"执行，标注"[审计范围未明确，按通用风险评估执行]" | 2次 | 建议用户明确审计范围和目标后重新制定审计计划 |
| 第二步：审计程序设计 | 业务流程文件缺失，无法设计穿行测试和控制测试程序 | 输出通用审计程序框架，标注"[具体业务流程未提供，审计程序待业务部门确认后细化]" | 2次 | 建议用户提供业务流程文件和制度文件后补充具体审计程序 |
| 第三步：数据分析审计技术 | 业务数据 Excel 缺失或字段不全，无法执行 Benford 检验/异常检测 | 跳过数据分析审计，标注"[业务数据不足，数据分析审计待补充]" | 1次 | 建议用户从业务系统导出完整交易明细后单独执行数据分析审计 |
| 第四步：工作底稿与抽样 | 总体规模未知或重要性水平未确定，无法计算样本量 | 按默认重要性水平（税前利润5%）计算样本量，标注"[重要性水平为默认值，建议审计主管确认]" | 1次 | 建议用户与审计主管确认重要性水平和抽样方案后重新执行 |
| 第五步：审计发现汇总 | 抽样证据不足以支撑风险等级评定 | 降低风险等级或标注"[证据不足，风险等级为初步判断]"，不作为最终结论 | 2次 | 建议扩大抽样范围或追加审计程序后重新评估风险等级 |
| 第六步：审计报告 | Notion 连接失败或整改事项无法录入数据库 | 降级为输出完整审计报告 Markdown 文件，附整改事项清单供手动跟踪 | 2次 | 输出 Markdown 报告并提示用户检查 Notion 连接器配置，整改事项建议人工录入跟踪系统 |
