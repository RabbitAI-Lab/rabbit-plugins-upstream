...
description: "当用户需要搜索 Bing Shopping、查找商品或购物结果时，使用此 skill。"
---
...
```bash
python3 scripts/bing_shopping.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/bing_shopping.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
