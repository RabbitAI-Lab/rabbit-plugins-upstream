# Temperature Anomaly Mapping (geoskill-temperature-anomaly-mapping)

> Computes temperature anomalies (current temperature minus the multi-year climatology for the same period) and standardized anomalies, classifies warm/cold anomaly levels, and outputs anomaly rasters, an anomaly class GeoTIFF, and a time-series JSON.

---

## 1. Overview

Computes and maps **temperature anomalies** to identify warm/cold anomaly regions relative to the climatology and their intensity levels. Suitable for monthly/annual temperature anomaly monitoring, extreme warm/cold event mapping, and regional diagnostics against the climate baseline. Core algorithm: - **Climatology**: groups the time series by seasonal phase (e.g., month, `--n-dates`) and averages the values within each group across years to derive the multi-year climatology for that phase. - **Anomaly** = current temperature − climatology. - **Standardized anomaly** = anomaly / multi-year standard deviation for the same period, removing units so anomaly intensity is comparable across seasons/regions. - **Anomaly class**: classified by ±1σ and ±2σ thresholds into severe warm anomaly / warm anomaly / normal / cold anomaly / severe cold anomaly, output as an integer-encoded class raster. A built-in `--synthetic` mode generates a simulated monthly temperature series containing climatology (spatial baseline + seasonal cycle) plus interannual noise, with a regional warm anomaly injected at the end, for offline validation.
...
python geoskill-temperature-anomaly-mapping.py --bbox 116 39 117 40 --synthetic --output-dir ./out
```
...
| `anomaly_class.tif` | GeoTIFF (float32, 1 band) | Anomaly class code (2 = severe warm…0 = normal…-2 = severe cold) |
| `timeseries.json` | JSON | Per-epoch spatial-mean anomaly / standardized anomaly + share of the latest class |
