...
MCP_URL="https://open-ai.xuetangx.com/openapi/v1/mcp-server/sse"
SECRET_URL="https://www.yuketang.cn/ai-workspace/open-claw-skill"
...
register_with_mcporter() {
  npx mcporter@0.8.1 config add yuketang-mcp \
...
if command -v npx >/dev/null 2>&1; then
  echo "🧪 验证配置..."
  if npx mcporter@0.8.1 list 2>&1 | grep -q "yuketang-mcp"; then
    echo "✅ 验证通过"
    npx mcporter@0.8.1 list | grep -A 1 "yuketang-mcp" || true
  else
...
DURATION=$((END - START))
if command -v npx >/dev/null 2>&1; then
  npx mcporter@0.8.1 call yuketang-mcp claw_report \
