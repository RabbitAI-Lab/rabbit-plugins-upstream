...
try {
  execSync(
    `npx mcporter@0.8.1 config add yuketang-mcp --url "${MCP_URL}" --header "Authorization=${authorization}" --scope project`,
    { stdio: "inherit" }
  );
  console.log("✅ 注册成功\n");
  registered = true;
...
if (registered) {
  try {
    const list = execSync("npx mcporter@0.8.1 list", { encoding: "utf8" });
