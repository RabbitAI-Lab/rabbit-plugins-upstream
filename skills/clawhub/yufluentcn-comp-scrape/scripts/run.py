...
SKILL_API_ID = "comp-scrape"


def main() -> int:
    parser = argparse.ArgumentParser(description="TokenApi Comp Scrape（云端）")
    parser.add_argument("--our-product", required=True, help="我方产品名")
    parser.add_argument(
        "--competitor-data",
        required=True,
        help="竞品 CSV/导出内容或 .csv/.txt 文件路径",
    )
    parser.add_argument("--our-listing", help="我方 Listing 文案或文件路径")
    parser.add_argument(
        "--source-type",
        dest="source_type",
        choices=["csv_export", "api_snapshot"],
        default="csv_export",
        help="数据来源类型",
    )
    ...
    payload: dict = {
        ...
        "competitor_data": read_text_arg(args.competitor_data),
