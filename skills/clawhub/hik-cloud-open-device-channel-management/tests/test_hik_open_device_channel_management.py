...
MODULE_PATH = (
    ...
    / "scripts"
    / "hik_open_device_channel_management.py"
)
SPEC = importlib.util.spec_from_file_location("hik_open_device_channel_management", MODULE_PATH)
...
class HikOpenDeviceChannelManagementTests(unittest.TestCase):
    ...
    def test_build_list_spec_uses_query_contract(self):
        args = MOD.parse_args(
            ["list", "--device-serial", "E05426006", "--page-no", "1", "--page-size", "50"]
        )
        spec = MOD.build_list_spec(args)
        self.assertEqual(spec.method, "GET")
        self.assertEqual(spec.path, MOD.LIST_PATH)
        self.assertEqual(
            spec.query,
            {"deviceSerial": "E05426006", "pageNo": 1, "pageSize": 50},
        )

    def test_build_sync_spec_uses_json_contract(self):
        args = MOD.parse_args(["sync", "--device-serial", "E05426006"])
        spec = MOD.build_sync_spec(args)
        self.assertEqual(spec.method, "POST")
        self.assertEqual(spec.path, MOD.SYNC_PATH)
        self.assertEqual(spec.json_body, {"deviceSerial": "E05426006"})
    ...
    def test_build_rename_spec_uses_json_contract(self):
        ...
        self.assertEqual(spec.method, "POST")
        self.assertEqual(spec.path, MOD.RENAME_PATH)
        self.assertEqual(
            spec.json_body,
            {
    ...
    def test_build_sync_names_spec_uses_json_contract(self):
        args = MOD.parse_args(["sync-names", "--device-serial", "E05426006"])
        spec = MOD.build_sync_names_spec(args)
        self.assertEqual(spec.method, "POST")
        self.assertEqual(spec.path, MOD.SYNC_NAMES_PATH)
        self.assertEqual(spec.json_body, {"deviceSerial": "E05426006"})
