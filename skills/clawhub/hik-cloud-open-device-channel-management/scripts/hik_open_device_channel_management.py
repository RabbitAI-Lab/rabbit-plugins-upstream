#!/usr/bin/env python3
"""Hik-Cloud OpenAPI device channel management helper."""
...
LIST_PATH = "/api/v1/open/basic/channels/list"
SYNC_PATH = "/api/v1/open/basic/channels/actions/sync"
RENAME_PATH = "/api/v1/open/basic/channels/update"
SYNC_NAMES_PATH = "/api/v1/open/basic/channels/actions/name/sync"
...
def build_sync_spec(args: argparse.Namespace) -> RequestSpec:
    return RequestSpec(
        method="POST",
        path=SYNC_PATH,
...
def build_rename_spec(args: argparse.Namespace) -> RequestSpec:
    return RequestSpec(
        ...
        json_body={
            ...
            "syncLocal": args.sync_local,
        },
    )


def build_sync_names_spec(args: argparse.Namespace) -> RequestSpec:
    return RequestSpec(
        method="POST",
        path=SYNC_NAMES_PATH,
        json_body={"deviceSerial": args.device_serial},
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hik-Cloud device channel management helper.")
    ...
    add_device_serial_argument(sync)
    sync.set_defaults(spec_builder=build_sync_spec)

    rename = subparsers.add_parser("rename")
    add_common_arguments(rename)
    add_device_serial_argument(rename)
    rename.add_argument("--channel-no", required=True, type=int)
    rename.add_argument("--channel-name", required=True)
    rename.add_argument("--sync-local", type=int, default=0)
    rename.set_defaults(spec_builder=build_rename_spec)

    sync_names = subparsers.add_parser("sync-names")
    add_common_arguments(sync_names)
    add_device_serial_argument(sync_names)
    sync_names.set_defaults(spec_builder=build_sync_names_spec)
