...
def build_plan(data_dict, input_lang="auto", user_input=""):
    ...
    # ── 不再使用默认示例数据 ──
    # 用户必须提供data_dict，否则报错
    if data_dict is None:
        raise ValueError("❌ data_dict is REQUIRED / data_dict为必传参数。请提供真实市场数据。")
    
    # ⚠️ 严格：data_dict已通过验证不为空
    # 所有字段必须由调用方提供，硬编码的示例数据已全部移除
    # 只有纯结构性/模板默认值被保留（如交易所基本信息，不含真实行情数据）
    structural_defaults = {
        'exchange': '上海期货交易所',
        'unit': '5吨/手',
        ...
        'data_source': '用户手动输入或API获取',
        'data_note': '',
    }
    
    d = {**structural_defaults, **data_dict}
