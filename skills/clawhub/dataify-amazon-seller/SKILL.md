...
description: "Collect Amazon seller profile and seller information from a known seller URL. Do not use for product details, listings, or reviews."
---

# Dataify Amazon Seller

Submit Amazon seller information collection jobs by URL through Dataify Builder and continue through final-result retrieval. After submission, continue monitoring the returned `task_id` and return the final result by default.


## Quick Start

**Input:** an Amazon seller URL.

```bash
python3 scripts/submit_amazon_seller.py --url "https://www.amazon.com/sp?seller=SELLER_ID"
```

The command waits up to 10 minutes and prints the final JSON result. Add `--no-wait` only when you want the submission response and `task_id` without waiting.
