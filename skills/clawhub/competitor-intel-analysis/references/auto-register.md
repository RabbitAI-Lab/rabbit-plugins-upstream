...
- 用户同意 → 继续步骤 1
- 用户拒绝 → 输出手动注册链接 `https://ai.zhiliaobiaoxun.com/?ch=s118`，本流程终止
...
```
此时不要重试，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s118` 手动登录注册。
...
def on_balance_exhausted(api_key, source):
    if source == "auto":
        ...
        print("https://ai.zhiliaobiaoxun.com/?ch=s118")
```
