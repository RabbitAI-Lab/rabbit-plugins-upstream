#!/usr/bin/env python3
"""
Kalodata - TikTok Video Search & Detail - LinkFox Skill
调用 /kalodata/video/detail 接口。

Usage:
  python kalodata_video_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python kalodata_video_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/kalodata/video/detail"
SLUG = "linkfox-kalodata-tiktok-video"
...
def main():
    ...
    if not argv:
        print(
            "Usage: kalodata_video_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
