# -*- coding: utf-8 -*-
"""因子分析报告生成器：读取 stock-factor 技能 output/*.xlsx，自动统计分析并生成 HTML 报告。"""
...
FILES = {
    "Qlib Alpha158":        "facotr-Qlib_alpha158.xlsx",
    "Qlib Alpha360":        "facotr-Qlib_alpha360.xlsx",
    "WorldQuant Alpha101":  "factor-Alpha101.xlsx",
    "国泰君安 GTJA Alpha191": "factor-GTJA_Alpha191.xlsx",
    "TA-Lib 技术指标":       "factor-Indicators.xlsx",
    "stock_daily 基础因子":   "factor-stock_daily.xlsx",
    "stock_report 财务因子":  "factor-stock_report.xlsx",
}
SHORT = {
    "Qlib Alpha158": "Alpha158", "Qlib Alpha360": "Alpha360", "WorldQuant Alpha101": "Alpha101",
    "国泰君安 GTJA Alpha191": "GTJA191", "TA-Lib 技术指标": "TA", "stock_daily 基础因子": "Daily",
    "stock_report 财务因子": "Report",
}
...
for fam, fn in FILES.items():
    df = pd.read_excel(os.path.join(OUT, fn))
    df["family"] = fam
    df["short"] = SHORT[fam]
    frames.append(df)
    print(f"[load] {fam}: {len(df)} rows, stock_count~{df['stock_count'].median():.0f}")
all_df = pd.concat(frames, ignore_index=True)
print(f"[load] TOTAL factors = {len(all_df)}")
...
fams = list(FILES.keys())
tot = [stats[f]["n"] for f in fams]
eff = [stats[f]["eff"] for f in fams]
x = np.arange(len(fams))
ax.bar(x, tot, color="#cbd5e1", label="因子总数")
ax.bar(x, eff, color="#2563eb", label="有效因子(|IR|>0.3)")
ax.set_xticks(x); ax.set_xticklabels([SHORT[f] for f in fams], rotation=25, ha="right")
ax.set_ylabel("因子数量")
...
# 2) 平均 |IR|
fig, ax = plt.subplots(figsize=(8.2, 4.2))
m = [stats[f]["mean_absIR"] for f in fams]
ax.bar(x, m, color=PALETTE[:len(fams)])
...
for i, f in enumerate(fams):
    d = all_df[all_df.family == f]
    ax.scatter(d["IC"], d["IR"], s=10, alpha=0.6, color=PALETTE[i], label=SHORT[f])
...
def summary_table():
    ...
    for f in fams:
        s = stats[f]
        rows += (f"<tr><td>{esc(f)}</td><td>{s['n']}</td><td>{s['eff']}</td>"
...
def family_detail_table():
    out = ""
    for f in fams:
        d = topn(all_df[all_df.family == f], 10)
        out += f"<h4>{esc(f)} — Top 10 by |IR|</h4><table><thead><tr>"\
...
<h1>股票因子分析报告</h1>
<div class="sub">数据来源：stock-factor 技能 output/*.xlsx ｜ 生成工具：WorkBuddy 自动分析 ｜
...
<div class="card">
  ...
  <p class="note">本次共分析 <b>{grand['n']}</b> 个因子，覆盖 7 个因子族。按 |IR|&gt;0.3 口径，
