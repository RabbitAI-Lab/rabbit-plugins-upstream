...
name: stock-factor
description: >
  股票因子（Stock Factor）技能 —— 为 AI 提供一套已收集、已转写、可由 QuantAll（全A解析）直接运行的
...
> **定位边界**：本技能当前是**因子清单（字典 + 初筛）**，提供"有哪些因子、各自怎么算、初步 IC/IR 表现"。**尚未做精选有效因子、因子去冗余、因子合成与策略挖掘**——这些后续可由 AI 基于 `output/*.xlsx` 继续深入（见 §4.3 边界说明）。
...
- 📁 关键文件：`scripts/task/*.json`（可执行因子代码）、`scripts/output/*.xlsx`（结果）、`scripts/因子初始参考文件/how_code.txt`（代码环境权威说明）、`scripts/因子初始参考文件/因子表达式初始收集.md`（收集基线）、`data/db_translate.json`（字段映射）。
