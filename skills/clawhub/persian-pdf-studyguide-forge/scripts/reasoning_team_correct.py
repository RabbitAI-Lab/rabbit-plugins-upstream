...
Network is opt-in: this script requires --providers and environment variables
named by that config. Successful batch JSON is cached for safe resume. Provider
rotation and split-batch fallback avoid single-model and output-size failures.
"""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('evidence',type=Path);ap.add_argument('--providers',type=Path,required=True)
