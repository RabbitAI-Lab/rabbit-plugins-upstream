...
def extract_traceback(exc: BaseException, *, include_locals: bool = True) -> list[dict]:
    """Walk an exception's traceback and return a structured frame list.

    Each frame dict has: file, line, lineno, function, source_line, locals
    (if include_locals=True). Useful for reviewing what happened without
    entering pdb — e.g., to summarize for the user or grep for patterns.
    """
    frames = []
...
def summarize_exception(exc: BaseException, *, max_frames: int = 20) -> dict:
    ...
    frame list with locals, and the formatted traceback string.
    """
    chain = []
    current = exc
    ...
    return {
        ...
        "message": str(exc),
        "cause_chain": chain,
        "frames": extract_traceback(exc, include_locals=True)[:max_frames],
        "traceback_str": "".join(traceback.format_exception(type(exc), exc, exc.__traceback__)),
    }


def format_exception(exc: BaseException, *, max_frames: int = 10, show_locals: bool = True) -> str:
