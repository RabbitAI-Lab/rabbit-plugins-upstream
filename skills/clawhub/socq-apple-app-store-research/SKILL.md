---
name: socq-apple-app-store-research
description: Research public Apple App Store content, accounts, keywords, and performance data with SocQ. Use when an agent needs Apple App Store-specific discovery, collection, endpoint selection, credit estimates, asynchronous task execution, pagination, or raw exports through the SocQ CLI, MCP, or REST API.
metadata:
  openclaw:
...
- API documentation: <https://docs.socq.ai/api-manual/apple-app-store>
...
Use this Skill to select and run SocQ endpoints for public Apple App Store research. SocQ requests are asynchronous and credit-metered.

## Use When

- The user asks to discover, collect, compare, or analyze public Apple App Store data.
- The workflow needs Apple App Store-specific endpoint selection, input validation, pagination, or raw exports.
...
1. Prefer an already configured SocQ MCP server at `https://api.socq.ai/mcp?platforms=apple-app-store`; use the typed tool listed in [platform.md](references/platform.md), or compact `socq_execute` when needed.
