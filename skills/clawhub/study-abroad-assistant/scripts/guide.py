...
用法示例:
  ...
  python3 guide.py schools --gpa 3.5 --direction "systems,ai" --top 6
  python3 guide.py feedback --type sop --text "I have always been passionate about ..."
  python3 guide.py outreach --mode first --professor '{"name":"Prof. X","field":"Distributed Systems","topics":["systems"]}'
  python3 guide.py report --type decision --payload '{"offers":[{"program":"A","funding":80,"rank":90,"fit":80,"cost":50}]}'
  python3 guide.py plan       # 无参数 = 对话引导
"""
import argparse
import json
import os
...
def cmd_register(args):
    ...
    if not key:
        print("请提供 API Key：`python3 guide.py register --key sk_xxx`"
