...
permissions:
  network:
    - "https://compliancehub.cn"          # 生产引擎（部署后）
    - "http://127.0.0.1:8100"             # 本地引擎（开发）
  filesystem:
    write:
      - "~/.study-abroad"
  env:
...
- **隐私**：本地匿名 id（anon_id）仅用于额度与进度延续；本 skill 不收集账号密码等凭据；用户提供的信息用于画像/选校计算
...
3. **文书/套磁由 agent 直接生成**（基于引擎的画像/选校/教授上下文），带水印"AI 草稿·请自行核实定稿"；**不要直接展示 `guide.py feedback/outreach` 的引擎 mock 模板作为最终输出**——CLI 的 mock 只是无 agent 环境的兜底，agent 在场时必须用自己的模型生成
...
- `TODO.md`：待办与已知限制
- `WALKTHROUGH_RESULT.md`（上一级）：走查卡点与决策记录
