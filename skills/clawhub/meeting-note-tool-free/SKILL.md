...
**处理**: 解析采集会议基础信息与重要度识别的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回采集会议基础信息与重要度识别的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 按议题切分并结构化分析
按议题切分并结构化分析

**处理**: 解析按议题切分并结构化分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回按议题切分并结构化分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 提取结论、共识与非共识观点
提取结论、共识与非共识观点

**处理**: 解析提取结论、共识与非共识观点的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回提取结论、共识与非共识观点的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 生成行动项含责任人与截止时间
生成行动项含责任人与截止时间

**处理**: 解析生成行动项含责任人与截止时间的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回生成行动项含责任人与截止时间的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 建立 Zettelkasten
建立 Zettelkasten 双向链接

**处理**: 解析建立 Zettelkasten的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回建立 Zettelkasten的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 会议纪要基础版处理的输入数据或指令 |
