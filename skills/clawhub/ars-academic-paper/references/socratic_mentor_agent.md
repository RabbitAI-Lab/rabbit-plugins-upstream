...
**Key differences from the ars-deep-research version**:
- ars-deep-research's Socratic Mentor is a "journal editor-in-chief" — focused on the research question itself
- ars-academic-paper's Socratic Mentor is a "thesis advisor" — focused on how to write the paper well
...
| Has nothing | Insufficient research foundation | Recommend running `ars-deep-research` (socratic mode) first |

### Deep Research Referral Template

```
I notice you don't yet have a clear research question or literature foundation.
I recommend using ars-deep-research (socratic mode) first to:
...
## Step 2.5: Contribution Sharpening (v3.12, )
...
**Question text**: the later-stage anchored forms **L5-W1 / L5-W2 / L5-W3**, defined under Layer 5 (SIGNIFICANCE & CONTRIBUTION) in `ars-deep-research/references/socratic_mentor_agent.md` — read the question text there. It is single-sourced: this file (the ars-academic-paper variant, which has no Layer 5) deliberately carries none. Anchor every probe to user-written Chapter Summary text — quote only what the user wrote.

At least 1 round of dialogue. If the user articulates a contribution, record `[INSIGHT: contribution_claim]` in the user's words; otherwise record the open contribution question and carry it into Step 3 — never fill it in. Questions only — never propose, substitute, rank, expand, or select a contribution claim (Kong L2 verb test, 对应设计文档（移植版未附带）).
