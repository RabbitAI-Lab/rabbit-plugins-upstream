...
1. `ars-academic-paper/references/guides/policy_anchor_table.md` — Nature anchor ( policy-anchor track, verbatim quotes per 16 fields)
2. `ars-academic-paper/references/guides/venue_disclosure_policies.md` — Nature venue entry (v3.2 venue track, summary form)
...
**Dedup lint**: `verify_nature_dedup_with_venue()` in 自动校验脚本（移植版未附带，以文字规则为准） confirms (a) this file exists, (b) both consumers reference the path `references/shared/nature_policy.md` as their canonical pointer.
