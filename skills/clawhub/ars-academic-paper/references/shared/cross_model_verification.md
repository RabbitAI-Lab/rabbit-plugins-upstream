...
**This is entirely optional.** All ARS skills work with the primary session model alone. Cross-model verification is an additional layer for users who want higher confidence in integrity checks, devil's advocate challenges, and review judgments.
...
**Citation-only ChatGPT-subscription transport:** A fourth, deliberately
...
the  constructive-value plan is a synthetic evaluation with its own frozen
...
|-------|--------|----------|----------|
| Primary (session model) | _(inherited agent session model — e.g., the current frontier generation)_ | Anthropic | Primary model (default for all ARS skills) |
...
**Recommended cross-verification pair:** the inherited agent session model (primary) + a current-generation second-family verifier — Gemini 3.1 Pro (validated) or GPT-5.6 Sol (provisional; see the note below).
...
You need API keys from at least one additional provider. ARS itself runs inside the agent runtime, so the session's primary model is always available.
...
# run 自动校验脚本（移植版未附带，以文字规则为准） against your key before relying on it):
...
python3 自动校验脚本（移植版未附带，以文字规则为准） detect
# The producer sends one closed codex_citation_request/1.0 object on stdin:
printf '%s' "$CITATION_REQUEST_JSON" | 自动校验脚本（移植版未附带，以文字规则为准）
```
...
emits only the closed receipt schema under `references/shared/contracts/cross_model/`; it
...
自动校验脚本（移植版未附带，以文字规则为准） is a manual live smoke using public
...
In the agent runtime, you can test by asking:
```
...
### Integrity Verification (ars-pipeline-orchestrator, Stage 2.5 / 4.5)

**When `ARS_CROSS_MODEL` is set:**
- Primary model runs full Phase A-E verification as normal
- After Phase A completes, a **risk-stratified** selection of references is sent for a blind cross-model verification pass (see step 2 below; replaces the pre- uniform random 30%)
...
5. Compare results: if the primary model said VERIFIED but cross-model said NOT_FOUND or MISMATCH, flag as `[CROSS-MODEL-DISAGREEMENT]`. Treat `NOT_SEARCHED` / ungrounded exactly as **not verified** — it never counts as agreement with a primary-model `VERIFIED`, and a sample that returns `NOT_SEARCHED` is surfaced for re-run or human review, never silently passed.
6. Include disagreements in the integrity report under a new section:
   ...
   | # | Reference | Tier | Primary | Cross-Model | Source (URL/DOI) | Status |
   |---|-----------|------|--------|-------------|------------------|--------|
   ```
...
### Devil's Advocate (ars-deep-research + ars-academic-reviewer)
...
### Cross-Model Reviewer Track ( — ars-academic-reviewer full mode)
...
**Typed provenance is not a binary independence score.** The closed
contract and field semantics are defined in
`ars-academic-reviewer/references/guides/review_panel_provenance_protocol.md`.
...
#### Calibration transport exception ( — non-sprint, attempt-atomic)
...
8. **Flag unset.** With `ARS_CROSS_MODEL` unset, owners emit no envelope and behavior is identical to the cross-model feature disabled; a stray envelope encountered with the flag unset is logged `[CROSS-MODEL-SKIPPED]` and not transported.
...
> **Why there is no generic "6th reviewer."** An earlier version of this document planned a cross-model 6th reviewer for peer review. That design is retired, not deferred (, 2026-07): the conditions under which an extra generic reviewer becomes counterproductive — score averaging, role duplication, findings treated as confirmed defects, majority-vote false confidence, synthesizer context burn — match ARS's documented anti-patterns one-for-one. The blind disagreement checkpoints above are the replacement: cross-model judgment concentrated at the two decisions that are hardest to reverse, compared blind, with divergence escalated to the human instead of blended into a consensus.
...
`citation_context`) and invokes 自动校验脚本（移植版未附带，以文字规则为准） on stdin.
...
对应设计文档（移植版未附带） and
machine-checked by the  test suite. The Bash entrypoints use syntax compatible
with macOS Bash 3.2.
...
# the cross-model verification module so they are behavior-tested in CI (a from-memory verdict, a
...
http="${resp##*$'\n'}"; body="${resp%$'\n'*}"
# Grounding guard + source extraction are canonical jq filters under the cross-model verification module
...
if [ "$http" -lt 200 ] || [ "$http" -ge 300 ]; then
  ...
  if [ -z "$text" ]; then
    ...
    # Canonical normalization lives in 自动校验脚本（移植版未附带，以文字规则为准）
    # (behavior-tested in 自动校验脚本（移植版未附带，以文字规则为准）) and is INVOKED here rather than
...
if [ -n "$ARS_CROSS_MODEL" ]; then
  ...
  # ID STATUS is a separate axis from routing: routing answers "which provider
  ...
  announce_id_status() {
    ...
    case "$status" in
      provisional) echo "NOTE: $ARS_CROSS_MODEL is provisional — endpoint support confirmed, ARS-specific behavior unvalidated (see Supported Models). Run 自动校验脚本（移植版未附带，以文字规则为准） before relying on it." ;;
      unlisted)    echo "WARNING: $ARS_CROSS_MODEL matches a first-party prefix and routes grounded, but is NOT a known-good id — the API may reject it. Check the id, or run 自动校验脚本（移植版未附带，以文字规则为准） before trusting results." ;;
    esac
  }
...
The run that flips a provisional id (today: `gpt-5.6-sol`) to validated is defined here so a future promotion argues against numbers, not vibes. Validation and recommendation are separate axes. (2026-08-19, : the recommendation moved to the current generation on lifecycle grounds — GPT-5.5 was superseded — ahead of validation; that flip carries no measurement claim. This bakeoff remains the only route to `validated`, and any claim of measured parity or superiority still requires the run below.)

> **Recorded run (2026-08-19/20,  — codex-transport variant).** The procedure below was executed over the  ChatGPT-subscription citation transport (entry gate: 自动校验脚本（移植版未附带，以文字规则为准） PASS for baseline and candidate; measure analogues: grounding evidence = receipt `searched`, measure 4 = zero fail-closed receipt-guard misfires). All five measures passed in the counterbalanced gate fleet, with superiority on measures 2 (fabrication recall) and 5 (latency) and a tie on measure 1 — see `audits/bakeoff-gpt-5-6-sol-codex-2026-08-19.md` (probe set `evals/bakeoff/2026-08-19-gpt-5-6-sol-codex/`, sha256 in the report). The result is **transport-qualified**: `gpt-5.6-sol` is validated for the subscription citation transport; it remains provisional on the first-party API route, whose jq grounding guards that run did not exercise. A scored fleet is bound to its preregistered frozen instrument; later instrument hardening that validates only surfaces outside every consumed path applies from the next fleet and does not retroactively invalidate a recorded gate result (boundary rationale in the run report's Instrument-freeze decision record). An API-route run requires a FRESH probe set under the sealed-preregistration protocol tracked in  — the 2026-08-19 set's labels are public, so reusing it would expose a live-search run to answer-key retrieval.

- **Entry gate:** 自动校验脚本（移植版未附带，以文字规则为准） passes against the candidate id.
...
Web-search results vary day to day; the 3-repeat majority verdict and same-day paired runs are what make the comparison fair. Thresholds are the  spec's choice and are tunable in a future spec without redesigning the procedure.
