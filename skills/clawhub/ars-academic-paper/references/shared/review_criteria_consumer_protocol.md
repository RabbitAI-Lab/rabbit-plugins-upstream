# Review criteria consumer protocol

This protocol binds the  `ReviewTargetContext` across formative planning,
the ars-academic-paper internal evaluator, and the ars-academic-reviewer external
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） init \
  --context phase0/review_target_context.json \
  --context-ref phase0/review_target_context.json \
  --registry references/shared/review_criteria_registry.json \
  --registry-ref references/shared/review_criteria_registry.json \
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） marker \
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） record \
  --manifest phase0/review_criteria_binding.json \
...
Owner: `ars-academic-paper` Phase 2 `structure_architect_agent`.
...
Owner: `ars-academic-paper` Phase 6 `peer_reviewer_agent`.
...
Owner: the ars-academic-reviewer orchestrating layer.
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） validate-findings \
  --manifest phase0/review_criteria_binding.json \
  --context phase0/review_target_context.json \
  --registry references/shared/review_criteria_registry.json \
...
review, or run the pre- field-general workflow with an explicit
