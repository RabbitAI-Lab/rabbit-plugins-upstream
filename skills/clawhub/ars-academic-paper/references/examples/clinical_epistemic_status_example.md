...
```yaml
# Illustrative only; not a  schema proposal.
...
The YAML block is illustrative. It is not a proposed schema for ; fields such as `upgrade_kind`, `clinical_safety_note`, `source_finding_status`, `draft_claim_problem`, and `illustrative_evidence_type` are included only to make the clinical overclaim pattern visible.
