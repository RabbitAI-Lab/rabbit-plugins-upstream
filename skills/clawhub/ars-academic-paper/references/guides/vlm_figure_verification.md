...
figure_table_trace:
  - artifact_id: "fig-3"               # figure number or table id, stable within the package
    ...
    transformation: {script: "自动校验脚本（移植版未附带，以文字规则为准）", hash: "a1b2c3d"}
...
3. **`caption_claim`** — the interpretive claim the caption makes. May be **compound** ("accuracy improves AND variance decreases"); the integrity gate decomposes it into atomic sub-claims before judging (borrowing the  decomposition *as prose guidance only* — no `PARTIAL` verdict, no `sub_claim_breakdown[]` schema is imported).
...
- Kong, L. et al. (2026). AI for Auto-Research: Roadmap & User Guide. *arXiv:2605.18661*. — §3.4 (figure/table fidelity failures; the motivation for the trace layer, ).
