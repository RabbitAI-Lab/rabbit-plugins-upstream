...
**Not Applicable When**:
- User has no idea about research direction (→ use `ars-deep-research` first)
...
- Just received materials from ars-deep-research and needs to transform them into a paper plan
...
- Chapter Plan → ars-academic-reviewer (review the plan)
...
- Need an in-depth research investigation (→ ars-deep-research)
...
`committee_correspondence_protocol.md` and emits the separate  byte-accounted
...
## Paths from ars-deep-research

```
ars-deep-research completed
  │
  ├── ars-deep-research (full mode) outputs:
  │   RQ Brief + Methodology Blueprint + Annotated Bibliography + Synthesis Report
  │   │
  │   ├── Want to write the paper directly ──→ ars-academic-paper (full mode)
  │   │   intake_agent auto-detects materials, skips redundant questions
  │   │
  │   └── Want to plan before writing ──→ ars-academic-paper (plan mode)
  │       socratic_mentor leverages existing materials to accelerate guidance
  │
  └── ars-deep-research (socratic mode) outputs:
      INSIGHT Collection + Synthesis Report
      │
      ├── INSIGHTs are sufficiently clear ──→ ars-academic-paper (full mode)
      │
      └── Need more guidance ──→ ars-academic-paper (plan mode)
          socratic_mentor continues deepening from INSIGHTs
```

## Connecting to ars-academic-reviewer

```
ars-academic-paper completed
  │
  ├── full mode produces complete paper ──→ ars-academic-reviewer (full / guided)
  │   Complete peer review + revision suggestions
  │
  ├── plan mode produces Chapter Plan ──→ ars-academic-reviewer (guided)
  │   Review the plan's feasibility and completeness
  │
  └── reviewer feedback ──→ ars-academic-paper (revision mode)
...
| "Help me search for literature" / 「幫我找文獻」 | lit-review | First confirm: Is it a literature review for a paper or a research investigation? | May need ars-deep-research |
| "I have ars-deep-research results, help me write a paper" / 「我有研究結果，幫我寫成論文」 | full (skip Phase 0 directly) | full (but intake needs to detect handoff) | Materials need to be properly imported |
...
| ars-deep-research results | Complete paper | full mode (auto-handoff) |
| ars-deep-research results | Guided planning | plan mode |
