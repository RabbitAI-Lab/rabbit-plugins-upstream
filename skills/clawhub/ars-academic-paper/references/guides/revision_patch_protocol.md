...
**Spec:** 对应设计文档（移植版未附带） (mechanism §3, coverage claim §4, escalation §3.6).
**Toolchain:** Slice A — 自动校验脚本（移植版未附带，以文字规则为准）, 自动校验脚本（移植版未附带，以文字规则为准）, 自动校验脚本（移植版未附带，以文字规则为准）, schemas under `references/shared/contracts/patch/`.
...
| Author adjudication | 自动校验脚本（移植版未附带，以文字规则为准） build-adjudication` from explicit choices | `author-adjudication/1.0`; complete choices and exact authority only |
...
| Integrity authorization | 自动校验脚本（移植版未附带，以文字规则为准） build-integrity-authorization` | `integrity-correction-authorization/1.0`; a deterministic sidecar that copies the author-approved patch hash and adds exact base/list/round bindings |
| Patch document | `draft_writer_agent` (revision invocation) | current `patch_format_version: 1.1`, schema `references/shared/contracts/patch/revision_patch.schema.json` |
...
The apply report shares the revised draft's lifecycle: it is a **required input to re-review and the Stage 4.5 integrity gate** — re-reviewers read it to see exactly which blocks changed (`ops_applied[]`, `fresh_block_ids`, `pure_move_pairs`) and which are machine-guaranteed untouched. Consumers verify report-to-artifact freshness by the  §11 ORDERED-CHAIN rule (which supersedes the old single-report `output_draft_hash`-vs-handed-draft check — that rule is wrong for multi-round sequences and misses base-link breaks): with reports ordered as applied, the FIRST report's `base_draft_hash` must equal the original (pre-revision) draft's hash prefix, each subsequent report's `base_draft_hash` its predecessor's `output_draft_hash`, and the LAST report's `output_draft_hash` — and only the last's — the handed revised draft's hash prefix. Any broken link means some patch ran against a different text than the chain claims (a rewritten-after-apply draft breaks the last link; a wrong base skews every diff-supported judgment) — treat the affected report(s) as stale and re-derive provenance before relying on them (the same report-to-artifact freshness class as the submission verifier's `STALE-REPORT` guard in 自动校验脚本（移植版未附带，以文字规则为准）; at Stage 3' the chain is checker-enforced as `apply_chain_witness`, `manifest_hash_mismatch` on breakage).
...
python 自动校验脚本（移植版未附带，以文字规则为准） draft.md
...
python 自动校验脚本（移植版未附带，以文字规则为准） validate-adjudication \
...
**On exit 3 (structural flags):** the patch touches structure — heading rewrites/deletes, net section-count change, or `touched_ratio` strictly above **0.6** (the  ship decision; `insert_after` merely *anchored* on a heading is exempt — inserting body text under a section heading is routine, not structural). Read the flags in the refusal output, then either narrow the patch, or — if the structural change is intended — re-run with the acknowledgment recorded:

```bash
python 自动校验脚本（移植版未附带，以文字规则为准） draft.md patch.json \
    --block-manifest draft.md.block-manifest.json \
    --roadmap roadmap.json \
    --author-adjudication author-adjudication.json \
    --claim-surface-manifest claim-surface-manifest.json \
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） build-integrity-authorization \
...
**Structural scope escalation:** current  rounds remain patch-based. If the
...
- The current CLI rejects patch 1.0. Archived 1.0 schema/runtime live only
  under `references/shared/contracts/patch/legacy/v1_0/` and the original repository's legacy archive (not shipped).
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） validate-bundle \
    revision-evidence-bundle.json --root revision-authority/
```

## Marker lifecycle (one rule for all marker kinds)

`<!--block:-->` markers live in **working drafts only**, exactly like `<!--ref:-->` / `<!--anchor:-->`. Two authoritative rules govern them — this doc indexes them rather than re-owning the wording, so the rule cannot drift out of sync with the surfaces the  lint guards:

- **Word counts exclude markers** — strip every `<!--...-->` before `len(body.split())`. Authoritative: `references/shared/word_count_conventions.md` § HTML-comment markers.
