# Committee Correspondence Protocol

This is the `committee-correspondence` variant of `ars-academic-paper`
...
Deterministic checker: 自动校验脚本（移植版未附带，以文字规则为准）。
Design authority:
对应设计文档（移植版未附带）。
...
When a user later selects a  profile, profile-derived artifact names and
