...
6. **Handoff awareness** — detect materials from ars-deep-research and auto-import
...
1. Check the conversation context for materials produced by ars-deep-research
...
Before auto-populating prose fields, strict-validate the  sidecar schema,
...
`ars-deep-research/references/templates/preregistration_template.md`. Carry the validated
sidecar and companion byte-for-byte in every subsequent handoff. If a current
ars-deep-research handoff lacks the explicit receipt, report `HANDOFF_INCOMPLETE`;
...
3. Notify the user:
   "I detected that you already have ars-deep-research materials. The following parameters have been auto-populated:
...
declaration to the deterministic  resolver. Do not infer missing target
...
| **Handoff Source** | [ars-deep-research / none] |
...
**Venue-profile follow-up (v3.12,  — optional, only when a target journal was named):** offer to record the venue's submission limits as a venue profile, consumed by the deterministic submission-package verifier:
...
- Store the answers as a YAML file validating against `references/shared/contracts/submission/venue_profile.schema.json` with `declared_by: scholar` (the only provenance value that exists), and record its path in the PCR `Venue Profile` row. The profile is re-feedable across runs like any declared input.
...
**Format-profile follow-up ( — optional, only when the output format is DOCX / PDF / LaTeX / Combined):** offer to record a layout profile the formatter follows when rendering. Skip entirely for a Markdown-only target (raw Markdown has no layout to declare).
...
- Store the answers as a YAML file validating against `references/shared/contracts/submission/format_profile.schema.json`, and record its path in the PCR `Format Profile` row. The profile is re-feedable across runs like any declared input. A synthetic example lives at `references/shared/contracts/submission/format_profile.example.yaml`.
...
1. Read each sample and extract style dimensions per `references/shared/style_calibration_protocol.md`
2. Produce a Style Profile artifact (see `references/shared/handoff_schemas.md` Schema 10)
...
**Mid-pipeline override.** If the scholar later changes the profile (a fresh `ars-academic-paper` invocation that re-runs intake, or an in-session correction), overwrite the PCR row. An override recorded **before Phase 1 runs** is consumed normally. An override recorded when **Phase 1 has already run OR was explicitly skipped** (the corpus is already fixed) cannot retroactively re-screen it, so you MUST emit a one-line `[PROFILE-OVERRIDE-NO-RESCREEN]` advisory: "the literature corpus is already fixed (already screened, or this run skips literature screening); to apply this profile, run Phase 1." The override is still honored for any future Phase-1 run.
...
### Step 13: Citation Verification Level (v3.12, )

The v3.11 deterministic citation-existence gate always *detects* unverifiable citations; whether a detection *blocks* output is the scholar's choice via `terminal_policies.citation_existence`. The default has always been advisory, but until this step nothing surfaced the choice at the moment it matters — ask:
...
- Answer `mark only`, or no answer → record `advisory (mark only, default)` in the PCR row and **write nothing to the passport** (per-key absence already means advisory; writing an explicit key would break byte-equivalence with pre- runs for no semantic gain).
