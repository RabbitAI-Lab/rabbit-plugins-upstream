...
| Feature | `ars-academic-paper` | `ars-deep-research` |
|---------|-------------------|-----------------|
...
| 12 | `revision_coach_agent` | Parse unstructured reviewer comments into a Revision Roadmap, or explicitly identified real-committee comments into the separate  source-accounted concern tracker; works standalone | Revision-Coach mode |
...
When Phase 0 has produced an author-confirmed `ReviewTargetContext`, the
...
3. **Phase 6a — evaluator paper-blind pre-commitment.**
   ...
   - User content: `evaluator_full` contract JSON + paper metadata + the writer's most recent `<phase4a_output>` (the writer artefact the evaluator must verify per `disagreement_handling.pre_commitment_check_protocol.check_writer_artifact`) +, when active, the pointer-only  manifest/Target Criteria Brief/`INTERNAL` marker.
...
- **Writer Phase 4 unusable** → `[GENERATOR-PHASE-ABORTED: role=writer, contract=<id>, reason=<lint_failure_kind>]` → abort `ars-academic-paper` Phase 4 → user intervention decides retry / fallback / regression to Phase 3 (Argument Blueprint).
- **Evaluator Phase 6 unusable** → `[GENERATOR-PHASE-ABORTED: role=evaluator, contract=<id>, reason=<lint_failure_kind>]` → abort `ars-academic-paper` Phase 6 → user intervention decides retry / fallback / regression to Phase 5 (Drafting completion).
...
| `revision` | "Revise paper" | 8->5->6 | Patch document + deterministically applied revised draft + apply report (; revision log via `references/templates/revision_tracking_template.md`) |
...
`references/guides/committee_correspondence_protocol.md`; do not infer official authority
...
`intake_agent` automatically detects ars-deep-research materials (RQ Brief /
...
deterministic builder. See `ars-deep-research/SKILL.md` Handoff Protocol and
...
|---------|---------|
| Insufficient research foundation | Recommend running `ars-deep-research` first |
...
- Process: `failure_paths` (12 scenarios), `mode_selection_guide` (11 modes), `plan_mode_protocol`, `workflow_phase_details`, `revision_patch_protocol` ( Mode B commands + marker lifecycle)
...
- Policy anchors: `policy_anchor_table`, `policy_anchor_disclosure_protocol`
- Meta: `changelog` (version history)
- Also: `ars-deep-research/references/guides/apa7_style_guide.md` (base reference, extended here)
...
|---|-------------|-------------|-----------------|
| 1 | **AI-typical overused terms** | "delve into", "crucial", "it is important to note" = instant AI detection | Use discipline-specific vocabulary; see `references/guides/writing_quality_check.md` |
...
| 5 | **⚠️ IRON RULE: Fabricated citations** | Inventing plausible-sounding references that don't exist | Every citation must be verified via DOI or WebSearch; see `ars-pipeline-orchestrator/references/integrity_verification_agent.md` |
...
| 8 | **Ignoring failure paths** | Continuing despite desk-reject signals or fatal methodology flaws | Check `references/guides/failure_paths.md`; invoke F11 Desk-Reject Recovery when triggered |
...
### Writing Quality
1. **Every claim must have a citation** or be supported by the paper's own data — or, for  absence/novelty claims, carry documented-search provenance plus the named nearest prior work where one exists (the explicit absence-of-adjacent-work statement suffices otherwise; no source can cite an absence)
...
```
ars-academic-paper + tw-hei-intelligence  -> Evidence-based HEI paper with real MOE data
ars-academic-paper + ars-deep-research        -> Deep research phase -> paper writing phase (auto-handoff)
ars-academic-paper + report-to-website    -> Interactive web version of the paper
ars-academic-paper + notebooklm-slides-generator -> Presentation slides from paper
ars-academic-paper + ars-academic-reviewer -> Peer review -> revision loop
```
...
| Dependent Skills | ars-deep-research v1.0+ (upstream), ars-academic-reviewer v1.0+ (downstream) |
