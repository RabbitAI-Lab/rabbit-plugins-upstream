...
def load_prana_credentials(prana_base_url: Optional[str] = None) -> Tuple[str, str]:
    ...
    base = (prana_base_url or DEFAULT_PRANA_BASE or "").strip()
    ...
    print(
        ...
        "  2) 保证 --base-url 可访问，并确保未设置 PRANA_SKILL_NO_AUTO_API_KEY；"
...
def main() -> None:
    ...
    base_url = DEFAULT_PRANA_BASE.strip()
    public_key, secret_key = load_prana_credentials(base_url)
