...
工具名称：WallySmarter-商品详情
功能说明：按Walmart商品ItemId查询商品详情，包含销量历史和价格历史趋势数据

**Prompt 模板：**

> 查询沃尔玛商品 {{5177343351}} 的详细信息，包含历史价格和销量数据

`{{}}` 中的内容为需要替换的参数。

**参数约束：**

- **商品ID（ItemId，必填）**: 商品详情链接中的数字ID，例如 `https://www.walmart.com/ip/5169493923` 中的 `5169493923`
- **是否包含历史价/历史销量（默认true）**: `true`=包含历史价/历史销量，`false`=不包含

限制：本工具返回的数据是非结构化的，不支持 @智能数据查询 进行二次分析。
