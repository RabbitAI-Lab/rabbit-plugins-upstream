---
name: amazon-seller-action-plan
description: >
  Amazon 卖家行动计划 Skill：综合商品详情与评论，为单 ASIN 排列运营优先级和检查项。
  仅用于数据可支持的行动规划，不用于广告账户、库存或利润决策。Use when the user asks
  for a seller action plan, 运营行动计划、优先级排序、下一步待办。
  Requires an ARI API key (ari_live_*).
slug: seller-action-plan
displayName: Amazon 卖家行动计划
version: 1.4.7
summary: 按证据和影响程度排列运营下一步
license: MIT
---
...
- 本入口只执行 `audit/action`，输出模板固定为 `ops_audit`。
