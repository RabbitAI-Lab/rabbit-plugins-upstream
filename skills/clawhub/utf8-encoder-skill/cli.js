...
工具命令:
  ...
  test-reddit <clientId> <clientSecret> <username> <password> <subreddit> <title> <内容>  发布测试帖子到Reddit
...
示例:
  ...
  utf8-encoder test-reddit <clientId> <clientSecret> <username> <password> r/test "测试标题" "测试内容"
...
环境变量:
  ...
  REDDIT_CLIENT_ID        Reddit应用Client ID
  REDDIT_CLIENT_SECRET    Reddit应用Client Secret
  REDDIT_USERNAME         Reddit用户名
  REDDIT_PASSWORD         Reddit密码
  REDDIT_SUBREDDIT        Reddit子版块（默认"r/test"）
  REDDIT_TITLE           Reddit帖子标题（默认"UTF-8编码测试帖子"）
`;
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'test-reddit': {
        let clientId, clientSecret, username, password, subreddit, title, content;
        
        if (args.length >= 8) {
          // 全参数模式: clientId clientSecret username password subreddit title content
          clientId = args[1];
          clientSecret = args[2];
          username = args[3];
          password = args[4];
          ...
          subreddit = process.env.REDDIT_SUBREDDIT || 'r/test';
          title = process.env.REDDIT_TITLE || 'UTF-8编码测试帖子';
          content = `# UTF-8编码测试 Reddit帖子\n\n生成时间: ${new Date().toISOString()}\n\n中文测试内容：这是一个测试Reddit帖子，用于验证UTF-8编码是否正确。\n\nEmoji测试: 🎯 ✅ 🔤\n\n特殊字符: !@#$%^&*()`;
        } else {
          console.error('❌ 参数不足，需要提供Reddit API凭据');
          console.log('用法: utf8-encoder test-reddit <clientId> <clientSecret> <username> <password> <subreddit> <title> <内容>');
          console.log('或设置环境变量:');
          console.log('  REDDIT_CLIENT_ID, REDDIT_CLIENT_SECRET');
          console.log('  REDDIT_USERNAME, REDDIT_PASSWORD');
          console.log('  REDDIT_SUBREDDIT (可选，默认"r/test")');
          console.log('  REDDIT_TITLE (可选，默认"UTF-8编码测试帖子")');
          process.exit(1);
        }
        
        if (!clientId || !clientSecret || !username || !password) {
          console.error('❌ 缺少Reddit API凭据');
          console.log('请提供全部4个凭据: clientId, clientSecret, username, password');
          console.log('或设置环境变量');
          process.exit(1);
        }
        
        console.log(`📝 发布测试帖子到Reddit...`);
        console.log(`子版块: ${subreddit}`);
        ...
        try {
          const result = await encoder.sendToReddit(
            clientId,
            clientSecret,
            username,
            password,
          ...
          console.log(`状态码: ${result.statusCode}`);
          console.log(`平台: ${result.platform}`);
          console.log(`标题长度: ${result.titleLength} 字符`);
          console.log(`内容长度: ${result.contentLength} 字符`);
          
          if (result.postUrl) {
            console.log(`帖子URL: ${result.postUrl}`);
          }
          
          if (!result.success && result.response && result.response.errors) {
            console.log(`错误信息: ${JSON.stringify(result.response.errors)}`);
          }
        } catch (error) {
          console.error(`❌ Reddit发布失败: ${error.message}`);
          console.log('提示: 请检查Reddit API凭据是否正确，以及用户是否有发布权限');
        }
