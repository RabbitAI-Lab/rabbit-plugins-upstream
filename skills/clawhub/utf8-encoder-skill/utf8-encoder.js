...
class UTF8Encoder {
  ...
  /**
   * 平台适配器：Reddit 帖子
   * @param {string} clientId - Reddit应用Client ID
   * @param {string} clientSecret - Reddit应用Client Secret
   * @param {string} username - Reddit用户名
   * @param {string} password - Reddit密码
   * @param {string} subreddit - 发布的子版块（如"r/test"）
   * @param {string} title - 帖子标题
   * @param {string} content - 帖子内容
   * @param {string} kind - 帖子类型（"self"文本帖，"link"链接帖）
   * @returns {Promise<object>} 发布结果
   */
  async sendToReddit(clientId, clientSecret, username, password, subreddit, title, content, kind = 'self') {
    console.log(`📝 发布到Reddit: ${subreddit}`);
    
    // 第一步：获取OAuth2访问令牌
    const authPayload = `grant_type=password&username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}`;
    const authHeaders = {
      'Authorization': 'Basic ' + Buffer.from(`${clientId}:${clientSecret}`).toString('base64'),
      'Content-Type': 'application/x-www-form-urlencoded',
      'User-Agent': 'UTF8-Encoder/1.0.0 (by /u/' + username + ')'
    };
        ...
        res.on('end', () => {
          try {
            ...
            if (json.access_token) {
              console.log('✅ Reddit OAuth2认证成功');
              resolve(json.access_token);
            } else {
              reject(new Error(`Reddit认证失败: ${json.error || '未知错误'}`));
            }
          } catch (error) {
            reject(new Error(`Reddit响应解析失败: ${error.message}`));
          }
        });
      });
    ...
    // 第二步：提交帖子
    const postPayload = `api_type=json&sr=${subreddit}&title=${encodeURIComponent(this.ensureUTF8(title))}&text=${encodeURIComponent(this.ensureUTF8(content))}&kind=${kind}`;
    const postHeaders = {
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = https.request(postOptions, (res) => {
        ...
        res.on('end', () => {
          try {
            const json = JSON.parse(data);
            const success = !json.json.errors || json.json.errors.length === 0;
            ...
            console.log(`  状态码: ${res.statusCode}, 标题: "${title.substring(0, 30)}${title.length > 30 ? '...' : ''}"`);
            
            if (success) {
              console.log('✅ Reddit帖子发布成功');
              if (json.json.data && json.json.data.url) {
                result.postUrl = json.json.data.url;
                console.log(`  帖子URL: ${json.json.data.url}`);
              }
            } else {
              console.error(`❌ Reddit帖子发布失败: ${JSON.stringify(json.json.errors)}`);
            }
      ...
      req.on('error', (error) => {
        console.error(`❌ Reddit请求失败: ${error.message}`);
        reject(error);
      });
...
class UTF8Infrastructure {
  ...
  integrateAsMiddleware() {
    ...
    return {
       ...
       * @param {string} username - Reddit用户名
       * @param {string} password - Reddit密码
       * @param {string} subreddit - 子版块
       * @param {string} title - 帖子标题
       * @param {string} content - 帖子内容
       * @param {string} kind - 帖子类型
       * @returns {Promise<object>} 发布结果
       */
      async sendToRedditWithRetry(clientId, clientSecret, username, password, subreddit, title, content, kind = 'self') {
        const check = self.shouldProcess(content);
        ...
        for (let attempt = 0; attempt <= self.maxRetries; attempt++) {
          try {
            console.debug(`尝试 #${attempt + 1} 发布到Reddit`);
            const result = await self.encoder.sendToReddit(clientId, clientSecret, username, password, subreddit, title, content, kind);
            ...
            if (result.success) {
              ...
              lastError = new Error(`Reddit发布失败，状态码: ${result.statusCode}`);
            }
          } catch (error) {
            lastError = error;
          }
        ...
        // 备选方案：保存到本地文件
        const fs = require('fs');
        const backupPath = `./backup-reddit-${Date.now()}.md`;
        fs.writeFileSync(backupPath, content, 'utf8');
        console.debug(`✅ 内容已备份到本地文件: ${backupPath}`);
        
        throw lastError || new Error('Reddit帖子发布失败');
      },
        ...
        for (const platform of platforms) {
          try {
            ...
            switch (platform.type) {
              ...
              case 'reddit':
                result = await this.sendToRedditWithRetry(
                  platform.clientId,
                  platform.clientSecret,
                  platform.username,
                  platform.password,
                  platform.subreddit || 'r/test',
                  platform.title || 'UTF-8测试帖子',
                  content,
                  platform.kind || 'self'
                );
                break;
