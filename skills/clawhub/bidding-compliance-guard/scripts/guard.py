#!/usr/bin/env python3
"""招投标合规护栏检测内核 (bidding-compliance-guard)。

实时护栏：在投标文书、招标公告、资格预审文件、中标公示、商务应答、企业宣传
发布 / 提交前，检测其中与招投标合规相关的危险表述
（串通投标/围标 / 虚假资质业绩 / 诋毁竞争对手 / 报价操纵 /
商业贿赂 / 排斥性条款），按风险分级输出命中与整改建议，供 Agent 主动调用。
...
用法：
    # 检测一段投标文案
    python3 scripts/guard.py --text "我方承诺保证中标，并可安排陪标单位"
    # 从标准输入读取（适合管道 / Agent 调用）
    echo "招标文件限定特定品牌，拒绝其他厂商" | python3 scripts/guard.py --stdin
