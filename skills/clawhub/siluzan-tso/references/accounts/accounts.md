...
- **禁止臆测授权过期**：403 / 拉数失败时**禁止**口头说「可能授权过期」。须 ID 核验后，对 Google 执行 `account check-access -a <mediaCustomerId>`，以返回 `status` 为准（见 [`accounts-permissions.md`](accounts-permissions.md)）；仅当 `list-accounts` 输出含激活字段且可判定未激活时，勿用其判断授权过期；非 Google 看 `invalidOAuthToken`。
