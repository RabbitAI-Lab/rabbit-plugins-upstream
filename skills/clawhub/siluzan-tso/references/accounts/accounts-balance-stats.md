...
**读盘要点**：先看 `meta.dataIssue`（非空则失败）；再按 `remainingDays` 升序交付；`hitReason="none"` 表示未触阈值；用 **`dailySpend`** 做日均（口径见 `meta.spendDays` / `meta.spendWindow`），勿把 `stats` 区间合计 `spend` 当「每天消耗」。与 `balance` / `accounts-digest` 分工见 `references/core/agent-conventions.md` §八。
