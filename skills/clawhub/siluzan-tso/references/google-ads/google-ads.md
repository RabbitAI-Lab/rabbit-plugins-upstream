...
- **禁止臆测授权过期**：`ad *` / geo / 拉数出现 403/500 时，**禁止**对用户说「授权可能过期」。须核验 ID 后，在套餐已激活前提下执行 `account check-access -a <mediaCustomerId>`，仅当 `reauth_required`（或与 `invalidOAuthToken=true` 交叉确认）才走重授权。见 [`accounts-permissions.md`](../accounts/accounts-permissions.md)。
...
- **batch diff 交付**：把 `reportMarkdown`（或 `reportMarkdownFile`）**全文以 Markdown 发给用户**；**禁止**只说「未发现缺失」。
