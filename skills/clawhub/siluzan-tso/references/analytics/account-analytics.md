...
### Bing 近实时报表（`bing-analysis --aggregation`，日期可含今天）

> `bing-analysis` 默认面向周期报告，日期**禁止**含今天/昨天；一旦显式传 `--aggregation` 或
> `--return-only-complete-data`，即视为「单日花费」「近两小时 CPA」等近实时监控意图，
> **自动放开**日期含今天/昨天的限制。仅 `campaigns`/`ad-groups`/`ads` 三个 section 支持这两个参数
