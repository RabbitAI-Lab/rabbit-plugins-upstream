...
- **步骤（Search 一体化创建，无现成方案文件）**：
  0. **最小清单**（详见 `google-ads-campaign-plan.md` §创建前最小命令清单）：`list-accounts -m Google -k …` →（已激活）`account check-access` → geo → validate → 审查稿 → create → `batch get`（看 `agentWorkflow`）→ **仅有 campaignId 时** `batch diff`。命令是 `ad campaigns`（禁 `ad-campaigns`/`campaign list`）；geo/ad 失败禁臆测授权过期。
