...
- **Google 广告创建完成后必出系列详情报告**（用户确认计划并执行 `campaign-create` / `pmax-create` 之后）：
  ...
  - **交付物必须是 Markdown 文档正文**：执行 `ad batch diff --json-out … --md-out …` 后，将落盘 JSON 的 **`reportMarkdown` 全文**（或 Read `reportMarkdownFile` 的完整内容）作为 **Markdown 消息发给用户**（可附文件路径）。报告已含概览、数量汇总、按广告组展开的关键词/RSA 标题与描述状态表、仍未创建项。
  - 多系列：每个系列各跑一次 diff，**各发一份** Markdown（或同一回复里多份 `#` 一级标题分节）。
  - **禁止**只说「已创建成功 / 未发现缺失」、**禁止**只贴 `counts` 摘要而不发 `reportMarkdown`；代改过用户方案时另加修改表（见 `google-ads-plan-source-fidelity.md`）。
...
- **Bing** `bing-analysis`：「不能含今天/昨天」仅当区间碰到**执行日**的今天/昨天时才需把结束日压到前天；**已结束的历史自然月必须用该月最后一天**，细则见 `report-templates/bing-period-report.md` §日期规则。交付前核对 `overview` 的 `activeDays` = 该月日历天数。
