---
name: amazon-product-improvement
display_name: 亚马逊产品改进建议 · 评论驱动迭代
description: >
  亚马逊产品改进建议 Skill：把评论抱怨转成可执行的产品迭代清单，
  按影响人数与严重度排优先级，区分改模具、换材质、改包装、补说明书等不同代价的动作，
  供下一批次开发参考。Use when the user asks about product improvement,
  iteration priorities, quality issues from reviews, 产品改进、产品迭代、优化方向、
  质量改善、开发需求、改款建议。Requires an ARI API key (ari_live_*).
author: ARI (funewa)
agent_created: true
slug: product-improve
displayName: 亚马逊产品改进建议 · 评论驱动迭代
version: 1.4.7
summary: 抱怨转迭代清单，按影响人数 × 严重度排序
license: MIT
---
