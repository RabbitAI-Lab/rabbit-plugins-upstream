...
"""
跨境魔方海关公司贸易区域列表查询
根据公司ID和公司类型获取公司贸易的区域列表数据（支持游标分页）。
"""
import argparse
import sys
...
def get_company_area_list(params: dict) -> dict:
    """
    根据查询参数获取公司贸易区域列表。
    ...
    Returns:
        包含区域列表数据的API响应
    """
    response = make_request('/agent/customs/company/area/list', params)
    return response


def main():
    parser = argparse.ArgumentParser(
        description='从跨境魔方开放平台获取公司贸易区域列表'
    )
    parser.add_argument(
        '--params',
        required=True,
    ...
    response = get_company_area_list(params)
