...
class TestParseNote(unittest.TestCase):
    ...
    def test_agent_delu_flat_response(self):
        resp = {
            "data": {
                "note": {
                    "note_type": "image",
                    "display_title": "扁平结构标题",
                    "description": "扁平结构描述",
                    "image_urls": ["https://x.com/a.jpg"],
                    "tags": ["穿搭[话题]", {"tag_name": "新中式"}],
                    "topic_list": [{"topic_name": "春季穿搭"}],
                    "interact_info": {
                        "like_count": "88",
                        "collect_count": "66",
                        "comment_count": "7",
                    ...
                    "author": {"name": "作者A", "user_id": "u2", "redId": "r2"},
                }
            }
        }
        n = parse_note(resp, "flat_id")
        self.assertEqual(n.type, "normal")
        self.assertEqual(n.title, "扁平结构标题")
        self.assertEqual(n.desc, "扁平结构描述")
        self.assertEqual(n.image_urls, ["https://x.com/a.jpg"])
        self.assertEqual(n.hashtags, ["穿搭", "新中式"])
        self.assertEqual(n.topics, ["春季穿搭"])
        self.assertEqual(n.liked_count, 88)
        self.assertEqual(n.collected_count, 66)
        self.assertEqual(n.comments_count, 7)
        self.assertEqual(n.shared_count, 3)
        self.assertEqual(n.author_nickname, "作者A")
...
class TestParseComments(unittest.TestCase):
    ...
    def test_agent_delu_flat_comments(self):
        resp = {"data": {"comments": [
            {
                "comment_id": "c1",
                "text": "想问怎么卖",
                "likes": "12",
                "author": {"name": "用户A", "redId": "ra"},
                "reply_count": 2,
                "pinned": True,
            }
        ]}}
        ...
        self.assertEqual(comments[0].content, "想问怎么卖")
        self.assertEqual(comments[0].like_count, 12)
        self.assertEqual(comments[0].user_nickname, "用户A")
        self.assertEqual(comments[0].user_red_id, "ra")
        self.assertEqual(comments[0].sub_count, 2)
