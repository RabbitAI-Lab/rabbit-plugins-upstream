...
3. agent.delu.cn 爬虫接口配置
4. 网络：agent.delu.cn API + xhscdn.com CDN（图片/视频走 CDN）
5. 工作区可写
...
from .client import DeluCrawlerClient
...
def check_delu_crawler_credentials() -> Check:
    token = DeluCrawlerClient._load_token()
    if not token:
        from .client import _help_message
        searched = "\n  ".join(str(p) for p in DeluCrawlerClient.token_search_paths())
        return Check(
            name="agent.delu.cn 认证",
            ok=False,
            fatal=False,
            message="未配置 token（将以无认证方式调用；如接口要求认证会失败）",
            fix=_help_message(),
            detail=[f"已查找的路径：\n  {searched}"],
        )
...
def check_network_delu_crawler() -> Check:
    """测试 agent.delu.cn 爬虫接口域名是否可达。"""
    base_url = DeluCrawlerClient.configured_base_url()
    ok, msg = _http_ping(base_url + "/")
    return Check(
        name=f"网络 → {base_url}",
        ok=ok,
        fatal=False,
        ...
        fix=(
            "确认 agent.delu.cn 在当前网络可访问；\n"
            "如使用自建/内网代理，在 .env 设置 AGENT_DELU_BASE_URL=https://your-host"
        ) if not ok else "",
    )
...
def check_network_xhs_cdn() -> Check:
    ...
    XHS CDN 与 agent.delu.cn 是不同 host，可能代理覆盖不全。
    """
    ...
    return Check(
        ...
        fix=(
            "图片和视频从 xhscdn.com 下载，独立于 agent.delu.cn API。\n"
...
def run_all(workspace: Path | None = None) -> list[Check]:
    ...
    return [
        ...
        check_delu_crawler_credentials(),
        check_network_delu_crawler(),
