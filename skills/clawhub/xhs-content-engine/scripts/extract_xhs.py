...
from content_engine.client import DeluCrawlerClient, DeluCrawlerError
...
def main() -> int:
    ...
    ap.add_argument(
        "--check",
        action="store_true",
        help="仅运行环境检查（ffmpeg / agent.delu.cn / Python 版本 / 网络），不执行拆解",
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
