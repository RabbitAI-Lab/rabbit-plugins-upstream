---
name: 小红书内容引擎
description: "小红书内容引擎。两种 mode：①拆解（v1）— 输入 XHS 爆款链接，输出 18 维结构化拆解卡；②生成（v2）— 在拆解卡基础上结合品牌信息，用 Ofox（LLM + Nano Banana）生成我方版本的脚本/文案/素材图/封面/标签全套产出。视频生成（Seedance 2.0）规划在 v2.1。同时维护 graph/ 知识图谱（品牌声音、平台 playbook、钩子库、风格词典），mode 之间共享，越用越聪明。架构受 Ronin Skill Graph 启发。后续阶段陆续接入抖音 / 视频号等平台 + evaluate 评估 mode。"
---
...
|---|---|---|
| **小红书** | ✅ v1 | 视频笔记 + 图文笔记 |
| 抖音 | 🔜 v1.1 | 短视频拆解（计划接 agent.delu.cn Douyin 爬虫接口） |
| 视频号 | 🔜 v1.2 | 短视频拆解 |
| B 站 | 🔜 v2 | 短视频 + 长视频 |
...
这个 skill 会在用户显式触发拆解或生成时使用以下本地能力与外部服务：

- **本地文件读写**：读取 `graph/`、`references/`、`.env` 配置和工作区输入；写入 `docs/deconstructions/`、临时工作区、`note.json`、`comments.json`、抽帧图片、生成脚本、生成图片/视频和质量报告；按 Step 6 只追加回写 `graph/engine/` 与 `graph/platforms/`。
- **本地命令**：调用 `python3` 运行本目录脚本；在视频拆解或拼接时调用 `ffmpeg`。脚本只把经过校验的 note id、文件路径和本地工作区路径传给这些命令。
- **外部网络**：默认调用 `https://agent.delu.cn` 获取小红书笔记/评论数据；按资源 URL 下载 XHS CDN 图片/视频；generate mode 会按用户选择调用 Ofox/OpenRouter、Nano Banana、火山方舟 Ark/Seedance；飞书发布仅在会话里已有飞书插件且用户需要时由插件完成。
- **凭据读取**：只读取显式配置的环境变量或 `.env` 中的 `AGENT_DELU_*`、`OFOX_*`、`OPENROUTER_*`、`ARK_*` 等 provider key；不会把 key 写入输出文件或日志。
- **可控成本**：Seedance 真视频生成默认会打印成本估算并给 3 秒中断窗口；也可用 `--no-real-video` 只生成 prompt，不调用付费视频接口。

不要把 `AGENT_DELU_BASE_URL`、`OFOX_BASE_URL`、`OPENROUTER_BASE_URL` 或 `ARK_BASE_URL` 指向不可信主机。处理保密品牌资料时，先确认上述 provider 可以接收对应内容。
...
└── scripts/
    ...
    └── content_engine/               Python 包（zero deps）
        ├── client.py                  DeluCrawlerClient（agent.delu.cn 爬虫接口）
...
```
（检查 Python 版本 / ffmpeg / agent.delu.cn / 网络 / 工作区可写。详见下方"## 安装与依赖"）
...
- 检查 OFOX_API_KEY（必需）+ agent.delu.cn 爬虫接口（fallback 拆解用）
...
完整配置、crawler gateway 契约和替换爬虫方案见 `references/configuration.md`。
...
**v1 拆解默认使用 `https://agent.delu.cn` 爬虫接口**（如接口需要认证，配置 `AGENT_DELU_API_TOKEN`）
...
```bash
# v1 拆解：agent.delu.cn 爬虫接口
mkdir -p ~/.config/content-engine
echo 'AGENT_DELU_BASE_URL=https://agent.delu.cn' >> ~/.config/content-engine/.env
# 如接口需要认证，再加：
echo 'AGENT_DELU_API_TOKEN=你的_agent_delu_token' >> ~/.config/content-engine/.env
...
| Token | 注册 | 用途 | 必需吗？ |
|---|---|---|---|
| `AGENT_DELU_API_TOKEN` | `agent.delu.cn` | XHS 爬虫接口认证（拆解原始数据） | 视接口配置而定 |
...
Token 查找顺序（第一个找到即用）：
1. 对应环境变量（`AGENT_DELU_API_TOKEN` / `OFOX_API_KEY` / `ARK_API_KEY` / `OPENROUTER_API_KEY`）
2. 当前工作目录的 `.env`
3. `~/.config/content-engine/.env`（XDG 标准）
4. Skill 根目录的 `.env`

agent.delu.cn 爬虫接口可配置项：

| 环境变量 | 默认值 | 用途 |
|---|---|---|
| `AGENT_DELU_BASE_URL` | `https://agent.delu.cn` | 爬虫接口域名 |
| `AGENT_DELU_NOTE_PATH` | `/api/v1/xiaohongshu/app/get_note_info` | 笔记详情接口 |
...
| `AGENT_DELU_AUTH_HEADER` | `Authorization` | token header 名 |
| `AGENT_DELU_AUTH_SCHEME` | `Bearer` | token 前缀；设为 `raw` 可直接传 token |
...
- `extract_xhs.py` 默认请求 `https://agent.delu.cn`。
- 如果内部爬虫接口路径变化，只改上面的 `AGENT_DELU_*` 环境变量，不需要改脚本。
- 视频和图片仍从 XHS CDN 直链下载；如果 CDN 不通，元数据和评论仍可先写入工作区。
