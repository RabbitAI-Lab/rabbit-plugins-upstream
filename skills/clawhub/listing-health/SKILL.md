...
description: >
  Amazon Listing 健康检查 Skill：检查页面信息完整度、评论证据与常见误解。
  仅用于 Listing 健康诊断，不用于完整文案代写、广告或库存分析。Use when the user asks
  about listing health, content completeness, Listing健康检查、页面问题诊断。
  Requires an ARI API key (ari_live_*).
slug: listing-health
displayName: Amazon Listing 健康检查
version: 1.4.7
summary: 检查页面完整度、误解风险与评论证据
license: MIT
---
...
- 本入口只执行 `audit/listing`，输出模板固定为 `ops_audit`。
