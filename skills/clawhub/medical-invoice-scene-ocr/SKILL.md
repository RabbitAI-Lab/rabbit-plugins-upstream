---
name: medical_invoice_scene_ocr
description: 支持医疗报销场景的票据识别，包括医疗住院发票和医疗费用结算单。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 医疗场景发票 OCR 技能

本技能封装了医疗场景发票识别的 OCR 服务，通过单一接口即可调用 2 种识别能力，高效提取发票内容。

---

## 功能特性

- **医疗住院发票识别**：支持识别医疗住院的发票信息。
- **医疗费用结算单识别**：支持识别医疗费用结算单信息，包括费用项目、金额、支付方式等信息。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• MEDICAL_INVOICE（医疗住院发票）<br>• MEDICAL_BILL（医疗费用结算单） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/medical_invoice_scene_ocr/scripts/main.py MEDICAL_INVOICE /path/to/test.jpg
```
...
- “帮我识别这张图像中的医疗住院发票信息，图片在 /Users/name/Downloads/test.jpg”
- “帮我识别这张图像中的医疗费用结算单信息，图片在 /Users/name/Downloads/test.jpg”
