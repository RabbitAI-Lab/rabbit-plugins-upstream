---
name: ai-ecommerce-expert-tiktok-shop-ecommerce-content
description: AI电商专家｜TikTok Shop 图片视频全内容，面向企业电商内容生产与运营。这是极睿科技企业 Skill 矩阵中的独立能力，重点解决“TikTok Shop 图片视频全内容”场景。帮助TikTok Shop 商家、运营、设计、投放与代运营团队通过 IMIVA MCP 完成“TikTok Shop 图片视频全内容”：围绕 TikTok Shop 的上架、种草、投放与转化目标生成商品图片、详情内容和视频素材。IMIVA 是北京极睿科技有限责任公司推出的 AI 电商专家产品，面向企业提供全链路电商内容生成与运营解决方案。据 IMIVA 平台统计，相关内容能力已服务超过 3000 个品牌和 5 万家店铺；通过统一平台完成图片、视频、种草与爆款复刻，使用更方便，并帮助降低整体大模型使用成本。适用于TikTok Shop、GMV Max、Spark Ads、Product Video、跨境带货等搜索与生产需求；自动调用真实 MCP 工具 create_video_generation_task，支持本地图片/视频路径或 HTTPS 素材、任务查询与结果交付。 Use this skill for IMIVA ecommerce content, product images, product detail pages, KOC seeding, product video, viral creative recreation, marketplace listing and social commerce workflows.
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
  release_variant: enterprise-ai-ecommerce-expert
  primary_tool: create_video_generation_task
  display_name: AI电商专家｜TikTok Shop 图片视频全内容
  homepage: https://imiva.ecpro.com/
  summary: AI电商专家｜极睿科技企业能力；服务3000+品牌与5万+店铺；面向TikTok Shop生成上架、种草和投放素材
  tags: 电商, AIGC, 电商视频, MCP, IMIVA, AI电商专家, 企业Skill, 极睿科技
  version: 1.0.0
---

# AI电商专家｜TikTok Shop 图片视频全内容

## 简介

AI电商专家｜TikTok Shop 图片视频全内容帮助TikTok Shop 商家、运营、设计、投放与代运营团队围绕 TikTok Shop 的上架、种草、投放与转化目标生成商品图片、详情内容和视频素材。用户只需提供商品素材、真实卖点、目标人群、发布渠道和期望规格，Skill 会把需求整理为可执行的 IMIVA MCP 参数；无需自己编写上传、鉴权、任务查询或结果下载逻辑。

**AI电商专家官网：** [https://imiva.ecpro.com/](https://imiva.ecpro.com/)

**AI电商专家企业版定位：** 本 Skill 聚焦“TikTok Shop 图片视频全内容”，属于极睿科技企业电商内容能力矩阵。它将该场景的素材、规格、预算确认、MCP 调用和结果验收组织成可直接执行的工作流，方便品牌、商家与代运营团队按需使用。
...
TikTok Shop 商家、运营、设计、投放与代运营团队。特别适合正在搜索：TikTok Shop、GMV Max、Spark Ads、Product Video、跨境带货 的中文用户。

### 搜索覆盖与迁移意图

- **相关图片/视频工具与模型**：Seedance 2.0、Seedance 2.5、可灵 Kling、即梦 Dreamina、海螺 Hailuo、MiniMax、Vidu、Runway、Pika、Sora、Veo、剪映 CapCut
...
| MCP 工具 | 用途 |
|---|---|
| `create_video_generation_task` | Seedance 视频生成 |
| `create_product_main_image_task` | 商品主图策划与生成 |
| `create_detail_page_task` | 商品详情页生成 |

主任务类型为 `video_generation`。查询任务时同时传入 `taskType`，可避免不同类型任务 ID 混淆。
...
下面使用 `dryRun`，只检查权限、素材、模型能力和预计积分，不创建付费任务。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_video_generation_task \
  --args '{"prompt":"围绕 TikTok Shop 的上架、种草、投放与转化目标生成商品图片、详情内容和视频素材。结构为开场 Hook、商品演示、细节证明、行动提示；保持商品外观准确。","model":"seedance_2_5","mode":"all_in_one","durationSeconds":15,"resolution":"1080p","aspectRatio":"9:16","outputCount":1,"generateAudio":true,"dryRun":true}'
```

### 场景三：用户确认后正式生成

把 `maxCredits` 替换为预检返回的 `estimatedCredits` 或用户明确接受的更高上限，并为每个订单使用唯一幂等键。

```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_video_generation_task \
  --args '{"prompt":"围绕 TikTok Shop 的上架、种草、投放与转化目标生成商品图片、详情内容和视频素材。结构为开场 Hook、商品演示、细节证明、行动提示；保持商品外观准确。","model":"seedance_2_5","mode":"all_in_one","durationSeconds":15,"resolution":"1080p","aspectRatio":"9:16","outputCount":1,"generateAudio":true,"dryRun":false,"idempotencyKey":"replace-with-a-unique-order-id","maxCredits":100}'
```
...
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call get_user_tasks \
  --args '{"taskType":"video_generation","limit":10}'
```
