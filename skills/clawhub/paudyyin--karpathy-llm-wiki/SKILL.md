...
version: 1.0.0
description: "Use when building or maintaining a personal LLM-powered knowledge base. Triggers: ingesting sources into a wiki, querying wiki knowledge, linting wiki quality, 'add to wiki', 'what do I know about', or any mention of 'LLM wiki' or 'Karpathy wiki'. 中文触发：'加入知识库'、'知识库查询'、'检查知识库'、'我知道什么关于'、'LLM wiki'、'编译知识'。"
---

# Karpathy LLM Wiki

Build and maintain a personal knowledge base using LLMs. You manage two directories under `wiki-kb/`: `wiki-kb/raw/` (immutable source material) and `wiki-kb/wiki/` (compiled knowledge articles). Sources go into wiki-kb/raw/, you compile them into wiki-kb/wiki/ articles, and the wiki compounds over time.
...
- `wiki-kb/wiki/index.md` — Global index. One row per article, grouped by topic, with link + summary + Updated date.
- `wiki-kb/wiki/log.md` — Append-only operation log.
...
- `wiki-kb/raw/` directory (with `.gitkeep`)
- `wiki-kb/wiki/` directory (with `.gitkeep`)
- `wiki-kb/wiki/index.md` — heading `# Knowledge Base Index`, empty body
- `wiki-kb/wiki/log.md` — heading `# Wiki Log`, empty body
...
2. Pick a topic directory. Check existing `wiki-kb/raw/` subdirectories first; reuse one if the topic is close enough. Create a new subdirectory only for genuinely distinct topics.

3. Save as `wiki-kb/raw/<topic>/YYYY-MM-DD-descriptive-slug.md`.
...
- Raw field: markdown links to wiki-kb/raw/ files, semicolon-separated.
- Relative paths from `wiki-kb/wiki/<topic>/` use `../../raw/<topic>/<file>.md` (two levels up to wiki-kb/ root).
...
2. Scan `wiki-kb/wiki/index.md` entries in other topics for articles covering related concepts.
...
1. Read `wiki-kb/wiki/index.md` to locate relevant articles.
2. Read those articles and synthesize an answer.
3. Prefer wiki content over your own training knowledge. Cite sources with markdown links: `[Article Title](wiki-kb/wiki/topic/article.md)` (workspace-relative paths for in-conversation citations; within wiki-kb/wiki/ files, use paths relative to the current file).
...
1. Write the answer as a new wiki page. See `references/archive-template.md`. When converting conversation citations to the archive page, rewrite workspace-relative paths (e.g., `wiki-kb/wiki/topic/article.md`) to file-relative paths (e.g., `../topic/article.md` or `article.md` for same-directory).
   - Sources: markdown links to the wiki articles cited in the answer.
   - No Raw field (content does not come from wiki-kb/raw/).
...
3. Update `wiki-kb/wiki/index.md`. Prefix the Summary with `[Archived]`.
4. Append to `wiki-kb/wiki/log.md`:
   ```
...
**Internal links** — for every markdown link in wiki-kb/wiki/ article files (body text and Sources metadata), excluding Raw field links (validated by Raw references below) and excluding index.md/log.md (handled above):
- Target does not exist → search wiki-kb/wiki/ for a file with the same name elsewhere.
...
- Target does not exist → search wiki-kb/raw/ for a file with the same name elsewhere.
  - Exactly one match → fix the path.
  - Zero or multiple matches → report to the user.
...
- wiki-kb/wiki/ supports one level of topic subdirectories only. No deeper nesting.
...
- Inside wiki-kb/wiki/ files, all markdown links use paths relative to the current file. In conversation output, use workspace-relative paths (e.g., `wiki-kb/wiki/topic/article.md`).
- Ingest updates both `wiki-kb/wiki/index.md` and `wiki-kb/wiki/log.md`. Archive (from Query) updates both. Lint updates `wiki-kb/wiki/log.md` (and `wiki-kb/wiki/index.md` only when auto-fixing index entries). Plain queries do not write any files.
