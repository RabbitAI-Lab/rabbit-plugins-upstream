...
ALLOWED_PARAMS = BASE_PARAMS | {
    f"{field}{suffix}"
    for field in NUMERIC_RANGE_PARAMS
    for suffix in ("Min", "Max")
}
SORT_FIELDS = NUMERIC_RANGE_PARAMS | {"catLevel", "createTime", "updateTime"}
...
def parse_params(raw_params):
    ...
    if "catLevel" in values:
        parse_int("类目层级", values["catLevel"], minimum=1)
    if "parentCatId" in values:
        parse_int("父类目 ID", values["parentCatId"], minimum=0)
    validate_page(values)
    validate_sort(values, SORT_FIELDS)
    validate_range_pairs(
        values,
        NUMERIC_RANGE_PARAMS,
        integer_fields=INTEGER_RANGE_PARAMS,
        nonnegative_fields=NONNEGATIVE_RANGE_PARAMS,
    )
    return params


def main():
    parser = argparse.ArgumentParser(description="查询 SHEIN 类目并输出 JSON")
    ...
    try:
        ...
        payload = validate_search_response(payload, "类目查询失败")
