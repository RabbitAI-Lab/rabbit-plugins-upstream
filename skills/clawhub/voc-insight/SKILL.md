...
display_name: VOC洞察
description: >
  VOC 洞察 Skill：从亚马逊评论中提炼消费者之声——用户画像、购买动因、使用场景、
  情感倾向与未被满足的需求，生成可执行的 Listing 文案与产品改进建议。
  Use when the user asks about customer insights, user persona, purchase motivation,
  listing optimization, 用户画像、购买动机、场景分析、情感分析、Listing 优化、文案优化、
  需求挖掘。Requires an ARI API key (ari_live_*).
author: ARI (funewa)
agent_created: true
slug: voc-insight
displayName: VOC洞察
version: 1.4.7
summary: 评论里的消费者之声：画像、动因与 Listing 优化
license: MIT
---
