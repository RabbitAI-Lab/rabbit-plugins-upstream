...
- For a one-off artifact or result, use the registered `gpt-image` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
```shell
runapi gpt-image <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service gpt-image --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/gpt-image-sdk
