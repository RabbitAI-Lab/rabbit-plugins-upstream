#!/usr/bin/env python3
"""
Walmart 产品分析 - LinkFox Skill
调用 /sorftime/walmart/productAnalysis 接口。

Usage:
  python sorftime_walmart_product_analysis.py '<JSON parameters>'           # 始终落盘；小结果全量输出，大结果输出摘要
  python sorftime_walmart_product_analysis.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/sorftime/walmart/productAnalysis"
SLUG = "linkfox-sorftime-walmart-product-analysis"
...
def main():
    ...
    if not argv:
        print(
            "Usage: sorftime_walmart_product_analysis.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
