...
license: MIT
edition: free
description: "核心能力: 沟通协作领域的专业化 AI 辅助工具,包含核心基础功能兼容. 适用于需要comm skill tool相关能力的开发场景,包含结构化的工作流程和可复用的模板,帮助用户快速完成任务并保持代码质量. 适用于需要comm skill tool相关能力的开发场景,包含结构化的工作流程和配置指引."
tags:
  - 沟通
  - comm
  - skill
...
**处理**: 解析综合对话上下文生成回复草稿的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回综合对话上下文生成回复草稿的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检测情感潜台词与沟通动态
检测情感潜台词与沟通动态

**处理**: 解析检测情感潜台词与沟通动态的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回检测情感潜台词与沟通动态的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 提供多种沟通结构模板(反馈/请
提供多种沟通结构模板(反馈/请求/困难对话)

**处理**: 解析提供多种沟通结构模板(反馈/请的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回提供多种沟通结构模板(反馈/请的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 根据关系与情境调整语气
根据关系与情境调整语气

**处理**: 解析根据关系与情境调整语气的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回根据关系与情境调整语气的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 支持邮件、消息、面对面等多种渠
支持邮件、消息、面对面等多种渠道

**处理**: 解析支持邮件、消息、面对面等多种渠的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回支持邮件、消息、面对面等多种渠的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：帮助撰写情境感知、的共情消息、综合对话历史与情、感线索生成回复草、沟通协作领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
