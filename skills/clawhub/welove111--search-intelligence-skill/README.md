...
**Zero API keys. 90+ engines. Intelligent dork generation.**

**⚠️ Network notice:** Search queries are sent over the network to your configured SearXNG instance, which may relay them to third-party search engines (Google, Bing, DuckDuckGo, etc.). Queries are not processed locally or kept private — do not submit sensitive, confidential, or personally identifiable information without the end user's informed consent.
