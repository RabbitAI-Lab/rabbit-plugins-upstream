...
|---|---|
| 🏷️ 技能名称 | **宠物行为识别技能** |
| 🎯 核心目标 | 宠物行为识别技能，识别抓挠、啃咬、拆家、跳跃、刨地、追逐、独处焦虑等常见宠物异常行为，帮助主人了解宠物行为习惯 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `PET_BEHAVIOR_DETECTION` |
...
通过视频/监控对宠物进行行为识别分析，识别常见宠物行为尤其是异常行为，输出结构化的宠物行为分析报告
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供宠物监控视频 URL 或文件需要进行宠物行为识别时，默认触发本技能 |
| 🔎 明确分析意图 | 当用户明确需要进行宠物行为识别，提及抓挠、啃咬、拆家、焦虑行为、宠物行为分析等关键词，并且上传了视频 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看历史行为报告、宠物行为分析报告清单、行为识别报告列表、查询历史行为分析 |
...
```bash
python -m scripts.pet_behavior_detection_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.pet_behavior_detection_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 3 | ⚙️ 执行宠物行为识别 | 调用 `-m scripts.pet_behavior_detection_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| `--duration-min` | 统计时长（分钟），默认自动识别 | 按需填写 |
| `--list` | 显示宠物行为识别历史分析报告列表清单 | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/pet_behavior_detection_analysis.py`](scripts/pet_behavior_detection_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 🧑‍⚖️ 结果性质 | 分析结果仅供参考，行为矫正建议请咨询专业宠物训练师 |
...
```bash
# 分析本地监控视频
python -m scripts.pet_behavior_detection_analysis --input /path/to/monitor.mp4 --duration-min 10

# 分析网络监控视频
python -m scripts.pet_behavior_detection_analysis --url https://example.com/pet_monitor.mp4

# 显示历史行为分析报告（自动触发关键词：查看历史行为报告、历史报告、行为报告清单等）
python -m scripts.pet_behavior_detection_analysis --list

# 输出精简报告
python -m scripts.pet_behavior_detection_analysis --input video.mp4 --detail basic

# 保存结果到文件
python -m scripts.pet_behavior_detection_analysis --input video.mp4 --output result.json
```
