...
def ocr_office_via_ocr(filepath, source_ext):
    ...
    Returns:
        str: 提取的文本。失败返回 None。
    """
    tmp_dir = "/tmp/office_ocr_work"
    os.makedirs(tmp_dir, exist_ok=True)
    
    # 1. soffice 转 PDF
    pdf_path = os.path.join(tmp_dir, uuid.uuid4().hex + ".pdf")
    proc = subprocess.Popen(
    ...
    try:
        ...
        for fp in [converted_pdf, ocr_pdf]:
            if fp and os.path.exists(fp):
                try:
                    os.remove(fp)
                except:
