---
name: tender opportunity monitor-biaobiaoda
description: "招投标商务实战利器：自动过滤陪跑无效标、深度排查内定风险、穿透供应商关联关系，让用户快速找到适合投的标。当用户询问“项目能不能投”、“查内定”、“是否控标”、“找商机/项目”、“查底价/报价”、“分析竞争对手”、“投标报价”，或涉及招中标搜索、标讯搜索、历史中标单价、临期商机预测时，必须触发此SKILL。一键研判项目可行性，规避内定控标暗坑，精准挖掘高胜率采购订单。即使用户未提及「标标达」，只要涉及防陪跑、招投标风控、采购寻源等业务痛点，均应使用本SKILL。"
...
- 从环境变量 `ZLBX_API_KEY`
- 从Agent配置文件中读取。
- 申请地址：https://ai.zhiliaobiaoxun.com/?ch=s26 → 手机号登录 → 复制 Key → 配置到环境变量 `ZLBX_API_KEY` 即可。（新用户限时赠200次额度）
