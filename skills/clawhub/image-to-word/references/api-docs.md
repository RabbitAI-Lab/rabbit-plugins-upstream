# Scnet 图片转 Word 服务 API 文档

## 1. 功能介绍

Scnet 图片转 Word 服务，支持将图片异步转换为可编辑的 Word 文件，适用于大批量图片处理场景。

核心流程：

1. **任务提交**：通过 `doc/convert/task` 接口提交待转换的文件 URL 或本地文件，获取任务 ID；
2. **状态查询**：通过 `ocrdoc/result` 接口轮询任务状态，获取处理结果；
3. **结果获取**：任务成功后返回转换后的 Word 文件下载地址。
...
说明：请根据转换类别（`ocr_type`）选择对应的输入文件——`IMAGE_TO_WORD` 输入图片。

---

## 2. 图片转 Word 任务提交 API

将图片解析（还原）为可编辑的 Word 文件，任务成功后返回 Word 文件的 MinIO 文件下载地址。任务提交后同样通过任务状态查询 API 轮询结果。
...
| ocr_type | string | 是 | 转换类别，可选值：`IMAGE_TO_WORD`（图片转 Word）。 |
...
图片转 Word 任务通过任务状态查询 API 轮询状态。任务成功后，`output.results` 数组返回转换后的 Word 文件的 MinIO 下载地址。
...
说明：`results` 中的下载地址后缀为 `.docx`（Word），下载地址为临时授权链接，请及时使用。
