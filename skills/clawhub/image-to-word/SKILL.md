---
name: image_to_word
description: 图片转Word 服务，支持将 图片异步转换为可编辑的 Word 文件。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
dependencies:
  ...
  - requests
input:
  - ocrType : 转换类型，IMAGE_TO_WORD
  - filePath : 待转换文件的本地路径
output: 转换后的 Word 文件下载地址
---
# Sugon-Scnet 图片转Word 服务技能

本技能封装了 Scnet 图片转Word 服务，支持将 图片异步转换为可编辑的 Word 文件，并通过轮询获取转换结果。

---

## 功能特性

- **图片转 Word**：将图片中的文档内容还原为 `.docx`。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 转换类型。必须为 IMAGE_TO_WORD（图片转 Word） |
| filePath | string | 是 | 待转换文件的本地绝对路径。图片格式。 |

### 命令行调用示例

```bash
   python .claude/skills/image_to_word/scripts/main.py IMAGE_TO_WORD /path/to/image.png
```
...
- “帮我把这张图片转换成 Word，图片路径 /Users/name/Downloads/slide.png”
...
- 输入文件必须与 `ocrType` 匹配：PDF_TO_WORD 输入 PDF，IMAGE_TO_WORD / IMAGE_TO_PPT 输入图片。
- 下载地址为临时授权链接，请及时下载使用。
