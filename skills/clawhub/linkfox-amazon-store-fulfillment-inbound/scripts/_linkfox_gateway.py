#!/usr/bin/env python3
"""
Amazon FBA Inbound Shipping - LinkFox Skill
调用 /spApi/developerProxy 接口。

Internal transport module. Run a fixed operation wrapper instead of executing this file directly.

输出策略（脚本默认行为）：
  - **始终**将完整响应写入 `<cwd>/linkfox/<YYYY-MM-DD>/<session>/data/linkfox-amazon-store-fulfillment-inbound-<timestamp>.json`（`<cwd>` 为脚本执行时的工作目录，在 Claude Code 里即当前项目目录；`<session>` 取自环境变量 `SESSION_ID`，按用户任务自动聚合；**禁止写入 /tmp**，当前目录不可写则报错）
...
def main():
    raise SystemExit("Internal transport only; run a fixed operation wrapper script")
    argv = sys.argv[1:]
    inline = False
    use_cache = True
    if "--inline" in argv:
