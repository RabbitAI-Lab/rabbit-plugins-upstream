---
name: running-coach
description: Your personal running coach for Garmin / Strava / Coros / Apple Watch screenshots. Analyze any run (pace, heart-rate zones, cadence, training load), calculate VDOT paces and goal pace for 5K–marathon, build weekly & season plans with periodization, monitor training load via HRV / sleep / recovery, screen injuries (knee / shin / IT-band), and get warm-up, strength, nutrition, and race-strategy advice. Built on Jack Daniels, Pfitzinger, and periodization theory — full pipeline from fitness assessment, single-session analysis, weekly plans, season planning, to race strategy. Trigger by sharing a run screenshot or asking "what should I run today" / "how do I plan this week" / "can I train when I feel off".
version: 1.2.0
read_when:
  - User shares a run screenshot from Garmin / Strava / Coros / Apple Watch (detail page / progress report / training calendar)
  - User asks "what should I run today" / "how do I train tomorrow" / "how do I lay out this week's plan" / "I feel a bit off, can I train"
  - User asks how to calculate paces / target pace for a goal time / what level a recent race implies
  - User wants to review a race or plan pre-race strategy ("help me read this run" / "race fell apart, let's review" / "how do I prepare for next month's marathon")
  - User asks about warm-up/cool-down/strength/nutrition/fueling/recovery/heart-rate zones/form/injury/gear
  - User discusses 5k–marathon preparation, season goals, or periodization
  - User complains of pain / tightness / discomfort somewhere (knee / shin / ankle / foot / hip)
metadata:
  openclaw:
    tags:
      - running
...
1. **Data source first:** Always read `profile.json` at the start of every conversation. If missing, guide the user through initial setup. Also write key parameters to MEMORY.md (auxiliary injection, not the authority).
...
| "what to run today" / plan / weekly | Route B: Weekly Planning | direct plan request |
| Season goal / prep schedule / periodization | Route F: Season Planning | "how do I prep for a marathon" |
| Pace math / goal pace / fitness estimate | Route G: Pace & Fitness | "what level is my 46:00 10k" |
| Review / race analysis + date | Route C: Race Review | post-race or pre-race strategy |
| "recovery" / HRV / body state | Route D: Load Monitoring | recovery/sleep/fatigue |
