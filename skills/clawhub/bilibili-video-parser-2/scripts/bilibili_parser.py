...
def get_metadata(bvid):
    ...
    raw = run_cmd(
        f'curl -s "https://api.bilibili.com/x/web-interface/view?bvid={bvid}" '
...
def get_subtitles(bvid, cid):
    ...
    raw = run_cmd(
        f'curl -s "https://api.bilibili.com/x/player/v2?bvid={bvid}&cid={cid}" '
        f'-H "User-Agent: Mozilla/5.0"'
    )
    data = json.loads(raw)
    ...
    if subtitles:
        ...
        sub_raw = run_cmd(f'curl -s "{sub_url}"')
...
def download_streams(bvid, cid, work_dir, quality=16):
    ...
    # Get stream URLs
    raw = run_cmd(
        f'curl -s "https://api.bilibili.com/x/player/playurl?bvid={bvid}&cid={cid}'
        f'&qn={quality}&fnver=0&fnval=16&fourk=0" '
    ...
    run_cmd(
        f'curl -L -o "{video_m4s}" "{video_url}" '
        f'-H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" '
        f'-H "Referer: https://www.bilibili.com"',
        timeout=120
    )
    ...
    run_cmd(
        f'curl -L -o "{audio_m4s}" "{audio_url}" '
        f'-H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" '
        f'-H "Referer: https://www.bilibili.com"',
        timeout=120
    )
    ...
    run_cmd(f'ffmpeg -i "{video_m4s}" -i "{audio_m4s}" -c copy "{mp4_path}" -y', timeout=30)
...
def analyze_frames(mp4_path, work_dir, interval=5, sample_count=None):
    ...
    # Extract frames
    run_cmd(
        f'ffmpeg -i "{mp4_path}" -vf "fps=1/{interval}" -q:v 2 '
    ...
    for i, fname in enumerate(frame_files):
        ...
        result = run_cmd(
            f'z-ai vision -p "请用中文描述这张视频截图的内容，包括人物、场景、屏幕上的文字等所有细节" '
            f'-i "{fpath}"',
            timeout=60
        )
        # Parse VLM response
...
def transcribe_audio(mp4_path, work_dir, chunk_seconds=25):
    ...
    run_cmd(
        f'ffmpeg -i "{mp4_path}" -vn -acodec pcm_s16le -ar 16000 -ac 1 "{audio_wav}" -y',
        timeout=30
    )
    ...
    run_cmd(
        f'ffmpeg -i "{audio_wav}" -f segment -segment_time {chunk_seconds} '
        f'-c copy "{chunks_dir}/chunk_%03d.wav" -y',
        timeout=30
    )
    ...
    for i, fname in enumerate(chunk_files):
        ...
        result = run_cmd(f'z-ai asr -f "{fpath}" -o "{out_path}"', timeout=60)
