...
This skill sends video frame screenshots and audio clips to external AI service providers configured via the `z-ai` CLI for VLM (visual analysis) and ASR (speech-to-text transcription). Please observe the following guidelines:

- **Data Handling**: Video frames and audio segments are transmitted over the network to third-party AI services. No video metadata or personal account information is included in these requests.
- **User Responsibility**: You should only use this skill with videos for which you have appropriate processing rights. Do not analyze videos containing sensitive, confidential, or personally identifiable information without proper authorization.
- **Path Safety**: Avoid using untrusted `work-dir` paths or media/subtitle URLs from unverified sources, as these may lead to unintended data exposure.
- **Opt-Out**: If you wish to disable remote AI processing, use the `--skip-visual` flag to bypass VLM frame analysis and/or the `--skip-audio` flag to bypass ASR transcription. Metadata extraction and subtitle retrieval are performed via Bilibili's public API only and do not involve third-party AI services.
