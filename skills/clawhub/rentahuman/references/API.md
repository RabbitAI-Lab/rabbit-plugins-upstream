...
> Auto-generated from `rentahuman-mcp@1.23.0` — do not edit manually.
> Run `node --import tsx scripts/sync-clawhub.mjs` to regenerate.

Complete reference for all 82 MCP tools available through the `rentahuman-mcp` server.
...
Search the authenticated admin API catalog by capability, route, source file, HTTP method, or risk. Use this before run_admin_operation. Only available when RENTAHUMAN_ADMIN_API_KEY is configured.

**Parameters:**

- `query` (optional) — string; Capability or route text such as users, disputes, payout, or reconciliation.
- `method` (optional) — `"DELETE"` | `"GET"` | `"PATCH"` | `"POST"` | `"PUT"`
- `risk` (optional) — `"read"` | `"mutating"` | `"high_impact"`
- `limit` (optional) — number; min 1, max 200

---

### `run_admin_operation`

Execute one operation returned by search_admin_operations. Path parameters and query values are encoded safely. Set confirmMutation=true only after the user explicitly requests a mutating or high-impact action. Only available when RENTAHUMAN_ADMIN_API_KEY is configured.

**Parameters:**

- `operationId` (required) — string
- `pathParameters` (optional) — object
- `query` (optional) — object
- `body` (optional) — unknown
- `confirmMutation` (optional) — boolean; Required for every non-read operation. Set only when the user explicitly requested the mutation.
