...
> Auto-generated from `rentahuman-mcp@1.26.0` — do not edit manually.
> Run `node --import tsx scripts/sync-clawhub.mjs` to regenerate.

Complete reference for all 84 MCP tools available through the `rentahuman-mcp` server.
...
- `requiredQualificationIds` (optional) — array of string; Versioned qualification definition IDs every applicant must satisfy. Applicants with matching active badges skip the qualification flow.; max items 10
...
- `requiredQualificationIds` (optional) — array of string; Replace the versioned qualification definitions required for future applications.; max items 10
...
Hire one human to rewrite private text without generative AI. Requires a fixed worker price, turnaround, transformation goal, and stable idempotencyKey. Optionally provide applicantScreening.aiGeneratedSourceText to require a proportional pre-acceptance rewrite sample (capped at 500 words) with its own document, continuous screen recording, and dual detector analysis. A languageRequirement and resume upload can also be required. The main source is revealed only to the accepted worker. Copyleaks and Pangram results are advisory and never prove authorship. Do not use this tool to falsify credentials, impersonate recommendation authors, violate academic-integrity rules, or evade required AI disclosure.
...
- `languageRequirement` (optional) — object { language: string, minimumProficiency?: `"conversational"` | `"professional"` | `"fluent"` | `"native"` }
...
Get an owned humanization bounty, including funding and assignment state, deadline attempts, submitted document and screen recording, review state, and separate advisory Copyleaks and Pangram results.
