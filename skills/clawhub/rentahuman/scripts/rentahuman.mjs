...
function generateIdentity(name = 'default') {
  ...
  fs.writeFileSync(getIdentityPath(name), JSON.stringify(identity, null, 2), {
    mode: 0o600,
  });
  return identity;
}
...
function signRequest(identity, actionContext = '') {
  ...
  const signature = crypto
    .sign(null, Buffer.from(message), privKey)
    .toString('base64');
  ...
  if (!key) {
    console.error(
      JSON.stringify({
        ...
        help: 'Get an API key at https://rentahuman.ai/account (requires $9.99/mo verification)',
      })
    );
    process.exit(1);
  }
...
const commands = {
  ...
  async reviews(args) {
    console.log(
      JSON.stringify(await get(`/reviews?humanId=${args[0]}`), null, 2)
    );
  },
  ...
  async 'start-conversation'(args) {
    ...
    console.log(
      JSON.stringify(await authPost('/conversations', body), null, 2)
    );
  },
  ...
  async 'send-message'(args) {
    ...
    const verification = agentVerification(
      id,
      `message:${params.conversationId}:${contentPreview}`
    );
    const body = {
      senderType: 'agent',
      senderId: id.agentId,
  ...
  async 'accept-application'(args) {
    ...
    const verification = agentVerification(
      id,
      `accept_application:${params.bountyId}:${params.applicationId}`
    );
    const body = {
      action: 'accept',
    ...
    console.log(
      JSON.stringify(
        await authPatch(
          `/bounties/${params.bountyId}/applications/${params.applicationId}`,
          body
        ),
        null,
        2
  ...
  async 'reject-application'(args) {
    ...
    const verification = agentVerification(
      id,
      `reject_application:${params.bountyId}:${params.applicationId}`
    );
    const body = {
      action: 'reject',
      response: params.response,
...
Environment:
  RENTAHUMAN_API_KEY      Required for write operations (get at rentahuman.ai/account)`);
  process.exit(1);
}
try {
