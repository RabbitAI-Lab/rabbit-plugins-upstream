#!/bin/bash
# OpenClaw Cron 批量修复脚本（带语法校验）
# 用途：自动修复常见的cron配置错误，并在写入前校验语法
...
VALIDATE_SCRIPT="$(dirname "$0")/validate-jobs-syntax.sh"

echo "========================================"
echo "  OpenClaw Cron 批量修复工具 v3.0"
echo "  (带语法校验保护)"
echo "========================================"
echo ""
...
if [ ! -f "$VALIDATE_SCRIPT" ]; then
  echo -e "${COLOR_RED}❌ 错误: 找不到校验脚本 $VALIDATE_SCRIPT${COLOR_NC}"
  exit 1
fi
...
# 在写入前先校验语法
echo -e "${COLOR_BLUE}🔍 校验修复后的JSON语法...${COLOR_NC}"
echo ""

if "$VALIDATE_SCRIPT" "$WORK_FILE" 2>&1 | tee /tmp/validate_output_$$.txt; then
  echo ""
  echo -e "${COLOR_GREEN}✅ 语法校验通过！${COLOR_NC}"
else
  echo ""
  echo -e "${COLOR_RED}❌ 语法校验失败！拒绝写入。${COLOR_NC}"
  echo ""
  echo "这是为了防止破坏 jobs.json 文件。"
  echo "校验输出："
  cat /tmp/validate_output_$$.txt
  echo ""
  echo "备份文件: $BACKUP_FILE"
  rm -f "$TEMP_FILE" "$TEMP_FILE.tmp" "/tmp/validate_output_$$.txt"
  exit 1
fi
