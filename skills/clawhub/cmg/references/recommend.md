...
当用户首次触发推荐请求时，先检查环境（只读，不做任何修改）：

```bash
{baseDir}/scripts/setup.sh --check-only
```

若 mcporter 未安装或未配置，**先向用户索取 MCP Server 地址**，再执行：

```bash
{baseDir}/scripts/setup.sh --server-url https://<用户提供的地址>
```

脚本会依次：检查 Node.js、（经确认后）全局安装 mcporter、写入 `~/.mcporter/mcporter.json`、验证连通性。安装与写配置前都会请求确认。

> ⚠️ **安全要求 — 不得绕过**
>
> - **本 Skill 不内置默认 MCP Server 地址。** 地址必须由用户显式提供。
>   推荐请求会把用户的云资源清单（实例规格、地域、数量、操作系统等）发送到该地址，
>   这些信息足以刻画用户的生产环境拓扑。
> - **只接受 `https://`**（`localhost` / `127.0.0.1` 可用 `http://` 本地调试）。
>   明文 HTTP 会让链路上任意一跳读取或篡改这些数据，脚本会直接拒绝。
> - **不要替用户猜测地址，也不要在失败后回退到任何默认值。**
>   若用户没有可用地址，如实告知选型推荐当前不可用，并说明原因。
> - setup 会修改全局 npm 环境并写入用户主目录配置，属于持久化变更，必须由用户知情确认。
