...
在「从 asale 市场买」和「用它自己的订阅」之间切换 OpenClaw，并查看哪些正在运行的会话还用着旧配置。

## 触发关键词

- 让 OpenClaw 从市场买
- 把 OpenClaw 指向 asale
- 给 OpenClaw 加 asale provider
- asale 买入 openclaw
...
OpenClaw 在代理上走它自己的 `/openclaw/v1` 前缀。这个前缀不是装饰：OpenClaw 说的是它被
配置成什么方言就说什么，所以它的请求和 Claude Code 或 Codex 的逐字节相同——没有这个前缀，
代理会拿另一个工具的开关来卡它、把另一个工具的模型选择塞给它。

`models.mode` 会设成 `merge`，这样 OpenClaw 自带的 provider 会和 asale 的并存，而不是
被整个替换掉。
...
`openclaw` 那一行带着 `installed`、`enabled`（开关）、`in_effect`（它的实时配置真的指向代理）、
...
不传 `models` 就保持当前选择，传 `[]` 是清空。会改写的配置：`~/.openclaw/openclaw.json` —— 在 `models.providers` 下加一个 `asale` 条目，选了模型时再加上 `agents.defaults.model.primary`。
...
OpenClaw **只在启动时**读一次配置，所以已经在跑的会话拿的还是旧的。`running.openclaw` 会按 pid 列出
它们，并带上启动时的命令行。
...
> **如果这个 agent 本身就跑在 OpenClaw 里**，这个开关改写的就是托管着当前会话的那个工具的
> 配置。调用之前先说明，让用户决定。正在跑的会话保持当前配置——配置只在启动时读——但这是
> 唯一一个关于「用户自己的 agent」的买入开关，所以它绝不能被当成顺带一步翻掉。
