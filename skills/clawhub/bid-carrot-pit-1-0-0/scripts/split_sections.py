...
# -*- coding: utf-8 -*-
"""
split_sections.py — 招标文件分章节切分（萝卜坑识别器·可选加速器）

把一份长招标文件按中文层级标题切成结构化章节，帮助 LLM 精准定位
"萝卜坑"所在章节（第X章第Y条），避免超长上下文丢失条款。
...
# 这些区域是"萝卜坑"高发且 LLM 易漏扫的死角，切分时直接打标，
...
def main():
    ...
    ap = sum(1 for s in secs if s['flags'].get('appendix'))
    pm = sum(1 for s in secs if s['flags'].get('payment'))
    print(f"分章节完成 → {args.out}：{len(secs)} 个章节，正文约 {total} 字")
    print(f"盲区标记 → 高危条款(★/偏离/废标):{hm}  附件/附录/图纸:{ap}  付款/保证金:{pm}")
    for s in secs:
        tag = ''
        f = s['flags']
