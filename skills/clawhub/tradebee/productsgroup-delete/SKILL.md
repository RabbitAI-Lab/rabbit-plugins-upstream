---
name: productsgroup-delete
description: A product group deletion skill based on the "Tradebee Website Builder" Open API. It is used to batch delete one or more product groups under a specified site language and returns separate success and failure ID lists.
...
Use the Tradebee Website Builder Open API to delete one or more product groups.

Supports batch deletion of product group IDs under a specified language and returns separate success and failure item lists.
...
Product group ID list, up to 100 items.
...
|------------|---------|-------------|
| `approved` | boolean | Must be `true` only after the user explicitly confirms the language and exact product group IDs to delete. |
| `summary`  | string  | Restates the language and exact product group IDs shown to the user before execution. |
...
```json
{
  ...
  "confirmation": {
    "approved": true,
    "summary": "Confirmed by user: delete product groups 18468028, 18467384, and 18466967 in language en."
  },
  "id_list": [
    18468028,
    18467384,
...
```json
{
  ...
  "data": {
    ...
    "fail": [
      ...
      {
        "id": 18466967,
        "msg": "There are products under the group"
      }
    ]
  }
}
