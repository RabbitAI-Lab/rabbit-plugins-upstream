...
export default async function NewsGroupDelete(args = {}) {
    ...
    if (!hasConfirmation) {
        return {
            status: false,
            msg: "Explicit user confirmation is required before deleting news groups. Show the language and exact news group ID list first."
        };
    }
    ...
    try {
        const response = await fetch(
            "https://platform.tradew.com/openapis/newsgroup/delete",
            {
                method: "POST",
                headers: {
