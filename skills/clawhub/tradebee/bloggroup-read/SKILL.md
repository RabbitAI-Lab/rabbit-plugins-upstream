---
name: bloggroup-read
description: A blog group list retrieval skill based on the "Tradebee Website Builder" Open API. It is used to retrieve blog group data configured on the website. It supports filtering by language, exact blog group ID, field selection, and paginated retrieval for blog navigation, content organization, SEO planning, and content operations workflows.
...
Use the Tradebee Website Builder Open API to retrieve blog group list data.

Supports language filtering, exact blog group ID filtering, field selection, and paginated queries. It can be used for blog navigation building, group selection, content analysis, SEO organization, and similar blog-content workflows.
...
Exact blog group filter.

- Omit this field to read all blog groups
- If provided, use one positive integer (`> 0`) that matches an existing blog group

---

### `fields` (array of strings, Optional)

Field selection list. If omitted, all supported blog group fields are returned.
...
Blog group data list.

| Field             | Type             | Description                              |
|-------------------|------------------|------------------------------------------|
| bloggroup_id      | integer          | Blog group ID                            |
| language          | string           | Site language                            |
| group_name        | string           | Blog group name                          |
| tags              | array            | Keyword list                             |
| brief_description | string           | Short blog group description             |
| seo               | object           | SEO information                          |
| bloggroup_url     | string           | Blog group page URL path                 |
...
- `bloggroup_id` reads one exact blog group when provided
...
### 2. Read One Exact Blog Group

Use this when the user already has one real `bloggroup_id`.
...
### 3. Read Blog Groups With Selected Fields
...
### 4. Read One Exact Blog Group With SEO Fields
...
- Omit `bloggroup_id` to read all blog groups.
- Send `bloggroup_id` only when reading one exact blog group.
- When returning a list to the user, include one `bloggroup_id` and one preview URL in the final answer so the user can decide whether to preview it.
- Do not tell the user they must preview it.
- Send `bloggroup_url` and `bloggroup_id` in `fields` when the caller needs the blog group page URL path or when a list response must include one ID and one preview URL.
