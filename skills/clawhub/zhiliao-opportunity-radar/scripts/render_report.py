#!/usr/bin/env python3
"""把商机雷达扫描结果 JSON 渲染成自包含 HTML 清单。

零第三方依赖，内联 CSS，可直接浏览器打开/打印/转发。
用法：
    python3 render_report.py --in report.json [-o 输出目录]
输出：<输出目录>/<标题>_商机雷达.html，默认目录 ~/zlbx-opportunity-radar-files/

输入 JSON 结构（所有字段可缺省，缺省则该板块不渲染）：
{
  "radar_title": "安防监控 × 广东",
  "generated_at": "2026-07-15 10:00",
  "scan": {
    "scope": "关键词：安防/视频监控 · 广东 · 50万以上",
    "counts": {"proposed": 8, "intent": 12, "expiring": 5},
    "sort_note": "金额档位 + 紧急/成熟度 + 标的匹配度"
  },
  "top_picks": [
    {"stars": 3, "route": "拟建", "name": "...", "reason": "预算2400万、审批已办结", "url": "可选"}
  ],
  "next_step": "一句话行动建议",
  "proposed": [{"stars": 3, "name": "...", "caller": "...", "money": "2400万",
                "status": "办结", "pub_time": "2026-07-01", "action": "...", "url": "可选"}],
  "intent":   [{"stars": 2, "name": "...", "caller": "...", "money": "32.5万",
                "sm": "标的物摘要", "pub_time": "2026-07-14", "action": "...", "url": "可选"}],
  "expiring": [{"stars": 3, "name": "...", "caller": "...", "winner": "现供应商",
                "money": "7.5万", "end_date": "2026-08-14", "days": 30, "action": "...", "url": "可选"}],
  "data_notes": {"source": "数据来源说明", "gaps": ["数据缺口"], "cost_units": 12}
}

拟建条目的链接为主站登录页面（无免登录 sk），表头统一标注，条目链接仍原样输出。
报告头部固定展示知了标讯品牌标识（所有 skill 版本统一，与页脚的平台声明一致，不做马甲差异化）。
"""
from __future__ import annotations

import argparse
import json
import os
import re
from datetime import datetime
from xml.sax.saxutils import escape as _esc


def esc(s) -> str:
    return _esc(str(s if s is not None else ""))


ROUTE_COLOR = {"拟建": "#7c5cbf", "意向": "#0b7a6a", "临期": "#b9770e"}

# 知了标讯白色 logo（299x96 PNG base64 内嵌，保证报告离线/打印/转发时不裂图）
_LOGO_B64 = ""

CSS = """
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Microsoft YaHei',sans-serif;
     background:#f2f5f4;color:#1f2d2a;line-height:1.7;padding:24px 12px}
.page{max-width:920px;margin:0 auto}
.hero{background:linear-gradient(135deg,#0e9f8a,#0b7a6a);border-radius:14px;color:#fff;
      padding:28px 30px;margin-bottom:18px}
.hero .brand{display:flex;align-items:center;gap:12px;margin-bottom:14px}
.hero .brand-logo{height:30px;display:block}
.hero .brand-sub{font-size:11px;opacity:.7;letter-spacing:1.5px}
.hero .tag{font-size:13px;opacity:.85;letter-spacing:2px}
.hero h1{font-size:24px;margin-top:6px;font-weight:600}
.hero .meta{font-size:12px;opacity:.75;margin-top:10px}
.overview{border-radius:14px;padding:22px 26px;margin-bottom:18px;background:#e6f7f0;
      border:1px solid rgba(0,0,0,.05)}
.overview .counts{display:flex;gap:14px;flex-wrap:wrap;margin-bottom:8px}
.overview .count{background:#fff;border-radius:10px;padding:10px 18px;text-align:center;
      border:1px solid rgba(0,0,0,.04)}
.overview .count b{font-size:22px;color:#0b7a6a;display:block}
.overview .count span{font-size:12px;color:#5c6b68}
.overview .scope{font-size:13px;color:#374a46}
.toppicks{background:#fff;border-radius:14px;padding:22px 26px;margin-bottom:16px;
      border:1px solid rgba(0,0,0,.05);box-shadow:0 1px 3px rgba(14,60,50,.04)}
.toppicks h2{font-size:16px;color:#0b7a6a;margin-bottom:12px}
.toppicks ol{margin-left:22px;font-size:14.5px}
.toppicks li{margin-bottom:8px}
.toppicks .route{font-size:11px;font-weight:700;color:#fff;border-radius:4px;padding:1px 7px;margin:0 4px}
.toppicks .nextstep{margin-top:12px;padding:10px 14px;background:#f0f7f5;border-radius:8px;
      font-size:14px;font-weight:500}
.card{background:#fff;border-radius:14px;padding:22px 26px;margin-bottom:16px;
      border:1px solid rgba(0,0,0,.05);box-shadow:0 1px 3px rgba(14,60,50,.04)}
.card h2{font-size:16px;color:#0b7a6a;margin-bottom:6px;padding-left:10px;
         border-left:4px solid #0e9f8a}
.card .subnote{font-size:12px;color:#7a8a86;margin-bottom:12px;padding-left:14px}
table{width:100%;border-collapse:collapse;font-size:13px}
th{background:#f0f7f5;color:#3c534e;text-align:left;padding:8px 10px;font-weight:600;white-space:nowrap}
td{padding:8px 10px;border-top:1px solid #edf2f0;vertical-align:top}
.stars{white-space:nowrap}
.days-urgent{color:#c0392b;font-weight:700}
a{color:#0b7a6a}
.footer{font-size:12px;color:#7a8a86;background:#eef3f1;border-radius:14px;padding:18px 24px}
.footer .cta{margin-bottom:10px;font-size:13px}
.footer .disclaim{border-top:1px dashed #cfdad6;padding-top:10px;margin-top:6px}
.toolbar{position:fixed;right:18px;bottom:18px;z-index:9;display:flex;gap:10px}
.toolbar button{display:flex;align-items:center;gap:6px;border:none;cursor:pointer;
      background:#0b7a6a;color:#fff;font-size:13px;font-weight:600;
      padding:10px 18px;border-radius:99px;box-shadow:0 4px 14px rgba(11,122,106,.35)}
.toolbar button:hover{background:#0e9f8a}
.toolbar button:disabled{opacity:.6;cursor:wait}
@page{margin:10mm}
@media print{
*{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important}
body{background:#fff;padding:0}
.card,.overview,.toppicks{box-shadow:none;break-inside:avoid;border:1px solid #e3ebe8}
.hero{box-shadow:none;break-inside:avoid;border-radius:10px}
table,tr{break-inside:avoid}
.toolbar{display:none}}
"""


def _stars(n) -> str:
    try:
        n = int(n)
    except (TypeError, ValueError):
        n = 1
    return "⭐" * max(1, min(n, 3))


def _link(text, url):
    return f'<a href="{esc(url)}" target="_blank">{esc(text)}</a>' if url else esc(text)


def _route_table(title: str, subnote: str, head_cols: list[str], rows_html: str) -> str:
    if not rows_html:
        return ""
    sub = f'<div class="subnote">{esc(subnote)}</div>' if subnote else ""
    head = "".join(f"<th>{esc(c)}</th>" for c in head_cols)
    return (
        f'<div class="card"><h2>{esc(title)}</h2>{sub}'
        f'<div style="overflow-x:auto"><table><tr>{head}</tr>{rows_html}</table></div></div>'
    )


def render(d: dict) -> str:
    title = d.get("radar_title", "商机扫描")
    parts = []

    # 头部：品牌标识固定展示（所有 skill 版本统一，与页脚平台声明一致）
    brand_html = (
        f'<div class="brand"><img class="brand-logo" alt="知了标讯" '
        f'src="data:image/png;base64,{_LOGO_B64}"/>'
        '<span class="brand-sub">全网招中标大数据 · zhiliaobiaoxun.com</span></div>'
    )
    parts.append(
        f'<div class="hero">{brand_html}<div class="tag">商机雷达 · 早期商机扫描清单</div><h1>{esc(title)}</h1>'
        f'<div class="meta">生成时间：{esc(d.get("generated_at", datetime.now().strftime("%Y-%m-%d %H:%M")))}'
        f' · 数据来源：知了标讯全网拟建/招中标数据</div></div>'
    )

    # 扫描概览
    scan = d.get("scan") or {}
    counts = scan.get("counts") or {}
    count_defs = [("proposed", "拟建项目"), ("intent", "采购意向"), ("expiring", "临期续约")]
    count_html = "".join(
        f'<div class="count"><b>{esc(counts.get(k, 0))}</b><span>{label}</span></div>'
        for k, label in count_defs
    )
    scope = f'<div class="scope">扫描范围：{esc(scan["scope"])}</div>' if scan.get("scope") else ""
    sort_note = f'<div class="scope">排序依据:{esc(scan["sort_note"])}</div>' if scan.get("sort_note") else ""
    parts.append(f'<div class="overview"><div class="counts">{count_html}</div>{scope}{sort_note}</div>')

    # Top 机会
    picks = d.get("top_picks") or []
    if picks:
        lis = "".join(
            f'<li><span class="stars">{_stars(p.get("stars"))}</span>'
            f'<span class="route" style="background:{ROUTE_COLOR.get(p.get("route"), "#5c6b68")}">{esc(p.get("route", ""))}</span>'
            f'{_link(p.get("name"), p.get("url"))} —— {esc(p.get("reason", ""))}</li>'
            for p in picks
        )
        next_step = f'<div class="nextstep">🎯 下一步先做什么：{esc(d["next_step"])}</div>' if d.get("next_step") else ""
        parts.append(f'<div class="toppicks"><h2>🎯 Top 机会</h2><ol>{lis}</ol>{next_step}</div>')

    # 一、拟建项目
    rows = "".join(
        f'<tr><td class="stars">{_stars(x.get("stars"))}</td>'
        f'<td>{_link(x.get("name"), x.get("url"))}</td><td>{esc(x.get("caller", ""))}</td>'
        f'<td>{esc(x.get("money", ""))}</td><td>{esc(x.get("status", ""))}</td>'
        f'<td>{esc(x.get("pub_time", ""))}</td><td>{esc(x.get("action", ""))}</td></tr>'
        for x in d.get("proposed", [])
    )
    parts.append(_route_table(
        "一、拟建项目（提前 6-18 个月）",
        "拟建详情页需登录知了标讯主站查看完整信息",
        ["优先级", "项目", "立项单位", "投资额", "审批状态", "发布", "下一步动作"], rows))

    # 二、采购意向
    rows = "".join(
        f'<tr><td class="stars">{_stars(x.get("stars"))}</td>'
        f'<td>{_link(x.get("name"), x.get("url"))}</td><td>{esc(x.get("caller", ""))}</td>'
        f'<td>{esc(x.get("money", ""))}</td><td>{esc(x.get("sm", ""))}</td>'
        f'<td>{esc(x.get("pub_time", ""))}</td><td>{esc(x.get("action", ""))}</td></tr>'
        for x in d.get("intent", [])
    )
    parts.append(_route_table(
        "二、采购意向（发标前 1-3 个月）", "",
        ["优先级", "项目", "采购单位", "预算", "标的物", "发布", "下一步动作"], rows))

    # 三、临期续约
    def _days_cell(x):
        days = x.get("days")
        if days is None:
            return esc(x.get("end_date", ""))
        cls = ' class="days-urgent"' if isinstance(days, (int, float)) and days <= 30 else ""
        return f'{esc(x.get("end_date", ""))}（<span{cls}>{esc(days)}天</span>）'

    rows = "".join(
        f'<tr><td class="stars">{_stars(x.get("stars"))}</td>'
        f'<td>{_link(x.get("name"), x.get("url"))}</td><td>{esc(x.get("caller", ""))}</td>'
        f'<td>{esc(x.get("winner", ""))}</td><td>{esc(x.get("money", ""))}</td>'
        f'<td>{_days_cell(x)}</td><td>{esc(x.get("action", ""))}</td></tr>'
        for x in d.get("expiring", [])
    )
    parts.append(_route_table(
        "三、临期续约（现供应商合同到期窗口）", "",
        ["优先级", "项目", "采购单位", "现供应商", "合同额", "到期", "下一步动作"], rows))

    # 数据说明 + 页脚
    n = d.get("data_notes") or {}
    gaps = "；".join(n.get("gaps", [])) or "无"
    cost = f' · 本次扫描消耗约 {n["cost_units"]} 积分' if n.get("cost_units") else ""
    parts.append(
        '<div class="footer">'
        f'<div class="cta">📡 这套扫描条件可固化成「商机晨报」定时跑增量 · 清单涉及单位的完整档案与更多商机，见 '
        f'<a href="https://agent.zhiliaobiaoxun.com" target="_blank">知了商机大师</a>'
        f' · 本清单由 <a href="https://ai.zhiliaobiaoxun.com" target="_blank">知了标讯 AI 开放平台</a> 商机雷达 Skill 生成</div>'
        f'<div>数据说明：{esc(n.get("source", "知了标讯全网拟建/招中标数据"))} · 数据缺口：{esc(gaps)}{cost}</div>'
        '<div class="disclaim">免责声明：本清单基于公开的拟建审批与招中标数据自动生成，仅供商机线索参考，不构成商业决策建议，'
        '亦不构成对任何单位或个人行为的认定。数据可能存在不完整或滞后，请结合实际情况独立判断并自行承担决策结果。</div></div>'
    )

    # 右下角悬浮工具条：保存长图（SVG foreignObject 原生截图，零依赖；页面无外链资源故 canvas 不受污染）
    # + 存为 PDF（浏览器原生打印）。打印输出中自动隐藏。
    toolbar = (
        '<div class="toolbar">'
        '<button type="button" onclick="zlbxSavePng(this)" '
        'title="导出整页长图（PNG），适合微信/朋友圈转发">🖼 保存长图</button>'
        '<button type="button" onclick="window.print()" '
        'title="在打印对话框中选择「存储为 PDF」">🖨 存为 PDF</button></div>'
    )
    save_png_js = """<script>
function zlbxSavePng(btn){
  var node=document.querySelector('.page');
  var w=node.scrollWidth,h=node.scrollHeight;
  var styles='';
  document.querySelectorAll('style').forEach(function(s){styles+=s.textContent;});
  var xml=new XMLSerializer().serializeToString(node);
  var svg='<svg xmlns="http://www.w3.org/2000/svg" width="'+w+'" height="'+h+'">'
    +'<foreignObject width="100%" height="100%">'
    +'<div xmlns="http://www.w3.org/1999/xhtml"><style>'+styles+'</style>'+xml+'</div>'
    +'</foreignObject></svg>';
  var orig=btn.textContent;btn.textContent='生成中…';btn.disabled=true;
  function done(){btn.textContent=orig;btn.disabled=false;}
  function fail(){done();alert('当前浏览器不支持长图导出，请改用「存为 PDF」或截图');}
  var img=new Image();
  img.onload=function(){
    try{
      var scale=2;
      var canvas=document.createElement('canvas');
      canvas.width=w*scale;canvas.height=h*scale;
      var ctx=canvas.getContext('2d');
      ctx.scale(scale,scale);
      ctx.fillStyle='#f2f5f4';ctx.fillRect(0,0,w,h);
      ctx.drawImage(img,0,0);
      canvas.toBlob(function(b){
        if(!b){fail();return;}
        var a=document.createElement('a');
        a.href=URL.createObjectURL(b);
        a.download=document.title.replace(/[\\\\/:*?"<>|]/g,'_')+'.png';
        document.body.appendChild(a);a.click();a.remove();
        setTimeout(function(){URL.revokeObjectURL(a.href);},5000);
        done();
      },'image/png');
    }catch(e){fail();}
  };
  img.onerror=fail;
  img.src='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg);
}
</script>"""

    return (
        "<!DOCTYPE html><html lang='zh-CN'><head><meta charset='utf-8'>"
        "<meta name='viewport' content='width=device-width,initial-scale=1'>"
        f"<title>{esc(title)} · 商机雷达</title><style>{CSS}</style></head>"
        f"<body><div class='page'>{''.join(parts)}</div>{toolbar}{save_png_js}</body></html>"
    )


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--in", dest="infile", required=True, help="扫描结果 JSON 路径")
    ap.add_argument("-o", "--outdir", default=os.path.expanduser("~/zlbx-opportunity-radar-files"))
    args = ap.parse_args()

    with open(args.infile, encoding="utf-8") as f:
        data = json.load(f)

    os.makedirs(args.outdir, exist_ok=True)
    safe = re.sub(r'[\\/:*?"<>|\s]+', "_", data.get("radar_title", "商机扫描"))[:60]
    out = os.path.join(args.outdir, f"{safe}_商机雷达.html")
    with open(out, "w", encoding="utf-8") as f:
        f.write(render(data))
    print(out)


if __name__ == "__main__":
    main()
