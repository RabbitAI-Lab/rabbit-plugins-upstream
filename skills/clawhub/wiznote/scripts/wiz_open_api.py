...
class WizOpenApi:
    ...
    def _ws_write_collaboration(self, doc_guid, editor_token, version, blocks,
        ...
        # 将带 __id 的子单元格从 blocks 移到 doc_data 顶层
        clean_blocks = []
        extra_data_map = {}
        for block in blocks:
            if '__id' in block:
                cell_id = block['__id']
                # __val 是数组格式（表格子数据），直接放入顶层
                if '__val' in block:
                    extra_data_map[cell_id] = block['__val']
                else:
                    cell_data = {k: v for k, v in block.items() if not k.startswith('__')}
                    extra_data_map[cell_id] = cell_data
            else:
        ...
        # 将子单元格数据合并到 doc_data 顶层
        doc_data.update(extra_data_map)
        ...
        while i < len(lines):
            ...
            if '|' in line and i + 1 < len(lines) and re.match(r'^\s*\|[\s\-:|]+\|\s*$', lines[i + 1]):
                ...
                for key, val in extra.items():
                    blocks.append({'__id': key, '__val': val})
                continue
    ...
    def _parse_block(self, full_data, block):
        ...
        if block_type == 'text':
            ...
            for child_id in children:
                child_data = full_data.get(child_id)
...
""",
    ...
    def table_block(headers, rows, block_id=None):
        ...
        # 计算每列的最大内容宽度（中文字符算2倍宽），用于动态分配列宽
        def display_width(s):
            """计算字符串显示宽度：中文字符算2，英文/数字算1"""
            w = 0
            for ch in str(s):
        ...
        for ci, h in enumerate(headers):
            col_max_len[ci] = max(col_max_len[ci], display_width(h))
        for row in rows:
            for ci, cell in enumerate(row):
                col_max_len[ci] = max(col_max_len[ci], display_width(cell))

        # 基于内容宽度按比例分配列宽，总宽度=600px
        total_len = sum(col_max_len) or 1
        total_width = 600
        min_width = 80
        cols_width = [max(min_width, int(total_width * (l / total_len))) for l in col_max_len]
        # 修正：确保总宽度不超过total_width
        while sum(cols_width) > total_width:
            widest = cols_width.index(max(cols_width))
            cols_width[widest] -= 10

        def make_cell(text):
            nonlocal cell_idx
            cell_id = f'_t_{block_id}_{cell_idx}'
            inner_id = f'_tc_{block_id}_{cell_idx}'
            children.append(cell_id)
            extra_data[cell_id] = [{'text': [{'insert': str(text)}], 'id': inner_id, 'type': 'text'}]
            cell_idx += 1
    ...
    def link_block(text, url):
        ...
        return {
            'id': str(uuid.uuid4())[:8],
            'type': 'text',
            'text': [{'insert': text, 'attributes': {'link': url}}],
        }

    @staticmethod
    def webpage_block(url):
        """快捷方法：创建网页嵌入块"""
        return {
            'id': str(uuid.uuid4())[:8],
            'type': 'embed',
    ...
    @staticmethod
    def audio_block(src, file_name='', file_size=0, file_type='audio/mpeg'):
        """快捷方法：创建音频块（需先上传附件获取src）"""
        return {
            'id': str(uuid.uuid4())[:8],
            'type': 'embed',
            'embedType': 'audio',
    ...
    @staticmethod
    def file_block(src, file_name='', file_size=0, file_type=''):
        """快捷方法：创建文件嵌入块（需先上传附件获取src）"""
        return {
            'id': str(uuid.uuid4())[:8],
            'type': 'embed',
            'embedType': 'office',
    ...
    @staticmethod
    def drawio_block(src):
        """快捷方法：创建流程图/UML块（需先上传SVG获取src）"""
        return {
            'id': str(uuid.uuid4())[:8],
            'type': 'embed',
            'embedType': 'drawio',
    ...
    @staticmethod
    def encrypt_text_block(password, encrypted_text, prompt=''):
        """快捷方法：创建加密文本块"""
        return {
            'id': str(uuid.uuid4())[:8],
            'type': 'embed',
            ...
            'align': 'center',
            'embedData': {
                'realText': encrypted_text,
                'hasPassword': True,
                'prompt': prompt or password,
                'showPromptForAll': True,
                'height': 160,
                'mobileHeight': 80,
            },
        }
    ...
    @staticmethod
    def formula_block(tex):
        """快捷方法：创建数学公式块（LaTeX语法）"""
        block_id = str(uuid.uuid4())[:8]
        return {
            'id': block_id,
            'type': 'text',
    ...
    def make_inline_block(parts):
        ...
        for text, is_bold in parts:
            item = {'insert': text}
            if is_bold:
                item['attributes'] = {'style-bold': True}
