---
name: amazon-competitor-image-gap
description: >
  对比主 ASIN 与已授权竞品商品页的图片字段、可见信息和评论反馈，整理图片表达缺口与拍摄说明。仅用于图片信息审查；不用于广告素材投放、销量预测、库存、订单或自动发布。
  Requires an ARI API key (ari_live_*).
slug: competitor-images
displayName: Amazon 竞品图片差距
version: 1.4.7
summary: 对比商品页图片信息与评论反馈的表达缺口
license: MIT
---
...
- 本入口只执行 `page_compare/images`，输出模板固定为 `ops_page_compare`。
