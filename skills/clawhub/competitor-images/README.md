...
~~~text
对比主 ASIN 与竞品商品页图片信息，整理表达差距。商品 B0XXXXXXXX（美国站），竞品 B0YYYYYYYY（美国站）。
~~~

示例 ASIN 是占位符。AI 会识别商品、站点和目标，无需填写接口参数。
客户端未选中时，在问题前加“使用 $amazon-competitor-image-gap”。
...
固定工作流：`page_compare/images`，输出 `ops_page_compare`。
