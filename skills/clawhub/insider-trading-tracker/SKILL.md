...
- Any HTTP client, or no install at all via the official CLI (`npx -y sentisense@0.46.0`). Plain `curl` works, or Python 3.8+ using only the standard library.
...
```bash
npx -y sentisense@0.46.0 insiders NVDA --days 90
npx -y sentisense@0.46.0 insiders NVDA --days 180 --json
npx -y sentisense@0.46.0 insiders NVDA --full     # all rows in plain output, not the top 15
```

Plain output in a terminal, exact API JSON with `--json` (envelope included). Auth: `SENTISENSE_API_KEY` in the environment, or store it once with `npx -y sentisense@0.46.0 auth "$SENTISENSE_API_KEY"` (saved to `~/.config/sentisense/`, file mode 600, local to your machine, removable with `auth --remove`). The version is pinned deliberately: a pinned version runs reviewed, immutable code.
...
```bash
npx -y sentisense@0.46.0 insiders NVDA --days 90 --json
```
