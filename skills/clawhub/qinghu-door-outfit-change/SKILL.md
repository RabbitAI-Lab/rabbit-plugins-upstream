---
name: qinghu-door-outfit-change
description: 青虎AI 女装开门换装爆款仿拍：上传一张模特图和最多 4 套服饰图，快速生成「开门换装」变装视频，可选配音频，成本低出片快，适配女装带货与穿搭创作。当用户要做开门换装视频、变装视频、一镜到底换装、女装穿搭视频、多套衣服连续切换时必须触发。关键词：青虎AI、开门换装、变装视频、女装、穿搭、仿拍、带货视频、换装、多套服饰。
user-invocable: true
...
抖音上常见的**开门换装变装视频**：一个模特 + 最多 4 套衣服，出一条连续换装的成片。

固定计费（快照 12 积分/次），不按秒算，所以出片成本可控。
...
```bash
# 字段表
qhkit workflow options '{"workflow":"wf_041"}'
...
| --- | --- | --- |
| 模特 | 否（实际必给） | 模特图 1 张 |
| 服饰1 ~ 服饰4 | 否（实际必给） | 每个字段 1 张，**按顺序对应换装顺序** |
| 声音 | 否 | 背景音频，不传则用默认 |

字段在线上标为选填，但**模特和至少一套服饰不给就出不了片**——提交前替用户确认齐。

服饰按 `服饰1 → 服饰2 → …` 的顺序出现，用户想换顺序就调整字段对应关系。
服饰图建议是**平铺图或正面穿着图**，背景干净、单品完整。

音频用本地文件时优先传公网 URL 更稳妥。
...
- 最多 4 套服饰；要更多套数就分多条视频跑。
- 只做女装开门换装这一种形态；照着任意爆款视频重拍走「爆款视频模仿(女装)」。
- 模特肖像需已获授权。
...
- **爆款视频模仿(女装)**：照着现成爆款视频重拍
- **模特换装高一致性还原**：只要换装静态图不要视频时
