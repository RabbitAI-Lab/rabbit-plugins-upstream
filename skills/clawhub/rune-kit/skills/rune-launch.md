...
Use the metric-based rollback decision matrix defined in `rune-deploy.md` Step 4.5. Compare error rate, latency (p95), and availability against pre-deploy baseline for 15 minutes:

- ALL metrics ADVANCE → proceed to Phase 4
- ANY metric HOLD → extend monitoring to 30 minutes, alert user before proceeding
- ANY metric ROLLBACK → stop pipeline, invoke `rune-incident.md`, do NOT proceed to marketing
