...
> Rune L3 Skill | security | model: tier:heavy
...
- Auto-trigger: PostToolUse on `mcp__.*` / `WebFetch` / read_file of `**/uploads/**`
...
3. If `tool_name` is read_file: check `tool_input.file_path` against `**/uploads/**`. Skip otherwise.
...
5. MUST NOT advisory-tag source-code reads (read_file matches only when path is `**/uploads/**`) — false-positive cost is high
...
- **For repo source-code reads.** read_file matches only `**/uploads/**` paths by design. Source-code reads are the operator's own trust boundary.
...
- Every `mcp__*` (untrusted) / `WebFetch` / upload-read_file call gets a `[QUARANTINE-NOTICE]` in next-turn context
