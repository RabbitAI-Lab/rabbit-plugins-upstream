#!/usr/bin/env python3
"""
TianAPI AncBooks Fetcher
------------------------
根据章节ID查询古籍名著的详情内容，涵盖《论语》《道德经》《山海经》等经典文献。
零外部依赖，仅使用 Python 标准库。

Usage:
    # 根据章节ID查询古籍内容
    python3 fetch_ancbooks.py --key YOUR_KEY --id 9872ed9fc22fc182

Env Config:
    export TIANAPI_ANCBOOKS_KEY=your_api_key
    # 或在脚本同级目录创建 .env 文件: TIANAPI_ANCBOOKS_KEY=your_api_key
"""
...
# --- 配置常量 ---
API_ENDPOINT = "https://apis.tianapi.com/ancbooks/index"
ENV_KEY_NAME = "TIANAPI_ANCBOOKS_KEY"
DEFAULT_TIMEOUT = 10
...
class Config:
    ...
    def _exit_with_help(self, msg: str):
        ...
        print(f"💡 提示: 申请 Key -> https://www.tianapi.com/apiview/265", file=sys.stderr)
        sys.exit(1)
...
class TianAPIClient:
    ...
    def fetch_ancbooks(self, anc_id: str) -> Dict[str, Any]:
        """
        执行 API 请求
...
def main():
    parser = argparse.ArgumentParser(description="TianAPI AncBooks Fetcher")
    parser.add_argument("--key", help="API Key (可选，优先使用环境变量)")
    parser.add_argument("--id", required=True, help="古籍章节ID (如: 9872ed9fc22fc182)")
    ...
    # 2. 请求
    result = client.fetch_ancbooks(anc_id=args.id)
