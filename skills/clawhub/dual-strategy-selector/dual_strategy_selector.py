...
"""
双策略A股选股器 V1.0
策略1: 仙人指路B + 持股20天 (胜率54.2%, 均益+3.53%)
策略2: 老鸭头B + 持股5天 (胜率72%, 均益+0.77%)
...
def load_data(data_path, target_date=None):
    ...
    if not os.path.exists(data_path):
        ...
        print("[INFO] 请准备包含以下列的CSV文件:")
        print("  symbol, name, date, open, high, low, close,")
        print("  volume, turnover, ma5, ma10, ma20, ma60,")
        print("  volume_ratio, pct_change, is_up_limit")
        print("[INFO] 数据来源建议:")
        print("  - 同花顺问财API")
        print("  - Tushare Pro")
        print("  - 东方财富Choice")
    ...
    print(f"[OK] 日期范围: {df['date'].min().strftime('%Y-%m-%d')} ~ {df['date'].max().strftime('%Y-%m-%d')}")
...
def main():
    parser = argparse.ArgumentParser(description='双策略A股选股器 V1.0')
