...
在「从 asale 市场买」和「用它自己的订阅」之间切换 Gemini CLI，并查看哪些正在运行的会话还用着旧配置。

## 触发关键词

- 让 Gemini CLI 从市场买
- 把 Gemini CLI 指向 asale
- 别再消耗我的 Gemini 订阅
- asale 买入 gemini
...
是两个文件不是一个：`.env` 存端点和 key，`settings.json` 存的是端点一变、CLI 没有它就
起不来的那一项。两个都在备份里，关掉开关时两个都还原。
...
`gemini` 那一行带着 `installed`、`enabled`（开关）、`in_effect`（它的实时配置真的指向代理）、
...
不传 `models` 就保持当前选择，传 `[]` 是清空。会改写的配置：`~/.gemini/.env`（`GOOGLE_GEMINI_BASE_URL` / `GEMINI_API_KEY`）和 `~/.gemini/settings.json`。
...
Gemini CLI **只在启动时**读一次配置，所以已经在跑的会话拿的还是旧的。`running.gemini` 会按 pid 列出
它们，并带上启动时的命令行。
