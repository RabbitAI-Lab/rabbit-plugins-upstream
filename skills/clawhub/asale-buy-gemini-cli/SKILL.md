...
Switch Gemini CLI between buying from the asale market and using its own subscription, and see which running sessions are still on the old config.

## Trigger Keywords

- make Gemini CLI buy from the market
- point Gemini CLI at asale
- stop spending my Gemini subscription
- asale buy gemini
...
Two files, not one: `.env` carries the endpoint and the key, and
`settings.json` carries the one setting the CLI will not start without once the
endpoint changes. Both are captured in the backup, and both come back on the way
out.
...
The `gemini` row carries `installed`, `enabled` (the switch), `in_effect` (its live
...
```bash
call set_buy_tool '{"tool":"gemini","enabled":true}'
```

`models` omitted leaves the current selection alone; `[]` clears it. Config
rewritten: `~/.gemini/.env` (`GOOGLE_GEMINI_BASE_URL` / `GEMINI_API_KEY`) and `~/.gemini/settings.json`.
...
Gemini CLI only reads its config **at startup**, so a session that is already running
keeps the old one. `running.gemini` lists the ones still on it, by pid, with the
...
```bash
call set_buy_tool '{"tool":"gemini","enabled":false}'
```
