# Serenity Background (Serenity 背景介绍)

## English

### Who is Serenity?

- **X.com handle**: [@aleabitoreddit](https://x.com/aleabitoreddit)
- **Profession**: Semiconductor day trader
- **Location**: Unknown (likely US-based)
- **Experience**: 5+ years in semiconductor trading

### Trading Style

- **Primary strategy**: Supply chain bottleneck analysis (Bottleneck Theory)
- **Execution**: Day trading around core positions
- **Risk management**: Stop-thesis (not stop-loss)
- **Position sizing**: Tiered approach (Core/Swing/Lottery)
- **Research method**: Red-team/Blue-team adversarial testing (v2.1 new)
- **Margin usage**: Strategic use of margin (~1.4x), with caution
- **Information sources**: OSINT supply chain mapping (a16z, pitchbook, SMM prices, Gov publications)

### Information Sources

1. **X.com**: Primary platform for sharing analysis
2. **OSINT community**: "Poet OSINT community" (likely a paid group)
3. **Company earnings**: Reads supply chain company reports
4. **Supply chain research**: Tracks Korean, Chinese, Japanese companies

### Performance (Extracted from Tweets)

- **$AAOI**: Bought at $28, trimmed at $195 (357% gain)
- **$AXTI**: Target ~$15 (2025 year-end) → $82 (2026-05), precise hit
- **$NBIS**: PT $225 (2025-09-19) → included in Nasdaq 100 (2026-06-12)
- **$RPI (Raspberry Pi)**: Predicted revenue growth 55% vs analyst 14%, actual 58%
- **$SIVE**: Core position, CW laser bottleneck, margin ~1.4x used
- **$POET**: Core position, multiple accounts (conviction play)
- **$MU**: Core position, benefits from HBM bottleneck

**v2.1 Added Cases:**
- **$SIVE**: Short-term "murdered" (drawdown >50%), still holding millions of shares waiting for volume ramp
- **Photonics/CPO theme**: Multiple positions, $SIVE emphasized laser volume ramp, fabless model, NASDAQ listing potential

**Disclaimer**: These are self-reported results. Actual performance may vary.

### Philosophy

> "I don't look at price action first. I look at supply chain constraints, then work backwards to which companies can capture the margin."

> "Position sizing is more art than science."

> "My framework is high-risk. I've had 50% drawdowns. If you can't handle volatility, don't copy me."

### How to Follow

1. **X.com Premium**: Subscribe to see all tweets
2. **Read carefully**: Serenity often shares supply chain intelligence
3. **Verify independently**: Don't blindly copy trades
4. **Start small**: Use small positions to practice the framework

---

## 中文

### 谁是 Serenity？

- **X.com 账号**: [@aleabitoreddit](https://x.com/aleabitoreddit)
- **职业**: 半导体日内交易员
- **地点**: 未知（可能在美国）
- **经验**: 5+ 年半导体交易经验

### 交易风格

- **主要策略**: 供应链瓶颈分析
- **执行**: 围绕核心仓位进行日内交易
- **风险管理**: Stop-thesis（不是 stop-loss）
- **仓位管理**: 分层方法（核心/摆动/彩票）

### 信息来源

1. **X.com**: 分享分析的主要平台
2. **OSINT 社区**: "Poet OSINT community"（可能是付费群组）
3. **公司财报**: 阅读供应链公司报告
4. **供应链研究**: 跟踪韩国、中国、日本公司

### 表现（从推文中提取）

- **$AAOI**: 买入价 $28，减仓价 $195（357% 收益）
- **$POET**: 核心仓位，多个账户（高确信度）
- **$MU**: 核心仓位，受益于 HBM 瓶颈

**免责声明**: 这些是自我报告的结果。实际表现可能有所不同。

### 哲学

> "我不先看价格走势。我先看供应链约束，然后倒推哪些公司能捕获利润。"

> "仓位管理是艺术多于科学。"

> "我的框架是高风险的。我曾经有 50% 的回撤。如果你无法承受波动性，不要复制我。"

### 如何关注

1. **X.com Premium**: 订阅以查看所有推文
2. **仔细阅读**: Serenity 经常分享供应链情报
3. **独立验证**: 不要盲目复制交易
4. **从小开始**: 用小仓位练习框架
