---
name: linkedin-person-experience-zh
description: 调取 LinkedIn 用户的完整任职履历清单，梳理目标人员职业发展轨迹、就职企业变动情况以及专业从业背景，为商务尽调、客户背景分析提供参考依据。
...
使用跨境魔方开放平台API从领英数据查询人物的历史工作经历信息。

## 概述

本技能提供对跨境魔方领英人物工作经历信息的查询。通过人物ID（hid）获取历史工作经历列表，包括公司名称、职位名称、任职起止时间及任职状态等。可选传入公司ID（pid）筛选指定公司的工作经历。
...
**重要**：始终使用直接脚本调用，如 `python scripts/linkedin_person_experience_list.py`。**不要使用** shell 复合命令如 `cd scripts && python linkedin_person_experience_list.py`

### 工作经历列表查询 (`linkedin_person_experience_list.py`)
- **返回粒度**：每条工作经历记录为一行
...
- **示例**：
  ...
  - "获取下一页工作经历数据"
- **参数**：查看参数说明 [工作经历列表 API](references/linkedin-person-experience-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/linkedin/person/experience/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"领英工作经历查询失败，服务端异常"}'
```
...
```bash
python scripts/linkedin_person_experience_list.py --hid H_67890 --cursor '上一次响应返回的cursor字符串'
```
...
- 工作经历列表：查看 [references/linkedin-person-experience-list-api.md](references/linkedin-person-experience-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/linkedin-person-experience-list-api.md](references/linkedin-person-experience-list-api.md)
...
3. **跨技能使用**：
   - `hid`（人物ID）可从 **linkedin-person-search**（人物搜索）结果中获取
   - 工作经历结果中的 `pid`（公司ID）可与 **linkedin-company-search** 配合使用，查找公司更多详情
