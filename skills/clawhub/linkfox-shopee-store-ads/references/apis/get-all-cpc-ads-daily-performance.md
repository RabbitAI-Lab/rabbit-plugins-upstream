...
| 项 | 值 |
|----|-----|
| 脚本 | `scripts/get_all_cpc_ads_daily_performance.py` |
| Method / path | GET `api/v2/ads/get_all_cpc_ads_daily_performance` |
...
- Registry notes：All CPC ads daily performance
...
python scripts/get_all_cpc_ads_daily_performance.py '{"shopId": "67890"}'

# 通用入口
python scripts/ads_api.py '{"api": "get_all_cpc_ads_daily_performance", "shopId": "67890"}'
```
...
2. 再读 **`getAllCpcAdsDailyPerformance`**（Shopee `response` 解析结果）
3. 字段以官方文档为准
