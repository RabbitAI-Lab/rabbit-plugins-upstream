---
name: customs-company-hscode-stats
description: "Query company customs trade statistics by HS code dimension — analyze HS code distribution, trade volume breakdown, and monthly trade trends.\n\nTrigger: customs HS code statistics, trade HS code analysis, company product category breakdown, import-export classification analysis, global trade code intelligence"
...
# Customs Company HS Code Trade Statistics

Query company trade statistics aggregated by HS code dimension from customs data using the UpKuaJing Open Platform API.

## Overview

This skill provides access to company trade statistics by HS code dimension from UpKuaJing's customs database. Given a company ID and company type, it returns the trade summary broken down by HS code, including monthly trade trends and HS code distribution with trade counts and percentages.
...
**Important**: Always use direct script invocation like `python scripts/customs_company_hscode_stats.py`. **Do NOT use** shell compound commands like `cd scripts && python customs_company_hscode_stats.py`

### Company HS Code Trade Statistics Query (`customs_company_hscode_stats.py`)
- **Return granularity**: One record per company, containing HS code dimension aggregated trade statistics
- **Use cases**: View a company's trade distribution by HS code, analyze which product categories drive trade volume
- **Examples**:
  - "Get HS code statistics for company 100001 as a supplier"
  - "Query trade distribution by HS code for company 100001 as a buyer"
- **Parameters**: See [Company HS Code Trade Statistics API](references/customs-company-hscode-stats-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/customs/company/hscode/stats","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"Company HS code stats query failed with a server error"}'
```
...
Billed by **number of calls**, each call returns HS code statistics for one company:
...
| User Intent | Use API |
|-------------|---------|
| "Get HS code statistics for company 100001 as a supplier" | Company HS Code Trade Statistics Query |
| "Show trade distribution by product category for company 100001" | Company HS Code Trade Statistics Query |

## Usage Examples

### Query Company HS Code Trade Statistics

**User request**: "Get HS code statistics for company 100001 as a supplier"
```bash
...
- Company HS Code Trade Statistics: Check [references/customs-company-hscode-stats-api.md](references/customs-company-hscode-stats-api.md)
...
1. **Check API documentation**:
   ...
   - Check [references/customs-company-hscode-stats-api.md](references/customs-company-hscode-stats-api.md)
   - Do not guess parameter names, get accurate parameter names and formats from documentation

2. **Data interpretation**:
   - `totalHscodes` is the total number of unique HS codes
   - `tradeHscodes` shows HS code distribution with trade counts and percentages
   - `percentTrade` is the percentage of trade (as a decimal, e.g., 23.45 means 23.45%)

3. **Cross-skill usage**:
   - The company ID from results can be used with **customs-company-hscode-list** to get detailed HS code list
