...
def annotate_image(image_path, risks=None, output_path=None, bbox_space='original'):
    ...
    is_long_image = (orig_h / max(orig_w, 1)) >= 4
    max_display_h = 13500 if is_long_image else 4000
    max_display_w = 900 if is_long_image else 4000
    scale = 1.0
    ...
    # ============================================================
    panel_w = 448 if is_long_image else 520
    panel_h = display_h
    panel_padding = 12 if is_long_image else 18
    card_padding = 9 if is_long_image else 12
    card_gap = 8 if is_long_image else 10
    title_size = 18 if is_long_image else 22
    body_size = 12 if is_long_image else 15
    small_size = 11 if is_long_image else 13
    line_ratio = 1.22
    ...
    header_h = 54 if is_long_image else 68
    footer_h = 28
    top_limit = header_h + panel_padding
    bottom_limit = panel_h - footer_h - panel_padding
    if all(item['anchor_y'] is None for item in item_data_list):
        current_y = top_limit
    ...
    LINE_WIDTH = max(int(min(img_w, img_h) * (0.002 if is_long_image else 0.001)), 2)
    ...
    for risk in risks:
        for bbox in risk_bboxes(risk):
            ...
            center_x = int(np.clip(pts[0][0], circle_radius + 1, img_w - circle_radius - 1))
            center_y = int(np.clip(pts[0][1], circle_radius + 1, img_h - circle_radius - 1))
