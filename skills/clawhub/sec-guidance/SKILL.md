---
name: sec-guidance
description: Extract management guidance and forward-looking statements from SEC 10-K/10-Q filings. Self-contained by default (fetches from EDGAR, in-memory BM25, Claude/OpenAI). Optional heavy mode delegates to a local RAG pipeline.
version: 0.2.0
metadata:
  openclaw:
    requires:
      ...
      bins: ["python3"]
    optionalEnv: ["OPENAI_API_KEY", "SEC_PIPELINE_DIR", "SEC_GUIDANCE_UA", "SEC_GUIDANCE_MODEL"]
    homepage: https://github.com/TINGHAO0724/sec-guidance-skill
---

# SEC Guidance Extractor

Fetches the latest 10-K or 10-Q for a ticker from SEC EDGAR, ranks passages
with BM25, and asks Claude (or OpenAI) to answer forward-looking questions
...
| `OPENAI_API_KEY` | one of these | OpenAI API key (fallback) |
| `SEC_GUIDANCE_UA` | no | User-Agent for EDGAR requests (default: generic). SEC requires a real email — set to `"yourname you@example.com"` if you make many requests. |
...
source list showing the chunk index and BM25 score. Filings are cached under
...
See the reference implementation at the homepage repo.

## Limitations of light mode

- One filing at a time (the most recent 10-K or 10-Q). Heavy mode indexes
  the full history.
- BM25 only, no semantic embeddings or reranking. Roughly ~20% quality gap
  on the same single-filing query, larger gap on ambiguous questions.
