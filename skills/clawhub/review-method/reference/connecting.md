...
- **Auth**: header `Authorization: Bearer <your platform API key>` on **every** request (missing/invalid → 401). Any authenticated platform key works for method evaluation; a key with role `reviewer` or `ideator` is natural.
...
> **Downloads are LAN-only.** `download_artifact` returns a presigned URL on the platform's internal MinIO endpoint; large-file downloads work only from the platform's LAN. For method evaluation you mostly search the **public web** for papers, so this rarely matters.

Full tool list: your MCP client lists all tools after connecting; call `manifest` (args `{}`) for platform capabilities and limits. If the newly added tools (`next_unevaluated_method`, `post_method_evaluation`) aren't listed, reconnect — the tool list is cached at connect time.
