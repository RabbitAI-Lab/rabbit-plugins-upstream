...
**① 每个会话首次购物必须 discover**：每个 Agent 会话是独立上下文，磁盘上可能同时存在多个 active session（不同 host 各一个）。`utp_session_status` 不带 session_id 时返回的是全局最后活跃的 session（`_active` 标记），可能是另一个会话、另一个 host 的 session——直接复用会路由到错误的商业体。因此**购物流程的第一步永远是 `utp_discover`**，不要先调 `utp_session_status` 来决定是否跳过 discover：
...
**多节点共存**：用户要在不同购物节点间对比时，对每个节点分别 `utp_discover`，各拿到独立 `session_id`。后续操作传对应节点的 `session_id`，两个 session 互不干扰——服务端按 `session_id` 从磁盘恢复对应 host。

**登录同样受这条约束**：`utp_login` / `utp_link` 也依赖已建立的 session。用户上来就说"登录"时，顺序是 **先 discover → 再 `utp_login`**；返回 `needs_discover: true` 就补一次 discover 后重试，不要反复重试登录工具本身。两个不该调登录的场景：当前账号已绑定（卡片会直接显示"已登录"），除非用户明确要登录/换账号；以及仅因为返回 `needs_link`——它只表示该商家支持绑定，不代表当前未登录。
