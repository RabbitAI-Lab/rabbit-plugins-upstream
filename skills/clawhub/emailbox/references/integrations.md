...
> ⚠️ 以下工作流涉及发票、合同、简历等敏感数据的邮件发送。
> 执行前务必确认收件人地址正确，且用户确实授权发送这些数据。
...
> ⚠️ 本流程将发送发票金额、号码、购方/销方信息等财务数据。发送前确认收件人是正确的财务负责人邮箱。
...
> ⚠️ 合同内容、法务分析、风险点属于高度敏感信息。发送前向用户确认收件人（法务邮箱）无误，且用户清楚邮件包含合同原文及审核意见。
...
> ⚠️ 简历包含姓名、学历、工作经历等个人敏感信息（PII）。发送前确认用户有权转发该简历，且收件人（HR 邮箱）正确。
