...
1. 使用用户显式指定的名称或命令参数。
2. 使用对应场景的专用环境变量；没有专用变量时再使用 `TENCENT_COS_DATASET_NAME`。
3. 已知存储桶时，执行 `find-datasets-by-bucket` 获取候选数据集，并按模板筛选。
4. 仍有多个候选或缺少必要上下文时，再向用户确认。
...
| field key | 业务名 | 说明 |
| --- | --- | --- |
| `datasetName` | 数据集名称 | 用户可读的数据集名；优先使用用户指定值或 `TENCENT_COS_DATASET_NAME` |
