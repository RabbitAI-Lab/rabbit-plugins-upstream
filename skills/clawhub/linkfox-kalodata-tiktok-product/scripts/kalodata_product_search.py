#!/usr/bin/env python3
"""
Kalodata - TikTok Product Search & Detail - LinkFox Skill
调用 /kalodata/product/rank 接口。

Usage:
  python kalodata_product_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python kalodata_product_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/kalodata/product/rank"
SLUG = "linkfox-kalodata-tiktok-product"
...
def main():
    ...
    if not argv:
        print(
            "Usage: kalodata_product_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
