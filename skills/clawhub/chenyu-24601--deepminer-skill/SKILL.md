...
**背景**：DM 的所有异步任务（包括批量标注、批量媒体下载等）注册后都会进入 `PENDING` 状态，**必须通过 `task lifecycle --action start` 显式启动**，不会自动执行。`PENDING` 不是"排队等候"，而是"等待用户确认才启动"。

**通用规则：看到任何异步任务返回 `state: pending`，立即执行 `task lifecycle --action start`。**

常见会触发异步任务的工具调用：
- `sample_data_label` / 批量标注 → 返回 `task_id` + `PENDING`
- `media_download_task_register` / 批量媒体下载 → 返回 `task_id` + `pending`

两者都需要 `start`，否则任务永远不会执行。
...
**⚠️ 已知 CLI Bug**：当 `estimate_time` 字段返回数字（如 `300`）时，CLI 的 Go 结构体期望 string，会报 `json: cannot unmarshal number into Go struct field` 解析错误。遇到此错误时任务本身正常运行，可改用 `thread result` 查看 thread 级别状态，或直接用 `task lifecycle --action start` 确认任务状态。
...
**⚠️ 注意**：`task files` 仅适用于**批量下载/采集类任务**，标注任务不支持。
