...
```bash
export TIBETAN_AI_SKILL_API_KEY="ais_..."
```

可选地覆盖站点根地址：

```bash
export AI_SKILLS_API_URL="https://ai-skills.open-idea.net"
```
...
```text
Authorization: Bearer $TIBETAN_AI_SKILL_API_KEY
```

每个 Key 只绑定一个 Skill，其他产品的 Key 不能调用藏语接口。Key 缺失时引导用户在
