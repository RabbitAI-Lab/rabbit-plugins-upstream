...
def build_message(c, threshold):
    ...
    return "{emoji} 金价{direction}提醒{note}：当前 ${price:.2f}，相对{benchmark}变动 {chg:+.2f}%（阈值 ±{thr}%）".format(
        emoji=emoji, direction=c["direction"], note=note,
        price=c["price"], benchmark=c["benchmark"], chg=c["change_pct"], thr=threshold)
...
def format_alerts(alerts):
    ...
    for a in alerts:
        ...
        lines.append("- 类型: {} / 基准: {} (阈值 ±{}%)".format(
            a.get("type"), a.get("benchmark"), a.get("threshold_pct")))
        lines.append("- 状态: {} / 创建: {}".format(a.get("status"), a.get("created_at")))
        if a.get("sent_via"):
...
def main():
    ...
    if action == "detect":
        ...
        print("[信息] 动态阈值: ±{}%".format(threshold))
        if alerts:
        ...
        closes = history.daily_closes(history.load_series())
        print("动态阈值: ±{}%".format(compute_threshold(cfg, closes)))
