...
常规规则下的固定奖：三等奖 5+1 = 3000 元 / 四等奖 5+0·4+1 = 200 元 / 五等奖 4+0·3+1 = 10 元 / 六等奖 2+1·1+1·0+1 = 5 元。一、二等奖为浮动奖。

**「福运奖」是奖池联动的特别规定，不是固定规则**：

| 条件 | 状态 | 3+0（中 3 红、蓝球未中） |
|---|---|---|
| 奖池 ≥ 15 亿元 | 自动启动 | 中 5 元（中奖概率约 6.71% → 8.26%） |
| 奖池 < 3 亿元 | 自动停止 | 不中奖（回归常规规则） |

该规定于第 2026075 期后奖池降至 2.83 亿元，**自第 2026076 期（2026-07-05）起暂停**；当前（2026-08-29）奖池约 3.2 亿元，仍处暂停状态，因此**本技能默认按常规规则计算，3+0 记为未中奖**。

代码侧由 `ssq_power_engine.set_fuyun_mode()` / `set_fuyun_by_pool()` 切换口径。若奖池重新累积至 15 亿元，3+0 将恢复计奖，请以官方公告为准。
