...
set WD=%~dp0
set TASK=SSQ_Watchdog
set LAUNCHER=%WD%ssq_watchdog_launcher.bat
set LOG=%WD%ssq_watchdog_register.log
...
echo.
echo %date% %time% [OK] SSQ_Watchdog registered, SYSTEM daily 21:30 >> "%LOG%"
goto :done
...
echo.
echo %date% %time% [FAIL] SSQ_Watchdog registration failed >> "%LOG%"
