# -*- coding: utf-8 -*-
"""
双色球系统 — 强化引擎 / Power Engine  (V1.0 新增)
====================================================================
...
三大能力:
    1. walk_forward_backtest : 样本外滚动回测
       - 对历史上每个测试期 k, 只用 draws[:k] 训练(完全复刻 ssq_auto 的
         CDM 加权/蓝球评分/9项过滤), 生成与线上同构的 5 组推荐,
         再用真实 draws[k] 核对命中。
...
严谨性说明:
    - 组合评分用 top-15 号码枚举近似全量穷举(双色球6红球过滤后约数万组合)。
      因为评分是号码级平滑加权, top-15 在实测中已覆盖最优组合,
      且保证可复现与快速; 这是有记录的工程近似, 非逻辑改变。
    - 一等奖/二等奖为浮动奖, 此处用代表性固定值 (500万/20万),
...
# 单一可信源: 过滤器/蓝球评分与线上完全一致, 杜绝漂移
from ssq_common import passes_filters, back_score, calc_ac
import ssq_common as _C
...
#   注意: 双色球 3+0 / 2+0 / 1+0 / 0+0 均不中奖 (蓝球未中即无小奖)。
# 一等奖单注封顶500万(基本2元), 此处用代表性固定值。
PRIZE_PAYOUT_REGULAR = {
    (6, 1): 5_000_000,   # 一等奖 (浮动, 代表性固定值)
    (6, 0): 200_000,     # 二等奖 (浮动, 代表性固定值, 单注封顶500万)
    (5, 1): 3000,        # 三等奖
    (5, 0): 200,         # 四等奖
    (4, 1): 200,         # 四等奖
    (4, 0): 10,          # 五等奖
...
# ---- 「福运奖」特别规定: 奖池联动, 非固定规则 (2026-08-29 由静态写死改为动态) ----
# 规则(中福彩 2026-01-16 变更, 第2026014期起):
#   启动: 奖池资金 >= 15亿元 时自动进入特别规定, 增设福运奖 —— 中 3 个红球
#         (蓝球未中也算) 即中 5 元。中奖概率由约 6.71% 提升至 8.26%。
#   停止: 某期开奖后奖池资金 < 3亿元 时, 自动退出特别规定, 福运奖取消。
# 历史: 2026-02-01(2026014) 启动 → 2026-07-02 第2026075期后奖池跌至 2.83亿
#       → 自第2026076期(2026-07-05)起暂停。当前(2026-08-29)奖池约 3.2亿, 仍暂停。
# 修复前隐患: 系统曾把"福运奖暂停"这一**状态**写死进奖金表。若奖池重新累积到
#   15亿, 福运奖会自动恢复, 而系统将把 3+0 判为"未中奖" → 漏判中奖。
#   现改为双模式 + 显式开关, 默认沿用当前真实状态(暂停), 并在报告中标注口径。
PRIZE_PAYOUT_FUYUN = dict(PRIZE_PAYOUT_REGULAR)
PRIZE_PAYOUT_FUYUN[(3, 0)] = 5    # 福运奖: 3红(蓝未中) → 5元

POOL_FUYUN_TRIGGER = 1_500_000_000   # 15亿: 达此线自动启动福运奖
POOL_FUYUN_STOP = 300_000_000        # 3亿: 跌破此线自动停止福运奖

# 当前生效模式: False=常规规则(福运奖暂停, 默认, 符合当前奖池实况)
_FUYUN_ACTIVE = False


def set_fuyun_mode(active):
    """切换福运奖模式。True=特别规定执行中(3+0中5元), False=常规规则(默认)。

    **原地更新** PRIZE_PAYOUT 的内容(不重新绑定), 这样所有已经
    `from ssq_power_engine import PRIZE_PAYOUT` 的模块持有的仍是同一字典
    对象, 切换后自动同步 —— 若改成重新绑定, 那些模块会停留在旧表上,
    造成"口径已切但判定未变"的静默不一致。
    """
    global _FUYUN_ACTIVE
    _FUYUN_ACTIVE = bool(active)
    target = PRIZE_PAYOUT_FUYUN if _FUYUN_ACTIVE else PRIZE_PAYOUT_REGULAR
    PRIZE_PAYOUT.clear()
    PRIZE_PAYOUT.update(target)
    return PRIZE_PAYOUT


def set_fuyun_by_pool(pool_amount):
    """按奖池金额自动判定福运奖是否生效, 返回是否激活。

    注意: 规则是"滞回"的(15亿启动/3亿停止), 仅凭当期奖池无法判断处于哪一侧;
    此处按"是否达到启动线"作保守判定, 需要精确控制时用 set_fuyun_mode 手动覆盖。
    """
    active = pool_amount is not None and pool_amount >= POOL_FUYUN_TRIGGER
    set_fuyun_mode(active)
    return active


def prize_table(fuyun_active=None):
    """返回指定模式的奖金表; 不传参则用当前全局模式。"""
    if fuyun_active is None:
        fuyun_active = _FUYUN_ACTIVE
    return PRIZE_PAYOUT_FUYUN if fuyun_active else PRIZE_PAYOUT_REGULAR


def fuyun_label(fuyun_active=None):
    """返回人类可读的福运奖口径说明(勿含括号, 避免嵌套), 供报告/日志标注。"""
    if fuyun_active is None:
        fuyun_active = _FUYUN_ACTIVE
    return ('福运奖执行期·3红即中5元' if fuyun_active
            else '福运奖暂停期·常规规则3+0不中奖')


# 向后兼容主入口: 全系统既有 import 均走这里, 默认=常规规则(福运奖暂停)。
# 必须是**独立副本**而非 PRIZE_PAYOUT_REGULAR 的别名, 否则 set_fuyun_mode
# 的 clear() 会把 PRIZE_PAYOUT_REGULAR 一起清空。
PRIZE_PAYOUT = dict(PRIZE_PAYOUT_REGULAR)
PRIZE_NAME = {
    (6, 1): '一等奖', (6, 0): '二等奖',
    (5, 1): '三等奖',
    (5, 0): '四等奖', (4, 1): '四等奖',
...
# ============================================================
# 1. 模型计算 (逐字复刻 ssq_auto.compute_models 的评分逻辑)
# ============================================================
def compute_models(draws):
    ...
    n_total = total * 6
    # empirical Bayes (flat 先验, V1.0 双色球版本修复)
...
def _sample_blue(score_dict, used_backs, seed):
    """双色球蓝球为单码, 从 1-16 按权重抽 1 码 (返回长度1列表)。"""
    rng = random.Random(seed)
    for _ in range(400):
        combo = _weighted_sample_k(score_dict, 1, rng, list(range(1, 17)))
...
def recommend(draws_hist, valid_cache, seed_base=20260729):
    """生成与线上同构的 5 组推荐 (红球6元组 + 蓝球单码列表)。
    与 ssq_auto.generate_predictions 完全同构, 以期号种子后验采样。
    seed_base 传测试期期号即可复现。"""
    ...
    # 蓝球(单码)综合评分分布 (与 ssq_auto 一致: 走 ssq_common.back_score)
    back_scored = {}
    for num in range(1, 17):
    ...
    # 专家数据(双色球版可选, 缺省回落遗漏优选)
    expert_picks = []
    try:
        _ep = json.load(open('ssq_expert_picks.json', 'r', encoding='utf-8'))
    ...
    if expert_picks:
        ...
        eci_front_dist = {n: 0.6 * cs[n] + 0.4 * (100 - front_eci_pct.get(n, 0)) / 100 for n in range(1, 34)}
        back_eci_count = Counter()
        for _e, _f, _b in expert_picks:
            for n in (_b if isinstance(_b, (list, tuple)) else [_b]):
                back_eci_count[n] += 1
        ...
        eci_front_dist = {n: models['front_omit'].get(n, 0) for n in range(1, 34)}
    ...
    back_dist = {
        ...
        '冷号回补': {n: models['back_omit'].get(n, 0) for n in range(1, 17)},
        '逆向专家': eci_back_dist,
    ...
    strategy_defs = [
        ...
        ('冷号回补', {n: models['front_omit'].get(n, 0) for n in range(1, 34)}, back_dist['冷号回补']),
        (eci_name, eci_front_dist, back_dist['逆向专家']),
        ('熵均衡', {n: 1.0 for n in range(1, 34)}, back_dist['熵均衡']),
    ]
    ...
    for idx, (name, front_dist, back_dist_i) in enumerate(strategy_defs):
        ...
        for attempt in range(8):
            seed = seed_base * 100003 + idx * 7919 + 17 + attempt * 1000003
            ...
            if front is None:
                continue
            if any(len(set(front) & set(g[1])) >= 5 for g in groups):
                continue
            blue = _sample_blue(back_dist_i, used_backs, seed + 1)
            if blue is None:
                b0 = sorted(sorted(range(1, 17), key=lambda n: back_dist_i.get(n, 0), reverse=True)[:1])[0]
                blue = [b0]
            if any(len(set(blue) & set(g[2])) >= 1 for g in groups):
                continue
            used.add(tuple(sorted(front)))
            used_backs.add(tuple(sorted(blue)))
            groups.append((name, sorted(front), sorted(blue)))
            placed = True
            break
        if not placed:
            front = sorted(range(1, 34), key=lambda n: front_dist.get(n, 0), reverse=True)[:6] if front is None else front
            b0 = sorted(sorted(range(1, 17), key=lambda n: back_dist_i.get(n, 0), reverse=True)[:1])[0]
            groups.append((name, sorted(front), sorted([b0])))

    # 去重
    seen = set()
    out = []
    for name, front, blue in groups:
        t = tuple(sorted(front))
        if t in seen:
            continue
        seen.add(t)
        out.append((name, sorted(front), sorted(blue)))
    return out
...
# 3. 命中与派彩
# ============================================================
def score_one(front_rec, blue_rec, actual):
    """front_rec: 6元组; blue_rec: 蓝球单码列表(长度1); actual: 真实开奖。
    返回 (红球命中, 蓝球命中, 派彩)。蓝球命中=|blue_rec ∩ actual.back|∈{0,1}。"""
    fh = len(set(front_rec) & set(actual['front']))
    bh = len(set(blue_rec) & set(actual['back']))
...
def walk_forward_backtest(draws, start_idx=50, end_idx=None, seed=20260729):
    ...
    for k in range(start_idx, end_idx):
        ...
        period_payout = 0
        period_won = False
        for _, front, blue in recs:
            # 每组 = 1 红球组合 × 1 蓝球 = 1 注
            period_cost += COST_PER_BET
            bh = len(set(blue) & set(actual['back']))
...
def random_baseline(draws, start_idx=50, end_idx=None, n_sims=20, seed=12345):
    ...
    if end_idx is None:
        raise ValueError("end_idx required")
    rng = random.Random(seed)
    valid_cache = _build_valid_cache()
    ...
    for s in range(n_sims):
        ...
        for k in range(start_idx, end_idx):
            ...
            for _ in range(5):  # 5 组
                ...
                blue = r.choice(blue_all)
                period_cost += COST_PER_BET
                bh = 1 if blue in set(actual['back']) else 0
                fh = len(set(front) & set(actual['front']))
                pay = PRIZE_PAYOUT.get((fh, bh), 0)
                if pay > 0:
                    period_won = True
...
def run(window='sample'):
    ...
    print("=" * 70)
    print("【Power Engine V1.0: 强化引擎 / 科学严谨性自检】")
    print("=" * 70)
    ...
    _out = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ssq_power_report.json')
    with open(_out, 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2, default=str)
    print(f"\n  ✓ 强化报告已保存: ssq_power_report.json")
    return report


def _load_baseline(path='ssq_power_baseline.json'):
...
def _save_baseline(strat, path='ssq_power_baseline.json'):
    try:
...
def _self_test():
    # 奖级表完整性 (检查代表性中奖组合)
    assert PRIZE_PAYOUT[(6, 1)] > 0 and PRIZE_PAYOUT[(0, 1)] > 0 and PRIZE_PAYOUT[(3, 1)] > 0
    # 合成 60 期数据, 校验模型与评分
    import random as _r
    _r.seed(1)
    synth = []
    for i in range(60):
        front = sorted(_r.sample(range(1, 34), 6))
        back = [_r.choice(range(1, 17))]
        synth.append({'period': f'{i:05d}', 'date': '2020-01-01', 'front': front, 'back': back})
    m = compute_models(synth)
    assert len(m['combined_score']) == 33
    assert all(0 <= v < 1e9 for v in m['combined_score'].values())
    # 命中与派彩
    fh, bh, pay = score_one([1, 2, 3, 4, 5, 6], [7], {'front': [1, 2, 3, 9, 10, 11], 'back': [7]})
...
if __name__ == '__main__':
    if '--self-test' in sys.argv:
        _self_test()
        print("ssq_power_engine self-test OK")
    else:
