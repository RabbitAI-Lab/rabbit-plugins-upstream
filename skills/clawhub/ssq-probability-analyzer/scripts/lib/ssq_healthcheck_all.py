...
def check_cross_validate():
    ...
    if not _has_pred and not _has_html:
        check("三方交叉验证 52项", True,
              ...
              "生产环境(排程运行后)会自动校验")
        return
    # 【关键·顺序死锁修复 2026-08-25】交叉验证比对的是「目标期」的 JSON/HTML。
    # 本闸门在 ssq_smart Phase 0.5 (预测之前) 运行, 新一期首跑时目标期产物必然
    # 尚未生成 → 交叉验证脚本 FileNotFoundError 退出码1 → 闸门判 FAIL → 阻断预测,
    # 而预测正是产出该 JSON 的唯一途径, 形成「要预测须先验证/要验证须先预测」死锁。
    # 旧逻辑只判断「有任意期 JSON」(老期号文件恒存在)故永远无法命中跳过分支。
    # 正确语义: 目标期产物缺失 = 尚无内容可校验 → 跳过(非正确性失败)。
    # 真实校验不会丢失: ssq_smart Phase 4 在预测生成后仍会完整跑 52 项交叉验证。
    try:
        sys.path.insert(0, WORK_DIR)
        from ssq_period import next_period as _next_period_func
        with open(os.path.join(WORK_DIR, "ssq_history.json"), "r", encoding="utf-8") as _f:
            _hist = json.load(_f)
        _latest = _hist[-1]
        _target = str(_next_period_func(int(_latest["period"]), _latest.get("date")))
        _target_json = os.path.join(WORK_DIR, f"ssq_prediction_{_target}_v8.json")
        if not os.path.exists(_target_json):
            check("三方交叉验证 52项", True,
                  f"跳过(预测前置态): 目标期 {_target} 的预测产物尚未生成"
                  f"(ssq_prediction_{_target}_v8.json 不存在), 无内容可校验; "
                  f"本次预测生成后由 ssq_smart Phase 4 完整校验 52 项")
            return
    except Exception as _e:
        # 目标期推算失败不应静默放行, 但也不应误判为交叉验证回归, 仅记录后继续实跑
        print(f"  · 目标期产物预检异常({_e}), 继续实跑交叉验证")
    try:
