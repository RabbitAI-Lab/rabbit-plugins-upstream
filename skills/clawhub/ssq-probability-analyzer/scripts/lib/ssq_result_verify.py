...
def verify_prediction(pred_file, actual_front, actual_back):
    ...
    for i, group in enumerate(groups):
        ...
        # 双色球蓝球只有 1 个: 预测组给出 N 个蓝球候选 = N 注(每注红球相同 + 1 个蓝球),
        # 因此该组蓝球命中数最多为 1(实际开奖蓝球只有一个)。
        # ⚠️ 历史教训: 此处曾误抄大乐透「后区4选2」逻辑(min(back_hits,2)), 与双色球 6+1 规则不符。
        back_hits = min(back_hits, 1)
        prize, prize_money = determine_prize_level(front_hits, back_hits)
    ...
    if dantuo:
        # ⚠️ 历史教训(与 DLT 同源 BUG): 预测文件里 dantuo 是
        # {'standard':{dan,tuo,back}, 'optimized':{...}, ...} 结构,
        # 直接 dantuo.get('dan') 恒为空 -> 胆拖命中永远算成 0/0, 系统性欠报中奖。
        # 现兼容两种结构: 优先扁平 {dan:...}, 否则取 optimized/standard 子结构。
        if 'dan' in dantuo:
            dt = dantuo
        else:
            dt = dantuo.get('optimized') or dantuo.get('standard') or {}
        dan = set(dt.get('dan', []))
        tuo = set(dt.get('tuo', []))
        dt_back = set(dt.get('back', []))
        
        dan_hits = len(dan & actual_front_set)
        tuo_hits = len(tuo & actual_front_set)
        dt_back_hits = len(dt_back & actual_back_set)
        
        # 胆拖的最佳情况：胆全中 + 拖中剩余(双色球红球 6 个) + 蓝球(双色球仅 1 个蓝球)
        # ⚠️ 历史教训: 此处曾误抄大乐透「前区5个/后区2个」公式 (5-len(dan) / min(...,2))。
        best_front = dan_hits + min(tuo_hits, 6 - len(dan))
        best_back = min(dt_back_hits, 1)
            ...
            for r in results:
                ...
                if isinstance(r.get('group'), int):
                    print(f"    第{r['group']}组({r['name']}): "
                          f"红球{r['front_hits']}/6, 蓝球{r['back_hits']}/1, "
...
def print_performance_summary():
    ...
    print(f"  平均红球命中: {stats.get('avg_front_hits', 0):.2f}/6")
    print(f"  平均蓝球命中: {stats.get('avg_back_hits', 0):.2f}/1")
    print(f"  平均总命中: {stats.get('avg_total_hits', 0):.2f}/7")
    print(f"  最高红球命中: {stats.get('max_front_hits', 0)}/6")
