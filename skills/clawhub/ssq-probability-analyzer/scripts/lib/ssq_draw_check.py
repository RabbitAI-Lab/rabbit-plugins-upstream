...
# 福运奖与六等奖同为 5 元、同属最低中奖档(排名均=1); 置于六等奖之前,
# 使 RANK_TO_NAME 反查落在"六等奖"上(两档金额相同, 展示无歧义)。
# 若不登记, PRIZE_RANK.get('福运奖', 0) 会退化成"未中奖"排名, 漏计中奖。
PRIZE_RANK = {
    '一等奖': 6, '二等奖': 5, '三等奖': 4, '四等奖': 3,
    '五等奖': 2, '福运奖': 1, '六等奖': 1, '未中奖': 0,
}
...
def prize_of(fh, bh):
    ...
    if (fh == 2 and bh == 1) or (fh == 1 and bh == 1) or (fh == 0 and bh == 1):
        return '六等奖', pay
    # 福运奖(特别规定执行期): 中 3 个红球、蓝球未中 → 固定 5 元。
    # 常规规则下 3+0 不中奖(奖金表里没有该键), 只有 set_fuyun_mode(True) 后才会命中。
    if fh == 3 and bh == 0:
        return '福运奖', pay
    return '未中奖', pay
...
<div class='card'>
  <div class='verdict'>
    ...
    <p>① 本系统本期最佳成绩为 <span class='key'>{best_prize}</span>（理论奖金 {total_win} 元）。
       下面用<b>两个互相独立的维度</b>与随机基线对照，请勿混读：</p>
    <p style='margin-left:14px'>
       <b>维度A — 最佳奖级排名：</b>在同等 {real_bets} 注的随机机选基线下，
       <strong>仅 {my_percentile:.0f}% 的随机单期最佳奖级比我更差</strong>，
       有 {worse_or_equal:.0f}% 不差于我——即<b>按最佳奖级</b>我落在约第 {my_percentile:.0f} 百分位。
       <span style='color:#666'>（注：{real_bets} 注下几乎必然命中蓝球，
       故绝大多数随机机选都能拿到六等奖，此维度天然拥挤，处于低百分位属正常，
       不代表系统更差。）</span><br>
       <b>维度B — 总奖金金额：</b>我的 {total_win} 元 vs 随机机选中位数 {median_win} 元
       （P90 {p90_win} 元）。
       <span style='color:#666'>（注：本维度与维度A衡量对象不同，
       出现"金额高于中位数、但奖级百分位偏低"是正常现象，二者并不矛盾。）</span></p>
    <p style='margin-left:14px'>两个维度<b>共同指向同一结论：本期结果完全在正常随机波动区间内，
       未观察到任何超越随机的优势(no_edge)</b>。</p>
    ...
    <p>⑤ 本系统定位为<strong>反诈科普 / 理性购彩教育工具</strong>，不是"中奖说法"。
...
def verify_period(period, win_front, win_back, sim=20000):
    ...
    # 两个维度必须分别标注衡量对象, 否则"金额高于中位数"与"奖级低百分位"会被误读为矛盾
    print(f"[基线A·最佳奖级] 我={best_prize} → 约第 {my_pct:.0f} 百分位 "
          f"(仅 {my_pct:.0f}% 随机单期奖级比我差; {real_bets}注下几乎必中蓝球, "
          f"该维度天然拥挤, 低百分位属正常)")
    print(f"[基线B·总奖金]   我={total_win}元 vs 随机中位数 "
          f"{baseline['median_total_win']}元 / P90 {baseline['p90_total_win']}元 "
          f"(均值受头奖长尾≈{baseline['avg_total_win']:.0f}元, 参考即可)")
    print("[基线] 注: A/B 衡量对象不同, '金额高于中位数但奖级百分位偏低'并不矛盾; "
          "两者共同结论 = 正常随机波动, 无优势(no_edge)")
