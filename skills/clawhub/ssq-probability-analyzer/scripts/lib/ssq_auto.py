...
from ssq_expert_roster import render_expert_views_html, render_expert_baseline_html
...
def _gen_core(draws, models, valid_combos, expert_picks, salt=0):
    ...
    # ---- 最小成本最大收益覆盖方案 (V1.1 新增) ----
    # 纯组合覆盖(与号码置信度无关), 蒙特卡洛估计"至少中第 N 等奖"的概率。
    # 双色球结构要点: 六等奖=命中蓝球(1/16), 故覆盖 N 个不同蓝球 ≈ N/16 命中率。
    try:
        from ssq_coverage_optimizer import optimize, honest_ev_note
        _red_pool = sorted(combined_score, key=lambda n: combined_score[n], reverse=True)[:14]
        _blue_pool = sorted(back_scored, key=lambda n: back_scored[n], reverse=True)[:12]
        _best, _all = optimize(_red_pool, _blue_pool, budget=30, target_tier=6, n_sims=120000)
        if _best:
            dantuo['coverage_plan'] = {
                'name': _best.name, 'cost': _best.cost, 'notes': len(_best.tickets),
                'p_six': _best.p_capture.get(6, 0), 'p_five': _best.p_capture.get(5, 0),
                'p_any': _best.p_any,
                'red_pool': _red_pool, 'blue_pool': _blue_pool,
                'honesty': honest_ev_note(_best),
            }
            print(f"\n  最小成本最大收益方案: {_best.name} = {len(_best.tickets)}注 = ¥{_best.cost}"
                  f" | P(任意奖)={_best.p_any*100:.1f}% P(五等+)={_best.p_capture.get(5,0)*100:.2f}%")
        else:
...
def generate_prop_minigames_html():
    """理性购彩助手 · 三框（选号有依据 / 省钱有工具 / 中奖有核对）空白区填充互动小游戏。
...
def generate_box_games_js():
    ...
    "var msgs=['\u672A\u4E2D\u5956\u2026\u4E0B\u671F\u52A0\u6CB9\U0001F4AA','\u4E2D\u516D\u7B49\u5956 \u00A55\uff08\u6A21\u62DF\uff09','\u4E2D\u4E94\u7B49\u5956 \u00A510\uff08\u6A21\u62DF\uff09','\u4E2D\u56DB\u7B49\u5956 \u00A5200\uff08\u6A21\u62DF\uff09','\u4E09\u7B49\u5956\uff01\u00A53000\uff08\u6A21\u62DF\uff09','\u4E8C\u7B49\u5956\uff01\u00A5200000\uff08\u6A21\u62DF\uff09','\u4E00\u7B49\u5956\uff01\u00A75000000\uff08\u6A21\u62DF\uff09'];"
...
def generate_wheel_html():
    """理性购彩助手 · 右下角空位 → 幸运转盘小游戏（纯 CSS/JS，零联网、零卡死）。
...
def generate_engage_html():
    ...
    try:
        ...
        'var names=["新股民","小彩民","理性达人","娱乐高手","反诈小卫士","运势收藏家","彩票明白人","幸运星","锦鲤本鲤","双色球之王"];'
...
def generate_report(draws, models, groups, dantuo, expert_picks, data_issues, back_top4_main):
    ...
    # ---- 最小成本最大收益方案(覆盖优化 · 专攻六等奖) ----
    coverage_block = ""
    _cp = dantuo.get('coverage_plan') if isinstance(dantuo, dict) else None
    if _cp and not _cp.get('error'):
        _p6 = _cp.get('p_six', 0) * 100
        _p5 = _cp.get('p_five', 0) * 100
        _pa = _cp.get('p_any', 0) * 100
        coverage_block = f"""
<div class="section">
<div class="section-title">💰 最小成本最大收益方案（覆盖优化 · 专攻六等奖）</div>
<div class="info" style="border-color:#ffaa00;">
<p style="color:#ffdd88; font-size:14px; line-height:1.8; margin-bottom:12px;">
<strong>核心结构发现</strong>：双色球 <strong>六等奖 = 命中蓝球</strong>（0+1 / 1+1 / 2+1），而蓝球仅 1/16。
因此"用同样的钱尽量多覆盖不同蓝球"是捕获中小奖成本效率最高的结构 —— 这与"猜对哪个号"无关，纯属组合覆盖数学。
</p>
<div class="stat-grid" style="margin:12px 0;">
  <div class="stat"><div class="stat-label">推荐方案</div><div class="stat-value" style="font-size:15px;">{_cp.get('name','')}</div></div>
  <div class="stat"><div class="stat-label">成本</div><div class="stat-value">¥{_cp.get('cost',0)}</div></div>
  <div class="stat"><div class="stat-label">注数</div><div class="stat-value">{_cp.get('notes',0)}</div></div>
  <div class="stat"><div class="stat-label">P(任意奖)</div><div class="stat-value" style="color:#00dd88;">{_pa:.1f}%</div></div>
  <div class="stat"><div class="stat-label">P(五等奖或更好)</div><div class="stat-value">{_p5:.2f}%</div></div>
  <div class="stat"><div class="stat-label">对照 · 单式1注</div><div class="stat-value" style="color:#ff6666;">6.7%</div></div>
</div>
<p style="color:#ccc; font-size:13px; line-height:1.8;">
蒙特卡洛（12 万次随机开奖）估计：本方案 <strong>P(任意奖) ≈ {_pa:.1f}%</strong>，
而同样只买 1 注单式仅 <strong>6.7%</strong>（与官方公布总中奖率一致，验证模拟可靠）。
概率提升来自<strong>覆盖结构</strong>，不是预测能力 —— 每多覆盖 1 个不同蓝球，命中概率约 +1/16。
</p>
<p style="color:#ff9999; font-size:12px; line-height:1.7; margin-top:10px;">
⚠️ <strong>诚实声明</strong>：{_cp.get('honesty','')}
</p>
</div>
</div>"""
...
<p style="color:#888; font-size:12px; margin-top:8px;">AC={ac}({'✓' if 5 <= ac <= 9 else '✗'}) | 和值={s}({'✓' if 70 <= s <= 140 else '✗'}) | 跨度={span}({'✓' if 16 <= span <= 31 else '✗'}) | 奇偶={oc}({'✓' if oc in [2,3,4] else '✗'}) | 大小={sc}({'✓' if sc in [2,3,4] else '✗'}) | 质合={pc}({'✓' if pc in [1,2,3] else '✗'}) | 012路={r0}{r1}{r2}({'✓' if r0>0 and r1>0 and r2>0 else '✗'}) | 连号={cg}({'✓' if cg<=1 else '✗'}) | 重号={rn}({'✓' if rn<=2 else '✗'})</p>
        ...
        if i == 1:
            ...
            html += ('<div class="entertain-interlude"><div class="ei-title">🛡️ 娱乐间歇 · 手气摇奖机 + 理性购彩提示</div>'
                     '<div class="antiscam">'
                     '<div class="shield-title">🛡️ 理性购彩提示 · 双色球责任彩票科普助手</div>'
                     '<div class="lead">双色球开奖是纯随机，<strong style="color:#ff8888;">任何"大师指导 / 可确保的中奖结果说法 / 不实数据"都是虚假收费服务</strong>。本工具不宣称中奖，只做三件事，帮你买得明白：</div>'
                     '<div class="props">'
...
<li><strong>「福运奖」特别规定与本报告口径</strong>：福运奖是<strong>奖池联动</strong>机制——奖池 <strong>≥15 亿元</strong>时自动启动，中 <strong>3 个红球</strong>（蓝球未中）即中 <strong>5 元</strong>（中奖概率由约 6.71% 提升至 8.26%）；奖池<strong>跌破 3 亿元</strong>时自动停止。该规定于第 2026075 期后奖池降至 2.83 亿，<strong>自第 2026076 期（2026-07-05）起暂停</strong>，当前奖池约 3.2 亿元。因此<strong>本报告按「福运奖暂停期 · 常规规则」计算，3+0 记为未中奖</strong>。若奖池重新累积至 15 亿元，3+0 将恢复计奖，届时请以官方公告为准。</li>
</ul>
