...
def load_history(filepath='ssq_history.json'):
    """加载历史数据"""
...
def weighted_frequency_model(history, window=30):
    ...
    front_scores = {num: front_features[num]['score'] for num in range(1, 34)}
    back_scores = {num: back_features[num]['score'] for num in range(1, 17)}
...
def simplified_random_forest(history, n_trees=50, window=50, seed=42):
    ...
    front_votes = {num: 0 for num in range(1, 34)}
    back_votes = {num: 0 for num in range(1, 17)}
    ...
    # 归一化为概率
    front_scores = {num: front_votes[num] / n_trees for num in range(1, 34)}
    back_scores = {num: back_votes[num] / n_trees for num in range(1, 17)}
    
    return front_scores, back_scores


# === 模型3: 遗传算法优化组合 (已弃用 V1.0.3) ===
# 说明: 旧 genetic_optimizer 收敛到 argmax, 在 2900+ 期频率下每期输出相同(跨期冻结),
# 与 26085/26086 主模型冻结是同类BUG。V1.0.3 起 ML 改为 generate_ml_prediction 内
# 的"按分布采样" (见 _weighted_sample_k), 该函数在 V1.0.3 移除。


# === 主函数：生成ML预测 ===
def generate_ml_prediction(history, valid_combos, target_period=None):
    """生成机器学习预测 (V1.0.3 起: 按分布采样而非argmax/收敛, 解冻跨期冻结)。
    ...
    def sample_front(dist):
        for _ in range(800):
            combo = _weighted_sample_k(dist, 6, rng, list(range(1, 34)))
    ...
    def sample_back(dist):
        for _ in range(400):
            combo = _weighted_sample_k(dist, 4, rng, list(range(1, 17)))
            if combo:
                return list(combo)
        return None

    def argmax_front(dist):
        return sorted(range(1, 34), key=lambda x: dist.get(x, 0), reverse=True)[:6]

    def argmax_back(dist):
        return sorted(range(1, 17), key=lambda x: dist.get(x, 0), reverse=True)[:4]
    ...
    blend_f = {n: 0.5 * (wf_front.get(n, 0) + rf_front.get(n, 0)) for n in range(1, 34)}
    blend_b = {n: 0.5 * (wf_back.get(n, 0) + rf_back.get(n, 0)) for n in range(1, 17)}
    ...
    result = {
        ...
        'genetic_optimal': {
            ...
            'strategy': 'ML集成(加权频率+随机森林)混合分布采样 (V1.0.3解冻)'
        }
    }
