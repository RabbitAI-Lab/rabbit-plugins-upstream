...
def render_expert_views_html(target_period):
    ...
    两层结构（诚实区分数据来源与新鲜度）：
      ① 本期实时检索专家推荐：来自 ssq_expert_picks.json（由 WebSearch 实时检索公开来源得到，
         标注期号/来源/抓取时间戳/真实来源URL，verified 标记是否经原文二次核对一致）。
    ...
    try:
        ...
        if _os.path.exists(_p):
            try:
                ...
                for e in _experts:
                    fr = e.get('front') or e.get('red') or []
                    bk = e.get('back') or e.get('blue') or []
                    ...
                    _vtag = ('<span style="color:#2ecc71;font-size:11px;">✓ 已核实</span>' if e.get('verified')
                             else '<span style="color:#ff8866;font-size:11px;">⚠ 未原文核对</span>')
                    _sa = e.get('scraped_at', '') or '—'
                    picks_rows += f"""<tr>
...
<td style="font-size:12px;color:#aab4ff;line-height:1.6;">来源：<a href="{e.get('source_url','')}" style="color:#66ccff;" target="_blank">{e.get('source','')}</a><br><span style="color:#ffaa66;">{e.get('note','')}</span><br>{_vtag} · 抓取 {_sa}</td>
...
<b style="color:#ff8866;">① 本期实时检索专家推荐</b>由 WebSearch 实时检索公开来源得到，每条带来源链接+抓取时间戳，<b>经原文二次核对者标「✓已核实」</b>，动态页无法核对者标「⚠未原文核对」且 <b>不参与战绩自算</b>；<b>② 常驻历史观点库</b>为系统内置静态名录，<b>非本期实时、不自动刷新</b>。
</p>
</div>
...
def render_expert_baseline_html():
    """专家战绩 vs 随机基线（诚实对照）。

    仅用 ssq_expert_accuracy.json 中「系统自算」的真实开奖核对数据（不采信平台自报命中率）。
    若文件为空/无真实记录，则明确显示「暂无真实战绩，待开奖后自算」，绝不编造。
    """
    try:
        import json as _json
        import os as _os
        import statistics
        _p = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), 'ssq_expert_accuracy.json')
        if not _os.path.exists(_p):
            return _baseline_empty()
        d = _json.load(open(_p, encoding='utf-8'))
        recs = d.get('records', [])
        if not recs:
            return _baseline_empty()
        # 聚合: 系统自算的每专家·期 前/后区命中
        all_fh = []
        all_bh = []
        for r in recs:
            for name, s in r.get('experts', {}).items():
                all_fh.append(s.get('front_hits', 0))
                all_bh.append(s.get('back_hits', 0))
        n = len(all_fh)
        if n == 0:
            return _baseline_empty()
        mean_fh = statistics.mean(all_fh)
        mean_bh = statistics.mean(all_bh)
        # 随机基线期望(理论): 双色球 6/33 红 + 1/16 蓝
        base_fh = 6 * (6 / 33)
        base_bh = 1 / 16
        import math
        sd_fh = statistics.pstdev(all_fh) if n > 1 else 0.0
        # 单专家单期前区命中理论SD: sqrt(6*(6/33)*(27/33))≈1.047
        se = sd_fh / math.sqrt(n) if n > 1 else 0.0
        diff = mean_fh - base_fh
        t = diff / se if se > 0 else 0.0
        sig = '达到统计显著 (p<0.05)' if abs(t) > 1.96 else '未达统计显著 (no_edge)'
        rows = ''.join(
            f"<tr><td>专家单期前区均命中</td><td>{mean_fh:.3f}</td><td>{base_fh:.3f}</td>"
            f"<td style='color:#ff8866;'>{diff:+.3f}（t≈{t:.2f}，{sig}）</td></tr>"
            f"<tr><td>专家单期后区(蓝)均命中</td><td>{mean_bh:.3f}</td><td>{base_bh:.3f}</td>"
            f"<td style='color:#ff8866;'>{mean_bh-base_bh:+.3f}（蓝1个，理论浮动极小）</td></tr>"
        )
        return f"""
<div class="section">
<div class="section-title">📊 专家战绩 vs 随机基线（诚实对照 · 系统自算）</div>
<div class="info" style="border-color:#ff8866; background:#1a1320;">
<p style="color:#ffb38a; font-size:12px; line-height:1.7; margin:6px 0;">
<b>诚实结论：</b>下表为系统用真实开奖逐期核对专家 6+1 推荐后<b>自算</b>的命中（{n} 个专家·期样本），<b>不采信平台自报命中率</b>。
双色球任一组合中奖概率恒为 1/17,721,088，专家推荐数学上<b>无预测优势</b>——差异来自随机波动，<b>{sig}</b>。本栏仅供观察，<b>绝不可据其认为某专家更可能中奖</b>。
</p>
</div>
<table>
<tr><th>指标</th><th>专家(系统自算)</th><th>随机基线(理论)</th><th>判定</th></tr>
{rows}
</table>
<p style="color:#888; font-size:11px; margin-top:8px;">追踪样本：{n} 个专家·期（来自 ssq_expert_accuracy.json，开奖后独立核对，非平台自报）。</p>
</div>
"""
    except Exception as ex:
        return f'<div class="section"><div class="section-title">专家战绩 vs 随机基线</div>' \
               f'<div class="info" style="border-color:#ff5555;">⚠ 基线对照渲染异常: {ex}</div></div>'
...
<div class="info" style="border-color:#ff8866; background:#1a1320;">
<p style="color:#ffb38a; font-size:12px; line-height:1.7; margin:6px 0;">
<b>暂无真实战绩记录。</b>专家推荐尚未经系统用真实开奖逐期核对（ssq_expert_accuracy.json 为空，不采信平台自报命中率）。
待对应期开奖后，系统将自动比对专家 6+1 推荐并计算命中，届时在此展示诚实的「专家 vs 随机基线」对照。
<b>在此之前，专家推荐仅作描述性/娱乐参考，绝不构成选号建议（no_edge）。</b>
</p>
</div>
</div>
"""
