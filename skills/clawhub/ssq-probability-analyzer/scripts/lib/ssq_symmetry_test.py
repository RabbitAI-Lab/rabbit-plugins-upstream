...
用户观察: 双色球鸟瞰图(期号×号码的栅格图)上存在规则、对称图形;
...
本文件已加 main() 守卫, 可被 ssq_healthcheck_all.py 作为常驻"反诈骗诚实闸门"无副作用调用。
"""
import json
import random
...
def main():
    ...
    for k in range(W+1, N):
        ...
        p = {n: cnt.get(n, 0)/tot for n in range(1, 34)}
        actual = H[k]['front']
        G_actual.append(sum(1 - p[n] for n in actual) / 5)
        rs = rng.sample(range(1, 34), 6)
    ...
    for k in range(W+1, N):
        ...
        hot = [n for n in range(1, 34) if cnt.get(n, 0) >= q]
        nxt = set(H[k]['front'])
        for n in hot:
            pairs += 1
            if (36 - n) in nxt:
    ...
    print("="*64)
    # 生成与真实同规模、同结构的纯随机数据(每期从1-33无放回抽5)
    syn = []
    rng2 = random.Random(999)
    for _ in range(N):
        syn.append({'front': rng2.sample(range(1, 34), 6)})
    # 用同样指标在合成数据上跑
    G_syn, G_syn_rand = [], []
    for k in range(W+1, N):
        window = syn[k-W:k]
    ...
    def col_var(data):
        ...
        for d in data:
            c.update(d['front'])
        dens = [c.get(n, 0) for n in range(1, 34)]
        m = sum(dens)/len(dens)
