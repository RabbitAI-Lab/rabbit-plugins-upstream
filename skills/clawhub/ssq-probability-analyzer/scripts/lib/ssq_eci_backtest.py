# -*- coding: utf-8 -*-
"""
双色球 ECI 逆向策略 —— 可回测版（热度代理）
============================================
问题背景:
  V1 之前 ECI 逆向标记为"无法回测(无历史专家推荐数据)"，因为系统只有
  当前期的一批专家推荐 (ssq_expert_picks.json)，没有历史每一期的推荐可回放。
...
说明 (诚实标注):
  ...
  - 真实 ECI 回测需要 ssq_expert_history.json 积累历史专家推荐 (见 ssq_smart.py)。
...
# 复用 ssq_auto.py 的 AC / 过滤器定义，确保与预测管线完全一致 (才能得到 38,537)
try:
    import ssq_auto as _auto
    passes_static = _auto.passes_filters
    PRIMES = _auto.PRIMES
    print("  [复用] ssq_auto.py 过滤器定义 (AC值算法一致)")
...
def load_expert_history(path='ssq_expert_history.json'):
    """加载历史专家推荐存档 (由 ssq_smart.py 每期积累)。
...
def build_eci_reverse_groups(front_pop, back_pop, valid_combos):
    """用热度代理构造 ECI 逆向红球组 (仿 ssq_auto.py 第4组逻辑):
...
# ============================================================
# 回测框架 (与 ssq_enhanced_backtest.py 一致)
# ============================================================
...
def main():
    ...
    # 优先尝试真实专家历史 (由 ssq_smart.py 每期积累); 否则回退玩家热度代理
    expert_agg = load_expert_history()
    if expert_agg is not None:
        print(f"  [数据源] 真实专家历史存档 (ssq_expert_history.json, 红球{sum(expert_agg['front'].values())}次/蓝球{sum(expert_agg['back'].values())}次推荐)")
    else:
        print(f"  [数据源] 玩家热度代理 (无真实专家历史, 生日号偏好+近期热号)")

    # 构造有效组合 (静态 8 项过滤器, 与 ssq_auto.py 完全一致, 得 38,537)
    ...
    print(f"  红球热度最高 TOP5: {sorted(range(1,36), key=lambda n:-front_pop[n])[:5]}")
    print(f"  蓝球热度最高 TOP4: {sorted(range(1,13), key=lambda n:-back_pop[n])[:4]}")
    print(f"  蓝球 ECI 逆向 TOP4 (最冷): {back_eci_top4(back_pop)}")
    ...
    print(f"\n  {'策略':<14}{'命中均值':<12}{'命中标准差':<12}{'红球>=4频次':<14}")
    print(f"  {'-'*52}")
    ...
    out = {
        ...
        'note': 'ECI逆向用历史开奖构造的玩家热度代理(生日号偏好+近期热号)回测, 无历史真实专家推荐; 真实回测需ssq_expert_history.json积累'
    }
    with open('ssq_eci_backtest_result.json', 'w', encoding='utf-8') as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print(f"\n  结果已保存: ssq_eci_backtest_result.json")
