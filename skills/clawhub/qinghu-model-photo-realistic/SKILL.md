---
name: qinghu-model-photo-realistic
description: 青虎AI 模特图去 AI 感超写实：上传模特图一键去除 AI 感，提亮肤色、修复细节、还原真实皮肤质感并做高清超分，专为电商模特图优化设计。当用户说模特图太假、有 AI 感、皮肤像塑料、要更真实的质感、要洗图、要提亮肤色时必须触发。关键词：青虎AI、模特图、去AI感、超写实、洗图、皮肤质感、提亮肤色、修复细节、高清超分、电商图。
user-invocable: true
...
AI 生成的模特图那种「塑料皮肤 + 过度磨皮」的感觉，一键洗掉：**去 AI 感、提亮肤色、修复细节、超清放大**。

高定版洗图，专门针对电商模特图调过。固定计费（快照 2 积分/次），出图快。

## 何时触发

- 「这张模特图太假了」「有 AI 感，帮我处理下」
- 「皮肤像塑料 / 磨皮过度」
- 「洗一下这张图」「提亮肤色」
...
```bash
# 字段表
qhkit workflow options '{"workflow":"wf_037"}'

# 报价（固定计费）
qhkit workflow estimate '{"workflow":"wf_037","fields":{"替换图片":"./素材/模特图.jpg"}}'

# 提交
qhkit workflow generate '{"workflow":"wf_037","fields":{"替换图片":"./素材/模特图.jpg"}}'
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| 替换图片 | 否（实际必给） | 待处理的模特图，1 张 |

线上标为选填，但不给图跑不出结果——**提交前确认图给了**。

适用范围：**模特图 / 人像**。商品图、场景图请用「超清修复强化细节质感」或「图片高清写实去AI感」，
用错应用会把优化重点放在皮肤上，商品细节反而不理想。

多张图要逐张提交（一次一张），批量场景提醒用户积分按张累计。
...
- **专为模特图/人像设计**；商品、景物类图片用「超清修复强化细节质感」或「图片高清写实去AI感」。
- 一次处理一张，批量要逐张提交。
- 不做换装、换脸、换背景——那些走 LinkPix 系列或「模特换装高一致性还原」。

## 相关技能

同系列的其他「青虎AI」技能（按需转交，不要在本技能里硬做）：

- **图片高清写实去AI感**：非人像图片的去 AI 感
- **超清修复强化细节质感**：只要放大补细节、不改质感时
- **模特换装高一致性还原**：给模特换衣服
