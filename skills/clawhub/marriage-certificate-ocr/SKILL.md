---
name: marriage_certificate_ocr
description: 自动提取持证人、登记日期、结婚证字号及双方姓名、出生日期、身份证件号等信息。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
本技能封装了结婚证识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取结婚证里核心信息。

## 功能特性

- **结婚证识别**：自动提取持证人、登记日期、结婚证字号及双方姓名、出生日期、身份证件号等信息。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• MARRIAGE_CERTIFICATE（结婚证识别） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/marriage_certificate_ocr/scripts/main.py MARRIAGE_CERTIFICATE /path/to/test.jpg
```
...
- “帮我识别这张图像中的结婚证信息，图片在 /Users/name/Downloads/test.jpg”
