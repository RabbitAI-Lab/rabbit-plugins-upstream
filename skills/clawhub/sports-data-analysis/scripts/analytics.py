...
def _build_match_body(data, uid=""):
    ...
    analysis = data.get("analysis", data.get("analysis_logic", ""))
    confidence = data.get("confidence", "")  # 已弃用：本工具不做赛果判断 / 信心评级
    risk = data.get("risk", "体育比赛存在大量不可建模的偶然因素，任何分析都无法消除不确定性；"
                          "本报告只做信息整理，请自行研判、理性观赛。")
    factors = data.get("factors") or []  # 赛前情况与细节清单（纯事实/维度，供彩民自行分析）
    ...
    info_part = (
        ...
        + _sec("六、分析逻辑（本场对比了哪些维度）",
               "<div class='analysis'>%s</div>" % _esc(analysis) if analysis else "<div class='muted'>未提供分析逻辑说明</div>")
        + _sec("七、赛前情况与细节清单（供你自行分析）",
               _factors_html(factors) if factors else
               "<div class='muted'>未提供赛前情况清单；可在生成报告时由智能体联网补充"
               "（近期状态 / 交锋 / 伤停 / 主场 / 赛程密度 / 天气 / 战术对位 / 关键球员 / 战意 等维度）。</div>")
        + _sec("八、风险与理性提醒",
               "<div class='analysis'>%s</div>"
               "<div class='oh-note'>⚠️ 本报告仅做公开信息的整理、对比与可视化，<b>不做赛果判断、不给出任何方向性结论</b>；"
               "清单中的「情况与细节」是供你自行研判的素材，最终如何看由你结合基本面决定。</div>" % _esc(risk))
    )
    players_part = ""
    if spotlight_html:
...
def _factors_html(factors):
    """赛前情况与细节清单：把可分析维度结构化呈现（纯事实/数据，无方向性结论）。"""
    items = ""
    for fct in factors[:12]:
        ...
        if not (dim or det):
            continue
        items += ("<li><span class='fct-dim'>%s</span><span class='fct-det'>%s</span></li>"
                  % (_esc(dim), _esc(det)))
    if not items:
        return "<div class='muted'>未提供有效情况条目</div>"
    return ("<ul class='factors'>%s</ul>"
            "<div class='note'>⚠️ 以上为公开信息整理的「可分析维度」，仅呈现事实与数据，"
            "不构成任何赛果判断；如何研判由你自行决定。</div>") % items
...
REPORT_CSS = "<style>" + """
 ...
 .factors{list-style:none;margin:6px 0;padding:0}
 .factors li{display:flex;gap:12px;align-items:flex-start;padding:9px 11px;border:1px solid #e8eef3;border-radius:9px;margin:7px 0;background:#fbfdff;font-size:13.5px;line-height:1.65}
 .fct-dim{flex:0 0 116px;font-weight:800;color:#0b3d2e;background:#eafaf1;border:1px solid #b7e4c7;border-radius:7px;padding:2px 8px;text-align:center;font-size:12.5px;white-space:nowrap}
 .fct-det{flex:1;color:#333}
...
def _build_md(data, rows, combos_raw, team_txt, tz_html):
    ...
    experts = data.get("experts", [])
    analysis = data.get("analysis", data.get("analysis_logic", ""))
    confidence = data.get("confidence", "")  # 已弃用
    risk = data.get("risk", "体育比赛存在大量不可建模的偶然因素，任何分析都无法消除不确定性；本报告只做信息整理，请自行研判、理性观赛。")
    ...
    md += ["", "## 分析逻辑（本场对比维度）", analysis or "（未提供）", "",
           "## 赛前情况与细节清单（供分析）"]
    if factors:
        for fct in factors[:12]:
            if isinstance(fct, dict):
                md.append("- **%s**：%s" % (fct.get("dim", ""), fct.get("detail", "")))
            else:
                md.append("- %s" % fct)
    else:
        md.append("- （未提供；可在生成报告时由智能体联网补充近期状态/交锋/伤停/主场/赛程/天气/战术对位等维度）")
    md += ["", "## 风险与理性提醒", risk, "",
           "---", "_体育赛事数据可视化 Skill · 仅合法赛事 · 理性观赛 · 不做赛果判断_"]
    return "\n".join(md)
...
def _resolve_match_json(query):
    """自然语言 'A vs B' 解析：从 live_today.json / daily_sample.json 抽出该场并写出单场 match JSON。

    返回临时文件路径；找不到返回 None。仅做本地赛事数据匹配，不做任何联网。
    """
    import re
    q = query.replace(" VS ", " vs ").replace("VS", "vs")
    names = set()
    for part in re.split(r"\s+vs\s+", q, flags=re.I):
    ...
    if not names:
        return None
    for src in (LIVE_TODAY, DEMO_DAILY):
        ...
        for m in d.get("matches", []):
            ...
            hay = (mtext + " " + " ".join(teams)).lower()
            if all(nm.lower() in hay for nm in names if nm):
                tmp = os.path.join(_ASSETS, "_single_%s.json" % _stable(mtext))
...
def _cmd_report(a):
    if a.match:
        path = _resolve_match_json(a.match)
        if not path:
            print("❌ 未在本地赛事数据中找到「%s」对应的对阵；请先运行 live.py 补齐当日数据，"
                  "或改用 --input 指定单场 match JSON。" % a.match)
            return
        a.input = path
...
def main():
    ...
    pr = sub.add_parser("report", help="生成完整分析报告（零输入即出内置示例；或用 --match 'A vs B' 抽取本地赛事）")
    pr.add_argument("--input", default=DEMO_REPORT, help="单场 match JSON；缺省使用内置示例")
    pr.add_argument("--match", default="", help="自然语言对阵，如 '曼城 vs 阿森纳'；从本地赛事数据抽取该场生成报告")
