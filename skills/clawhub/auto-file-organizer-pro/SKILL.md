...
license: Proprietary
edition: pro
description: "自动文件整理器专业版面向高效文件治理场景，在免费版基础上扩展全功能自动化能力。解决文件整理的"规模与智能"痛点：大量重复文件占用空间需要清理、按扩展名分类不够准确需要内容感知、多个文件夹需要批量定时整理、团队需要共享分类规则、文件变更需要实时监控自动整理。Use。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。"
...
> **功能说明**: 本技能涵盖 auto, file, organizer 等核心能力。
...
# ...
python3 （请参考skill目录中的脚本文件） dedup scan $HOME/Documents/ $HOME/Downloads/
# ...
python3 （请参考skill目录中的脚本文件） rules set rules/smart-rules.yaml
# ...
python3 （请参考skill目录中的脚本文件） schedule add --cron "0 9 * * *" --paths $HOME/Downloads $HOME/Desktop
# ...
python3 （请参考skill目录中的脚本文件） watch $HOME/Downloads/ --auto-organize
```
...
# ...
python3 （请参考skill目录中的脚本文件） dedup report $HOME/Documents/
```

**处理**: 解析重复文件清理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回重复文件清理的响应数据,包含状态码、结果和日志.
### 2. 智能内容分类

**处理**: 解析智能内容分类的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回智能内容分类的响应数据,包含状态码、结果和日志.
### 3. 批量定时整理
```bash
...
python3 （请参考skill目录中的脚本文件） batch organize \
  $HOME/Downloads $HOME/Desktop $HOME/Documents/临时 \
...
**处理**: 解析整理报告导出的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回整理报告导出的响应数据,包含状态码、结果和日志.
### 6. 文件变更实时监控
```bash
python3 （请参考skill目录中的脚本文件） watch $HOME/Downloads/ --auto-organize
# ...
python3 （请参考skill目录中的脚本文件） watch $HOME/Downloads $HOME/Desktop --auto-organize
# ...
python3 （请参考skill目录中的脚本文件） watch $HOME/Downloads/ --auto-organize --dedup
```
...
**处理**: 解析团队共享规则配置的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回团队共享规则配置的响应数据,包含状态码、结果和日志.
...
**解决方案**：
```bash
python3 （请参考skill目录中的脚本文件） dedup scan $HOME/Documents/ --min-size 1KB
# ...
python3 （请参考skill目录中的脚本文件） dedup plan $HOME/Documents/ --keep latest
# ...
python3 （请参考skill目录中的脚本文件） dedup execute $HOME/Documents/ --keep latest
...
```bash
python3 （请参考skill目录中的脚本文件） smart-classify $HOME/Documents/ --rules smart-rules.yaml
```
...
```json
{
  ...
  "data": {
    ...
    "metadata": {
      "version": "1.0",
      "processor": "auto file organizer pro"
    }
  },
...
- **自动化执行**: 全功能文件自动整理工具，支持重复清理、智能内容分类、批量定时整理与多目录批量处理.。自动文件整理器专业版面向高效文件治理
