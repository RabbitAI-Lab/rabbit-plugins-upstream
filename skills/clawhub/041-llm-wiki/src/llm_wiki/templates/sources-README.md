...
llm-wiki 协议要求 Agent 在下载文件后、摄入前，**主动验证文件内容是否与你的请求一致**。

Agent 会检查：
1. **文件可读性**：文件是否损坏或无法解析
2. **错误页面检测**：内容是否包含 `404`、`Access Denied`、`Subscribe` 等关键词
3. **格式一致性**：文件头是否与扩展名匹配（如 PDF 应以 `%PDF` 开头）
4. **元数据比对**：提取标题、作者、DOI/arXiv ID，与你提供的描述进行比对

**如果验证失败**，Agent 会：
- **停止摄入**，不创建任何 wiki 页面
- 向你报告提取到的实际元数据 vs 你的期望
- 等待你的确认或提供正确的 URL

**如果你怀疑 Agent 可能伪造了来源文件**，还可以人工检查：
