...
# Integrity gate: the expected digest is pinned in EXPECTED_SHA256 next to this
# script. Extracting an unverified archive was flagged by the ClawHub security
# audit as an "unauthenticated self-extracting code path" — we now fail closed
# rather than executing whatever the payload happens to contain.
expected_file = payload_path.parent / "EXPECTED_SHA256"
if expected_file.exists():
    expected = expected_file.read_text().split()[0].strip()
    if digest != expected:
        raise SystemExit(
            f"ERROR: payload integrity check FAILED\n"
            f"  expected {expected}\n  actual   {digest}\n"
            f"Refusing to extract. Re-install the skill from ClawHub.")
    print("    integrity OK (matches EXPECTED_SHA256)")
else:
    print("    WARNING: EXPECTED_SHA256 not found — integrity NOT verified.")
# Reject path traversal / absolute members before writing anything to disk.
zf = zipfile.ZipFile(io.BytesIO(data))
for member in zf.namelist():
    if member.startswith(("/", "\\")) or ".." in Path(member).parts:
        raise SystemExit(f"ERROR: unsafe path in payload: {member}")
zf.extractall('.')
