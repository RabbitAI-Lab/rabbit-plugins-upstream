...
> 📁 **路径限制**：`cookies` / `storage` 的 `export`/`import` 默认被限制在 `~/.pw-browser/` 目录内——这是刻意的安全设计，防止凭证被静默散落到 `/tmp` 或加载来自任意路径的攻击者构造文件。自定义路径也必须位于 `~/.pw-browser/` 之下；若要写到/读自该目录之外，必须显式传 `--unsafe`（**不推荐**，仅当你清楚自己在做什么时）。命令返回里都带 `warning` 字段提示该文件含实时会话凭证。
...
> - 用完即删（`rm`）；若需暂存，请放到受限目录（如 `~/.pw-browser/`，它本身即受限）并对文件设 `chmod 600`。
