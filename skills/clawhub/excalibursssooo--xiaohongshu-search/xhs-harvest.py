...
def run_fetch(args, timeout=60):
    """subprocess 调 xhs-fetch.py, 返回 (returncode, stdout, stderr) 三元组

    harvest 所有错误判断都应该用 parse_struct_error(stderr) 拿 error_code,
...
def fetch_one_note(note_id, xsec_token, out_path, comments=10, sleep_after=4, user_id=None, xsec_source=None):
    """抓一条 note; 成功落盘返 (True, msg), 失败返 (False, msg)
    ...
    错误处理:v1.4.0 走 parse_struct_error 拿 error_code,不再字符串 grep
    (fetch 改字面量 harvest 也不会漏判)
    captcha / IP 风控:**不重试**,直接报错。连发会加锁,手动重跑更安全。
    """
    if out_path.exists():
        # 已落盘 → 跳过 (增量)
        return True, "已存在 (跳过)"
    ...
    rc, out, err_out = run_fetch(note_args)
    time.sleep(sleep_after)

    # 成功判断 (优先看文件,辅以 stdout)
    if out_path.exists():
        return True, "ok"

    # 错误判断:优先用结构化 error_code,回退到字符串 grep (防御性)
    code, msg = parse_struct_error(err_out)
    if code is None:
        # fetch 漏了结构化输出,fallback 到字符串匹配
        combined = out + err_out
        if '300012' in combined:
            code, msg = '300012', 'IP 风控'
        elif '300031' in combined:
            code, msg = '300031', '笔记不可见'
        elif 'Security Verification' in combined or 'verifyType=124' in combined:
            code, msg = 'captcha', '滑块验证'
        elif '300017' in combined:
            code, msg = '300017', 'url_invalid'
        else:
            return False, f"未知失败 (rc={rc}): {(out + err_out)[:200]}"

    # 把 error_code 翻译成 harvest 友好的错误描述
    if code == '300012':
        return False, "IP 风控 (300012) — 换网络或等 5-30 分钟"
    if code == '300031':
        return False, "笔记不可见 (300031) — xsec_token 失效,需要重拿"
    if code == '300017':
        return False, "url_invalid (300017) — xsec_token 跟 xsec_source 不匹配,改用 via-user-profile"
    if code == 'captcha':
        return False, "captcha 锁中 — 等 30+ 分钟或换 IP 后重跑 (不自动重试避免加锁)"
    if code == 'blocked':
        return False, f"页面被风控: {msg}"
    if code == 'parse_fail':
        return False, f"fetch 内部解析失败: {msg}"
...
def cmd_hot(args):
    ...
    for kw in keywords:
        ...
        rc, out, err_out = run_fetch(['search', kw, '--sort', args.sort,
                                      '--limit', str(args.per_keyword), '--out', str(search_json)])
        if rc != 0 or not search_json.exists():
            code, msg = parse_struct_error(err_out)
            err_struct(code or 'unknown',
                       f"search '{kw}' 失败",
                       hint=f'msg: {msg}\nstderr: {err_out[:200]}')
            continue
...
def auto_resolve_tokens(candidates_needing_token, sleep=4, sort='time', token_cache=None):
    ...
    for i, c in enumerate(candidates_needing_token, 1):
        ...
        for kw in kw_candidates:
            ...
            rc, out, err_out = run_fetch(['search', kw, '--sort', sort, '--limit', '10', '--out', str(tmp)])
            out_combined = out + err_out
            # 优先用结构化 error_code, fallback 字符串 grep
            code, _ = parse_struct_error(err_out)
            if code in ('captcha', '300012'):
                print(f"  [{i}/{len(candidates_needing_token)}] {title[:30]:30s} ❌ search {code or 'captcha'},停止 auto-token (避免连发加锁)")
                tmp.unlink(missing_ok=True)
                captcha_failed = True
                break
            if code is None and ('Security Verification' in out_combined or 'verifyType=124' in out_combined):
                print(f"  [{i}/{len(candidates_needing_token)}] {title[:30]:30s} ❌ search captcha,停止 auto-token (避免连发加锁)")
                tmp.unlink(missing_ok=True)
                captcha_failed = True
                break
            if rc != 0:
                print(f"  [{i}/{len(candidates_needing_token)}] {title[:30]:30s}  search 失败: {err_out[:60]}")
                tmp.unlink(missing_ok=True)
                time.sleep(sleep)
                continue
            try:
...
def cmd_user(args):
    ...
    # --from-name 路径: 优先调 user-resolve (鲁棒,逐个验证),失败时 fallback user-search (快)
    user_id = args.user_id
    if args.from_name:
        print(f"🔍 从显示名解析 user_id: {args.from_name}")
        # 优先 user-resolve (Plan B: 逐个验证, 鲁棒)
        rc, out, err_out = run_fetch(['user-resolve', args.from_name])
        if rc == 0:
            m = re.search(r'user_id = ([a-f0-9]+)', out)
            if m:
                user_id = m.group(1)
                print(f"  ✅ user-resolve → {user_id}\n")
            else:
                print(f"  ⚠️  user-resolve 返回 0 但未找到 user_id 行,fallback user-search")
                rc = 1
        if rc != 0:
            # 拿结构化错误码,告诉用户为什么 user-resolve 失败
            rcode, rmsg = parse_struct_error(err_out)
            if rcode == '300012':
                print(f"  ⚠️  user-resolve 失败 (search 桶 IP 风控 300012), fallback user-search 也不会成功")
                err_struct('300012', 'user-resolve IP 风控,search 桶被锁',
                           hint=('等 5-10 分钟,或换网络 (4G 热点 / 代理)\n'
                                 '如果急着要,手动 fallback: 浏览器登录 xhs → 搜该用户 → 进主页 → URL 末尾 32 位 hex = user_id'))
                return 1
            print(f"  ⚠️  user-resolve 失败 (rc={rc}, code={rcode or 'unknown'}), fallback user-search ...")
            rc, out, err_out = run_fetch(['user-search', args.from_name])
            if rc != 0:
                code, msg = parse_struct_error(err_out)
                err_struct(code or 'unknown',
                           f"user-search 也失败: {msg or out[:200]}",
                           hint='手动 fallback: 浏览器登录 xhs → 搜该用户 → 进主页 → URL 末尾 32 位 hex = user_id')
                return 1
            m = re.search(r'user_id = ([a-f0-9]+)', out)
            if not m:
                err_struct('parse_fail', f"未从 user-search 输出中解析出 user_id: {out[:200]}")
                return 1
            user_id = m.group(1)
            print(f"  ✅ user-search → {user_id}\n")
        # (P0 修复) 记录 from_name 用于 Phase 1 后的 author 验证
        expected_name = args.from_name
    else:
        expected_name = None
    ...
    rc, out, err_out = run_fetch(['user', user_id, '--notes', str(args.notes), '--out', str(user_json)])
    if not user_json.exists():
        code, msg = parse_struct_error(err_out)
        if code == 'captcha':
            err_struct('captcha',
                       'user/profile 路径 captcha 锁中 (verifyType=124)',
                       hint='等 30+ 分钟或换 IP 后重跑')
        elif code == '300012':
            err_struct('300012', 'IP 风控', hint='换网络或等 30+ 分钟')
        elif code:
            err_struct(code, msg or '拿用户主页失败',
                       hint=f'fetch stderr: {err_out[:200]}')
        else:
            err_struct('unknown', f'拿用户主页失败 (rc={rc})',
                       hint=f'stderr: {err_out[:200]}')
        return 1
    ...
    # (P0 修复) author 验证: --from-name 路径下,必须确认主页 name 跟 keyword 匹配
    #   不匹配 → 说明 user-search 拿错了 user_id (侧栏用户之类的),直接报错退出
    if expected_name:
        actual_name = user_data.get('name', '').strip()
        if not actual_name:
            err_struct('parse_fail',
                       f'无法拿取主页 name(可能 SPA 还没渲染完),跳过验证,后续可能抓错用户',
                       hint='可重试一次,或人工 xhs-fetch.py user {user_id} 确认')
        elif not author_matches(expected_name, actual_name):
            err_struct('author_mismatch',
                       f"author 不匹配: 期望 '{expected_name}' , 实际 '{actual_name}'",
                       hint=(f'  user_id {user_id} 不是 {expected_name!r} 本人\n'
                             f'  跑 `xhs-fetch.py user-search {expected_name!r} --list-candidates` 看完整候选'))
            return 1
        else:
            print(f"  ✅ author 验证: '{actual_name}' 匹配 '{expected_name}'")
    print()
    ...
    if args.auto_token:
        ...
        if need_idx:
            ...
            for j, i in enumerate(need_idx, 1):
                nid = ids[i]
                tmp = Path(tempfile.mkstemp(suffix='.json')[1])
                rc, out, err_out = run_fetch(['search', nid, '--sort', 'time', '--limit', '10', '--out', str(tmp)])
                code, msg = parse_struct_error(err_out)
                if rc != 0:
                    print(f"  [{j}/{len(need_idx)}] {nid[:14]} ❌ search fail [{code or 'rc='+str(rc)}]: {msg or err_out[:60]}")
                    tmp.unlink(missing_ok=True)
                    time.sleep(args.sleep)
                    continue
                try:
