...
Mistral Vibe loads Skills from custom paths, `.agents/skills/`, `.vibe/skills/`, `~/.vibe/skills/`, and `~/.agents/skills/` in documented precedence. Configuration is `.vibe/config.toml` then `~/.vibe/config.toml`; MCP uses the `mcp_servers` TOML subobject.
