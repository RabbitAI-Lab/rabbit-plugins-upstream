...
- **mcp**: user `~/.gemini/settings.json` · project `.gemini/settings.json` · root key `mcpServers` · JSON. Each server must provide at least one documented endpoint: `command` (stdio), `url` (legacy SSE), or `httpUrl` (Streamable HTTP); optional documented fields include `args`, `headers`, `env`, `cwd`, `timeout`, `trust`, `includeTools`, and `excludeTools`. Do not rewrite `url` to `httpUrl`, or vice versa, during migration without a server-owner endpoint change. The generic mapper converts only the user file and validates the target shape; project MCP remains manual because the workflow has no scope selector.
