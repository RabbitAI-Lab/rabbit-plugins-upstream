...
- **mcp/config**: global `~/.config/goose/config.yaml` · root key `extensions` · YAML, not JSON · extension entries use documented type-specific fields such as `builtin`/`platform`/`stdio`/`streamable_http`/`frontend`/`inline_python`, `cmd`, `args`, `envs`, `uri`, `headers`, `enabled`, `timeout`, and `available_tools`; legacy `sse` may appear for compatibility. Do not promote that legacy type to `streamable_http` by rewriting a migrated configuration; use a replacement endpoint supplied by the server owner.
