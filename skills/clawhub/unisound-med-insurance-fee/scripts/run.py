#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""医保与费用规则 — 独立 skill，可单独拷贝发布；不依赖其他 skill 或 _shared。"""
...
TASKS: Dict[str, Dict[str, str]] = {'insure-check': {'title': '医保理赔单据合规审核', 'hint': '审核单据是否符合医保理赔合规要求。'}, 'insure-calc': {'title': '医保费用核算与规则应用', 'hint': '按规则完成费用核算或规则应用类作答。'}}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="医保与费用规则：由调用方传入题目与 --task，直连内部医疗大模型。")
