# Metadata Standards Checker (geoskill-metadata-standards-checker)

> Parse ISO 19115 / FGDC XML metadata, validate required fields, controlled vocabularies, and consistency rules, and report a completeness score with an issue list. Parse ISO 19115 / FGDC XML metadata, validate required fields and controlled vocabularies, and report a completeness score.

---

## 1. Overview

Performs standard-compliance checks on XML metadata of geospatial data. Two mainstream standards are supported: **ISO 19115** (the `gmd:MD_Metadata` schema) and **FGDC** (the US Federal Geographic Data Committee CSDGM schema). The checker parses the XML, extracts elements in a namespace-agnostic way, and validates each item against the required-field lists and controlled vocabularies of each standard (e.g. `hierarchyLevel`, `topicCategory`). Core algorithm: namespace-stripped element extraction → automatic standard detection (ISO / FGDC) → required-field hit counting yields a 0-1 completeness score → two-level issue list (error / warning) → controlled-vocabulary compliance check. Suitable for metadata QA before data submission and for catalog ingestion validation. The built-in `--synthetic` mode generates complete and deliberately deficient ISO / FGDC sample metadata locally, so the full checking workflow can be demonstrated without real data.
...
python geoskill-metadata-standards-checker.py --bbox 116 39 117 40 --synthetic --output-dir ./out
```
...
|---|---|---|
| `metadata_report.json` | JSON | Per-file scores, error/warning issue lists, overall statistics |
