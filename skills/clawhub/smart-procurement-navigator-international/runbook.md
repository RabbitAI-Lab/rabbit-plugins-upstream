...
- Establish a government procurement regulation update tracking mechanism.
  - **Regulatory source list**: PRC: NPC (law amendments), State Council (administrative regulations), MOF (departmental rules), NDRC (normative documents), provincial finance departments / public resource trading centers (local rules). International: WTO GPA amendments, World Bank Procurement Framework updates, EU OJEU/TED threshold revisions, FIDIC contract updates, individual country procurement law changes.
  - **Tracking frequency**: First working day of each month for all sources; ad-hoc tracking within 3 days of major regulatory announcements.
  - **Update path**: Regulatory change → verify original text (official gazette/website) → update `knowledge/` regulation database → cross-check SKILL.md referenced provisions for currency → publish changelog.
  - **Responsibility assignment**: Recommend designating 1 legal/commercial staff member for regulation tracking; output a monthly "Procurement Regulatory Update Brief."
  - **Automation tools**: RSS subscriptions (MOF/NDRC official notice pages, WTO e-GPA portal, EU TED, World Bank procurement notices), WeChat official accounts (China Government Procurement News / China Bidding & Tendering), NLP keyword aggregation (tender/bid/procurement/evaluation/disqualification/challenge/complaint) for automated new-regulation aggregation.
