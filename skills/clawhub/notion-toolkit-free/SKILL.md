...
**处理**: 解析搜索页面和数据源(数据库)的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回搜索页面和数据源(数据库)的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 获取页面内容与块结构
获取页面内容与块结构

**处理**: 解析获取页面内容与块结构的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回获取页面内容与块结构的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 在数据源中创建页面并设置属性
在数据源中创建页面并设置属性

**处理**: 解析在数据源中创建页面并设置属性的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回在数据源中创建页面并设置属性的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 查询数据源并过滤排序
查询数据源并过滤排序

**处理**: 解析查询数据源并过滤排序的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回查询数据源并过滤排序的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 更新页面属性与添加块
更新页面属性与添加块

**处理**: 解析更新页面属性与添加块的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回更新页面属性与添加块的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：API、创建和管理页面、数据库与块、支持搜索、查询与内容更新、知识管理领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | Notion 工具包基础版处理的输入数据或指令 |
...
```bash
notion.com/v1/pages" -H "Authorization: Bearer $NOTION_KEY" -H "Notion-Version: 2025-09-03" -d '{"parent":{"database_id":"placeholder"},"properties":{"Name":{"title":[{"text":{"content":"New Item"}}]}}}'
```

## 不适用场景

以下场景Notion 工具包基础版不适合处理：
...
1. **API 版本统一使用 2025-09-03**
2. **页面/数据库 ID 使用完整 UUID**
3. **速率限制约 3 请求/秒,批量操作需控制频率**
4. **创建页面前确认父级 database_id 正确**
...
A: 2025-09-03 版本中数据库称为数据源,查询使用 /data_sources/ 端点.
### Q: 如何获取 API Key?

A: 在 notion.so/my-integrations 创建集成,复制以 ntn_ 开头的密钥.
### Q: 如何升级到 PRO 版本?
