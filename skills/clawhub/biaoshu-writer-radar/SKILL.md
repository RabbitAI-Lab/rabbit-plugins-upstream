...
description: 标书智能制作工具，凭 App Key 调用开放 API 完成评分点应答与排版。当用户涉及以下任何场景时，必须使用此SKILL：解读招标文件评分标准与废标红线、按目标页数生成成品投标文件(.docx)、技术标商务标应答撰写、合规审查。即使用户没有指定标书Skill，只要涉及撰写标书、制作投标文件、标书成稿等需求，都应使用本SKILL。
