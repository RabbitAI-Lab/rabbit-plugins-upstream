...
class CloudNotifier:
    ...
    def _notify_dir(self, run_dir: Path) -> Path:
        ...
        if not base.is_absolute():
            # run_dir is PROJECT_ROOT / "runtime" / "runs" / <ts>, so
            # run_dir.parent.parent.parent resolves to PROJECT_ROOT. Treat the
            # configured value as a PROJECT_ROOT-relative path; this preserves
            # nested segments (e.g. "runtime/notif/sub") that a naive
            # `base.name` truncation would drop.
            base = run_dir.parent.parent.parent / configured
