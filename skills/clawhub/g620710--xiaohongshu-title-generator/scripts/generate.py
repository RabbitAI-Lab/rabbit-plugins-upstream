...
API_BASE = os.environ.get("XHS_API_URL", "http://47.109.39.255:8765/api/v4")
USER_KEY = os.environ.get("XHS_API_USER_KEY", "")
...
def cmd_register(args):
    ...
    print()
    print(f"  ⚠️  请设置环境变量：")
    print(f"  export XHS_API_USER_KEY={result.get('user_key')}")
    print("=" * 50)
    print()
...
def cmd_info(args):
    if not USER_KEY:
        print("❌ 请先设置 XHS_API_USER_KEY 环境变量", file=sys.stderr)
        sys.exit(1)
...
def cmd_generate(args):
    if not USER_KEY:
        ...
        sys.exit(1)
    print(f"🚀 小红书标题生成中...", file=sys.stderr)
    data = {}
    data['topic'] = args.topic
    if args.style: data['style'] = args.style
    data['count'] = args.count if args.count else 10
    result = api_call("xhs-title", data)
    content = result.get("titles", "")
...
def main():
    parser = argparse.ArgumentParser(description="小红书标题生成 - SaaS版，1点/次")
    ...
    g = sub.add_parser("generate", help="小红书标题生成")
    g.add_argument("--topic", required=True, help="话题关键词")
    g.add_argument("--style", default="mixed", help="风格: mixed/curiosity/list/contrast/emotion/number")
    g.add_argument("--count", type=int, default=10, help="生成数量")
