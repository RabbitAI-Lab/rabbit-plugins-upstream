...
For Threads data lookup, ranking, analysis, search, comparison, or reporting, use KeyAPI API execution and official KeyAPI docs. Do not use platform web pages as a substitute for API results.
...
- Entity: the Threads object being searched, analyzed, compared, ranked, or monitored
- Scope: username/user ID, post URL or ID, keyword, top/recent/profile search mode, reply/repost context, and date window
- Metric or sort: recency, replies, likes, reposts, discussion context, author relevance
...
- For Threads reporting, separate user profiles, posts, reposts, replies, post details, comments, and search-mode assumptions.
...
```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --query-param example=value
```

Execute query parameters from a JSON file:

```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --query-file query.json
```

Preview a large response without saving:

```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --query-param example=value --stdout preview
```

Write a complete response to a temporary file for internal analysis:

```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --query-param example=value --output-file .tmp-keyapi-threads-response.json
```

Save or export a complete response for the user:

```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --query-param example=value --output-file response.json
```
...
```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --method POST --body-file request.json
```

Execute an image JSON endpoint from a local file:

```bash
node scripts/keyapi-api.mjs --path /v1/threads/... --method POST --query-param example=value --image-file ./image.jpg --image-field image_base64
```
...
- `references/threads-profile-rules.md`
- `references/threads-content-rules.md`
- `references/threads-search-rules.md`
