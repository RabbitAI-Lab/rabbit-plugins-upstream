---
name: amazon-review-based-copywriter
description: >
  将 Amazon 评论中的购买动因、疑问和体验语言整理为完整 Listing 优化方向与证据清单。仅用于证据驱动的页面文案规划；不用于无数据自由创作、广告投放或自动发布。
  Requires an ARI API key (ari_live_*).
slug: review-copywriter
displayName: Amazon 评论证据文案规划
version: 1.4.7
summary: 将评论语言整理为完整 Listing 优化方向
license: MIT
---
...
- 本入口只执行 `listing/full`，输出模板固定为 `ops_listing`。
