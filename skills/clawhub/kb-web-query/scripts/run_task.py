...
def main():
    parser = argparse.ArgumentParser(description="Research KB team-context-guided web query helper")
    subparsers = parser.add_subparsers(dest="command", required=True)

    prepare = subparsers.add_parser("prepare", help="Read overview team context, KB catalog cards, and reference attachments into a web-query planning context")
    ...
    fetch = subparsers.add_parser("fetch", help="Read optional OpenClaw-selected supplemental KB pages into an evidence bundle")
    ...
    apply = subparsers.add_parser("apply", help="Validate OpenClaw web answer sources and write backend result without QA persistence")
...
def recover_invalid_answer_doc(answer_path, context, exc):
    ...
    team_sources = metadata.get("teamSources") or metadata.get("teamContextSources")
    if not isinstance(team_sources, list):
        team_sources = []
    web_sources = metadata.get("webSources") or metadata.get("externalSources")
    if not isinstance(web_sources, list):
        web_sources = []
    if not sources:
        sources = web_sources or team_sources or evidence_page_sources(context)
    ...
    return {
        ...
        "teamSources": team_sources,
        "webSources": web_sources,
        "sources": sources,
        "usedSearchQueries": metadata.get("usedSearchQueries") if isinstance(metadata.get("usedSearchQueries"), list) else [],
...
def recover_answer_metadata(raw):
    ...
    for key in ["sources", "teamSources", "teamContextSources", "webSources", "externalSources", "usedAttachments", "usedSearchQueries", "errors"]:
...
def context_summary(context, context_output):
    ...
    return {
        ...
        "overviewPageCount": len(((context.get("teamContext") or {}).get("overviewPages") or [])),
