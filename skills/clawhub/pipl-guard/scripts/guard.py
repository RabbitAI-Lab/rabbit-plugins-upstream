...
from rules import common as _common
...
# ============ 检测核心 ============
def detect(text, profile_name="common"):
...
def evaluate(text, profile_name="common", action="mask", block_on="high"):
    """护栏主入口。返回结构化裁决结果。
...
def main(argv=None):
    p = argparse.ArgumentParser(description="PIPL 运行时护栏检测内核")
    src = p.add_mutually_exclusive_group()
    src.add_argument("--text", help="直接传入待检测文本")
    src.add_argument("--stdin", action="store_true", help="从标准输入读取")
    p.add_argument("--profile", default="common", help="规则包（默认 common）")
