---
name: meta-agent-browser-core
version: 1.0.0
description: |
  由 model-distillation 从教师技能 agent-browser-core 蒸馏并增强的超越型元技能，
  在教师能力之上叠加自验证、自我反思、super-agent 编排与持续自进化闭环，逐步超越教师。
agent_created: true
visibility: public
---
# meta-agent-browser-core（蒸馏超越型元技能）

> 由 `model-distillation` 从教师技能 **agent-browser-core** 蒸馏并增强生成。
> 生成时间：2026-07-23 05:11:45 ｜ 蒸馏机制：跨模型蒸馏（见 meta-evolver 北极星策略）

## 来源能力签名（教师）
- 标题层级：Agent Browser Skill (Core), Purpose, Quick orientation, Required inputs, Expected output, Operational notes, Safe mode defaults, Security notes
- 显性工作流步骤（0 步）：
  （教师未显式编号工作流）
...
## 教师 vs 学生 对比
| 维度 | 教师(agent-browser-core) | 学生(meta-agent-browser-core) |
| --- | --- | --- |
| 能力来源 | 原始 SKILL.md（4622 字符） | 蒸馏提取 + 元进化增强 |
...
## 使用
直接调用本技能完成「agent-browser-core」领域的任务；本技能在教师能力之上叠加自验证与反思，输出更可靠、可追溯。
