...
version: 1.0.0
displayName: 图像处理专业版
summary: "企业级图像处理流水线,支持批量处理、CDN 集成、响应式图片生成与无障碍优化。核心能力: 图像处理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 图像处理领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
...
本工具是 **图像处理** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析批量图像处理与自动化流水线的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量图像处理与自动化流水线的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 响应式图片生成
响应式图片生成:多尺寸 srcset 自动输出

**处理**: 解析响应式图片生成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回响应式图片生成的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### CDN 集成与图片上传管道
CDN 集成与图片上传管道

**处理**: 解析CDN 集成与图片上传管道的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回CDN 集成与图片上传管道的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 无障碍优化
无障碍优化:Alt 文本建议与对比度检查

**处理**: 解析无障碍优化的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回无障碍优化的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 图片质量评分与自动优化建议
图片质量评分与自动优化建议

**处理**: 解析图片质量评分与自动优化建议的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回图片质量评分与自动优化建议的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级图像处理流、支持批量处理、响应式图片生成与、核心能力、图像处理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 图像处理专业版处理的输入数据或指令 |
...
- **自动化执行**: 企业级图像处理流水线,支持批量处理、CDN 集成、响应式图片生成与无障碍优化。核心能力: 图像处理领域的专业化 AI 辅
