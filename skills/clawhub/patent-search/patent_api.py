...
def format_patent_detail(patent_id: str, patent: Dict[str, Any]) -> str:
...
class PatentAPI:
    ...
    def format_search_result(
        ...
        *,
        ...
        if detailed:
            output.append(
                "| 序号 | 专利号 | 标题 | 申请人 | 申请日 | IPC | 摘要 |"
            )
            output.append("| --- | --- | --- | --- | --- | --- | --- |")
        else:
            output.append("| 序号 | 专利号 | 标题 | 申请人 | 申请日 |")
            output.append("| --- | --- | --- | --- | --- |")
        ...
        for i, patent in enumerate(patents[:10], 1):
            ...
            app_date = (
                patent.get("applicationDate")
                or patent.get("documentDate")
                or "-"
            )
            if detailed:
                ipc = patent.get("ipc", "-")
                ...
                row = (
                    f"| {i} | {_md_table_cell(pid, 22)} | {_md_table_cell(title, 36)} "
                    f"| {_md_table_cell(applicant, 20)} | {_md_table_cell(app_date, 12)} "
                    f"| {_md_table_cell(ipc, 16)} | {_md_table_cell(summary, 40)} |"
                )
            else:
                row = (
                    f"| {i} | {_md_table_cell(pid, 22)} | {_md_table_cell(title, 42)} "
                    f"| {_md_table_cell(applicant, 22)} | {_md_table_cell(app_date, 12)} |"
                )
            output.append(row)
    ...
    def format_patent_detail(self, patent_id: str, patent: Dict[str, Any]) -> str:
        return format_patent_detail(patent_id, patent)
