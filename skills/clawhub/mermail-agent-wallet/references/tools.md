...
These tools appear only on Mermail MCP **OAuth** full-profile sessions for the **workspace owner**. API-key catalogs and the agent-inbox profile never include them. Legacy `wallet:read` / `wallet:transact` scope strings are compatibility-only and are not used for tool visibility. Read live schemas from MCP `tools/list`.
...
- `get_paybox_connection`: lightweight PayBox status for one mailbox. Returns `connect_handoff.console_url` when not connected or `reauth_handoff.console_url` when reauth is required. Present the console link; never send users to Claude/ChatGPT/Codex connector settings.
...
- **x402 paid service:** exploration is read-only. Before `paybox_pay_x402`, require a user-selected service/origin, resource/action, and maximum spend; validate the live 402 quote within that envelope and preview exact terms. Funding is separate and never authorizes payment. Call once and stop on pending. The authenticated browser may poll the exact request and call app-only `reopen_signing_window` once if the signing plan is missing; the model must never call a replacement payment. Keep `x_payment` out of chat and logs, and use it only to retry the exact paid resource after terminal success.
...
Buy / checkout / approval / signing-plan URLs from tools such as `paybox_get_buy_link` are redacted for the model. Prefer `get_agent_wallet` → `funding_handoff.console_url` (Mermail deep link with `fund=1`). If `funding_handoff.needs_mailbox` is true, resolve `mailboxId` via `get_agent_wallet` instead of guessing. Signing handoffs are different: use only the returned invocation-scoped URL and never construct one. See [SKILL.md](../SKILL.md).
