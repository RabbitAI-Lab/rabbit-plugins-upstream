---
slug: tri-platform-video-analysis
displayName: 三平台视频解析
version: 1.0.5
summary: 丢一条抖音/B站/小红书视频链接，自动下载、离线转写、AI分析，输出标准化胶囊卡片交互 HTML 报告。纯本地转写，零 API 费用。
license: MIT
---

# tri-platform-parse — 三平台视频解析

**支持平台**：抖音 (douyin.com)、B站 (bilibili.com)、小红书 (xiaohongshu.com)
...
- 脚本从第三方平台（抖音/B站/小红书）下载视频文件
- 首次运行从 Hugging Face 下载 Whisper `small` 模型（约 500MB）
- 抖音通过 iesdouyin API 直连（无 cookies）；B站/小红书使用 yt-dlp 下载
...
> 三平台实际生成效果（点击胶囊卡片可展开详情）：

**抖音** — 口播类视频：

![抖音报告效果](https://8eb04cd68f594e89a6515cb53567c5a1.sh5.agentos-app.net/douyin.png)

**B站** — 产品测评类视频：

![B站报告效果](https://8eb04cd68f594e89a6515cb53567c5a1.sh5.agentos-app.net/bilibili.png)

**小红书** — 职场观点类视频：

![小红书报告效果](https://8eb04cd68f594e89a6515cb53567c5a1.sh5.agentos-app.net/xiaohongshu.png)
...
- **处理前确认** — 检测到视频信息后，向用户展示平台、标题、时长，等待确认后再继续
- **单次一条链接** — 每次调用处理一条视频链接
...
- **抖音**：优先使用 iesdouyin API 直连（无需 cookies/yt-dlp），降级到 yt-dlp 时可能需要 cookies
- **小红书**：链接须含 `xsec_token` 参数（「分享→复制链接」获取）
- **转写速度**：small 模型约 1:1 实时比（CPU），large-v3 约 1:3
