---
name: qinghu-douyin-social
description: 青虎AI 抖音社媒运营：串起抖音热搜榜、话题详情与话题下视频、关键词视频搜索、达人主页数据与粉丝画像，完成「热点选题 → 脚本卖点挖掘 → 粉丝人群画像匹配」的内容引流与种草闭环。当用户要做抖音内容运营、找选题、看热搜和话题、拆解爆款视频卖点、看达人粉丝画像、做人群匹配、规划内容日历时必须触发。关键词：青虎AI、抖音、社媒运营、热搜、话题、选题、爆款视频、粉丝画像、人群匹配、种草、内容日历。
user-invocable: true
...
抖音内容运营的完整闭环：**热点选题 → 卖点脚本 → 人群匹配**。

比「随便找几个热点」多做一步：**用粉丝画像验证内容触达的是不是你的目标人群**——热点对了但人群错了，播放再高也不出单。
...
| 密钥来源 | 向用户索取，或读环境变量 `QINGHU_TOKEN` / `QHKIT_TOKEN`；用户先打开 https://www.iqinghu.com/workbench/login?urlCode=agentch 注册/登录，再到 https://www.iqinghu.com/workbench/dashboard/api-keys 生成 |
...
| 工具 | 中文名 | 用在哪一步 |
| --- | --- | --- |
| `hot_douyin_rank` | 热搜榜数据 | 全站热点，选题起点 |
| `douyin_hashtag_detail` | 话题详情数据 | 话题体量（需要 ch_id） |
| `douyin_hashtag_video_list` | 话题下视频数据 | 话题下的内容表现与形式 |
| `douyin_video_search` | 关键词下视频数据 | 按行业词找对标内容 |
| `douyin_video_info_with_play` | 查视频数据 | 单条视频的播放与互动 |
| `douyin_video_comments` | 查视频评论 | 评论区舆情与用户语言 |
| `douyin_user_profile` | 查达人数据 | 对标账号的粉丝、作品、点赞（需要主页链接） |
| `douyin_fans_portrait` | 查达人粉丝画像 | 年龄、性别、兴趣、地域、设备价格等全维画像 |
| `douyin_user_post_videos` | 查主页视频数据 | 对标账号的更新节奏与内容走向 |
| `douyin_link_convert` | 抖音短链接转换为正式链接 | 短链先转换（免费） |

> 抖音工具的入参多为**链接**：视频链接、主页链接、话题 ch_id。短链（`v.douyin.com/xxx`）必须先用 `douyin_link_convert`（免费）转换。
> `douyin_fans_portrait` 返回的画像维度很全（含城市层级与设备价格），**设备价格分布是判断消费力最直接的指标**，做客单价决策时别漏了它。

## 工作流程

### 1) 选题：热点 + 行业双线

`hot_douyin_rank` 找全站热点，`douyin_video_search` 按行业词找垂类内容。
**热点只是切口**，必须能和产品自然挂钩才用，硬蹭热点的内容转化极差。

### 2) 话题体量核实：`douyin_hashtag_detail` + `douyin_hashtag_video_list`

话题的播放体量、内容供给密度、主流内容形式。
供给已经很饱和的话题，新账号进去基本没有露出机会。

### 3) 拆卖点：`douyin_video_info_with_play` + `douyin_video_comments`

对标视频的互动率（而不是纯播放）+ 评论区高频问题。
**评论区就是现成的卖点素材库**：用户反复问什么，你的脚本就该先回答什么。

### 4) 人群匹配（这一步最容易被跳过，也最关键）

`douyin_user_profile` 拿对标账号的基础数据 → `douyin_fans_portrait` 看粉丝画像：
年龄、性别、地域、兴趣、**设备价格**（消费力代理指标）。

把画像和你的目标客群比对：
- 匹配 → 这个账号/内容方向可以直接借鉴或合作；
- 不匹配 → 播放再高也别投，人群对不上就是白花钱。

### 5) 盯节奏：`douyin_user_post_videos`

对标账号的更新频率、发布时段、内容转型信号。

### 6) 出内容规划

交付**可执行的选题清单 + 人群结论**，而不是一堆数据。

## 输出

- **选题清单（5~10 条）**：选题、热点依据、建议形式、目标人群。
- **卖点素材**：来自评论区的用户高频关注点与原话。
- **人群画像结论**：对标账号粉丝的年龄/性别/地域/兴趣/消费力，以及和自身客群的匹配度判断。
- **对标账号动态**：更新节奏与内容方向变化。
- 明细按「数据交付」导出。
...
- 本技能做**内容运营**，不做选品与上货——选品走「抖音-蓝海爆品采集师」，铺货走「抖音-极速上货助手」。
- 不生成视频与图片素材，那属于 LinkPix / qhkit 系列技能。
- 达人合作的商务对接、报价与结算不在范围内。
- 热点有强时效性，结论必须标注数据周期。
...
- **小红书-社媒运营专家 / B站-社媒运营专家**：多平台内容矩阵
- **抖音-蓝海爆品采集师**：内容验证过的方向反过来指导选品
- **爆款视频模仿系列技能**：把选题变成成品视频
