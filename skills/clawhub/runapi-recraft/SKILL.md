...
- For a one-off artifact or result, use the registered `recraft` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi recraft --help
runapi recraft <operation> --help
curl --fail --location https://runapi.ai/docs/api/recraft/<operation>.md --output contract.md
```
...
```shell
runapi recraft <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service recraft --action <operation> > result.json
```
