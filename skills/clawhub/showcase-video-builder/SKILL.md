...
- **Timed VO with `adelay`** — generate each narration line separately, then mix with `adelay=<ms>|<ms>` and `amix=inputs=N:duration=longest:normalize=0`, then add `apad` to hit target duration
- **Ken Burns frame count** — `zoompan` needs explicit `d=<frames>` where frames = duration * fps. Calculate with `python3 -c "print(int(dur * 30))"`
