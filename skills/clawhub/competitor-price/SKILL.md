---
name: amazon-competitor-price-positioning
description: >
  基于主 ASIN 与已授权竞品商品页中记录的展示价格快照，说明相对价格位置、时间点和证据缺口。仅用于快照对比；不宣称实时价格，不用于销量、利润、库存、订单或自动调价。
  Requires an ARI API key (ari_live_*).
slug: competitor-price
displayName: Amazon 竞品价格位置
version: 1.4.7
summary: 基于商品页快照比较展示价格位置与证据缺口
license: MIT
---
...
- 本入口只执行 `page_compare/price`，输出模板固定为 `ops_page_compare`。
