...
if [ -d "$SAGE_HOME/product" ] && grep -R "待填写" "$SAGE_HOME/product" >/dev/null 2>&1; then
  echo "[ONBOARDING] Product extension still has placeholders."
  ISSUES=$((ISSUES + 1))
fi
...
if [ "$ISSUES" -eq 0 ]; then
  echo "[OK] Sage DNA and product extension look healthy."
else
