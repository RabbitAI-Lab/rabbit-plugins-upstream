...
def main() -> None:
    ...
    if chat_type == "group":
        ...
        if error:
            out(error)
            return
        if args.scope in {"personal", "both"} or any(word in args.question for word in PERSONAL_WORDS):
            out(json_fail("personal_scope_not_allowed_in_group", "群聊中不能基于个人知识库生成综述。请私聊我处理个人资料。"))
            return
        team = group["team"]
        out({
            "success": True,
            ...
            "project_id": find_project_id(args.question, team),
            "personal_kb": None,
            "team_kb": {
    ...
    scope = args.scope
    if not scope:
        if any(w in args.question for w in PERSONAL_WORDS) and any(w in args.question for w in TEAM_WORDS):
            scope = "both" if has_team else "personal"
        elif any(w in args.question for w in TEAM_WORDS):
            scope = "team" if has_team else "personal"
        elif any(w in args.question for w in PERSONAL_WORDS):
            scope = "personal"
        else:
            scope = "both" if has_team else "personal"
    ...
    out({
        ...
        "user": user,
        "team": team,
        "project_id": find_project_id(args.question, team) if has_team else "",
        "personal_kb": {
