...
## Discoverability layer — post-publish verification

| Check | How to verify | Why |
|---|---|---|
| Topics/tags/categories accurate | Tag the skill accurately with `--tags` / `--categories` / `--topics` at publish time; declared topics must describe what the skill actually does | Misleading topics are metadata dishonesty — the same trust violation as a wrong `requires` declaration |
| Skill is findable | Run `clawhub search <keyword>` for 2-3 keywords a real user would type, or browse `clawhub explore` — does it show up when someone searches a relevant keyword? | A clean skill nobody finds serves nobody; search rank is the install funnel |
| Evidence captured | Record the tags/categories/topics used and screenshot the search results | Discoverability claims should be verifiable, like every other audit claim |

CLI reality check (v0.23.x): `--topics` (max 5) drive search and render as
hash-labels; `--tags` are npm-style dist-tags (`latest`), not keywords;
`--categories` requires valid taxonomy slugs and can be omitted.
