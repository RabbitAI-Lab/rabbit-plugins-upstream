---
name: account-opening-permit-recognition-and-extract
description: 基于来也科技ADP平台的中国开户许可证智能识别与信息抽取Skill。支持开户许可证的核准号、编号、基本存款账户、法定代表人、开户银行、银行账号等关键字段的精准抽取，输出结构化JSON，零配置开箱即用，适用于企业资质核验、工商信息查询等场景。
---

# 开户许可证识别与抽取 Skill

能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=clawhub)提供，支持中国开户许可证的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成开户许可证图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（每月刷新），相当于每月可免费抽取 **200 张开户许可证**。ADP 提供标准可商用 API，**1 小时即可快速集成接入业务系统**。
...
3. **获取应用列表**：首次执行时，通过 `adp app-id list --app-type 0` 获取开箱即用应用列表，找到开户许可证抽取应用并记录其 `app_id`（以 `ootb_` 开头）。后续优先使用 `adp app-id cache`。
4. **执行抽取**：运行 `adp extract url <URL> --app-id <开户许可证抽取应用ID>` 或 `adp extract local <文件路径> --app-id <开户许可证抽取应用ID>`。
5. **结果处理**：解析返回的 JSON，提取开户许可证注册号、名称、类型、法定代表人、成立日期、营业期限、经营范围、登记机关、发证日期等结构化字段。
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张开户许可证图片（URL） | `adp extract url <URL> --app-id <开户许可证抽取应用ID>` |
| 识别一张本地开户许可证图片 | `adp extract local <文件路径> --app-id <开户许可证抽取应用ID>` |
| 识别 Base64 编码的开户许可证 | `adp extract base64 <base64> --app-id <开户许可证抽取应用ID> --file-name <文件名.后缀>` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的开户许可证 | `adp extract local <文件夹路径> --app-id <开户许可证抽取应用ID>` |
| 批量识别多个 URL | `adp extract url <URL列表文件> --app-id <开户许可证抽取应用ID>` |

**异步处理**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 异步提交大文件 | `adp extract url <URL> --app-id <开户许可证抽取应用ID> --async` |
| 异步批量处理 | `adp extract local <文件夹路径> --app-id <开户许可证抽取应用ID> --async` |
...
ADP 为中国开户许可证提供了**开箱即用**的内置抽取应用，无需额外配置。
...
开户许可证识别属于**开箱即用应用**，可通过 `--app-type 0` 筛选查询。
...
从返回列表中找到 `app_label` 包含 **"开户许可证"** 的应用，记录其 `app_id`：

```json
[
  {
    "app_id": "ootb_******8a1b",
    "app_label": ["开户许可证", "银行凭证", "企业开户", "金融证件", "信息提取"],
    "app_name": "开户许可证",
    "app_type": 0
  }
]
...
> 上例中 `"app_id": "ootb_******8a1b"` 即为开户许可证抽取应用。`app_type` 为 `0` 表示开箱即用应用，`1` 表示自定义应用。
...
**重要提示**：每个账号下的 `app_id` 是唯一且固定的，除非用户主动删除应用，否则 `app_id` 不会变更。建议 Agent 将开户许可证应用的 `app_id` 保存在上下文中，下次直接使用。
...
```bash
adp extract url https://example.com/account-opening-permit.jpg --app-id <开户许可证抽取应用ID>
```

### 单张开户许可证抽取（本地文件）

```bash
adp extract local ./account-opening-permit.jpg --app-id <开户许可证抽取应用ID>
```

### 单张开户许可证抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <开户许可证抽取应用ID> --file-name <文件名.后缀>
```

### 返回结果示例

ADP 开户许可证抽取统一返回结构化 JSON，支持开户许可证以下**6个**关键字段识别抽取。
...
[
  {
    "field_key": "approval_number",
    "field_name": "核准号",
    "field_values": [
      {
        "field_value": "110108000000000"
      }
    ]
  },
  {
  ...
  {
    "field_key": "basic_deposit_account",
    "field_name": "基本存款账户",
    "field_values": [
      {
        "field_value": "110108000000000"
  ...
  {
    "field_key": "opening_bank",
    "field_name": "开户银行",
    "field_values": [
      {
        "field_value": "中国银行北京分行营业部"
      }
    ]
  },
  {
...
| --- | --- | --- | --- |
| `approval_number` | 核准号 | 开户许可证上的核准号 | **高（PII）** |
| `number` | 编号 | 开户许可证上的编号 | **高（PII）** |
| `basic_deposit_account` | 基本存款账户 | 开户许可证上的基本存款账户信息 | **高（PII）** |
| `legal_representative` | 法定代表人 | 开户许可证上的法定代表人姓名 | **高（PII）** |
| `opening_bank` | 开户银行 | 开户许可证上的开户银行名称 | **中** |
| `account` | 账号 | 开户许可证上的银行账号 | **高（PII）** |
| `company_name` | 公司名称 | 开户许可证上的公司名称 | **高（PII）** |
| `company_address` | 公司地址 | 开户许可证上的公司地址 | **高（PII）** |
| `establishment_date` | 成立日期 | 开户许可证上的公司成立日期 | **中** |  


> **字段地区变体说明**：ADP 平台对中国大陆地区开户许可证使用上述 `field_key`；如调用海外站点（`adp-global.laiye.com`），部分字段命名可能存在地区差异，请以实际返回的 `field_key` 为准。
...
```bash
adp extract local ./account-opening-permits/ --app-id <开户许可证抽取应用ID> --export ./results/ 
```
...
```bash
# 提交异步任务
adp extract url https://example.com/account-opening-permit.jpg --app-id <开户许可证抽取应用ID> --async
...
开户许可证识别只是来也科技 ADP 平台众多开箱即用能力之一。ADP 基于大模型通用理解能力，提供覆盖全品类文档的智能处理解决方案：
