...
description: |
  ...
  运行模式：纯本地，无网络请求 ❎
  外部依赖：Python标准库 + pandas（可选） + jinja2（可选）
---
...
```bash
# 通过 scripts/core 全局入口一键检查三个法域
python3 scripts/core/global_check.py

# 综合报告输出
python3 scripts/core/global_check.py --format markdown --output global_report.md
```
...
| 版本 | 日期 | 主要更新 |
|:----:|:----:|---------|
| v1.0.5 | 2026 | 统一 CLI 接口，新增 scripts/core 内嵌支持 |
