...
const STATE_FILE =
  process.env.PACKAGE_TRACKER_STATE_FILE ||
...
function webhookPath(config: PluginConfig): string {
...
interface PluginConfig {
  kuaidi100: { customer: string; key: string; endpoint?: string };
  calendar?: {
    client_id?: string; client_secret?: string;
    refresh_token?: string; calendar_id?: string;
  };
  schedule?: {
  ...
  webhook?: {
    baseUrl?: string; // e.g. https://xxxx.trycloudflare.com
    token?: string;   // shared secret, verified on ingress (embedded in path)
    salt?: string;    // optional kuaidi100 callback signature salt
    rateLimitPerMinute?: number; // local webhook rate limit per client IP
    signatureMode?: "soft" | "strict"; // soft: accept; strict: require valid signature
  };
}
...
function respondKuaidi100Ok(res: ServerResponse, message = "成功") {
...
// ── Very small in-memory rate limiter (per IP, rolling minute bucket) ─────────
type Bucket = { windowStartMs: number; count: number };
const rateBuckets = new Map<string, Bucket>();

function rateLimitOk(ip: string, limitPerMinute: number): boolean {
...
export default definePluginEntry({
  ...
  register(api) {
    const cfg = (): PluginConfig => {
      ...
      const c = (api as any).pluginConfig as PluginConfig | undefined;
      if (c) return c;
    ...
    api.registerHttpRoute({
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        try {
          ...
          const result = await callCore(cfg(), "handle_push", { push_body: body }) as any;
    ...
    api.registerTool({
      ...
      description:
        "Subscribe to push tracking updates for a package number via Kuaidi100. " +
    ...
    api.registerTool({
      name: "list_packages",
      description:
        "List all tracked packages with latest status from local cache (no API quota used). " +
    ...
    api.registerTool({
      ...
      description:
        "Manually trigger Google Calendar sync for today's expected deliveries. " +
    ...
    api.registerTool({
      ...
      description:
        "Remove a tracking number from the watch list. " +
