...
### Graph state as of v3.0.1
- Entities: 681 (see `graph.json` for exact current count)
- Relations: 1146 (see `graph.json` for exact current count)
- Both entities and relations include `scientific_status` field (canonical, with underscore)
- Entity status distribution:
  - established: 224
  - hypothesis: 157
  - partially_established: 120
  - non-existent, purely philosophical: 91
  - metaphor: 64
  - open_question: 25
- Relation status distribution:
  - non-existent, purely philosophical: 340
  - metaphor: 237
  - hypothesis: 233
  - partially_established: 115
  - established: 111
  - open_question: 110
...
- As of this version: All entities include a `scientific_status` field (counts as of this release: ~681 entities - see graph.json for current)
