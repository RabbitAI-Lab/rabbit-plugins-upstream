...
更细的拼写示例、与源码行号对齐的说明见本目录同级 [`../SKILL.md`](../SKILL.md) 及 [`../references/qips/`](../references/qips/) 下的参考文档。
...
| 全站搜索页 | **`channelid=115`**：`third_play_url` 可为**搜索词明文**（encode）；或 **`encodeURIComponent(JSON.stringify({ fromAiSuggest, query }))`** 指定是否走 Ai 搜及关键词 | `6` |
...
完整映射与 path/key 见 [`../references/qips/channel-table.md`](../references/qips/channel-table.md)。下表为集成时最常查的 **channelId**。
...
| 文档 | 用途 |
|------|------|
| [`../SKILL.md`](../SKILL.md) | Agent 执行 qips 的契约、自检与执行模板 |
| [`../references/qips/vtype-recipes.md`](../references/qips/vtype-recipes.md) | 各 `vtype` 示例 qips 与字段细则 |
| [`../references/qips/channel-table.md`](../references/qips/channel-table.md) | channelId 全量表与 path/key |
| [`../references/qips/api-usage.md`](../references/qips/api-usage.md) | `PlayInfo` / `ChannelInfo` / `Launcher` 开发者用法 |
| [`../references/qips/launch-checklist.md`](../references/qips/launch-checklist.md) | 本机拉起验收用例 |
