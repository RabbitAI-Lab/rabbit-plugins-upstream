...
python3 md_supervisor.py loop     # infinite: poll every INTERVAL s; on terminal session:
                                  #   pull -> sessions/<ts>/ ; read run_state.json ;
                                  #   version_resume() -> kaggle datasets version -m 'resume ns=...'
                                  #   launch() again. Stops on status=="done" or after 3 failures.
