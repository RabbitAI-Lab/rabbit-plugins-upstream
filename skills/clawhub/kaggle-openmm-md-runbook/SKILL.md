...
description: OpenMM 8.3.1 molecular dynamics runbook for Kaggle free GPU (P100 sm_60): 100 ns checkpoint/resume across ~12 h sessions, fact registry (21 traps / 16 rules / 20 errors, stable IDs) + query CLI for token-efficient grounding, 20-gate static preflight checker, append-only lesson log. Use when planning, executing, debugging, or handing off an OpenMM MD run on Kaggle.
version: 1.1.3
...
4. **[R04] Reinstall the `kaggle` CLI every fresh sandbox** (`pip install -q kaggle`); the API key file
...
`md_preflight.py` gained `--explain Gxx` (one-line fix per gate), `--version`, and 5 new
parameter gates (G16 HMR · G17 OPC water · G18 removeCMMotion · G19 checkpoint writes ·
G20 accelerator-advisory) — 20 gates total.
...
3. **The supervisor loop is opt-in and user-visible.** Start it only when the human asks for unattended operation;
...
4. **All executables are local and bounded.** `scripts/md_preflight.py` is a static checker: it reads
   ...
   only reads this skill's own `registry/*.json` files. `scripts/learn.py` only appends lines to this
   skill's own `registry/lessons.jsonl` (and never edits existing lines). Everything else is markdown.
5. **Account & supply-chain guarantees (with disclosure).** Every `kaggle kernels` command shown acts
   ...
   micromamba binary (a mutable `latest` endpoint — risk disclosed in RUNBOOK.md §4.2 bootstrap note,
   with SHA256-pinned-release guidance for non-ephemeral environments) and the conda-forge channel
