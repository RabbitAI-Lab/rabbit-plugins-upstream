...
FILES INCLUDED:
---------------
1. test_ms_todo_oauth.py        - Automated test script (33 tests)
...
2. Install dependencies into the Python environment used for testing:
   python -m pip install -r requirements.txt

3. Run the automated tests from the skill root:
   python scripts/test_ms_todo_oauth.py

   To run only non-destructive CLI/configuration checks:
   python scripts/test_ms_todo_oauth.py --preflight-only

What It Tests:
--------------
The automated script runs 33 comprehensive tests covering:

✓ CLI Configuration (4 tests)
  - Help output includes credential override options
  - Built-in fallback and explicit credential override resolution
  - Environment variable credential override resolution
  - Authorization code URL decoding and callback extraction
...
- Delete them manually: python scripts/ms-todo-oauth.py delete-list "<list_name>" -y
