...
class MicrosoftTodoClient:
    ...
    # Built-in fallback OAuth app credentials. Override with CLI options or env vars.
    DEFAULT_CLIENT_ID = "ca6ec244-002c-435b-bafd-06e470d37edc"
    DEFAULT_CLIENT_SECRET = "TwQ8Q~mHv6C_scYqI7PC2dZKWFeM931.8AczhasR"
    ...
    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None, tenant_id: str = "common", redirect_uri: str = "http://localhost:8000/callback", cache_file: Optional[str] = None, debug: bool = False):
        ...
        Args:
            client_id: Azure application client ID (optional, uses env var or built-in ID by default)
            client_secret: Client secret (optional, uses env var or built-in secret by default)
        ...
        """
        self.client_id = client_id or os.getenv("MS_TODO_CLIENT_ID") or self.DEFAULT_CLIENT_ID
        self.client_secret = client_secret or os.getenv("MS_TODO_CLIENT_SECRET") or self.DEFAULT_CLIENT_SECRET
    ...
    def verify_device_code_flow(self) -> bool:
        ...
        """
        print("⚠️  Device code flow is deprecated. Use OAuth2 instead:")
        print('  python scripts/ms-todo-oauth.py login verify "<code-or-callback-url>"')
        return False
...
def _normalize_authorization_code(code_input: str) -> str:
    """
    Normalize a copied OAuth callback value.

    Accepts either the raw code, a URL-encoded code value, or the full callback URL.
    """
    value = code_input.strip().strip("\"'")
    ...
    if parsed.query or parsed.fragment:
        ...
        if "code" in query_values and query_values["code"]:
            return query_values["code"][0].strip()
        if "code" in fragment_values and fragment_values["code"]:
            return fragment_values["code"][0].strip()
...
def cmd_login_get(args, client):
    ...
    print("Copy the full callback URL or the 'code' parameter and run:\n")
    print('  python scripts/ms-todo-oauth.py login verify "<authorization_code_or_callback_url>"\n')
    print("=" * 70 + "\n")
...
def cmd_login_verify(args, client):
    ...
    if not args.code:
        ...
        print('Usage: python scripts/ms-todo-oauth.py login verify "<authorization_code_or_callback_url>"')
        sys.exit(1)
    
    auth_code = _normalize_authorization_code(args.code)

    if client.acquire_token_by_auth_code(auth_code):
...
def create_parser():
    ...
    parser.add_argument("--client-id", help="Azure app client ID. Overrides MS_TODO_CLIENT_ID and the built-in fallback")
    parser.add_argument("--client-secret", help="Azure app client secret. Overrides MS_TODO_CLIENT_SECRET and the built-in fallback")
    parser.add_argument("--tenant-id", default=os.getenv("MS_TODO_TENANT_ID", "consumers"), help="Azure tenant ID or account type. Defaults to MS_TODO_TENANT_ID or 'consumers'")
    ...
    verify_parser.add_argument("code", help="Authorization code or full callback URL from redirect URI")
...
def main():
    ...
    if not client.authenticate():
        ...
        print("    python scripts/ms-todo-oauth.py login get")
