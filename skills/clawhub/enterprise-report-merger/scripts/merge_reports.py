...
def calculate_financial_ratios(bs_data: dict, is_data: dict) -> dict:
    """Calculate common financial ratios from balance sheet and income statement data.

    Uses find_in_data() for robust key lookup — handles Chinese full-width quotes
    and avoids '流动负债合计' matching when '负债合计' is requested.
    """
    ratios = {}
    total_assets = parse_numeric(find_in_data(bs_data, '资产总计'))
    total_liab = parse_numeric(find_in_data(bs_data, '负债合计'))
    curr_assets = parse_numeric(find_in_data(bs_data, '流动资产合计'))
    curr_liab = parse_numeric(find_in_data(bs_data, '流动负债合计', '流动负债小计'))
    inventory = parse_numeric(find_in_data(bs_data, '存货'))
    equity = parse_numeric(
        find_in_data(bs_data, '所有者权益合计', '股东权益合计',
                     '归属于母公司股东权益小计')
    )
    revenue = parse_numeric(find_in_data(is_data, '营业收入'))
    net_profit = parse_numeric(find_in_data(is_data, '净利润'))
    cost = parse_numeric(find_in_data(is_data, '营业成本'))
    paid_in_capital = parse_numeric(
        find_in_data(bs_data, '实收资本', '股本', '实收资本（或股本）')
    )
    total_profit = parse_numeric(find_in_data(is_data, '利润总额', '税前利润'))
    interest_expense = parse_numeric(find_in_data(is_data, '利息支出'))

    if total_assets and total_liab is not None and total_assets != 0:
        ratios['资产负债率'] = f'{total_liab / total_assets * 100:.2f}%'
    if curr_assets is not None and curr_liab and curr_liab != 0:
        ratios['流动比率'] = f'{curr_assets / curr_liab:.2f}'
        ratios['速动比率'] = f'{(curr_assets - (inventory or 0)) / curr_liab:.2f}'
    if total_liab is not None and equity and equity != 0:
        ratios['产权比率'] = f'{total_liab / equity * 100:.2f}%'
    if revenue and net_profit is not None and revenue != 0:
        ratios['销售净利率'] = f'{net_profit / revenue * 100:.2f}%'
    if equity and net_profit is not None and equity != 0:
        ratios['净资产收益率'] = f'{net_profit / equity * 100:.2f}%'
    if revenue and cost is not None and revenue != 0:
        ratios['毛利率'] = f'{(revenue - cost) / revenue * 100:.2f}%'
    if paid_in_capital and net_profit is not None and paid_in_capital != 0:
        ratios['资本收益率'] = f'{net_profit / paid_in_capital * 100:.2f}%'
    if total_assets and total_assets != 0:
        # 标准版：(利润总额 + 利息支出) / 资产总计；利息支出不可得时用简化版
        if total_profit is not None and interest_expense is not None:
            ratios['总资产报酬率'] = f'{(total_profit + interest_expense) / total_assets * 100:.2f}%'
        elif net_profit is not None:
            ratios['总资产报酬率'] = f'{net_profit / total_assets * 100:.2f}%'
    return ratios
...
def find_in_data(data_dict: dict, *keywords) -> str:
    ...
    Returns the value string, or '0' if not found.
    """
    if not data_dict:
        return '0'
