...
RATIO_REFERENCES = {
    '资产负债率': {'good': 60, 'warning': 70, 'direction': 'lower_better'},
    '流动比率': {'good': 2.0, 'warning': 1.0, 'direction': 'higher_better'},
    '速动比率': {'good': 1.0, 'warning': 0.5, 'direction': 'higher_better'},
    '产权比率': {'good': 100, 'warning': 150, 'direction': 'lower_better'},
    '销售净利率': {'good': 10, 'warning': 3, 'direction': 'higher_better'},
    '净资产收益率': {'good': 10, 'warning': 3, 'direction': 'higher_better'},
    '毛利率': {'good': 20, 'warning': 10, 'direction': 'higher_better'},
    '资本收益率': {'good': 10, 'warning': 3, 'direction': 'higher_better'},
    '总资产报酬率': {'good': 8, 'warning': 3, 'direction': 'higher_better'},
}
...
def generate_profit_analysis(is_data, prev_is_data=None):
    ...
    if rev_val and rev_val != 0 and np_val is not None:
        parts.append(f'销售净利率为{np_val / rev_val * 100:.2f}%。')
...
def generate_ratio_analysis(ratios):
    """Generate financial ratio analysis text."""
    parts = []

    # 偿债能力
    debt_ratio = ratios.get('资产负债率', '')
    curr_ratio = ratios.get('流动比率', '')
    quick_ratio = ratios.get('速动比率', '')
    eq_ratio = ratios.get('产权比率', '')

    solvency_parts = []
    if debt_ratio:
        solvency_parts.append(f'资产负债率{debt_ratio}')
    if curr_ratio:
        solvency_parts.append(f'流动比率{curr_ratio}')
    if quick_ratio:
        solvency_parts.append(f'速动比率{quick_ratio}')
    if solvency_parts:
        parts.append(f'偿债能力方面，{"，".join(solvency_parts)}。')

    # 盈利能力
    npm = ratios.get('销售净利率', '')
    roe = ratios.get('净资产收益率', '')
    gm = ratios.get('毛利率', '')
    roa = ratios.get('总资产报酬率', '')

    profit_parts = []
    if npm:
        profit_parts.append(f'销售净利率{npm}')
    if roe:
        profit_parts.append(f'净资产收益率{roe}')
    if gm:
        profit_parts.append(f'毛利率{gm}')
    if profit_parts:
        parts.append(f'盈利能力方面，{"，".join(profit_parts)}。')
...
def build_ratio_table_data(ratios, prev_ratios=None):
    """Build rows for financial ratio table."""
    rows = []
    for name, value in ratios.items():
        row = [name, value]
        if prev_ratios:
            row.append(prev_ratios.get(name, '-'))
        # Add reference value
        ref = RATIO_REFERENCES.get(name)
        if ref:
            if ref['direction'] == 'lower_better':
                row.append(f"<= {ref['warning']}%")
            else:
                row.append(f">= {ref['warning']}")
        else:
            row.append('-')
...
def generate_report(
    ...
    ratios = calculate_financial_ratios(bs_data, is_data)
    prev_ratios = None
    if prev_bs_data and prev_is_data:
        prev_ratios = calculate_financial_ratios(prev_bs_data, prev_is_data)

    ratio_headers = ['指标', '本期值']
    if prev_ratios:
        ratio_headers.append('上期值')
    ratio_headers.append('参考值')
    ratio_rows = build_ratio_table_data(ratios, prev_ratios)
    add_data_table(doc, ratio_headers, ratio_rows)
    doc.add_paragraph()
