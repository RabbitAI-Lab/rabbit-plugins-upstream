...
# ── Exclusion Keywords Library ──
EXCLUSION_FILE = os.path.join(_SKILLS_DIR, "data", "exclusion_keywords.json")
API_EXCLUDE_MAX_CHARS = 10240

def _empty_exclusion_data():
    return {"version": 1, "global_keywords": [], "category_keywords": {}}

def load_exclusion_keywords():
    if not os.path.exists(EXCLUSION_FILE):
        return _empty_exclusion_data()
    try:
...
def save_exclusion_keywords(data):
    d = os.path.dirname(EXCLUSION_FILE)
    os.makedirs(d, exist_ok=True)
    with open(EXCLUSION_FILE, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

def add_exclusion_keywords(words_str, category_node_id=None, category_label=None):
    words = [w.strip() for w in words_str.split(',') if w.strip()]
    if not words:
        return {"added": 0, "total": 0, "api_chars": 0}
    data = load_exclusion_keywords()
    added = 0
    if category_node_id:
        cat = data["category_keywords"].setdefault(category_node_id, {"label": category_label or "", "keywords": []})
        if category_label:
            cat["label"] = category_label
        for w in words:
            if w not in cat["keywords"]:
                cat["keywords"].append(w)
                added += 1
    else:
        for w in words:
            if w not in data["global_keywords"]:
                data["global_keywords"].append(w)
                added += 1
    save_exclusion_keywords(data)
    total = len(data["global_keywords"]) + sum(len(c["keywords"]) for c in data["category_keywords"].values())
    all_kw = ",".join(data["global_keywords"])
    return {"added": added, "total": total, "api_chars": len(all_kw)}

def remove_exclusion_keyword(word):
    word = word.strip()
    if not word:
        return {"removed": 0, "total": 0}
    data = load_exclusion_keywords()
    removed = 0
    if word in data["global_keywords"]:
        data["global_keywords"].remove(word)
        removed += 1
    for cat_id in list(data["category_keywords"].keys()):
        cat = data["category_keywords"][cat_id]
        if word in cat["keywords"]:
            cat["keywords"].remove(word)
            removed += 1
    data["category_keywords"] = {k: v for k, v in data["category_keywords"].items() if v["keywords"]}
    save_exclusion_keywords(data)
    total = len(data["global_keywords"]) + sum(len(c["keywords"]) for c in data["category_keywords"].values())
    return {"removed": removed, "total": total}

def clear_exclusion_keywords():
    data = load_exclusion_keywords()
    total = len(data["global_keywords"]) + sum(len(c["keywords"]) for c in data["category_keywords"].values())
    save_exclusion_keywords(_empty_exclusion_data())
    return {"cleared": total}

def list_exclusion_keywords(category_node_id=None):
    data = load_exclusion_keywords()
    if category_node_id:
        cat = data["category_keywords"].get(category_node_id, {"label": "", "keywords": []})
        return {"category": category_node_id, "label": cat.get("label", ""), "keywords": cat.get("keywords", []), "count": len(cat.get("keywords", []))}
    global_kw = data["global_keywords"]
    cat_summary = [{"node_id": k, "label": v["label"], "count": len(v["keywords"])} for k, v in data["category_keywords"].items()]
    all_global_str = ",".join(global_kw)
    return {
        "global_keywords": global_kw,
        "global_count": len(global_kw),
        "categories": cat_summary,
        "total_count": len(global_kw) + sum(len(v["keywords"]) for v in data["category_keywords"].values()),
        "api_chars": len(all_global_str),
        "api_max_chars": API_EXCLUDE_MAX_CHARS,
        "api_overflow": max(0, len(all_global_str) - API_EXCLUDE_MAX_CHARS),
    }

def get_active_exclusion_keywords(node_id_path=None):
    """Get applicable exclusion keywords for a search.
    Returns (all_keywords_list, api_keywords_str, overflow_keywords_list).
    """
    data = load_exclusion_keywords()
    keywords = list(data["global_keywords"])
    if node_id_path:
        node_ids = [n.strip() for n in node_id_path.split(':') if n.strip()]
        for nid in node_ids:
            cat = data["category_keywords"].get(nid)
            if cat:
    ...
    current_len = 0
    for kw in keywords:
        kw_len = len(kw) + (1 if api_str else 0)
        if current_len + kw_len <= API_EXCLUDE_MAX_CHARS:
            api_str = api_str + "," + kw if api_str else kw
            current_len += kw_len
        else:
            overflow.append(kw)
    return keywords, api_str, overflow

def client_side_filter(products, overflow_words):
    """Filter products by title against overflow exclusion words.
    Returns (filtered_products, excluded_count).
    """
    if not overflow_words:
        return products, 0
    overflow_lower = [w.lower() for w in overflow_words]
    filtered = []
    excluded = 0
    for p in products:
        title = (p.get('title', '') or '').lower()
        if any(w in title for w in overflow_lower):
            excluded += 1
        else:
            filtered.append(p)
...
# ── Main round logic ──
def run_round(params, json_output=False, overflow_keywords=None):
    ...
    # Client-side overflow exclusion filtering
    excluded_by_client = 0
    if overflow_keywords:
        all_p, excluded_by_client = client_side_filter(all_p, overflow_keywords)
    ...
    fetched_raw = len(all_p) + excluded_by_client
    print(f"  Fetched: {fetched_raw} | New: {len(new_p)} | Dupes: {len(dup_p)} | Total: {total}")
    if excluded_by_client:
        print(f"  Excluded by client filter: {excluded_by_client} (overflow keywords)")
...
def parse_args():
    ...
    # ── Exclusion keywords ──
    ap.add_argument('--exclude-add', default=None, help='Add exclusion keywords (comma-separated)')
    ap.add_argument('--exclude-list', action='store_true', help='List exclusion keywords library')
    ap.add_argument('--exclude-remove', default=None, help='Remove an exclusion keyword')
    ap.add_argument('--exclude-clear', action='store_true', help='Clear all exclusion keywords')
    ap.add_argument('--no-exclude', action='store_true', help='Skip exclusion for this round')
    ap.add_argument('--exclude-add-category', default=None, help='Category nodeId to bind exclusion keywords')
    ap.add_argument('--exclude-list-category', default=None, help='List exclusion keywords for a specific category')
    ap.add_argument('--node-id-path', default=None, help='Search within a category (colon-separated nodeIds)')
    ap.add_argument('--node-label', default=None, help='Category label (with --node-id-path)')
    return ap.parse_args()

def main():
    args = parse_args()

    # ── Exclusion keywords management (mutually exclusive commands) ──
    if args.exclude_clear:
        r = clear_exclusion_keywords()
        print(f"Exclusion library cleared. Removed {r['cleared']} keywords.")
        return
    if args.exclude_list:
        r = list_exclusion_keywords(args.exclude_list_category)
        if args.exclude_list_category:
            print(f"\nExclusion keywords for category {r['category']} ({r.get('label','')}):")
            for kw in r["keywords"]:
                print(f"  - {kw}")
            print(f"  Total: {r['count']}")
        else:
            print(f"\n{'='*60}\nExclusion Keywords Library\n{'='*60}")
            print(f"  Global keywords ({r['global_count']}): {', '.join(r['global_keywords']) if r['global_keywords'] else '(empty)'}")
            if r["categories"]:
                print(f"\n  Category-mapped keywords:")
                for cat in r["categories"]:
                    print(f"    [{cat['node_id']}] {cat['label']} ({cat['count']} keywords)")
            print(f"\n  Total: {r['total_count']} keywords")
            print(f"  API chars: {r['api_chars']}/{r['api_max_chars']} (overflow: {r['api_overflow']} via client filter)")
        return
    if args.exclude_add:
        r = add_exclusion_keywords(args.exclude_add, args.exclude_add_category, args.node_label)
        loc = f"category {args.exclude_add_category}" if args.exclude_add_category else "global"
        print(f"Added {r['added']} keywords to {loc}. Total: {r['total']} keywords (API chars: {r['api_chars']}).")
        return
    if args.exclude_remove:
        r = remove_exclusion_keyword(args.exclude_remove)
        print(f"Removed '{args.exclude_remove}' from {r['removed']} location(s). Total: {r['total']} keywords remaining.")
        return
    ...
    # ── Node path / label for category search ──
    if args.node_id_path:
        params['nodeIdPath'] = args.node_id_path
    if args.node_label:
        params['nodeLabel'] = args.node_label

    # ── Exclusion keywords injection ──
    overflow_keywords = []
    if not args.no_exclude:
        kw_list, api_str, overflow = get_active_exclusion_keywords(args.node_id_path)
        if api_str:
            params['excludeKeywords'] = api_str
        overflow_keywords = overflow
        if kw_list:
            print(f"  Exclusion: {len(kw_list)} keywords ({len(api_str)} chars via API, {len(overflow)} via client filter)")

    global PAGES_PER_ROUND
    PAGES_PER_ROUND = args.pages_per_round
    try:
        run_round(params, json_output=args.json_output, overflow_keywords=overflow_keywords or None)
