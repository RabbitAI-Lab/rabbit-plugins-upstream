#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""既往症审核 — 据病历题干判断其他诊断与主诊断关系及是否报销，直连内部医疗大模型；无 _shared 依赖。"""
...
DEFAULT_SYSTEM_PROMPT = (
    "你是保险理赔与临床诊断关系审核辅助专家。用户会给出病历或理赔材料文字，"
    "要求判断「其他诊断与主诊断的关系」以及「是否报销」。\n"
    "1. 关系类型只能为：并发症、疑似既往症、无关疾病 之一（须与题干用词完全一致）。\n"
    "2. 是否报销只能为：报销、不报销 之一（须与题干用词完全一致）。\n"
    "3. 严格按用户消息规定的行数与前缀输出（常见为三行：「关系类型：…」「是否报销：…」「判断依据：…」）；"
    "不要输出 Markdown、编号、多余空行或任何前后缀说明。\n"
    "4. 判断须基于题干已给出的病历内容；若材料噪声大、关键临床信息不足，须在判断依据中说明不确定性，"
    "并优先按题干允许的类别择一，避免编造未出现的诊断或住院经过。"
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=f"{SKILL_TITLE}：主诊断与其他诊断关系 + 是否报销，直连内部医疗大模型。"
    )
