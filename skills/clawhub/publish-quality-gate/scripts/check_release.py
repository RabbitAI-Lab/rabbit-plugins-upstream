...
LAYERS = {
    ...
    "L2 本机信息": {
        ...
        "电脑用户名": r'(?i)(Administrator|DESKTOP-|PC-\w+)',
    ...
    "L4 机密信息": {
        ...
        "平台账号": r'(?i)(ima[_-]?mcp|agent[_-]?mail)',
        "内部代号": r'(?i)(内部系统名|项目代号)',
...
def is_author_credit(hit):
    ...
    # 公开品牌豁免（如 MedXpert 公司品牌、注册老炮作者品牌）
    ...
    if any(brand.lower() in ctx for brand in BRAND_NAMES):
        if any(kw in ctx for kw in ['medxpert', '注册老炮', '"name"', 'author', '作者', '署名', '版权', '©']):
            return True
    # 作者署名豁免
    if hit['type'] != '人名':
        return False
...
def main():
    ...
    if not real_hits:
        print("\n✅ 四层检查通过（无真实敏感信息泄露）!")
        if credit_hits:
            print(f"\n📝 另发现 {len(credit_hits)} 处作者署名/公开品牌（如 注册老炮、MedXpert），请确认是有意保留的:")
