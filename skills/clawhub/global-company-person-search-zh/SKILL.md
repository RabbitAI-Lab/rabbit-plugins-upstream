---
name: global-company-person-search-zh
description: 依托全球企业数据库，结合姓名、任职公司、所属行业以及个人资料 URL 筛选目标人员，助力外贸从业者找到采购负责人、企业对接人员以及高层决策人。
...
使用跨境魔方开放平台API从全球企业库数据搜索人物。

## 概述

本技能通过一个列表搜索脚本提供对跨境魔方全球企业库人物数据的访问。通过`auth.py`脚本提供 API密钥生成、充值；
...
**重要**：始终使用直接脚本调用，如 `python scripts/global_company_person_search.py`。**不要使用** shell 复合命令如 `cd scripts && python global_company_person_search.py`

### 全球企业库人物列表搜索 (`global_company_person_search.py`)
- **返回粒度**：每个人物为一行记录
- **适用场景**：关心的是全球企业库数据中"有哪些人"
- **示例**：
  - "找XXXX的CTO"
  - "找中国区的销售总监"
- **参数**：查看参数说明 [全球企业库人物列表](references/global-company-person-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/depth_company/person/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"人物搜索查询失败，服务端异常"}'
```
...
| 用户意图 | 使用API |
|-------------|---------|
| "找XX公司的CTO" | 全球企业库人物列表（companyNames + keywords） |
| "找美国有邮箱的人" | 全球企业库人物列表 existEmail=1 + countryCodes=["US"] |
| "通过领英/官网链接找人" | 全球企业库人物列表 humanUrls |
...
- [全球企业库人物列表 API](references/global-company-person-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/global-company-person-list-api.md](references/global-company-person-list-api.md)
   - 不要猜测参数名称，从文档中获取准确的参数名称和格式

2. **优化查询参数**：
   - 使用 `humanIndustries` / `companyNames` / `humanUrls` 精确筛选，行业名称需使用**英文**
   - 使用 `existEmail=1` 或 `existPhone=1` 筛选有联系方式的人物
