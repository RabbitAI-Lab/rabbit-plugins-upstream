...
- Requires web access to collect current headlines and source URLs.
- Prefer direct source pages first, then search results if source pages are unavailable.
- If fewer than three domestic sources are accessible, continue with available sources, set confidence to `Low`, and list inaccessible sources.
...
- `Medium`: >= 3 sources with moderate concentration bias or partial freshness gaps, but sector/ticker narratives are still cross-validated.
- `Low`: < 3 sources, stale window, or highly concentrated/noisy coverage with weak confirmation.
