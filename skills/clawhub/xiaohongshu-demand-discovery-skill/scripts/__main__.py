...
def cmd_demand_discovery(args):
    """Demand discovery collection command."""
    result = demand_discovery.collect_demand_discovery(
        keywords=args.keywords,
        keywords_file=args.keywords_file,
        days=args.days,
        search_publish_time=args.search_publish_time,
        sort_by=args.sort_by,
        note_type=args.note_type,
        posts_per_keyword=args.posts_per_keyword,
...
def main():
    ...
    # demand-discovery (read-only demand collection)
    dd_p = subparsers.add_parser("demand-discovery", help="Xiaohongshu demand discovery collector")
    dd_p.add_argument("--keywords", help="Comma-separated keywords, for example: 求推荐,避雷,平替")
    dd_p.add_argument("--keywords-file", help="Path to a UTF-8 keywords file, one keyword per line")
    dd_p.add_argument("--days", type=int, default=3, help="Keep notes published within this many days")
    dd_p.add_argument("--search-publish-time", default="一周内", help="Search publish-time filter")
    dd_p.add_argument("--sort-by", default="最多评论", choices=["最多评论", "最多点赞"], help="Search sort mode")
    dd_p.add_argument("--note-type", default="不限", help="Search note type filter")
    dd_p.add_argument("--posts-per-keyword", type=int, default=3, help="Collected notes per keyword")
    dd_p.add_argument("--search-limit", type=int, default=8, help="Search results to inspect per keyword")
    dd_p.add_argument("--max-comments", type=int, default=20, help="Max valid comments saved per note")
    dd_p.add_argument("--output-dir", help="Output directory; defaults to data/demand_discovery/<timestamp>/")
    dd_p.add_argument("--timezone", default="Asia/Shanghai", help="Timezone for recent-day filtering")
    dd_p.add_argument("--headless", default='true')
    dd_p.set_defaults(func=cmd_demand_discovery)
