---
name: credit-card-recognition-and-extract
description: 基于来也科技ADP平台的银行卡智能识别与信息抽取Skill。支持银行卡正面关键字段的精准抽取——卡号、发卡机构、有效期、卡类型，输出结构化JSON，零配置开箱即用，适用于开户审核、支付绑定、金融信息录入等场景。
---

# 银行卡识别与抽取 Skill

能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=clawhub)提供，支持各类银行卡（储蓄卡、信用卡、联名卡、生肖卡等）的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成银行卡图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（后续每月初自动刷新赠送资产），相当于每月可免费抽取 **200 张银行卡**。来也 ADP平台所有功能提供标准可商用 API，1小时即可快速集成接入业务系统。
...
3. **获取应用列表**：首次执行时，通过 `adp app-id list --app-type 0` 获取开箱即用应用列表，找到银行卡抽取应用并记录其 `app_id`（以 `ootb_` 开头）。后续优先使用 `adp app-id cache`。
4. **执行抽取**：运行 `adp extract url <URL> --app-id <银行卡抽取应用ID>` 或 `adp extract local <文件路径> --app-id <银行卡抽取应用ID>`。
5. **结果处理**：解析返回的 JSON，提取卡号、发卡机构、有效期、卡类型等结构化字段。
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张银行卡图片（URL） | `adp extract url <URL> --app-id <ID>` |
| 识别一张本地银行卡图片 | `adp extract local <文件路径> --app-id <ID>` |
| 识别 Base64 编码的银行卡 | `adp extract base64 <base64> --app-id <ID> --file-name bank-card.jpg` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的银行卡 | `adp extract local <文件夹路径> --app-id <ID>` |
...
### 3. 完成认证配置
初次使用ADP Skill需要进行API Key、BaseURL配置，后续使用可直接使用
```bash
...
ADP 为银行卡提供了**开箱即用**的内置抽取应用，无需额外配置。
...
银行卡识别属于**开箱即用应用**，可通过 `--app-type 0` 筛选查询。
...
从返回列表中找到 `app_label` 包含 **"银行卡"** 的应用，记录其 `app_id`：

```json
[
  {
    "app_id": "ootb_******y5z8",
    "app_label": ["银行卡", "借记卡", "信用卡", "金融证件", "信息提取"],
    "app_name": "银行卡",
    "app_type": 0
  }
]
...
> 上例中 `"app_id": "ootb_******y5z8"` 即为银行卡抽取应用。`app_type` 为 `0` 表示开箱即用应用，`1` 表示自定义应用。
...
**重要提示**：每个账号下的 `app_id` 是唯一且固定的，除非用户主动删除应用，否则 `app_id` 不会变更。建议 Agent 将银行卡应用的 `app_id` 保存在上下文中，下次直接使用。
...
```bash
adp extract url https://example.com/bank-card.jpg --app-id <银行卡抽取应用ID>
```

### 单张银行卡抽取（本地文件）

```bash
adp extract local ./bank-card.jpg --app-id <银行卡应用ID>
```

### 单张银行卡抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <银行卡应用ID> --file-name bank-card.jpg
```

### 返回结果示例

ADP 银行卡抽取统一返回结构化 JSON，精准识别银行卡正面关键信息。
...
| --- | --- | --- |
| `card_number` | 卡号 | 银行卡卡号 |
| `issuing_institution` | 发卡机构 | 发卡机构名称 |
| `validity_period` | 有效期 | 卡片有效期，格式如 "08/28" |
| `card_type` | 卡类型 | 借记卡 / 信用卡 等 |
...
```bash
adp extract local ./bank-cards/ --app-id <银行卡应用ID> --export ./results/ 
```
...
```json
{
  ...
  "files": [
    {"input": "bank-card-001.jpg", "output": "bank-card-001.jpg.json", "status": "success"},
    {"input": "bank-card-002.jpg", "output": "bank-card-002.jpg.json", "status": "success"},
...
```bash
# 提交异步任务
adp extract url https://example.com/bank-card.jpg --app-id <银行卡应用ID> --async
...
# 银行卡抽取（URL）
adp extract url <文件URL> --app-id <银行卡抽取应用ID>

# 银行卡抽取（本地文件）
adp extract local <文件路径> --app-id <银行卡抽取应用ID>

# 银行卡抽取（Base64）
adp extract base64 <base64字符串> --app-id <银行卡抽取应用ID> --file-name bank-card.jpg

# 批量抽取
adp extract local <文件夹路径> --app-id <银行卡抽取应用ID> --export <输出路径> 

# 异步抽取
adp extract url <文件URL> --app-id <银行卡抽取应用ID> --async
...
| --- | --- |
| 银行卡抽取费用 | **0.5 积分/张** |
| 新用户免费额度 | 每月赠送 **100 积分，相当于每月可免费处理 200 张银行卡**，每月初重置 |
| 查询余额 | `adp credit` |
...
银行卡识别只是来也科技 ADP 平台众多开箱即用能力之一。ADP 基于大模型通用理解能力，提供覆盖全品类文档的智能处理解决方案：
...
5. **应用 ID 复用**：银行卡应用的 `app_id` 在账户下唯一且固定，建议记住后直接使用，无需每次查询。
