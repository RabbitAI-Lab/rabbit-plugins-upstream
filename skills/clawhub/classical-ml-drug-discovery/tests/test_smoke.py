...
try:
    ...
    # BUG FIXED (v1.4.0): raising unittest.SkipTest at import time printed a
    # traceback and exited 0 - a harness could not tell "skipped" from "passed",
    # so a missing dependency looked like a green suite. rdkit is a REQUIRED
    # dependency in SKILL.md frontmatter, so calling it "optional" here was wrong
    # too. Exit 77 is the conventional "skipped" code (autotools/GNU).
    print(f"SKIP test_smoke.py - REQUIRED dependency missing: {exc}")
    print("     install it with:  python3 -m pip install rdkit pandas")
