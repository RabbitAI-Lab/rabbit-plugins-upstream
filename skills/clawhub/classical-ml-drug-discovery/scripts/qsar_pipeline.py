...
# NOTE: kept in lockstep with SKILL.md `version:` - they had drifted
# (code said 1.3.0 while SKILL.md said 1.3.2), so model cards recorded the
# wrong provenance. A test now asserts they match.
TOOL_VERSION = "1.4.2"
DEFAULT_FP_RADIUS = 2
...
def estimate_domain_threshold(
    ...
    Returns 0.0 when fewer than 3 training molecules are available. A threshold of
    0.0 marks EVERY query compound `in_domain`, which is not a real applicability
    domain — callers must surface `domain_threshold_degenerate` so the claim is not
    read as evidence (v1.4.0).
    """
    if len(train_fps) < 3:
        return 0.0
...
def run_train(args: argparse.Namespace) -> int:
    ...
    # BUG FIXED (v1.4.0): a 0.0 threshold silently marks EVERY compound in_domain.
    # That is not an applicability domain, and downstream readers (human or model)
    # were free to cite it as evidence. Announce it loudly and record it in the JSON.
    domain_degenerate = bool(ad_threshold <= 0.0)
    if domain_degenerate:
        print("  [!] applicability-domain threshold is 0.0 - every compound will be "
              "labelled in_domain. This is NOT an applicability domain; it means too "
              "few training molecules (<3) or an explicit --ad-threshold of 0. Do not "
              "cite in_domain as evidence for this run.")
    ...
    metrics = {
        ...
        "ad_threshold": float(ad_threshold),
        "domain_threshold_degenerate": domain_degenerate,
        "domain_warning": (
            "threshold 0.0 marks every compound in_domain; not a valid applicability "
            "domain - do not cite in_domain as evidence" if domain_degenerate else None
        ),
...
def conformal_ncs(task: str, model: Any, X: np.ndarray, y_true: np.ndarray, class_names: list[str]) -> np.ndarray:
    ...
    if task == "classification":
        ...
        if probabilities is not None:
            ...
            # Guard the column mapping. `--trust-model` accepts externally built
            # bundles, and an estimator whose score matrix is not (n_samples,
            # n_classes) - one-vs-one decision shapes, a calibrated wrapper with a
            # different class order, a custom estimator - would be indexed by
            # true_idx anyway and return a nonconformity score read from the WRONG
            # column. That yields a confident, silently misleading prediction set.
            # Refuse instead of guessing.
            if probs.ndim != 2 or probs.shape[1] != n_classes:
                raise SystemExit(
                    f"Model probability matrix has shape {probs.shape}, which does not "
                    f"match the bundle's {n_classes} classes {class_names}. Per-class "
                    f"columns cannot be mapped reliably, so any prediction set would be "
                    f"silently wrong. Refusing. Retrain the bundle with this pipeline."
                )
            # nonconformity = 1 - probability of the true class
            return 1.0 - probs[rows, true_idx]
        # BUG FIXED (v1.4.0): this used to substitute np.ones(...) when the model
        # exposed no predict_proba. Every nonconformity score became 1-1 = 0, the
        # threshold became 0, and the tool reported 100% calibration coverage for a
        # prediction set that means nothing - confidently invalid statistics, the
        # worst possible failure for a skill that claims defensibility.
        #
        # Conformal validity does NOT require probabilities: ANY nonconformity
        # function preserves the coverage guarantee under exchangeability. So fall
        # back to the decision-function margin (higher margin for the true class =
        # less nonconforming). Raw decision values need no [0,1] rescaling, because
        # only the RANK of the scores enters the quantile.
        if decisions is not None:
            dec = np.asarray(decisions, dtype=float)
            if dec.ndim == 1:      # binary: positive value favours class_names[1]
                if n_classes != 2:
                    raise SystemExit(
                        f"Model returned a 1-D decision function but the bundle declares "
                        f"{n_classes} classes {class_names}. Refusing to guess the class "
                        f"mapping for a conformal prediction set."
                    )
                signed = np.where(true_idx == 1, dec, -dec)
                return -signed
            # Multiclass: only an (n_samples, n_classes) one-vs-rest matrix can be
            # indexed by class. A one-vs-one matrix has n*(n-1)/2 columns and would
            # silently return the margin of an unrelated class pair.
            if dec.ndim != 2 or dec.shape[1] != n_classes:
                raise SystemExit(
                    f"Model decision function has shape {dec.shape}, which does not match "
                    f"the bundle's {n_classes} classes {class_names} (a one-vs-one matrix "
                    f"has {n_classes * (n_classes - 1) // 2} columns). Column-to-class "
                    f"mapping is undefined, so the prediction set would be misleading. "
                    f"Refusing. Use decision_function_shape='ovr' or a probability model."
                )
            return -dec[rows, true_idx]
        raise SystemExit(
            "Conformal prediction needs a per-class score. This model exposes "
            "neither predict_proba nor decision_function, so no valid nonconformity "
            "measure can be formed. Refusing to emit a prediction set rather than "
            "report a meaningless 100% coverage. Retrain with probability=True "
            "(SVC) or use a model that exposes class scores."
        )
    # regression
...
def run_select(args: argparse.Namespace) -> int:
    ...
    max_candidates = int(args.max_candidates if args.max_candidates else len(frame))
    # BUG FIXED (v1.4.0): this read `args.budget or default`. Python treats 0 as
    # falsy, so `--budget 0` silently fell through to the default and returned the
    # ENTIRE candidate pool instead of an empty batch — a silent wrong answer for
    # anyone computing a budget programmatically. `--budget -3` likewise became 1.
    # validate_args() now rejects budget < 1 outright; `is None` keeps the default
    # only when the flag is genuinely absent.
    budget = int(args.budget) if args.budget is not None else min(max_candidates, 100)
    pool_size = len(frame)
    budget_note = None
    if budget > pool_size:
        budget_note = (f"requested budget {budget} exceeds the {pool_size} available "
                       f"candidates after filtering; returning all {pool_size}")
        print(f"  [!] {budget_note}")
    ...
    report = {
        ...
        # distinct from candidate_pool above: this is the count BEFORE --max-candidates
        # truncation, i.e. everything that survived the domain/score/feasibility filters
        "candidates_after_filters": int(pool_size),
...
def run_validate(args: argparse.Namespace) -> int:
    """Y-randomization (Y-scrambling) chance-correlation check.

    NEW in v1.4.0. A QSAR model can score well purely by chance when the dataset is
    small, highly correlated, or heavily tuned. The standard guard - required by
    OECD-aligned QSAR validation practice and absent from this skill until now - is
    to refit the SAME pipeline many times on SHUFFLED labels and confirm the real
    score sits outside the resulting null distribution.

    Reports an empirical p-value with the conventional add-one correction,
        p = (1 + #{null >= real}) / (1 + n_permutations),
    which is never 0 and stays valid for small n.

    Compute is bounded on purpose (a multi-model panel flagged refit cost as the main
    risk): labels are shuffled and refit on a capped subsample with a reduced CV, so
    the default run is cheap. Raise --n-permutations for a tighter p-value.
    """
    require_rdkit()
    from sklearn.base import clone
    ...
    frame = read_csv_checked(Path(args.input).expanduser().resolve(),
                             [args.smiles_column, args.target_column])
    # _canonicalize_list returns only the standardization columns (plus `_input_row`),
    # so the labels have to be joined back by original row index rather than assumed
    # to survive on the returned frame.
    labels_by_row = frame[args.target_column].reset_index(drop=True)
    valid, molecules, _ = _canonicalize_list(frame, args.smiles_column)
    if valid.empty:
        raise SystemExit("No parsable molecules in --input.")
    row_idx = valid["_input_row"].to_numpy()
    y_all = labels_by_row.iloc[row_idx].to_numpy()

    rng = np.random.default_rng(args.seed)
    if len(molecules) > args.max_samples:
        keep = rng.choice(len(molecules), size=args.max_samples, replace=False)
        keep.sort()
        molecules = [molecules[i] for i in keep]
        y_all = y_all[keep]

    X, _, _ = featurize_molecules(
        molecules,
        fp_radius=bundle["feature_spec"]["fp_radius"],
        fp_bits=bundle["feature_spec"]["fp_bits"],
    )
    y_raw = y_all

    classification = task == "classification"
    if classification:
        y, class_names = encode_class_labels(y_raw, None)
        scoring = args.scoring or "average_precision" if len(class_names) == 2 else "balanced_accuracy"
        if len(class_names) < 2:
            raise SystemExit(
                "Y-randomization needs at least two classes; this dataset has one. "
                "A chance-correlation test is undefined for a constant label."
            )
    else:
        y = np.asarray([float(v) for v in y_raw], dtype=float)
        class_names = []
        scoring = args.scoring or "r2"

    estimator = bundle["model"]
    folds = effective_cv_folds(y, args.cv_folds, classification)
    if folds < 2:
        raise SystemExit("Not enough samples per class for cross-validated Y-randomization.")
    cv, _ = make_inner_cv(
        split="random",
        y_train=y,
        groups_train=None,
    ...
    def _score(labels: np.ndarray) -> float:
        est = clone(estimator)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            scores = cross_val_score(est, X, labels, cv=cv, scoring=scoring,
                                     n_jobs=args.n_jobs, error_score=np.nan)
    ...
    for i in range(args.n_permutations):
        ...
        null_scores.append(_score(shuffled))
        if (i + 1) % max(1, args.n_permutations // 4) == 0:
            print(f"  ... {i + 1}/{args.n_permutations} permutations")

    attempted = int(args.n_permutations)
    null = np.asarray([v for v in null_scores if not np.isnan(v)], dtype=float)
    failed = attempted - int(null.size)
    if null.size == 0:
        raise SystemExit("Every permutation failed to score; cannot judge chance correlation.")
    if failed:
        # Silently dropping failed permutations would shrink the denominator and
        # misstate how much evidence the run actually gathered.
        print(f"  [!] {failed}/{attempted} permutations failed to score and were "
              f"excluded; p-value is based on {null.size} usable permutations.")

    n_ge = int(np.sum(null >= real_score))
    p_value = float((1 + n_ge) / (1 + null.size))
    null_mean, null_std = float(np.mean(null)), float(np.std(null))
    z = float((real_score - null_mean) / null_std) if null_std > 1e-12 else None

    # A permutation test can never report a p-value below 1/(n+1). With 12 shuffles
    # the floor is 0.077, so p <= 0.05 is UNREACHABLE no matter how strong the signal
    # is. Reporting "chance not excluded" in that situation would be a measurement
    # artefact presented as a scientific conclusion - exactly the kind of confident
    # wrong answer this skill exists to prevent. Detect it and say so instead.
    min_achievable_p = 1.0 / (1 + null.size)
    alpha_level = 0.05
    underpowered = min_achievable_p > alpha_level

    if underpowered:
        verdict = "UNDERPOWERED"
        interpretation = (
            f"Only {null.size} permutations were run, so the smallest reachable "
            f"p-value is {min_achievable_p:.3f} - above the {alpha_level} threshold. "
            f"This run CANNOT exclude chance correlation regardless of the observed "
            f"separation (z={z if z is None else round(z, 2)}). Re-run with "
            f"--n-permutations >= {int(np.ceil(1 / alpha_level)) - 1} to make the test "
            f"capable of a significant result."
        )
    elif p_value > alpha_level:
        verdict = "CHANCE_CORRELATION_NOT_EXCLUDED"
        interpretation = (
            "The real score is NOT clearly better than models trained on shuffled "
            "labels. Do not present this model as predictive. Treat any ranking or "
            "selection built on it as unsupported."
        )
    elif z is not None and z < 3.0:
        verdict = "WEAK_SEPARATION"
        interpretation = (
            "The real score beats the null distribution but by a small margin. "
            "Report the p-value alongside every performance claim."
        )
    else:
        verdict = "CHANCE_CORRELATION_EXCLUDED"
        interpretation = (
            "The real score lies well outside the shuffled-label null distribution, "
            "so measured performance is unlikely to be chance correlation. This does "
            "NOT establish generalization, applicability domain, or biological "
            "relevance - only that the signal is not random."
        )

    payload = {
        "schema": "qsar_validate.v1",
        "check": "y_randomization",
        "generated_at": utc_now(),
        "task": task,
        "scoring": scoring,
        "n_samples": int(len(y)),
        "n_permutations": int(null.size),
        "n_permutations_attempted": attempted,
        "n_permutations_failed": failed,
        "cv_folds": int(folds),
        ...
        "permutations_at_or_above_real": n_ge,
        "p_value": round(p_value, 6),
        "p_value_formula": "(1 + #{null >= real}) / (1 + n_permutations)",
        "min_achievable_p": round(min_achievable_p, 6),
        "underpowered": bool(underpowered),
        "z_score": round(z, 4) if z is not None else None,
        "verdict": verdict,
        "interpretation": interpretation,
        "limitations": (
            "Y-randomization tests only chance correlation on THIS data under THIS "
            "cross-validation. It says nothing about novel-chemotype generalization, "
            "applicability domain, calibration, or biological validity."
        ),
        "null_scores": [round(float(v), 6) for v in null],
    }
    output_dir = Path(args.output_dir).expanduser().resolve()
    output_dir.mkdir(parents=True, exist_ok=True)
    write_json(output_dir / "y_randomization.json", payload)
    print(json.dumps({k: v for k, v in payload.items() if k != "null_scores"}, indent=2))
    if verdict == "CHANCE_CORRELATION_NOT_EXCLUDED":
        return 4
...
OUTPUT_SCHEMAS: dict[str, dict[str, Any]] = {
    "qsar_validate.v1": {
        ...
        "$id": "qsar_validate.v1",
        "title": "Y-randomization chance-correlation report",
        "type": "object",
        "required": ["schema", "check", "real_score", "null_mean", "p_value",
                     "verdict", "underpowered", "interpretation", "limitations"],
        "properties": {
            "check": {"const": "y_randomization"},
            "real_score": {"type": "number", "description": "cross-validated score on TRUE labels"},
            "null_mean": {"type": "number", "description": "mean score over shuffled labels"},
            "p_value": {"type": "number",
                        "description": "(1 + #{null >= real}) / (1 + n_permutations); floor is 1/(n+1)"},
            "min_achievable_p": {"type": "number",
                                 "description": "1/(n+1). If above 0.05 the run cannot reach significance."},
            "underpowered": {"type": "boolean",
                             "description": "true = too few permutations; verdict carries NO evidence either way"},
            "verdict": {"enum": ["CHANCE_CORRELATION_EXCLUDED", "WEAK_SEPARATION",
                                 "CHANCE_CORRELATION_NOT_EXCLUDED", "UNDERPOWERED"]},
            "z_score": {"type": ["number", "null"]},
        },
        "exit_codes": {"0": "excluded or weak", "4": "chance not excluded", "5": "underpowered"},
    },
    "qsar_select.v1": {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "qsar_select.v1",
        "title": "Diversity-aware batch selection",
        "type": "object",
        "required": ["budget", "budget_requested", "candidate_pool",
                         "candidates_after_filters", "mean_pairwise_similarity"],
        "properties": {
            "budget": {"type": "integer", "description": "compounds actually returned"},
            "budget_requested": {"type": "integer"},
            "candidate_pool": {"type": "integer",
                               "description": "pool actually searched (after --max-candidates)"},
            "candidates_after_filters": {"type": "integer",
                                         "description": "survivors of domain/score/feasibility filters"},
            "budget_note": {"type": ["string", "null"],
                            "description": "set when the requested budget exceeded the pool"},
            "mean_pairwise_similarity": {"type": "number",
                                         "description": "lower = more diverse batch"},
        },
    },
    "qsar_train.v1": {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": "qsar_train.v1",
        "title": "Training report (metrics.json)",
        "type": "object",
        "properties": {
            "ad_threshold": {"type": "number"},
            "domain_threshold_degenerate": {
                "type": "boolean",
                "description": "true = threshold 0.0, every compound trivially in_domain; "
                               "in_domain is NOT evidence for this run"},
            "domain_warning": {"type": ["string", "null"]},
            "test_in_domain_fraction": {"type": "number"},
        },
    },
}
...
TRUST_CONTRACT = {
    "tool_verdicts": [
        "metrics.json", "y_randomization.json", "analysis.json",
        "conformal coverage", "applicability-domain labels",
    ],
    "never_evidence": [
        "a prediction score is not binding, efficacy, safety, mechanism, or clinical utility",
        "in_domain when domain_threshold_degenerate is true",
        "any verdict when underpowered is true",
        "feature importance as biological causality",
    ],
    "required_language": "say 'predicted candidate' / 'prioritized compound' / "
                         "'computational hit' - never 'discovery' before prospective experiments",
}


def run_schema(args: argparse.Namespace) -> int:
    """Emit machine-readable JSON Schemas for this CLI's outputs.

    NEW in v1.4.0. Lets any model - including one that never reads SKILL.md - parse
    and trust-rank the outputs from the contract alone, and see explicitly which
    fields may never be cited as evidence.
    """
    payload: dict[str, Any] = {
        "skill": "classical-ml-drug-discovery",
        "version": TOOL_VERSION,
        "schemas": OUTPUT_SCHEMAS if not args.name else {
            args.name: OUTPUT_SCHEMAS.get(args.name, {"error": f"unknown schema {args.name!r}",
                                                      "available": sorted(OUTPUT_SCHEMAS)})
        },
        "trust_contract": TRUST_CONTRACT,
    }
    print(json.dumps(payload, indent=None if args.compact else 2,
                     ensure_ascii=False, separators=(",", ":") if args.compact else None))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    select_parser.add_argument("--budget", type=int, default=None,
                               help="Batch size (>=1). Default: min(candidate pool, 100).")
    ...
    validate_parser = subparsers.add_parser(
        "validate",
        help="Y-randomization (Y-scrambling) chance-correlation check for a trained bundle.",
    )
    validate_parser.add_argument("--model", required=True)
    validate_parser.add_argument("--trust-model", action="store_true")
    validate_parser.add_argument("--input", required=True, help="Labeled CSV used for the check.")
    validate_parser.add_argument("--smiles-column", default="smiles")
    validate_parser.add_argument("--target-column", required=True)
    validate_parser.add_argument("--task", choices=["classification", "regression"])
    validate_parser.add_argument("--n-permutations", type=int, default=20,
                                 help="Label shuffles (>=5). More = tighter p-value, more compute.")
    validate_parser.add_argument("--max-samples", type=int, default=400,
                                 help="Subsample cap so the refits stay cheap.")
    validate_parser.add_argument("--cv-folds", type=int, default=3)
    validate_parser.add_argument("--scoring", help="sklearn scoring name (default: task-appropriate).")
    validate_parser.add_argument("--n-jobs", type=int, default=-1)
    validate_parser.add_argument("--seed", type=int, default=2026)
    validate_parser.add_argument("--output-dir", required=True)
    validate_parser.set_defaults(func=run_validate)

    schema_parser = subparsers.add_parser(
        "schema",
        help="Print JSON Schemas of this CLI's outputs plus the trust contract.",
    )
    schema_parser.add_argument("--name", help="Emit only this schema id.")
    schema_parser.add_argument("--compact", action="store_true",
                               help="Minified single-line JSON (fewer tokens).")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "budget", None) is not None and args.budget < 1:
        raise SystemExit(
            f"--budget must be at least 1 (got {args.budget}). A batch of zero or "
            f"fewer compounds is not a selection; omit --budget for the default."
        )
    if getattr(args, "n_permutations", None) is not None:
        if args.n_permutations < 5:
            raise SystemExit("--n-permutations must be at least 5 for a meaningful p-value.")
        if args.n_permutations < 19:
            print(f"  [!] --n-permutations {args.n_permutations}: the smallest reachable "
                  f"p-value is {1/(1+args.n_permutations):.3f}, so p <= 0.05 is impossible. "
                  f"Use >= 19 for a test that can reach significance.")
    if getattr(args, "max_samples", None) is not None and args.max_samples < 20:
        raise SystemExit("--max-samples must be at least 20.")
    ...
    if getattr(args, "alpha", None) is not None and not 0.0 < args.alpha < 1.0:
