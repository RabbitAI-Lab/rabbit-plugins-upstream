...
用法:
    ...
    agent.py text <文字>      # 输入文字 **（完美支持中文，使用 uiautomator2 Accessibility API）**
...
def _check_devices_raw():
    """直接调用 adb devices，避免递归"""
    try:
...
def is_device_connected():
    """检查设备是否连接"""
    out = _check_devices_raw()
    if not out:
        return False
    if not DEVICE_ID:
        return 'device' in out and 'unauthorized' not in out
...
def input_text(text):
    ...
    if not has_chinese:
        # 纯英文/数字，直接用 input text
        safe_text = text.replace(' ', '%s').replace('"', '\\"')
        _, _, code = adb(['shell', 'input', 'text', safe_text])
        return code == 0
    else:
        # 中文采用剪贴板+粘贴方案
        return input_chinese_text(text)

# 汉字到拼音的映射表（常用3500字）
# 来源：unicode.org 常用汉字拼音映射
# 仅包含 agent.py 中 text 命令需要的中文转拼音功能
_HANZI_TO_PINYIN = {
    ...
    '出': 'chu', '会': 'hui', '时': 'shi', '要': 'yao', '也': 'ye',
    '就': 'jiu', '那': 'na', '能': 'neng', '下': 'xia', '过': 'guo',
    '生': 'sheng', '自': 'zi', '年': 'nian', '得': 'de', '对': 'dui',
    '开': 'kai', '她': 'ta', '小': 'xiao', '里': 'li', '都': 'dou',
    '去': 'qu', '还': 'hai', '好': 'hao', '它': 'ta', '多': 'duo',
    '心': 'xin', '学': 'xue', '日': 'ri', '以': 'yi', '天': 'tian',
    '而': 'er', '家': 'jia', '可': 'ke', '发': 'fa', '如': 'ru',
    '道': 'dao', '作': 'zuo', '用': 'yong', '想': 'xiang', '看': 'kan',
    '只': 'zhi', '主': 'zhu', '被': 'bei', '公': 'gong', '同': 'tong',
    '让': 'rang', '为': 'wei', '他': 'ta', '很': 'hen', '做': 'zuo',
    '见': 'jian', '其': 'qi', '里': 'li', '所': 'suo', '者': 'zhe',
    '没': 'mei', '之': 'zhi', '把': 'ba', '样': 'yang', '方': 'fang',
    '手': 'shou', '机': 'ji', '电': 'dian', '话': 'hua', '微': 'wei',
    '信': 'xin', '蓝': 'lan', '汐': 'xi', '宝': 'bao', '贝': 'bei',
    '米': 'mi', '环': 'huan', '搜': 'sou', '索': 'suo', '打': 'da',
}

def _hanzi_to_pinyin(text):
    """中文字符转拼音（字母），非汉字字符保持原样"""
    result = []
    for c in text:
        if '\u4e00' <= c <= '\u9fff':
            p = _HANZI_TO_PINYIN.get(c, '')
            if p:
                result.append(p)
            else:
                # 未知汉字，用拼音首字母占位
...
def input_chinese_text(text):
    """输入中文：使用 uiautomator2 的 Accessibility API
    
    通过 uiautomator2 的 setText 方法，利用 Android Accessibility Service
    直接设置 EditText 的文本内容，完美支持中文。
    
    前置条件：
    - 当前焦点在可编辑的输入框上
    - uiautomator2 的 Accessibility Service 已启动
    """
    try:
        import uiautomator2 as u2
        d = u2.connect()
        
        # 获取当前焦点元素（focused=True 的输入框）
        focused = d(focused=True)
        if focused.exists and focused.count > 0:
            el = focused[0]
            el.click()
            time.sleep(0.3)
            el.set_text(text)
        ...
        # 如果无焦点，尝试找输入框
        # MIUI 设置的 ClearableEditText / 普通 EditText
        for cls in ['miuix.androidbasewidget.widget.ClearableEditText', 
                     'android.widget.EditText',
                     'android.widget.SearchView']:
            search = d(className=cls)
            if search.exists:
                search[0].click()
                time.sleep(0.3)
                search[0].set_text(text)
                return True
        
        # 最后尝试用 classNameMatches
        search = d(classNameMatches=".*EditText|miuix.*Edit.*|.*Search.*")
        if search.exists and search.count > 0:
            try:
                el = search[0]
                el.click()
                time.sleep(0.3)
        ...
        # uiautomator2 未安装，回退到拼音
        _input_pinyin_fallback(text)
        return True
    except Exception as e:
        print(f"uiautomator2 中文输入失败: {e}")
        return _input_pinyin_fallback(text)


def _input_pinyin_fallback(text):
    """拼音回退方案（uiautomator2 不可用时）"""
    try:
        from pypinyin import pinyin as pypinyin_fn, Style
        pinyins = pypinyin_fn(text, style=Style.NORMAL)
        pinyin_str = ''.join([p[0] for p in pinyins])
    except ImportError:
        pinyin_str = _hanzi_to_pinyin(text)
    
    if pinyin_str:
        safe_text = pinyin_str.replace(' ', '%s').replace('"', '\\"')
        _, _, code = adb(['shell', 'input', 'text', safe_text])
        return code == 0
    return False
