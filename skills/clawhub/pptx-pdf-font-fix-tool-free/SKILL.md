...
license: Proprietary
edition: free
description: PPT字体修复工具 - （免费版），可处理提升工作效率
...
PPT字体修复工具是针对文档处理领域的专业化AI辅助工具。免费版面向个人用户，提供核心功能与基础用法，适合快速上手和日常使用.
本工具经过深度优化，增强元数据和适用关键词，完全适配SkillHub平台规范.
## 核心能力

字体检测、字体嵌入、字体替换、PDF转换修复、批量处理、字体映射
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：PPT、时的字体缺失与乱、码问题、支持字体嵌入、替换与批量处理、字体修复工具、免费版等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
将PPT转为PDF并确保字体正确显示。**示例指令**：`

`把PPT转为PDF，确保字体正确

### 场景3：批量字体修复

批量修复多个PPT文件的字体问题。**示例指令**：`
...
| 参数名 | 类型 | 必填 | 说明 |
|---|---|---|---|
| input | string | 是 | PPT字体修复工具（免费版）处理的输入数据或指令 |
...
def check_fonts(pptx_path):
    ...
    return fonts
# ...
found_fonts = check_fonts("presentation.pptx")
print(f"使用的字体: {found_fonts}")
print(f"字体数量: {len(found_fonts)}")
```
...
| libreoffice | 系统工具 | 可选 | PDF转换需要LibreOffice |
...
```json
{
  "success": true,
  "data": {
    "result": "PPT字体修复工具（免费版）处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "pptx pdf font fix"
    }
  },
