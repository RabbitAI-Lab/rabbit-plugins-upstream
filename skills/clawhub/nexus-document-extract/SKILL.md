...
> NEXUS Agent-as-a-Service on Cardano + Stellar + XRPL | Price: $0.30/request | Accepts: ADA, DJED, iUSD, USDCx, USDM (Cardano) + USDC, XLM (Stellar) + XRP, RLUSD (XRPL)
...
curl -X POST https://ai-service-hub-15.emergent.host/api/original-services/document-extract \
  ...
  -d '{
  "input": "Example input for Document Extractor"
}'
```
