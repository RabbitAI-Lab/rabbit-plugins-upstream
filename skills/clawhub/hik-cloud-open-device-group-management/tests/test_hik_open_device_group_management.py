...
MODULE_PATH = (
    ...
    / "scripts"
    / "hik_open_device_group_management.py"
)
SPEC = importlib.util.spec_from_file_location("hik_open_device_group_management", MODULE_PATH)
...
class HikOpenDeviceGroupManagementTests(unittest.TestCase):
    ...
    def test_build_create_spec_uses_json_contract(self):
        args = MOD.parse_args(
            ["create", "--group-name", "name", "--group-no", "712", "--parent-no", "7"]
        )
        spec = MOD.build_create_spec(args)
        self.assertEqual(spec.method, "POST")
        self.assertEqual(spec.path, MOD.CREATE_GROUP_PATH)
        self.assertEqual(spec.json_body["groupName"], "name")
        self.assertEqual(spec.json_body["groupNo"], "712")
        self.assertEqual(spec.json_body["parentNo"], "7")

    def test_build_delete_spec_uses_query_contract(self):
        args = MOD.parse_args(["delete", "--group-no", "712"])
        spec = MOD.build_delete_spec(args)
        self.assertEqual(spec.method, "POST")
        self.assertEqual(spec.query["groupNo"], "712")

    def test_build_list_children_spec_omits_parent_no_when_not_set(self):
        args = MOD.parse_args(["list-children"])
        spec = MOD.build_list_children_spec(args)
        self.assertEqual(spec.method, "GET")
        self.assertEqual(spec.path, MOD.LIST_CHILDREN_PATH)
        self.assertIsNone(spec.query)

    def test_build_device_transfer_spec_uses_json_contract(self):
        args = MOD.parse_args(
            [
                "device-transfer",
                "--device-serial",
                "ABC1234567",
                "--target-group-id",
                "bc441199bec54f5a8d09b1b1c88c413d",
            ]
        )
        spec = MOD.build_device_transfer_spec(args)
        self.assertEqual(spec.path, MOD.DEVICE_TRANSFER_PATH)
        self.assertEqual(spec.json_body["deviceSerial"], "ABC1234567")
        self.assertEqual(spec.json_body["targetGroupId"], "bc441199bec54f5a8d09b1b1c88c413d")
