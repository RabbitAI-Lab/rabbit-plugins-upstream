---
name: amazon-market-entry-brief
description: >
  根据主 ASIN 与已授权竞品的商品页和评论证据，整理市场进入前的页面差异、用户疑问和验证清单。仅用于进入前研究；不用于市场规模、销量、利润、广告、库存、订单或退货率预测。
  Requires an ARI API key (ari_live_*).
slug: market-entry-brief
displayName: Amazon 市场进入对照简报
version: 1.4.7
summary: 用商品页与评论证据整理进入前的对照问题
license: MIT
---
...
- 本入口只执行 `page_compare/entry`，输出模板固定为 `ops_page_compare`。
