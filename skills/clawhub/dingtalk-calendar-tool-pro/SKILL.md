...
version: 1.0.0
displayName: 钉钉日历专业版
summary: "企业级钉钉日历管理,支持批量日程操作、组织架构集成与会议资源优化。核心能力: 日程管理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 日程管理领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。 功能涵盖: dingtalk。"
...
**处理**: 解析批量日程创建与会议邀请自动化的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量日程创建与会议邀请自动化的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 组织架构集成
组织架构集成:按部门批量查询闲忙

**处理**: 解析组织架构集成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回组织架构集成的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 会议室资源优化
会议室资源优化:智能推荐最优会议室

**处理**: 解析会议室资源优化的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回会议室资源优化的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 重复日程与周期会议批量管理
重复日程与周期会议批量管理

**处理**: 解析重复日程与周期会议批量管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回重复日程与周期会议批量管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日程数据分析
日程数据分析:会议时间分布与成本统计

**处理**: 解析日程数据分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回日程数据分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级钉钉日历管、支持批量日程操作、组织架构集成与会、议资源优化、核心能力、日程管理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 钉钉日历专业版处理的输入数据或指令 |
...
```json
{
  ...
  "data": {
    ...
    "metadata": {
      "version": "1.0",
      "processor": "dingtalk calendar pro"
    }
  },
...
- **自动化执行**: 企业级钉钉日历管理,支持批量日程操作、组织架构集成与会议资源优化。核心能力: 日程管理领域的专业化 AI 辅助工具,提供
