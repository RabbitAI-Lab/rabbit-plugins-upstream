# PoYo GPT-5.5 Chat Completions API Reference
...
- `gpt-5.5`: OpenAI-compatible chat completion model on PoYo.
...
curl --fail-with-body --request POST \
  ...
  --data '{
    ...
    "messages": [
      {
        "role": "system",
        "content": "You are a careful backend reviewer. Return concise findings first."
      },
      {
        "role": "user",
        "content": "Review this API handler for missing validation and error handling."
      }
    ],
    "temperature": 0.2,
    "max_tokens": 700
...
- Avoid logging private user messages, API keys, raw request bodies, or raw authorization headers.
