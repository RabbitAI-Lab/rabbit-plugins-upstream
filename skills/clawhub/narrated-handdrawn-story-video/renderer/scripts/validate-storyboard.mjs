...
const files = process.argv.slice(2).filter((value) => value !== '--strict-assets');
...
const validate = (file) => {
  ...
  if (project.opening_poster) {
    const poster = project.opening_poster;
    if (!poster.asset || !poster.tag || !poster.title || !poster.synopsis || !poster.takeaway) {
      errors.push('opening_poster requires asset, tag, title, synopsis, and takeaway');
    }
    const posterPath = resolve(root, 'public', poster.asset || '');
    if (!existsSync(posterPath) && strictAssets) {
      errors.push(`opening_poster: missing asset at public/${poster.asset}`);
    } else {
      const dimensions = pngDimensions(posterPath);
      if (dimensions && Math.abs(dimensions.width / dimensions.height - project.width / project.height) > 0.001) {
        errors.push(`opening_poster: asset ratio must match ${project.width}x${project.height}`);
      }
    }
  }
  ...
  for (const scene of storyboard.scenes) {
    ...
    for (const [key, path] of Object.entries(scene.assets)) {
      ...
      if (!existsSync(absolute) && strictAssets) {
