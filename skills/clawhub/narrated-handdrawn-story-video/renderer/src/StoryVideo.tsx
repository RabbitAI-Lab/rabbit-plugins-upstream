...
import {openingPosterFramesFor, storyboard, totalFramesFor, transitionFramesFor} from './storyboard';
...
const OpeningPoster: React.FC<{value: Storyboard}> = ({value}) => {
  const poster = value.project.opening_poster;
  const {width, height} = useVideoConfig();
  if (!poster) return null;

  return (
    <AbsoluteFill style={{backgroundColor: '#fdfaf2', overflow: 'hidden'}}>
      <Img
        src={staticFile(poster.asset)}
        style={{width: '100%', height: '100%', objectFit: 'cover'}}
      />
      <AbsoluteFill
        style={{
          alignItems: 'flex-start', justifyContent: 'flex-start',
          paddingTop: height * 0.085, paddingLeft: width * 0.085,
          color: '#382e27', textAlign: 'left',
        }}
      >
        <div style={{padding: `${height * 0.007}px ${width * 0.018}px`, border: '1px solid rgba(143,87,62,.52)', borderRadius: 999, backgroundColor: 'rgba(255,252,244,.72)', fontFamily: 'system-ui,sans-serif', fontSize: width * 0.021, fontWeight: 600, letterSpacing: 2.5, color: '#8f573e'}}>{poster.tag}</div>
        <div style={{marginTop: height * 0.018, fontFamily: 'Ma Shan Zheng,serif', fontSize: width * 0.101, lineHeight: 1, letterSpacing: 5, textShadow: '0 2px 10px rgba(255,255,255,.95)'}}>{poster.title}</div>
        <div style={{width: width * 0.115, height: 4, marginTop: height * 0.019, backgroundColor: '#a55a3b', borderRadius: 99}} />
        <div style={{marginTop: height * 0.018, maxWidth: width * 0.56, whiteSpace: 'pre-line', fontFamily: 'system-ui,sans-serif', fontSize: width * 0.028, fontWeight: 600, letterSpacing: 1.1, lineHeight: 1.45, color: '#5e493b'}}>{poster.synopsis}</div>
        <div style={{marginTop: height * 0.018, maxWidth: width * 0.58, paddingLeft: width * 0.018, borderLeft: '3px solid #c58a55', whiteSpace: 'pre-line', fontFamily: 'system-ui,sans-serif', fontSize: width * 0.022, letterSpacing: 1.2, lineHeight: 1.5, color: '#8a6d59'}}>{poster.takeaway}</div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
};
...
export const StoryboardVideo: React.FC<{value: Storyboard}> = ({value}) => {
  const posterFrames = openingPosterFramesFor(value);
  const story = value.project.transition === 'page-flip' && value.scenes.length > 1
    ? <PageFlipStoryVideo value={value} />
    : <CutStoryVideo value={value} />;
  if (!posterFrames) return story;
  return <Series><Series.Sequence durationInFrames={posterFrames} name="Opening poster"><OpeningPoster value={value} /></Series.Sequence><Series.Sequence durationInFrames={totalFramesFor(value)} name="Story">{story}</Series.Sequence></Series>;
};
