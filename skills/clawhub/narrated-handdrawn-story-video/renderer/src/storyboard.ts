...
export const openingPosterFramesFor = (value: Storyboard) =>
  value.project.opening_poster
    ? Math.max(1, Math.round((value.project.opening_poster.duration_sec ?? 3) * value.project.fps))
    : 0;
...
export const openingPosterFrames = openingPosterFramesFor(storyboard);
