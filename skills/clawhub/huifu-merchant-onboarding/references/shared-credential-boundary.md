...
- `sys_id`、`product_id` 和 RSA 私钥只从服务端安全配置读取。`skill_source` 优先读取服务端显式配置；缺失时按请求头策略生成 `hfms/1.0.2` 或双 Skill 组合值 `hfps/1.3.5;hfms/1.0.2`。
...
- `jpt-x-skill-source` 使用已确认合同值；未配置时按当前请求实际参与生成的 Skill 集合取值：仅进件为 `hfms/1.0.2`，支付与进件都实际参与时为 `hfps/1.3.5;hfms/1.0.2`；仅安装未参与的不计入。
