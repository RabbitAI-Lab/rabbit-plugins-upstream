...
- `head_office_flag=0` 表示下级商户；必须按对应总部 `head_type` 补材料：集团客户 F725，大型商业综合体 F726，品牌连锁客户按加盟/直营提交 F727。个人商户接口本身没有 `head_type` 请求字段，不能为满足说明而补造该键。
...
- 个人基础进件请求没有 `data.huifu_id`，按官网字段合同生成，不因 SDK 内部请求头增加该业务字段。
