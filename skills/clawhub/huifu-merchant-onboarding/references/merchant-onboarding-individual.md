...
- `data` 使用当日唯一的 `req_seq_id`、`req_date`。`upper_huifu_id:String(18) C`：`sys_id` 主体为渠道商时填写真实渠道商号，总部商户主体时选填；其余资料按负责人、证件、经营信息、联系人、结算卡、结算/取现规则分组校验。
- `scene_type=OFFLINE` 或 `ALL` 时必须提供经营详细地址。身份证、银行卡和门店图片字段只接收图片上传后的文件标识。
- `store_header_pic`、`store_indoor_pic`、`store_cashier_desk_pic` 均为 `String(64) C`，`scene_type` 含线下场景时必填。
- `card_info`、`settle_config`、`cash_config` 是 String(JSON) 边界：先建对象、校验条件字段、统一序列化；不直接透传未经校验的字符串。
- `data.prov_id/area_id` 是经营省市，`data.card_info.prov_id/area_id` 是银行所在省市。不得按 `prov_id/area_id` 叶字段名去重；适用时两组都保留，并把银行卡省市序列化在 `card_info` 内。
...
- **按语言区分处理**：PHP `2.0.30` 在 `MerConfig.skill_source` 非空时直接读取请求参数中的 `huifu_id`，个人进件又没有该字段；这会产生未定义键 warning/空头风险。无论运行环境是否把 warning 提升为异常，本 Skill 都阻断对应 PHP 可运行代码；必须使用已修复 SDK，不能添加虚构/空业务字段、关闭 warning 或吞异常。Python `2.0.24` 会发送空的 `jpt-x-skill-huifu_id`，但这只是 SDK 兼容性告警，不单独阻断个人进件代码。保持业务报文不含 `data.huifu_id`，单列 `SDK 兼容性提示`，在联调/上线前验证网关结果；若因空头被拒绝，再升级或修复 SDK。不得询问客户“是否允许空请求头”。
...
Java `3.0.40`、PHP `2.0.30`、Python `2.0.24` 都有 `V2MerchantBasicdataIndvRequest` 作为可选封装。它只证明 URI、字段序列化和 SDK 默认的请求/同步响应处理路径；主体判断、材料条件和渠道准入仍以本页和官方联调为准。每次调用显式传入新的当天唯一 `req_seq_id` / `req_date`，Python 不会自动生成。
