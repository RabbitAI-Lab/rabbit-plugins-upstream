...
Java `3.0.40`、PHP `2.0.30`、Python `2.0.24` 都有 `V2MerchantBusiOpenRequest` 作为可选封装，并覆盖 `sign_user_info`、`agreement_info` 等 String(JSON) 边界。它不替代支付业务、费率、协议、实名和渠道权限的人工确认；每次调用显式传入新的当天唯一 `req_seq_id` / `req_date`。
