...
Java `3.0.40`、PHP `2.0.30`、Python `2.0.24` 都有 `V2MerchantBasicdataStatusQueryRequest` 作为可选封装，字段定位到 `req_seq_id`、`req_date`、`apply_no`、`huifu_id`。Python 不会自动生成 `req_seq_id` / `req_date`；三个 SDK 的默认同步验签路径也不替代本页的状态分层解释。
