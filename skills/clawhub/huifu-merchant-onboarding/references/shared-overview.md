...
当前字段快照核验于 `2026-07-29`，十一接口共 2,111 个字段路径。企业进件相对 `2026-07-28` 快照新增请求 `material_card_info` 七个路径，其余十页逐字节一致；此前同步的 `upper_huifu_id:String(18) C`、门店图片条件和详情响应 `material_card_info` 继续保留。官网 URL 只用于来源追溯和维护刷新，不计入普通回答的本轮 references。
