---
name: finxdata
description: 当用户需要查询 FinXData 金融数据 API 时使用本技能，包括股票行情、股票图谱、财务报表、市场新闻、龙虎榜、限售解禁、宏观经济、FRED、异动追踪、额度、更新频率、请求限速与重试、API Key 配置、错误处理或服务健康状态。本技能调用与 FinXData MCP 工具相同的公开 HTTP 接口。
---
...
- 收到 429 后立即停止该接口的后续调用，不通过切换命令、代码或 `agent-type` 规避限制。优先遵守响应中的 `Retry-After`；没有该字段时，本轮不自动重试，向用户说明稍后再试。
