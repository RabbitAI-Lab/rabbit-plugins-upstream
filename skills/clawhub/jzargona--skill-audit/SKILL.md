...
## Manual Fallback (when skilllens is unavailable)

If `skilllens` is not installed, perform a manual audit:

1. **Inventory skills**: `find <path> -name "SKILL.md"`
2. **Scan each SKILL.md** for the 6 risk categories:
   - **Exfiltration**: `exec`, `subprocess`, `os.system`, `curl|sh`, base64, wget — sends data out
   - **Execution**: `eval`, `exec`, shell injection, fetch-and-execute patterns
   - **Persistence**: cron, launch agent, shell profile, ~/.config mod
   - **Privilege bypass**: `sudo`, `chmod 777`, approval bypass, policy override
   - **Prompt injection**: "ignore previous", "always comply", "never reveal"
   - **Overbroad trigger**: vague description that fires on unrelated requests
3. **Rate each skill**: Safe (0 exec, 0 exfil) → Suspicious (some exec) → Unsafe (direct exfil or credential access)
4. **Report**: name, path, verdict, risk 0–100, evidence quotes, fix recommendations
