...
description: >-
  ...
  discovering an activity (one with is_bookable true and ticket pricing) when
  the user wants to purchase tickets or complete a booking ("book 2 tickets
  to that show", "get us seats for Friday", "buy the General Admission
  passes"). Bookings are async and resolve via webhook. Powered by Outgoing
...
general **activity-discovery-and-booking**) — you need its `activity_id` and exact
ticket type/session from `ticket_price`. Only activities with
...
- `Authorization: Bearer $OUTGOING_API_KEY` on every call (or an AAuth token). By
  default the booking is made for the user bound to your key; pass
...
| `external_user_id` | no | Book for a specific provisioned user instead of the default user bound to your key. |
...
> Need something to book? Find it first with the **activity-discovery-and-booking**
> skill (or any of its occasion-specific companions).
