...
name: dining
description: >-
  Find restaurants and places to eat — by cuisine, vibe, neighborhood, or
  occasion — via natural-language search. Use when the user wants a restaurant
  or somewhere to eat ("where should we eat tonight", "best sushi nearby",
  "brunch this weekend", "a cozy Italian spot"). Powered by Outgoing
...
# Dining — powered by Outgoing

Find restaurants and places to eat.

## When to use

The user wants somewhere to eat. Triggers: "restaurant", "where to eat",
"dinner", "lunch", "brunch", "best [cuisine] near me", "a place for dinner
tonight".
...
Lead with the meal and capture cuisine, vibe, budget, group size, and
neighborhood. Note timing (tonight, this weekend) when known.

- "lively spot for dinner tonight — good cocktails, not too loud"
- "best sushi near downtown, mid-range"
- "cozy brunch this weekend that takes a group of six"
...
curl -sS -G https://api.outgoing.world/partner/v1/search \
  ...
  --data-urlencode "prompt=great dinner spots for tonight — lively but easy to talk" \
...
- `city` — an optional hint to ground the search if the query doesn't say
  ...
  needed (e.g. "sushi in the West Village") — the query overrides `city`.
...
To reserve a table, use the **make-reservation** skill (`POST /partner/v1/bookings`).
