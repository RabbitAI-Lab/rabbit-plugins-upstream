...
version: 1.0.0
displayName: 飞书日历专业版
summary: "企业级飞书日历管理,支持共享日历、多人协作、事件同步与智能调度。核心能力: 日程管理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 日程管理领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要AI模型调用、智能对话、Agent编排、LLM应用时使用。不适用于需要100%确定性的关键决策。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。 功能涵盖: feishu。"
...
本工具是 **日程管理** 领域的 **PRO 版本** AI Skill,专为企业级场景与专业用户设计。通过自然语言指令驱动 AI Agent 执行任务,提供高级功能、批量操作与团队协同.
...
**处理**: 解析共享日历创建与成员权限管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回共享日历创建与成员权限管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多人协作日历与事件共享
多人协作日历与事件共享

**处理**: 解析多人协作日历与事件共享的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多人协作日历与事件共享的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 跨时区日程同步与自动转换
跨时区日程同步与自动转换

**处理**: 解析跨时区日程同步与自动转换的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回跨时区日程同步与自动转换的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 智能调度
智能调度:自动寻找团队空闲时段

**处理**: 解析智能调度的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回智能调度的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日历事件 Webhook 通知
日历事件 Webhook 通知与自动化

**处理**: 解析日历事件 Webhook 通知的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回日历事件 Webhook 通知的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：企业级飞书日历管、支持共享日历、事件同步与智能调、核心能力、日程管理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 飞书日历专业版处理的输入数据或指令 |
| options | object | 否 | 附加配置选项,如模式选择、格式偏好等 |
| callback_url | string | 否 | 异步处理完成后的回调通知URL |

```bash
node skills/feishu-calendar/create.js --summary "Task: 完成周报" --desc "周五前提交" --start "2026-02-07T10:00:00+08:00" --end "2026-02-07T11:00:00+08:00" --attendees "ou_未指定"
```
...
- **自动化执行**: 企业级飞书日历管理,支持共享日历、多人协作、事件同步与智能调度。核心能力: 日程管理领域的专业化 AI 辅助工具,提供企
