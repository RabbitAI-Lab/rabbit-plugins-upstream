---
name: amazon-mobile-listing-optimizer
description: >
  结合商品详情与评论证据，检查 Amazon Listing 在移动端的阅读顺序、信息密度和理解阻碍。仅用于移动端表达诊断；不用于广告、库存或自动发布。
  Requires an ARI API key (ari_live_*).
slug: mobile-listing
displayName: Amazon 移动端 Listing 优化
version: 1.4.7
summary: 检查移动端阅读路径和信息优先级
license: MIT
---
...
- 本入口只执行 `listing/mobile`，输出模板固定为 `ops_listing`。
