---
name: smart-tender-procurement-search
description: 招标采购信息检索服务，按关键词、地区、金额、时间、行业等多维度检索全网招标公告与采购信息，支持高级逻辑（关键词分组、排除词）、获取标讯完整详情、查询临期周期性项目。当用户需要查招标公告、找采购信息、检索标讯、获取标书详情、跟踪某地区或行业项目动态时，必须使用此SKILL。
---

# 招标采购信息智能查询 - 全网标讯一键检索

聚焦“查得全、查得准”。覆盖全国招标公告与采购信息，支持关键词、地区、金额、时间、行业多维筛选与高级逻辑检索（分组、排除词），快速定位目标标讯并获取完整正文。
...
本助手专注“把目标标讯快速、精准地捞出来”，常见用法：

- **精准检索**："查华东地区 1000 万以上、近三个月的医疗设备招标" → `query_bids_advanced` 组合关键词 + 地区 + 金额 + 时间。
- **排除干扰**："要‘数据中心’相关、但排除‘维保’的项目" → `query_bids_advanced` 关键词分组 + 排除词。
- **取全文**："把这条公告的完整招标文件内容给我" → `get_bid_detail`。
- **盯周期项目**："哪些周期性采购项目快到期了" → `search_expiring_projects`。
...
| INSUFFICIENT_BALANCE / QUOTA_EXCEEDED | **判断 API Key 来源**：<br>① 来自 `~/.zlbx/config.json` 且 `source == "auto"` → 调用 `POST /web-api/auth/generate-device-sid`（带 `X-API-Key` Header）拿到 `sid`，输出充值链接 `https://ai.zhiliaobiaoxun.com/auto-login?sid=<sid>`，提示文案：「免费额度已用完，点击链接自动登录并充值；首次会引导绑定手机号，绑定即送 100 次」<br>② 来自 `$ZLBX_API_KEY` → 提示访问 `https://ai.zhiliaobiaoxun.com/?ch=s132` 手动登录充值（不输出自动登录链接） |
