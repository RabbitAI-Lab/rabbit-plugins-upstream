"""CLI argument parsing tests for land-degradation-assessment."""
import subprocess
import sys
import os
...
class TestCLIBasics:
    ...
    def test_bbox_only_runs(self, tmp_path):
        """Only --bbox → auto synthetic → exit 0 + manifest."""
    ...
    def test_synthetic_mode_runs(self, tmp_path):
        ...
        r = run_cli([
            ...
            "--n-dates", "4", "--output-dir", out, "--quiet",
        ])
        assert r.returncode == 0, r.stderr
        assert os.path.exists(os.path.join(out, "degradation.tif"))
