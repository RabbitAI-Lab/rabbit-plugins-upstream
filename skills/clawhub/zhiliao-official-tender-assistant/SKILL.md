...
description: 知了标讯官方招投标数据助手，覆盖招标公告与中标结果查询、企业工商与招中标画像、竞争对手分析、市场趋势统计、Top采购/中标单位与品牌、历史中标价格、临期项目商机挖掘等。当用户涉及招投标、政府采购、中标查询、供应商/竞对分析、采购市场研究等任何场景时，必须使用此SKILL；即使未出现「招投标」字样，只要涉及中标、采购、供应商、竞对、市场份额等需求均应使用。
---

# 知了标讯官方招投标助手 - 全网招中标数据查询与分析

知了标讯官方出品的招投标数据助手，一站式覆盖全网招标公告、中标结果、企业工商画像、市场聚合分析、竞争对手识别、历史中标价格等数据能力。查标讯、做尽调、析竞对、挖商机，统一调用官方数据接口，给出可信、结构化的结果。
...
知了标讯官方助手覆盖招投标全流程数据需求，常见用法：

- **查标讯**："查近一个月广东省 500 万以上的服务器采购公告" → `search_bids` / `query_bids_advanced` 多维筛选，`get_bid_detail` 取完整正文。
- **企业尽调**："分析华为在政府采购市场的招中标表现" → `get_company_profile` + `get_company_business_keywords` + `find_competitors`。
- **市场洞察**："这个行业谁在买、谁在中标、价格怎么走" → `get_top_purchasers` + `get_top_suppliers` + `aggregate_bids_advanced` + `get_price_trends`。
- **商机挖掘**："找未来三个月即将到期、可能续标的项目" → `search_expiring_projects` + `find_potential_bidders`。
...
| INSUFFICIENT_BALANCE / QUOTA_EXCEEDED | **判断 API Key 来源**：<br>① 来自 `~/.zlbx/config.json` 且 `source == "auto"` → 调用 `POST /web-api/auth/generate-device-sid`（带 `X-API-Key` Header）拿到 `sid`，输出充值链接 `https://ai.zhiliaobiaoxun.com/auto-login?sid=<sid>`，提示文案：「免费额度已用完，点击链接自动登录并充值；首次会引导绑定手机号，绑定即送 100 次」<br>② 来自 `$ZLBX_API_KEY` → 提示访问 `https://ai.zhiliaobiaoxun.com/?ch=s131` 手动登录充值（不输出自动登录链接） |
