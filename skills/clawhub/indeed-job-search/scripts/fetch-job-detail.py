...
def main():
    ...
    # Security: validate jobkey format before injecting into JS string.
    # Indeed job keys are lowercase hex strings (typically 16 chars).
    # Reject anything outside [a-f0-9] to prevent JS injection.
    if not re.fullmatch(r'[a-f0-9]{8,32}', jobkey):
        print('echo "Error: invalid job key format (expected hex string 8-32 chars)"')
        sys.exit(1)
