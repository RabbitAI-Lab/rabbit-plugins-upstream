...
description: "Search and extract job listings from Indeed.com by keyword, location, and country. Returns structured data: job title, company, salary, rating, description, benefits, and apply links. Use when user explicitly mentions Indeed, Indeed jobs, Indeed job listings, scrape Indeed, extract Indeed data, Indeed job data, get jobs from Indeed, Indeed salary data, Indeed job scraper, Indeed hiring data, job postings Indeed, Indeed career search, bulk job extraction Indeed."
---
...
This Skill requires the following capabilities — no others:

| Capability | Reason |
|---|---|
| `browser-act navigate` | Open Indeed search and job pages |
| `browser-act eval` | Run JS to extract job data from page context |
| `browser-act wait` | Wait for page stability before extraction |
| `browser-act solve-captcha` | Resolve security check pages that block access |
| Read file: `browser-act-skill-forge-memories/indeed-jobs-scraper-indeed-job-search.memory.md` | Read past execution notes (optional, read-only at start) |
| Write file: `browser-act-skill-forge-memories/indeed-jobs-scraper-indeed-job-search.memory.md` | Append experience notes only when explicitly triggered (see Experience Notes) |

No other tools, network endpoints, or filesystem paths are accessed.
...
Invoke `browser-act` via Skill tool to load usage and confirm it is available. If installation or configuration issues arise, follow its guidance to resolve then retry. Do not skip this step.

### 2. Login Verification (for pagination beyond page 1)

Open Indeed and observe the page login status each time before executing paginated requests:
...
- **Batch orchestration**: Write a bash script to loop through the command templates serially within a single session; do not parallelize within one browser (prone to triggering anti-scraping restrictions). Add 2-3 second intervals between detail fetches.
...
**After execution**: Write to this file ONLY when an unexpected situation is encountered (strategy became ineffective, page redesigned, anti-scraping upgraded, better path discovered) AND the user has been informed and has not objected. Append a single line:
`{YYYY-MM-DD}: {what happened} → {conclusion}`

Do not write to any other file path. Normal execution (no unexpected situations) does not write to the file. Do not record what keywords were used or how many results were returned — those are task outputs, not experience.
