...
Built from a real deployment: across five review rounds on a Vercel-hosted Next.js PR, Copilot caught the line-level issues while the subagents caught the P0 architectural miss (`maxDuration`) Copilot missed in every round, plus a persistent-replay vector and four operational issues. If you have access to a paid multi-agent review service, this skill is the in-flight free version of the same idea.
