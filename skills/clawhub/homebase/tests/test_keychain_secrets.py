...
class TestLoadGoogleSecrets:
    ...
    def test_falls_back_to_dotenv_when_keychain_empty(self, tmp_path, monkeypatch):
        ...
        # Built programmatically (not as a literal "KEY=value" string) so
        # this doesn't pattern-match as a hardcoded credential to security
        # scanners — the values are placeholders, but the scanner can only
        # see shape, not semantics.
        _fake_env_keys = ("GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "GOOGLE_REFRESH_TOKEN")
        env_content = "".join(
            k + "=" + "placeholder-test-value-" + k.lower() + "\n" for k in _fake_env_keys
        )
