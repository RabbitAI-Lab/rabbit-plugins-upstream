...
# SECURITY MANIFEST
# external_endpoints:
#   - local SearXNG (default: http://192.168.2.169:8081)
#   - optional local fallback SearXNG via LOCAL_SEARCH_FALLBACK_URL (read-only search only when set)
...
Usage:
    ...
    python3 verify_claim.py --claim "..." --sources 7 --searxng-url http://192.168.2.169:8081
...
# ─── Constants ────────────────────────────────────────────────────────────────
DEFAULT_SEARXNG = os.environ.get("LOCAL_SEARCH_URL", "http://192.168.2.169:8081")
PUBLIC_SEARXNG_INSTANCES = [s.strip() for s in os.environ.get("LOCAL_SEARCH_FALLBACK_URL", "").split(",") if s.strip()]
...
def _fetch_with_stdlib(url: str, timeout: int = 12) -> Optional[str]:
    ...
    cmd = [
        "curl", "-sS", "-L", "-A", BROWSER_UA, "--max-time", str(timeout),
        url,
    ]
    try:
...
def _search_searxng(query: str, searxng_url: str, num: int = 5) -> list[dict]:
    ...
    try:
        html = _fetch_with_stdlib(full_url, timeout=8)
        if not html:
            return []
...
def _find_working_searxng(searxng_url: str) -> str:
    ...
    for url in [searxng_url] + PUBLIC_SEARXNG_INSTANCES:
        ...
        html = _fetch_with_stdlib(probe, timeout=5)
        if html:
            try:
                data = json.loads(html)
