...
#   External endpoints called: default local SearXNG at 192.168.2.169:8081, optional fallback via LOCAL_SEARCH_FALLBACK_URL
...
============================================================
Multi-engine parallel search via local SearXNG, optional fallback URL.
...
Features:
  ...
  - Fallback chain     : local SearXNG first, then optional LOCAL_SEARCH_FALLBACK_URL, then graceful error
...
# ─── Configuration ────────────────────────────────────────────────────────────
DEFAULT_LOCAL_URL   = os.environ.get("LOCAL_SEARCH_URL",          "http://192.168.2.169:8081")
PUBLIC_FALLBACK_URL = os.environ.get("LOCAL_SEARCH_FALLBACK_URL", "")
...
ALL_ENGINES = ["bing", "duckduckgo", "google", "reddit", "startpage", "qwant"]

# Google web currently serves CAPTCHA/enable-JS interstitials from the local
# SearXNG egress path, which looks like a successful HTTP 200 with zero
# parseable results. Keep it available for explicit --engines overrides, but do
# not select it by default.
INTENT_ENGINE_MAP = {
    "factual":    ["bing", "duckduckgo", "startpage"],
    "news":       ["bing", "duckduckgo", "reddit", "startpage"],
    "research":   ["bing", "duckduckgo", "startpage"],
    "tutorial":   ["bing", "duckduckgo", "startpage"],
    "comparison": ["bing", "duckduckgo", "reddit"],
    "privacy":    ["duckduckgo", "startpage", "qwant"],
    "general":    ["bing", "duckduckgo", "startpage"],
}
...
def _fetch_single_stdlib(base_url: str, query: str, engine: str,
                          limit: int, freshness: str) -> list:
    """stdlib fallback via local curl command (urllib against this host is unreliable in this environment)."""
    params = {
        "q": query, "format": "json", "language": "en-US",
        "pageno": "1", "engines": engine,
    }
    ...
    try:
        proc = subprocess.run(cmd, capture_output=True, check=False, text=True, timeout=20)
...
def main() -> int:
    ...
    parser.add_argument("--engines", default=None,
                        help="Override engines (comma-separated): bing,duckduckgo,google,reddit,startpage,qwant")
    ...
    if not results:
        root = _project_root()
        print("ERROR: No local results returned.", file=sys.stderr)
        print(f'Check local SearXNG access: {DEFAULT_LOCAL_URL}', file=sys.stderr)
        print(f'Try from this host: cd "{root}" && ./start_local_search.sh', file=sys.stderr)
        return 1
    ...
    fetcher_label = "Scrapling (TLS fingerprint)" if SCRAPLING_AVAILABLE else "stdlib curl (urllib fallback disabled)"
    ...
    source_label = "local SearXNG" if not is_fallback else f"fallback ({used_url})"
