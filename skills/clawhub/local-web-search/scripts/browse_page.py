...
try:
    ...
    if not os.environ.get("_BROWSE_SCRAPLING_WARNED"):
        if os.environ.get("BROWSER_WORKER_URL", "").strip():
            print(
                "[browse_page] WARNING: Scrapling not installed locally. Fast mode may fall back to stdlib-urllib, but delegated auto/stealth/dynamic fetches can still use BROWSER_WORKER_URL.",
                file=sys.stderr,
            )
        else:
            print(
...
BROWSER_WORKER_URL = os.environ.get("BROWSER_WORKER_URL", "").strip()
...
def _browser_worker_fetch(url: str, mode: str, timeout: int) -> dict:
    payload = {
        "url": url,
        "mode": mode,
        "extract": "text",
    ...
    req = urllib.request.Request(
        f"{BROWSER_WORKER_URL.rstrip('/')}/render",
        data=json.dumps(payload).encode("utf-8"),
        headers={"Content-Type": "application/json"},
    )
    request_timeout = max(timeout + 10, timeout * 3)
    if mode in ("auto", "stealth", "dynamic"):
        request_timeout = max(request_timeout, 75)
    with urllib.request.urlopen(req, timeout=request_timeout) as resp:
        body = json.loads(resp.read().decode("utf-8"))
    content = body.get("text") or ""
    return {
        "url": body.get("final_url", url),
        "title": body.get("title", ""),
        "content": content,
        "word_count": len(content.split()),
        "pub_date": "",
        "confidence": _score_confidence(content, int(body.get("status", 0)), f"browser-worker:{body.get('fetcher_used', mode)}", bool(body.get("blocked_reason"))),
        "status": int(body.get("status", 0)),
        "fetcher_used": f"browser-worker:{body.get('fetcher_used', mode)}",
        "extraction_method": "browser-worker",
        "error": f"Blocked reason: {body.get('blocked_reason')}" if body.get("blocked_reason") else "",
    }
...
def fetch_page(url: str, mode: str = "auto", timeout: int = 20) -> dict:
    ...
    if BROWSER_WORKER_URL and mode in ("auto", "stealth", "dynamic"):
        try:
            return _browser_worker_fetch(url, mode, timeout)
        except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, json.JSONDecodeError) as e:
            result["error"] = f"browser-worker: {e}"
...
def main() -> int:
    ...
    elif fetcher.startswith("browser-worker:"):
        print(f"  -> [REMOTE MODE] {fetcher}: external browser-worker handled the fetch.")
