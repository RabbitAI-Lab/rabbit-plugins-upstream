...
If local SearXNG is unavailable, scripts can fall back to `LOCAL_SEARCH_FALLBACK_URL` when that env var is explicitly set. By default, fallback is disabled.
...
## Default policy across agents

- Treat local-web-search as the **default first-choice** web lookup path in this workspace.
- Prefer search + browse + verify before using any external/public search provider.
- Use external fallback only when `LOCAL_SEARCH_FALLBACK_URL` is explicitly set in the runtime.
- Keep local search active for real-time lookups, current events, and research-style tasks.
