...
# =============================================================================
# SECURITY: All commands are hardcoded strings. Uses `bash -c` (child shell)
# instead of `eval` to minimize injection surface from environment variables.
# =============================================================================
...
# Execute verifications using child shell — safer than eval
check() {
    local name="$1"
    local cmd="$2"
    # Run in child shell to safely handle pipes, !, $(), and redirects
...
warn_check "memory/ has files" 'count=$(ls ~/.openclaw/workspace/memory/*.md 2>/dev/null | wc -l); test "$count" -gt 0'
