---
name: amazon-product-image-planner
description: >
  根据 Amazon 商品详情和评论中的理解障碍，规划主图与辅图应表达的信息。仅用于图片信息规划；不用于生成图片文件、广告投放或自动发布。
  Requires an ARI API key (ari_live_*).
slug: image-planner
displayName: Amazon 商品图片规划
version: 1.4.7
summary: 依据商品字段与评论问题规划图片信息
license: MIT
---
...
- 本入口只执行 `listing/images`，输出模板固定为 `ops_listing`。
