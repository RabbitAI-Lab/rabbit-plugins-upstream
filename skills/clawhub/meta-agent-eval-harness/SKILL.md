---
name: meta-agent-eval-harness
version: 1.0.0
description: |
  由 model-distillation 从教师技能 agent-eval-harness 蒸馏并增强的超越型元技能，
  在教师能力之上叠加自验证、自我反思、super-agent 编排与持续自进化闭环，逐步超越教师。
agent_created: true
visibility: public
---
# meta-agent-eval-harness（蒸馏超越型元技能）

> 由 `model-distillation` 从教师技能 **agent-eval-harness** 蒸馏并增强生成。
> 生成时间：2026-07-23 06:18:03 ｜ 蒸馏机制：跨模型蒸馏（见 meta-evolver 北极星策略）

## 来源能力签名（教师）
- 标题层级：agent-eval-harness（智能体回归评测）, 何时使用, 工作流, 脚本, 自验证, 与四引擎闭环的关系, 已知限制, 自进化学习系统（越用越好用、越用越高效）
- 显性工作流步骤（5 步）：
...
## 教师 vs 学生 对比
| 维度 | 教师(agent-eval-harness) | 学生(meta-agent-eval-harness) |
| --- | --- | --- |
