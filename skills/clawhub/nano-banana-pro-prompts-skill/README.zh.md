# AI 图像提示词推荐 — 26,000+ Nano Banana Pro 提示词
...
> **别再花时间到处找 AI 图像提示词了。** 一句话告诉 AI 助手你的需求 — 它会从 26,000+ 条精选 Nano Banana Pro 提示词中搜索并返回最匹配的 3 条，附带示例图片，开箱即用。
>
> 🖼️ [浏览数据集 →](https://huggingface.co/datasets/Goku-OpenLab/nano-banana-pro-prompts-datasets)

![Demo](public/cover.png)

## 这是什么？

一个 **AI Agent Skill**，让 Claude、OpenClaw、Cursor 等 AI 助手能够智能搜索 26,000+ 条精选 Nano Banana Pro（Gemini 图像模型）提示词库，为你的需求推荐最佳匹配，甚至能根据你的内容定制提示词。

**Nano Banana Pro** 是 Google Gemini 的图像生成模型 — 目前最强大的 AI 图像生成器之一。好的提示词是生成好图片的关键。

## 为什么使用这个 Skill？

- ✅ **26,000+ 条提示词** — 覆盖各种使用场景
...
| 总提示词数 | 26,624 |
| 语言 | 英文、中文（双语） |
| 来源 | [GokuOpenLab 数据集](https://huggingface.co/datasets/Goku-OpenLab/nano-banana-pro-prompts-datasets) |
...
**节能设计**：Skill 永不加载完整提示词文件，使用 grep 风格搜索，只提取匹配行，即使词库有 26,000+ 条提示词也能保持极低的 token 消耗。

---

## 数据来源

提示词来自公开社区，数据源于 HuggingFace 上的 [GokuOpenLab 数据集](https://huggingface.co/datasets/Goku-OpenLab/nano-banana-pro-prompts-datasets) — 26,624 条提示词，包含完整元数据、预览图片和双语支持。
...
**问：什么是 Nano Banana Pro？**
Nano Banana Pro 是 Google Gemini 的图像生成模型（model ID: `gemini-3-pro-image-preview`），能够从文本提示词生成高质量的写实和艺术图片。
...
词库经过预分类和筛选，26,000+ 条提示词都是精选优质内容，每条都附带示例图片，复制前就能看到效果。混搭模式还能根据你的内容个性化定制。

**问：我可以贡献提示词吗？**
数据源是开源的（CC BY 4.0）。欢迎访问 [HuggingFace 数据集](https://huggingface.co/datasets/Goku-OpenLab/nano-banana-pro-prompts-datasets) 了解更多。

**问：词库多久更新一次？**
HuggingFace 数据集由 GokuOpenLab 社区定期更新。

**问：这些提示词能用于其他图像生成模型吗？**
提示词针对 Nano Banana Pro（Gemini）优化，但也适用于 GPT Image、Midjourney、DALL-E、Flux、Stable Diffusion 等其他模型。
...
│   ├── .gitkeep
│   └── metadata.jsonl       # 26,000+ 条提示词（由 setup.js 生成）
└── .claude-plugin/
    └── marketplace.json
```
...
- 🖼️ [GokuOpenLab 数据集](https://huggingface.co/datasets/Goku-OpenLab/nano-banana-pro-prompts-datasets) — HuggingFace 上的源数据集（26,000+ 提示词、图片、元数据）
