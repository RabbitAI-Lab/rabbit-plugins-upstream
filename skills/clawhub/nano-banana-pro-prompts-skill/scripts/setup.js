#!/usr/bin/env node
/**
 * setup.js - Downloads Nano Banana Pro prompt library from HuggingFace
 *
 * Usage:
...
const DATA_URL = 'https://huggingface.co/datasets/Goku-OpenLab/nano-banana-pro-prompts-datasets/resolve/main/metadata.jsonl';
...
async function setup() {
  ...
  console.log(`[setup] ${label} Nano Banana Pro prompt library from HuggingFace...`);

  // Check if we need to download
  const needsDownload = forceMode || !existsSync(destFile) || (existsSync(destFile) && statSync(destFile).size < 100);

  if (needsDownload) {
    process.stdout.write(`  → metadata.jsonl (26,000+ prompts) ... `);
    try {
