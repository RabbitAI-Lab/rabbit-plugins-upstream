...
`maton.google_sheets` mirrors the `maton google-sheets` commands, and `maton.api` reaches any endpoint. `login()` opens a browser once per machine and writes the session to the SDK's own store — `maton login` does not carry over, and the SDK never signs in implicitly.
...
```bash
curl --config - "https://api.maton.ai/google-sheets/v4/spreadsheets/{spreadsheetId}" <<EOF
header = "Authorization: Bearer $MATON_API_KEY"
header = "User-Agent: maton-google-sheets-skill/1.1"
