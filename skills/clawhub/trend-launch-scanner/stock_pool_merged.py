...
# -*- coding: utf-8 -*-
"""合并股票池 - 预过滤版
自动过滤：ST股、退市股、科创板、白酒、银行、房地产、名字含"地产"
"""
...
# 过滤后的股票池缓存
_FILTERED_POOL = None
_FILTERED_POOL_PATH = r"C:\Users\Administrator\.qclaw\workspace-ag01\data\filtered_pool.json"


def _is_static_filtered(stock):
    """判断股票是否应该被静态过滤"""
    code = stock["code"]
    name = stock["name"]
    ...
    # 需要行业数据的股票，从industry_data获取
    try:
        import sys
        from pathlib import Path
        industry_path = Path(__file__).parent
        if str(industry_path) not in sys.path:
            sys.path.insert(0, str(industry_path))
        from industry_data import get_industry
        
        industry = get_industry(code)
        if industry in ["白酒", "银行", "房地产", "证券"]:
            return True, f"行业:{industry}"
    except:
        pass
    
    # 名字含"地产"或"证券"
    if "地产" in name or "证券" in name:
        return True, "名字含地产/证券"
    
    return False, None


def _create_filtered_pool():
    """创建预过滤股票池"""
    global _FILTERED_POOL
    
    filtered = []
    stats = {"总数": len(_RAW_STOCK_LIST), "保留": 0}
    
    for stock in _RAW_STOCK_LIST:
        should_filter, reason = _is_static_filtered(stock)
        if should_filter:
    ...
    _FILTERED_POOL = filtered
    
    # 保存到文件
    import os
    os.makedirs(os.path.dirname(_FILTERED_POOL_PATH), exist_ok=True)
    with open(_FILTERED_POOL_PATH, "w", encoding="utf-8") as f:
        json.dump(filtered, f, ensure_ascii=False, indent=2)
    
    print("预过滤股票池统计:")
    for k, v in stats.items():
        print(f"  {k}: {v}")
    print(f"已保存到: {_FILTERED_POOL_PATH}")
...
def get_merged_stock_list():
    ...
    # 优先使用内存缓存
    if _FILTERED_POOL is not None:
        return _FILTERED_POOL
    
    # 尝试从文件读取
    if os.path.exists(_FILTERED_POOL_PATH):
        try:
            with open(_FILTERED_POOL_PATH, "r", encoding="utf-8") as f:
                _FILTERED_POOL = json.load(f)
            return _FILTERED_POOL
        except:
            pass
    
    # 创建新的预过滤池
    return _create_filtered_pool()
