...
def calc_sell_score(df, buy_price=None, buy_date=None):
    ...
    if buy_price:
        ...
        # 6a. 移动止盈：计算买入后最高浮盈和回撤比例
        if buy_date:
            try:
                buy_dt = pd.to_datetime(buy_date)
                after_buy = df[df['date'] >= buy_dt]
                if len(after_buy) > 0:
                    peak_close = after_buy['high'].max()
                    peak_profit = (peak_close - buy_price) / buy_price * 100
                    if peak_profit > 0:
                        profit_drawdown = (peak_profit - profit_pct) / peak_profit * 100  # 回撤占最高浮盈的比例
            except:
                pass
        
        # 6b. 移动止盈信号（优先级高于固定止盈/止损）
        if peak_profit is not None and peak_profit > 3:
            # 曾盈利超过3%，启动移动止盈监控
            if profit_pct < 0:
                # 浮盈转亏 → 强烈卖出
                sell_signals.append((f'浮盈转亏(曾赚{peak_profit:+.1f}%)', 30))
                total_score += 30
            elif profit_drawdown is not None and profit_drawdown >= 80:
                # 利润回吐80%以上
                sell_signals.append((f'利润回吐{profit_drawdown:.0f}%(曾赚{peak_profit:+.1f}%)', 20))
                total_score += 20
            elif profit_drawdown is not None and profit_drawdown >= 30:
                # 利润回吐30%以上 → 建议获利了结
                sell_signals.append((f'利润回吐{profit_drawdown:.0f}%(曾赚{peak_profit:+.1f}%)', 15))
                total_score += 15
        
        # 6c. 固定止盈/止损
        if profit_pct > 20:
            warning_signals.append(('获利>20%注意止盈', 5))
            total_score += 5
        elif profit_pct > 15:
            warning_signals.append(('获利>15%', 3))
            total_score += 3
    ...
    return {
        ...
        'peak_profit': round(peak_profit, 1) if peak_profit is not None else None,
        'profit_drawdown': round(profit_drawdown, 0) if profit_drawdown is not None else None,
        'hold_days': hold_days,
        'buy_price': buy_price
    }
