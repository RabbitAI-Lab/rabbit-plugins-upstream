...
|---|---|---|---|
| `ai` | [`image-generator.md`](./image-generator.md) | `image_gen.py --manifest` (Path A, if `IMAGE_BACKEND` configured) | `Generated` |
...
> **CRITICAL — ai rows MUST use `image_gen.py --manifest` (Path A) when `IMAGE_BACKEND` is configured.** The host's built-in multimodal tool (if any) is a Path B fallback only — see [`image-generator.md` §7](./image-generator.md) for the strict A→B→C chain. Before invoking any other image tool, verify `IMAGE_BACKEND` is set in `.env` and run Path A.
