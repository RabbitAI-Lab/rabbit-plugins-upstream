...
| `IMAGE_BACKEND` NOT configured (genuinely absent) AND host has a native image tool | **Path B**: Host-native tool | Agent invokes the host's image capability; outputs land at `project/images/<filename>` |
...
**Selection logic** — strict A-first fallback chain (automatic, no user prompting):

1. **MUST try Path A first** — if `IMAGE_BACKEND` is configured (env or `.env`), `image_gen.py --manifest` is MANDATORY. **The host always having a built-in multimodal tool (e.g. WorkBuddy 多模态内容生成、Codex image tool) is NOT a reason to skip Path A.** Path A is skipped ONLY when `IMAGE_BACKEND` is genuinely absent from both the process environment and the `.env` file. Verify by reading `${SKILL_DIR}/.env` — if `IMAGE_BACKEND=<value>` exists, Path A is required, full stop.
2. **Path A explicit failure → fall to Path B** — only after `image_gen.py --manifest` fails twice in a row (non-recoverable errors: auth failure, backend unreachable, all items `Failed`), attempt Path B.
3. **Try Path B** — if `IMAGE_BACKEND` was genuinely not configured (A truly skipped at step 1), and the host has a native image tool, invoke the host's image capability. **Do NOT conflate "host has a tool" with "skip Path A"** — step 1 already gatekeeps this.
4. **Fall to C (Offline Manual)** — if B is also unavailable or fails, write prompts to `images/image_prompts.json` and hand off to the user.

**User override**: If the user explicitly names Path B ("use Codex's image tool" / "用多模态生成"), skip A and start at B. Explicit naming is the only way to bypass Path A; otherwise the chain is strictly monotonic.
...
> **Diagnostic checkpoint (Path A verification)**: Before running `image_gen.py --manifest`, confirm the `.env` is readable and `IMAGE_BACKEND` is set. Run `python3 -c "from config import load_prefixed_env_file; import os; load_prefixed_env_file(); print('IMAGE_BACKEND:', os.environ.get('IMAGE_BACKEND', 'NOT SET'))"` from `${SKILL_DIR}/scripts/`. If the output says `NOT SET`, Path A is genuinely unavailable — proceed to Path B. If a value is printed, Path A MUST be attempted.
...
- First `.env` found in this order: current working directory, clone repo root, `~/.dog-slide/.env`
