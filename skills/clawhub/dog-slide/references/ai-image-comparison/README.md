...
Dog Slide's AI images are governed by three orthogonal dimensions: **rendering (visual style) × palette (color behavior) × type (internal composition)**.
...
> Reference images were generated with the **OpenAI gpt-image-2** backend. Other backends (gemini / doubao / qwen / etc.) will produce visually different results — this reflects model-level differences, not differences in Dog Slide's dimension system.

To reproduce or regenerate:

```bash
python3 skills/dog-slide/scripts/image_gen.py \
    --manifest skills/dog-slide/references/ai-image-comparison/rendering/_manifest.json \
    -o skills/dog-slide/references/ai-image-comparison/rendering/ \
    --backend openai

python3 skills/dog-slide/scripts/image_gen.py \
    --manifest skills/dog-slide/references/ai-image-comparison/palette/_manifest.json \
    -o skills/dog-slide/references/ai-image-comparison/palette/ \
    --backend openai

python3 skills/dog-slide/scripts/image_gen.py \
    --manifest skills/dog-slide/references/ai-image-comparison/type/_manifest.json \
    -o skills/dog-slide/references/ai-image-comparison/type/ \
    --backend openai
```
