...
description: >
  ...
  "生成PPT", "做PPT", "制作演示文稿", or mentions "dog-slide".
---
...
> 3. **NO CROSS-PHASE BUNDLING** — Cross-phase bundling is FORBIDDEN. (Note: Steps marked ⛔ BLOCKING — Step 4 Eight Confirmations, Step 4.5 Outline Review, Step 6.5 Page Review, Step 8 Final Confirmation — each require a full stop. The AI MUST present and wait for explicit user confirmation at every BLOCKING gate before proceeding to the next step)
...
>
> - `dog-slide` is a repository-specific workflow, not a general application scaffold
...
| Index | Path | Purpose |
|-------|------|---------|
| Layout templates | `${SKILL_DIR}/templates/layouts/layouts_index.json` | 18 built-in templates including work_summary (工作总结), project_review (方案评审), value_proposition (产品价值传递) |
...
### Step 1: Requirement Confirmation & Source Content Processing

🚧 **GATE**: AI has confirmed the following requirements with the user before any processing:

1. **PPT topic / title** — what is the presentation about?
2. **Source / reference** — is the source content sufficient? (PDF/URL/text/conversation — any form is acceptable, but assess whether quality and depth are adequate for the expected page count)
3. **Page count** — how many slides does the user expect? If not specified, suggest 10–12 as a reasonable default for internal sharing, or ask the user to decide
4. **Use case / scenario** — internal sharing? client presentation? conference? academic defense? Each maps to a different visual style and depth level
5. **AI image requirement** — does the user want AI-generated images (配图)? If so, confirm approximate number per page or overall
6. **Other requirements** — brand colors? logo? template preference? any specific content to include/exclude?

> If any of the above is missing, the AI **MUST explicitly ask** the user for clarification before proceeding. Do not infer defaults for ambiguous requirements.

After all requirements are confirmed, proceed to source content processing:
...
>
> **Do NOT convert EMF/WMF to PNG.** The Dog Slide pipeline preserves them as external
...
### Step 2.5: Template Recommendation (Optional)

⛔ **BLOCKING**: AI proposes, user decides.

🚧 **GATE**: Step 2 complete; project directory structure is ready.

**When to trigger**: When the user's source material or stated needs have a clear scenario (work summary, project review, business proposal, academic defense, etc.) that matches one or more built-in templates. Do NOT trigger if the user explicitly says "no template" or the topic is very generic.

**AI action**:
1. Review the source material to identify the presentation scenario
2. Look up `layouts_index.json` for matching templates (max 3 recommendations)
3. Present recommendations with scenario match reasoning

**Output format**:
```
📋 根据您的素材，推荐以下模板：

1. **[模板名称]** (`templates/layouts/<id>/`)
   - 主色: #HEX
   - 适用场景: ...
   - 匹配理由: ...
...
| Response | Action |
|----------|--------|
| An explicit template directory path | Copy template → proceed to Step 3 |
| `skip` / `no` / `自由设计` | Jump to Step 3 (free design, no copy) |
| Other questions | Answer naturally, do NOT advance pipeline |

**✅ Checkpoint — Template decision made. Proceed to Step 3.**
...
|---|---|
| An explicit path to a template directory (e.g. `skills/dog-slide/templates/layouts/academic_defense/`, `projects/foo/template/`, or any other absolute / relative path that resolves to a directory containing `design_spec.md` and one or more page SVGs) | Copy that directory's SVGs + `design_spec.md` + assets into the project, advance |
...
The path may live anywhere — `skills/dog-slide/templates/layouts/<name>/` (the built-in library), `projects/<other_project>/template/` (reusing a previous project's templates), or any other location. Location is irrelevant; what matters is that the user named the path.
...
|---|---|
| An explicit path to a brand directory (e.g. `skills/dog-slide/templates/brands/acme/`, or any path that resolves to a directory whose `design_spec.md` declares `kind: brand`) | Copy `design_spec.md` + logo files + any present asset subdirectories into `<project_path>/templates/` |
...
⛔ **BLOCKING**: present the Eight Confirmations as a single bundled recommendation set and **wait for explicit user confirmation or modification** before outputting Design Specification & Content Outline. This is the first core confirmation point — once confirmed, proceed to Step 4.5 Outline Review.
...
**✅ Checkpoint — Phase deliverables ready for review**:
```markdown
...
- [ ] **Next**: Proceed to Step 4.5 Outline Review
```

---

### Step 4.5: Content Outline Review (Mandatory)

⛔ **BLOCKING**: User reviews the full content outline. AI MUST wait for explicit confirmation before proceeding.

🚧 **GATE**: Step 4 complete; `design_spec.md` and `spec_lock.md` written to project directory.

**AI action**:
1. Read the completed `design_spec.md` from the project directory
2. Present the full content outline (Section IX) to the user in a clear format, showing each page's:
   - Page title
   - Content points / key messages
   - Image plan (which page uses which image)
3. Ask the user to confirm the outline or request changes
...
| # | 页面 | 类型 | 标题 | 要点 | 配图 |
|---|------|------|------|------|------|
| P01 | Cover | anchor | {title} | {key message} | {image or none} |
| P02 | TOC | dense | 目录 | {items} | — |
| P03 | 市场规模 | dense | {title} | {key points} | {image or none} |
| ...

⛔ 请审核以上大纲。确认无误后我将开始图片生成和 SVG 制作。
如有修改（增删页、调整内容、更换配图等），请说明。
```

**User responses**:
| Response | Action |
|----------|--------|
| `确认` / `approve` / `ok` | Proceed to Step 5 (Image Acquisition) |
| Specific modifications (add/remove pages, change titles, adjust image plan) | Apply changes to both `design_spec.md` and `spec_lock.md`, then re-present for confirmation |
| Other questions | Answer naturally, do NOT advance pipeline |

**✅ Checkpoint — Outline confirmed. Proceed to image generation**:
```markdown
## ✅ Outline Review Complete
- [x] Full content outline reviewed and confirmed by user
- [ ] **Next**: Proceed to [Image_Generator / Executor] phase
```
...
🚧 **GATE**: Step 4.5 complete; content outline reviewed and confirmed by user.
...
**✅ Checkpoint — Confirm all SVGs and notes are fully generated and quality-checked. Proceed to Step 6.5 Page Review**:
```markdown
...
### Step 6.5: Page Review (Mandatory)

⛔ **BLOCKING**: AI presents generated pages, user MUST review and confirm before proceeding. Cannot be skipped.

🚧 **GATE**: Step 6 complete; all SVGs generated to `svg_output/`; quality check passed.

**AI action**: Present a summary of all generated pages:
...
| # | 文件名 | 页面类型 | 标题 |
|---|--------|---------|------|
| 1 | 01_cover.svg | cover | {title} |
| 2 | 02_agenda.svg | agenda | {title} |
| 3 | 03_content.svg | content | {title} |
...
```

Ask the user if they want to modify any page. User can:
- Specify a page number and modification request — AI regenerates that SVG page and re-runs quality check
- Regenerate specific pages by specifying the page number with detailed revision instructions (e.g., "page 3: change the chart type to bar, update section heading to 'Q2 Results'")
- Input `done` to continue to Step 7 post-processing

**✅ Checkpoint — All pages confirmed. Proceed to Step 7.**
...
⛔ **BLOCKING**: User reviews the exported PPTX and decides to deliver or iterate.

🚧 **GATE**: Step 7 complete; `exports/*.pptx` generated.

**AI action**: Present the export result:

```
✅ PPTX 已生成：
   📁 {project_path}/exports/{filename}.pptx

是否确认交付？如有修改需求请说明，我可以：
- 修改单页 SVG → 重新导出
- 调整配色 → 更新 spec_lock → 重新导出
- 重新生成全部 → 从 Step 4 开始

输入 done 确认交付，或描述您的修改需求:
```

**User responses**:
| Response | Action |
|----------|--------|
| `done` / `确认` / `deliver` | Confirm delivery, summarize final output |
| Modification request | Route to the appropriate step: single SVG page → return to Step 6; color/typography change → update spec_lock → re-run Step 7; full redesign → return to Step 4 |
| Other questions | Answer naturally, do NOT close out |

**✅ Final — Pipeline complete. PPTX delivered.**
