---
name: amazon-customer-expectation-gap
description: >
  Amazon 消费者预期差距 Skill：核对商品页承诺与评论中的真实使用体验。
  仅用于承诺证据核查，不用于法律合规审核或无证据营销创作。Use when the user asks
  about expectation gaps, promise audit, 消费者预期差距、页面承诺核查。
  Requires an ARI API key (ari_live_*).
slug: customer-gap
displayName: Amazon 消费者预期差距
version: 1.4.7
summary: 核对商品承诺与真实使用体验
license: MIT
---
...
- 本入口只执行 `audit/promise`，输出模板固定为 `ops_audit`。
