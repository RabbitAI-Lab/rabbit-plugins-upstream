...
- For a one-off artifact or result, use the registered `openai-transcription` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi openai-transcription --help
runapi openai-transcription <operation> --help
curl --fail --location https://runapi.ai/docs/api/openai-transcription/<operation>.md --output contract.md
```
...
```shell
runapi openai-transcription <operation> --input-file request.json > result.out
```
