...
**Issue:** . **External motivation:** DELEGATE-52 (arXiv:2604.15597) — round-trip
document editing corrupts scientific content by *subtle modification, not deletion*.
's block-anchored patch confines that exposure to touched blocks; this reference
...
the original ARS community (`sci-ssci-polishing` skill discussion).

**Epistemic status:** the field-relative ladder remains advisory / interpretive
guidance. Since , exact claim surfaces that were explicitly registered and
...
unregistered prose. The deterministic sibling `` separately conserves
numbers and citation tokens; see 自动校验脚本（移植版未附带，以文字规则为准）。
...
- **Hedges.** `references/shared/protected_hedging_phrases.md` protects hedges
...
- **Novelty.**  bounds *priority* language ("first study to…"); the ladder
  governs *causal/evidential* strength. Disjoint concerns, same anti-overclaim family.
- **Numbers/citations.**  conserves numeric and citation *tokens*
...
- `draft_writer_agent` § Claim-Strength Ladder — revision mode: a patch op
  touching a registered claim surface carries its exact  authorization when
...
-  cross-document consistency advisory — semantic callers may use the ladder
