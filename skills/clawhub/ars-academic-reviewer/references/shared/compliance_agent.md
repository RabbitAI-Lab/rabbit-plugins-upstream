...
version: 1.0.0
owner_skill: shared
invoked_by: [ars-pipeline-orchestrator, ars-deep-research, ars-academic-paper]
data_access_level: verified_only
task_type: open-ended
status: active
related_skills: [ars-pipeline-orchestrator, ars-deep-research, ars-academic-paper]
...
related_protocols:
  ...
  - ars-academic-paper/references/guides/anti_leakage_protocol.md
  - ars-pipeline-orchestrator/references/guides/ai_research_failure_modes.md
---
...
- **Does not hallucinate missing items.** Anti-Leakage Protocol applies: missing material → `[MATERIAL GAP: <item_id>]` in the gap reason (see [`references/shared/compliance_checkpoint_protocol.md#canonical-gap-tag-vocabulary`](compliance_checkpoint_protocol.md#canonical-gap-tag-vocabulary)).
...
`compliance_report` conforming to `references/shared/compliance_report.schema.json` (Schema 12). Appended to `material_passport.compliance_history[]` by the orchestrator.
...
| ID | Question | On fail |
|---|---|---|
| CA-1 | Am I quoting PRISMA-trAIce items from memory or from `references/shared/prisma_trAIce_protocol.md`? | Re-read the protocol file; requote. |
...
| Mode/context mismatch (e.g. `ars-academic-paper full` passes SR mode) | Refuse with `{decision: "abort", reason: "mode/context mismatch"}`. Orchestrator must re-evaluate and re-invoke. |
...
The orchestrator (or standalone skill) passes the input contract via sub-agent dispatch at mid-tier model or higher. The agent returns the serialised compliance_report. The orchestrator validates against Schema 12 before appending to passport.

## Related reading

- `references/shared/prisma_trAIce_protocol.md` — item-level checks
- `references/shared/raise_framework.md` — principle definitions and role matrix
- `references/shared/compliance_checkpoint_protocol.md` — checkpoint behaviour and override ladder
- `ars-academic-paper/references/guides/anti_leakage_protocol.md` — gap-marking discipline
- `ars-pipeline-orchestrator/references/guides/pipeline_state_machine.md` — FAIL-loop integration
