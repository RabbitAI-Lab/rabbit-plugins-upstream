...
[A zero-strength review legitimately has no entries here — skip this block rather than invent praise to fill it ( A1).]
...
### R1-W4..Wn
[Repeat the same structure for EVERY additional weakness in the report — weakness lists are unbounded ( A1), and every emitted weakness gets a response entry; none may be dropped for exceeding a template count.]
...
[Same format as above: Strengths Acknowledged → W1..Wn (every emitted weakness, unbounded —  A1) → Questions → Minor Issues]
