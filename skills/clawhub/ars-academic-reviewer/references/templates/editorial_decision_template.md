...
- **Resolution Rationale**: [Arbitration rationale — based on evidence/expertise/unresolved-dissent principle ( B1)]
...
Please use the format in `references/templates/revision_response_template.md` to respond to every reviewer comment item by item.
...
6. **Compatibility**: Emit the closed `revision-roadmap/1.0` machine artifact
   for `ars-academic-paper` revision mode

Finding severity is the Schema 6 enum (`critical` / `major` / `minor`), transported from the reviewer cards ( A3). `obligation_class` is a separate editorial gate and is never derived as a work rank from severity.
