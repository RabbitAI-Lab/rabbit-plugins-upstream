...
Each item carries its `sub_claim_id`, traces back to the Step 1b inventory, and flows forward into `ars-academic-paper` revision mode unchanged in format. The minority-risk sub-claim (the grouping factor) is now its own Priority-1 item rather than buried in a bundle.

---

## What this example demonstrates (acceptance for )
