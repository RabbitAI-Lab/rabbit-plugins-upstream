...
自动校验脚本（移植版未附带，以文字规则为准） renders the bounded slots below
...
byte-for-byte against the agent file. The shared scoring text therefore
...
`methodology-extraction` fragment ( step 5). It is dispatched only by an
