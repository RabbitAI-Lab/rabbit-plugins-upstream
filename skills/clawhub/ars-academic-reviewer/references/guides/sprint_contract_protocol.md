...
> Schema: `references/shared/sprint_contract.schema.json` (Schema 13.2).
> Templates: `references/shared/contracts/reviewer/*.json`.
> Design spec: 对应设计文档（移植版未附带）。
...
> the dispatcher sends those prompt sections verbatim; 自动校验脚本（移植版未附带，以文字规则为准）
> enforces byte-exact mirrors plus an explicit content-hash re-pin.
>
> **v3.6.6 cross-reference**: this reviewer protocol is byte-equivalent across v3.6.2 → v3.6.6 (zero-touch promise per §3.6 of 对应设计文档（移植版未附带）). The v3.6.6 release adds a parallel generator-evaluator protocol inside `ars-academic-paper` for the in-pair writer / evaluator pair (see `ars-academic-paper/SKILL.md` § "v3.6.6 Generator-Evaluator Contract Protocol" and design doc §5).
...
4. **Phase 2 call (paper-visible).**
   ...
   - User content: contract JSON (re-injected) + Phase 1 output wrapped in `<phase1_output>...</phase1_output>` data delimiter + full paper wrapped in `<paper_content>...</paper_content>` data delimiter ( A6 — the manuscript is author-supplied untrusted material; the reviewer prompts carry the matching data-not-instructions rule).
...
5. **Phase 2 output lint.** Run 自动校验脚本（移植版未附带，以文字规则为准） --contract <C> --role <dispatch-role> --phase1 <P1> --phase2 <P2> --manuscript <paper> --metadata <metadata.json>` before synthesis. Exit 3 emits `[PROTOCOL-VIOLATION: phase_conformance=<check>]` and makes the seat unusable; exit 2 is an infra abort. See §5.
...
- In a  criteria-aware call, Phase 1 also receives the pointer-only manifest,
...
### 5.1 Methodology arithmetic-receipt gate
...
> adjudication (`VERIFIED`/`CLAIM_ONLY`/`MISCOMPUTED`/`MISSED`,  spec
...
mandatory grammar is a tracked  §7.1 reporting metric, not a silent
tradeoff. Procedure boundaries and the red-flag classification live in
`references/guides/statistical_reporting_standards.md` §8.

### 5.2 Script-adapter dispatch ( step 5, optional orchestration)
...
of the Phase 1 evidence class; then 自动校验脚本（移植版未附带，以文字规则为准）, which
...
| `reviewer_re_review`          | —          | NOT a Schema 13 mode ( Spec B — removed from the enum): governed by the dedicated contract family `references/shared/contracts/re_review/` + 自动校验脚本（移植版未附带，以文字规则为准）; see `re_review_mode_protocol.md` § Three-Gate Orchestration |
...
## 8.1 Executable recomputation

After the synthesizer emits its output, the orchestrator runs
自动校验脚本（移植版未附带，以文字规则为准） --contract <contract.json> --report <r1.md> ...
...
New expression forms require a PR updating this §9, the synthesizer prompt's recognised-pattern list, and the 自动校验脚本（移植版未附带，以文字规则为准） expression grammar in lockstep.
...
| `[RECEIPT-MISSING/-GRAMMAR/-TAILS/-LINKAGE: <path>: ...]` |  methodology arithmetic-receipt gate failure (exit 3, §5.1) | mark reviewer unusable |
| `[RECEIPT-SECTION-FORBIDDEN: <path>: ...]` | a non-methodology seat emitted `## Arithmetic Receipts` (exit 3, §5.1) | mark reviewer unusable |
| `[RECEIPT-ATTESTATION: declaration-only — ...]` |  attestation-path advisory on a PASSING methodology card (§5.1) | none — advisory; adjudication judges the attestation |
