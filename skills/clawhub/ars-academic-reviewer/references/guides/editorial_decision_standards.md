...
| `re-review` |  three-gate contract: `re_review_mode_protocol.md` § Decision Derivation, recomputed by 自动校验脚本（移植版未附带，以文字规则为准） | item verdicts (FULLY/PARTIALLY/NOT_ADDRESSED/MADE_WORSE/CANNOT_VERIFY) | Accept / Minor Revision / Major Revision / user_review_required (Reject is not a Stage 3' decision) |
...
### Decision Symmetry and Register Independence ( B1)
...
- First-pass acceptance when the paper genuinely meets every criterion — the decision follows the criteria, never a frequency expectation ( B1: no base-rate anchoring, qualitative or numeric)
...
- Affirm genuine merits where they exist — do not manufacture praise to soften the decision ( A1/B1)
...
- Resolve on the arbitration principles (evidence first, expertise first). A genuinely unresolvable split records the dissent and routes the author to respond to it — revision is the vehicle for that response, not a policy of rounding splits toward the stricter verdict ( B1)
...
Finding severity is transported from the reviewer cards (the Schema 6 enum, `references/shared/handoff_schemas.md` § Weakness Object) — this table assigns NO severities ( A3); it describes how criterion judgements bear on the DECISION only.
