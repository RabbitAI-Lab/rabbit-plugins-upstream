...
Since  Spec B, this mode is governed by the **three-gate evidence-before-persuasion contract** (`references/shared/contracts/re_review/` + 自动校验脚本（移植版未附带，以文字规则为准）): criteria are committed before the revision is seen, evidence verdicts are committed before the author's persuasion is seen, and every later verdict change rides a typed, evidence-bound adjustment record. The pre-contract single-pass behaviour survives only behind the `ARS_RE_REVIEW_LEGACY` option (§ Legacy Mode).
...
Then: checker -> decision derivation -> New Decision (or deferral / abort)
```
...
Inputs add the Response to Reviewers, fenced as UNTRUSTED author-authored persuasion ( A6 class). The committed 2A verdicts ride as `<phase2a_output>`.
...
**Execution shape:** routing changes the PERSONA, not the call count — the three gates stay three sequential fenced calls (§ Three-Gate Orchestration). Within Phase 1 and Phase 2A, each must_fix/should_fix item's pre-commitment and verdict are produced UNDER its routed seat persona (the frozen card supplies the persona; the dispatch context includes the cards for exactly this reason), and `verified_by` records which seat verified each item. Phase 2B is the dedicated Journal-Fit Reviewer (`EIC` wire label) / synthesizer-function integration call, not a dispatch of either same-named first-round agent file; the closed rules derive the candidate decision state and 自动校验脚本（移植版未附带，以文字规则为准） recomputes it before surfacing.
...
- **Roadmap machine form:** validate the immutable reviewer-owned core against `references/shared/contracts/revision/revision_roadmap.schema.json`. It binds exact draft/manifest bytes, uses deterministic `source_refs` order, separates severity/obligation/cost/consequence, and proposes exact targets. It carries no author decision or display view.
...
Contract-governed re-review is the Stage 3' default. Current artifacts are version 1.1 and the current checker rejects 1.0 or mixed chains. Only explicit `ARS_RE_REVIEW_LEGACY=1` dispatches the archived schemas/checker under `references/shared/contracts/re_review/legacy/v1_0/` and the original repository's legacy archive (not shipped); every such output begins `[LEGACY-NO-CONTRACT]` and carries no current author/bundle witness. There is no silent fallback.

### Judge Provenance and Correlated-Error Boundary (/)
...
| Transport ref | Original Review Comment | Author triage | Author's Claim | Response Status | Revision Location | Verified? | Cross-model | Quality Assessment |
|---|------------------------|---------------|---------------|-----------------|-------------------|-----------|--------------------|--------------------|
...
The machine-readable counterpart of this report is the traceability sidecar (`references/shared/contracts/re_review/traceability.schema.json`) — Schema 11 prose remains the human surface; the sidecar is what the checker recomputes from.

## Sprint contract status

Since  Spec B, `reviewer_re_review` is NOT a Schema 13 mode (removed from the `sprint_contract.schema.json` enum — the paper-blind premise, `panel_size` grammar, and `block|warn|pass` vocabulary are structurally incompatible with re-review). This mode is governed by the dedicated contract family `references/shared/contracts/re_review/{precommitment,verdict_record,traceability,input_manifest}.schema.json` + 自动校验脚本（移植版未附带，以文字规则为准）。 `reviewer_calibration` / `reviewer_guided` remain reserved Schema 13 modes.
