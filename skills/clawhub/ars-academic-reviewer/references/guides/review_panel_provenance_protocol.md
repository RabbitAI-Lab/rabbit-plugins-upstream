...
- Input: `references/shared/contracts/reviewer/review_panel_provenance_input.schema.json`
- Output: `references/shared/contracts/reviewer/review_panel_provenance.schema.json`
- Schema 6 carrier: `references/shared/contracts/reviewer/review_panel_provenance_carrier.schema.json`
- Builder and replay validator: 自动校验脚本（移植版未附带，以文字规则为准）
...
SHA-256 of the exact raw bytes of `references/shared/contracts/reviewer/full.json`. Its
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） build panel-input.json --output panel-provenance.json
python 自动校验脚本（移植版未附带，以文字规则为准） validate panel-provenance.json
python 自动校验脚本（移植版未附带，以文字规则为准） build-carrier panel-provenance.json --artifact-ref artifacts/panel-provenance.json --output panel-carrier.json
python 自动校验脚本（移植版未附带，以文字规则为准） validate-carrier panel-carrier.json --artifact-root .
python 自动校验脚本（移植版未附带，以文字规则为准） validate-schema6 review-report.json --mode reviewer_full --artifact-root .
```
