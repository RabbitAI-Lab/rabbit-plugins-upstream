...
description: "当用户使用 Google AI Model 搜索信息时，使用此 skill。"
---

# Dataify Google AI Mode 中文版
...
```bash
python3 scripts/google_ai_mode.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/google_ai_mode.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
