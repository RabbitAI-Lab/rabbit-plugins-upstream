#!/usr/bin/env python3
"""Call Dataify Scraper API Google AI Mode and print the raw response body."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Call Dataify Google AI Mode API and print the raw response body."
    )
    parser.add_argument("--token", help="Dataify API token. Bearer prefix is optional.")
    parser.add_argument("--params-json", help="JSON object containing Google AI Mode fields.")
...
def extract_query(text: str) -> str | None:
    ...
    patterns = (
        r"(?:搜索|查询|检索|问|提问|抓取|采集)\s*(.+?)(?:[，,。；;]|$)",
        r"(?:ai\s*mode|google\s+ai\s+mode|ask|query|search\s+for)\s+(.+?)(?:\s+with\b|\s+in\b|[,;.]|$)",
    )
...
def parse_natural_request(text: str) -> dict[str, str]:
    ...
    if "location" not in params and "uule" not in params:
        location_match = re.search(r"(?:location|位置|地区|地理位置)\s*[:=：]\s*([^,，;；]+)", text, flags=re.IGNORECASE)
        if location_match:
...
def normalize_params(params: dict[str, Any]) -> dict[str, str]:
    normalized: dict[str, str] = {"engine": "google_ai_mode"}
    ...
    if not normalized.get("q"):
        raise ValueError("Missing required query q. Parse q from the user request or ask the user what to query.")
