...
通用要求:
  ...
  - **必须使用 Noto Sans CJK SC Bold 中文字体**，不能使用 Helvetica/Arial/DejaVu 等西文字体（不支持中文）
  - 生成1个标题即可，直接使用
  - 内容真诚务实、不引导讨论
  - 话题标签不超过5个
  - 必须包含#留学新加坡 #新加坡私立大学
...
build_prompt() {
    ...
    if [ "$mode" = "auto" ]; then
        # 自动选题模式
        prompt="生成一篇关于新加坡私立大学留学的笔记，主题自行确定。要求：生成1个标题直接使用；封面用ImageMagick生成浅色背景深色字体封面，使用Noto-Sans-CJK-SC-Bold中文字体（-font Noto-Sans-CJK-SC-Bold），不能用西文字体；封面文字与标题一致；内容真诚务实；话题标签≤5个，包含#留学新加坡 #新加坡私立大学；直接发布无需确认；仅发布一次即停止"
    else
        # 外部主题模式
        prompt="生成一篇关于${topic}的小红书笔记。要求：生成1个标题直接使用；封面用ImageMagick生成浅色背景深色字体封面，使用Noto-Sans-CJK-SC-Bold中文字体（-font Noto-Sans-CJK-SC-Bold），不能用西文字体；封面文字与标题一致；内容真诚务实；话题标签≤5个，包含#留学新加坡 #新加坡私立大学；直接发布无需确认；仅发布一次即停止"
    fi
