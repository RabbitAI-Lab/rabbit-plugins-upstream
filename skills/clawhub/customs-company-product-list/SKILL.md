---
name: customs-company-product-list
description: "Query paginated product trade data for a company — retrieve product names with trade counts, amounts, quantities, and associated HS codes for product-mix analysis.\n\nTrigger: customs product list, paginated product query, company product trade details, import-export product breakdown, trade product drill-down"
...
# Customs Company Product Trade List

Query paginated product trade data for a company from customs data using the UpKuaJing Open Platform API.

## Overview

This skill provides access to paginated product trade data from UpKuaJing's customs database. Given a company ID and company type, it returns a list of products with trade statistics including trade count, amount, quantity, weight, percentage, and associated HS codes.
...
**Important**: Always use direct script invocation like `python scripts/customs_company_product_list.py`. **Do NOT use** shell compound commands like `cd scripts && python customs_company_product_list.py`

### Company Product List Query (`customs_company_product_list.py`)
- **Return granularity**: One record per product
- **Use cases**: Browse through a company's traded products with detailed stats, analyze product mix and associated HS codes
- **Examples**:
  - "List products for company 100001 as a supplier"
  - "Get product trade details for company 100001 as a buyer"
- **Parameters**: See [Company Product List API](references/customs-company-product-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/customs/company/product/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"Company product list query failed with a server error"}'
```
...
Billed by **number of calls**, each call returns a page of product list for one company:
...
| User Intent | Use API |
|-------------|---------|
| "List products for company 100001 as a supplier" | Company Product List Query |
| "Show product trade breakdown for company 100001" | Company Product List Query |

## Usage Examples

### Query Company Product List

**User request**: "List products for company 100001 as a supplier"
```bash
python scripts/customs_company_product_list.py --params '{"companyId":100001,"companyType":1}'
```

**Query the next page**:
```bash
python scripts/customs_company_product_list.py --params '{"companyId":100001,"companyType":1,"cursor":"eyJsYXN0X2lkIjogMTAwMX0="}'
```

**Query with filters**:
```bash
python scripts/customs_company_product_list.py --params '{"companyId":100001,"companyType":1,"dateStart":1700000000000,"dateEnd":1735689599999,"port":"BUSAN"}'
```
...
- Company Product List: Check [references/customs-company-product-list-api.md](references/customs-company-product-list-api.md)
...
1. **Check API documentation**:
   ...
   - Check [references/customs-company-product-list-api.md](references/customs-company-product-list-api.md)
...
3. **Data interpretation**:
   - Each product entry includes its associated `hscodes` array — use this to understand which HS codes map to each product
   - `percentTrade` is the percentage of trade (as a decimal, e.g., 12.34 means 12.34%)

4. **Cross-skill usage**:
   - The company ID can be obtained from **customs-company-stats** or **upkuajing-customs-trade-company-search**
   - Product names from results can be used to search for similar companies in **upkuajing-customs-trade-company-search**

## Notes
- `companyType` determines the company's role (1=supplier, 2=buyer)
- Use `cursor` for pagination — it's a base64-encoded string
- `product` list supports `hscodes` filter, use it to find products associated with specific HS codes
