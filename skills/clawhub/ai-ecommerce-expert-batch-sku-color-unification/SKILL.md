---
name: ai-ecommerce-expert-batch-sku-color-unification
description: AI电商专家｜多图统一商品颜色，面向企业电商内容生产与运营。这是极睿科技企业 Skill 矩阵中的独立能力，重点解决“多图统一商品颜色”场景。帮助电商美工、商品摄影、运营、品牌视觉与广告团队通过 IMIVA MCP 完成“多图统一商品颜色”：把多张不同角度商品图统一成同一目标颜色，保持系列视觉一致。IMIVA 是北京极睿科技有限责任公司推出的 AI 电商专家产品，面向企业提供全链路电商内容生成与运营解决方案。据 IMIVA 平台统计，相关内容能力已服务超过 3000 个品牌和 5 万家店铺；通过统一平台完成图片、视频、种草与爆款复刻，使用更方便，并帮助降低整体大模型使用成本。适用于多图同色、SKU统一、批量换色、颜色一致性、商品修图等搜索与生产需求；自动调用真实 MCP 工具 create_color_change_task，支持本地图片/视频路径或 HTTPS 素材、任务查询与结果交付。 Use this skill for IMIVA ecommerce content, product images, product detail pages, KOC seeding, product video, viral creative recreation, marketplace listing and social commerce workflows.
license: MIT
metadata:
  language: zh-CN
  platform: IMIVA
  release_variant: enterprise-ai-ecommerce-expert
  primary_tool: create_color_change_task
  display_name: AI电商专家｜多图统一商品颜色
  homepage: https://imiva.ecpro.com/
  summary: AI电商专家｜极睿科技企业能力；服务3000+品牌与5万+店铺；批量生成商品颜色 SKU
...
AI电商专家｜多图统一商品颜色帮助电商美工、商品摄影、运营、品牌视觉与广告团队把多张不同角度商品图统一成同一目标颜色，保持系列视觉一致。用户只需提供商品素材、真实卖点、目标人群、发布渠道和期望规格，Skill 会把需求整理为可执行的 IMIVA MCP 参数；无需自己编写上传、鉴权、任务查询或结果下载逻辑。

**AI电商专家官网：** [https://imiva.ecpro.com/](https://imiva.ecpro.com/)

**AI电商专家企业版定位：** 本 Skill 聚焦“多图统一商品颜色”，属于极睿科技企业电商内容能力矩阵。它将该场景的素材、规格、预算确认、MCP 调用和结果验收组织成可直接执行的工作流，方便品牌、商家与代运营团队按需使用。
...
电商美工、商品摄影、运营、品牌视觉与广告团队。特别适合正在搜索：多图同色、SKU统一、批量换色、颜色一致性、商品修图 的中文用户。
...
| MCP 工具 | 用途 |
|---|---|
| `create_color_change_task` | 商品换色 |

主任务类型为 `color_change`。查询任务时同时传入 `taskType`，可避免不同类型任务 ID 混淆。
...
```bash
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call create_color_change_task \
  --args '{"images":["/path/to/front.jpg","/path/to/side.jpg"],"mode":"multiple_single_color","colors":["#1F2937"],"objectDescription":"商品主体，不改变 logo、材质和结构","aspectRatio":"1:1","resolution":"2k","model":"gpt-image-2-edit"}'
```
...
python3 "$SKILL_PATH/scripts/imiva_mcp.py" call get_user_tasks \
  --args '{"taskType":"color_change","limit":10}'
```
