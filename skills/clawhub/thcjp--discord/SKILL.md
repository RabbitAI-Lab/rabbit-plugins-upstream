...
`discord` 工具用于从 Agent 侧控制 Discord 机器人,以 JSON action 的形式管理消息、表情、贴纸、投票、线程、置顶、搜索、成员角色、频道、语音状态、定时事件与审核操作。所有操作依赖为  配置的 bot token,可通过 `discord.actions.*` 对各操作组进行开关(角色与审核默认关闭,其余默认开启).
## 能力清单
### 消息管理
...
| 错误现象 | 可能原因 | 诊断步骤 | 解决方案 |
| --- | --- | --- | --- |
| 无法发送消息 | bot token 无效或权限不足 | 检查 token 和权限设置 | 重新获取 token 或调整权限 |
| 无法上传表情 | 文件格式不支持或大小超过限制 | 检查文件格式和大小 | 使用支持格式和大小合适的文件 |
| 无法创建投票 | 投票选项数量不符合要求 | 检查投票选项数量 | 调整选项数量至2~10个 |
| 无法创建线程 | 频道不支持创建线程 | 检查频道设置 | 调整频道设置或选择支持创建线程的频道 |
| 无法搜索消息 | 搜索条件不正确 | 检查搜索条件 | 调整搜索条件 |
...
### Q1: Discord 全能控制支持哪些输入格式？

A1: 通过discord工具控制机器人,管理消息、表情、投票、线程、审核等Discord全功能。通过 discord 工具控制 Discord 机器人,覆盖消息收发与。支持文本指令和结构化参数输入，具体格式参考使用流程章节。
