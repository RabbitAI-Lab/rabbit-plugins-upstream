---
name: amazon-product-description-writer
description: >
  基于 Amazon 商品字段和评论反馈，识别商品描述的信息缺口并给出改写建议。仅用于商品描述诊断和建议；不用于五点、广告投放或自动发布。
  Requires an ARI API key (ari_live_*).
slug: description-writer
displayName: Amazon 商品描述优化
version: 1.4.7
summary: 基于证据改善商品描述的信息传达
license: MIT
---
...
- 本入口只执行 `listing/description`，输出模板固定为 `ops_listing`。
