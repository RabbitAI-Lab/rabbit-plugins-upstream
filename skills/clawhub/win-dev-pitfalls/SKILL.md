...
# Windows 开发踩坑经验库（bat / PowerShell / Win32 GUI / Flutter 桌面端）

> 从「银狐木马检测工具」Windows .bat + PowerShell 双文件架构开发中总结的踩坑与最佳实践，随实战扩展至 Win32 GUI 与 Flutter 桌面端。
> 适用于:制作 .bat 启动器、.ps1 引擎、Win32 GUI 程序、Flutter 桌面端、跨平台(pwsh)脚本。
...
更新规则:
1. **追加到对应章节**(编码→§1、cmd→§2、PS兼容→§3、跨平台→§4、工程实践→§5、实测→§6),没有合适章节就新增小节。**新增/修订章节后必须同步更新三层索引**(§0.1 症状索引/§0.2 任务索引/§0.3 主题地图), 索引行从章节正文提炼一句话, 不复述细节。
...
> 本 skill 章节编号按时间倒序追加(§14-§41 实战经验在前, §1-§13 基础在后), **遇到症状直接查本表**, 不必通读。备注里的"必读"标记 = 该症状的真因所在, 别的章节只是弯路或入口。

**A. 编码 / 乱码 / 闪退**

| 症状 | 优先读 | 备注 |
|------|--------|------|
| bat 闪退/语法错 | §1, §11, §29 | 嵌套括号陷阱最常见; §11 块内 echo 括号是高发点 |
| PS 5.1 无输出 exit 1 | §21 | 双重 BOM 静默拒绝执行 |
| BOM/chcp 65001 与 GBK 混合乱码 | §1, §21, §23 | 中文 Windows 环境头号坑 |
| bat 转码后乱码/二次损坏 | §35 | 转码脚本非幂等, UTF-8 误读 GBK 再写回所致 |
| 转码交付不知怎么验 | §35 | 五步门禁: 无损往返/字节头/CRLF计数/回读equals/Parser复查 |

**B. cmd / bat 语法**

| 症状 | 优先读 | 备注 |
|------|--------|------|
| `if (echo ...(...))` 块内语法错 | §11 | 块内 echo 一律方括号; §27 有二次实战复现 |
| goto 跳到不存在标签 | §29, §36 | goto 集合−标签集合求差集, 唯一豁免 :eof |
| 变量延迟展开取不到值 | §2 | `!var!` 与 `%var%` 的时机差异 |
| for /f 引用命令报错 | §2 | 单引号包命令、`2^>nul` 转义、多段后缀截取 |

**C. PowerShell**

| 症状 | 优先读 | 备注 |
|------|--------|------|
| TrimEnd('str') 行为怪异 | §23 | PS 5.1 字符串隐式转 char, 只删首字符 |
| Register-CimIndicationEvent 报参数错 | §23 | 参数集互斥, -SourceIdentifier 与 -Query 不同用 |
| Start-Process -ArgumentList 传参失败 | §22 | "-Command 字符串"下 $args 恒 null + 不接受空集合 |
| Add-Type C# 编译错(CS0165/入口点找不到) | §25 | ref 实参必须显式初始化; DllImport 模块名运行时才校验 |
| Get-AuthenticodeSignature 挂起数分钟 | §32 | 对大文件/云盘占位文件会卡死, 只对关键 exe 验签 |
| pwsh 返回 9020 | §20 | WindowsApps 执行别名在长中文路径下失败, 真实路径优先 |

**D. Win32 GUI / DLL**

| 症状 | 优先读 | 备注 |
|------|--------|------|
| GUI 不出现(双击无界面) | §13, §14, §15 | 提权子实例跳过 GUI / 可选 DLL panic / 启动期加载拖垮 |
| 1114 错误(DLL 初始化失败) | §16→§17→§18→**§19** | **必读 §19 真因**(自保护缓解策略); §17/§18"360 hook"假设已证伪 |
| 看门狗无限重启 | §16, §31 | 致命错误退出前必须 removeGuardFlag |
| 看门狗误判存活(PID 复用) | §31 | PID 被新进程复用, 需附加校验 |
| 子控件点击无响应 | §10 | 容器窗口吞 WM_COMMAND, 需转发 |
| lazyDLL 调用即崩 | §14 | LazyProc.Call 失败是 panic 不是 error, 统一 safeCall 包装 |
...
| 拦不住系统关机 | §24, §26 | 杀 shutdown.exe 无效; WM_QUERYENDSESSION=FALSE 只拦非强制, /f 拦不住(用户态边界) |
| hosts 被劫持/下载失败排障 | §28 | 检测→移除→重试链; -EncodedCommand 避开引号地狱(但见 §28.1 撞脸提醒) |

**F. Flutter / 桌面端**

| 症状 | 优先读 | 备注 |
|------|--------|------|
| 桌面端页面能开但永远空/功能静默失效 | §37, §38 | MethodChannel 无桌面实现; §37 识别+纯Dart适配, §38 三分法 |
| zip/.mcworld/mrpack 打包后嵌套文件丢失 | §38 | Windows 反斜杠条目分隔符, replaceAll('\\','/') |
| Forge/NeoForge 中文或空格路径安装必挂 | §38 | Java 加载器四连坑(argfile/JVM编码/runInShell/javaagent) |
| wmic 取值失败(Win11 24H2+) | §38 | 已移除, 改 PowerShell CIM |
| `flutter build windows` 报 No CMAKE_CXX_COMPILER / 路径编成锟斤拷 | §39 | CMake toolchain 强设 C+CXX 编译器(不只 CXX); 项目必须纯英文路径 |
| 基岩版启动器存档扫描不到/地图列表空 | §40 | `Users\<uid>` 层不可漏扫; Shared 空目录按 worldCount>0 过滤 |
| 点"打开目录"按钮页面卡死/无响应 | §41 | url_launcher 开目录会长期等待; 改 detached explorer.exe+存在性预检 |

**G. 工程 / 交付 / 发布**

| 症状 | 优先读 | 备注 |
|------|--------|------|
| 交付物拿不准是否可靠 | §29, §7 | goto 求差集+版本链漂移自查; 交付检查清单 |
| 发布流程手动太累 | §34 | 文本替换定界+版本正则身份锚点+GUI 单实例 |
| 要给包配"一键下载第三方工具" | §27 | 便携+动态识别+启动检测+按序降级, DRYRUN 模式 |
| 工具改名后实机报"文件被篡改" | §30 | 改名必须全链(exe/bat引用/manifest条目/标题/文档), grep 三编码 |
| 删除/清理操作怕误伤 | §30, §33 | 先备份→二次确认→保留跳过选项 |
| 被杀软误报困扰 | §30, §33, §5 | 工作目录铁律(严禁 C 盘用户目录)+字符串敏感度+改名策略 |
| 合并/重构旧 bat 工具箱 | §36 | 动手前必跑 goto/标签对照, 辅助转码脚本只写 ASCII |
| 多档入口用户不会选 | §26 | 按用户意图命名(「如果主程序打不开点我.exe」) |
| Wine 验证"假装正常" | §12, §13, §14 | 凡依赖真实 Windows 加载器行为的 bug, 纯 wine 复现不出 |

## 0.2 按任务场景索引(「我要做 X」入口)

> 从任务出发的反向入口。列出该任务的完整章节链, 按依赖顺序排列。

| 任务 | 章节链(按依赖顺序) |
|------|-------------------|
| 写 .bat 启动器 | §1 → §2 → §11 → §22 → §27 |
| 写 PowerShell 引擎(.ps1) | §3 → §21 → §23 → §25 → §32 |
| 做双文件交付(bat 调 ps1) | §1 → §2 → §9 → §35 |
| 开发 Win32 GUI 程序 | §9 → §10 → §14 → §15 → §16 → §19 |
| 无 Windows 机器要验证 Windows 程序 | §4 → §6 → §12(注意 Wine 局限) |
| GBK/UTF-8 编码转码交付 | §1 → §35(五步门禁) |
| 合并/重构旧 bat 工具箱 | §36 → §35 → §29 |
| 发布版本/做发布脚本 | §34 → §29 → §26 |
| 做安全/杀毒相关工具 | §5 → §28 → §30 → §33 |
| Flutter 单代码库适配桌面端 | §37 → §38 |
| 桌面端上线前审计 | §38(A-H 清单逐条过) |
| 从零编译 Flutter Windows 桌面应用(SDK→exe) | §39 → §38(A-H 审计) → §41(目录交互) |
| Flutter 桌面端扫描 Minecraft 存档 | §40 → §37(路径定位范式) |

## 0.3 章节主题地图(理解全库结构)

> 41 章 = 5 个主题域。编号是时间倒序追加的(新经验插前面), 与主题无关; 本表按主题域归类, 每个域内按"基础→实战"排列。

| 主题域 | 章节 | 说明 |
|--------|------|------|
| 基础地基 | §1-§8 | 编码/cmd/PS 兼容/跨平台/安全工具实践/实测流程/交付清单/开发环境坑 |
| Win32 GUI 链 | §9-§13, §14-§19 | 子进程启动→控件路由→**§14-§19 证伪链(全库最有价值)**→提权/UAC |
| 系统交互 | §20-§28 | pwsh 别名/BOM/Start-Process/Add-Type/关机/下载器/hosts 劫持 |
| 工程与交付 | §29-§36 | 交付审查/改名链/看门狗/验签/恢复杀软/发布自动化/转码门禁/大合并 |
| Flutter 桌面端 | §37-§41 | 双端适配范式 + 审计清单 A-H + 编译全链路 + 存档路径 + 目录交互 |

**新读者建议路径**: 基础域(§1-§8)扫一遍 → 遇到症状查 §0.1 → 接到任务查 §0.2。§14-§19 的"猜错→证伪→真凶"链建议完整读一遍——这是本 skill 最有价值的部分, 能避免你重走相同的弯路。
...
## 37. Flutter 响应式双端：移动端 MethodChannel 独占功能在桌面端"静默不可用"的识别与纯 Dart 适配(实战)

**场景**: MineAPK 客户端为 Flutter 响应式单代码库(桌面端侧边栏导航 + 移动端底部导航, 共用一套页面)。某页面在桌面端"能打开但永远空列表/数据全无", 移动端正常。

- **症状识别**: 数据来源全部走 `MethodChannel` 原生通道(如 content:// SAF 授权、Shizuku 提权、Android 路径解析、原生 zip 打包导出), 桌面端无对应原生插件实现 → 通道调用抛 `PlatformException` 被各层 try/catch 吞掉 → **降级返回 null/空列表**, 表现是"静默不可用、不报错不崩溃"。
- **排查法**: ① grep `MethodChannel` + `PlatformException` 定位 service 层(功能数据在 service, 不进 UI); ② 对照两端入口清单找缺口(桌面侧边栏导航项 vs 移动端首页快捷卡片, 常见移动端独有的入口缺失于桌面侧边栏); ③ 全库已有平台守卫范本 `isSupported => Platform.isAndroid` + 方法体 `if (!isSupported) return 降级值` + UI 侧 else 分支(如 LAN 悬浮球覆盖层), 新适配照抄该模式。
- **适配手法(纯 Dart 分支, Android 原路径一字不动防回归)**:
  1. **目录定位**: `dart:io` 展开 `%LOCALAPPDATA%`(为空兜底 `%USERPROFILE%\AppData\Local`), 默认路径 + 前缀扫描兜底, 返回"候选列表"; 找不到时把候选路径展示给用户(而不是白屏), 附"先启动一次游戏生成目录"等说明。
  2. **文件扫描**: `Directory` 流式 `await for` 遍历子目录(逐项让出事件循环, 大目录不卡 UI), 以 `level.dat`/`levelname.txt` 存在作为世界的判据; 名称取 `levelname.txt` 首行(trim 后 fallback 目录名); 封面取同级图标文件; 体积用递归求和; 按 mtime 降序。
  3. **打包导出**: `.mcworld` 本质是 zip。用 `archive` 包 `ZipFileEncoder` 流式逐文件 `addFile`(几千个 db/region 文件也不整体入内存), 归档路径=相对世界目录的相对路径(绝不套外层文件夹名, 否则导入错位); 单文件失败(被游戏占用/无权限) try/catch 跳过; 中文名需先清洗 Windows 非法字符 `\ / : * ? " < > |`。
  4. **UI 平台分流**: 所有原生入口按钮按平台分流(桌面 FAB 文案改为"打开目录"、教程/权限弹窗隐藏、空状态显示候选路径 + 打开文件夹/重新扫描按钮)。
  5. **图标本地化**: Android 的 `content://` URI 走 `Image.network`, 桌面本地绝对路径走 `Image.file` —— 用 `^[A-Za-z]:[\\/]` 或 `file://` 前缀判别, **不能无脑把 Image.network 全替换**(会破坏 Android 端)。
- **教训**:
  1. "页面能开但永远空" 第一时间查 service 层 MethodChannel 的 catch 降级, 不要先猜 UI。
  2. 直接 `import` 传递依赖(如 `archive`)必须显式写进 pubspec 并 `pub get`, 否则 `depend_on_referenced_packages` 告警。
  3. 平台分支完整隔离在 `Platform.isWindows`/`isDesktop` 之后, Android 原路径零改动, 用全量 `flutter analyze` 保 0 error 防回归。
  4. 桌面端无原生通道时优先用 `dart:io` + 标准包补 Dart 实现, 不必为此开发原生插件。
- **脱敏**: 本条不承载本机路径/签名/密钥/账号; 目录一律用 `%LOCALAPPDATA%`/`%USERPROFILE%` 范式, 方法通道名与世界文件名按已公开项目名保留。

## 38. Windows 桌面端确定性漏洞审计与修复清单(实战)

> 对 Flutter 响应式双端项目做 Windows 端代码审计，修复了一批"桌面端必现"的确定性问题。三路并行审计(进程/路径/文件、移动端插件误用、UI 列表/索引/null 崩溃)交叉验证。

- **A. MethodChannel 移动端独占 → 桌面"静默降级"**(识别特征:页面能开但永远空/功能静默失效)
  - 地图管理/世界属性/SAF 选目录/Shizuku/广告 SDK/QQ 绑定/扫码/文件选择——全是 Android 原生通道,Windows 无实现。
  - 识别法:grep `MethodChannel` + `PlatformException` 定位 service;功能数据全走 service 不进 UI。
  - 守卫模式:`isSupported => Platform.isAndroid` + 方法体 `if (!isSupported) return 降级值` + UI 侧分支。
  - 修复分层:**有桌面等价能力的给桌面分支**(文件选择→`file_selector` 系统对话框;本地通知→`WindowsInitializationSettings`/`WindowsNotificationDetails`);**无意义的能力桌面隐藏**(QQ 绑定入口、看广告按钮禁用/隐藏);**需要 UI 出路的给占位**(桌面扫码页显示"请用手机扫码"占位,不构造摄像头)。
- **B. zip 打包条目分隔符**(mrpack/.mcworld 必现)
  - Windows 上 `path.relative` 返回 `\`;zip 规范条目必须 `/`。`ArchiveFile(name)` 带反斜杠 → 导入器(MrpackImporter/Modrinth)认不出,嵌套文件静默丢失。
  - 对策:`archivePath.replaceAll(r'\', '/')`,对已是 `/` 的路径幂等安全。
- **C. wmic 已从 Win11 24H2+ 移除**(3 处) → PowerShell `Get-CimInstance`(总内存/可用内存/进程工作集)。旧代码全部返回 8192/4096 假值,内存自适应被静默限死。
- **D. Java 加载器链(4 子问题,安装 Forge/NeoForge 中文/空格路径必挂)**
  1. @argfile 裸 token:Java 按空白拆 token + 平台 charset 读 → 带空格的 classpath/中文路径被拆断/乱码。对策:每条含空格/引号/反斜杠的 token 用双引号包裹。
  2. JVM 读 argfile:argfile 按 UTF-8 写,JDK8/11 默认按平台码页(GBK)读 → 显式加 `-Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8`。
  3. `runInShell: !isFullPath && isWindows`:走 cmd.exe 会重新拆分参数 + OEM 码页转码损坏 → 恒 `false`(无 shell 时 Dart/Windows 仍按系统可执行搜索规则找 java,不需要 shell)。
  4. `-javaagent:` argv 预加引号:Process.start 底层拼命令行会自动给含空格参数加引号,预加的引号被 CreateProcess 二次解析残留 → 去掉预加引号。
- **E. Microsoft Store WindowsApps 0 字节 java 跳板**:`where java` 可能返回该别名,运行会弹商店。过滤 `\windowsapps\` 路径 + 0 字节文件;并由 `java -version` 运行时验证兜底。
- **F. reg query 解析**:输出形如 "值名 + REG_SZ + 数据",旧代码 `split('REG_')[0]` 只拿到值名。用 `RegExp(r'REG_\w+\s+(.+)$')` 取数据;环境变量(`LOCALAPPDATA`/`APPDATA`)判空,避免拼出 `null\路径`。
- **G. 服务器动态类型强转全是崩点**:`user['level'] as int` / `(certification??0) as int` / `count as int` / `exp_percent` 直接除——服务器可能给字符串/浮点。对策统一 `int.tryParse(x?.toString() ?? '') ?? 0`;进度条 `double.tryParse + clamp(0,1)`。
- **H. `substring(0, 10)` 先判 `length >= 10`**(8 处,服务器日期可能是 "2026-1-5" 短串)。抽共享工具 `safeDate10()` 一处实现全项目复用。
- **脱敏**: 本条不承载本机路径/签名/密钥/账号;目录一律用 `%LOCALAPPDATA%`/`%USERPROFILE%` 范式,方法通道名按已公开项目保留。

## 39. Flutter 桌面端 `flutter build windows` 全链路实战：从 CMake 找不到编译器到产出 exe(2026-08-29)

> 在无预装 Flutter 的机器上，从装 SDK 到 `flutter build windows --release` 成功产出 exe，走通整条链。这一节把「环境搭建 + 编译踩坑 + 源码隐错 + 外部依赖缺失」全部串起来，是 §37/§38(代码审计)的**编译执行**另一半。

### 0. 环境搭建(无 Flutter 机器)
- **Flutter SDK 下载**：`winget` 无官方 Flutter 包；官方 GitHub/存储极慢(200KB/s 要 3.5h)。**国内镜像** `mirrors.cloud.tencent.com/flutter/flutter_infra_release/releases/stable/windows/flutter_windows_<ver>-stable.zip` 可达且 **22MB/s**。清华(TUNA)返回 403、上交返回 404、阿里 404——**镜像的文件路径结构不一致，别逐个猜，先 `curl -I` 试出可达的那个**。
- **下载后校验(铁律)**：`sha256sum` 对比官方 release 清单(releases_windows.json 里 `archive: stable/windows/...` 的 `sha256` 字段) + `unzip -t` 测 zip 完整性。zip 解压后**自带一层 `flutter/` 顶层目录**，需 `mv` 上移到 `E:\flutter` 再配 PATH。

### 1. 核心坑：CMake 4.x + VS 2026 生成器"不自测 cl.exe",报 `No CMAKE_CXX_COMPILER`
- **症状**：`flutter build windows` 报 `CMake Error at CMakeLists.txt:3 (project): No CMAKE_CXX_COMPILER could be found.` 但 `flutter doctor` 显示 Visual Studio √ + Windows SDK √ + `vswhere` 能找到 VS。
- **真因**：flutter 的 windows 构建硬编码用 **`cmake -G "Visual Studio 18 2026" -A x64`(VS 生成器)**(build_windows.dart 里 `generator: cmakeGenerator`,无 ninja 选项)。CMake 4.3 + VS 18 2026 生成器在**非交互/cmd 环境**下**不自测 MSVC 路径**——即使 `vcvarsall` 加载了、`cl.exe` 在 PATH、vswhere 识别出 MSVC 19.51，CMake 仍报找不到。
- **解法(两步)**：
  1. **`CMAKE_TOOLCHAIN_FILE` toolchain 文件**(推荐,能穿透所有子项目)：写一个 `msvc_toolchain.cmake` 含 `set(CMAKE_C_COMPILER "$ENV{CMAKE_C_COMPILER}" CACHE FILEPATH "" FORCE) set(CMAKE_CXX_COMPILER "$ENV{CMAKE_CXX_COMPILER}" CACHE FILEPATH "" FORCE)`，靠**环境变量** `CMAKE_TOOLCHAIN_FILE` 让 CMake 加载。**关键优势**：toolchain 文件在**每个 `project()` 之前**都会加载，包括 `add_subdirectory` 插件子项目(sqlite3 等 `LANGUAGES C CXX`)。只设 `CMAKE_CXX_COMPILER` 不够——**插件要 C 编译器**,必须同时设 `CMAKE_C_COMPILER`(MSVC 下 C/C++ 都是 cl.exe)。
  2. 但**光设环境变量 `CMAKE_CXX_COMPILER` 对 VS 生成器不生效**(VS 生成器只认 `-D` 缓存变量,不认环境变量),所以必须靠 **toolchain 文件读环境变量转成 cache 变量**,或用 CMakeLists 里 `set(CMAKE_CXX_COMPILER "$ENV{CMAKE_CXX_COMPILER}")`(仅根项目,插件子项目不继承)。
- **为什么不靠 `vcvarsall`**：`call vcvarsall.bat` 在 `cmd` 里会**重置 PATH**,把后加的 `flutter\bin` 挤掉(`where flutter` 变"不是内部命令");且 vcvarsall 行为不稳(静默 vs 非静默结果不同)。**不要依赖它**,直接手动设环境变量 + toolchain。

### 2. 核心坑：中文项目路径让 MSBuild 把路径编成乱码 `锟斤拷`
- **症状**：项目在 `E:\网站`,Dart 产物 `app.dill` 明明**生成成功**(73MB),但 MSBuild 报 `Unable to read file: E:\锟斤拷站\.dart_tool\flutter_build\...\app.dill`。
- **真因**：`网站` 的 UTF-8 字节(`C39C E7AB 99`)被 MSBuild 子进程按 **GBK** 解码 → 变成经典乱码 `锟斤拷`。`app.dill` 路径在 Dart 层用 UTF-8 生成,MSBuild 读时用 GBK,编码失真。
- **铁律解法**：**项目放纯英文路径**。Flutter Windows 的 MSBuild/Dart 对含非 ASCII 的项目路径支持很差。把工程改名到 `E:\mineapk-by` 后一切正常。这是**一劳永逸**的规避(比逐个处理 UTF-8/GBK 转换可靠得多)。
- 注意:改名后要**删旧的 `build/windows` CMakeCache**(CMakeCache.txt 记录了旧路径,报 `CMakeCache.txt directory ... is different than ... Re-run cmake with a different source directory`),重跑即清。

### 3. 源码隐错(编译时逐个暴露,非 analyze)
- `flutter analyze` 在该项目**因中文路径崩溃**(Dart analysis server 解析 `E:/%E7%BD%91%E7%AB%99/` JSON 报 `FormatException`)。所以**只能靠 `flutter build` 的 Dart 编译器报错**逐个修。英文路径后 analyze 也不崩,但 build 报错最快。
- **`world_windows_export_service.dart`**：
  - `ZipFileEncoder` 需 `import 'package:archive/archive_io.dart'`(**不是** `archive.dart`——`archive_io.dart` 才导出 `src/io/zip_file_encoder.dart`)。
  - `return const ExportResult(...)` 报 `Cannot invoke a non-'const' constructor in a const expression`——`ExportResult` 构造器**非 const**。(去 const)。
- **`world_scan_service.dart`**：`import 'package:path/path.dart' as p;` 后要用 `p.basename()`(**不是** `p.path.basename()`——`path` 是包名不是对象,`p.path` 不存在,报 `Undefined name 'path'`)。
- **`world_manage_page.dart`**：`const TextStyle(... color: Colors.blue[900] ...)`——`Colors.blue[900]` 是**运行时索引**非常量,不能放 const 里(报 `Constant evaluation error`)。去 const。
- **`java_manager.dart`**：`_detectFromPath()` **花括号不匹配**(try/for 的 `{}` 数量错,导致 `catch` 游离、`_javaVersions` 后续引用全部报 Undefined)。这类"结构错乱"会引发**次生的大量 Undefined 报错**,先修根因(括号配对)再处理次生。

### 4. 外部依赖缺失：`windivert` 插件缺 `WinDivert.lib`
- **症状**：`LNK1181: 无法打开输入文件 E:\网站\windows\windivert\x64\WinDivert.lib`。
- **真因**：`windivert_plugin.cpp` 调用 `WinDivertRecv` 等真实符号,`windows/runner/CMakeLists.txt` 硬编码 `target_link_libraries(windivert_plugin PRIVATE "${CMAKE_SOURCE_DIR}/windivert/x64/WinDivert.lib")`,但仓库只提交了 `include/windivert.h`,**没提交 `x64/` 下的 .lib/.dll/.sys**(内部二进制不入 git)。这是"MineAPK LAN VPN"功能依赖,不能伪造,必须真实 lib。
- **解法**：WinDivert 官方站 **`reqrypt.org/windivert.html`**(GitHub 被墙、reqable.com 也 567)。从 `https://reqrypt.org/download/WinDivert-2.2.2-A.zip` 下载 `-A`(x64) 包,取 `x64/WinDivert.lib` + `WinDivert.dll` + `WinDivert64.sys` 放到 `windows/windivert/x64/`。**下载后校验是 PE** `file WinDivert.dll`。项目自带的 `windivert.h` 与官方 2.2.2 略有差异,不影响链接(API 符号按 lib 解析)。

### 5. 链接警告(正常)
- `LNK4078: 找到多个 'voltbl' 节,它们具有不同特性` —— 是 MSVC RELEASE 构建的**正常链接警告**(_has_exceptions=0 等),**不阻断**,`√ Built ...\mineapk_app.exe` 即成功。

### 6. 交付物完整性(Windows 必须带全,缺一个 exe 打不开)
- `flutter build windows --release` 产出在 `build\windows\x64\runner\Release\`:主 exe + **`flutter_windows.dll`(Flutter 引擎,必须)** + **全部插件 dll**(file_selector/window_manager/sqlite3/window_manager/tray_manager/url_launcher/audioplayers/windivert/wintun 等) + **`data/` 目录**(`app.so` + `flutter_assets/` + `icudtl.dat`)。
- **拷到目标目录**：exe + 所有 `*.dll` + `data/` 递归 + 驱动类 `.sys`(windivert 的 `WinDivert64.sys`/`wintun`)。`.lib`/`.exp` 是编译中间物**不用**拷。某插件 dll 缺失→该功能报错但不一定崩;`flutter_windows.dll` 或 `data/` 缺失→**必崩**。
- 验证 exe 有效:`file mineapk_app.exe` 应为 `PE32+ executable ... x86-64`。

### 7. 教训(方法论)
1. **先试可达源再逐个猜**:国内镜像/GitHub 加速站要在"镜像源"上先 `curl -I` 测,命中一个就定下来;清华/上交/阿里路径结构不同,别一个个试 URL 拼法。
2. **中文路径是 Flutter Windows 死穴**:项目、SDK、中间产物目录尽量全英文;`E:\网站` 这种必须改名(这是最省事的根治,不是临时绕)。
3. **CMake 编译器找不到别在 vcvarsall/PATH 上绕**:VS 生成器不自测,直接 toolchain 文件指 cl.exe,且 C/CXX 都要设。
4. **build 报的"大量 Undefined name"常是结构错的次生**:先看第一个 `Expected a class member`/`catch` 游离这类根因,再依次修,不要被表面数量吓到。
5. **外部二进制(.lib/.dll)可能不进 git**:打包前先看 CMake 链接了哪些 `target_link_libraries(...路径...)`,缺失就补,别指望 git 里有。
- **脱敏**: 本条不承载本机路径/签名/密钥/账号;Flutter SDK 版本、WinDivert 2.2.2 为已公开版本号,允许保留;目录用 `E:\...` 作路径范式描述。

## 40. Windows 基岩版启动器存档路径：`Users\\<uid>` 层不可漏扫，且要避开空的 `Shared` 目录(2026-08-29)

- **症状**：地图管理页面在 Windows 上能打开，但列表为空；UWP 商店版默认路径不存在，实际已有基岩版世界。
- **根因**：基岩版启动器（Bedrock Launcher）存档不在 UWP 的 `%LOCALAPPDATA%\\Packages\\Microsoft.MinecraftUWP_*\\LocalState\\games\\com.mojang\\minecraftWorlds`，而在 `%APPDATA%\\Minecraft Bedrock\\Users\\<uid>\\games\\com.mojang\\minecraftWorlds`。`Users` 下还可能同时有 `Shared` 空目录；只判断目录存在会选中空目录，继续显示空列表。
- **对策**：目录解析器同时支持两种发行版：①遍历 `%APPDATA%\\Minecraft Bedrock\\Users\\*\\games\\com.mojang\\minecraftWorlds` 的所有用户目录；②保留 UWP 固定包路径与 `Packages` 前缀兜底。默认候选选择按 `worldCount > 0` 优先，再选存在但为空的目录，最后才返回第一个候选。
- **世界判据**：每个世界子目录存在 `level.dat` 或 `levelname.txt` 即收录；启动器版实际世界目录还包含 `db/`、`world_icon.jpeg` 等文件。扫描后复用自定义昵称并按修改时间倒序。
- **验证**：在真实 Bedrock Launcher 存档结构中发现多个用户存档区，其中一个用户区含 3 个世界；修复后 `flutter analyze` 针对解析器 `No issues found`，Windows Release 重新编译成功并覆盖交付目录。
- **脱敏**：本条不承载具体用户 UID、账号、机器绝对路径或凭证；路径使用 Windows 环境变量与 `<uid>` 占位符。

## 41. Windows 目录按钮通过 `url_launcher` 未响应：改用 detached `explorer.exe`(2026-08-29)

- **症状**：Flutter Windows 地图管理页的“打开世界目录”按钮点击后界面长时间无响应，目录没有稳定打开。
- **根因**：Windows 上把目录直接封装为 `Uri.file(directory)` 并交给 `url_launcher.launchUrl()`，不同默认关联程序和目录 URI 处理方式可能等待或不返回，阻塞用户操作反馈。
- **对策**：Windows 目录专用入口直接调用 `Process.start('explorer.exe', [directoryPath], runInShell: false, mode: ProcessStartMode.detached)`；调用前确认目录存在，异常返回 `false`，页面显示失败提示。`ProcessStartMode.detached` 让资源管理器生命周期与 Flutter 主进程分离，不等待资源管理器退出。
- **边界**：`openWithDefaultApp()` 仍可用于 `.mcworld` 等文件的默认关联打开；只有目录打开使用 `explorer.exe` 专用分支。
- **验证**：修改后针对相关 Dart 文件检查无新增 error，Windows Release 构建成功并覆盖交付目录；主程序为有效 x64 PE，关键依赖和 `data/` 均存在。
- **脱敏**：不记录具体本机路径、账号或凭证。

更新日志(倒序):
- **v1.72.0 (2026-08-30)**:**第四次双副本分叉合流——新增 Flutter 桌面端实战三章**:用户侧副本(2026-08-29 迭代)新增三章, 其本地版本号 v1.71.0/v1.72.0/v1.73.0 与主线已发布 tag 链 v1.71.0(三层索引)撞车, 按 ClawHub tag 链原则用户侧编号让位, 三章合流为单一 v1.72.0: ① **§39 `flutter build windows` 全链路**(原用户侧v1.71.0, 从装 SDK 到产出 exe 完整链): SDK 国内镜像(官方/清华/上交/阿里路径结构不同, `curl -I` 先测)+SHA256 对比 release 清单+zip 顶层 `flutter/` 上移; CMake 4.x+VS 2026 生成器不自测 cl.exe 报 No CMAKE_CXX_COMPILER——**CMAKE_TOOLCHAIN_FILE toolchain 强设 C+CXX 编译器**(C/CXX 都要设, 穿透 add_subdirectory 插件子项目; VS 生成器不认纯环境变量, 必须经 toolchain 转 cache 变量); **中文项目路径让 MSBuild 把路径编成锟斤拷**——铁律=项目放纯英文路径(改名后删旧 build/ 下 CMakeCache); flutter analyze 因中文路径崩(analysis server JSON FormatException)只能靠 build 报错逐修; 源码隐错四类(ZipFileEncoder 走 archive_io 非 archive.dart / ExportResult 非 const / p.basename 非 p.path.basename / Colors.blue[900] 非 const 不能进 const); windivert 缺 WinDivert.lib 报 LNK1181——外部二进制不入 git, reqrypt.org 取 x64 三件套; LNK4078 voltbl 正常警告不阻断; 交付物完整性(exe+flutter_windows.dll+全部插件 dll+data/ 含 app.so/flutter_assets/icudtl.dat; dll 或 data 缺则必崩, .lib/.exp 可不拷); 方法论(镜像先 curl -I 测可达 / 中文路径是 Flutter Windows 死穴应改名根治 / CMake 编译器找不到别在 vcvarsall 上绕 / build 大量 Undefined 常是结构错次生先修根因); ② **§40 基岩版启动器存档路径**(原用户侧v1.72.0): 地图管理原仅扫 UWP 商店版 `%LOCALAPPDATA%\Packages\Microsoft.MinecraftUWP_*\LocalState\...`, 漏掉 Bedrock Launcher 的 `%APPDATA%\Minecraft Bedrock\Users\<uid>\games\com.mojang\minecraftWorlds`; **Users 下同时有 UID 真实存档目录与 Shared 空目录**, defaultRoot() 改 worldCount>0 优先, 避免"目录存在但地图空"误选; 世界判据维持 level.dat/levelname.txt; 路径拼接统一 `p.join()` 防手工拼分隔符; ③ **§41 目录按钮 url_launcher 未响应**(原用户侧v1.73.0): `launchUrl(Uri.file(directory))` 打开目录可能长期等待致页面无响应; 目录专用分支改 `Process.start('explorer.exe', [path], runInShell:false, mode:ProcessStartMode.detached)`+调用前检查目录存在, 失败返回并提示; 文件默认关联打开保留给 .mcworld 等; ④ **导航层同步**: §0.1 症状索引 F 组补 3 行(编译报错/存档扫描/目录卡死), §0.2 任务场景补 2 行(从零编译链/存档扫描), §0.3 主题地图 Flutter 域 §37-§38→§37-§41(38→41 章), 实战范围 §14-§38→§14-§41; ⑤ 脱敏: UID/本机路径均用 `<uid>`/`%LOCALAPPDATA%` 范式, 不承载签名/密钥/账号。
- **v1.71.0 (2026-08-29)**:**导航层重构——三层索引体系**:① **§0.1 症状索引全面重写**: 由 22 行单表扩为 7 个主题组(A 编码乱码/B cmd 语法/C PowerShell/D Win32 GUI DLL/E 提权系统/F Flutter 桌面端/G 工程交付)共 36 行, 补齐旧索引遗漏的 §25(Add-Type)/§26(多档入口)/§27(下载器)/§30(改名链)/§33(恢复杀软)/§10(控件路由)/§31(PID 复用)等 8 个章节的入口; 备注列强化"必读=真因所在, 其余是弯路"的指引语义(§19/§28.1 反向提醒); ② **新增 §0.2 任务场景索引**: 11 条「我要做 X」反向入口(写 bat 启动器/写 PS 引擎/做双文件交付/开发 Win32 GUI/无 Windows 验证/转码交付/合并工具箱/发布/安全工具/Flutter 适配/上线前审计), 章节链按依赖顺序排列——解决"不知道自己症状叫什么"的冷启动问题; ③ **新增 §0.3 章节主题地图**: 38 章按 5 个主题域归类(基础地基/Win32 GUI 链/系统交互/工程与交付/Flutter 桌面端), 说明时间倒序编号与主题无关的事实, 新读者路径改为「基础域→§0.1 症状→§0.2 任务」; ④ 索引行内容全部从章节正文提炼, 无新增正文章节(38 章不变); ⑤ 脱敏: 索引不承载机器路径/签名/密钥。
- **v1.70.0 (2026-08-29)**:**第三次双副本分叉合流——新增 Windows 桌面端 Flutter 适配两章**:用户侧副本(2026-08-28 迭代)新增两章, 原编号 §35/§36 与主线已发布 tag 链的 §35(转码幂等)/§36(合并工具箱)冲突, 合流重编号为 §37/§38, 用户侧原版本号 v1.68.0/v1.69.0 让位主线(以 ClawHub tag 链为准, §29 版本链漂移原则): ① **§37 Flutter 响应式双端**(原§35): 移动端 MethodChannel 独占功能在桌面端"静默不可用"(PlatformException 被 catch 降级空值)的识别与纯 Dart 适配——症状识别(页面能开但永远空)/排查法(grep MethodChannel 定位 service 层 + 两端入口清单对照, 复用 isSupported 守卫范本)/适配手法(目录定位 %LOCALAPPDATA%+前缀扫描兜底 / Directory 流式扫描 / ZipFileEncoder 流式打包 / UI 平台分流 / 图标 content:// 与本地路径正则判别)/教训(显式 pubspec 声明传递依赖 / 分支全隔离在 Platform.isX 后防 Android 回归); ② **§38 Windows 桌面端确定性漏洞审计与修复清单**(原§36): A MethodChannel 桌面静默降级三分法(桌面分支/隐藏/占位)、B zip 条目分隔符 replaceAll('\\','/')、C wmic→PowerShell CIM、D Java 加载器四连坑(argfile 引号包裹+JVM UTF-8+runInShell 恒 false+-javaagent 去预引号)、E WindowsApps 0字节 java 跳板过滤、F reg query 正则取数据段、G 服务器动态类型 tryParse+clamp、H substring 先判长(共享 safeDate10); ③ **§0.1 索引补 3 行**(桌面空数据/zip 嵌套丢失/中文路径 Java 挂), 实战章节范围 §14-§36→§14-§38; ④ **合流要点**: 主线的 §0.1 索引/§17 证伪引导/§28.1 撞脸提醒/双语 description 为用户侧副本所缺, 两侧互补无覆盖冲突(除编号); ⑤ **改名发布**: `bat-ps1-dev` → `win-dev-pitfalls`(内容范围已扩展至 Flutter 桌面端/Win32 GUI, 原名"bat/ps1"名不副实; 旧 slug 在 ClawHub 保留, 新名独立成库, 关键词兼容 bat/ps1 检索); ⑥ 脱敏: 不承载机器路径/签名/密钥/账号。
