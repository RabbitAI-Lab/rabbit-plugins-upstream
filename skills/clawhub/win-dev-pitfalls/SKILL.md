...
> 本 skill 章节编号按时间倒序追加(§14-§38 实战经验在前, §1-§13 基础在后), **遇到症状直接查本表**, 不必通读。备注里的"必读"标记 = 该症状的真因所在, 别的章节只是弯路或入口。
...
> 38 章 = 5 个主题域。编号是时间倒序追加的(新经验插前面), 与主题无关; 本表按主题域归类, 每个域内按"基础→实战"排列。
...
| Flutter 桌面端 | §37-§38 | 双端适配范式 + 确定性审计清单 A-H |
