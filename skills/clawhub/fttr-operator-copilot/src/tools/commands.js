...
const EXECUTE_CMD_NON_STREAM = "/bison.admin.fttrmanage.v1.FttrManageService/ExecuteCMDNonStream";
const GET_EXECUTE_RESULT = "/bison.admin.fttrmanage.v1.FttrManageService/GetExecuteResult";
...
async function executeNamedCommand(client, args, commandSpec) {
  ...
  const executeResponse = await client.unary(EXECUTE_CMD_NON_STREAM, {
    mac: resolved.mac,
    command,
  });
  ...
  const result = await pollExecuteResult(client, resolved.mac, sequenceID, waitTimeoutMs, pollIntervalMs);
...
async function pollExecuteResult(client, mac, sequenceID, waitTimeoutMs, pollIntervalMs) {
  ...
  for (;;) {
    try {
      ...
      if (!isPendingResult(error)) throw error;
    }

    if (Date.now() >= deadline) return { timed_out: true, response: null };
...
async function tryFallback(fallback) {
  try {
    ...
    return { title: result.title, summary: result.summary, data: result.data };
  } catch (error) {
    return { error: { code: error.code || "unknown", message: error.message } };
  }
}
