...
def load_practice_entries():
    """只取练习记录（过滤掉 reflection 类型的看清记录）。"""
    return [e for e in load_entries() if e.get("type", "practice") != "reflection"]
...
def cmd_export(args):
    ...
    if fmt == "json":
        ...
        lines = ["# 我的斯多葛练习档案", "",
                 ...
                 f"共 {len(entries)} 条练习记录、{len(reflections)} 条看清记录", ""]
        # 困境地图章节：先看地图，再看流水
        if reflections:
            uns = [r for r in reflections if r.get("uncontrollable")]
            cs = [r for r in reflections if r.get("controllable")]
        ...
        with open(out, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))
    print(f"✅ 已导出：{len(entries)} 条练习 + {len(reflections)} 条看清 → {out}")
    return 0
