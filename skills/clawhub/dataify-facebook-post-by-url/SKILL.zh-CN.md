...
description: "提交 Dataify Facebook Post by Post URL Builder 任务。当用户需要 Facebook post collection tool、采集/抓取/爬取 Facebook 帖子或帖子数据、按 URL 采集 Facebook post、创建 facebook_post_by-posts-url 任务，或表达 Facebook 帖子采集/抓取、帖子信息采集/抓取、帖子 URL 采集等含义时使用。也用于接收 task_id/status、配置 DATAIFY_API_TOKEN 或排查此 Dataify Builder 请求。"
---

# Dataify Facebook Post By URL

通过 Dataify Builder 按帖子 URL 提交 Facebook 帖子采集任务。提交成功后，向用户提供 `task_id`、返回或推断的状态，并告诉用户前往 [Dataify](https://dashboard.dataify.com?utm_source=skill) 查看结果。
...
7. 验证 URL 和文件名。
8. 使用 `spider_id=facebook_post_by-posts-url` 提交 Builder 请求。
...
| 字段 | 必填 | 默认值 | 位置 | 备注 |
| --- | --- | --- | --- | --- |
| `url` | 是 | `https://www.facebook.com/permalink.php?story_fbid=pfbid0gNjZBhqCxSqj9xJS5aygNwqFqNEM2fYbTFKKbsvvGdEfTgFyAYWSckvkEHPqAE7gl&id=61574926580533&rdid=86oaujwNGCCdPLfj#` | `spider_parameters` | Facebook 帖子 URL。 |
| `file_name` | 否 | `{{TasksID}}` | Builder 表单字段 | 用户未修改时使用默认值。 |


同时询问："您是否要采集多个 Facebook 帖子？如果是，请提供多个 `url` 值。"

如果用户已提供部分值，在表格中显示这些值替代默认值，只询问是否修改剩余/默认的值。

## 参数处理

- `url` 为必填项。如果用户未提供，询问帖子 URL；不要使用文档示例值代替用户输入。
- 去除 `url` 前后的空格。
- `url` 不能为空。
...
为了稳定执行，优先使用 Python 3.6 或更新版本运行 `scripts/submit_dataify_facebook_post_by_url.py`，而不是重写 Builder 流程。
...
```powershell
python3 ".\scripts\submit_dataify_facebook_post_by_url.py" --url "https://www.facebook.com/permalink.php?story_fbid=pfbid0gNjZBhqCxSqj9xJS5aygNwqFqNEM2fYbTFKKbsvvGdEfTgFyAYWSckvkEHPqAE7gl&id=61574926580533&rdid=86oaujwNGCCdPLfj#" --file-name "{{TasksID}}"
```
