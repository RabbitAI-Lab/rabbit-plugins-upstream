...
- `studio/market/twstock/kbar/<stock_id>` — 分K（1分鐘 OHLCV）**legacy, prefer `minute/ohlcv/<stock_id>/1m`**: reads the same on-disk minute store as `minute/ohlcv` (no upstream call); `start`/`end` YYYY-MM-DD required; max 31 days per request; fields: `date`, `minute` (HH:MM:SS, bar start, 09:00–13:30), `stock_id`, `open`, `high`, `low`, `close`, `volume`; data from 2019-01-01 for ids in `minute/ohlcv/symbols` (unknown / delisted id → 400); today's bars are provisional intraday, official after 16:10 Taipei
