...
- `studio/market/twstock/batch/<data_type>` — **批次查詢（大型 universe 用）**: `data_type` ∈ {`price_adj`, `institutional`, `shareholding`, `foreign_shareholding`, `financials`, `balance_sheet`, `monthly_revenue`}; `?stock_ids=2330,2317,...`（最多 50 支）+ `start`/`end`（視類型而定）; 回傳 `{"data_type": "...", "data": {"2330": [...], ...}}`; server-side 平行 fetch + Redis cache；在 Blave Agent 中用對應 `_batch` lib 函式而非直接呼叫此 endpoint
