...
## 資金費率 Funding Rate (FR)

永續合約多空之間的週期性費用，用來把永續價格錨定到現貨。Binance only。`alpha` = 資金費率 × 100（即百分比值），`close` 為永續價格。

| Value | Meaning |
|---|---|
| 正值 | 多頭付費給空頭，永續溢價，市場偏多／多頭擁擠 |
| 負值 | 空頭付費給多頭，永續折價，市場偏空／空頭擁擠 |
| 絕對值越大 | 槓桿與單邊擁擠程度越高，反轉與軋倉風險升高 |

- 長期高正值 → 多頭過度擁擠，回調／long squeeze 風險升高
- 長期負值 → 空頭過度擁擠，反彈／short squeeze 風險升高
- 接近 0 → 多空平衡，無明顯方向偏好
...
## 異常漲跌 Unusual Movement (UM)

將幣種價格表現與歷史波動性（365d 滾動標準差）做 z-score 標準化後排行，屬動能分析指標。

| Value | Meaning |
|---|---|
| > 0.5 / > 1 / > 2 | 較強 / 明顯強力 / 極端的向上動能，可能暗示潛在多頭行情 |
| < -0.5 / < -1 / < -2 | 較強 / 明顯強力 / 極端的向下動能，可能暗示潛在空頭行情 |

**注意：**
- 是相對歷史波動性的「罕見程度」排行，不是絕對漲跌幅
- 可搭配巨鯨警報 / 籌碼集中度判斷是否為巨鯨行為驅動
...
## 指數除息點數 Index Dividend Points（台股）

`/studio/market/twmarket/dividend_points` — 正逆價差（期現價差）判讀的必要修正項。

**核心規則：台指期的價差不能拿期貨價直接減現貨指數。** 除息季（6-9 月尤其密集）指數會因成分股除息而「憑空下跌」，期貨價格已預先反映——未扣除息點數的原始價差在除息季永遠深逆價差，會被誤讀成空頭訊號。

```
fair basis = 期貨價 −（現貨指數 − 結算日前未除息點數合計）
```
