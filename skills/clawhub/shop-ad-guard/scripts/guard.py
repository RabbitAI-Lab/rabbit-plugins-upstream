#!/usr/bin/env python3
"""电商广告合规护栏检测内核 (shop-ad-guard)。

实时护栏：在商品标题、详情页、直播话术、促销文案发布前，检测其中
高频违规用语（绝对化用语 / 医疗功效宣称 / 虚假夸大 / 虚假促销 /
比较贬低 / 迷信诱导），按风险分级输出命中与整改建议，供 Agent 主动调用。
...
用法：
    ...
    echo "全网最低价限时特惠" | python3 scripts/guard.py --stdin
...
def main(argv=None):
    p = argparse.ArgumentParser(description="电商广告合规护栏检测内核")
    ...
    if args.format == "json":
        ...
        if result["decision"] == "clean":
            print("未检出已知高频违规用语；发布前仍建议结合完整合规审计。")
        else:
            print("检出违规用语，建议按建议修改后再发布；如需完整审计可使用 shop-ad-check-pro。")
    return 0
