...
一份跟著專案走的常駐檔案，記錄 40 個考量項目各自的狀態。**不是每次重新推導的報告**，而是持續增量維護的狀態表。
...
```yaml
  S21:
    status: tbd
    note: 是否引入 Redis 作為訊息狀態快取？決定後會影響 S13 架構圖與 S32 部署
    owner: <技術負責人>
    blocks: [S13, S32]
```
...
```yaml
❌ note: Cache 策略待定
✅ note: 是否引入 Redis？決定後影響 S13 架構圖與 S32 部署
```
...
```yaml
  S31:
    status: n/a
    note: 本系統不處理任何檔案上傳，所有附件由 Shopee 端託管
```
...
items:
  ...
  S06:
    ...
    note: 含 Shopee Open Platform 審核限制
  S08: { status: confirmed, file: 02-business-design/08-business-rules.md }
  S09:
    status: draft
    file: 02-business-design/09-state-machines.md
  ...
  S21:
    status: tbd
    note: 是否引入 Redis？影響 S13 架構圖與 S32 部署
    owner: <技術負責人>
    blocks: [S13, S32]
  S28:
    status: missing
  ...
  S31:
    status: n/a
    note: 本系統不處理檔案上傳，附件由 Shopee 端託管
  S33: { status: missing, note: 尚未討論備份與復原，建議上線前補 }
  S39:
    status: tbd
    note: AI 評測基準線需在 S18 模型選型後才能定義
    owner: <技術負責人>
```

未列出的項目視同 `missing`，但**建議明確列出**所有 40 項 —— 一份看得見全貌的檔案才有稽核價值。
