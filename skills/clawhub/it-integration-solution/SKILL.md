...
| 项目名称 | 方正小标宋简体 | Times New Roman | 小一号（24pt） | 居中 |
| 文档类型 | 黑体 | Times New Roman | 小二号（18pt） | 居中 |
...
- `表格表头` — 表头内容（黑体、小五号、加粗）
- `题注` — 表格/图片题注（宋体、五号、居中）
- `落款` — 落款信息（方正仿宋、小三号、右对齐，需手动调用 createSignatureBlock）
...
- 落款不自动添加，如需落款请手动调用 `createSignatureBlock(company, date)` 函数
