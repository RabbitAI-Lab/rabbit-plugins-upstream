"""CLI argument parsing tests for lulc-classification-ml."""
import os
import subprocess
import sys

sys.path.insert(0, os.path.dirname(__file__))
from conftest import mod as ml, SCRIPT
...
class TestCLIBasics:
    ...
    def test_bad_method_exit_2(self):
        r = run_cli(["--bbox", "116", "39", "117", "40", "--method", "svm"])
        assert r.returncode == 2
...
class TestCLIRun:
    def test_synthetic_run_ok(self, tmp_path):
        ...
        r = run_cli([
            ...
            "--n-classes", "4", "--method", "rf", "--quiet",
            "--output-dir", out,
        ])
        assert r.returncode == 0, r.stderr
        assert os.path.exists(os.path.join(out, "lulc_classified.tif"))
        assert os.path.exists(os.path.join(out, "accuracy.json"))
    ...
    def test_bbox_only_autosynthetic_ok(self, tmp_path):
        ...
        r = run_cli([
            "--bbox", "116", "39", "117", "40",
            "--n-classes", "3", "--quiet", "--output-dir", out,
        ])
        assert r.returncode == 0, r.stderr
        assert os.path.exists(os.path.join(out, "output-manifest.json"))
