...
def cmd_providers(args):
    """Vores provider-register (Northcap x402 API) + leaderboard."""
    import ssl
    ctx = ssl.create_default_context()
    ctx.check_hostname = False
    ctx.verify_mode = ssl.CERT_NONE
    for path, label in [("https://localhost:8791/v1/providers", "REGISTER"),
                        ("https://localhost:8791/v1/providers/leaderboard", "LEADERBOARD")]:
        try:
            ...
            print(f"=== {label} ===")
            if label == "REGISTER":
                print(f"  Providers: {d.get('count', 0)} | feeRate: {d.get('feeRate')}")
                for p in d.get("providers", [])[:10]:
                    print(f"  • {p.get('agentName')} | {p.get('status')} | referrals: {p.get('referrals',0)} | badges: {p.get('badges',[])}")
            else:
                for r2 in d.get("leaderboard", [])[:10]:
                    print(f"  • {r2.get('agentName')} | referrals: {r2.get('referrals')} | badges: {r2.get('badges')}")
...
def main():
    ...
    cmds = {"post": cmd_post, "comment": cmd_comment, "engagement": cmd_engagement,
            ...
            "replies": cmd_replies, "hot": cmd_hot, "providers": cmd_providers}
