---
name: google-hotels-compare
description: "Find the cheapest available rate for one hotel via the Google Hotels backbone — the offers it exposes plus a computed min and median over them. Use for \"cheapest way to book this hotel\". Powered by StayingAPI."
version: "1.0.0"
license: MIT-0
author: StayingAPI
...
tags: ["google-hotels", "price-comparison", "cross-ota", "metasearch", "travel"]
metadata: {"openclaw":{"emoji":"⚖️","requires":{"env":["STAYINGAPI_KEY"]},"primaryEnv":"STAYINGAPI_KEY","homepage":"https://stayingapi.com"},"hermes":{"tags":["google-hotels","price-comparison","cross-ota","metasearch","travel"],"category":"integrations"}}
---

# Google Hotels cross-OTA comparison

Rate-shop one hotel through the Google Hotels backbone: StayingAPI returns the offers it can resolve for the property, normalized, with a computed lowest and median over those offers. Check `offers.length` — coverage ranges from several OTA rates down to a single aggregated-lowest rate.
...
- "What is the cheapest site to book this hotel?"
- "Compare OTA rates for this property"

**Do NOT use when:**

- You want a list of hotels — use google-hotels-search
...
On an MCP-capable runtime, connect `https://mcp.stayingapi.com/mcp` (OAuth 2.1 + PKCE) and use: `compare_prices`.
