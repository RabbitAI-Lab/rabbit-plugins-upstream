...
# -*- coding: utf-8 -*-
"""
沪深300多因子投研系统 - 完整版（纯Python+akshare，无numpy/pandas依赖）
"""
...
def generate_report(results):
    ...
    report.append(f"**数据源：{'AKShare真实行情' if HAS_AKSHARE else '模拟数据（演示）'}**")
    report.append("")
    report.append("---")
    ...
    if not HAS_AKSHARE:
        report.append("*注：当前使用模拟数据，请安装AKShare获取真实行情数据*")
...
def run_analysis():
    ...
    if HAS_AKSHARE:
        print("✅ AKShare已加载，将获取真实行情数据")
    else:
        print("⚠️  AKShare未安装，使用模拟数据（演示模式）")
        print("   运行: pip install akshare 以获取真实数据")
    print()
    
    # 获取所有股票数据
    print(f"开始分析 {len(HS300_STOCKS)} 只沪深300成分股...")
    print()
    ...
    print()
    print(f"分析完成！成功 {success_count}/{len(HS300_STOCKS)} 只股票")
    print()
