---
name: telegraph_transfer_ocr
description: 支持电汇凭证识别,自动提取汇款人与收款人名称、账号、金额、日期及汇出行/汇入行等核心信息。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 电汇凭证识别 OCR 技能

本技能封装了电汇凭证识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，自动提取汇款人与收款人名称、账号、金额、日期及汇出行/汇入行等核心信息。

## 功能特性

- **电汇凭证识别**：支持识别电汇凭证信息，自动提取汇款人与收款人名称、账号、金额、日期及汇出行/汇入行等核心信息。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• TELEGRAPHIC_TRANSFER_VOUCHER（电汇凭证）|
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/telegraph_transfer_ocr/scripts/main.py TELEGRAPHIC_TRANSFER_VOUCHER /path/to/test.jpg
```
...
- “帮我识别这张图像中的电汇凭证信息，图片在 /Users/name/Downloads/test.jpg”
