...
license: Proprietary
edition: pro
description: "核心能力: 内容营销领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要生成营销文案、写作内容、标题优化、内容创作时使用。不适用于纯技术文档撰写。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。 功能涵盖: blog, writer。"
...
> **功能说明**: 本技能涵盖 中文交互、营销文案、化工作流场景 等核心能力。
...
**处理**: 解析竞品内容差距分析的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回竞品内容差距分析的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 长尾关键词矩阵生成
长尾关键词矩阵生成,支持单次产出 20+ 篇主题集群

**处理**: 解析长尾关键词矩阵生成的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回长尾关键词矩阵生成的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多语言多地区 SEO 适配与 
多语言多地区 SEO 适配与 hreflang 标签建议

**处理**: 解析多语言多地区 SEO 适配与 的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多语言多地区 SEO 适配与 的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 与 CMS 发布管道集成自动发
与 CMS 发布管道集成自动发布

**处理**: 解析与 CMS 发布管道集成自动发的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回与 CMS 发布管道集成自动发的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 结构化数据标记建议(Schem
结构化数据标记建议(Schema.org)

**处理**: 解析结构化数据标记建议(Schem的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回结构化数据标记建议(Schem的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：内容工坊、支持竞品分析、长尾词矩阵、多语言生成与发布、管道自动化、核心能力、内容营销领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | SEO 博客写作专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "SEO 博客写作专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "blog seo writer pro"
    }
  },
...
- **自动化执行**: 企业级 SEO 内容工坊,支持竞品分析、长尾词矩阵、多语言生成与发布管道自动化。核心能力: 内容营销领域的专业化 AI 
