...
把 Claude Code / Claude 订阅挂到 asale 市场上：导入账号、设定价格底价与并发上限，并查看它的哪些模型真的在卖。
...
Claude 订阅会在 Claude Code 的钥匙串条目、`~/.claude/.credentials.json`
或 opencode 的 `auth.json` 里找到。三处存的是同一次登录时会合并成一个 `claude` 账号——
行上的 `sources` 会列出它在哪些存储里被看到过。

挑 `provider` 是 `claude` 的那一行，它的 `account_id` 就是后面每个调用用来指代它的名字。
...
下架是本地操作，不需要会话——因为会话过期就没法**停止**卖出，那才是更糟的失败。
