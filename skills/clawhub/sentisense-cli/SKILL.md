...
install: `npx` fetches and runs it on demand. Use version 0.44.0 or newer.
...
mode 600). `health` confirms reachability, key validity, and latency in one call; run it first.
...
Run `npx -y sentisense@latest --help` for the full list, and `help <command>` for flags and
...
```bash
npx -y sentisense@latest insiders "$TICKER" || echo "exit $? tells you which way it failed"
```
...
npx -y sentisense@latest quote NVDA AMD AVGO

# Feed a field into another tool
npx -y sentisense@latest quote NVDA --json | jq .changePercent

# Screen, then inspect the top hit
npx -y sentisense@latest screen --filter SENTI_SCORE_7D:GTE:13 --limit 5
```
