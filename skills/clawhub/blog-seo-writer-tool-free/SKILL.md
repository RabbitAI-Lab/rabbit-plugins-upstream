...
description: "创建 SEO 优化博客文章,包含关键词集成、元描述生成与结构化标题,适合个人博主。Use when 需要生成营销文案、写作内容、标题优化、内容创作时使用。不适用于纯技术文档撰写。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
license: Proprietary
...
**输出**: 返回根据目标关键词生成 SEO 友的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自动撰写 150-160 字符
自动撰写 150-160 字符 Meta Description

**输出**: 返回自动撰写 150-160 字符的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 生成 H2/H3 层级结构化大
生成 H2/H3 层级结构化大纲

**输出**: 返回生成 H2/H3 层级结构化大的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自然融入主关键词与长尾变体
自然融入主关键词与长尾变体

**输出**: 返回自然融入主关键词与长尾变体的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 提供内链外链建议与图片 Alt
提供内链外链建议与图片 Alt 文本

**输出**: 返回提供内链外链建议与图片 Alt的执行结果,包含操作状态和输出数据。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
