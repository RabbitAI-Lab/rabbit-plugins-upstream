---
name: business-license-recognition-and-extract
description: 基于来也科技ADP平台的中国营业执照智能识别与信息抽取Skill。支持营业执照全量字段的精准抽取——注册号、名称、类型、法定代表人、成立日期、营业期限、经营范围、登记机关、发证日期，输出结构化JSON，零配置开箱即用，适用于企业资质核验、工商信息查询等场景。
---

# 营业执照识别与抽取 Skill

能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=clawhub)提供，支持中国营业执照的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成营业执照图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（每月刷新），相当于每月可免费抽取 **200 张营业执照**。ADP 提供标准可商用 API，**1 小时即可快速集成接入业务系统**。
...
3. **获取应用列表**：首次执行时，通过 `adp app-id list --app-type 0` 获取开箱即用应用列表，找到营业执照抽取应用并记录其 `app_id`（以 `ootb_` 开头）。后续优先使用 `adp app-id cache`。
4. **执行抽取**：运行 `adp extract url <URL> --app-id <营业执照抽取应用ID>` 或 `adp extract local <文件路径> --app-id <营业执照抽取应用ID>`。
5. **结果处理**：解析返回的 JSON，提取营业执照注册号、名称、类型、法定代表人、成立日期、营业期限、经营范围、登记机关、发证日期等结构化字段。
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张营业执照图片（URL） | `adp extract url <URL> --app-id <营业执照抽取应用ID>` |
| 识别一张本地营业执照图片 | `adp extract local <文件路径> --app-id <营业执照抽取应用ID>` |
| 识别 Base64 编码的营业执照 | `adp extract base64 <base64> --app-id <营业执照抽取应用ID> --file-name <文件名.后缀>` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的营业执照 | `adp extract local <文件夹路径> --app-id <营业执照抽取应用ID>` |
| 批量识别多个 URL | `adp extract url <URL列表文件> --app-id <营业执照抽取应用ID>` |

**异步处理**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 异步提交大文件 | `adp extract url <URL> --app-id <营业执照抽取应用ID> --async` |
| 异步批量处理 | `adp extract local <文件夹路径> --app-id <营业执照抽取应用ID> --async` |
...
ADP 为中国营业执照提供了**开箱即用**的内置抽取应用，无需额外配置。
...
营业执照识别属于**开箱即用应用**，可通过 `--app-type 0` 筛选查询。
...
从返回列表中找到 `app_label` 包含 **"营业执照"** 的应用，记录其 `app_id`：

```json
[
  {
    "app_id": "ootb_******5a8b",
    "app_label": ["营业执照", "企业资质", "工商凭证", "商业证件", "信息提取"],
    "app_name": "营业执照",
    "app_type": 0
  }
]
...
> 上例中 `"app_id": "ootb_******5a8b"` 即为营业执照抽取应用。`app_type` 为 `0` 表示开箱即用应用，`1` 表示自定义应用。
...
**重要提示**：每个账号下的 `app_id` 是唯一且固定的，除非用户主动删除应用，否则 `app_id` 不会变更。建议 Agent 将营业执照应用的 `app_id` 保存在上下文中，下次直接使用。
...
```bash
adp extract url https://example.com/business-license.jpg --app-id <营业执照抽取应用ID>
```

### 单张营业执照抽取（本地文件）

```bash
adp extract local ./business-license.jpg --app-id <营业执照抽取应用ID>
```

### 单张营业执照抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <营业执照抽取应用ID> --file-name <文件名.后缀>
```

### 返回结果示例

ADP 营业执照抽取统一返回结构化 JSON，支持营业执照正页和副页共 **12 个**关键字段识别抽取。

> **隐私脱敏提示**：以下示例中证件号码、姓名、住址等个人敏感信息均已脱敏（使用 `*` 占位）。

```json
[
  {
    "field_key": "unified_social_credit_code",
    "field_name": "统一社会信用代码",
    "field_values": [
      {
        "field_value": "91110108MA01XXXXXX"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "field_values": [
      {
        "field_value": "软件开发、技术服务、技术咨询、技术交流、技术转让、技术推广；计算机系统服务；数据处理；基础软件服务；应用软件服务；信息系统集成服务；销售自行开发的软件产品。"
      }
    ]
  },
  {
    "field_key": "registered_capital",
    "field_name": "注册资本",
    "field_values": [
      {
        "field_value": "100万元人民币"
      }
    ]
  },
  {
    "field_key": "business_operation_period",
    "field_name": "营业期限",
    "field_values": [
      {
  ...
  },
  {
    ...
    "field_values": [
      {
        "field_value": "北京市朝阳区某某街道某某号院某某楼某某室"
      }
    ]
  },
  {
    "field_key": "composition_form",
    "field_name": "组成形式",
    "field_values": [
      {
        "field_value": "有限责任公司(自然人投资或控股)"
      }
    ]
  },
  {
...
ADP 营业执照抽取返回以下字段（覆盖正副页 12 个字段）：

| field_key | field_name | 说明 | 敏感级别 |
| --- | --- | --- | --- |
| `unified_social_credit_code` | 统一社会信用代码 | 是中国给企业、事业单位、社团等法人 / 组织发的18 位唯一身份编码，相当于单位的 “身份证号” | **高（PII）** |
| `is_copy` | 是否为副本 | 标识该营业执照是否为副本，值为 “是” 或 “否” | 低 |
| `name` | 姓名 | 法定代表人姓名 | **高（PII）** |
| `type` | 类型 | 证件类型，值为 “营业执照” | 低 |
| `business_scope` | 经营范围 | 营业执照上登记的经营范围描述文本 | 中 |
| `registered_capital` | 注册资本 | 营业执照上登记的注册资本金额及币种 | 中 |
| `business_operation_period` | 营业期限 | 营业执照上登记的营业期限，通常格式为 “YYYY-MM-DD 至 YYYY-MM-DD” | 中 |
| `business_place` | 营业场所 | 营业执照上登记的营业场所地址 | **高（PII）** |
| `composition_form` | 组成形式 | 营业执照上登记的组成形式，如 “有限责任公司(自然人投资或控股)” | 低 |
| `operator` | 经营者 | 营业执照上登记的经营者姓名，可能与法定代表人相同或不同 | **高（PII）** |
| `registered_date` | 注册日期 | 营业执照上登记的日期，通常为企业成立日期 | 中 |
| `registration_date` | 登记日期 | 营业执照上登记的日期，通常为企业成立日期或最近一次变更日期 | 中 |


> **字段地区变体说明**：ADP 平台对中国大陆地区营业执照使用上述 `field_key`；如调用海外站点（`adp-global.laiye.com`），部分字段命名可能存在地区差异，请以实际返回的 `field_key` 为准。
...
| `field_values[].confidence` | number | 置信度分值，范围 [0, 1]，数值越大表示模型对该字段的识别结果越有信心。注意：部分字段可能不返回该值，具体以实际返回为准。 |
...
```bash
adp extract local ./business-licenses/ --app-id <营业执照抽取应用ID> --export ./results/ 
```
...
```bash
# 提交异步任务
adp extract url https://example.com/business-license.jpg --app-id <营业执照抽取应用ID> --async
...
# 营业执照抽取（URL）
adp extract url <文件URL> --app-id <营业执照抽取应用ID>

# 营业执照抽取（本地文件）
adp extract local <文件路径> --app-id <营业执照抽取应用ID>

# 营业执照抽取（Base64）
adp extract base64 <base64字符串> --app-id <营业执照抽取应用ID> --file-name <文件名.后缀>

# 批量抽取
adp extract local <文件夹路径> --app-id <营业执照抽取应用ID> --export <输出路径> 

# 异步抽取
adp extract url <文件URL> --app-id <营业执照抽取应用ID> --async
...
| --- | --- |
| 营业执照抽取费用 | **0.5 积分/张** |
| 新用户免费额度 | 每月赠送 **100 积分，相当于每月可免费处理 200 张营业执照**，每月初重置 |
| 查询余额 | `adp credit` |
...
营业执照识别只是来也科技 ADP 平台众多开箱即用能力之一。ADP 基于大模型通用理解能力，提供覆盖全品类文档的智能处理解决方案：
