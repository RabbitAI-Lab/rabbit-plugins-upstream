...
先确认用户使用 Codex 还是 OpenClaw：

- **Codex**：执行下方“Codex 配置”，然后跳过 OpenClaw 专用的 Step 2-4。手动任务管理不需要 cron。
- **OpenClaw**：继续执行原有 Step 1-5。

### Codex 配置

默认数据目录是 `${CODEX_HOME:-$HOME/.codex}/kinema-tasks`。Windows PowerShell：

```powershell
$env:TASK_DIR = Join-Path $HOME '.codex\kinema-tasks'
New-Item -ItemType Directory -Force -Path (Join-Path $env:TASK_DIR 'active'), (Join-Path $env:TASK_DIR 'archived'), (Join-Path $env:TASK_DIR 'snapshots')
Get-ChildItem $env:TASK_DIR
```

类 Unix 环境：

```bash
export TASK_DIR="${CODEX_HOME:-$HOME/.codex}/kinema-tasks"
mkdir -p "$TASK_DIR/active" "$TASK_DIR/archived" "$TASK_DIR/snapshots"
```

需要定时报告时，必须先询问时间、时区和通知目标，再创建 Codex Automation；不要执行本文件的 `openclaw cron` 命令。

## Prerequisites | 前置条件

- Agent 可执行文件系统操作
- OpenClaw 模式：OpenClaw cron 可用且 Bash 可用
- Codex 模式：Codex 已安装；Bash 仅在直接运行辅助脚本时需要
