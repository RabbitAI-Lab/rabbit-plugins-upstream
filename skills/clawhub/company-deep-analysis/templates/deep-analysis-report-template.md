...
![8 期营收归母柱状图与净利率折线](./chart_revenue_profit.png)
...
![财务四维得分雷达图（外层=实际得分，内层=行业中位数）](./chart_radar.png)
...
- [ ] 关键走势图保存为独立 PNG 文件，MD 用相对路径引用（`./chart_xxx.png`），2 张以内：营收柱状图 + 财务四维雷达图
...
> 本报告为**深度分析报告**（MD 主报告，含 2 张关键走势图：营收柱状图 + 财务四维雷达图，以独立 PNG 文件存放于同目录）。配套产物：
