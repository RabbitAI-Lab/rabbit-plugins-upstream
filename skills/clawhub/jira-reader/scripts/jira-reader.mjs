...
try {
  ...
  if (command === 'me') {
    print(await jiraGet('/rest/api/3/myself'));
    ...
    const fields = option(args, '--fields') || 'summary,status,assignee,reporter,priority,issuetype,created,updated,description,labels,components,fixVersions';
    const issue = await jiraGet(`/rest/api/3/issue/${encodeURIComponent(issueKey)}?fields=${encodeURIComponent(fields)}`);
    print(summarizeIssue(issue));
  } else if (command === 'search') {
    const jql = requiredArg(option(args, '--jql'), '--jql');
    const max = parseMax(option(args, '--max'), 20);
    print(await search(jql, max));
  } else if (command === 'recent') {
    const max = parseMax(option(args, '--max'), 10);
    print(await search(`project = ${config.project} ORDER BY updated DESC`, max));
  } else if (command === 'my-open') {
    const max = parseMax(option(args, '--max'), 20);
    print(await search(`project = ${config.project} AND assignee = currentUser() AND statusCategory != Done ORDER BY updated DESC`, max));
  } else {
...
function requireConfig(activeCommand) {
  ...
  if (['project', 'search', 'recent', 'my-open'].includes(activeCommand) && !config.project) missing.push('JIRA_PROJECT');
...
async function jiraRequest(method, path, body) {
  ...
  if (!response.ok) {
    ...
    throw new JiraRequestError(response.status, `Jira API error ${response.status}: ${redact(text).slice(0, 800)}`);
  }
...
async function discoverCloudId() {
  const response = await fetch(`${config.baseUrl}/_edge/tenant_info`, {
    headers: { Accept: 'application/json' },
  });
  if (!response.ok) {
    throw new Error(`Could not discover Jira Cloud ID from JIRA_BASE_URL: HTTP ${response.status}`);
  }
...
function collectText(node) {
  ...
  if (Array.isArray(node.content)) {
    result += ' ' + node.content.map(collectText).join(' ');
  }
  return result;
}
...
function usage() {
  console.log(`jira-reader ${VERSION}\n\nUsage:\n  jira-reader me\n  jira-reader project [PROJECT_KEY]\n  jira-reader issue ISSUE-123 [--fields summary,status]\n  jira-reader search --jql "project = PROJ ORDER BY updated DESC" [--max 20]\n  jira-reader recent [--max 10]\n  jira-reader my-open [--max 20]\n\nNotes:\n  Configuration is read from process.env after optionally loading .env.\n  Set JIRA_ENV_FILE to load a different env file.\n  Existing process.env values take precedence over .env values.\n  Authentication supports only scoped Atlassian API tokens.\n  Requests use api.atlassian.com/ex/jira/{cloudId} with Basic email:token.\n  search uses Jira's /rest/api/3/search/jql endpoint.\n  my-open filters statusCategory != Done, not only unresolved resolution.\n`);
}
