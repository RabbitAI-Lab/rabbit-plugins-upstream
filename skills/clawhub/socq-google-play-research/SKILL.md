---
name: socq-google-play-research
description: Research public Google Play content, accounts, keywords, and performance data with SocQ. Use when an agent needs Google Play-specific discovery, collection, endpoint selection, credit estimates, asynchronous task execution, pagination, or raw exports through the SocQ CLI, MCP, or REST API.
metadata:
  openclaw:
...
- API documentation: <https://docs.socq.ai/api-manual/google-play>
...
Use this Skill to select and run SocQ endpoints for public Google Play research. SocQ requests are asynchronous and credit-metered.

## Use When

- The user asks to discover, collect, compare, or analyze public Google Play data.
- The workflow needs Google Play-specific endpoint selection, input validation, pagination, or raw exports.
...
1. Prefer an already configured SocQ MCP server at `https://api.socq.ai/mcp?platforms=google-play`; use the typed tool listed in [platform.md](references/platform.md), or compact `socq_execute` when needed.
