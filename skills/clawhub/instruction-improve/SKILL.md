---
name: amazon-instruction-improvement
description: >
  从 Amazon 评论中的安装、使用和维护疑问提炼说明内容的改进重点与证据。仅用于说明改进建议；不用于医疗、法律或安全认证，也不自动发布文档。
  Requires an ARI API key (ari_live_*).
slug: instruction-improve
displayName: Amazon 使用说明改进
version: 1.4.7
summary: 从评论疑问提炼说明书改进重点
license: MIT
---
...
- 本入口只执行 `product/instructions`，输出模板固定为 `ops_product`。
