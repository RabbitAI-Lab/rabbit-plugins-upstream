...
def safe_cell(value):
    """Apply formula-injection neutralization to a single cell value."""
    return excel_safe_text(value) if isinstance(value, str) else value


def safe_append(ws, row):
    """Central safe write path: every string cell is sanitized automatically.
    Use this everywhere a row of values is appended to a sheet."""
...
def create_degree_planner(wb, data):
    ...
    if not req_categories:
        safe_append(ws,["REQ-PLACEHOLDER", "Core", "Core Requirements", 0, 0, 0, "Not Started", "", "Add your degree requirements here"])
        row_idx += 1
    ...
    for s in schedule:
        ...
        if cc and cc not in seen_courses:
            seen_courses.add(cc)
            safe_append(ws,[
                excel_safe_text(f"PLAN-{semester_code}-{cc[:8]}"),
                excel_safe_text(cc),
                "",
                excel_safe_text(semester_code),
                "Planned",
                "Medium",
                0,
                "",
...
def create_weekly_timetable(wb, data):
    ...
    for day, sessions in day_sessions.items():
        ...
        for s in sessions:
            ...
            cell_text = excel_safe_text(f"{cc}\n{st}\n{location}")
            if instructor and instructor != NOT_FOUND:
                cell_text += excel_safe_text(f"\n{instructor}")
            ...
            if start_row and end_row:
                ...
                cell = ws.cell(row=start_row, column=col, value=excel_safe_text(cell_text))
...
def create_academic_calendar(wb, data):
    ...
    if semester_info:
        ...
        safe_append(ws,[
            excel_safe_text(f"EVT-{semester_code}-START"),
            excel_safe_text(f"{semester_code} Semester Start"),
            "Semester Start",
            excel_safe_text(parts[1].replace("starts ", "").strip() if len(parts) > 1 else ""),
            excel_safe_text(parts[1].replace("starts ", "").strip() if len(parts) > 1 else ""),
            "", "", excel_safe_text(semester_code), False, "", "", "Semester start date", "", 0
        ])
        if len(parts) > 2:
            safe_append(ws,[
                excel_safe_text(f"EVT-{semester_code}-END"),
                excel_safe_text(f"{semester_code} Semester End"),
                "Semester End",
                excel_safe_text(parts[2].replace("ends ", "").strip()),
                excel_safe_text(parts[2].replace("ends ", "").strip()),
                "", "", excel_safe_text(semester_code), False, "", "", "Semester end date", "", 0
            ])
...
def create_raw_schedule(wb, data):
    ...
    for s in schedule:
        safe_append(ws,[
            ...
            excel_safe_text(s.get("start_week", "")),
            excel_safe_text(s.get("end_week", "")),
...
def create_degree_audit(wb, data):
    ...
    safe_append(ws,["Credit Progress", excel_safe_text(f"Total planned: {total} | {note}" if total else note), ""])

    # Prerequisite issues
    safe_append(ws,["Prerequisite Issues", "", ""])
    for issue in audit.get("prerequisite_issues", []):
        safe_append(ws,[excel_safe_text(issue.get("course_code", "")),
                   excel_safe_text("Missing prereqs: " + ", ".join(issue.get("missing_prereqs", []))),
                   excel_safe_text(issue.get("severity", "Medium"))])

    # Overload weeks
    safe_append(ws,["Overload Weeks", "", ""])
    for ow in audit.get("overload_weeks", []):
        safe_append(ws,[excel_safe_text("Week " + str(ow.get("week", ""))),
                   excel_safe_text(", ".join(ow.get("courses", [])) + (" | " + ow.get("risk", "") if ow.get("risk") else "")),
                   "High"])

    # Sequence alerts
    safe_append(ws,["Sequence Alerts", "", ""])
    for sa in audit.get("sequence_alerts", []):
        safe_append(ws,[excel_safe_text(sa.get("course_code", "")), excel_safe_text(sa.get("alert", "")), excel_safe_text(sa.get("severity", "Medium"))])

    # Summary
    if audit.get("summary"):
        safe_append(ws,["Summary", excel_safe_text(audit.get("summary")), ""])
