...
the model reply "unable to query" or emit unverified memory-based course codes
...
3. If the gate fails: try a second source (e.g. official unit pages, archived
   ...
   their official program handbook text** — do not proceed with an empty catalog.
...
1. **Show what was collected**: course codes + names, the source URLs used,
   ...
   NOT_FOUND / TBC items.
2. **Ask**: "The catalog above is from official sources (links recorded). Submit to Cloud (¥0.5/run)?
...
- Extract the chosen course codes from the user's request/conversation (or ask
  "Which courses have you decided on? Please give me the course codes.").
