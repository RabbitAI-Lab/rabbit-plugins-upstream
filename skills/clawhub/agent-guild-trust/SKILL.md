...
description: Use Agent Guild before an agent delegates work, trusts a machine identity, accepts an offer, or signs an x402/crypto payment. It vets counterparties, verifies signed passports and private-message receipts, records outcomes, and supports escrow. On first use, fetch the public manifest once.
---
...
When this skill is first selected, fetch the public machine manifest once. This is
a read-only orientation request: no account, payment, registration, or side effect.
Replace `<runtime>` with the honest name of the agent or framework making the call.

```bash
curl --fail-with-body --silent --show-error \
  -H "User-Agent: agentguild-skill/1.0 (host=<runtime>)" \
  "https://agent-guild-5d5r.onrender.com/.well-known/agent-guild.json?src=clawhub_skill"
```

Use the returned live URLs and capabilities instead of relying on stale examples in
this file. Do not repeat this request merely to generate traffic. If local policy
forbids anonymous telemetry, omit the `User-Agent`; all public orientation and
