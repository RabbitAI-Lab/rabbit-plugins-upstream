# LinkFox 账号与环境引导（内置于 linkfox-os）

承接两类引导场景：

1. **缺 Key / 鉴权失败**：未配置 `LINKFOXAGENT_API_KEY`，或任何 linkfox-* skill 返回 `errcode=401` / `authorized error` → 引导用户配置 key（支持脚本化注册）。
...
```bash
[ -n "$LINKFOXAGENT_API_KEY" ] && echo ok || echo missing
```
...
> 检测到已配置 LINKFOXAGENT_API_KEY，但鉴权失败（401）。常见原因：
...
> 还未检测到 LINKFOXAGENT_API_KEY。是否已注册 LinkFox Agent？
...
调 `python <skill>/scripts/onboarding/send_verify_code.py <phone>`，脚本返回 JSON：
...
收到用户回复的验证码后，调 `python <skill>/scripts/onboarding/login_and_get_key.py <phone> <code>`，脚本返回 JSON：
...
> 变量名：`LINKFOXAGENT_API_KEY`（无下划线；网关地址变量：`LINKFOX_AGENT_API_URL` 或回退 `LINKFOX_TOOL_GATEWAY`）。
...
1. **列套餐**：调 `python <skill>/scripts/onboarding/list_plans.py`，输出 JSON 套餐清单（含 `plan_id`、`name`、`price`、`currency`、`credits`、`description`、`available_methods`）。
2. **选套餐（校验，按宿主分流）**：
...
4. **下单 + 渲染二维码**：调 `python <skill>/scripts/onboarding/create_order.py <plan_id> <pay_method>`，脚本返回 JSON：
   - `order_id`：订单号（后续查询用）
   - `qr_content`：二维码原始内容（`weixin://` 或支付宝链接，不能直接点击）
   - `pay_url`：可点击的支付链接（兜底）
   - `png_path`：本地生成的 PNG 路径（由同目录内置的 `_qrgen.py` 从 `qr_content` 现场生成，无外部依赖）
   - `ascii_qr`：ASCII 二维码字符串（兜底）
   - `qr_url`：向后兼容字段，当前网关不返回图片 URL，恒为 `null`
5. **展示给用户**（按优先级）：
...
提供 `python <skill>/scripts/onboarding/query_order.py <order_id>`，返回 JSON `{"order_id":..., "status":..., "paid_at":...}`，`status` ∈ `paid` / `unpaid` / `expired` / `unknown`。可在用户表示"已付款"后调用，或结合 `/loop` 周期性查询，但本流程不主动轮询。

## 依赖

- Python 3（标准库 urllib/json/os）
- `requests`：登录链路 HTTP 调用（生产 WAF 对 urllib 敏感），`pip install requests`
- 二维码 PNG / ASCII 生成：内置 `_qrgen.py`（stdlib 纯 Python 实现，byte mode + ECC-L，与官方 qrcode 库同版本+同 mask 逐位一致），**无需再装 qrcode/pillow**
- 自包含，不依赖 `_shared/linkfox_paths.py`

## 接口契约

所有端点（网关 `/account/*`、`/package/*`、`/order/*`，登录链路 `/user/v1|v3/web/login`、`/account/loginByToken`、`/linkFoxApp/api/userCenter/userInfo`、`/group/getApiToken|generateApiToken`）的完整请求/响应/错误码见 [`onboarding-api.md`](onboarding-api.md)。
