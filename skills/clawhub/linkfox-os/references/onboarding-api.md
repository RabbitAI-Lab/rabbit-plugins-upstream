# linkfox-os onboarding 接口契约

本 skill（内嵌在 linkfox-os）调用 5 个网关端点（JWT 解码在本地，无需后端）。所有端点均挂在 `LINKFOX_AGENT_API_URL`（生产 `https://tool-gateway.linkfox.com`，测试 `https://test-sz-tool-gateway.linkfox.com`）下。

## 通用约定

| 项 | 值 |
|----|-----|
| 鉴权 | Header `Authorization: <LINKFOXAGENT_API_KEY>`。key 从环境变量 `LINKFOXAGENT_API_KEY` 读取 |
...
|---------|------|--------------|
| 200 | 成功 | 正常返回 |
| 401 | 鉴权失败 | 提示检查 `LINKFOXAGENT_API_KEY`，引导走入口 1（缺 Key 引导） |
...
**用途**：从 `LINKFOXAGENT_API_KEY` 的 JWT payload 解出 uid，作为下单接口的 `memberId`。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `token` | string | **API key 本体**（JWT），作为 `LINKFOXAGENT_API_KEY` 配置。空字符串表示无 token |
...
6. 拿到 token 即为 `LINKFOXAGENT_API_KEY`，按三平台示例配置环境变量
