#!/usr/bin/env python3
"""linkfox-os onboarding 共享模块：网关调用、二维码渲染、会话目录、JWT 解码、套餐/订单接口。

被 list_plans.py / create_order.py / query_order.py / send_verify_code.py / login_and_get_key.py 复用。
...
def _get_agent_base() -> str:
    """网关基础地址：env LINKFOX_AGENT_API_URL 优先，回退 LINKFOX_TOOL_GATEWAY，最后回退正式地址。"""
...
def _get_api_key() -> str:
    """读 API key：LINKFOXAGENT_API_KEY。"""
...
def gateway_post(path: str, body: dict) -> dict:
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if status == 401:
                raise RuntimeError(f"鉴权失败（401），请检查 LINKFOXAGENT_API_KEY：{raw}")
            if status == 402:
...
def gateway_get(path: str) -> dict:
    """GET 版本，请求头 / 错误处理 / 重试策略与 gateway_post 一致。"""
    base = _get_agent_base()
    url = f"{base}{path}"
    api_key = _get_api_key()
...
def decode_jwt_uid() -> dict:
    """解码 LINKFOXAGENT_API_KEY 的 JWT payload，返回 payload dict。
...
def create_order_real(plan_id: str, pay_method: str) -> dict:
    ...
    if not member_id:
        raise RuntimeError("无法从 JWT 解出 uid，请检查 LINKFOXAGENT_API_KEY")
