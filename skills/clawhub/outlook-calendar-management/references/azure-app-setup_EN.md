...
6. **API permissions** → Add a permission → Microsoft Graph → Delegated permissions → add all three in turn: `User.Read`, `Calendars.ReadWrite`, `MailboxSettings.Read` (their purposes are listed in the connection-steps table of `configuration_EN.md`)
