#!/bin/bash
# 真实账户边界条件回归：106 项行为断言（中文输出版）
# 前置：已用 outlook_setup.py 对专用测试账户完成认证（token 在 ~/.outlook_cal_token.json）
# 用法：bash tests/integration/drill.sh confirm <测试账户邮箱>
#       或 TEST_ACCOUNT=<测试账户邮箱> bash tests/integration/drill.sh confirm
# ⚠️ 危险：本脚本会删除测试账户 ±400 天窗口内的全部日程！只允许对专用测试账户运行。
# 防呆（双重锁）：① 必须显式传参 confirm；② 必须指定测试账户且与当前连接账户一致，否则拒绝执行。
set -u
if [ "${1:-}" != "confirm" ]; then
  echo "❌ 防呆保护：drill.sh 会删除测试账户 400 天内的所有日程，请确认目标账户是【专用测试账户】。"
  echo "   确认无误后请运行：bash tests/integration/drill.sh confirm <测试账户邮箱>"
  exit 2
fi
...
try:
    ...
    print("基线:", e)
EOF
OUT=$($PY list --past 400 --days 400 --summary); c "$OUT" "没有符合条件的日程" "清理后 ±400 天窗口为空"

echo "══════ 1. 时间解析边界 ══════"
OUT=$($PY add "边-补零小时" "2026-08-17 9:00" 2>&1); c "$OUT" "✅ 已添加到日历" "小时不补零 9:00 可解析"
...
OUT=$($PY add "边-end小于start" "2026-08-17 10:00" "2026-08-17 09:00" 2>&1); c "$OUT" "结束时间必须晚于" "end<start 报错"
OUT=$($PY add "边-等时" "2026-08-17 10:00" "2026-08-17 10:00" 2>&1); c "$OUT" "结束时间必须晚于" "end==start 报错"

echo "══════ 2. remind 边界 ══════"
OUT=$($PY add "边-remind0" "2026-08-17 09:00" --remind 0 --force 2>&1); c "$OUT" "✅" "--remind 0 允许(开始即提醒)"
OUT=$($PY add "边-remind负" "2026-08-17 09:00" --remind -1 2>&1); c "$OUT" "提醒时间不能为负数" "--remind -1 报错"
OUT=$($PY add "边-全天remind超" "2026-08-17" --all-day --remind 2000 2>&1); c "$OUT" "最多支持 1826 天" "全天提醒超上限报错"
...
OUT=$($PY add "边-until早于" "2026-08-17 09:00" --repeat "每天" --repeat-until "2026-08-01" 2>&1); c "$OUT" "早于开始日期" "--repeat-until 早于开始 友好报错"
...
OUT=$($PY add "边-相接不重叠" "2026-08-17 10:00" "2026-08-17 11:00" 2>&1); c "$OUT" "✅ 已添加到日历" "相接(10:00起)不警告"
$PY add "边-自由时段不算占用" "2026-08-17 11:00" "2026-08-17 12:00" --busy free --force > /dev/null 2>&1
OUT=$($PY add "边-与free重叠" "2026-08-17 11:30" "2026-08-17 12:30" 2>&1); c "$OUT" "✅ 已添加到日历" "与 showAs=free 重叠不警告"
...
OUT=$($PY update "$ID" 2>&1); c "$OUT" "没有要修改的字段" "无字段 update 提示"
OUT=$($PY update "$ID" --subject "" -y 2>&1); c "$OUT" "✅ 已更新" "--subject \"\" 清空标题"
OUT=$($PY update "$ID" --location "" -y 2>&1); c "$OUT" "✅" "-l \"\" 清空地点"
OUT=$($PY update "$ID" --start "2026-08-17 10:30" -y 2>&1); c "$OUT" "结束时间必须晚于" "update 只给start且晚于原end 报错"
OUT=$($PY update "$ID" --start "2026-08-17 08:00" --end "2026-08-17 07:00" -y 2>&1); c "$OUT" "结束时间必须晚于" "update end<start 报错"
...
OUT=$($PY delete "$REALID" < /dev/null 2>&1); c "$OUT" "已取消" "delete 非交互EOF取消"
...
OUT=$($PY read "$OCC"); c "$OUT" "第 1 次出现" "月度系列第N次计算"
OUT=$($PY update "$OCC" --subject "深-例外" -y 2>&1); c "$OUT" "✅" "修改单次出现创建例外"
OUT=$($PY list --search "深-例外" --days 365); c "$OUT" "🔁(已修改)" "例外在 list 标记"
OUT=$($PY next "$SID"); c "$OUT" "下次出现" "next master 可用"
OUT=$($PY next "$OCC"); c "$OUT" "下次出现" "next exception occurrence 可用"
OUT=$($PY next "不存在ID" 2>&1); c "$OUT" "❌" "next 不存在ID 报错"
# 非定期 next
NID=$($PY add "深-单次" "2026-08-25 09:00" --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY next "$NID" 2>&1); c "$OUT" "不是定期事件" "next 非定期 报错"
# 例外删除（仅删本次）→ 系列其余保留
printf "1\ny\n" | $PY delete "$OCC" > /dev/null 2>&1
OUT=$($PY list --search "深-每月一次" --days 365); c "$OUT" "🔁(系列)" "删例外后系列其余保留"
# 删整系列（master 确认路径，含整系列警告）
OUT=$(printf "y\n" | $PY delete "$SID" 2>&1); c "$OUT" "整个系列" "删整系列(master+警告+确认)"
OUT=$($PY list --search "深-每月一次" --days 365); c "$OUT" "无匹配" "删整系列后无残留"
...
OUT=$($PY free "2026-08-17" --from 09:00 --to 18:00 2>&1); c "$OUT" "空闲" "free 正常输出"
...
OUT=$($PY add "边-日期乱序" "2026-08-27 09:00" "2026-08-26 10:00" 2>&1); c "$OUT" "结束时间必须晚于" "跨天乱序时间 报错"

echo "══════ 11. move 专项 ══════"
MVID=$($PY add "移-待移" "2026-08-19 14:00" "2026-08-19 15:00" --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY move "$MVID" --days 2 -y 2>&1); c "$OUT" "✅ 已移动" "move --days 平移"
OUT=$($PY read "$MVID"); c "$OUT" "08/21 14:00" "move --days 后时间正确"
OUT=$($PY move "$MVID" --to "2026-08-25" -y 2>&1); c "$OUT" "✅ 已移动" "move --to 移到指定日期"
OUT=$($PY read "$MVID"); c "$OUT" "08/25 14:00" "move --to 后时间正确"
OUT=$($PY move "$MVID" --days 0 -y 2>&1); c "$OUT" "移动天数不能为 0" "move 0 天报错"
OUT=$($PY move "$MVID" --days 1 --to "2026-08-26" -y 2>&1); c "$OUT" "不能同时使用" "move --days+--to 报错"
OUT=$($PY move "不存在ID" --days 1 -y 2>&1); c "$OUT" "❌" "move 不存在ID 报错"
MAID=$($PY add "移-全天" "2026-08-20" --all-day --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY move "$MAID" --days 3 -y 2>&1); c "$OUT" "✅ 已移动" "move 全天平移"
OUT=$($PY read "$MAID"); c "$OUT" "08月23日" "move 全天后日期正确"
MSID=$($PY add "移-系列" "2026-08-22 09:00" --repeat "每周六" --repeat-times 3 --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY move "$MSID" --days 1 -y 2>&1); c "$OUT" "整个系列的所有出现" "move 系列主事件警告"
...
OUT=$($PY move "$COCC" --to "2026-08-25" -y 2>&1); c "$OUT" "相邻出现" "move 例外跨相邻出现报错"

echo "══════ 12. 多天全天 / 快捷命令 / 筛选 ══════"
OUT=$($PY add "多-旅行" "2026-09-05" "2026-09-07" --all-day --force 2>&1); c "$OUT" "✅ 已添加到日历" "add 多天全天"
DID=$($PY list --search "多-旅行" --days 60 | sed -n 's/^    🆔 \([^ ]*\)$/\1/p' | head -1)
OUT=$($PY read "$DID"); c "$OUT" "09月05日 ~ 09月07日" "read 显示多天全天范围"
OUT=$($PY add "多-坏end" "2026-09-06" "2026-09-06 18:00" --all-day 2>&1); c "$OUT" "时间格式错误" "add 全天带时间 end 报错"
UID2=$($PY add "多-单天" "2026-09-08" --all-day --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY update "$UID2" --start "2026-09-08" --end "2026-09-10" -y 2>&1); c "$OUT" "✅ 已更新" "update 改多天全天"
OUT=$($PY read "$UID2"); c "$OUT" "09月08日 ~ 09月10日" "update 后显示多天范围"
...
OUT=$($PY add "多-私密" "2026-08-28 09:00" --private --importance 高 --force 2>&1); c "$OUT" "🔒 私密" "add --private 显示"
OUT=$($PY add "多-重要" "2026-08-28 10:00" --importance 高 --force 2>&1); c "$OUT" "⭐ 重要度: 高" "add --importance 显示"
...
OUT=$($PY update "$RID" --no-all-day --start "2026-08-29 09:00" --end "2026-08-29 10:00" --remind 10 -y 2>&1); c "$OUT" "✅ 已更新" "update 全天转时段+remind"
...
OUT=$($PY free "2026-08-30" --from 09:00 --to 18:00); c "$OUT" "整天空闲" "已取消单次不算占用(free)"
DXID=$($PY add "回-单次删" "2026-08-31 09:00" --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY delete "$DXID" -y 2>&1); c "$OUT" "已从日历中移除「回-单次删」" "delete 单次日程中性文案"
QRID=$($PY add "回-解除" "2026-09-01 09:00" --repeat "每天" --repeat-times 3 --force | sed -n 's/^   🆔 \(.*\)$/\1/p')
OUT=$($PY update "$QRID" --repeat "" -y 2>&1); c "$OUT" "已解除定期" "update --repeat \"\" 解除定期"
...
OUT=$(TZ=America/New_York $PY add "夏-回拨" "2026-11-01 01:30" "2026-11-01 02:30" --force 2>&1); c "$OUT" "✅ 已添加到日历" "回拨日(11/01) 事件创建"
...
OUT=$(TZ=America/New_York $PY add "夏-跳变" "2026-03-08 02:30" "2026-03-08 03:30" --force 2>&1); c "$OUT" "不存在" "跳变日(03/08) 不存在的本地时间有警告"
OUT=$(TZ=America/New_York $PY free "2026-11-01" --from 09:00 --to 18:00 2>&1); c "$OUT" "空闲" "回拨日 free 可用"
...
OUT=$(TZ=America/New_York $PY status 2>&1); c "$OUT" "邮箱时区" "status 提示邮箱时区与本机不同"
...
echo "══════ 17. 相对时间 ══════"
OUT=$($PY add "相-今天" "今天 09:00" "今天 10:00" --force 2>&1); c "$OUT" "✅ 已添加到日历" "add 今天(相对时间) 可用"
...
OUT=$($PY add "相-明天" "明天 09:00" "明天 10:00" --force 2>&1); c "$OUT" "✅ 已添加到日历" "add 明天(相对时间) 可用"
