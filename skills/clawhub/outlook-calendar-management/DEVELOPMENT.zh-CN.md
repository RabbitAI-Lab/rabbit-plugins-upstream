...
tests/
  ...
  trigger-eval.zh-CN.md       # 触发评估集：改 description 时对照验证触发/不触发（英文版 trigger-eval.md）
  protocol-eval.zh-CN.md      # 输出协议评估集：新会话里端到端验证提取规则，11/11（英文版 protocol-eval.md）
  integration/                # 可选：真实账户实机演练，需专用测试账户，见其 README
    drill.sh                  # 106 项行为断言，中文输出
    drill-en.sh               # 同上，英文输出
    README.zh-CN.md           # 用法与警告（英文版 README.md）

.github/workflows/
  tests.yml             # CI：三平台（Linux/Windows/macOS）× Python 3.10/3.13 跑离线测试

references/             # 用户文档
  commands.zh-CN.md           # 命令完整参考（英文版 commands.md）
  recurring-events.zh-CN.md   # 定期日程专题（英文版 recurring-events.md）
  configuration.zh-CN.md      # 连接配置（英文版 configuration.md）
  troubleshooting.zh-CN.md    # 故障排除（英文版 troubleshooting.md）
  azure-app-setup.zh-CN.md    # 自带 Azure 应用注册（英文版 azure-app-setup.md）
```
...
- 新增行为时须同步更新 `tests/` 中的断言，中英文两个语言版本均需更新；输出格式变更必须同步更新 `tests/protocol-eval.zh-CN.md` 与 `test_protocol.py`
...
- 文档命名约定：README.md 为英文默认落地页，README.zh-CN.md 为中文页（页顶互挂语言切换）；SKILL.md 为英文默认 agent 手册，SKILL.zh-CN.md 为中文镜像（frontmatter description 为英文以便市场读取；正文含输出语言检测指令）；英文文档为默认名，中文加 `.zh-CN` 后缀成对维护；不再使用 `_EN` 后缀。
...
description 决定 skill 何时被触发，修改前应先运行一遍 `tests/trigger-eval.zh-CN.md`：
12 条应触发的请求与 6 条不应触发的请求，需在全新会话中逐条验证。
对于漏触发，补充关键词；对于误触发，增加排除条件。合格标准为 12/12 与 6/6。

### 协议评估：改输出格式用

输出格式是 agent/脚本解析所依赖的契约，修改前应先运行一遍 `tests/protocol-eval.zh-CN.md`：
11 条"从输出中提取 🆔/时段/错误"的端到端用例，需在全新会话中逐条验证；
同一套正则已被 `test_protocol.py` 自动化固化。合格标准为 11/11。

### 实机集成演练：可选

单元测试无法验证真实 Graph 的行为，`tests/integration/` 中提供两份 106 项断言脚本作为补充验证。**必须使用专用测试账户**：脚本开头的基线清理会删除 ±400 天窗口内的全部日程，严禁指向个人真实日历。用法参见 `tests/integration/README.zh-CN.md`。
...
27. **协议测试双保险**。test_protocol.py 使用 agent 实际采用的提取正则固化 🆔 缩进/stdout 纯净性；protocol-eval.zh-CN.md 是面向人/agent 的端到端评估集（11 条用例）。修改输出格式必须两处同步。**协议只到结构层**：正则只钉锚点/缩进/时段/JSON 结构，不钉自然语言文案；文案逐字稳定由 i18n 表测试（test_i18n.py）保证，两套测试互不越界
