#!/usr/bin/env python3
"""COPPA 美国儿童隐私护栏检测内核 (coppa-guard)。

实时护栏：在面向美国市场的儿童向 App / 游戏 / 电商 / 教育产品的文案、
隐私政策、应用商店描述发布前，检测其中触发 COPPA 适用的表述与儿童隐私
违规用语（面向儿童 / 收集儿童个人信息 / 儿童行为定向广告 / 缺少可验证家长
同意 / 第三方披露儿童数据 / 儿童持久标识符追踪），按风险分级输出命中与
整改建议，供 Agent 主动调用。
...
def main(argv=None):
    p = argparse.ArgumentParser(description="COPPA 美国儿童隐私护栏检测内核")
    ...
    if args.format == "json":
        ...
        if result["decision"] == "clean":
            print("未检出已知儿童隐私触发/违规表述；发布前仍建议结合完整 COPPA 合规审计。")
        else:
            print("检出儿童隐私相关表述，建议按建议修改或补正；如需完整审计可对接 coppa-check/audit。")
    return 0
