...
内部获取商品明细并完成评分，输出 Top-N 精简结果 + C 级候选（供 Step0 第二发现入口使用）。
...
# C 级候选硬性封顶（与 multi_shop_product_analysis 的 20 条硬封顶保持一致）
C_GRADE_CANDIDATES_LIMIT = 20


def fetch_item_detail(strategy: str, limit: int) -> dict:
    """调用远程接口获取商品明细数据。"""
    return api_post(
        "/api/alibaba.1688.skill.item.select.get.item.detail/1.0.0",
...
def score_and_select(shop_total: dict, strategy: str = "comprehensive",
    ...
    Returns:
        包含 total_scored / returned_count / products / summary / c_grade_candidates 的字典。
        c_grade_candidates: 全部 C 级（综合评分 < 40）商品列表，按综合得分升序排列（最差的排最前），
        最多 20 条（硬封顶）。即使商品已进入 products（Top-N）也会保留，
        供 product-analysis Step0 第二发现入口（评分分层）使用。
        注意：c_grade_candidates 里每项的 rank 字段是候选池内部从 1 开始的序号（按总分
        升序，即"最差优先"排列），不代表该商品在全量商品中的真实排名；这与 products
        （Top-N，rank=1 代表全局最优）的 rank 语义相反，下游使用时不要混淆。
    """
    # 1. 获取商品明细
    ...
    # 4. C 级候选：包含全部 C 级，按总分升序（最差优先），封顶 20 条。
    # products 服务正向选品，c_grade_candidates 服务问题品诊断，两条意图互斥，无需跨数组去重。
    c_grade_pool = [p for p in scored if p['classification']['level'] == 'C级']
    c_grade_pool.sort(key=lambda x: x['scores']['total_score'])
    c_grade_candidates = [_compact_product(p, i + 1) for i, p in enumerate(c_grade_pool[:C_GRADE_CANDIDATES_LIMIT])]
    ...
    return {
        ...
        "c_grade_candidates": c_grade_candidates,
    }
