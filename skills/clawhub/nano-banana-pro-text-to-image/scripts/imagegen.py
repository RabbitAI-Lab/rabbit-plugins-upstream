...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana Pro 制作高完成度文生图，主体准确，构图清晰", "keywords": "Nano Banana Pro、NanoBanana Pro、NanoBananaPro、香蕉 Pro、Gemini 图片、文生图、文字生成图片、文本转图片、text to image、T2I、AI绘画、AI图片、AIGC图片", "model": "public_model_nano_banana_pro", "name": "nano-banana-pro-text-to-image", "rule": "none", "search": "固定调用 Nano Banana Pro 的文生图裸接口能力", "title": "Nano Banana Pro 文生图"}')
...
def skill_generate(client, args):
    ...
    forwarded = argparse.Namespace(
        ...
        prompt=args.prompt, image=[], batch=args.batch,
