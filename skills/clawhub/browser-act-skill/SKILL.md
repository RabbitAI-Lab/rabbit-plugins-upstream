...
metadata:
  ...
  permissions:
    - "Network access — required for: CLI install from PyPI; optional captcha-solving API call (sends only the challenge image, no cookies or page content)"
...
persistent login sessions, multi-browser parallel operation, user-configured
proxy support, and human-agent collaboration.
...
**Do NOT truncate the output** — it contains directives and environment state
at the end that are critical for correct operation. Truncating will cause you
to miss browser selection rules and safety constraints.
