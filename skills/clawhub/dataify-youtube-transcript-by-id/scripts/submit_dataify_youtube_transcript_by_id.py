...
def ensure_python_version():
    if sys.version_info < MIN_PYTHON:
        print(
            "Python {}.{} or newer is required. Run this script with a Python 3 interpreter, for example: python3 scripts/submit_dataify_youtube_transcript_by_id.py --video-id \"{}\"".format(
                MIN_PYTHON[0],
                MIN_PYTHON[1],
                DEFAULT_VIDEO_ID,
            ),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Submit a Dataify YouTube Transcript by Video ID Builder task.")
