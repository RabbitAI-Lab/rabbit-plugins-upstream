...
description: "提交 Dataify YouTube Transcript by Video ID Builder 任务，用于 YouTube 字幕文件采集工具。当用户需要 collect/scrape/crawl/fetch/extract YouTube subtitle files、captions、transcripts、video subtitles、subtitles by video ID，或创建 youtube_transcript_by-id 任务时使用；也覆盖 YouTube 字幕文件采集工具、字幕文件采集/抓取、字幕采集/抓取、视频字幕采集/抓取等中文表述。也用于接收 task_id/status、配置 DATAIFY_API_TOKEN 或排查此 Dataify Builder 请求。"
---

# Dataify YouTube Transcript By ID

通过 Dataify Builder 按视频 ID 提交 YouTube 字幕/转录采集任务。提交成功后，向用户提供 `task_id`、返回的或推断的状态，并告诉他们访问 [Dataify](https://dashboard.dataify.com?utm_source=skill) 查看结果。
...
2. 对于下拉字段，以包含 `Label` 和 `Value` 列的 Markdown 表格展示所有允许的选项。使用 `scripts/submit_dataify_youtube_transcript_by_id.py --list-options` 打印完整的下拉表格。
3. 询问用户在运行任务前是否需要修改任何值。
4. 询问用户是否需要采集多组 YouTube 转录。如果是，要求提供多个 `video_id` 值。
...
| 字段 | 必填 | 默认值 | 位置 | 说明 |
| --- | --- | --- | --- | --- |
| `video_id` | 是 | `8RePenzQH80` | `spider_parameters` | 唯一的 YouTube 视频 ID，用于标识需要采集字幕的视频。 |
...
同时提问："您是否需要采集多组 YouTube 转录？如果是，请提供多个 `video_id` 值。"
...
为确保稳定执行，建议使用 Python 3.6 或更高版本运行 `scripts/submit_dataify_youtube_transcript_by_id.py`，而不是重写 Builder 流程。
...
- 不要硬编码本地 Python 路径。
- 不要声称 Builder 响应包含 YouTube 转录文件。
- 不要编造结果字段。
