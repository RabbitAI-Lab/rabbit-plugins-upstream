---
name: tender-opportunity-intelligence-officer
description: 招投标商机情报与竞对动态分析助手。当用户涉及以下任何场景时，必须使用此SKILL：查找临期/即将到期项目、续约商机挖掘、商机预测、推荐潜在投标供应商、追踪竞争对手最新中标动态、竞对画像分析、查询某采购单位的Top供应商、寻找潜在采购客户、客户开发与渠道拓展、监控某行业/某地区最新招标动态等场景。即使用户没有提到「商机」二字，只要涉及临期项目、续约、竞对动态、客户开发、潜在供应商等需求，都应使用本SKILL。
---

# 招投标商机情报官 - 临期商机与竞对动态雷达

专注「高时效商机线索」与「竞对动态追踪」的情报助手：第一时间发现即将到期的续约项目、追踪竞争对手最新中标动向、推荐可切入的潜在客户，让销售与商务团队抢在对手前面。
...
本助手主打**高时效商机线索 + 竞对追踪**，常见用法：

- **临期续约挖掘**：用 `search_expiring_projects` 找出 90 天内到期的周期性项目，`days_until_expiry` 越小越紧急，提前联系采购方。
- **竞对动态追踪**：用 `find_competitors` + `get_company_business_keywords` 看竞争对手近期都中了哪些标、主攻哪些品类。
- **潜在客户开发**：用 `get_top_purchasers` 锁定某品类的重点采购单位，作为客户开发清单。
- **项目投不投**：用 `find_potential_bidders` 评估某项目的历史参与供应商，判断竞争激烈程度。
...
| INSUFFICIENT_BALANCE / QUOTA_EXCEEDED | **判断 API Key 来源**：<br>① 来自 `~/.zlbx/config.json` 且 `source == "auto"` → 调用 `POST /web-api/auth/generate-device-sid`（带 `X-API-Key` Header）拿到 `sid`，输出充值链接 `https://ai.zhiliaobiaoxun.com/auto-login?sid=<sid>`，提示文案：「免费额度已用完，点击链接自动登录并充值；首次会引导绑定手机号，绑定即送 100 次」<br>② 来自 `$ZLBX_API_KEY` → 提示访问 `https://ai.zhiliaobiaoxun.com/?ch=s55` 手动登录充值（不输出自动登录链接） |
