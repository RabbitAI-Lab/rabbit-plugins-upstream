...
ENDPOINT = "/api/v1/temu/goods/ai-image-search"
...
def parse_params(raw_params):
    ...
    for raw_param in raw_params:
        ...
        if not is_allowed_param(key):
            raise ValueError(f"当前图搜同款 Skill 不支持参数: {key}")
        if key == "siteId":
            try:
                site_id = int(value)
...
def validate_response(payload):
    if not isinstance(payload, dict):
        raise ValueError("图搜接口响应必须是 JSON 对象")
    if payload.get("code") != 0:
        raise ValueError(response_message(payload, "图搜接口返回失败"))
    if not isinstance(payload.get("data"), dict):
        raise ValueError("图搜成功响应缺少 data 对象")
...
def _read_remote_image(source, timeout):
    request = Request(source, headers={"User-Agent": "GeekBI-Temu-Image-Search-Skill"})
...
def main():
    parser = argparse.ArgumentParser(description="用图片查询 Temu 同款商品并输出 JSON")
    parser.add_argument(
        "--image",
        required=True,
