...
# 发布到公众号
python scripts/cli.py publish article.md --cover cover.png -t xinming-lab

# 头条发布准备
python scripts/cli.py toutiao prepare article.md
...
所有主题 CSS 已过微信兼容性清洗（无 border-radius / box-shadow / gradient 等无效属性）。
...
内容发布、公众号发布、头条发布、内容同步、选题讨论、爆款选题、GEO优化、数据复盘、内容引擎、omnipub

## 技术栈

- Python 3.11+
- PyYAML（配置解析）
- BeautifulSoup4（HTML 兼容转换）
- requests（微信 API 调用）
