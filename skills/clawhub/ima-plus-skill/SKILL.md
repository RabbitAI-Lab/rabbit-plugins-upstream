---
name: ima.plus-skill-1.0.7
description: IMA.plus 技能（V1.0.7）——笔记管理与知识库操作。支持自然语言目录路径（--path 自动解析知识库/文件夹/笔记本，无需 kb_id）、上传/导出文件、打包导出知识库为 zip、创建文件夹、移动/重命名/置顶知识条目、创建与修改知识库、标签管理、权限管理、广场发现、添加网页到知识库、搜索/浏览/创建/编辑笔记。凭证支持 skill 目录 config.json / ~/.config/ima / 环境变量多层加载。
...
0. **自然语言路径优先（所有模块通用，强制）：** 用户给出知识库/文件夹/笔记本的名称或路径（如"我的知识库/项目/文档"、"存到学习笔记"）时，**一律用解析器自动转 ID，禁止让用户提供 kb_id / folder_id**。知识库用根目录 `resolve_path.cjs --path`，笔记本用 `notes/scripts/resolve_notebook.cjs --path`。面向用户永远只展示名称，禁止暴露任何 ID。
   - ⛔ **禁止擅自使用 `--kb <解析出的知识库ID>`**：即使解析过程中拿到了 kb_id，**也不得主动用它调用脚本**——所有脚本调用一律走 `--path "知识库名/文件夹..."`（内部自动解析）。`--kb` 仅在**用户明确要求使用某个具体 ID** 时才允许出现。违反此规则视为流程错误。
...
> ✅ **在 ima.copilot 环境中运行本技能时，无需任何配置**——环境变量里已自动注入凭证，
> 直接可用（优先级：环境变量排在 config.json 之后，但 config.json 不存在时即用环境变量）。
...
**方式一（推荐，最稳）：写入 `~/.config/ima/`**（技能目录外，一般不受平台影响）
...
### 加载优先级（与 ima_api.cjs 代码一致）

```
显式参数（脚本内传）> skill 目录 config.json > 环境变量 IMA_OPENAPI_CLIENTID/APIKEY > ~/.config/ima/
```

缺凭证 → 脚本退出码 `-100`（stderr 输出 `{"code":-100,"msg":...}`）。

**推荐双保险配置**：`config.json`（自包含）+ `~/.config/ima/`（兜底）同时写入。
万一 skill 目录 `config.json` 被平台重置，`~/.config/ima/` 自动兜底，不影响使用。
（在 ima.copilot 环境中即使两者都没有，环境变量里的凭证也直接可用。）
