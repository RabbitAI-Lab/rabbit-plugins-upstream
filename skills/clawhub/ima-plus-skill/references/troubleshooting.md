...
- **凭证获取**：个人在 https://ima.qq.com/agent-interface 申请的 key **未开通导出权限**（export 报 220030）；需在 ima app 的 copilot 对话索要环境变量 `IMA_OPENAPI_CLIENTID`/`IMA_OPENAPI_APIKEY` 的凭证（已开通导出）。完整指令见主 SKILL.md「Credential Check」。
- **加载优先级**：`config.json` > 环境变量 > `~/.config/ima/`。缺凭证 → 脚本退出码 `-100`。
...
- **文件上传优先用一键脚本**：`upload_to_kb.cjs` 封装 5 步流程（preflight → 重名 → create_media → COS → add_knowledge），支持 `--kb`/`--kb-name`/`--folder`/`--cancel-if-dup`，避免手工拼 JSON 出错。
...
3. 按 `path` 保持目录结构落盘；**同名文件会覆盖**（如「深渊像素V1.0.76」出现两次）→ 加 media_id 短后缀防重。
