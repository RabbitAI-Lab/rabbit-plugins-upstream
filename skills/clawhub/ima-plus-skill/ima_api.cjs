...
// Unified skill config file: holds BOTH credentials and persisted state
// (e.g. the last successful update-check date). The path is derived from
// __dirname so it is never hard-coded; the file name lives in one shared
// constant. Override via options.configFile or the IMA_CONFIG_FILE env var.
const CONFIG_FILENAME = 'config.json';
const DEFAULT_CONFIG_FILE = path.join(__dirname, CONFIG_FILENAME);
...
function loadCredentials(options = {}) {
  // 1) skill 目录自包含配置（优先，学习 mix-downloader 自包含模式）
  const skillConfigPath = path.join(__dirname, 'config.json');
  let skillCfg = {};
  try {
    if (fs.existsSync(skillConfigPath)) {
      skillCfg = JSON.parse(fs.readFileSync(skillConfigPath, 'utf8'));
    }
  ...
  if (!clientId || !apiKey) {
    ...
    err.msg =
      '未找到 IMA 凭证（clientId / apiKey）。请在 skill 目录创建 config.json、设置环境变量，或将凭证放置在 ~/.config/ima/ 目录下。';
    throw err;
  }
...
function readLastUpdateCheck(configFile) {
  try {
    const cfg = JSON.parse(fs.readFileSync(configFile, 'utf8'));
    return cfg.last_update_check || '';
  } catch {
    return '';
  }
}

// Persist today's date into the unified config.json, merging rather than
// clobbering the existing clientId / apiKey fields.
function saveTodayChecked(configFile, today) {
  let cfg = {};
  try {
    cfg = JSON.parse(fs.readFileSync(configFile, 'utf8'));
  } catch {
    // start fresh if config is missing or corrupt
  }
...
async function checkSkillUpdateIfNeeded(requestOptions) {
  const { forceCheck, skillVersion, configFile } = requestOptions;

  const today = readToday();
  const checkedDay = readLastUpdateCheck(configFile);
  ...
  saveTodayChecked(configFile, today);
...
async function imaApi(apiPath, body, options = {}) {
  ...
  const configFile = options.configFile || process.env.IMA_CONFIG_FILE || DEFAULT_CONFIG_FILE;
