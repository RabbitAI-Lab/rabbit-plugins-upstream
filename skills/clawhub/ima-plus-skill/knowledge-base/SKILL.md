...
### 📌 脚本 / `ima_api` 调用的跨平台写法（Windows / Linux）

本 skill 的所有 `.cjs` 脚本为**单一 Node.js 实现，Windows / Linux / macOS 命令本身完全一致**，差异只在 shell 包装与中文编码：

- **直接调脚本**（如 `create_folder.cjs`、`upload_to_kb.cjs`、`tag_add.cjs`）：命令三平台完全相同，仅 Windows 下路径分隔符可用 `\` 或 `/`（Node 均接受）。
  - Linux / macOS：`node knowledge-base/scripts/create_folder.cjs --kb <id> --name "新文件夹"`
  - Windows PowerShell：`node knowledge-base/scripts/create_folder.cjs --kb <id> --name "新文件夹"`
  - Windows cmd：`chcp 65001` 后同 Linux（`node knowledge-base/scripts/create_folder.cjs --kb <id> --name "新文件夹"`）

- **`ima_api` 包装函数**：本文件下文形如 `ima_api "<path>" "<json>"` 的写法，是 Linux/macOS 的 bash 便捷函数，等价于：
  `node "<SKILL_DIR>/ima_api.cjs" "<path>" "<json>"`
  - Windows PowerShell 等价：`& node "$env:SKILL_DIR/ima_api.cjs" '<path>' '<json>'`
  - Windows cmd 等价：`node "%SKILL_DIR%\ima_api.cjs" "<path>" "<json>"`（先 `chcp 65001`）

- **中文编码**：
  - Linux / macOS：默认 UTF-8，无需处理。
  - Windows PowerShell 5.1：请求 Body 会被静默转 GBK，必须 UTF-8 字节数组模式（见根 SKILL.md「PowerShell 5.1 Environment Detection」）。
  - Windows cmd：先 `chcp 65001 >nul` 再运行 node，避免中文参数乱码。

> **`SKILL_DIR` 取值**：本 skill 根目录的绝对路径（符号链接解析后的真实目录）。Windows 用 `$env:SKILL_DIR`，Linux 用 `$SKILL_DIR`，cmd 用 `%SKILL_DIR%`。

#### 📌 调用指令速查（Linux / Windows PowerShell / Windows cmd，路径保持硬编码相对路径）
...
> 完整批量导出/打包经验见下方「实战经验教训 · 教训 25」与「本轮实测结论（2026-07-22 批量导出）」。
...
### 教训 5：模糊查询指令 → 直接调 API，别过度读 SKILL.md
- **坑**：用户说「看看知识库有啥」，我花时间深读 SKILL.md 的「决策表」想找最佳接口，而不是直接调 API。
- **经验**：
  - 「看看有啥」→ 立刻 `search_knowledge_base(query="")`，一步拿全
  - 「XX 知识库里有什么」→ 立刻 `get_knowledge_list(knowledge_base_id=...)`
  - 浅读 SKILL.md 即可，调用优先；拿回数据后需要深入再读文档

### 教训 6：订阅知识库有权限墙，只能看列表不能读文件
- **坑**：「ima怎么用」有 134 条内容，但 `get_media_info` 返回 `code: 220030 没有权限通过skill获取订阅知识库的文件`。
- **经验**：
  - **个人知识库（创建者身份）**：全权限 ✅
  - **订阅知识库（普通成员）**：只能看列表和元信息，**不能读文件/笔记原文** ❌
  - 想知道订阅 KB 里某篇内容 → 告诉用户去 IMA 客户端读，别反复重试

### 教训 10：知识库 ID 很长且含特殊字符，shell 中别手拼
- **坑**：`kb_id` 是 Base64 URL 安全变体字符串，如 `QwnOj388Wi-10YthxwUDLCwyNm_PSmIorUPx_erOPA0=`，末尾有 `=`、`+`、`_`，shell 里非常容易丢字符。
- **经验**：
  - ID 写进变量 + 双引号包住：`"$KB_ID"`
  - 不要用反引号包值（PowerShell 反引号是转义符）
  - 长 ID 复制后先 `echo` 一次核对，再传入 API 调用

### 教训 11：folder_id 确认规则 — 根目录省略，子目录必须 folder_ 开头
- **坑**：把 `knowledge_base_id` 当 `folder_id` 传入 `get_knowledge_list`，返回了根目录列表而非目标文件夹内容。
- **经验**：
  - **根目录**：省略 `folder_id` 字段，不要传 `""` 或 `null`
  - **子目录**：ID 必须 `folder_xxxx` 开头
  - 看到 `current_path` 字段 = 走对路径了，没看到 = 还在根目录

### 教训 12：media_type 枚举别凭印象记，拿不准就查
- **坑**：一度以为 `media_type=11` 是微信公众号文章（其实是笔记），`media_type=6` 才是微信文章。
- **经验**：
  - 常用枚举速记：`1=PDF` | `2=网页URL` | `4=PPT` | `5=Excel` | `6=微信文章` | `11=笔记` | `16=网页视频` | `99=文件夹`
  - 拿不准 → 搜一次拿样例，别硬猜（猜错会导致上传走错流程）
  - 完整列表见 `references/api.md §MediaType`

### 教训 13：搜索结果 `info_list: []` 不一定是真没东西
- **坑**：在「ima怎么用」搜「经验」返回空 `info_list`，差点以为真没内容。
- **经验**：
  - 搜不出 → **换关键词**（用知识库名称的核心词：技巧/教程/指南）
  - 换路径：`search_knowledge` 没结果 → 试试 `get_knowledge_list` 翻文件夹
  - **别只看一次搜索就放弃**，切换策略再试一轮

### 教训 15：跨模块任务必须先读两个 SKILL.md
- **坑**：用户说「把知识库里的 XX 记到笔记」，我可能只读 notes 就开干，但其实需要先 KB 搜原文 → 再 notes 写入。
- **经验**：
  - 看到 "知识库" + "笔记" 同时出现 → **暂停，先读两个 SKILL.md**
  - 口诀：**先取数据，再写存储**（KB 读 → notes 写；notes 搜 → KB 关联）
  - 父级 SKILL.md「跨模块任务」决策表已列出常见场景，先对照再行动

---

### 教训 25：知识库批量导出/打包（2026-07-22 实测）

> 背景：用户要求「把知识库所有内容全部打包」。这是一次完整的端到端导出实战，沉淀为可复用流程。

- **坑 1**：`export_media_for_ima_sandbox` 直接返回 `220030 无权限访问该接口`（API key 未开通导出权限），此路不通。
- **坑 2**：误以为订阅库也能下载——实测 5 个订阅库（普通成员）`get_media_info` 全部 `220030`，只有个人库（创建者）能拿到下载链接。
- **正路（个人库可导出）**：
  1. `list_all_files.cjs --json --kb <id>` 拿全量文件清单（路径 + media_id + media_type）。
  ...
  3. 按 `path` 保持目录结构落盘；**同名文件会覆盖**（如「深渊像素V1.0.76」出现两次），需加 media_id 短后缀防重。
- **可下载 / 不可下载实测**：
  - ✅ 个人库 TXT(13) / Markdown(7) / MP3录音(15) / PDF 等：`get_media_info` 返回有效 url。
  - ❌ HTML(20)：即使个人库也返回 `220030 该文件获取失败，请至ima内查看处理`（类型受限）。
  - ❌ 订阅库任意类型：`220030 没有权限通过skill获取订阅知识库的文件`。
- **规模与健壮性**：订阅库可能万级条目（如鸿蒙 14661 条）且无法下载；个人库注意大文件（实测一个 103MB 无后缀音频，需按 media_type 补 `.mp3`）；批量下载建议并发 ≤8，单文件失败跳过不中断。
- **结论**：「全部打包」在 API 层面只能覆盖个人库（创建者）；订阅库须引导用户到 IMA 客户端手动导出或申请导出权限。

---

### 上传实战经验（文件上传流程特有，本轮沉淀）

> 通用调用层坑（JSON 解析、凭证、双层 code、模糊指令先问）见父级 SKILL.md；以下仅限「上传文件到知识库」。

**经验 A：文件上传优先用 `scripts/upload_to_kb.cjs` 一键脚本**
- 手工 5 步（preflight → 重名检查 → create_media → cos-upload → add_knowledge）要手动解析 JSON、拼临时文件，繁琐易错。
- 一键脚本封装全流程，复用底层 `ima_api.cjs` / `preflight-check.cjs` / `cos-upload.cjs`，不重复造轮子。
- 用法：`node knowledge-base/scripts/upload_to_kb.cjs --file <path> --kb <id>`（详见上文「一键快速上传」章节）。

**经验 B：目标知识库必须显式指定，脚本绝不自动选**
- `--kb <id>` 或 `--kb-name <名称>` 二选一必填；两者都缺 → 脚本直接报错退出。
- 无论账号有 1 个还是多个可添加库，都不自动挑，避免误传到错误知识库。
- `--kb-name` 走 `search_knowledge_base` 按名匹配，可用名字代替手抄长 id。

**经验 C：`search_knowledge_base` 返回 `info_list` / `kb_id` / `kb_name`**
- **易错点**：不要按 `knowledge_base_list` / `id` / `name` 解析（那是别的接口结构），否则永远匹配不到。
- 空 `query` 返回该账号**全部**知识库（含订阅库），不限"可添加"。
- 匹配策略：先精确相等，再 `includes` 包含。

**经验 D：文件夹上传用 `--folder <folder_id>`，支持任意层级嵌套**
- 文件夹的 `media_id` 就是它的 `folder_id`（形如 `folder_7480930444798661`）。
- 嵌套多深都只认最内层 folder_id，脚本 / API 均不限制层级深度。
- 拿内层 folder_id：IMA 客户端复制，或 `get_knowledge_list` 逐级浏览（文件夹条目的 media_id 即其 folder_id）。

**经验 E：create_folder 接口已支持建文件夹（早期误判已纠正）**
- 早期用错误接口 `add_knowledge` + `media_type=99` 建文件夹，返回 `文件夹不存在` / `参数错误`，曾误判「API 不能建文件夹」。
- 正确接口是 `create_folder.cjs`（`openapi/wiki/v1/create_folder`），**实测可成功建文件夹**（返回 `data.media_id` 即 `folder_id`）。
- 结论：可用 `create_folder.cjs` 建文件夹；仍不支持通过 API **移动**文件夹（`move_knowledge` 仅支持文件）。

**经验 F：`check_repeated_names` 重名判定有索引延迟**
- 刚上传完一个文件，短时间内再查同名可能 `is_repeated=false`，过一会儿才变 `true`。
- 不要仅凭一次查询就认定"库里没重名"。
- 脚本默认重名时**保留两者**（文件名追加 `_YYYYMMDDHHmmss`）；`--cancel-if-dup` 可在重名时直接取消。

---

## 本轮实测结论与试错记录（2026-07-21 全功能实测）

> 在知识库「测试知识库1」(`kb_id=8qXpfeutePHoMWMa8sJmIUCpaKIw5GHJtmf9eeCzVEw=`) 实测 **25 项 KB 功能**，仅 `export_media` 因后端权限未开通而失败，其余全部通过。通用坑见父级 SKILL.md「本轮实测结论与试错记录」。

**KB 专属试错 / 已知坑**

- **export_media 接口 220030 无权限**：`export_media.cjs` 脚本逻辑正确，但后端 `export_media_for_ima_sandbox` 返回 `code=220030 无权限访问该接口`——当前 API key 未开通该导出接口权限（非脚本缺陷）。遇到此报错引导用户到 IMA 客户端导出。
- **update_knowledge_base_permission 真实副作用**：改权限影响全库成员，建议在专用临时库验证（本轮在「ima.plus-skill 接口测试临时库」测试），**正式库改前务必向用户确认**。
- **join_knowledge 真实加入公开库**：有副作用，调用前确认用户确实要加入（本轮误加入「Python入门教程」需手动退出）。
- **move_knowledge 不支持文件夹**：仅支持移动文件（`infos` 最多 10 个），传文件夹 `media_id` 会失败。
- **tag_delete 不可逆**：删除标签会移除该标签及其与所有文件的关联，调用前必须显式确认；`tag_rename` 新名已存在时自动合并两组标签，同样需确认。
- **API 不支持删除任何实体**：文件 / 文件夹 / 知识库 / 标签均不可通过 OpenAPI 删除，测试会在库中留痕，需用户到 IMA 客户端手动清理（本轮在「测试知识库1」残留文件夹 A/B、关联笔记、网页、txt 等）。
- **create_folder 已支持建文件夹**：见「实战经验教训·经验 E」，纠正早期「API 不能建文件夹」误判。
- **普通成员写操作返回 220030**：建库/改名/置顶/标签/权限等写操作，角色为普通成员时返回 220030，需先确认角色为创建者/协作成员/管理员。
- **2026-07-22 批量导出个人库实测（新能力）**：用户要「打包知识库全部内容」。结论——① `export_media_for_ima_sandbox` 当前 key 无权限（220030），改走 `get_media_info` 拿下载链接强制下载；② 仅个人库（创建者）可下载，订阅库（普通成员）`get_media_info` 同样 220030；③ HTML(20) 个人库也 220030；④ 新增 `scripts/list_all_files.cjs` 递归列全量目录树（支持 `--kb / --kb-name / --json / --with-url`），先拿清单再逐个下载；⑤ 同名文件需加 media_id 短后缀防覆盖；⑥ 个人库「深渊像素」等含大文件（103MB 无后缀音频按 media_type 补 `.mp3`）。

**跨平台一致性确认**：19 个 `.cjs` 脚本 + `ima_api.cjs` 为单一 Node.js 实现（纯 `node:` 内置模块、LF 行尾、Node 18+），Windows/Linux/macOS 命令本身完全一致；Windows 注意 emoji 日志在老 cmd（GBK）下乱码（功能不受影响），`upload_to_kb.cjs` 已用 `process.execPath` 加固。
