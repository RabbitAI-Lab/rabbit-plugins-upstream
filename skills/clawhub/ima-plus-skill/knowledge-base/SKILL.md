...
| **打包导出为 zip（整库 / 文件夹 / 多文件）** | `export_kb_zip.cjs`                                                   | `--path`，可选 `--count`/`--estimate`/`--dry-run`/`--media-ids`/`--out`；打包前先 `--count` 看调用量，超 50 次/分钟自动限速 |
...
#   --path <知识库名/文件夹路径>   首选（自动解析）
#   --kb-name <知识库名称>        按名称匹配（兼容）
node knowledge-base/scripts/upload_to_kb.cjs --file /path/to/file.pdf --path "我的知识库/项目/文档"

# 上传到知识库根目录
node knowledge-base/scripts/upload_to_kb.cjs --file a.pdf --path "我的知识库"

# 按名称匹配知识库（兼容旧用法）
node knowledge-base/scripts/upload_to_kb.cjs --file a.pdf --kb-name "我的知识库"

# 上传到指定文件夹
node knowledge-base/scripts/upload_to_kb.cjs --file a.pdf --folder <解析出的文件夹ID>
...
**所有需要知识库/文件夹的操作，优先用自然语言路径，不要让用户提供 kb_id。**

统一解析器 `resolve_path.cjs`（skill 根目录），输入"知识库名/文件夹1/文件夹2"，
自动解析为 `kb_id` + `folder_id`：

```bash
# 完整路径：知识库名 + 多级文件夹
node resolve_path.cjs --path "我的知识库/项目/文档"
# → { "kb_id": "...", "kb_name": "我的知识库", "folder_id": "folder_...", "folder_name": "文档" }

# 仅知识库（根目录）
node resolve_path.cjs --path "我的知识库"
# → folder_id 为空 = 根目录

# 唯一知识库时可省略库名，直接给文件夹路径
node resolve_path.cjs --path "项目/文档"

# 知识库名 + 单文件夹名（--kb-name + --folder-name）
node resolve_path.cjs --kb-name "我的知识库" --folder-name "文档"
```

**使用规则**：

1. 用户说了知识库/文件夹的名称或路径（如"我的知识库/项目/文档"）→ **直接调 `resolve_path.cjs --path`**，不要手动 search_knowledge_base + get_knowledge_list 逐步找 ID
2. 解析器输出 `kb_id`/`folder_id` 后，后续 API 调用直接填入
3. 所有封装脚本（`upload_to_kb.cjs` / `create_folder.cjs` / `list_all_files.cjs` / `export_kb_zip.cjs` 等）均支持 `--path "知识库名/文件夹..."` 直接传入，脚本内部自动解析
4. 用户没给任何位置信息 → 用 `--kb-name` 或让用户选择
5. **面向用户永远只展示名称**（知识库名、文件夹名），禁止暴露 kb_id / folder_id / media_id
6. ⛔ **禁止擅自使用 `--kb <解析出的知识库ID>`**：即使解析得到 kb_id 也**不得主动用它调用脚本**，一律 `--path "知识库名/文件夹..."`（内部自动解析）。`--kb` 仅在用户明确要求指定 ID 时可用。

**脚本调用方式速查**（统一支持 `--path`，同时兼容旧的 `--kb` / `--kb-name` / `--folder`）：
...
# 在 我的知识库/项目 下创建子文件夹
node knowledge-base/scripts/create_folder.cjs --path "我的知识库/项目" --name "新文件夹"

# 浏览 我的知识库/项目 的内容
node knowledge-base/scripts/list_all_files.cjs --path "我的知识库/项目"

# 打包导出 我的知识库/项目/文档
node knowledge-base/scripts/export_kb_zip.cjs --path "我的知识库/项目/文档" --out ./out.zip

# 打标签 / 重命名 / 置顶 / 权限（只需知识库，不带文件夹）
node knowledge-base/scripts/tag_list.cjs --path "我的知识库"
```
...
**首选：直接用 `resolve_path.cjs --path` 一步解析**（见上方「自然语言目录解析」章节）：

```bash
node resolve_path.cjs --path "我的知识库/项目/文档"
```

仅当解析器不可用或需要手动探索时，才用以下 API 方式：
...
| --- | --- | --- | --- |
| **直接调脚本**（如建文件夹） | `node knowledge-base/scripts/create_folder.cjs --path "我的知识库" --name "新文件夹"` | 同 Linux：`node knowledge-base/scripts/create_folder.cjs --path "我的知识库" --name "新文件夹"` | 先 `chcp 65001 >nul`，再同 Linux |
...
```bash
# 在根目录建文件夹（必须指定知识库）
node knowledge-base/scripts/create_folder.cjs --path "我的知识库" --name "新文件夹"

# 按名称匹配知识库
node knowledge-base/scripts/create_folder.cjs --kb-name "我的知识库" --name "新文件夹"

# 在指定父文件夹下建子文件夹
node knowledge-base/scripts/create_folder.cjs --path "我的知识库/父文件夹" --name "子文件夹"
```
...
```bash
node knowledge-base/scripts/rename_knowledge.cjs --path "我的知识库" --media-id <media_id> --name "新名称"
...
node knowledge-base/scripts/move_knowledge.cjs \
  --src-path "源知识库" --dst-path "目标知识库" \
  --media-id <media_id> [--dst-path "目标知识库/目标文件夹"]
...
返回 `data.id`（新建知识库的 ID，仅供脚本内部使用）。

### 修改知识库基本信息 — update_knowledge_base_basic_info.cjs

```bash
node knowledge-base/scripts/update_knowledge_base_basic_info.cjs --path "我的知识库" --name "新名称"
...
```bash
node knowledge-base/scripts/set_knowledge_top.cjs --path "我的知识库" --media-id <media_id> --is-top true
...
node knowledge-base/scripts/tag_add.cjs --path "我的知识库" --media-id <media_id> --tag-name "重要"

# 移除标签（仅解除文件↔标签关联，不删标签本身）
node knowledge-base/scripts/tag_remove.cjs --path "我的知识库" --media-id <media_id> --tag-name "重要"

# 列出 / 搜索标签
node knowledge-base/scripts/tag_list.cjs --path "我的知识库" --keyword "重要" --limit 50

# 重命名标签（新名已存在会自动合并，调用前确认）
node knowledge-base/scripts/tag_rename.cjs --path "我的知识库" --old-tag-name "旧名" --new-tag-name "新名"

# 删除标签（不可逆，调用前确认）
node knowledge-base/scripts/tag_delete.cjs --path "我的知识库" --tag-name "待删标签"
```
...
node knowledge-base/scripts/update_knowledge_base_permission.cjs --path "我的知识库" --visible-export-status 3
...
node knowledge-base/scripts/update_knowledge_access_status.cjs --path "我的知识库" --media-id id1,id2,id3 --access-status 3
```
...
```bash
ima_api "openapi/wiki/v1/get_knowledge_base" '{"ids": ["<解析出的知识库ID>"]}'
```
...
```bash
# 根目录
ima_api "openapi/wiki/v1/get_knowledge_list" '{"knowledge_base_id": "<解析出的知识库ID>", "cursor": "", "limit": 20}'

# 指定文件夹
ima_api "openapi/wiki/v1/get_knowledge_list" '{"knowledge_base_id": "<解析出的知识库ID>", "folder_id": "<解析出的文件夹ID>", "cursor": "", "limit": 20}'
...
```bash
ima_api "openapi/wiki/v1/search_knowledge" '{"query": "关键词", "knowledge_base_id": "<解析出的知识库ID>", "cursor": ""}'
...
```bash
# 列举知识库内容
node knowledge-base/scripts/list_all_files.cjs --path "我的知识库"
...
node knowledge-base/scripts/list_all_files.cjs --path "我的知识库" --json

# 同时获取每个文件的下载链接（额外逐个调 get_media_info，较慢；订阅库会标 url_error）
node knowledge-base/scripts/list_all_files.cjs --path "我的知识库" --with-url
```
...
```bash
# ① 整个知识库
node knowledge-base/scripts/export_kb_zip.cjs --path "我的知识库"

# ② 指定文件夹（递归子内容）
node knowledge-base/scripts/export_kb_zip.cjs --path "我的知识库/目标文件夹"

# ③ 指定多个文件（media_id，逗号分隔；跨文件夹也会保留各自原路径）
node knowledge-base/scripts/export_kb_zip.cjs --path "我的知识库" --media-ids id1,id2,id3

# 常用选项
#   --count        轻量统计（JSON）：文件数/调用次数/是否超限，打包前先问一句
#   --estimate     详细预估：文件夹+文件数、调用拆分、限速提示
...
**实测 API 频率限制：50 次/分钟**（第 51 次返回 `code=200001`，约 40-45s 恢复）。

每次导出 zip 的 API 调用量 = **文件夹数（列目录） + 文件数（下载）**。打包前**必须先跑 `--count` 看规模**：

```bash
node knowledge-base/scripts/export_kb_zip.cjs --path "我的知识库/项目/文档" --count
```

输出 JSON：`{ files, folders, api_calls_estimate, over_limit, note }`。判断：

- `over_limit: false`（≤ 50 次）→ 直接打包，一次跑完
- `over_limit: true`（> 50 次）→ 告知用户"约需 X 分钟"，或建议缩小范围（只打子文件夹）；若用户坚持全打，脚本会自动限速（每 10 次暂停 1s + 限流自动等 45s 重试）

**正常打包会自动显示统计行**，无需额外参数：
```
📊 64 个文件 / 9 个子文件夹 · 预估调用 73 次 · ⚠️ 超限将自动限速
```
...
- 逐个导出下载（带进度 `[3/64] 文件名 … OK`）；失败的如实报告，不影响其余文件；**全部失败则不生成 zip**
