 ...
 *   node upload_to_kb.cjs --file a.pdf --kb <knowledge_base_id>
...
async function resolveKbId(args) {
  ...
  if (args['kb-name']) {
    ...
    // search_knowledge_base 返回 info_list，元素字段为 kb_id / kb_name
...
async function main() {
  ...
  const cosRun = spawnSync('node', cosArgs, { encoding: 'utf8' });
  if (cosRun.status !== 0) {
