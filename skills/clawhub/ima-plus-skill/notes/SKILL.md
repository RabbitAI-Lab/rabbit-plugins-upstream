...
本 skill 的所有接口（`notes` 模块无独立脚本，统一通过 `ima_api.cjs` 调用）为**单一 Node.js 实现，Windows / Linux / macOS 命令本身完全一致**，差异只在 shell 包装与中文编码处理。运行需 **Node 18+**。
...
### 教训 14：UTF-8 编码检测不是建议，是强制
- **坑**：`type config.json` 在 PowerShell 5.1 中文 Windows 输出乱码。如果直接把这串乱码当 `content` 写入笔记，就是**永久乱码，不可恢复**。
- **经验**：
  - **写笔记前必须校验**：`python3 -c "sys.stdout.buffer.write(data.decode('utf-8','ignore'))"` 或 `iconv`
  - **标题和正文两个字段都要校验**，不只 content
  - **任何来源的字符串写入前都先 decode 一次**：文件读取、WebFetch、用户输入、外部 API
  - 父级 SKILL.md §Detailed UTF-8 Encoding Rules 有各环境具体方法，写入前对照执行

### 教训 15：跨模块协调 — 笔记模块也可能需要先读 KB 的 SKILL.md
- **坑**：用户说「把知识库里的那篇笔记记下来」，可能直接搜笔记而忽略了要先去知识库定位内容。
- **经验**：
  - 场景：用户意图涉及「知识库」+「笔记」→ **先读两个 SKILL.md**
  - 流程：KB 搜索/获取原文 → Notes 写入
  - 常见模式：
    - 「把知识库里的 XX 记到笔记」：KB `get_media_info` 取原文 → Notes `import_doc`/`append_doc`
    - 「查看原文」（笔记型媒体）：KB `get_media_info` → `media_type=11` → Notes `get_doc_content`
  - 父级 SKILL.md「跨模块任务」决策表覆盖了典型场景，执行前先对照

### 教训 7：模糊指令中的 title/summary 参考
- **坑**：用户说「有什么经验吗」「之间有什么」，在笔记场景中可能指「笔记里有没有相关经验记录」。
- **经验**：
  - 先反问确认意图（搜已有笔记 vs 其他含义）
  - 如果确认搜笔记 → 用 `search_note(search_type=1, query_info.content=...)` 搜正文
  - 不确定是搜标题还是正文 → 两种都搜，对比结果
