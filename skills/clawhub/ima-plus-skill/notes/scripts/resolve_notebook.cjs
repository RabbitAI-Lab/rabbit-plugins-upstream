...
async function main() {
  ...
  const hit =
    notebooks.find((n) => n.name === name || n.folder_name === name) ||
    notebooks.find((n) => ((n.name || n.folder_name) || '').includes(name));
  if (!hit) {
    const names = notebooks.map((n) => n.name || n.folder_name).join('、');
    throw new Error(`未找到笔记本「${name}」。当前笔记本：${names || '(无)'}`);
  }
  const hitName = hit.name || hit.folder_name;
  console.log(JSON.stringify({ note_folder_id: hit.folder_id, note_folder_name: hitName }, null, 2));
}
