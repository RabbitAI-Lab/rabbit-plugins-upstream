...
function formatWeight(kg, unit = "lbs") {
    if (kg === null)
        return "-";
    if (unit === "lbs") {
...
function formatSet(set, unit = "lbs") {
    const parts = [];
...
function printWorkoutSummary(workout, unit = "lbs") {
...
function printWorkoutDetail(workout, unit = "lbs") {
...
function printRoutineSummary(routine) {
...
function printRoutineDetail(routine, unit = "lbs") {
    console.log(`\n🏋️ ${routine.title}`);
    if (routine.notes) {
...
program
    ...
    try {
        ...
        if (options.muscle) {
            ...
            templates = templates.filter(t => t.primary_muscle_group.toLowerCase().includes(muscle) ||
...
program
    ...
    .option("--json", "Output as JSON")
    .option("--kg", "Show weights in kg instead of lbs")
    .action(async (exerciseId, options) => {
    const client = new HevyClient(getConfig());
    const unit = options.kg ? "kg" : "lbs";
    const limit = parseInt(options.limit, 10);
    try {
...
program
    ...
    try {
        ...
        if (!options.force) {
            ...
            const exactMatch = existing.find(e => e.title.toLowerCase() === options.title.toLowerCase());
            if (exactMatch) {
        ...
        const exercise = await client.createExerciseTemplate({
            exercise: {
                ...
                exercise_type: options.type,
                equipment_category: options.equipment,
                other_muscles: options.otherMuscles?.split(",").map(m => m.trim()),
            }
        });
        if (options.json) {
