...
exports.createPQSafeOpenClawSkill = createPQSafeOpenClawSkill;
// ---------------------------------------------------------------------------
...
async function prodCreateEnvelope(input) {
    const { createEnvelope, signEnvelope } = await Promise.resolve().then(() => __importStar(require('@pqsafe/agent-pay')));
    const { hexToBytes } = await Promise.resolve(`${'@noble/hashes/utils.js'}`).then(s => __importStar(require(s)));
...
async function prodVerifyEnvelope(input) {
    const { verifyEnvelope } = await Promise.resolve().then(() => __importStar(require('@pqsafe/agent-pay')));
    const { hexToBytes } = await Promise.resolve(`${'@noble/hashes/utils.js'}`).then(s => __importStar(require(s)));
 ...
 */
function createPQSafeOpenClawSkill(config = {}) {
...
exports.default = createPQSafeOpenClawSkill;
