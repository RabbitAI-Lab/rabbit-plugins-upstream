...
class MailClient {
    ...
    async listMessages(req) {
        ...
        if (acc.protocol.receive === "imap") {
            ...
            return this.withImap(acc, async (client) => {
                ...
                const selected = (uids || []).slice(-limit); // Take most recent N messages
                const messages = [];
                for await (const msg of client.fetch(selected, { envelope: true, flags: true, bodyStructure: true, source: false })) {
                    ...
                    const struct = m.bodyStructure;
                    const hasAttachments = (struct?.childNodes || []).some((n) => n.disposition?.type?.toLowerCase() === "attachment");
                    messages.push({
                        id,
                        subject,
                        from,
                        ...
                        isRead,
                        hasAttachments,
                        snippet: "", // Can fetch text body separately and truncate if needed
                    });
                }
                return {
                    accountId: req.accountId,
        ...
        if (acc.protocol.receive === "pop3") {
            // TODO: Implement POP3 listing (e.g. using node-pop3)
            return {
                accountId: req.accountId,
                folder: "INBOX",
                total: 0,
    ...
    async getMessage(req) {
        ...
        if (acc.protocol.receive === "imap") {
            ...
            return this.withImap(acc, async (client) => {
                ...
                const isRead = flags.includes("\\Seen") ?? null;
                const attachments = [];
                const bodyStruct = m.bodyStructure;
                ...
                // Simplified: extract text/plain portion from m.source
                let bodyText;
        ...
        if (acc.protocol.receive === "pop3") {
            // TODO: Implement POP3 single message retrieval
            return {
                accountId: req.accountId,
                id: req.id,
                folder: "INBOX",
    ...
    async updateMessageStatus(req) {
        ...
        return {
            ...
            accountId: req.accountId,
            updated: {
            // Actual status optionally returned
            },
        };
    }
}
