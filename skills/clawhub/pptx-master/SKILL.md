...
name: pptx-master
description: >-
  专业级 AI 驱动 SVG 内容生成系统。支持 PDF/DOCX/URL/Markdown 转换为原生可编辑的 PPTX。
  多角色协作流程，输出高质量演示文稿，适合财报分析、技术汇报、商业演示等专业场景。
  版本 v1.2.3，新增 XML 实体自动转义（修复孤立 & 符号导致解析失败），支持 HUAWEI、red grey 等新模板。

read_when:
  - 用户说"生成 PPT"、"做 PPT"、"制作演示文稿"、"做个 PPT"
  - 用户提供 PDF 文件要求转换成 PPT
  - 用户提供 DOCX/Word 文档要求转换成 PPT
  - 用户提供 URL 要求提取内容生成 PPT
  - 用户需要专业演示文稿（财报、技术汇报、商业演示）
  - 用户明确要求"原生可编辑"、"可修改"的 PPT
  - 用户提到"pptx-master"、"ppt pro"
...
>
> - `pptx-master` is a repository-specific workflow skill, not a general application scaffold
