...
version: 1.3.6
description: "音频搜索工具：检索 Pixabay Music，并返回免版税音乐链接与元数据，适合挑选背景音乐。查询请用 2-3 个简短英文风格词（如 'upbeat pop energetic'），整句或过长关键词会返回 0 结果；每条结果的音乐链接在 `url` 字段。"
...
音频搜索工具：检索 Pixabay Music，并返回免版税音乐链接与元数据，适合挑选背景音乐。查询请用 2-3 个简短英文风格词（如 'upbeat pop energetic'），整句或过长关键词会返回 0 结果；每条结果的音乐链接在 `url` 字段。
...
执行 `dlazy search_audio` 命令获取结果。

```bash
dlazy search_audio -h
...
```bash
dlazy search_audio --prompt '提示词内容'
```
