---
name: amazon-listing-promise-check
description: >
  核对 Amazon Listing 中的功能、效果和使用承诺与评论反馈是否一致，并标出证据缺口。仅用于页面承诺证据核查；不用于法律合规结论、自由创作或自动发布。
  Requires an ARI API key (ari_live_*).
slug: listing-promise
displayName: Amazon Listing 承诺核查
version: 1.4.7
summary: 核对页面承诺与评论体验是否一致
license: MIT
---
...
- 本入口只执行 `listing/promise`，输出模板固定为 `ops_listing`。
