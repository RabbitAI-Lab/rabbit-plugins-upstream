...
license: MIT
edition: free
description: "核心能力: 工作汇报领域的专业化 AI 辅助工具,包含核心基础功能兼容. 适用于需要daily report tool相关能力的开发场景,包含结构化的工作流程和可复用的模板,帮助用户快速完成任务并保持代码质量. 适用于需要daily report tool相关能力的开发场景,包含结构化的工作流程和配置指引."
tags:
  - 日报
  - daily
  - report
...
**处理**: 解析校验输入参数完整性的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回校验输入参数完整性的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 按固定模板生成日报内容
按固定模板生成日报内容

**处理**: 解析按固定模板生成日报内容的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回按固定模板生成日报内容的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 写入 reports 目录并返
写入 reports 目录并返回状态

**处理**: 解析写入 reports 目录并返的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回写入 reports 目录并返的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 支持 highlights 与
支持 highlights 与 blockers 字段

**处理**: 解析支持 highlights 与的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回支持 highlights 与的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 返回结构化结果含 nextAc
返回结构化结果含 nextAction

**处理**: 解析返回结构化结果含 nextAc的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回返回结构化结果含 nextAc的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 日报生成器基础版处理的输入数据或指令 |
