...
# HF Daily Deep Researcher v5.2.4 — 多Agent编排版
...
**HF Daily Papers API 说明**：
HuggingFace 提供公开 API `https://huggingface.co/api/daily_papers?date=YYYY-MM-DD`，返回 JSON 格式的当日 curated 论文列表。**注意：此 API 在某些网络环境中可能无法访问**。当不可用时，Searcher 会自动增加 arXiv 查询组数（从 8 组增加到 12 组）来补偿覆盖。这是务实的降级策略，确保 Skill 在任何网络环境下都能正常工作。
