---
name: pdf_to_word
description: PDF 转 Word 服务，支持将 PDF / 图片异步转换为可编辑的 Word 文件。
version: 1.0.1
author: SCNet
license: MIT
tags:
...
dependencies:
  ...
  - requests
input:
  - ocrType : 转换类型，PDF_TO_WORD
  - filePath : 待转换文件的本地路径
output: 转换后的 Word（.docx）文件下载地址
---
# Sugon-Scnet PDF 转 Word 服务技能

本技能封装了 Scnet PDF 转 Word 服务，支持将 PDF 或图片异步转换为可编辑的 Word 文件，并通过轮询获取转换结果。

> **⚠️ 数据安全警告**：本技能会将您指定的本地文件上传到 **SCNet 第三方外部服务** 进行处理。请勿上传包含敏感、机密或受监管数据的文件，除非您已确认可以传输到外部服务。继续操作即表示您理解并同意该文件内容将离开本地环境。

---

## 功能特性

- **PDF 转 Word**：将 PDF 文档还原为可编辑的 `.docx`。
...
- 本技能会把您的本地文件上传到 SCNet 外部服务，请勿处理包含机密信息的文档。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 转换类型。必须为 PDF_TO_WORD（PDF 转 Word） |
| filePath | string | 是 | 待转换文件的本地绝对路径。PDF 格式。 |

### 命令行调用示例

```bash
   python .claude/skills/pdf_to_word/scripts/main.py PDF_TO_WORD /path/to/document.pdf
```
...
- “帮我把这份 PDF 转成 Word，文件在 /Users/name/Downloads/document.pdf”

AI 会根据 description 中的关键词自动触发本技能。**触发前会明确提示用户：文件将被上传至 SCNet 外部服务处理，并需用户确认后方可执行。**

### AI 调用建议

- 当用户请求涉及 PDF 转 Word 时，**必须先展示以下警告信息并等待用户确认**，再调用本技能：
  > “本操作将把您指定的本地 PDF 文件上传到 SCNet 第三方服务进行转换。文件内容会离开本地环境。如涉及敏感信息，请勿继续。是否继续？”
...
- 输入文件必须与 `ocrType` 匹配：PDF_TO_WORD 输入 PDF。
- 下载地址为临时授权链接，请及时下载使用。
- 转换任务为异步处理，脚本会自动轮询直到完成或超时。
- **数据隐私**：本技能会先把本地文件上传到外部服务，转换完成后再返回下载链接。请勿上传含有隐私、商业机密或受监管数据的文件。
