...
# Security: pickle removed — BM25 index now stored as JSON to prevent arbitrary code execution
# from untrusted index files. See ClawHub security audit 2026-07-26.
...
def _check_api_url_safety(api_url: str):
    ...
    if host not in safe_hosts and not host.startswith("192.168.") and not host.startswith("10."):
        ...
        print(f"   Document text and search queries will be sent to this remote endpoint.")
...
def load_index(index_dir: Path) -> tuple:
    ...
    # Load BM25 if available (JSON format — pickle removed for security)
    bm25 = None
    bm25_json_file = index_dir / "bm25_index.json"
    if bm25_json_file.exists():
        try:
            from rank_bm25 import BM25Okapi
            with open(bm25_json_file, 'r', encoding='utf-8') as f:
                bm25_data = json.load(f)
            # Reconstruct BM25 from stored tokenized corpus
            bm25 = BM25Okapi(bm25_data["tokenized_corpus"])
        except Exception as e:
            print(f"Warning: Could not load BM25 index: {e}")
    elif bm25_file.exists():
        # Legacy pickle file — warn but don't load (security: arbitrary code execution risk)
        print("⚠️  WARNING: Found legacy bm25_index.pkl — refusing to load (pickle security risk).")
        print("   Rebuild index with updated build_index.py to use safe JSON format.")
