"""Run registered TikTok Shop ERP return_refund APIs via /tiktokShop/developerProxy."""
...
from _return_refund_endpoints import RETURN_REFUND_ENDPOINTS, RESERVED_PARAM_KEYS
from _shop_return_refund_common import (
...
def resolve_shop_cipher(params: dict, open_id: str) -> str:
    ...
    if business and business.get("code") == 0:
        cipher = _extract_cipher_from_shops(
            business.get("data"), shop_id=str(shop_id) if shop_id else None
        )
    ...
    print(
        ...
        '  Example: {"api":"get_reject_reasons","openId":"...","order_ids":["..."],'
        '"shop_cipher":"GCP_..."}',
        file=sys.stderr,
    )
    sys.exit(1)
...
def run_return_refund_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in RETURN_REFUND_ENDPOINTS:
        print(
            f"Unknown api: {api_name}. Valid: {', '.join(sorted(RETURN_REFUND_ENDPOINTS))}",
            file=sys.stderr,
        )
        sys.exit(1)

    spec = RETURN_REFUND_ENDPOINTS[api_name]
...
def run_return_refund_proxy(params: dict, caller: str = "return_refund_proxy.py") -> dict:
    ...
    shop_cipher = None
    needs_cipher = str(path).lstrip("/").startswith("return_refund/")
    if needs_cipher:
        shop_cipher = resolve_shop_cipher(params, open_id)
