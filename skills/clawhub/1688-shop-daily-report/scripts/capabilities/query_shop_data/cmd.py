...
def main():
    parser = argparse.ArgumentParser(description="商家数据自由查询")
    parser.add_argument("--data_source", "-s", required=True,
                        help="数据源标识，如 SYCM、AD、ITEM")
    ...
    parser.add_argument("--NEWTON_SHOP_LOGIN_ID", default=None,
                        help="可选，目标店铺的 loginId，用于多店铺查询")
    args = parser.parse_args()

    ak_id, _ = get_ak_from_env()
    if not ak_id:
        print_output(False,
                     "AK 未配置。请运行: `cli.py configure YOUR_AK`",
                     {"data": {}})
        return
    ...
    try:
        ...
        result = query_shop_data(data_source, api_path, params, login_id=login_id)
