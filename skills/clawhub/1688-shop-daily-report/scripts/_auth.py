...
def extract_ak_keys(raw_input: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    Args:
        raw_input: AK 原始字符串（Base64 编码，前32位为 Secret，其余为 ID）
    Returns:
...
def build_signature(
    ...
    return headers
def get_auth_headers(method: str, uri: str, body: str = "", ak_raw: Optional[str] = None) -> Optional[Dict[str, str]]:
    """
    获取认证头（便捷函数）
    Args:
        method: HTTP 方法
        uri: 请求 URI
        body: 请求体（JSON字符串）
        ak_raw: 可选，指定 AK 原始字符串（用于多店铺场景）。
                 为 None 时从环境变量读取。
    Returns:
        请求头字典，如果 AK 未配置则返回 None
    """
    if ak_raw:
        ak_id, ak_secret = extract_ak_keys(ak_raw)
    else:
