---
name: linkedin-person-education-zh
description: 调取 LinkedIn 平台的用户教育履历清单，获取目标人员学历层次、毕业院校信息，梳理校友人脉网络，为客户画像搭建和商务关系挖掘提供支撑。
...
使用跨境魔方开放平台API从领英数据查询人物的教育经历信息。

## 概述

本技能提供对跨境魔方领英人物教育经历数据的查询。通过人物ID（hid）获取其教育经历列表，包括就读学校、学位、专业、辅修科目、GPA等信息。
...
**重要**：始终使用直接脚本调用，如 `python scripts/linkedin_person_education_list.py`。**不要使用** shell 复合命令如 `cd scripts && python linkedin_person_education_list.py`

### 教育经历列表查询 (`linkedin_person_education_list.py`)
- **返回粒度**：每条教育经历记录为一行
- **适用场景**：查看人物的教育背景，了解某人曾在哪些学校就读
- **示例**：
  - "查询H_67890的教育经历"
  - "获取下一页教育经历数据"
- **参数**：查看参数说明 [教育经历列表 API](references/linkedin-person-education-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/linkedin/person/education/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"领英教育经历查询失败，服务端异常"}'
```
...
```bash
python scripts/linkedin_person_education_list.py --hid H_67890 --cursor '上一次响应返回的cursor字符串'
```
...
- 教育经历列表：查看 [references/linkedin-person-education-list-api.md](references/linkedin-person-education-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/linkedin-person-education-list-api.md](references/linkedin-person-education-list-api.md)
...
3. **跨技能使用**：
   - `hid`（人物ID）来自 **linkedin-person-search**（领英找人）获取
   - 教育经历返回的 `sid`（学校ID）可与 **linkedin-person-school-detail** 配合使用，查询学校详情（名称、位置、网站等）

## 注意事项
- 教育经历记录用 `hid` 作为人员唯一标识；`sid` 为学校标识
- `degrees` 是学业程度数组；`majors` 和 `minors` 分别是专业和辅修科目数组
- 文件路径在所有平台上都使用正斜杠
...
- linkedin-person-school-detail — 领英学校详情查询
