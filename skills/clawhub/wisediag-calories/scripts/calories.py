#!/usr/bin/env python3
"""
WiseDiag Calories CLI — Estimate food calories from an image (powered by WiseDiag)
...
Usage:
    ...
    python3 calories.py -f "/path/to/food.jpg" --question "这顿饭有多少卡路里？"
    python3 calories.py -f "/path/to/f1.jpg" -f "/path/to/f2.jpg"
...
def _analyze_via_url(
    image_urls: list[str],
    question: str,
    output_dir: str | None,
    name: str | None,
) -> bool:
    """Call WiseDiag calories API with image URLs and stream results."""
    key = _get_api_key()
...
def _analyze_via_file(
    ...
    key = _get_api_key()
    url = f"{DEFAULT_SERVICE_URL}/api/medicine/calories"
    ...
    try:
        for fp in file_paths:
            ...
            files_list.append(("files", (os.path.basename(fp), f, _mime(fp))))
...
def _parse_stream(resp, image_sources, question, output_dir, name, is_file_mode: bool) -> bool:
    """Parse SSE stream from calories response."""
    content_text = ""
    reasoning_text = ""
    vl_result = None
    in_thinking = False
...
def main():
    parser = argparse.ArgumentParser(
        description="WiseDiag Calories CLI — Estimate food calories from an image via WiseDiag AI",
...
Examples:
  ...
  python3 calories.py --image "https://example.com/food.jpg" -n "lunch_20260324"
    ...
    parser.add_argument(
        ...
        help="Local file path to upload (image, repeat for multiple, max 5). Mutually exclusive with --image.",
    )
    parser.add_argument(
        "--image", metavar="URL", action="append", dest="images",
        help="Public URL of the food image (repeat for multiple, max 5). Mutually exclusive with --file.",
    )
    parser.add_argument(
