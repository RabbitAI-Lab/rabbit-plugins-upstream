...
check "Critical" "Reference Documents section" "$(grep -qi 'Reference Documents' "$SKILL_MD" && echo PASS || echo FAIL)"
...
# High checks
check "High" "Overview section" "$(grep -qi 'Overview' "$SKILL_MD" && echo PASS || echo FAIL)"
check "High" "Prerequisites section" "$(grep -qi 'Prerequisites' "$SKILL_MD" && echo PASS || echo FAIL)"
check "High" "Workflow section" "$(grep -qi 'Workflow' "$SKILL_MD" && echo PASS || echo FAIL)"
check "High" "Core Commands section" "$(grep -qi 'Core Commands' "$SKILL_MD" && echo PASS || echo FAIL)"
check "High" "Parameter Confirmation section" "$(grep -qi 'Parameter Confirmation' "$SKILL_MD" && echo PASS || echo FAIL)"
check "High" "CLI installation guide" "$([ -f "$SKILL_PATH/references/cli-installation-guide.md" ] && echo PASS || echo FAIL)"

# Medium checks
check "Medium" "description includes trigger words" "$(grep -qi 'trigger' "$SKILL_MD" && echo PASS || echo FAIL)"
...
check "Medium" "Service name uppercase" "$(grep -q 'hcloud VPCEP' "$SKILL_MD" && echo PASS || echo FAIL)"
