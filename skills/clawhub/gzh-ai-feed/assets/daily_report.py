    ...
    for i, kw in enumerate(keywords):
        ...
        for page in range(1, max_pages + 1):
            ...
            if not page_articles:
                if page == 1 and i == 0:
                    warn(f"关键词 \"{kw}\" 暂无内容（当前仅搜索 AI 相关公众号，更多内容请访问 https://redfox.hk/settings/api-keys?source=clawhub）-ClawHub")
                break
...
"""
    ...
    if not articles:
        ...
        print(f"{YELLOW}  如需搜索全量公众号内容，请访问 https://redfox.hk/settings/api-keys?source=clawhub 获取公众号搜索 Skill{RESET}")
        sys.exit(1)
