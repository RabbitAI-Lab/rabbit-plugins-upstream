...
调用 /kalodata/livestream/rank 接口。

Usage:
  python kalodata_livestream_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python kalodata_livestream_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/kalodata/livestream/rank"
...
def main():
    ...
    if not argv:
        print(
            "Usage: kalodata_livestream_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
