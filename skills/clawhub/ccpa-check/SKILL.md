...
description: |
  ...
  Locale: zh-CN — 面向中文合规团队（交互与提示以中文为主，法律条款名称保留英文原文以确保准确）。
permissions:
  network:
...
2. If the user wants a scored report, point them to the account page (compliancehub.cn/account.html?skill=ccpa-check) to get a free Key, then run the full check with the Key set via COMPLIANCEHUB_API_KEY or the key file.
