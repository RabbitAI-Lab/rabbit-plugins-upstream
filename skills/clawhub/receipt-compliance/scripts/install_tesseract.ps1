...
function Install-Tesseract {
    ...
    Write-Host ""
    Write-Host "  1. 使用winget一键安装（推荐，自动配置PATH）"
    Write-Host "     命令: winget install UB-Mannheim.TesseractOCR"
    Write-Host ""
    Write-Host "  2. 使用scoop包管理器"
    Write-Host "     命令: scoop install tesseract"
    Write-Host ""
    Write-Host "  3. 从GitHub官方Release下载（适合网络受限环境）"
    Write-Host "     地址: https://github.com/UB-Mannheim/tesseract/wiki"
    Write-Host ""
    ...
    switch ($choice) {
        ...
        "3" {
            Write-Host ""
            Write-Host "请手动从GitHub下载安装：" -ForegroundColor Yellow
            Write-Host "  https://github.com/UB-Mannheim/tesseract/wiki" -ForegroundColor Gray
            Write-Host "  安装时务必勾选 Chinese (Simplified) 语言包！" -ForegroundColor Yellow
            Write-Host "  安装后将目录添加到系统PATH并重启终端。" -ForegroundColor Yellow
            return
        }
        "4" {
            Write-Host ""
