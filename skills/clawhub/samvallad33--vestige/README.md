...
```bash
npm install -g vestige-mcp-server@2.6.0
...
macOS and Linux are fine with the npm package. 2.6.0 is in on Ubuntu 22.04 / Debian 12 (glibc 2.35). Do not upgrade system glibc. Intel Mac still needs ORT_DYLIB_PATH in the MCP env block. Local/synthetic only. Do not use `npm install -g vestige-mcp-server@latest` as the Ubuntu 22.04 / Debian 12 path.
...
`vmem` finds `vestige-mcp` with `command -v vestige-mcp`.
