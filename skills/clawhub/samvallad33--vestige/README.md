...
macOS and newer-glibc Linux are fine with the npm package. Current latest is **2.3.0** and does **not** start on Ubuntu 22.04 / Debian 12 (glibc older than 2.38). Wait for **v2.4.0**. Do not upgrade system glibc. Do not use `npm install -g vestige-mcp-server@latest` as the Ubuntu 22.04 / Debian 12 path.
