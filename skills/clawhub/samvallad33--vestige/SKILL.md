...
macOS and Linux are fine with the npm package. 2.6.0 is in on Ubuntu 22.04 / Debian 12 (glibc 2.35). Do not upgrade system glibc. Intel Mac still needs ORT_DYLIB_PATH in the MCP env block. Local/synthetic only.

Skill files (this listing):
...
`vmem` resolves `vestige-mcp` with `command -v`, not `~/bin/vestige-mcp`.
