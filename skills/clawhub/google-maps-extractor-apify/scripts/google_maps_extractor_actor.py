#!/usr/bin/env python3
"""Run the Google Maps Extractor Apify actor from an agent-friendly CLI."""
...
DEFAULT_ACTOR_ID = "2A4RTA5PjN7McqJXx"
...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    forbidden = sorted(field for field in REMOVED_FULL_SCRAPER_FIELDS if field in payload)
    if forbidden:
        raise SkillError(
            "This extractor does not scrape review rows or image lists. "
    ...
    if "maxCrawledPlacesPerSearch" in payload:
        ...
        payload["maxCrawledPlacesPerSearch"] = 50
...
def add_extractor_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--language", default="en", help="Google Maps language code")
    parser.add_argument("--details", action="store_true", help="Enable scrapePlaceDetailPage")
    parser.add_argument("--with-contacts", action="store_true", help="Enable scrapeContacts")
    parser.add_argument("--only-with-website", action="store_true", help="Set website=withWebsite")
    parser.add_argument("--category-keyword", action="append", help="Add categoryFilterWords value; repeatable")
    parser.add_argument("--minimum-stars", choices=sorted(VALID_MIN_STARS - {""}), help="Set placeMinimumStars")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Google Maps Extractor actor on Apify")
    sub = parser.add_subparsers(dest="command", required=True)

    quick = sub.add_parser("quick-search", help="Run a search term + location payload")
    add_common_args(quick)
    add_extractor_options(quick)
