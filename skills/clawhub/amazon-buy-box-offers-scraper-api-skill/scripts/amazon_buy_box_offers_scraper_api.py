...
# API Configuration
TEMPLATE_ID = '113873366575789991'
API_BASE_URL = "https://api.browseract.com/v3/bots"
API_KEY_URL = 'https://www.browseract.com/reception/integrations?co-from=amazon-buy-box-offers-scraper'


def run_amazon_buy_box_offers_scraper_task(api_key, marketplace_url='https://www.amazon.com', asin='B005O2ZU68'):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python amazon_buy_box_offers_scraper_api.py <marketplace_url> [asin]", flush=True)
        sys.exit(1)
    ...
    asin = sys.argv[2] if len(sys.argv) > 2 else 'B005O2ZU68'

    result = run_amazon_buy_box_offers_scraper_task(api_key, marketplace_url, asin)
    if result:
