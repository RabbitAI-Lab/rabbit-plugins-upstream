...
```bash
python3 scripts/temu_category_search.py \
  --param "keyword=clothing" \
  --param "siteId=48" \
  --param "catLevel=2" \
  --param "size=20" \
...
| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `keyword` | string | 无 | 类目中文或英文名称，最长 300 个字符，使用内置模糊搜索 |
| `siteId` | integer | `48` | 极鲸云站点 ID；美国站直接使用默认值，其他国家或地区先实时解析站点 |
| `catLevel` | integer | 无 | 类目层级：1、2、3 或 4；作为通用市场筛选条件时优先 1 或 2 |
| `parentCatId` | integer | 无 | 父类目 ID，只返回该父类目的直接子类目；`0` 代表根层 |
...
- 响应中的 `catId` 属于 Temu 通用类目 ID 体系，可作为后续支持类目筛选的请求条件；用于广泛市场筛选时优先使用一级或二级类目。
...
增长率的查询和响应均使用小数；例如用户要求增长率至少 20%，请求值使用 `0.2`。商品数和店铺数的周期值是变化量，可能为负数。平均价格和所有销售额使用当前查询站点币种。
...
- `data.total` 是当前条件下的命中总数，`data.list` 是当前页类目列表。
- 基础字段包括站点 ID、类目 ID、类目名称、缩略图、层级、父类目 ID、是否叶子类目、Temu 前台 ID 和数据时间。
- `parentCatItems` 是从一级类目到当前类目的完整路径，用它确认类目归属。
- 市场指标包括蓝海指数、总销量、平均价格、总销售额、商品数、店铺数、半托管商品/店铺数，以及对应日/周/月指标与增长率。
- `linkUrl` 指向该类目在极鲸云的 Temu 类目详情页。展示结果时必须将每个类目名称或完整类目路径写成 `[类目名称或完整类目路径](<linkUrl>)`；该要求覆盖列表、表格、排名、候选清单和正文，不因用户未主动要求链接、要求简洁回答或表格空间有限而省略。
