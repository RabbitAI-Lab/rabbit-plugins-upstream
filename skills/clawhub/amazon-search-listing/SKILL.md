...
- **Batch orchestration**: Write a bash script that iterates page numbers serially within a single session; do not parallelize inside one browser (triggers Amazon rate limits). Add a 3-6 second sleep between pages. For higher throughput, open multiple stealth browser sessions with different fingerprints (and ideally different proxy IPs) and shard pages across them.
