...
def generate_report(test_results, stats):
    ...
    for query, data in test_results.items():
        ...
        for i, r in enumerate(data["lexical"]):
            lex_lines.append(f"  {i+1}. **{r['source']}** (bm25: {r['bm25']:.4f}, cat: {r['category']}) — {r['snippet'][:100]}...")
        vec_lines = []
        for i, r in enumerate(data["vector"]):
            vec_lines.append(f"  {i+1}. **{r['source']}** (dist: {r['distance']:.4f}, cat: {r['category']}) — {r['snippet'][:100]}...")
        hybrid_lines = []
        for i, r in enumerate(data["hybrid"]):
            lex_r = f"#{r['lex_rank']}" if r['lex_rank'] else "—"
            vec_r = f"#{r['vec_rank']}" if r['vec_rank'] else "—"
            hybrid_lines.append(f"  {i+1}. **{r['source']}** (RRF: {r['rrf']:.6f}, lex:{lex_r}, vec:{vec_r}, cat: {r['category']}) — {r['snippet'][:100]}...")
