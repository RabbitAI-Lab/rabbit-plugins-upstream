"""CLI argument parsing tests for lulc-accuracy-assessment."""
import subprocess
import sys
import os
import json
...
class TestCLIBasics:
    ...
    def test_bbox_only_runs(self, tmp_path):
        ...
        assert os.path.exists(os.path.join(out, "accuracy_report.html"))

    def test_synthetic_oa_written(self, tmp_path):
        out = str(tmp_path / "out")
        r = run_cli([
            "--bbox", "116", "39", "117", "40", "--synthetic",
            "--n-points", "150", "--output-dir", out, "--quiet",
        ])
        assert r.returncode == 0, r.stderr
        with open(os.path.join(out, "accuracy_metrics.json"), encoding="utf-8") as f:
            m = json.load(f)
