...
// Run a tiny snippet against data.js in a child node process. Returns
// { status, stderr, stdout } so the test can assert exit code + message.
...
// Defense-in-depth: drive safeUserPath's own resolve-based guard directly,
// bypassing sanitizeId. sanitizeId's charset forbids '.' and '/', so no input
// that passes it can ever reach this branch via the CLI — it is a second layer.
// We feed an escaping id straight to safeUserPath and assert the non-zero exit.
