...
"""
export.py
翔云银行卡 OCR Skill — 结果导出工具

用法:
  # 将标准输入（JSON 格式识别结果）导出为指定格式
  python recognize.py --file card.jpg | python export.py --format csv --output result.csv
  python recognize.py --file card.jpg | python export.py --format excel --output result.xlsx
  python recognize.py --file card.jpg | python export.py --format json --output result.json
    ...
    for path in sorted(glob.glob(pattern)):
        try:
            with open(path, "r", encoding="utf-8-sig") as f:
                data = json.load(f)
            # 跳过导出产物（含状态字段的是已导出行数据）
            if isinstance(data, list) or "银行卡类型" in str(data):
                continue
            # 单条识别结果：补充文件名信息
...
def main():
    parser = argparse.ArgumentParser(description="翔云银行卡 OCR 识别结果导出工具")
