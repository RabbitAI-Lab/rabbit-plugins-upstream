...
const PRIVATE_IP_RANGES = [
  ...
  /^169\.254\./,     // link-local
  /^0\./,            // 0.0.0.0/8
  /^\[?::1\]?$/,   // IPv6 localhost (bare or bracketed)
...
function validateUrl(inputUrl) {
  ...
  // Block localhost variants (with and without IPv6 brackets)
  const localhostVariants = ['localhost', '0.0.0.0', '[::]', '::', '[::1]', '::1'];
  if (localhostVariants.includes(parsed.hostname)) {
    return { valid: false, error: `Blocked localhost: ${parsed.hostname}` };
  }

  // Strip IPv6 brackets before regex matching private ranges
  const hostnameForCheck = parsed.hostname.replace(/^\[|\]$/g, '');
  for (const range of PRIVATE_IP_RANGES) {
    if (range.test(hostnameForCheck)) {
      return { valid: false, error: `Blocked private/internal IP: ${hostnameForCheck}` };
    }
  }

  // NOTE: SSRF protection is string-level — hostnames are not resolved to IPs.
  // DNS rebinding attacks could bypass these checks. Use network-level
  // containment (firewall, sandbox) for high-risk environments.
