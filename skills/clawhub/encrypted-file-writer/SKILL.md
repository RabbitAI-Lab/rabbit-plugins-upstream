...
description: 企业安全策略环境下的文件写入工具。支持文本文件、Word 文档 (.docx)、Excel 表格 (.xlsx) 等 80+ 种格式，通过正确的编码处理避免乱码问题。
---

# Enterprise File Writer - 企业安全策略文件写入器

> 企业安全策略环境下的文件写入工具，支持企业安全策略兼容
...
如果您的场景需要文件加密，请使用操作系统提供的加密功能（如 Windows EFS）或专业的加密工具。
...
本工具适用于企业安全策略环境下的文件写入，仅在以下特定场景激活：

**高优先级触发**：
- "企业安全策略写入" - 明确需要在企业安全策略环境下写入文件
- "避免乱码写入" - 需要确保编码正确的文件写入
- "写入受保护的文件" - 在企业安全策略环境中写入文件（注意：不是加密文件）
- "写入 docx/xlsx 文件" - 需要创建或编辑 Office 文档

**低优先级触发**：
- "写入文本文件" - 仅当需要编码安全保证时

**不触发**：
- 普通的"写入文件"、"保存文件"、"创建文件"请求（应使用标准文件写入工具）
- 需要文件加密的场景（本工具不提供加密功能）
- 需要特殊权限绕过或访问控制的场景
...
| 1.2.0 | 2026-07-16 | 修复 SkillSpector 报告的安全问题：移除误导性描述、修复 --encoding 参数、收紧触发条件、移除有漏洞的依赖 |
