...
### news report-ths（同花顺数据源）

用于获取个股研报列表，数据来自同花顺。

**CLI 对应命令:** `daxiapi news report-ths <code> --size <size>`

**参数说明:**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| code | string | 是 | 股票代码，支持6位数字代码，自动识别沪深北市场 |
| size | number | 否 | 返回条数，默认 `20`，最多 `100` |
...
**区别说明:**
- `news report`（东方财富）：支持时间区间筛选、翻页、包含评级信息
- `news report-ths`（同花顺）：数据更简洁，适合快速获取近期研报

---

### news notice-ths（同花顺数据源）

用于获取个股公告列表，数据来自同花顺。

**CLI 对应命令:** `daxiapi news notice-ths <code> --size <size>`
...
**区别说明:**
- `news notice`（东方财富）：支持翻页、包含公告分类标签
- `news notice-ths`（同花顺）：数据更简洁，适合快速获取近期公告
