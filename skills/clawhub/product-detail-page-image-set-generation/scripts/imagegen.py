...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "商品详情页套图，按首屏主视觉、核心卖点、使用场景、材质细节、参数对比和行动提示组织", "keywords": "详情页套图、商品详情页、卖点图、参数图、场景图、材质图、详情页长图、PDP", "model": "public_model_nano_banana_pro", "name": "product-detail-page-image-set-generation", "product_promise": "从商品事实生成信息任务清晰、风格统一的详情页图片组合，避免一张图堆叠过多卖点", "rule": "optional", "search": "面向爆款封面、电商主图、详情页、广告、海报和社媒套图等结果型搜索意图", "title": "商品详情页套图一键生成", "trend_inspired": true}')
