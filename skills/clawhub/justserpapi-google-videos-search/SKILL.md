---
name: Google SERP Videos Search API
...
homepage: https://justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_videos_search&utm_content=project_link
metadata: {"openclaw":{"homepage":"https://justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_videos_search&utm_content=project_link","primaryEnv":"JUST_SERP_API_KEY","requires":{"bins":["node"],"env":["JUST_SERP_API_KEY"]},"skillKey":"justserpapi_google_videos_search"}}
---

# Google SERP Videos Search

Use this focused Just Serp API skill for Google SERP Videos Search. It targets `GET /api/v1/google/videos/search`. Required inputs are `query`. OpenAPI describes it as: Get Google video Search data, including titles, sources, and dates, for video SEO and content monitoring.

## Endpoint Scope

- Group key: `google`
- Endpoint key: `videos/search`
- Group family: Google SERP
- Skill slug: `justserpapi-google-videos-search`

| Operation | Version | Method | Path | OpenAPI summary |
| --- | --- | --- | --- | --- |
| `videosSearch` | `v1` | `GET` | `/api/v1/google/videos/search` | Search |
...
| `query` | `query` | all | n/a | `string` | The search query for Google Videos (e.g., 'coding tutorial', 'movie trailers') |
| `result_time` | `query` | n/a | all | `string` | Filter results by publication time (e.g., 'qdr:d' for past 24 hours, 'qdr:w' for past week) |
...
Use `videosSearch` for the documented `v1` endpoint. There are no alternate versions grouped in this skill.

## Run This Endpoint

Supported operation IDs in this skill: `videosSearch`.

```bash
node {baseDir}/bin/run.mjs --operation "videosSearch" --api-key "$JUST_SERP_API_KEY" --params-json '{"query":"<query>"}'
```
...
- Project site: [Just Serp API](https://justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_videos_search&utm_content=project_link).
- Authentication details: [Just Serp API Docs](https://docs.justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_videos_search&utm_content=project_link).

## Output Focus

- State the operation ID and endpoint path used, for example `videosSearch` on `/api/v1/google/videos/search`.
- Echo the required lookup scope (`query`) before summarizing results.
- Prioritize fields that support this endpoint purpose: Get Google video Search data, including titles, sources, and dates, for video SEO and content monitoring.
