---
name: global-company-person-education-zh
description: 调取全球人员资料库查询目标人员的履历与学术背景，获取完整教育经历清单，完善 B2B 销售客户画像，依托学历背景挖掘潜在人脉关联。
...
使用跨境魔方开放平台API从全球企业库数据查询人物的教育经历信息。

## 概述

本技能提供对跨境魔方全球企业库人物教育经历数据的查询。通过人物ID（hid）获取其教育经历列表，包括就读学校、学位、专业、辅修科目、GPA等信息。
...
**重要**：始终使用直接脚本调用，如 `python scripts/person_education_list.py`。**不要使用** shell 复合命令如 `cd scripts && python person_education_list.py`

### 教育经历列表查询 (`person_education_list.py`)
- **返回粒度**：每条教育经历记录为一行
- **适用场景**：查看人物的教育背景，了解某人曾在哪些学校就读
- **示例**：
  - "查询H_67890的教育经历"
  - "获取下一页教育经历数据"
- **参数**：查看参数说明 [教育经历列表 API](references/person-education-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/depth_company/person/education/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"教育经历查询失败，服务端异常"}'
```
...
```bash
python scripts/person_education_list.py --hid H_67890 --cursor '上一次响应返回的cursor字符串'
```
...
- 教育经历列表：查看 [references/person-education-list-api.md](references/person-education-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/person-education-list-api.md](references/person-education-list-api.md)
...
3. **跨技能使用**：
   - 教育经历返回的 `sid`（学校ID）可与 **global-company-person-school-detail** 配合使用，查询学校详情（名称、位置、网站等）
