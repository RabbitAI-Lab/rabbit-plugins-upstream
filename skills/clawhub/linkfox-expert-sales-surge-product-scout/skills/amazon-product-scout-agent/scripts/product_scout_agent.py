...
EXPORT_FIELDS = ['rank','asin','title','price','monthlySalesUnits','monthlySalesRevenue','bsr','rating','ratings','profit','fulfillment','brand','sellerNation','sellerName','availableDateString','weight','nodeLabelPath','asinUrl','monthlySalesUnitsGrowthRate','fba','sellerNum','badgeNewRelease','ratingsRate']
...
def export_excel(products, round_num):
    ...
    if openpyxl:
        ...
        with open(excel_path, 'w', encoding='utf-8-sig') as f:
            ...
            for i, p in enumerate(products, 1):
                row = {fld: p.get(fld, '') for fld in EXPORT_FIELDS}
                row['rank'] = i
