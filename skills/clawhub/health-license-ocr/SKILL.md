---
name: health_license_ocr
description: 支持卫生许可证的票据识别。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 卫生许可证 OCR 技能

本技能封装了卫生许可证识别的 OCR 服务，通过单一接口即可调用 1种识别能力，高效提取票据内容。

---

## 功能特性

- **卫生许可证识别**：支持识别卫生许可证的票据信息。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• HYGIENE_LICENSE（卫生许可证） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/health_license_ocr/scripts/main.py HYGIENE_LICENSE /path/to/test.jpg
```
