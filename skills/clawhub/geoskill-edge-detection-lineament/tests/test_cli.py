"""CLI argument parsing tests for edge-detection-lineament."""
import subprocess
import sys
import os
...
class TestCLIMethods:
    def test_canny_mode(self):
        r = run_cli([
            "--bbox", "116", "39", "117", "40",
            "--synthetic", "--method", "canny", "--quiet",
            "--output-dir", "./_test_canny",
        ])
        assert r.returncode == 0, r.stderr
        assert os.path.exists("./_test_canny/edges.tif")
        assert os.path.exists("./_test_canny/lineaments.geojson")

    def test_sobel_mode(self):
        r = run_cli([
            "--bbox", "116", "39", "117", "40",
            "--synthetic", "--method", "sobel", "--threshold", "0.2", "--quiet",
            "--output-dir", "./_test_sobel",
        ])
        assert r.returncode == 0, r.stderr
        assert os.path.exists("./_test_sobel/edges.tif")
