...
在「从 asale 市场买」和「用它自己的订阅」之间切换 Claude Code，并查看哪些正在运行的会话还用着旧配置。

## 触发关键词

- 让 Claude Code 从市场买
- 把 Claude Code 指向 asale
- 别再消耗我的 Claude 订阅
- asale 买入 claude code
...
Claude Code 同时认 `ANTHROPIC_API_KEY`，所以开关会清掉这个键——留着一个旧的会盖住 asale
写进去的 token，工具就会不声不响地继续用老凭证。
...
`claude` 那一行带着 `installed`、`enabled`（开关）、`in_effect`（它的实时配置真的指向代理）、
`models` 和 `config_paths`。

这个工具的模型来自每次请求，所以 `models` 留空（「市场上有什么都能买」）本身就是
可用状态。只有用户想限制它能买什么时才传选择。
...
不传 `models` 就保持当前选择，传 `[]` 是清空。会改写的配置：`~/.claude/settings.json`（`env.ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN`）。
...
Claude Code **只在启动时**读一次配置，所以已经在跑的会话拿的还是旧的。`running.claude` 会按 pid 列出
它们，并带上启动时的命令行。
...
> **如果这个 agent 本身就跑在 Claude Code 里**，打开这个开关就是在改写托管着当前会话的
> 那个 CLI 的配置。调用之前先说明，让用户决定。它不会打断正在跑的会话——配置只在启动时
> 读——但那是用户的工具，不是你的。
