---
name: asale-buy-claude-code
version: 0.2.7
description: "Switch Claude Code between buying from the asale market and using its own subscription, and see which running sessions are still on the old config. 在「从 asale 市场买」和「用它自己的订阅」之间切换 Claude Code，并查看哪些正在运行的会话还用着旧配置。"
metadata: {"clawdbot":{"emoji":"📥","requires":{"bins":["curl"]},"install":"curl -fsSL https://asale.ai/dl/install.sh | sh","installAlternative":"irm https://asale.ai/dl/install.ps1 | iex","homepage":"https://asale.ai","source":"https://github.com/asale-ai/asale","author":"asale","license":"see-repo","configLocation":"~/.asale/daemon.token","apiEndpoints":["127.0.0.1:9700"]},"openclaw":{"systemPrompt":"Drive the asale daemon at 127.0.0.1:9700 with the token from ~/.asale/daemon.token. Always run buy_tools before set_buy_tool for tool claude. tool_processes is listing only — never signal those pids."}}
---

# asale-buy-claude-code

[English](./SKILL.md) · [中文](./SKILL-cn.md)

Switch Claude Code between buying from the asale market and using its own subscription, and see which running sessions are still on the old config.

## Trigger Keywords

- make Claude Code buy from the market
- point Claude Code at asale
- stop spending my Claude subscription
- asale buy claude code
...
Claude Code also honours `ANTHROPIC_API_KEY`, so the switch clears that key —
a stale one left in place would shadow the token asale writes and the tool would
quietly keep using the old credential.
...
The `claude` row carries `installed`, `enabled` (the switch), `in_effect` (its live
config really points at the proxy), `models` and `config_paths`.

This tool takes the model from each request, so an empty `models` ("any model
the market offers") is a working setup. Pass a selection only when the user wants
to restrict what it may buy.

### 2. Turn buying on

```bash
call set_buy_tool '{"tool":"claude","enabled":true}'
```

`models` omitted leaves the current selection alone; `[]` clears it. Config
rewritten: `~/.claude/settings.json` (`env.ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN`).
...
Claude Code only reads its config **at startup**, so a session that is already running
keeps the old one. `running.claude` lists the ones still on it, by pid, with the
...
```bash
call set_buy_tool '{"tool":"claude","enabled":false}'
```
...
> **If this agent is itself running inside Claude Code**, turning this switch on
> rewrites the config of the CLI hosting this very session. Say so and let the
> user decide before calling it. It will not disturb the running session — the
> config is read at startup — but it is their tool, not yours.
