...
import { tokenBreaker } from "./token_breaker.js";
...
interface TokenEntry { data: any; cachedAt: number; lastWarningDate?: string }
...
function stripTokenFromResponse(data: any): any {
...
// 进程级内存缓存 (避免每次读盘)
const memTokenCache: Record<string, TokenEntry> = loadTokenCache();
const tokenRateState: Map<string, { tokens: number; lastRefill: number }> = new Map();

function tokenRateAllow(key: string): boolean {
  const now = Date.now();
...
let tokenStatus: { daysLeft: number | string; msg: string | null; lastCheck: number } | null = null;
...
async function checkClawhubLatest(): Promise<string | null> {
  return new Promise((resolve) => {
    // Windows 上 npx 是 .cmd, 需要 shell:true 避免 EINVAL
    const isWin = process.platform === "win32";
    const cmd = isWin ? "npx.cmd" : "npx";
    const opts: any = { timeout: 12000, shell: isWin };
    execFile(cmd, ["clawhub", "info", "stocktoday-skill"], opts, (err, stdout, stderr) => {
...
function buildWarnMsg(daysLeft: number | string): string | null {
  if (daysLeft === "永久") return null;
  if (daysLeft === "已过期") return "⚠️ Token 已过期! 请立即续费, 否则所有接口将不可用";
  if (typeof daysLeft === "number") {
    if (daysLeft <= 0) return "⚠️ Token 已过期! 请立即续费";
    if (daysLeft <= 7) return `🔴 Token 即将过期: 还有 ${daysLeft} 天. 请及时续费`;
  }
  return null;
}
...
async function refreshTokenStatus(): Promise<void> {
  ...
  if (data?.code !== 0 || !data?.data?.summary) return;
...
const server = new Server({ name: "stocktoday-skill", version: "1.3.11" }, { capabilities: { tools: {} } });
...
import { classifyUserAccess } from "./api_permissions.js";
...
function enrichTokenInfo(raw: any): any {
  if (!raw || raw.code !== 0 || !raw.data) return raw;
  const d = raw.data;

  // 1. 解析到期日 + 算剩余天数
  let daysUntilExpire: number | string = "永久";
  let expireParsed: string | null = null;
  if (d.expireDate) {
  ...
  try {
    if (typeof d.plugins === "string") pluginList = JSON.parse(d.plugins);
    else if (Array.isArray(d.plugins)) pluginList = d.plugins;
  } catch {}

  // 4. 计算可用 API 分类 (基于 gateway.py 的 check_api_permission)
  const apiAccess = classifyUserAccess(d.permission, pluginList);
...
async function fetchOne(url: string, endpoint: string, formData: URLSearchParams): Promise<{status: number; data: any; raw: string}> {
  try {
...
async function callAPI(endpoint: string, params: Record<string, any> = {}, token: string = TOKEN): Promise<any> {
  ...
  if (endpoint === '/TOKEN' && result.data?.code === 0) {
    const key = tokenHash(token);
    const entry: TokenEntry = { data: result.data, cachedAt: Date.now() };
...
const _legacyParams: Record<string, Record<string, string>> = {};
...
const schemaMap: Record<string, typeof TOOLS[0]> = {};
for (const t of TOOLS) {
  schemaMap[t.name] = t;
}
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  const allNames = new Set<string>([...Object.keys(descMap), ...Object.keys(schemaMap)]);
  const result = [];
...
const INDEX_TOOLS_REDIRECT: Record<string, string> = {
  daily: '/index_daily',
  weekly: '/index_weekly',
...
function validateArgs(name: string, args: Record<string, any>): string | null {
  if (!STRICT_DATE_TOOLS.has(name)) return null;
  for (const [k, v] of Object.entries(args || {})) {
    if (typeof v !== 'string' || !DATE_FIELDS.has(k)) continue;
...
interface HolidayInfo { date: string; isOpen: boolean; pretradeDate: string; cachedAt: number }
const holidayCache = new Map<string, HolidayInfo>();
const HOLIDAY_CACHE_TTL_MS = 3600 * 1000;

async function checkHoliday(date: string, token: string): Promise<HolidayInfo | null> {
  if (!/^\d{8}$/.test(date)) return null;
  const cached = holidayCache.get(date);
  if (cached && Date.now() - cached.cachedAt < HOLIDAY_CACHE_TTL_MS) return cached;
  try {
    const res: any = await callAPI('/trade_cal', { start_date: date, end_date: date }, token);
    const items = res?.data?.items || [];
    const sse = items.find((i: any) => i.exchange === 'SSE');
    if (sse) {
      const info: HolidayInfo = { date, isOpen: sse.is_open === 1, pretradeDate: sse.pretrade_date, cachedAt: Date.now() };
...
function getLatestReportPeriod(): string {
  const d = new Date();
  const y = d.getFullYear();
  const m = d.getMonth() + 1;  // 1-12
  if (m >= 1 && m <= 3) return `${y - 1}1231`;   // 1-3 月: 用上年 Q4
  if (m >= 4 && m <= 6) return `${y}0331`;        // 4-6 月: 用本年 Q1
  if (m >= 7 && m <= 9) return `${y}0630`;        // 7-9 月: 用本年 Q2
  return `${y}0930`;                              // 10-12 月: 用本年 Q3
}

// === Patch 4 helper: 日期参数严格 8 位数字校验 (如 trade_date='ABC123' 静默返空修复) ===
function validateDateFormat(args: Record<string, any>): string | null {
  for (const [k, v] of Object.entries(args || {})) {
    if (typeof v !== 'string' || !DATE_FIELDS.has(k)) continue;
    // 必须 8 位数字 (YYYYMMDD)
    if (v && !/^\d{8}$/.test(v)) {
...
function validateRequired(name: string, args: Record<string, any>): string | null {
  ...
  if (HIST_MIN_TOOLS.has(name)) {
    if (!args?.ts_code) return `${name} 必须传 ts_code (例 600519.SH)`;
    if (!args?.freq) return `${name} 必须传 freq, 可选: 1min/5min/15min/30min/60min (历史分钟用小写)`;
  }
  const RT_MIN_TOOLS = new Set(['rt_min', 'rt_etf_min', 'rt_idx_min', 'rt_fut_min']);
  if (RT_MIN_TOOLS.has(name)) {
    if (!args?.ts_code) return `${name} 必须传 ts_code (例 600519.SH)`;
    if (!args?.freq) return `${name} 必须传 freq, 可选: 1MIN/5MIN/15MIN/30MIN/60MIN (实时分钟大写)`;
  }

  return null;
}

function isEmptyData(result: any): boolean {
  if (!result || result.data === null) return true;
  const d = result.data;
...
async function enrichHolidayHint(name: string, args: any, result: any, token: string): Promise<any> {
  if (!HOLIDAY_CHECK_TOOLS.has(name)) return result;
  if (!isEmptyData(result)) return result;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const breakerCheck = tokenBreaker.check();
    ...
    if (result?.code === 401 || /TOKEN|token.*invalid|token.*无效|auth.*error|401/i.test(resultMsg)) {
      const breakerStatus = tokenBreaker.recordAuthError();
      // 用熔断器消息替换原始错 (友好提示)
      return { content: [{ type: "text", text: JSON.stringify({ code: 1, data: null, msg: breakerStatus.message || resultMsg }) }] };
    }
    // 调通, 记录成功
    if (result?.code === 0) tokenBreaker.recordSuccess();
    // === P1b: 休市友好提示 ===
    result = await enrichHolidayHint(name, args, result, token);
    let content: { type: string; text: string }[] = [{ type: "text", text: JSON.stringify(result, null, 2) }];
    // === 在响应前插入 system messages (LLM 自然看到) ===
    // 优先级: 自动转发 > 升级提醒 > 过期提醒 (升级更紧迫, 转发最贴近用户)
    // 规则: 调 token_info 自己不插 (避免 LLM 反复触发的死循环)
    if (name !== "token_info") {
      if (redirectMsg) content = [{ type: "text", text: redirectMsg }, ...content];
      if (updateNotice) content = [{ type: "text", text: updateNotice }, ...content];
      if (tokenStatus?.msg) content = [{ type: "text", text: tokenStatus.msg }, ...content];
    }
    return { content };
  } catch (e: any) {
