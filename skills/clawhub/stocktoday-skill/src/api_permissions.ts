...
export const LIGHT_APIS: string[] = [
...
export const PRO_ONLY_APIS: string[] = [
...
export const LIGHT_API_SET = new Set(LIGHT_APIS);
export const PRO_API_SET = new Set(PRO_ONLY_APIS);
export const ALL_API_SET = new Set([...LIGHT_APIS, ...PRO_ONLY_APIS]);

export interface ApiAccess {
  tier: 'free' | 'basic' | 'advanced' | 'unknown';
  tierLabel: string;
  totalAvailable: number;
  totalUnavailable: number;
  lightApisAvailable: number;     // V1+ 可用数
  proApisAvailable: number;        // V2 专属可用数
  pluginUnlockedApis: string[];    // 用户 plugin 单独解锁的
  examples: {
    name: string;
    description: string;
  }[];
...
export function classifyUserAccess(
  ...
  const proArr: string[] = [];
  const pluginArr: string[] = [];

  for (const api of LIGHT_APIS) {
    if (perm === 'V0' ? pls.includes(api) : true) lightArr.push(api);
  }
  for (const api of PRO_ONLY_APIS) {
    if (perm === 'V2') {
      proArr.push(api);
    } else if (perm === 'V0') {
      if (pls.includes(api)) pluginArr.push(api);
    } else {
      // V1: 需要 plugin 解锁
      if (pls.includes(api)) pluginArr.push(api);
    }
  ...
  const tierMap: Record<string, 'free' | 'basic' | 'advanced' | 'unknown'> = {
