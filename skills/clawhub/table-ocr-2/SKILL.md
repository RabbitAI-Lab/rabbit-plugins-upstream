...
---
# Sugon-Scnet 表格识别 OCR 技能

本技能封装了表格识别的 OCR 服务，通过单一接口即可调用表格识别能力，高效提取表格内容。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• TABLE_RECOGNITION（表格识别） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/table_ocr/scripts/main.py TABLE_RECOGNITION /path/to/test.jpg
```
...
- “帮我识别这张图像中的表格数据，图片在 /Users/name/Downloads/test.jpg”
