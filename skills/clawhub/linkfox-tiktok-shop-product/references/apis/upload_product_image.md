...
> 官方文档：https://partner.tiktokshop.com/docv2/page/upload-product-image-202309

## LinkFox 转发映射（本 skill）

| 项 | 值 |
|----|----|
| 具名 api | `upload_product_image` |
| 脚本 | `scripts/(multipart 暂不可经 proxy)` |
| 网关 | `POST /tiktokShop/developerProxy` |
| appType | **`erp`（固定）** |
| 上游 path | `product/202309/images/upload` |
| method | `POST` |
...
```bash
python scripts/product_api.py '{"api":"upload_product_image","openId":"...","shop_cipher":"GCP_..."}'
```
...
* CUSTOMIZATION_IMAGE: Images used for POD product customization, such as background images, mask images, or default images configured for customizable areas. These images are used to render or preview consumer-customizable product content.
