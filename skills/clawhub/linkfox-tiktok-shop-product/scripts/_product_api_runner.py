"""Run registered TikTok Shop ERP product APIs via /tiktokShop/developerProxy."""
...
from _product_endpoints import PRODUCT_ENDPOINTS, RESERVED_PARAM_KEYS
from _shop_product_common import (
...
def resolve_shop_cipher(params: dict, open_id: str) -> str:
    ...
    print(
        ...
        "  Example: {\"api\":\"check_listing_prerequisites\",\"openId\":\"...\",\"shop_cipher\":\"GCP_...\"}",
        file=sys.stderr,
    )
    sys.exit(1)
...
def run_product_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in PRODUCT_ENDPOINTS:
        print(
            f"Unknown api: {api_name}. Valid: {', '.join(sorted(PRODUCT_ENDPOINTS))}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_product_proxy(params: dict, caller: str = "product_proxy.py") -> dict:
    ...
    shop_cipher = None
    needs_cipher = str(path).lstrip("/").startswith("product/")
    if needs_cipher:
        shop_cipher = resolve_shop_cipher(params, open_id)
