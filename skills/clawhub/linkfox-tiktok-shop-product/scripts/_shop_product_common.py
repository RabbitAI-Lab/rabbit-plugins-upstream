"""Shared helpers for linkfox-tiktok-shop-product (ERP developerProxy, token backendized)."""
...
# ERP product skill path whitelist (relative paths, no host prefix)
ALLOWED_PATH_PREFIXES = ("product", "authorization")


def ensure_auth_skill_available(caller: str = "shop-product script") -> None:
...
def require_open_id(params: dict) -> str:
    ...
    ttsAccessToken is deprecated and ignored by the gateway; do not call storeTokens /
    refreshToken before business APIs — proxy auto-refreshes on 401 / token expired.
    """
...
def parse_proxy_business_json(proxy: dict) -> Optional[dict]:
    """Parse TikTok business JSON from developerProxy.body if present."""
    body_raw = proxy.get("body")
    if body_raw is None or not str(body_raw).strip():
        return None
    try:
