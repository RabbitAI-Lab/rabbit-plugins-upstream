...
def verify_entry(entry, check_pdf=True):
    ...
    if actual is None and entry.get("title"):
        # 无 id 或 id 查无 → 按标题检索解析（ аналог suggest_correct_pmid）
    ...
    if actual is None:
        out["verdict"] = "invalid"
        out["reason"] = "维普OA查无此文献（id 与标题检索均未命中）"
        return out
...
_HTML_FOOT = """<p style="color:#888;font-size:12px">验证链: 维普详情回查 → 标题双重模糊匹配(词级Jaccard/字级Sequence) → 作者/期刊/年份交叉 → PDF首页窗口匹配+卷期三方一致性(objectInfo vs PDF页眉 vs DOI正则)。判定语义与 pubmed-verifier 一致。</p>
</body></html>"""
