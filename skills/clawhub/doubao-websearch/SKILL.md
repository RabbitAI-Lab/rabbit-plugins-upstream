...
脚本自动完成：
1. **提交搜索**：`POST /story/api/doubaoSearch/submit`，传入 `{"inquiry_text": "...", "source": "豆包websearch-ClawHub"}`
...
| 情况 | 处理方式 |
|------|---------|
| 未配置 `REDFOX_API_KEY` | 提示用户前往 [红狐hub](https://redfox.hk/settings/api-keys?source=clawhub) 获取 API Key |
| 提交失败 | 输出错误信息，建议用户重试 |
