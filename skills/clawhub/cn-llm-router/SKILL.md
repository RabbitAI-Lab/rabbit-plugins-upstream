...
| v2.3.0 | 2026-08-07 | 优化：路由准确度——静态画像(60%)+动态胜率(40%)加权，新模型上线后无需手填画像，竞技场投票自动驱动路由；优化：合并 route/chat 建议模式为统一 --dry-run，消除重复的「展示推荐」逻辑；优化：缓存误命中——可选 sentence-transformers embedding 相似度（阈值 0.85），不装时降级 difflib 字符匹配 |
...
*版本：v2.3.0 ｜ 许可：MIT ｜ 核心纯标准库（讯飞星火可选 websocket-client）、零密钥打包、可只读审计。*