#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""个性化用药与康复指导 — 独立 skill，可单独拷贝发布；不依赖其他 skill 或 _shared。"""
...
TASKS: Dict[str, Dict[str, str]] = {'mc': {'title': '临床用药与治疗方案咨询', 'hint': '结合题干信息讨论用药与治疗方案要点，注明需遵医嘱。'}, 'rx-plan': {'title': '个性化用药教育问答', 'hint': '用患者可理解的语言解释用药目的、用法与注意事项。'}, 'rehab': {'title': '个性化康复指导问答', 'hint': '围绕康复评估与训练原则给出可执行的康复建议框架。'}}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="个性化用药与康复指导：由调用方传入题目与 --task，直连内部医疗大模型。")
