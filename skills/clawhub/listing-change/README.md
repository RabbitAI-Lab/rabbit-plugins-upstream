...
~~~text
查看一个 Amazon ASIN 已保存快照中的 Listing 字段变化提醒。商品 B0XXXXXXXX（美国站）。
~~~

示例 ASIN 是占位符。AI 会识别商品、站点和目标，无需填写接口参数。
客户端未选中时，在问题前加“使用 $amazon-listing-change-alert”。
...
固定工作流：`watch/listing`，输出 `watch_digest`；确定性 digest 为 0 积点，自动扫描不调用 LLM。
