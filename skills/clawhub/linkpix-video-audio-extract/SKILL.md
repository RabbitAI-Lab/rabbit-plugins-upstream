...
只有抖音等平台**分享链接**（不是直链）时，先用 qhkit 解析。缺 qhkit（npm 包 `@iqinghu/qhkit`，需 Node ≥ 18）就地安装：`npm i -g @iqinghu/qhkit`（官方源慢时加 `--registry=https://registry.npmmirror.com`）；无密钥时执行 `qhkit config set --token <密钥> --env prod`（让用户在 https://www.iqinghu.com/workbench/login?urlCode=agentch 注册/登录后，到工作台 APIKeys 页 https://www.iqinghu.com/workbench/dashboard/api-keys 创建密钥发给你；图文教程 https://xcnzsfe4uxrw.feishu.cn/wiki/KJ0Ywsyw8iAXmRkz5l4cddDbn6g）：
