---
name: med-llm-content-generation
description: 医学科普、文档摘要、术语释义、教学案例与诊疗文书辅助生成。通过 --task 选择类型；仅含 `scripts/run.py`，可独立拷贝部署。
metadata:
  {
    "openclaw":
      {
...
面向医学内容生产与通俗化解释，输出须符合医学规范且标注为辅助材料。
...
```bash
# 指定任务类型与题干
python3 scripts/run.py --task explain --question "题干……" --appkey YOUR_KEY

# 从 jsonl 取题（other.task 可省略若命令行已传 --task）
python3 scripts/run.py --task explain --input /path/to/items.jsonl --index 0 --appkey YOUR_KEY
```
