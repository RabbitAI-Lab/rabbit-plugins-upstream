...
def serve(port):
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def _reply_as_sse(self, raw):
            # wrap a whole (fallback) completion as a single SSE chunk stream
            # so clients that asked for stream:true still get valid SSE
            try:
                c = json.loads(raw)
    ...
    print("point your runtime's OpenAI-compatible base URL here (any api key value works)")
    srv.serve_forever()
