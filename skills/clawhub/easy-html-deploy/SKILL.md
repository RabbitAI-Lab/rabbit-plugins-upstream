...
description: Deploy a single self-contained HTML page to htmlcode.fun for instant sharing. Alias of html-deploy, updated for required descriptions, versioned updates, manual-like preservation, and safe unlocked-version edits.
---

> Alias note: `easy-html-deploy` is maintained as a compatibility alias of `html-deploy`. Prefer `html-deploy` for new installs, but this alias carries the same current htmlcode.fun workflow.
