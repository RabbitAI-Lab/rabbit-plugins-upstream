#!/usr/bin/env python3
"""Focused eKYC Suite AI Guardian cloud client."""
...
def call_cloud(tool, file_input):
    endpoint, api_key = cloud_config()
    headers = {
        "authorization": f"Bearer {api_key}",
        "content-type": "application/json",
        "user-agent": f"ekyc-suite-ai-guardian-skill/{CLIENT_VERSION}",
        "x-ekyc-client": "clawhub-skill-ai-guardian",
    ...
    response = requests.post(
        ...
        json={"file": media_input(file_input)},
        headers=headers,
        timeout=120,
    )
    try:
...
def main():
    parser = argparse.ArgumentParser(description="eKYC Suite AI Guardian")
    sub = parser.add_subparsers(dest="mode", required=True)
    sub.add_parser("photo").add_argument("--file", required=True)
    sub.add_parser("video").add_argument("--file", required=True)
    args = parser.parse_args()
    tool = "photo_liveness_detect" if args.mode == "photo" else "video_liveness_detect"
    try:
        result = call_cloud(tool, args.file)
