...
DEFAULT_LABEL = "com.cheeseclaw.somnia-nightly"
...
def build_plist(args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "StandardOutPath": str(log_dir / "somnia-nightly.out.log"),
        "StandardErrorPath": str(log_dir / "somnia-nightly.err.log"),
        "RunAtLoad": False,
        "KeepAlive": False,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Install or remove a macOS LaunchAgent for standalone Somnia review.")
    ...
    parser.add_argument("--report-root", default=str(Path.home() / ".openclaw" / "workspace" / ".learnings" / "somnia-nightly"))
    parser.add_argument("--update-output", default=str(Path.home() / ".openclaw" / "workspace" / "somnia-proposals"))
