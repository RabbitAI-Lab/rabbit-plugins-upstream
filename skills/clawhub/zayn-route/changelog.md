...
1. 增加内部业务策略与对外客户表达的参数传递边界。
2. 明确暂停、降级、退出和无成交空间不得直接改写成客户话术。
3. 新增价格倒挂后的 `PRICE() → QUALIFY() → REPLY()` 路由示例。
4. 增加保留商业关系入口但不得承诺价格、货源或更新时间的风险检查。
5. 新增 1 个真实案例回归测试。

## v0.1.6

1. 新增 `REFERENCE_TECH_DIAGNOSIS()` 参考作品技术拆解路由。
2. 建立与普通视觉修改、网页开发、视频、3D 和实体制造任务的边界。
3. 明确参考证据不足、指定路线与还原目标冲突、核心原型失败时的停止条件。
4. 新增 3 个路由示例和 3 个测试案例。
