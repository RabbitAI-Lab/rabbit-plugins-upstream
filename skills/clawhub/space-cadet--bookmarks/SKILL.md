...
When replying `/save` to an assistant (cloudybot) message, the skill retrieves the full assistant response from session history instead of using the truncated context snippet. This is done by:
1. Detecting `[reply target] cloudybot` in the context
