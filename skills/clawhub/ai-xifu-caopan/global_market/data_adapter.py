...
def get_gs_realtime(market, symbol):
    ...
    ⚠️ [安全提示] 此函数会通过subprocess调用外部脚本 gs_stock_market_query.py
    获取实时行情数据。调用过程中会继承当前环境变量，仅用于数据获取，
    不会修改任何文件或系统配置。
    
    调用脚本路径: ~/.openclaw/workspace/skills/guosen-finance-all/scripts/gs_stock_market_query.py
    """
...
def get_gs_history(market, symbol, days=30):
    ...
    ⚠️ [安全提示] 此函数会通过subprocess调用外部脚本 gs_stock_market_query.py
    获取历史K线数据。调用过程中会继承当前环境变量，仅用于数据获取，
    不会修改任何文件或系统配置。
