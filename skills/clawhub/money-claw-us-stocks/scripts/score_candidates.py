...
VALID_PATH_TYPES = {"CONVENTIONAL_GAP", "CPHI_SUBTYPE", "NONE"}
...
def classify(row: dict[str, Any]) -> dict[str, Any]:
    ...
    valuation_price = first_positive(last_price, pre_price, prev_close)
...
def markdown(results: Iterable[dict[str, Any]]) -> str:
    ...
    output = [
        "| Symbol | Status | Path | Score | Pre-gap | Open-gap | Pre-turnover | Spread | Risks | Unknown |",
        "|---|---|---|---:|---:|---:|---:|---:|---|---|",
    ]
    for item in rows:
        output.append(
            "| {symbol} | {status} | {path} | {score} | {pre_gap} | {open_gap} | {turnover} | {spread} | {risks} | {unknown} |".format(
                symbol=item["symbol"],
                status=item["status"],
                path=item["path_type"],
