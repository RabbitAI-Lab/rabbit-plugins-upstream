...
## After-Hours Cases: FGI and GXAI

The after-hours leaderboard is a next-session discovery layer, not a direct buy signal. The workflow separates candidates into two auditable routes:
...
flowchart LR
    ...
    AH -->|"Yes"| AR["Earnings support / verified tight supply"]
...
- `path_type`: `CONVENTIONAL_GAP`, `CPHI_SUBTYPE`, `AFTER_HOURS_EARNINGS`, `AFTER_HOURS_LOW_SUPPLY`, or `NONE`
