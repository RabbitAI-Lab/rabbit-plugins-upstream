...
function nestedBoolean(value, keys) {
  if (!value || typeof value !== 'object') return null;
  for (const [key, child] of Object.entries(value)) {
    if (keys.includes(key) && typeof child === 'boolean') return child;
    const nested = nestedBoolean(child, keys);
...
async function prepare(values) {
  ...
  const pluginDir = path.resolve(
    values.get('--plugin-dir') ?? path.join(root, 'extensions', 'clawtopics-link'),
  );
  ...
  if (nestedBoolean(inspect, ['enabled', 'isEnabled']) === false) {
    const error = new Error('Plugin is installed but disabled');
