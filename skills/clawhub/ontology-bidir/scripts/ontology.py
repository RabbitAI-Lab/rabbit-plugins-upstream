...
def get_related(entity_id: str, rel_type: str, graph_path: str, direction: str = "both") -> list:
    """
    Get related entities.

    Args:
        entity_id: The entity whose relations to traverse.
        rel_type: If set, only return relations of this type.
        graph_path: Path to the graph.jsonl file.
        direction: 'outgoing' shows relations where entity is source; 'incoming' where it
                   is target; 'both' (default since v1.1) shows the full network in one call
                   and includes a 'direction' field on each result.
    """
...
def main():
    ...
    related_p.add_argument("--dir", "-d", choices=["outgoing", "incoming", "both"], default="both",
                           help="Direction of relations to follow. Default 'both' shows incoming + outgoing for full network view. Use 'outgoing'/'incoming' to filter to one direction.")
