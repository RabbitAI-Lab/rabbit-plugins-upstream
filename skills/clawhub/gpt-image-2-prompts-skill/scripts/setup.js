#!/usr/bin/env node
/**
 * setup.js - Downloads GPT Image 2 prompt library from HuggingFace
 *
 * Usage:
...
const DATA_URL = 'https://huggingface.co/datasets/Goku-OpenLab/gpt-image-2-prompts-datasets/resolve/main/metadata.jsonl';
...
async function setup() {
  ...
  console.log(`[setup] ${label} GPT Image 2 prompt library from HuggingFace...`);

  // Check if we need to download
  const needsDownload = forceMode || !existsSync(destFile) || (existsSync(destFile) && statSync(destFile).size < 100);

  if (needsDownload) {
    process.stdout.write(`  → metadata.jsonl (15,600+ prompts) ... `);
    try {
