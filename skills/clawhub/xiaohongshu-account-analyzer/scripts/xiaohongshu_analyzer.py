...
def cmd_query(user_ids):
    ...
    for user_id in user_ids:
        ...
        if failure_count >= FAILURE_THRESHOLD:
            print(json.dumps({
                "status": "error",
                "message": "当前账号ID已超过失败阈值，请联系客服邮箱redfoxdata@proton.me 或访问 https://redfox.hk/?source=clawhub 处理",
    ...
    try:
        ...
        print(json.dumps({
            "status": "error",
            "message": "未查询到相关账号：当前 Skill 仅收录热门账号。如需定制数据，可邮件联系[红狐数据](https://redfox.hk/?source=clawhub)咨询：redfoxdata@proton.me",
    ...
    if len(full_data_items) > 1:
        ...
        if not accounts_with_works:
            print(json.dumps({
                "status": "success",
                "query_type": "no_works",
                "message": "未查询到该账号近30天有发布作品，当前 Skill 仅收录热门账号。如需定制数据，可邮件联系[红狐数据](https://redfox.hk/?source=clawhub)咨询：redfoxdata@proton.me"
            }, ensure_ascii=False))
            return
