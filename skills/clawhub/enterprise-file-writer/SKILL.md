...
description: 写入内容到本地文件，支持企业安全策略环境。明确支持：文本文件(.txt/.md/.log)、代码文件(.java/.py/.js等30+种)、配置文件(.json/.yaml/.ini等)、Office文档(.docx/.xlsx)。通过正确的编码处理(UTF-8/GBK)避免乱码问题。
---
...
**高优先级触发**（明确的文件写入动作）:
- "使用 enterprise-file-writer 写入文件"
- "用 Python 写入文件到 [路径]"
- "编码写入文件避免乱码"
- "写入 .docx/.xlsx 文件"
- "在企业环境中写入文件"
...
**中优先级触发**（需要确认用户意图）:
- "写入文本文件到 [具体路径]"（必须包含明确路径）
- "创建代码文件并写入内容"（必须包含"写入"动作）

**不触发的情况**（仅是讨论或理解）:
- ❌ "如何保存文件"（只是询问方法，不是执行写入）
- ❌ "这个文件怎么创建"（只是询问创建方式）
- ❌ "文件保存失败怎么办"（只是询问问题解决方法）

**激活前确认**: 如果用户表述模糊，先确认："您是想让我使用 enterprise-file-writer 写入文件，还是只是想了解文件操作方法？"
...
**Agent 调用本工具时必须遵守以下规则**：

1. **用户确认优先**：写入敏感路径或覆盖重要文件前，必须向用户明确说明风险并获得确认
2. **禁止自动绕过**：不得使用 `--force` 参数自动跳过安全检查，除非用户明确要求
3. **路径验证**：调用前验证目标路径是否符合用户意图，防止路径注入攻击
4. **内容审查**：写入的内容必须与用户需求相关，不得写入恶意代码或可疑内容
5. **敏感路径拦截**：以下路径需要额外确认：
   - 系统目录（Windows: `C:\Windows`, `C:\Program Files` 等；Linux: `/etc`, `/usr`, `/bin` 等）
   - 配置文件（`.env`, `.git/config`, SSH 配置等）
   - 凭据文件（包含 `password`, `secret`, `credential` 的文件）
   - 可执行脚本（`.sh`, `.bat`, `.ps1` 等）
...
# 跳过安全警告（谨慎使用）
python D:\ai\workspace\skills\enterprise-file-writer\write_file.py "文件路径" "内容" --force
```
...
⚠️ **安全警告**:
- **覆盖操作不可恢复**：默认模式下，已存在的文件将被完全覆盖，原内容永久丢失
- **敏感路径保护**：写入系统目录、配置文件、凭据文件、可执行脚本时会触发安全警告
- **Agent 调用限制**：Agent 不得自动使用 `--force` 跳过安全检查，必须获得用户明确确认
- **路径验证**：调用前验证目标路径符合用户意图，防止路径注入或意外覆盖
- 写入前请确认目标路径和内容正确无误
