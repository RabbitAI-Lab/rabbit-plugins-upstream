...
- For a one-off artifact or result, use the registered `flux` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi flux --help
runapi flux <operation> --help
curl --fail --location https://runapi.ai/docs/api/flux/<operation>.md --output contract.md
```
...
```shell
runapi flux <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service flux --action <operation> > result.json
```
