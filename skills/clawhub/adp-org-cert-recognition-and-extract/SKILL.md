---
name: org-cert-recognition-and-extract
description: 基于来也科技ADP平台的中国组织机构代码证智能识别与信息抽取Skill。支持组织机构代码证的统一社会信用代码、名称、类型、法定代表人、成立日期、营业期限、经营范围、登记机关、发证日期等关键字段的精准抽取，输出结构化JSON，零配置开箱即用，适用于企业资质核验、工商信息查询等场景。
---

# 组织机构代码证识别与抽取 Skill

能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=clawhub)提供，支持中国组织机构代码证的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（每月刷新），相当于每月可免费抽取 **200 张组织机构代码证**。ADP 提供标准可商用 API，**1 小时即可快速集成接入业务系统**。
...
3. **获取应用列表**：首次执行时，通过 `adp app-id list --app-type 0` 获取开箱即用应用列表，找到组织机构代码证抽取应用并记录其 `app_id`（以 `ootb_` 开头）。后续优先使用 `adp app-id cache`。
4. **执行抽取**：运行 `adp extract url <URL> --app-id <组织机构代码证抽取应用ID>` 或 `adp extract local <文件路径> --app-id <组织机构代码证抽取应用ID>`。
5. **结果处理**：解析返回的 JSON，提取组织机构代码证统一社会信用代码、名称、类型、法定代表人、成立日期、营业期限、经营范围、登记机关、发证日期等结构化字段。
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张组织机构代码证图片（URL） | `adp extract url <URL> --app-id <组织机构代码证抽取应用ID>` |
| 识别一张本地组织机构代码证图片 | `adp extract local <文件路径> --app-id <组织机构代码证抽取应用ID>` |
| 识别 Base64 编码的组织机构代码证 | `adp extract base64 <base64> --app-id <组织机构代码证抽取应用ID> --file-name <文件名.后缀>` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的组织机构代码证 | `adp extract local <文件夹路径> --app-id <组织机构代码证抽取应用ID>` |
| 批量识别多个 URL | `adp extract url <URL列表文件> --app-id <组织机构代码证抽取应用ID>` |

**异步处理**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 异步提交大文件 | `adp extract url <URL> --app-id <组织机构代码证抽取应用ID> --async` |
| 异步批量处理 | `adp extract local <文件夹路径> --app-id <组织机构代码证抽取应用ID> --async` |
...
## 第三步：获取组织机构代码证抽取应用 ID

ADP 为中国组织机构代码证提供了**开箱即用**的内置抽取应用，无需额外配置。
...
[
  {
    "app_id": "ootb_******8a1b",
    "app_label": ["组织机构代码证", "银行凭证", "企业开户", "金融证件", "信息提取"],
    "app_name": "组织机构代码证",
    "app_type": 0
  }
]
...
> 上例中 `"app_id": "ootb_******8a1b"` 即为组织机构代码证抽取应用。`app_type` 为 `0` 表示开箱即用应用，`1` 表示自定义应用。
...
```bash
adp extract url https://example.com/organization-cert.jpg --app-id <组织机构代码证抽取应用ID>
```

### 单张组织机构代码证抽取（本地文件）

```bash
adp extract local ./organization-cert.jpg --app-id <组织机构代码证抽取应用ID>
```

### 单张组织机构代码证抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <组织机构代码证抽取应用ID> --file-name <文件名.后缀>
```

### 返回结果示例

ADP 组织机构代码证抽取统一返回结构化 JSON，支持组织机构代码证以下**6个**关键字段识别抽取。
...
ADP 组织机构代码证抽取返回以下6个字段：

| field_key | field_name | 说明 | 敏感级别 |
| --- | --- | --- | --- |
| `code` | 代码 | 组织机构代码证上的统一社会信用代码 | 中 |
| `institution_name` | 机构名称 | 组织机构代码证上的注册名称 | 中 |
| `institution_type` | 机构类型 | 组织机构代码证上的机构类型 | 低 |
| `legal_representative` | 法定代表人 | 组织机构  代码证上的法定代表人姓名 | 高 |
| `address` | 地址 | 组织机构代码证上的注册地址 | 高 | 
| `validity_period` | 有效期 | 组织机构代码证上的营业期限 | 低 |
| `issuing_organization` | 颁发单位 | 组织机构代码证上的登记机关/颁发单位 | 低 |
| `registration_number` | 登记号 | 组织机构代码证上的登记号码（部分版本可能存在） | 中 | 
...
```bash
adp extract local ./organization-certs/ --app-id <组织机构代码证抽取应用ID> --export ./results/ 
```
...
```bash
# 提交异步任务
adp extract url https://example.com/organization-cert.jpg --app-id <组织机构代码证抽取应用ID> --async
...
# 组织机构代码证抽取（URL）
adp extract url <文件URL> --app-id <组织机构代码证抽取应用ID>

# 组织机构代码证抽取（本地文件）
adp extract local <文件路径> --app-id <组织机构代码证抽取应用ID>

# 组织机构代码证抽取（Base64）
adp extract base64 <base64字符串> --app-id <组织机构代码证抽取应用ID> --file-name <文件名.后缀>

# 批量抽取
adp extract local <文件夹路径> --app-id <组织机构代码证抽取应用ID> --export <输出路径> 

# 异步抽取
adp extract url <文件URL> --app-id <组织机构代码证抽取应用ID> --async
