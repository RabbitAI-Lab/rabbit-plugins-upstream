...
|---|---|
| 🏷️ 技能名称 | **植物物种识别技能** |
| 🎯 核心目标 | 植物物种识别技能，基于深度学习与计算机视觉技术，通过图像快速识别植物物种，输出物种名称、科属分类、生长习性及养护要点等结构化信息，为园艺、生态调研、自然教育提供专业识别服务 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `PLANT_DISEASE_RECOGNITION` |
...
识别视频/图片中的植物，准确判断植物物种名称，提供完整的植物知识信息
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供植物视频/图片需要识别物种时，默认触发本技能 |
| 🔎 明确分析意图 | 当用户明确需要植物识别、物种鉴定时，提及植物识别、植物种类、花草识别、树木识别、物种分类等关键词，并且上传了视频/图片，自动触发本技能 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看历史识别报告、植物识别报告清单、识别报告列表、查询历史识别报告、显示所有识别报告、植物物种分析报告，查询植物物种识别分析报告 |
...
```bash
python -m scripts.plant_species_recognition_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.plant_species_recognition_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 要求项 | 说明 |
|---|---|
| 植物主体完整 | ，叶片、花朵等识别特征完整出镜，避免过度遮挡 |
| 光线充足清晰 | ，避免过度模糊、过暗过曝和严重色差 |
| 多株植物同框时尽量保持间距适中，便于分别识别 | 多株植物同框时尽量保持间距适中，便于分别识别 |
...
| 3 | ⚙️ 执行植物物种识别分析 | 调用 `-m scripts.plant_species_recognition_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/plant_species_recognition_analysis.py`](scripts/plant_species_recognition_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 🧑‍⚖️ 结果性质 | 分析结果仅供知识参考，药用食用等用途请务必咨询专业人士确认安全性 |
...
```bash
# 识别本地视频/图片中的植物
python -m scripts.plant_species_recognition_analysis --input /path/to/plant.mp4

# 识别网络视频/图片
python -m scripts.plant_species_recognition_analysis --url https://example.com/flower.mp4

# 显示历史识别报告/显示识别报告清单列表/显示历史植物识别（自动触发关键词：查看历史识别报告、历史报告、识别报告清单等）
python -m scripts.plant_species_recognition_analysis --list

# 输出精简报告
python -m scripts.plant_species_recognition_analysis --input plant.jpg --detail basic

# 保存结果到文件
python -m scripts.plant_species_recognition_analysis --input plant.jpg --output result.json
```
