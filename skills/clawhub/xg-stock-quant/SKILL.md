...
name: 小果股票量化分析助手
description: |
  小果(微信:xg_quant)股票量化分析助手专注于股票量化分析工具，基于小果量化策略系统，提供股票历史行情数据、股票分钟数据、股票因子数据、股票财务数据、指数数据、
  股票策略回测、股票组合分析等核心功能。
  适用于股票投资者、量化研究员和策略开发者。
  触发关键词：股票量化、股票回测、股票分析、股票策略、股票数据、分钟数据、财务数据、指数数据。
author: 小果
contact: 微信 xg_quant
version: 1.0.0
---
...
| 功能模块 | 说明 |
| :--- | :--- |
| 📊 **股票行情数据** | 获取股票历史日线行情（开、高、低、收、量、额） |
| 📈 **股票因子数据** | 提取股票技术因子（均线、MACD、KDJ、RSI、布林带、Alpha等数百种） |
| ⏰ **股票分钟数据** | 获取5/15/30/60分钟K线数据 |
| 💰 **股票财务数据** | 查询股票财务数据（资产负债表、利润表、现金流量表、成长/盈利/偿债能力等） |
| 📊 **指数数据** | 获取指数日线/周线/月线K线数据 |
| 🔄 **股票策略回测** | 定投、动量、资产配置、资产配置平衡、网格、海龟、综合动量、均值方差、条件因子、排序多因子等10+种策略 |
| 📊 **股票组合分析** | 多股票相关性矩阵、协方差矩阵、投资组合优化、组合收益分析 |
...
2.2 股票行情数据接口
函数名	功能说明
get_stock_hist_data(stock, start_date, end_date)	获取股票历史日线行情数据
2.3 股票分钟数据接口
函数名	功能说明
get_mini_data_5(stock, start_date, end_date, frequency, adjustflag)	获取股票5分钟K线数据
get_mini_data_15(stock, start_date, end_date, frequency, adjustflag)	获取股票15分钟K线数据
get_mini_data_30(stock, start_date, end_date, frequency, adjustflag)	获取股票30分钟K线数据
get_mini_data_60(stock, start_date, end_date, frequency, adjustflag)	获取股票60分钟K线数据
query_history_k_data_plus_d(stock, start_date, end_date, frequency, adjustflag)	获取股票日线数据
query_history_k_data_plus_w(stock, start_date, end_date, frequency, adjustflag)	获取股票周线数据
query_history_k_data_plus_m(stock, start_date, end_date, frequency, adjustflag)	获取股票月线数据
2.4 指数数据接口
函数名	功能说明
query_history_k_data_plus_index_d(stock, start_date, end_date, frequency)	获取指数日线数据
query_history_k_data_plus_index_w(stock, start_date, end_date, frequency)	获取指数周线数据
query_history_k_data_plus_index_m(stock, start_date, end_date, frequency)	获取指数月线数据
2.5 股票因子数据接口
函数名	功能说明
get_stock_factor_data(stock, start_date, end_date, columns)	获取股票因子数据（支持数百种技术因子）
2.6 股票财务数据接口
函数名	功能说明
get_stock_finance_data(table, date, columns)	获取股票财务数据（资产负债表/利润表/现金流量表等9种报表）
query_profit_data(code, year, quarter)	获取盈利能力数据（ROE、销售净利率、毛利率等）
query_operation_data(code, year, quarter)	获取营运能力数据（应收账款周转率、存货周转率等）
query_growth_data(code, year, quarter)	获取成长能力数据（营收、净利润同比增长率等）
query_balance_data(code, year, quarter)	获取偿债能力数据（流动比率、速动比率、资产负债率等）
query_cash_flow_data(code, year, quarter)	获取现金流量数据
query_dupont_data(code, year, quarter)	获取杜邦分析数据（ROE分解、权益乘数等）
2.7 股票策略回测接口
函数名	策略类型	功能说明
xg_dt_backtrader(...)	定投策略	定期定额投资策略回测，支持止盈/补仓
xg_mom_backtrader(...)	动量策略	基于价格动量选择强势股票回测
xg_pz_backtrader(...)	资产配置策略	按固定权重配置多股票组合回测
...
xg_condi_factor_backtrader(...)	条件因子策略	自定义因子条件与买卖信号的多因子选股策略回测
xg_rank_factor_backtrader(...)	排序多因子策略	通过因子打分排序进行股票选择与轮动策略回测
2.8 股票组合分析接口
函数名	功能说明
xg_stock_cov_correlation(start_date, end_date, stock_list, max_workers, method, risk_free_rate)	计算多只股票收益率的相关性矩阵
xg_stock_cov_covariance(start_date, end_date, stock_list, max_workers, method, risk_free_rate, annualized)	计算多只股票收益率的协方差矩阵（可年化）
xg_stock_cov_portfolio(start_date, end_date, stock_list, max_workers, method, risk_free_rate, target_return)	多股票投资组合优化（最小方差/最大夏普/风险平价）
xg_stock_analysis(start_date, end_date, stock_list, stock_weight, index_stock, max_workers, risk_free_rate)	股票组合收益分析（总收益/年化收益/夏普/最大回撤/Beta/Alpha等50+项指标）
2.9 数据查询接口
函数名	功能说明
get_wencai_data(query)	获取问财数据
get_user_def_data(name, func)	获取自定义数据（支持AKShare等）
get_user_base_data(file_path, file_name)	获取数据库中的数据
root()	系统根路径测试
health()	系统健康检查
三、股票策略回测参数速查
...
weight_list	str	权重配置，与股票列表一一对应
index_stock	str	基准指数代码
cash	float	初始资金
sell_zdf	float	止盈涨幅
buy_zdf	float	补仓跌幅
...
min_hold_days	int	最少持有天数
risk_free_rate	float	无风险利率
3.11 股票组合分析 xg_stock_analysis
参数	类型	说明
start_date	str	开始日期 YYYYMMDD
end_date	str	结束日期 YYYYMMDD
stock_list	str	股票列表，逗号分隔
stock_weight	str	股票权重，逗号分隔（自动归一化）
index_stock	str	基准指数代码
risk_free_rate	float	无风险利率
四、股票因子字段参考
4.1 基础因子
...
result = client.get_stock_finance_data(
    table='资产负债表',
    date='2024-06-30',
    columns='secu_code,secu_abbr,end_date,total_assets,total_liability'
)
df = client._to_dataframe(result)
...
result = client.xg_dt_backtrader(
    start_date='20240101',
    end_date='20241231',
    stock_list='600031.SH,000858.SZ',
...
result = client.xg_mom_backtrader(
    start_date='20240101',
    end_date='20241231',
    stock_list='600031.SH,000858.SZ,600519.SH',
    index_stock='000300.SH',
    cash=100000,
    mom_type='百分比',
    mom_value=1,
...
上海	代码.SH	600031.SH
深圳	代码.SZ	000858.SZ
分钟数据（mini）	sh.代码	sh.600031
8.2 日期格式说明
接口类型	日期格式	示例
行情/因子/回测	YYYYMMDD	20240101
财务数据	YYYY-MM-DD	2024-06-30
分钟数据	YYYY-MM-DD	2026-04-01
8.3 策略类型可选值
策略类型	说明
定投策略	定期定额投资
动量策略	动量因子选股
...
分钟数据支持5/15/30/60分钟周期

财务数据支持9种报表类型

指数数据支持上证指数、深证成指等主流指数

因子数据支持数百种技术指标

回测结果包含完整的绩效指标和交易记录

如有问题可联系小果微信: xg_quant
# 小果量化策略回测系统
# 一、介绍
