#!/usr/bin/env python3
"""List and render agentic development email templates."""
...
CATALOG_MD = ROOT / "references" / "email-catalog.md"
...
def extract_template(template_id: str) -> str:
    ...
    if not match:
        raise KeyError(f"Template not found in email-catalog.md: {template_id}")
    ...
    if args.list:
        for item in templates:
            if args.stage and item["stage"] != args.stage:
                continue
            print(f"{item['id']}\t{item['stage']}\t{item['subject']}")
        return 0
