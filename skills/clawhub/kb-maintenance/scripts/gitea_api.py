...
class GiteaClient:
    ...
    def read_text(self, path):
        return self.read_file(path).get("content") or ""
    ...
    def read_file(self, path):
        ...
        return {"content": decoded, "sha": response.get("sha")}

    def exists(self, path):
        return bool(self.read_text(path))

    def upsert_text(self, path, content, message):
        if self.dry_run:
            self.last_commit = "dry-run-" + sha256_text(path + "\n" + content)[:12]
    ...
    def upsert_text_with_sha(self, path, content, message, sha):
        if self.dry_run:
            self.last_commit = "dry-run-" + sha256_text(path + "\n" + content)[:12]
            return {"path": path, "commit": self.last_commit}
        body = {
