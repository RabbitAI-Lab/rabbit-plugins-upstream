...
CATALOG_PATH = {
    ...
    "sensenova": "/models",
    "siliconflow": "/models",
    "google": "/models",  # 走 /v1beta/openai/models
}
GOOGLE_OPENAI_CATALOG = "https://generativelanguage.googleapis.com/v1beta/openai/models"
...
def fetch_catalog(provider, key, base):
    ...
    if provider == "google":
        st, body = C.http_json(GOOGLE_OPENAI_CATALOG, None,
...
def main():
    ...
    providers = (args.providers.split(",") if args.providers
                 else ["agnes", "sensenova", "siliconflow", "google"])
    ...
    if not available:
        print(json.dumps({
            ...
            "note": ("未检测到任何可服务免费媒体模型的平台密钥。"
                     ...
                     "硅基流动 https://cloud.siliconflow.cn ｜ Google https://aistudio.google.com")
        }, ensure_ascii=False))
        sys.exit(2)
    ...
    if added:
        for c in added:
            media.append({
                ...
                "free": True,
                "needs_vpn": c["provider"] == "google",
                "watermark_removable": False,
                "default_size": "1024x1024",
                "features": "审计自动发现，待人工确认免费策略",
                "status": "verified",
            })
...
def _render_report(today, media, candidates, added, unreachable, available, no_live):
    ...
    return "\n".join([
        ...
        "- Gemini 等海外模型需有效 VPN；不可达不代表模型失效。",
