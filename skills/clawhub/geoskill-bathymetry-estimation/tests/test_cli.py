"""CLI argument parsing tests for bathymetry-estimation."""
import subprocess
import sys
import os
...
class TestCLIBasics:
    ...
    def test_lyzenga_method_runs(self):
        out = os.path.join(os.path.dirname(SCRIPT), "_test_cli_lyzenga")
        r = run_cli(["--bbox", "116", "39", "117", "40", "--synthetic",
                     "--method", "lyzenga", "--output-dir", out, "--quiet"])
        assert r.returncode == 0, r.stderr
        assert os.path.exists(os.path.join(out, "bathymetry.tif"))
