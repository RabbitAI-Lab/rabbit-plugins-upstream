#!/usr/bin/env python3
"""HIPAA 美国医疗健康护栏检测内核 (hipaa-guard)。

实时护栏：在面向美国市场的医疗健康产品（远程医疗 / 医疗 SaaS / 健康 App /
患者门户）的文案、隐私政策、产品描述发布前，检测其中触发 HIPAA 适用的表述
与医疗隐私违规用语（处理受保护健康信息 PHI / 缺少商业伙伴协议 BAA / PHI 未加密 /
第三方披露 PHI / 缺失泄露通知 / 限制个人访问权），按风险分级输出命中与
整改建议，供 Agent 主动调用。
...
def main(argv=None):
    p = argparse.ArgumentParser(description="HIPAA 美国医疗健康护栏检测内核")
    ...
    if args.format == "json":
        ...
        if result["decision"] == "clean":
            print("未检出已知 PHI 触发/违规表述；发布前仍建议结合完整 HIPAA 合规审计。")
        else:
            print("检出医疗隐私相关表述，建议按建议修改或补正；如需完整审计可对接 hipaa-check/audit。")
    return 0
