...
|---|---|
| 🏷️ 技能名称 | **宠物日常健康监测分析工具** |
| 🎯 核心目标 | 宠物日常健康监测分析技能，基于计算机视觉通过摄像头/喂食器监控视频分析宠物的进食频次、饮水频次、排泄状态、精神状态、呕吐行为、跛行异常等健康指标，及时发现宠物异常健康状况，输出健康监测报告 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `PET_HEALTH_MONITORING` |
...
通过监控视频/图片对宠物进行日常健康监测分析，获取结构化的宠物健康监测报告
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供宠物监控视频 URL 或文件需要进行健康监测分析时，默认触发本技能 |
| 🔎 明确分析意图 | 当用户明确需要进行宠物健康监测，提及宠物健康监测、进食分析、饮水分析、排泄状态、精神萎靡、呕吐行为、跛行异常等关键词，并且上传了视频文件或者图片文件 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看历史监测报告、宠物健康报告清单、监测报告列表、查询历史报告、显示所有监测报告、宠物健康监测历史记录，查询宠物健康监测分析报告 |
...
```bash
python -m scripts.pet_health_monitoring_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.pet_health_monitoring_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 3 | ⚙️ 执行宠物健康监测分析 | 调用 `-m scripts.pet_health_monitoring_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| `--monitor-days` | 监测天数/视频覆盖时长，单位：天，默认 1 | 按需填写 |
| `--list` | 显示宠物健康监测历史分析报告列表清单（可以输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/pet_health_monitoring_analysis.py`](scripts/pet_health_monitoring_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
```bash
# 分析本地监控视频
python -m scripts.pet_health_monitoring_analysis --input /path/to/pet_monitor.mp4 --pet-type cat --monitor-days 1 分析网络监控视频
python -m scripts.pet_health_monitoring_analysis --url https://example.com/pet_monitor.mp4 --pet-type cat --monitor-days 1 分析狗狗监控视频
python -m scripts.pet_health_monitoring_analysis --input /path/to/dog_monitor.mp4 --pet-type dog --monitor-days 3 显示历史监测报告/显示监测报告清单列表/显示历史宠物健康报告（自动触发关键词：查看历史检测报告、历史报告、监测报告清单等）
python -m scripts.pet_health_monitoring_analysis --list

# 输出精简报告
python -m scripts.pet_health_monitoring_analysis --input video.mp4 --pet-type cat --detail basic

# 保存结果到文件
python -m scripts.pet_health_monitoring_analysis --input video.mp4 --pet-type cat --output result.json
```
