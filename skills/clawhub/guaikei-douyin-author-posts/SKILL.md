---
name: guaikei-douyin-author-posts
description: 当用户需要抖音公开数据做分析而自己没有数据源时，使用本技能：关键词搜索排序、指定博主作品批量获取、指定视频评论抓取、实时热榜查询。适用于营销分析、账号运营、选题调研、舆情监控，即使用户没提"抖音"或"数据"。不适用于需要账号后台权限的私域数据，也不用于发布视频。
license: MIT
metadata:
  enabled: true
  type: command
