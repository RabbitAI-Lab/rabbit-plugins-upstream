---
name: qinghu-viral-video-womens
description: 青虎AI 爆款视频模仿（女装）：上传一条参考视频和一张模特参考图，把视频里的角色动作精准迁移到新形象上，适配真人与虚拟形象，高效还原动作细节，快速产出女装带货短视频。当用户要仿拍女装爆款视频、换女模特、做女装带货短视频、把爆款视频的动作套到自己模特身上时必须触发。关键词：青虎AI、爆款视频模仿、女装、模特、动作迁移、仿拍、带货短视频、换模特、AI视频。
user-invocable: true
...
女装版的**爆款视频仿拍**：上传参考视频 + 模特参考图，AI 把动作迁移到你的形象上，真人和虚拟形象都支持。

按视频秒数计费（`per_second`），**参考视频越短越省**。

## 何时触发

- 「仿拍这条女装爆款视频」「把这个视频换成我的模特」
- 「做一条女装带货视频」
- 「女模特换一下」
...
```bash
# 字段表
qhkit workflow options '{"workflow":"wf_007"}'
...
想要的是「开门换装」那种变装效果，用「女装开门换装爆款仿拍」，不是这个。
...
- 男装 / 童装场景请用对应的专项应用；开门换装变装效果用「女装开门换装爆款仿拍」；双人场景用「双人爆款视频模仿」。
...
- **女装开门换装爆款仿拍**：开门变装类的女装视频
- **爆款视频模仿(男装) / 爆款视频模仿(童装)**：其他服装品类
