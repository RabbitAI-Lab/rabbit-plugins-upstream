...
Options:
  --prompt [prompt]                    提示词
  --generation_mode [generation_mode]  生成模式（frames=首尾帧; extend=视频延展） [default: frames] (choices: "frames", "extend")
  ...
  --duration [duration]                时长（秒） (4/6/8 in frames mode; fixed 7 in extend mode) Options depend on "generation_mode". when generation_mode="": 4 (4s), 6 (6s), 8 (8s); when generation_mode="generation_mode=extend": 7 (+7s) [default: 8] (choices: "4", "6", "8", "7")
...
# 复杂调用：
dlazy veo-3.1-fast --prompt '提示词内容' --image 'https://files.dlazy.com/image.png'
```
