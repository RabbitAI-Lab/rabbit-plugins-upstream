...
[Skip to main content](#content-area)OpenClaw home pageEnglishSearch...⌘KSearch...NavigationMessaging platformsDiscordGet startedInstallChannelsAgentsToolsModelsPlatformsGateway & OpsReferenceHelpOverview
Chat Channels
Messaging platforms
...
- [Native commands and command auth](#native-commands-and-command-auth)
- [Feature details](#feature-details)
- [Tools and action gates](#tools-and-action-gates)
- [Components v2 UI](#components-v2-ui)
...
Discord DMs default to pairing mode.## Slash commands

Native command behavior and command catalog.## Channel troubleshooting

Cross-channel diagnostics and repair flow.
​Quick setup
1Create a Discord bot and enable intents
...
3Invite the bot and start gateway

Invite the bot to your server with message permissions.Copy```
openclaw gateway

```

4Approve first DM pairing
...
If you only set `DISCORD_BOT_TOKEN` and do not create a `channels.discord` block, runtime fallback is `groupPolicy="open"` (with a warning in logs).Guild messages are mention-gated by default.Mention detection includes:
...
Presence intent is optional and only required if you want to receive presence updates. Setting bot presence (`setPresence`) does not require enabling presence updates for members.OAuth scopes and baseline permissions
...
Avoid `Administrator` unless explicitly needed.Copy IDs
...
Note: `off` disables implicit reply threading. Explicit `[[reply_to_*]]` tags are still honored.Message IDs are surfaced in context/history so agents can target specific messages.History, context, and thread behavior
...
Channel topics are injected as **untrusted** context (not as system prompt).Reaction notifications
...
Presence updates are applied only when you set a status or activity field.Status only example:Copy```
{
  channels: {
    discord: {
      status: "idle",
