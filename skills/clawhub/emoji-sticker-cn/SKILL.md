...
description: 中文平台(微信表情开放平台 / 小红书 / 抖音)合规表情包生成助手。提供平台尺寸硬约束裁切与命名、违禁词校验(实时 API + 离线兜底)、静态贴纸转内容匹配动效 GIF、合规参考数据维护与平台规范规则巡检工作流。
version: 3.0.2
license: MIT
trigger: "微信表情上架|微信表情包|微信表情开放平台|小红书表情包|抖音表情|中文表情合规模板"
agent_created: true
tools:
  - filesystem
  - http
...
3. **文案校验**:表情名称 / 介绍 / 含义词优先调 `multi-wordcheck`(未装则用 `scripts/check_compliance.py` 离线兜底并提示局限);不通过则改写后复检。
4. **上架实操**(sticker.weixin.qq.com):
   1. 登录微信表情开放平台,进入「提交表情」;
   2. 创建专辑:填名称(≤8 汉字)、介绍(≤80 字)、版权信息(≤10 字)、分类、标签;
   3. 按上述七件套路径逐项上传主图 / 图标 / 封面 / 横幅 / 缩略图 / 赞赏图 / 致谢图(编号一一对应);
   4. 逐张填写含义词(每张 ≤4 汉字,同套不重复);
   5. 预览确认后提交审核;被驳回时检查整套差异是否充分(官方明文的第一大拒因)。
...
## Hard rules(Safety guardrails for the agent executing this skill)
...
> 路线 A 进阶脚本(`check_assets.py` 素材机检 / `make_submit.py` 提交清单生成)未随本 skill 发布,如需请反馈补发。
