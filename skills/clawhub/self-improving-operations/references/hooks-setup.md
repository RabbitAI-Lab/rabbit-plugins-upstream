...
- **UserPromptSubmit**: Reminder after matching prompts to evaluate operational learnings
...
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "matcher": "incident|deploy|alert|capacity|outage|rollback|SLO|runbook",
        "hooks": [
          {
            "type": "command",
...
2. **Narrow the matcher further**. Current domain matcher examples: operations-related prompts:
