---
name: qinghu-viral-video-kids
description: 青虎AI 爆款视频模仿（童装）：上传一条参考视频和一张儿童模特参考图，把视频里的角色动作精准迁移到新模特身上，适配各类孩童形象，细腻还原可爱灵动的动作，快速产出童装带货短视频。当用户要仿拍童装爆款视频、换儿童模特、做童装带货短视频、把爆款视频的动作套到自己模特身上时必须触发。关键词：青虎AI、爆款视频模仿、童装、儿童模特、动作迁移、仿拍、带货短视频、换模特、AI视频。
user-invocable: true
...
**照着一条爆款视频重拍**：上传参考视频 + 一张儿童模特图，AI 把视频里的动作迁移到你的模特身上。

童装专用版本，对孩童体态和灵动动作做过适配。按视频秒数计费（`per_second`），所以**参考视频越短越省**。

## 何时触发

- 「仿拍这条童装爆款视频」「把这个视频的动作换成我的模特」
- 「做一条童装带货视频」
- 「儿童模特换一下」
...
```bash
# 字段表
qhkit workflow options '{"workflow":"wf_043"}'
...
1. **参考图比例要和视频一致**（字段名里就写着）——竖屏视频配竖屏图，比例不一致容易变形；
2. **参考视频尽量短**：按秒计费，30 秒的片段和 50 秒的价差接近一倍，而且越长越容易出瑕疵；
3. **参考视频里的人物要清晰完整**，遮挡多、镜头晃动大的素材迁移效果差。
...
- 男装 / 女装场景请用对应的专项应用，双人场景用「双人爆款视频模仿」——用错版本效果会明显下降。
- 参考视频必须是自有或已获授权的素材；直接搬运他人视频用于商用有侵权风险。
- 涉及未成年人形象的素材，需确保已获得监护人授权。

## 相关技能

同系列的其他「青虎AI」技能（按需转交，不要在本技能里硬做）：

- **爆款视频模仿(男装) / 爆款视频模仿(女装)**：其他服装品类
- **双人爆款视频模仿**：双人带货场景
