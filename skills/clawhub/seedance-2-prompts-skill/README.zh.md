# AI 视频提示词推荐 — 8,000+ Seedance 2 提示词
...
> **别再花时间到处找 AI 视频提示词了。** 一句话告诉 AI 助手你的需求 — 它会从 8,000+ 条精选 Seedance 2 提示词中搜索并返回最匹配的 3 条，附带封面图片和视频链接，开箱即用。
>
> 🎬 [浏览数据集 →](https://prompthub.gokuscraper.com/prompts/?model=seedance-2)

![Demo](public/cover.png)

## 这是什么？

一个 **AI Agent Skill**，让 Claude、OpenClaw、Cursor 等 AI 助手能够智能搜索 8,000+ 条精选 Seedance 2（字节跳动视频模型）提示词库，为你的需求推荐最佳匹配，甚至能根据你的内容定制提示词。

**Seedance 2** 是字节跳动最新的视频生成模型 — 目前最强大的 AI 视频生成器之一。好的提示词是生成好视频的关键。

## 为什么使用这个 Skill？

- ✅ **8,000+ 条提示词** — 覆盖各种使用场景
- ✅ **每条提示词都附带封面图 + 视频链接** — 复制前先看效果
- ✅ **智能语义搜索** — 描述需求，AI 自动匹配
...
| 总提示词数 | 8,000 |
| 语言 | 英文、中文（双语） |
| 来源 | [GokuScraper Seedance 2 数据集](https://huggingface.co/datasets/GokuScraper/seedance-2-prompts-datasets) |
...
**节能设计**：Skill 永不加载完整提示词文件，使用 grep 风格搜索，只提取匹配行，即使词库有 8,000+ 条提示词也能保持极低的 token 消耗。

---

## 数据来源

提示词来自公开社区，数据源于 HuggingFace 上的 [GokuScraper Seedance 2 数据集](https://huggingface.co/datasets/GokuScraper/seedance-2-prompts-datasets) — 8,000+ 条提示词，包含完整元数据、预览图片和视频链接。
...
**问：什么是 Seedance 2？**
Seedance 2 是字节跳动最新的视频生成模型，能够从文本提示词生成高质量的电影感视频。
...
词库经过预分类和筛选，8,000+ 条提示词都是精选优质内容，每条都附带示例图片，复制前就能看到效果。混搭模式还能根据你的内容个性化定制。

**问：我可以贡献提示词吗？**
数据源是开源的（CC BY 4.0）。欢迎访问 [HuggingFace 数据集](https://huggingface.co/datasets/GokuScraper/seedance-2-prompts-datasets) 了解更多。

**问：词库多久更新一次？**
HuggingFace 数据集由 GokuOpenLab 社区定期更新。

**问：这些提示词能用于其他图像生成模型吗？**
提示词针对 Seedance 2 优化，但也适用于其他视频生成模型。
...
│   ├── .gitkeep
│   └── metadata.jsonl       # 8,000+ 条提示词（由 setup.js 生成）
└── .claude-plugin/
    └── marketplace.json
```
...
- 🎬 [GokuScraper Seedance 2 数据集](https://huggingface.co/datasets/GokuScraper/seedance-2-prompts-datasets) — HuggingFace 上的源数据集（8,000+ 提示词、视频、元数据）
