...
`409 conflict`（409 冲突）时检查 `errors.idempotency_key`；不要换新键绕过冲突，也不要把同一键用于不同 JSON。
...
不要改成 REST 风格自造路径，也不要直接请求任何服务提供方接口地址。

## `202` 轮询

五个操作在当前产品定义中都是本地同步操作，但客户端仍必须兼容平台返回 `202`。读取 `task_id` 后，只 GET 同一操作的任务地址。使用有界指数退避，例如 1、2、4、8、16 秒，总等待最多 31 秒；不要在客户端假设工作进程休眠，也不要自动重复 POST。

终态为 `succeeded`（成功）、`partial`（部分成功）、`failed`（失败）或 `reconciliation_required`（需要人工对账）。到达等待上限时返回 `task_id`、最后状态和可继续查询的路径，不宣称成功或失败。

## 响应提取

成功时提取响应中的结构化 `result`/`data`、`artifacts`、`task_id` 与状态。任务 GET 失败读取 `error.code`；POST 验证失败通常读取 `errors.code`，同步执行故障也可能使用 `error.code`，因此先读 `error.code`，再回退 `errors.code`。
