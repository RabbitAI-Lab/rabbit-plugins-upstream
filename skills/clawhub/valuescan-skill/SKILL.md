...
description: ValueScan数据查询Skill。支持代币搜索、K线数据、主力资金流分析、链上数据查询等加密货币市场数据分析功能。
version: 1.0.7
user-invocable: true
...
- ~~大盘分析订阅~~ **（已废弃，2026-06-01 停止服务）** → 迁移至大盘解析列表
- 大盘解析列表      **→**  按代币 ID 查询 AI 大盘分析历史，支持时间范围和批量拉取
...
│   │   ├── ai-analyse.json          # 大盘分析订阅（已废弃）
│   │   └── market-analysis-list.json # 大盘解析列表
│   │
│   ├── fund/                         # 主力资金
