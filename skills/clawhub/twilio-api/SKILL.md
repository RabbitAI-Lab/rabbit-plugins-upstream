...
```bash
maton api -X DELETE '/twilio/2010-04-01/Accounts/{AccountSid}/Calls/{CallSid}.json'
```
...
```bash
maton api -X DELETE '/twilio/2010-04-01/Accounts/{AccountSid}/IncomingPhoneNumbers/{PhoneNumberSid}.json'
```
...
```bash
maton api -X DELETE '/twilio/2010-04-01/Accounts/{AccountSid}/Queues/{QueueSid}.json'
```
...
Twilio has no typed accessor yet, so calls go through the `api` passthrough, which takes the app and the path after it. `login()` opens a browser once per machine and writes the session to the SDK's own store — `maton login` does not carry over, and the SDK never signs in implicitly.
...
```bash
curl --config - "https://api.maton.ai/twilio/2010-04-01/Accounts.json" <<EOF
header = "Authorization: Bearer $MATON_API_KEY"
header = "User-Agent: maton-twilio-skill/1.1"
