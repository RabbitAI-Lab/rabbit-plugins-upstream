...
class ClientConfig:
    ...
    def from_args(cls, args: argparse.Namespace) -> "ClientConfig":
        username = args.username or os.getenv("MAVERICK_SMARTBILL_USERNAME")
        token = args.token or os.getenv("MAVERICK_SMARTBILL_TOKEN")
        base_url = args.base_url if args.base_url is not None else os.getenv("MAVERICK_SMARTBILL_API_BASE", DEFAULT_BASE_URL)
        timeout_raw: Any = args.timeout_seconds if args.timeout_seconds is not None else os.getenv(
            "MAVERICK_SMARTBILL_TIMEOUT_SECONDS", DEFAULT_TIMEOUT_SECONDS
        )
        retries_raw: Any = args.retries if args.retries is not None else os.getenv(
            "MAVERICK_SMARTBILL_RETRIES", DEFAULT_RETRIES
        )
        debug = getattr(args, "debug", False) or os.getenv("MAVERICK_SMARTBILL_DEBUG", "").lower() in ("1", "true", "yes")
        ...
        if not username:
            raise CliError(
                "Missing SmartBill username. Set MAVERICK_SMARTBILL_USERNAME or pass --username."
            )
        if not token:
            raise CliError(
                "Missing SmartBill token. Set MAVERICK_SMARTBILL_TOKEN or pass --token."
            )
        if not base_url:
...
def resolve_cif(args: argparse.Namespace, invoice: Optional[Dict[str, Any]] = None) -> str:
    cif = args.cif or os.getenv("MAVERICK_SMARTBILL_COMPANY_VAT_CODE")
    if not cif and isinstance(invoice, dict):
        cif = invoice.get("companyVatCode")
    if not cif:
        raise CliError(
            "Missing company VAT code (CIF). Set MAVERICK_SMARTBILL_COMPANY_VAT_CODE, pass --cif, "
...
def _safe_output_path(output: Path) -> Path:
    ...
    * https://github.com/openclaw/openclaw/blob/main/src/infra/tmp-openclaw-dir.ts
    ...
    state_dir = Path(
        os.environ.get("MAVERICK_OPENCLAW_STATE_DIR", Path.home() / ".openclaw")
    ).resolve()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--username", help="SmartBill login email. Defaults to MAVERICK_SMARTBILL_USERNAME.")
    parser.add_argument("--token", help="SmartBill API token. Defaults to MAVERICK_SMARTBILL_TOKEN.")
    parser.add_argument(
        "--base-url",
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Print request/response payloads to stderr. Also enabled by MAVERICK_SMARTBILL_DEBUG=1. Off by default.",
    )
