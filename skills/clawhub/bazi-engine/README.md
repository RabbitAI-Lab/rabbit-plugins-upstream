...
- **合婚评分**：七大维度综合评分
- **五行补救**：喜用神、调候用神（穷通宝鉴）、五行缺失与补救建议
- **神煞速查**：24 项吉凶星曜按命中展示
- **三式宫位**：胎元、命宫、身宫
- **断语可追溯**：504 条断语，每条标注古籍出处（三命通会 / 渊海子平 / 滴天髓 / 子平真诠 / 穷通宝鉴）与行动建议
...
- **UI**：`ui/index.html` — 单文件离线（零外部依赖），内置 206 年节气数据 + 504 条断语
...
断语库（`kb/04-rules-db/rules.json`）内含**溯源指纹句**（水印，具体位置不公开）。未经许可复用本断语库（如直接搬运数据做商业产品），指纹句会原样出现在对方内容中，可据此识别与取证。遵守 [COPYRIGHT](COPYRIGHT) / [DATA_RIGHTS](DATA_RIGHTS) 的正常使用不受任何影响。

## License

本仓库采用**双许可**结构：

- **代码**（排盘引擎 / UI / 构建测试脚本 / Skill 定义）：[MIT](COPYRIGHT) — 自由使用
- **数据**（`kb/` 断语库与古籍汇编等）：[CC BY-NC-SA 4.0](DATA_RIGHTS) — 署名 · 非商业 · 相同方式共享（古籍原文属公有领域，可自由引用）

> 双许可边界、来源归属与商业授权说明详见 [DATA_RIGHTS](DATA_RIGHTS)。
