---
name: socq-facebook-ad-library
description: Research public ads, advertisers, creatives, and campaign activity with SocQ. Use when an agent needs Facebook Ad Library-specific discovery, collection, endpoint selection, credit estimates, asynchronous task execution, pagination, or raw exports through the SocQ CLI, MCP, or REST API.
metadata:
  openclaw:
...
- Platform page: <https://socq.ai/apis/facebook-ad-library>
- API documentation: <https://docs.socq.ai/api-manual/facebook-ad-library>
...
Use this Skill to select and run SocQ endpoints for public Facebook Ad Library research. SocQ requests are asynchronous and credit-metered.

## Use When

- The user asks to discover, collect, compare, or analyze public Facebook Ad Library data.
- The workflow needs Facebook Ad Library-specific endpoint selection, input validation, pagination, or raw exports.
...
1. Prefer an already configured SocQ MCP server at `https://api.socq.ai/mcp?platforms=facebook-ad-library`; use the typed tool listed in [platform.md](references/platform.md), or compact `socq_execute` when needed.
