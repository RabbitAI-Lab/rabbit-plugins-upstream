...
在「从 asale 市场买」和「用它自己的订阅」之间切换 Hermes，并查看哪些正在运行的会话还用着旧配置。
...
Hermes 是唯一一个不把配置放在 `~/.<tool>` 目录里的工具。设了 `HERMES_HOME` 就以它为准——
它的安装器把这个写成**用户级**环境变量，所以 Hermes 装好之前就开着的终端里是没有它的。
没设的时候，已存在的配置优先于默认值（POSIX 上是 `~/.hermes`，Windows 上是
`%LOCALAPPDATA%\hermes`）。`buy_tools` 返回的 `config_paths` 才是这台机器上的权威答案——
读它，别猜。

开关也会拒绝去改一个它解析不了的 `config.yaml`：Hermes 会忽略的文件，往里写 `base_url`
什么也改变不了，而把这种情况报成「已生效」，正是一个开关能连着好几天看起来是开的、
而 agent 一直在用默认值的原因。
...
`hermes` 那一行带着 `installed`、`enabled`（开关）、`in_effect`（它的实时配置真的指向代理）、
...
不传 `models` 就保持当前选择，传 `[]` 是清空。会改写的配置：`<HERMES_HOME>/config.yaml` —— 它 `model:` 块里的四个标量键。
...
Hermes **只在启动时**读一次配置，所以已经在跑的会话拿的还是旧的。`running.hermes` 会按 pid 列出
它们，并带上启动时的命令行。
