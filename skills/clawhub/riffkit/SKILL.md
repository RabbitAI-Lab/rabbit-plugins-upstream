...
No params. Returns `[{id, name, price_usd, credits, seconds, videos, unit_price_usd, purchasable}]` (solo/starter/daily/pro; `purchasable=false` = payments not configured). **Introducing a plan is a pre-payment money moment: lead with dollars** (`price_usd`/mo, e.g. "$39/mo"), then what it buys ("3 videos at 720p, 7 on MiniMax H3"); `credits` is RAW INTERNAL (÷ 100 for the wallet number), and never re-price credits in dollars.
