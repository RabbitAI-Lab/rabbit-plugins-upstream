...
这是 `hekouwang-harness-check-skill` 独立仓库的运行时快照说明。独立源码仓库是 [`huiyonghkw/hekouwang-harness-check-skill`](https://github.com/huiyonghkw/hekouwang-harness-check-skill)。本 Skill 不重新发明测试框架，也不把“检查脚本运行过”包装成“任务已经完成”；它把现有 Harness 检查能力编排成 Agent 可以稳定执行、解释和交接的工作流。
...
当前 `hekouwang-content-agent` 将 Skill 快照放在 `.agents/skills/hekouwang-harness-check-skill/`，并通过 `.claude/skills/`、`.cursor/skills/`、`.codebuddy/skills/` 建立适配链接；最新源码和开源发布入口以独立仓库为准。适配链接只解决“宿主能发现 Skill”，不改变检查真源。
...
这个 Skill 不是 npm 或 PyPI 包，也不需要网络服务、API Key 或远程数据库。安装独立仓库时保留 Skill 目录结构，并为目标项目提供自己的 `.harness/` 实现；当前快照与独立仓库均按 MIT License 发布。
...
- [AGENTS.md](../../../AGENTS.md)：当前仓库运行时红线；
- [验证与治理流程](../../../验证与治理流程.md)：仓库级治理说明。
...
当前目录是 `hekouwang-content-agent` 的运行时快照；独立源码、发布版本和许可证以 [`hekouwang-harness-check-skill`](https://github.com/huiyonghkw/hekouwang-harness-check-skill) 为准。目标项目的 `.harness/` 脚本、契约、Memory、媒体和第三方依赖仍由目标项目自行负责。
