# lexiang-private-skills

**乐享知识库 MCP Skill（私有化部署版，v1.0.0）**

为 AI Agent 提供乐享知识库的全功能操作能力，包括搜索阅读、文档写入、Block 编辑、文件上传、外部导入等。

> **适用场景**：企业私有化部署的乐享实例，访问域名为企业自定义域名，与 SaaS 版本独立维护。

---

## 快速开始

访问你的乐享私有化 MCP 配置页面获取配置信息：

```
https://{你的乐享私有化域名}/ai/claw
```
...
| 导入腾讯会议 | `references/connectors.md` | 「导入会议录制」 |
...
- **链接生成**：使用 `whoami().company.company_domain` 作为私有化域名直接拼接，无需追加 `company_from` 参数
- **401 处理**：不重试，引导用户打开 `{私有化域名}/ai/claw` 续期（点续期按钮即可恢复，无需重新配置）
