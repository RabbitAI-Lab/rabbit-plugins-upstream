...
`{domain}` = `whoami()` 返回的 `company.company_domain`（即企业私有化域名，如 `https://lexiang.mycompany.com`）
...
私有化域名已指向对应企业，**直接使用 `{domain}` 拼接，无需附加 `company_from` 参数**：

```
链接 = {domain}/pages/{entry_id}
```

示例：`whoami()` 返回 `company_domain` 为 `https://lexiang.mycompany.com`，则：
- 条目链接：`https://lexiang.mycompany.com/pages/abc123`
- 知识库链接：`https://lexiang.mycompany.com/spaces/xyz789`
...
| 条件 | 示例 |
|------|------|
| 用户提供了明确 URL | `"写到这里：https://lexiang.mycompany.com/spaces/xxx"` |
