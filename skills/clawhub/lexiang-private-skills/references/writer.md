...
3. **如果存在个人知识库**：
   ...
   - 写入完成后拼接访问链接：`{domain}/pages/{entry_id}`
     - `{domain}` = `whoami()` 返回的 `company.company_domain`（私有化域名，无需追加 `company_from`）
...
- `{domain}` = `whoami()` 返回的 `company.company_domain`（企业私有化域名）
- 私有化域名已指向对应企业，**无需追加 `company_from` 参数**
