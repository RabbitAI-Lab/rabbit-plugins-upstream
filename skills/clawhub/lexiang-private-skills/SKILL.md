---
name: lexiang-knowledge-private
version: 1.0.0
description: "乐享知识库 MCP 全功能 Skill（私有化部署版）。当用户提到「乐享」「知识库」「个人知识库」「我的知识库」「lexiang」，或提供乐享链接，或涉及知识库的搜索/写入/编辑/文件/配置等操作时使用。"
---

# 乐享知识库 MCP Skill（私有化部署版）

当用户提到「**乐享**」「**知识库**」「**个人知识库**」「**我的知识库**」「**lexiang**」，或提供乐享链接，或给出 `space_id`、`entry_id`、`/spaces/`、`/pages/` 等乐享标识时，读取本 Skill。

> **私有化说明**：本 Skill 适用于私有化部署的乐享实例。用户访问域名为企业自定义私有域名（如 `https://lexiang.mycompany.com`），与 SaaS 版不同，**链接直接使用私有化域名，无需判断域名结构**。

---

## ⛔ MANDATORY RULES — 必须遵守

1. **遇到 401 / 连接断开**：立即停止重试，读取 `references/setup.md`；引导用户重新获取 token
...
4. **company_from**：私有化域名已指向对应企业，链接中**不需要拼接 `company_from` 参数**
...
**`{domain}` = `whoami()` 返回的 `company.company_domain`（即企业私有化域名）**

链接格式：`{domain}/pages/{entry_id}`

> **私有化域名已指向对应企业，无需拼接 `company_from` 参数。**
> 如 `whoami()` 返回 `company_domain` 为 `https://lexiang.mycompany.com`，则链接为：
> `https://lexiang.mycompany.com/pages/{entry_id}`
...
MCP Tool: whoami
 → 成功：返回用户信息，继续执行
 → 401：读取 references/setup.md，引导用户重新获取 token 或续期
...
| 文件 | 职责 |
|------|------|
| `references/setup.md` | Token 配置、续期、故障排查 |
