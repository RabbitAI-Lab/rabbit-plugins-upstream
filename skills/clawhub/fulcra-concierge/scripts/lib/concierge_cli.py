...
def local_now() -> "datetime":
    """Current time in the user's configured timezone (CONCIERGE_TZ). Day boundaries ('today')
    MUST be derived from this, not the server's UTC clock — otherwise 'today' is shifted by the
    UTC offset (e.g. 8pm→8pm ET) and the wrong day's calendar/data gets read."""
    tz = os.environ.get("CONCIERGE_TZ", "America/New_York")
    try:
