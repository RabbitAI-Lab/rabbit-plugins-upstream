---
name: global-company-employee-zh
description: 依托全球企业数据库查询企业员工清单与人员规模（Headcount），摸清企业内部组织架构，帮外贸销售、猎头从业者挖掘目标企业潜在对接人员。
...
使用跨境魔方开放平台API从全球企业库数据查询公司员工信息。

## 概述

本技能提供对跨境魔方全球企业库员工信息的查询。通过公司ID（pid）获取员工列表，包含人物ID和职位名称。
...
**重要**：始终使用直接脚本调用，如 `python scripts/company_employee_list.py`。**不要使用** shell 复合命令如 `cd scripts && python company_employee_list.py`

### 员工列表查询 (`company_employee_list.py`)
- **返回粒度**：每位员工为一条记录
- **适用场景**：查询指定公司的员工列表
- **示例**：
  - "查询US_12345公司的员工列表"
  - "获取下一页员工数据"
- **参数**：查看参数说明 [员工列表 API](references/company-employee-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/depth_company/company/employee/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"员工列表查询失败，服务端异常"}'
```
...
```bash
python scripts/company_employee_list.py --pid US_12345 --cursor '上一次响应返回的cursor字符串'
```
...
- 员工列表：查看 [references/company-employee-list-api.md](references/company-employee-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/company-employee-list-api.md](references/company-employee-list-api.md)
