---
name: ous-upload-v2
description: "将本地文件上传到 对象存储 并返回最终 CDN 链接。适用于已经拿到 token 响应中的 `ousToken`、`globalDomain`、`blockSize`。需要由 agent 自行分析本地文件、判断走单文件上传还是分片上传、在需要时拆分文件、调用 OUS 接口。获取 token 响应不在本 skill 的能力范围内。"
version: 1.1.0
---
...
这个 skill 是纯 `SKILL.md` 工作流，不依赖任何预置上传脚本。执行上传时，由 agent 自己读取文件内容、分析文件大小、计算 MD5、在必要时按 `blockSize` 拆分文件、依次调用 OUS V2 接口并轮询状态；只有在上传进入成功终态后，才返回最终的 `url` 和 `uploadKey`。
...
- 本地文件路径，支持绝对路径或仓库相对路径
- `ousToken`
- `globalDomain`
- `blockSize`

除非其他系统明确要求，否则忽略 `bucketType`。

`ousToken`、`globalDomain`、`blockSize`这三个字段需要单独调用 soa token 生成接口。不要在这个 skill 里调用 SOA token 生成接口。

如果调用方给的是完整 SOA 响应，而不是拆开的字段，可以直接从响应 JSON 中提取：
...
这个 skill 当前没有自带 `references/` 目录。只有在需要确认精确接口名、请求字段、状态码、`lackBlocks` 语义或上传流程细节时，再从当前仓库实现、测试客户端或用户额外提供的协议文档中取证；不要默认依赖不存在的本地引用文件。
