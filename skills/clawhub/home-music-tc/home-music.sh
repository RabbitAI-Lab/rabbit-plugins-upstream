...
PLAYLIST_FOCUS="spotify:playlist:37i9dQZF1DWZeKC4gAKpQo"   # Deep Focus
...
# Focus scene: Deep work mode
# - Sonos Move only at 25% volume
# - Focus playlist for concentration
scene_focus() {
    ...
    # Connect Sonos Move at 25%
    airfoil_connect "Sonos Move"
    sleep 0.5
    airfoil_volume "Sonos Move" 0.25
    
    # Start playlist
    "$SPOTIFY_CMD" play "$PLAYLIST_FOCUS"
    "$SPOTIFY_CMD" volume 100
    
    echo "✅ Focus: Sonos Move @ 25%, Deep Focus Playlist"
}

# Off scene: Stop everything
# - Optionally fade out volume over a few seconds
# - Pause Spotify
...
scene_off() {
    ...
    if [[ "$fade" == "--fade" ]]; then
        echo "🔇 Fading out music..."
        # Gradually reduce volume on all connected speakers over ~3 seconds
        local connected
        connected=$(airfoil_connected_speakers)
        if [[ -n "$connected" && "$connected" != "None" ]]; then
            for vol_level in 0.7 0.5 0.3 0.1 0.0; do
                for speaker in "${ALL_SPEAKERS[@]}"; do
                    airfoil_volume "$speaker" "$vol_level" 2>/dev/null || true
                done
                sleep 0.6
            done
        fi
    ...
    if [[ "$fade" == "--fade" ]]; then
        echo "✅ Music faded out, all speakers disconnected"
    else
...
Scenes:
  ...
  focus      Focus Playlist on Sonos Move (25% volume)
  off        Stop music, disconnect all speakers
  stop       Same as off; use --fade for gradual fade-out
