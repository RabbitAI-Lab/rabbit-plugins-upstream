---
name: amazon-bullet-point-writer
description: >
  结合 Amazon 商品详情和评论证据，诊断五点描述的卖点、疑问与表达缺口。仅用于五点诊断和建议；不用于标题、广告投放或自动发布。
  Requires an ARI API key (ari_live_*).
slug: bullet-writer
displayName: Amazon 五点描述优化
version: 1.4.7
summary: 把评论证据转成五点表达优化建议
license: MIT
---
...
- 本入口只执行 `listing/bullets`，输出模板固定为 `ops_listing`。
