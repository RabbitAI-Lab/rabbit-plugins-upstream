...
description: "专业UI/UX设计资源库,含风格库、配色方案、字体配对基础查询,适合个人设计师快速参考"
license: Proprietary
...
**输入**: 用户提供UI风格库所需的指令和必要参数。
**处理**: 按照skill规范执行UI风格库操作,遵循单一意图原则。
**输出**: 返回UI风格库的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供配色方案查询所需的指令和必要参数。
**处理**: 按照skill规范执行配色方案查询操作,遵循单一意图原则。
**输出**: 返回配色方案查询的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供字体配对推荐所需的指令和必要参数。
**处理**: 按照skill规范执行字体配对推荐操作,遵循单一意图原则。
**输出**: 返回字体配对推荐的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供设计决策框架所需的指令和必要参数。
**处理**: 按照skill规范执行设计决策框架操作,遵循单一意图原则。
**输出**: 返回设计决策框架的执行结果,包含操作状态和输出数据。
...
**输入**: 用户提供Tailwind CSS快速应用所需的指令和必要参数。
**处理**: 按照skill规范执行Tailwind CSS快速应用操作,遵循单一意图原则。
**输出**: 返回Tailwind CSS快速应用的执行结果,包含操作状态和输出数据。
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：设计资源库、含风格库、字体配对基础查询、适合个人设计师快、速参考、面向个人设计师和、开发者的、提供界面风格、字体配对的基础查、询和设计决策框架、核心能力、风格库浏览、种界面风格、精选组合、基础设计决策框架、快速应用指引等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持。
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
