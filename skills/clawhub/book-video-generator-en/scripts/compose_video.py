#!/usr/bin/env python3
"""
3-Minute Book Digest — English video composition script
Composes images + audio into a subtitled MP4 video, replacing the Jianying
assistant in the original Coze workflow. (English version)

v2.6.0 improvements:
- Progress bar: section titles at top, current section highlighted (orange #FF7F72), bottom progress line
- Keyword highlight: ASS subtitle format, keywords highlighted in orange
- Per-line single-line subtitles: precise sync via TTS word timestamps

v2.3.0 improvements:
- Per-line single-line subtitles: edge-tts WordBoundary word timestamps, split long subtitles into short single lines
- Precise speech sync: each subtitle's start/end time matches the speech
- Larger font (36pt), single line only, no longer occluding the frame

Dependencies:
- imageio-ffmpeg (auto-provides ffmpeg binary)
- Pillow (progress bar PNG generation)
...
{
  "output": "output.mp4",
  "cover": "cover.png",                          // optional, cover image (shown 5s before first shot)
  "cover_duration": 5.0,                         // optional, cover display time (seconds, default 5)
  "chapter_titles": ["Introduction", "Core Ideas", "Practical Tips", "Conclusion"],  // optional, progress bar titles
  "segment_chapters": [0, 0, 0, 1, 1, ...],      // optional, each shot's section index
  "keywords": ["Atomic Habits", "tiny changes"],  // optional, highlighted subtitle keywords
  "segments": [
    {"image": "1.png", "audio": "1.mp3", "caption": "Subtitle text"},
    ...
  ]
}
"""
...
def detect_font():
    """Auto-detect a system English font, returns the FontName usable by the ffmpeg subtitles filter."""
    import platform
    ...
    if system == "windows":
        ...
        candidates = [
            ...
            ("/usr/share/fonts/opentype/liberation/LiberationSans-Regular.ttf", "Liberation Sans"),
        ]
...
def detect_font_path():
    """Auto-detect a system English font file path for Pillow."""
    import platform
...
# ── Subtitle phrase breaking ─────────────────────────────────────

# Sentence-break punctuation: break the line when encountered (kept at line end)
_BREAK_PUNCT = set(".,!?;:…'\"。！？；：、")
# All punctuation (skipped during word-char matching)
_ALL_PUNCT = _BREAK_PUNCT | set("\"'()[]{} \n\t\r-—–…")


def build_phrases(caption, words, max_chars=42):
    """Build punctuated single-line phrases from the raw caption + word timestamps.

    Walks the raw caption character by character, matching each character to the
    word sequence to get precise timestamps. Breaking strategy:
    1. Break at sentence punctuation (., !, ?, etc.); punctuation stays at line end.
    2. Mark over_max once the line exceeds max_chars; break at the next break point.
    3. For English, also break at spaces once over_max (word-boundary wrapping).
    4. Force a break after 1.5x max_chars if no break point appears.

    Args:
        caption: raw subtitle text (with punctuation)
        words: [{"text": "word", "start": 0.0, "end": 0.5}, ...]
    ...
    for ch in caption:
        if ch in _ALL_PUNCT:
            ...
            if ch in _BREAK_PUNCT:
                ...
                # Break at a word boundary only once the line is too long
                if over_max and buf.strip():
                    phrases.append((buf.strip(), buf_start, buf_end))
                    buf = ""
                    over_max = False
                    if wi < len(words):
...
# ASS color (BGR format): #FF7F72 → B=72, G=7F, R=FF
HIGHLIGHT_COLOR = "&H727FFF&"  # orange #FF7F72
DEFAULT_COLOR = "&HFFFFFF&"    # white


def highlight_keywords(text, keywords):
    """Highlight keywords in the text using ASS color tags.

    Matches by descending keyword length to avoid a short keyword matching
    inside a longer one. ASCII keywords use word boundaries (\\b) so e.g.
    "read" does not match inside "bread".
    ...
    Returns:
        text with ASS color tags, e.g. {\\c&H727FFF&}Atomic Habits{\\c&HFFFFFF&}
    """
    if not keywords:
        return text

    # Keep only keywords that actually appear in the text, longest first
    active_kw = sorted(set(kw for kw in keywords if kw and kw.lower() in text.lower()),
                       key=len, reverse=True)
    if not active_kw:
        return text

    # Build pattern: word boundaries for ASCII keywords, literal for the rest
    pattern_parts = []
    for kw in active_kw:
        if kw.isascii():
            pattern_parts.append(rf"\b{re.escape(kw)}\b")
        else:
...
def make_ass(segments, durations, output_path, gap=0.0, max_chars=42,
             keywords=None, fade_in_ms=200, fade_out_ms=150):
    """Generate an ASS subtitle file with keyword highlight + entrance/exit animation.

    Prefers word-level timestamps (.words.json) for precise speech sync, splitting
    long subtitles into single short lines of at most max_chars characters. Keywords
    are highlighted in orange. Each subtitle has a fade-in + slide-up entrance and
    a fade-out exit animation.

    Args:
        segments: list of shots
        durations: list of per-shot durations
        output_path: ASS file output path
        gap: gap between shots (seconds)
        max_chars: max characters per line
        keywords: highlight keyword list (optional)
        fade_in_ms: entrance fade-in duration (ms)
        fade_out_ms: exit fade-out duration (ms)
...
def make_srt(segments, durations, output_path, gap=0.0, max_chars=42):
...
def compose_video(
    ...
    Args:
        segments: list of shots [{"image":..., "audio":..., "caption":...}, ...]
        output_path / output: output MP4 path
        cover: cover image path (optional, shown N seconds before the first shot, narration continues)
        chapter_titles: section title list (optional, for the progress bar)
        segment_chapters: per-shot section index list (optional, evenly distributed if omitted)
        keywords: subtitle highlight keyword list (optional)
        bgm: background music path (optional, auto-finds assets/bgm_reading.mp3)
        bgm_volume: BGM 音量 (0.0-1.0)
