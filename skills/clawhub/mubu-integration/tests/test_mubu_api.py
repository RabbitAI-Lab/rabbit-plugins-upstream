...
class TestApiMethodPayloads:
    ...
    def test_save_doc_auto_fetches_version_and_definition(self, isolated_client):
        ...
        assert root["id"] == "D9"
        # v1.3.14（hypothesis 2 加固）：build_update_event 对节点做归一化，
        # 断言归一化后的关键字段而非精确形状（时间戳为动态值，仅校验类型）。
        child = root["children"][0]
        assert child["id"] == "n1"
        assert child["text"] == "A"
        assert child["note"] == ""
        assert child["collapsed"] is False
        assert child["finish"] is False
        assert child["priority"] == 0
        assert child["color"] == 0
        assert isinstance(child["createTime"], int)
        assert isinstance(child["modifyTime"], int)
        assert isinstance(child["timestamp"], int)
        # 幂等：updated == original
    ...
    def test_build_update_event_shape(self, isolated_client):
        ...
        # v1.3.14（hypothesis 2 加固）：节点经归一化补全契约字段，断言关键字段而非
        # 精确形状（时间戳为动态值，仅校验类型）。
        child = pair["updated"]["children"][0]
        assert child["id"] == "n1"
        assert child["text"] == "A"
    ...
    def test_save_doc_raises_without_member_id(self, isolated_client):
        """issue #8 修复（P0）：member_id 缺失时 save_doc 必须明确抛 MubuError，
        ...
        with pytest.raises(MubuError) as ei:
            isolated_client.save_doc(
                "D9",
                events=[isolated_client.build_update_event(
                    {"nodes": [{"id": "n1", "text": "A"}]}, "D9")],
                version=3,
            )
