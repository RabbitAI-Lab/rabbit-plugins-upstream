#!/usr/bin/env bash
# cron-to-transcript — send a Telegram (or other channel) message via OpenClaw
# AND append the delivered text into the target session transcript, so the agent
# that "owns" that chat/topic remembers the cron/script delivery.
#
...
# This helper appends OpenClaw's transcript-only delivery row (the same shape the
...
#
# Exit codes:
#   0  delivered (+ appended, or append skipped as best-effort with a warning)
...
usage() { sed -n '2,40p' "$0"; }
die_usage() { echo "cron-to-transcript: $1" >&2; exit 2; }
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --dry-run)        DRY=1; shift;;
    --no-send)        NO_SEND=1; shift;;        # append only (testing)
    --no-transcript)  NO_TRANSCRIPT=1; shift;;  # send only (behaves like plain send)
    --no-mirror)      NO_TRANSCRIPT=1; shift;;  # compatibility alias
    -h|--help)        usage; exit 0;;
    *) echo "cron-to-transcript: unknown arg: $1" >&2; usage >&2; exit 2;;
  esac
done
...
STATE_DIR="$OPENCLAW_HOME/cron-to-transcript/state"
LOG="$OPENCLAW_HOME/cron-to-transcript/cron-to-transcript.log"
...
if [ -n "$IDEM" ]; then
  ...
  if grep -qxF "$IDEM" "$SEEN" 2>/dev/null; then
    ...
    echo "cron-to-transcript: idem '$IDEM' already handled, nothing to do" >&2
    exit 3
  fi
fi
...
if [ "$DRY" = "1" ]; then
  ...
  echo "no-send : $NO_SEND   no-transcript: $NO_TRANSCRIPT"
  echo "--- message ---"
  printf '%s\n' "$MESSAGE"
  exit 0
fi
...
if [ "$NO_SEND" != "1" ]; then
  ...
  if ! SEND_JSON="$("$OPENCLAW_BIN" "${send_args[@]}" 2>&1)"; then
    ...
    echo "cron-to-transcript: send failed: $SEND_JSON" >&2
    exit 4
  fi
  log "sent source=$SOURCE to=$TO thread=${THREAD:-}"
else
  log "no-send source=$SOURCE (append only)"
fi

# ---- 2) append into the session transcript (best-effort) --------------------
if [ "$NO_TRANSCRIPT" != "1" ]; then
...
def warn(m): print(f"cron-to-transcript: WARN append: {m}", file=sys.stderr)

try:
    with open(sj_path) as f:
        sessions = json.load(f)
except Exception as e:
    warn(f"cannot read sessions.json ({sj_path}): {e}; delivery ok, append skipped")
    sys.exit(0)
...
if entry is None:
    warn(f"no session entry matched to={to} thread={thread or '-'}; delivery ok, append skipped")
    sys.exit(0)

session_file = entry.get("sessionFile")
if not session_file:
    warn("matched session has no sessionFile; delivery ok, append skipped")
    sys.exit(0)
...
lock_path = session_file + ".transcript.lock"
try:
    lf = open(lock_path, "w")
    fcntl.flock(lf, fcntl.LOCK_EX)
except Exception as e:
    warn(f"could not acquire lock {lock_path}: {e}; delivery ok, append skipped")
    sys.exit(0)

try:
    # parentId + trailing-newline check must happen UNDER the lock, so two
    # concurrent appends can't read the same tail / clobber each other.
    parent_id = None
    needs_newline = False
    try:
    ...
    with open(session_file, "a", encoding="utf-8") as f:
        f.write(line)
    print(f"cron-to-transcript: appended into {session_file}", file=sys.stderr)
except Exception as e:
