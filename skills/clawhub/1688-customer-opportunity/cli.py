#!/usr/bin/env python
"""
1688-customer-opportunity —— 1688客户机会挖掘工具集 CLI 统一入口

Usage:
    python cli.py <command> [options]

Commands（更多参数见项目根目录 SKILL.md）:
    list_customer_cluster          获取客户客群列表
    list_customer_details          获取客户详情列表
    list_cluster_buyer_detail      获取客群买家明细
    customer_reception_advice      实时客户接待画像 + 跟进建议
    customer_crowd_analysis        客群分析
    activate_cluster_plan          开启运营计划
    get_cluster_marketing_plan     获取运营计划
    configure                      配置 AK
...
def _usage(commands: dict):
    lines = ["**1688-customer-opportunity 用法**\n", "```"]
    for name in sorted(commands):
        try:
            mod = importlib.import_module(commands[name])
            desc = getattr(mod, 'COMMAND_DESC', '')
            lines.append(f"python cli.py {name:<32} {desc}")
        except Exception:
            lines.append(f"python cli.py {name}")
    lines.append("```")
