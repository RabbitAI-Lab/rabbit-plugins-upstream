...
`GET https://{domain}/oapi/v1/appstack/organizations/{organizationId}/globalVars/{name}`
...
| revisionSha | string | query | 否 | 全局变量组版本。 | my-revision-sha |
| organizationId | string | path | 是 | 组织 ID。 | organization-id-xxx |

## 请求示例

`curl -X 'GET' \ 'https://test.rdc.aliyuncs.com/oapi/v1/appstack/organizations/organization-id-xxx/globalVars/my-global-var-group?revisionSha=my-revision-sha' \ -H 'Content-Type: application/json' \ -H 'x-yunxiao-token: pt-0fh3****0fbG_35af****0484'`
