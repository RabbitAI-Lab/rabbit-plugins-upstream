...
| repositoryIds | array | query | 否 | 代码库 ID 列表。 | \[2813489 2813490\] |
| authorUserIds | array | query | 否 | 创建人用户 ID 列表。 | \[62c795xxxb468af8\] |
| assigneeUserIds | array | query | 否 | 评审人用户 ID 列表。 | \[62c795xxxb468af8\] |
| subscriberUserIds | array\[string\] | query | 否 | 订阅人用户 ID 列表。 | \[62c795xxxb468af8\] |
| search | string | query | 否 | 合并请求标题搜索关键字。 | mr title |
| orderBy | string | query | 否 | 排序字段，均为降序排列：id-按 id 降序，updated\_at - 按更新时间降序。 | updated\_at |
| createdAfter | string | query | 否 | 创建时间截止范围，格式为：yyyy-MM-dd。 | 2024-09-09 |
| createdBefore | string | query | 否 | 创建时间起始范围，格式为：yyyy-MM-dd。 | 2024-09-09 |
| targetBranch | string | query | 否 | 按目标分支查询。 | master |

## **请求示例**

`curl -X 'GET' \ 'https://{domain}/oapi/v1/codeup/organizations/60d54f3daccf2bbd6659f3ad/mergeRequests?page=1&perPage=20&repositoryIds=[2813489 2813490]&authorUserIds=[62c795xxxb468af8]&assigneeUserIds=[62c795xxxb468af8]&subscriberUserIds=[62c795xxxb468af8]&state=opened&search=mr title&orderBy=updated_at&createdAfter=2024-09-09&createdBefore=2024-09-09&targetBranch=master' \ -H 'Content-Type: application/json' \ -H 'x-yunxiao-token: pt-0fh3****0fbG_35af****0484'`
