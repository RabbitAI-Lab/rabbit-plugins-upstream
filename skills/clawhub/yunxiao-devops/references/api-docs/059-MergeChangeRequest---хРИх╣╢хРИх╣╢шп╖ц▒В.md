...
| mergeType | string | body | 是 | 合并类型：ff-only - fast-forward-only 合并方式；no-fast-forward - 普通合并方式；squash - 压缩合并方式；rebase - rebase 合并方式。 | ff-only |
...
`curl -X 'POST' \ 'https://{domain}/oapi/v1/codeup/organizations/60d54f3daccf2bbd6659f3ad/repositories/2813489或者60de7a6852743a5162b5f957%2FDemoRepo/changeRequests/1/merge' \ -H 'Content-Type: application/json' \ -H 'x-yunxiao-token: pt-0fh3****0fbG_35af****0484' \ --data ' { "mergeMessage": "merge message", "mergeType": "ff-only", "removeSourceBranch": true }'`

### **Region版**

```
curl -X 'POST' \
  'https://{domain}/oapi/v1/codeup/repositories/2813489或者60de7a6852743a5162b5f957%2FDemoRepo/changeRequests/1/merge' \
