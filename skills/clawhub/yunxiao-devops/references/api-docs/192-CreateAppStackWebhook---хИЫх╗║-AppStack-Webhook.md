# CreateAppStackWebhook - 创建 AppStack Webhook

创建新的 AppStack Webhook 配置。
...
`POST https://{domain}/oapi/v1/appstack/organizations/{organizationId}/hook/api`
...
| \- | object | body | 否 | 创建 AppStack Webhook 请求。 |  |
| description | string | body | 是 | Webhook 描述，长度不超过256字符。 | 测试 webhook |
...
| token | string | body | 否 | Webhook 令牌，长度不超过128字符。 | aaaa |
| type | string | body | 是 | 类型。 | WEBHOOK |
| url | string | body | 是 | Webhook URL。 | https://devops.aliyun.com/appstack/setting/webhooks1 |
| type | string | body | 是 | 类型。 | WEBHOOK |

## **请求示例**

`curl -X 'POST' \ 'https://{domain}/oapi/v1/appstack/organizations/ec766e6xxxxxxxxxx34d6exe671/hook/api' \ -H 'Content-Type: application/json' \ -H 'x-yunxiao-token: pt-0fh3****0fbG_35af****0484' \ --data ' { "description": "测试webhook", "events": [ { "action": "Create", "event": "App" } ], "scopeId": "企业ID,应用名称", "scopeType": "ORGANIZATION", "spec": { "token": "aaaa", "type": "WEBHOOK", "url": "https://devops.aliyun.com/appstack/setting/webhooks1" }, "type": "WEBHOOK" }'`
