...
`PUT https://{domain}/oapi/v1/appstack/organizations/{organizationId}/globalVars/{name}`
...
| --- | --- | --- | --- | --- | --- |
| name | string | path | 是 | 全局变量组名称。 | my-global-var-group |
| organizationId | string | path | 是 | 组织 ID。 | organization-id-xxx |
| \- | object | body | 否 | 更新全局变量组请求。 |  |
| content | array | body | 是 | 变量列表。 |  |
| \- | object | body | 否 | 变量模型。 |  |
| description | string | body | 否 | 变量描述。 | 命名空间 |
| key | string | body | 否 | 变量键。 | namespace |
| value | string | body | 否 | 变量值。 | default |
| fromRevisionSha | string | body | 是 | 更新源版本信息。 | from-revision-sha-xxx |
| message | string | body | 否 | 全局变量组信息。 | message-xxx |

## 请求示例

`curl -X 'PUT' \ 'https://test.rdc.aliyuncs.com/oapi/v1/appstack/organizations/organization-id-xxx/globalVars/my-global-var-group' \ -H 'Content-Type: application/json' \ -H 'x-yunxiao-token: pt-0fh3****0fbG_35af****0484' \ --data ' { "content": [ { "description": "命名空间", "key": "namespace", "value": "default" } ], "fromRevisionSha": "from-revision-sha-xxx", "message": "message-xxx" }'`
...
| commitTime | number | 提交时间。 | 1722250824321 |
