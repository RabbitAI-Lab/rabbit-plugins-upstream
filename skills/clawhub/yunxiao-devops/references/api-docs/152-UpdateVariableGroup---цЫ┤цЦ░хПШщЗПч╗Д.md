...
| variableGroupName | string | path | 是 | 变量组名。 | dev |
| organizationId | string | path | 是 | 组织 ID。 | ec766e63aee3437d9a51f334d6exxxxx |
| \- | object | body | 否 | 变量组模型。 |  |
| branchName | string | body | 否 | 版本分支，默认 master。 | master |
| displayName | string | body | 否 | 变量组展示名。 | dev |
| fromRevisionSha | string | body | 是 | 变量组版本号。 | 1.0.0 |
| message | string | body | 否 | 变量组描述信息。 | message |
| name | string | body | 是 | 变量组唯一名。 | dev |
| vars | array | body | 否 | 变量列表。 |  |
| \- | object | body | 否 | 变量模型。 |  |
...
`curl -X 'PUT' \ 'https://test.rdc.aliyuncs.com/oapi/v1/appstack/organizations/ec766e63aee3437d9a51f334d6exxxxx/apps/my-web-service/variableGroup/dev' \ -H 'Content-Type: application/json' \ -H 'x-yunxiao-token: pt-0fh3****0fbG_35af****0484' \ --data ' { "branchName": "master", "displayName": "dev", "fromRevisionSha": "1.0.0", "message": "message", "name": "dev", "vars": [ { "description": "命名空间", "key": "namespace", "value": "default" } ] }'`
