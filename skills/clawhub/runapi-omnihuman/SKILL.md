...
- For a one-off artifact or result, use the registered `omnihuman` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi omnihuman <operation> --help
curl --fail --location https://runapi.ai/docs/api/omnihuman/<operation>.md --output contract.md
```
...
```shell
runapi omnihuman <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service omnihuman --action <operation> > result.json
```
...
## Variants
- `omnihuman-1.5`: https://runapi.ai/models/omnihuman/1.5.md
- `omnihuman-1.5-human-identification`: https://runapi.ai/models/omnihuman/1.5-human-identification.md
- `omnihuman-1.5-subject-detection`: https://runapi.ai/models/omnihuman/1.5-subject-detection.md
