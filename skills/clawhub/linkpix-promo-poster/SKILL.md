...
促销海报走 `qhkit image` 自定义生图：活动主题、促销文案、商品一起写进提示词，有商品图就带上参考图。

## 何时触发

- 「做一张双11/黑五/圣诞促销海报」「新品发布图」
- 「给这个商品配一张折扣营销图」

## 使用配方

```bash
# 有商品图（推荐）：官方模板（与 LinkPix 工作台「折扣营销」功能同款）
qhkit image generate '{"modelLabel":"智慧模型","uploadedImages":["./商品图.jpg"],"prompt":"将产品变为营销活动图，活动内容如下：【双11狂欢 全场5折起，满300减50】，红金节日配色，促销氛围强烈"}'
# 无商品图：纯文字直出
qhkit image generate '{"modelLabel":"智慧模型","prompt":"圣诞节美妆礼盒促销海报，标题「圣诞焕新礼」，雪花与礼盒元素，红绿金配色，高级电商海报风格"}'
```

官方模板：`将产品变为营销活动图，活动内容如下：【活动文案】`——【】里写活动主题与主/副标题，可追加配色氛围与版式要求；海报常用竖版，`sizePreset` 先 options 查后逐字选用。
- 出图后核对海报上的文案与数字（生成式文字可能出错），错了重提交。
- 注意：旧版 `qhkit poster` 命令已于 2026-08 随线上活动海报入口下线，海报一律走本配方，不要尝试调用 `poster`。
...
- 长图促销页走「电商商品详情图生成器 | LinkPix」；成套营销图走「AI生成电商主图轮播图、主图套图 | LinkPix」。
