...
- `<media:content>`: NS3 royalty-free cover image(s), delivered as 1–2 repeated `<media:content medium="image">` elements per article. AI-matched from an NS3 image library based on the article's key subjects — each subject is backed by a group of images selected at random, so the same subject may appear with different images across articles. Images are square or landscape (wide) rectangles, vary in size, and are not standardized. The first element (`media:content[0]`) is the primary representative image. When no match is found, a single element returns the NS3 default logo (`https://ns3.ai/default.png`), which partners may replace with their own image — the field is never empty. **Recommended display: 1.91:1 (landscape)** for desktop and link-preview surfaces — when 2 images are provided, render them side by side, X (Twitter) style; when 1 is provided, it fills the frame. **1:1 (square)** for mobile and compact surfaces — use only the primary image (`media:content[0]`) even when 2 are provided.
