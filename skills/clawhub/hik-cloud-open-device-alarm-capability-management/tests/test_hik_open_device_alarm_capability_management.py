...
MODULE_PATH = (
    ...
    / "scripts"
    / "hik_open_device_alarm_capability_management.py"
)
SPEC = importlib.util.spec_from_file_location(
    "hik_open_device_alarm_capability_management", MODULE_PATH
)
...
class HikOpenDeviceAlarmCapabilityManagementTests(unittest.TestCase):
    ...
    def test_build_list_spec_uses_query_contract(self):
        args = MOD.parse_args(
            ["list", "--device-serial", "123456789"]
        )
        spec = MOD.build_list_spec(args)
        self.assertEqual(spec.method, "GET")
        self.assertEqual(spec.query, {"deviceSerial": "123456789"})

    def test_build_update_status_spec_uses_json_contract(self):
        args = MOD.parse_args(
            [
                "update-status",
                "--channel-id",
                "4da6ac157d61421999b82d4aa6e1e64e",
                "--ability-code",
                "10600",
                "--status",
                "1",
        ...
        spec = MOD.build_update_status_spec(args)
        self.assertEqual(spec.method, "POST")
        self.assertEqual(
            spec.json_body,
            {
                "channelId": "4da6ac157d61421999b82d4aa6e1e64e",
                "abilityCode": "10600",
                "status": 1,
            },
        )

    def test_build_intelligence_switch_spec_includes_optional_fields(self):
        args = MOD.parse_args(
            [
                "intelligence-switch",
                "--device-serial",
        ...
        spec = MOD.build_intelligence_switch_spec(args)
        self.assertEqual(spec.method, "POST")
        self.assertEqual(
            spec.json_body,
            {
