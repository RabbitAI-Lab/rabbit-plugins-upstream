---
name: amazon-quality-issue-detector
description: >
  从 Amazon 低星评论和商品字段中归纳产品质量问题线索、证据和优先级。仅用于质量问题线索；不用于替代质检、认证、召回或供应商执行。
  Requires an ARI API key (ari_live_*).
slug: quality-issue
displayName: Amazon 产品质量问题线索
version: 1.4.7
summary: 从差评与低星体验定位质量改进线索
license: MIT
---
...
- 本入口只执行 `product/quality`，输出模板固定为 `ops_product`。
