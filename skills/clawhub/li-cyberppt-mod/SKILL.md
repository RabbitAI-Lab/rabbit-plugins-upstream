---
name: li-cyberppt-mod
description: Multi-platform AI Agent Skill for generating evidence-based, editable, consulting-style PowerPoint presentations. Transforms documents (DOCX/PDF/TXT/XLSX) into high-density PPTX with strict quality gates, evidence chains, and visual QA. Supports OpenCode, OpenAI Codex, Hermes, OpenClaw, and Anthropic/Claude platforms.
version: 1.0.0
...
CyberPPT 支持多种 AI Agent 平台，实现跨平台的一致性体验。每个平台都有专门的配置文件，位于 `agents/` 目录下。

### 支持的平台

| 平台 | 配置文件 | 状态 | 说明 |
|---|---|---|---|
| OpenCode | `agents/opencode.yaml` | ✅ 已验证 | 完全支持，推荐使用 |
| OpenAI Codex | `agents/openai.yaml` | ✅ 已验证 | 原生支持，完全兼容 |
| Hermes | `agents/hermes.yaml` | ✅ 已支持 | 完全兼容 |
| OpenClaw | `agents/openclaw.yaml` | ✅ 已支持 | 完全兼容 |
| Anthropic/Claude | `agents/anthropic.yaml` | ✅ 已支持 | 完全兼容 |
| 通用平台 | `agents/generic.yaml` | ✅ 已支持 | 适用于自定义平台 |
...
- **统一的配置格式**: 所有平台使用相同的 YAML 配置结构
- **一致的工作流**: 三阶段流程（分析→蓝图→还原）在所有平台一致
- **统一的质量门禁**: 16个质量门禁在所有平台生效
- **平台自适应**: 自动检测运行平台并加载对应配置
...
如需为新的 Agent 平台添加支持：
1. 复制 `agents/generic.yaml` 为 `agents/[平台名].yaml`
2. 修改 `compatibility.platforms` 字段
3. 根据平台特性调整 `configuration` 部分
4. 更新本文档的支持平台列表
