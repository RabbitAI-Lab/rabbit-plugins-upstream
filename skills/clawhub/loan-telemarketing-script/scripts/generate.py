...
API_BASE = os.environ.get("TELE_SCRIPT_API_URL", "http://47.109.39.255:8765/api/v4")
USER_KEY = os.environ.get("TELE_SCRIPT_USER_KEY", "")
...
def cmd_register(args):
    ...
    print()
    print(f"  ⚠️  请设置环境变量：")
    print(f"  export TELE_SCRIPT_USER_KEY={result.get('user_key')}")
    print("=" * 50)
    print()
...
def cmd_info(args):
    if not USER_KEY:
        print("❌ 请先设置 TELE_SCRIPT_USER_KEY 环境变量", file=sys.stderr)
        sys.exit(1)
...
def cmd_generate(args):
    if not USER_KEY:
        ...
        sys.exit(1)
    print(f"🚀 电销话术生成中...", file=sys.stderr)
    data = {}
    data['product_name'] = args.product
    if args.product_type: data['product_type'] = args.product_type
    if args.target: data['target_customer'] = args.target
    if args.pain: data['customer_pain_point'] = args.pain
    if args.tone: data['tone'] = args.tone
    if args.length: data['script_length'] = args.length
    result = api_call("tele-script", data)
...
def main():
    parser = argparse.ArgumentParser(description="电销话术生成 - SaaS版，2点/次")
    ...
    g = sub.add_parser("generate", help="电销话术生成")
    g.add_argument("--product", required=True, help="产品名称")
    g.add_argument("--product-type", default="消费贷", help="产品类型")
    g.add_argument("--target", default="general", help="目标客户")
    g.add_argument("--pain", default="", help="客户痛点")
    g.add_argument("--tone", default="professional", help="语气风格: professional/friendly/urgent")
    g.add_argument("--length", default="medium", help="长度: short/medium/long")
