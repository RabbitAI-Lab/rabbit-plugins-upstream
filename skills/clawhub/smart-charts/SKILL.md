...
license: MIT
compatibility: "Python 3.9+; requires pandas==3.0.1, numpy==2.4.3, openpyxl==3.1.5, xlrd==2.0.1; no network access needed (ECharts JS bundled offline); install with: pip install -r requirements.txt --require-hashes"
metadata:
  author: smart-charts
  version: 5.1.0
  cost:
...
**y_axis cardinality key:** `1` = only first column used (extras silently ignored); `1~N` = each column becomes a series; `N` = multiple columns expected; `2` = exactly 2 numeric columns required; `special` = auto-detects source/target/value columns.
