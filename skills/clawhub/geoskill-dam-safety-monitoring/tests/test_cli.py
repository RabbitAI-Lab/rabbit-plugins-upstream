"""CLI argument parsing tests for dam-safety-monitoring."""
import subprocess
import sys
import os
...
class TestCLIBasics:
    ...
    def test_synthetic_runs_exit_0(self, tmp_path):
        ...
        assert os.path.exists(os.path.join(out, "composite_risk.tif"))
        assert os.path.exists(os.path.join(out, "anomaly_zones.geojson"))
