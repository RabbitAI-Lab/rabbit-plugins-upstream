...
summary: 在后台运行定时器与闹钟，完成后通过系统通知提醒用户，支持多种时间格式与多任务并行管理。在后台设置定时器和闹钟。当后台定时器完成时，会收到系统通知，必须将提醒内容传达给用户.
  支持秒、分、时
summary_zh: 在后台运行定时器与闹钟，完成后通过系统通知提醒用户，支持多种时间格式与多任务并行管理。在后台设置定时器和闹钟。当后台定时器完成时，会收到系统通知，必须将提醒内容传达给用户.
  支持秒、分、时
license: MIT
description: |-。在后台运行定时器与闹钟，完成后通过系统通知提醒用户，支持多种时间格式与多任务并行管理。在后台设置定时器和闹钟。当后台定时器完成时，会收到系统通知，必须将提醒内容传达给用户。Use when 需要项目管理、任务规划、进度跟踪、团队协作时使用。不适用于实际人员绩效评估。适用于独立开发者、企业团队和自动化工作流场景。
  支持秒、分、时。支持自动化配置和灵活的参数设置，适适配多种工作环境，增强工作效率。。在后台运行定时器与闹钟，完成后通过系统通知提醒用户，支持多种时间格式与多任务并行管理。在后台设置定时器和闹钟。当后台定时器完成时，会收到系统通知，必须将提醒内容传达给用户.
  支持秒、分、时'
tags:
- 通用办公
- Productivity
...
领先个参数为时长，第二个可选参数为提醒文案。提醒文案会在定时器完成时回传给 Agent，再由 Agent 转达给用户。若不提供文案，将使用默认提示.
**执行步骤**:
...
## 异常管理
### 启动命令未识别 time 参数
当领先个参数为空或无法解析为已知时间格式时，timer.js 会立即退出并输出错误。处理：确认参数使用了支持的格式（`Ns`/`Nm`/`Nh`/`N`/`MM:SS`/`HH:MM:SS`），不要传入纯字符串如 "ten minutes" 或空值.
### 提醒文案含特殊字符导致命令解析失败
...
### 如何取消一个正在运行的定时器？
先执行 `process action:list` 找到目标定时器的 sessionId，再执行 `process action:kill sessionId:未指定` 终止。终止后会收到退出码 130 的完成通知，Agent 应识别为取消而不向用户提示到期.
### Q1: 如何设置一个定时器，使其在特定日期和时间提醒我？
A: 可以通过输入特定的时间格式来设置，例如使用 `HH:MM:SS` 格式指定具体的小时、分钟和秒。例如，若要设置在下午3点整提醒，可以输入 `15:00:00`。

### Q3: 如果我在设置定时器时输入了错误的时间格式，会发生什么？
A: 如果输入的时间格式不正确，定时器将不会启动，并且会显示错误信息。请确保按照支持的时间格式（如 `Ns`、`Nm`、`Nh`、`MM:SS`、`HH:MM:SS`）输入正确的时间。

### Q4: 我可以同时运行多个定时器吗？
A: 是的，可以同时运行多个定时器。每个定时器会以独立的 sessionId 运行，互不干扰。你可以通过多次调用定时器命令来设置多个定时器。

### Q5: 如果我的设备突然断电，已经设置的定时器还会工作吗？
A: 如果设备断电，后台运行的定时器将不会工作，因为它们依赖于设备的电源。当设备重新启动并恢复到正常状态时，已经设置的定时器需要重新启动。
...
| --- | --- | --- | --- | --- |
| 设置定时器 | 2分钟 | 10秒 | 1分50秒 | 100% |
| 管理多个定时器 | 10分钟 | 2分钟 | 8分钟 | 80% |
| 查看定时器状态 | 1分钟 | 5秒 | 54秒 | 500% |
| 终止定时器 | 1分钟 | 5秒 | 54秒 | 500% |
| 记录定时器日志 | 5分钟 | 1分钟 | 4分钟 | 200% |
...
| 易用性 | 高 | 低 | 中 | 高 |
| 多任务并行 | 支持 | 不支持 | 支持 | 支持 |
| 自定义提醒文案 | 支持 | 不支持 | 支持 | 支持 |
| 进程生命周期管理 | 支持 | 不支持 | 支持 | 支持 |
| 成本 | 低 | 高 | 中 | 高 |
| 系统兼容性 | 高 | 低 | 中 | 高 |
...
| --- | --- | --- | --- | --- |
| 忘记设置提醒 | 容易忘记重要事项的时间节点 | 影响工作效率和日常生活 | 自动化定时器提醒 | 提高工作效率 80% |
| 手动管理多个定时器 | 管理复杂，耗时费力 | 影响工作效率 | 多定时器并行管理 | 提高工作效率 70% |
| 定时器准确性 | 手动设置容易出错 | 影响任务完成 | 高准确率计时 | 准确率提升 100% |

## 排障手册
| 错误现象 | 可能原因 | 诊断步骤 | 解决方案 |
| --- | --- | --- | --- |
| 定时器未按时触发 | 时间设置错误 | 检查时间设置是否正确 | 修正时间设置 |
| 系统通知未收到 | 通知设置未开启 | 检查系统通知设置 | 开启系统通知 |
| 定时器无法终止 | 权限不足 | 检查运行权限 | 获取相应权限 |
| 日志记录异常 | 日志配置错误 | 检查日志配置 | 修正日志配置 |
| 定时器运行缓慢 | 资源占用过高 | 检查系统资源使用情况 | 优化系统资源 |

## 安全原则
1. 定时器内容应避免包含敏感信息，如密码、财务数据等。
2. 定时器操作应限制在可信环境中进行，防止信息泄露。
3. 定时器不应设置过于频繁的提醒，避免造成系统负担。
4. 定时器不应设置过长的运行时间，防止系统资源耗尽。
5. 定时器操作应避免在公共网络环境下进行，防止数据被窃取。
...
## 主要特点
- **自动化执行**: 在后台运行定时器与闹钟，完成后通过系统通知提醒用户，支持多种时间格式与多任务并行管理。在后台设置定时器和闹钟。当后台定时
- **文件处理**: 支持多种文件格式的读取、解析和写入操作
- **API集成**: 通过标准化接口调用外部服务并处理响应
- **命令执行**: 在安全沙箱中执行系统命令并收集结果

## 异常修复
针对后台定时器与提醒使用中可能遇到的常见问题,提供以下排查方案:
