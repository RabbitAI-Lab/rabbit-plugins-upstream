...
description: 客户管理中心技能。提供客户档案管理、客户画像分析、风险提醒、客户时间线查询。基于示例产业园A真实业务数据（Excel台账）。触发场景：(1) 查询客户信息（@企服助手 查询客户 T1-601），(2) 查看客户画像（@企服助手 客户画像 示例企业），(3) 查询风险客户（@企服助手 查询风险客户），(4) 定时任务生成风险提醒日报。
---
...
本技能为企服助手的**核心模块**，统一管理示例产业园A的客户信息，提供：
...
### 数据文件
**文件路径**: `/Users/mac/示例产业园AC+服务.xlsx`
...
| 字段序号 | 字段名 | 用途 | 示例值 |
|---------|--------|------|--------|
| 5 | 合同号 | 关联费用收缴表 | {TODO: 填写合同编号} |
| 6 | 单元号 | 主键，关联所有表 | T1-601 |
| 7 | 租户名 | 关联C+服务记录 | 示例企业名称 |
