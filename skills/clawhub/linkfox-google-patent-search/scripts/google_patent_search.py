#!/usr/bin/env python3
"""
Google Patents Search - LinkFox Skill
调用 /googlePatent/search 接口。

Usage:
  python google_patent_search.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python google_patent_search.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/googlePatent/search"
SLUG = "linkfox-google-patent-search"
...
def main():
    ...
    if not argv:
        print(
            "Usage: google_patent_search.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
