...
```bash
curl -X POST ${LINKFOX_TOOL_GATEWAY}/googlePatent/search \
...
```json
{
  "skillName": "linkfox-google-patent-search",
  "sentiment": "POSITIVE",
  "category": "OTHER",
...
**Field rules:**
- `skillName`: Use this skill's `name` from the YAML frontmatter (`linkfox-google-patent-search`)
