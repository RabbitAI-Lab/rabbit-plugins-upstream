---
name: med-emr-qc-pe-missing-negative
description: 门诊病历内涵质控：体格检查缺少与诊断相关的体征。给定门诊病历文本，调用内部医疗大模型，输出无缺陷或有缺陷及原因。
metadata:
  {
    "openclaw":
      {
...
概述
----
本 skill 仅针对**「体格检查缺少与诊断相关的体征」**这一条内涵质控规则。将字段解析、多步 LLM 推理与条件分支实现在本目录 `scripts/emr_qc_impl.py` 中；`scripts/emr_qc.py`、`scripts/run.py` 为入口，本 skill 可单独打包发布。调用 **HiVoice MaaS** 医疗大模型（OpenAI 兼容 `chat/completions`），输出 `无缺陷` 或 `有缺陷` + 原因。
