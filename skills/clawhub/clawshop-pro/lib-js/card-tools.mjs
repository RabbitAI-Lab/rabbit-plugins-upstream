...
export function pollStarted(channel = null, sessionKey = null) {
...
export async function startPollProcess(sessionKey = null) {
  ...
  pollStarted(curChannel, sessionKey);
...
export async function sendText(text, trackToolCalls = false) {
  ...
  // webchat 渠道：通过 Gateway sessions_send 推送到触发时的 session
  if (channel === 'webchat') {
    const state = pollStateLoad();
    const targetSession = state.sessionKey || 'agent:main:main';
    try {
      const payload = { tool: 'sessions_send', args: { sessionKey: targetSession, message: `[tkseller_text]\n${text}` } };
      const r = await fetch(GATEWAY_URL, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${_gatewayToken()}`,
      ...
      // 非 200（如 404）：fallback 到 buffer，防止文字丢失
...
async function _webchatSendCard(text, mediaUrlValue, buttons, mediaDescription) {
  ...
  if (mediaUrlValue) {
    // webchat 直接显示链接，control-ui 不支持图片内联渲染
    output += `\n🖼️ 商品图: ${mediaUrlValue}`;
  }
