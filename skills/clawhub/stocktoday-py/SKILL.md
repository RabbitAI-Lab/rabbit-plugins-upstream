---
name: stocktoday-data
description: A股 / 港股 / 美股 / 基金 / 期货 / 期权 / 债券 / 宏观 数据研究技能. 通过劫持官方 tushare 库走 StockToday 自定义后端加速, 接口跟 tushare 100% 兼容, 速度更快, 价格更低. 适用于把"看走势 / 查财报 / 比较公司 / 看板块 / 看资金流 / 梳理公告新闻 / 看宏观 / 拉数据导出"这类请求, 转成可执行的 tushare 数据获取 / 清洗 / 对比 / 筛选 / 导出流程.
...
通过官方 tushare 库调用 235+ 个数据接口, 流量自动劫持到 StockToday 自定义后端 (`https://tushare.citydata.club/`). 跟 tushare 官方接口**完全一致**, LLM 用标准 tushare 写法即可.

```
import tushare as ts
pro = ts.pro_api("your_token")
pro._DataApi__http_url = "https://tushare.citydata.club/"  # 劫持到 stocktoday gateway
df = pro.daily(ts_code="600519.SH", start_date="20240101", end_date="20241231")
```

> 详见 [scripts/proxy_demo.py](./scripts/proxy_demo.py), 提供 `get_client()` 封装, 一行初始化完成劫持.
