...
通过劫持官方 tushare 库, 让所有 tushare 调用走 StockToday 自定义后端 (https://tushare.citydata.club/).
...
def get_client(token: str = None, gateway: str = None) -> "ts.pro_api":
    """
    获取已劫持到 StockToday gateway 的 tushare pro client
    ...
    # 劫持 tushare 内部属性, 让流量走 stocktoday gateway
...
if __name__ == "__main__":
    pro = get_client()
    print(f"✅ StockToday client 已劫持到 gateway: {pro._DataApi__http_url}")
