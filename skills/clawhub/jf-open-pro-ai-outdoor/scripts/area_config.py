...
API:
- GET: POST /aisvr/v3/gateway/api/outdoorSecurity/areaConfig/get
- UPDATE: POST /aisvr/v3/gateway/api/outdoorSecurity/areaConfig/update
"""
...
CONFIG = {
    ...
    'api_get': '/aisvr/v3/gateway/api/outdoorSecurity/areaConfig/get',
    'api_update': '/aisvr/v3/gateway/api/outdoorSecurity/areaConfig/update'
}
...
def get_area_config(sn, user, uuid, appkey, secret, movecard, auth):
    """查询检测区域配置"""
...
def update_area_config(sn, user, uuid, appkey, secret, movecard, auth, area_data):
    """设置检测区域"""
    time_millis, signature = generate_signature(uuid, appkey, secret, movecard)
...
def main():
    parser = argparse.ArgumentParser(description='杰峰室外安防检测区域设置')
    parser.add_argument('--action', required=True, choices=['get', 'update'], help='操作类型：get=查询区域，update=设置区域')
    ...
    parser.add_argument('--area-data', help='检测区域 JSON 数据（仅 update 操作需要）')
    ...
    if args.action == 'update' and not args.area_data:
        print('❌ update 操作需要 --area-data 参数')
        print('区域数据格式示例：{"points":[{"x":0.5,"y":0.5},{"x":0.8,"y":0.5},{"x":0.8,"y":0.8},{"x":0.5,"y":0.8}]}')
        sys.exit(1)
    
    print('🗺️  杰峰室外安防检测区域设置')
    print('=' * 50)
    ...
    try:
        if args.action == 'get':
            result = get_area_config(sn, user, uuid, appkey, secret, movecard, auth)
        else:
            area_data = json.loads(args.area_data)
            result = update_area_config(sn, user, uuid, appkey, secret, movecard, auth, area_data)
        
        if result.get('code') == 2000:
            print('\n✅ 操作成功')
            if args.action == 'get':
                data = result.get('data', {})
                area_data = data.get('areaData', {})
                points = area_data.get('points', [])
                print(f'检测区域配置:')
                if points:
                    print(f'  多边形顶点数：{len(points)}')
