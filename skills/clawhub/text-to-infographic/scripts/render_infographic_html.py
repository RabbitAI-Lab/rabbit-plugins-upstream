...
# ---------------------------------------------------------------------------
# Premium themes (visual skin layer; CSS variables only -> content untouched)
# ---------------------------------------------------------------------------
...
_PREMIUM_CSS = {
    "data-journal": """
/* premium: data-journal — 数据新闻风（纸感网格 + 衬线大数字 + 刊头双线） */
:root {
  --bg: #F6F3EC;
  --panel: #FFFFFF;
  --text: #1F232B;
  ...
  --border: #E1DACB;
  --radius: 2px;
  --shadow: 0 1px 0 rgba(31,35,43,.05);
  --on-primary: #FFFFFF;
}
h1, h2, h3 { font-family: Georgia, "Noto Serif SC", "Songti SC", serif; letter-spacing: -0.01em; }
body {
...
.ig-header { border-bottom: 1px solid var(--text); padding-bottom: 6px; margin-bottom: 18px; }
.ig-header::after { content: ""; display: block; border-bottom: 3px double var(--text); margin-top: 5px; opacity: .9; }
.ig-h1 { font-weight: 700; }
.ig-sub { letter-spacing: .01em; }
.ig-takeaway { border-left-width: 3px; }
.ig-svg { border-radius: 0; }
.ig-node, .ig-metric-card, .ig-tl-card, .ig-pf-card, .ig-pyramid-inner { border-radius: 0; box-shadow: var(--shadow); }
.ig-metric-value { font-family: Georgia, "Times New Roman", serif; font-size: 27px; font-weight: 700; font-variant-numeric: tabular-nums; letter-spacing: -0.01em; }
.ig-dash-group-title { border-bottom: 1px solid var(--border); padding-bottom: 4px; }
.ig-tl-badge, .ig-pf-badge { background: var(--text); color: var(--panel); border-radius: 2px; }
.ig-pyramid-inner { border-radius: 0; }
.ig-note-card { border-radius: 0; }
.ig-meta { letter-spacing: .04em; }
""",
    "night-ops": """
/* premium: night-ops — 深色风（自动提亮品牌色保证深底可读） */
:root {
  --bg: #0B1220;
  --panel: #141E33;
  --text: #E7EEF9;
...
body {
  background-image: radial-gradient(1100px 720px at 78% -12%, color-mix(in srgb, var(--primary) 16%, transparent), transparent);
  background-color: var(--bg);
}
.ig-takeaway { border-left-color: var(--accent); }
.ig-node, .ig-metric-card, .ig-tl-card, .ig-pf-card, .ig-pyramid-inner { box-shadow: 0 2px 10px rgba(0,0,0,.22); }
.ig-metric-value { font-weight: 700; }
.ig-pyramid-up { color: var(--accent); }
.ig-tl-arrow { color: var(--accent); }
.ig-note-card { color: var(--text); }
@media print {
  :root { --bg:#fff; --panel:#fff; --text:#1F232B; --muted-text:#5F6B7A; --border:#E2E8F0; --note-bg:#FEF9C3; --note-border:#EAB308; --shadow:none; }
}
""",
}
...
def readable_on(hex_color: str) -> str:
    """Pick dark text on light surfaces, white text on dark surfaces."""
    return "#0F172A" if _relative_luminance(hex_color) > 0.42 else "#FFFFFF"


def build_premium_css(premium_style: Optional[str], palette: Dict[str, str]) -> str:
    """Return an appended CSS block for the requested premium skin ('' when none)."""
    if not premium_style or premium_style not in _PREMIUM_CSS:
        return ""
    css = _PREMIUM_CSS[premium_style]
    if premium_style == "night-ops":
        primary = lighten_hex(palette["primary"], 0.45)
        accent = lighten_hex(palette["accent"], 0.40)
        warn = lighten_hex(palette["warn"], 0.30)
        css = (
            css.replace("__PRIMARY__", primary)
            .replace("__ACCENT__", accent)
            .replace("__WARN__", warn)
            .replace("__ON_PRIMARY__", readable_on(primary))
        )
    return css
...
def build_radial(plan: Dict[str, Any]) -> str:
    ...
    def node_svg(slot: Tuple[Dict[str, Any], float, float]) -> str:
        block, x, y = slot
        role = block.get("visual_role", "secondary")
        fill = "var(--panel)" if role != "primary" else "var(--primary)"
        stroke = "var(--primary)" if role != "primary" else "var(--primary)"
        text_fill = "var(--text)" if role != "primary" else "var(--on-primary)"
...
def build_spine_branch(plan: Dict[str, Any]) -> str:
    ...
    def head_svg() -> str:
        out = [
            ...
            f'font-weight="800" fill="var(--on-primary)">{esc(truncate(effect_label, 14))}</text>',
            f'<text x="{head_x + head_w/2}" y="{spine_y + 22}" text-anchor="middle" font-size="12.5" '
            f'fill="var(--on-primary)" opacity="0.9">{esc(truncate(effect_content, 26))}</text>',
        ]
        return "\n".join(out)
...
def _build_sankey(plan: Dict[str, Any]) -> str:
    ...
    def node_svg(block: Dict[str, Any], x: float, y: float, h: float) -> str:
        ...
        if block.get("visual_role") == "primary":
            fill = "var(--primary)"
            stroke = "var(--primary)"
        text_fill = "var(--on-primary)" if fill == "var(--primary)" else "var(--text)"
...
.ig-takeaway {{
  margin-top: 14px; padding: 12px 16px; background: color-mix(in srgb, var(--primary) 8%, var(--panel));
...
.ig-pyramid-inner.role-primary {{ border-color: var(--primary); background: color-mix(in srgb, var(--primary) 6%, var(--panel)); }}
...
.ig-tl-badge, .ig-pf-badge {{
  display: inline-block; background: var(--primary); color: var(--on-primary); font-size: 11.5px; font-weight: 700;
...
.ig-trend.up {{ color: var(--positive); }}
.ig-trend.down {{ color: var(--warn); }}
.ig-trend.flat {{ color: var(--muted-text); }}
.ig-dash-highlights {{ background: color-mix(in srgb, var(--warn) 7%, var(--panel)); border: 1px solid color-mix(in srgb, var(--warn) 30%, var(--panel)); border-radius: var(--radius); padding: 12px 16px; }}
...
.ig-note-card {{ background: var(--note-bg); border: 1px solid var(--note-border); border-radius: var(--radius); padding: 10px 14px; }}
@media print {{
...
def build_html(plan: Dict[str, Any]) -> str:
    vs = plan.get("visual_system", {}) or {}
    palette = resolve_palette(vs)
    emphasis = vs.get("emphasis_style") or "clean"
    ...
    premium_style = vs.get("premium_style")
    css += build_premium_css(premium_style, palette)
