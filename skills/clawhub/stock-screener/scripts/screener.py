...
def cmd_rank(rows, col_map, args):
    """综合评分排序。"""
...
def cmd_score(rows, col_map, args):
    """单只股票详细评分。"""
    code_col = col_map.get("code", "")
    name_col = col_map.get("name", "")
    price_col = col_map.get("price", "")
...
def cmd_summary(rows, col_map, args):
    """筛选摘要统计。"""
    print("=" * 60)
    print("  股票池概览")
    print("=" * 60)
...
def cmd_correlation(rows, col_map, args):
    """因子相关性分析。"""
...
def build_parser():
    ...
    parser.add_argument("--file", required=True, help="股票数据CSV文件路径")
    parser.add_argument("--output", help="输出文件路径 (filter/rank命令)")

    subparsers = parser.add_subparsers(dest="command", help="子命令")

    # filter
    p_filter = subparsers.add_parser("filter", help="多条件筛选股票")
    ...
    # rank
    p_rank = subparsers.add_parser("rank", help="综合评分排序")
    ...
    # score
    p_score = subparsers.add_parser("score", help="单只股票详细评分")
    p_score.add_argument("--code", required=True, help="股票代码或名称")

    # summary
    subparsers.add_parser("summary", help="数据摘要统计")

    # correlation
    subparsers.add_parser("correlation", help="因子相关性分析")
