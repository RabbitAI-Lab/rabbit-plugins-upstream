...
version: 1.3.5
description: "网站转视频、URL 转视频、链接转视频、落地页转视频、网页宣传片——抓取站点、提取品牌、分镜、配音、合成、校验,基于 Remotion 模板。当用户给一个 URL(或直接贴链接)、想要宣传片 / 社媒广告 / 产品演示时使用。"
...
网站转视频、URL 转视频、链接转视频、落地页转视频、网页宣传片——抓取站点、提取品牌、分镜、配音、合成、校验,基于 Remotion 模板。当用户给一个 URL(或直接贴链接)、想要宣传片 / 社媒广告 / 产品演示时使用。
...
此技能与 dlazy 沙箱 agent 对话，**已固定到 `website-to-video` 模板** —— 一个以项目为单位、可端到端运行该模板的助手，支持多轮对话。
...
> **智能体关键指令**:
>
> 1. 新任务运行 `dlazy chat --skill website-to-video --prompt "..."`。
