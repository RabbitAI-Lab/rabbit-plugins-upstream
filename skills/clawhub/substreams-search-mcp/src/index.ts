...
// backed by the JWT-authenticated api.substreams.dev. The /packages HTML no
// longer contains the #packages-grid we scrape, and there is no public
// unauthenticated search endpoint (the CLI only does login/publish/verify).
// An empty scrape is therefore NOT evidence that no package exists — so we must
// never report "No packages found". This builds an honest fallback instead:
// the browsable registry URL, CLI guidance, and the better alternative.
function unavailablePayload(query: string, sort: string, network?: string) {
  const params = new URLSearchParams({ search: query, sort });
  if (network) params.set("network", network);
  const browseUrl = `${REGISTRY_URL}?${params}`;
  // Word-boundary match so "uniswap" doesn't trip "swap", etc.
  const tokenish =
    /\b(erc-?20|erc-?721|transfers?|balances?|holders?|tokens?|swaps?|nfts?)\b/i.test(query);
  const payload: Record<string, unknown> = {
    results: [],
    registry_search_unavailable: true,
    message:
      `Substreams full-text search is currently unavailable programmatically: ` +
      `substreams.dev moved to a client-rendered app behind the JWT-gated ` +
      `api.substreams.dev, and there is no public search endpoint. This is NOT ` +
      `evidence that no package exists for '${query}' — browse the registry directly ` +
      `or use the CLI.`,
    browse_url: browseUrl,
    cli_hint:
      "Install the Substreams CLI (https://docs.substreams.dev), run `substreams auth`, " +
      "then browse/run packages via https://substreams.dev.",
  };
  if (tokenish) {
    payload.recommended_alternative = {
      service: "token-api",
      why:
        "For ERC-20/721 transfers, balances, holders, swaps and NFT activity, the Token API " +
        "(https://api.pinax.network) returns ready-to-use data directly — usually faster and " +
        "simpler than building or running a Substreams.",
    };
  }
  return payload;
}
...
server.registerTool(
  ...
  async ({ query, sort, network }) => {
    ...
    try {
      packages = await fetchPackages(query, sort, network);
    } catch {
      packages = []; // network/parse error — fall through to honest guidance
    }

    const result =
      packages.length === 0
        ? unavailablePayload(query, sort, network)
        : { results: packages, count: packages.length, source: "substreams.dev" };
