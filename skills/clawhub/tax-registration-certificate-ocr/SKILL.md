---
name: tax_registration_certificate_ocr
description: 支持税务登记证的票据识别。
version: 1.0.1
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 税务登记证 OCR 技能

本技能封装了税务登记证识别的 OCR 服务，通过单一接口即可调用 1种识别能力，高效提取票据内容。
...
> **⚠️ 重要隐私提示**
>
> 使用本技能时，您指定的本地图片/PDF/压缩包会被上传到 **SCNET OCR API（默认 `https://api.scnet.cn/api/llm/v1/ocr/recognize`）** 进行识别。
>
> 上传的内容及识别结果中可能包含以下敏感信息：
> - 纳税人名称、证件编号
> - 法定代表人姓名
> - 地址、经营范围
> - 批准设立机关、发证日期等
...
> 请确保：
> 1. 您对该文件拥有合法处理权限；
> 2. 理解并同意 SCNET 的数据处理与存储策略；
> 3. 遵守适用的数据保护法规（如 GDPR、《个人信息保护法》等）；
> 4. 不要通过本技能上传涉密或未经授权的证件。

## 功能特性

- **税务登记证识别**：支持识别税务登记证的票据信息。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• TAX_REGISTRATION_CERT（税务登记证） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/tax_registration_certificate_ocr/scripts/main.py TAX_REGISTRATION_CERT /path/to/test.jpg
```
...
- “帮我识别这张图像中的税务登记证票据信息，图片在 /Users/name/Downloads/test.jpg”
