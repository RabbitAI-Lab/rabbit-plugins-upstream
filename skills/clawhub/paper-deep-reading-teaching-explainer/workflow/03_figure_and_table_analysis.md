...
- dataset scale, label definition, split protocol, metric definition, and compared baseline source when the visual or nearby text reports them
...
- exceptions to the headline trend, negative results, weak baselines, or conditions where the method does not win
- likely reasons for any mismatch, anomaly, or controversy
- why the layout is persuasive or potentially misleading
- reproduction risks caused by missing hardware, runtime, random seed, hyperparameters, baseline rerun/reimplementation status, preprocessing, or data split details
