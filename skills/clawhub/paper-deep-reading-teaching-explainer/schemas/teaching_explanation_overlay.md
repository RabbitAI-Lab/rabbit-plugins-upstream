...
### 12. Reproducibility and defense teaching pass

Add a teaching pass that makes the paper explainable at implementation and defense depth:

- Explain every key concept as `intuition -> formula -> concrete example -> limitation`.
- Teach in dependency order: symbols -> data/input -> model/modules -> training/inference -> experiments/limitations.
- For complex modules, include input, output, symbols, dimensions, trainable parameters, fixed hyperparameters, and data flow.
- Separate paper-stated facts from reasonable inference and nearby-work inference.
- Build a missing-detail ledger for hardware, runtime, hyperparameters, baseline source, baseline rerun/reimplementation status, data splits, seeds, and preprocessing.
- Make experiments concrete: dataset scale, label definition, baseline provenance, metric meaning, result exceptions, ablations, and reproduction risks.
- Include one small numeric walkthrough that links training and inference.
