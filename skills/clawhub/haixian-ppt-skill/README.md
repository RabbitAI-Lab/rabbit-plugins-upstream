...
一个适配 OpenClaw / ClawHub 和其他 Agent 环境的网页 PPT技能,用于生成**单文件 HTML 横向翻页 PPT**、PPT 配图和多平台封面。
...
| 平台 | 状态 | 说明 |
|------|------|------|
| OpenClaw | 支持 | 可通过 ClawHub 安装,适合生成和迭代 HTML deck |
| Codex / Claude Code | 可用 | 适合生成 PPT、调用图片生成能力、做浏览器视觉检查 |
...
装好后,OpenClaw 会在对话里发现并调用这个 skill。触发关键词:
...
5. **可选配图** — 当前 Agent 环境支持图片生成时,询问是否生成配图,再按页面比例插入
...
│   └── screenshot-backgrounds/ ← 截图美化内置背景(SVG):style-a 5 套 / style-b 4 套
├── scripts/
...
ClawHub 发布版本遵循 ClawHub skill 规则:发布到 ClawHub 的 skills 统一按 MIT-0 授权。
