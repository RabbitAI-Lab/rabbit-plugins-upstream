...
- **在 OpenClaw 中**:用普通对话直接询问用户,一次最多问 1-3 个最关键问题;如果信息缺口不影响开工,先做合理假设并在回复里说明。
- **在其他 Agent 环境中**:优先使用当前环境原生的澄清方式,但不要假设存在某个特定平台的提问工具。
...
如果当前运行环境支持图片生成,完成 deck 初稿后,主动问用户是否需要生成配图并插入 PPT。不要默认生成。
...
6. 如果当前环境支持生成配图,读 `image-prompts.md` 挑图片类型、比例和基础提示词;如果是用户原始截图,先读 `screenshot-framing.md`,优先使用 `assets/screenshot-backgrounds/` 的内置背景资产
