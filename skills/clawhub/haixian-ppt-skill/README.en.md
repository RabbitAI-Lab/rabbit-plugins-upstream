...
![OpenClaw](https://img.shields.io/badge/OpenClaw-ClawHub-111111?style=flat-square)
...
An agent skill for OpenClaw / ClawHub and similar coding-agent environments. It generates **single-file HTML horizontal-swipe decks**, deck visuals, and social cover pages.
...
```bash
openclaw skills install guizang-ppt-skill
```

Maintainers can publish with:

```bash
clawhub skill publish ./guizang-ppt-skill \
...
|----------|--------|-------|
| OpenClaw | Supported | Installable through ClawHub for creating and iterating HTML decks |
| Codex / Claude Code | Usable | Good for deck generation, image generation, and browser-based visual QA |
...
You can inspect the ClawHub listing with:

```bash
clawhub inspect guizang-ppt-skill
```

### How to trigger it

Once installed, OpenClaw can discover and use the skill. Trigger phrases:
...
5. **Optional image generation** — when the current Agent environment supports image generation, ask whether to generate images, then insert them at page-appropriate ratios
...
│   └── screenshot-backgrounds/ ← bundled SVG screenshot backgrounds: 5 style-a / 4 style-b
├── scripts/
...
ClawHub releases follow the ClawHub skill rules: skills published to ClawHub are licensed as MIT-0.
