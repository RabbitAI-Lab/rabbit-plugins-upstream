...
运行 `scripts/aliexpress_goods_search.py`，查询条件以重复 `--param 名称=值` 传入。
...
| `sort` | `monthSold`、`monthSales`、`totalSold`、`totalSales`、`goodsScore`、`reviewNum`、`onSaleTime`、`mallOpenTime` 或 `mallSold` |
| `order` | `asc` 或 `desc` |
...
已知无效占位价 `999999`，以及高于累计总量的周期销量/销售额，会按缺失值返回；不得用 0 或边界值补齐。

## 商品详情

运行 `scripts/aliexpress_goods_info.py --goods-id <商品ID> --site-id <站点ID>`。
