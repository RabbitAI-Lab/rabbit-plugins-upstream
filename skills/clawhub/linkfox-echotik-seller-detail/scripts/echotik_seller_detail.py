#!/usr/bin/env python3
"""
EchoTik TikTok Seller Detail - LinkFox Skill
调用 /echotik/sellerDetail 接口。

Usage:
  python echotik_seller_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_seller_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/sellerDetail"
SLUG = "linkfox-echotik-seller-detail"
...
def main():
    ...
    if not argv:
        print(
            "Usage: echotik_seller_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
