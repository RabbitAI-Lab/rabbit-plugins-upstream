#!/usr/bin/env python3
"""
X(Twitter) Video Downloader - API Version
Uses redfox.hk API to parse X(Twitter) video links and return watermark-free download URLs.
...
def main():
    parser = argparse.ArgumentParser(
        description="X(Twitter) Video Downloader - parse videos via redfox.hk API and return download links",
...
Configure API Key via environment variable:
  ...
  python3 downloader.py <url>
    ...
    parser.add_argument("url", help="X(Twitter) video link, e.g. https://x.com/user/status/xxxxx")
...
"""
    ...
    try:
        resp = session.post(API_URL, json={"url": url, "source": "twitter/X Video Download-ClawHub"}, timeout=30)
        result = resp.json()
