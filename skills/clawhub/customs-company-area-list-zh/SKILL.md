---
name: customs-company-area-list-zh
description: "查询公司海关贸易区域列表数据 — 获取国家/地区的贸易次数、金额和占比，用于市场分析。\n\nTrigger: 海关区域列表，分页区域查询，公司贸易国家分解，进出口市场分析，贸易区域下钻"
...
通过跨境魔方开放平台API，查询公司分页的贸易区域数据。

## 概述

本技能提供从跨境魔方海关数据库中查询公司分页贸易区域数据的能力。给定公司ID和公司类型，返回国家/地区列表及其贸易统计数据，包括交易次数、金额、数量、重量和占比。
...
**重要提示**：始终使用直接的脚本调用方式，如 `python scripts/customs_company_area_list.py`。**不要使用** `cd scripts && python customs_company_area_list.py` 这种复合命令。

### 公司贸易区域列表查询（`customs_company_area_list.py`）
- **返回粒度**：每个国家/地区一条记录
- **使用场景**：浏览公司的贸易国家/地区及详细统计，分析按国家划分的市场覆盖
- **示例**：
  - "列出公司100001作为供应商的贸易区域"
  - "查看公司100001作为采购商的国家贸易详情"
- **参数说明**：参见 [公司贸易区域列表API](references/customs-company-area-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/customs/company/area/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"公司贸易区域列表查询失败，服务端异常"}'
```
...
| 用户意图 | 使用API |
|---------|--------|
| "列出公司100001作为供应商的贸易区域" | 公司贸易区域列表查询 |
| "查看公司100001按国家的贸易分解" | 公司贸易区域列表查询 |

## 使用示例

### 查询公司贸易区域列表

**用户请求**："列出公司100001作为供应商的贸易区域"
```bash
...
- 公司贸易区域列表：查看 [references/customs-company-area-list-api.md](references/customs-company-area-list-api.md)
...
1. **检查API文档**：
   ...
   - 查看 [references/customs-company-area-list-api.md](references/customs-company-area-list-api.md)
...
3. **数据解读**：
   ...
   - `countryCode` 是ISO国家代码（如"US"、"KR"）

4. **跨技能使用**：
   - 公司ID可以从 **customs-company-stats** 或 **upkuajing-customs-trade-company-search** 获取
   - 结果中的国家代码可用于筛选 **customs-company-trends** 和 **customs-company-partner-stats** 的数据

## 注意事项
- `companyType` 决定公司角色（1=供应商，2=采购商）
- 使用 `cursor` 进行分页 — 它是一个 base64 编码的字符串
- 此端点提供详细的国家级分解，补充 **customs-company-area-stats** 的聚合视图
- 所有平台均使用正斜杠路径
