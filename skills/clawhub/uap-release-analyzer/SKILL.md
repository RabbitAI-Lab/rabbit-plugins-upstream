...
version: 1.0.1
description: Use when analyzing folders/tranches of declassified UAP/UFO/FOIA PDFs such as war.gov/UFO PURSUE, FBI Vault, NARA boxes, or AARO releases. Runs inventory, text extraction, entity/redaction analytics, and builds a standardized 11-section REPORT.md with caveats.
author: Chase Wang; OpenClaw port by Hermes Agent
license: MIT
---
# UAP / Declassified Release Analyzer

> OpenClaw port note: this is an OpenClaw-compatible skill adapted from Chase Wang's MIT-licensed `ckpxgfnksd-max/uap-release-analyzer`. Use the bundled scripts from the installed skill directory, e.g. `python3 ~/.openclaw/workspace/skills/uap-release-analyzer/scripts/run_all.py <release_root>`. If your OpenClaw skills directory is customized, locate the installed `uap-release-analyzer` directory and run the same `scripts/run_all.py` entrypoint there.
...
- `references/openclaw_usage.md` — OpenClaw/ClawHub install and usage notes
