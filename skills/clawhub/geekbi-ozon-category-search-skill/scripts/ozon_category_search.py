...
ENDPOINT = "/api/v1/ozon/category/ai-search"
BASE_PARAMS = {"keyword", "parentCatId", "catLevel", "siteId", "page", "size", "sort", "order"}
RANGE_FIELDS = {"dsr", "itemCount", "mallCount"}
ALLOWED_PARAMS = BASE_PARAMS | {
    f"{field}{suffix}" for field in RANGE_FIELDS for suffix in ("Min", "Max")
}
SORT_FIELDS = {
    "updateTime", "totalSold", "totalSales", "itemCount", "mallCount", "dsr",
    "catLevel", "daySold", "weekSold", "monthSold", "daySales", "weekSales",
    "monthSales", "avgPrice",
}
...
def parse_params(raw_params):
    ...
    validate_range_pairs(values, RANGE_FIELDS, integer_fields=RANGE_FIELDS)
    for field in ("parentCatId", "catLevel"):
        if field in values:
...
def main():
    parser = argparse.ArgumentParser(description="搜索 Ozon 类目与类目指标")
    ...
    try:
        ...
        payload = validate_search_response(payload, "Ozon 类目查询失败")
