...
"""
跨境魔方海关公司贸易HS编码列表查询
根据公司ID和公司类型获取公司贸易的HS编码列表数据（支持游标分页）。
"""
import argparse
import sys
...
def get_company_hscode_list(params: dict) -> dict:
    """
    根据查询参数获取公司贸易HS编码列表。
    ...
    Returns:
        包含HS编码列表数据的API响应
    """
    response = make_request('/agent/customs/company/hscode/list', params)
    return response


def main():
    parser = argparse.ArgumentParser(
        description='从跨境魔方开放平台获取公司贸易HS编码列表'
    )
    parser.add_argument(
        '--params',
        required=True,
    ...
    response = get_company_hscode_list(params)
