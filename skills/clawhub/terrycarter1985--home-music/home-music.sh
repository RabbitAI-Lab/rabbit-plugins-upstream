...
PLAYLIST_FOCUS="spotify:playlist:37i9dQZF1DWZeKCadgRdKQ"   # Deep Focus / work session
...
# Focus scene: Work session background music
# - Sonos Move only at a low, non-distracting volume
# - Deep Focus playlist; used by the daily report automation
scene_focus() {
    local vol_pct="${1:-30}"
    # Convert 0-100 integer percent to Airfoil's 0.0-1.0 float
    local vol_float
    vol_float=$(awk "BEGIN { printf \"%.2f\", $vol_pct/100 }")

    echo "🎯 Starting Focus scene (${vol_pct}%)..."

    # Set Airfoil source to Spotify
    airfoil_set_source_spotify

    # Connect Sonos Move at requested volume
    airfoil_connect "Sonos Move"
    sleep 0.5
    airfoil_volume "Sonos Move" "$vol_float"

    # Start playlist
    "$SPOTIFY_CMD" play "$PLAYLIST_FOCUS"
    "$SPOTIFY_CMD" volume 100

    echo "✅ Focus: Sonos Move @ ${vol_pct}%, Deep Focus"
}
...
case "${1:-}" in
    ...
    chill)
        ...
        ;;
    focus)
        # Optional second arg: volume percent (default 30)
        scene_focus "${2:-30}"
        ;;
    off|stop)
        scene_off
...
Scenes:
  ...
  focus      Deep Focus playlist on Sonos Move (default 30%, work sessions)
  off|stop   Stop music: pause Spotify + disconnect all speakers
