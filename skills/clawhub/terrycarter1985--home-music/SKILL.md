...
### 🎯 Focus
*Heads-down work sessions (used by automations)*

```bash
home-music focus       # default 30%
home-music focus 25    # custom volume percent
```
- **Speaker:** Sonos Move
- **Volume:** 30% (override with an integer percent)
- **Playlist:** Deep Focus
- **Vibe:** 🧠 Low-key background music for getting things done
...
```bash
home-music off     # alias: home-music stop
```
- Pauses Spotify
...
## 🐍 Python API (for automations)

Automations (e.g. a daily report job) can drive scenes — and, crucially,
**stop** music when work is done — via the bundled `home_music` Python module:

```python
from home_music import play_focus_mode, play_chill_mode, stop_music

play_focus_mode(volume=30)   # start background work music
# ... do your work ...
stop_music()                 # ALWAYS stop when finished
```

The module shells out to `home-music.sh`, so CLI and API behavior never drift.

- Set `HOME_MUSIC_DRY_RUN=1` to print intended commands without touching
  macOS-only tooling — handy for testing the call chain on CI/Linux.
- Override the script location with `HOME_MUSIC_SCRIPT=/path/to/home-music.sh`.
- Functions never raise by default (music control must not crash your job);
  pass `raise_on_error=True` to opt into exceptions.

> 💡 **Tip:** Wrap your work in `try/finally` and call `stop_music()` in the
> `finally` block so the music stops even if the job errors out.
...
├── home-music.sh   # The main script (scenes + stop)
└── __init__.py     # Python API wrapper (home_music: play_focus_mode/stop_music/...)
```
