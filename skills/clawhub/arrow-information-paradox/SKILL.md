...
- The question is **pure price/terms negotiation** with no disclosure-leakage risk — use batna-zopa.
- The asymmetry is a **within-relationship incentive problem**, not a one-shot disclosure gate — use principal-agent.
...
2. Check fit against When to Use / When NOT to use. If the buyer can inspect a product without getting the design, or the info is already public, redirect (often to batna-zopa).
...
- **Patent and secrecy are a moat choice, not a filing chore.** Patenting *publishes* your method in exchange for a time-limited, enforceable monopoly; secrecy keeps it hidden but is lost the instant it leaks or is independently discovered. Choose secrecy when the advantage is hard to reverse-engineer and would outlive a patent term; choose a patent when it is easy to copy once seen and you can afford to enforce it. (See economic-moat.)
...
- **The buyer has the mirror problem.** If you are the *buyer*, everything withheld is potential adverse selection — price the risk that what you were *not* shown is worse than what you were (see winners-curse), and prefer intermediary certification or escrow to taking the seller's word.
