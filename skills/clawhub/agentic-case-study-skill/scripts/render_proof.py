#!/usr/bin/env python3
"""List and render agentic development proof asset templates."""
...
CATALOG_MD = ROOT / "references" / "proof-catalog.md"
...
def extract_template(template_id: str) -> str:
    ...
    if not match:
        raise KeyError(f"Template not found in proof-catalog.md: {template_id}")
...
def main() -> int:
    ...
    parser.add_argument("--list", action="store_true", help="List available proof asset IDs.")
    parser.add_argument("--stage", help="Filter --list by stage.")
    parser.add_argument("--type", dest="asset_type", help="Filter --list by type.")
    ...
    if args.list:
        for item in templates:
            if args.stage and item["stage"] != args.stage:
                continue
            if args.asset_type and item["type"] != args.asset_type:
                continue
