---
name: socq-douyin-research
description: Research public Douyin content, accounts, keywords, and performance data with SocQ. Use when an agent needs Douyin-specific discovery, collection, endpoint selection, credit estimates, asynchronous task execution, pagination, or raw exports through the SocQ CLI, MCP, or REST API.
metadata:
  openclaw:
...
- API documentation: <https://docs.socq.ai/api-manual/douyin>
...
Use this Skill to select and run SocQ endpoints for public Douyin research. SocQ requests are asynchronous and credit-metered.

## Use When

- The user asks to discover, collect, compare, or analyze public Douyin data.
- The workflow needs Douyin-specific endpoint selection, input validation, pagination, or raw exports.
...
1. Prefer an already configured SocQ MCP server at `https://api.socq.ai/mcp?platforms=douyin`; use the typed tool listed in [platform.md](references/platform.md), or compact `socq_execute` when needed.
