...
- the task clearly needs factor data for 产品碳足迹建模 / LCA建模 / 排放清单核算 / 情景测算
- the user provides BOM or material list and asks for carbon footprint calculation
- the task involves 供应链碳排放 / scope 3 核算 and activity data is present but no factor has been supplied

---

## Broad factor coverage

This skill is designed for **broad factor retrieval + selection**, not just narrow single-source lookup.

It can help the user search and choose across:
- **China domestic factor sources**
- **international factor sources**
- **institutional / published datasets**
- **ecoinvent-related records available in CCDB**

This matters because many real-world carbon-accounting tasks do not fail at "finding any factor" — they fail at **choosing the right one from a wide set of possible sources**.
...
- Search ecoinvent-related factors available in CCDB for resin materials
...
- `Search ecoinvent-related factors available in CCDB for polyester resin`
...
### Example 4 — ecoinvent-related lookup
> 帮我找 CCDB 里和 ecoinvent 相关的聚合物因子，优先能直接用于 LCA 建模的。

Expected behavior:
- search broad source candidates
- surface ecoinvent-related records where available in CCDB
- compare candidates by unit / region / suitability
- explain whether the result is safe for direct use or needs review

### Example 5 — suitability review
> 这个因子能不能直接用于正式报告？
...
For some categories — especially electricity — available results may include both:
...
- Chinese core term
- English equivalent
- nearby synonyms where needed

### Rank candidates instead of trusting the first hit
...
Keep scoring / filtering logic in code rather than overloading the skill page with implementation detail.
