...
summary: "日历管理与调度工具,支持创建事件、安排会议与跨平台同步,适合个人用户。核心能力: 日程管理领域的专业化 AI 辅助工具,提供核心基础功能支持."
license: MIT
edition: free
description: "核心能力: 日程管控领域的专业化 AI 辅助工具,包含核心基础功能兼容. 适用于需要calendar toolkit相关能力的开发场景,包含结构化的工作流程和可复用的模板,帮助用户快速完成任务并保持代码质量. 适用于需要calendar toolkit相关能力的开发场景,包含结构化的工作流程和配置指引."
tags:
  - 日历管理
  - calendar
  - toolkit
...
**处理**: 解析创建日历事件并设置提醒的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回创建日历事件并设置提醒的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 安排会议并查看参与者可用时间
安排会议并查看参与者可用时间

**处理**: 解析安排会议并查看参与者可用时间的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回安排会议并查看参与者可用时间的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 查看今日/本周日程安排
查看今日/本周日程安排

**处理**: 解析查看今日/本周日程安排的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回查看今日/本周日程安排的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 支持重复事件设置
支持重复事件设置

**处理**: 解析支持重复事件设置的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回支持重复事件设置的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 跨 Google/Apple/
跨 Google/Apple/Outlook 平台同步

**处理**: 解析跨 Google/Apple/的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回跨 Google/Apple/的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：日历管理与调度工、支持创建事件、安排会议与跨平台、适合个人用户、日程管理领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 日历管理工具包基础版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "日历管理工具包基础版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "calendarkit"
    }
  },
