...
RESOLUTION_OPTIONS = [
    ("360p", "360p"), ("480p", "480p"), ("720p", "720p"), ("1080p", "1080p"), ("1440p", "1440p"), ("2160p", "2160p"),
]
VIDEO_CODEC_OPTIONS = [("H.264/AVC", "avc1"), ("VP9", "vp9"), ("AV1", "av01")]
...
def ensure_python_version():
    if sys.version_info < MIN_PYTHON:
        print(
            "Python {}.{} or newer is required. Run this script with a Python 3 interpreter, for example: python3 scripts/submit_dataify_youtube_video_by_url.py --url \"{}\"".format(
                MIN_PYTHON[0],
                MIN_PYTHON[1],
                DEFAULT_URL,
            ),
...
def list_options():
    print_option_table("resolution", RESOLUTION_OPTIONS)
    print_option_table("resolution_direction", DIRECTION_OPTIONS)
    print_option_table("video_codec", VIDEO_CODEC_OPTIONS)
...
def normalize_universal(args):
    resolution = normalize_choice(args.resolution, RESOLUTION_OPTIONS, "resolution")
    resolution_direction = normalize_direction(args.resolution_direction, "resolution_direction")
    ...
    return {
        "resolution": with_direction(resolution_direction, resolution),
        "video_codec": normalize_choice(args.video_codec, VIDEO_CODEC_OPTIONS, "video_codec"),
