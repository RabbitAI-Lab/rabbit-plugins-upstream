...
description: Submit Dataify YouTube Video File by URL Builder tasks. Use when the user wants the YouTube video file collection tool, collect YouTube video files, scrape YouTube video files, crawl YouTube video media, collect video files by URL, create a Dataify youtube_video_by-url task, or asks in Chinese with meanings like "YouTube视频文件采集", "YouTube视频文件抓取", "YouTube视频采集", "YouTube视频抓取", "视频文件采集", "视频文件抓取", "视频采集", "视频抓取", or similar video file noun plus collection/scraping action wording. Also use when receiving task_id/status, configuring DATAIFY_API_TOKEN, or troubleshooting this Dataify Builder request.
---

# Dataify YouTube Video By URL

Submit YouTube video file collection jobs through Dataify Builder by URL. After a successful submission, give the user the `task_id`, the returned or inferred status, and tell them to visit [Dataify](https://dashboard.dataify.com?utm_source=skill) to view results.
