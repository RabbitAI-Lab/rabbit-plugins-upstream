...
# 验证路径是否为有效 workspace（检查是否有 skills/ 子目录）
# 支持 workspace（无后缀）和 workspace-xxx（带后缀）两种命名
if not (WORKSPACE_ROOT / "skills").is_dir():
    # 回退到路径检测：支持 workspace、workspace-xxx 两种命名
    workspaces = list(WORKSPACE.glob("workspace*"))
    # 优先选择包含 skills/ 目录的
    valid = [w for w in workspaces if (w / "skills").is_dir()]
    if valid:
        WORKSPACE_ROOT = valid[0]
    else:
        # 找不到有效 workspace，报错退出（不静默回退）
        print(f"❌ 找不到有效的 workspace 目录（在 {WORKSPACE} 下未找到含 skills/ 的 workspace*）", file=sys.stderr)
        sys.exit(1)
...
def check_gateway(result: CheckResult):
    ...
    # 检查 Gateway 端口（本机端口 18789）
    gateway_port = 18789
    code, stdout, _ = run_cmd(f"ss -tlnp | grep {gateway_port}")
    if code == 0:
        result.add_pass(f"Gateway 端口 {gateway_port} 监听中")
    else:
        # 尝试检查其他常见端口
        for alt_port in [23789, 8789]:
            code2, stdout2, _ = run_cmd(f"ss -tlnp | grep {alt_port}")
            if code2 == 0:
                result.add_warning(f"Gateway 监听在端口 {alt_port}（预期 {gateway_port}），请检查配置")
                break
        else:
...
def check_backup_status(result: CheckResult):
    """检查最近的 NAS 备份状态"""
    nas_gmx = Path("/mnt/nas-gmx")
    backup_dir = nas_gmx / "backups"

    if not nas_gmx.exists():
        result.add_warning("备份状态：/mnt/nas-gmx 未挂载，跳过备份检查")
        return

    if not backup_dir.exists():
        result.add_warning(f"备份状态：目录 {backup_dir} 不存在")
        return
    ...
    for check_date in [today, yesterday]:
        fname = backup_dir / f"gmx-backup-{check_date}.tar.gz"
        if fname.exists():
            size_mb = fname.stat().st_size / 1024 / 1024
            found = (check_date, size_mb)
            break

    if found:
        date_str, size_mb = found
        label = "今天" if date_str == today else "昨天"
        result.add_pass(f"备份状态：{label} ({date_str}) 有备份，大小 {size_mb:.1f} MB")
    else:
        # 查找最近备份
        backups = sorted(backup_dir.glob("gmx-backup-*.tar.gz"), reverse=True)
        if backups:
            latest = backups[0]
            size_mb = latest.stat().st_size / 1024 / 1024
            result.add_issue(
                f"备份状态：超过2天无备份（最近: {latest.name}, {size_mb:.1f} MB）",
                "检查备份脚本是否正常运行",
                "error"
            )
        else:
            result.add_issue(
                "备份状态：备份目录为空，未找到任何备份文件",
                f"确认备份脚本已配置并指向 {backup_dir}",
                "error"
            )


def check_openclaw_logs(result: CheckResult):
    """检查 OpenClaw 最近日志中的 ERROR/WARN"""
    log_dir = WORKSPACE / "logs"

    log_lines = []

    # 方式1：~/.openclaw/logs/ 目录
    if log_dir.exists():
        log_files = sorted(log_dir.glob("*.log"), key=lambda f: f.stat().st_mtime, reverse=True)
        if log_files:
            latest_log = log_files[0]
            code, stdout, _ = run_cmd(f"tail -100 '{latest_log}'")
            if code == 0:
                log_lines = stdout.splitlines()
                result.add_pass(f"日志来源：{latest_log.name}（最近100行）")

    # 方式2：journalctl（如果方式1没拿到日志）
    if not log_lines:
        code, stdout, _ = run_cmd(
            "journalctl --user -u openclaw-gateway -n 100 --no-pager 2>/dev/null",
            timeout=10
        )
        if code == 0 and stdout:
            log_lines = stdout.splitlines()
            result.add_pass("日志来源：journalctl（最近100行）")

    if not log_lines:
        result.add_warning("日志检查：未找到日志文件，跳过")
        return

    errors = [l for l in log_lines if "ERROR" in l]
    warns = [l for l in log_lines if "WARN" in l and "ERROR" not in l]

    if errors:
        recent_errors = errors[-3:]
        result.add_issue(
            f"日志报错：最近100行发现 {len(errors)} 条 ERROR\n   最近3条:\n" +
            "\n".join(f"   {e}" for e in recent_errors),
            "查看完整日志排查原因",
            "error"
        )
    else:
        result.add_pass("日志错误：最近100行无 ERROR")

    if warns:
        result.add_warning(f"日志警告：最近100行发现 {len(warns)} 条 WARN")
    else:
        result.add_pass("日志警告：最近100行无 WARN")


def check_disk_status(result: CheckResult):
    """检查关键挂载点磁盘使用率"""
    mounts = ["/home/gmkjie", "/mnt/nas-gmx", "/mnt/nas-wjie"]

    code, stdout, _ = run_cmd(f"df -h {' '.join(mounts)} 2>/dev/null")
    if code != 0 or not stdout:
        result.add_warning("磁盘检查：df 命令失败")
        return
    ...
    for line in data_lines:
        ...
        if len(parts) < 6:
            continue
        # df -h 格式: Filesystem Size Used Avail Use% Mounted
        filesystem = parts[0]
        ...
        try:
            ...
            result.add_warning(f"磁盘 {mountpoint}：无法解析使用率 ({parts[4]})")
            continue

        if use_pct < 70:
            result.add_pass(f"磁盘 {mountpoint}：{use_pct}% 已用 ✅")
        elif use_pct <= 85:
            result.add_warning(f"磁盘 {mountpoint}：{use_pct}% 已用（建议清理）")
        else:
            result.add_issue(
                f"磁盘 {mountpoint}：{use_pct}% 已用（严重偏高）",
                f"清理 {mountpoint} 下的旧文件或扩容",
                "error"
            )
    ...
    for line in data_lines:
        parts = line.split()
        if len(parts) >= 6:
            mounted_set.add(parts[5])

    for mp in mounts:
        if mp not in mounted_set and not Path(mp).exists():
            result.add_warning(f"磁盘 {mp}：⚠️ 未挂载")
...
def main():
    ...
    print("📡 检查 OpenClaw 日志...")
    check_openclaw_logs(result)

    print("📡 检查磁盘状态...")
    check_disk_status(result)
