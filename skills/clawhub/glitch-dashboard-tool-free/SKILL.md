...
license: Proprietary
edition: free
description: '核心能力: 运维监控领域的专业化 AI 辅助工具,提供核心基础功能支持.
...
本工具是 **运维监控** 领域的 **FREE 版本** AI Skill,专为个人用户与轻量级场景设计。通过自然语言指令驱动 AI Agent 执行任务,提供核心功能与简洁易用的操作体验.
...
**处理**: 解析实时监控 CPU/内存/负载与的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回实时监控 CPU/内存/负载与的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 任务队列查看与管理
任务队列查看与管理

**处理**: 解析任务队列查看与管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回任务队列查看与管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 完成当前任务与清空队列
完成当前任务与清空队列

**处理**: 解析完成当前任务与清空队列的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回完成当前任务与清空队列的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 日志输出流查看与过滤
日志输出流查看与过滤

**处理**: 解析日志输出流查看与过滤的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回日志输出流查看与过滤的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### CLI 快速状态检查
CLI 快速状态检查

**处理**: 解析CLI 快速状态检查的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回CLI 快速状态检查的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 功能详情

本版本提供 5 项核心能力,覆盖 运维监控 的常见使用场景。如需批量操作、团队协作、数据分析等高级功能,可升级至 PRO 版本.
**FREE 版本核心功能清单:**
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：仪表盘管理任务队、监控系统指标与查、看网络状态、运维监控领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | 统一仪表盘基础版处理的输入数据或指令 |
