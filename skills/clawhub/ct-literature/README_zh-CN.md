...
**Q：一次检索要跑多久？有限额吗？** A：
- **典型耗时**：各启用源**并行**执行（按源并发）；Europe PMC ~1秒/页，OpenAlex ~2秒/页，所以墙钟时间是*最慢的那个源*，而非各源之和。拉取约 50 篇文献的 3 源合并通常 **10–30 秒**完成（开启全量引文验证时再多 ~1–4 分钟——见运行前的耗时预估）。加预印本（bioRxiv/medRxiv/arXiv）再多数秒。
