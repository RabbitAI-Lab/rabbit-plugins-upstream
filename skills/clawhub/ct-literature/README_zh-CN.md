...
- **结果上限**：默认 `max_results` 为**每源 30 篇**；无硬性上限（调大允许），但耗时与 API 用量线性增长。**2026-08-14 实测**（双源、"奥希替尼 间质性肺病"）：`--max 100` → 抓取+合并**约 20 秒**、全量验证**约 78 秒**（跨源去重后约 0.64 秒/篇；双源去重后保留约 **62%**）。**按 5 分钟总耗时建议：双源每源 ≤300、三源每源 ≤250**（两种配置合并去重后均约 370–380 篇；去重是自动的，3×250 并不等于报告里 750 篇）。超出后总耗时近似线性增长——大批量需求请配置 OpenAlex key 并改用 `--verify top 15`，而非一味调大 `max_results`。
- **速率限制：**
...
**版本**：v0.9.0 | **许可证**：MIT | **作者**：medstatstar, phoe-zip
