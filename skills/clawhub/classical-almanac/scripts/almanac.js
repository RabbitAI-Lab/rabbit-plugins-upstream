 ...
 * 以月支起寅，按日支顺数
 */
const HUANGDAO_SHIER = ['青龙','明堂','天刑','朱雀','金匮','天德','白虎','玉堂','天牢','玄武','司命','勾陈'];
const HUANGDAO_JIXIONG = {
  '青龙': { good: true, desc: '生气之星，百事皆宜' },
  '明堂': { good: true, desc: '贵人星，利于谒见诉讼' },
  '天刑': { good: false, desc: '刑伤之星，忌诉讼远行' },
  '朱雀': { good: false, desc: '口舌之星，忌争讼签约' },
  '金匮': { good: true, desc: '财库之星，利于纳财交易' },
  '天德': { good: true, desc: '天恩德星，百事皆宜' },
  '白虎': { good: false, desc: '病符之星，忌动土远行' },
  '玉堂': { good: true, desc: '喜庆之星，利于嫁娶开市' },
  '天牢': { good: false, desc: '牢狱之星，忌诉讼远行' },
  '玄武': { good: false, desc: '盗贼之星，忌交易出行' },
  '司命': { good: true, desc: '天命之星，利于祭祀祈福' },
  '勾陈': { good: false, desc: '阻滞之星，忌远行动土' }
};
 ...
 * 以月支起寅，按日支顺数
 */
function getHuangDao(date) {
  const gz = _getDayGanZhi(date);
  const dayZhiIdx = _ZHI.indexOf(gz[1]);
  const m = date.getMonth() + 1;
  // 月支：寅月(1)=2, 卯月(2)=3, ..., 丑月(12)=1
  const monthZhiIdx = (m + 1) % 12;
  const offset = ((dayZhiIdx - monthZhiIdx) + 12) % 12;
  const shenName = HUANGDAO_SHIER[offset];
  return { name: shenName, ...HUANGDAO_JIXIONG[shenName] };
}
 ...
 * 计算天德/月德/天赦
 */
function getDeSha(date) {
  const gz = _getDayGanZhi(date);
  const dayGan = gz[0];
  const m = date.getMonth() + 1;
  const season = getSeason(m);
  
  const tianDe = TIAN_DE_YUE[m] === dayGan;
  const yueDe = YUE_DE_YUE[m] === dayGan;
  const tianShe = TIAN_SHE[season] === gz;
  
  return { tianDe, yueDe, tianShe, hasDe: tianDe || yueDe || tianShe };
}

/**
 * 凶煞计算（简化版）
 */
function getEvilSha(date) {
  const gz = _getDayGanZhi(date);
  const dayZhi = gz[1];
  const m = date.getMonth() + 1;
  const year = date.getFullYear();
  const yearZhiIdx = (year - 4) % 12; // 2024=子(0)
  
  const shaList = [];
  
  // 劫煞（按年支查日支）
  const jieShaMap = { '子': '巳', '丑': '寅', '寅': '亥', '卯': '申', '辰': '巳', '巳': '寅', '午': '亥', '未': '申', '申': '巳', '酉': '寅', '戌': '亥', '亥': '申' };
  const yearZhi = _ZHI[yearZhiIdx];
  if (jieShaMap[yearZhi] === dayZhi) shaList.push('劫煞');
  
  // 月破（月支对冲日支）
  const monthZhiIdx = (m + 1) % 12;
  const monthZhi = _ZHI[monthZhiIdx];
  if (_CHONG[monthZhi] === dayZhi) shaList.push('月破');
  
  return { shaList, hasEvil: shaList.length > 0 };
}
...
function scoreDate(date, activityType, userDayStem = null) {
  ...
  if (activity.good.includes(zhiXing)) {
    score += 25;
    factors.push({ name: '建除', value: `${zhiXing}日`, bonus: 25, good: true });
  } else if (activity.bad.includes(zhiXing)) {
    score -= 30;
    factors.push({ name: '建除', value: `${zhiXing}日`, bonus: -30, good: false });
  } else {
    score += 5;
  ...
  // === 2. 黄道黑道十二神（权重25%，±25分）===
  const huangDao = getHuangDao(date);
  if (huangDao.good) {
    score += 20;
    factors.push({ name: '黄道', value: huangDao.name, bonus: 20, good: true });
  } else {
    score -= 25;
    factors.push({ name: '黑道', value: huangDao.name, bonus: -25, good: false });
  }
  
  // === 3. 天德/月德/天赦（权重20%，+25分）===
  const deSha = getDeSha(date);
  if (deSha.tianDe) {
    score += 15;
    factors.push({ name: '天德', value: '天德值日', bonus: 15, good: true });
  }
  if (deSha.yueDe) {
    score += 10;
    factors.push({ name: '月德', value: '月德值日', bonus: 10, good: true });
  }
  if (deSha.tianShe) {
    score += 25;
    factors.push({ name: '天赦', value: '天赦日', bonus: 25, good: true });
  }
  
  // === 4. 凶煞（权重15%，-25分）===
  const evilSha = getEvilSha(date);
  if (evilSha.hasEvil) {
    const penalty = evilSha.shaList.includes('月破') ? -25 : -15;
    score += penalty;
    factors.push({ name: '凶煞', value: evilSha.shaList.join('、'), bonus: penalty, good: false });
  }
  ...
  if (yiMatch) score += 10;
  if (jiMatch) score -= 10;
  factors.push({ name: '宜忌', value: yiMatch ? '配合较好' : (jiMatch ? '有冲忌' : '一般'), bonus: yiMatch ? 10 : (jiMatch ? -10 : 0), good: yiMatch ? true : (jiMatch ? false : null) });
  ...
  if (qimen.zhiFu.good) {
    score += 8;
    factors.push({ name: '值符', value: qimen.zhiFu.name, bonus: 8, good: true });
  } else {
    factors.push({ name: '值符', value: qimen.zhiFu.name, bonus: 0, good: false });
  }
  if (qimen.zhiShi.good) {
    score += 8;
    factors.push({ name: '值使', value: qimen.zhiShi.name, bonus: 8, good: true });
  } else {
...
🏆 综合最优（${activityType}）
   ...
   干支：${best.dayGanZhi}
   建除：${best.zhiXing}日
   黄道：${best.huangDao.name}（${best.huangDao.desc}）
...
`;
    ...
    if (best.deSha.hasDe) {
      const deList = [];
      if (best.deSha.tianDe) deList.push('天德');
      if (best.deSha.yueDe) deList.push('月德');
      if (best.deSha.tianShe) deList.push('天赦');
      report += `   吉神：${deList.join('、')}\n`;
    }
    
    // 凶煞
    if (best.evilSha.hasEvil) {
      report += `   凶煞：${best.evilSha.shaList.join('、')}\n`;
    }
...
💡 评分说明
   ...
   评分因素（协纪辨方书体系）：建除十二神(±30)、黄道黑道(±25)、
   天德月德天赦(+25)、凶煞(-25)、宜忌配合(±15)、日冲(±20)、
   值符值使(±16)、五行生克(±15)
`;
...
`;
  ...
  if (best.deSha.hasDe) {
    ...
    if (best.deSha.yueDe) deList.push('月德');
    if (best.deSha.tianShe) deList.push('天赦');
    output += `   吉神：${deList.join('、')}\n`;
  }
  
  if (best.evilSha.hasEvil) {
    output += `   凶煞：${best.evilSha.shaList.join('、')}\n`;
  }
