---
name: vehicle-license-recognition-and-extract
description: 基于来也科技ADP平台的中国机动车行驶证智能识别与信息抽取Skill。支持行驶证正副页全量字段的精准抽取——号牌号码、车辆类型、所有人、住址、使用性质、品牌型号、车辆识别代号VIN、发动机号码、注册日期、发证日期等20+关键字段，输出结构化JSON，零配置开箱即用，适用于车辆登记、保险投保、网约车/货运车辆准入等场景。
---

# 中国机动车行驶证识别与抽取 Skill

能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=clawhub)提供，支持中国机动车行驶证（正页、副页）的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成行驶证图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（每月刷新），相当于每月可免费抽取 **200 张行驶证**。ADP 提供标准可商用 API，**1 小时即可快速集成接入业务系统**。
...
4. **执行抽取**：运行 `adp extract url <URL> --app-id <行驶证抽取应用ID>` 或 `adp extract local <文件路径> --app-id <行驶证抽取应用ID>`。
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张行驶证图片（URL） | `adp extract url <URL> --app-id <行驶证抽取应用ID>` |
| 识别一张本地行驶证图片 | `adp extract local <文件路径> --app-id <行驶证抽取应用ID>` |
| 识别 Base64 编码的行驶证 | `adp extract base64 <base64> --app-id <行驶证抽取应用ID> --file-name <文件名.后缀>` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的行驶证 | `adp extract local <文件夹路径> --app-id <行驶证抽取应用ID>` |
| 批量识别多个 URL | `adp extract url <URL列表文件> --app-id <行驶证抽取应用ID>` |

**异步处理**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 异步提交大文件 | `adp extract url <URL> --app-id <行驶证抽取应用ID> --async` |
| 异步批量处理 | `adp extract local <文件夹路径> --app-id <行驶证抽取应用ID> --async` |
...
ADP 为中国行驶证提供了**开箱即用**的内置抽取应用，无需额外配置。
...
行驶证识别属于**开箱即用应用**，可通过 `--app-type 0` 筛选查询。
...
> 上例中 `"app_id": "ootb_******xx"` 即为行驶证抽取应用。`app_type` 为 `0` 表示开箱即用应用，`1` 表示自定义应用。
...
```bash
adp extract url https://example.com/vehicle-license.jpg --app-id <行驶证抽取应用ID>
```

### 单张行驶证抽取（本地文件）

```bash
adp extract local ./vehicle-license.jpg --app-id <行驶证抽取应用ID>
```

### 单张行驶证抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <行驶证抽取应用ID> --file-name <文件名.后缀>
```

### 返回结果示例

ADP 行驶证抽取统一返回结构化 JSON，支持行驶证正页和副页共 **21 个**关键字段识别抽取。
...
```bash
adp extract local ./vehicle-licenses/ --app-id <行驶证抽取应用ID> --export ./results/ 
```
...
```bash
# 提交异步任务
adp extract url https://example.com/vehicle-license.jpg --app-id <行驶证抽取应用ID> --async
...
# 行驶证抽取（URL）
adp extract url <文件URL> --app-id <行驶证抽取应用ID>

# 行驶证抽取（本地文件）
adp extract local <文件路径> --app-id <行驶证抽取应用ID>

# 行驶证抽取（Base64）
adp extract base64 <base64字符串> --app-id <行驶证抽取应用ID> --file-name <文件名.后缀>

# 批量抽取
adp extract local <文件夹路径> --app-id <行驶证抽取应用ID> --export <输出路径> 

# 异步抽取
adp extract url <文件URL> --app-id <行驶证抽取应用ID> --async
