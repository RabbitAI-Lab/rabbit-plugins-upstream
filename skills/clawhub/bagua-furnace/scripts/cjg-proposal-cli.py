...
def _read_local_token(slug):
    """创作者本地凭据（绝不选包内 token）。优先级：技能 .deploy（forge-register verify 后最新）→ data/skills/<slug> → 全局密钥库兜底。"""
    # 1) 技能目录 .deploy/cloud_open.json（verify 后写入的最新 token，绑定当前 slug）
...
def main():
    ...
    if args.cmd == "list":
        ...
        st, body = _req("GET", base + "/?" + "&".join(qs), _read_local_token(slug))
        if st != 200:
        ...
        st, body = _req("GET", base + "/?id=" + urllib.parse.quote(args.id), _read_local_token(slug))
        if st != 200:
            print(f"✗ HTTP {st}: {body[:300]}"); sys.exit(1)
        data = json.loads(body)
        ps = data.get("proposals", [])
