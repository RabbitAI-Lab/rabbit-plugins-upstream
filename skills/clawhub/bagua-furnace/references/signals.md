> 本文件由「技能锻造炉」为 **bagua-furnace v1.2.1** 定制注入：
> 全文示例的 skill_slug / skill_version 已按本技能替换；层码 L1–L7 语义可按本技能的实际能力层调整。

# 进化燃料 · 信号规范（本地安装即开 + 云端上传默认关需显式开启 · 双模态）
...
| `skill_slug` | 技能标识 | `bagua-furnace` |
| `skill_version` | 本次应答时的技能版本号 | 如 `1.0.0`，用于检测发版后完成率骤降 |
...
```json
{"ts":"...","signal_id":"<uuid>","skill_slug":"bagua-furnace","skill_version":"<ver>","method_layer":"L2",
...
```json
{"ts":"...","signal_id":"<uuid>","client_signal_id":"<uuid>","skill_slug":"bagua-furnace",
...
printf '%s\n' "$(python -c "import json,uuid,datetime; print(json.dumps({'ts':datetime.datetime.now().astimezone().isoformat(),'signal_id':str(uuid.uuid4()),'skill_slug':'bagua-furnace','skill_version':'1.0.0','method_layer':'<L1..L7>','event':'<helpful|unhelpful|confusion|suggestion|abandoned|misdiagnosis>','weight':<1-5>,'note':'<触发类标签>','anon_id':open('<dir>/.anon_id').read().strip() if __import__('os').path.exists('<dir>/.anon_id') else ''},ensure_ascii=False))")" >> "<dir>/signals-log.jsonl"
```
...
> 层码 L1–L7 按本技能的锻造能力分层；缺口上报走 `event=suggestion` + `note`（不设 `coverage_gap` 字段）。
