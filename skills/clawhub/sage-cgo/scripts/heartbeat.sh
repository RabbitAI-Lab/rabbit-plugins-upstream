...
if [ -d "$SAGE_HOME/growth" ] && grep -R "待填写" "$SAGE_HOME/growth" >/dev/null 2>&1; then
  echo "[ONBOARDING] Growth extension still has placeholders."
  ISSUES=$((ISSUES + 1))
fi
...
if [ "$ISSUES" -eq 0 ]; then
  echo "[OK] Sage DNA and growth extension look healthy."
else
