...
- 用户同意 → 继续步骤 1
- 用户拒绝 → 输出手动注册链接 `https://ai.zhiliaobiaoxun.com/?ch=s52`，本流程终止
...
> **`ch` 字段说明**：固定填 `"s52"`。**不要从用户环境或动态来源读**。非法值（非 `^[A-Za-z0-9_]{1,16}$`）服务端会静默丢弃，不影响主流程。
...
```
此时不要重试，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s52` 手动登录注册。
...
**如果当前 api_key 来自 `$ZLBX_API_KEY`**：跳过 SID 流程，提示用户访问 `https://ai.zhiliaobiaoxun.com/?ch=s52` 手动登录充值。
...
def on_balance_exhausted(api_key, source):
    if source == "auto":
        ...
        print("https://ai.zhiliaobiaoxun.com/?ch=s52")
```
