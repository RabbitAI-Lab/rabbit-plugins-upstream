...
server.registerTool("redact", {
    ...
    description: "Replace patient identifiers and PII in text with labelled tokens " +
    ...
    inputSchema: {
        ...
        categories: z
            ...
            .optional()
            .describe("Which pattern sets to apply. Defaults to clinical + general. " +
    ...
    const cats = categories && categories.length ? categories : ["clinical", "general"];
...
server.registerTool("reinstate", {
    title: "Re-identify (restore originals)",
    description: "Reverse a redaction: replace tokens with their original values using a " +
...
server.registerTool("self_check", {
    title: "Self-check for residual identifiers",
    description: "Scan already-redacted text for anything that still looks like an " +
