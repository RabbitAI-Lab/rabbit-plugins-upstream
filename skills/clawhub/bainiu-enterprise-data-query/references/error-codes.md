...
批量导出企业信息时（通过 `upload_file.js` 上传文件），服务端可能返回以下状态码：

| 状态码 | 说明 | 处理建议 |
|-------|------|---------|
| 401 | 缺少必传参数 | 检查上传请求是否包含必需的文件参数 |
| 400 | 传入参数有误 | 检查参数格式是否正确 |
| 405 | 文件大小超过限制 | 压缩文件内容或拆分为多个小文件（单文件不超过5MB） |
| 406 | 文件类型错误 | 确保上传的是 .txt 文本文件 |
| 238 | 批量导出条数不足 | 可用导出条数少于文件中的企业数量，需减少导出的企业数量，并告知用户前往[白牛商查官网](https://skill.bainiudata.com/home)查看剩余导出条数或申请更多用量 |
| 239 | 查询的企业名称有误 | 文件中所有企业名称均无法匹配，需核对企业名称是否正确后重新上传 |
