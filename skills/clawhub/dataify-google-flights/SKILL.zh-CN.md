...
description: "当用户请求“调用 Google Flights”或“搜索航班价格/行程”，或明确提到航班查询字段时，触发 dataify-google-flights skill。"
---

# Dataify Google Flights 中文版
...
```bash
python3 scripts/google_flights.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/google_flights.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
