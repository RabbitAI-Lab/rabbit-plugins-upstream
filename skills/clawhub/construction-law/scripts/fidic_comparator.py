...
# Aliases for cross-script consistency (e.g. fidic-red → red)
FORM_ALIASES = {
    "fidic-red": "red",
    "fidic-yellow": "yellow",
    "fidic-silver": "silver",
    "fidic-emerald": "emerald"
}

def compare(forms, topic, fmt, output):
    form_list = [FORM_ALIASES.get(f.strip().lower(), f.strip().lower()) for f in forms.split(",")]
    for f in form_list:
        if f not in FORM_NAMES:
            print(f"Error: Unknown form '{f}'. Available: {', '.join(FORM_NAMES.keys())} (aliases: {', '.join(FORM_ALIASES.keys())})")
            sys.exit(1)
...
def main():
    ...
    parser.add_argument("--forms", help="Comma-separated forms: red,yellow,silver,emerald (or fidic-red etc)")
    ...
    parser.add_argument("--list-forms", action="store_true", help="List available FIDIC forms and exit")
    parser.add_argument("--list-topics", action="store_true", help="List available comparison topics and exit")
    args = parser.parse_args()

    if args.list_forms:
        print("Available FIDIC forms:")
        for k, v in FORM_NAMES.items():
            print(f"  {k:10s} — {v}")
        print("\nAliases (for consistency with other scripts):")
        for k, v in FORM_ALIASES.items():
            print(f"  {k:14s} → {v}")
        sys.exit(0)
    if args.list_topics:
        print("Available topics:")
        for k, v in COMPARISONS.items():
            print(f"  {k:12s} — {v.get('title', k)}")
        sys.exit(0)
    if not args.forms:
        parser.error("--forms is required (use --list-forms to see options)")
