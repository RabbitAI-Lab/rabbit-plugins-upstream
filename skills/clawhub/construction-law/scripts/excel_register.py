...
try:
    ...
    print("  or: pip3 install construction-law[excel]")
    sys.exit(1)
...
def get_obligations(form):
    import obligations_register as mod
...
def main():
    ...
    parser.add_argument("--form", required=True, choices=["fidic-red", "fidic-yellow", "fidic-silver", "psscoc", "sia", "nec4"])
    ...
    # Pre-flight: refuse to write half-empty workbooks.
    # Real coverage as of v2.6.0:
    #   obligations: fidic-red, psscoc, sia
    #   notices:     fidic-red, fidic-yellow, psscoc, sia, nec4
    OBLIGATIONS_FORMS = {"fidic-red", "fidic-yellow", "fidic-silver", "psscoc", "sia"}
    NOTICES_FORMS = {"fidic-red", "fidic-yellow", "fidic-silver", "psscoc", "sia", "nec4"}

    want_obl = args.type in ("obligations", "both")
    want_not = args.type in ("notices", "both")

    missing = []
    if want_obl and args.form not in OBLIGATIONS_FORMS:
        missing.append(f"obligations data not available for '{args.form}' (supported: {sorted(OBLIGATIONS_FORMS)})")
    if want_not and args.form not in NOTICES_FORMS:
        missing.append(f"notices data not available for '{args.form}' (supported: {sorted(NOTICES_FORMS)})")

    if missing:
        for m in missing:
            print(f"Error: {m}", file=sys.stderr)
        if args.type == "both":
            print("Hint: try --type notices (broader coverage) or pick a different --form.", file=sys.stderr)
        sys.exit(2)
