...
const API_KEY_JSON_FILE = path.join(CONFIG_DIR, 'api_key.json');
const SKILL_MD_FILE = path.join(SKILL_ROOT, 'SKILL.md');

// 封装打包时由服务端替换为对象；仓库模板为 null
const ENCAPSULATION_EMBEDDED = {"skill_key": "", "original_skill_key": "", "encapsulation_target": "prana"};
...
function persistFetchedApiKeyJson(publicKey, secretKey) {
  ...
  const payload = {
    code: 200,
    message: 'success',
    data: { api_key: { public_key: publicKey, secret_key: secretKey } },
  };
  fs.writeFileSync(API_KEY_JSON_FILE, `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
}
...
function loadSkillConfig() {
  ...
  const originalFm = String(frontmatter.original_skill_key || '').trim();
  const pubKeyFm = String(frontmatter.skill_key || '').trim();
  if (!original) original = originalFm;
  let pub = String((runtime && runtime.skill_key) || '').trim();
  if (!pub) pub = pubKeyFm;
...
function buildApiKeysFetchUrl(baseUrl) {
  ...
  const email = (process.env.PRANA_API_KEYS_EMAIL || process.env.EMAIL || '').trim();
  if (email) q.set('email', email);
  const phone = (
    process.env.PHONE_NUMBER ||
    process.env.PRANA_PHONE ||
    process.env.phone_number ||
    ''
  ).trim();
  if (phone) q.set('phone_number', phone);
  const qs = q.toString();
...
async function loadPranaCredentials(pranaBaseUrl) {
  ...
  if (fs.existsSync(API_KEY_JSON_FILE)) {
    const pj = parseCredentialsJson(fs.readFileSync(API_KEY_JSON_FILE, 'utf8'));
    if (pj) return pj;
  }
  ...
  const base = (pranaBaseUrl || DEFAULT_PRANA_BASE || '').trim();
  if (base && !autoFetchApiKeyDisabled()) {
    ...
    if (fetched) {
      ...
      if (truthyEnv('PRANA_SKILL_PERSIST_FETCHED_KEY')) {
        try {
          persistFetchedApiKeyJson(pub, sec);
        } catch (e) {
          console.error(`警告: 无法写入 config/api_key.json：${e && e.message ? e.message : e}`);
        }
      }
      return [pub, sec];
    }
  ...
  console.error(
    '错误: 未配置 API 凭证（public_key + secret_key），且自动 GET /api/v1/api-keys 失败或未启用。\n' +
      ...
      '  2) 保证 Prana 根 URL（脚本内 DEFAULT_PRANA_BASE）可访问，并确保未设置 PRANA_SKILL_NO_AUTO_API_KEY；' +
      '可选 ACCOUNT_ID / PRANA_ACCOUNT_ID、PRANA_API_KEYS_EMAIL / EMAIL、PHONE_NUMBER 等查询参数；成功后默认写入 config/api_key.txt（' +
      '若不想写盘可设 PRANA_SKILL_SKIP_WRITE_API_KEY=1；另存 JSON 可设 PRANA_SKILL_PERSIST_FETCHED_KEY=1）。',
  );
  process.exit(2);
}
...
async function main() {
  ...
  try {
    ...
    if (values.help) {
      console.log('用法: node scripts/prana_skill_client.js -m "用户消息" [-t thread_id]');
      process.exit(0);
    }
    if (!values.message) {
  ...
  const baseUrl = DEFAULT_PRANA_BASE.trim();
