...
description: Full access to Exchange 2010/2013 EWS for managing emails, folders, attachments, calendar events, contacts, tasks, and out-of-office settings.
version: 1.1.0
author: pes0 (modified for enn.cn)
topics:
  - Calendar
  - Email
  - Exchange
...
# Exchange 2010/2013 EWS Skill

Exchange EWS integration for emails, calendar, contacts, and tasks.
...
1. **邮箱密码**：使用密码登录 EnnewClaw 时，密码会自动同步到 Exchange
2. **邮箱地址**：首次使用邮箱功能时，系统自动通过 DWS 获取你的企业邮箱地址
3. **开始使用**！直接使用邮箱、日历等功能即可

### 密码验证失败

如果自动同步的密码与邮箱密码不一致，使用时会提示：

> Exchange 邮箱密码验证失败。请在对话中提供正确的邮箱密码，我会更新配置后重新尝试连接。

提供正确密码后，调用 `update_exchange_password` 即可更新：

```python
from skills.exchange2010 import update_exchange_password
result = update_exchange_password('correct-password')
```

## Setup (English)

Credentials are auto-configured:

1. **Password**: Synced automatically when you log in to EnnewClaw with your password
2. **Email**: Auto-discovered from DWS on first use
3. No manual configuration needed!

### If password verification fails

If the saved password doesn't match your email password, you'll be prompted to provide the correct one. Use `update_exchange_password` to update it:
...
| Function | Description |
|----------|-------------|
| `update_exchange_password(new_password)` | Update saved password when verification fails |
...
- **28 functions** available in total (including credential management)

## Security Considerations

⚠️ **Important**: This skill requires live Exchange credentials and provides broad mailbox access.

- Use least-privilege credentials
- Verify credential configuration before use
- Protect the `.env.credentials` file
- Require explicit confirmation before sending mail or modifying Exchange data
- Restrict attachment downloads to a safe directory with sanitized filenames
