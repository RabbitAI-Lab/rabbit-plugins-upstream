...
As of v0.6.0, Memory Crystal handles Layers 1-4 (Dream Weaver is now integrated via `crystal dream-weave`). Layer 5 is partially automated via CONTEXT.md generation. All layers work together.
...
|----------|------|---------------|------------|
| `crystal.db` (`~/.ldm/memory/`) | ~2.5 GB, 207,518 chunks | All conversations + files + manual memories | Memory Crystal (our product) |
| `main.sqlite` (`~/.openclaw/memory/`) | 6.1 GB | Workspace files + sessions | OpenClaw core (not ours) |

**crystal.db is the one database for all conversation memory.** As of 2026-03-04, context-embeddings has been migrated into crystal.db (12,206 chunks, $0 cost) and all 801 raw JSONL transcript files have been backfilled (24,180 chunks, ~$0.24 cost). The context-embeddings plugin is disabled. main.sqlite stays (OpenClaw built-in, not ours to touch).
...
## What's Built (as of v0.6.1, 2026-03-05)
