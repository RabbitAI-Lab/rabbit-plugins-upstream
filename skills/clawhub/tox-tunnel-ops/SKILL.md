...
- One `bootstrap_mode: lan` daemon per host: LAN-discovery announces target
  UDP 33445 only, so a second instance (bound 33446+) is never discovered.
- **Avoid the v0.4.8 Linux packages**: a release-toolchain `fs::path` bug made
  the daemon create `tox_save.dat` as a directory, losing the server identity
  on every restart (deb/rpm only; macOS/Windows unaffected). Fixed in v0.4.9,
  which also self-heals the leftover directory.
...
11. **Use `print-id` for Tox ID sharing.** When users need to transfer a Tox ID between machines, suggest `toxtunnel print-id --data-dir <dir> --qr` to generate a QR code that can be scanned with a phone camera. Do not use `-c` for this subcommand. **For a service-managed daemon, run it AS the service account** (`sudo -u toxtunnel toxtunnel print-id -d /var/lib/toxtunnel` on Linux) or read the ID from the service log — an identity created by root/an Administrator is unreadable to the service account (0600 / owner-only DACL), and pre-0.4.9 daemons then silently minted a different identity (v0.4.9+ refuses to start instead; its Windows DACL also admits SYSTEM+Administrators).
