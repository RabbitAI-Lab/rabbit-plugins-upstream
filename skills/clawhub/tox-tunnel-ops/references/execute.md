...
> **Windows over SSH:** processes launched from a Win32-OpenSSH session
> (even via `Start-Process -WindowStyle Hidden`) are killed when that SSH
> session closes. For a daemon that must outlive the session, register the
> SCM service (`install-windows-service`, Step 3) or run it via a scheduled
> task: `Register-ScheduledTask` + `Start-ScheduledTask`.
