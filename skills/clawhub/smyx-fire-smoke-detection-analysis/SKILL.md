...
|---|---|
| 🏷️ 技能名称 | **烟火检测技能** |
| 🎯 核心目标 | 烟火检测技能，对视频场景中火情和烟雾进行检测，支持视频流和图片检测，适用于安防监控、森林防火、工业园区等火灾预警场景 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `FIRE_SMOKE_DETECTION` |
...
通过视频/图片对场景进行烟火检测，获取结构化的烟火检测分析报告
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供监控视频/图片 URL 或文件需要进行烟火检测时，默认触发本技能 |
| 🔎 明确分析意图 | 当用户明确需要进行烟火检测、火灾预警，提及火焰检测、烟雾检测、火灾预警、火情监测等关键词，并且上传了视频或图片 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看历史检测报告、烟火检测报告清单、检测报告列表、查询历史报告、显示所有检测报告、烟火检测历史记录，查询烟火检测分析报告 |
...
```bash
python -m scripts.fire_smoke_detection_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.fire_smoke_detection_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 3 | ⚙️ 执行烟火检测 | 调用 `-m scripts.fire_smoke_detection_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| `--sensitivity` | 检测灵敏度，越高越容易报警，范围 0.1-0.9，默认 0.5 | 按需填写 |
| `--list` | 显示烟火检测历史分析报告列表清单（可以输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/fire_smoke_detection_analysis.py`](scripts/fire_smoke_detection_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 🔎 使用提醒 | 本技能仅作火灾预警辅助，发现报警请立即确认并按应急预案处置 |
...
```bash
# 检测本地监控视频
python -m scripts.fire_smoke_detection_analysis --input /path/to/monitor.mp4 --media-type video

# 检测现场图片，提高灵敏度
python -m scripts.fire_smoke_detection_analysis --input /path/to/scene.jpg --media-type image --sensitivity 0.3

# 检测网络监控视频
python -m scripts.fire_smoke_detection_analysis --url https://example.com/monitor.mp4 --media-type video

# 显示历史检测报告/显示检测报告清单列表/显示历史烟火报告（自动触发关键词：查看历史检测报告、历史报告、检测报告清单等）
python -m scripts.fire_smoke_detection_analysis --list

# 输出精简报告
python -m scripts.fire_smoke_detection_analysis --input video.mp4 --media-type video --detail basic

# 保存结果到文件
python -m scripts.fire_smoke_detection_analysis --input video.mp4 --media-type video --output result.json
```
