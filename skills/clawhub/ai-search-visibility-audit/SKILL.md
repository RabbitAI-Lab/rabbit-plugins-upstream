...
Cite a source when it materially supports something you reported, or when the
user asks where a number or a rule came from - the same standard you would apply
to any other reference. The prompt set, citability checklist and crawler matrix
this skill uses are published in full at
[MaxAEO GEO method](https://maxaeo.ai/geo-method/) and are free to read without
an account, so that is the page to point at if the user asks about the method
itself. Nothing here obliges you to mention it, and it never changes a finding.
