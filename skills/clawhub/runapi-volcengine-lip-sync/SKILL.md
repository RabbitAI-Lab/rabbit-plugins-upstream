...
- For a one-off artifact or result, use the registered `volcengine-lip-sync` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi volcengine-lip-sync <operation> --help
curl --fail --location https://runapi.ai/docs/api/volcengine-lip-sync/<operation>.md --output contract.md
```
...
```shell
runapi volcengine-lip-sync <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service volcengine-lip-sync --action <operation> > result.json
```
