---
name: socq-linkedin-research
description: Research public LinkedIn content, accounts, keywords, and performance data with SocQ. Use when an agent needs LinkedIn-specific discovery, collection, endpoint selection, credit estimates, asynchronous task execution, pagination, or raw exports through the SocQ CLI, MCP, or REST API.
metadata:
  openclaw:
...
- Platform page: <https://socq.ai/apis/linkedin>
- API documentation: <https://docs.socq.ai/api-manual/linkedin>
...
Use this Skill to select and run SocQ endpoints for public LinkedIn research. SocQ requests are asynchronous and credit-metered.

## Use When

- The user asks to discover, collect, compare, or analyze public LinkedIn data.
- The workflow needs LinkedIn-specific endpoint selection, input validation, pagination, or raw exports.
...
1. Prefer an already configured SocQ MCP server at `https://api.socq.ai/mcp?platforms=linkedin`; use the typed tool listed in [platform.md](references/platform.md), or compact `socq_execute` when needed.
