---
name: customs-company-area-stats-zh
description: "查询公司海关贸易区域维度统计数据 — 分析贸易量、金额、月度趋势和国家分布，覆盖全球市场。\n\nTrigger: 海关贸易区域统计，区域贸易分析，公司进出口国家分布，全球贸易情报，供应商市场覆盖"
...
通过跨境魔方开放平台API，查询公司按区域/国家维度聚合的海关贸易统计数据。

## 概述

本技能提供从跨境魔方海关数据库中查询公司贸易区域维度统计数据的能力。给定公司ID和公司类型，返回按区域维度的贸易汇总数据，包括总交易次数、总金额、总数量、总重量、月度贸易趋势和国家分布。
...
**重要提示**：始终使用直接的脚本调用方式，如 `python scripts/customs_company_area_stats.py`。**不要使用** `cd scripts && python customs_company_area_stats.py` 这种复合命令。

### 公司贸易区域维度统计查询（`customs_company_area_stats.py`）
- **返回粒度**：每个公司一条记录，包含区域维度的聚合贸易统计数据
- **使用场景**：查看公司按国家/地区的贸易分布，分析市场覆盖情况和区域贸易量
- **示例**：
  - "查询公司100001作为供应商的区域贸易统计"
  - "查看公司100001作为采购商的区域贸易分布"
- **参数说明**：参见 [公司贸易区域维度统计API](references/customs-company-area-stats-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/customs/company/area/stats","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"公司贸易区域维度统计查询失败，服务端异常"}'
```
...
| 用户意图 | 使用API |
|---------|--------|
| "查询公司100001作为供应商的区域贸易统计" | 公司贸易区域维度统计查询 |
| "查看公司100001的贸易国家分布" | 公司贸易区域维度统计查询 |

## 使用示例

### 查询公司贸易区域维度统计

**用户请求**："查询公司100001作为供应商的区域贸易统计"
```bash
...
- 公司贸易区域维度统计：查看 [references/customs-company-area-stats-api.md](references/customs-company-area-stats-api.md)
...
1. **检查API文档**：
   ...
   - 查看 [references/customs-company-area-stats-api.md](references/customs-company-area-stats-api.md)
   - 不要猜测参数名称，从文档中获取准确的参数名称和格式

2. **数据解读**：
   - `countTrade` 是总交易次数
   - `tradeDates` 显示月度活跃贸易月份
   - `tradeCountries` 显示国家级别的分布及占比
   - `percentTrade` 是贸易占比（百分比数值，如21.05表示21.05%）

3. **跨技能使用**：
   - 结果中的公司ID可用于 **customs-company-area-list** 获取区域详细列表
   - 公司ID也可用于 **customs-company-trends** 获取月度贸易趋势
