---
name: education_filing_form_ocr
description: 支持学历证书电子注册备案表识别,提取姓名、更新日期、入学日期、证件号码、院校、学历类别及专业等。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
本技能封装了学历证书电子注册备案表识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取学历证书电子注册备案表里核心信息。

## 功能特性

- **学历证书电子注册备案表识别**：支持识别学历证书电子注册备案表信息，识别提取姓名、更新日期、入学日期、证件号码、院校、学历类别及专业等。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• ONLINE_VERIFICATION_REPORT_OF_HE（学历证书电子注册备案表）|
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/education_filing_form_ocr/scripts/main.py ONLINE_VERIFICATION_REPORT_OF_HE /path/to/test.jpg
```
...
- “帮我识别这张图像中的学历证书电子注册备案表信息，图片在 /Users/name/Downloads/test.jpg”
