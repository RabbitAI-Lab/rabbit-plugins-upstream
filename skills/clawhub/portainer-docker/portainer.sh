...
api_get() {
    curl -s -k -H "$AUTH_HEADER" "$API$1"
}

api_post() {
    curl -s -k -X POST -H "$AUTH_HEADER" -H "Content-Type: application/json" "$API$1" -d "$2"
}

api_put() {
    curl -s -k -X PUT -H "$AUTH_HEADER" -H "Content-Type: application/json" "$API$1" -d "$2"
}
