#!/usr/bin/env python3
"""实时查询 AliExpress 站点并按国家、地区代码或站点 ID 解析。"""
...
from aliexpress_search_common import DEFAULT_BASE_URL, sanitize_site
from geekbi_auth import ActionRequired, authenticated_json_request, response_message


ENDPOINT = "/api/v1/aliexpress/site/ai-list"
...
def validate_response(payload):
    ...
    if payload.get("code") != 0:
        raise ValueError(response_message(payload, "AliExpress 站点查询失败"))
...
def resolve_site(sites, country):
    ...
    for site in sites:
        ...
        normalized_candidates = {normalize_name(value) for value in candidates if isinstance(value, str)}
        ...
        matches.append(site)
    if not matches:
        raise ValueError("未找到该 AliExpress 站点，请确认国家中文名、英文名或地区代码")
    return matches


def main():
    parser = argparse.ArgumentParser(description="实时解析 AliExpress 站点")
