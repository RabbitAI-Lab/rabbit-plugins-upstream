...
> **亚马逊消费者洞察 · 深度需求报告**
> 技术名 `amazon-consumer-insight`，ARI 官方出品的 Amazon 评论采集与消费者洞察 Skill，已适配 WorkBuddy。
...
```text
使用 $amazon-consumer-insight 为 ASIN B0XXXXXXXX 生成 VOC 报告，站点 amz_us，我确认扣积分。
```
...
保持文件夹名 `amazon-consumer-insight`，整个放进对应作用域即可：

| 作用域 | 路径 |
|---|---|
| 用户级（推荐） | `~/.workbuddy/skills/amazon-consumer-insight/` |
| 项目级 | `<项目目录>/.workbuddy/skills/amazon-consumer-insight/` |
