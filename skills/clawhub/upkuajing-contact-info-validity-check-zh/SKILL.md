---
name: upkuajing-contact-info-validity-check-zh
description: 核验邮箱、手机号及其余联系方式的真实有效性，在开展客户触达前筛选无效数据，提升联系信息准确度，降低消息无效发送概率，服务外贸获客、CRM 数据清洗和销售线索核验工作。
...
使用跨境魔方开放平台API检测联系方式的有效性。本技能提供三种检测接口：电话有效性、邮件有效性、域名有效性。

## 概述

本技能通过三个脚本提供对跨境魔方联系方式有效性检测服务的访问：
- **电话有效性检测** (`phone_validity_check.py`): 检测电话号码的有效性、类型及WhatsApp状态
...
**重要**：始终使用直接脚本调用，如 `python scripts/phone_validity_check.py`。**不要使用** shell 复合命令如 `cd scripts && python phone_validity_check.py`。

## 三种检测API

### 电话有效性检测 (`phone_validity_check.py`)

检测电话号码的有效性、类型及WhatsApp状态。

**参数**：查看参数说明 [电话有效性检测](references/phone-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/validation/email","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"联系方式有效性检测失败，服务端异常"}'
```
...
- 电话有效性检测：查看 [references/phone-api.md](references/phone-api.md)
