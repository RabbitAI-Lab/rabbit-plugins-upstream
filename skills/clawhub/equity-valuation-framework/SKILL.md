...
Portfolio handoff contract:
- Always include `Trigger to add risk`, `Trigger to reduce risk`, `Invalidation`, `Horizon`, `Confidence`, and `Data gaps` when the output may feed `portfolio-risk-manager`.
- If any of these fields cannot be supported by available evidence, include the field anyway and state the missing input.
