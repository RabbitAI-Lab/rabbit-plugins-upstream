...
def compile_panel_prompt(panel_plan: Dict[str, Any], style: Dict[str, Any], panel: Dict[str, Any], clean_suffix: str) -> Dict[str, Any]:
    ...
    # text_budget advisory: flag panels whose dialogue text may overflow readability
    text_budget = panel.get("text_budget")
    text_budget_warning = None
    if text_budget:
        total_chars = sum(len(d.get("text", "")) for d in panel.get("dialogue", []))
        if total_chars > text_budget:
            text_budget_warning = f"dialogue text ({total_chars}) exceeds text_budget ({text_budget}); consider shortening or moving text out of the image"
    ...
    return {
        ...
        "text_budget_warning": text_budget_warning,
    }
