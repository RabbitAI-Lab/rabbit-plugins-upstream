...
- `examples/webtoon-demo.json`：竖屏条漫示例

#### 真实成品漫画

以下是用本 skill 实际生成的成品图（两篇日记复用同一个主角）：

| 示例 | 内容 |
|------|------|
| `examples/climbing-diary-final.jpg` | 6格(2×3)日记漫画：第一次室内攀岩 |
| `examples/ocean-park-diary-final.jpg` | 10格(2×5)日记漫画：极地海洋公园+大马戏的一天 |
...
上面两篇漫画用的是**同一个角色设定**（短发圆框眼镜女生），只是随季节换了服装（攀岩穿卫衣、夏天穿T恤短裤）。复用同一个 character bible，会让个人日记漫画读起来像一部连贯的"系列剧"（第1集、第2集……）。这是 v2.1 的一等工作流。
...
- `scripts/assemble_page.py`
  - 把渲染好的各格图片拼成最终页/画布，可选叠加中文旁白
...
- `examples/webtoon-demo.json`: vertical scroll (webtoon) example

#### Real finished comics

These are actual outputs produced with this skill (same protagonist across two diary entries):

| Example | What it shows |
|---------|---------------|
| `examples/climbing-diary-final.jpg` | A 6-panel (2x3) diary comic: first-time indoor climbing |
| `examples/ocean-park-diary-final.jpg` | A 10-panel (2x5) diary comic: a day at a polar ocean park + circus show |
...
Both comics above use the **same character bible** (a short-haired woman with round glasses) — only the outfit changed with the season (hoodie in climbing, T-shirt + shorts in summer). Reusing one character bible makes a personal diary comic read like a coherent "series" (episode 1, episode 2, ...). This is a first-class workflow in v2.1.
...
- `scripts/assemble_page.py`
  - assembles rendered panel images into a final page/canvas with optional Chinese caption overlay
