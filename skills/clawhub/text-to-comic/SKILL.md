...
name: text-to-comic
description: |
  Turn text, photo notes, diary entries, poems or knowledge content into comics, picture-book spreads, infographics, or hybrid visual pages via structured storyboard, panel-by-panel rendering, validation and assembly. Triggers: comic, webtoon, manga, picture book, infographic, storyboard, 漫画, 绘本, 条漫, 分镜, 信息图, 日记转漫画.
  将用户文字、照片说明或知识内容转化为漫画、绘本或信息图；先做结构化 storyboard/panel plan，再逐格出图、校验与拼版。
...
- `no_numbering`: panel must NOT contain embedded page/panel numbering like "4/6" or "Part 3"
- `no_watermark`: panel must NOT contain a baked-in corner watermark or AI-signature (e.g. "AI generated WORKBUDDY")
- readable composition
- dialog within `text_budget`
- no obviously broken face or limb rendering

> Note: Image models sometimes add panel numbering or a corner watermark even when the clean-image suffix is present. These are dedicated checks (`no_numbering`, `no_watermark`), not just a prompt suffix. If either fails, retry that panel before assembling.
