...
downstream as verified fact without explicit gate passage. The `ars-deep-research`
skill operates here.
...
is synthetic or public-safe — evaluation harnesses
...
|---|---|
| Source verification (S2 API) | `ars-deep-research/references/guides/semantic_scholar_api_protocol.md` |
| Anti-leakage protocol | `ars-academic-paper/references/guides/anti_leakage_protocol.md` |
| Integrity gates (Stage 2.5/4.5) + 7-mode failure checklist | `ars-pipeline-orchestrator/references/guides/ai_research_failure_modes.md` |
| Reviewer calibration mode (FNR/FPR with private gold set) | `ars-academic-reviewer/references/guides/calibration_mode_protocol.md` |
| Cross-model verification | `references/shared/cross_model_verification.md` |
| Declarative posture | `references/shared/handoff_schemas.md` (`data_access_level` and `task_type` sections) |
...
annotation, and CI lint via 自动校验脚本（移植版未附带，以文字规则为准）。 That script
