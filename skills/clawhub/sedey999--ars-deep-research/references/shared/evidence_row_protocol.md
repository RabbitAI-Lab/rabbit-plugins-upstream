...
field shape is `references/shared/contracts/evidence/evidence_row.schema.json`; cross-field
semantics are implemented by 自动校验脚本（移植版未附带，以文字规则为准）; the frozen decision is
对应设计文档（移植版未附带）。
...
The canonical display labels come only from 自动校验脚本（移植版未附带，以文字规则为准）。 “Verified”
...
A positively identified pre- Schema 5 report may omit `evidence_rows`; use
...
validation, cache replay, and rendering never call , never write a
...
## Version 1.1 (`evidence-row/1.1`) authority-profile advisory extension

`references/shared/contracts/evidence/evidence_row_v1_1.schema.json` is a separate closed
...
`references/shared/contracts/human_subjects/content_coverage_advisory.schema.json` and
`references/shared/authority_content_coverage_advisory_protocol.md`. Their
labels remain `LLM-ADVISORY`; neither an exact passage nor a checked-no-match
state changes  deterministic status, readiness, authorization, or
institutional acceptance, and neither is an adequacy or efficacy finding.

## Version 1.2 cross-document consistency extension

`references/shared/contracts/evidence/evidence_row_v1_2.schema.json` is a separate closed
...
自动校验脚本（移植版未附带，以文字规则为准）。 It does not widen either
earlier schema or enter the Phase E Integrity Report. The 1.0 and 1.1 schemas,
自动校验脚本（移植版未附带，以文字规则为准） behavior, cache semantics, rendering, and serialized
...
## Version 1.3 claim-standing advisory surface

`references/shared/contracts/evidence/evidence_row_v1_3.schema.json` defines
`evidence-row/1.3`, surface `claim_standing_advisory`: one provenance-only row
per (probe claim, selected work-family candidate) in the  claim-standing
