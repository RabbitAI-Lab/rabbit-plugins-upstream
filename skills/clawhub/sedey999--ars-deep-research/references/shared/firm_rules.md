...
**Related mechanisms:** the Phase Boundary enforcement-status sentence (factual status prose, not a behavioral firm rule) is single-sourced as `CANONICAL_ENFORCEMENT` inside 自动校验脚本（移植版未附带，以文字规则为准） rather than here — same defrift goal, separate mechanism co-located with its only consumer.
...
- `R-CIM-*` = **Claim Intent Manifest emission** rules (origin:  claim-alignment spec; the agent prompts borrowed `R-L3-2-A/B/C` until v3.10 PR-A renamed them to remove the collision).
...
- `ars-academic-paper/references/formatter_agent.md` — R-L3-2-A + R-L3-2-E (in the contamination pass-through paragraph).
- `ars-deep-research/references/guides/crossref_api_protocol.md` — R-L3-2-A (user-discretion reference).
- `ars-deep-research/references/guides/openalex_api_protocol.md` — R-L3-2-A reference.
- `ars-pipeline-orchestrator/references/pipeline_orchestrator_agent.md` — R-L3-2-C / R-L3-2-D / R-L3-2-E (finalizer logic).
- `ars-deep-research/references/bibliography_agent.md` — R-L3-2-B (ingest-time computation).
- `references/shared/bibliographic_integrity_signals.md`, `ars-deep-research/references/ethics_review_agent.md`, and 对应设计文档（移植版未附带） — R-L3-2-F.
...
- `ars-deep-research/references/synthesis_agent.md` (`<AGENT>` = "synthesis agent")
- `ars-deep-research/references/report_compiler_agent.md` (`<AGENT>` = "compiler"; R-CIM-B carries an extra standalone-mode sentence — see prompt)
- `ars-academic-paper/references/draft_writer_agent.md` (`<AGENT>` = "writer")
- `references/shared/contracts/passport/claim_intent_manifest.schema.json` — references `R-CIM-A` by ID in the `claims` field description.

---

## Collision guard

自动校验脚本（移植版未附带，以文字规则为准） additionally asserts:
