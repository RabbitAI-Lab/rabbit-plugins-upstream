...
`references/shared/contracts/passport/bibliographic_integrity_signal.schema.json` is the
...
additive migration carrier. Version 1.1 is the  retraction-status policy
cutover. Both preserve the one-advisory-token reference-marker grammar.
Version 1.2 is the  tortured-phrase advisory profile; it is additive and
...
## Tortured-phrase advisory profile (v1.2 / )
...
## Retraction authority cutover (v1.1 / )
...
2. **Dual write:** updated producers append canonical v1 records while still
   ...
   behavior. 自动校验脚本（移植版未附带，以文字规则为准） defines the
...
4. **Policy cutover:**  cuts over only `retraction_status` through v1.1 and
