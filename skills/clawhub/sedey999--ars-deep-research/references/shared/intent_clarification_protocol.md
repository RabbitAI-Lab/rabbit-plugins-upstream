...
**Spec:** v3.9.2 hot-fix per 对应设计文档（移植版未附带）
**Issue:**  (phase scope inflation hot-fix)
**Forward note:** v3.10 active conductor will replace this clarification gate with structured intake + task envelope dispatch. v3.9.2 ships clarification-only as interim.

## Purpose

When ARS receives input that does not unambiguously identify the intended workflow, the main session **clarifies before dispatching any agent**. Auto-routing to a single-phase agent on cross-phase materials caused  (silent scope inflation).

This protocol defines:

1. When clarification fires (3 trigger classes)
2. How the clarification message is structured (a-d options in markdown body, NOT platform question tool tool)
...
|---|---|---|
| User explicitly names a skill or mode | Explicit | Route directly to the named skill/mode; no clarification |
...
| 4 — Composition | Full draft, abstract, report body | `phase4_*/`; `draft.md`; `abstract.md` (or 5b for ars-academic-paper) |
...
(a) **Full pipeline** — go from materials through to a complete deliverable (research + write + review + revise). Use `ars-pipeline-orchestrator` orchestrator (`full` 模式).
(b) **<phase-specific workflow 1>** — <one-line description>. Use `<skill>` (name the mode explicitly).
(c) **<phase-specific workflow 2>** — <one-line description>. Use `<skill>` (name the mode explicitly).
...
- Do NOT call `platform question tool` tool — options live in the markdown body of the chat response (per `feedback_no_askuserquestion_use_brainstorm.md`)
...
- `[direct-mode] `lit-review` 模式 on these papers` → dispatches ars-academic-paper lit-review mode on the provided papers, no clarification
...
>
> (a) **Full paper from existing scaffolding** — use your abstract as paper seed, expand literature into bibliography, draft full paper, then review/revise. Use `ars-pipeline-orchestrator` (`full` 模式).
> (b) **Lit-review only** — produce an annotated bibliography matrix from the literature; don't touch your abstract. Use `ars-academic-paper` (`lit-review` 模式).
> (c) **Quality-check the abstract against the literature** — verify abstract claims are supported by the cited sources. Use `ars-academic-reviewer` (`citation-check` 模式).
> (d) **Something else** — let me know what you're trying to do.
>
> Pick a-d, or describe the target deliverable. If you want me to dispatch a specific agent directly without this clarification, prefix your message with `[direct-mode]`.
...
**Main session response:** Routes directly to `ars-academic-paper `lit-review` 模式 on the provided PDFs. No clarification fires (explicit intent overrides materials check).
...
>
> (a) **I have an idea but no clear research question** — Socratic mode to clarify direction. Use `ars-deep-research` (`plan` 模式).
> (b) **I have a question and need to do the research** — full research pipeline. Use `ars-pipeline-orchestrator` (`full` 模式).
> (c) **I have research notes and need to write the paper** — drafting pipeline. Use `ars-academic-paper`.
> (d) **I have a draft and want feedback** — peer-review simulation. Use `ars-academic-reviewer` (对应模式 equivalent).
>
...
- **Within a skill that has already been routed.** If `ars-pipeline-orchestrator` is already running and the orchestrator is dispatching Phase 3 → Phase 4 internally, this protocol does not fire on every internal dispatch. It governs the **entry point** of an ARS session, not in-pipeline transitions.
...
When the active conductor architecture ships, this protocol's logic moves into the conductor's intake stage with structured task envelope dispatch. The conductor will:
