...
- `phase2_investigation/version_records.yaml` (academic citation version-family evidence for preprint -> proceedings -> journal chains; Kong )
...
The downstream 自动校验脚本（移植版未附带，以文字规则为准） verifier looks up `confidence` for each `<!--ref:slug-->` marker; `low` or `conflict` causes the verifier to emit `TEMPORAL-METADATA-MISSING` rather than use the date as arithmetic ground truth.
...
3. Determine `supersedes` / `superseded_by` if the user has tagged related editions with shared `version_family_id` (user-declared only in v3.9.4 stub; Kong  adds academic citation version-family candidate discovery in `version_records.yaml`, not corpus mutation).
...
- `references/shared/contracts/passport/timeline.schema.json` (aggregate-level with `$defs`)
- `references/shared/contracts/passport/citation_provenance.schema.json` (aggregate-level)
- `references/shared/contracts/passport/version_records.schema.json` (aggregate-level academic citation version-family sidecar)
- Temporal sidecars are validated by 自动校验脚本（移植版未附带，以文字规则为准）; `version_records.schema.json` is covered by 自动校验脚本（移植版未附带，以文字规则为准）。
