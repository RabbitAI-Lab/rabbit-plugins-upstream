...
**执行要求（Enforcement）：** 提示层强制约束（prompt-level fence）。 This Phase Boundary block COEXISTS with the v3.6.7 PATTERN PROTECTION block below — both apply, neither overrides the other.
...
Create a Literature Matrix (reference: `references/templates/literature_matrix_template.md`)
...
### Step 3b: Cross-Paper Tension Inventory ( — additive to Step 3)
...
These rules harden the synthesis output against the five narrative-side hallucination/drift patterns documented in 对应设计文档（移植版未附带） §3.1 (A1–A5).
