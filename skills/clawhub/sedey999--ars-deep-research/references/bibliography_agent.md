...
You MUST NOT:
- WRITE files in `phase{M}_*/` directories where M ≠ 2 (no inflate into Phase 3 synthesis, Phase 4 drafting, Phase 5 review, Phase 6 revision — **this is the exact  failure pattern**)
...
`references/shared/ground_truth_isolation_pattern.md` § 2A.
...
Reference: `references/guides/semantic_scholar_api_protocol.md`
...
### Step 4.6: Distributional Skew Advisory (Kong )
...
**Backpointer**: see [`ars-pipeline-orchestrator/references/guides/literature_corpus_consumers.md`](ars-pipeline-orchestrator/references/guides/literature_corpus_consumers.md) for the full consumer protocol, BAD/GOOD examples, and shared template.
...
3. **NEVER emit `human_read_source` or `human_read_at` on the entry.** Those keys are USER-OWNED and live in the §3.6 peer file `<session>_human_read_log.yaml`, set only by the user-issued  <citation_key> --scope <level>` command. The entry schema is `additionalProperties: false` and adapter-owned (per `ars-pipeline-orchestrator/references/guides/literature_corpus_consumers.md`); emitting these keys from `bibliography_agent` would mutate `literature_corpus[]` and break the v3.6.5 corpus-consumer protocol. The orchestrator joins the peer file at frontmatter-read time to derive the human-read signal.
...
Compute via the existing Semantic Scholar API lookup protocol (`references/guides/semantic_scholar_api_protocol.md`). The check runs as part of Step 4.5 Semantic Scholar Deduplication (same API call, additional signal).
...
- `openalex_unmatched` — per `ars-deep-research/references/guides/openalex_api_protocol.md`
- `crossref_unmatched` — per `ars-deep-research/references/guides/crossref_api_protocol.md`
...
**Omission reason-provenance ( Part A):** every field omitted BECAUSE OF API degradation is recorded in the entry's optional `contamination_signal_omissions` object with reason `api_degraded` (e.g. `contamination_signal_omissions: {openalex_unmatched: "api_degraded"}`) — otherwise a degraded lookup is indistinguishable from "never computed". Record ONLY degradation-caused omissions: the manual exemption is derivable from `obtained_via='manual'` (and the schema forbids the object on manual entries), so it is never recorded. A signal key never appears in both `contamination_signals` and `contamination_signal_omissions` (schema-enforced mutual exclusion). When no lookup degraded, omit the object entirely. Schema: `references/shared/contracts/passport/literature_corpus_entry.schema.json`; registry row: `contamination_signal_api_degradation` in 降级状态登记规则（以本节文字为准）。
...
normalized envelopes to 自动校验脚本（移植版未附带，以文字规则为准）。 Append its schema-valid
...
## Tortured-Phrase Advisory Production

This  path is a deterministic local metadata enricher, not a search or source-retrieval path. Given an exact local `literature_corpus[]` passport plus an explicit snapshot/manifest pair, invoke 自动校验脚本（移植版未附带，以文字规则为准） enrich-passport` with distinct input and output paths. The command writes a separate passport, preserves every other signal and legacy row, and leaves consumers read-only; never update a passport in place. Consumers validate and render the resulting rows but do not re-run the matcher.
...
**Boolean**: ...
**Date Range**: ...
**Last Searched**: [ISO date the search was executed — Schema 2 `last_searched_at`]
**Inclusion Criteria**: ...
**Exclusion Criteria**: ...
