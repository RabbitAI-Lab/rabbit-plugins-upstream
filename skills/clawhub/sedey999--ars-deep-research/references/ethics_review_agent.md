...
3. **Retraction-status authority**: For journal articles, consume the canonical v1.1 `bibliographic_integrity_signals[].retraction_status` row produced by the citation gate
...
- [ ] Any displayed  candidate rule trace is exact-replay validated and surface-linted, preserves unresolved predicates and authority anchors, and is not used as a result or action source
...
- [ ] Data terminology is used according to `references/shared/irb_terminology_glossary.md`; anonymity, pseudonymization, and de-identification are not treated as universal synonyms
...
judgments; never open packet paths, scan siblings, select a profile, or alter the
 manifest. A missing or unavailable string is `not_checked` with null
...
自动校验脚本（移植版未附带，以文字规则为准）。 Preserve `LLM-ADVISORY`,
...
## References
- `references/guides/ethics_checklist.md`
- `references/guides/irb_decision_tree.md` — portable navigation only
- `references/shared/human_subjects_authority_protocol.md` — exact selection, replay, and consumer rules
- `references/shared/human_subjects_authority_registry.json` — bounded actor/scope-tagged requirements
- `references/shared/contracts/human_subjects/resolved_authority_context.schema.json` — pointer-only result shape; schema alone is not replay validation
- `references/shared/review_pathway_rule_trace_protocol.md` —  candidate ownership, exact predicate replay, renderer/lint, and non-consumer rules
- `references/shared/contracts/human_subjects/review_pathway_rule_trace.schema.json` — closed candidate-only trace shape; schema alone is not replay validation
- `references/shared/submission_packet_manifest_protocol.md` — deterministic packet structure, replay, and status boundaries
- `references/shared/contracts/human_subjects/submission_packet_manifest.schema.json` — pointer-only  manifest shape; schema alone is not replay validation
- `references/shared/authority_content_coverage_advisory_protocol.md` —  draft, replay, aggregation, and noninterference rules
- `references/shared/contracts/human_subjects/content_coverage_advisory.schema.json` — closed final advisory and draft definition; schema alone is not replay validation
- `references/shared/contracts/evidence/evidence_row_v1_1.schema.json` — bounded requirement/expectation/artifact passage carrier
