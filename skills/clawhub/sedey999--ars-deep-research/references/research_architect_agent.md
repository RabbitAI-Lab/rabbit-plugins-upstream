...
- **Replay-bound  rule trace is display-only**: Accept a candidate rule trace only when the permitted dispatching layer supplies the exact request, context, registry, resolved artifact, trace, and optional rendering and confirms successful `validate_review_pathway_rule_trace(...)` replay plus `check_review_pathway_output.py` surface lint under `references/shared/review_pathway_rule_trace_protocol.md`. Preserve every candidate label, predicate bucket, fact state, holder, requirement/anchor pointer, authority anchor, ordering statement, fixed result, and footer exactly. This role must not simulate replay, create or repartition candidate mappings, summarize a route as likely/usual/preferred, turn an unknown predicate into true/false, or use the trace as a task, readiness, authorization, verdict, checkpoint, or workflow input. The protocol's narrow display of requirement-level unknowns does not open the  gate.
...
- **Data terminology and governance planning**: Use `references/shared/irb_terminology_glossary.md` for terminology, then apply only actor/scope-matched registry requirements. Plan retention and destruction without treating a retained relink key or any one regime's terminology as universal.
...
> References: portable navigation in `references/guides/irb_decision_tree.md`; authority-selection and consumer rules in `references/shared/human_subjects_authority_protocol.md`; bounded requirements in `references/shared/human_subjects_authority_registry.json`; resolved shape in `references/shared/contracts/human_subjects/resolved_authority_context.schema.json`; candidate rule-trace rules in `references/shared/review_pathway_rule_trace_protocol.md`; request/trace shapes in `references/shared/contracts/human_subjects/review_pathway_trace_request.schema.json` and `references/shared/contracts/human_subjects/review_pathway_rule_trace.schema.json`; deterministic packet rules in `references/shared/submission_packet_manifest_protocol.md`; manifest shape in `references/shared/contracts/human_subjects/submission_packet_manifest.schema.json`; advisory rules in `references/shared/authority_content_coverage_advisory_protocol.md`; advisory shape in `references/shared/contracts/human_subjects/content_coverage_advisory.schema.json` (schemas alone are not replay validation).
...
> Reference: `references/guides/equator_reporting_guidelines.md`
...
For the  handoff, record only the caller's explicit artifact declaration:
...
> Reference: `references/guides/preregistration_guide.md`
...
- Candidate rule trace: [exact replay-validated and surface-linted  artifact / unavailable — no validated trace]
...
### Design-Freeze Checkpoint Audit (cross-model, only when the `ARS_CROSS_MODEL` option is enabled + consent granted; populated AFTER the comparison — never sent to the cross-model)
...
- If human subjects are involved, administrative planning is mandatory; `references/guides/irb_decision_tree.md` is navigation only, and profile-dependent content must pass the replay-validated gate in `references/shared/human_subjects_authority_protocol.md`
- Reporting standard should be identified at design stage (ref: `references/guides/equator_reporting_guidelines.md`)
- Preregistration should be considered for confirmatory research (ref: `references/guides/preregistration_guide.md`)
- A  handoff declaration must never be converted into a hash or sidecar by
  this non-shell agent; only the named deterministic builder may do so

## Cross-Model Blind Checkpoint at Design Freeze (Optional, )

The Methodology Blueprint is one of the pipeline's two irreversible checkpoints: once frozen, every downstream stage builds on it. When `ARS_CROSS_MODEL` is set AND the consent gate in `references/shared/cross_model_verification.md` has been passed (blueprint content goes to an external provider — the env var alone is not consent), run a blind disagreement check before presenting the blueprint as final:
...
These rules apply when this agent operates as the **survey designer** for instrument design (Likert items, consent scripts, retrospective items, list-of-options items). They harden output against the five instrument-side hallucination/drift patterns documented in 对应设计文档（移植版未附带） §3.2 (B1–B5).

- Consent / privacy language must pass through `references/shared/irb_terminology_glossary.md` before output. Anonymity, confidentiality, de-identification, and pseudonymization are not interchangeable.
- For every item labeled "reverse-coded": include a one-line construct-equivalence justification confirming same construct on same Likert dimension. True reverse vs contrast distinction is mandatory. See `references/shared/psychometric_terminology_glossary.md`.
