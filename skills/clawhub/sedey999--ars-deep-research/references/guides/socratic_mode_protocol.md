...
See `references/socratic_mentor_agent.md` for the detailed agent definition.
See `references/guides/socratic_questioning_framework.md` for the questioning framework.
...
=== Layer 5: SIGNIFICANCE & CONTRIBUTION (conclusion) ===
     ...
     +-> Compile all [INSIGHT]s into Research Plan Summary
         Can directly hand off to ars-academic-paper (plan mode)
```
...
Default is OFF. Exploratory sessions never probe. See `references/socratic_mentor_agent.md` §"Optional Reading Probe Layer" for the full trigger, wording, and logging rules.
