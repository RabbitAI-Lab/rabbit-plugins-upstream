...
This assessment is **advisory** — it routes to a Responsible Use Statement and never to a hard block. (Hard blocks are reserved for integrity violations; see `references/ethics_review_agent.md` Blocking Conditions.) Escalation rests on concrete enabling specifics, not subject matter.
...
> **Authority boundary (//):** Mixed-jurisdiction review-level mappings are not part of this checklist. This section collects facts and exact bounded requirement pointers only; the output pathway is `institutional determination required`. Unknown selection or applicability stays unresolved, and neither this checklist nor  emits a review determination, readiness result, compliance conclusion, or authorization.

Apply `references/shared/human_subjects_authority_protocol.md` and use only
curated rows from `references/shared/human_subjects_authority_registry.json`. All shipped
profiles are bounded subsets.

### 8.1 Authority Context and Exact Facts

- [ ] The review-ethics and data-protection axes each have an exact selected  profile or a visible unresolved state
...
- [ ] The exact  fact catalogue has been addressed where relevant:
...
| Axis | Exact  requirement ids currently available |
|---|---|
...
- [ ] Terminology is bound to the named convention in `references/shared/irb_terminology_glossary.md`
...
> For the portable fact flow, exact requirement anchors, survey metadata disclosure rule, and illustrative Taiwan/institution planning questions, see `references/guides/irb_decision_tree.md`. Do not treat that reference as authorization.
