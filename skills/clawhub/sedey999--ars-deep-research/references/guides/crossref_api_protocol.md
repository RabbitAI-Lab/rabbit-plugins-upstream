...
## `retraction_status` observation

For a DOI-matched record, retain both top-level and relation-form
`updated-by`/`update-to` metadata. The  resolver recognizes an update only
...
See 自动校验脚本（移植版未附带，以文字规则为准）。 Class `CrossrefClient` exposes `doi_lookup_with_title_check(doi, expected_title)` and `title_search(title, year=None)`. Both return `dict | None` (the dict being either the `message` for DOI, or one item from `message.items` for title search). Both raise `CrossrefUnavailable` on degradation per the table above.

## Cross-references

- Spec: 对应设计文档（移植版未附带） §3.5
- Mirror template: `ars-deep-research/references/guides/semantic_scholar_api_protocol.md`
- Sibling protocol: `ars-deep-research/references/guides/openalex_api_protocol.md`
