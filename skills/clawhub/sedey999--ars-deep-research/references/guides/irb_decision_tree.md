...
`references/shared/human_subjects_authority_protocol.md`; the only curated
authority rows available to this aid are in
`references/shared/human_subjects_authority_registry.json`. Every profile in that registry
...
│
├── Select exact  profile id/version/digest pointers only when confirmed
...
### 1.1 Exact  declared facts

For a  context, record `true`, `false`, or `unknown` against these exact
...
### 1.3 Derived  candidate rule trace

The  runtime may display only an explicit, replay-bound request that
...
selected by exact id/version/digest and the  resolved artifact permits
profile-dependent use. The separately replayed  display-only trace may
...
These are **questions, not Taiwan legal requirements**. The current  Taiwan
...
Use `references/shared/irb_terminology_glossary.md` for explicitly
...
3. [ ] Does every authority-bound statement point to an exact  requirement id and its selected profile?
