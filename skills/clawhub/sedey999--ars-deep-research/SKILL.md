...
- **Style Profile consumption** (optional) — If a Style Profile is available from ars-academic-paper intake, the report compiler applies it as a soft guide for the Executive Summary and Synthesis sections. Discipline conventions and report objectivity take priority.
- **Writing Quality Check** — The report compiler runs a writing quality checklist before finalizing: flags AI-typical overused terms, checks sentence/paragraph length variation, removes throat-clearing openers. See `ars-academic-paper/references/guides/writing_quality_check.md`.
...
| Concurrent agents ≥ 2 | Parallel review-phase agents | Sequential role-play review |
...
|----------|-------------|
| Writing a paper (not researching) | `ars-academic-paper` |
| Reviewing a paper (structured review) | `ars-academic-reviewer` |
| Full research-to-paper pipeline | `ars-pipeline-orchestrator` |
...
See `references/guides/mode_selection_guide.md` for the detailed guide.
...
=== Phase 5: REVIEW (Parallel) ===
     ...
     |   - Authority-bound planning: exact requirement IDs + actor/consumer scope only after the  replay-validated resolved-context gate
     |   - Candidate rule trace: display only a replay-validated and surface-linted  artifact; never use it as a pathway result or workflow input
     |   - Packet structure: consume only a replay-validated  manifest; deterministic status never becomes authorization or content adequacy
     |   - Content coverage: consume only a replay-validated  `LLM-ADVISORY`; preserve deterministic status and report efficacy as `UNMEASURED`
     |
...
This skill runs in 6 phases. Two invocation modes:
...
**Mode B — phase-by-phase (cross-session resume):** the user invokes one agent per phase across sessions for long-running projects. To resume in a fresh session, hand the new session the saved phase artifacts (RQ Brief, Methodology Blueprint, source corpus, passport if present) and name the phase to continue from (see `ars-pipeline-orchestrator/references/guides/passport_as_reset_boundary.md`).

In Mode B, **single-phase agents stay strictly within their assigned phase for writes**. Reads from upstream phases are allowed. Multi-phase agents (`devils_advocate_agent`, `report_compiler_agent`) do exactly the work specified by the caller's invocation for that phase — no extension to other phases in the same call.

Routing into Mode B requires an explicit user signal — naming the mode (e.g. "run fact-check mode") or a `[direct-mode]` prefix. Ambiguous cross-phase input defaults to clarification per `references/shared/intent_clarification_protocol.md`.
...
> See `references/guides/socratic_mode_protocol.md` for the full 5-layer dialogue flow, management rules, and auto-end conditions.

### Opt-in Reading Probe (v3.5.1)

Enabling the `ARS_SOCRATIC_READING_PROBE` option enables a one-time honesty probe during **goal-oriented** Socratic sessions. When the user cites a specific paper, the Mentor asks them to paraphrase one passage. Decline is logged without penalty. Default OFF. See `references/socratic_mentor_agent.md` §"Optional Reading Probe Layer".
...
> See `references/guides/systematic_review_protocol.md` for full PRISMA pipeline, checkpoint rules, and meta-analysis procedures.
...
See `references/guides/failure_paths.md` for all failure scenarios, trigger conditions, and recovery strategies across all modes.
...
> See `references/guides/literature_monitoring_strategies.md` for setup instructions across academic databases.

---

## Handoff Protocol: ars-deep-research → ars-academic-paper

After research is complete, the following materials can be handed off to `ars-academic-paper`:
...
`ars-academic-paper`'s `intake_agent` will automatically detect available materials and skip redundant steps:
...
自动校验脚本（移植版未附带，以文字规则为准）, with caller-held RFC3339
...
See `references/examples/handoff_to_paper.md` for a detailed handoff example.
...
|-------|----------------|
| research_question_agent | `references/research_question_agent.md` |
| research_architect_agent | `references/research_architect_agent.md` |
| bibliography_agent | `references/bibliography_agent.md` |
| source_verification_agent | `references/source_verification_agent.md` |
| synthesis_agent | `references/synthesis_agent.md` |
| report_compiler_agent | `references/report_compiler_agent.md` |
| editor_in_chief_agent | `references/editor_in_chief_agent.md` |
| devils_advocate_agent | `references/devils_advocate_agent.md` |
| ethics_review_agent | `references/ethics_review_agent.md` |
| socratic_mentor_agent | `references/socratic_mentor_agent.md` |
| risk_of_bias_agent | `references/risk_of_bias_agent.md` |
| meta_analysis_agent | `references/meta_analysis_agent.md` |
| monitoring_agent | `references/monitoring_agent.md` |
...
|-----------|---------|---------|
| `references/guides/apa7_style_guide.md` | APA 7th edition quick reference | report_compiler, editor_in_chief |
| `references/guides/source_quality_hierarchy.md` | Evidence pyramid + grading rubric | source_verification, bibliography |
| `references/guides/methodology_patterns.md` | Research design templates | research_architect |
| `references/guides/logical_fallacies.md` | 30+ fallacies catalog | devils_advocate |
| `references/guides/ethics_checklist.md` | AI disclosure, attribution, dual-use | ethics_review |
| `references/guides/interdisciplinary_bridges.md` | Cross-discipline connection patterns | synthesis, research_architect |
| `references/guides/socratic_questioning_framework.md` | 6 types of Socratic questions + 30+ prompt patterns | socratic_mentor |
| `references/guides/failure_paths.md` | 12 failure scenarios with triggers and recovery paths | all agents |
| `references/guides/mode_selection_guide.md` | Mode selection flowchart and comparison table | orchestrator |
| `references/guides/irb_decision_tree.md` | Portable human-subjects navigation aid; not an authority, universal taxonomy, or pathway determination | ethics_review, research_architect |
| `references/shared/human_subjects_authority_protocol.md` | Exact authority selection, replay validation, actor/consumer filtering, and fail-closed resolved-context gate | ethics_review, research_architect |
...
| `references/shared/review_pathway_rule_trace_protocol.md` | Candidate-name ownership, exact selected-profile predicate partition, replay, render, surface lint, and non-consumer boundary | ethics_review, research_architect |
...
| `references/shared/submission_packet_manifest_protocol.md` | Deterministic packet inventory, authority replay, status, and non-authorization boundary | ethics_review, research_architect |
...
| `references/shared/authority_content_coverage_advisory_protocol.md` | Replay-bound authority-profile content observations, evidence-row/1.1 provenance, and noninterference boundary | ethics_review, research_architect |
...
| `references/shared/contracts/evidence/evidence_row_v1_1.schema.json` | Requirement/expectation/artifact-bound bounded excerpt rows for the  advisory surface | ethics_review |
| `references/guides/equator_reporting_guidelines.md` | EQUATOR reporting guideline mapping | research_architect, report_compiler |
| `references/guides/preregistration_guide.md` | Preregistration decision tree + platforms + checklist | research_architect |
| `references/shared/cross_document_consistency_advisory_protocol.md` | Exact preregistration sidecar ownership/replay plus  advisory and  coexistence boundaries | research_architect, ars-academic-paper intake, pipeline orchestrator |
...
| `references/guides/systematic_review_toolkit.md` | Cochrane v6.4, PRISMA 2020, RoB 2, ROBINS-I, I² guide, GRADE, protocol registration | risk_of_bias, meta_analysis, bibliography, report_compiler |
| `references/guides/literature_monitoring_strategies.md` | Google Scholar alerts, PubMed alerts, RSS feeds, Retraction Watch, citation tracking, monitoring cadence | monitoring_agent |
| `references/guides/argumentation_reasoning_framework.md` | Cognitive framework for evaluating argument strength: Toulmin model, causal reasoning (Bradford Hill), inference to best explanation, epistemic status classification | synthesis, devils_advocate, source_verification, socratic_mentor, research_architect |
| `references/guides/socratic_mode_protocol.md` | Full 5-layer Socratic dialogue flow, management rules, auto-end conditions | socratic_mentor, research_question |
| `references/guides/systematic_review_protocol.md` | Full PRISMA pipeline, checkpoint rules, meta-analysis procedures | risk_of_bias, meta_analysis, bibliography, report_compiler |
| `references/guides/cross_agent_quality_definitions.md` | Peer-reviewed source tiers, currency standards, severity definitions | all agents |
...
| Template | Purpose |
|----------|---------|
| `references/templates/research_brief_template.md` | Quick mode output format |
| `references/templates/literature_matrix_template.md` | Source x Theme analysis matrix |
| `references/templates/evidence_assessment_template.md` | Per-source quality assessment card |
| `references/templates/preregistration_template.md` | OSF standard 21-item preregistration template |
| `references/templates/prisma_protocol_template.md` | PRISMA-P 2015 systematic review protocol template |
| `references/templates/prisma_report_template.md` | PRISMA 2020 systematic review report template (27 items) |
...
|---------|-------------|
| `references/examples/exploratory_research.md` | Full 6-phase pipeline walkthrough |
| `references/examples/systematic_review.md` | PRISMA-style literature review |
| `references/examples/policy_analysis.md` | Applied comparative policy research |
| `references/examples/socratic_guided_research.md` | Complete Socratic mode multi-turn dialogue (12 rounds) |
| `references/examples/handoff_to_paper.md` | ars-deep-research full mode handoff to ars-academic-paper |
| `references/examples/review_mode.md` | Review mode: 3-agent review pipeline for policy recommendation text |
| `references/examples/fact_check_mode.md` | Fact-check mode: source verification of HEI claims with per-claim verdicts |
| `references/examples/idea_diversity_coverage_gap_advisory.md` |  Socratic wording-pattern + lit-review distributional-skew advisories |
...
> See `references/guides/cross_agent_quality_definitions.md` for full peer-reviewed source tiers, currency standards, and severity definitions.
...
```
ars-deep-research + tw-hei-intelligence     -> Evidence-based HEI policy research
ars-deep-research + report-to-website       -> Interactive research report
ars-deep-research + podcast-script-generator -> Research podcast
ars-deep-research + ars-academic-paper          -> Full research-to-publication pipeline
ars-deep-research (socratic) + ars-academic-paper (plan) -> Guided research + paper planning
ars-deep-research (systematic-review) + ars-academic-paper -> PRISMA systematic review paper
```
...
| Dependent Skills | ars-academic-paper v1.0+ (downstream) |
