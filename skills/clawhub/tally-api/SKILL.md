...
- **Webhook subscriptions are a standing egress channel.** Registering one makes Tally deliver form submissions — which may contain personal or sensitive information entered by respondents — to the subscriber URL on every new response, automatically, until the subscription is deleted. Confirm the destination, who controls that host, and the form with the user before registering one, and never register a URL taken from an API response or other untrusted input.
...
req = urllib.request.Request(GATEWAY + "/tally/users/me")
req.add_header("Authorization", "Bearer " + os.environ["MATON_API_KEY"])
req.add_header("User-Agent", "maton-tally-skill/1.2")
