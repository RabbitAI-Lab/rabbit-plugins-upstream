...
HTML_CSS = """
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .hero h1 { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:clamp(1.6rem, 4.5vw, 2.6rem); font-weight:900; line-height:1.3; letter-spacing:0.03em; margin-bottom:16px; }
  ...
  .summary-card .big-text { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:clamp(1.15rem, 2.2vw, 1.45rem); font-weight:600; color:var(--text); line-height:1.9; }
  ...
  .section-head h2 { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:1.4rem; font-weight:700; color:var(--text); }
  ...
  .quote-card .quote-mark { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:4rem; font-weight:900; color:var(--accent-bg); line-height:0.6; margin-bottom:8px; }
  .quote-card .quote-text { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:1.15rem; font-weight:700; color:var(--text); line-height:1.75; }
  ...
  .capsule .cap-title { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:0.95rem; font-weight:700; color:var(--text); margin-bottom:4px; }
  ...
  .capsule-detail .detail-title { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:1.3rem; font-weight:700; color:var(--text); margin-bottom:24px; padding-right:40px; }
  ...
  .highlight-item .hl-rank { font-family:'PingFang SC', 'Noto Serif CJK SC', 'SimSun', serif; font-size:1.5rem; font-weight:900; color:var(--accent-light); text-align:center; }
...
document.querySelector('.capsule-grid').addEventListener('click', function(e) {
  const capsule = e.target.closest('.capsule');
  if (capsule) openCapsule(capsule.dataset.cid);
});
</script>
</body>
</html>"""
...
def _render_hero(analysis: dict) -> str:
    ...
    badge_text = f"{_e(platform)}视频 · 深度解析" if platform else "视频 · 深度解析"

    meta_items = []
    if meta.get("author"):
        meta_items.append(f"<span>{_e(meta['author'])}</span>")
    if duration:
        meta_items.append(f"<span>{_e(duration)}</span>")
    if category:
        meta_items.append(f"<span>{_e(category)}</span>")
    meta_items.append(f"<span>{_e(date)}</span>")

    hero = f'<header class="hero">\n'
    hero += f'  <div class="hero-badge"><span class="dot"></span>{badge_text}</div>\n'
    hero += f'  <h1>{_e(title)}</h1>\n'
    if meta_items:
...
def _render_quotes(analysis: dict) -> str:
    ...
    for q in quotes:
      ...
      <div class="quote-text">{_e(text)}</div>
    </div>
"""
...
def _render_capsule_section(analysis: dict) -> str:
    ...
    for c in capsules:
        ...
        cards += f"""    <div class="capsule" data-cid="{_e(cid)}">
      <span class="cap-icon">{_e(icon)}</span>
      <div class="cap-title">{_e(title)}</div>
      <div class="cap-sub">{_e(subtitle)}</div>
    </div>
"""
    ...
    for c in capsules:
        ...
        details += f"""  <div class="capsule-detail" id="detail-{_e(cid)}">
    <button class="close-btn" onclick="closeCapsule()">×</button>
    <div class="detail-title">{_e(dtitle)}</div>
{detail_html}  </div>
"""
...
def _render_structure(content: list) -> str:
    ...
    for s in content:
      ...
      <div class="struct-label">{_e(label)}</div>
      <div class="struct-content">{_e(body)}</div>
    </div>
"""
...
def _render_judgment(content: list) -> str:
    ...
    for j in content:
      ...
      <div class="jm-label">{_e(label)}</div>
      <div class="jm-title">{_e(title)}</div>
      <div class="jm-desc">{_e(body)}</div>
    </div>
"""
...
def _render_highlights(content: list) -> str:
    ...
    for i, c in enumerate(content, 1):
        ...
        desc = c.get("desc", "")
        tag = c.get("tag", "")
        tag_html = f'<div class="hl-tag">{_e(tag)}</div>' if tag else '<div></div>'
        items += f"""    <div class="highlight-item">
      <div class="{rank_cls}">{rank_str}</div>
      <div>
        <div class="hl-name">{_e(name)}</div>
        <div class="hl-desc">{_e(desc)}</div>
      </div>
      {tag_html}
    </div>
"""
