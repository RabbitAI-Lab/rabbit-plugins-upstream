#!/usr/bin/env python3
"""US / Partner price API helpers; local gateway helpers."""
...
def us_proxy_call(params: dict, api_type: str, business=None, timeout: int = 150) -> dict:
...
def us_file_download_call(params: dict, timeout: int = 150) -> dict:
    if "tokenPurpose" not in params and params.get("storeKey") and not params.get("accessToken"):
        params = dict(params)
        params.setdefault("tokenPurpose", DEFAULT_TOKEN_PURPOSE)
