...
import { rewriteMigrationManifest as rewriteMigrationManifestDefault, } from "./manifest-write.js";
...
export const ALL_MIGRATION_LAYERS = ["l0", "l1", "l2", "l3"];
...
function getRequiredString(values, key) {
...
function getOptionalString(values, key) {
    const value = values[key];
...
function resolveBooleanOption(values, key, defaultValue) {
    const value = values[key];
    return typeof value === "boolean" ? value : defaultValue;
}
function resolveTcvdbApiKey(values) {
...
function parseLayers(rawLayers) {
    if (!rawLayers)
        return [...ALL_MIGRATION_LAYERS];
    ...
    const invalid = uniqueLayers.filter((layer) => !ALL_MIGRATION_LAYERS.includes(layer));
    if (invalid.length > 0) {
        throw new Error(`Unsupported layer(s): ${invalid.join(", ")}`);
    }
    return uniqueLayers;
}
function parseBm25Language(rawLanguage) {
    if (!rawLanguage)
        return "zh";
 ...
 */
function buildEmptySummary(options) {
    return {
...
async function ensureReadablePath(filePath, label) {
    try {
...
async function ensureReadableDirectory(dirPath, label) {
...
function createTargetStoreDefault(options) {
...
async function ensureTargetIsEmpty(options, targetStore) {
    ...
    if (existingL1 > 0 || existingL0 > 0 || existingProfiles > 0) {
        throw new Error(`Target store is not empty (L1=${existingL1}, L0=${existingL0}, profiles=${existingProfiles})`);
    }
}
async function migrateL1Records(sourceStore, targetStore, pageSize) {
    let migrated = 0;
    let cursor = "";
    ...
    while (true) {
        ...
        if (useBatch) {
            ...
            for (const record of records) {
                ...
                if (!ok)
                    throw new Error(`Failed to migrate L1 record ${record.id}`);
            }
        }
...
async function migrateL0Records(sourceStore, targetStore, pageSize) {
    let migrated = 0;
    let cursor = "";
    ...
    while (true) {
        ...
        if (useBatch) {
            ...
            for (const record of records) {
                ...
                if (!ok)
                    throw new Error(`Failed to migrate L0 record ${record.id}`);
            }
        }
        migrated += rows.length;
        cursor = rows[rows.length - 1].record_id;
...
async function migrateProfiles(pluginDataDir, targetStore) {
...
async function verifyMigratedCounts(summary, targetStore, delayMs) {
    if (delayMs > 0) {
    ...
    if (profileCount !== summary.source.profileCount) {
        throw new Error(`Profile count verification failed: source=${summary.source.profileCount}, target=${profileCount}`);
    }
    log("校验通过");
    return { l1Count, l0Count, profileCount };
}
async function writeSummaryJson(summaryJsonPath, summary) {
    if (!summaryJsonPath)
        return;
...
export function resolveMigrationCliOptions(argv) {
    ...
    const sqlitePath = path.resolve(getOptionalString(values, "sqlite-path") ?? path.join(pluginDataDir, "vectors.db"));
...
export async function collectMigrationPreflight(options) {
...
export async function runMigrationCli(argv, deps = {}) {
    ...
    const rewriteMigrationManifest = deps.rewriteMigrationManifest ?? (async (params) => await rewriteMigrationManifestDefault(params));
    const migration = {
        l1Migrated: 0,
        l0Migrated: 0,
        profileMigrated: 0,
