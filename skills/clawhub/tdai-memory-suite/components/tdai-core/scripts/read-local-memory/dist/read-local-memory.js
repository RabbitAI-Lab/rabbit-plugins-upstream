...
function parseCli() {
    ...
    const sort = (values.sort?.toLowerCase() ?? "desc");
    if (!["asc", "desc"].includes(sort)) {
        console.error(`❌  无效的排序方向: ${values.sort}  （可选: asc, desc）`);
        process.exit(1);
    }
    const format = (values.format?.toLowerCase() ?? "table");
...
function resolveColumnName(field, allowedColumns) {
    // 直接匹配蛇形列名
    if (allowedColumns.has(field))
        return field;
    // 尝试驼峰转换
    const mapped = CAMEL_TO_COLUMN[field];
    if (mapped && allowedColumns.has(mapped))
        return mapped;
    return field; // 返回原值，后续校验会报错
}
/** 校验过滤条件的列名是否在白名单中 */
function validateFilterColumns(conditions, allowedColumns, level) {
...
function filtersToRecord(conditions) {
    const result = {};
    for (const c of conditions) {
...
function filtersToDisplayString(conditions) {
 ...
 */
function buildWhereClause(level, sinceDate, untilDate, filterConditions) {
    const clauses = [];
    const params = [];
...
function mapL1Row(row) {
    ...
    const timestamps = [
        ...(new Set([row.timestamp_str, row.timestamp_start, row.timestamp_end]
            .filter(Boolean)))
    ];
    return {
        id: row.record_id,
...
function querySqlite(db, level, opts) {
...
// Query: L0 / L1 (SQLite)
// ─────────────────────────────────────────────
function querySqliteLevel(db, opts, level) {
...
function displayWidth(str) {
    ...
    for (const char of str) {
        ...
        if ((code >= 0x4e00 && code <= 0x9fff) || // CJK 基本
...
function printTable(headers, rows, colWidths) {
    const hLine = (left, mid, right, fill) => left + colWidths.map((w) => fill.repeat(w + 2)).join(mid) + right;
    console.log(hLine("┌", "┬", "┐", "─"));
...
function renderL0Table(records) {
    ...
    const maxContentWidth = Math.min(80, Math.max(30, ...rows.map((r) => displayWidth(r[3]))));
    colWidths[3] = maxContentWidth;
    printTable(headers, rows, colWidths);
    console.log();
}
function renderL1Table(records) {
    ...
    const maxContentWidth = Math.min(80, Math.max(30, ...rows.map((r) => displayWidth(r[4]))));
...
function queryL2(opts) {
    ...
    const entries = files.map((f) => parseL2File(path.join(dirPath, f)));
...
function showOverview(db, opts) {
    ...
    // ── L0 ──
    try {
        const l0Count = db.prepare("SELECT COUNT(*) AS cnt FROM l0_conversations").get().cnt;
        const l0Roles = db.prepare("SELECT role, COUNT(*) AS cnt FROM l0_conversations GROUP BY role").all();
    ...
    // ── L1 ──
    try {
        const l1Count = db.prepare("SELECT COUNT(*) AS cnt FROM l1_records").get().cnt;
        const l1Types = db.prepare("SELECT type, COUNT(*) AS cnt FROM l1_records GROUP BY type").all();
...
function tryOpenSqlite(dataDir) {
...
function emptyL0L1Result(opts, level) {
    if (opts.format === "json") {
        const result = {
            level,
            total: 0,
