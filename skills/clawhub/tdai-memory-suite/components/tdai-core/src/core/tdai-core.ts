...
export class TdaiCore {
  ...
  private wirePipelineRunners(): void {
    ...
    // Always use OpenClaw Gateway's built-in LLM (runEmbeddedPiAgent / CleanContextRunner).
    // Standalone LLM mode has been removed — L1/L2/L3 extraction now exclusively
    // relies on the host's embedded model with no separate API key or baseUrl needed.
    const openclawConfig =
      (this.hostAdapter as { getOpenClawConfig?(): unknown }).getOpenClawConfig?.();

    // L1 runner (llmRunner=undefined → falls back to CleanContextRunner → gateway built-in)
    ...
    this.logger.debug?.(`${TAG} Pipeline runners wired (gateway built-in LLM)`);
  }
