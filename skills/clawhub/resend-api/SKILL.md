...
```bash
maton api '/resend/contacts/{contact_id}' -X DELETE
```
...
req = urllib.request.Request(GATEWAY + "/resend/emails")
req.add_header("Authorization", "Bearer " + os.environ["MATON_API_KEY"])
req.add_header("User-Agent", "maton-resend-skill/1.2")
