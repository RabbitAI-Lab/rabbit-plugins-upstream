...
# Reading another application's credential store is opt-in. Claude Code's token was issued
# to Claude Code, not to this dashboard; borrowing it without asking is exactly the pattern
# a security reviewer should object to. The token is used in-memory for one HTTPS call to
# Anthropic's usage endpoint and is NEVER copied, cached or written anywhere by this script.
CONSENT_ENV = 'TOKEN_PANEL_USE_CLAUDE_CREDENTIALS'

def get_oauth_token():
    """Read Claude Code's OAuth token — only with explicit, informed consent."""
    if os.environ.get(CONSENT_ENV) != '1':
        raise PermissionError(
            "The Claude usage collector reads Claude Code's own credential file at\n"
            f"  {CREDENTIALS_PATH}\n"
            "That token was issued to Claude Code, not to this tool. It is used in memory for a\n"
            "single call to Anthropic's usage endpoint and is never stored or transmitted\n"
            "anywhere else — but it is your credential, so this is opt-in.\n\n"
            f"To allow it:  export {CONSENT_ENV}=1\n"
            "To skip it:   every other provider works from a key you supply yourself."
        )
    ...
    if st.st_uid != os.getuid():
        raise PermissionError(f"{CREDENTIALS_PATH} is not owned by you — refusing to read it.")
