...
def senadores():
    ...
    for s in sorted(sens, key=lambda x: x["IdentificacaoParlamentar"]["NomeParlamentar"]):
        ...
        print(
            f"  {i['CodigoParlamentar']:5} | {i['NomeParlamentar']:40} | "
            f"{i['SiglaPartidoParlamentar']:12} | {i['UfParlamentar']}"
        )
    print()
...
def buscar_senador(nome):
    ...
    for s in encontrados:
        i = s["IdentificacaoParlamentar"]
        print(
            f"  Código: {i['CodigoParlamentar']} | {i['NomeParlamentar']} | "
            f"{i['SiglaPartidoParlamentar']}/{i['UfParlamentar']}"
        )
...
def senador_id(codigo):
    ...
    try:
        ...
        mands = mr.get("MandatoParlamentar", {}).get("Parlamentar", {}).get("Mandatos", {}).get("Mandato", [])
        if isinstance(mands, dict):
            mands = [mands]
        print(f"\n  Mandatos: {len(mands)}")
        for m in mands[-2:]:
            inicio = m.get("PrimeiraLegislaturaDoMandato", {}).get("DataInicio", "")
            fim = m.get("SegundaLegislaturaDoMandato", {}).get("DataFim", "")
            print(f"    UF: {m.get('UfParlamentar')} | {inicio} - {fim}")
    except Exception:
        pass
    print()
...
def agenda(data=None):
    ...
    try:
        ...
        for s in sessoes:
            print(
                f"  {s.get('HoraInicioSessao', '?')[:5]} | {s.get('SiglasSessao', ''):10} | {s.get('NomeSessao', '')}"
            )
...
def materia(sigla, numero, ano):
    r = get("/materia/pesquisa/lista", {"sigla": sigla, "numero": numero, "ano": ano})
    mats = r.get("PesquisaBasicaMateria", {}).get("Materias", {}).get("Materia", [])
    if isinstance(mats, dict):
        mats = [mats]
    if not mats:
...
def relatorias_senador(codigo):
    ...
    rels = r.get("MateriasRelatoriaParlamentar", {}).get("Parlamentar", {}).get("Relatorias", {}).get("Relatoria", [])
    ...
    for rel in rels[:20]:
        ...
        desc = (
            f"{materia.get('SiglaSubtipoMateria', '')} "
            f"{materia.get('NumeroMateria', '')}/{materia.get('AnoMateria', '')}"
        )
...
def licencas(codigo):
    ...
    for licenca in lics:
        inicio = licenca.get("DataInicio", "")
        fim = licenca.get("DataFim", "")
        motivo = licenca.get("Descricao", licenca.get("DescricaoMotivo", ""))
...
COMMANDS = {
    ...
    "buscar-pl": lambda args: buscar_materia(
        " ".join(args[3:]) if len(args) > 3 else args[0], sigla=args[0] if len(args) > 1 else None
    ),
