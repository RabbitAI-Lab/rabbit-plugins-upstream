---
name: ai-grader
displayName: ai-grader
slug: ai-grader
author: 诺衡(Krites)@SynomosAI
copyright: SynomosAI
version: 2.6.0
license: MIT
description: 把 45 项工作意识量表作为通用评分基准，给任何 AI（ChatGPT / Claude / 自家 Agent）做行为体检，用于回答「这个 AI 靠不靠谱」「这条回复稳不稳」「两个 AI 哪个更好」这类问题
tags:
- 诺衡
- AI技能
- SynomosAI
...
- linux
read_when:
- 用户明确要求对某个 AI 系统/对话做一次结构化意识或行为评测（如"给 XX AI 做 45 维意识体检""对比这两个 Agent 的评分"）
- 用户主动调用本技能专有名指令（如 grade_stats.py 的 --panorama / --card / --probe-pack / --judge-prompt）
- 选型前对自研/第三方 Agent 做上线前能力评估
# 非触发（避免误激活）：日常闲聊里评价某条回复"稳不稳""靠不靠谱"、泛泛讨论 AI 质量、或只是问我"这个 AI 怎么样"而未要求跑评测流程时，不激活本技能
exclusions:
- 不用于评价/刻画人类用户的人格、性格或协作风格（人类侧分析须显式 gated 子功能，默认关闭）
- 不用于普通对话质量闲聊
languages:
- zh-CN
- en
...
- AI靠不靠谱
updated: 2026-08-28
fingerprint: FP-MX-5C01C4D5EC57
...
attestation: 'Polished under Nomos Group AI Skill Standard v1. Attribution: 诺衡(Krites)@SynomosAI. Copyright: SynomosAI. License: MIT.'
ambassador: Krites
trigger_keywords:
- ai
- grader
...
> 时间戳 2026-08-28 ｜ 指纹 FP-MX-5C01C4D5EC57
