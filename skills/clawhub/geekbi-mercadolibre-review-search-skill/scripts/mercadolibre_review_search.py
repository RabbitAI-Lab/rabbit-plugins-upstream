#!/usr/bin/env python3
"""调用极鲸云 Mercado Libre 评论搜索接口。"""

import argparse
import json
import sys
from urllib.error import HTTPError, URLError

from geekbi_auth import ActionRequired, authenticated_json_request
from mercadolibre_search_common import (
    DEFAULT_BASE_URL,
    build_url,
    parse_int,
    parse_pairs,
    validate_page,
    validate_range_pairs,
    validate_search_response,
    validate_sort,
)


ENDPOINT = "/api/v1/mercadolibre/review/ai-search"
ALLOWED_PARAMS = {
    "goodsId", "siteId", "keyword", "scoreMin", "scoreMax", "helpfulMin",
    "helpfulMax", "sort", "order", "page", "size",
}
SORT_FIELDS = {"commentTime", "helpful", "score"}


def parse_params(raw_params):
    params, values = parse_pairs(raw_params, ALLOWED_PARAMS)
    goods_id = values.get("goodsId", "").strip()
    if not goods_id:
        raise ValueError("商品 ID 不能为空")
    if len(goods_id) > 100:
        raise ValueError("商品 ID 不能超过 100 个字符")
    if len(values.get("keyword", "")) > 300:
        raise ValueError("评论关键词不能超过 300 个字符")
    if "siteId" in values:
        parse_int("站点 ID", values["siteId"], minimum=1)
    for field in ("scoreMin", "scoreMax"):
        if field in values:
            parse_int(field, values[field], minimum=1, maximum=5)
    for field in ("helpfulMin", "helpfulMax"):
        if field in values:
            parse_int(field, values[field], minimum=0)
    validate_page(values)
    validate_sort(values, SORT_FIELDS)
    validate_range_pairs(values, {"score", "helpful"}, integer_fields={"score", "helpful"})
    return params


def main():
    parser = argparse.ArgumentParser(description="按商品查询 Mercado Libre 评论并输出 JSON")
    parser.add_argument("--base-url", default=DEFAULT_BASE_URL)
    parser.add_argument("--param", action="append", default=[], help="名称=值；必须传 goodsId")
    parser.add_argument("--timeout", type=float, default=30)
    args = parser.parse_args()
    try:
        params = parse_params(args.param)
        payload = authenticated_json_request(
            build_url(args.base_url, ENDPOINT, params), args.base_url, args.timeout
        )
        payload = validate_search_response(payload, "Mercado Libre 评论查询失败")
    except ActionRequired as error:
        print(json.dumps(error.public_payload(), ensure_ascii=False, indent=2))
        return 2
    except (ValueError, HTTPError, URLError, TimeoutError) as error:
        print(json.dumps({"error": True, "msg": str(error)}, ensure_ascii=False), file=sys.stderr)
        return 1
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
