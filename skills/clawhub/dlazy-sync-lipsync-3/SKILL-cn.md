...
fal.ai sync-lipsync v3 唇形对齐模型，输入一段视频和一段音频，输出说话人嘴型与音频对齐的新视频。适合配音、外语本地化、虚拟人重新对口型。
...
执行 `dlazy sync-lipsync-3` 命令获取结果。

```bash
dlazy sync-lipsync-3 -h

Options:
  --video_url [video_url]              视频链接 [video: url or local path]
  --audio_url [audio_url]              音频链接 [audio: url or local path]
  --sync_mode [sync_mode]              同步模式 [default: cut_off] (choices: "cut_off", "loop", "bounce", "silence", "remap")
...
```bash
dlazy sync-lipsync-3 --prompt '提示词内容'
```
