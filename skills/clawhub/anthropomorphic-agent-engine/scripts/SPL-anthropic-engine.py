...
# ================================================================
# ⚠️ 隐私与安全（v2.4.0）：
#   - 本适配器默认不启用；启用 = 显式 opt-in。
#   - OpenAI / Claude 适配器会把 prompt 内容发送到第三方 API
#     (api.openai.com / api.anthropic.com)，prompt 可能含用户敏感
#     交互或推断心理状态。接入前必须取得用户知情同意。
#   - 输出变为非确定性，会削弱引擎的"完全可复现"承诺。
#   - 推荐仅用 LocalFallbackAdapter 保证本地确定性问题。
# 用法：
