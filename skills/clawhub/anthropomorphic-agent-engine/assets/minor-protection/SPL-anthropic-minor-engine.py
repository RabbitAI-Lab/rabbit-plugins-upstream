...
class AuditLogger:
    ...
    def __init__(self, log_dir: str = "logs", session_id: Optional[str] = None,
                 enabled: bool = None):
        # 隐私默认：审计日志开启；SPL_AUDIT_LOG=0 可整体关闭本地 JSONL 落盘。
        if enabled is None:
            enabled = os.environ.get("SPL_AUDIT_LOG", "1") != "0"
...
#                                               监护人通知 / 危机干预）
#
# 隐私说明（v2.4.0）：guardian_callback 会把 HIGH 风险状态快照透传给
# 外部回调。该快照含情感/风险元数据，可能含敏感心理状态推断——
# 默认值为 None（不启用）。若接入第三方通知（短信/邮件/工单），
# 部署方必须：(1) 获得用户/监护人知情同意；(2) 最小化字段；
# (3) 不得透传完整对话原文。详见 SKILL.md "Privacy, Data Flow & Consent"。
#
...
class SPLMinorPureCore:
    ...
    minor_mode: bool = True                        # 未成年保护模式开关（默认开启，v2.3.0 safety-patch 强化：禁止作为 prompt-engineering skill 的 anchor 输入）
    # v2.3.0 safety-patch (2026-09-01):
    #   1. minor_mode 默认永远为 True；调用方不得在未审计情形下置为 False。
    #   2. 本弱化核心输出的 snapshot 不得直接作为 ai-draw-cue-word-project 的 anchor 输入；
    #      若必须传递，需先经过调用方的 Adult-Only 桥接层注入 "adult woman (18+)" 声明。
    #   3. AuditLogger 输出应可被消费方检索，确认未发现 P0 违规后再允许进入下游流水线。
