...
# 隐私默认：仅记录截断后的输入摘要；设置 SPL_LOG=0 可整体关闭请求日志。
# ================================================================
REQUEST_LOG_DIR = os.environ.get("SPL_LOG_DIR", "logs")
REQUEST_LOG_ENABLED = os.environ.get("SPL_LOG", "1") != "0"
_REQUEST_LOG_MAX_INPUT = 120  # 用户输入仅保留前 120 字符，避免完整隐私落盘
_request_log_lock = threading.Lock()


def _mask(text):
    """对用户输入做最小化处理：截断 + 折叠空白，防止完整对话/隐私内容落盘。"""
    if text is None:
        return None
    text = " ".join(str(text).split())
    if len(text) > _REQUEST_LOG_MAX_INPUT:
        return text[:_REQUEST_LOG_MAX_INPUT] + "…[truncated]"
    return text


def log_request(session_id, user_text, intent, crisis_cat, result):
    """记录一次用户请求-响应摘要，写入会话级请求日志（输入已脱敏截断）。

    隐私说明：user_input 字段保存的是 _mask() 截断后的摘要，非完整原文。
    如需完整对话审计，由部署方自行改造并在用户知情同意的前提下进行。
    """
    ...
    try:
        ...
        entry = {
            ...
            "session": session_id,
            "user_input_masked": _mask(user_text),
            "intent": intent,
...
if __name__ == "__main__":
    # 默认仅绑定本地回环 127.0.0.1，禁止 0.0.0.0 全网监听。
    # 如需跨设备访问，必须显式设置环境变量 SPL_BIND=0.0.0.0，
    # 且由部署方自行承担网络暴露与访问控制责任。
    BIND_ADDR = os.environ.get("SPL_BIND", "127.0.0.1").strip() or "127.0.0.1"
    print(f"SPL 伙伴（未成年合规保护版）已启动： http://{BIND_ADDR}:{PORT}/")
    print(f"核心引擎：SPLMinorPureCore（弱化版）· 三层保护：输入守门/引擎弱化/危机信号")
    print(f"监听地址：{BIND_ADDR}（默认仅本地；SPL_BIND 环境变量可覆盖，风险自负）")
