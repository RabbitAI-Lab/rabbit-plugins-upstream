---
name: amazon-product-faq-generator
description: >
  从 Amazon 评论中的重复疑问和商品字段中提炼商品页 FAQ 主题与回答证据。仅用于 FAQ 主题建议；不用于客服自动回复、法律审核或自动发布。
  Requires an ARI API key (ari_live_*).
slug: faq-generator
displayName: Amazon 商品 FAQ 建议
version: 1.4.5
summary: 从评论疑问提炼商品页 FAQ 建议
license: MIT
---
...
- 本入口只执行 `listing/faq`，输出模板固定为 `ops_listing`。
