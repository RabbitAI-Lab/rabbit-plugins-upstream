...
description: |
  ...
  Use this skill when the user names a connected app and a concrete action in it - read a mailbox, query a CRM, file an issue, update a spreadsheet. Not for general web fetching or for apps with no Maton connection.
...
- **Tool scope is declared and narrow.** `allowed-tools` limits this skill to `Bash` (to run `maton`), plus `Read`, `Grep`, and `Glob` (to consult the reference files under `references/`). It requests no file-writing, editing, or web-fetching tools: this skill routes API calls and reads its own docs, nothing else.
...
- **Do not `export` it in a shell profile or commit it to a file.** Scope it to the single command that needs it and clear it afterwards.
- **Do not pass it on a command line** (`--header "Authorization: Bearer $KEY"`), where it lands in `ps` output and shell history. Pass it through the environment of the one process that needs it.
...
```bash
# MATON_KEY holds the key for this shell only. Source it from wherever the environment
# already keeps secrets (a CI secret store, a secrets manager); interactively in bash, prompt for it
# without echoing or recording it in history:  read -rs MATON_KEY

MATON_API_KEY="$MATON_KEY" python3 <<'EOF'
import urllib.request, os, json, urllib.parse
key = os.environ.pop('MATON_API_KEY')  # read once, then dropped so child processes do not inherit it
