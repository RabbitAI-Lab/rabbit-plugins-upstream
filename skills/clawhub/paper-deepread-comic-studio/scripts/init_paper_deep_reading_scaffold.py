...
def render_report_stub(paper_title: str, paper_id: str, paper_slug: str, existing_collection_root: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "## 核心方法到底在干什么",
            "",
            "- 知识依赖顺序（先符号/设定 -> 数据/输入 -> 模型 -> 训练/推理 -> 实验/不足）：",
            "- 核心判断 / 核心直觉：",
            "- 方法结构拆解：",
            "- key modules：",
            "- 关键概念四步解释（直觉 -> 数学公式或 formal definition -> 具体例子 -> 局限）：",
            "- 复杂模块输入/输出/符号/维度/训练参数/固定超参/训练与推理数据流：",
            ...
            "- 主实验在证明什么：",
            "- 数据集规模、标签定义、划分协议和预处理：",
            "- baseline 来源，以及是否重跑、重构或直接引用：",
            "- 指标含义、适用边界和容易误读之处：",
            "- bridge experiments 在证明什么：",
            "- 哪些 ablation 真正支撑了机制：",
            "- 证据和 claim 之间还差什么：",
            "- 缺失实验细节（硬件、耗时、超参、随机种子、数据划分、计算预算等）：",
            "- headline result 的例外、反常现象和复现风险：",
            ...
            "- complete numeric training-to-inference walkthrough：",
            "- learned parameters vs fixed hyperparameters：",
            ...
            "- multi-image camera/style/logic continuity plan：",
            "- split into multiple images, not one compressed image：",
            "- source grounding and hallucination check：",
            "",
            "### 实验图表设计可借鉴点",
            "",
            "-",
            ...
            "",
            "- `使用这个skill，根据状态，继续完成这篇论文的完整精读报告。`",
...
def main() -> None:
    ...
    routing["possible_user_inputs_for_next_stage"] = [
        "使用这个skill，根据状态，继续完成这篇论文的完整精读报告。",
        "使用这个skill，根据状态，执行第1步：生成多张连续的卡通图，展示背景、旧方法缺陷、论文问题和灵感来源。",
        "使用这个skill，根据状态，告知下一步应该问什么。",
    ]
