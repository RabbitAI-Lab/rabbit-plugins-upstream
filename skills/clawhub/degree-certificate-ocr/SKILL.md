---
name: degree_certificate_ocr
description: 支持学位证书识别,提取获得者姓名、授予学位类别、授予单位及授予日期等。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 学位证书识别 OCR 技能

本技能封装了学位证书识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取获得者姓名、授予学位类别、授予单位及授予日期等。

## 功能特性

- **学位证书识别**：支持识别学位证书信息，识别提取获得者姓名、授予学位类别、授予单位及授予日期等
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• DIPLOMA（学位证书）|
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/degree_certificate_ocr/scripts/main.py DIPLOMA /path/to/test.jpg
```
...
- “帮我识别这张图像中的学位证书信息，图片在 /Users/name/Downloads/test.jpg”
