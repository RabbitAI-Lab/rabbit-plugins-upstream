...
description: "当用户想使用 Bing 进行图片搜索时，使用此 skill。"
---
...
```bash
python3 scripts/bing_images.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/bing_images.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
