#!/usr/bin/env python3
"""
City of Toronto Open Data — CLI for the CKAN API.
No dependencies beyond stdlib. Accesses open.toronto.ca via CKAN.

Usage:
  python3 toronto_data.py <command> [args]
...
def load_catalog():
    """Fetch the full dataset list (cached locally for 1 hour)."""
    ...
    for name in names:
        try:
            detail = api_request("package_show", {"id": name})
            pkg = detail.get("result", {})
            datasets.append({
                "name": pkg.get("name", ""),
                "title": pkg.get("title", ""),
                ...
                "tags": [t.get("name", "") for t in pkg.get("tags", [])[:5]]
            })
        except:
            continue
...
def cmd_search(query):
    ...
    for d in matches:
        ...
        queryable = f" [{ds_count} queryable]" if ds_count else ""
        print(f"  • {title}{queryable}")
        print(f"    ID: {name}")
        print(f"    https://open.toronto.ca/dataset/{name}")
    print()
...
def cmd_list():
    ...
    print(f"\n📁 Toronto Open Data — {len(datasets)} datasets\n")
...
def cmd_searchable():
    ...
    print(f"\n📊 Queryable Datasets (CKAN Datastore) — {len(queryable)} of {len(datasets)}\n")
...
def cmd_info(dataset_id):
    ...
    print(f"   URL: https://open.toronto.ca/dataset/{dataset_id}")
    ...
    if org:
        print(f"   Organization: {org.get('title', '?')}")
...
def cmd_fetch(dataset_id, options):
    ...
    if options.get("where"):
        params["filters"] = "{}"  # CKAN uses filters dict
        # Use q parameter for SQL-like queries (if datastore supports it)
        params["q"] = options["where"]
...
def main():
    ...
    if cmd == "search" and len(sys.argv) >= 3:
        ...
        while i < len(args):
            ...
            elif args[i] == "--resource-id" and i + 1 < len(args):
                options["resource_id"] = args[i + 1]; i += 2
            elif args[i] == "--csv":
