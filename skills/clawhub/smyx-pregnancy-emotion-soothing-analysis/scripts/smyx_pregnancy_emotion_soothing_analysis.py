...
def main():
    parser = argparse.ArgumentParser(description="孕妇情绪波动舒缓工具")
    parser.add_argument("--input", help="本地孕妇家中/产检候诊室固定摄像头（可选麦克风）音视频文件路径")
    parser.add_argument("--url", help="网络孕妇家中/产检候诊室固定摄像头（可选麦克风）音视频的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：孕期情绪舒缓场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示孕妇情绪波动舒缓历史安抚记录清单")
    ...
    try:
        ...
        print("🔍 正在检测孕妇情绪波动信号（哭泣/皱眉焦虑/长时间静坐不语/对话语气急促）并触发舒缓动作，请稍候...")
