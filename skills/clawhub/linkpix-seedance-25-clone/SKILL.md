---
name: linkpix-seedance-25-clone
description: "帮助短视频创作者、MCN机构、品牌市场部通过青虎AI完成“Seedance 2.5 爆款视频复刻”：利用更强的内容理解能力，快速提取爆款视频中的情感节奏和画面质感，将其完美平移到自身商品上。适用于抖音、TikTok、小红书、快手、视频号等主流平台的爆款搬运和重新演绎，帮助卖家在竞争激烈的社媒环境中快速获取初始流量，助力电商大促节点爆发。 Use this skill for Seedance2.5爆款复刻, 爆款视频重演, 情感节奏, 画面质感, 视频号, 快手, 社媒引流, 电商流量, 二创爆款, AI视频复刻。通过青虎AI统一接入，支持视频分析、任务轮询和结果下载。 当用户要求用 Seedance 2.5 爆款视频复刻 或 Seedance 2.5 复刻/对标/照着做爆款视频时必须触发。关键词：LinkPix、qhkit、青虎、Seedance 2.5、爆款复刻、情感节奏、画面质感、抖音、TikTok、小红书、快手、视频号、二创。"
user-invocable: true
...
帮助短视频创作者、MCN机构、品牌市场部通过青虎AI完成“Seedance 2.5 爆款视频复刻”：利用更强的内容理解能力，快速提取爆款视频中的情感节奏和画面质感，将其完美平移到自身商品上。适用于抖音、TikTok、小红书、快手、视频号等主流平台的爆款搬运和重新演绎，帮助卖家在竞争激烈的社媒环境中快速获取初始流量，助力电商大促节点爆发。 Use this skill for Seedance2.5爆款复刻, 爆款视频重演, 情感节奏, 画面质感, 视频号, 快手, 社媒引流, 电商流量, 二创爆款, AI视频复刻。通过青虎AI统一接入，支持视频分析、任务轮询和结果下载。

链接进、成片出：`video-inspire` 反推脚本 → 改写成用户商品 → `video` 用 **Seedance 2.5** 成片。适合把爆款的情感节奏和画面质感平移到自己的商品。

## 何时触发

- 「用 Seedance / Seedance 2.5 复刻这条爆款 / 对标 / 照着做」
- 用户同时点名该模型 + 给了抖音/TikTok/快手/视频号链接

## 使用配方

**首选模型**：本技能对外对应 **Seedance 2.5**。先跑 `qhkit video options '{"queryParams":["modelLabel","models"]}'` 拿实时清单，在 `models`/`modelLabel` 里按别名匹配：`Seedance2.5` / `Seedance 2.5`。命中后把返回的标签**逐字**当作 `modelLabel`（视频模型常带时长，如 `Seedance2.0 15秒`，只写模型名且存在多时长时按 CLI 提示补 `duration`）。

- 清单里暂时没有对应项：把最接近的 2–3 个候选（含单价/`credits`）列给用户选，说明「当前目录未上架 Seedance 2.5」，不要自造标签硬提交。
...
qhkit video generate '{"modelLabel":"Seedance2.5 15秒","prompt":"<改写后的脚本>","uploadedImages":["./我的商品图.jpg"],"uploadedVideo":"<playVideo 直链>"}'
qhkit video status   '{"videoTaskId":"task-123"}'
qhkit video estimate '{"modelLabel":"Seedance2.5 15秒","uploadedImages":["./我的商品图.jpg"],"uploadedVideo":"<playVideo 直链>"}'
```

- 中间步骤交付「原脚本 + 改写版」对照，确认后再提交成片。
- 模型不支持参考视频时不要传 `uploadedVideo`，改用脚本约束运镜。
- `uploadedVideo` / `uploadedAudio` 可数组，上限见 options。
- 复刻只借鉴结构与创意，不照搬原素材与台词。
...
- 只要脚本不要成片走「AI电商带货脚本生成器 | LinkPix」；不复刻、直接带货走「Seedance 2.5 电商带货视频 | LinkPix」。
