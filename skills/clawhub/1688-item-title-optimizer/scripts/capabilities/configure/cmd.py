...
from capabilities.configure.service import validate_ak, check_existing_config
...
def main():
    try:
        ...
        if len(sys.argv) < 2:
            if has_existing:
                md = f"✅ AK 已配置: `{_mask_ak(existing_ak)}`（来源: 环境变量）"
            else:
                md = "❌ 尚未配置 AK\n\n请通过环境变量 `ALI_1688_AK` 配置"
        ...
        md = f"✅ AK 已设置: `{_mask_ak(ak)}`（当前会话生效）"
