...
- For a one-off artifact or result, use the registered `flux-kontext` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi flux-kontext <operation> --help
curl --fail --location https://runapi.ai/docs/api/flux-kontext/<operation>.md --output contract.md
```
...
```shell
runapi flux-kontext <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service flux-kontext --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/flux-kontext-sdk
