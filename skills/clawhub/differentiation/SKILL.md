---
name: amazon-product-differentiation
description: >
  从主 ASIN 与已授权竞品的商品字段、图片和评论证据提炼可验证的差异化方向与待验证问题。仅用于定位讨论；不用于市场规模、销量、利润、库存、广告或订单决策。
  Requires an ARI API key (ari_live_*).
slug: differentiation
displayName: Amazon 产品差异化机会
version: 1.4.7
summary: 从主品与竞品证据提炼可验证差异化方向
license: MIT
---
...
- 本入口只执行 `page_compare/differentiation`，输出模板固定为 `ops_page_compare`。
