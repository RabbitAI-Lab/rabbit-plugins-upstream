...
#include <pthread.h>
static pthread_mutex_t hipool_mutex = PTHREAD_MUTEX_INITIALIZER;
#define LOCK()   pthread_mutex_lock(&hipool_mutex)
#define UNLOCK() pthread_mutex_unlock(&hipool_mutex)
#else
#define LOCK()   ((void)0)
...
typedef struct { Pool pool; HashTable table; TagIndex tag_index; SkipList sorted; char data_dir[512]; int file_ttl, initialized, wal_disabled; } MemoryCtx;
...
void memory_destroy(MemoryCtx *ctx) {
    ...
    free(ctx->pool.base); memset(ctx,0,sizeof(MemoryCtx));
}
...
const char *memory_get(MemoryCtx *ctx, const char *key) {
    ...
    if (e) { UNLOCK(); return (char*)e + sizeof(MemEntry) + e->key_len + 1; }
...
int memory_search_by_tag(MemoryCtx *ctx, const char *tag, SearchResult **r, int *cnt) {
    *r=NULL; *cnt=0;
    ...
    if (!es||ec==0) { free(es); UNLOCK(); return 0; }
    SearchResult *sr=(SearchResult*)calloc(ec,sizeof(SearchResult));
    if (!sr) { free(es); UNLOCK(); return -1; }
...
int memory_search_text(MemoryCtx *ctx, const char *q, SearchResult **r, int *cnt) {
    ...
    *r=sr; *cnt=f; UNLOCK(); return 0;
}

int memory_search_date(MemoryCtx *ctx, const char *ds, SearchResult **r, int *cnt) {
    LOCK();
    *r=NULL; *cnt=0; int y,m,d; if (sscanf(ds,"%d-%d-%d",&y,&m,&d)!=3) { UNLOCK(); return -1; }
    ...
    if (sl_range(&ctx->sorted, (uint64_t)ds0, (uint64_t)de, &entries, &ec) >= 0 && ec > 0) {
        SearchResult *sr = (SearchResult*)calloc(ec, sizeof(SearchResult));
        if (!sr) { free(entries); UNLOCK(); return -1; }
        int f = 0;
        for (uint32_t i = 0; i < ec; i++) {
...
int memory_search_range(MemoryCtx *ctx, uint64_t t_start, uint64_t t_end,
    ...
    *r = NULL; *cnt = 0;
    ...
    MemEntry **entries = NULL;
    uint32_t ec = 0;
    if (sl_range(&ctx->sorted, t_start, t_end, &entries, &ec) < 0) { UNLOCK(); return -1; }
    if (ec == 0) { free(entries); UNLOCK(); return 0; }
...
int memory_load(MemoryCtx *ctx) { LOCK(); int r=ovf_load(ctx); UNLOCK(); return r; }
int memory_cleanup_ttl(MemoryCtx *ctx) { LOCK(); int r=ovf_clean(ctx); UNLOCK(); return r; }
...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd,"set")==0) {
        ...
        int rc = memory_set_with_ts(&ctx,argv[2],argv[3],tp,tc,explicit_ts);
        if (rc == -2) printf("skipped (duplicate)\n");
        else if (rc < 0) { fprintf(stderr,"set failed\n"); memory_destroy(&ctx); return 1; }
        else printf("ok\n");
    } else if (strcmp(cmd,"get")==0) {
        if (argc<3) { memory_destroy(&ctx); return 1; }
        const char *v=memory_get(&ctx,argv[2]);
        printf("%s\n",v?v:"(not found)");
    } else if (strcmp(cmd,"del")==0) {
        if (argc<3) { memory_destroy(&ctx); return 1; }
        printf("%s\n",memory_del(&ctx,argv[2])<0?"(not found)":"ok");
    } else if (strcmp(cmd,"search")==0) {
        if (argc<3) { memory_destroy(&ctx); return 1; }
        SearchResult *r=NULL; int cnt=0;
        if (strcmp(argv[2],"--tag")==0&&argc>3) memory_search_by_tag(&ctx,argv[3],&r,&cnt);
        else if (strcmp(argv[2],"--date")==0&&argc>3) memory_search_date(&ctx,argv[3],&r,&cnt);
        else if (strcmp(argv[2],"--range")==0&&argc>4) {
            uint64_t ts_start=(uint64_t)atoll(argv[3]), ts_end=(uint64_t)atoll(argv[4]);
            memory_search_range(&ctx,ts_start,ts_end,&r,&cnt);
        } else memory_search_text(&ctx,argv[2],&r,&cnt);
    ...
    else if (strcmp(cmd,"stats")==0) { char b[1024]; memory_stats(&ctx,b,sizeof(b)); printf("%s\n",b); }
    else { fprintf(stderr,"unknown: %s\n",cmd); usage(); memory_destroy(&ctx); return 1; }
    memory_destroy(&ctx); return 0;
}
#endif

/* crash_test: 进程内测 WAL 恢复 */
int main(void) {
    /* 这不需要另外的 main, memory.c 已有 main */
    return 0;
}
