...
description: "当用户请求“调用 Google Search”或“网页搜索/SERP 抓取”，或明确提到网页搜索字段时，触发 dataify-google-search skill。"
---

# Dataify Google Search 中文版
...
```bash
python3 scripts/google_search.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/google_search.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
