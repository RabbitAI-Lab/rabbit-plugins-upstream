...
const EVK_AGENT_DECISION_FILE_NAME = 'agent-decision.json';

const SUPPORTED_PROTOCOLS = new Set(['evk', 'morpho']);
const SUPPORTED_COMMANDS = new Set([
  'plan-market',
  'preflight-evk-workflow',
  'explain-next',
  'verify-evk-deployment-handoff',
  'discover-markets',
  'ensure-feeds',
...
const UNSAFE_TEXT_PLACEHOLDER_RE = /replace-with|example\.com|\.example\b|dummy|placeholder|changeme|<set[_-]/i;
const UNSAFE_HEX_PLACEHOLDER_RE = /^0x([0-9a-fA-F])\1{39}$|^0x([0-9a-fA-F])\2{63}$/;
...
function printUsage() {
  console.error(`Usage:
  node bin/part2-planner.js plan-market --input '{"protocol":"evk",...}'
  node bin/part2-planner.js preflight-evk-workflow --input-file ./request.json
  ...
  node bin/part2-planner.js discover-markets --input-file ./request.json
  node bin/part2-planner.js ensure-feeds < request.json
  node bin/part2-planner.js prepare-euler-oracle --input-file ./request.json
  node bin/part2-planner.js deploy-euler-oracle --input-file ./request.json
  node bin/part2-planner.js prepare-evk-market --input-file ./request.json
  node bin/part2-planner.js prepare-evk-deployment --input-file ./request.json
  node bin/part2-planner.js deploy-evk-market --input-file ./request.json
  node bin/part2-planner.js run-evk-workflow --input-file ./request.json
...
function getRunDirFromOptions(options = {}) {
  ...
  return path.resolve(ROOT_DIR, String(runDir));
}

function resolveEvkRunArtifactPaths(options = {}) {
  const runDir = options.runDir
    ? path.resolve(ROOT_DIR, String(options.runDir))
    : options.resumeFromRunDir
      ? path.resolve(ROOT_DIR, String(options.resumeFromRunDir))
      : null;
  ...
  return {
    ...
    responsePath: path.join(runDir, 'response.json'),
    statusPath: path.join(runDir, 'status.json'),
    summaryPath: path.join(runDir, 'summary.json'),
    agentDecisionPath: path.join(runDir, EVK_AGENT_DECISION_FILE_NAME),
    rollbackPlanPath: path.join(runDir, 'rollback-plan.json'),
    planMarketPath: path.join(runDir, 'plan-market.json'),
    ensureFeedsPath: path.join(runDir, 'ensure-feeds.json'),
    feedFundingPath: path.join(runDir, 'feed-funding.json'),
    prepareEulerOraclePath: path.join(runDir, 'prepare-euler-oracle.json'),
    prepareEvkMarketPath: path.join(runDir, 'prepare-evk-market.json'),
    prepareEvkDeploymentPath: path.join(runDir, 'prepare-evk-deployment.json'),
    deployEvkMarketPath: path.join(runDir, 'deploy-evk-market.json'),
  };
}

function loadEvkRunArtifacts(paths) {
  return {
    request: readJsonFileIfExists(paths.requestPath),
    response: readJsonFileIfExists(paths.responsePath),
    status: readJsonFileIfExists(paths.statusPath),
    ...
    planMarket: readJsonFileIfExists(paths.planMarketPath),
    ensureFeeds: readJsonFileIfExists(paths.ensureFeedsPath),
    feedFunding: readJsonFileIfExists(paths.feedFundingPath),
    prepareEulerOracle: readJsonFileIfExists(paths.prepareEulerOraclePath),
    prepareEvkMarket: readJsonFileIfExists(paths.prepareEvkMarketPath),
    prepareEvkDeployment: readJsonFileIfExists(paths.prepareEvkDeploymentPath),
    deployEvkMarket: readJsonFileIfExists(paths.deployEvkMarketPath),
  };
}
...
function loadInput(options) {
  ...
  if (options.inputFile) {
    return readJsonFile(path.resolve(ROOT_DIR, String(options.inputFile)));
  }
  ...
  if (options.resumeFromRunDir) {
    const paths = resolveEvkRunArtifactPaths({ resumeFromRunDir: options.resumeFromRunDir });
    if (paths && fs.existsSync(paths.requestPath)) {
      return readJsonFile(paths.requestPath);
    }
  }
...
function loadRegistry(options) {
  const registryPath = options.registryFile
    ? path.resolve(ROOT_DIR, String(options.registryFile))
    : REGISTRY_PATH;
...
function loadFeedStatus(options) {
  const filePath = options.feedStatusFile
    ? path.resolve(ROOT_DIR, String(options.feedStatusFile))
    : FEED_STATUS_PATH;
  return readJsonFile(filePath);
}

function isUnsafePlaceholderValue(value) {
  ...
  return UNSAFE_TEXT_PLACEHOLDER_RE.test(trimmed) || UNSAFE_HEX_PLACEHOLDER_RE.test(trimmed);
}

function collectUnsafeLiveRequestPlaceholders(request) {
  const findings = [];

  const inspect = (value, label) => {
    if (typeof value === 'string' && isUnsafePlaceholderValue(value)) {
      findings.push(`${label} contains unsafe placeholder value`);
    }
  };

  inspect(request.rpcPreference?.rpcUrl, 'request.rpcPreference.rpcUrl');
  if (Array.isArray(request.rpcPreference?.rpcUrls)) {
    request.rpcPreference.rpcUrls.forEach((value, index) => inspect(value, `request.rpcPreference.rpcUrls[${index}]`));
  }

  request.collateralAssets?.forEach((asset, index) => inspect(asset?.address, `request.collateralAssets[${index}].address`));
  request.borrowAssets?.forEach((asset, index) => inspect(asset?.address, `request.borrowAssets[${index}].address`));

  inspect(request.vaultContext?.factoryAddress, 'request.vaultContext.factoryAddress');
  inspect(request.vaultContext?.assetAddress, 'request.vaultContext.assetAddress');
  inspect(request.shareAwareRouter?.evcAddress, 'request.shareAwareRouter.evcAddress');
  inspect(request.shareAwareRouter?.governorAddress, 'request.shareAwareRouter.governorAddress');
  if (Array.isArray(request.shareAwareRouter?.resolvedVaults)) {
    request.shareAwareRouter.resolvedVaults.forEach((value, index) => inspect(value, `request.shareAwareRouter.resolvedVaults[${index}]`));
  }

  inspect(request.executionProfile?.executorAddress, 'request.executionProfile.executorAddress');
  inspect(request.broadcast?.signerAddress, 'request.broadcast.signerAddress');
  inspect(request.send?.rpcUrl, 'request.send.rpcUrl');
  inspect(request.send?.privateKey, 'request.send.privateKey');
  inspect(request.feedFunding?.rpcUrl, 'request.feedFunding.rpcUrl');
  inspect(request.feedFunding?.privateKey, 'request.feedFunding.privateKey');
...
function requestMayExecuteLiveTransactions(request) {
  return Boolean(
    request?.feedFunding?.mode === 'real-send'
    || request?.broadcast?.enabled === true
    || request?.executionProfile?.mode === 'broadcast-ready'
    || (request?.send?.enabled === true && request?.send?.dryRun !== true)
  );
}

function makeEvkPreflightRequest(request) {
  const cloned = JSON.parse(JSON.stringify(request));

  if (cloned.feedFunding) {
    cloned.feedFunding.mode = 'classify-only';
  }

  if (cloned.executionProfile) {
    cloned.executionProfile.mode = 'simulate';
  }

  if (cloned.broadcast) {
    cloned.broadcast.enabled = false;
  }

  if (cloned.send) {
    cloned.send.enabled = false;
    cloned.send.dryRun = true;
  }
...
function findRouteFeedForArtifact(routeFeeds, artifact) {
  ...
  return routeFeeds.find((feed) => (feed.feedName || `${feed.base.symbol}/${feed.quote.symbol}`) === (artifact.feedName || `${artifact.base?.symbol || 'BASE'}/${artifact.quote?.symbol || 'QUOTE'}`)) || null;
}
...
function uniqueByFeedName(feeds) {
  ...
  return feeds.filter((feed) => {
    if (seen.has(feed.feedName)) {
      return false;
    }
    seen.add(feed.feedName);
    return true;
  });
}
...
function buildEvkRollbackPlan(response) {
  const deployResult = response.result.deployEvkMarket;
  const sendSummary = deployResult ? deployResult.sendSummary : null;
  const transactionPlan = deployResult ? deployResult.transactionPlan : [];
  const submittedTransactions = transactionPlan.filter((entry) => entry && entry.sendStatus === 'submitted');

  return {
    stateType: submittedTransactions.length > 0 ? 'forward-only-onchain' : 'local-only',
    submittedTransactions: submittedTransactions.map((entry) => ({
      transactionId: entry.transactionId,
      txHash: entry.txHash || null,
      description: entry.description || null,
    })),
    sendRequested: Boolean(sendSummary && sendSummary.requested),
    sendCompleted: Boolean(sendSummary && sendSummary.completed),
    mustNotDo: submittedTransactions.length > 0
      ? [
          'Do not blindly rerun live send from this run directory without reconciling submitted tx hashes first.',
          'Do not claim the market is borrowable until a separate proof step succeeds.',
        ]
      : [
          'Do not claim live deployment success from a planning-only or dry-run artifact bundle.',
        ],
  };
}

function buildEvkRunSummary(response, artifactPaths) {
  const status = response.status;
  const rollbackPlan = buildEvkRollbackPlan(response);
  const deployResult = response.result.deployEvkMarket;
  const nextSteps = [];

  if (status.blockers.length > 0) {
    status.blockers.forEach((blocker) => nextSteps.push({ type: 'blocker', message: blocker }));
  } else if (deployResult && deployResult.sendSummary && deployResult.sendSummary.completed) {
    nextSteps.push({ type: 'command', command: `node ./bin/part2-planner.js verify-evk-deployment-handoff --run-dir ${artifactPaths.runDir}`, message: 'Verify the persisted EVK deployment bundle before handing off to proof or reporting full success.' });
  } else if (status.state === 'browser-assisted funding ready') {
    nextSteps.push({ type: 'manual', message: 'Complete the browser-assisted funding branch, then resume from this run directory.' });
  } else if (status.state === 'real-send ready') {
    nextSteps.push({ type: 'manual', message: 'Human approval is still needed before any real send. Reuse request.json from this run dir.' });
  } else if (status.state === 'EVK dry-run ready') {
    nextSteps.push({ type: 'manual', message: 'Inspect deploy-evk-market.json; if live deployment is intended, rerun with broadcast-ready execution inputs.' });
  }

  return {
    phase: status.phaseReached,
    status: status.state,
    blockers: [...status.blockers],
    warnings: [...status.warnings],
    rollbackPlan,
    nextSteps,
    successClaimAllowed: Boolean(
      artifactPaths
      && deployResult
      && status.blockers.length === 0
      && response.result.ensureFeeds?.final?.ready === true
      && deployResult.sendSummary?.completed === true
    ),
  };
}

function buildEvkAgentDecision(response, artifactPaths) {
  const summary = buildEvkRunSummary(response, artifactPaths);
  const rollbackPlan = summary.rollbackPlan;
  const liveSendCompleted = response.result.deployEvkMarket?.sendSummary?.completed === true;
  const nextCommand = summary.nextSteps.find((entry) => entry.type === 'command')?.command || null;

  return {
    safeStatus: response.status.blockers.length > 0 ? 'blocked' : response.status.state,
    canContinue: response.status.blockers.length === 0,
    nextCommand,
    mustNotDo: uniqueStrings([
      ...(rollbackPlan.mustNotDo || []),
      ...(response.status.state === 'browser-assisted funding ready'
        ? ['Do not pretend browser-assisted funding already happened.']
        : []),
      ...(response.status.state === 'real-send ready' && !liveSendCompleted
        ? ['Do not describe readiness as completed live deployment.']
        : []),
    ]),
    successClaimAllowed: summary.successClaimAllowed,
    requiresUserApproval: response.status.state === 'real-send ready' && !liveSendCompleted,
    liveTxAllowed: response.status.executable.realSend === true || liveSendCompleted,
  };
}
...
function persistRunEvkWorkflowArtifacts({ request, response }) {
  ...
  const files = {
    ...
    agentDecision: path.join(bundleDir, EVK_AGENT_DECISION_FILE_NAME),
    rollbackPlan: path.join(bundleDir, 'rollback-plan.json'),
  ...
  const artifactPaths = { runDir: bundleDir };
  const summary = buildEvkRunSummary(response, artifactPaths);
  const agentDecision = buildEvkAgentDecision(response, artifactPaths);
  writeArtifactJson(files.summary, summary);
  writeArtifactJson(files.agentDecision, agentDecision);
  writeArtifactJson(files.rollbackPlan, summary.rollbackPlan);
...
async function preflightEvkWorkflowRequest(request, options = {}) {
  validateRunEvkWorkflowRequest(request);

  const previewRequest = makeEvkPreflightRequest(request);
  const preview = await runEvkWorkflow(previewRequest, options);
  const placeholderFindings = collectUnsafeLiveRequestPlaceholders(request);
  const liveTxRequested = requestMayExecuteLiveTransactions(request);
  const blockers = [...preview.status.blockers];
  const warnings = [...preview.status.warnings];

  if (placeholderFindings.length > 0) {
    blockers.push(...placeholderFindings);
  }

  if (liveTxRequested && request.broadcast?.enabled === true && !hasExplicitBroadcastOptIn(request.broadcast)) {
    blockers.push('request.broadcast.acknowledgement must equal I_UNDERSTAND_THIS_WILL_SEND_TRANSACTIONS before live EVK broadcast is safe to run');
  }

  const safeToRun = uniqueStrings(blockers).length === 0;
  const recommendedCommand = safeToRun
    ? 'node ./bin/part2-planner.js run-evk-workflow --input-file ./request.json'
    : null;

  return {
    command: 'preflight-evk-workflow',
    safeToRun,
    liveTxRequested,
    liveTxAllowed: safeToRun && liveTxRequested && preview.status.executable.realSend === true,
    placeholderSafe: placeholderFindings.length === 0,
    preview: {
      status: preview.status,
      artifactPersistence: preview.result.artifactPersistence,
    },
    blockers: uniqueStrings(blockers),
    warnings: uniqueStrings(warnings),
    nextSteps: safeToRun
      ? [
          'Enable request.artifacts.enabled for a persisted run bundle before any real deployment claim.',
          'Run run-evk-workflow and inspect status.json, response.json, and deploy-evk-market.json before claiming success.',
        ]
      : [
          'Replace every placeholder or dummy live value flagged above.',
          'Resolve the preview blockers before attempting broadcast-ready or real-send execution.',
        ],
    recommendedCommand,
  };
}

function explainNextEvkRun(options = {}) {
  const runDir = getRunDirFromOptions(options);
  const paths = resolveEvkRunArtifactPaths({ resumeFromRunDir: runDir });
  const artifacts = loadEvkRunArtifacts(paths);

  if (!artifacts.response || !artifacts.status) {
    return {
      currentState: 'blocked',
      blocker: 'Missing response.json or status.json in the run directory.',
      nextCommand: null,
      liveTxAllowed: false,
      userApprovalNeeded: false,
      successClaimAllowed: false,
      mustNotDo: ['Do not resume or report status from a partial EVK run directory without response.json and status.json.'],
      runDir,
    };
  }

  const agentDecision = artifacts.agentDecision || buildEvkAgentDecision(artifacts.response, { runDir });

  return {
    currentState: agentDecision.safeStatus,
    blocker: artifacts.status.blockers[0] || null,
...
function verifyEvkDeploymentHandoff(options = {}) {
  const runDir = getRunDirFromOptions(options);
  const paths = resolveEvkRunArtifactPaths({ resumeFromRunDir: runDir });
  const artifacts = loadEvkRunArtifacts(paths);
  const blockers = [];
  ...
  if (!artifacts.request) {
    blockers.push('Missing request.json; cannot verify deployment intent.');
  }
  if (!artifacts.status) {
    blockers.push('Missing status.json; cannot verify wrapper state.');
  }
  if (!artifacts.response) {
    blockers.push('Missing response.json; cannot verify staged wrapper output.');
  }
  if (!artifacts.prepareEvkDeployment) {
    blockers.push('Missing prepare-evk-deployment.json; cannot verify canonical deployment bundle.');
  }
  if (!artifacts.deployEvkMarket) {
    blockers.push('Missing deploy-evk-market.json; cannot verify deploy-stage output.');
  }

  const canonicalHash = artifacts.prepareEvkDeployment?.deploymentBundle?.canonicalHash || null;
  const deployCanonicalHash = artifacts.deployEvkMarket?.executionSummary?.canonicalHash || null;
  const verifyStep = Array.isArray(artifacts.deployEvkMarket?.stepResults)
    ? artifacts.deployEvkMarket.stepResults.find((entry) => entry.action === 'verify-evk-market')
    : null;
  const deployStep = Array.isArray(artifacts.deployEvkMarket?.stepResults)
    ? artifacts.deployEvkMarket.stepResults.find((entry) => entry.action === 'deploy-evk-vault')
    : null;

  if (canonicalHash && deployCanonicalHash && canonicalHash !== deployCanonicalHash) {
    blockers.push('Canonical hash mismatch between prepare-evk-deployment.json and deploy-evk-market.json.');
  }
  if (!deployStep) {
    blockers.push('Missing deploy-evk-vault step in deploy-evk-market.json.');
  }
  if (!verifyStep) {
    blockers.push('Missing verify-evk-market step in deploy-evk-market.json.');
  }
  if (verifyStep?.verificationChecklist?.canonicalHash && canonicalHash && verifyStep.verificationChecklist.canonicalHash !== canonicalHash) {
    blockers.push('verify-evk-market checklist canonical hash does not match the prepared deployment bundle.');
  }
  if (artifacts.status && Array.isArray(artifacts.status.blockers) && artifacts.status.blockers.length > 0) {
    blockers.push('status.json still contains blockers.');
  }
  if (artifacts.response?.result?.ensureFeeds?.final?.ready !== true) {
    blockers.push('Final ensure-feeds result does not prove feeds are ready.');
  }

  const liveSendRequested = artifacts.request?.send?.enabled === true && artifacts.request?.send?.dryRun !== true;
  if (liveSendRequested && artifacts.deployEvkMarket?.sendSummary?.completed !== true) {
    blockers.push('Live send was requested but deploy-evk-market.json does not show completed sendSummary.');
  }

  return {
    command: 'verify-evk-deployment-handoff',
    runDir,
    verified: blockers.length === 0,
    checks: {
      requestArtifactPresent: Boolean(artifacts.request),
      statusArtifactPresent: Boolean(artifacts.status),
      responseArtifactPresent: Boolean(artifacts.response),
      prepareArtifactPresent: Boolean(artifacts.prepareEvkDeployment),
      deployArtifactPresent: Boolean(artifacts.deployEvkMarket),
      canonicalHashMatches: Boolean(canonicalHash) && Boolean(deployCanonicalHash) && canonicalHash === deployCanonicalHash,
      deployStepPresent: Boolean(deployStep),
      verifyStepPresent: Boolean(verifyStep),
      finalFeedsReady: artifacts.response?.result?.ensureFeeds?.final?.ready === true,
      liveSendCompletedWhenRequested: !liveSendRequested || artifacts.deployEvkMarket?.sendSummary?.completed === true,
    },
    blockers: uniqueStrings(blockers),
    warnings: uniqueStrings(artifacts.status?.warnings || []),
  };
}
...
function buildFeedFundingOptions(request, feedName) {
  ...
  return {
    ...
    ...(feedFunding.privateKey ? { privateKey: feedFunding.privateKey } : {}),
...
async function runEvkWorkflow(request, options = {}) {
  ...
  if (options.resumeFromRunDir) {
    ...
    if (artifacts.request && !requestsCompatibleForResume(request, artifacts.request)) {
      throw new Error('The provided request does not match the request.json stored in the resume run directory.');
    }

    if (artifacts.response) {
      validateRunEvkWorkflowResponse(artifacts.response);
...
function prepareEulerOracle(request) {
  ...
  if (integrationMode === 'chainlink-oracle' || integrationMode === 'chainlink-infrequent-oracle') {
    ...
    const quoteLegFeed = Array.isArray(route.feeds)
      ...
      : null;
    const baseLegArtifact = baseLegFeed ? readyArtifacts.find((artifact) => (artifact.feedName || '') === (baseLegFeed.feedName || '')) : null;
    const quoteLegArtifact = quoteLegFeed ? readyArtifacts.find((artifact) => (artifact.feedName || '') === (quoteLegFeed.feedName || '')) : null;
...
function loadContractArtifact(contractName) {
  if (!CONTRACT_ARTIFACT_PATHS[contractName]) {
    return null;
  }
  ...
  try {
    const artifact = readJsonFile(CONTRACT_ARTIFACT_PATHS[contractName]);
    const normalizedArtifact = artifact && typeof artifact.bytecode === 'string' && artifact.bytecode.startsWith('0x')
      ? artifact
      : null;
...
function validateRunEvkWorkflowResponse(response) {
  ...
  assertAllowedKeys(response.result.artifactPersistence.files, ['request', 'response', 'status', 'summary', 'agentDecision', 'rollbackPlan', 'planMarket', 'ensureFeeds', 'feedFunding', 'prepareEulerOracle', 'prepareEvkMarket', 'prepareEvkDeployment', 'deployEvkMarket'], 'response.result.artifactPersistence.files');
...
async function runCli(argv = process.argv.slice(2)) {
  ...
  const request = ['explain-next', 'verify-evk-deployment-handoff'].includes(command) ? null : loadInput(args);
  let result;

  if (command === 'plan-market') {
    result = planMarket(request, args);
  } else if (command === 'preflight-evk-workflow') {
    result = await preflightEvkWorkflowRequest(request, args);
  } else if (command === 'explain-next') {
    result = explainNextEvkRun(args);
  } else if (command === 'verify-evk-deployment-handoff') {
    result = verifyEvkDeploymentHandoff(args);
