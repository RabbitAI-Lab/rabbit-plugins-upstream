---
name: conqueror-seo-project-setup
description: "Set up a durable local SEO workspace: working folder, site scope, goals, positioning, MCP and project check, and Search Console intake. Use at the start of a new SEO project. Requires a connected Conqueror MCP server."
---
...
### 5. Verify Conqueror MCP

After the user has described the company, website, goals, and positioning, check that Conqueror MCP is configured and mapped to the right project:
...
5. If the MCP is unavailable, tell the user to connect Conqueror MCP before continuing with live Conqueror data.
...
**Preferred (hosted): connect it natively.** On the project's Integrations page, connect Google Search Console and pull live data with `get_search_console_performance`. Once connected, the agent reads it directly in `conqueror-keyword-research` and `conqueror-keyword-clustering` — no manual files to maintain.
...
After intake, recommend one next Conqueror workflow:

- `conqueror-keyword-research`: when the user needs ideas from seed topics
- `conqueror-keyword-clustering`: when they have keywords or GSC data to map to pages
- `conqueror-competitive-landscape`: when the market is unclear
- `conqueror-competitor-analysis`: when they know a competitor to study
- `conqueror-link-prospecting`: when they have a linkable asset or target page
- `conquer-keyword`: when the site runs on WordPress and they already know a keyword to win
