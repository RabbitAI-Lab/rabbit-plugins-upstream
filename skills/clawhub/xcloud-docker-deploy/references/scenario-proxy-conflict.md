...
- Any host port mapping to `80` or `443` (for example `80:80` or `443:443`)
...
5. Expose a single non-reserved host port (default `3080`) for xCloud to proxy to
...
Remove port mappings `"80:80"` and `"443:443"`. If the app needs container port `80`, rewrite it as `"3080:80"` and use xCloud exposed/primary port `3080`.
...
Do not set xCloud's exposed port to `80` or `443`; those are reserved for xCloud's Nginx/SSL layer.
