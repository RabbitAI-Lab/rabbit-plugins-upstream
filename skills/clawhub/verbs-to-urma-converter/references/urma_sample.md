# URMA Complete Working Example: urma_sample.c

> Source: umdk/src/urma/examples/urma_sample.c
> This is the canonical reference implementation for URMA client/server applications, demonstrating RC/RM/UM transport modes, event-driven operations, RDMA read/write, Send/Recv, multi-client support, and client/server coexistence in the same process.
...
- `urma_init()` -> device query -> `urma_create_context` -> JFEC -> JFC -> JFR -> Jetty -> register segment
- Cleanup in reverse order in `uninit_context`

### 2. Address Exchange via Socket
- Pack EID + UASID + seg VA + Jetty ID into struct
- Exchange via TCP socket
- Peer unpacks

### 3. RC Mode Import + Bind
- Call `urma_import_jetty()` first
- Only RC mode needs to call `urma_bind_jetty()`
- Must set `rjetty.tp_type`

### 4. RDMA Write/Read Operations
- Write: local source -> remote destination
- Read: swap src/dst (remote source -> local destination)

### 5. Send/Recv with Buffer Recycling
- Pre-post receive buffers
- Send messages
- Poll for send and receive completions
- Distinguish send vs receive completions via `cr.flag.bs.s_r`

### 6. Event-Driven Mode
- `urma_wait_jfc()` -> `urma_poll_jfc()` -> `urma_ack_jfc()` -> `urma_rearm_jfc()`
- Polling fallback: loop with sleep