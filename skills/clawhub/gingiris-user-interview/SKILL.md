...
**客户材料缺失时不要停摆**：会前用官网、公开案例和现有截图搭一个可讨论的粗版；会上直接让用户展示真实流程并索取已有材料。把缺失材料记为待验证项，不要把“客户回去整理完整资料”设为下一次学习的前置条件。
...
优先顺序：真实付费/高频用户 → 已激活未付费 → 新注册未激活 → 流失用户 → 泛目标画像。首轮先发 **10 条逐人定制消息**，记录回复、预约、到场和有效洞察，再决定是否扩量。访谈中优先让用户共享屏幕完成真实任务；观点必须与行为日志、激活和支付时间戳交叉验证。
