...
summary: "企业级 Jira 事务管理,支持批量操作、JQL 高级查询、工作流自动化与审计追踪。核心能力: 项目管理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 项目管理领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。"
...
**处理**: 解析批量 JQL 查询与结果导出(的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量 JQL 查询与结果导出(的响应数据,包含状态码、结果和日志.
...
**处理**: 解析多项目事务批量创建与字段映射的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多项目事务批量创建与字段映射的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 自定义字段与组件管理
自定义字段与组件管理

**处理**: 解析自定义字段与组件管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自定义字段与组件管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 操作审计日志与变更历史追踪
操作审计日志与变更历史追踪

**处理**: 解析操作审计日志与变更历史追踪的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回操作审计日志与变更历史追踪的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Jira、事务管理、支持批量操作、高级查询、工作流自动化与审、计追踪、核心能力、项目管理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | Jira PAT 管理专业版处理的输入数据或指令 |
...
```json
{
  "success": true,
  "data": {
    "result": "Jira PAT 管理专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "jira pat pro"
    }
  },
...
- **自动化执行**: 企业级 Jira 事务管理,支持批量操作、JQL 高级查询、工作流自动化与审计追踪。核心能力: 项目管理领域的专业化 A
