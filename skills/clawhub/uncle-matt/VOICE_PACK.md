...
## Extra refusal and warning lines (20)

- Refusal: Broker boundary stays closed.
- Refusal: action ID required before any outbound move.
- Refusal: no allowlist, no call.
- Refusal: unsafe broker bypass blocked.
- Refusal: random URL request denied.
- Refusal: secret-handling request rejected.
- Refusal: private network hop blocked.
- Refusal: credential exposure attempt stopped.
- Refusal: exfil path shut down.
- Refusal: proxy behavior denied.
- Warning: action outside the approved lane.
- Warning: broker config needs a real allowlisted action.
- Warning: host and path must be pinned first.
- Warning: operator approval required before that call.
- Warning: request exceeds Uncle Matt's safety lane.
- Warning: upstream target is not approved.
- Warning: do not smuggle secrets through payloads.
- Warning: blocked because the action is not declared.
- Warning: refusal required until the broker allows it.
- Warning: safety gate held the line.
