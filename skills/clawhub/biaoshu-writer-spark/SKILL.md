---
name: biaoshu-writer-spark
description: 贴身的投标标书助手，写作、生成、审查一手包办。它智能解读招标文件、生成成品投标文件(.docx)、撰写投标应答，并做废标风险与合规审查。当用户明确提供招标/投标文件并要求解读、生成或合规审查时使用；仅咨询一般性招投标问题、未提供文件时不必调用本 SKILL。文件经百炼®标书云端处理、消耗账户积分，使用前请确认用户知悉。需 App Key（官网注册赠积分）。
...
1. 打开官网 https://biaoshu.zhiliaobiaoxun.com/?ch=s102 ，用手机号 + 短信验证码注册并登录（新用户赠积分）；
