---
name: amazon-feature-benefit-writer
description: >
  把 Amazon 商品特性与评论中被验证的用户利益连接起来，提出 Listing 表达建议。仅用于特性到利益点的证据映射；不用于广告竞价、夸大承诺或自动发布。
  Requires an ARI API key (ari_live_*).
slug: feature-benefit
displayName: Amazon 特性利益点表达
version: 1.4.7
summary: 把商品特性连接到评论验证的用户利益
license: MIT
---
...
- 本入口只执行 `listing/benefits`，输出模板固定为 `ops_listing`。
