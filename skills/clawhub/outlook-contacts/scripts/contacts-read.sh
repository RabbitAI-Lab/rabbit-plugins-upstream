...
_check_contacts_scope() {
    ...
    if [[ "$scope" != *"Contacts.Read"* && "$scope" != *"Contacts.ReadWrite"* ]]; then
        cat >&2 <<EOF
[error] token scope does not include Contacts.Read or Contacts.ReadWrite.
        Current scopes: $scope

        The outlook-calendar setup must be re-run with --force to re-authenticate
        and add the Contacts.ReadWrite permission:
