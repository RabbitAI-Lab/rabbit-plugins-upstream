...
REGISTER_GUIDE = """\
缺少凭证：未配置 App Key。请用户自行到官网获取（本 skill 不代注册、不收集手机号）。
转述给用户时逐步骤完整给出，链接原样显示完整 URL：

  1. 打开官网 https://biaoshu.zhiliaobiaoxun.com/
  2. 用手机号 + 短信验证码注册并登录（新用户赠积分）；
  3. 登录后点左侧菜单『开放 API』，在弹出面板中生成/查看 App Key
     （形如 bk_live_xxxxx，重置后旧 Key 立即失效）；
  4. 把 Key 直接粘贴到对话里让助手保存（下次免输），例如：
     「我的 App Key 是 bk_live_xxxxx，帮我保存一下」。
     注意：粘贴的 Key 会留在会话记录中；Key 泄露或想作废时，
