...
description: "当用户请求“调用 Google News”或“新闻搜索/信息”，或明确提到新闻搜索字段时，触发 dataify-google-news skill。"
---

# Dataify Google News 中文版
...
```bash
python3 scripts/google_news.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/google_news.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
