#!/usr/bin/env python3
"""Call Dataify Scraper API Google News and print the raw response body."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Call Dataify Google News API and print the raw response body."
    )
    parser.add_argument("--token", help="Dataify API token. Bearer prefix is optional.")
    parser.add_argument("--params-json", help="JSON object containing Google News fields.")
...
def normalize_sort_order(value: Any) -> str:
    ...
    if text in {"0", "relevance", "relevant", "相关性", "按相关性"}:
        return "0"
    if text in {"1", "date", "latest", "newest", "recent", "日期", "最新", "按日期"}:
        return "1"
    return str(value).strip()
...
def clean_query_text(query: str) -> str:
    query = re.sub(r"\s*(?:新闻|news)\s*$", "", query.strip(), flags=re.IGNORECASE)
    return query.strip()
...
def extract_query(text: str) -> str | None:
    ...
    patterns = (
        r"(?:搜索|查询|检索|抓取|采集|新闻)\s*(.+?)(?:[，,。；;]|$)",
        r"(?:google\s+news|news\s+for|search\s+news\s+for|search\s+for)\s+(.+?)(?:\s+with\b|\s+in\b|[,;.]|$)",
    )
...
def parse_natural_request(text: str) -> dict[str, str]:
    ...
    if "so" not in params:
        if any(marker in lowered for marker in ("按日期", "最新", "latest", "newest", "recent", "sort by date")):
            params["so"] = "1"
        elif any(marker in lowered for marker in ("按相关性", "相关性", "relevance", "sort by relevance")):
            params["so"] = "0"
...
def normalize_params(params: dict[str, Any]) -> dict[str, str]:
    normalized: dict[str, str] = {"engine": "google_news"}
    ...
    if not normalized.get("q"):
        raise ValueError("Missing required query q. Parse q from the user request or ask the user what news to search.")
    ...
    if "so" in normalized:
        normalized["so"] = normalize_sort_order(normalized["so"])
