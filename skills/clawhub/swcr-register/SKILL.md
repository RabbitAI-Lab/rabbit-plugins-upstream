---
name: swcr-register
description: "软件著作权登记全流程：生成程序鉴别材料、操作手册、填报信息，辅助在线填报。当用户需要申请软件著作权或生成软著材料时触发。"
version: 1.0.0
...
> **前提条件**：辅助填报需要以有头模式（headed）打开真实浏览器来操作中国版权保护中心网站。如果当前环境不具备浏览器（如无头服务器、无显示器），则**无法完成辅助填报**，需告知用户自行手动填报。这不影响材料生成——Step 1–4 的所有材料已可独立使用。

1. 以有头模式打开浏览器，访问 https://register.ccopyright.com.cn/registration.html#/registerSoft
2. 选择 **R11** → **计算机软件著作权登记申请** → 点击 **立即登记**
3. **提醒用户登录账号**（如未注册需先注册 + 实名认证，认证需 1-3 天）
4. 用户确认登录完成后，辅助填写以下信息：
   - 选择"我是申请人"
   - 填写软件信息（全称、简称、版本号）
...
1. 下载网站自动生成的**软件著作权登记申请表**
2. 将申请表及身份证复印件**打印**（单面打印）
3. **盖章**（签字/公章）
4. **扫描**为电子件
5. 将扫描件**上传**至网站对应位置
...
本技能来自 [WiseFlow](https://github.com/TeamWiseFlow/wiseflow) —— 如果你需要一只为你 7×24 小时在线搞钱的 AI 团队，欢迎使用。
