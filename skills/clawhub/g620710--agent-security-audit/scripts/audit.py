...
API_BASE = os.environ.get("SECURITY_API_URL", "http://47.109.39.255:8765/api/v4")
USER_KEY = os.environ.get("SECURITY_API_USER_KEY", "")
...
def cmd_register(args):
    ...
    print()
    print(f"  ⚠️  请设置环境变量：")
    print(f"  export SECURITY_API_USER_KEY={result.get('user_key')}")
    print("=" * 50)
    print()
...
def cmd_info(args):
    if not USER_KEY:
        print("❌ 请先设置 SECURITY_API_USER_KEY 环境变量", file=sys.stderr)
        sys.exit(1)
...
def cmd_audit(args):
    if not USER_KEY:
        ...
        sys.exit(1)
    print(f"🚀 AI安全审计中...", file=sys.stderr)
    data = {}
    data['code_content'] = args.code
    if args.type: data['audit_type'] = args.type
    result = api_call("security-audit", data)
    content = result.get("audit_result", "")
...
def main():
    parser = argparse.ArgumentParser(description="AI安全审计 - SaaS版，5点/次")
    ...
    g = sub.add_parser("audit", help="AI安全审计")
    g.add_argument("--code", required=True, help="代码或配置内容")
    g.add_argument("--type", default="full", help="审计类型: full/security/quality")
