...
# ── 工具函数 ────────────────────────────────────────────────────────────────
KEYRING_SERVICE = "wechat-article-publisher"
ENV_APP_SECRET = "WECHAT_APP_SECRET"


def load_secret(app_id: str, config_fallback: str = "") -> str:
    """
    三层 fallback 读取 AppSecret（keyring 优先，防 env 残留旧值）：
    1. Windows Credential Manager（keyring，service=KEYRING_SERVICE, username=app_id）
    2. 环境变量 WECHAT_APP_SECRET（CI / 临时覆盖，**警告**）
    3. config.json 中的明文（仅供本地开发，**警告**）
    """
    try:
        import keyring
        kr_val = keyring.get_password(KEYRING_SERVICE, app_id)
        if kr_val:
            print(f"  [SECRET] 来源: Windows Credential Manager (keyring)")
            return kr_val
    except Exception as e:
        print(f"  [SECRET] keyring 读取失败: {type(e).__name__}: {e}")
    env_val = os.environ.get(ENV_APP_SECRET, "").strip()
    if env_val:
    ...
    raise RuntimeError(
        ...
        f"设置环境变量 {ENV_APP_SECRET}、或在 config.json 中临时填写。"
    )
...
class WeChatClient:
    ...
    def upload_image_retry(self, token: str, image_path: Path,
        ...
        """上传图片到永久素材，返回 CDN URL。失败重试 retries 次。"""
        url = f"{MATERIAL_ADD_URL}?access_token={token}&type=image"
        last_err = None
...
def prepare_cover(cover_path: Optional[Path], default_cover_dir: Path,
    ...
    if cover_path and cover_path.exists():
        ...
        # 正文图片已替换为 CDN URL，下载作为封面
        print(f"  下载封面图: {article.first_image_url[:60]}...")
        try:
            r = requests.get(article.first_image_url, timeout=60)
            r.raise_for_status()
            suffix = Path(article.first_image_url).suffix or ".jpg"
            tmp_cover = default_cover_dir / f"cover_cdn{suffix}"
            tmp_cover.write_bytes(r.content)
            src_path = tmp_cover
        except Exception as e:
            print(f"  封面下载失败: {e}，尝试备选方案")
...
def main():
    ...
    if not app_id:
        raise RuntimeError("config.json 缺少 app_id")

    # AppSecret: 环境变量 → keyring → config.json fallback
    app_secret = load_secret(app_id, wc.get("app_secret", ""))
    ...
    # 更新 first_image_url 为 CDN URL（rewrite_local_images 已将正文图片替换为 CDN URL）
    # 找到第一个被替换的 CDN URL 作为封面图候选
    soup_cdn = BeautifulSoup(article.content, "html.parser")
    first_img_cdn = soup_cdn.find("img")
    if first_img_cdn and first_img_cdn.get("src", "").startswith("http"):
        article.first_image_url = first_img_cdn["src"]
