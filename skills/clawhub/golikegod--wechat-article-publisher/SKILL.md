...
tags: [yuanzi, gzh, articles, wechat, publisher, suite:yuanzi-wechat-series, deprecated, redirect:yuanzi-wechat-suite]
version: 2.1.1
metadata:
  homepage: https://github.com/victor-skills/wechat-article-publisher
  series: yuanzi-wechat-series
  series-position: 发布桨
  fork-of: wechat-article-publisher@1.0.0 (zhenglong2015)
  requirements:
    bins: ["python"]
    python_pkgs: ["markdown", "requests", "Pillow", "beautifulsoup4", "pyyaml", "keyring"]
  deprecated: true
...
> ```
>
> 本技能的 `publish_wechat.py` 已迁入 `yuanzi-wechat-suite/scripts/publisher/`，并接入 `yuanzi.py publish` 总调度命令。一装即用，工作流不中断。
...
> 🦞 yuanzi-wechat-series · 第 4/4 站「发布桨」

**安装方式：** `clawhub install yuanzi-wechat-publisher`

本技能属于「元子公众号图文系列」4 件套之一：
1. 写作舵 — `yuanzi-article-master`
2. 读稿锚 — `yuanzi-article-extractor`
3. 配图帆 — `yuanzi-image-generator`
4. **发布桨** — `yuanzi-wechat-publisher`（本技能）

**注：** 本技能另有 `wechat-article-publisher` 同名发布（受 AMBIGUOUS_SKILL_SLUG 限制未合并），使用建议以 yuanzi- 前缀版为准。

推荐工作流：读稿 → 写作 → 配图 → 发布。

v2.1.0 元子系列升级：归 zhenglong2015 v1.0.0，叠加 keyring 凭据 + HTML 输入 + 本地图片 CDN 上传 + 2.35:1 封面强制 + 3 次重试 + token 缓存。

*🦞 元子公众号图文系列 v2.1.0 · 2026-07-04 · yuanzi- 前缀版*