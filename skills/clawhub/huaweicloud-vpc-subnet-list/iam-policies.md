...
| 服务 | 权限（Action） | 说明 | 资源 |
|------|----------------|------|------|
| 虚拟私有云 VPC | `vpc:subnets:get` | 查询子网列表（ListSubnets，只读） | `*`（或限定 `subnets` 资源类型） |
| 统一身份认证 IAM | `iam:projects:listProjects` | 查询项目列表（ListProjects），用于按 Region 自动解析 project_id | `*` |

## 说明

- 仅需**只读**权限，不涉及子网的创建 / 修改 / 删除等写操作。
- 若在配置中显式提供 `project_id`，则不会调用 IAM ListProjects，只需授予 `vpc:subnets:get`。
