...
import -window root /tmp/mt5_nav.png 2>/dev/null
...
img = Image.open('/tmp/mt5_nav.png')
crop = img.crop((0,0,400,720)).resize((1200,2160), Image.LANCZOS)
crop.save('/tmp/nav_big.png')
"""], check=True)
subprocess.run(["tesseract","/tmp/nav_big.png","/tmp/nav","tsv","--psm","11"], check=True, capture_output=True)
found = None
with open('/tmp/nav.tsv') as f:
