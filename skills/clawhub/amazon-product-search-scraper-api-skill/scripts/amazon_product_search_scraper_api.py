...
# API Configuration
TEMPLATE_ID = '113863091927653287'
API_BASE_URL = "https://api.browseract.com/v3/bots"
API_KEY_URL = 'https://www.browseract.com/reception/integrations?co-from=amazon-product-search-scraper'


def run_amazon_product_search_scraper_task(api_key, marketplace_url='https://www.amazon.com', keyword='iPhone', count='10'):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python amazon_product_search_scraper_api.py <marketplace_url> [keyword] [count]", flush=True)
        sys.exit(1)
    ...
    keyword = sys.argv[2] if len(sys.argv) > 2 else 'iPhone'
    count = sys.argv[3] if len(sys.argv) > 3 else '10'

    result = run_amazon_product_search_scraper_task(api_key, marketplace_url, keyword, count)
    if result:
