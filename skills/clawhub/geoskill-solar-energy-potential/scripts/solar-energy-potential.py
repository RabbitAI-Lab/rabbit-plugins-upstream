...
__version__ = "0.2.0"
USER_AGENT = f"solar-energy-potential/{__version__}"
...
def write_qa_summary(qa_path, *, skill, command, args, payload, extra=None):
    ...
    The sidecar includes the user-facing args, the computed metrics and
    a UTC timestamp so QA / regression tests can match run output to inputs.
    """
    ...
    # Only include JSON-serializable primitive values — skip Mock / file-handle /
    # custom objects so the sidecar is always clean.
    for flag in ("lat", "lon", "place", "year", "efficiency", "performance_ratio",
                 "capacity", "cost_per_kwp", "electricity_price", "input",
                 "lat_col", "lon_col", "no_nominatim", "output", "json"):
...
def cmd_assess(args):
    ...
    if args.place:
        if args.lat is not None or args.lon is not None:
            print("ERROR: --place is mutually exclusive with --lat/--lon.", file=sys.stderr)
            return 2
        try:
            from _place import resolve_place as _resolve_place
            place_info = _resolve_place(
                args.place, allow_nominatim=not args.no_nominatim
            )
            args.lat = place_info["lat"]
            args.lon = place_info["lon"]
            print(
                f"[solar-energy-potential] place: "
                f"{place_info.get('display_name') or args.place}",
                file=sys.stderr,
            )
            print(
                f"[solar-energy-potential] resolved to ({args.lat:.4f}, {args.lon:.4f}) "
                f"via {place_info.get('source')}",
                file=sys.stderr,
            )
        except ImportError:
            print("ERROR: _place.py helper not found in this folder.", file=sys.stderr)
            return 2
        except Exception as e:
            print(f"ERROR: --place resolution failed: {e}", file=sys.stderr)
            return 2
    if args.lat is None or args.lon is None:
        print("ERROR: --lat/--lon (or --place) is required.", file=sys.stderr)
        return 2
    ...
    if args.qa:
        write_qa_summary(args.qa, skill="solar-energy-potential", command="assess",
                         args=args, payload=result)
        print(f"QA: {args.qa}")
...
def cmd_batch(args):
    ...
    if args.qa:
        # batch may have many rows; keep the sidecar small by including
        # the summary fields + first row as a sample.
        n_total = len(results)
        n_valid = sum(1 for r in results if "error" not in r)
        write_qa_summary(
            args.qa, skill="solar-energy-potential", command="batch",
            args=args,
            payload={
                "n_total": n_total,
                "n_valid": n_valid,
...
def cmd_economic(args):
    ...
    # Resolve --place if given (v0.2.0)
    if getattr(args, "place", None):
        if args.lat is not None or args.lon is not None:
            print("ERROR: --place and --lat/--lon are mutually exclusive", file=sys.stderr)
            sys.exit(2)
        try:
            ...
            if _os.path.isfile(_shared):
                ...
                pi = place_resolver.resolve_place(args.place, allow_nominatim=not args.no_nominatim)
                args.lat = pi["lat"]
                args.lon = pi["lon"]
                print(f"[place] {args.place} -> ({args.lat:.4f}, {args.lon:.4f})", file=sys.stderr)
            else:
                print(f"ERROR: place_resolver helper not found at {_shared}", file=sys.stderr)
                sys.exit(2)
        except Exception as e:
            print(f"ERROR: --place resolution failed: {e}", file=sys.stderr)
            sys.exit(2)
    if args.lat is None or args.lon is None:
        print("ERROR: either --place or --lat/--lon is required", file=sys.stderr)
        sys.exit(2)
    lat, lon = validate_latlon(args.lat, args.lon)
    year = validate_year(args.year)
    efficiency = validate_efficiency(args.efficiency)
    ...
    if args.qa:
        write_qa_summary(args.qa, skill="solar-energy-potential", command="economic",
                         args=args, payload=result)
        print(f"QA: {args.qa}")
...
Examples:
  ...
  %(prog)s economic --lat 39.9 --lon 116.4 --capacity 5.0 --cost-per-kwp 800
    ...
    p_assess.add_argument("--lat", type=float, help="Latitude (-90 to 90). Use --place instead for natural-language location.")
    p_assess.add_argument("--lon", type=float, help="Longitude (-180 to 180). Use --place instead for natural-language location.")
    p_assess.add_argument(
        "--place",
        help="Place name (Chinese or English). Auto-resolved to lat/lon via Open-Meteo. "
             "Mutually exclusive with --lat/--lon / 行政地名 (自动解析为经纬度)",
    )
    p_assess.add_argument(
        "--no-nominatim",
        action="store_true",
    ...
    p_assess.add_argument("--qa", metavar="PATH", default=None,
    ...
    p_batch.add_argument("--qa", metavar="PATH", default=None,
                         help="Write a JSON run-summary sidecar to PATH (Phase 5).")
    p_batch.set_defaults(func=cmd_batch)

    # ── economic ──
    p_econ = subparsers.add_parser("economic", help="Economic analysis for a location")
    p_econ.add_argument("--lat", type=float, help="Latitude (use --place instead)")
    p_econ.add_argument("--lon", type=float, help="Longitude (use --place instead)")
    p_econ.add_argument(
        "--place",
        help="Place name (Chinese or English). Auto-resolved to lat/lon. / 行政地名",
    )
    p_econ.add_argument("--no-nominatim", action="store_true",
                       help="Skip Nominatim lookup in --place resolution")
    ...
    p_econ.add_argument("--qa", metavar="PATH", default=None,
                        help="Write a JSON run-summary sidecar to PATH (Phase 5).")
