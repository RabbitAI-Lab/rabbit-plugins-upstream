...
# ── transport ────────────────────────────────────────────────────────────────
def _call_stream(cmd, payload):
    """v2.4.0 SSE passthrough on the SAME security path (headers via 0600 file,
    payload via stdin). Prints deltas live; returns (text, http_code, error).
    Non-SSE bodies fall through to a buffered read and normal parsing upstream
    is bypassed — we parse the buffered JSON here instead."""
    ...
    try:
        ...
        proc.stdin.close()
        streamed = False
        carry = ''            # SSE data payloads may split across TCP chunks
        for line in proc.stdout:
            ...
            if s.startswith('data:'):
                ...
                try:
                    ...
                    carry = d            # incomplete JSON — wait for the rest
            else:
                raw.append(line)
        proc.wait()
        # curl's own exit code distinguishes transport failure (28 timeout,
        # 7 refused, 6 DNS …) from an HTTP status when no __H__ marker arrived.
        code = 0
        if '__H__' in tail:
            try:
                code = int(tail.rsplit('__H__', 1)[1].strip())
            except Exception:
                code = 0
        ...
        if streamed:
            text = ''.join(out).strip()
            return (text or None), code, (None if text else 'empty stream')
        # graceful degrade: server answered non-SSE (plain JSON or error body)
        body = ''.join(raw)
        try:
            j = json.loads(body)
            if isinstance(j, dict) and ('error' in j or j.get('type', '').endswith('error')):
                e = j.get('error', j)
                msg = (str(e.get('message', e))[:110] if isinstance(e, dict) else str(e)[:110])
            ...
            c = (c or '').strip()
            if c:
                print(c, end='', flush=True)   # deliver in one piece, still correct
                return c, code, None
            return None, code, 'empty content'
        except Exception:
            return None, code, (body or 'no SSE and unparseable body')[:90]
    except BrokenPipeError:
        proc.kill()
        return None, 0, 'stream broken pipe'
    finally:
        try:
            proc.stdout.close()
        except Exception:
...
def call(prov, model, prompt, system=None, max_tokens=3000, timeout=90,
    ...
    stream=True (v2.4.0, OpenAI-compatible routes only): request SSE and print
    content deltas live as they arrive; returns the assembled text. Non-SSE
    responses degrade gracefully to the normal parse path."""
    try:
        if prov == 'gemini':
        ...
        if stream:
            hdr = hdr + ['Accept: text/event-stream']
    except FileNotFoundError:
    ...
    try:
        ...
        if stream:
            cmd += ['-N', '--no-buffer']
            return _call_stream(cmd, payload)
...
# ── v2.4.0: self-improvement overlay (--learn) ────────────────────────────────
# Bounded learning from live outcomes: per-route EWMA latency + ok/fail counts
# in ~/.cache/ai_router/learn.json. It only REORDERS routes inside a tier
# (multiplier clamped to [0.5, 1.2], neutral 1.0 until 5 observations) — it can
# never disable a route, and cooldowns/state stay authoritative. Zero API cost.
LEARN = os.path.join(HOME, '.cache', 'ai_router', 'learn.json')
...
def _learn_rel(d, prov, model):
    """Reliability multiplier in [0.5, 1.2]; 1.0 until 5 observations."""
    r = d.get(f'{prov}|{model}')
    if not isinstance(r, dict) or r.get('n', 0) < 5:
        return 1.0
    ok = r.get('ok', 0)
    bad = r.get('f429', 0) * 2 + r.get('fother', 0)
    if ok + bad == 0:
        return 1.0
    return max(0.5, min(1.2, 1.2 * ok / (ok + bad)))

def _learn_latency(d, prov, model, default):
    r = d.get(f'{prov}|{model}')
    if isinstance(r, dict) and r.get('ewma_s'):
        try:
            return max(0.05, float(r['ewma_s']))
        except Exception:
            pass
    return default

def _learn_update(prov, model, ok, http, latency_s):
    """Record one live outcome. Append-only per route, pruned at 14 days,
    flock-serialized, atomic replace — learning must never break routing."""
    def _apply(learn):
        k = f'{prov}|{model}'
        r = learn.get(k) if isinstance(learn.get(k), dict) else {}
        prev = r.get('ewma_s')
        try:
            base = float(prev) if prev else float(latency_s)
        except Exception:
            base = float(latency_s)
        r['ewma_s'] = round(0.25 * float(latency_s) + 0.75 * base, 3)
        r['ok'] = r.get('ok', 0) + (1 if ok else 0)
        if not ok:
            if http == 429:
                r['f429'] = r.get('f429', 0) + 1
            else:
                r['fother'] = r.get('fother', 0) + 1
        r['n'] = r.get('n', 0) + 1
        r['last_ts'] = time.time()
        learn[k] = r
        cutoff = time.time() - 14 * 86400
        return {k2: v for k2, v in learn.items()
                if isinstance(v, dict) and v.get('last_ts', 0) >= cutoff}
    try:
        os.makedirs(os.path.dirname(LEARN), exist_ok=True)
        with open(LEARN + '.lock', 'w') as lf:
            fcntl.flock(lf, fcntl.LOCK_EX)
            try:
                try:
                    learn = json.load(open(LEARN))
                    if not isinstance(learn, dict):
                        learn = {}
                except Exception:
                    learn = {}
                learn = _apply(learn)
                fd, tmp = tempfile.mkstemp(prefix=f'.learn-{os.getpid()}-',
                                           dir=os.path.dirname(LEARN))
                with os.fdopen(fd, 'w') as f:
                    json.dump(learn, f)
                os.replace(tmp, LEARN)
            finally:
                fcntl.flock(lf, fcntl.LOCK_UN)
    except Exception:
        pass
...
def candidates(task='general', quality=0, exclude_providers=()):
    _lrn = _learn_load()      # v2.4.0: bounded self-improvement (see --learn)
    rows = [r for r in ROUTES if r[2] >= quality and r[0] not in exclude_providers]
    if task == 'code':
        rows.sort(key=lambda r: (0 if 'code' in r[7] else 1, TIER_ORDER[r[5]], -r[2],
                                 -(r[3] or 0) * _learn_rel(_lrn, r[0], r[1])))
    elif task == 'fast':
        rows.sort(key=lambda r: (TIER_ORDER[r[5]], 'fast' not in r[7],
                                 _learn_latency(_lrn, r[0], r[1], r[6])))
    elif task == 'best':
        rows.sort(key=lambda r: (-r[2], TIER_ORDER[r[5]], r[6]))
    else:                                   # general: cheapest tier, highest rpm, best quality
        rows.sort(key=lambda r: (TIER_ORDER[r[5]],
                                 -(r[3] or 0) * _learn_rel(_lrn, r[0], r[1]), -r[2]))
    return rows
...
def route(prompt, task='general', quality=0, system=None, max_tokens=3000,
    ...
    for prov, model, qual, rpm, rpd, tier, sec, tags in candidates(task, quality):
        ...
        t0 = time.time()
        if stream and prov != 'gemini':     # gemini transport has no SSE path here
            text, code, err = call(prov, model, prompt, system, max_tokens, stream=True)
        else:
        ...
        _locked(_record)
        if learn and code != -1:            # structural failures teach nothing
            _learn_update(prov, model, bool(text), code, lat)
        if text:
            if use_cache and not stream:    # streamed text printed live; not cached
                try:
...
def _apply_discovered(p_, new_ids):
    ...
    """
    global ROUTES, PROVIDER_SPECS   # FIX #26: without this the "hot-reload"
    path = SPECS_FILE               # below assigned locals — stale routing table
    try:
        with open(path) as f:
            doc = json.load(f)
    except Exception:
...
def main():
    ...
    ap.add_argument('--json', action='store_true',
                    help='machine-readable output (ask/status/plan/learn)')
    ap.add_argument('--stream', action='store_true',
                    help='stream the answer live from the chosen route (SSE passthrough)')
    ap.add_argument('--learn', action='store_true',
                    help='show the learned-reliability report (see --no-learn)')
    ap.add_argument('--learn-reset', action='store_true',
                    help='clear the learning overlay (~/.cache/ai_router/learn.json)')
    ap.add_argument('--no-learn', action='store_true',
                    help='do not update the learning overlay for this call')
    a = ap.parse_args()
    ...
    if a.learn_reset:
        try:
            ...
            pass
        print('learning overlay cleared')
        return

    if a.learn:
        d = _learn_load()
        if a.json:
            print(json.dumps({'schema': 'ai_router.learn.v1', 'routes': d}, indent=2))
        else:
            print(f'learned reliability ({len(d)} routes observed):')
            for k, r in sorted(d.items(), key=lambda kv: -kv[1].get('ok', 0)):
                print(f"  {k:54} ok={r.get('ok', 0):4} f429={r.get('f429', 0):3} "
                      f"other={r.get('fother', 0):3} ewma={r.get('ewma_s', 0):5.2f}s")
            if not d:
                print('  (nothing learned yet — routing uses measured tables only)')
        return

    if a.stream and a.json:
        ap.error('--stream and --json are mutually exclusive')
    ...
    text, prov, model, cached = route(prompt, a.task, a.quality, a.system,
                                      a.max_tokens, not a.no_cache, a.verbose,
                                      _json_out=a.json, stream=a.stream,
    ...
    if text is None:
        ...
        # FIX #27: spec providers (e.g. a local Ollama gateway) count as
        # configured — their exhaustion is a routing failure, not "no keys".
        configured += sorted(p for p, s in PROVIDER_SPECS.items()
                             if s.get('auth') == 'none' or _spec_key(p))
        if not configured:
        ...
        sys.exit(2)
    if a.stream and text is not None:
        print()                              # live text already on stdout
    else:
        print(text)
    if a.verbose:
