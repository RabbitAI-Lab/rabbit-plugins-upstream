...
def route(prompt, task='general', quality=0, system=None, max_tokens=3000,
          use_cache=True, verbose=False, tries=6):
    if use_cache:
    ...
    for prov, model, qual, rpm, rpd, tier, sec, tags in candidates(task, quality):
        ...
        if not okflag:
            if verbose: print(f"[skip {prov}/{model}: {why}]", file=sys.stderr)
            continue
...
def main():
    ...
    if a.doctor:
        ...
        for p_ in provs:
            ...
            print(f'  {mark} {p_:11} {note}')
        if not have:
            print('\n' + SETUP_HELP); sys.exit(3)
        print(f'\n  {len(have)}/5 providers configured')
        st = load_state()
        ...
        return
    if a.status:
        st = load_state(); print(f"{'route':58} {'tier':7} {'rpm':>5} {'today':>6}  status")
        for prov, model, qual, rpm, rpd, tier, sec, tags in ROUTES:
            e = st.get(key_of(prov, model), {})
            used = e.get('day_count', 0) if e.get('day') == today() else 0
            okflag, why = available(prov, model, st)
            cap = f"{used}/{rpd}" if rpd else str(used)
            print(f"  {prov+'/'+model:56} {tier:7} {rpm or '-':>5} {cap:>6}  "
                  f"{'✅ ready' if okflag else '⏳ '+why}")
        return
    if a.plan:
        ...
        for i, (prov, model, qual, rpm, rpd, tier, sec, tags) in enumerate(candidates(a.task, a.quality), 1):
            okflag, why = available(prov, model, st)
            print(f"  {i:2}. {prov}/{model:48} q{qual} {tier:7} {'✅' if okflag else '⏳ '+why}")
        return
