---
name: amazon-competitor-weekly-report
description: >
  在用户明确确认并完成报价后，结合主 ASIN 与已授权竞品的商品详情、快照和评论证据生成对照周报。仅用于主动触发的 weekly AI 报告；
  不用于免费监控摘要、小时级监控、实时价格、销量、库存、广告、订单或真实退货率。
  Requires an ARI API key (ari_live_*).
slug: competitor-weekly
displayName: Amazon 竞品周报
version: 1.4.7
summary: 经确认后生成主品与竞品的证据周报
license: MIT
---
...
- 本入口只执行 `weekly/competitor`，输出模板固定为 `ops_weekly`。
