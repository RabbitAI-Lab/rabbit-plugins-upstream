...
# ============================================================
# Apparel Expert Configuration
# ============================================================
try:
    import sys
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
    from templates.apparel_expand_templates import (
        APPAREL_EXPAND_TEMPLATES,
        APPAREL_ALL_MODIFIERS,
        DEFAULT_MODES as APPAREL_DEFAULT_MODES,
        TEMPLATE_PRIORITY,
    )
    HAS_APPAREL_TEMPLATES = True
except ImportError:
    HAS_APPAREL_TEMPLATES = False
    APPAREL_EXPAND_TEMPLATES = {}
    APPAREL_ALL_MODIFIERS = []
    APPAREL_DEFAULT_MODES = ["expand", "az", "numbers", "reverse", "gap"]
    TEMPLATE_PRIORITY = []
...
def generate_prefixes(seed, mode='expand', market_code="US", full_templates=False):
    ...
    if mode in ('expand', 'deep'):
        ...
        # 服装专用模板增强：注入裙长/领型/袖型/版型/场合等，提升完整属性短语产出
        if HAS_APPAREL_TEMPLATES and APPAREL_EXPAND_TEMPLATES:
            for group_name in TEMPLATE_PRIORITY:
                mods = APPAREL_EXPAND_TEMPLATES.get(group_name, [])
                for mod in mods:
                    prefixes.append(f"{seed} {mod}")
                    prefixes.append(f"{mod} {seed}")
                    if not mod.startswith("for "):
                        prefixes.append(f"{seed} for {mod}")
    
    # 空格间隙插入法：只在 gap 模式启用（不再默认混入 expand，避免前缀爆炸）
    if mode == 'gap':
        words = seed.split()
        if len(words) >= 2:
            # 服装专用修饰词优先，显著提升完整属性短语产出
            if HAS_APPAREL_TEMPLATES and APPAREL_ALL_MODIFIERS:
                common_mid_modifiers = APPAREL_ALL_MODIFIERS[:40]  # 控制请求量
            else:
...
# ============================================================
# LinkFox aigc-textgen LLM backend (direct HTTP, no subprocess)
# ============================================================
def linkfox_llm_call(system_prompt, user_prompt):
    """Call linkfox-aigc-textgen API directly via HTTP for semantic tagging.

    Creates an async text generation task, polls until done, returns content.
    No subprocess overhead — direct urllib calls to the LinkFox gateway.
    """
    import time as _time
    from urllib.request import urlopen as _urlopen, Request as _Request

    _GATEWAY = (os.environ.get("LINKFOX_TOOL_GATEWAY") or "https://tool-gateway.linkfox.com").rstrip("/")
    _API_KEY = os.environ.get("LINKFOX_AGENT_API_KEY") or os.environ.get("LINKFOXAGENT_API_KEY")
    if not _API_KEY:
        raise RuntimeError("LINKFOX_AGENT_API_KEY not set")
    ...
    # 1. Create task
    create_resp = _post("/aigc/textGenAsync", params)
    task_id = create_resp.get("taskId")
    if not task_id:
        raise RuntimeError(f"No taskId in response: {json.dumps(create_resp, ensure_ascii=False)[:300]}")

    # 2. Poll until done (max 600s)
    start = _time.time()
    interval = 5
    while _time.time() - start < 600:
        _time.sleep(interval)
        result = _post("/aigc/textTaskQuery", {"taskId": task_id, "memberId": ""})
        status = result.get("status")
        if status == "SUCCESS":
            # Extract content from data.content or top-level content
            data = result.get("data") or result
            content = data.get("content") if isinstance(data, dict) else None
            if not content:
                content = result.get("content")
            if not content:
                raise RuntimeError(f"No content in SUCCESS response: {json.dumps(result, ensure_ascii=False)[:300]}")
            # Decode ⏎ placeholder to newlines
            return content.replace("\u23ce", "\n")
        if status == "FAILED":
            raise RuntimeError(f"Task FAILED: {json.dumps(result, ensure_ascii=False)[:300]}")
        interval = max(interval - 1, 3)

    raise RuntimeError(f"Polling timeout after 600s for taskId={task_id}")


# ============================================================
# Optimized tagging: rule pre-filter + two-stage LLM
# ============================================================

def _safe_float(val, default=0.8):
...
_COARSE_SYSTEM = """You are an Amazon Apparel Keyword classifier. Classify each keyword into exactly one library based on the Product Context.
...
_COARSE_USER_TMPL = """【Product Context】
{product_context}
...
_FINE_SYSTEM = """You are an Amazon Apparel Keyword attribute expert. For each keyword, identify its primary_type and whether it contains a complete attribute phrase.
...
_FINE_USER_TMPL = """【Product Context】
{product_context}
...
def _rule_prefilter(keyword, pc_lower, seed_lower):
    """Return (library, primary_type, reason) or None if ambiguous."""
    ...
    conflict_cats = [c for c in non_dress_cats if c not in ["skirt"]] if is_dress else []
    ...
    if is_womens and not is_mens:
        if re.search(r'\bmen\b|\bmen\'s\b|\bboys?\b|\bgentleman\b', kw_lower):
            if not re.search(r'\bwomen\b|\bwomen\'s\b|\bladies?\b', kw_lower):
                return ("negative", "Audience", "gender_conflict")
    ...
    if has_long_sleeve and not has_short_sleeve:
        if "sleeveless" in kw_lower or "short sleeve" in kw_lower or "short-sleeve" in kw_lower:
            return ("negative", "Sleeve Type", "sleeve_conflict")
    ...
    if is_mini and not is_midi and not is_maxi:
        if "maxi" in kw_lower or "floor length" in kw_lower or "ankle length" in kw_lower:
            return ("negative", "Dress Length", "length_conflict:maxi_for_mini")
        if "midi" in kw_lower and "above the knee" not in kw_lower:
            return ("negative", "Dress Length", "length_conflict:midi_for_mini")
    if is_maxi and not is_mini and not is_midi:
        if "above the knee" in kw_lower or "mini" in kw_lower or "mid thigh" in kw_lower:
            return ("negative", "Dress Length", "length_conflict:mini_for_maxi")
    ...
    # 7. Obvious positive: seed match
    if kw_lower == seed_lower or kw_lower == seed_lower + "s":
        return ("positive", "Core Product", "seed_match")

    # 8. Obvious positive: multiple attribute match
    if is_dress and is_womens:
        has_dress = "dress" in kw_lower
        has_women = any(w in kw_lower for w in ["women", "women's", "ladies"])
        has_ls = "long sleeve" in kw_lower or "long-sleeve" in kw_lower
        has_floral = "floral" in kw_lower
        has_vn = "v neck" in kw_lower or "v-neck" in kw_lower
        has_pkt = "pocket" in kw_lower
        match_count = sum([has_dress, has_ls, has_floral, has_vn, has_pkt, has_women])
...
def optimized_tag_and_split(keywords, seed, product_context, batch_size=100, llm_call_fn=None):
    """Rule pre-filter + two-stage LLM tagging. Returns same structure as llm_tag_and_split."""
    from llm_tagger import validate_item, batch_list

    if llm_call_fn is None:
        from llm_tagger import default_llm_call
        llm_call_fn = default_llm_call

    pc_lower = (product_context or "").lower()
    seed_lower = seed.lower()

    # Stage 0: Rule pre-filter
    rule_pos, rule_neg, ambiguous = [], [], []
    for kw in keywords:
        result = _rule_prefilter(kw, pc_lower, seed_lower)
        if result is None:
            ambiguous.append(kw)
        else:
            ...
            (rule_pos if library == "positive" else rule_neg).append(item)

    print(f"  规则预筛: 肯定={len(rule_pos)} 否定={len(rule_neg)} 疑难={len(ambiguous)} ({100*(1-len(ambiguous)/max(len(keywords),1)):.0f}%规则处理)")

    # Stage 1: LLM coarse split (ambiguous only)
    coarse_results = []
    for batch in batch_list(ambiguous, batch_size):
        user_prompt = _COARSE_USER_TMPL.format(
            product_context=product_context or "Not provided", seed=seed,
            n=len(batch),
        ...
        for attempt in range(2):
            try:
                raw = llm_call_fn(_COARSE_SYSTEM, user_prompt)
                ...
                print(f"    coarse attempt {attempt+1} failed: {e}")
                time.sleep(3)
        if not ok:
            for kw in batch:
    ...
    print(f"  LLM粗分: 肯定={len(coarse_pos)} 否定={sum(1 for r in coarse_results if r['library']=='negative')} 待确认={sum(1 for r in coarse_results if r['library']=='review')}")

    # Stage 2: LLM fine tag (positive only)
    fine_kws_all = [r["keyword"] for r in rule_pos] + [r["keyword"] for r in coarse_pos]
    fine_results = {}
    for batch in batch_list(fine_kws_all, batch_size):
        user_prompt = _FINE_USER_TMPL.format(
            product_context=product_context or "Not provided", n=len(batch),
            keyword_list="\n".join(f"{i+1}. {kw}" for i, kw in enumerate(batch))
        )
        ok = False
        for attempt in range(2):
            try:
                raw = llm_call_fn(_FINE_SYSTEM, user_prompt)
                raw_stripped = raw.strip()
                if raw_stripped.startswith("{"):
                    parsed = json.loads(raw_stripped)
                    items = parsed.get("results", parsed.get("data", []))
                ...
                for item in items:
                    ...
                    fine_results[kw] = {
                        ...
                        "attribute_categories": item.get("attribute_categories", []) if isinstance(item.get("attribute_categories"), list) else [],
                        "is_complete_attribute_phrase": bool(item.get("is_complete_attribute_phrase", False)),
                        "suggested_positions": item.get("suggested_positions", []) if isinstance(item.get("suggested_positions"), list) else [],
                        "confidence": _safe_float(item.get("confidence", 0.8)),
                    }
                ok = True
                break
            except Exception as e:
                print(f"    fine attempt {attempt+1} failed: {e}")
                time.sleep(3)
        if not ok:
            for kw in batch:
                fine_results[kw] = {
                    "primary_type": "Core Product", "attribute_categories": [],
                    "is_complete_attribute_phrase": False,
                    "suggested_positions": ["title", "bullet"], "confidence": 0.6,
                }
    ...
    for item in rule_pos:
        ...
        if kw in fine_results:
            ft = fine_results[kw]
            item["primary_type"] = ft["primary_type"]
            item["attribute_categories"] = ft["attribute_categories"] if isinstance(ft["attribute_categories"], list) else []
            item["is_complete_attribute_phrase"] = ft["is_complete_attribute_phrase"]
            item["suggested_positions"] = ft["suggested_positions"] if isinstance(ft["suggested_positions"], list) else []
            try:
                item["confidence"] = float(ft["confidence"])
            except (TypeError, ValueError):
                item["confidence"] = 0.7
        all_results.append(item)

    for r in coarse_results:
        kw, library, relevance = r["keyword"], r["library"], r["relevance"]
        if library == "positive" and kw in fine_results:
            ft = fine_results[kw]
            all_results.append(validate_item({
                "keyword": kw, "primary_type": ft["primary_type"],
                "secondary_types": [], "attribute_categories": ft["attribute_categories"],
                "is_complete_attribute_phrase": ft["is_complete_attribute_phrase"],
                "relevance": relevance, "library": library,
                "suggested_positions": ft["suggested_positions"], "confidence": ft["confidence"],
            }))
    ...
    return {"results": all_results, "positive": pos, "negative": neg, "review": rev, "summary": summary}
...
def main():
    ...
    # 模式（服装专家默认：expand,az,numbers,reverse,gap；支持逗号分隔多模式）
    parser.add_argument('--mode', '-m', 
                        default='expand,az,numbers,reverse,gap', 
                        help='运行模式，支持逗号分隔多模式。可选: expand | az | az_prefix | numbers | gap | reverse | deep。服装默认: expand,az,numbers,reverse,gap')
    parser.add_argument('--product-context', type=str, default='',
                        help='产品上下文描述（强烈推荐，提升 LLM 打标准确度）')
    parser.add_argument('--product-context-file', type=str, default='',
                        help='从文件读取产品上下文（避免 shell 转义问题，与 --product-context 二选一）')
    parser.add_argument('--llm-backend', type=str, default='linkfox',
                        choices=['linkfox', 'default'],
                        help='LLM 打标后端：linkfox=使用 linkfox-aigc-textgen（默认）；default=使用 default_llm_call（需 XAI/OPENAI API Key）')
    parser.add_argument('--tag', action='store_true', default=True,
                        help='开启 LLM 语义打标与词库分流（默认开启）')
    parser.add_argument('--no-tag', action='store_true',
                        help='关闭 LLM 打标，仅输出原始挖掘结果')
    parser.add_argument('--batch-size', type=int, default=100,
                        help='LLM 打标每批关键词数量（默认100）')
    ...
    # ============================================================
    # 执行：支持多模式串联（服装默认 expand,az,numbers,reverse,gap）
    # ============================================================
    verbose = args.verbose
    do_tag = getattr(args, 'tag', True) and not getattr(args, 'no_tag', False)
    product_context = getattr(args, 'product_context', '') or ''
    # 支持从文件读取 product_context
    pc_file = getattr(args, 'product_context_file', '') or ''
    if pc_file:
    ...
    # 解析多模式
    valid_modes = {'expand', 'az', 'az_prefix', 'numbers', 'gap', 'reverse', 'deep'}
    mode_list = [m.strip().lower() for m in args.mode.split(',') if m.strip()]
    mode_list = [m for m in mode_list if m in valid_modes]
    if not mode_list:
        mode_list = ['expand', 'az', 'numbers', 'reverse', 'gap']

    if verbose:
        print(f"\n🎯 服装专家模式组合: {', '.join(mode_list)}")
        if product_context:
            print(f"📝 产品上下文: {product_context[:80]}{'...' if len(product_context) > 80 else ''}")
        print(f"🏷️  LLM 打标: {'开启' if do_tag else '关闭'}")

    all_results = []
    merged_keywords = {}   # keyword -> info dict (dedup across modes)
    merged_widget_items = []
    ...
    for mk in target_markets:
        ...
        for local_seed, trans_source in candidates:
            ...
            for current_mode in mode_list:
                ...
                elif current_mode == 'gap':
                    result = mode_gap(local_seed, delay=args.delay,
                                     jitter_ratio=getattr(args, 'jitter', 0.6),
                                     verbose=verbose, market_code=mk,
                                     full_templates=args.full_templates)
                elif current_mode == 'reverse':
                    result = mode_reverse(local_seed, top_n=args.top_n or 100, delay=args.delay,
                                         verbose=verbose, market_code=mk)
                ...
                for kw in result.get('keywords', []):
                    k = kw.get('keyword', '').strip().lower()
                    if k and k not in merged_keywords:
                        merged_keywords[k] = kw
                for wi in result.get('widget_items', []):
                    merged_widget_items.append(wi)
    ...
    # 汇总原始词列表
    raw_keyword_list = list(merged_keywords.values())
    raw_keyword_strs = [kw.get('keyword', '') for kw in raw_keyword_list if kw.get('keyword')]

    if verbose:
        print(f"\n📊 多模式合并后去重关键词总数: {len(raw_keyword_strs)}")

    # ============================================================
    # LLM 打标 + 词库分流
    # ============================================================
    tagged_payload = None
    if do_tag and raw_keyword_strs:
        try:
            # 选择 LLM 后端
            llm_backend = getattr(args, 'llm_backend', 'linkfox')
            if llm_backend == 'linkfox':
                if verbose:
                    print(f"\n🏷️  开始优化打标（共 {len(raw_keyword_strs)} 词，batch={batch_size}，后端=linkfox-aigc-textgen）...")
                    print(f"    流程: 规则预筛 → LLM粗分(疑难词) → LLM细标(肯定词)")
                tagged_payload = optimized_tag_and_split(
                    keywords=raw_keyword_strs,
                    seed=args.seed,
                    product_context=product_context,
                    batch_size=batch_size,
                    llm_call_fn=linkfox_llm_call,
                )
            else:
                from llm_tagger import llm_tag_and_split
                if verbose:
                    print(f"\n🏷️  开始 LLM 语义打标（共 {len(raw_keyword_strs)} 词，batch={batch_size}，后端=default_llm_call）...")
                tagged_payload = llm_tag_and_split(
                    keywords=raw_keyword_strs,
                    seed=args.seed,
                    product_context=product_context,
                    batch_size=batch_size,
            ...
            if verbose:
                s = tagged_payload.get('summary', {})
                print(f"  ✅ 打标完成 → 肯定:{s.get('positive_count',0)}  否定:{s.get('negative_count',0)}  待确认:{s.get('review_count',0)}  完整属性短语:{s.get('complete_attribute_phrase_count',0)}")
        except Exception as e:
            print(f"  ⚠️ LLM 打标失败，降级为仅输出原始挖掘结果: {e}")
            tagged_payload = None
    ...
    main_result = {
        ...
        'product_context': product_context,
        'results': all_results,
        'keywords': raw_keyword_list,
        'total_keywords': len(raw_keyword_strs),
        'total_widget_items': len(merged_widget_items),
    ...
    # 优先使用服装专用 Excel 写出；失败则回退原 save_xlsx
    if args.xlsx:
        try:
            ...
            if tagged_payload:
                write_apparel_excel(
                    ...
                    seed=args.seed,
                    product_context=product_context,
                    tagged_results=tagged_payload.get('results', []),
                    positive=tagged_payload.get('positive', []),
                    negative=tagged_payload.get('negative', []),
                    review=tagged_payload.get('review', []),
                    raw_keywords=raw_keyword_strs,
                    summary=tagged_payload.get('summary', {}),
                    modes=mode_list,
                    market=target_markets[0] if target_markets else 'US',
                )
                print(f"\n✅ 服装专家 Excel 已保存: {args.xlsx}")
                print(f"   Sheet: 摘要 / 打标明细 / 肯定词库 / 否定词库 / 待确认词库 / 完整属性短语 / 原始挖掘结果")
            else:
                # 无打标结果时仍写原始结构
                save_xlsx(main_result, args.xlsx)
        except Exception as e:
            print(f"  ⚠️ 服装 Excel 写出失败，回退原格式: {e}")
            save_xlsx(main_result, args.xlsx)
