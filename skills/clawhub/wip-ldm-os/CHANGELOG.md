...
## 0.4.85-alpha.33 (2026-07-06)

Installer/boot P1 batch: single-owner boot-hook registration (deploy.mjs), one boot-hook deploy truth + doctor stale-at-exec check (boot-dir-truth), SessionStart dedupe + doctor repairs, boot-payload trim (~42% smaller), bin ownership manifest + self-heal + prepublish gate, dev-guide into installer library, MCP install hardening phases 3a-3d (registerMCP precheck, stale-entry cleanup, doctor MCP path check, buildSourceInfo parent-repo fix), universal-installer eight-interface doc alignment.

## 0.4.85-alpha.32 (2026-07-05)

boot-hook SessionStart dedupe + persist fix, ldm doctor duplicate-hook collapse + control-char invalid-model repair with backups; boot payload trim with staleness cutoff and payload summary; security.md doctor-strips-keys rule

## 0.4.85-alpha.31 (2026-07-04)

boot-hook SessionStart dedupe + persist fix, ldm doctor duplicate-hook collapse + control-char invalid-model repair with timestamped backups (PR #1086); boot payload trim: default step caps, staleness cutoff, payload summary, 45.1KB to 26.2KB (PR #1088); security.md doctor-strips-keys rule (PR #1089); speedup master plan + dispatcher design docs (PR #1087, #1090). npm publish deferred: token expired, pending Parker

## 0.4.85-alpha.30 (2026-05-13)

alpha prerelease

## 0.4.85-alpha.29 (2026-05-13)

alpha prerelease

## 0.4.85-alpha.28 (2026-05-13)

Phase 1 source-types refactor: honest cleanup of legacy source.npm 404 entries. ldm install auto-migrates ~/.ldm/extensions/registry.json (with timestamped backup): bad-probe entries move to updateSource.type=untracked with legacy source.npm and source.repo preserved in provenance; phantom rows (no on-disk directory) and known-duplicate pairs removed and reported as explicit summary deltas. ldm status adds an Untracked extensions section. ldm doctor warns on future source.npm 404 drift. Per-type dispatch (git, bundled, private) and ldm doctor --reclassify-sources come in Phase 2.

## 0.4.85-alpha.27 (2026-05-13)

alpha prerelease

## 0.4.85-alpha.26 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.25 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.24 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.23 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.22 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.21 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.20 (2026-05-12)

alpha prerelease

## 0.4.85-alpha.19 (2026-05-12)

Require daemon identity acceptance before replacing an online Remote Control daemon socket.

## 0.4.85-alpha.18 (2026-05-12)

Reject daemon reconnect E2EE key replacement unless the key change goes through fresh-presence pair-complete.

## 0.4.85-alpha.17 (2026-05-12)

Fix Remote Control daemon reconnect after pair/relink audit deployment by casting E2EE audit timestamps to Postgres timestamptz.

## 0.4.85-alpha.16 (2026-05-11)

Remote Control now requires fresh passkey-derived pair presence for daemon public-key pairing and relink, records daemon key audit fingerprints, and invalidates stale browser E2EE sessions when a daemon key is replaced.

## 0.4.85-alpha.15 (2026-05-11)

alpha prerelease

## 0.4.85-alpha.14 (2026-05-11)

alpha prerelease

## 0.4.85-alpha.13 (2026-05-11)

alpha prerelease

## 0.4.85-alpha.12 (2026-05-11)

Include the Remote Control E2EE registry module in hosted MCP deploy manifests

## 0.4.85-alpha.11 (2026-05-11)

Add Remote Control E2EE restart regression coverage for persisted keys and daemon reconnect self-heal

## 0.4.85-alpha.10 (2026-05-11)

Remote Control persists daemon E2EE pubkeys and restores them on hosted relay startup

## 0.4.85-alpha.9 (2026-05-06)

alpha prerelease

## 0.4.85-alpha.8 (2026-05-06)

alpha prerelease

## 0.4.85-alpha.7 (2026-05-06)

alpha prerelease

## 0.4.85-alpha.6 (2026-05-06)

Align WIP AI Chat UI installer docs with the public npm skill package path.

## 0.4.85-alpha.5 (2026-05-06)

alpha prerelease

## 0.4.85-alpha.4 (2026-05-06)

alpha prerelease

## 0.4.85-alpha.3 (2026-04-30)

dev-guide compat-write at ~/.ldm/shared/ during migration window (PR #768)

## 0.4.85-alpha.2 (2026-04-29)

alpha prerelease

## 0.4.85-alpha.1 (2026-04-29)

ldm install --alpha now upgrade-aware (passes --alpha to npm pack); new ldm uninstall <pkg> [--dry-run] for per-package removal. Pairs with wip-codex-remote-control alpha.5.
