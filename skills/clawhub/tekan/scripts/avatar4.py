#!/usr/bin/env python3
"""Generate a talking avatar video from a photo using Tekan Avatar4.
...
_DEFAULT_CREDITS_PER_SECOND = 0.1
_CHAR_CREDITS_PER_15 = 0.1
_OFF_PEAK_DISCOUNT = 0.5


def estimate_cost(mode: str | None = None,
                  text: str | None = None,
                  audio_duration: float | None = None,
                  off_peak: bool = False) -> int:
    """Return estimated Tekan credits for an avatar4 task.

    Priority: audio_duration > text character count.
    """
    cps = _CREDITS_PER_SECOND.get(mode or "avatar4", _DEFAULT_CREDITS_PER_SECOND)

    if audio_duration and audio_duration > 0:
        import math
        topview = math.ceil(audio_duration) * cps
        if off_peak:
            topview *= _OFF_PEAK_DISCOUNT
        return to_tekan_credit(topview)

    if text and text.strip():
        import math
        char_count = len(text.strip())
        topview = math.ceil(char_count / 15) * _CHAR_CREDITS_PER_15
        if off_peak:
            topview *= _OFF_PEAK_DISCOUNT
        return to_tekan_credit(topview)
...
def print_result(result: dict, args) -> None:
    ...
    if board_task_id and board_id:
        print(f"  edit: https://video.tekan.cn/board/{board_id}?boardResultId={board_task_id}")
...
def _guard(client, args):
    """Check credits before submission."""
    required = estimate_cost(
        mode=getattr(args, "mode", None),
        text=getattr(args, "text", None),
    )
    client.guard_credit(required, "avatar4")
...
def main():
    parser = argparse.ArgumentParser(
        description="Tekan Avatar4 — generate a talking avatar video from a photo.",
