#!/usr/bin/env python3
"""Generate videos using Tekan Common Task APIs.
...
def estimate_cost(model: str, resolution: int | None, duration: int,
                  sound_on: bool = False, count: int = 1) -> int | None:
    """Return estimated total cost in Tekan credits (TopView ×100), or None if unknown."""
    ...
    for key in [(res, sound_on), (res, None), (0, sound_on), (0, None)]:
        if key in rates:
            topview_cost = rates[key] * duration * count
            return to_tekan_credit(topview_cost)
    return None
...
def _guard(client, args):
    ...
    sound_on = getattr(args, "sound", None) == "on"
    model = getattr(args, "model", None) or ""
    duration = getattr(args, "duration", None) or 5
    resolution = getattr(args, "resolution", None)
    count = getattr(args, "count", None) or 1
    required = estimate_cost(model, resolution, duration, sound_on, count)
    client.guard_credit(required or 0, "video_gen")
...
def main():
    parser = argparse.ArgumentParser(
        description="Tekan Video Generation — i2v / t2v / omni reference.",
