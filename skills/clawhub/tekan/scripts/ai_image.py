#!/usr/bin/env python3
"""Generate images or edit existing images using Tekan Common Task APIs.
...
def estimate_cost(task_type: str, model: str, resolution: str | None,
    ...
    if resolution and resolution in prices:
        return to_tekan_credit(prices[resolution] * count)
    if "default" in prices:
        return to_tekan_credit(prices["default"] * count)
    return None
...
def _guard(client, args):
    ...
    task_type = getattr(args, "type", "") or ""
    model = getattr(args, "model", None) or ""
    resolution = getattr(args, "resolution", None)
    count = getattr(args, "count", None) or 1
    required = estimate_cost(task_type, model, resolution, count)
    client.guard_credit(required or 0, "ai_image")
...
def main():
    parser = argparse.ArgumentParser(
        description="Tekan AI Image — text-to-image and image editing.",
