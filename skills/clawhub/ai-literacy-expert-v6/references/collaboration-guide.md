...
## 九、WorkBuddy 原生协作（V5 升级 · 推荐路径）

> 本指南原有的「自建实时协作 Web App（OT 算法/自建服务器）」仍可作为离线/私有化方案保留；但在 **WorkBuddy** 中，**优先使用原生能力**，零运维、即可协作。

### 9.1 协作载体映射

| 协作需求 | WorkBuddy 原生落地 | 替代原方案 |
|----------|-------------------|------------|
| 多人实时编辑 | **腾讯文档**（原生资料库，支持多人协同/评论/版本历史） | 自建 OT 服务器 |
| 共享素材库 | **IMA 知识库**（备课组共享检索课程素材） | 自建素材服务 |
| 分发与提醒 | WorkBuddy **分享工具**发文档链接 + **日历事件**排备课会 + **备忘录**记待办 | 自建通知 |
| 角色分工 | 腾讯文档内不同作者/批注者身份（主讲/助教/审核） | 自建权限系统 |

### 9.2 标准协作流程（WorkBuddy 内）

```
① 创建：用腾讯文档建备课文档（或 IMA 知识库建课程文件夹）
② 分配：在文档/群内明确主讲、助教、审核角色
③ 编辑：各人负责模块，腾讯文档实时同步
④ 批注：用评论/@提及讨论，IMA 存共享素材
⑤ 分发：WorkBuddy 分享工具发链接给教研组；日历排"备课评审会"；备忘录记"发布前待办"
⑥ 发布：定稿后存 IMA 知识库 + 分享终版
```

### 9.3 门控对齐
- 协作专项门控（原第九章）的"实时同步/冲突检测/版本历史/权限"在腾讯文档中由平台原生保障；技能仅需确认：文档已创建、协作者已邀请、分享范围正确。
- 离线编辑能力由腾讯文档/IMA 客户端原生提供，不再要求自建 OfflineQueue。
