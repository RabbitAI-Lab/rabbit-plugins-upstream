...
# API Configuration
TEMPLATE_ID = '113866346164408743'
API_BASE_URL = "https://api.browseract.com/v3/bots"
API_KEY_URL = 'https://www.browseract.com/reception/integrations?co-from=article-content-extractor'


def run_article_content_extractor_task(api_key, article_urls='https://blog.apify.com/what-is-web-scraping/', count='5'):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python article_content_extractor_api.py <article_urls> [count]", flush=True)
        sys.exit(1)
    ...
    article_urls = sys.argv[1] if len(sys.argv) > 1 else 'https://blog.apify.com/what-is-web-scraping/'
    count = sys.argv[2] if len(sys.argv) > 2 else '5'

    result = run_article_content_extractor_task(api_key, article_urls, count)
    if result:
