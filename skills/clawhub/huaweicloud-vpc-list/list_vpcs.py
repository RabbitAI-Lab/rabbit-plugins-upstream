#!/usr/bin/env python3
"""查询华为云 VPC 列表（输出 VPC 名称 + ID 精简列表）。

读取配置文件中的 AK/SK（region 默认 cn-north-4 可配置），通过华为云
ListVpcs 接口查询指定 Region 下的 VPC 列表，仅输出 name 与 id。

用法:
    python3 list_vpcs.py [-c 配置文件路径] [-r region]
...
VPC_ENDPOINT = "https://vpc.{region}.myhuaweicloud.com/v1/{project_id}/vpcs"
IAM_PROJECTS_ENDPOINT = "https://iam.myhuaweicloud.com/v3/projects"
PAGE_LIMIT = "1000"


class VpcListError(Exception):
    """VPC 查询错误基类。"""


class ConfigError(VpcListError):
    """配置错误（凭证缺失、配置文件异常等）。"""


class AuthError(VpcListError):
    """认证失败（AK/SK 无效或无权访问）。"""


class ApiError(VpcListError):
    """接口调用异常（网络错误、服务端错误等）。"""
...
def _extract_next_marker(data):
    """从 ListVpcs 响应的 vpcs_links 中解析下一页 marker（OpenStack 风格分页）。

    真实响应为 vpcs_links 列表，每项形如 {"href": "...?marker=xxx", "rel": "next"}，
    通过 href 中的 marker 参数继续翻页；无下一页时返回 None。
    """
    for link in data.get("vpcs_links") or []:
...
def list_vpcs(ak, sk, region, project_id=None):
    """查询指定 Region 的 VPC 列表，返回 [{'name':..., 'id':...}, ...]。"""
    pid = resolve_project_id(ak, sk, region, project_id)
    base = VPC_ENDPOINT.format(region=region, project_id=pid)
    vpcs = []
    marker = None
    while True:
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="查询华为云 VPC 列表，输出 VPC 名称 + ID。")
    parser.add_argument(
    ...
    try:
        ...
        vpcs = list_vpcs(cfg["ak"], cfg["sk"], region, cfg["project_id"])
    except VpcListError as exc:
        print(f"[错误] {exc}", file=sys.stderr)
        return 1
    if not vpcs:
        print(f"Region {region} 下暂无 VPC。")
        return 0
    print(f"Region {region} 的 VPC 列表（共 {len(vpcs)} 个）：")
    for vpc in vpcs:
