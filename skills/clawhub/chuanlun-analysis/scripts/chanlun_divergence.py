...
class DivergenceDetector:
    ...
    def __init__(self, df: pd.DataFrame, df_original: pd.DataFrame, strokes: List[Dict]):
        """
        初始化背驰检测器
        
        Args:
            df: DataFrame，包含 OHLCV 数据
            df_original: DataFrame，包含原始 OHLCV 数据
    ...
    def detect_trend_divergence(self) -> Dict:
        ...
        if len(up_strokes) >= 2:
            for i in range(len(up_strokes) - 1):
                ...
                # 计算 MACD 柱面积 (使用kline_start和kline_end作为K线索引)
                area_b = self.calculate_histogram_area(stroke_b.get('kline_start', stroke_b['start']), 
                                                       stroke_b.get('kline_end', stroke_b['end']))
                area_c = self.calculate_histogram_area(stroke_c.get('kline_start', stroke_c['start']), 
                                                       stroke_c.get('kline_end', stroke_c['end']))
    ...
    def detect_consolidation_divergence(self, pivots: List[Dict]) -> Dict:
        ...
        # 计算面积 (使用kline_start和kline_end作为K线索引)
        area_entry = self.calculate_histogram_area(entry_stroke.get('kline_start', entry_stroke['start']), 
                                                   entry_stroke.get('kline_end', entry_stroke['end']))
        area_exit = self.calculate_histogram_area(exit_stroke.get('kline_start', exit_stroke['start']), 
                                                  exit_stroke.get('kline_end', exit_stroke['end']))
