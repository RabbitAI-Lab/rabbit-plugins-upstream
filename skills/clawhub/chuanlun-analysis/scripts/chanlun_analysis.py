...
class TimeoutTimer:
    """跨平台超时计时器（Windows / Linux / macOS 通用）
    使用 threading.Timer 替代 Unix 专属的 signal.alarm()，
    在超时时间到达后，通过 os._exit() 强制终止整个进程，
    避免因主线程阻塞导致 Timer 回调无法触发的问题。
    """
    ...
    @staticmethod
    def _on_timeout(seconds):
        """超时回调 —— 强制退出整个进程"""
        print(f'\n[X] 脚本执行超时 ({seconds}秒)，自动退出...')
        # 使用 os._exit 强制终止进程（包括所有线程），
        # 因为 sys.exit() 仅抛出 SystemExit，在主线程阻塞时无法被捕获
        os._exit(1)
...
# 注册 Ctrl+C 信号处理（Windows / Linux / macOS 均支持）
# signal.signal(signal.SIGINT, signal_handler)

# SIGTERM 在 Windows 下行为不同，仅在非 Windows 平台注册
...
def generate_report(stock_code: str, analyzer, output_dir: str = './workspace', chart_path: str = None) -> str:
    ...
    # 背驰检测
    detector = DivergenceDetector(analyzer.processed_klines, analyzer.df, strokes)
...
"""
    if divergence['detected']:
        report += f"| **背驰类型** | {'[涨] 看涨背驰（底背驰）' if divergence['type'] == 'bullish' else '[跌] 看跌背驰（顶背驰）'} |\n"
...
def main():
    ...
    parser.add_argument('--period', default='daily', help='周期：daily, weekly, monthly 等 (默认：daily)')
    parser.add_argument('--bars', default=500, type=int, help='K线数量 (默认：550)')
    parser.add_argument('--output', default='./workspace', help='输出目录 (默认：./workspace)')
    ...
    # 创建跨平台超时计时器并启动
    #timeout_timer = TimeoutTimer(SCRIPT_TIMEOUT)
    #timeout_timer.start()
    ...
    try:
        # 创建分析器
        print('\n[1/5] 获取股票数据...')
        analyzer = analyze_stock(args.stock_code, args.period, args.bars)
        print(f'[OK] 数据获取成功，共 {len(analyzer.df)} 根 K 线')

        # 识别分型
        print('\n[2/5] 识别分型...')
        fractals = analyzer.identify_fractals()
        print(f'[OK] 顶分型：{len(fractals["tops"])}, 底分型：{len(fractals["bottoms"])}')

        # 识别笔
        print('\n[3/5] 识别笔...')
        strokes = analyzer.identify_strokes()
        print(f'[OK] 笔数量：{len(strokes)}')

        # 识别线段和中枢
        print('\n[4/5] 识别线段和中枢...')
        segments = analyzer.identify_segments()
        pivots = analyzer.identify_pivots()
        print(f'[OK] 线段：{len(segments)}, 中枢：{len(pivots)}')
        ...
        divergence = detector.detect_trend_divergence()
        if divergence['detected']:
            print(f'[OK] 发现背驰：{"看涨" if divergence["type"] == "bullish" else "看跌"} (置信度：{divergence["confidence"]:.1%})')
        else:
            print('[OK] 未发现背驰')
