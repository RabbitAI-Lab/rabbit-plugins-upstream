...
class ChanLunChart:
    ...
    def plot_full_chart(self, stock_code: str, save: bool = True, show_macd: bool = True) -> str:
        ...
        if show_macd:
            ...
            ax_macd.set_xlim(ax_kline.get_xlim())
    ...
    def _plot_fractals(self, ax):
        ...
        for top in fractals.get('tops', []):
            ax.annotate('T', xy=(top['kline'], top['price']), 
        ...
        for bottom in fractals.get('bottoms', []):
            ax.annotate('B', xy=(bottom['kline'], bottom['price']), 
                       xytext=(0, -12), textcoords='offset points',
    ...
    def _plot_strokes(self, ax):
        ...
        for stroke in strokes:
            color = 'blue' if stroke['direction'] == 'up' else 'blue'
            linestyle = '-' if stroke['direction'] == 'up' else '-'
            linewidth = 2
            
            ax.plot([stroke['kline_start'], stroke['kline_end']], 
                   [stroke['start_price'], stroke['end_price']], 
                   color=color, linestyle=linestyle, linewidth=linewidth, 
                   alpha=0.8, zorder=5)
            
            # Add direction marker at midpoint
            #mid_idx = (stroke['kline_start'] + stroke['kline_end']) / 2
    ...
    def _plot_segments(self, ax):
        ...
        for segment in segments:
            color = 'green' if segment['direction'] == 'up' else 'green'
            linestyle = '-' if segment['direction'] == 'up' else '-'
            linewidth = 1
            
            ax.plot([segment['kline_start'], segment['kline_end']], 
        ...
        for i, segment in enumerate(segments):
            ...
            ax.axvspan(segment['kline_start'], segment['kline_end'], 
                      alpha=alpha, color=color)
            
            ax.annotate(f'Seg{i+1}', xy=(segment['kline_start'], 
                                         self.df.iloc[segment['kline_start']]['low'] * 0.98),
    ...
    def _plot_pivots(self, ax):
        ...
        for i, pivot in enumerate(pivots):
            rect = Rectangle((pivot['kline_start'], pivot['low']), 
                            pivot['kline_end'] - pivot['kline_start'],
            ...
            ax.annotate(f'中枢{i+1}', 
                       xy=((pivot['kline_start'] + pivot['kline_end']) / 2, pivot['high']),
    ...
    def _plot_macd(self, ax):
        ...
        detector = DivergenceDetector(self.analyzer.processed_klines, self.analyzer.df, self.analyzer.strokes)
        macd = detector.macd
        
        x = range(len(macd))
        
        colors = ['red' if v >= 0 else 'green' for v in macd['hist']]
        ax.bar(x, macd['hist'], color=colors, alpha=0.7, width=0.3, label='MACD Hist')
...
def plot_chanlun_chart(stock_code: str, analyzer, output_dir: str = './workspace', 
