...
### 4.6 能力治理与合规（v1.4.0 新增）

| 能力 | 模块 | 简述 |
|------|------|------|
| 统一能力注册表 | `capabilities/registry.yaml` + `core/capability_registry.py` | 声明式 name/kind/enabled(default_off)/requires_consent/degrade_to/health_probe；`is_enabled` / `grant_consent` / `degrade_chain` |
| 代偿层 | `scripts/capability_compensator.py` | `compensate(cap, handlers, *args)` → `CompensateResult(used, trail, result, gap_flag)`，消费 degrade_to 链 |
| 合规闸 | `core/capability_errors.py` | `ConsentRequired` / `CapabilityUnavailable`；Maigret/Sherlock 上抛 ConsentRequired，pipeline 捕获降级 |
| 身份归因客户端 | `scripts/maigret_client.py` / `scripts/sherlock_client.py` | 懒加载 CLI（兼容 .exe），`search(username, consent=False)` 默认关闭、双闸口（env + consent） |
| 跨平台安装器 | `install.sh` | POSIX 一键安装到 `~/.workbuddy/skills/infoseek`，可选隔离 venv + maigret/sherlock |

> 身份归因（OSINT）默认关闭，需显式授权（`INFOSEEK_ENABLE_IDENTITY_ATTRIBUTION` + consent），详见 PRIVACY.md。
