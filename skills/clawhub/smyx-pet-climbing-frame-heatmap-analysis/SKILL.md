...
# 🗺️ Pet Climbing Frame / Cat Tree Activity Heatmap | 宠物爬架/猫树活动热力图
...
|---|---|
| 🏷️ 技能名称 | **宠物爬架/猫树活动热力图** |
| 🎯 核心目标 | 当用户提供猫爬架/猫树区域的视频URL或文件时，触发本技能进行活动热力图分析；支持通过上传本地视频或网络视频URL，调用服务端API进行多区域活动识别，分析各层停留时长、跳跃/转移次数，生成2D活动热力图，用于评估运动量与心理健康（不诊断疾病）。应用场景：猫爬架+摄像头、宠物行为监测、猫咪健康管理。 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `SMYX_PET_CLIMBING_FRAME_HEATMAP_ANALYSIS` |
...
| 角色要点 | 说明 |
|---|---|
| 说明 1 | **假设你是一个专业的宠物行为分析AI。你的任务是基于猫爬架（猫树）多区域的连续视频，检测猫咪在不同层级和位置的停留时长、跳跃或转移次数，生成标准化的活动热力图数据。不要提供健康诊断，仅客观描述活动分布指标。 ** |
...
通过猫爬架/猫树区域视频生成猫咪 2D 活动热力图，获取标准化的运动量与活动分布观察结果
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供猫爬架/猫树区域视频 URL 或文件需要分析时，默认触发本技能进行活动热力图分析 |
| 🔎 明确分析意图 | 当用户明确需要进行猫咪活动监测时，提及猫爬架、猫树、活动热力图、停留时长、跳跃次数、运动量评估、猫咪心理健康、行为丰富度等关键词，并且上传了视频文件 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能**：查看历史活动热力图报告、历史猫爬架报告、活动热力图报告清单、查询活动热力图报告、显示所有猫爬架报告、显示猫咪运动量报告 |
...
| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.smyx_pet_climbing_frame_heatmap_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 3 | ⚙️ 执行活动热力图分析 | 调用 `-m scripts.smyx_pet_climbing_frame_heatmap_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| `--list` | 显示猫爬架/猫树活动热力图视频历史分析报告列表清单（可以输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/smyx_pet_climbing_frame_heatmap_analysis.py`](scripts/smyx_pet_climbing_frame_heatmap_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 🔎 使用提醒 | 建议使用固定机位、能覆盖整座爬架/猫树全部层级的视角，连续记录时长不少于若干分钟，便于生成有意义的热力分布 |
| 🧑‍⚖️ 结果性质 | 分析结果仅供运动量与丰富度参考，不提供疾病诊断或治疗建议 |
