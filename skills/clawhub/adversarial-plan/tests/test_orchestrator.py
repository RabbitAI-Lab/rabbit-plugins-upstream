...
def test_ensure_ids_fills_and_deduplicates():
    ...
    out = orch.ensure_finding_ids(findings)
...
def test_unresolved_keeps_open_findings():
    ...
    assert orch.unresolved_findings(findings, results) == [{"id": "P3"}]


def test_unresolved_ignores_results_without_id():
    findings = [{"id": "P1"}]
    results = [{"status": "resolved"}]  # no id: must not settle anything
    assert orch.unresolved_findings(findings, results) == findings
...
def test_positive_int_rejects_zero_and_garbage():
    with pytest.raises(Exception):
        orch.positive_int("0")
    with pytest.raises(Exception):
        orch.positive_int("nope")
    assert orch.positive_int("3") == 3


def test_lifecycle_helpers_are_imported_without_local_duplicates():
    expected = {
        "banner": pipeline_base.banner,
        "write_json": pipeline_base.write_json,
        "ensure_finding_ids": pipeline_base.ensure_finding_ids,
        "unresolved_findings": pipeline_base.unresolved_findings,
        "setup_git": pipeline_base.setup_git,
        "restore_git": pipeline_base.restore_git,
        "phase_failure": pipeline_base.phase_failure,
        "finish_pipeline": pipeline_base.finish_pipeline,
        "ci_exit_from_final": pipeline_base.ci_exit_from_final,
        "positive_int": pipeline_base.positive_int,
    }
    for name, shared in expected.items():
        assert getattr(orch, name) is shared

    duplicate_names = {
        "_banner", "_write_json", "_ensure_ids", "_unresolved",
        "_record_provider_history", "_log_retrospective", "_phase_failed",
        "_restore", "_setup_git", "_final_md", "_finish", "_positive_int",
        "_ci_exit_code_from_final",
    }
    ...
    assert local_functions.isdisjoint(duplicate_names)


def test_plan_lifecycle_policies_preserve_plan_contract():
    assert orch._GIT_SETUP_POLICY.prefix == "plan"
    assert orch._GIT_SETUP_POLICY.gitignore_entry == ".adversarial-plan/"
    assert orch._RETROSPECTIVE_POLICY.infrastructure_exit == orch.EXIT_INFRA
    assert orch._FINISH_POLICY.pipeline_name == "Adversarial Plan"
    assert orch._FINISH_POLICY.approval_label == "plan"
    assert orch._FINISH_POLICY.loop_label == "Revise/verify loops"
    assert orch._FINISH_POLICY.rejected_exit == orch.EXIT_REJECTED
...
def test_finish_merge_failure_returns_infra_and_records_error(
    ...
    code = orch.finish_pipeline(
        args, str(tmp_path), "demo", tmp_path, state, "APPROVED",
        extra={"complexity": None}, policy=orch._FINISH_POLICY,
    )
...
def test_finish_ci_html_and_optional_payloads(tmp_path, monkeypatch):
    monkeypatch.setattr(orch.gitops, "reject_marker", lambda *_args: None)
    args = orch.build_parser().parse_args([
        "--ci", "--html", "--fail-on", "verdict:reject",
    ])
    state = {"branch": "plan/demo/1", "parent_branch": "main"}
    costs = {
        "total": {
    ...
    research = {"status": "complete", "findings": [{"id": "R1"}]}
    delegated = {"status": "synthesized", "tasks_dispatched": 2}

    code = orch.finish_pipeline(
        args, str(tmp_path), "demo", tmp_path, state, "REJECT",
        reason="open findings", loops=2,
        extra={
            "costs": costs,
            "complexity": {"score": 8, "level": "high"},
            "research": research,
            "delegated": delegated,
        },
        policy=orch._FINISH_POLICY,
    )

    assert code == 2
    payload = json.loads((tmp_path / "final.json").read_text())
    assert payload["research"] == research
    assert payload["delegated"] == delegated
    assert payload["error"] == ""
    assert "conditions" not in payload
    assert (tmp_path / "report.html").is_file()
    final_md = (tmp_path / "final.md").read_text()
    assert "# Adversarial Plan — demo" in final_md
    assert "Revise/verify loops: 2" in final_md


def test_phase_failure_uses_shared_retrospective(tmp_path):
    state = {"feature": "demo", "branch": "plan/demo/1"}
    result = {
        "error": "provider crashed",
        "stdout": "prefix-" + ("x" * 250),
    }

    code = orch.phase_failure(
        "challenge", result, state, tmp_path,
        policy=orch._RETROSPECTIVE_POLICY,
    )

    assert code == orch.EXIT_INFRA
    issues = (tmp_path / "ISSUES.md").read_text()
    assert "challenge failed for demo" in issues
    assert "provider crashed" in issues
    assert "prefix-" not in issues
    assert "x" * 200 in issues
...
def test_no_provider_available_exits_three_with_snapshots(
    ...
    monkeypatch.setattr(orch.gitops, "reject_marker", lambda *_args: None)
    monkeypatch.setattr(orch, "restore_git", lambda *_args, **_kwargs: None)
...
def test_no_provider_available_routes_through_finish_pipeline(
        tmp_path, monkeypatch):
    spec = tmp_path / "demo-feature.md"
    spec.write_text(VALID_SPEC)
    monkeypatch.setattr(orch, "load_provider_config", lambda _path: None)
    ...
    code = orch.main([
        "--spec", str(spec), "--workdir", str(tmp_path),
        "--out", str(tmp_path / "out"),
        "--ci", "--html", "--fail-on", "minor",
    ])

    out_dir = tmp_path / "out" / "demo-feature"
    assert (out_dir / "report.html").is_file()
    # --fail-on must not launder a no-provider outcome to a clean exit: even
    # a selector that would otherwise match nothing (no findings were ever
    # produced) must still fail CI.
    assert code == orch.EXIT_REJECTED


def test_no_provider_exit_code_ignores_fail_on(tmp_path, monkeypatch):
    spec = tmp_path / "demo-feature.md"
    spec.write_text(VALID_SPEC)
    monkeypatch.setattr(orch, "load_provider_config", lambda _path: None)
    monkeypatch.setattr(orch.gitops, "ensure_git_available", lambda: (True, ""))
...
def test_no_provider_finish_policy_surfaces_git_finalize_failure_as_infra(
        tmp_path, monkeypatch):
    # A git error while finalizing a no-provider REJECT must not be masked as
    # a plain rejection: _no_provider_ci_exit must report the infrastructure
    # failure (EXIT_INFRA) instead of always returning EXIT_REJECTED (A1).
    def fail_reject_marker(*_args, **_kwargs):
        raise orch.gitops.GitError("reject marker commit failed: disk full")

    monkeypatch.setattr(orch.gitops, "reject_marker", fail_reject_marker)
    args = orch.build_parser().parse_args(["--ci", "--fail-on", "blocker"])
    state = {"branch": "plan/demo/1", "parent_branch": "main"}

    code = orch.finish_pipeline(
        args, str(tmp_path), "demo", tmp_path, state, "REJECT",
        reason="no provider available",
        policy=orch._NO_PROVIDER_FINISH_POLICY,
    )
...
def test_no_provider_finish_policy_records_ci_exit_code_for_remap(
        tmp_path, monkeypatch):
    # The persisted final.json must carry a fixed ci.exit_code so a later,
    # independent remap of the artifact (e.g. via ci_exit_from_final outside
    # this process) reproduces EXIT_REJECTED instead of deriving a clean exit
    # from the empty findings list on a finding-style REJECT verdict (A1).
    monkeypatch.setattr(orch.gitops, "reject_marker", lambda *_a, **_k: None)
    args = orch.build_parser().parse_args(["--ci", "--fail-on", "blocker"])
    state = {"branch": "plan/demo/1", "parent_branch": "main"}
    ...
    assert code == orch.EXIT_REJECTED
    final = json.loads((tmp_path / "final.json").read_text())
    assert final.get("infrastructure") is not True
    assert final["ci"]["exit_code"] == orch.EXIT_REJECTED

    remapped = pipeline_base.ci_exit_from_final(
        tmp_path, orch.EXIT_APPROVED, fail_on_selector="blocker",
    )
    assert remapped == orch.EXIT_REJECTED
...
def test_skill_cli_flag_table_matches_parser_actions():
    """Keep the documented CLI inventory synchronized with argparse."""
    skill_path = Path(orch.__file__).resolve().parents[1] / "SKILL.md"
    skill_text = skill_path.read_text(encoding="utf-8")
    start_marker = "<!-- CLI-FLAG-TABLE:START -->"
    end_marker = "<!-- CLI-FLAG-TABLE:END -->"

    assert skill_text.count(start_marker) == 1
    assert skill_text.count(end_marker) == 1
    table = skill_text.split(start_marker, 1)[1].split(end_marker, 1)[0]
    documented_flags = set(re.findall(
        r"^\| `(--[a-z0-9-]+)` \|", table, flags=re.MULTILINE
    ))
    parser_flags = {
        option
...
def test_main_folds_unexpected_ci_code_to_legacy_in_non_ci_mode(
        tmp_path, monkeypatch):
    spec = tmp_path / "demo-feature.md"
    spec.write_text(VALID_SPEC)
    monkeypatch.setattr(orch, "load_provider_config", lambda _path: None)
    monkeypatch.setattr(orch.gitops, "ensure_git_available", lambda: (True, ""))
    monkeypatch.setattr(orch, "resolve_role_cmd", lambda *_args: "echo provider")
    monkeypatch.setattr(orch, "_pipeline", lambda *_args, **_kwargs: 10)
    monkeypatch.setattr(orch, "restore_git", lambda *_args, **_kwargs: None)
...
def test_pipeline_restores_dirty_workdir(tmp_path):
    ...
    restored_state = json.loads(
        (workdir / ".adversarial-plan" / "demo-feature" / "state.json").read_text()
    )
    assert restored_state["stash_id"] == ""
