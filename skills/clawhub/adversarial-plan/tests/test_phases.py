...
def _run_with_hard_timeout(fn, timeout=5):
    """Run *fn* on a daemon thread; fail (rather than hang) if it blocks.

    A regression that opens a FIFO for a blocking read would otherwise hang
    the whole test session; the daemon thread lets the process exit even if
    *fn* never returns.
    """
    result = {}
    ...
    assert not thread.is_alive(), "call blocked past the hard timeout (regression)"
    return result["value"]
...
def test_read_utf8_regular_reads_file_content(tmp_path):
    path = tmp_path / "plan.md"
    path.write_text(VALID_PLAN)
    assert phases.read_utf8_regular(path) == VALID_PLAN


def test_read_utf8_regular_missing_file_raises_not_found(tmp_path):
    with pytest.raises(FileNotFoundError):
        phases.read_utf8_regular(tmp_path / "missing.md")


def test_read_utf8_regular_directory_raises_not_regular(tmp_path):
    directory = tmp_path / "plan.md"
    directory.mkdir()
    with pytest.raises(phases.NotRegularFileError):
        phases.read_utf8_regular(directory)


def test_read_utf8_regular_fifo_raises_without_blocking(tmp_path):
    # Opening a FIFO for a blocking read hangs until a writer shows up; the
    # non-blocking-open + fstat guard in read_utf8_regular must reject the
    # FIFO via its opened descriptor without ever blocking on open() or
    # falling for a stat/open TOCTOU race (see A2).
    path = tmp_path / "plan.md"
    os.mkfifo(path)

    def _call():
        with pytest.raises(phases.NotRegularFileError):
            phases.read_utf8_regular(path)
        return True

    assert _run_with_hard_timeout(_call)
...
def test_challenge_does_not_embed_inputs(tmp_path):
    plan_sentinel = "UNIQUE_PLAN_INPUT_SENTINEL"
    spec_sentinel = "UNIQUE_SPEC_INPUT_SENTINEL"
    (tmp_path / "plan.md").write_text(f"{VALID_PLAN}\n{plan_sentinel}\n")
    (tmp_path / "spec.md").write_text(f"{VALID_SPEC}\n{spec_sentinel}\n")
    calls = []
    ...
    assert spec_sentinel not in prompt
    assert "--- plan.md ---" not in prompt
    assert "--- spec.md ---" not in prompt
    assert "`plan.md`" in prompt
    assert "`spec.md`" in prompt
    assert "current directory" in prompt
    assert branch_point in prompt
    assert "HEAD~1" not in prompt
    assert calls[0]["cwd"] == str(tmp_path)
    assert calls[0]["role"] == "plan-challenger"


def test_challenge_fails_on_undecodable_plan(tmp_path):
    # Binary plan.md must fail cleanly: read+decode mirrors phase_verify.
    (tmp_path / "plan.md").write_bytes(b"\xff")
    (tmp_path / "spec.md").write_text(VALID_SPEC)
    calls = []
...
def test_challenge_error_message_matches_failure_mode(tmp_path):
    # Missing vs undecodable must produce distinct messages.
    (tmp_path / "spec.md").write_text(VALID_SPEC)
    missing = phase_challenge.run_challenge("rev", str(tmp_path), 60,
                                            run=fake_run(stdout=CHALLENGE_OK))
    assert missing["exit_code"] == 1
    assert "not found" in missing["error"]
    assert "not valid UTF-8" not in missing["error"]

    (tmp_path / "plan.md").write_bytes(b"\xff")
    undecodable = phase_challenge.run_challenge("rev", str(tmp_path), 60,
                                                run=fake_run(stdout=CHALLENGE_OK))
    assert undecodable["exit_code"] == 1
    assert "not valid UTF-8" in undecodable["error"]
    assert "not found" not in undecodable["error"]


def test_challenge_fifo_plan_fails_cleanly(tmp_path):
    # A FIFO opened for a blocking read would hang the orchestrator forever;
    # run_challenge must reject it via is_file() before ever opening it.
    (tmp_path / "spec.md").write_text(VALID_SPEC)
    os.mkfifo(tmp_path / "plan.md")
    calls = []

    result = _run_with_hard_timeout(lambda: phase_challenge.run_challenge(
        "rev", str(tmp_path), 60, run=fake_run(stdout=CHALLENGE_OK, calls=calls)))
...
def test_verify_fifo_plan_fails_cleanly(tmp_path):
    # A FIFO opened for a blocking read would hang the orchestrator forever;
    # run_verify must reject it via is_file() before ever opening it.
    os.mkfifo(tmp_path / "plan.md")
    calls = []

    result = _run_with_hard_timeout(lambda: phase_verify.run_verify(
        [{"id": "P1"}], "rev", str(tmp_path), 60,
        run=fake_run(stdout=VERIFY_OK, calls=calls)))
