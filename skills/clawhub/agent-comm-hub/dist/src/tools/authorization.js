import { z } from "zod";
import { authorizationService, AUTH_OP_TYPES } from "../authorization.js";
import { requireAdmin } from "../security.js";
import { authed, mcpError } from "../utils.js";
/**
 * 注册授权相关工具
 */
export function registerAuthorizationTools(server, authContext) {
    // ────────────────────────────────────────────────────
    // request_authorization — member 可调用
    // Agent 在执行敏感操作前提交授权请求，Hub 建 pending 并推 authorization_requested
    // ────────────────────────────────────────────────────
    server.tool("request_authorization", "提交一次操作级授权请求（人在环）。用于 delete_data / cancel_task / revoke_token / cross_agent_delete / send_external_email / external_api / paid_api / schema_change 等敏感操作。默认创建 pending 请求并推送仪表盘等待人类批准；批准后经 SSE 回推解锁。已认证 Agent（member 及以上）可调用。", {
    ...
    // ────────────────────────────────────────────────────
    server.tool("resolve_authorization", "决议一个授权请求。approved=放行并解锁 Agent Promise；rejected=拒绝（Agent 优雅中止）。grant_window_ms 仅在批准时建立类目级信任窗口。仅 admin 可调用（主决议路径为仪表盘 REST 端点）。", {
        ...
        try {
            const req = authorizationService.resolve(request_id, decision, ctx.agentId, reason, grant_window_ms);
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
    ...
    // ────────────────────────────────────────────────────
    server.tool("list_authorization_requests", "列出授权请求。可传 status 过滤（pending|approved|rejected|expired）。供仪表盘或调试使用。已认证 Agent 可调用。", {
        status: z.enum(["pending", "approved", "rejected", "expired"]).optional()
            .describe("状态过滤"),
    }, authed(authContext, "list_authorization_requests", async (_ctx, { status }) => {
        try {
            const requests = authorizationService.list(status);
            return {
                content: [{
                        type: "text",
