...
name: Meeting
version: 2.0.1
description: "Orchestrate meeting preparation, capture, and commitment tracking with draft-first confirmation"
tags: [communication, general, template-based, report-generation, memory-based]
metadata:
  openclaw:
    primaryEnv: null
    requires:
...
- **Calendar connector**: One of Lark, Outlook, or Google Workspace (for meeting data retrieval)
- **Task system**: Host platform's task/reminder system (for commitment tracking)
- **No external recording**: This skill does not require or use audio/video recording
...
|-------|-------|------------|
| Calendar connector unavailable | No calendar integration configured | Fall back to user-provided meeting details; skip stakeholder context |
| No prior meeting data found | First meeting or data not accessible | Skip "Prior Commitments" check; note as "No history available" |
| Task system unavailable | Host platform lacks task storage | Store action items in conversation context as structured text |
| Transcript not available | No recording or transcription service | Rely on user-provided fragments during/after meeting |
| Attendee info incomplete | Calendar entry missing participants | Ask user to provide attendee list manually |
| Distribution fails | Email/messaging connector error | Save minutes as draft; inform user to distribute manually |

### Degradation Strategy

1. **Full capability**: Calendar + Task system + Messaging -> Complete lifecycle management
2. **Partial capability**: Calendar only -> Preparation + Minutes generation (Mode 1)
3. **Minimal capability**: No connectors -> User provides all context; skill generates structured output only
4. **No connectors at all**: Skill operates as pure advisory/template engine
