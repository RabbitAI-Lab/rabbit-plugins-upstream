...
| 项目 | 口径 |
| --- | --- |
| Skill | `huifu-pay-integration 1.3.3` |
...
版本回答必须说明：Skill 不能主动联网检查或推送升级；只有用户触发版本问题时提示。未显式配置 `skill_source` 时，按当前请求实际参与生成的 Skill 集合取值：仅支付使用既有来源值 `hfps/1.3.3`；支付与进件都实际参与当前请求时使用 `hfps/1.3.3;hfms/1.0.0`。仅安装或加载但未参与本次生成的不计入；显式合同值优先并原样透传。
