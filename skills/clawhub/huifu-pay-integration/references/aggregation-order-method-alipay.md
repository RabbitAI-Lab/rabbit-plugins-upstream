...
> 本页表格用于解释支付宝场景和易错父子关系，不是完整 DTO。所有字段类型、长度、Y/N/C 以 `payment-complete-field-catalog.md` 的“聚合支付下单”完整路径为准。
...
- 官方开发指引要求交易前完成账户及支付宝业务开通；进件实施交给 `$huifu-merchant-onboarding`。
