...
> 本页 `resp_data` 示例只针对官网明确标出该容器的托管支付/关单通知。托管退款官网异步表标为 `data`，不得套用本示例；非交易通知同样必须使用其所属 Skill 和官方协议。

### 托管退款的独立边界

托管退款官方页把异步业务内容标为 `data`，与支付/关单的 `resp_data` 不同。现有资料没有给出足以复用下方 Controller 的完整外层编码、取参和验签样本，因此退款通知必须单独建模并标记 `[需要官方确认]`；不得让 `/notify/payment` 同时接收退款。
