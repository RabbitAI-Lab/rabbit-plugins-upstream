...
5. `skillSource` 默认可直接使用 `hfps/1.3.3`；如需覆盖，可通过配置项 `huifu.skill-source` 传入，初始化时按 `<skill_source>` 原样透传
6. 当前锁定通用 Java SDK 的 HTTP 客户端不验证证书链和主机名；本片段只说明初始化形态，不代表可直接用于联调或生产，必须先满足 `shared-server-sdk-matrix.md` 的传输安全硬停。
