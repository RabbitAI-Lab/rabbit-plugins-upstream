...
本文件定义支付 Skill `1.3.5` 已覆盖支付接口的统一请求头约束。`hfps/1.3.5` 是支付来源合同值；商户进件 Skill 的默认来源值为 `hfms/1.0.2`。
...
local-sandbox `1.0.1` 的冻结 r4 快照使用 `hfps/1.3.1;sandbox/1.0.1`。官方联调或生产环境使用 `hfps/1.3.5`，不要携带 `;sandbox/...` 后缀。
