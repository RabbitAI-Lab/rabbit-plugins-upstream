...
1. **同步上游展览索引**：读 `data/{museum}-exhibitions.json`（已覆盖十馆：国博/故宫/上博/南博/陕历博/湖博/粤博/苏博/河南博物院/中国美术馆）；不存在或 `lastSynced` 超 30 天，按 `references/data-sources.md` 从馆方官方源同步一次（十馆抓取规则已实测固化：国博静态页+要闻流、故宫检索接口、上博/南博 JSON API、陕历博静态列表、湖博推介栏目、粤博热展列表、苏博展览频道、河南博物院资讯流、中国美术馆列表页），增量合并并告知变更摘要。
...
version: 1.5.1 name: museum-explorer description: 看展全链路助手，行前策展卡-行中观展-行后电子手帐，展品打卡印章，支持分享相约看展 tags: museum,exhibition,journal,stamp,culture
