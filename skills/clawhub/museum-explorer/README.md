...
- **v1.5.0（积累中，未发布）**：十馆索引体系 + 闭展倒计时——新增粤博/苏博/河南博物院/中国美术馆四馆索引（合计 261 条目）；`data-sources.md` 三至十馆抓取规则重写：粤博 TLS 阻断走 WebFetch 通道（多源交叉核展期）、苏博静态列表自带展期展厅、河南博物院"展览资讯"混合流须逐条核地点、中国美术馆域名纠正为 namoc.cn；SKILL.md 阶段1 新增**闭展倒计时**规则（≤7 天 ⚠️ / 8-30 天 ⏳ / 已闭改荐 / 无 dateEnd 标常设），策展卡头与观展单头各加展期状态行。本版为本地积累，按迭代节奏在新增下载接近 0 时触发发布。
