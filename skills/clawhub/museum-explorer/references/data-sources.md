...
| 中华世纪坛（世界艺术博物馆） | A | 文物局展讯清单 + 官方公众号 | ✅ **官网 worldartmuseum.cn 沙箱不可达**，走多源替代链（见 3.11） |
...
### 3.11 中华世纪坛艺术馆——官网不可达，走多源替代链

域名 `www.worldartmuseum.cn`。**关键实测**：官网 curl 与 WebFetch 均不可达（2026-09-06），改走多源替代链：

```
北京市文物局月度展讯清单：wwj.beijing.gov.cn（搜索"wwj.beijing.gov.cn 新展 月度"，A级，含展期/展厅/票价）
北京市政府门户展览日历：  beijing.gov.cn/fwcj/calendar/bwgzl/（A级，逐展条目页）
新京报等权威媒体开幕报道：  m.bjnews.com.cn（B级，含展品照片与现场细节）
官方公众号"中华世纪坛"：    经搜索引擎索引推文全文（A级内容）
```

- **场馆性质不同**：世纪坛是临展高频场馆（年展览 40+），无传统常设展厅；专题展多为**商业特展（购票制 68-108 元）**，公共区域展（世纪大厅等）免费但需小程序实名登记
- 票务/预约统一入口：微信小程序"中华世纪坛"/"中华世纪坛开放服务"
- 开放：周二至周日 9:00-17:00（16:10 停止入场），周一闭馆；**节展活动期可能周一不闭馆**（如北京摄影周 2026，9/5-9/13）——同步时必须核当时段公告
- 数据结构：文物局清单一次给出多馆多展（展期/展厅/票价/简介齐全），适合**按月增量同步**，替代官网失效
...
| 中华世纪坛 | 微信小程序"中华世纪坛"/"中华世纪坛开放服务"实名登记 | 周一闭馆（节展活动期可能不闭）；公共区免费、专题特展购票（68-108 元），特展票经小程序/公众号购买 |
...
5. **合并前确认（必做 gate）**：把解析出的变更（新增/更新/闭展）列成摘要，**先向用户展示并征得明确同意，再写入 `data/{museum}-exhibitions.json`**——用户说"不用同步/先不写"则仅输出变更简报、不落盘。默认不自动静默更新本地库。
