...
- **Exit (any of):**
  ...
  - Trailing stop fires (Phase 2 — see `optimizations/dsl-exit-engine.md`)
...
- Designed to coexist with `bollinger-reverter-4h.md`
- Uses the triple-gate pattern from `optimizations/regime-overlay.md`
- Uses the Phase 2 trailing stop from `optimizations/dsl-exit-engine.md`

## Deployment recommendation

Run as its own sub-account so the regime gate's "trade nothing for weeks" behavior doesn't fight a mean-reversion strategy in the same wallet. See SKILL.md sub-account section.
