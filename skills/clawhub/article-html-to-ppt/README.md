...
- **当前版本：** `3.0.1`（双路径生成、八条专业路线、视觉叙事、可信交付与公开 PMO 运行时）
- **开源定位：** 核心引擎、基础五风格、通用组件、可编辑对象、QA 与可信交付

> PPTSmith 的 GitHub / ClawHub 开源版用于分发、获客与建立可信度。v3.0.1 公开 PMO 的可执行运行时、schemas、路由、质量规则、示例与文档；测试夹具、生成的 PPTX/PDF、渲染图、QA 日志、缓存，以及企业品牌适配、专属页面原型、定制组件和代生成/部署服务不随注册表包分发。
