...
<p align="center">
  <img src="./assets/samples/pptsmith-github-zh.png" alt="MeowClaw PPT Smith 图标" width="836">
</p>
...
- **当前版本：** `3.0.2`（双路径生成、八条专业路线、视觉叙事、可信交付与公开 PMO 运行时）
- **分发定位：** 可审阅、可复用的公开核心能力；第三方商业使用须获书面授权

> PPTSmith 的 GitHub / ClawHub / SkillHub 公开发行版用于分发、试用与建立可信度。v3.0.2 保留公开 PMO 的可执行运行时、schemas、路由、质量规则、示例与文档；测试夹具、生成的 PPTX/PDF、渲染图、QA 日志、缓存，以及企业品牌适配、专属页面原型、定制组件和代生成/部署服务不随注册表包分发。发行包适用 [MeowClaw Lab Non-Commercial Source License v1.0](./LICENSE)：第三方商业使用须先取得书面授权；作者与书面授权主体保留商业使用权。
...
### 3.0.2：第三方非商用，作者保留商业权利

v3.0.2 仅更新许可与发行说明，不改变已公开的 PMO 运行时和功能边界：个人学习、教育、研究、评估、公益等非商用用途可以按随包许可证使用；第三方出售、收费交付、SaaS/托管、商业内部运营或其他营利用途必须先取得 MeowClaw Lab 书面授权。MeowClaw Lab、著作权人及其书面授权主体不受该限制。

请阅读完整 [`LICENSE`](./LICENSE) 与 [v3.0.2 许可说明](docs/v3.0.2-license-notice.md)。旧版 Apache-2.0 / MIT-0 授权不能追溯撤销，历史版本仍按其原许可证处理。

### 3.0.1：公开 PMO 运行时，并保留双路径生产边界

v3.0.1 保留 v3.0.0 的两条生成路径，并把 PMO 生产运行时纳入公开发行包：
...
两条路径不是优劣替代：Path A 追求模型能力所能达到的视觉上限，正式交付仍需真实渲染与人工视觉复核；Path B 追求可验证与可重复的生产下限。PMO 路由使用公开的 PMO 运行时；其测试和生成证据不进入注册表包。详见 [v3.0.1 PMO 公开发行说明](docs/v3.0.1-release-notes.md) 与 [v3.0.0 发布说明、v1.2.0 对比与多模型试验](docs/v3.0.0-release-notes.md)。
...
- [v3.0.1 PMO 公开发行说明](docs/v3.0.1-release-notes.md)
...
本仓库与 ClawHub / SkillHub 包提供可审阅、可复用的 PPTSmith 核心能力。该公开发行版不是 OSI 开源许可证；完整条款以随包 [`LICENSE`](./LICENSE) 为准。

- 个人学习、教育、研究、评估、公益及其他非商用用途可在许可证条件内使用、修改和分享。
- 第三方商业使用（包括收费服务、SaaS/托管、咨询、培训、商业内部运营或营利性再分发）须先取得 MeowClaw Lab 书面授权。
...
这样既保留非商用场景的真实可用性，也保护高价值商业资产与作者的商业化权利。
