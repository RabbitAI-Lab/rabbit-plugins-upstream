...
function loadConfig(): GrillGateConfig {
  const defaults: GrillGateConfig = {
  ...
  try {
    ...
    console.warn(
      `[grill-gate] Config load failed, using defaults:`,
...
function hasValidToken(config: GrillGateConfig): boolean {
  try {
    if (!existsSync(config.tokenDir)) return false;

    const files = readdirSync(config.tokenDir).filter((f) =>
      f.endsWith(".token")
    );
...
function containsTrigger(text: string, config: GrillGateConfig): boolean {
  if (!text) return false;
  const lower = text.toLowerCase();

  // Check exemptions first
  for (const exempt of config.exemptions) {
    if (lower.includes(exempt.toLowerCase())) return false;
  }
...
function isBlockedCommand(command: string, config: GrillGateConfig): boolean {
...
export default definePluginEntry({
  ...
  name: "Grill Gate",
  description:
    "Runtime-level grill enforcement. Blocks exec/spawn calls for " +
      ...
      async (event) => {
        ...
        if (toolName === "exec") {
          ...
          if (isBlockedCommand(command, config)) {
            if (!hasValidToken(config)) {
              console.log(
                `[grill-gate] 🛑 BLOCKED exec: command matches blocked pattern, no grill token`
              );
              return {
                block: true,
                blockReason:
                  "🛑 Grill Gate: Blocked command detected without grill token. " +
            ...
            console.log(
              `[grill-gate] ✅ exec allowed (valid grill token found)`
            );
          }
          return;
        }
        ...
        if (toolName === "sessions_spawn") {
          ...
          if (containsTrigger(task, config)) {
            if (!hasValidToken(config)) {
              console.log(
                `[grill-gate] 🛑 BLOCKED sessions_spawn: task contains grill trigger, no token`
              );
              return {
                block: true,
                blockReason:
                  "🛑 Grill Gate: Task contains research/development keywords but no grill token. " +
