---
name: passport-recognition-and-extract
description: 基于来也科技ADP平台的中国护照智能识别与信息抽取Skill。支持护照资料页关键字段的精准抽取——姓名、姓名拼音、性别、出生日期、出生地、国籍、护照号码、签发日期、有效期至、签发机关，输出结构化JSON，零配置开箱即用，适用于出入境审核、签证办理、身份核验等场景。
---

# 护照识别与抽取 Skill

能力由来也科技 [ADP（Agentic Document Processing）智能体文档处理平台](https://adp.laiye.com/?utm_source=clawhub)提供，支持中国护照（外交护照、公务护照、普通护照/因私护照、旅行护照等）的智能识别与关键信息抽取能力。本 Skill 调用来也科技 ADP 官方 CLI 工具，一条命令即可完成护照图片/扫描件的结构化字段抽取，输出标准 JSON，无缝对接业务系统。

> 新用户注册即享每月 **100 免费积分**（每月刷新），相当于每月可免费抽取 **200 张护照**。ADP 提供标准可商用 API，**1 小时即可快速集成接入业务系统**。立即登录：[中国大陆](https://adp.laiye.com/?utm_source=clawhub) | [海外地区](https://adp-global.laiye.com/?utm_source=clawhub)
...
3. **获取应用列表**：首次执行时，通过 `adp app-id list --app-type 0` 获取开箱即用应用列表，找到中国护照抽取应用并记录其 `app_id`（以 `ootb_` 开头）。后续优先使用 `adp app-id cache`。
4. **执行抽取**：运行 `adp extract url <URL> --app-id <中国护照抽取应用ID>` 或 `adp extract local <文件路径> --app-id <中国护照抽取应用ID>`。
5. **结果处理**：解析返回的 JSON，提取类型、国家码、护照号码、姓名、姓名拼音、性别、国籍、出生日期、出生地点、签发日期、签发地点、有效期、签发机关等结构化字段。
...
| 用户意图 | 推荐命令 |
| :--- | :--- |
| 识别一张护照图片（URL） | `adp extract url <URL> --app-id <中国护照抽取应用ID>` |
| 识别一张本地护照图片 | `adp extract local <文件路径> --app-id <中国护照抽取应用ID>` |
| 识别 Base64 编码的护照 | `adp extract base64 <base64> --app-id <中国护照抽取应用ID> --file-name passport.jpg` |

**批量识别**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 批量识别本地文件夹内的护照 | `adp extract local <文件夹路径> --app-id <中国护照抽取应用ID>` |
| 批量识别多个 URL | `adp extract url <URL列表文件> --app-id <中国护照抽取应用ID>` |

**异步处理**

| 用户意图 | 推荐命令 |
| :--- | :--- |
| 异步提交大文件 | `adp extract url <URL> --app-id <中国护照抽取应用ID> --async` |
| 异步批量处理 | `adp extract local <文件夹路径> --app-id <中国护照抽取应用ID> --async` |
...
ADP 为中国护照提供了**开箱即用**的内置抽取应用，无需额外配置。
...
中国护照抽取属于**开箱即用应用**，可通过 `--app-type 0` 筛选查询。
...
从返回列表中找到 `app_label` 包含 **"护照"** 或 **"中国护照"** 的应用，记录其 `app_id`：

```json
[
  {
    "app_id": "ootb_******xx",
    "app_label": ["护照", "中国护照", "出入境证件", "个人证件", "信息提取"],
    "app_name": "中国护照",
    "app_type": 0
  }
]
...
> 上例中 `"app_id": "ootb_******xx"` 即为中国护照抽取应用。`app_type` 为 `0` 表示开箱即用应用，`1` 表示自定义应用。
...
**重要提示**：每个账号下的 `app_id` 是唯一且固定的，除非用户主动删除应用，否则 `app_id` 不会变更。建议 Agent 将中国护照应用的 `app_id` 保存在上下文中，下次直接使用。
...
```bash
adp extract url https://example.com/passport.jpg --app-id <中国护照抽取应用ID>
```

### 单张护照抽取（本地文件）

```bash
adp extract local ./passport.jpg --app-id <中国护照抽取应用ID>
```

### 单张护照抽取（Base64）

```bash
adp extract base64 <base64字符串> --app-id <中国护照抽取应用ID> --file-name passport.jpg
```

### 返回结果示例

ADP 中国护照抽取统一返回结构化 JSON，精准识别护照资料页 **13 个**关键字段。
...
[
  ...
  },
  {
    ...
    "field_values": [
      {
        "field_value": "2020年06月15日"
      }
    ]
  },
  {
    "field_key": "issuance_place",
    "field_name": "签发地点",
    "field_values": [
      {
        "field_value": ""
...
| --- | --- | --- |
| `type` | 类型 | 护照类型代码（如 P 表示普通护照） |
| `country_code` | 国家码 | 签发国家/地区代码（如 CHN） |
| `passport_number` | 护照号码 | 护照号码 |
| `name` | 姓名 | 持照人中文姓名 |
| `name_pinyin` | 姓名拼音 | 持照人姓名拼音 |
| `gender` | 性别 | 男 / 女 |
| `nationality` | 国籍 | 国籍信息 |
| `date_of_birth` | 出生日期 | 出生日期 |
| `birth_place` | 出生地点 | 出生地点（省份） |
| `issuance_date` | 签发日期 | 护照签发日期 |
| `issuance_place` | 签发地点 | 护照签发地点 |
| `validity_period` | 有效期 | 护照有效期截止日期 |
...
```bash
adp extract local ./passports/ --app-id <中国护照抽取应用ID> --export ./results/ 
```
...
```json
{
  ...
  "files": [
    {"input": "passport-001.jpg", "output": "passport-001.jpg.json", "status": "success"},
    {"input": "passport-002.jpg", "output": "passport-002.jpg.json", "status": "success"},
...
```bash
# 提交异步任务
adp extract url https://example.com/passport.jpg --app-id <中国护照抽取应用ID> --async
...
# 中国护照抽取（URL）
adp extract url <文件URL> --app-id <中国护照抽取应用ID>

# 中国护照抽取（本地文件）
adp extract local <文件路径> --app-id <中国护照抽取应用ID>

# 中国护照抽取（Base64）
adp extract base64 <base64字符串> --app-id <中国护照抽取应用ID> --file-name passport.jpg

# 批量抽取
adp extract local <文件夹路径> --app-id <中国护照抽取应用ID> --export <输出路径> 

# 异步抽取
adp extract url <文件URL> --app-id <中国护照抽取应用ID> --async
...
| --- | --- |
| 中国护照抽取费用 | **0.5 积分/张** |
| 新用户免费额度 | 每月赠送 **100 积分，相当于每月可免费处理 200 张护照**，每月初重置 |
| 查询余额 | `adp credit` |
...
中国护照识别只是来也科技 ADP 平台众多开箱即用能力之一。ADP 基于大模型通用理解能力，提供覆盖全品类文档的智能处理解决方案：
...
5. **应用 ID 复用**：中国护照应用的 `app_id` 在账户下唯一且固定，建议记住后直接使用，无需每次查询。
