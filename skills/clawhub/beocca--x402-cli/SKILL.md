...
| `request info <url>` | no | no | **yes** (`--no-save` to opt out) | Inspect payment requirements, no payment sent |
| `request pay <url>` | **yes** | **yes** | **yes** (`--no-save` to opt out) | Pay and fetch the service response |
...
| `--save` / `--no-save` | **save (on)** | Write input+response to JSON (see [Saved Files](#saved-requestresponse-files)) |
...
`request info` and `request pay` write a JSON file **by default** (`--no-save` to disable; `--output-dir`/`-o` to choose where). Filename pattern: `x402_request_info_<timestamp>.json` / `x402_request_pay_<timestamp>.json`.
...
- `input.header` / `input.data` are exactly what was passed via `--header` / `--data` (`null` if omitted).
...
- **What this CLI touches on your machine:**
  - Reads only `CLIENT_EVM_WALLET_SECRET` from the environment (via `.env` or shell) — no other env vars.
  - Makes outbound HTTP requests to the Coinbase CDP discovery endpoints, plus whatever arbitrary URL is passed to `request info`/`request pay` — that URL and its caller-supplied headers/data go to a third party you don't control.
  - Writes JSON files locally per [Saved Request/Response Files](#saved-requestresponse-files) and `discover ... --save`. No other files, no shell/subprocess execution.
