...
from diagnosis_review import DEFAULT_LLM_BASE, DEFAULT_LLM_MODEL, review_diagnosis_payload
...
def save_prepared(payload: dict[str, Any], output_json: str, input_path: Path) -> None:
    save_dir = Path(output_json).parent if output_json else ICD_DRG_DIR / "runs" / "diagnosis-review"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="诊断编码审核统一入口：支持 pdf/doc/docx/xls/xlsx/csv/txt/json 输入。")
...
def main() -> int:
    ...
    try:
        ...
        response = review_diagnosis_payload(payload)
