...
# 🌡️ Pet Drying Box Heat Stress Analysis | 宠物烘干箱内热应激预警
...
|---|---|
| 🏷️ 技能名称 | **宠物烘干箱内热应激预警** |
| 🎯 核心目标 | 当用户提供宠物烘干箱区域的视频URL或文件时，触发本技能进行烘干箱内热应激预警分析；支持通过上传本地视频或网络视频URL，调用服务端API进行热应激信号识别，分析张口喘气强度、舌体颜色（粉红/紫绀）、身体移动频率，识别热应激早期信号，输出风险等级，支持自动降温或停止烘干。应用场景：宠物烘干箱、宠物美容店、宠物医院。 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `SMYX_PET_DRYING_BOX_HEAT_STRESS_ANALYSIS` |
...
| 角色要点 | 说明 |
|---|---|
| 说明 1 | * *假设你是一个专业的宠物热应激监测AI。你的任务是基于宠物烘干箱内的连续视频，检测宠物的热应激相关行为信号，包括张口喘气强度、舌体颜色（粉红/紫绀）、身体移动频率等，综合评估热应激风险等级，并给出干预建议（如自动降温、停止烘干）。不要提供疾病诊断，仅客观描述观察到的生理和行为迹象。 ** |
...
通过烘干箱内的视频监测宠物热应激信号，获取标准化的观察结果和风险等级预警
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供宠物烘干箱内视频 URL 或文件需要分析时，默认触发本技能进行热应激预警分析 |
| 🔎 明确分析意图 | 当用户明确需要进行烘干箱内宠物监测时，提及烘干箱、宠物烘干、热应激、张口喘气、舌体紫绀、宠物中暑、烘干安全等关键词，并且上传了视频文件 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能**：查看历史热应激报告、历史烘干箱报告、热应激预警报告清单、查询热应激报告、显示所有烘干箱报告、显示宠物热应激报告 |
...
| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.smyx_pet_drying_box_heat_stress_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 3 | ⚙️ 执行热应激预警分析 | 调用 `-m scripts.smyx_pet_drying_box_heat_stress_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| `--list` | 显示烘干箱热应激预警视频历史分析报告列表清单（可以输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/smyx_pet_drying_box_heat_stress_analysis.py`](scripts/smyx_pet_drying_box_heat_stress_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
