...
> 基于场景 + 数据规模 + 任务复杂度，智能匹配 26 个原子方法论的组合。
> v3.1 新增：M12 下钻触发 / M13 中间逻辑可追溯 / M14 增量同步 / M15 迭代式可视化 / M16 多维数据联动。
> v3.3 新增：M17 CRISP-DM 7 步 SOP / M18 清洗决策审查 / M19 图表三秒体检 / M20 相关≠因果验证 / M21 AI 背答案识别。
> v3.4.0 新增：M22 SPA 动态 API 识别 / M23 动态 API Key 模拟 / M24 增量唯一 ID 设计 / M25 HTML 元素定位 / M26 飞书多维表格双存储。
...
| **1 采集（SPA 网站）**（v3.4.0） | 任意 | M1+M2+M7+**M22** | 加 SPA 动态 API 识别（80% 现代网站） |
| **1 采集（动态 Key 服务）**（v3.4.0） | 任意 | M1+M2+M7+**M22**+**M23** | 加动态 API Key 模拟（Algolia 等服务） |
| **1 采集（增量+SPA）**（v3.4.0） | 任意 | M1+M2+M7+**M14**+**M22**+**M24** | 加增量唯一 ID 设计（区分已抓/未抓） |
| **1 采集（飞书双存储）**（v3.4.0） | 任意 | M1+M2+M7+**M14**+**M26** | 加飞书 Base 双存储（本地+云端双写） |
| **1 采集（AI 识别失败兜底）**（v3.4.0） | 任意 | M1+M2+M7+**M25** | 加 HTML 元素定位（用户协助提供 HTML） |
| **1 采集（旗舰版：SPA+增量+双存储）**（v3.4.0） | 任意 | M1+M2+M7+**M14**+**M22**+**M24**+**M26** | 全套爬虫组合（场景 1 顶配） |
...
| **8 完整分析全流程**（v3.3） | 任意 | M1+M2+M7+**M17**+**M18**+**M19**+**M20** | CRISP-DM 7步骨架+清洗审查+图表体检+因果验证 |
| **8 完整分析（教学场景）**（v3.3） | 任意 | M1+M2+M7+**M17**+**M18**+**M19**+**M20**+**M21** | 加 AI 背答案识别（数据手术+三问诊断） |
| **8 完整分析（含下钻）**（v3.3） | 任意 | M1+M2+M7+**M12**+**M13**+**M17**+**M18**+**M19**+**M20** | 加下钻触发+可追溯（探索性全流程） |
...
| 方法论 | 核心定位 | 触发条件 | 联动组合 |
|--------|---------|---------|---------|
| **M22 SPA 动态 API 识别** | 现代 SPA 网站抓取前置 | "requests.get 抓不到""网页内容是空的""XHR/Fetch" | M1（五要素）+ M7（验真） |
| **M23 动态 API Key 模拟** | Algolia 等动态 Key 服务 | "API Key 失效""Key 几分钟就过期""Algolia""requests.Session" | M22（SPA 识别）+ M2（防幻觉） |
| **M24 增量唯一 ID 设计** | 增量抓取区分已抓/未抓 | "增量""重复数据""唯一 ID""tid""fingerprint" | M14（增量同步）+ M6（分批） |
| **M25 HTML 元素定位** | AI 识别失败兜底 | "AI 没识别到""抓不到字段""DevTools""F12" | M2（防幻觉）+ M22（SPA 识别） |
| **M26 飞书多维表格双存储** | 本地 CSV + 飞书 Base 双写 | "飞书多维表格""双存储""bitable""PERSONAL_BASE_TOKEN" | M14（增量同步）+ M7（验真） |

### 场景 1 子场景识别决策树（v3.4.0 新增）
...
Step 1: 评估网站类型
    │
    ├─ 源码中能搜到目标数据关键词 → 静态网站 → M1+M2+M7
    │
    └─ 源码中搜不到关键词 → SPA 网站 → 继续 Step 2
    │
    ▼
Step 2: 评估是否需要动态 Key
    │
    ├─ Network 面板中 API 直接返回数据 → 普通 SPA → M1+M2+M7+M22
    │
    └─ API 请求含动态生成 Key → 动态 Key 服务 → M1+M2+M7+M22+M23
    │
    ▼
Step 3: 评估增量与存储需求
    │
    ├─ 需要增量同步 → 追加 M14+M24（增量+唯一 ID）
    │
    ├─ 需要飞书双存储 → 追加 M26
    │
    └─ AI 识别失败 → 追加 M25（兜底）
...
graph TD
    ...
    M15 --> M16
    M16 --> M15
    M17[CRISP-DM 7步<br/>M17] --> M18[清洗决策审查<br/>M18]
    M17 --> M19[图表三秒体检<br/>M19]
    M17 --> M20[相关≠因果<br/>M20]
    M18 --> M17
    M19 --> M15
    M20 --> M2
    M21[AI背答案识别<br/>M21] --> M17
    M21 --> M2
```
...
- **M17→M18/M19/M20**（v3.3）：CRISP-DM 7 步骨架串联清洗审查+图表体检+因果验证
- **M19→M15**（v3.3）：图表体检是迭代式可视化的验收关
- **M20→M2**（v3.3）：因果验证是防幻觉的统计特化
- **M21→M17/M2**（v3.3）：背答案识别用于教学场景，串联全流程防幻觉
