---
name: med-emr-qc-hypertension-missing-bp
description: 门诊病历内涵质控：高血压未记录血压最高值。给定门诊病历文本，调用内部医疗大模型，输出无缺陷或有缺陷及原因。
metadata:
  {
    "openclaw":
      {
...
概述
----
本 skill 仅针对**「高血压未记录血压最高值」**这一条内涵质控规则。将字段解析、多步 LLM 推理与条件分支实现在本目录 `scripts/emr_qc_impl.py` 中；`scripts/emr_qc.py`、`scripts/run.py` 为入口，本 skill 可单独打包发布。调用 **HiVoice MaaS** 医疗大模型（OpenAI 兼容 `chat/completions`），输出 `无缺陷` 或 `有缺陷` + 原因。
...
python3 doctor/emr-qc/hypertension-missing-bp/scripts/emr_qc.py \
  --input data/med-emr-qc/record.txt \
  --appkey <your-appkey>

# 多格式入口
python3 doctor/emr-qc/hypertension-missing-bp/scripts/run.py \
...
- `--output PATH`：输出文件路径（默认：`../runs/med-emr-qc/hypertension-missing-bp.txt`）。
