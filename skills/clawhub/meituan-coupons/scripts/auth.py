...
LOCAL_VERSION  = "1.0.38"  # 本文件的版本号，与 SKILL.md 中 version 字段保持一致
...
def _ensure_workspace_symlink() -> None:
    """
    确保 ~/.xiaomei-workspace 存在。

    兼容策略：
    - 如果 .xiaomei-workspace 已存在（无论是真实目录还是符号链接），以现有数据为准，不做任何修改。
      这是为了兼容旧版本小美搭档自行创建 .xiaomei-workspace 的逻辑。
    - 仅当 .xiaomei-workspace 完全不存在时，才创建指向持久化目录的符号链接。
    """
    link = Path.home() / ".xiaomei-workspace"

    # 已存在（目录、符号链接、或其他文件）→ 直接使用，不修改
    if link.exists() or link.is_symlink():
        return

    # 不存在 → 尝试创建符号链接到持久化目录
    target = Path.home() / ".openclaw" / "workspace"
    if target.is_dir():
        try:
...
def get_token_data() -> dict:
    ...
    本函数只负责读取本地缓存。记忆系统的兜底恢复由 Agent 在调用前完成
    （参见 SKILL.md 中的「Token 记忆兜底机制」章节）。
    """
...
def save_token_data(token_data: dict):
    """
    保存本 Skill 的 Token 数据到本地缓存。

    Agent 在调用此函数后，应同步将 token_data 写入记忆系统
    （参见 SKILL.md 中的「Token 记忆兜底机制」章节）。
    """
...
def cmd_restore_from_memory(user_token: str = "", device_token: str = "", data_json: str = ""):
    """
    从 Agent 记忆恢复 Token 数据到本地缓存。

    支持两种调用方式：
    1. KV 模式（推荐）：--user-token <token> --device-token <token>
    2. JSON 模式（兼容）：--data '<JSON字符串>'

    KV 模式下自动设置 terms_accepted=true（有 user_token 即视为曾登录同意过协议）。

    Args:
        user_token: 用户登录 Token
        device_token: 设备标识 Token
        data_json: （兼容旧版）完整 JSON 字符串
    """
    # 优先使用 KV 参数
    if user_token:
        token_data = get_token_data()  # 保留本地已有的其他字段
        token_data["user_token"] = user_token
        if device_token:
            token_data["device_token"] = device_token
        elif not token_data.get("device_token"):
            # 没有提供且本地也没有，自动生成
            token_data["device_token"] = generate_device_token("memory_restore")
        token_data["terms_accepted"] = True  # 有 user_token 即视为同意过协议
    elif data_json:
        # 兼容旧版 JSON 模式
        try:
        ...
        if not isinstance(token_data, dict) or not token_data.get("user_token"):
            print(json.dumps({
                "success": False,
                "error": "NO_USER_TOKEN",
                "message": "数据中不包含有效的 user_token"
            }, ensure_ascii=False))
            sys.exit(1)
    else:
        print(json.dumps({
            ...
            "message": "必须提供 --user-token 或 --data 参数"
        }, ensure_ascii=False))
        sys.exit(1)
    ...
    print(json.dumps({
        "success": True,
        "message": "Token 数据已从记忆恢复到本地缓存",
        "user_token": token_data["user_token"][:8] + "..." + token_data["user_token"][-8:],
        "device_token": token_data.get("device_token", "")
    }, ensure_ascii=False))
...
def main():
    ...
    # 记忆恢复命令
    p_restore = subparsers.add_parser("restore-from-memory", help="从 Agent 记忆恢复 Token 数据到本地缓存")
    p_restore.add_argument("--user-token", required=True, help="用户登录 Token")
    p_restore.add_argument("--device-token", default="", help="设备标识 Token")
    p_restore.add_argument("--data", default="", help="（兼容旧版）完整 JSON 字符串")
    ...
    elif args.command == "restore-from-memory":
        cmd_restore_from_memory(user_token=args.user_token, device_token=args.device_token, data_json=args.data)
