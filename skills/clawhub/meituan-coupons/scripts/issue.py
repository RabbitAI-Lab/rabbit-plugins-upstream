...
def _run_auto_checks():
    """
    --auto 模式：内部自动完成 terms-check + token-verify，返回 (user_token, phone_masked)。
    任一检查失败则打印 JSON 错误并 sys.exit(0)。
    """
    import subprocess as _sp
    script_dir = os.path.dirname(os.path.abspath(__file__))
    auth_py = os.path.join(script_dir, "auth.py")
    python_exe = sys.executable

    # 1. terms-check
    try:
        r = _sp.run([python_exe, auth_py, "terms-check"], capture_output=True, text=True, timeout=15)
        tc = json.loads(r.stdout.strip()) if r.stdout.strip() else {}
    except Exception as e:
        print(json.dumps({"success": False, "error": "TERMS_CHECK_ERROR", "message": f"协议检查异常：{e}"}, ensure_ascii=False))
        sys.exit(0)

    if not tc.get("terms_accepted"):
        print(json.dumps({"success": False, "error": "TERMS_NOT_ACCEPTED", "message": "用户尚未接受服务协议，请先完成协议确认"}, ensure_ascii=False))
        sys.exit(0)

    # 2. token-verify
    try:
        r = _sp.run([python_exe, auth_py, "token-verify"], capture_output=True, text=True, timeout=15)
        tv = json.loads(r.stdout.strip()) if r.stdout.strip() else {}
    except Exception as e:
        print(json.dumps({"success": False, "error": "TOKEN_VERIFY_ERROR", "message": f"Token校验异常：{e}"}, ensure_ascii=False))
        sys.exit(0)
    ...
    if not tv.get("valid"):
        print(json.dumps({
            "success": False,
            "error": "TOKEN_INVALID",
            "message": "用户未登录或Token已过期，请先完成登录认证",
            "phone_masked": tv.get("phone_masked", "")
        }, ensure_ascii=False))
        sys.exit(0)

    return tv.get("user_token", ""), tv.get("phone_masked", "")


def main():
    parser = argparse.ArgumentParser(description="美团权益包发放")
    parser.add_argument("--auto", action="store_true", help="自动模式：内部完成 terms-check + token-verify，无需手动传 token")
    parser.add_argument("--token", default=None, help="用户 user_token")
    parser.add_argument("--phone-masked", default=None, help="脱敏手机号（用于生成 redeem_code）")
    args = parser.parse_args()

    # 互斥校验
    if args.auto and args.token:
        print(json.dumps({"success": False, "error": "ARGS_CONFLICT", "message": "--auto 和 --token 不能同时使用，请选择其一"}, ensure_ascii=False))
        sys.exit(0)

    if args.auto:
        user_token, phone_masked = _run_auto_checks()
        args.token = user_token
        args.phone_masked = phone_masked
    else:
        # 原有模式：--token 和 --phone-masked 必须提供
        if not args.token or not args.phone_masked:
            print(json.dumps({"success": False, "error": "MISSING_ARGS", "message": "非 --auto 模式下，--token 和 --phone-masked 为必填参数"}, ensure_ascii=False))
            sys.exit(0)
