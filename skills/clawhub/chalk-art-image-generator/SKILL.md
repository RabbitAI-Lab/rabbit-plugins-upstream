---
name: chalk-art-image-generator
description: Generate chalkboard art and chalk drawings. Use when the user asks for chalk art, blackboard menu, or pastel drawing on dark background.
version: 0.5.0
metadata: { "pattern": ["tool-wrapper"], "openclaw": { "emoji": "🖍️", "primaryEnv": "IMAGE_GEN_API_KEY", "requires": { "env": ["IMAGE_GEN_API_KEY"], "anyBins": ["bun", "npx"], "bins": ["node", "npm"] } } }
---

# Chalk Art Image Generator (`chalk-art-image-generator`)
...
1. Always append `--style chalk` to your `chalk-art-image-generator` CLI command.
2. When building the prompt, seamlessly blend these keywords into the visual description: "chalk drawing, on blackboard, dusty, textured lines, pastel colors on dark background".
Do not ask the user which style they want, because this skill is strictly dedicated to Chalk Art Image Generator.
...
- **Files written**: Output images and optional local config under `.image-skills/chalk-art-image-generator/` (project) and/or `~/.image-skills/chalk-art-image-generator/` (home).
...
- If `IMAGE_GEN_API_KEY` is missing: tell the user image generation needs an API key and offer to configure it now by writing `.image-skills/chalk-art-image-generator/.env` on the user's behalf
...
1. Start from the bundled starter registry shipped with this skill. If the model list looks stale or a requested model is missing, refresh it silently with `npm run discover-image-models -- --out .image-skills/chalk-art-image-generator/MODELS.json`
