...
- **file_ops** - Read, write, append, copy, move, delete, diff, and list files with consistent error handling
- **process** - Start, monitor, tail logs, wait on, and kill background processes
- **sub_agent** - Delegate tasks to spawned agents, with parallel execution support
- **notify** - Send webhook notifications to Slack, Discord, ntfy, or any custom HTTP endpoint

### 🔧 Changed

- Merged `fetch_url` and `call_api` into a single `http_request` capability (simpler, less decision fatigue)
- Trimmed SKILL.md from 21KB to ~13KB by removing duplicate best-practices sections and redundant workflow examples
- Tightened `description` frontmatter to improve trigger precision
- Updated `manifest.json` tags to reflect new capabilities
