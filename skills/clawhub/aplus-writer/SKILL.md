---
name: amazon-aplus-content-writer
description: >
  结合 Amazon 商品详情和评论证据规划 A+ 内容模块、信息层次与买家疑问回应。仅用于 A+ 内容规划；不用于广告投放、图片制作或直接发布页面。
  Requires an ARI API key (ari_live_*).
slug: aplus-writer
displayName: Amazon A+ 内容规划
version: 1.4.7
summary: 用商品与评论证据规划 A+ 内容重点
license: MIT
---
...
- 本入口只执行 `listing/aplus`，输出模板固定为 `ops_listing`。
