#!/usr/bin/env python3
"""
Auto-generated test suite for skill: meeting-transcript-to-summary
...
def test_do_not_section_has_content():
        with open(_skill_md(), 'r', encoding='utf-8', errors='ignore') as f:
            text = f.read()
        do_idx = text.find('### Do not')
        if do_idx < 0:
            do_idx = text.find('## Do not')
        if do_idx < 0:
            return _p('do_not_has_rules', False, ' (no Do not section found)')
        remaining = text[do_idx + len('### Do not'):]
        next_h = re.search(r'\n#{1,3}\s+\w', remaining)
        end = next_h.start() if next_h else len(remaining)
        section = remaining[:end].strip()
        rules = re.findall(r'^\s*[-*]\s+\w', section, re.MULTILINE)
