...
COMBO_TOTAL = 1107568 * 16  # 双色球 6+1 总组合数 17,721,088
TICKET_COST = 2         # 单注 6+1 基本投注


def _prize(fh, bh):
    """双色球 6+1 奖级映射返回派彩(元)。无中奖=0。
...
def _freq(past, key):
    ...
    top = 34 if key == 'front' else 17
    return {k: c.get(k, 0) / n for k in range(1, top)}


def _method_hot(past):
    ff = _freq(past, 'front'); bf = _freq(past, 'back')
    front = sorted(range(1, 34), key=lambda x: (-ff[x], x))[:6]
    back = sorted(range(1, 17), key=lambda x: (-bf[x], x))[:3]
    return front, back
...
def _method_avoid_last(past):
    ...
    ff = _freq(past, 'front'); bf = _freq(past, 'back')
    fpool = [x for x in range(1, 34) if x not in last]
    bpool = [x for x in range(1, 17) if x not in last]
    front = sorted(fpool, key=lambda x: (-ff[x], x))[:6]
...
def _method_balanced(past):
    ff = _freq(past, 'front'); bf = _freq(past, 'back')
    top = sorted(range(1, 34), key=lambda x: (-ff[x], x))
    best = None
    for combo in combinations(top[:12], 6):
        odd = sum(1 for x in combo if x % 2 == 1)
        if odd in (2, 3, 4):
            best = combo
            break
    front = list(best) if best else top[:6]
    back = sorted(range(1, 17), key=lambda x: (-bf[x], x))[:3]
    return front, back
...
def _method_omission(past):
    ...
    front = sorted(range(1, 34), key=lambda x: ff[x])[10:16]   # 遗漏偏冷中段
    back = sorted(range(1, 17), key=lambda x: bf[x])[4:6]
    return front, back


def _method_interval(past):
    """区间分布法: 红球分5区间, 对近期出号最少区间回补热号"""
    intervals = [(1, 7), (8, 14), (15, 21), (22, 28), (29, 33)]
    cnt = [0] * 5
    for d in past[-60:]:
        for x in d['front']:
    ...
    for idx in order[:2]:
        ...
        front.extend(pool[:3])
    if len(front) < 6:
        for x in sorted(range(1, 34), key=lambda x: (-ff[x], x)):
            if x not in front:
                front.append(x)
            if len(front) == 6:
                break
    back = sorted(range(1, 17), key=lambda x: (-_freq(past, 'back')[x], x))[:3]
    return sorted(front[:6]), back
...
def _method_back_refine(past):
    """蓝球细化法: 蓝球强制奇偶1:1 且 和值∈[11,15]; 红球用balanced"""
    ...
    for a in range(1, 17):
        for b in range(a + 1, 17):
            if (a % 2) != (b % 2) and 11 <= a + b <= 15:
                ...
                if s > best_s:
                    best_s, best = s, [a, b]
    back = best or sorted(range(1, 17), key=lambda x: (-bf[x], x))[:3]
    return front, back
...
def run(draws):
    ...
    for _ in range(RANDOM_TRIALS):
        def rand_pick(past, rng=rng):
            f = rng.sample(range(1, 34), 6)
            b = rng.sample(range(1, 17), 3)
            return f, b
    ...
    conclusion = (
        ...
        "ROI 仍为负，远不足以克服 house edge，不有正收益。此引擎价值在于'主动猎杀伪模式'闭环，"
        "而非发现真边缘。"
    )
    print(f"\n  🔎 结论:\n{conclusion}")
    out = {
    ...
    with open('ssq_method_explorer.json', 'w', encoding='utf-8') as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print(f"  ✓ 已保存 ssq_method_explorer.json")
    return out


if __name__ == '__main__':
    print("=" * 70)
    print("双色球 方法发现+证伪引擎 V1.0.0")
    print("=" * 70)
