# -*- coding: utf-8 -*-
"""
模块自完整性守卫 (ssq_self_integrity.py)  深度升级新增 (看门狗加固)

目的: 弥补"三体一致性只靠手动比对"的缺口——一旦 lib 内模块或根目录关键文件
被意外改坏 / 被篡改 / 损坏，应能被自动发现并告警，而不是等排程跑挂才暴露。

机制:
  - 首次运行生成基线清单 (ssq_integrity_manifest.json, SHA256)。
  - 之后每次运行比对: 任何关键文件哈希变化 -> 判定 tamper/corruption -> 告警。
  - 关键文件 = lib/ 下全部 ssq_*.py(守卫自身除外) + 顶层 run_ssq.py
    / ssq_history.json(数据完整性基线, 篡改意味着历史被污染)。

用法:
  python ssq_self_integrity.py            # 检查(缺失清单则先初始化)
  python ssq_self_integrity.py --init      # 强制重建基线
看门狗 ssq_watchdog_win.py 每日运行时会调用 check_self_integrity()。
"""
import datetime
...
MANIFEST = os.path.join(WORK_DIR, "ssq_integrity_manifest.json")
...
def _targets():
    ...
    for f in sorted(os.listdir(WORK_DIR)):
        if f.startswith("ssq_") and f.endswith(".py") and f != "ssq_self_integrity.py":
            t.append((f, [os.path.join(WORK_DIR, f)]))
    for f in ("run_ssq.py", "ssq_history.json"):
...
def generate_manifest():
    ...
    manifest = {
        ...
        "note": "SHA256 基线, 由 ssq_self_integrity 自动生成",
        "files": files,
    }
...
# ============================================================
def _history_path():
    for p in (os.path.join(WORK_DIR, "ssq_history.json"),
              os.path.join(SCRIPTS_DIR, "ssq_history.json")):
        if os.path.exists(p):
            return p
    return os.path.join(WORK_DIR, "ssq_history.json")
...
def check_data_freshness(attempt_refresh=True):
    ...
    if not os.path.exists(hp):
        return {"stale": True, "age_days": None, "latest_date": None,
                "refreshed": False, "note": "ssq_history.json 不存在"}
    try:
...
def _latest_backup_dir():
    ...
    try:
        import ssq_restore
        return ssq_restore._latest_backup()
    except Exception:
        return None
...
def self_heal(heal=False):
    ...
    一次合法升级后未重初始化, 误判为'篡改'时盲目恢复会把刚写的新代码覆盖掉。故:
      ...
      - 仅当显式 heal=True (操作员确认) 才调用 ssq_restore 真正恢复并重建基线。
    ...
    if not heal:
        return {"tampered": r["tampered"], "backed_up": bak, "healed": False,
                "note": (f"检测到篡改 {r['tampered']}; 备份位于 {bak or '无'}; "
                          f"确认后运行 `python ssq_self_integrity.py --heal` 恢复")}
    # 显式恢复: 调用 ssq_restore 把关键文件从备份还原, 并重建基线
    try:
        import ssq_restore
        ssq_restore.restore_files(WORK_DIR, force=True)
        generate_manifest()
...
def check_operational_consistency():
    ...
    """
    hp = _history_path()
    perf_path = os.path.join(WORK_DIR, "ssq_performance.json")
    unverified, orphans = [], []
    try:
        import re as _re
        hist = json.load(open(hp, encoding="utf-8"))
    ...
    # 已生成的预测文件(期号)
    pred_periods = set()
    for f in _glob.glob(os.path.join(WORK_DIR, "ssq_prediction_*_v8.json")):
        m = _re.search(r"ssq_prediction_(\d+)_v8\.json", f)
        if m:
