# -*- coding: utf-8 -*-
"""
双色球系统 · 独立 Windows 看门狗 (Standalone Watchdog)
======================================================

不依赖 WorkBuddy 的 OS 级看门狗: 即便 WorkBuddy 停用/重装,
只要本机 Windows 计划任务还在, 失败告警仍能触达用户。

复用 ssq_healthcheck_all.py 第22项(排程真实运行结果)的四项检查:
  (A) 上次结果 LastTaskResult 必须为 0
  (B) 排程无"主动杀任务"反模式(DisallowStartIfOnBatteries 等)
  (C) 上次排程日志无"开跑即中断"特征(^C / 行数过少)
  (D) 未处理告警文件 ssq_run_alert.txt 不得比最新产物更新

异常时(任一检查不过):
  - 写 ssq_run_alert.txt (首行 ALERT, 供 WorkBuddy 看门狗/体检复用)
  - 写 ssq_watchdog_status.txt (首行 ALERT + 时间戳|原因, 与现状格式兼容)
  - 追加 ssq_watchdog_alerts.log (持久化, 跨重启可见)
  - (可选) 弹 Windows 原生提示: 仅当显式传 --toast 时(BurntToast, 否则回退 msg *)
    默认不弹窗 —— 因为本看门狗通常以 SYSTEM 账户在后台运行(无论是否登录都跑),
    后台会话无桌面, 弹窗既无意义也易吊死; 告警以文件落盘为准。

正常时:
  - 删除陈旧 ssq_run_alert.txt
  - 写 ssq_watchdog_status.txt (首行 OK + 时间戳|说明)

始终 exit 0 (看门狗自身失败不应让排程任务报错)。

用法
----
  python ssq_watchdog_win.py            # 真实检查; 异常则写告警文件(默认不弹窗)
  python ssq_watchdog_win.py --toast   # 额外弹 Windows 原生提示(仅交互会话有意义)
  python ssq_watchdog_win.py --dry     # 只评估并打印, 不写告警/不弹窗(用于验证检测逻辑)
"""
import sys
import os
import io
...
ALERT_FILE = os.path.join(WORK_DIR, "ssq_run_alert.txt")
STATUS_FILE = os.path.join(WORK_DIR, "ssq_watchdog_status.txt")
ALERTS_LOG = os.path.join(WORK_DIR, "ssq_watchdog_alerts.log")
...
def evaluate():
    ...
    if os.path.exists(ALERT_FILE):
        try:
            reports = glob.glob(os.path.join(WORK_DIR, "*增强版*.html")) + \
                      glob.glob(os.path.join(WORK_DIR, "ssq_prediction_*_v8.json"))
            newest = max((os.path.getmtime(p) for p in reports), default=0)
            if os.path.getmtime(ALERT_FILE) > newest:
                problems.append("ssq_run_alert.txt 比最新产物还新 -> 存在未处理的排程失败告警")
            else:
...
def raise_os_alert(reasons, toast=TOAST):
    ...
    ps = (
        ...
        "Import-Module BurntToast;New-BurntToastNotification -Text '双色球排程告警',$m}"
        "else{msg * $m}"
        "}catch{msg * $m}"
    )
    try:
...
def main():
    ...
    print("=" * 64)
    print("  双色球系统 · 独立看门狗 (Standalone Watchdog)")
    print(f"  时间: {_now()}")
    if dry:
    ...
    if problems:
        ...
        if dry:
            print("  (dry) 未写 ssq_run_alert.txt / 未弹窗")
