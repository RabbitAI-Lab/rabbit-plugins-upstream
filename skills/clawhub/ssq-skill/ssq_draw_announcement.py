...
# -*- coding: utf-8 -*-
"""
ssq_draw_announcement.py — 体彩中心双色球开奖公告获取器

数据源: 中国体彩网官方API (webapi.sporttery.cn)
功能: 获取指定期号的开奖公告(销售金额/摇奖球/出球顺序/各奖级中奖/奖池/兑奖截止等)
缓存: 本地 JSON (ssq_draw_announcement.json), 避免频繁请求
...
GAME_NO = "85"  # 双色球
CACHE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ssq_draw_announcement.json")
...
HEADERS = {
    ...
    'Referer': 'https://www.cwl.gov.cn/kj/kjlb.html?ssq',
}
...
def _parse_announcement(item, is_pool_draw=False):
    ...
    result = {
        ...
        'game_name': item.get('lotteryGameName', '超级双色球'),
...
def format_marquee_text(a):
    ...
    if a.get('unsorted_front') and a.get('unsorted_back'):
        ...
        parts.append("出球顺序: 红球 {} | 蓝球 {}".format(uf, ub))

    # 开奖号码
    if a.get('front') and a.get('back'):
        f = ' '.join(a['front'])
        b = ' '.join(a['back'])
        parts.append("开奖号码: 红球 {} | 蓝球 {}".format(f, b))
...
<div style="margin: 10px 0; overflow: hidden; background: linear-gradient(90deg, #0a0e27, #0d1b3e, #0a0e27); border: 1px solid #1a3a6e; border-radius: 8px; padding: 0; position: relative;">
  <div style="display: flex; align-items: center; height: 38px;">
    ...
    <div style="overflow: hidden; flex: 1; position: relative; height: 38px;">
      <div style="display: inline-block; white-space: nowrap; color: #aaccff; font-size: 13px; line-height: 38px; padding-left: 100%; animation: ssq-marquee {duration}s linear infinite;">
        {text}
      </div>
    </div>
  </div>
