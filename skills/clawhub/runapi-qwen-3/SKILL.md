---
name: qwen-3
description: "Generate and image to images with Qwen 3 through RunAPI. Use when the user asks an agent to create, edit, or transform images with Qwen 3. Default to the RunAPI CLI for one-off generation; use SDKs only when the user is integrating RunAPI into an app or backend."
documentation: https://runapi.ai/models/qwen-3.md
provider_page: https://runapi.ai/providers/alibaba.md
catalog: https://runapi.ai/models.md
metadata:
  openclaw:
    homepage: https://runapi.ai/models/qwen-3
    requires:
      bins:
      - runapi
    install:
...
- For a one-off artifact or result, use the registered `qwen-3` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi qwen-3 --help
runapi qwen-3 <operation> --help
curl --fail --location https://runapi.ai/docs/api/qwen-3/<operation>.md --output contract.md
```
...
```shell
runapi qwen-3 <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service qwen-3 --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/qwen-3-sdk

## Variants
- `qwen-3-edit-image`: https://runapi.ai/models/qwen-3/edit-image.md
- `qwen-3-pro-edit-image`: https://runapi.ai/models/qwen-3/pro-edit-image.md
- `qwen-3-pro-text-to-image`: https://runapi.ai/models/qwen-3/pro-text-to-image.md
- `qwen-3-text-to-image`: https://runapi.ai/models/qwen-3/text-to-image.md
