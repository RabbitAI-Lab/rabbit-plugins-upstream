---
name: agentic-engineering/coding
description: Agentic Engineering 分支：代码交付。Contract-first、PACT loop、merge-ready handoff。触发：当需要产出生产级代码、bug fix、feature merge 时使用。
---

## Positioning

本分支是 `agentic-engineering` 的 coding 子分支，专注：
- Contract-first implementation（先锁合同，再写代码）
- PACT loop 执行纪律
- Proof of fix + reviewer-ready handoff
