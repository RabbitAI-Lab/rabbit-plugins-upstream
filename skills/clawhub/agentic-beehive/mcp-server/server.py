...
@mcp.tool()
def skill_update(branch: str, status: str = None, effectiveness: float = None, description: str = None, reasoning: str = None) -> dict:
    """更新分支状态
    ...
    log_detail = {**updates}
    if reasoning:
        log_detail["reasoning"] = reasoning
    db.execute(
        "INSERT INTO evolution_log (action, target, detail, created_at) VALUES (?, ?, ?, ?)",
        ("update", branch, json.dumps(log_detail, ensure_ascii=False), now)
    )
    db.commit()
...
@mcp.tool()
def skill_evolve(branch: str, new_capability: str, detail: str = "", reasoning: str = "") -> dict:
    """触发分支进化 — 添加新能力
    ...
    Args:
        ...
        detail: 详细说明
        reasoning: 为什么需要进化（触发场景、推理过程）
    """
    db = get_db()
    now = datetime.now(timezone.utc).isoformat()
    
    log_detail = {"new_capability": new_capability, "detail": detail}
    if reasoning:
        log_detail["reasoning"] = reasoning
    
    db.execute(
        "INSERT INTO evolution_log (action, target, detail, created_at) VALUES (?, ?, ?, ?)",
        ("evolve", branch, json.dumps(log_detail, ensure_ascii=False), now)
    )
...
@mcp.tool()
def evolution_history(branch: str = None, limit: int = 20) -> dict:
    """查询演绎历史 — 过程回溯
    
    Args:
        branch: 分支名称（可选，不传则返回全部分支）
        limit: 返回条数上限
    """
    db = get_db()
    if branch:
        rows = db.execute(
            "SELECT * FROM evolution_log WHERE target = ? ORDER BY created_at DESC LIMIT ?",
            (branch, limit)
        ).fetchall()
    else:
        rows = db.execute(
