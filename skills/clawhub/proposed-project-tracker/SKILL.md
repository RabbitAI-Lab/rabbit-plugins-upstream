---
name: proposed-project-tracker
description: 拟建项目跟踪与早期商机发现助手。当用户想查询拟建项目、在建工程立项信息或挖掘早期商机时，必须使用此SKILL：拟建项目查询（发改立项/审批公示阶段，提前6-18个月）、审批状态筛选（未审批/审批中/办结）、立项单位跟进、项目进展跟踪（是否进入招标）、以及配套的采购意向与临期续约商机扫描。给一个行业/地区即输出按价值排序的商机清单。即使用户没有提到「拟建」，只要涉及立项项目查询、还没招标的项目、早期项目线索等需求，都应使用本SKILL。
metadata: { "openclaw": {"requires": {"env":["ZLBX_API_KEY"]},"primaryEnv": "ZLBX_API_KEY"}}
---

# 拟建项目跟踪 · 立项审批阶段就发现机会

专注**最早期的商机窗口**：项目还在发改立项/审批阶段就被捕获，比招标公告提前 6-18 个月。配合采购意向与临期续约两路，组成完整的早期商机雷达。

## 本助手典型场景

- 「帮我查查湖南最近有哪些医院建设的拟建项目」
- 「这条拟建项目进展到哪一步了？开始招标了吗」
- 「审批已办结的产业园区拟建项目有哪些？投资额5000万以上的」
...
| 路 | 工具 | 提前量 | 商机含义 |
|----|------|--------|----------|
| ① 拟建项目 | search_proposed_projects | 最早（审批期，6-18 个月） | 项目还在发改立项阶段（如「XX市中心医院异地新建项目」这类），抢跑窗口最大 |
| ② 采购意向 | search_bids（bid_process=[1]） | 次早（发标前 1-3 个月） | 采购计划已公开、公告未发布，正是准备方案的时候 |
| ③ 临期续约 | search_expiring_projects | 合同到期前 0-180 天 | 存量医院信息化配套类合同到期在即，续约窗口开放 |
