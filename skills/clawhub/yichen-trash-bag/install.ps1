# ============================================================
# 奕辰垃圾袋（Yichen Trash Bag）Skill · Windows 一键安装
# ============================================================
# 用法：
#   powershell -c "irm https://raw.githubusercontent.com/Liubuq-sys/yichen-trash-bag-skill/main/install.ps1 | iex"
# ============================================================

$Repo = "https://github.com/Liubuq-sys/yichen-trash-bag-skill.git"
$SkillName = "yichen-trash-bag"
...
Write-Host "=== 奕辰垃圾袋 Skill · 安装 ===" -ForegroundColor Green
...
if (Test-Path "$SkillPath\.git") {
    ...
    $TmpDir = Join-Path $env:TEMP "yichen-skill-$([System.Guid]::NewGuid())"
...
# 配置计划任务（每天 3:07）
$TaskName = "YichenTrashBagSkillUpdate"
...
if (-not $Existing) {
    ...
    Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger -Description "奕辰垃圾袋Skill自动更新" | Out-Null
    Write-Host "自动更新已配置（每天 3:07）" -ForegroundColor Green
}

Write-Host ""
Write-Host "Skill 路径: $SkillPath" -ForegroundColor White
Write-Host "已就绪！现在对 AI 说「奕辰垃圾袋」即可触发。" -ForegroundColor Green
