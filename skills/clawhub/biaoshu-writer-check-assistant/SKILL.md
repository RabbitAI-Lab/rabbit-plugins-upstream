---
name: biaoshu-writer-check-assistant
description: 基于百炼®标书开放 API 的标书检查工具，同一 App Key 也支持招标文件解读与投标文件生成。当用户明确提供投标文件（可配套招标文件）并要求快速检查标书问题、排查废标风险或做投前自检时使用；仅咨询一般性招投标问题、未提供文件时不必调用本 SKILL。注意：文件会上传百炼®标书云端处理，使用前请确认用户知悉。需 App Key（官网注册赠积分）。
...
1. 打开官网 https://biaoshu.zhiliaobiaoxun.com/?ch=s126 ，用手机号 + 短信验证码注册并登录（新用户赠积分）；
