...
try:
    ...
    _le.load_env_files()
except Exception:  # NOCA:broad-except(CLI script needs to catch all SDK exceptions for user-friendly error reporting)
    pass
...
try:
    from mps_poll_task import auto_upload_local_file, poll_video_task, auto_download_outputs  # NOCA:unused-import(may be needed for future use)
...
def build_input_info(url: str = None,
    ...
    if cos_input_key:
        ...
        return {
            "Type": "COS",
            "CosInputInfo": {"Bucket": bucket, "Region": r, "Object": cos_input_key if cos_input_key.startswith("/") else f"/{cos_input_key}"},  # NOCA:line-too-long(long SDK parameter or URL string)
        }
...
def main():
    ...
    if output_storage:
        print(f"   输出:     COS={output_storage['CosOutputStorage']['Bucket']} (region: {output_storage['CosOutputStorage']['Region']})")  # NOCA:line-too-long(long SDK parameter or URL string)
