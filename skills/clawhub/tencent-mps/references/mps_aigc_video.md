...
| `--duration` | 视频时长（秒）。各模型支持范围：<br>- Hunyuan: 默认 5s<br>- Hailuo: 6s（默认）/ 10s；**H3 为 4~15 秒连续区间（任意整数均可），未指定时约 5.17s；超出区间会被接口静默忽略，脚本已前置拦截**<br>- Kling: 5s / 10s，默认 5s<br>- Vidu: 1~10s，默认 4s<br>- OS: 4s / 8s / 12s，默认 8s<br>- GV: 5s / 10s，默认 5s<br>- **PixVerse: 1~15s 任意整数，默认 5s** |
