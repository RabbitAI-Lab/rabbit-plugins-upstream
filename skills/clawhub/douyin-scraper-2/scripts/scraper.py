...
class DouyinScraper:
    ...
    def search(self, keyword: str, limit: int, mock: bool = False) -> list[VideoData]:
        if mock or sync_playwright is None:
        ...
        try:
            with sync_playwright() as p:
                ...
                page.goto(f"https://www.douyin.com/search/{keyword}", wait_until="domcontentloaded", timeout=30000)
                time.sleep(self.delay)
                browser.close()
        except Exception as exc:
            print(f"⚠️  浏览器爬取失败 ({exc})，返回示例数据")
        return self._mock_search(keyword, limit)

    def hot(self, category: str, limit: int, mock: bool = False) -> list[VideoData]:
        if mock or sync_playwright is None:
        ...
        try:
            with sync_playwright() as p:
                ...
                page.goto("https://www.douyin.com/hot", wait_until="domcontentloaded", timeout=30000)
                time.sleep(self.delay)
                browser.close()
        except Exception as exc:
            print(f"⚠️  浏览器爬取失败 ({exc})，返回示例数据")
...
def main() -> None:
    ...
    search.add_argument("--mock", action="store_true", help="直接返回示例数据，不启动浏览器")
    ...
    hot.add_argument("--mock", action="store_true", help="直接返回示例数据，不启动浏览器")

    args = parser.parse_args()
    scraper = DouyinScraper()
    if args.command == "search":
        items = scraper.search(args.keyword, args.limit, mock=args.mock)
    else:
        items = scraper.hot(args.category, args.limit, mock=args.mock)
