---
name: meta-self-evolution-metrics
version: 1.0.0
description: |
  由 model-distillation 从教师技能 self-evolution-metrics 蒸馏并增强的超越型元技能，
  在教师能力之上叠加自验证、自我反思、super-agent 编排与持续自进化闭环，逐步超越教师。
agent_created: true
visibility: public
---
# meta-self-evolution-metrics（蒸馏超越型元技能）

> 由 `model-distillation` 从教师技能 **self-evolution-metrics** 蒸馏并增强生成。
> 生成时间：2026-07-23 08:26:00 ｜ 蒸馏机制：跨模型蒸馏（见 meta-evolver 北极星策略）

## 来源能力签名（教师）
- 标题层级：self-evolution-metrics（持续自我进化度量）, 何时使用, 工作流, 用法, 集成（超越盲飞）, 已知限制, 自进化学习系统（越用越好用、越用越高效）, 记忆文件
- 显性工作流步骤（3 步）：
...
## 教师 vs 学生 对比
| 维度 | 教师(self-evolution-metrics) | 学生(meta-self-evolution-metrics) |
| --- | --- | --- |
| 能力来源 | 原始 SKILL.md（1904 字符） | 蒸馏提取 + 元进化增强 |
| 工作流 | 3 步显性流程 | 同流程 + 自验证钩子 + 反思步 |
| 工具脚本 | evo_metrics.py, learner.py | 继承 + reason-verify/self-reflection 钩子 |
| 失败防护 | 已识别 1 处 | 显式 limits + 对抗验证 |
| 自进化 | 视技能而定 | 强制注入 learner，纳入 meta-evolver 闭环 |
| 集成 | 单点 | 接入 super-agent 感知→规划→执行→自验证→反思→记忆闭环 |

## 使用
直接调用本技能完成「self-evolution-metrics」领域的任务；本技能在教师能力之上叠加自验证与反思，输出更可靠、可追溯。

## 已知限制（来自教师蒸馏 + 元进化补充）
- - 覆盖度基于 evolver_patterns 的 closed/open 记账，若记忆被手动改会失真。
- 停滞检测用近期 history 窗口（默认 5），长周期健康波动可能误报。
- 未做跨会话时间序列趋势图（可按 memory-cross-engine 扩展历史回溯）。
---
