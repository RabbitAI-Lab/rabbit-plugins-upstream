...
- **智能拓展引导**：输入泛化大类词（如"美食""穿搭"）时，自动生成 10 个细分方向供选择，精准锁定目标赛道，避免搜索结果偏差。
- **无结果兜底推荐**：关键词无匹配时，自动推荐近期热门作品和话题，不会空手而归。
- **一键订阅推送**：查完即订，每日 10:00 自动推送该关键词最新爆款作品，持续追踪趋势变化。
- **数据可点击直达**：表格中每条作品标题均为可点击超链接，一键跳转抖音原始页面查看完整内容。
...
| 意图             | 示例话术                           | 效果                                |
| ---------------- | ---------------------------------- | ----------------------------------- |
| 查某类内容的爆款 | "今日美食热门作品"                 | 识别为泛化词，推荐 10 个细分方向供选择 |
| 查细分赛道表现   | "帮我看看减脂餐在抖音火不火"       | 直接搜索细分词，按点赞排序展示        |
| 口语化随意搜     | "最近搞笑视频挺火的"               | 自动提取"搞笑"，返回热门作品        |
| 按时间筛选       | "5月30号到6月2号的旅游作品"        | 解析日期范围，返回该时段爆款作品     |
| 无结果也不落空   | 搜索小众关键词无匹配时             | 自动展示近期热门作品和话题推荐       |
...
（默认展示前 20 条；超过 20 条时提示查看全部；无匹配结果时自动展示热门推荐。）
