"""CLI argument parsing tests for lake-area-change."""
import subprocess
import sys
import os
...
class TestCLIBasics:
    ...
    def test_bad_index_method_exit_2(self):
    ...
    def test_synthetic_runs_exit_0(self, tmp_path):
        ...
        r = run_cli([
            "--bbox", "116", "39", "117", "40", "--n-dates", "5",
            "--synthetic", "--output-dir", out, "--quiet",
        ])
        assert r.returncode == 0, r.stderr
        assert os.path.exists(os.path.join(out, "lake_boundaries.geojson"))
        assert os.path.exists(os.path.join(out, "area_timeseries.json"))
