...
| 推翻的决策 | 删除旧条目,留新决策 |
| 跨会话只用一次的临时上下文 | 删除 |
| 被证伪 / 被新证据推翻的**有谱系结论**(docs/KB/决策记录) | 不物理删,改**墓碑**:标注"已被证伪 + 出处 + 日期"保留,否则反向检查失去对象 |
| 状态类事实(测试结果、上线状态、版本号、端口/路径) | **重跑它的验证锚**(命令/文件/哈希)再决定改不改,别拿记忆文字对文档文字;锚断链 → 标 `needs_verification`,不照抄进 docs |
| 索引 / 记忆文件"看起来乱" | **不是重写理由**。只动被本次教训命中的条目;跨条目重组仅在尺寸硬预算被突破时发起,且带显式保留规则 |
| 可复用教训 / "系统怎么工作"的机制说明滞留在记忆里 | 属积累型内容 → 迁进有版本管理的 `docs/`,记忆侧删或缩成一行指针(毕业) |
