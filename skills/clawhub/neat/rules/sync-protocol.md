...
- **增量 delta 优于全文重写**（针对 memory）：只动被本次会话教训命中的条目/文件；**「把 MEMORY.md 整个重写一遍让它更简洁」是禁止动作**——迭代重写有两个实测失效（brevity bias：为简洁丢领域细节；context collapse：反复重写侵蚀细节）。跨条目重组只允许由尺寸硬预算触发，且必须带显式保留规则（点名不许丢的命令 / 版本号 / 路径 / 失败条件 / 日期）。详见 [rules/memory-lifecycle.md](memory-lifecycle.md)
- **删除优于保留**：完成的临时计划、推翻的决策、已被新版本取代的项目记忆、单次事故的流水账复盘——删。**整理包含删除**：过期 / 被证伪 / 被新证据推翻的条目必须处置——运行时记忆直接删，有谱系的资产（docs/KB 里的结论）改**墓碑**（标注"已被证伪 + 出处 + 日期"保留，物理删掉会让反向检查失去对象）。淘汰分两层：机械判据（长期未被引用、指向已删代码）只做候选筛，删不删的终审是语义判断。一次同步里一条删除 / 墓碑都没有，要在摘要里说明理由——长期零删除 = 遗忘义务失活
- **事实类条目重跑锚，不比对文字**：状态、测试结果、版本号、端口 / 路径这类条目，对账时跑它的验证锚（命令 / 文件 / 哈希）再决定改不改——两份文字可以互相一致地一起过期。锚断链（命令改名 / 文件迁移）的条目标注 `needs_verification` 并降级，**不照抄进 docs/CLAUDE.md**；与当前证据冲突时，信当前证据不信记忆
...
**记忆生命周期（判据详见 [rules/memory-lifecycle.md](memory-lifecycle.md)）**：
- [ ] 记忆改动是**增量 delta**（没有整份重写 `MEMORY.md` 或记忆文件）；若确实做了跨条目重组，是尺寸硬预算触发的，且动手前写了显式保留规则
- [ ] 本次有**删除 / 墓碑**记录；一条都没有则在摘要里写明理由（长期零删除 = 遗忘义务失活）
- [ ] 事实类条目（状态 / 测试结果 / 版本号）**重跑过验证锚**而不是只比对文字；锚断链的已标 `needs_verification` 且没被照抄进 docs/CLAUDE.md
- [ ] 记忆里没有积累型内容滞留（可复用教训 / "系统怎么工作"的机制说明 → 迁进有版本管理的 docs，记忆侧留指针）
...
- 新增：xxx
- 删除：xxx（原因）
- 墓碑：xxx（被 yyy 证伪，保留出处 + 日期）
- 若本次无删除也无墓碑：写一行理由（长期零删除 = 遗忘义务失活）
