---
name: bigtimer
description: 🕐 BigTimer · 定时任务+消息推送管家 — 统一管理定时任务（cron 调度）与消息推送（飞书/多端）。OpenClaw 环境自动走 openclaw cron + message send，DSH 环境自动走 crontab + 飞书 webhook。支持任务增删改查、立即执行、调度条目生成，biga/eyes/bigfocus 等技能的定时推送可统一接入。| Scheduled tasks & message push manager — cron jobs with auto push to Feishu/multi-channel, dual-env (OpenClaw/DSH).
triggers:
  - 定时任务
  - 定时推送
  - bigtimer
