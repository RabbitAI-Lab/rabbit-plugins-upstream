...
API_KEY_URL = 'https://www.browseract.com/reception/integrations?co-from=amazon-product-reviews-scraper'


def run_amazon_product_reviews_scraper_task(api_key, marketplace_url='https://www.amazon.com', asin='B085DTZQNZ', review_count='10'):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python amazon_product_reviews_scraper_api.py <marketplace_url> [asin] [review_count]", flush=True)
        sys.exit(1)
    ...
    marketplace_url = sys.argv[1] if len(sys.argv) > 1 else 'https://www.amazon.com'
    asin = sys.argv[2] if len(sys.argv) > 2 else 'B085DTZQNZ'
    review_count = sys.argv[3] if len(sys.argv) > 3 else '10'

    result = run_amazon_product_reviews_scraper_task(api_key, marketplace_url, asin, review_count)
    if result:
