...
|---|---|
| 🏷️ 技能名称 | **婴幼儿趴睡窒息风险识别** |
| 🎯 核心目标 | 通过婴儿监护器（智能摄像头）固定于婴儿床上方，实时分析婴儿睡眠视频，检测婴儿的睡姿（仰卧、侧卧、俯卧）以及口鼻区域是否被被子、枕头、玩偶等异物遮挡。当检测到俯卧或口鼻被遮挡时，输出风险等级（中风险/高风险），并立即向父母手机APP推送警报，预防婴儿猝死综合征（SIDS）和窒息意外。 |
| 🖼️ 输入类型 | 图片、视频、本地文件、网络 URL |
| 📝 输出能力 | 结构化分析报告、识别/监测结果、建议与报告链接 |
| 🧩 场景码 | `SMYX_INFANT_SUFFOCATION_RISK_DETECTION_ANALYSIS` |
...
| 角色要点 | 说明 |
|---|---|
| 说明 1 | **假设你是一个专业的婴儿睡眠安全 AI。你的任务是分析婴儿床上方固定摄像头的实时视频，检测婴儿的睡姿（仰卧 / 侧卧 / 俯卧）以及口鼻区域是否被被子、枕头、毛绒玩具等物体遮挡。根据风险等级输出预警。不要提供医疗诊断或具体处置方案，仅输出基于视觉的睡姿与遮挡判断结果。** |
...
基于婴儿床上方固定摄像头的实时视频，识别婴儿睡姿与口鼻遮挡情况，按风险等级（low / medium / high / critical）输出预警，预防 SIDS 与窒息意外
...
|---:|---|
| 1 | 婴儿目标检测与跟踪 |
| 2 | 睡姿分类（supine 仰卧 / side 侧卧 / prone 俯卧 / unknown 不明） |
| 3 | 口鼻区域检测 |
| 4 | 遮挡物识别（被子 / 枕头 / 毛绒玩具 / 床单褶皱 / 同床成人手臂） |
| 5 | 风险等级判定（low / medium / high / critical） |
| 6 | 风险持续时长统计 |
| 7 | 现场快照生成 |
| 8 | 紧急预警文本生成 |
...
| 触发类型 | 触发规则 |
|---|---|
| ✅ 默认触发 | **默认触发**：当用户提供婴儿床上方监控视频 URL 或文件需要分析时，默认触发本技能进行趴睡窒息风险识别 |
| 🔎 明确分析意图 | 当用户明确提及婴儿睡姿、趴睡、俯卧、口鼻遮挡、SIDS、婴儿猝死综合征、婴儿窒息、婴儿监护器、新生儿监护、宝宝盖被遮脸等关键词，并且上传了视频文件 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看婴儿趴睡历史报告、婴儿窒息风险报告清单、婴儿睡眠安全报告清单、查询历史婴儿趴睡记录、显示所有婴儿窒息风险报告、显示婴儿睡眠诊断报告，查询婴儿睡姿预警清单 |
...
```bash
python -m scripts.smyx_infant_suffocation_risk_detection_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.smyx_infant_suffocation_risk_detection_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 步骤 | 阶段 | 执行动作 |
|---:|---|---|
| 1 | 📥 准备婴儿床上方监控视频输入 | 提供本地文件路径或网络 URL；确保输入内容清晰、符合技能场景要求 |
| 2 | 🔐 系统自动完成身份关联 | 无需用户输入任何身份参数；不在回复中展示内部身份值 |
| 3 | ⚙️ 执行婴儿趴睡窒息风险识别 | 调用 `-m scripts.smyx_infant_suffocation_risk_detection_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| 参数 | 含义 | 备注 |
|---|---|---|
| `--input` | 本地婴儿床上方监控视频文件路径 | 适用于本地文件分析 |
| `--url` | 网络婴儿床上方监控视频 URL 地址（API 服务自动下载） | API 服务自动下载网络资源 |
| `--pet-type` | 类别标识，婴儿睡眠安全场景默认 `other` | 按需填写 |
| `--list` | 显示婴儿趴睡窒息风险历史分析报告列表清单（可以输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/smyx_infant_suffocation_risk_detection_analysis.py`](scripts/smyx_infant_suffocation_risk_detection_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 🔎 使用提醒 | 预警结果仅作为辅助监护工具，不可替代成人监护；触发高/危险等级时请立即上前查看婴儿 |
...
```bash
# 分析本地婴儿床监控视频
python -m scripts.smyx_infant_suffocation_risk_detection_analysis --input /path/to/crib.mp4

# 分析网络婴儿床监控视频
python -m scripts.smyx_infant_suffocation_risk_detection_analysis --url https://example.com/crib.mp4

# 显示历史婴儿趴睡窒息风险报告（自动触发关键词：查看婴儿趴睡历史报告、婴儿窒息风险报告清单等）
python -m scripts.smyx_infant_suffocation_risk_detection_analysis --list

# 输出精简报告
python -m scripts.smyx_infant_suffocation_risk_detection_analysis --input crib.mp4 --detail basic

# 保存结果到文件
python -m scripts.smyx_infant_suffocation_risk_detection_analysis --input crib.mp4 --output result.json
```
