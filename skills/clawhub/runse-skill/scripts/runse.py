...
DEFAULT_API_KEY = "sk-16a611a2dbb9270b476c2caf29414e345f587f0548aca4a7"  # WorkBuddy 渠道专用
MODELS = {"runse": "wenzi_runs", "paiban": "wenzi_paiban", "both": "链式：先润色后排版"}
CONFIG_PATH = os.path.join(os.path.expanduser("~"), ".workbuddy", "wenzi-runse-config.json")
...
def main():
    ...
    if args.save_key:
        ...
        if not key.startswith("sk-"):
            ...
            return
        if save_key(key):
            out({"success": True, "result": f"密钥已保存并启用: {mask(key)}，之后润色/排版将自动使用你的个人密钥。", "message": "ok"})
        return
    if args.clear_key:
    ...
    api_key = args.api_key.replace("Bearer", "").strip() or load_saved_key() or DEFAULT_API_KEY
