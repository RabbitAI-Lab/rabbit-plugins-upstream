...
def list_styles(api_key: str) -> list[str]:
    """List available image styles via SkillBoss API Hub."""
    from venice_common import pilot
    try:
        resp = pilot({"type": "image", "inputs": {"list_styles": True}, "prefer": "balanced"}, api_key, timeout=30)
...
def generate_image(
    ...
    """Call SkillBoss API Hub image generation.

    Args:
        variants: Number of images to generate (1-4).
    ...
    if variants > 1:
        inputs["variants"] = min(variants, 4)
    ...
    if style_preset:
        inputs["style_preset"] = style_preset
    if resolution:
        inputs["resolution"] = resolution
    if aspect_ratio:
        inputs["aspect_ratio"] = aspect_ratio
    if embed_exif:
        inputs["embed_exif_metadata"] = True
    if lora_strength is not None:
        inputs["lora_strength"] = lora_strength
    if enable_web_search:
        inputs["enable_web_search"] = True
    if steps is not None:
        inputs["steps"] = steps

    resp = pilot({"type": "image", "inputs": inputs, "prefer": "balanced"}, api_key, timeout=120)
    result = resp.get("result", {})
    # Normalize: return {"images": [...b64...]} for downstream compatibility
    if "images" in result:
        return result
    if "data" in result:
        images = [item.get("b64_json", item.get("url", "")) for item in result["data"]]
        return {"images": images}
    if "image_url" in result:
        return {"images": [result["image_url"]], "_url_mode": True}
    raise RuntimeError(f"Unexpected image response: {str(result)[:300]}")
