...
FIELD_ALIASES = {
    ...
    "phase": ["phase", "阶段"],
    "task_records": ["task_records", "任务记录"],
    "pain_scores": ["pain_scores", "疼痛评分"],
    "functional_assessments": ["functional_assessments", "功能评估"],
    "phase_progress": ["phase_progress", "阶段进展"],
}
...
SYSTEM_PROMPT = """你是一位康复医学数据分析师，帮助患者理解康复进展。

你的任务：
1. 根据康复任务完成记录、疼痛评分、功能评估数据，分析康复趋势
2. 评估任务完成率是否理想
3. 分析疼痛趋势（下降=好转，上升=需关注）和功能趋势（上升=改善）
4. 指出需要关注的滞后项
5. 给出鼓励和建议
...
def build(data: Dict[str, Any], appkey: str) -> Dict[str, Any]:
    ...
    task_records = as_list(data.get("task_records", []))
    pain_scores = as_list(data.get("pain_scores", []))
    functional_assessments = as_list(data.get("functional_assessments", []))
    phase_progress = data.get("phase_progress", "")

    # 本地计算趋势
    completed = sum(1 for t in task_records if t.get("status") == "completed")
    completion_rate = completed / len(task_records) if task_records else 0

    pain_vals = [to_float(p.get("score")) for p in pain_scores]
    pain_vals = [v for v in pain_vals if v is not None]
    pain_trend = None
    if len(pain_vals) >= 2:
        diff = pain_vals[-1] - pain_vals[0]
        pain_trend = {"trend": "down" if diff < 0 else ("up" if diff > 0 else "stable"),
                      "first": pain_vals[0], "last": pain_vals[-1], "delta": diff}

    func_vals = [to_float(f.get("score")) for f in functional_assessments]
    func_vals = [v for v in func_vals if v is not None]
    func_trend = None
    if len(func_vals) >= 2:
        diff = func_vals[-1] - func_vals[0]
        func_trend = {"trend": "up" if diff > 0 else ("down" if diff < 0 else "stable"),
                      "first": func_vals[0], "last": func_vals[-1], "delta": diff}

    trends = {
        "completion_rate": completion_rate,
        "pain_trend": pain_trend,
        "function_trend": func_trend,
        "phase_progress": phase_progress,
    }

    user_prompt = f"""请分析以下康复进度数据：

计划ID：{plan_id}
阶段：{phase}

任务完成率：{completion_rate*100:.0f}%（{completed}/{len(task_records)}）
疼痛趋势：{json.dumps(pain_trend, ensure_ascii=False)}
功能趋势：{json.dumps(func_trend, ensure_ascii=False)}
阶段进展：{phase_progress}

任务详情：{json.dumps(task_records, ensure_ascii=False)}
...
请分析康复进展，指出改善项和关注项。"""
    ...
    return {
        ...
        "data": {
            ...
            **trends,
            "attention_items": [t["name"] for t in task_records if t.get("status") != "completed"],
        },
        "text": text.strip(),
    }
...
def main():
    parser = argparse.ArgumentParser(description="康复进度追踪")
