...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--api-key",
        help="Gemini API key. If omitted, MAVERICK_GEMINI_API_KEY or MAVERICK_GOOGLE_API_KEY is used.",
    )
    return parser.parse_args()


def resolve_api_key(cli_api_key: str | None) -> str:
    api_key = cli_api_key or os.getenv("MAVERICK_GEMINI_API_KEY") or os.getenv("MAVERICK_GOOGLE_API_KEY")
    if not api_key:
        raise ValueError(
            "Missing API key. Set MAVERICK_GEMINI_API_KEY or MAVERICK_GOOGLE_API_KEY, or pass --api-key."
        )
    return api_key
