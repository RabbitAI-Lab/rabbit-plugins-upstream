/**
 * RiskShield 案件审批自动化 - 使用 React 原生方法填写金额
 */
...
(async () => {
    ...
    try {
        ...
        // 格式化金额为两位小数
        const formattedAmount = parseFloat(CREDIT_AMOUNT).toFixed(2);
        ...
        // 点击提交按钮 - 使用 Playwright 的 click 方法
        log('  - 点击提交按钮...');
        const submitBtn = detailPage.locator('button').filter({ hasText: /^提交$/ }).or(detailPage.locator('button').filter({ hasText: /^Submit$/ }));
