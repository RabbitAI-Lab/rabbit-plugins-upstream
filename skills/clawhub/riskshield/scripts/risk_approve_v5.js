...
(async () => {
    ...
    try {
        ...
        // 使用更精确的 URL 模式 - 路径包含 /anytask-web/task/case/page/main.html
        // 且 URL 中包含 sgin= 表示已登录
        log('   等待页面跳转...');
        try {
            await page.waitForURL('**/anytask-web/task/case/page/main.html?sgin=**', { timeout: 20000 });
            log('   检测到跳转到审批页面');
        } catch (e) {
            log('   waitForURL 超时');
            // 检查是否跳转到了其他页面
            const currentUrl = page.url();
            if (currentUrl.includes('sgin=')) {
                log('   检测到 sgin 参数，登录成功');
            } else {
                log(`❌ 登录失败，当前URL: ${currentUrl}`);
                return;
            }
        }
