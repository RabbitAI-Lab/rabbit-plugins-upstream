/**
 * RiskShield 案件审批自动化 - 修复登录检测 v7
 * 使用 URL 变化检测替代 waitForResponse
 */
...
(async () => {
    ...
    try {
        ...
        const loginPromise = page.waitForResponse(resp => resp.url().includes('auth/login') && resp.status() === 200, { timeout: 20000 })
            .then(() => { loginSuccess = true; })
            .catch(() => { loginSuccess = false; });

        // 同时等待 URL 变化
        const urlPromise = page.waitForFunction(() => window.location.href.includes('anytask-web'), { timeout: 20000 })
            .then(() => true)
            .catch(() => false);

        // 等待任意一个完成
        await Promise.race([loginPromise, urlPromise]);
        
        // 额外等待确保页面稳定
        await page.waitForTimeout(10000);

        if (page.url().includes('login') && !page.url().includes('sgin')) {
            log('❌ 登录失败，当前URL: ' + page.url());
            return;
        }
        log('[1/8] ✅ 登录成功');
