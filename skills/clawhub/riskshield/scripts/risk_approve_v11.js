/**
 * RiskShield 案件审批自动化 - 最终版 (修复 Pass 选项)
 */
...
(async () => {
    ...
    try {
        ...
        console.log('   可见下拉框数量:', visibleSelects.length);
        
        if (visibleSelects.length >= 1) {
            await visibleSelects[0].click();
        } else {
            // fallback: 使用所有下拉框中的第一个
        ...
        if (ACTION === 'pass') {
            ...
            // 选择 Refuse Code - 第 9 个下拉框 (index 8)
            log('  - 选择 Refuse Code...');
