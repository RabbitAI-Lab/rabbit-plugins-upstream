/**
 * RiskShield 案件审批自动化 - 简化登录检测
 * 信任 waitForResponse 返回成功即可
 */
...
(async () => {
    ...
    try {
        ...
        // 等待 auth/login 响应成功
        const authResp = await page.waitForResponse(
            resp => resp.url().includes('auth/login') && resp.status() === 200,
            { timeout: 15000 }
        ).catch(e => {
            log(`   等待 auth/login 响应失败: ${e.message}`);
            return null;
        });
        ...
        log('   auth/login 响应: ' + authResp.status());
