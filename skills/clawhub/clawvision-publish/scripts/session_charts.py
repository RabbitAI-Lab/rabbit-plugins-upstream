...
if ECHARTS_SRC is None:
    # No CDN fallback: analytics charts are skipped if the local bundle is missing.
    ECHARTS_SRC = None

def _echarts_available() -> bool:
    return ECHARTS_SRC is not None

def _echarts_block() -> str:
    if not _echarts_available():
        return "<!-- ECharts bundle missing; analytics charts disabled to avoid CDN load. -->"
    return f"<script>{ECHARTS_SRC}</script>"

def _charts_enabled() -> bool:
    if not _echarts_available():
        print(json.dumps({"warning": "echarts.min.js not found; charts disabled."}, ensure_ascii=False), file=sys.stderr)
        return False
    return True
...
def generate_charts_html(records: List[dict], preset: dict, annotations: Dict[str, str] = None, dark: bool = False) -> str:
    ...
    for idx, (chart_id, option) in enumerate(charts):
        rendered = _render_echarts(f"chart_{idx}", option, theme, height=360)
        if rendered:
    ...
    if not body_parts:
        body_parts.append(
            f"<div style='padding:24px;background:{theme['card']};border:1px solid {theme['border']};border-radius:12px;color:{theme['muted']};'\u003e"
            "Analytics charts are disabled because the local ECharts bundle (scripts/echarts.min.js) is missing. "
            "No CDN is loaded for security."
            "</div>"
        )

    echarts_block = _echarts_block()
...
def generate_for_session(messages: List[dict], output_dir: Path, slug: str, preset: dict = None, annotations: Dict[str, str] = None) -> dict:
    ...
    if not _charts_enabled():
        ...
        light_path.write_text(
            ...
            "<p>Analytics charts disabled: local ECharts bundle not found. No CDN loaded.</p>"
            "</body></html>",
            encoding="utf-8"
        )
        dark_path.write_text(
            "<!DOCTYPE html><html><body style='font-family:sans-serif;padding:24px;background:#111;color:#eee;'>"
            "<p>Analytics charts disabled: local ECharts bundle not found. No CDN loaded.</p>"
            "</body></html>",
            encoding="utf-8"
        )
        # Minimal 1x1 transparent PNG placeholder so callers don't crash on missing image.
        try:
        ...
        return {
            ...
            "charts_disabled_reason": "echarts.min.js not found; CDN fallback disabled for offline-by-default security.",
        }
