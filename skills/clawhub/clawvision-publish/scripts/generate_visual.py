...
from session_charts import generate_for_session
...
<div class="wrap">
  <div class="topbar">
    <div class="controls">
      <a id="export-md" class="export-btn" href="{{md_file}}" download>Export Markdown</a>
      <a id="export-pptx" class="export-btn" href="{{pptx_file}}" download>Export PowerPoint</a>
    </div>
    <div class="controls">
  ...
  <div id="analytics" class="panel">
    <p class="lead">{{analytics_takeaway}}</p>
    {{analytics_charts}}
  </div>
...
function setLang(l){
  ...
  if (exportMd && labels.export_md) exportMd.textContent = labels.export_md;
  const exportPptx = document.getElementById('export-pptx');
  if (exportPptx && labels.export_pptx) exportPptx.textContent = labels.export_pptx;
  const analyticsTitle = document.getElementById('analytics-title');
  if (analyticsTitle && labels.analytics) analyticsTitle.textContent = labels.analytics;
}
function setTheme(t){
...
LANG_LABELS = {
    "en": {"main": "Main takeaway", "format": "Format", "built": "What we built", "next": "Next steps", "dos": "Do", "donts": "Don't", "export_md": "Export Markdown", "export_pptx": "Export PowerPoint", "badge": "ClawVision · Summary", "analytics": "Analytics"},
    "ru": {"main": "Главный вывод", "format": "Формат", "built": "Что построено", "next": "Что дальше", "dos": "Нормально", "donts": "Риски", "export_md": "Экспорт Markdown", "export_pptx": "Экспорт PowerPoint", "badge": "ClawVision · Сводка", "analytics": "Аналитика"},
    "zh": {"main": "主要结论", "format": "形式", "built": "已完成", "next": "下一步", "dos": "建议", "donts": "风险", "export_md": "导出 Markdown", "export_pptx": "导出 PowerPoint", "badge": "ClawVision · 摘要", "analytics": "分析"},
}
...
def render_html(summary: dict, slug: str, md_file: str, pptx_file: str, lang: str = 'en', preset_name: str = 'minimal') -> str:
    ...
    has_analytics = bool(summary.get('analytics_charts'))
    template = template.replace('{{analytics_tab}}', f'<div class="tab" data-tab="analytics"><span id="analytics-title">{labels.get("analytics","Analytics")}</span></div>' if has_analytics else '')
    template = template.replace('{{analytics_takeaway}}', _h(summary.get('analytics_takeaway', '')))
    template = template.replace('{{analytics_charts}}', summary.get('analytics_charts', ''))
...
def render_pptx(summary: dict, pptx_path: Path, lang: str = 'en', preset_name: str = 'minimal'):
    ...
    slide = prs.slides.add_slide(blank)
    set_slide_bg(slide, BG)
    textbox(slide, Inches(0.6), Inches(0.4), Inches(12), Inches(0.7), labels.get('analytics', 'Analytics'), font_size=32, bold=True, color=TEXT)
    if summary.get('analytics_charts'):
        chart_png = pptx_path.parent / f"{pptx_path.stem}_charts.png"
        if chart_png.exists():
            lead = rounded_rect(slide, Inches(0.6), Inches(1.2), Inches(12), Inches(0.5), CARD)
            set_text(lead, summary.get('analytics_takeaway', ''), font_size=15, color=TEXT)
            slide.shapes.add_picture(str(chart_png), Inches(0.6), Inches(1.85), width=Inches(12), height=Inches(5.0))
        else:
            lead = rounded_rect(slide, Inches(0.6), Inches(1.2), Inches(12), Inches(0.9), CARD)
            set_text(lead, summary.get('analytics_takeaway', ''), font_size=18, color=TEXT)
    else:
        lead = rounded_rect(slide, Inches(0.6), Inches(1.2), Inches(12), Inches(0.9), CARD)
        set_text(lead, summary.get('analytics_takeaway', ''), font_size=18, color=TEXT)
...
def screenshot_tabs(html_path: Path, slug: str, width: int = 900, height: int = 1100, include_analytics: bool = False):
    tab_ids = ['main', 'format', 'built']
    if include_analytics:
        tab_ids.append('analytics')
    tab_ids.append('next')
    paths = []
    with sync_playwright() as p:
        ...
        for idx, tab_id in enumerate(tab_ids, start=1):
            ...
            page.wait_for_timeout(350 if tab_id == 'analytics' else 150)
            png_path = html_path.parent / f'{slug}_tab{idx}.png'
            page.screenshot(path=str(png_path), full_page=(tab_id == 'analytics'))
...
def main():
    ...
    parser.add_argument('--session', help='Path to session JSON for analytics charts')
    parser.add_argument('--include-charts', action='store_true', help='Generate session analytics charts (requires --session)')
    ...
    # Session analytics charts (smart-charts inspired, offline-only)
    if args.include_charts and args.session:
        session_path = Path(args.session)
        if session_path.exists():
            session_data = json.loads(session_path.read_text(encoding='utf-8'))
            messages = session_data if isinstance(session_data, list) else session_data.get('messages', [])
            chart_result = generate_for_session(messages, out_dir, slug, preset=PRESETS.get(args.preset, PRESETS['minimal']))
            if chart_result.get('charts_disabled'):
                summary['analytics_charts'] = (
                    f"<div style='padding:16px;background:var(--card-bg-alt);border:1px solid var(--border);border-radius:14px;color:var(--muted);'>"
                    f"Analytics charts disabled: {chart_result.get('charts_disabled_reason', 'local ECharts bundle missing')}. "
                    "No CDN is loaded."
                    "</div>"
                )
                summary['analytics_takeaway'] = "Session analytics disabled (offline-only bundle missing)."
            else:
                summary['analytics_charts'] = (
                    f'<img class="chart" src="{Path(chart_result["charts_png"]).name}" alt="Session analytics" style="width:100%;border-radius:14px;">'
                )
                metrics = chart_result.get('metrics', {})
                summary['analytics_takeaway'] = (
                    f"{sum(metrics.get('message_count_by_role', {}).values())} messages across "
                    f"{len(metrics.get('message_count_by_role', {}))} roles; "
                    f"{sum(metrics.get('tool_usage_counts', {}).values())} tool-involved turns."
                )
    ...
    if args.png:
        tab_paths = screenshot_tabs(html_path, slug, include_analytics=bool(summary.get('analytics_charts')))
