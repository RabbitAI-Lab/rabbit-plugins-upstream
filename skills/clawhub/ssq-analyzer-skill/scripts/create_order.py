...
Creates a local order file for clawtip payment processing and outputs
ORDER_NO, AMOUNT, INDICATOR.
...
Contains only essential payment metadata:
  payTo, amount, order_no, encrypted_data, slug, description, resource_url.
No user question, personal data, or lottery data is stored or transmitted.
"""
import hashlib
import json
import os
...
def create_order_file(indicator: str) -> dict:
    ...
    # Only essential payment metadata is persisted. No user-provided content is stored.
    order_data = {
        "payTo": pay_to,
        "amount": AMOUNT,
        "order_no": order_no,
...
def main() -> int:
    ...
    print("        Fields: orderNo, amount, slug, payTo, encrypted_data,")
    print("        description, resource_url (essential payment metadata only).")
    print("        No user question, personal info, or lottery data is stored.")
    print("=" * 60)

    try:
        order_data = create_order_file(indicator)
    except Exception as e:
        print(f"订单创建失败: {e}")
        return 1
