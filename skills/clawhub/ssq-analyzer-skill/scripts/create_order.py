...
Order file is stored at:
  ~/.openclaw/skills/orders/{indicator}/{order_no}.json  (Linux/macOS)
  ~/openclaw/skills/orders/{indicator}/{order_no}.json   (Windows)

Contains: payTo, amount, order_no, encrypted_data, slug, question, description, resource_url.
After payment, clawtip adds: payCredential, credentialTimestamp, payStatus.
These are clawtip payment verification fields only.
Caution: question text is stored locally - do not include passwords or secrets.
No SSQ draw data, purchase history, or personal info is stored or transmitted.
"""
import argparse
import hashlib
import json
...
DESCRIPTION = "SSQ (Double Color Ball) lottery intelligent analysis"
RESOURCE_URL = os.environ.get("CLAWTIP_RESOURCE_URL", f"https://clawhub.ai/skill/{SLUG}")
ENCODED_INFO_DATA = "ssq-analyzer-order-info"

# ============================================================
# SM4 encryption (GB/T 32907-2016) - inlined for payment flow
# Encrypts {orderNo, amount, payTo} for clawtip fee processing.
# No lottery data is ever encrypted or transmitted.
# ============================================================
...
def _sm4_encrypt_base64(key_b64, plain):
    import base64
    return base64.b64encode(_sm4_encrypt_ecb(base64.b64decode(key_b64), plain.encode("utf-8"))).decode()
...
def create_order_file(question: str, indicator: str) -> dict:
    ...
    # Encrypt {orderNo, amount, payTo} with SM4 for clawtip fee processing
    ...
    if sm4_key:
        try:
            if len(sm4_key) == 32 and all(c in "0123456789abcdefABCDEF" for c in sm4_key):
                ...
                encrypted_data = _sm4_encrypt_base64(sm4_key, encrypt_payload)
...
if __name__ == "__main__":
    ...
    print("        description, resource_url (clawtip payment metadata only).")
    print("        No SSQ data, purchase history, or personal info is stored.")
    print("        Caution: do not include passwords or secrets in the question.")
    print("=" * 60)
    ...
    summary = {"order_no": order_data["order_no"], "amount": order_data["amount"], "indicator": indicator, "slug": SLUG}
