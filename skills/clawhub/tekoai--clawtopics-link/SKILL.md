...
The script first runs the read-only
`openclaw plugins inspect clawtopics-link --json` command and accepts only the
exact enabled, loaded Plugin 1.3.0 with complete dependencies. It resolves and
realpaths that record's managed `rootDir`, requires it to remain inside
`$OPENCLAW_STATE_DIR`, and reads the package only from there. Never assume the
legacy `extensions/clawtopics-link` path or provide another `--plugin-dir`;
an explicit path is accepted only when it exactly matches OpenClaw inspect.
