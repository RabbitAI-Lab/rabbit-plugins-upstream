...
| ID | 任务类型 | 说明 | 依赖 | 元操作映射 |
|----|---------|------|------|-----------|
| D0-01 | 猜想类型识别 | 接收猜想陈述，判定所属数学领域（解析数论/代数/组合/几何等） | 无（入口） | S→C |
...
| D1-04 | 工具评估 | 可用定理、缺失工具、可迁移工具、数值校准策略可行性评估 | D1-03 | C |
...
| D2-02 | 引理设计 | 中间引理定义、严格证明、依赖关系；标注「解析已证」或「数值已验证」 | D2-01 | C→A |
| D2-03 | 管线编排 | 引理依赖DAG→执行顺序→并行/串行标注→校准回路设计 | D2-02 | C→A |
| D2-04 | 等价转化 | 原猜想→等价命题→转化证明→可攻击性评估 | D1-05, D2-01 | C→A |
| D2-05 | 桥接构造 | 桥接定理+攻击向量+路线评估+**近/远自动分类**（远项自动同号✅，近项需控制🔴） | D2-01, D2-04 | C→A |
| D2-06 | 死路排除 | 失败路径记录+严格证伪+避免后人重复 | D2-05 | C→A |
| D2-07 | 归约 | 将猜想压缩为最小可验证形态（如RH→|ξ|²单调性→导数求和公式→近零点不等式） | D2-05 | C→A |
| D2-08 | 数值校准 | 数值网格扫描验证方向正确性（非证明，是校准锚点） | D3-02 | S→C |
...
| ID | 任务类型 | 说明 | 依赖 | 元操作映射 |
|----|---------|------|------|-----------|
| D3-01 | 解析界推导 | 每个常数的有理数上下界推导+推导过程记录 | D2-07 | C→A |
| D3-02 | 数值扫描 | 网格覆盖（σ,γ范围+步长）+违反统计汇总 | D2-08 | S→C→A |
| D3-03 | 公式匹配验证 | 推导公式与独立数值差分对比+误差量化分析 | D2-07, D3-02 | C→G |
| D3-04 | 安全边际计算 | gap显式计算，必须>0；单调性方向确认 | D3-01 | C→A |
...
| D4-04 | 复核 | 五维复核（连贯性/引用/比例/结构/深度） | D4-02, D4-03 | G |
...
**D2⇄D3 回路**: D2-07 → D3-02 → D2-08（数值校准反馈）→ D2-05（桥接修正）→ D2-07（重新归约）
...
- **领域负载物节点**: RH 案例文件（assets/ + scripts/）
...
2. **近/远分类**: D2-05中必须执行
3. **数值校准**: D2-08不产生证明但防走错路线，建议在D2-07归约后立即执行
