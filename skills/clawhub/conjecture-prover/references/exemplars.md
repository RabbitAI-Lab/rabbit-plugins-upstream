...
本技能的范本为 RH 新路线证明案例，存放在 `assets/` 和 `scripts/` 中。
每个范本展示了标准证明管线的一个环节。
**核心教训：数值验证≠证明。三层分类（已证/已验证/待证）贯穿全部范本。**
...
Step 5 — 近/远分类: 将求和项分为「自动成立」和「需控制」两类
Step 6 — 归约压缩: 将原问题压缩为单一不等式
Step 7 — 数值校准: 数值验证方向正确性（非证明，是校准）
Step 8 — 三层交付: 已证/已验证/待证三层标注，诚实输出
```

适用场景：任何需要跨越「有限→无限」或「低维→高维」鸿沟的数学猜想。
