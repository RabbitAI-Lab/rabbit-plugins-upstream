#!/usr/bin/env node
/**
 * 通过企业域名查询企业详情、联系方式、社媒和职员信息。
 *
 * 用法:
 *   node ./scripts/search_by_domain.js "microsoft.com"
 */
...
function saveResult(domain, json) {
  const ts = new Date().toISOString().slice(0, 19).replace(/[:-]/g, "");
  const safeDomain = domain.replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 60) || "domain";
  const fileName = `easy_search_domain_${safeDomain}_${ts}.json`;
...
async function main() {
  const domain = normalizeDomain((process.argv[2] || "").trim());
  if (!domain) {
    console.error('用法: node ./scripts/search_by_domain.js "<企业域名>"');
    process.exit(1);
  }

  const url = new URL(`${BASE_URL}/EasySearch/GetCompanyDetailByDomain`);
  ...
  try {
    ...
    const saved = saveResult(domain, body);
    console.log("查询完成");
    console.log("搜索类型: 企业域名");
    console.log(`搜索内容: ${domain}`);
