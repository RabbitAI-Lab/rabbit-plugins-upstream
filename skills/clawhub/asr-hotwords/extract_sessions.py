...
def extract_sessions(
    ...
    for agent_dir in agent_dirs:
        ...
        for fpath in glob.glob(os.path.join(agent_dir, "*.jsonl")):
            ...
            # 快速跳过：文件修改时间早于目标日期起始则跳过
            # 注意：仅当 mtime 早于 target_date 前一天时才跳过，
            # 避免跨天 session 被误排除
            try:
                mtime = os.path.getmtime(fpath)
                mtime_date = datetime.fromtimestamp(mtime, tz=CST).date()
                target_date_obj = datetime.strptime(target_date, "%Y-%m-%d").date()
                if mtime_date < target_date_obj - timedelta(days=1):
                    continue
