...
name: stormproof
description: >-
  Look up historical NOAA hurricane weather data — peak wind speed, wind gust,
  and storm surge — for a SPECIFIC U.S. street address on a specific date. Use
  this skill ONLY when the user wants weather data tied to a specific property
  address — e.g. "wind speed at my address during Milton", "verify the storm
  for my insurance claim at [address]", "NOAA data for my date of loss", "how
  strong were the gusts at [address]". The tell is an address (or the user's
  intent to provide one) plus a verification purpose: an insurance claim, a
  date of loss, an adjuster dispute, forensic weather verification, wind vs.
  flood damage evaluation, or historical storm research on a property. Do NOT
  activate for general weather conversation, forecasts, storm news, or
  named-storm questions that aren't about a specific property.
---
...
**Consent script (used in step 2 of the mandatory sequence below):**
...
1. Confirm a full street address and a date (or a named storm you can convert to a date).
2. **Obtain consent** using the script above. This step is mandatory even when the user sounds eager, has already pasted their address, or consented in a previous conversation — consent is per conversation.
3. Call `stormproof_lookup` only after the user agrees.
4. Present findings with NOAA attribution.

Every example in this document shows step 2 explicitly because it is not optional boilerplate — an example without the consent exchange is a bug.
...
**Contextual signals (confirm a specific address and date first, then run the sequence):**
...
- General weather conversation or storm news that isn't about a specific property address
...
- Earthquake shaking questions — use the `@oasiseng/quakeproof` skill instead (USGS data, not NOAA)
...
```json
{
  ...
  "spcData": {
    ...
    "hail": 9,
    "tornado": 2,
    "alertsPageUrl": "https://alerts.hurricaneinspections.com/state/FL/2024-10-09"
  },
  "stationCount": 5,
...
- `hailData`: Present only if hail was detected in station observations near the address; `null` otherwise. When present, mention it — hail is an independently claimable peril.
- `spcData`: Present only if NOAA's Storm Prediction Center logged confirmed severe-weather reports in the state that day; `null` otherwise. Cite the counts as corroboration and offer `alertsPageUrl` for the user to browse the confirmed reports.
...
> Based on NOAA data from [stationCount] weather stations near [address], peak sustained winds during [storm/date] were in the **[windRange] mph** range, with gusts reaching **[gustRange] mph**. [If surge: Storm surge at the nearest tide gauge ([stationName], [distanceMiles] mi away) peaked at **[peakSurgeFt] ft** above predicted tide.] [If hail: Station observations also detected hail near the address.] [If spcData: NOAA's Storm Prediction Center logged [total] confirmed severe-weather reports in the state that day.]
>
...
See the "Data flow, privacy, and consent" section at the top of this file for the endpoint, retention policy, and consent script, and the "Invocation sequence" section for the mandatory ordering.
...
**Agent:** "I can check that. I'll use the StormProof NOAA data service at hurricaneinspections.com — the lookup sends your street address and the storm date (October 9, 2024) to that service, which logs the request for service improvement. No email or identifying info beyond the address is sent. OK to proceed?"

**User:** "Yes, go ahead."

**Agent:** *[calls stormproof_lookup]*
...
**Agent:** "I need a street address, not just a city — the tool geocodes to a specific point and finds the nearest NOAA stations. Can you share the full address?"

**User:** *[provides address]*

**Agent:** "Thanks. Before I run it: the lookup sends that address and the date (August 30, 2023) to the StormProof NOAA data service at hurricaneinspections.com, which logs the request for service improvement. OK to proceed?"

**User:** "OK."

**Agent:** *[calls stormproof_lookup, gets dataAvailable: false]*
...
**Agent:** "I can run that, but I need the full street address — the tool geocodes to a specific point so the 'nearest stations' lookup is meaningful. What's the street number and street name? And a heads-up before I run it: the lookup sends the address and the date to the StormProof NOAA data service at hurricaneinspections.com, which logs the request for service improvement. OK?"

### Example 4 — Inland address, no surge

**User:** "Wind speeds at 4501 W Flagler St, Miami FL during Irma?"

**Agent:** "I can pull that. The lookup sends that address and the date (September 10, 2017) to the StormProof NOAA data service at hurricaneinspections.com, which logs the request for service improvement — no other identifying info is sent. OK to proceed?"

**User:** "Yes."

**Agent:** *[calls stormproof_lookup for 2017-09-10]*
