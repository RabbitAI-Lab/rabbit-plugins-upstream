...
REVIEW_PROMPT_EOF
  ...
  REVIEW_CMD=$("$SWARM_DIR/fallback-swap.sh" reviewer 2>/dev/null) || REVIEW_CMD="codex exec --model gpt-5.5 --full-auto"

  # Build the reviewer wrapper — gemini needs -y -p; other CLIs accept prompt as final arg
  ...
  if echo "$REVIEW_CMD" | grep -q "^gemini"; then
    ...
    GEMINI_BASE=$(echo "$REVIEW_CMD" | sed -E 's/[[:space:]]+-p[[:space:]]*$//; s/[[:space:]]*$//')
...
# ============================================================
# STEP 11: NOTIFY — Clean "shipped" summary for the operator
# ============================================================
