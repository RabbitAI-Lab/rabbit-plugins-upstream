...
INTEG_PROMPT_EOF
  ...
  SWARM_DIR="$(cd "$(dirname "$0")" && pwd)"
  INTEG_CMD=$("$SWARM_DIR/fallback-swap.sh" integrator 2>/dev/null) || INTEG_CMD="codex exec --model gpt-5.5 --full-auto"

  INTEG_WRAPPER="/tmp/integ-wrapper-${INTEG_SESSION}.sh"
  if echo "$INTEG_CMD" | grep -q "^gemini"; then
    GEMINI_BASE=$(echo "$INTEG_CMD" | sed -E 's/[[:space:]]+-p[[:space:]]*$//; s/[[:space:]]*$//')
