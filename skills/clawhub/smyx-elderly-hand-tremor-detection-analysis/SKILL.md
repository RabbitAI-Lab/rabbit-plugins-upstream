...
| 🔎 明确分析意图 | 当用户明确提及手抖、手部震颤、静止性震颤、帕金森、神经系统早筛、手部周期性抖动等关键词，并且上传了视频文件 |
| 📚 历史报告查询 | 当用户提及以下关键词时，**自动触发历史报告查询功能** ：查看手部震颤历史报告、静止性震颤报告清单、帕金森早筛报告清单、查询历史震颤记录、显示所有手部震颤报告、显示老人神经系统诊断报告，查询震颤风险预警清单 |
...
```bash
python -m scripts.smyx_elderly_hand_tremor_detection_analysis --list
```

| 类型 | 要求 |
|---|---|
| ✅ 必须 | 使用 `python -m scripts.smyx_elderly_hand_tremor_detection_analysis --list` 调用 API 查询云端的历史报告数据 |
...
| 步骤 | 阶段 | 执行动作 |
|---:|---|---|
| 1 | 📥 准备老年人手部静止状态视频输入 | 提供本地文件路径或网络 URL；确保输入内容清晰、符合技能场景要求 |
| 2 | 🔐 系统自动完成身份关联 | 无需用户输入任何身份参数；不在回复中展示内部身份值 |
| 3 | ⚙️ 执行老年人手部震颤识别 | 调用 `-m scripts.smyx_elderly_hand_tremor_detection_analysis` 处理输入（**必须在技能根目录下运行脚本**） |
...
| 参数 | 含义 | 备注 |
|---|---|---|
| `--input` | 本地老年人手部静止状态视频文件路径 | 适用于本地文件分析 |
| `--url` | 网络老年人手部静止状态视频 URL 地址（API 服务自动下载） | API 服务自动下载网络资源 |
| `--pet-type` | 类别标识，老年人神经系统健康监测场景默认 `other` | 按需填写 |
| `--list` | 显示老年人手部震颤历史分析报告列表清单（可以输入起始日期参数过滤数据范围） | 用于云端历史报告查询 |
...
| 资源类型 | 路径 | 用途 | 何时读取 |
|---|---|---|---|
| 🐍 必要脚本 | [`scripts/smyx_elderly_hand_tremor_detection_analysis.py`](scripts/smyx_elderly_hand_tremor_detection_analysis.py) | 调用 API、执行分析或查询历史报告 | 执行分析或查询时使用 |
...
| 📁 格式支持 | 输入要求：支持 mp4/avi/mov 视频，最大 10MB；建议 ≥ 30 FPS、15-30 秒、手部完整入画 |
| 🧑‍⚖️ 结果性质 | 检测结果仅作为辅助筛查参考，本工具不替代专业神经科诊断；疑似帕金森请前往神经内科评估 |
| 🔏 隐私合规 | 隐私合规：手部视频可能涉及个人健康信息，使用前需取得被监护人或家属知情同意，并妥善保管/加密相关录像 |
...
```bash
# 分析本地手部静止视频
python -m scripts.smyx_elderly_hand_tremor_detection_analysis --input /path/to/hand_rest.mp4

# 分析网络手部静止视频
python -m scripts.smyx_elderly_hand_tremor_detection_analysis --url https://example.com/hand_rest.mp4

# 显示历史手部震颤识别报告（自动触发关键词：查看手部震颤历史报告、静止性震颤报告清单等）
python -m scripts.smyx_elderly_hand_tremor_detection_analysis --list

# 输出精简报告
python -m scripts.smyx_elderly_hand_tremor_detection_analysis --input hand.mp4 --detail basic

# 保存结果到文件
python -m scripts.smyx_elderly_hand_tremor_detection_analysis --input hand.mp4 --output result.json
```
