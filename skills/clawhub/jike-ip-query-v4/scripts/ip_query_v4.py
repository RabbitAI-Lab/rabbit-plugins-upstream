...
API_PATH = "/v1/ip/query/v4"
APPKEY_ENV_NAMES = ("JIKE_IP_QUERY_V4_KEY", "JIKE_APPKEY")
...
def load_appkey(cli_key: str | None = None) -> str:
    ...
    2. 优先使用命令行 `--key`，方便一次性调试。
    3. 其次读取环境变量，便于 QClaw/OpenClaw 等客户端统一配置。
    4. 最后读取脚本目录下 `.env` 文件，方便本地验证。
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    功能说明：
    1. 解析 IPv4 查询脚本参数。
    2. 校验用户是否传入 IP 地址、是否要求 JSON 输出。
    3. 返回 argparse 命名空间，后续逻辑统一读取该对象。

    @param argv 命令行参数列表
    @return argparse.Namespace 解析后的参数对象
    """
    parser = argparse.ArgumentParser(description="IPv4地址查询 - 即刻数据")
    parser.add_argument("ip", help="IPv4 地址，例如 60.31.46.0")
...
def validate_ipv4(ip_value: str) -> str:
    """
    功能说明：
    1. 校验用户输入是否为合法 IPv4 地址。
    2. 使用标准库 ipaddress 做格式判断，避免将域名或 IPv6 误传给 IPv4 接口。
    3. 返回标准化后的 IPv4 字符串。

    @param ip_value 用户输入的 IP 地址
    @return string 标准化后的 IPv4 地址
    @raises ValueError IP 格式不合法时抛出
    """
    try:
        ip_obj = ipaddress.ip_address(ip_value.strip())
    except ValueError as exc:
        raise ValueError("请提供合法的 IPv4 地址，例如 60.31.46.0") from exc

    if ip_obj.version != 4:
        raise ValueError("当前 Skill 仅支持 IPv4；IPv6 请使用 jike-ip-query-v6")
...
def request_api(ip_value: str, appkey: str) -> dict[str, Any]:
    """
    功能说明：
    1. 调用即刻数据 IPv4 地址查询接口。
    2. 使用 `ip` 和 `appkey` 两个查询参数。
    3. 将接口返回 JSON 解析为 Python 字典，网络异常时返回统一错误结构。

    @param ip_value IPv4 地址
    @param appkey 即刻数据 AppKey
    @return dict 接口返回或错误结构
    """
    params = urllib.parse.urlencode({"ip": ip_value, "appkey": appkey})
...
def print_text_result(payload: dict[str, Any]) -> int:
    """
    功能说明：
    1. 将接口结果转换为适合 AI 客户端展示的文本。
    2. 成功时展示 IP、国家、省份、城市、运营商和 Long 数值。
    3. 失败时展示接口返回的错误信息，便于用户修正参数或检查 AppKey。
...
def main() -> int:
    ...
    try:
        ip_value = validate_ipv4(args.ip)
    except ValueError as exc:
        print(f"错误: {exc}")
        return 1
    ...
    if not appkey:
        print("错误: 未找到 AppKey，请先配置环境变量 JIKE_IP_QUERY_V4_KEY 或 JIKE_APPKEY")
