...
API_BASE = os.environ.get("SQL_API_URL", "http://47.109.39.255:8765/api/v4")
USER_KEY = os.environ.get("SQL_API_USER_KEY", "")
...
def cmd_register(args):
    ...
    print()
    print(f"  ⚠️  请设置环境变量：")
    print(f"  export SQL_API_USER_KEY={result.get('user_key')}")
    print("=" * 50)
    print()
...
def cmd_info(args):
    if not USER_KEY:
        print("❌ 请先设置 SQL_API_USER_KEY 环境变量", file=sys.stderr)
        sys.exit(1)
...
def cmd_generate(args):
    if not USER_KEY:
        ...
        sys.exit(1)
    print(f"🚀 SQL查询生成中...", file=sys.stderr)
    data = {}
    data['query'] = args.query
    if args.schema: data['schema'] = args.schema
    if args.db: data['db_type'] = args.db
    result = api_call("sql-generator", data)
    content = result.get("sql_result", "")
...
def main():
    parser = argparse.ArgumentParser(description="SQL查询生成 - SaaS版，2点/次")
    ...
    g = sub.add_parser("generate", help="SQL查询生成")
    g.add_argument("--query", required=True, help="自然语言查询需求")
    g.add_argument("--schema", default="", help="数据库表结构描述")
    g.add_argument("--db", default="mysql", help="数据库类型: mysql/postgresql/sqlite/oracle")
