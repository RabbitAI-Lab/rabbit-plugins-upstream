...
Before any answer or workflow, run the deterministic difficulty gate **once** at entry:

> 🔴 **Code-based difficulty gate (2026-08-14 晚)**: execute `python scripts/route.py "<original_question>"` (stdlib-only, no LLM). It returns exactly one label — `simple` / `middle` / `complex` / `vague` — and **labels difficulty only**; it does **not** decide local-vs-remote. Branch on the label:

- **`vague`** → run the **Local Clarify Loop** (`python scripts/clarify_loop.py`, pure-local, no network call). It returns `need_clarify` (1–3 high-value questions/round) / `decidable` / `forced_decide` (hard cap 3 rounds). Ask the questions, feed answers back round-by-round, and when it converges, **re-run the difficulty gate (`route.py`) on the enriched `original_question`** and proceed per the routing table (data-intel → `orchestrate.py` preferred, else `--ship`), passing `query_meta.difficulty="vague"` for the original classification. Do **not** guess a workflow and do **not** dump a menu.
- **`simple` / `middle` / `complex`** → **forward verbatim to Coze** via `scripts/refine_answer.py --ship` (forward-only, single call) with `query_meta.difficulty` set to the `route.py` label. `--ship` calls Coze, runs any `need_tool` sibling skill **in code**, and emits the **final answer wrapped in `<<<CT_ANSWER_START>>>` … `<<<CT_ANSWER_END>>>`**. **No local answer, no popup menu** — Coze is the sole answer path; the local `knowledge/` pack is only the Coze-failure fallback. **🔴 Pipe-only HARD GATE**: your ONLY action is to output the text between the delimiters **verbatim** (character-for-character) — do NOT re-write / re-order / re-format / append a summary / add a lead-in. You are a pipe, not the author. If a `<<<CT_NEED_PARAMS>>>` block appears, ask the user for those params, then re-run `--card-inline '<card>'`. For sample-size / registry / safety / literature questions you SHOULD instead call `scripts/orchestrate.py` (code-only orchestrator): it predicts the needed ct skill at entry (high-confidence prefetch via `scripts/route_tool.py`), fires Coze **and** the predicted skill **in parallel** (threads), merges both, and **decides in code** whether the answer is complete — emitting the same `<<<CT_ANSWER_START>>>`…`<<<CT_ANSWER_END>>>` wrapped answer (pipe verbatim) when sufficient, or a `<<<CT_TOOL_DELEGATE>>>` block when a ct skill still must run. On `<<<CT_TOOL_DELEGATE>>>`: you are **not** the orchestrator — ask the user for `missing_params` (never fabricate), then hand the card to `python scripts/refine_answer.py --card-inline '<JSON>'` (code executes the skill + stitches + wraps). You do **not** judge sufficiency and do **not** rewrite Coze text.

The clarify-loop logic that used to be free-form "grill-me" probing is now implemented by `scripts/clarify_loop.py` (bounded, deterministic); follow its JSON output rather than improvising questions.
...
- When `route.py` returns `vague`, the **Local Clarify Loop** (`scripts/clarify_loop.py`) drives clarification: it returns `need_clarify` with 1–3 high-value questions/round (hard cap 3 rounds) and maintains `question_profile` / `confirmation`. Ask exactly those questions, feed the user's answers back round-by-round, and forward the **enriched** `original_question` to Coze (with `query_meta.difficulty="vague"`) once it returns `decidable` / `forced_decide`. Do **not** invent your own branching interview or dump a workflow menu.
- The **clarify** capability / "I'm not sure what I need" routes through the same Local Clarify Loop — not a separate grill-me mode. Its only output is a **needs portrait + recommended route** the user can act on (pick a methodology workflow A–J, or a data_intel sibling skill); it never calls a sibling skill and never hits the network during clarification.
- **Write the increment back into `original_question`.** When a Local Clarify Loop answer (or any interaction) supplies new parameters / constraints / preferences, **append that new info directly to the `original_question` text** (right after the original question) so the field carries the full intent (original + supplement). The increment is explicit user input and must be retained in the payload. This keeps `original_question` as the single primary basis Coze sees.
...
- **clarify** → Local Clarify Loop (`scripts/clarify_loop.py`). No Skill-tool handoff, no network. Use when the user is undecided or says "help me sort this out" / "I'm not sure what I want"; runs the bounded clarification loop and returns a needs portrait + recommended route (methodology workflow A–J or data_intel skill).
...
## K. Clarify mode (Local Clarify Loop) — scope the need when the user is undecided

**Trigger**: `scripts/route.py` returns `vague`, OR the user picks the **clarify** capability (`menu.cap.clarify`) / says they are not sure what they need ("help me sort this out" / "I'm not sure what I want"). Also a fallback whenever a vague ask would otherwise send the user into the wrong workflow.

This mode is a **pure-local clarification loop** (`scripts/clarify_loop.py`, stdlib-only, no network call); it does **NOT** call any sibling ct skill and does **NOT** hit the network. Its only output is a **needs portrait + recommended route** that the user can then act on.

**Protocol** (bounded, deterministic — do NOT improvise a free-form interview):
1. Run `clarify_loop.py` with `original_question` + `previous_answers` + the accumulated `question_profile` / `confirmation`, round starting at 0 (hard cap `max_rounds=3`).
2. If status = `need_clarify`, ask exactly the 1–3 `questions` it returns (each round surfaces the highest-impact signals first; no data fetch, no skill handoff). Feed the user's answers back into `previous_answers` and re-run for the next round.
3. If status = `decidable` / `forced_decide`, stop — append the clarified info to `original_question` (see the increment rule above), **re-run the difficulty gate on the enriched question, then route per the table** (data-intel → `orchestrate.py` preferred, else `--ship`) with `query_meta.difficulty="vague"`.
...
5. Emit a **needs portrait** (decision / role / asset-stage / data-in-hand / output) and a **recommended route**: a methodology workflow (A–J) to answer in-house, OR a data_intel sibling skill (`ct-registry` / `ct-safety` / `ct-literature` / `ct-samplesize`; for a full competitive-intel brief, call the three data skills and stitch in-house ⭐) to fetch real data, OR "still undecided — propose 2 concrete next steps".
