...
This mode is a **pure-local clarification loop** (`scripts/clarify_loop.py`, zero-outbound, stdlib-only); it does **NOT** call any sibling ct skill and does **NOT** hit the network. Its only output is a **needs portrait + recommended route** that the user can then act on.
