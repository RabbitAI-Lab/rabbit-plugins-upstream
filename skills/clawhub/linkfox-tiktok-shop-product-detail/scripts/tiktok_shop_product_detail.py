#!/usr/bin/env python3
"""
TikTok Shop Product Detail Lookup - LinkFox Skill
调用 /tiktok/shop/product/detail 接口。

Usage:
  python tiktok_shop_product_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python tiktok_shop_product_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/tiktok/shop/product/detail"
SLUG = "linkfox-tiktok-shop-product-detail"
...
def main():
    ...
    if not argv:
        print(
            "Usage: tiktok_shop_product_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
