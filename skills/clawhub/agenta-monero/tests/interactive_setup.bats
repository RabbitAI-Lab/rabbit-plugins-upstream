...
@test "interactive_setup starts wallet-rpc, writes PID, outputs ready JSON" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | jq -e '.ready==true and (.rpc_user|type=="string" and length==12) and (.rpc_password|type=="string" and length==24) and .rpc_port==18088 and (.wallet_pid|type=="number") and (.warnings|type=="array")' >/dev/null
