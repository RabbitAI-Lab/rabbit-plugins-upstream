...
- **First-time setup:** run `./scripts/interactive_setup.sh` (interactive prompts) **or** tell the Hermes agent "set up Monero payments" (agent-driven; see First-Time Setup below). Both paths generate RPC credentials, write `.env`, start `monero-wallet-rpc`, and verify readiness.
- After setup, every script runs from the skill root. Invoke `./setup.sh` again after changing `.env`.

## First-Time Setup

When a user asks to set up Monero payments, follow this workflow.
