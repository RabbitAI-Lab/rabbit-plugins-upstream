...
PLAYLIST_FOCUS="spotify:playlist:37i9dQZF1DX8Uebhn9wzrS"   # Lo-Fi Beats
PLAYLIST_DINNER="spotify:playlist:37i9dQZF1DX4sWSpwq3LiO"  # Jazz Vibes
PLAYLIST_SLEEP="spotify:playlist:37i9dQZF1DWZd79rJ6a7lp"   # Sleep Ambient
...
# Focus scene: Deep work / concentration
# - Computer speaker only at 35%
# - Lo-fi / ambient playlist for sustained attention
scene_focus() {
    echo "🎧 Starting Focus scene..."
    
    # Set Airfoil source to Spotify
    airfoil_set_source_spotify
    
    # Connect Computer speaker at 35%
    airfoil_connect "Computer"
    sleep 0.5
    airfoil_volume "Computer" 0.35
    
    # Start playlist
    "$SPOTIFY_CMD" play "$PLAYLIST_FOCUS"
    "$SPOTIFY_CMD" volume 80
    
    echo "✅ Focus: Computer @ 35%, Lo-Fi Beats"
}

# Dinner scene: Cooking & dining ambiance
# - Sonos Move + Living Room TV at conversational volume
# - Jazz / bossa nova for warm background
scene_dinner() {
    echo "🍷 Starting Dinner scene..."
    
    # Set Airfoil source to Spotify
    airfoil_set_source_spotify
    
    # Connect Sonos Move at 35% and Living Room TV at 25%
    airfoil_connect "Sonos Move"
    sleep 0.3
    airfoil_volume "Sonos Move" 0.35
    airfoil_connect "Living Room TV"
    sleep 0.3
    airfoil_volume "Living Room TV" 0.25
    
    # Start playlist
    "$SPOTIFY_CMD" play "$PLAYLIST_DINNER"
    "$SPOTIFY_CMD" volume 90
    
    echo "✅ Dinner: Sonos Move @ 35%, Living Room TV @ 25%, Jazz Vibes"
}

# Sleep scene: Bedtime wind-down
# - Sonos Move only at 15%
# - Ambient / drone for falling asleep
scene_sleep() {
    echo "🌙 Starting Sleep scene..."
    
    # Set Airfoil source to Spotify
    airfoil_set_source_spotify
    
    # Connect Sonos Move at 15%
    airfoil_connect "Sonos Move"
    sleep 0.5
    airfoil_volume "Sonos Move" 0.15
    
    # Start playlist at lower Spotify volume
    "$SPOTIFY_CMD" play "$PLAYLIST_SLEEP"
    "$SPOTIFY_CMD" volume 50
    
    echo "✅ Sleep: Sonos Move @ 15%, Sleep Ambient"
}
...
Scenes:
  ...
  focus      Lo-Fi Beats on Computer (35% volume)
  dinner     Jazz Vibes on Sonos Move + Living Room TV (35%/25%)
  sleep      Sleep Ambient on Sonos Move (15% volume)
