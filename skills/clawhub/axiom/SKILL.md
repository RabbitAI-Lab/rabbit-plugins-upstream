...
- `get_purchase_status` — polls a browser purchase started by `make_purchase`. Pass `browserPurchaseSessionID`. Each call waits up to 30 seconds for the next parked or terminal status change, then returns it (or a keep-polling response if work is still in progress): public `progressStage` / `progressLabel`, approval links, clarification requests, terminal outcomes, receipt, and cart details. It does not stream incremental progress — call it again in a loop. End-to-end purchases often take around 10–15 minutes.
