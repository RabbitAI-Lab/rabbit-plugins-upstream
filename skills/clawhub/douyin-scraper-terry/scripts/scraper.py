...
class DouyinScraper:
    ...
    def _try_browser(self, url: str) -> bool:
        """Try to open a URL with Playwright. Returns True if browser was available."""
        ...
        try:
            ...
            print(f"⚠️  浏览器不可用，使用模拟数据: {e}", file=__import__("sys").stderr)
            return False

    def search(self, keyword: str, limit: int) -> list[VideoData]:
        self._try_browser(f"https://www.douyin.com/search/{keyword}")
        return self._mock_search(keyword, limit)

    def hot(self, category: str, limit: int) -> list[VideoData]:
        self._try_browser("https://www.douyin.com/hot")
