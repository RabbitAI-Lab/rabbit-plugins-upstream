...
`PYTHON_BIN` 是安装器创建的包内 Python 3.12/3.13；JSON 提取只使用标准库，不依赖单行 JSON、空白或键顺序。Runner 在本地计算规范化清单的 SHA-256。`inspect.json` 包含宿主模型生成 SQL 所需的逻辑表名、列名与类型，不包含数据样例。
...
错误处理：`license_unavailable`（授权服务不可用）可使用同一次 Runner 调用的持久化幂等键有界重试；`authorization_invalid`（授权无效）、`dataset_invalid`（数据集无效）、`unsafe_sql`（不安全的 SQL）、`unknown_table`（未知数据表）、`query_timeout`（查询超时）、`query_resource_limit`（查询资源超限）、`report_failed`（报告生成失败）应先修正本地状态或输入，不循环盲重试。`X-AI-Skills-Billing-Currency`、`X-AI-Skills-Billing-Charged`、`X-AI-Skills-Billing-Balance` 等计费响应头仅用于遥测，不是票据验证机制；Runner 验证授权响应中的签名执行票据。
