...
# 商品图片元素智能消除工具 | LinkPix

指哪擦哪并自动补全背景：底层是 `qhkit image` 自定义生图的定向消除重绘。
...
```bash
qhkit image generate '{"modelLabel":"智慧模型","uploadedImages":["./原图.jpg"],"prompt":"消除图片中的【右下角水印文字】"}'
```

官方提示词模板（与 LinkPix 工作台「AI消除」功能同款）：`消除图片中的【目标元素】`——要更稳可追加「自动补全该区域背景，其余内容不变」。

- 目标元素要带位置描述（左上角/右下角/背景中的……），消除得更准。
- 多个元素一次列全；消除后目检补全区域是否自然。
...
- **视频**去水印/去字幕走「AI视频去水印工具 | LinkPix」「AI视频字幕消除工具 | LinkPix」（qhkit video-edit，那是专用接口更稳）。
- 改文字（而非删除）走「电商商品图文字修改器 | LinkPix」。
