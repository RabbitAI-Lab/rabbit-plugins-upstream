#!/usr/bin/env python3
"""
Product Infringement & TRO Risk Detection - LinkFox Skill
调用 /maidalv/checkApiFlash 接口。

Usage:
  python maidalv_check_api_flash.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python maidalv_check_api_flash.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/maidalv/checkApiFlash"
SLUG = "linkfox-maidalv-product-tro-detection"
...
def main():
    ...
    if not argv:
        print(
            "Usage: maidalv_check_api_flash.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
