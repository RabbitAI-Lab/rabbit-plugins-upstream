...
| 命令 | 用途 | 说明 |
|------|------|------|
| `/conspect start` | 启动报表任务 | 进入 9 阶段全自动状态机（v3.0 去除确认环节） |
...
> v3.0 起：全流程托管 AI 自动生成，已移除确认环节。用户偏好需在初始需求中说明（如配色、图表类型、输出格式），开始阶段会自动识别并记录到接力棒。
...
v3.0 起，用户偏好需在**初始需求**中说明，开始阶段会自动识别并记录到接力棒，全程不再有确认环节：

| 偏好类型 | 示例 | 识别时机 |
|---------|------|---------|
| 配色风格 | "用蓝色调"、"我想要暖色系" | 开始阶段识别 → 设计阶段应用 |
| 图表偏好 | "对比多用柱状图"、"占比用饼图" | 开始阶段识别 → 设计阶段应用 |
| 分析焦点 | "重点关注[维度A]趋势"、"按[维度B]分析" | 开始阶段识别 → 分析阶段应用 |
| 输出格式 | "生成 PDF 给我"、"做一个离线 HTML" | 开始阶段识别 → 报告生成阶段应用 |
| 自定义品牌色 | "主色调用 #1890FF" | 开始阶段识别 → 设计阶段应用 |

> **v3.0 变更**：原"确认阶段"已删除。偏好识别失败时使用默认值并继续推进，不再阻断流程。
