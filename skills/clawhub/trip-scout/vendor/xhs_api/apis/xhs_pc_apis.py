...
def _human_delay():
    """分页请求之间插入人类节奏的随机延迟，降低被风控判定为"自动浏览"的概率。

    可用环境变量 XHS_HUMAN_DELAY="min,max"（秒）调节，设为 "0" 禁用。
    默认 1.5~3.5s：单页搜索（require_num≤20）不触发，仅在翻页/拉全量评论时生效。"""
    cfg = os.environ.get("XHS_HUMAN_DELAY", "1.5,3.5")
    try:
        parts = [float(p) for p in cfg.split(",")]
        lo = parts[0]
