...
# === 多维度评价分析关键词库 (references/review-analysis.md "多维度综合分析") ===
DIMENSION_KEYWORDS = {
    "安静/隔音": {
        "positive": ["安静", "隔音好", "不吵", "睡眠好", "静谧", "听不到", "很安静",
                      "隔音不错", "隔音可以", "安静舒适", "晚上很安静"],
        "negative": ["吵", "噪音", "隔音差", "隔音不好", "隔壁", "临街", "轰隆",
                      "很吵", "窗户不隔音", "马路上", "太吵", "吵闹", "隔音太差",
                      "听到", "不隔音", "嘈杂"],
    },
    "卫生": {
        "positive": ["干净", "整洁", "卫生好", "清洁", "一尘不染", "很干净",
                      "无异味", "干净整洁", "卫生干净", "打扫", "清新"],
        "negative": ["脏", "卫生差", "有虫", "蟑螂", "老鼠", "异味", "有异味",
                      "不干净", "有头发", "污渍", "霉味", "卫生不行", "不卫生",
                      "有灰", "床单", "马桶脏", "浴室脏"],
    },
    "设施": {
        "positive": ["设施好", "设施新", "设备全", "功能齐全", "智能化", "马桶",
                      "健身房", "泳池", "洗衣机", "设施完善", "设施齐全", "很新",
                      "装修新", "电视大", "投影"],
        "negative": ["设施旧", "设施差", "设施损坏", "热水没有", "空调坏了",
                      "电视坏了", "老化", "陈旧", "破旧", "水压低", "wifi慢",
                      "空调声音大", "没热水", "吹风机坏", "设施老化", "不好用"],
    },
    "早餐": {
        "positive": ["早餐好", "早餐丰盛", "早餐不错", "品种多", "好吃",
                      "早餐丰富", "早餐很棒", "自助餐", "含早", "早餐种类多",
                      "早餐满意", "营养"],
        "negative": ["早餐差", "早餐少", "品种少", "难吃", "早餐一般",
                      "早餐不行", "不好吃", "没早餐", "早餐太简单", "早餐贵",
                      "不值", "早餐种类少", "不好"],
    },
    "服务": {
        "positive": ["服务好", "态度好", "热情", "贴心", "周到", "服务态度好",
                      "前台好", "响应快", "帮忙", "服务周到", "很专业",
                      "服务热情", "满意", "用心"],
        "negative": ["态度差", "态度恶劣", "服务差", "不处理", "推诿", "冷淡",
                      "不热情", "响应慢", "服务一般", "爱答不理", "敷衍",
                      "前台态度", "服务不行", "不专业", "怠慢"],
    },
    "特色": {
        "positive": ["特色", "风格", "设计感", "独特", "文化", "主题",
                      "有设计", "有格调", "氛围好", "文艺", "精致", "别致",
                      "很有特色", "装修风格", "高级感"],
        "negative": ["没特色", "普通", "千篇一律", "老气", "过时", "俗气",
                      "没有亮点", "大众化", "没什么特别的"],
    },
    "位置": {
        "positive": ["位置好", "交通便利", "方便", "地铁近", "打车方便",
                      "位置佳", "地段好", "出行方便", "近景区", "步行可达",
                      "位置优越", "好找", "中心"],
        "negative": ["位置偏", "交通不便", "太远", "不方便", "难找",
                      "偏远", "不在市区", "打车难", "位置不好", "太偏",
                      "荒凉", "前不着村"],
    },
    "周边": {
        "positive": ["周边好", "吃饭方便", "购物方便", "商场近", "夜市",
                      "配套全", "周边丰富", "楼下就是", "步行街", "便利店",
                      "餐饮多", "热闹", "商业街"],
        "negative": ["周边少", "没吃的", "什么都没有", "不方便", "荒",
                      "没超市", "没餐厅", "周边空", "配套差", "什么都没有",
                      "叫不到外卖", "前不着村后不着店"],
    },
}


def _hit_keywords(content: str, keywords: list, negate_words: tuple = ("不", "没", "无", "没有", "毫无")) -> bool:
    """检测关键词命中, 处理中文否定前缀(不/没/无)。
    "不嘈杂"不命中"嘈杂"(负面被否定=正面); "不干净"不命中"干净"(正面被否定=负面)。"""
    for kw in keywords:
        idx = content.find(kw)
        while idx >= 0:
            # 检查关键词前1-2个字符是否有否定词
            prefix = content[max(0, idx - 2):idx]
            if not any(neg in prefix for neg in negate_words):
                return True
            idx = content.find(kw, idx + 1)
    return False


def score_dimensions(comments: List[Dict], months: int = 24) -> Dict[str, Any]:
    """
    对全量评论做多维度评分。

    与差评分析不同, 这里分析全部评论(好评+中评+差评),
    按8个维度统计正面/负面提及, 输出维度评分。
    ...
    for c in comments:
        ...
        try:
            ...
            pass  # 无日期的评论保留
        recent_comments.append(content)

    total_reviews = len(recent_comments)
    dimensions_result = {}
    ...
    for dim_name, keywords in DIMENSION_KEYWORDS.items():
        ...
        for content in recent_comments:
            hit_pos = _hit_keywords(content, keywords["positive"])
            hit_neg = _hit_keywords(content, keywords["negative"])

            if hit_pos and not hit_neg:
                positive_count += 1
                if not sample_positive:
                    sample_positive = content[:150]
            elif hit_neg and not hit_pos:
                negative_count += 1
                if not sample_negative:
                    sample_negative = content[:150]
        ...
        if total_mentions == 0:
            ...
            positive_rate = round(positive_count / total_mentions, 3)
            coverage = round(total_mentions / total_reviews, 3) if total_reviews else 0
            score = round(positive_rate * 10, 1)
            if score >= 8.0:
                verdict = "优秀"
            elif score >= 6.0:
                verdict = "良好"
        ...
        dimensions_result[dim_name] = {
            ...
            "coverage": coverage,
            "samplePositive": sample_positive,
            "sampleNegative": sample_negative,
        }

    # 维度排名
    scored = [(name, data["score"]) for name, data in dimensions_result.items()
              if data["score"] is not None]
    scored.sort(key=lambda x: x[1], reverse=True)

    ranking = {
        "strongest": scored[0][0] if scored else None,
        "weakest": scored[-1][0] if scored else None,
        "ranking": [name for name, _ in scored],
    }
...
def fetch_and_analyze_dimensions(
    hotel_id: str,
    months: int = 24,
    max_pages: int = 100,
    headless: bool = True,
    ...
    """
    多维度分析: 抓取全量评论(非仅差评) + 8维度评分 + 差评分析。

    与 fetch_and_analyze 的区别:
    - negative_only=False (抓全部评论)
    - months=24 (近2年, 非近1年)
    - max_pages=50 (最多1000条)
    - 输出含多维度评分

    Args:
        hotel_id: 携程酒店ID
        months: 分析近N个月(默认24=2年)
        max_pages: 最多翻页数(默认50, 约1000条)
        headless: 是否无头模式
        cookie_path: Cookie文件路径
        prefer_api: 优先尝试API模式
    ...
    try:
        ...
        # 抓取全部评论(API模式, negative_only=False)
        comments = []
        method = "browser"
        if prefer_api:
            ...
            if comments:
                method = "api"
                print(f"API模式成功, 获取{len(comments)}条评论(全量)", file=sys.stderr)

        if not comments:
            print("API模式无数据, 降级到浏览器模式...", file=sys.stderr)
            # 浏览器模式抓全部评论(非差评标签)
            comments = fetcher.fetch_reviews_browser(
                hotel_id, max_pages=max_pages, negative_only=False
            )
        ...
        # 多维度评分
        dim_result = score_dimensions(comments, months=months)

        # 差评分析(复用现有逻辑, negative_only=True)
        neg_result = analyze_reviews(comments, months=months, negative_only=True)
        ...
        return {
            ...
            "dimensionRanking": dim_result["dimensionRanking"],
            "totalReviewsAnalyzed": dim_result["totalReviews"],
            "negativeAnalysis": {
                "negativeReviewsInPeriod": neg_result.get("negativeReviewsInPeriod", 0),
                "totalReviewsInPeriod": neg_result.get("totalReviewsInPeriod", 0),
                "negativeRate": neg_result.get("negativeRate", 0),
                "categoryBreakdown": neg_result.get("categoryBreakdown", {}),
                "categoryPercent": neg_result.get("categoryPercent", {}),
                "last3MonthsCount": neg_result.get("last3MonthsCount", 0),
                "trendNote": neg_result.get("trendNote", ""),
                "踩雷风险": neg_result.get("踩雷风险", ""),
                "sampleReviews": neg_result.get("sampleReviews", []),
            },
        }
