...
用法:
    ...
    # 多维度分析（8维度评分 + 差评分析）
    python scripts/ctrip_reviews.py <hotelId> --dimensions  # 近2年全量评价多维度评分
...
from ctrip.reviews import fetch_and_analyze, fetch_and_analyze_dimensions, search_hotel_id
...
示例:
  ...
  python scripts/ctrip_reviews.py 17509632 --dimensions  # 多维度分析(8维度+差评)
  python scripts/ctrip_reviews.py logout            # 清除登录状态
    ...
    p.add_argument("--dimensions", action="store_true",
                   help="多维度分析: 抓取近2年全量评价, 输出8维度评分+差评分析")

    ns = p.parse_args(args)

    if ns.dimensions:
        # 多维度分析: 抓全量评价(近2年) + 8维度评分 + 差评分析
        result = fetch_and_analyze_dimensions(
            hotel_id=ns.hotelId,
            months=24,
            max_pages=100,
...
用法:
  ...
  python scripts/ctrip_reviews.py <hotelId> --dimensions  # 多维度分析(8维度+差评)
  python scripts/ctrip_reviews.py logout              # 清除登录状态
