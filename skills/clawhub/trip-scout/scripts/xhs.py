...
用法:
    ...
    python scripts/xhs.py qrcode                          # QR码登录（默认保存PNG并打开图片查看器）
...
def cmd_qrcode(args):
    """QR码登录（纯API，默认保存二维码PNG并打开图片查看器，最易扫码）"""
    from apis.xhs_pc_login_apis import XHSLoginApi

    login_api = XHSLoginApi()
    cookies_str = login_api.qrcode_login(
        show_in_terminal=args.terminal,
...
def cmd_search(args):
    ...
    if success:
        # success=True 但 0 结果（账号软限流返回空 data，或确实无匹配笔记）也走正常输出，
        # 不报 error；只有 success=False（API 错误/签名失效）才进 else 报错。
        results = []
        for item in res_json or []:
...
def build_parser():
    ...
    # qrcode
    qr = sub.add_parser("qrcode", help="QR码登录（默认保存二维码PNG并自动打开图片查看器）")
    qr.add_argument("--terminal", action="store_true", help="在终端用字符渲染二维码（默认为保存PNG图片；SSH/无图形界面时使用）")
    qr.add_argument("--no-open", action="store_true", help="保存PNG但不自动打开查看器（仅打印路径和二维码URL）")
