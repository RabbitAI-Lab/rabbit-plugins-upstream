...
description: >
  ...
  智能酒店筛选（品牌信任梯度、加盟/直营识别、差评分析、多维度评价分析、黑榜），自进化学习；
  机票价格监控（携程API抓取+价格历史+入手区间分析+降价预测+flyai交叉验证）；
  场景二（自驾游行程规划）小红书路线推荐→酒店联动调整→地图渲染（H5行程页+高德专属地图双轨）→飞书攻略生成；
  租车网点查询（神州租车API+一嗨租车页面解析，查城市取还车网点）。
  触发词：搜机票、搜酒店、找酒店、订机票、机酒搜索、监控机票、机票价格、入手时机、自驾游、行程规划、路线推荐、
  租车网点、租车门店、取车点、还车点、car rental stores、road trip、trip plan、travel search、flight search、hotel search。
---
...
| 监控机票、机票价格、入手时机、降价提醒、flight monitor | **场景一：机票监控**（见下方"机票价格监控"章节） |
| 自驾游、行程规划、路线推荐、road trip、trip plan、X天X路线 | **场景二：自驾游行程规划** |
| 租车网点、租车门店、取车点、还车点、异地还车网点 | **场景二：租车网点查询**（读 `references/car-rental-stores.md`） |
...
3. **黑名单过滤** — 直接移除已拉黑酒店
4. **品牌信任过滤** — 低信任品牌需额外验证
5. **差评分析 + 多维度分析** — 差评分类(成长性/本质性) + 8维度评分(安静/卫生/设施/早餐/服务/特色/位置/周边)（必须先调用 `python scripts/ctrip_reviews.py <hotelId> --dimensions`，详见 `references/review-analysis.md`）
6. **加盟/直营判断** — 加盟翻牌标注⚠️
...
用户需要持续监控某条航线票价、判断入手时机时触发。

```bash
# 单次抓取（存入历史 + 输出航班列表）
python scripts/flight_monitor.py --route "成都->伊宁" --date 2026-09-25

# 分析报告（抓取最新 + 历史分析 + flyai 交叉验证）
python scripts/flight_monitor.py --route "成都->伊宁" --date 2026-09-25 --analyze
```

**数据源**：携程 batchSearch API（主源）-> DOM 解析 -> 最低价 API（降级），flyai 仅 `--analyze` 时交叉验证。

**分析输出**：价格历史 + 趋势 + 余票状态 + 携程降价预测（如有）+ 入手区间 + 下单时机建议。

**历史数据**：`~/.trip-scout/flight-monitor/history.json`，保留60天。

详见 `references/flight-search.md` "机票价格监控"章节。
...
3. **异地还车决策** — 检查路线沿途是否有可用机场（高德POI搜索"机场"），并用 `python scripts/car_stores.py <城市>` 查取还车城市的神州/一嗨实际网点（机场店/高铁站店优先，决定取还车节点），评估异地还车 vs 环线回原点的成本和时间对比，输出取还车节点方案（读 `references/road-trip-planning.md` 异地还车策略 + `references/car-rental-stores.md`）
...
**租车提醒**（报价/下单不集成，用户自行在一嗨或神州等平台选择；网点可用 `python scripts/car_stores.py <城市>` 查询，读 `references/car-rental-stores.md`）：
...
| **租车网点查询** | 神州/一嗨城市取还车网点（内置 vendor/car_rental） | 无需安装，见 `references/car-rental-stores.md` |
...
# 可选: 小红书反风控节奏调节。翻页/拉全量评论时每页间随机延迟(秒)，默认 1.5,3.5；
#       设为 "0" 禁用。被风控警告"自动浏览"时可调大如 "3,6"
# export XHS_HUMAN_DELAY="1.5,3.5"
...
# 方式a: QR码登录(默认保存二维码PNG到 ~/.xiaohongshu/qrcode.png 并自动打开图片查看器；SSH无图形界面加 --terminal 用终端字符渲染)
...
- `references/review-analysis.md` — 差评分析引擎 + 多维度综合分析
...
- `references/car-rental-stores.md` — 租车网点查询（神州API+一嗨页面解析）
...
- `vendor/car_rental/` — 内置神州+一嗨租车网点查询客户端(神州C端公开JSON API + 一嗨SSR页面Playwright解析)
- `scripts/xhs.py` — 小红书口碑验证+路线搜索入口
- `scripts/ctrip_reviews.py` — 携程酒店差评抓取分析
- `scripts/car_stores.py` — 租车网点查询入口（双平台，支持 --source/--json）
