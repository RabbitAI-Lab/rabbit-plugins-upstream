...
class VDBClient {
    ...
    async listCollections() {
        const result = await this.request("/collection/list", {
    ...
    async queryDocuments(collection, options) {
        ...
        const result = await this.request("/document/query", {
            database: this.database,
            collection,
    ...
    async describeCollection(collection) {
        const result = await this.request("/collection/describe", {
            database: this.database,
            collection,
        });
...
async function exportCollection(client, collection, outputDir, options) {
    ...
    while (hasMore && totalExported < maxDocs) {
        ...
        try {
            ...
            if (options.expectedTotal !== undefined && !isRangeMode) {
                const pct = Math.min(100, Math.round((totalExported / options.expectedTotal) * 100));
                process.stdout.write(`\r     进度: ${totalExported}/${options.expectedTotal} (${pct}%)`);
            }
            else {
...
async function main() {
    ...
    try {
        ...
        console.error(`❌ 列出 collection 失败: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
    }
    ...
    if (args.collection) {
        ...
        if (!found) {
            console.error(`❌ Collection "${args.collection}" 不存在。可用的 collection：`);
        ...
        console.log(`🔍 找到 ${targetCollections.length} 个 collection：`);
    ...
    for (const col of targetCollections) {
        try {
            ...
            const emb = schema.embedding;
            const embInfo = emb ? `embedding=${emb.field}→${emb.model}` : "无 embedding";
            console.log(`   ✅ ${col.collection} (${indexCount} 个索引, ${embInfo})`);
        }
        catch (err) {
            console.error(`   ⚠️ ${col.collection} 表结构获取失败: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    console.log();
    ...
    for (const col of targetCollections) {
        try {
            ...
            console.error(`❌ 导出 ${col.collection} 失败: ${err instanceof Error ? err.message : String(err)}`);
            exportResults.push({
    ...
    for (const r of exportResults) {
        ...
        console.log(`     ${status} ${r.collection}: ${r.docCount} 条`);
    }
