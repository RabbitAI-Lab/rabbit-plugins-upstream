...
export function createL1Runner(opts: {
  ...
  return async ({ sessionKey }) => {
    ...
    try {
      ...
      if (groups.length === 0) {
        // SELF-CHECK (2026-06-02): If cursor > 0 but no groups returned,
        // the cursor may have drifted past the max recorded_at in the store.
        // This was a real bug: cursor was set to exact max(recorded_at),
        // and the > comparison excluded all rows at that timestamp.
        if (runnerState.last_l1_cursor > 0) {
          logger.warn?.(
            `${TAG} [l1] SELF-CHECK: cursor=${runnerState.last_l1_cursor} but 0 rows returned. ` +
            `Possible cursor drift. Checking store for total messages...`,
          );
          // Quick diagnostic: query without cursor to see if data exists
          try {
            const totalInStore = vectorStore && !vectorStore.isDegraded()
              ? vectorStore.countL0()
              : 0;
            if (totalInStore > 0) {
              logger.error?.(
                `${TAG} [l1] SELF-CHECK ALERT: Store has ${totalInStore} L0 messages but cursor ` +
                `${runnerState.last_l1_cursor} excludes all of them! Resetting cursor to 0.`,
              );
              // Auto-repair: reset cursor so next run will reprocess all messages
              await checkpoint.mergeRunnerStates({ [sessionKey]: { last_l1_cursor: 0 } });
            } else {
              logger.debug?.(`${TAG} [l1] SELF-CHECK: Store is empty, no reset needed`);
            }
          } catch (checkErr) {
            logger.error?.(`${TAG} [l1] SELF-CHECK failed: ${checkErr instanceof Error ? checkErr.message : String(checkErr)}`);
          }
        }
      ...
      // BUGFIX (2026-06-02): Add +1ms buffer to cursor.
      // The downstream query uses `recorded_at > cursorIso`. If cursor equals
      // max(recorded_at), the > comparison EXCLUDES all rows at that timestamp.
      // Since multiple messages in the same batch share the same recorded_at,
      // without +1ms they are permanently excluded from future L1 queries.
      const l1Cursor = maxRecordedAtMs > 0 ? maxRecordedAtMs + 1 : 0;
      await checkpoint.markL1ExtractionComplete(sessionKey, totalStored, l1Cursor || undefined, lastSceneName);
      logger.info(
