...
from account_actions import handle_apikey_error, is_apikey_error
...
SKILL_ALLOWED_ACCOUNT_TYPE = "real_stock"
...
CONFIRMATION_WINDOW_SECONDS = 5 * 60
CONFIRMATION_TOKEN_LENGTH = 12
CONFIRMATION_STATE_PATH = SKILL_ROOT / ".pending_confirmation.json"


class NoRealStockAccountError(RuntimeError):
    """共享凭证下不存在实盘证券子账户（subAccountType=0）。"""
...
def shared_env_path() -> Path:
    ...
    唯一真源与安装位置无关：由本文件路径推出 real-trade-skill → 父目录即总入口，
    不猜 Agent 工作区等魔法前缀路径。
    """
...
def _configure_sdk_debug_logger() -> None:
    ...
    if any(getattr(h, "_real_debug", False) for h in logger.handlers):
        return
    handler = logging.StreamHandler(sys.stderr)
    handler.setFormatter(JsonFormatter())
    handler._real_debug = True  # type: ignore[attr-defined]
...
def _raise_no_real_stock_account(index: dict) -> None:
    """共享凭证下不存在 subAccountType=0 的实盘证券子账户时调用。"""
    visible = []
    for label in SUPPORTED_ACCOUNT_TYPES:
        for item in index.get(label) or []:
    ...
    msg_lines = [
        "你选择了【实盘】交易，但当前共享凭证下未发现实盘证券子账户（subAccountType=0）。",
        "本次操作已停止，等待用户决策；模型不得擅自切换到模拟盘 skill 或自动改用其他账户继续操作。",
    ]
    if visible:
    ...
    msg_lines.append(
        ...
        "  1) 是否确认当前共享凭证下已开通实盘证券账户后重试；\n"
        "  2) 是否要显式 `--sub-account-id <ID>` 用某个已可见账户做调试（必须由用户指定具体 ID）。"
    )
    raise NoRealStockAccountError("\n".join(msg_lines))
...
def _ensure_skill_account_type(account_type: str) -> None:
    if account_type == SKILL_ALLOWED_ACCOUNT_TYPE:
        return
    raise StructuredScriptError(
        f"实盘脚本禁止解析 account_type={account_type}。",
        error_code="CROSS_TRADING_GUARD",
        hint=(
            "当前脚本是实盘隔离域，只允许 real_stock 子账户；"
            "任何 mock / real_option 路径都会被本地硬拦截。"
        ),
        next_action=(
            "立即停手并向用户确认盘别；若用户明确要模拟盘，请切换到 moni-trade-skill，"
            "不要在实盘脚本里重试。"
        ),
        exit_code=2,
    )
    ...
    *,
    ...
    if actual_type is None:
        raise StructuredScriptError(
            ...
            error_code="CROSS_TRADING_GUARD",
            hint=(
                "无法确认该账户 ID 的盘别归属。为防串盘，实盘脚本不会盲信外部传入的账户 ID。"
            ),
            next_action=(
                "先运行 account_list.py 确认可见 real_stock 子账户，再由用户明确指定实盘账户 ID 重试；"
                "禁止自动切模拟盘或改用其他账户。"
            ),
            exit_code=2,
        )
    if actual_type != expected_account_type:
        raise StructuredScriptError(
            f"{source} 指定的 subAccountId={target} 属于 {actual_type}，与实盘要求 {expected_account_type} 不一致。",
            error_code="CROSS_TRADING_GUARD",
            hint=(
                "检测到跨盘账户注入风险：实盘脚本收到非 real_stock 子账户。"
                "为避免串盘，本次操作已被硬拦截。"
            ),
            next_action=(
                "立即停手并向用户确认盘别；若用户明确要实盘，请改用 real_stock 子账户 ID 后重试。"
            ),
            exit_code=2,
        )
    return target
...
def resolve_sub_account_id(
    ...
    - 仍未命中且 account_type=real_stock → 抛 NoRealStockAccountError，明确告知用户"无实盘证券账号"
    """
    ...
    env_value = _coerce_sub_account_id(os.environ.get("REAL_SUB_ACCOUNT_ID"))
    if env_value:
        return _validate_explicit_sub_account_id(
            client,
            env_value,
            source="环境变量 REAL_SUB_ACCOUNT_ID",
            expected_account_type=account_type,
        )
    ...
    if account_type == "real_stock":
        _raise_no_real_stock_account(refreshed)
...
def ensure_sim_trade_market(market_code: Optional[str]) -> None:
    if market_code and market_code not in SIM_TRADE_MARKETS:
        raise ValueError("当前实盘入口仅支持港股/美股交易，market-code 仅允许 hk 或 us")
...
def ensure_sim_trade_product_type(product_type: Optional[int]) -> None:
    if product_type is not None and product_type not in SIM_TRADE_PRODUCT_TYPES:
        raise ValueError("当前实盘入口仅支持港股/美股交易，product-type 仅允许 5=港股 或 6=美股")
...
def ensure_sim_trade_tuple_aligned(
    ...
    if not expected:
        raise ValueError("当前实盘入口 market-code 仅允许 hk 或 us")
    mismatches = []
...
def ensure_sim_trade_show_type(show_type: Optional[int]) -> None:
    if show_type is not None and show_type != 0:
        raise ValueError("当前实盘入口暂不支持期权订单，show-type 仅允许 0=只有正股订单")


def ensure_sim_sub_account_class(sub_account_class: Optional[int]) -> None:
    if sub_account_class == 9:
        raise ValueError("当前实盘入口暂不支持期权账户，sub-account-class 不能传 9")
...
def _confirmation_window_minutes() -> int:
    return max(1, CONFIRMATION_WINDOW_SECONDS // 60)
...
def _issue_confirmation_challenge(
    *,
    ...
    token = hashlib.sha256(seed.encode("utf-8")).hexdigest()[:CONFIRMATION_TOKEN_LENGTH].upper()
    challenge = {
        "action": action,
    ...
    try:
        os.chmod(CONFIRMATION_STATE_PATH, 0o600)
    except OSError:
        # 权限收紧失败时不放宽校验路径，后续仍走 token+fingerprint 强校验。
        pass
    return challenge
    ...
    if not confirmed:
        ...
        raise StructuredScriptError(
            ...
            next_action=(
                ...
                f"令牌 {_confirmation_window_minutes()} 分钟内有效，且只对当前意图生效。"
            ),
            exit_code=2,
        )
    ...
    输出契约：
        {
          ...
          ...directives,          # 决策辅助字段（如 real_stock_account_available, accounts_summary）
          # 末尾追加 data：
...
# 抓不到具体码时，至少给到分类级别的指引。
_GENERIC_PLAYBOOK = {
    "NoRealStockAccountError": {
        "error_code": "NO_REAL_STOCK_ACCOUNT",
        "exit_code": 3,
        "hint": "共享凭证下没有 subAccountType=0 的实盘证券子账户，本 skill 已直接拦截。",
        "next_action": "停手并把账户缺失情况完整转告用户，等待用户确认实盘证券账户是否已开通，或由用户显式指定 --sub-account-id 调试。禁止私自切模拟盘 skill 或挑选其他账户继续。",
    },
    "FileNotFoundError": {
        "error_code": "ENV_FILE_MISSING",
        "exit_code": 2,
        "hint": "FOSUN_ENV_PATH 指向的共享凭证文件不存在。",
        "next_action": "先通过同级 fosun-env-setup 生成共享凭证后再来。",
    },
    "EnvironmentError": {
        "error_code": "ENV_INCOMPLETE",
        "exit_code": 2,
        "hint": "共享凭证存在但关键字段缺失（FSOPENAPI_BASE_URL / FSOPENAPI_API_KEY）。",
        "next_action": "先通过同级 fosun-env-setup 修复或刷新共享凭证。",
    },
    "ValueError": {
        "error_code": "INVALID_PARAM",
        "exit_code": 2,
        "hint": "本地参数校验未通过（市场三件套错配 / 当前实盘入口能力边界外的入参等）。",
...
def run(action) -> None:
    ...
    退出码：
        ...
        3 = NO_REAL_STOCK_ACCOUNT（强拦截，必须停手转告用户）
    """
    ...
    try:
    ...
    except NoRealStockAccountError as exc:
        playbook = _GENERIC_PLAYBOOK["NoRealStockAccountError"]
        _emit_error(
            _format_error_payload(
        ...
        if is_apikey_error(code):
            payload, exit_code = handle_apikey_error(
                code, api_message=message or ""
            )
            _emit_error(payload, exit_code)
