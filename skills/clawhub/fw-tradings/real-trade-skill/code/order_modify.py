...
要点：
  - 当前实盘入口只开放限价单/市价单，因此改单只支持普通订单可改字段（数量、价格）
...
def validate_order_modify_args(args):
    if args.modify_type != 1:
        raise StructuredScriptError(
            f"当前实盘入口不支持 modifyType={args.modify_type}。",
            error_code="INVALID_PARAM",
            hint="当前实盘入口订单类型只支持限价单(3)和市价单(9)，不支持条件单改单。",
            next_action="若要修改实盘订单，只能按普通订单 modifyType=1 修改数量或价格；否则告知用户当前不支持。",
            exit_code=2,
        )
    ...
    if unsupported_fields:
        ...
        raise StructuredScriptError(
            f"当前实盘入口普通改单不支持这些条件类字段：{fields}",
            error_code="INVALID_PARAM",
            hint="当前实盘入口只支持限价单/市价单，不支持条件单、跟踪止损、止盈止损相关改单字段。",
