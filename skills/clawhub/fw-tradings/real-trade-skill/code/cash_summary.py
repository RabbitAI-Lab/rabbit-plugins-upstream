...
注意：
  - subAccountId 默认从共享凭证缓存自动取（real_stock 桶第一个），不需要先跑 account_list
  - 港股购买力只看 HKD，美股只看 USD，脚本不自动换汇
"""


def main():
    parser = argparse.ArgumentParser(
        description="查询账户资金金额（实盘）",
        epilog=EPILOG,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--sub-account-id", help="证券账号；默认自动取共享凭证缓存里 real_stock 桶第一个")
    ...
    dump_with_directive(
        ...
        next_action=(
            "已返回实盘资金/购买力，**先把关键数字汇报给用户、然后停手**等下一步指令。"
