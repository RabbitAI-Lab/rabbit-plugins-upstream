...
订单类型：
  - 当前实盘入口只开放 3=限价单、9=市价单
  - 美股盘前/盘后（timeInForce=2）和夜盘（timeInForce=4）都不支持市价单，只支持限价单。非交易时段提交后如服务端转为预埋单，也只允许预埋限价单/预埋市价单
  - 条件单、跟踪止损、止盈止损、竞价/增强/特殊限价、暗盘订单暂不开放

强制规则（本地拦截，零网络代价）：
  - 必须传 `--market hk|us`，或同时显式三件套（不允许混用）
  - 港股最小手数 100，下不到 100 不要试，先和用户确认
  - 当前实盘入口只支持港股 / 美股正股；A 股、期权能力后续补齐
...
def _raise_missing_for_order_type(args, missing):
    ...
    raise StructuredScriptError(
        ...
        hint=(
            "实盘限价单必须提供委托价；脚本在本地拦截，避免把不完整下单请求发到网关。"
        ),
        next_action=(
...
def validate_order_create_args(args):
    """校验当前实盘入口订单类型与字段：仅开放限价单/市价单。"""
    if args.order_type not in ORDER_TYPE_LABELS:
        allowed = ", ".join(f"{key}={value}" for key, value in ORDER_TYPE_LABELS.items())
        raise StructuredScriptError(
            f"当前实盘入口不支持 orderType={args.order_type}。",
            error_code="INVALID_PARAM",
            hint=(
                f"当前实盘入口仅开放 {allowed}。非交易时段如果形成预埋单，也只允许预埋限价单/预埋市价单；"
                "条件单、跟踪止损、止盈止损、竞价/增强/特殊限价、暗盘订单暂不开放。"
            ),
            next_action="向用户说明当前实盘入口订单类型边界，并让用户改为限价单(3)或市价单(9)后重新确认。",
            exit_code=2,
        )
    ...
    if unsupported_fields:
        ...
        raise StructuredScriptError(
            f"当前实盘入口限价/市价单不支持这些条件类字段：{fields}",
            error_code="INVALID_PARAM",
            hint=(
                "当前实盘入口只开放限价单(3)和市价单(9)。预埋单不是另一套高级订单类型，"
                "非交易时段也不能携带条件触发、跟踪止损或止盈止损字段。"
            ),
            next_action="移除这些字段；若用户想要条件/止损/止盈止损能力，明确告知当前实盘入口暂不支持。",
            exit_code=2,
        )
    ...
    if args.order_type == 9 and args.price:
        raise StructuredScriptError(
            ...
            error_code="INVALID_PARAM",
            hint="实盘市价单由 orderType=9 表达，不需要委托价；带 price 容易造成用户意图和请求体不一致。",
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "当前实盘入口订单类型：3=限价单，9=市价单。非交易时段预埋单也只支持这两类；"
            "市价是 9，不是 4。"
        ),
    )
