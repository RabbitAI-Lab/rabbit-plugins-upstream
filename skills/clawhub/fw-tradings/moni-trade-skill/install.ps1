...
$VenvDir = if ($env:MONI_VENV) { $env:MONI_VENV } else { Join-Path $HOME ".openclaw\workspace\.venv-moni" }
$PinPythonVersion = if ($env:MONI_PYTHON_VERSION) { $env:MONI_PYTHON_VERSION } else { "3.13" }
$SdkVersion = if ($env:MONI_SDK_VERSION) { $env:MONI_SDK_VERSION } else { "v1.2.0" }
$SdkZipUrlPrimary = if ($env:MONI_SDK_URL) { $env:MONI_SDK_URL } else { "https://hk.gh-proxy.org/https://github.com/fosunwealth/openapi-python-sdk/archive/refs/tags/v1.2.0.zip" }
...
$SdkCacheRoot = if ($env:MONI_CACHE_DIR) { $env:MONI_CACHE_DIR } else { $DefaultCacheRoot }
$SdkCacheDir = Join-Path $SdkCacheRoot "openapi-python-sdk-$SdkVersion"

function Log([string]$msg) { Write-Host "[moni-trade] $msg" -ForegroundColor Cyan }
function Fail([string]$msg) { Write-Host "[moni-trade ERROR] $msg" -ForegroundColor Red; exit 1 }
...
function Resolve-SdkFallbackUrl {
    if ($env:MONI_SDK_FALLBACK_URL) {
        return $env:MONI_SDK_FALLBACK_URL
    }
...
$TmpBuildDir = Join-Path ([System.IO.Path]::GetTempPath()) ("moni-trade-build-" + [guid]::NewGuid().ToString("N"))
