...
输出（status=valid 表示凭证可用 + 账户索引已刷新；status=invalid 写入 stderr）：
  {
    ...
    "mock_account_available": true|false, // 模拟盘是否可直接用
    "env_path": "/path/to/fosun.env",
    "counts": {"mock": 1, "real_stock": 1, "real_option": 0}
  }

读懂结果（重要）：
  - mock_account_available=false → 共享凭证下没有模拟盘账户，模拟盘脚本会拒绝执行
  - status=invalid → 先通过同级 fosun-env-setup 修复或重新生成共享凭证
"""

NEXT_ACTION_VALID_HAS_MOCK = (
    "凭证有效且模拟盘账户可用。把体检结果汇报给用户，等用户明确具体业务请求后再调对应脚本；"
    "禁止自动连环跑业务脚本。"
)
NEXT_ACTION_VALID_NO_MOCK = (
    "凭证有效，但共享凭证下没有模拟盘账户（subAccountType=2）。把这一情况转告用户并停手，"
    "等用户决定是否前往券商开通模拟盘账户后再继续；禁止私自切实盘 skill。"
)
NEXT_ACTION_INVALID = (
    "共享凭证不可用。请先通过同级 fosun-env-setup 生成或修复有效的 fosun.env；"
    "禁止在凭证未恢复前调任何模拟盘业务脚本。"
)
...
def main():
    ...
    try:
        ...
        mock_available = counts["mock"] > 0
        payload = {
            "status": "valid",
            "next_action": NEXT_ACTION_VALID_HAS_MOCK if mock_available else NEXT_ACTION_VALID_NO_MOCK,
        ...
        payload = {
            ...
            "action": "请先通过同级 fosun-env-setup 确保 FOSUN_ENV_PATH 指向的共享凭证文件已生成且有效",
        }
