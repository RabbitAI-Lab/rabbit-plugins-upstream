---
name: linkedin-person-search-zh
description: 依托 LinkedIn 数据库，结合岗位职位、所属企业、所在地区筛选目标人员，帮助销售团队、招聘专员及商务拓展人员精准挖掘企业决策者和潜在商务联系人。
...
使用跨境魔方开放平台API从领英数据搜索人物。

## 概述

本技能通过一个列表搜索脚本提供对跨境魔方领英人物数据的访问。通过`auth.py`脚本提供 API密钥生成、充值；
...
**重要**：始终使用直接脚本调用，如 `python scripts/linkedin_person_search.py`。**不要使用** shell 复合命令如 `cd scripts && python linkedin_person_search.py`

### 领英人物列表搜索 (`linkedin_person_search.py`)
- **返回粒度**：每个人物为一行记录
- **适用场景**：关心的是领英数据中"有哪些人"
- **示例**：
  - "找XXXX的CTO"
  - "找中国区的销售总监"
- **参数**：查看参数说明 [领英人物列表](references/linkedin-person-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/linkedin/person/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"领英人物搜索查询失败，服务端异常"}'
```
...
| 用户意图 | 使用API |
|-------------|---------|
| "找XX公司的CTO" | 领英人物列表（companyNames + titleNames） |
| "找美国有邮箱的人" | 领英人物列表 existEmail=1 + countryCodes=["US"] |
| "找某个行业的人" | 领英人物列表 humanIndustries |
...
- [领英人物列表 API](references/linkedin-person-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/linkedin-person-list-api.md](references/linkedin-person-list-api.md)
   - 不要猜测参数名称，从文档中获取准确的参数名称和格式

2. **优化查询参数**：
   - 使用 `humanIndustries` / `companyNames` 精确筛选，行业名称需使用**英文**
