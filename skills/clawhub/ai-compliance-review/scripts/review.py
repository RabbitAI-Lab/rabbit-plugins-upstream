...
API_BASE = os.environ.get("COMPLIANCE_API_URL", "http://47.109.39.255:8765/api/v4")
USER_KEY = os.environ.get("COMPLIANCE_API_USER_KEY", "")
...
def cmd_register(args):
    ...
    print()
    print(f"  ⚠️  请设置环境变量：")
    print(f"  export COMPLIANCE_API_USER_KEY={result.get('user_key')}")
    print("=" * 50)
    print()
...
def cmd_info(args):
    if not USER_KEY:
        print("❌ 请先设置 COMPLIANCE_API_USER_KEY 环境变量", file=sys.stderr)
        sys.exit(1)
...
def cmd_review(args):
    if not USER_KEY:
        ...
        sys.exit(1)
    print(f"🚀 AI合规审查中...", file=sys.stderr)
    data = {}
    data['text'] = args.text
    if args.scene: data['scene'] = args.scene
    result = api_call("compliance-review", data)
...
def main():
    parser = argparse.ArgumentParser(description="AI合规审查 - SaaS版，3点/次")
    ...
    g = sub.add_parser("review", help="AI合规审查")
    g.add_argument("--text", required=True, help="待审查文本")
    g.add_argument("--scene", default="all", help="审查场景: all/advertising/privacy/finance/labor/ecommerce")
