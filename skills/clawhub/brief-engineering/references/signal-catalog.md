...
| `jsm_..._operations_query_splunk_log_search` | `MCP_TOOL_CONFIGURATION_INVALID_INPUT` after the real required `service` field was supplied twice | log signal | JSM Ops Splunk-originated alerts |
| `support_insights_search_splunk` | exit 1 while its own healthcheck reports UP | log fallback | none |
| `ares_..._post_apollo_splunk_job_submit` | `MCP_SERVER_REGISTRATION_PERMISSION_DENIED` once the correct `spl_query` parameter is supplied | async log fallback | access request |
...
**Read the error text, not just the code.** The Ares Splunk tool reported
`Missing required action input: 'spl_query'` — its parameter is `spl_query`, not
`query`. Supplying it changed the failure from a fake input error into an honest
permission denial. Two other tools hid real problems behind the same generic message.

The two `PERMISSION_DENIED` cases share one root: the Ares MCP server is not registered
for this principal. One access grant would unblock both the async Splunk route and
...
## SignalFx — alerts work, direct values need a registered tool
...
## Splunk — alert events work, direct logs need a registered tool
...
The checked Rovo connector catalogue does not list a direct Splunk search connector.
Treat direct logs and DLQ counts as `not checked` until an approved remote Splunk MCP or
Integrations Service tool is registered for the RovoClaw principal. The skill cannot
grant that access.
