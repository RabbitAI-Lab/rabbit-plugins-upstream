...
`query`; the checked SignalFlow tool requires `programText`, not `program`. If a
registered tool still rejects a complete schema-valid request, report `failed` with the
actual error. If no registered tool exists, report `not checked`. Do not invoke a local
process and do not keep retrying a denied or repeatedly failing route.
...
- **An unavailable connector means "not checked", never "healthy".**
...
Validate before and after. Only design-kit classes are permitted.
...
signalfx-alerts   checked      current SignalFx-originated JSM Ops alerts
signalfx-values   not checked  no registered direct metric tool
...
direct-splunk     not checked  no registered direct log-search tool
...
- **Direct Splunk logs and SignalFx values**: the skill cannot create access. They need
  an approved remote MCP/Integrations Service registration granted to the RovoClaw
  principal. Until then, use alert-derived coverage and label direct telemetry as
  `not checked`.
