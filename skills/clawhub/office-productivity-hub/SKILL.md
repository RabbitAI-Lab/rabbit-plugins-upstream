...
license: MIT
description: |-
  常见办公流程自动化中枢，覆盖文档处理、数据整理、邮件管理、日程协调、报表生成与工作流编排。
  聚焦批量操作容错、格式保真、PII 脱敏、模板复用四大痛点，让繁琐办公一键稳妥完成。

  核心能力:
  - 文档批处理：Word/PDF/Excel/PPT 批量生成、转换、合并拆分，含失败隔离与重试
  - 数据整理：清洗、转换、合并、统计，支持 CSV/JSON/XML/SQL 互转
  - 邮件管理：批量发送、模板变量注入、附件大小治理、定时发送
  - 日程协调：冲突检测、跨时区对齐、会议优先级排序
  - 报表生成：数据采集、图表渲染、定时产出、自动分发
  - PI脱敏：身份证/手机号/银行卡自动脱敏，敏感字段排除导出

  适用场景:
  - HR/财务/销售/行政/运营部门的批量文档与数据任务
  - 合同/发票/报表的批量生成与转换
  - 定时邮件与报表的自动分发
  - 涉及敏感信息的合规化数据处理

  差异化:
  - 失败隔离 + 续跑机制，原版"一个失败全批崩"
  - 格式保真检查表，原版未提格式损失风险
  - PII 脱敏规则表，原版仅提"敏感信息脱敏"一句话
  - 模板变量注入规范，原版只给示例无规范

  触发关键词: 办公, 自动化, 批量, 文档, excel, word, pdf, 邮件, 报表, 模板, 脱敏
tags:
- 自动化
- 办公效率
- 文档处理
