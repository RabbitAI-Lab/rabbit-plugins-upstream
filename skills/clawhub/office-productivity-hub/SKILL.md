...
version: 1.0.1
displayName: 办公效率中枢
summary: 解决批量炸裂、格式丢失、PII泄露、模板难复用痛点，让文档/数据/邮件/报表快速稳妥产出。常见办公流程自动化中枢，覆盖文档处理、数据整理、邮件管理、日程协调、报表生成与工作流编排。聚焦批量操
license: MIT
description: 常见办公流程自发化中枢，覆盖文档处置、数据整理、邮件管控、日程协调、报表产出与工作流编排。聚焦成批操作容错、格式保真、PII 脱敏、模板复用四大痛点，让繁琐办公快速稳妥完成。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。 功能涵盖: office, productivity, hub。
  当需要officeductivity hub相关能力的开发场景,提供规范流程和配置说明. 该工具经过差异化增强,结合实际使用痛点进行了优化.
tags:
- 自动化
- 办公效率
- 文档处理
...
**Q5: 模板变量找不到对应数据怎么办？**
A: 默认失败隔离，该文件记入 `failed.log`。可在模板中用 `<动态配置>默认值` 提供默认值.
**Q6: 大批量操作要多久？**
...
| 模板变量未替换 | 检查变量名大小写 → 检查数据源字段名 → 用 `<动态配置>` 兜底 |
...
**能力覆盖范围**：本技能覆盖以下场景：解决批量炸裂、格式丢失、PII、模板难复用痛点、让文档、报表快速稳妥产出、常见办公流程自动、化中枢、覆盖文档处理、报表生成与工作流、聚焦批量操作容错、模板复用四大痛点、让繁琐办公快速稳、妥完成、Use、when、需要提升效率、自动化流程、批量处理、工作流优化时使用、不适用于需要人工、创意判断的任务、适用于独立开发者、企业团队和自动化、工作流场景等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 实操说明
1. 确认运行环境满足依赖说明中的要求
...
4. 如遇错误，参考错误处理章节
## 能力说明
- **自动化执行**: 解决批量炸裂、格式丢失、PII泄露、模板难复用痛点，让文档/数据/邮件/报表快速稳妥产出。常见办公流程自动化中枢，覆盖文
...
## 特色分析
| 对比维度 | 办公效率中枢 | 传统手动方式 | 通用脚本工具 |
|---------|------------|-------------|------------|
| 自动化程度 | 全流程自动 | 完全手动 | 部分自动 |
| 错误处理 | 内置错误恢复 | 依赖人工经验 | 基本try-catch |
| 可复用性 | 参数化配置 | 一次性脚本 | 模板化 |
| 安全合规 | 内置安全检查 | 无安全保障 | 无安全保障 |
| 适用场景 | 解决批量炸裂、格式丢失、PII泄露、模板难复用痛点，让文档/数据/邮件/报表快速 | 通用场景 | 通用场景 |
## 核心属性
- **文件处理**: 支持多种文件格式的读取、解析和写入操作
- **API集成**: 通过标准化接口调用外部服务并处理响应
- **命令执行**: 在安全沙箱中执行系统命令并收集结果
...
## 疑问解答精选
### Q1: 办公效率中枢支持哪些输入格式？
A1: 解决批量炸裂、格式丢失、PII泄露、模板难复用痛点，让文档/数据/邮件/报表快速稳妥产出。常见办公流程自动化中枢，覆盖文档处理、数据整理、邮件管理、日程协调、报。支持文本指令和结构化参数输入，具体格式参考使用流程章节。
### Q2: 需要配置API Key吗？
