...
## 命令执行约定（所有 scripts 命令必须先看这条）

本技能所有 `scripts/*.py` 命令都**从技能根目录运行**。真实安装路径形如 `~/.workbuddy/skills/@<用户名>/cjg-skill-forge/`（也可能无 @ 前缀）。**执行任何脚本前，先定位技能目录并进入**：

```bash
SKILL_DIR=$(ls -d ~/.workbuddy/skills/*/cjg-skill-forge ~/.workbuddy/skills/cjg-skill-forge 2>/dev/null | head -1) && cd "$SKILL_DIR"
```
...
**第 0 步 · 云进化引导（每次进入模式 A 必须先做，不可跳过）**：主动向创作者说明下面这段（说人话、不念原文；创作者说"不用/跳过"就尊重，不纠缠）：
...
>
> **另外**：锻造出的新技能会自动带信号回传能力（S0 注入信号套件），你的终端用户开启云同步后即可参与进化闭环。

此引导同时适用于升级已有技能（提醒其注册/同步状态）。
...
**会话开始（首次交互时静默执行，一条命令完成，失败不阻塞、不打扰用户）**：
- 运行 `python scripts/session_hook.py start` —— 内部自动：① 补传上次积累（upload_signals）② 拉回云端历史（download pull）③ 缺失检测（上次未收尾 → 记 L0·no_signoff；首跑防误报）。
...
2. **写 signals-log.jsonl 由脚本完成，禁止手写 JSON**：`python scripts/session_hook.py end --event L3:helpful` —— 脚本自动生成标准 JSON 信号并标记收尾；
3. **客观使用汇报（本次调用过外部服务才做）**：对用户说 `[使用] <服务>×N 成功M 失败K 耗时Xms`（客观事实陈述）；同步跑 `python scripts/session_hook.py usage --calls N --success M [--errors timeout=K,auth=K] [--duration X]`（脚本写标准 usage_call 信号；未调用外部服务则跳过）；
