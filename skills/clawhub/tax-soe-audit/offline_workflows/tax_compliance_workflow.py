#!/usr/bin/env python3
"""
Tax Compliance 7-Step Closed Loop Tool (SOE Audit edition)
Version: v1.0.0 | Updated: 2026-07-25
...
SCENARIO_GUARD = {
    ...
    "国企经责审计涉税": ["经济责任审计", "国企经责审计", "国有资本收益", "财政专项资金", "审计整改", "国资监管", "境外国有资产"],
...
def print_header():
    print("=" * 60)
    print("Tax Compliance 7-Step Closed Loop Tool v1.0.0 (SOE Audit)")
    print("=" * 60)
    print()
...
def quick_risk_check(scenario: str) -> Dict:
    ...
    if matched_scenarios:
        return {
            "level": "SCENARIO",
            "message": f"关联结构化情形: {', '.join(matched_scenarios)}（关键词引擎未命中，但可能涉及专项结构化风险，含资本交易 / 国企经责审计 / 促销返现 / 全生命周期审查等）",
