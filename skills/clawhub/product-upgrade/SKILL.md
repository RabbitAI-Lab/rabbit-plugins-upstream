---
name: amazon-product-upgrade-planner
description: >
  结合 Amazon 商品详情与评论证据，按影响程度规划产品升级方向和验证问题。仅用于产品升级规划；不用于销量预测、采购下单或自动改版执行。
  Requires an ARI API key (ari_live_*).
slug: product-upgrade
displayName: Amazon 产品升级规划
version: 1.4.7
summary: 按评论证据规划产品升级方向
license: MIT
---
...
- 本入口只执行 `product/upgrade`，输出模板固定为 `ops_product`。
