...
`maton.one_drive` mirrors the `maton one-drive` commands, and `maton.api` reaches any endpoint. `login()` opens a browser once per machine and writes the session to the SDK's own store — `maton login` does not carry over, and the SDK never signs in implicitly.
...
```bash
curl --config - "https://api.maton.ai/one-drive/v1.0/me/drive" <<EOF
header = "Authorization: Bearer $MATON_API_KEY"
header = "User-Agent: maton-one-drive-skill/1.1"
