...
var require_jgy = __commonJS({
  "scripts/jgy.js"(exports2, module2) {
    "use strict";
    var { AuthError } = require_jgy_auth();
    var { createApiClient } = require_jgy_api();
    ...
    async function main() {
      ...
      switch (command) {
        case "get_lottery": {
          ...
          const result = await getLottery(slug);
          return emit(result);
        }
        case "list_my_prizes": {
          const result = await listMyPrizes();
          return emit(result);
        }
        case "call": {
        ...
        default:
          return emit(err("invalid_command", "\u672A\u77E5\u547D\u4EE4\u3002\u53EF\u7528\uFF1Acall / get_lottery / list_my_prizes"));
      }
    }
    async function getLottery(slug, { lotteryApiFactory = createLotteryApi, claimStore = claims } = {}) {
    ...
    async function listMyPrizes({ lotteryApiFactory = createLotteryApi, claimStore = claims } = {}) {
    ...
    module2.exports = { runCli, getLottery, listMyPrizes, normalizePublicCapability };
  }
});
