---
name: coding
description: "Gunakan saat user punya preferensi gaya coding, keputusan stack, atau pola yang ingin diingat agent—HANYA dari koreksi/konfirmasi eksplisit, bukan observasi. Aktif saat user bilang 'ingat gaya Python saya', 'prefer X daripada Y', atau minta agent menerapkan preferensi coding yang tersimpan."
metadata:
  openclaw:
    version: 1.0.3
---
...
# coding — X∞ Compliance Layer

## 1. IDENTITY
Skill milik user: `coding`. Mengikuti Skill Architecture Standard X∞ (wajib).

## 2. PURPOSE
Menyimpan & menerapkan preferensi coding user (gaya, stack, pola) ke dalam file lokal `~/coding/`—hanya dari umpan balik eksplisit, tidak pernah mengamati atau menyimpulkan dari project. Mencegah agent kehilangan konteks gaya antar sesi.

## 3. METADATA
- name: coding
- version: 1.0.3
- standard: Skill Architecture Standard X∞ (21-node)
- scope: memory preferensi coding lokal (~/coding/)
- depends_on: tidak ada (mandiri, tanpa network)
...
**CHANGELOG**
- 1.0.3 — Perbaikan kualitas lapisan X∞: frontmatter `description` rusak (berisi teks changelog) diganti deskripsi trigger nyata; Node 2 (PURPOSE) & Node 3 (METADATA) diisi (bukan stub); `metadata.openclaw.version` diset 1.0.3 seragam dengan _meta.json. Konten domain (Core Rules, Scope, Architecture) dipertahankan.
