...
notes: >-
  ...
  No separate install spec—only usage instructions in SKILL/manifest; repo is
  Python scripts only (stdlib); does not auto-download or install packages;
  low install-surface risk.
---
...
- 本技能**没有**单独的「安装规范」文档，仅以本文与 **`manifest.yaml`** 中的使用说明为准。
- 仓库内**仅**包含 Python 脚本；脚本仅使用 **Python 标准库**，**不会**自动下载或安装 pip 包或其它二进制。
- 无内置安装器、无运行时拉取依赖，从安装/供应链面看**风险较低**。
