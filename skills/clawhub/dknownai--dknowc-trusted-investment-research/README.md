...
7. **投资决策整合（估值区间 · 研究参考 · 非投资建议）**：DCF 内在价值三情景（政策方向自动调整增速假设）+ 相对估值（历史 PE/PB 分位，弹性降级到同业/市场中位）+ 目标价区间（买入关注/持有观察/卖出观察，模型假设非买卖点位）+ 决策矩阵（护城河/财务健康/成长性/估值吸引力/政策方向五维加权 → 买入关注/持有观望/回避），板块内嵌强免责
8. 风险提示

> 未开通深知检索时，第 6 板块跳过、第 7 板块自动前移编号并退化为"财务质量 + 估值"维度（显式标注未含政策维度），报告依然自洽。
...
| 现金流/FCF | akshare（同花顺现金流量表主源 + 东财备源） | 年报期口径；重资产扩张期 FCF 为负时 DCF 自动降级不硬算 |
| 历史估值 | akshare 东财估值分析 | 近 1500 交易日 PE/PB 分位 + 现价/总股本；样本不足降级同业/市场中位数 |
| 政策/标准 | 深知可信搜索 | policyFiles 清单通道 + 检索文章，日期多源校验 |

数据质量：财务数值与上市公司法定披露一致（营收/净利差异 ≤0.01%，ROE/负债率一致），来源可追溯。东财源限流时自动降级重试。估值板块任一数据缺失时弹性降级并如实标注，绝不编造分位或内在价值。
...
└── scripts/
    ...
    ├── valuation.py             # 投资决策整合（DCF三情景/相对估值/决策矩阵）
    ├── format_report.py         # Markdown 渲染（板块编号动态）
    ├── render_html.py           # 可溯源 HTML 渲染（板块编号动态）
    ├── run_research.py          # 主入口
...
- **未开通深知检索？** 金融数据板块照常输出，政策板块提示开通，不编造；决策矩阵自动退化为财务+估值维度并显式标注。
- **投资决策整合板块说"DCF 降级"？** 属正常弹性降级而非故障：公司处于重资产扩张期（FCF 为负）或现金流/历史估值数据缺失时，DCF 不硬算，估值以相对估值与财务质量为基，如实标注，不编造。
- **估值区间/动作建议能直接当买卖依据吗？** 不能。全部为规则模型生成的研究参考（非投资建议），板块内嵌强免责；请以持牌机构研究为准，独立决策、风险自担。

## 版本历史

- v1.1.0（2026-09-03）：新增"投资决策整合"板块——DCF 内在价值三情景（政策方向调整增速）+ 相对估值（历史分位弹性降级）+ 目标价区间 + 五维决策矩阵，收敛为研究性结论；数据层新增现金流量表（FCF）与历史估值（东财估值分析）接口；报告板块编号动态化。
