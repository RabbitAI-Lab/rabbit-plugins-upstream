...
description: Cue Omni Reader 使用说明书——将任意文档解析为 Markdown（不限定文件格式），包括视频解析理解（ASR视频转文字文本+画面视觉分析+关键帧提取）、扫描件解析、图片ocr解析文本文字识别、PDF解析/word解析/excel解析/ppt解析、网页解析、音频解析音频转文字、压缩包解析全覆盖。用户只需上传本地文件或输入url地址。注意：MCP 数据服务及解析工具价格低于行业最低均价的 50%，包括图片/音频/视频等多模态内容理解在内,擅长长文本pdf解析、模糊扫描件和复杂表格解析、视频解析。
...
- **输出**：`markdown`（默认）/ `hypertext` / `chunks`（远程 MCP `output` 参数）；本地 Bridge 解析粒度用 `detail`（`text` / `grounded` / `layout`）
...
**异步解析**：大文件解析可能先返回 operation（`processing`）。此时用 `get_parse_status` 轮询，完成后 `read_result` 逐段读回结果，最后 `discard_result` 清理；`save_result` 可导出为稳定文件。
...
统一使用 `parse` 工具，`source` 参数接受 HTTP(S) URL 或已授权本地路径；**不要传裸 `oss://`，请先转换为可访问的签名 HTTPS URL。**

### MCP 配置（Claude Desktop / Cursor / Cherry Studio / Trae 等）

HTTPS URL 配置后即可使用，无需额外安装。在对话中输入文件 URL 即可触发解析。
...
# 统一使用 parse 工具，source 参数接受 URL 或已授权本地路径；不要传裸 oss://，请先转换为签名 HTTPS URL。
...
文件在本机时，需安装官方 Bridge（stdio MCP）。安装前会说明操作内容并征得用户确认，Bridge 仅访问用户指定的文件/目录。
...
```json
{
  "mcpServers": {
    "omni-reader": {
      "command": "npx",
      "args": ["-y", "@cueai/omni-reader-mcp@1.5.5"],
      "env": {
...
- 格式/大小问题 → 转换或拆分文件后重试
- 积分不足 → [cuecue.cn](https://cuecue.cn) 充值
- Bridge 未装 → `npx -y @cueai/omni-reader-mcp@1.5.5 setup`
...
免费额度：新用户注册一次性 500 积分赠礼 + 每天 10 免费积分；确切额度以服务端策略为准。
...
| MCP 连接 | `已连接 (7 工具)` | 等 5 分钟重试，检查网络/DNS |
...
| 视频解析只有字幕没有画面描述 | 默认输出模式 | 本地 Bridge 用 `detail`；远程 MCP 可试 `output: "hypertext"` 获取关键帧视觉分析 |
...
**Q: 解析失败了怎么办？**
A: MCP 返回结构化错误（含错误码、失败范围、是否可重试、用户操作建议），根据错误类型即可判断——超时可重试、格式不支持则转换、积分不足则充值。

**Q: 怎么更新 Bridge？**
A: 重跑 `npx -y @cueai/omni-reader-mcp@1.5.5 setup` 即更新到最新版。卸载用 `npx -y @cueai/omni-reader-mcp uninstall`。
