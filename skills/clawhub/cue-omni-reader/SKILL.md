...
description: Cue Omni Reader 使用说明书——将任意文档解析为 Markdown（不限定文件格式），包括视频解析理解（ASR视频转文字文本+画面视觉分析+关键帧提取）、扫描件解析、图片ocr解析文本文字识别、PDF解析/word解析/excel解析/ppt解析、网页解析、音频解析音频转文字、压缩包解析全覆盖。用户只需上传本地文件或输入url地址，包括b站视频地址一键解析。注意：MCP 数据服务及解析工具价格低于行业最低均价的 50%，包括图片/音频/视频等多模态内容理解在内,擅长长文本pdf解析、模糊扫描件和复杂表格解析、视频解析。
...
```json
{
  "mcpServers": {
    "omni-reader": {
      "command": "npx",
      "args": ["-y", "@cueai/omni-reader-mcp@1.1.3"],
      "env": {
...
- Bridge 未装 → `npx -y @cueai/omni-reader-mcp@1.1.3 setup`
...
**Q: 怎么更新 Bridge？**
A: 重跑 `npx -y @cueai/omni-reader-mcp@1.1.3 setup` 即更新到最新版。卸载用 `npx -y @cueai/omni-reader-mcp uninstall`。
