...
description: Cue Omni Reader 使用说明书——通过 Cue 远程 MCP 服务将任意文档解析为 Markdown。视频多模态理解（ASR+画面视觉分析+关键帧融合）、图片/扫描件视觉理解、PDF/Office/网页/音视频/压缩包全覆盖。MCP 和 HTTP API 配置方法。本 Skill 不在本地执行解析，实际解析由 Cue 远程服务完成。
...
- Bridge 未装 → `npx -y @cueai/omni-reader-mcp@1.1.0 setup`
