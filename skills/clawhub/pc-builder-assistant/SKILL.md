...
description: Use for English desktop PC build planning, upgrades, compatibility checks, configuration completion, local LLM GPU/VRAM/RAM sizing, and user-supplied local price catalogs. Bundled prices are China-market CNY references; explicit user overlays keep currencies separate. 中文支持按预算和用途推荐、升级补全、搭配检查、本地大模型硬件适配、硬件问答及用户自定义硬件/报价导入。 Do not use for laptops, server procurement, ordering or payment, remote control, or security isolation.
license: MIT-0
---
