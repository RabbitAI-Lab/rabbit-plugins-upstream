---
name: pc-builder-assistant
description: Use for English desktop PC build planning, upgrades, compatibility checks, configuration completion, and user-supplied local price catalogs. Bundled prices are China-market CNY references; explicit user overlays keep currencies separate. 中文支持按预算和用途推荐、升级补全、搭配检查、硬件问答及用户自定义硬件/报价导入。 Do not use for laptops, server procurement, ordering or payment, remote control, or security isolation.
license: MIT-0
---
...
6. 输出配置。只回答方向或原理且未给具体采购型号时，不强制套整机报价表。给出具体型号或清单时，分行列出八类配件及参考单价，并写总价、预算差额、兼容结论、取舍理由、下单前复核点和价格参考日期。核显整机的显卡行写“无独显，使用 CPU 核显，¥0”，并列出已核实的主板视频接口；接口字段缺失时不得写成完整通过。计划以后加卡时按目标显卡预留电源和机箱，目标未定则不得承诺未来显卡兼容。游戏帧率只引用 `scripts/query_game_fps.py` 已收录样本，未收录时不自行推算。
...
- 默认输出人民币价格并标注价格参考日期；用户显式提供本地币种 overlay 时只按该币种查询、排序和合计，不做换算或跨币种混算，人民币基础价仍保留为独立参考。没有匹配 overlay 时，英语叶子仅在用户明确要求时可附加已核实汇率的外币估算。缺价条目不参与总价。
