...
# Safety: Only calls legitimate academic APIs (Crossref, arXiv, Zotero) and WebDAV (if configured). No eval, no obfuscation.
# NOTE: Extracts PDF text for DOI/arXiv lookup and queries external academic APIs.
...
case "$1" in
    --help|-h)
        cat <<EOF
Usage: $0 PDF_FILE
Add a PDF to Zotero with automatic metadata fetching (DOI/arXiv).
