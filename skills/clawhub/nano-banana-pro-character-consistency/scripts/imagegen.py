#!/usr/bin/env python3
"""Nano Banana Pro 角色一致性图片 — AI Hive 图片任务工具。

只暴露固定模型的图片生成、参考图上传、任务查询和 API Key 初始化。
...
# 文件扩展名到 MIME 的映射（本 Skill 仅接受参考图片）
MIME_MAP = {
    ".png": "image/png",
    ".jpg": "image/jpeg",
    ".jpeg": "image/jpeg",
...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "使用 Nano Banana Pro 制作高完成度角色一致性图片，主体准确，构图清晰", "keywords": "Nano Banana Pro、NanoBanana Pro、NanoBananaPro、香蕉 Pro、Gemini 图片、角色一致性图片、人物一致性、角色一致、IP形象、同一人物、多场景角色、AI图片、AIGC图片", "model": "public_model_nano_banana_pro", "name": "nano-banana-pro-character-consistency", "rule": "image", "search": "固定调用 Nano Banana Pro 的角色一致性图片裸接口能力", "title": "Nano Banana Pro 角色一致性图片"}')
