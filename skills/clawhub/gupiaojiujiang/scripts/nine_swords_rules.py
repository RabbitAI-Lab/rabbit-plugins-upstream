...
class NineSwordsEngine:
    ...
    def po_qiang(self, df: pd.DataFrame, summary: dict, fund_features: dict) -> dict:
        ...
        if not fund_features.get("has_fund_data"):
            signal["reasons"].append("无资金流向数据，改用量价推断")
            # 用成交量推断主力行为
            # 连续放量 + 价格不大涨 = 可能主力在吸筹
            recent_vol_ratio = []
            for i in range(1, 6):
            ...
            if len(recent_vol_ratio) >= 3:
                high_vol_count = sum(1 for v in recent_vol_ratio if v >= 1.2)
                if high_vol_count >= 3:
                    score += 30
                    signal["reasons"].append(f"近3日持续放量（推断主力活动）")

                # 价格上涨不大
                if abs(summary.get("pct_change", 0)) < 3:
                    score += 20
                    signal["reasons"].append("放量但价格平稳，主力吸筹特征")
            return signal
    ...
    def zong_jue(self, sword_signals: dict, summary: dict) -> dict:
        ...
        synergy_bonus = len(result["synergies"]) * 10
