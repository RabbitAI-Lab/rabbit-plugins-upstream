...
# 2. 安装依赖
pip install akshare pandas numpy mplfinance matplotlib mcp
...
```json
{
  "mcpServers": {
    "dugu-9swords": {
      "command": "python",
      "args": ["-B", "E:/独孤九剑/.claude/skills/股票九剑/mcp_server/server.py"],
      "env": {
...
```json
{
  "dugu-9swords": {
    "transport": "stdio",
    "command": "python",
    "args": ["E:/独孤九剑/.claude/skills/股票九剑/mcp_server/server.py"],
    "env": {
      "PYTHONIOENCODING": "utf-8"
    }
  }
...
```bash
# 在终端手动启动
cd "E:/独孤九剑/.claude/skills/股票九剑/mcp_server"
...
```bash
npx @anthropic-ai/mcp dev E:/独孤九剑/.claude/skills/股票九剑/mcp_server/server.py
```
...
| 启动报错 `ModuleNotFoundError: akshare` | `pip install akshare pandas numpy mplfinance matplotlib` |
| 数据获取失败 | 检查网络连接，akshare 需要访问东方财富等数据源 |
