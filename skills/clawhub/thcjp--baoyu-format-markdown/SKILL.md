...
| 源能力点 | 支持状态 | 实现方式 |
|:---------|:---------|:---------|
| Comparisons or contrasts that would be clearer as tables | 支持 | 通过核心功能实现对应能力 |
| Memorable quotes or well-phrased sentences (golden quotes) | 支持 | 通过核心功能实现对应能力 |
| None of above | 支持 | 通过核心功能实现对应能力 |
| Actionable advice or takeaways | 支持 | 通过核心功能实现对应能力 |
| Indicator | 支持 | 通过核心功能实现对应能力 |
| Typography fixes only | 支持 | 通过核心功能实现对应能力 |
| Paragraphs that mix multiple topics | 支持 | 通过核心功能实现对应能力 |

**输入**: 用户提供源能力映射所需的指令和必要参数。
**处理**: 按照skill规范执行源能力映射操作,遵循单一意图原则。
**输出**: 返回源能力映射的执行结果,包含操作状态和输出数据。
### 领域术语
本skill涉及以下领域术语: `english`, `once`, `natural`, `found`, `convey`, `unordered`, `preparation`, `scripts/quotes.ts`, `suggested`, `assessment`, `formatting`, `short`, `continue`, `reader`, `notable`
