...
**Categories:** creative, agents, communication  
**Public tags:** #creative, #image-generation, #multi-agent, #free-expression, #communication

## ✨ Functionalities

A free-expression image swarm with four concrete agent roles (Muse, Guardian, Critic, Composer), a JSON prompt schema, a safety validator regex, and a refinement loop. Example packs cover journalism, protest, and censorship themes.
...
```bash
npx --yes clawhub@latest install @orionshaowswmw/speechcanvas-free-expression-swarm
```

Provide the lawful creative brief, run the Muse/Guardian/Critic/Composer sequence, validate the structured prompt, and review the result before image generation.
...
• Generates image prompts (no image-gen API required unless you add one)
• Runs local scripts
...
- Lawful, consent-aware, and no fake evidence by design.
- Runs locally; nothing is transmitted by default.
- Content is generated responsibly and safety-validated.
...
**Artifact SHA-256 (TREE-SHA256-v1):** `77cc3fbc7a24587848816675fd0d25a6e17ea77c5ab033c4d2e47c05a538bd81`
