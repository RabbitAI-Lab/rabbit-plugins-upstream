...
- **Local execution is out of scope.** No Todoist response should ever decide what gets executed, and nothing here writes or runs a script from API output. The only local commands are the documented ones you run yourself: installing the CLI or an SDK, and the fixed fallback request in [Appendix: Environments Without the CLI](#appendix-environments-without-the-cli).
...
- `project_id` - Project the task belongs to (defaults to Inbox)
...
req = urllib.request.Request(GATEWAY + "/todoist/api/v1/projects")
req.add_header("Authorization", "Bearer " + os.environ["MATON_API_KEY"])
req.add_header("User-Agent", "maton-todoist-skill/1.2")
