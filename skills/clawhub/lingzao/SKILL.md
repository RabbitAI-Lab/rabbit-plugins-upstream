---
name: lingzao
description: 灵造是给 WorkBuddy、OpenClaw、Codex 等 Agent 使用的跨平台创作者研究与自媒体运营 Skill。安装免费，可先做选题、标题、封面、账号诊断、发布检查、复盘，以及公众号对标适配判断和原创写作；查询公开内容、评论、短视频文案、公众号文章数据或生成图片时需要灵造积分和 API Key。
---
...
- paid lookup -> add `search-credit-notice`
...
that only the failed item was not charged, report any successful-item cost in
the same batch, and ask for a shorter video link.
...
`--count 1` calls. The CLI gives that one batch a stable request ID and
automatically resumes it when a POST response is ambiguous or polling is
interrupted, including when the Agent repeats the same command in a later
process. Do not invent another network-retry loop. After a terminal result, the
same command starts a new intent with a new ID, so an explicit user request to
generate again is not content-deduplicated. Use `--new-request` only when the
user explicitly wants to abandon an ambiguous pending intent and start a new
one; it does not bypass the one-active-batch limit. If the user wants distinct
concepts, vary the prompt for each concept or use one counted batch for
