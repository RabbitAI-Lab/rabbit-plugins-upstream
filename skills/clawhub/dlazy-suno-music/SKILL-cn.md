...
Options:
  --customMode [customMode]            自定义模式 [default: false]
  --instrumental [instrumental]        纯音乐（无人声） [default: false]
  --prompt [prompt]                    提示词 [default: ]
  --style [style]                      风格 [default: ] [only when customMode="true"]
  --title [title]                      标题（自定义） [default: ] [only when customMode="true"]
  --negativeTags [negativeTags]        反向标签（排除的风格/特征） [default: ]
  --vocalGender [vocalGender]          人声性别（m=男; f=女） [default: f] (choices: "m", "f") [only when instrumental="false"]
  --styleWeight [styleWeight]          风格权重（0-1） [default: 0.65]
  --weirdnessConstraint [weirdnessConstraint]怪异度（0-1） [default: 0.65]
  --audioWeight [audioWeight]          音频权重（0-1） [default: 0.65]
