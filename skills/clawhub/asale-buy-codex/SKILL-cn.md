...
在「从 asale 市场买」和「用它自己的订阅」之间切换 Codex，并查看哪些正在运行的会话还用着旧配置。
...
Codex 是别扭的那个。它不从请求里取模型，而是从自己的目录里挑——所以买到的模型必须借用
工具已经认识的**载体 slug** travel，守护进程会生成一份目录文件外加一张别名表来把它们映射
回去。这就是这里必须选模型的原因，也是为什么 ChatGPT 应用的模型菜单里显示的可能是一个
眼熟的名字、背后站着的是市场上的模型。

对 `config.toml` 的修改是保格式的，用户的注释和排版都会留下来。
...
`codex` 那一行带着 `installed`、`enabled`（开关）、`in_effect`（它的实时配置真的指向代理）、
...
不传 `models` 就保持当前选择，传 `[]` 是清空。会改写的配置：`~/.codex/config.toml`（一个 `[model_providers.asale]` 块、活动的 `model_provider`、模型，以及一份生成的目录）和 `~/.codex/auth.json`（`OPENAI_API_KEY`）。
...
Codex **只在启动时**读一次配置，所以已经在跑的会话拿的还是旧的。`running.codex` 会按 pid 列出
它们，并带上启动时的命令行。
