---
name: asale-buy-codex
version: 0.2.7
description: "Switch Codex between buying from the asale market and using its own subscription, and see which running sessions are still on the old config. 在「从 asale 市场买」和「用它自己的订阅」之间切换 Codex，并查看哪些正在运行的会话还用着旧配置。"
metadata: {"clawdbot":{"emoji":"📥","requires":{"bins":["curl"]},"install":"curl -fsSL https://asale.ai/dl/install.sh | sh","installAlternative":"irm https://asale.ai/dl/install.ps1 | iex","homepage":"https://asale.ai","source":"https://github.com/asale-ai/asale","author":"asale","license":"see-repo","configLocation":"~/.asale/daemon.token","apiEndpoints":["127.0.0.1:9700"]},"openclaw":{"systemPrompt":"Drive the asale daemon at 127.0.0.1:9700 with the token from ~/.asale/daemon.token. Always run buy_tools before set_buy_tool for tool codex. tool_processes is listing only — never signal those pids."}}
---
...
Switch Codex between buying from the asale market and using its own subscription, and see which running sessions are still on the old config.

## Trigger Keywords

- make Codex buy from the market
- point Codex at asale
- stop spending my ChatGPT subscription
- asale buy codex
...
Codex is the awkward one. It does not take the model from the request — it picks
from a catalog of its own — so the bought models have to travel under *carrier
slugs* the tool already knows, and the daemon generates a catalog file plus an
alias table to map them back. That is why a selection is not optional here, and
why the model shown in the ChatGPT app's menu may be a familiar name standing in
for the market model behind it.

The edit to `config.toml` is format-preserving, so the user's comments and
layout survive.
...
The `codex` row carries `installed`, `enabled` (the switch), `in_effect` (its live
...
```bash
call set_buy_tool '{"tool":"codex","enabled":true,"models":["claude-fable-5"]}'
```

`models` omitted leaves the current selection alone; `[]` clears it. Config
rewritten: `~/.codex/config.toml` (a `[model_providers.asale]` block, the active `model_provider`, the model and a generated catalog) and `~/.codex/auth.json` (`OPENAI_API_KEY`).
...
Codex only reads its config **at startup**, so a session that is already running
keeps the old one. `running.codex` lists the ones still on it, by pid, with the
...
```bash
call set_buy_tool '{"tool":"codex","enabled":false}'
```
