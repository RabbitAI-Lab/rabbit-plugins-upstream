...
### 文件级巴士因子披露（仅文件路径，不出现姓名）

```
⚠️ bus-factor 提醒：以下文件在分析区间内只有一位 distinct 历史作者，建议安排知识转移与评审轮换：
  - core/auth/session.ts （distinct 历史作者数：1）
  - infra/billing/legacy_adapter.go （distinct 历史作者数：1）
```

报告**不会**输出该作者的姓名。如果你需要联系责任人安排知识转移，可以在本地自行运行 `git log -- <file>` 查看 —— 这是用户的本地操作，不在本技能输出范围内。报告也**不会**在这里附加任何节奏、流失、返工、周末比例等评估性指标，更不会对相关贡献者作出任何行为性点评。
## 写在最后：数据不是审判
