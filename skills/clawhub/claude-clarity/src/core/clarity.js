...
function _formatForFeishu(result, input) {
  ...
  // 支持思维链结果格式（decision/reasoning 在顶层）
  const hasChainResult = !!result.decision;
  const output = result.output || {};
  const decision = result.decision || {};
  const reasoning = result.reasoning || {};

  const taskType = output.meta?.taskType || decision.taskType || 'general';
  const confidence = decision.confidence || output.meta?.confidence || 0.5;
  ...
  if (isSimple && (input?.length || 0) < 50) {
    return {
      output: output ? {
        conclusion: (output.conclusion || decision.conclusion || '').slice(0, 300),
        confidence,
        reasoningChain: (output.meta?.reasoningChain || decision.reasoningChain || []).slice(0, 2),
      } : null,
      decision: {
        shouldRespond: decision.shouldRespond ?? output.shouldRespond,
        suppressed: decision.suppressed ?? output.suppressed,
        confidence,
        conclusion: (decision.conclusion || '').slice(0, 200),
      },
      reasoning: reasoning ? {
        summary: (reasoning.summary || []).slice(0, 5),
        stages: reasoning.chain?.stages?.length || 0,
      } : null,
  ...
  formatted._meta = { taskType, compact: true, stages: result.chain?.stages?.length || result.reasoning?.chain?.stages?.length || 0 };
  return formatted;
}
...
class Clarity {
  constructor(config = {}) {
    ...
    // Proactive Layer — 主动引擎（已移除 MarkCode 残留模块）
