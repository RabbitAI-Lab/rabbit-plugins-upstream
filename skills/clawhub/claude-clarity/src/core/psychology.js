...
function detectPADFromText(text) {
  ...
  // 悲伤/难过（P--，A=0，D=0）
  if (/难过|悲伤|伤心|痛苦|委屈|压抑|无奈|心酸|受伤|失落|沮丧|遗憾|惋惜/.test(text)) {
    pleasure -= 4; arousal = 0; dominance = 0;
    const emotion = getEmotionFromPAD(pleasure, arousal, dominance);
    return { ...calculatePADState(pleasure, arousal, dominance), emotion: emotion.name, emotionZh: emotion.zh };
  }
  ...
  const negativeWords = ['烦', '累', '难', '挫败', '无聊', '讨厌', '糟糕', '失败', '困惑', '失望', '遗憾',
...
const DEFENSE_MECHANISMS = {
  ...
  denial: {
    ...
    patterns: [
      ...
      '我没', '不是我的问题', '不可能',
      // [v2.0.17] 扩展: 直接否认 + 弱化/模糊化
      '没有', '别瞎说', '我才没有',
      '并不准确', '不完全', '不属实', '不实报道', '失实',
      '夸大其词', '过度解读', '断章取义', '误解'
    ]
  },
  ...
  minimization: {
    ...
    zh: '最小化/Minimization',
    patterns: [
      '其实', '只不过', '只是', '只是小问题', '没什么大不了的',
      '并不严重', '影响有限', '范围很小', '个别现象'
    ]
  },
  // [v2.1] 新增: 转移责任 - 将责任推给外部因素
  blame_shifting: {
    name: 'blame_shifting',
    zh: '转移责任/Blame Shifting',
    patterns: [
      '舆论', '媒体', '报道', '放大', '误解', '不准确',
      '外部', '环境', '客观原因', '不可抗力', '不可避免'
    ]
  },
  // [v2.1] 新增: 投射 - 把自己的问题归咎于他人
  projection: {
    name: 'projection',
    zh: '投射/Projection',
    patterns: [
