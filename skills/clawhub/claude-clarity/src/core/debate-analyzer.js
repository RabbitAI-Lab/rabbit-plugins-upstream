...
class DebateAnalyzer {
  ...
  async analyze(input) {
    ...
    // 回退模式：如果没有提取到具体主张，将整个输入视为辩论话题进行分析
    if (agreements.length === 0 && disagreements.length === 0 && insight.length === 0) {
      const topicAnalysis = this._analyzeAsTopic(input);
      agreements.push(...topicAnalysis.agreements);
      disagreements.push(...topicAnalysis.disagreements);
      insight.push(...topicAnalysis.insights);
    }
  ...
  /**
   * 回退模式：将输入视为辩论话题，生成结构化分析
   * 当无法提取具体主张时使用
   */
  _analyzeAsTopic(input) {
    const agreements = [];
    const disagreements = [];
    const insights = [];
    ...
    // 检测辩题模式
    const isProPosition = /造福|应该|必须|必然|一定会|必将|是好的|是有益的/.test(trimmed);
    const isConPosition = /可能有害|不应该|不可行|存在风险|带来问题|是危险的|不可行/.test(trimmed);
    const isAnalysisRequest = /[？?]/.test(trimmed) || /请.*分析|请.*论证|请.*讨论|请.*构建|请.*评估|分析.*论证|分析.*利弊/.test(trimmed);

    if (isAnalysisRequest) {
      // 输入是一个分析请求，不是具体论点
      agreements.push({
        claim: '请求本身是有价值的——对复杂问题进行多角度分析',
        verdict: 'supported',
        confidence: 0.9,
        evidence: '结构化分析本身就是认知进步',
        type: 'meta_claim',
      });
      disagreements.push({
        claim: '分析请求未预设立场，但可能存在隐含倾向',
        correction: '需识别请求中隐含的立场偏向',
        severity: 1,
        confidence: 0.5,
        evidence: '分析请求的措辞可能暗示期望结论',
        type: 'bias_risk',
      });
      insights.push({
        type: 'hidden_assumption',
        observation: `分析请求隐含假设：该话题有确定答案`,
        significance: 3,
        reframing: '有些问题没有单一正确答案，分析的价值在于理清逻辑空间',
      });
    } else if (isProPosition) {
      agreements.push({
        claim: trimmed.slice(0, 50) + (trimmed.length > 50 ? '...' : ''),
        verdict: 'position_detected',
        confidence: 0.7,
        evidence: '明确的正方立场',
        type: 'pro_position',
      });
      disagreements.push({
        claim: '正方立场可能忽视了反面论据',
        correction: '需要构建反方论证来检验正方立场',
        severity: 2,
        confidence: 0.6,
        evidence: '任何确定性主张都需要反面检验',
        type: 'missing_counter',
      });
      insights.push({
        type: 'core_tension',
        observation: '正方主张 vs 反面证据之间的张力',
        significance: 4,
        reframing: '真正的答案往往在正反张力之间，而非任何一方',
      });
    } else if (isConPosition) {
      disagreements.push({
        claim: trimmed.slice(0, 50) + (trimmed.length > 50 ? '...' : ''),
        verdict: 'position_detected',
        confidence: 0.7,
      ...
      insights.push({
        type: 'hidden_assumption',
        observation: '反方立场可能预设了不可逾越的障碍',
        significance: 3,
        reframing: '风险≠不可行，需要区分程度和可能性',
      });
    } else {
      // 默认：任何输入都值得进行辩论分析
      agreements.push({
      ...
      insights.push({
        type: 'hidden_assumption',
        observation: `分析对象: ${trimmed.slice(0, 80)}${trimmed.length > 80 ? '...' : ''}`,
        significance: 3,
        reframing: '任何论点都值得从正反两面检验',
      });
    }

    // 通用洞察：检测话题中的关键词
    const topicKeywords = trimmed.match(/[一-鿿]{2,6}/g) || [];
    const significantTerms = topicKeywords.filter(w =>
      !['的', '了', '是', '在', '和', '与', '或', '不', '有', '这', '那', '为', '以', '及', '将', '会', '能', '可'].includes(w)
    ).slice(0, 5);

    if (significantTerms.length > 0) {
      insights.push({
        type: 'keyword_analysis',
        observation: `话题关键词: ${significantTerms.join('、')}`,
        significance: 2,
        reframing: `围绕这些关键词展开正反论证`,
      });
    }

    return { agreements, disagreements, insights };
  }
