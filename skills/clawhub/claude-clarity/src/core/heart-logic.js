...
class HeartLogic {
  ...
  _perceiveTruth(context) {
    ...
    // 感知认知深度信号：
    // - "为什么/如何/什么" → 寻求解释/方法/定义 → 内容具有认知结构
    // - "原因/原理/理由" → 因果思考 → 认知深化
    // - "分析/比较/区别" → 认知操作 → 认知活跃
    // - "逻辑/推理/推导" → 形式思维 → 认知秩序
    const cognitiveDepthPatterns = [
      '为什么', '如何', '怎样', '什么', '怎么', '为何',
      '原因', '理由', '原理', '关系', '区别', '比较',
      '分析', '逻辑', '推理', '推导', '思考', '判断',
      '哪个', '哪些', '是否', '能不能', '会不会'
    ];
    const depthCount = cognitiveDepthPatterns.reduce((sum, p) => {
      return sum + (output.includes(p) ? 1 : 0);
    }, 0);

    // 认知秩序评分逻辑：
    // - 有多元/条件/不确定表达：+ 认知秩序
    // - 有认知深度信号（问问题/分析/推理）：+ 认知秩序（内容差异化）
    // - 有高度确定表达：- 认知秩序
    // - 有二元对立模式：- 认知秩序
    let score = PERCEPTION.truth.base;
    score += opennessCount * PERCEPTION.truth.opennessBonus;
    score += depthCount * PERCEPTION.truth.opennessBonus; // 认知深度信号同样加分
  ...
  _perceiveKindness(context) {
    ...
    // 关系维护信号：表达尊重/认可/感谢 → 关系秩序
    const respectSignals = [
      '谢谢', '感谢', '抱歉', '对不起', '认同',
      '赞同', '理解你', '尊重', '体谅', '包容'
    ];
    const respectCount = respectSignals.reduce((sum, p) => {
      return sum + (output.includes(p) ? 1 : 0);
    }, 0);
    score += respectCount * 0.08; // 尊重信号加分（与 presenceBonus 同级）
  ...
  _perceiveBeauty(context) {
    ...
    const structureCount = structureSignals.reduce((sum, p) => {
      return sum + (output.includes(p) ? 1 : 0);
    }, 0);
    ...
    // - 标点使用适度 → 表达有序
    // - 过长无标点句子 → 表达混乱
    const totalPunctuation = (output.match(/[。！？；：，、]/g) || []).length;
    const punctuationRatio = totalPunctuation / Math.max(sentences.length, 1);
    const clarityBonus = punctuationRatio >= 0.5 ? 0.05 : 0;
    const clarityPenalty = sentences.some(s => s.trim().length > 80 && !s.match(/[。；，]/)) ? 0.1 : 0;
  ...
  feltSilence(context = {}) {
    ...
    // 感知不确定 — 仅匹配明确的认知不确定表达，避免情感描述误报
    // "心情很复杂"、"关系很复杂"等是情感状态描述，不是认知不确定
    const uncertainContextSignals = ['不确定', '看不懂', '不知道该怎么办', '无法判断', '难以分辨'];
    const isUncertain = uncertainContextSignals.some(s => (input || '').includes(s));
    // "复杂"单独出现时才是认知不确定，情感描述中的"复杂"不触发
    const bareComplex = (input || '').match(/(?:^|[\s，。！？、；：])复杂(?:$|[\s，。！？、；：])/);
    // "不知道"只有在明确表达不确定时才触发沉默，否定语境中不触发
    // "不是因为不知道"是反思性表达，不是认知不确定
    const hasNegatedBudong = (input || '').match(/(?:不是|并非|并不|而非|而不是)[为因]*不知道/);
    const hasBareBudong = (input || '').includes('不知道')
      && !(input || '').includes('我不知道')
      && !hasNegatedBudong;
    if (isUncertain || bareComplex || hasBareBudong) {
