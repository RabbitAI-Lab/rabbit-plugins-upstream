...
class DirectoryCrawler:
    def __init__(self, mirror_base_dir: str = "/var/lib/iran_chem_db/mirrors"):
        ...
        self.engine = HTTrackMirrorEngine(str(self.base_dir))

    def mirror_directory(self, directory_url: str, depth: int = 3) -> str:
        parsed = urlparse(directory_url)
        project = re.sub(r"[^a-z0-9]+", "_", parsed.netloc.lower()).strip("_")
        out_dir = str(self.base_dir / project)
        self.engine.mirror_using_url_list([directory_url], out_dir, project)
        return out_dir
