...
class SupplierDiscoveryEngine:
    ...
    def seed_suppliers(self) -> List[SupplierCandidate]:
        return [SupplierCandidate(url=u, source="seed", name=n) for n, u in SUPPLIER_SEEDS]
    ...
    def discover_via_directory_crawling_httrack(self, directories: Optional[List[str]] = None) -> List[SupplierCandidate]:
        urls: List[str] = []
        for directory_url in (directories or DIRECTORY_SEEDS):
            mirror_dir = self.directory_crawler.mirror_directory(directory_url)
            urls.extend(self.directory_crawler.extract_supplier_urls(mirror_dir))
    ...
    def run_full_discovery(self) -> List[SupplierCandidate]:
        ...
        for cand in candidates:
            ...
            cand.confidence = self.validator.score(cand.url)
            scored.append(cand)

        min_score = 60  # spec config.yaml: discovery.min_verification_score
        verified = [c for c in scored if c.confidence >= min_score]
        logger.info("discovery: %d candidates -> %d verified", len(scored), len(verified))
        return verified
