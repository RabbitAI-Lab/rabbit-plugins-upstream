...
def search_molecules(db: Session, query: str | None = None, cas: str | None = None,
                     ...
                     available: bool | None = None, page: int = 1, limit: int = 20):
...
def global_stats(db: Session) -> dict:
    return {
        "total_molecules": db.execute(select(func.count()).select_from(Molecule)).scalar_one(),
        ...
        "total_offerings": db.execute(select(func.count()).select_from(SupplierOffering)).scalar_one(),
