...
PARSEABLE_EXTENSIONS = {".html", ".htm", ".php", ".asp", ".aspx", ".jsp", ".pdf", ".xlsx", ".xls", ".csv"}


class HTTrackMirrorEngine:
    def __init__(self, base_dir: str = HTTRACK_BASE_DIR):
        self.base_dir = Path(base_dir)
        self._verify_httrack_installed()

    def _verify_httrack_installed(self) -> None:
        try:
    ...
    def build_command(self, config: HTTrackMirrorConfig, update: bool = False) -> List[str]:
        return config.to_flags(update=update)
    ...
    # ── URL list mirrors ──────────────────────────────────────────────────
    def mirror_using_url_list(self, urls: List[str], output_dir: str, project_name: str) -> dict:
        ...
        cmd = [
            ...
            "--depth=3", "--stay-on-same-domain", "--connection-per-second=1", "-q",
        ]
