...
Verified live 2026-09-05 against a production building on the service (read methods, plus a Booking3 book+cancel round-trip). Source: web app at https://prod.bokatvattid.se (Flutter), compiled `main.dart.js`. Generic: works for any v1-mode building; no building-specific constants below.
...
All calls: `GET https://prod.bokatvattid.se/api/api2?method=<METHOD>&<params>&lang=1`
- Everything in the query string; no headers, no body; URL-encode values.
...
- `method=checkLogin2&buildingid=<v1 id>&pincode=<PIN>&apartmentnumber=<apt>` → `{"error":0,"body":{"Token":"…","ApartmentID":<id>,"LaundryRoomDefaultID":<id>,"TotalLaundryRoom":6,…}}` (example shape)
- Later calls add `token=<Token>&userid=<ApartmentID>`.
...
## Booking3 (live-tested 2026-09-05: book → BookingID, cancel → clean)
