...
> **跨境电商评论分析 · 多站点消费者洞察**
> 技术名 `cross-border-review-analysis`，ARI 官方出品的 Amazon 评论采集与消费者洞察 Skill，已适配 WorkBuddy。
...
```text
使用 $cross-border-review-analysis 为 ASIN B0XXXXXXXX 生成 VOC 报告，站点 amz_us，我确认扣积分。
```
...
保持文件夹名 `cross-border-review-analysis`，整个放进对应作用域即可：

| 作用域 | 路径 |
|---|---|
| 用户级（推荐） | `~/.workbuddy/skills/cross-border-review-analysis/` |
| 项目级 | `<项目目录>/.workbuddy/skills/cross-border-review-analysis/` |
