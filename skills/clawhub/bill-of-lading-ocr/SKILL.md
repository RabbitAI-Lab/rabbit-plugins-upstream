...
---
# Sugon-Scnet 提单识别 OCR 技能

> **⚠️ 数据外传警告**：本技能会读取你指定的本地文件，并通过网络上传至 Scnet OCR 外部服务（`https://api.scnet.cn`）。提单文件可能包含收发货人、航次、货量、联系方式等商业或个人信息，使用前请确认文件内容可以外传并遵守相关隐私/保密规定。

本技能封装了提单识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取提单里核心信息。

---

## 功能特性

- **提单识别**：支持识别提取航次港口、收发货人、货量、提单号码与承运人核心信息。



## 前置配置

> **⚠️ 数据外传警告**：本技能调用 Scnet 在线 OCR 服务，你需要将待识别的本地文件上传至 `https://api.scnet.cn/api/llm/v1/ocr/recognize`。提单中可能包含商业敏感信息或个人数据，请确保你有权将该文件内容外传，并建议仅使用可信来源文件。
...
| 参数名 | 类型 | 必填 | 描述                                      |
|--------|------|------|-----------------------------------------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• BILL_OF_LADING（提单） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。      |

### 命令行调用示例

```bash
   python .claude/skills/bill_of_lading_ocr/scripts/main.py BILL_OF_LADING /path/to/test.jpg
```

### 在 AI 对话中使用

> **⚠️ 数据外传警告**：本技能会读取你指定的本地文件，并通过网络上传至 Scnet OCR 服务（`https://api.scnet.cn`）。
> 提单文件可能包含收发货人、航次、货量、联系方式等商业或个人信息，上传前请确认文件内容可以外传。

本技能**不会**自动触发。仅在用户**明确请求识别提单**、并提供了**可确认的具体文件路径**后，AI 才会调用本技能。

用户可以说：

- “帮我识别这张图像中的提单信息，图片在 /Users/name/Downloads/test.jpg”

AI 在调用前会向用户确认：文件路径、是否允许读取并上传该文件。
