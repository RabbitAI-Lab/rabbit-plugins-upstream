...
summary: "企业级 Jira 管理系统,支持批量操作、JQL 高级查询、敏捷看板管理与自动化工作流。核心能力: 项目管理领域的专业化 AI 辅助工具,提供企业级高级功能支持."
license: Proprietary
edition: pro
description: "核心能力: 项目管理领域的专业化 AI 辅助工具,提供企业级高级功能支持。Use when 需要提升效率、自动化流程、批量处理、工作流优化时使用。不适用于需要人工创意判断的任务。适用于独立开发者、企业团队和自动化工作流场景。支持中文交互，无需复杂配置即开即用。输出结果可直接使用，减少二次加工成本。提供结构化输出和错误处理机制。 功能涵盖: toolkit。"
...
**处理**: 解析批量 JQL 查询与结果导出的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回批量 JQL 查询与结果导出的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 敏捷看板管理与 Sprint 
敏捷看板管理与 Sprint 规划

**处理**: 解析敏捷看板管理与 Sprint 的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回敏捷看板管理与 Sprint 的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**处理**: 解析自定义字段与组件批量管理的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回自定义字段与组件批量管理的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 多项目事务关联与依赖追踪
多项目事务关联与依赖追踪

**处理**: 解析多项目事务关联与依赖追踪的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回多项目事务关联与依赖追踪的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Jira、管理系统、支持批量操作、高级查询、敏捷看板管理与自、动化工作流、核心能力、项目管理领域的专、辅助工具、提供企业级高级功、能支持等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | Jira 事务工具包专业版处理的输入数据或指令 |
...
1. **变更状态前先获取当前状态**
2. **protocol server 分配负责人需要 accountId 而非显示名**
3. **编辑描述前先展示原始内容**
4. **批量修改前需获得明确批准**
...
```json
{
  "success": true,
  "data": {
    "result": "Jira 事务工具包专业版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "jirakit pro"
    }
  },
...
- **自动化执行**: 企业级 Jira 管理系统,支持批量操作、JQL 高级查询、敏捷看板管理与自动化工作流。核心能力: 项目管理领域的专业化
