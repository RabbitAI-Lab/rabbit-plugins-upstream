...
环境变量：
  ...
  TENCENTCLOUD_COS_REGION  - COS Bucket 区域（必填，缺值即报错）
"""
...
示例：
  ...
  python3 mps_cos_download.py --cos-input-key output/result.mp4 --local-file ./result.mp4 \\
    ...
    parser.add_argument(
        "--region", "-r",
        default=None,
        help="COS Bucket 区域（默认使用环境变量 TENCENTCLOUD_COS_REGION，必填）"
    )
    parser.add_argument(
        "--secret-id",
        default=None,
...
def main():
    ...
    # 加载环境变量（仅校验 cos_download 自身实际需要的 4 个变量，不含 API_REGION）
    if _LOAD_ENV_AVAILABLE:
        ...
        loaded = ensure_env_loaded(required=_required, verbose=args.verbose)
        if not loaded:
            missing = check_required_vars(_required)
    ...
    region = args.region or os.environ.get("TENCENTCLOUD_COS_REGION")
    ...
    if not region:
        print("错误：缺少 COS Region 配置。请设置 TENCENTCLOUD_COS_REGION 环境变量，或使用 --region 参数。", file=sys.stderr)
        sys.exit(1)
