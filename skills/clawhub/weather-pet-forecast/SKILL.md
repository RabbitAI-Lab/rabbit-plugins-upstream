...
- **Chinese input** (中文输入) → use `--lang zh`
- **English input** → use `--lang en` (or omit, as it's default)
- **Other languages** → default to English

**Examples:**
- User asks "北京天气预报" (Chinese) → output in Chinese
- User asks "Weather forecast for Tokyo" (English) → output in English
...
- **Output language MUST match user input language** - use `--lang` parameter accordingly
