...
lang: zh
description: >
  论文配图一键生成，发稿前自动校验，杜绝返工。14种学术图表、7套配色（含色盲安全）、
  期刊投稿预设（nature/lancet 单双栏）、内置数据校验、PDF文字重叠+字号双重验证、
  补充图例生成器，中文零配置，PNG 600dpi + SVG + PDF + TIFF + EPS 输出。
metadata:
  clawdbot:
    emoji: "📊"
...
| 方案 | 说明 | 色盲安全 |
|------|------|---------|
| `okabe-ito` | Nature Methods金标准（Wong 2011）— 鲜艳，投稿首选 | ✅ 是 |
...
>
> **🎨 用户偏好**：避免大红大绿大黄配色方案，偏爱素雅柔和色系。中文医学出版物常优先选择 `cool` 或 `glm` 而非 `okabe-ito`。
>
> **🎨 用户偏好**：非投稿场景（演示、博客、报告）用户强烈偏好 **cool 冷色调**（`--theme cool`），明确拒绝过 Okabe-Ito 的橙/绿/黄（"大红大绿大黄"）。不确定时为该用户使用 `--theme cool`。
...
3. **Okabe-Ito 为默认推荐。** 始终推荐 `--theme okabe-ito` 保证色盲安全。`glm` 主题同样色盲安全，适合仍用白底的科技博客/演示。
