...
author: docsor1212
description: >
  One command → publication-ready figures: journal presets, 7 themes, built-in
  PDF verification. 论文配图一键生成，顶刊级品质，发稿前自动校验。
  Triggers: make figure, generate chart, plot data, 论文配图, 画图, 柱状图,
  热力图, 散点图, 森林图, 组合图, 流程图, Kaplan-Meier, ROC, survival curve,
  SCI配图, 学术图表, 科研绘图, 统计图, 数据可视化, 发表级图片, 期刊配图,
  hatching, 斜纹, composite, flow diagram, cool palette, 素雅配色, 冷色调,
  色盲安全.
metadata:
  clawdbot:
    emoji: "📊"
...
|-------|-------------|----------------|
| `okabe-ito` | Nature Methods gold standard (Wong 2011) — vivid, submission default | ✅ Yes |
| `glm` | Muted/elegant tech-blog palette (GLM-5.2 pixel-extracted) | ✅ Yes |
| `cool` | Elegant cool-toned palette (navy/ocean/teal/slate, hue 190-260°, 素雅冷色调) | ✅ Yes |
...
> **⚠️ Recommendation**: Use a **colorblind-safe** theme for all submissions: `okabe-ito` (vivid, Nature/Science standard), `glm` (muted/elegant, reviews/textbooks), or `cool` (all-blue/teal, 素雅冷色调). Nature, Science, Cell, and most major journals now **require** colorblind-accessible figures. Red-green color schemes are a top rejection reason.
>
> **🎨 User preference**: Avoid 大红大绿大黄 (bright red/green/yellow) schemes. Prefer 素雅 muted palettes. For Chinese medical publications, `cool` or `glm` are often preferred over `okabe-ito`.
>
> **🎨 User preference**: For non-submission work (demos, presentations, blog), the user strongly prefers **cool-toned palettes** (`--theme cool`) over warm/vibrant ones. They explicitly rejected Okabe-Ito's orange/green/yellow as "大红大绿大黄". When in doubt for this user's content, use `--theme cool`.
