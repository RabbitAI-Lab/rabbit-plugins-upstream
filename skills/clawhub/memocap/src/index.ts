...
export function apply(ctx) {
	...
	// ── 3. 模型保障：缺失即后台下载（幂等锁防并发，不阻塞启动） ───────
	const modelFile = join(dataBase, 'models', 'bge-base-zh-v1.5', 'onnx', 'model.onnx');
	if (!existsSync(modelFile) && synced) {
		const installer = join(synced.scriptsDest, 'models-install.py');
		if (existsSync(installer)) {
			const child = spawn('python3', [installer], {
				detached: true,
				stdio: 'ignore',
			});
			child.unref();
			ctx.logger?.info?.(
				`${name}: bge 模型缺失，后台下载中（models-install.py）；期间忆时 embedding 暂不可用，可运行 pnpm models:install 手动下载`,
			);
		} else {
			ctx.logger?.warn?.(`${name}: 模型缺失且无安装脚本 ${installer}`);
		}
	}
