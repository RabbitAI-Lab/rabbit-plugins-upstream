#!/usr/bin/env python3
"""
十万个为什么 - 即刻数据 Skill 脚本。

功能说明：
1. 支持关键词查询和随机返回两类能力。
2. 按 `--key`、环境变量、脚本目录 `.env` 的优先级读取 AppKey。
3. 分别调用 `/v1/tenwhy/query` 和 `/v1/tenwhy/random` 接口。
...
API_PATH_MAP = {"query": "/v1/tenwhy/query", "random": "/v1/tenwhy/random"}
APPKEY_ENV_NAMES = ("JIKE_TENWHY_QUERY_KEY", "JIKE_APPKEY")
REGISTER_URL = "https://www.jikeapi.cn/"
TITLE = "十万个为什么 - 即刻数据"
FAIL_TITLE = "十万个为什么失败"
ROW_FIELDS = [('question', '问题'), ('answer', '答案')]
QUERY_REQUIRES_PAGE = True
