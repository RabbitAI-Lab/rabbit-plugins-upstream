---
name: huaweicloud-ecs-list
version: 1.0.0
description: |
  查询华为云账号下指定 Region 的弹性云服务器（ECS）实例列表，输出精简的实例名称 + ID 清单。
  适合用于云资源盘点、运维巡检、自动化脚本前置查询。
tags:
  - huaweicloud
  - ecs
  - 资源查询
...
# 华为云 ECS 实例列表查询 Skill

## 能力

- 查询华为云账号下指定 Region 的 ECS 实例列表
- 输出精简结果：仅包含实例名称（name）与 ID（id）
...
# 指定配置文件
python3 list_ecs.py -c /path/to/config.json

# 临时覆盖 Region（不改配置文件）
python3 list_ecs.py -r cn-south-1
```

## 输出约定

成功时输出 Region 下的 ECS 实例名称 + ID 精简列表：

```
Region cn-north-4 的 ECS 实例列表（共 2 个）：
- my-ecs-01  7b9b0c8a-xxxx-xxxx-xxxx-xxxxxxxxxxxx
- my-ecs-02  8c0d1e2f-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```

无实例时输出：`Region cn-north-4 下暂无 ECS 实例。`
...
- 仅支持查询（ListServersDetails），不支持实例的创建/删除/修改
- 单次仅查询一个 Region
