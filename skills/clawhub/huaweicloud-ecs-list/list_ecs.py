#!/usr/bin/env python3
"""查询华为云 ECS 实例列表（输出实例名称 + ID 精简列表）。

读取配置文件中的 AK/SK（region 默认 cn-north-4 可配置），通过华为云
ListServersDetails 接口（GET /v1/{project_id}/cloudservers/detail）查询
指定 Region 下的弹性云服务器列表，仅输出 name 与 id。

用法:
    python3 list_ecs.py [-c 配置文件路径] [-r region]
...
ECS_ENDPOINT = "https://ecs.{region}.myhuaweicloud.com/v1/{project_id}/cloudservers/detail"
IAM_PROJECTS_ENDPOINT = "https://iam.myhuaweicloud.com/v3/projects"
PAGE_LIMIT = "1000"


class EcsListError(Exception):
    """ECS 查询错误基类。"""


class ConfigError(EcsListError):
    """配置错误（凭证缺失、配置文件异常等）。"""


class AuthError(EcsListError):
    """认证失败（AK/SK 无效或无权访问）。"""


class ApiError(EcsListError):
    """接口调用异常（网络错误、服务端错误等）。"""
...
def _has_more(total, collected, batch_size):
    """依据 count 总数与已收集数量判断是否还需要翻页。

    ListServersDetails 响应包含 count（总数）字段，采用 offset 分页：
    count 始终由接口返回，作为翻页终止的权威依据——已收集数量未达
    count 且本页非空时继续翻页；count 为 0 或已收集到 count 时停止。
    """
    if not batch_size:
        return False
    if not total:
...
def list_ecs(ak, sk, region, project_id=None):
    """查询指定 Region 的 ECS 实例列表，返回 [{'name':..., 'id':...}, ...]。

    ListServersDetails 采用 offset 分页（offset 从 1 开始），响应含 count
    字段表示实例总数；逐页累加直到收集到 count 或本页不满。
    """
    pid = resolve_project_id(ak, sk, region, project_id)
    base = ECS_ENDPOINT.format(region=region, project_id=pid)
    servers = []
    offset = 1
    while True:
        ...
        if not _has_more(total, len(servers), len(batch)):
            break
        offset += 1
    return [
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="查询华为云 ECS 实例列表，输出实例名称 + ID。")
    parser.add_argument(
    ...
    try:
        ...
        servers = list_ecs(cfg["ak"], cfg["sk"], region, cfg["project_id"])
    except EcsListError as exc:
        print(f"[错误] {exc}", file=sys.stderr)
        return 1
    if not servers:
        print(f"Region {region} 下暂无 ECS 实例。")
        return 0
    print(f"Region {region} 的 ECS 实例列表（共 {len(servers)} 个）：")
