---
name: global-company-person-experience-zh
description: 调取全球企业人员数据库获取目标人员的任职履历和完整工作时间线，充实 B2B 销售线索画像，梳理联系人过往任职经历，研判对方职业发展轨迹。
...
使用跨境魔方开放平台API从全球企业库数据查询人物的历史工作经历信息。

## 概述

本技能提供对跨境魔方全球企业库人物工作经历信息的查询。通过人物ID（hid）获取历史工作经历列表，包括公司名称、职位名称、任职起止时间及任职状态等。可选传入公司ID（pid）筛选指定公司的工作经历。

- `hid`（人物ID）可从 **global-company-person-search**（人物搜索）结果中获取
...
**重要**：始终使用直接脚本调用，如 `python scripts/person_experience_list.py`。**不要使用** shell 复合命令如 `cd scripts && python person_experience_list.py`

### 工作经历列表查询 (`person_experience_list.py`)
- **返回粒度**：每条工作经历记录为一行
- **适用场景**：查看某人的工作经历，了解其任职过的公司
- **示例**：
  - "查询H_67890的工作经历"
  - "查询H_67890在US_12345公司的工作经历"
  - "获取下一页工作经历数据"
- **参数**：查看参数说明 [工作经历列表 API](references/person-experience-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/depth_company/person/experience/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"工作经历查询失败，服务端异常"}'
```
...
| 用户意图 | 使用API |
|---------|--------|
| "查询H_67890的工作经历" | 工作经历列表查询 |
| "查询H_67890在US_12345公司的工作经历" | 工作经历列表查询（带 --pid） |
...
```bash
python scripts/person_experience_list.py --hid H_67890 --cursor '上一次响应返回的cursor字符串'
```
...
- 工作经历列表：查看 [references/person-experience-list-api.md](references/person-experience-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/person-experience-list-api.md](references/person-experience-list-api.md)
...
3. **跨技能使用**：
   - `hid`（人物ID）可从 **global-company-person-search**（人物搜索）结果中获取
   - 工作经历结果中的 `pid`（公司ID）可与 **global-company-search** 或 **global-company-employee** 配合使用，查找公司更多详情

## 注意事项
- 工作经历记录用 `hid` 作为人员唯一标识；`pid` 为公司标识
- `postStatus` 表示任职状态：0-未检测，1-在职，2-离职，3-不确定
- `endDate` 为 0 表示至今（仍在任职）
- 文件路径在所有平台上都使用正斜杠
