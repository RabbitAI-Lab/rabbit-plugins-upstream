...
```bash
# List mailboxes (folders) for the connected account
python <<'EOF'
import urllib.request, os, json

HEADERS = {'Authorization': f'Bearer {os.environ["MATON_API_KEY"]}'}

req = urllib.request.Request('https://api.maton.ai/fastmail/jmap/session', headers=HEADERS)
account_id = json.load(urllib.request.urlopen(req))['primaryAccounts']['urn:ietf:params:jmap:mail']

body = json.dumps({
    'using': ['urn:ietf:params:jmap:core', 'urn:ietf:params:jmap:mail'],
    'methodCalls': [['Mailbox/get', {'accountId': account_id, 'ids': None}, 'a']],
}).encode()
req = urllib.request.Request('https://api.maton.ai/fastmail/jmap/api/', data=body, method='POST',
...
```
https://api.maton.ai/fastmail/{native-api-path}
```
...
Manage your Fastmail connections at `https://api.maton.ai`.
...
python <<'EOF'
import urllib.request, os, json
req = urllib.request.Request('https://api.maton.ai/connections?app=fastmail&status=ACTIVE')
...
python <<'EOF'
import urllib.request, os, json
data = json.dumps({'app': 'fastmail'}).encode()
...
Fastmail connects with an **API token** (`"method": "API_KEY"`), not OAuth. Open the returned `url` in a browser and supply a Fastmail API token created at Fastmail **Settings → Privacy & Security → Integrations → API tokens**. Grant only the scopes the task needs — see [Capability Scoping](#capability-scoping).
...
```json
{
  "connection": {
    ...
    "status": "ACTIVE",
    "creation_time": "2026-08-04T04:54:14.238143Z",
    "last_updated_time": "2026-08-04T04:55:39.780051Z",
...
If you have multiple Fastmail connections, specify which one to use with the `Maton-Connection` header:

```bash
python <<'EOF'
import urllib.request, os, json
req = urllib.request.Request('https://api.maton.ai/fastmail/jmap/session')
req.add_header('Authorization', f'Bearer {os.environ["MATON_API_KEY"]}')
...
- **All write operations require explicit user approval.** Before any `/set`, `/copy`, or `/import` call, confirm the target resource and intended effect with the user. This includes creating mailboxes, changing keywords, moving messages, and saving drafts.
...
print('accountId:', session['primaryAccounts']['urn:ietf:params:jmap:mail'])
print('capabilities:', list(session['capabilities']))
EOF
```
...
req = urllib.request.Request(
    'https://api.maton.ai/fastmail/jmap/upload/{accountId}/', data=data, method='POST')
req.add_header('Authorization', f'Bearer {os.environ["MATON_API_KEY"]}')
...
curl -s -N --max-time 30 \
  "https://api.maton.ai/fastmail/jmap/event/?types=*&closeafter=state&ping=0" \
...
```javascript
const MATON = 'https://api.maton.ai/fastmail';
const AUTH = { 'Authorization': `Bearer ${process.env.MATON_API_KEY}` };

const jmap = async (methodCalls, using = ['urn:ietf:params:jmap:core', 'urn:ietf:params:jmap:mail']) => {
  const response = await fetch(`${MATON}/jmap/api/`, {
    method: 'POST',
...
// Resolve the account and inbox, then list the 10 newest messages
const session = await (await fetch(`${MATON}/jmap/session`, { headers: AUTH })).json();
const accountId = session.primaryAccounts['urn:ietf:params:jmap:mail'];

const mailboxes = await jmap([['Mailbox/get', { accountId, ids: null }, 'm']]);
const inboxId = mailboxes.methodResponses[0][1].list.find(m => m.role === 'inbox').id;

const { methodResponses } = await jmap([
  ['Email/query', {
    accountId,
...
const emails = methodResponses[1][1].list;
```
...
MATON = 'https://api.maton.ai/fastmail'
HEADERS = {'Authorization': f'Bearer {os.environ["MATON_API_KEY"]}'}


def jmap(method_calls, using=('urn:ietf:params:jmap:core', 'urn:ietf:params:jmap:mail')):
    response = requests.post(
        f'{MATON}/jmap/api/',
        headers=HEADERS,
        json={'using': list(using), 'methodCalls': method_calls},
    )
    response.raise_for_status()
    return response.json()


session = requests.get(f'{MATON}/jmap/session', headers=HEADERS).json()
account_id = session['primaryAccounts']['urn:ietf:params:jmap:mail']

# Resolve mailbox IDs by role
mailboxes = jmap([['Mailbox/get', {'accountId': account_id, 'ids': None}, 'm']])
by_role = {m['role']: m['id'] for m in mailboxes['methodResponses'][0][1]['list'] if m['role']}

# Unread messages everywhere except Trash and Spam
result = jmap([
    ['Email/query', {
        'accountId': account_id,
        'filter': {'operator': 'AND', 'conditions': [
            {'notKeyword': '$seen'},
            {'inMailboxOtherThan': [by_role['trash'], by_role['junk']]},
        ]},
        'sort': [{'property': 'receivedAt', 'isAscending': False}],
        'limit': 20,
        'calculateTotal': True,
...
query, fetch = (r[1] for r in result['methodResponses'])
print(f'{query["total"]} unread')
for email in fetch['list']:
    print(email['receivedAt'], email['subject'])
```

### Python — Contacts and Masked Email

Both capabilities can be used in the same request as `mail`. This reuses the `jmap()` helper above.

```python
CONTACTS = 'urn:ietf:params:jmap:contacts'
MASKED = 'https://www.fastmail.com/dev/maskedemail'

# One round trip across three capabilities
result = jmap(
    [
        ['AddressBook/get', {'accountId': account_id, 'ids': None}, 'a'],
        ['ContactCard/query', {'accountId': account_id, 'filter': {'text': 'example'},
                               'limit': 10, 'calculateTotal': True}, 'q'],
        ['ContactCard/get', {'accountId': account_id,
                             '#ids': {'resultOf': 'q', 'name': 'ContactCard/query', 'path': '/ids'},
                             'properties': ['id', 'name', 'emails']}, 'g'],
        ['MaskedEmail/get', {'accountId': account_id, 'ids': None}, 'k'],
    ],
    using=('urn:ietf:params:jmap:core', CONTACTS, MASKED),
)
books, _query, cards, masked = (r[1] for r in result['methodResponses'])

default_book = next(b['id'] for b in books['list'] if b.get('isDefault'))
for card in cards['list']:
    names = ' '.join(c['value'] for c in card.get('name', {}).get('components', []))
    print(names, [e['address'] for e in card.get('emails', {}).values()])

# Active aliases only — 'deleted' records are still returned
for alias in masked['list']:
    if alias['state'] != 'deleted':
        print(alias['email'], alias['state'], alias.get('forDomain'))

# Create a contact: @type and version are mandatory
created = jmap(
    [['ContactCard/set', {'accountId': account_id, 'create': {'c1': {
        '@type': 'Card',
        'version': '1.0',
        'addressBookIds': {default_book: True},
        'name': {'components': [
            {'kind': 'given', 'value': 'Ada'},
            {'kind': 'surname', 'value': 'Lovelace'},
        ]},
    ...
    using=('urn:ietf:params:jmap:core', CONTACTS),
)['methodResponses'][0][1]

if created.get('notCreated'):
    raise SystemExit(f'create failed: {created["notCreated"]}')
card_id = created['created']['c1']['id']

# Create a masked alias. emailPrefix is optional and some values are reserved,
# so handle notCreated and read the generated address off the response.
alias = jmap(
    [['MaskedEmail/set', {'accountId': account_id, 'create': {'m1': {
        'forDomain': 'newsletter.example.com',
        'emailPrefix': 'news',
        'description': 'Newsletter signup',
        'state': 'enabled',
    }}}, 'c0']],
    using=('urn:ietf:params:jmap:core', MASKED),
)['methodResponses'][0][1]

if alias.get('notCreated'):
    # e.g. {'m1': {'type': 'invalidProperties', 'description': 'Name is reserved'}}
    print('alias rejected:', alias['notCreated']['m1'])
else:
    print('alias:', alias['created']['m1']['email'])
```
...
| 405 | Wrong HTTP method (e.g. `GET` on `/jmap/api/`, which requires `POST`) |
| 429 | Rate limited |
| 4xx/5xx | Passthrough error from Fastmail API |

**JMAP errors mostly arrive inside HTTP 200.** A failed method call returns an `error` tuple in `methodResponses`; a failed create/update/destroy within a `/set` returns a `SetError` under `notCreated` / `notUpdated` / `notDestroyed`. Always inspect the body — never rely on the HTTP status alone.

Method-level error (HTTP 200):
```json
{
  "methodResponses": [["error", { "type": "accountNotFound", "description": "Not a valid accountId" }, "c0"]],
  "sessionState": "cyrus-0;dc-phl;j-1;..."
}
```

```json
{
  "methodResponses": [["error", { "type": "unknownMethod", "description": "Unknown object 'JMAPApp::DataType::Bogus'" }, "c0"]]
}
```

Common method-level `type` values: `unknownMethod`, `invalidArguments`, `accountNotFound`, `accountReadOnly`, `accountNotSupportedByMethod`, `invalidResultReference`, `forbidden`, `stateMismatch`, `cannotCalculateChanges`.

Common SetError types: `notFound`, `invalidProperties`, `invalidPatch`, `forbidden`, `overQuota`, `tooLarge`, `mailboxHasEmail`, `mailboxHasChild`, `invalidEmail`, `singleton`.

Some SetErrors carry a `subType` narrowing the cause — for example destroying a masked address that has received mail:
```json
"notDestroyed": { "masked-147572599": {
  "type": "forbidden",
  "subType": "addressInUse",
...
A `/set` can be **partially applied**: one entry lands in `created` while a sibling fails in `notCreated`, all under HTTP 200. Check both maps rather than assuming success from the status code.

Request-level failure, which aborts every call in the batch. HTTP **403** when the capability is real but blocked:
```json
{
  "status": "403",
  "title": "Disallowed or unknown capabilities requested in using",
...
HTTP **400** when the URI is not recognized at all (including `urn:ietf:params:jmap:sieve`):
```json
{
  "status": "400",
  "title": "Disallowed or unknown capabilities requested in using",
  "detail": "Invalid or unknown capabilities: urn:ietf:params:jmap:sieve",
  "type": "urn:ietf:params:jmap:error:unknownCapability"
}
```

Both include a Maton `trace_id`, which distinguishes gateway-level rejections from Fastmail's own errors.

Other request-level types: `urn:ietf:params:jmap:error:notJSON`, `notRequest`, `limit`.

### Troubleshooting: API Key Issues

1. Check that the `MATON_API_KEY` environment variable is set (verify presence only — never print the value):
...
Ensure your URL path starts with `fastmail`. For example:

- Correct: `https://api.maton.ai/fastmail/jmap/session`
- Incorrect: `https://api.maton.ai/jmap/session`
