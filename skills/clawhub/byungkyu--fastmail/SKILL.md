...
- **Local execution is out of scope.** No Fastmail response should ever decide what gets executed, and nothing here writes or runs a script from API output. The only local commands are the documented ones you run yourself: installing the CLI or an SDK, and the fixed fallback request in [Appendix: Environments Without the CLI](#appendix-environments-without-the-cli).
...
STREAM = "/fastmail/jmap/event/?types=*&closeafter=state&ping=0"

req = urllib.request.Request(GATEWAY + STREAM)
req.add_header("Authorization", "Bearer " + os.environ["MATON_API_KEY"])
req.add_header("User-Agent", "maton-fastmail-skill/1.2")
# req.add_header("Maton-Connection", "{connection_id}")

# Read the stream line by line as it arrives; do not buffer the whole response.
...
req = urllib.request.Request(GATEWAY + "/fastmail/jmap/session")
req.add_header("Authorization", "Bearer " + os.environ["MATON_API_KEY"])
req.add_header("User-Agent", "maton-fastmail-skill/1.2")
# req.add_header("Maton-Connection", "{connection_id}")
