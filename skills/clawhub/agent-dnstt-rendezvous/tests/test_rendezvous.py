...
class RendezvousV12Tests(RendezvousTests):
    """v1.2.0 additive surface: card-inspect, compare-status, doctor registry,
    compact --json envelopes, exit code 5."""
    ...
    def _status(self, card_path, card, *, agent="cli-b", role="client",
                state="planned", message="ok", previous=None, out="st.json"):
        pubf = card["server_key_fingerprint"]
        values = {
            "card": str(card_path), "agent_id": agent, "role": role,
            "state": state, "message": message,
            "previous_report": str(self.root / previous) if previous else None,
            "expected_fingerprint": pubf,
            "hmac_env": "LINK_SECRET",
            "output": str(self.root / out), "json": False,
        }
        env = {"LINK_SECRET": SECRET}
        with patch.dict(os.environ, env, clear=False), contextlib.redirect_stdout(io.StringIO()):
            rv.cmd_status(rv.argparse.Namespace(**{**values, "force_output": False,
                                                   "allow_expired": False, "require_hmac": False}))
        return json.loads((self.root / out).read_text(encoding="utf-8"))

    def _card_pair(self):
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False):
            return self.make_card(self.root)

    def test_finish_json_compact_single_line(self):
        card_path, card = self._card_pair()
        buf = io.StringIO()
        args = ns(card=str(card_path), expected_fingerprint=card["server_key_fingerprint"],
                  hmac_env="LINK_SECRET", require_hmac=True, output=None, json=True)
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
        ...
        self.assertTrue(payload.get("op_valid") is None)
        self.assertTrue('", "' not in text)  # compact separators

    def test_card_inspect_compact_summary(self):
        card_path, card = self._card_pair()
        args = ns(card=str(card_path), expected_fingerprint=card["server_key_fingerprint"],
                  hmac_env="LINK_SECRET", require_hmac=True, output=None, json=True)
        buf = io.StringIO()
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
             contextlib.redirect_stdout(buf):
            rv.cmd_card_inspect(args)
        d = json.loads(buf.getvalue())
        self.assertEqual(d["schema"], "agent-dnstt-card-inspect/v1")
        self.assertTrue(d["hmac_authenticated"])
        self.assertFalse(d["contains_secrets"])
        self.assertLessEqual(d["expires_in_seconds"], 1800)
        self.assertEqual(d["tunnel_domain"], "t.example.com")

    def test_card_inspect_rejects_expired_without_waiver(self):
        card_path, card = self._card_pair()
        old = dt.datetime.now(dt.timezone.utc) - dt.timedelta(hours=1)
        card["issued_at"] = rv.iso(old)
        card["expires_at"] = rv.iso(old + dt.timedelta(minutes=10))
        signed = rv.sign_card(card, SECRET.encode())
        card_path.write_text(json.dumps(signed), encoding="utf-8")
        args = ns(card=str(card_path), expected_fingerprint=None, hmac_env="LINK_SECRET",
                  require_hmac=True, output=None, json=True)
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False):
            with self.assertRaises(rv.RendezvousError) as ctx:
                rv.cmd_card_inspect(args)
        self.assertEqual(ctx.exception.code, "expired_card")

    def test_compare_status_topological_head_and_suggestion(self):
        card_path, card = self._card_pair()
        self._status(card_path, card, state="planned", out="s1.json")
        self._status(card_path, card, state="authorized", previous="s1.json", out="s2.json")
        args = ns(card=str(card_path), report=[str(self.root / "s1.json"), str(self.root / "s2.json")],
                  expected_fingerprint=card["server_key_fingerprint"], hmac_env="LINK_SECRET",
                  require_hmac=True, require_consistent=True, output=None, json=True)
        buf = io.StringIO()
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
             contextlib.redirect_stdout(buf):
            rv.cmd_compare_status(args)
        d = json.loads(buf.getvalue())
        self.assertTrue(d["consistent"])
        self.assertEqual(d["chains"][0]["head_state"], "authorized")
        self.assertEqual(d["suggested_next"], "configured")

    def test_compare_status_diverged_heads_exit5(self):
        card_path, card = self._card_pair()
        self._status(card_path, card, state="planned", out="s1.json")
        self._status(card_path, card, state="failed", message="dns path down", out="s3.json")
        args = ns(card=str(card_path), report=[str(self.root / "s1.json"), str(self.root / "s3.json")],
                  expected_fingerprint=card["server_key_fingerprint"], hmac_env="LINK_SECRET",
                  require_hmac=True, require_consistent=True, output=None, json=True)
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), contextlib.redirect_stdout(io.StringIO()):
            with self.assertRaises(rv.ConsistencyError) as ctx:
                rv.cmd_compare_status(args)
        self.assertEqual(ctx.exception.code, "inconsistent_chains")
        self.assertEqual(ctx.exception.exit_code, 5)

    def test_compare_status_partial_chain_issue(self):
        card_path, card = self._card_pair()
        self._status(card_path, card, state="planned", out="s1.json")
        rep = self._status(card_path, card, state="authorized", previous="s1.json", out="s2.json")
        args = ns(card=str(card_path), report=[str(self.root / "s2.json")],
                  expected_fingerprint=card["server_key_fingerprint"], hmac_env="LINK_SECRET",
                  require_hmac=True, require_consistent=False, output=None, json=True)
        buf = io.StringIO()
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
             contextlib.redirect_stdout(buf):
            rv.cmd_compare_status(args)
        ...
        self.assertTrue(any(i.startswith("partial_chain") for i in d["issues"]))
        self.assertEqual(d["chains"][0]["head_state"], "authorized")

    def test_compare_status_rejects_foreign_card_report(self):
        card_path, card = self._card_pair()
        self._status(card_path, card, state="planned", out="s1.json")
        with tempfile.TemporaryDirectory() as tmp2:
            card2_path, card2 = self.make_card(Path(tmp2), output_name="c2.json")
            self._status(card2_path, card2, state="planned", out=str(Path(tmp2) / "o1.json"))
            args = ns(card=str(card2_path), report=[str(self.root / "s1.json")],
                      expected_fingerprint=None, hmac_env="LINK_SECRET",
                      require_hmac=False, require_consistent=False, output=None, json=True)
            with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), contextlib.redirect_stdout(io.StringIO()):
                with self.assertRaises(rv.RendezvousError) as ctx:
                    rv.cmd_compare_status(args)
            self.assertEqual(ctx.exception.code, "status_card_mismatch")

    def test_compare_status_report_bound(self):
        card_path, card = self._card_pair()
        args = ns(card=str(card_path), report=[str(self.root / "x.json")] * (rv.MAX_CHAIN_REPORTS + 1),
                  expected_fingerprint=None, hmac_env=None, require_hmac=False,
                  require_consistent=False, output=None, json=True)
        with contextlib.redirect_stdout(io.StringIO()):
            with self.assertRaises(rv.RendezvousError) as ctx:
                rv.cmd_compare_status(args)
        self.assertEqual(ctx.exception.code, "too_many_reports")

    def test_compare_status_failed_head_takes_suggestion_precedence(self):
        card_path, card = self._card_pair()
        self._status(card_path, card, agent="cli-b", state="planned", out="s1.json")
        self._status(card_path, card, agent="srv-a", role="server", state="failed",
                     message="upstream refused", out="s2.json")
        args = ns(card=str(card_path), report=[str(self.root / "s1.json"), str(self.root / "s2.json")],
                  expected_fingerprint=card["server_key_fingerprint"], hmac_env="LINK_SECRET",
                  require_hmac=True, require_consistent=False, output=None, json=True)
        buf = io.StringIO()
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
            ...
            rv.cmd_compare_status(args)
        d = json.loads(buf.getvalue())
        self.assertTrue(d["consistent"])  # cross-role divergence is informational, not chain fault
        self.assertFalse(d["peer_state_mismatch"])  # pace difference, not failed-vs-progressed
        self.assertEqual(d["suggested_next"], "planned (retry) or closed")

    def test_compare_status_peer_mismatch_failed_vs_progressed(self):
        card_path, card = self._card_pair()
        prev = None
        for i, state in enumerate(("planned", "authorized", "configured", "reachable", "connected")):
            out = f"c{i}.json"
            self._status(card_path, card, agent="cli-b", state=state, previous=prev, out=out)
            prev = out
        self._status(card_path, card, agent="srv-a", role="server", state="failed",
                     message="upstream refused", out="s9.json")
        reports = [str(self.root / n) for n in ("c0.json","c1.json","c2.json","c3.json","c4.json","s9.json")]
        args = ns(card=str(card_path), report=reports,
                  expected_fingerprint=card["server_key_fingerprint"], hmac_env="LINK_SECRET",
                  require_hmac=True, require_consistent=False, output=None, json=True)
        buf = io.StringIO()
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
        ...
        d = json.loads(buf.getvalue())
        self.assertTrue(d["consistent"])
        self.assertTrue(d["peer_state_mismatch"])  # failed server while client progressed
        self.assertEqual(d["suggested_next"], "planned (retry) or closed")

    def test_compare_status_folds_identical_duplicate_reports(self):
        card_path, card = self._card_pair()
        self._status(card_path, card, state="planned", out="s1.json")
        self._status(card_path, card, state="authorized", previous="s1.json", out="s2.json")
        args = ns(card=str(card_path),
                  report=[str(self.root / "s1.json")] * 3 + [str(self.root / "s2.json")] * 2,
                  expected_fingerprint=card["server_key_fingerprint"], hmac_env="LINK_SECRET",
                  require_hmac=True, require_consistent=True, output=None, json=True)
        buf = io.StringIO()
        with patch.dict(os.environ, {"LINK_SECRET": SECRET}, clear=False), \
            ...
            rv.cmd_compare_status(args)
        d = json.loads(buf.getvalue())
        self.assertTrue(d["consistent"])  # duplicates folded, not spurious diverged_heads
        self.assertEqual(d["reports_checked"], 2)

    def test_observer_commands_hmac_by_default(self):
        card_path, card = self._card_pair()
        # drop the hmac field so the card is unsigned
        card.pop("coordination_hmac_sha256", None)
        card_path.write_text(json.dumps(card), encoding="utf-8")
        argv = ["--json", "card-inspect", "--card", str(card_path)]
        args = rv.build_parser().parse_args(argv)
        self.assertTrue(args.require_hmac)  # default for new observer commands
        with self.assertRaises(rv.RendezvousError) as ctx:
            rv.cmd_card_inspect(args)
        self.assertEqual(ctx.exception.code, "hmac_required")
        argv = ["--json", "card-inspect", "--card", str(card_path), "--allow-unsigned-card"]
        args = rv.build_parser().parse_args(argv)
        self.assertFalse(args.require_hmac)
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            rv.cmd_card_inspect(args)
        d = json.loads(buf.getvalue())
        self.assertFalse(d["hmac_authenticated"])

    def test_legacy_verify_status_hmac_default_unchanged(self):
        argv = ["verify-status", "--card", "c.json", "--status", "s.json"]
        args = rv.build_parser().parse_args(argv)
        self.assertFalse(args.require_hmac)  # v1.1.x additive contract preserved

    def test_doctor_registry_surface(self):
        args = ns(card=None, output=None, json=True, expected_fingerprint=None,
                  hmac_env=None, require_hmac=False, allow_expired=False, force_output=False)
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            rv.cmd_doctor(args)
        d = json.loads(buf.getvalue())
        ...
        reg = d["registry"]
        for cmd in ("card-inspect", "compare-status", "doctor", "client-plan"):
            self.assertIn(cmd, reg["commands"])
        self.assertEqual(len(reg["commands"]), 11)
        self.assertEqual(reg["limits"]["max_compare_reports"], rv.MAX_CHAIN_REPORTS)
        self.assertIn("5", reg["exit_codes"])
        self.assertIn("verified", reg["states"])
