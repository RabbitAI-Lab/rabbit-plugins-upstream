...
class ConsistencyError(RendezvousError):
    """Chains verified individually but disagree structurally; exit 5 so
    operator/observer agents can gate on peer-consistency separately from
    validation errors (2), filesystem errors (3), and internal errors (4)."""
    exit_code = 5
...
def write_json(value: Any, output: str | None, *, force: bool = False, announce_path: bool = True) -> None:
    validate_json_value(value)
...
def emit_compact_json(value: Any) -> None:
    """One compact machine-readable object per line — the agent token path."""
    sys.stdout.write(json.dumps(value, separators=(",", ":"), ensure_ascii=False, allow_nan=False) + "\n")


def finish_json(value: Any, args: argparse.Namespace) -> None:
    """Unified result envelope for every subcommand.

    --output keeps writing the canonical pretty JSON file (signatures/HMAC
    tooling read those bytes). --json additionally (or only, when no
    --output) prints one compact object line — the same data, fewer tokens.
    """
    if getattr(args, "json", False):
        if getattr(args, "output", None):
            # identical rejection criteria to the non---json path (validate_json_value
            # + size bound + atomic write); only the stdout form differs
            write_json(
                value, args.output,
                force=getattr(args, "force_output", False),
                announce_path=False,  # stdout is reserved for the compact envelope
            )
        emit_compact_json(value)
        return
    write_json(value, getattr(args, "output", None), force=getattr(args, "force_output", False))
...
def cmd_verify(args: argparse.Namespace) -> None:
    ...
    finish_json({"valid": True, **result}, args)
...
def cmd_card_inspect(args: argparse.Namespace) -> None:
    """One compact machine summary of a verified card — the cheap read path an
    agent takes before deciding whether to re-verify, plan, or renew."""
    card = load_card(Path(args.card))
    verification = verify_card(
        card,
        expected_fingerprint=args.expected_fingerprint,
    ...
    expires = parse_time(card["expires_at"])
    constraints = card["constraints"] if isinstance(card.get("constraints"), dict) else {}
    finish_json(
        {
            "schema": "agent-dnstt-card-inspect/v1",
            "card_id": verification["card_id"],
            "tunnel_domain": verification["domain"],
            "server_listener": card["server_listener"],
            "upstream_service": card["upstream_service"],
            "constraints": constraints,
            "issued_at": verification["issued_at"],
            "expires_at": verification["expires_at"],
            "expires_in_seconds": max(0, int((expires - utcnow()).total_seconds())),
            "expired": verification["expired"],
            "hmac_authenticated": verification["hmac_authenticated"],
            "authorization_ref": verification["authorization_ref"],
            "purpose": verification["purpose"],
            "fingerprint_display": verification["fingerprint_display"],
...
MAX_CHAIN_REPORTS = 64


def cmd_compare_status(args: argparse.Namespace) -> None:
    """Observer-side chain verdict: verify every supplied report, rebuild the
    authenticated chains sharing this card_id, and report per-role head state
    plus structural issues (diverged heads, partial chains, out-of-order links).
    Cross-role disagreement is never a chain fault: a failed-vs-progressed
    role split surfaces as the informational peer_state_mismatch, while mere
    pacing differences (peers at different states) stay unflagged."""
    paths = list(args.report or [])
    if len(paths) > MAX_CHAIN_REPORTS:
        raise RendezvousError(
            f"compare-status accepts at most {MAX_CHAIN_REPORTS} reports per call",
    ...
    card_id = card_verification["card_id"]
    reports = []
    seen_status_ids: dict[str, dict[str, Any]] = {}
    for path in paths:
        raw = load_json_object(Path(path), maximum=MAX_STATUS_BYTES, label="status report")
        result = verify_status(raw, card_id=card_id, secret=secret, require_hmac=args.require_hmac)
        result["previous_status_id"] = raw.get("previous_status_id")
        result["source_name"] = Path(path).name
        sid = result["status_id"]
        if sid in seen_status_ids:
            if seen_status_ids[sid] != result:
                raise RendezvousError("duplicate status_id with differing content", code="status_id_collision")
            continue  # identical report supplied twice (e.g. same path): fold, don't alarm
        seen_status_ids[sid] = result
        reports.append(result)
    if not reports:
        raise RendezvousError("compare-status needs at least one --report file", code="reports_required")
    # structural pass
    references = {r["previous_status_id"] for r in reports if r["previous_status_id"]}
    heads = [r for r in reports if r["status_id"] not in references]
    issues: list[str] = []
    missing_links = [r["status_id"] for r in reports
                     if r["previous_status_id"] and r["previous_status_id"] not in seen_status_ids]
    if missing_links:
        issues.append("partial_chain: " + ",".join(sorted(missing_links)[:8]))
    by_identity: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for r in reports:
        by_identity.setdefault((r["agent_id"], r["role"]), []).append(r)
    chains_summary = []
    diverged = False
    for (agent_id, role), rs in sorted(by_identity.items()):
        ident_heads = sorted(
            (r for r in heads if r["agent_id"] == agent_id and r["role"] == role),
            key=lambda r: r["observed_at"],
        )
        if len(ident_heads) > 1:
            diverged = True
        # the topological head is authoritative; fall back to the newest
        # observed report only when predecessors are missing from the supply
        head = ident_heads[-1] if ident_heads else max(rs, key=lambda r: r["observed_at"])
        chains_summary.append(
            {
                ...
                "head_observed_at": head["observed_at"],
                "heads": len(ident_heads),
            }
        )
    if diverged:
        issues.append("diverged_heads: one agent/role produced multiple chain heads")
    role_latest = {}
    for c in chains_summary:
        prev = role_latest.get(c["role"])
        if prev is None or STATES.index(c["head_state"]) < STATES.index(prev):
            role_latest[c["role"]] = c["head_state"]
    peer_mismatch = (
        "failed" in role_latest.values() and
        any(s in role_latest.values() for s in ("reachable", "connected", "verified"))
    )
    consistent = not issues
    # "failed" is terminal rather than early-progress, so it must take
    # precedence over the weakest progressing peer in the suggestion.
    weakest = (
        "failed"
        if "failed" in role_latest.values()
        else (min(role_latest.values(), key=lambda s: STATES.index(s)) if role_latest else "planned")
    )
    suggested_next = {
        "planned": "authorized (server + client exchange approvals)",
        "authorized": "configured",
        "configured": "reachable",
        "reachable": "connected",
        "connected": "verified",
        "verified": "closed",
        "failed": "planned (retry) or closed",
        "closed": "none — session finished; issue a new card to reconnect",
    }.get(weakest, "planned")
    verdict = {
        "schema": "agent-dnstt-compare-status/v1",
        "card_id": card_id,
        # historical cards stay analyzable; their staleness is evidence, not a gate
        "card_expired": card_verification["expired"],
        "reports_checked": len(reports),
        "chains": chains_summary,
        "consistent": consistent,
        "issues": issues,
        "peer_state_mismatch": peer_mismatch,
        "suggested_next": suggested_next,
        "contains_secrets": False,
    }
    finish_json(verdict, args)
    if getattr(args, "require_consistent", False) and not consistent:
        raise ConsistencyError(
            "status chains are structurally inconsistent",
            code="inconsistent_chains",
            hint="inspect the issues field; reconcile the divergent agents before planning onward steps",
        )
...
def cmd_doctor(args: argparse.Namespace) -> None:
    ...
    # Machine-readable capability registry: the anti-hallucination contract an
    # agent reads before composing calls. Additive keys only — existing
    # "ok"/"checks"/"forbidden_imports" consumers keep working.
    registry = {
        "commands": {
            "keygen-plan": "reviewed key-generation argv; reads no private key",
            "server-card": "create a short-lived public rendezvous card",
            "verify-card": "verify schema, expiry, fingerprint, optional HMAC",
            "server-plan": "reviewed server argv plan (private-key metadata checks)",
            "client-plan": "reviewed client argv plan + public-key handoff",
            "status-report": "create an authenticated secret-free state report",
            "verify-status": "verify one report + optional predecessor link",
            "compare-status": "observer verdict across chains: structure, heads, suggestions",
            "card-inspect": "compact machine summary of a verified card",
            "diagnose": "bounded read-only troubleshooting guidance",
            "doctor": "implementation invariants + this registry",
        },
        "schemas": {"card": SCHEMA, "status": STATUS_SCHEMA, "plan": "agent-dnstt-plan/v1",
                    "canonicalization": CARD_CANON,
                    "compact_envelopes": ["agent-dnstt-card-inspect/v1", "agent-dnstt-compare-status/v1"]},
        "states": list(STATES),
        "roles": list(ROLES),
        "transitions": {k: sorted(v) for k, v in ALLOWED_TRANSITIONS.items()},
        "limits": {
            "max_card_bytes": MAX_CARD_BYTES, "max_status_bytes": MAX_STATUS_BYTES,
            "max_json_depth": MAX_JSON_DEPTH, "max_json_nodes": MAX_JSON_NODES,
            "min_hmac_secret_bytes": 32, "card_ttl_minutes": [5, 1440],
            "clock_skew_minutes": 5, "max_compare_reports": MAX_CHAIN_REPORTS,
        },
        "exit_codes": {
            "0": "success", "2": "validation/policy error (stable error code in stderr JSON w/ --json-errors)",
            "3": "filesystem error", "4": "internal error (fail-closed)",
            "5": "compare-status: chains structurally inconsistent (--require-consistent)",
        },
        "global_flags": ["--debug", "--json-errors", "--json"],
        "env": {"hmac_secret": "32+ byte secret supplied through a named env var; the value is never printed, logged, or written into outputs"},
        "safety_invariants": ["no network or subprocess use", "no private-key reads",
                              "loopback-by-default upstream and listeners",
                              "HMAC-authenticated operational cards by default",
                              "atomic mode-0600 writes, symlink rejection"],
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--json", action="store_true", help="emit one compact machine-readable JSON line instead of pretty output (agent token path)")
    ...
    p = sub.add_parser("card-inspect", help="compact machine summary of a verified card")
    p.add_argument("--card", required=True)
    p.add_argument("--expected-fingerprint")
    p.add_argument("--allow-expired", action="store_true")
    add_card_auth(p)
    # observer read paths are HMAC-by-default: a third-party confidence verdict
    # must not be buildable from unsigned material unless the operator opts out
    p.add_argument("--allow-unsigned-card", action="store_false", dest="require_hmac",
                   help="accept an unsigned card/report (authorized offline workflows only)")
    p.set_defaults(require_hmac=True)
    add_common_output(p); p.set_defaults(func=cmd_card_inspect)

    p = sub.add_parser("compare-status", help="observer verdict across authenticated status chains")
    p.add_argument("--card", required=True)
    p.add_argument("--report", action="append", metavar="PATH",
                   help="status report JSON file (repeat, up to 64)")
    p.add_argument("--expected-fingerprint")
    p.add_argument("--require-consistent", action="store_true",
                   help="exit 5 when chains are structurally inconsistent")
    add_card_auth(p)
    p.add_argument("--allow-unsigned-card", action="store_false", dest="require_hmac",
                   help="accept unsigned cards/reports (authorized offline workflows only)")
    p.set_defaults(require_hmac=True)
    add_common_output(p); p.set_defaults(func=cmd_compare_status)
    return parser
