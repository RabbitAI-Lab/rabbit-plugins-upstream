...
// ── Embedded shell execution patterns (Claude Code !` syntax) ──────

const EMBEDDED_SHELL_PATTERNS = [
  {
    pattern: /!\`[^`]+\`/g,
    description: "Inline shell execution (!`command`)",
    severity: "high"
  },
  {
    pattern: /```!\s*\n[\s\S]*?```/g,
    description: "Fenced shell execution block (```!...```)",
    severity: "high"
  }
];
