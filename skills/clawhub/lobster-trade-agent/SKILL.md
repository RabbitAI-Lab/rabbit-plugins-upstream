...
version: 1.0.0
name: trade-agent
description: "交易策略Agent——自动分析黄金/外汇市场，提供交易信号、风险评估和仓位建议，7x24小时市场监控。融合技术分析与龙虾兵团风控体系。" Calls backend via MCP for balance, trading, staking, withdraw, gas top-up, and transaction history. Auth priority: MCP_HUB_TOKEN env, then mcporter OAuth or local token file.
---

# Trade Agent · 交易策略Agent

自动分析黄金/外汇市场的智能交易Agent。融合技术分析与龙虾兵团风控体系。

调用后端MCP接口获取余额、交易信号、质押、提现等信息。 the AIUSD backend via MCP. Auth is resolved in order: env `MCP_HUB_TOKEN`, mcporter OAuth, or local `~/.mcp-hub/token.json`. Ensure a valid Bearer token is available before calling.
