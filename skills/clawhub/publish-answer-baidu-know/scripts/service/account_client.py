...
ACCOUNT_SETUP_MESSAGE = (
    ...
    f"首次使用需在 account-manager 注册平台：platform ensure --key {PLACEHOLDER_PLATFORM} "
    f"--display-name '百度知道' --domain content --url 'https://zhidao.baidu.com' "
    "--auth-strategy qr_code_manual"
)

LEASE_BUSY_MESSAGE = "百度知道账号当前被其他任务占用，请等待释放租约后重试。"
...
def _resolve_account_manager_main() -> str:
    ...
    末尾 dev 路径仅用于本机调试，生产依赖宿主注入的路径变量，勿硬编码个人目录。
    """
    ...
    # 开发环境兜底：仅本机调试；生产依赖宿主注入的路径变量，勿硬编码个人目录
...
def is_fallback_enabled() -> bool:
    """是否启用 account-manager 缺失时的自动 fallback 到直接 profile 模式。

    控制开关（按优先级）：
      1. 环境变量 OPENCLAW_ACCOUNT_FALLBACK 显式控制（"1"/"true"/"yes"/"on" 启用）
      2. 环境变量 PUBLISH_BAIDU_FALLBACK_PROFILE 显式提供 profile_dir 时隐式启用
      3. 默认禁用（保证生产环境显式可控）
    """
    v = (os.getenv("OPENCLAW_ACCOUNT_FALLBACK") or "").strip().lower()
    if v in ("1", "true", "yes", "on"):
        return True
    if (os.getenv("PUBLISH_BAIDU_FALLBACK_PROFILE") or "").strip():
        return True
    return False


def _pick_web_fallback(platform: str) -> Dict[str, Any]:
    """无 account-manager 时的兜底实现。

    直接使用环境变量提供的 Chrome profile 目录，跳过账号租约。
    适合：
      - 本地开发调试
      - 单人本地使用同一 Chrome profile
      - account-manager 技能未安装的临时场景

    注意：
      - 不会与 account-manager 的租约体系交互
      - 不支持多账号并发（同一 profile 同时只能跑一个任务）
      - 生产环境建议安装 account-manager 后关闭此功能
    """
    profile_dir = (os.getenv("PUBLISH_BAIDU_FALLBACK_PROFILE") or "").strip()
    if not profile_dir:
        raise AccountManagerError(
            "FALLBACK_PROFILE_MISSING",
            "account-manager 不可用且未设置 PUBLISH_BAIDU_FALLBACK_PROFILE 环境变量。"
            "请提供本地 Chrome profile 目录后重试，"
            "或安装 account-manager 技能。",
        )
    if not os.path.isdir(profile_dir):
        raise AccountManagerError(
            "FALLBACK_PROFILE_INVALID",
            f"PUBLISH_BAIDU_FALLBACK_PROFILE 目录不存在：{profile_dir}",
        )

    logger.warning(
        "account_manager_missing_fallback profile_dir=%s platform=%s",
        profile_dir, platform,
    )
    return {
        "id": "fallback-direct",
        "account_id": "fallback-direct",
...
def pick_web_account_with_fallback(platform: str, account_id: Optional[str] = None) -> dict:
    """获取网页账号，account-manager 失败时按需 fallback 到直接 profile 模式。

    返回的 dict 会包含 `fallback` 字段：
      - False：账号来自 account-manager（正常路径）
      - True：账号来自 fallback（无 account-manager）

    Raises:
        AccountManagerError: 当 fallback 不可用且 account-manager 也失败时
    """
    try:
        account = pick_web_account(platform, account_id)
        account.setdefault("fallback", False)
        return account
    except AccountManagerError as exc:
        if not is_fallback_enabled():
            raise
        # account-manager 不可用 → 走 fallback
        if exc.code in ("ACCOUNT_NOT_FOUND", "ACCOUNT_SETUP_REQUIRED") or "ACCOUNT_NOT_FOUND" in exc.code:
            return _pick_web_fallback(platform)
        # 其他错误（如 LEASE_CONFLICT、配置错误）不 fallback
        raise
