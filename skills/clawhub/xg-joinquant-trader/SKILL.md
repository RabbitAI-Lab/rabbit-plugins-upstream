...
name: 小果聚宽量化交易助手
description: |
  小果(微信 xg_quant)聚宽量化交易助手-聚宽（JoinQuant）量化平台策略生成与辅助工具。根据用户需求，生成可在聚宽平台运行的 Python 量化交易策略代码，提供 API 使用指导、常见问题排查与策略优化建议。
  触发关键词：聚宽、JoinQuant、JQ、写策略、生成策略、量化策略、回测代码、模拟交易、双均线、网格交易、因子选股、Alpha因子、多因子、取数据、财务数据、聚宽API、聚宽报错。
  作者：小果
  联系方式：微信 xg_quant
version: 2.0.0
metadata:
...
小果聚宽量化交易助手 本 Skill 是专为聚宽（JoinQuant）量化交易平台设计的策略生成与辅助工具。它能够根据用户的需求描述，自动生成符合聚宽平台规范的 Python 策略代码，并提供完整的 API 使用指导、常见问题排查、策略优化建议以及代码检查服务。

**⚠️ 核心原则：聚宽策略使用 `run_daily()` / `run_weekly()` / `run_monthly()` 定时运行，通过 `history()` / `attribute_history()` / `get_price()` / `get_fundamentals()` 获取数据，使用 `order()` / `order_target()` / `order_target_value()` 下单。所有策略必须包含 `initialize()` 函数。**
...
根据用户描述的交易逻辑，自动生成完整的聚宽策略代码，包括：

| 功能子项 | 说明 |
|---------|------|
| **趋势跟踪策略** | 双均线、MACD、布林带、海龟交易法则等 |
| **均值回归策略** | 网格交易、配对交易、RSI 超买超卖等 |
| **多因子选股策略** | 基本面因子（PE/PB/ROE）、技术因子、Alpha101/191 因子等 |
| **事件驱动策略** | 财报发布、分红送股、指数调仓等事件触发 |
| **期货 CTA 策略** | 商品期货/股指期货的趋势跟踪、套利策略 |
| **期权策略** | 备兑开仓、保护性看跌、价差策略等 |
| **融资融券策略** | 融资买入、融券卖出、多空组合策略 |
| **Tick 级策略** | 高频交易、盘口数据驱动的超短线策略 |
| **组合优化策略** | 风险平价、最大夏普、最小方差等投资组合优化 |

### 2.2 代码检查与优化模块

| 功能子项 | 检查内容 |
|---------|---------|
| **语法检查** | 变量命名、缩进、导入语句是否正确 |
| **API 合规检查** | 是否使用了聚宽支持的 API，是否混用其他平台 API |
| **未来函数检查** | 是否使用了 context.current_dt 之后的数据 |
| **交易合规检查** | 下单前是否检查资金/持仓、是否处理停牌 |
| **性能优化** | 数据缓存建议、循环优化、避免冗余查询 |
| **风险检查** | 是否设置止损、是否过度集中持股 |

### 2.3 数据获取指导模块

| 数据类型 | 推荐 API | 说明 |
|---------|---------|------|
| 历史行情数据 | `get_price()` / `history()` / `attribute_history()` / `get_bars()` | 日/分钟/tick 级别 |
| 实时行情数据 | `get_current_data()` / `get_current_tick()` | 涨跌停价、停牌状态、最新价 |
| 财务数据 | `get_fundamentals()` / `get_fundamentals_continuously()` | 三大报表、财务指标 |
| 指数成分股 | `get_index_stocks()` / `get_index_weights()` | 沪深300/中证500等 |
| 行业/概念数据 | `get_industry_stocks()` / `get_concept_stocks()` / `get_industry()` | 行业分类、概念板块 |
| 标的列表 | `get_all_securities()` / `get_security_info()` | 全市场股票/基金/期货列表 |
| 交易日数据 | `get_trade_days()` / `get_all_trade_days()` | 交易日历 |
| 资金流向 | `get_money_flow()` | 主力/超大单/大单/中单/小单净额 |
| 融资融券 | `get_mtss()` / `get_margincash_stocks()` / `get_marginsec_stocks()` | 两融数据 |
| 龙虎榜 | `get_billboard_list()` | 龙虎榜数据 |
| 基金数据 | `get_extras()` | 基金净值、累计净值 |
| 期货数据 | `get_dominant_future()` / `get_future_contracts()` | 主力合约、可交易合约列表 |

### 2.4 交易执行指导模块

| API 分类 | 函数列表 | 说明 |
|---------|---------|------|
| **订单下单** | `order()` / `order_target()` | 按股数/手数下单 |
| **金额下单** | `order_value()` / `order_target_value()` | 按金额下单 |
| **订单管理** | `cancel_order()` / `get_open_orders()` / `get_orders()` | 撤单、查询订单 |
| **成交查询** | `get_trades()` | 查询当日成交记录 |
| **批量下单** | `batch_submit_orders()` / `batch_cancel_orders()` | 篮子下单/撤单 |
| **账户操作** | `inout_cash()` / `transfer_cash()` | 出入金、账户间转账 |

### 2.5 策略设置指导模块

| 设置项 | API | 说明 |
|-------|-----|------|
| 业绩基准 | `set_benchmark()` | 沪深300/自定义组合 |
| 真实价格模式 | `set_option('use_real_price', True)` | 强烈建议开启 |
| 交易费用 | `set_order_cost()` | 佣金、印花税、最低费用 |
| 滑点设置 | `set_slippage()` | 固定滑点/百分比滑点/跳数滑点 |
| 成交量限制 | `set_option('order_volume_ratio')` | 限制每笔订单成交量占比 |
| 定时运行 | `run_daily()` / `run_weekly()` / `run_monthly()` | 设置策略运行频率和时间 |
| 日志级别 | `log.set_level()` | 设置日志输出级别 |
| 微信推送 | `send_message()` | 模拟交易微信消息推送 |

### 2.6 因子分析与多因子模块

| 功能子项 | 说明 |
|---------|------|
| **Alpha101 因子** | 101 个价量因子，使用 `from jqlib.alpha101 import *` |
| **Alpha191 因子** | 191 个短周期价量因子，使用 `from jqlib.alpha191 import *` |
| **技术分析指标** | MACD、KDJ、RSI、布林带等，使用 `from jqlib.technical_analysis import *` |
| **自定义因子** | 继承 `Factor` 类，实现 `calc()` 方法 |
| **因子计算** | `calc_factors()` 计算自定义因子值 |
| **因子分析** | `analyze_factor()` 单因子分析（IC/IR/分组收益/换手率等） |
| **因子处理** | `neutralize()` 中性化、`winsorize()` 去极值、`standardlize()` 标准化 |
...
| `MinVariance` | 最小化组合方差 |
| `MaxProfit` | 最大化组合收益 |
| `MaxSharpeRatio` | 最大化夏普比率 |
| `MinTrackingError` | 最小化跟踪误差 |
| `RiskParity` | 风险平价配置 |
| `MaxScore` / `MinScore` | 打分最大化/最小化 |
| `MaxFactorValue` / `MinFactorValue` | 因子值最大化/最小化 |
| 约束条件 | 权重约束、行业约束、市场约束、风险暴露约束等 |

### 2.8 回测与模拟交易管理模块

| 功能子项 | 说明 |
|---------|------|
| **创建回测** | 研究中使用 `create_backtest()` 创建回测任务 |
| **获取回测结果** | 研究中使用 `get_backtest()` 获取回测/模拟交易结果 |
| **回测结果分析** | 获取收益曲线、持仓详情、交易详情、风险指标等 |
| **模拟交易管理** | 创建/暂停/重启/关闭模拟交易、替换代码、开启微信通知 |
| **数据持久化** | `write_file()` / `read_file()` 读写研究文件 |
...
def initialize(context):
    g.stocks = ['000001.XSHE', '000002.XSHE', '600000.XSHG', '600036.XSHG']
    set_benchmark('000300.XSHG')
    set_option('use_real_price', True)
    run_daily(market_open, time='every_bar')
...
def market_open(context):
    for security in g.stocks:
        vwap = attribute_history(security, 5, '1d', ['close'])['close'].mean()
        price = get_current_data()[security].last_price

        if price > 1.02 * vwap and context.portfolio.available_cash > 0:
            order_value(security, context.portfolio.available_cash / len(g.stocks))
        elif price < 0.98 * vwap and context.portfolio.positions[security].closeable_amount > 0:
            order_target(security, 0)
#####################################################################
核心 API 详细速查表
4.1 数据获取 API 详解
API	参数	返回值	使用场景
get_price()	security, start_date, end_date, frequency, fields, skip_paused, fq, count	DataFrame/Panel	多标的、多字段历史数据
history()	count, unit, field, security_list, skip_paused, fq	DataFrame	多标的单字段历史数据
attribute_history()	security, count, unit, fields, skip_paused, fq	DataFrame	单标的多字段历史数据
get_bars()	security, count, unit, fields, include_now, end_dt, fq_ref_date	numpy.ndarray	Bar 结构数据
get_current_data()	无	dict	实时数据（涨跌停/停牌/开盘价）
get_current_tick()	security, dt	Tick 对象	Tick 数据
get_fundamentals()	query_object, date, statDate	DataFrame	财务数据查询
get_fundamentals_continuously()	query_object, end_date, count	Panel/DataFrame	多日财务数据
get_index_stocks()	index_symbol, date	list	指数成分股列表
get_index_weights()	index_id, date	DataFrame	指数成分股权重
get_industry_stocks()	industry_code, date	list	行业成分股列表
get_concept_stocks()	concept_code, date	list	概念成分股列表
get_all_securities()	types, date	DataFrame	全市场标的列表
get_security_info()	code	SecurityInfo	单标的信息
get_industry()	security, date	dict	股票所属行业
get_extras()	info, security_list, start_date, end_date	DataFrame	基金净值/期货结算价/ST 状态
get_money_flow()	security_list, start_date, end_date, fields	DataFrame	资金流向数据
get_mtss()	security_list, start_date, end_date, fields	DataFrame	融资融券数据
get_billboard_list()	stock_list, start_date, end_date	DataFrame	龙虎榜数据
get_trade_days()	start_date, end_date, count	list	交易日列表
get_dominant_future()	underlying_symbol, date	str	主力合约代码
get_future_contracts()	security, date	list	可交易合约列表
4.2 交易 API 详解
API	参数	返回值	说明
order()	security, amount, style, side, pindex, close_today	Order	按股数下单
order_target()	security, amount, style, side, pindex, close_today	Order	目标股数下单
order_value()	security, value, style, side, pindex, close_today	Order	按价值下单
order_target_value()	security, value, style, side, pindex, close_today	Order	目标价值下单
cancel_order()	order	Order	撤单
get_open_orders()	无	dict	获取未完成订单
get_orders()	order_id, security, status	dict	获取订单信息
get_trades()	无	dict	获取成交记录
batch_submit_orders()	orders_list	list	篮子下单
batch_cancel_orders()	orders_list	list	篮子撤单
inout_cash()	cash, pindex	None	出入金
transfer_cash()	from_pindex, to_pindex, cash	None	账户间转账
4.3 设置 API 详解
API	参数	说明
set_benchmark()	security	设置基准
set_option()	key, value	设置选项（use_real_price/order_volume_ratio/t0_mode 等）
set_order_cost()	cost, type, ref	设置交易费用
set_slippage()	object, type, ref	设置滑点
set_subportfolios()	[SubPortfolioConfig]	设置子账户
run_daily()	func, time, reference_security	每日定时运行
run_weekly()	func, weekday, time, reference_security	每周定时运行
run_monthly()	func, monthday, time, reference_security	每月定时运行
unschedule_all()	无	取消所有定时任务
log.set_level()	name, level	设置日志级别
send_message()	message, channel	微信消息推送
4.4 对象与属性详解
对象	属性	说明
g	自定义变量	全局变量对象，可持久化保存
context	current_dt, previous_date, portfolio, universe	策略上下文
context.portfolio	available_cash, total_value, positions, returns	账户总览
context.subportfolios[i]	available_cash, total_value, long_positions, short_positions	子账户信息
Position	security, price, total_amount, closeable_amount, avg_cost, value	持仓信息
Order	order_id, security, amount, filled, price, status, commission	订单信息
Trade	trade_id, order_id, security, amount, price, time	成交记录
SecurityUnitData	open, close, high, low, volume, money, factor, paused	行情数据单元
❓ 5. 常见问题与解决方案
5.1 数据相关问题
问题	原因	解决方案
回测结果前后不一致	未开启真实价格模式	set_option('use_real_price', True)
获取的数据包含未来信息	使用了 context.current_dt 之后的数据	确保 end_date <= context.current_dt
停牌股票价格异常	未处理停牌	使用 skip_paused=True 或 get_current_data()[security].paused
财务数据为空	查询日期不当或股票未上市	检查 date 参数，确认股票已上市
获取不到当天的涨跌停价	在非交易时段调用	使用 get_current_data() 在盘中获取
get_price 返回数据不足	股票上市时间晚于开始日期	检查股票上市日期
期货主力合约与其他平台不一致	主力合约判断逻辑不同	了解聚宽主力合约切换规则
指数成分股与预期不符	date 参数设置错误	指定正确的 date 参数
5.2 交易相关问题
问题	原因	解决方案
order 返回 None	股票停牌/退市/资金不足	检查股票状态和可用资金
下单量不是 100 的整数倍	A 股交易规则	系统自动调整，无需处理
模拟交易订单未成交	限价单价格偏离市场价	使用市价单或合理设置限价
期货平今手续费过高	未设置平今费率	set_order_cost(OrderCost(close_today_commission=...))
可用资金为负	市价单滑点导致超买	降低滑点设置或预留安全垫
持仓成本计算异常	未使用真实价格模式	开启 use_real_price
科创板市价单报错	未设置保护价	MarketOrderStyle(limit_price)
5.3 模拟交易与代码替换
问题	解决方案
模拟交易未运行	检查交易位是否到期，是否超过 30 天未登录
替换代码后变量丢失	在 after_code_changed() 中重新初始化
微信收不到消息	检查是否开启微信通知，是否超过每日 5 条限制
模拟交易状态过大（超过 30M）	优化 g 对象，删除不必要的变量
模拟交易进程异常退出	检查是否有 sys.exit()、raise SystemExit、字符串格式化错误
实时模拟交易变延时运行	VIP/SVIP 到期或积分兑换交易位到期
5.4 因子与策略优化
问题	解决方案
因子分析报错 "No objects to concatenate"	检查因子数据索引是否为 DatetimeIndex
自定义因子计算缓慢	减少 max_window，优化 calc 函数
多因子策略过拟合	减少因子数量，增加回测周期
策略回撤过大	增加止损条件、分散投资、降低仓位
✅ 6. 代码检查清单
生成策略后，按以下清单逐项检查：

6.1 基本结构检查
□ 包含 import jqdata
□ 包含 def initialize(context) 函数
□ 调用了 set_benchmark() 设置基准
□ 调用了 set_option('use_real_price', True)
□ 使用了 run_daily() / run_weekly() / run_monthly() 定时运行
□ 至少有一个定时运行的策略函数
6.2 数据获取检查
□ 数据获取 API 的 end_date <= context.current_dt（无未来函数）
□ 使用 skip_paused=True 或处理停牌股票
□ 股票代码格式正确（如 000001.XSHE）
□ 财务数据查询使用正确的 date 或 statDate 参数
6.3 交易执行检查
□ 下单前检查了可用资金（context.portfolio.available_cash）
□ 卖出前检查了可卖数量（position.closeable_amount）
□ 使用 order_target() 时检查了目标股数是否为非负
□ 期货策略设置了正确的账户类型（type='futures'）
6.4 日志与记录检查
□ 使用 log.info() 记录关键交易日志
□ 使用 record() 记录关键数据用于绘图
□ 日志级别设置合理（建议保留默认 debug 级别）
6.5 性能与安全检查
□ 未在策略中保存跨日期的行情数据缓存
□ 未使用 sys.exit() 或 raise SystemExit
□ g 对象中存储的数据大小合理（< 30M）
□ 期货策略设置了保证金比例
📞 7. 联系与支持
项目	信息
作者	小果
微信	xg_quant
平台	聚宽（JoinQuant）量化平台
API 使用咨询、策略优化建议或问题排查，可通过微信联系作者
#####################################################################
...
注意 该函数只在开始模拟交易的时候运行一次, 直接替换/修改代码导致的初始化函数变动并不会在模拟盘中生效 , 应该使用after_code_changed等函数
...
在日级模拟中使用时，如果设置time='9:30'，策略的实际运行时间是9:27~9:30之间。策略内获取到逻辑时间(context.current_dt)仍然是 9:30。 注意只有在使用相同的参照标的时，定时运行函数的优先级别为:run_monthly>run_weekly>run_daily且与函数被注册的顺序无关；handle_data与handle_tick的执行顺序与前述函数无关。用户策略不应该依赖于这些计划任务执行的顺序。
...
reference_security	时间的参照标的代码，字符串类型，默认为‘000001.XSHG’。 如参照 '000001.XSHG'，交易时间为 9:30-15:00；如参照'IF9999.CCFX'，2016-01-01之后的交易时间为 9:30-15:00，在此之前为 9:15-15:15；如参照'A9999.XDCE'，因为有夜盘，因此开始时间为21:00，结束时间为15:00。期货策略一定要修改参考标的，建议修改为对应的主力合约。当time为具体时间时请勿设置此参数
返回值 None
...
设置是否开启动态复权（真实价格）模式，默认是False(主要是为了让旧的策略不会出错)。 是否开启动态复权模式对模拟交易是有影响的，原理参考拆分合并与分红，【API解析】| 动态复权与技术指标。
...
为了让编写代码简单, 通过数据获取函数API 拿到的都是基于当天日期的前复权价格. 比如: 回测运行到了2015-01-01这一天, 那么
history(3, '1d', 'close')
取得的就是你穿越到2015-01-01这一天所看到的前复权价格. 另一方面, 你在不同日期调用数据获取函数API返回的价格可能是不一样的, 因为我们在不同日期看到的前复权价格是不一样的. 所以不要跨日期缓存这些API返回的结果.
...
注意： (1)对期货不生效,对场内基金会生效, 但因场内基金在拆分/合并时除权日披露不标准,目前采用的是折算基准日，和实际除权日可能有差异，鉴于此原因不建议给含有场内基金的策略开启动态复权 (2) 设置 use_real_price 为 True之后, 如下的按天回测的代码是不对的：
...
'pre'
, 前复权(根据'use_real_price'选项不同含义会有所不同, 参见[set_option]), 默认是前复权
None
,不复权, 返回实际价格
'post'
,后复权
...
如果是一支股票, 则返回[pandas.DataFrame]对象, 行索引是[datetime.datetime]对象, 列索引是行情字段名字, 比如'open'/'close'. 比如:
...
如果是多支股票, 则返回[pandas.Panel]对象, 里面是很多[pandas.DataFrame]对象, 索引是行情字段(open/close/…), 每个[pandas.DataFrame]的行索引是[datetime.datetime]对象, 列索引是股票代号. 比如
...
df=True: [pandas.DataFrame]对象, 行索引是[datetime.datetime]对象, 列索引是股票代号. 比如: 如果当前时间是2015-01-07, universe是['000300.XSHG', '000001.XSHE'],
history(2, '1d', 'open')
将返回:
---	000300.XSHG	000001.XSHE
2015-01-05	3566.09	13.21
...
df=False: dict, key是股票代码, value是一个numpy数组[numpy.ndarray], 对应上面的DataFrame的每一列, 例如
history(2, '1d', 'open', df=False)
将返回:
python { '000300.XSHG': array([ 3566.09, 3608.43]), '000001.XSHE': array([ 13.21, 13.09]) }
示例
...
df=True [pandas.DataFrame]对象, 行索引是[datetime.datetime]对象, 列索引是属性名字. 比如: 如果当前时间是2015-01-07,
...
df=False 一个dict, key是基金代号, value是[numpy.ndarray], 比如
...
详细介绍 函数计算公式、API 调用方法，用法注释， 输入输出值详情请见:数据字典 - 技术分析指标.
...
使用方法： 在order/order_target/order_value/order_target_value函数中指定委托类型为停止单。停止单会在标的价格突破（向上或向下）时转化为对应的市价单或限价单
...
因子	因子解释	构造方式	回归系数解释
市场因子(RM)	受市场走势变化造成的不确定性收益率	市场组合收益率减去无风险收益率	当βi>0
，说明在样本期间内，该组合的运行趋势与市场整体运行趋势是一致的，如果大于1，说明该组合可能偏激进型。
规模因子(SMB)	由于上市公司规模不同导致的收益率差异	小市值组合的收益率减去大市值组合的收益率	当si>0
，说明该组合可能偏好于配置小盘股
估值因子(HML)	由于上市公司账面市值比不同导致的收益率差异	较高账面市值比的公司组合收益率减去减低账面市值比的公司组合收益率	当hi>0
，说明该组合可能偏好于配置账面市值比高的公司，也就是价值型的公司
盈利因子（RMW）	由于盈利水平不同造成的收益率差异	高盈利公司组合收益率减去低盈利公司组合收益率	当ri>0
，说明该组合可能偏好于配置盈利高的公司
投资因子（CMA）	由于投资水平的不同造成的收益率差异	投资率低的公司组合收益率减去投资率高的公司组合收益率	当ci>0
...
期权和对应的标的后缀相同，具体的可以查看期权合约资料
其他数据后缀请查询数据字典
发现数据有问题怎么办？
（1）查看下我们的数据常见疑问汇总，也许您认为的问题已经有说明了；
...
（4）更多行情数据的说明请查看数据常见疑问汇总

财务数据不同
（1）财务指标数据有单季度和报告期两种，一般财经网站上提供的财务指标数据都是上市公司披露的原始报告期数据，所以请先确定对比的是相同的数据；
（2）我们同时提供的有单季度的和报告期的数据，单季度和报告期的财务数据，您也可以分别查看使用。例如可以通过下面的方式获取报告期的eps:
image
（3）单季度财务数据提供上市之后的数据（不包含上市之前的），报告期有上市前的财务数据；核查时请注意上市时间；
（4）上市公司公布财报分四种，一季度报、中期年度报、前三季度报、全年年报，统计的周期跨度分别为第一季度、前两个季度、前三个季度、全年，而聚宽考虑到量化分析，所以默认查询的财务指标数据是单季度的；例如同花顺上的单季度数据查询方法，选择按单季度统计，如下图： image （5）市值（valuation）数据中market_cap、pb_ratio、pe_ratio、ps_ratio等由于算法或者数据源不同，各个平台数据不一定完全相同；
（6）公告没有披露的数据为空值；

查询过程有问题的话，请查看数据常见疑问汇总

还有问题请在聚宽社区发帖，详细描述下您的问题，并附上对应的查询代码、结果截图以及预期结果，我们核查下答复您。

JQData，jqdatasdk和jqdata的关系
（1）JQData和jqdatasdk是同一个产品不同的名称，是一个Python库(或模块)，是聚宽提供的数据接口，在您自己搭建的本地环境中使用（可以脱离官网独立使用），具体方法见JQData的API；
（2）jqdata是官网的数据产品，主要在官网使用，具体使用方法见官网的API；
...
JQData使用过程过程有问题怎么处理？
（1）有关账户、权限、价格等方面，可以找添加JQData管理员微信（jqdata02）咨询；
（2）查看JQData常见问题；
（3）在聚宽社区中社区提问题模块发帖提问，详细描述下问题，发下对应的测试代码及结果截图，我们核查后答复您。
...
代码中有随机因素，包括但不限于这些例子：(A)遍历 dict中的元素. 因为dict 是无序存储的, 每次遍历的顺序可能不一样；(B)集合在Python内部通过哈希表实现，其本征无序，输出时所显示的顺序具有随机性；(c)使用的一些Python模块的函数是随机，例如一些机器学习算法；(d)MySQL、sqlalchemy等数据查询出现的随机因素
...
参数优化及并行回测参考教程
多回测运行和参数分析框架
多回测运行和参数分析框架(Python3)
在投资研究中调用回测或者模拟交易，多基准绘图
研究中写策略并回测
区分create_backtest(策略ID)和get_backtest(回测ID)
...
具体策略：可以参考性能分析，查看下哪部分比较耗费时间，然后优化下策略；
模拟交易
...
如何进行模拟交易？
您可以通过以下两种方法进行模拟交易(推荐使用方法一)， 创建模拟交易的前提是已经运行了回测，如果没有回测的话，请先创建回测
...
如果提示没有可用的模拟交易位，请点击模拟交易列表最下面的获取新的模拟交易位，使用积分兑换模拟交易位；请注意模拟交易位是和频率有关的，需要选择对应运行频率的模拟交易位。
模拟交易支持哪些操作？
模拟交易支持暂停、重启、关闭；
...
仔细查看替换代码注意事项
仔细查看怎样替换代码
找到此模拟交易对应的策略，修改策略后并回测；
在模拟交易页面依次点击：代码--->替换代码；
在跳出的窗口中选择需要替换的回测，并点击确定；
查看替换后的代码。
替换代码与修改代码的区别
...
如何开启微信通知？
开启微信通知后，您可以通过微信接收模拟交易的下单信号，也可以通过send_message发送自定义微信内容。
回测不支持微信通知；
盘中开启微信通知，当日不推送微信信号；
模拟交易的下单信号由腾讯微信发送，可能有30s内的延迟；
延时模拟交易不支持微信通知，开启实时运行
默认每天发送自定义消息条数为5，获取更多每日发送自定义消息数据
...
绑定微信账号.png enter image description here

微信通知示例
模拟交易下单信号及异常信号提醒

微信图片_20181029102058.png
...
错误的字符串格式化方式 使用 % 来格式化字符串，可能会遇到类似 ValueError: unsupported format character 的错误，导致进程异常退出。字符串格式化中希望输出 % 的情况如： 收益：%s% ，实际意图是希望输出 收益：0.1% ，但用法可能是错误的，因为输出%时需要对其进行转义。建议使用 str.format 的方式格式化字符串。点击这里了解更多信息。

调用第三方库 部分第三方库是用C语言扩展，直接调用动态库，如：numpy，错误的调用可能会导致段错误，这样策略进程也会异常退出。另有部分第三方库可能创建新的进程、线程、锁等，平台资源有限，所以限制了策略进程所能占用的资源，若占用资源过大，可能导致进程崩溃。

其他不正确的代码书写方式 Python 是动态型语言，很多错误的书写方式是语法检测不出来的，如：错误的使用了字符串格式化方式，这些错误只有当代码运行到该处时才会报错，否则表现为正常运行的情况。

模拟交易或者回测怎么读取本地文件或者保存文件到研究
官网的策略是不能读取您本地文件或数据的，但是我们支持您上传到我们的投资研究中读取，同时也支持连网获取数据，具体使用方法请参考教程在回测及模拟交易中读取研究中数据。将回测或者模拟的数据保存到研究中中的方法贴中也有说明。
模拟交易没有运行？
鼠标放在收益曲线最右侧, 可以看到最后一次运行的时间;
查看模拟交易什么时候开始运行的，一般是当天开启，下一个交易日才运行；
如果是对run_weekly/run_monthly有疑问, 注意这两个方法设置的是第几个交易日, 不是周几/几号 , 开始策略的那一周/月第一个交易日是从开始的那一天开始计算的, 比如本周w五个交易日, 策略从周二开始, 注册运行run_weekly(day=3) , 第一个交易日是周二, 第三个交易日是周四 ,所以第一周是在周四运行, 往后每周的第三个交易日运行。
刷新下页面；
查看模拟交易的状态是否正常，查看日志是否运行失败；
在模拟交易列表中查看对应的模拟交易位到期时间，检查是否已经到期，被关闭；
是否有一个多月没有登录，如果一个月没有登录官网的话，系统会停掉模拟交易，对应的日志也有说明，登录后，下一个交易日即可运行；
查看是不是延时运行的，是的话可以改为实时运行，然后下一个交易日看看；
是不是替换了代码，替换代码注意事项及替换后什么时候生效请查看如何替换代码;
...
加入聚宽会员，模拟交易会实时运行；
或者使用积分兑换的模拟交易位，模拟交易也会实时运行；积分兑换后,再在对应的模拟交易设置页面，将原来的模拟交易位替换成新的模拟交易位即可 tihuan
...
研究内核的选择
在原有 Python2 与 Python 3 内核的基础上， 我们新增加了一个内核， Python2(PacVer 2.0) ，查看详情;
...
研究和回测中都支持哪些第三方Python库
研究和回测（模拟）中都支持哪些第三方Python库

投资研究的使用方法请参考：投资研究使用教程
...
pandas的shift(…., axis=1)在数据集含有nan值时存在异常(同时存在于研究、回测中) 问题复现代码:
import pandas as pd
...
如果需要购买JQData数据，需要您自己联系JQData管理员咨询(微信：jqdata02)；
如果误买的话，也可以联系小秘书申请退订。
...
实盘说明
注意 :一创聚宽已于2023-12-29停止维护，见一创聚宽（实盘）项目终止公告

聚宽目前没有提供任何形式的实盘及交易通道服务。

推荐开户券商： 第一创业证券，开户立享佣金优惠及专属量化服务。开户后可提供其他量化实盘解决方案。具体咨询券商客服。

详情请可扫码开户了解或直接联系第一创业证券账户的客户经理 :
...
当use_real_price=True时使用动态复权数据 , 为False时使用后复权数据	dependencies=[‘open']
聚宽因子库数据	包含质量因子、基础因子、情绪因子、成长因子、风险因子、每股因子等数百个因子数据
详细的因子列表请参考因子库	质量因子: 营业周期、市场杠杆
dependencies = ['OperatingCycle','MLEV']
单季度财务指标因子	每日可看到的最新单季度财务指标。包含市值数据（valuation）、资产负债数据（balance）、现金流数据（cash_flow）、利润数据（income）、财务指标数据（indicator）。
可以直接使用该指标的名称获取数据。详细的指标列表请参考：股票财务数据	获取利润表（income）中的营业收入（operating_revenue）数据
dependencies = [‘operating_revenue']
前 N 季度的财务数据	前1-8季度的单季度财务指标。
包含资产负债数据（balance）、现金流数据（cash_flow）、利润数据（income）、财务指标数据（indicator）。
可以通过在因子后加『_1』的方式， 获取前几个季度的财务指标。
详细的指标列表请参考：股票财务数据	某公司于6月23日发布半年报，当前的逻辑时间是6月24日
operating_revenue 表示第二季度的营业收入
operating_revenue_1 表示第一季度的营业收入。
过去五年的年度财务数据	过去五年的年度财务数据
包含资产负债数据（balance）、现金流数据（cash_flow）、利润数据（income）、财务指标数据（indicator）。
可以通过在因子后加『_y1』的方式， 获取前几年的财务指标。
详细的指标列表请参考：股票财务数据	当前的逻辑时间是2016年9月24日
...
详细的行业列表请参考行业数据	获取聚宽一级能源行业因子
dependencies = [‘HY001’]
概念因子	因子的名称是概念代码，因子值是一个哑变量， 如果某股票属于某个概念，则返回1； 否则，返回0。
详细的概念列表请参考概念数据	获取智能电网概念因子
dependencies = [‘GN028’]
指数因子	因子名称是指数代码， 因子值是一个哑变量， 如果某股票属于某个指数，则返回1； 否则，返回0。
详细的指数列表请参考指数数据	获取沪深300指数因子
dependencies = [‘000300.XSHG’]
资金流因子	即 get_money_flow API 查询的数据。
可以使用的字段包括：change_pct(涨跌幅(%)、net_amount_main(主力净额(万))、net_pct_main(主力净占比(%))、net_amount_xl(超大单净额(万))、net_pct_xl(超大单净占比(%))、net_amount_l(大单净额(万))、net_pct_l(大单净占比(%))、net_amount_m(中单净额(万))、net_pct_m(中单净占比(%))、net_amount_s(小单净额(万))、net_pct_s(小单净占比(%))	获取主力净占比因子
