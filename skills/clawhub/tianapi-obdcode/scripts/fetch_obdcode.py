#!/usr/bin/env python3
"""
TianAPI OBD Code Fetcher
------------------------
查询汽车OBD故障码的中英文含义、适用车型及详细描述。
零外部依赖，仅使用 Python 标准库。

Usage:
    python3 fetch_obdcode.py --key YOUR_KEY --code P0101

Env Config:
    export TIANAPI_OBDCODE_KEY=your_api_key
    # 或在脚本同级目录创建 .env 文件: TIANAPI_OBDCODE_KEY=your_api_key
"""
...
# --- 配置常量 ---
API_ENDPOINT = "https://apis.tianapi.com/obdcode/index"
ENV_KEY_NAME = "TIANAPI_OBDCODE_KEY"
DEFAULT_TIMEOUT = 10
...
class Config:
    ...
    def _exit_with_help(self, msg: str):
        ...
        print(f"💡 提示: 申请 Key -> https://www.tianapi.com/apiview/247", file=sys.stderr)
        sys.exit(1)
...
class TianAPIClient:
    ...
    def fetch_obdcode(self, code: str) -> Dict[str, Any]:
        """
        执行 API 请求
        ...
        params = {
            "key": self.api_key,
            "code": code.strip().upper()  # 确保故障码大写格式
        }
...
def main():
    parser = argparse.ArgumentParser(description="TianAPI OBD Code Fetcher")
    parser.add_argument("--key", help="API Key (可选，优先使用环境变量)")
    parser.add_argument("--code", required=True, help="OBD故障码 (如: P0101)")
    ...
    # 2. 请求
    result = client.fetch_obdcode(args.code)
