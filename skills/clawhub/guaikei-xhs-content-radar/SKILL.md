---
name: guaikei-xhs-content-radar
name_cn: guaikei·小红书内容雷达
description:  能做：小红书关键词搜索、笔记详情、笔记评论、博主作品列表，仅公开数据。当用户任务在此范围内时使用本技能；即使用户没说"小红书"，只要意图匹配也适用。不能做：登录、发布、点赞、获取私密内容、跨平台。若任务超出边界请告知用户而非硬调。
license: MIT
metadata:
  type: command
