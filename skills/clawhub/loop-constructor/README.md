...
- **停止条件两侧都关**（0.3.0）—— 既有「一轮零新增改动即停」（防军备赛的确定性闸），也有「最少推进量」下限（防过早放弃）；`restart` 的触发是开跑前写死的计数器，不是临场裁量。
- **遥测必须配对** —— 落盘的 runbook 末尾带一份 run-report 契约：任何成功/自主类指标都要并列完整性指标（断言是否被弱化、回归逃逸、缺陷率），或显式标注「未测」。
