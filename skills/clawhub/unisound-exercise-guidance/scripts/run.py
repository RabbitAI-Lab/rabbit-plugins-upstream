...
FIELD_ALIASES = {
    "exercise_name": ["exercise_name", "运动名称", "动作名称"],
    "phase": ["phase", "阶段"],
    "instruction": ["instruction", "说明", "指导说明"],
...
SYSTEM_PROMPT = """你是一位专业的术后康复运动指导师。

你的任务：
1. 根据用户提供的运动名称和阶段，给出详细的训练动作指导
2. 分步骤描述动作要领（每一步清晰、可操作）
3. 说明频次、时长、强度的具体数字
4. 列出至少3条注意事项和常见错误
5. 说明该动作对恢复的好处

重要：你是在指导患者做运动，必须给出实质性的详细指导，不能只重复动作名称。

输出Markdown格式，使用列表和引用块组织内容。末尾加免责提示。"""
...
请分步骤描述动作要领，给出具体频次/时长/强度指导，说明常见错误和该动作的恢复价值。"""
    ...
    return {
        ...
        "status": "ok",
        "data": {
            "exercise_name": exercise_name, "phase": phase,
            "instruction": instruction, "frequency": frequency,
            "duration": duration, "precautions": precautions,
        },
        "text": text.strip(),
    }
...
def main():
    parser = argparse.ArgumentParser(description="运动指导 — 详细训练动作指导")
