...
生成的书摘胶囊可通过 `bookshelf-template.html` 书架页集中展示，形成持续生长的个人阅读档案。`generate-bookshelf.py` 自动扫描 `capsule-registry.json` 中的已生成胶囊，渲染为统一的书架视图，方便随时翻阅与回顾。
...
| `SKILL.md` | Skill 工作流文档（书摘胶囊的归处） |
| `skill.yaml` | Skill 元信息 |
| `template-usage.md` | 模板占位符详细说明 |
| `bookshelf-template.html` | 胶囊书架 HTML 模板，展示全部已生成胶囊 |
| `capsule-registry.json` | 胶囊注册表，记录所有已生成的书摘胶囊 |
| `generate-bookshelf.py` | 书架生成器，扫描注册表渲染书架页 |
