#!/usr/bin/env python3
"""
TikTok 达人（Creator）数据 - LinkFox Skill
调用 /tiktokShop/developerProxy 接口。

Usage:
  python creator_proxy.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python creator_proxy.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/tiktokShop/developerProxy"
...
def main():
    ...
    if not argv:
        print(
            "Usage: creator_proxy.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
