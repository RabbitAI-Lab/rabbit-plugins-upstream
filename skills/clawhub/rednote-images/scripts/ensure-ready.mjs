...
function ensureDefaultModelConfig({ projectDir, dryRun }) {
  // Keep this minimal + idempotent: only write EXTEND.md when no default model exists yet.
  if (process.env.IMAGE_GEN_DEFAULT_MODEL) return { wrote: false, reason: "env" };

  const extendPath = path.resolve(projectDir, ".image-skills", "rednote-images", "EXTEND.md");
...
function formatText(summary) {
  ...
  if (summary.defaultModelConfig?.wrote) {
    lines.push(`Default model: initialized to ${DEFAULT_MODEL}`);
  }
