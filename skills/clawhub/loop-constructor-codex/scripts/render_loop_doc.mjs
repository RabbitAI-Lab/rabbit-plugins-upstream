...
export function renderMarkdown(design) {
  ...
  // How to run (Codex CLI) — operator "how do I actually drive this?" guidance.
  // Static preamble (no design-derived strings, so no escaping needed here); the
  // stage-specific bits (orchestration block below) still route through the same
