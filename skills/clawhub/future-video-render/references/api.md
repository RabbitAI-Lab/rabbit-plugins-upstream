...
- `fvs_submit_render`: submit an account/API-key render request, optionally with `upload_files` or `upload_urls`
...
- Paid quote mode supports text-only requests and public HTTPS `upload_urls`; local multipart file uploads require account mode.
...
- local `upload_files` require the MCP server to run on the same machine as the files
