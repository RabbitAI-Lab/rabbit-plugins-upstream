...
Switch opencode between buying from the asale market and using its own subscription, and see which running sessions are still on the old config.

## Trigger Keywords

- make opencode buy from the market
- point opencode at asale
- add an asale provider to opencode
- asale buy opencode
...
Two things about opencode's config are worth knowing, because both were found by
asking the binary rather than the docs:

- The directory is XDG on **every** platform, Windows included:
  `opencode debug paths` answers `C:\Users\<u>\.config\opencode`, not
  `%APPDATA%`.
- opencode accepts both `opencode.json` and `opencode.jsonc`, and **`.jsonc`
  ...
  `.jsonc` when there is none.

The switch refuses to rewrite a config it cannot round-trip: `.jsonc` allows
