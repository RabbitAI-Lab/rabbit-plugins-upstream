...
當 Pipeline Chain 中包含 `target="external_api"` 的 mutation 時，外部 API 呼叫不受 DB transaction 控制。若 Atomic Chain 後續失敗觸發 rollback，已執行的外部 API 呼叫無法自動撤銷。此場景需要 Compensate Pipeline 執行反向操作（例如發送「註冊失敗」通知簡訊）。Compensate 機制為未來擴展點，將在後續版本中實作。
