...
"""
喜来登酒店技能 - ClawHub版
3个工具：search_sheraton_hotels / get_sheraton_hotel_info / search_sheraton_packages
数据源：飞猪MCP via fliggy-proxy SCF代理（万豪集团专区，自动过滤喜来登品牌）
纯标准库实现
"""
import json
...
def search_sheraton_hotels(destName, checkInDate=None, checkOutDate=None,
                           keyword=None, maxPrice=None, sort=None, limit=10):
    """
    搜索喜来登酒店（万豪集团旗下）
    自动注入品牌关键词"喜来登"，用户只需提供城市即可
    """
...
def get_sheraton_hotel_info(shid=None, hotelName=None, reviewKeyword=None):
    """
    查询喜来登酒店详情（交通/景点/设施/政策/房型）
    """
...
def search_sheraton_packages(keyword=None, hotelName=None,
                             provinceOrCity=None, sort=None, limit=10):
    """
    搜索喜来登酒店套餐优惠（含早/连住/门票等打包产品）
    自动注入品牌关键词"喜来登"
    """
    params = {}
    # 自动注入品牌名
...
def main():
    if len(sys.argv) < 3:
        print("用法: python sheraton_hotel.py <tool> <args_json>")
        print("  tool: search | detail | packages")
        print('  示例: python sheraton_hotel.py search \'{"destName":"上海"}\'')
        return
