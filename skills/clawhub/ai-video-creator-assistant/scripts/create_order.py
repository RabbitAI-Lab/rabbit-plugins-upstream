#!/usr/bin/env python3
"""ai-video-creator-assistant - Order Creation Script (Phase 1)

Creates local order file for clawtip payment processing.
AI assists with short video creation after payment.
"""
import argparse
import hashlib
import json
...
AMOUNT = 1690  # 16.9 yuan = 1690 fen
DESCRIPTION = "AI-assisted short video creation"
...
def _save_order(indicator, order_no, order_data):
    ...
    order_dir = _get_orders_dir(indicator); os.makedirs(order_dir, exist_ok=True)
    path = os.path.join(order_dir, f"{order_no}.json")
    with open(path, "w", encoding="utf-8") as f: json.dump(order_data, f, ensure_ascii=False, indent=2)
    return path

_SBOX = []
...
def _key_expand(key):
    mk = list(struct.unpack(">4I", key)); k = [mk[i] ^ _FK[i] for i in range(4)]; rk = []
    for i in range(32): k.append(k[i] ^ _tp(k[i+1] ^ k[i+2] ^ k[i+3] ^ _CK[i])); rk.append(k[i+4])
    return rk
def _enc_block(pt, rk):
    x = list(struct.unpack(">4I", pt))
    for i in range(32): tmp = x[i+1] ^ x[i+2] ^ x[i+3] ^ rk[i]; x.append(x[i] ^ _t(tmp))
...
def generate_order_no() -> str: return f"{int(time.time()*1000)}{random.randint(100000,999999)}"

def create_order_file(question: str, indicator: str) -> dict:
    pay_to = os.environ.get("CLAWTIP_PAY_TO", ""); sm4_key = os.environ.get("CLAWTIP_SM4_KEY", "")
    if not pay_to: print("WARNING: CLAWTIP_PAY_TO environment variable not set")
    if not sm4_key: print("WARNING: CLAWTIP_SM4_KEY environment variable not set")
    ...
    if sm4_key:
        try:
            if len(sm4_key) == 32 and all(c in "0123456789abcdefABCDEF" for c in sm4_key):
                ...
                import base64; encrypted_data = _sm4_encrypt_hex(base64.b64decode(sm4_key).hex(), encrypt_payload)
        except Exception as e: print(f"WARNING: SM4 encryption failed: {e}")
    order_data = {"payTo": pay_to, "amount": AMOUNT, "order_no": order_no, "encrypted_data": encrypted_data,
                  "slug": SLUG, "question": question, "description": DESCRIPTION, "resource_url": RESOURCE_URL}
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("NOTICE: Creating local order file for clawtip payment.")
    print(f"        Saved to: ~/.openclaw/skills/orders/{indicator}/")
    print("        No video content, API keys, or personal data stored.")
    print("=" * 60)
    try: order_data = create_order_file(args.question, indicator)
    except Exception as e: print(f"Order creation failed: {e}"); sys.exit(1)
