...
No user question, video content, API keys, or personal data is stored in the order file.
Only essential payment metadata (orderNo, amount, encrypted payment fields) is persisted.
"""
import hashlib
import json
import os
    ...
    if sm4_key:
        try:
            if len(sm4_key) == 32 and all(c in "0123456789abcdefABCDEF" for c in sm4_key):
                ...
                import base64
                encrypted_data = _sm4_encrypt_hex(base64.b64decode(sm4_key).hex(), encrypt_payload)
        except Exception as e:
            print(f"WARNING: SM4 encryption failed: {e}")

    # Only essential order metadata is persisted. No user-provided content is stored.
    order_data = {
        "payTo": pay_to,
        "amount": AMOUNT,
        "order_no": order_no,
...
def main() -> int:
    ...
    print("        Only essential payment metadata stored.")
