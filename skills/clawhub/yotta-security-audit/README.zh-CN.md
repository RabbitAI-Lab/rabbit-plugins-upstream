...
<p align="center">Python 3.8+ 标准库实现，零依赖；Windows + Linux 通用；只读检测、报告默认脱敏、含授权与法律边界声明。</p>
...
- **威胁捕获模型**：8 检测点（供应链 / 命令执行 / 网络请求与数据外传 / 文件操作与敏感路径访问 / 提示注入 / 远程脚本下载执行 / 编码混淆 / 其他）+ 13 行为项；双视角报告（安全健康度评分 0-100）。
