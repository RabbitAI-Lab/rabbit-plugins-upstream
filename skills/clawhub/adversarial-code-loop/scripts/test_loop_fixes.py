...
def test_merge_failure_returns_infra_and_records_error(tmp_path, monkeypatch):
    ...
    # R1: the infra classification must not be discarded during the public
    # status re-derivation -- a merge failure is not "clean".
    assert final["status"] == "infrastructure_error"


def test_shared_finish_preserves_verdict_exit_and_git_policies(
        tmp_path, monkeypatch):
    calls = []

    def finalize(_workdir, _feature, _parent, verdict, _evidence, **kwargs):
        calls.append((verdict, kwargs["no_merge"]))
        return {"exit_code": 0, "merged": False}

    monkeypatch.setattr(loop.phase_git, "finalize_git", finalize)
    cases = [
        ("APPROVED", loop.EXIT_APPROVED, "APPROVE", [], {}),
        ("REJECT", loop.EXIT_REJECTED, "REJECT", [], {}),
        (
            "ARBITRATED", loop.EXIT_ARBITRATED, "APPROVE",
            ["keep compatibility"], {"arbitrated": True},
        ),
    ]
    for index, (verdict, expected_code, git_verdict, conditions, extra) in enumerate(
            cases):
        out_dir = tmp_path / str(index)
        state = {
            "parent_branch": "main", "branch": f"loop/demo/{index}",
            "completed": [], "findings": [],
        }
        code = loop._finish(
            SimpleNamespace(no_merge=True), str(tmp_path), "demo", out_dir,
            state, verdict, conditions=conditions, extra=extra,
        )
        final = json.loads((out_dir / "final.json").read_text())

        assert code == expected_code
        assert calls[-1] == (git_verdict, True)
        assert final["verdict"] == verdict
        assert final["conditions"] == conditions
        assert final["arbitrated"] is extra.get("arbitrated", False)
        assert state["completed"][-1] == "done"
        assert state["exit_code"] == expected_code


def test_shared_finish_uses_ci_exit_mapping_when_requested(tmp_path, monkeypatch):
    monkeypatch.setattr(
        loop.phase_git,
        "finalize_git",
    ...
    state = {
        "parent_branch": "main", "branch": "loop/demo/1",
        "completed": [], "findings": [],
    }

    code = loop._finish(
        SimpleNamespace(no_merge=True, ci=True, fail_on=None),
        str(tmp_path), "demo", tmp_path, state, "APPROVED",
    )

    assert code == loop.runner.CI_EXIT_CLEAN
    assert json.loads((tmp_path / "final.json").read_text())["status"] == "clean"


def test_phase_failure_preserves_state_without_new_retrospective(
        tmp_path, capsys):
    state = {
        "feature": "demo", "branch": "loop/demo/1", "completed": [],
    }
    code = loop._phase_failed(
        "verify_2", {"error": "provider crashed"},
        state, tmp_path,
    )

    assert code == loop.EXIT_INFRA
    assert state["error"] == "verify_2: provider crashed"
    assert state["phase"] == "verify_2_failed"
    assert not (tmp_path / "ISSUES.md").exists()
    assert "X verify_2 failed: provider crashed" in capsys.readouterr().out


def test_restore_stash_conflict_keeps_recovery_state_and_cleans_worktrees(
        tmp_path, monkeypatch, capsys):
    worktrees = tmp_path / "worktrees"
    worktrees.mkdir()
    (worktrees / "orphan").write_text("leftover", encoding="utf-8")
    state = {
        "parent_branch": "main", "branch": "loop/demo/1",
        "stash_id": "stash@{0}",
    }
    monkeypatch.setattr(loop.gitops, "get_current_branch", lambda _path: "main")

    def conflict(_workdir, _stash_id):
        raise loop.gitops.GitError("stash pop conflict")

    monkeypatch.setattr(loop.gitops, "unstash", conflict)

    result = loop._restore(str(tmp_path), state, tmp_path)

    assert result.branch_restored is True
    assert result.stash_restored is False
    assert result.state_persisted is False
    assert "stash pop conflict" in result.error
    assert state["stash_id"] == "stash@{0}"
    assert not worktrees.exists()
    assert "could not pop stash@{0}" in capsys.readouterr().out
