#!/usr/bin/env bash
# Install adversarial-code-loop with its single dependency: adversarial-common (sibling layout).
# Usage:
#   Bootstrap (from anywhere):  curl -fsSL https://raw.githubusercontent.com/chpomob/adversarial-code-loop/main/scripts/install.sh | bash
#   From a checkout:            bash scripts/install.sh [TARGET_DIR]
# Target layout (siblings required):
#   <TARGET>/adversarial-code-loop            (this skill)
#   <TARGET>/adversarial-common
set -euo pipefail

SKILL_DIR="adversarial-code-loop"
SKILL_NAME="adversarial-code-loop"
...
echo
echo "Sanity check (import adversarial_common + adversarial_loop_v4)..."
SKILL_SCRIPTS_DIR="$TARGET/$SKILL_DIR/scripts"
if [ -n "$HERE" ] && git -C "$HERE" rev-parse --git-dir >/dev/null 2>&1; then
  SKILL_SCRIPTS_DIR="$(git -C "$HERE" rev-parse --show-toplevel)/scripts"
fi
PYTHONPATH="$TARGET/$COMMON_REPO:$SKILL_SCRIPTS_DIR" python3 -c "import adversarial_common, adversarial_loop_v4; print('OK: adversarial_common + adversarial_loop_v4 imported')"
echo "Done."
