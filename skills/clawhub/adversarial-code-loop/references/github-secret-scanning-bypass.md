> **Scope: this bypasses a GitHub security control. Only use it for a secret that is public by design (e.g. a Google installed-app OAuth client_id / client_secret shipped inside the binary). Never use it to push a real secret — rotate and fix instead. Prefer the env-var-fallback alternative in the last section when you can change the code.**
