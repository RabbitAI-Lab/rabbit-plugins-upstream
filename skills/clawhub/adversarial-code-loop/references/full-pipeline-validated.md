...
```
brief → adversarial-spec → spec.md → adversarial-plan → plan.md → one code loop per step with --spec → code
```

All 3 stages completed in 1 cycle each. The implementation stage used separately
launched `--spec` loops. The entire pipeline from brief to 94 passing tests ran in
~3 hours of wall-clock time with 10 implementation steps.
...
## Correction: plan mode was never wired

The original record incorrectly attributed this run to a functional `--plan` mode.
The orchestrator has no such argparse option. The successful result came from running
the ten implementation steps as individual `--spec` loops; it must not be used as
evidence that plan-mode orchestration exists.
