...
3. Launch the code loop:
   ...
   python3 adversarial_loop.py \
     ...
     --review-cmd "...claude-tmux.py --model sonnet --timeout 600 --hard-timeout 1800 --cwd <repo-dir>" \
     --max-loops 2 \
     --timeout 3600 \
     --no-arbiter \
     --feature p1-my-step
...
- **Workdir must be the target repo**, NOT a parent directory
- **claude-tmux --cwd must match the workdir**
...
- **Fable 5 has its own usage limit** separate from Claude Pro (pitfall #33). Switch to Sonnet when hit
