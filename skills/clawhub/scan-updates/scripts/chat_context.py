...
def resolve_group_context(sender_id: str, chat_id: str) -> tuple[dict | None, dict | None]:
    ...
    if user.get("team_id") != team_id or sender_id not in team.get("members", []):
        return None, json_fail("not_team_member", "你不是本群绑定团队的成员，不能扫描该团队资料源。")
    return {
        "binding": binding,
        "team_id": team_id,
        "team": team,
...
def ensure_source_matches_group(source: dict, team_id: str) -> tuple[bool, dict]:
    if source.get("target_scope") != "team" or source.get("target_team_id", "") != team_id:
        return False, json_fail("source_not_in_bound_team", "该资料源不属于本群绑定的团队，不能在这个群里扫描。")
    return True, {}
