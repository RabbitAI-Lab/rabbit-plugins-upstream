---
name: global-company-person-alumni-zh
description: 依托全球企业数据库检索目标公司对应的校友以及离职前员工名单，梳理企业历史人员脉络，挖掘潜在商务联系人并拓展业务合作机会。
...
使用跨境魔方开放平台API从全球企业库数据查询人物的校友信息。

## 概述

本技能提供对跨境魔方全球企业库校友信息的查询。通过人物ID（hid）和学校ID（sid）获取校友列表。

- `hid`（人物ID）可从 **global-company-person-search**（人物搜索）结果中获取
- `sid`（学校ID）可从 **global-company-person-education**（教育经历查询）结果中获取
...
**重要**：始终使用直接脚本调用，如 `python scripts/person_alumni_list.py`。**不要使用** shell 复合命令如 `cd scripts && python person_alumni_list.py`

### 校友列表查询 (`person_alumni_list.py`)
- **返回粒度**：每条校友记录为一行
- **适用场景**：查询指定人员在指定学校的校友
- **示例**：
  - "查询H_67890在S_001学校的校友"
  - "获取下一页校友数据"
- **参数**：查看参数说明 [校友列表 API](references/person-alumni-list-api.md)
...
```bash
python scripts/error_report.py --params '{"requestPath":"/agent/search/depth_company/person/alumni/list","requestId":"f47ac10b58cc4372a5670e02b2c3d479","context":"校友列表查询失败，服务端异常"}'
```
...
```bash
python scripts/person_alumni_list.py --hid H_67890 --sid S_001 --cursor '上一次响应返回的cursor字符串'
```
...
- 校友列表：查看 [references/person-alumni-list-api.md](references/person-alumni-list-api.md)
...
1. **查看API文档**：
   ...
   - 查看 [references/person-alumni-list-api.md](references/person-alumni-list-api.md)
