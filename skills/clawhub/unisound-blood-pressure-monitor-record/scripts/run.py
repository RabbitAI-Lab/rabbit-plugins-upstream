...
FIELD_ALIASES = {
    "systolic": ["systolic", "收缩压", "高压"],
    "diastolic": ["diastolic", "舒张压", "低压"],
    "heart_rate": ["heart_rate", "心率", "脉搏"],
    "unit": ["unit", "单位"],
...
SYSTEM_PROMPT = """你是一位专业的高血压管理助手。

你的任务：
1. 根据血压记录数据，进行结构化记录
2. 按中国高血压防治指南分级：理想(<120/80)、正常(120-129/80-84)、正常高值(130-139/85-89)、1级高血压(140-159/90-99)、2级高血压(160-179/100-109)、3级高血压(≥180/110)
3. 多条记录时分析趋势（上升/下降/波动/稳定）
4. 标注异常值并给出风险提示
5. 给出生活方式建议（限盐、运动、戒烟限酒、减压）

输出自然语言Markdown文本，面向患者，通俗易懂。末尾加免责提示。"""
...
def build(data: Dict[str, Any], appkey: str) -> Dict[str, Any]:
    ...
    for r in raw:
        records.append({
            "systolic": require(r, "systolic"),
            "diastolic": require(r, "diastolic"),
    ...
    user_prompt = f"请分析以下血压监测数据：\n```json\n{json.dumps(records, ensure_ascii=False, indent=2)}\n```"
    text = _call_llm(SYSTEM_PROMPT, user_prompt, appkey)

    return {
        "skill": "血压监测记录",
        "status": "ok",
        "data": {"record_type": "blood_pressure", "record_count": len(records), "records": records},
        "text": text.strip(),
    }
        ...
        for data_row in all_rows[1:]:
            ...
            for key, idx in col_map.items():
                if idx < len(data_row) and data_row[idx].strip():
                    ...
                    except (json.JSONDecodeError, ValueError): rec[key] = val
...
def main():
    parser = argparse.ArgumentParser(description="血压监测记录")
