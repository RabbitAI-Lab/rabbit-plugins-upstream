...
description: "当用户请求“调用 Google Hotels”或“搜索酒店价格/可用性”，或明确提到酒店查询字段时，触发 dataify-google-hotels skill。"
---

# Dataify Google Hotels 中文版
...
```bash
python3 scripts/google_hotels.py --params-json '{"q":"搜索关键词"}'
```

如果用户在对话中提供了 token，使用 `--token` 传递：

```bash
python3 scripts/google_hotels.py --token "USER_TOKEN" --params-json '{"q":"搜索关键词"}'
```
