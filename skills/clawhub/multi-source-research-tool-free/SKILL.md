...
**处理**: 解析数据源覆盖的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回数据源覆盖的响应数据,包含状态码、结果和日志.
### 可信度分类体系
...
| 参数名 | 类型 | 必填 | 说明 |
|---:|---:|---:|---:|
| input | string | 是 | 多源研究助手免费版处理的输入数据或指令 |
...
**处理**: 解析可信度分类体系的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回可信度分类体系的响应数据,包含状态码、结果和日志.
