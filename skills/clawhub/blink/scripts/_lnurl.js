 ...
 */
async function fetchWithRetry(url, { timeoutMs = DEFAULT_LNURL_TIMEOUT_MS, retries = DEFAULT_RETRIES, headers = {} } = {}) {
  let lastErr;
  ...
  throw new Error(`LNURL request failed after ${retries + 1} attempt(s): ${lastErr ? lastErr.message : 'unknown error'}`);
}
