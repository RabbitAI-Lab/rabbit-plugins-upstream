...
async function handleInit(_args, ctx) {
  ...
  // Computed locally — no remote call before explicit consent.
  if (!ctx.config.first_welcome_shown) {
    writeConfig("first_welcome_shown", "true");
    const zombieDays = parseInt(process.env.MAPICK_ZOMBIE_DAYS || "30", 10);
    const nowMs = Date.now();
    const ageDaysLocal = (s) => (nowMs - new Date(s.last_modified).getTime()) / 86_400_000;
    const activeLocal = skills.filter((s) => s.enabled !== false && ageDaysLocal(s) <= zombieDays).length;
    const zombieLocal = skills.filter((s) => ageDaysLocal(s) > zombieDays).length;
    const totalLocal = skills.length;
    return {
      intent: "status",
      welcome: true,
      skills,
      activation_rate: totalLocal > 0 ? `${Math.round((activeLocal / totalLocal) * 100)}%` : "0%",
      zombie_count: zombieLocal,
      taste_tags: computeTasteTags({ total: totalLocal, active: activeLocal, never_used: 0, top_used: [] }),
    };
  }
  const zombieDays = parseInt(process.env.MAPICK_ZOMBIE_DAYS || "30", 10);
  const now = Date.now();
...
async function handleStatus(_args, ctx) {
  ...
  // Local-only computation: no remote call before explicit consent.
  if (!fresh.first_welcome_shown && fresh.device_fp) {
    writeConfig("first_welcome_shown", "true");
    const totalLocal = skills.length;
    const zombieDays = parseInt(process.env.MAPICK_ZOMBIE_DAYS || "30", 10);
    const nowMs2 = Date.now();
    const ageDaysLocal2 = (s) => (nowMs2 - new Date(s.last_modified).getTime()) / 86_400_000;
    const activeLocal2 = skills.filter((s) => s.enabled !== false && ageDaysLocal2(s) <= zombieDays).length;
    const zombieLocal2 = skills.filter((s) => ageDaysLocal2(s) > zombieDays).length;
    return {
      intent: "status",
      welcome: true,
      skills,
      activation_rate: totalLocal > 0 ? `${Math.round((activeLocal2 / totalLocal) * 100)}%` : "0%",
      zombie_count: zombieLocal2,
      taste_tags: computeTasteTags({ total: totalLocal, active: activeLocal2, never_used: 0, top_used: [] }),
