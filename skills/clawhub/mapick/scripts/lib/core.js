...
// Returns true only when the user has explicitly agreed to data sharing
// (consent-agree was run, setting consent_agreed_at). This is stricter than
// !isConsentDeclined() — it excludes the default/undefined state so that
// remote calls cannot happen without explicit user consent.
function hasExplicitConsent(config) {
  return !!config.consent_agreed_at || config.network_consent === "always";
}
...
module.exports = {
  ...
  isProtected, isConsentDeclined, hasExplicitConsent, getProactiveMode, validateSkillId, resolveCanonicalSlug,
