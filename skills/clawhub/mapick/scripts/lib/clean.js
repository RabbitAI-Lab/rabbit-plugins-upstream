...
const {
  ...
  isoNow, isProtected, isConsentDeclined, hasExplicitConsent, validateSkillId,
} = require("./core");
...
async function handleClean(_args, ctx) {
  // Only call backend when user has explicitly agreed to data sharing.
  // Undeclined or not-yet-decided consent → local-only by design.
  if (!hasExplicitConsent(ctx.config)) {
    return {
      intent: "clean",
      local_heuristic: true,
      reason: isConsentDeclined(ctx.config) ? "consent_declined" : "consent_required",
...
function handleBackupRestore(args) {
  ...
  const id = args[0];
  // Validate skill ID before any file system access — prevents path traversal.
  // validateSkillId returns true for valid IDs; return error only for invalid ones.
  if (!validateSkillId(id)) return { error: "invalid_skill_id", skillId: id };
