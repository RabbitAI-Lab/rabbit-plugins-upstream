...
function printUsage() {
  ...
  node bin/api3-feed-manager.js quote-subscription \
    --dapi-name ETH/USD \
    --rpc-url https://arb1.arbitrum.io/rpc \
    --chain arbitrum \
  ...
  node bin/api3-feed-manager.js execute-buy-subscription \
    ...
    --deviation-choice 0.5 \
    --duration 7776000 \
    --private-key-env LIVE_SIGNER_ENV
  ...
  node bin/api3-feed-manager.js deploy-communal-proxy \
    --dapi-name ETH/USD \
    --rpc-url https://arb1.arbitrum.io/rpc \
    --chain arbitrum \
    --private-key-env LIVE_SIGNER_ENV
...
function resolvePrivateKeyInput(options = {}, label = 'options') {
  const { privateKey, privateKeyEnv } = options;
  if (privateKey !== undefined && privateKey !== null && String(privateKey).trim() !== '') {
    const normalized = String(privateKey).trim();
    if (!PRIVATE_KEY_HEX_RE.test(normalized)) {
      throw new Error(`${label}.privateKey must be a valid 32-byte hex private key with or without a 0x prefix`);
    }
    return normalized.startsWith('0x') ? normalized : `0x${normalized}`;
  }

  if (privateKeyEnv === undefined || privateKeyEnv === null || String(privateKeyEnv).trim() === '') {
    return null;
  }

  const envName = String(privateKeyEnv).trim();
  if (!PRIVATE_KEY_ENV_NAME_RE.test(envName)) {
    throw new Error(`${label}.privateKeyEnv must be an uppercase environment variable name`);
  }

  const resolved = process.env[envName];
  if (!resolved || !PRIVATE_KEY_HEX_RE.test(String(resolved).trim())) {
    throw new Error(`${label}.privateKeyEnv did not resolve to a valid 32-byte hex private key with or without a 0x prefix`);
  }
...
async function executePreparedContractCall({ preparedContractCall, rpcUrl, privateKey, privateKeyEnv, submit, acknowledgement, simulationFromAddress }) {
  ...
  try {
    resolvedPrivateKey = resolvePrivateKeyInput({ privateKey, privateKeyEnv }, 'execution');
  } catch (error) {
    executionSummary.failureReason = error.message;
    return { executionSummary, transactionRequest: null, callResult: null };
  }

  if (!rpcUrl || !resolvedPrivateKey) {
    executionSummary.failureReason = 'Execution requires rpcUrl and privateKey or privateKeyEnv.';
    return { executionSummary, transactionRequest: null, callResult: null };
  }
  ...
  const wallet = new Wallet(resolvedPrivateKey, provider);
...
async function deployCommunalProxy(options) {
  ...
  let resolvedPrivateKey = null;
  try {
    resolvedPrivateKey = resolvePrivateKeyInput(options, 'options');
  } catch (error) {
    executionSummary.failureReason = error.message;
    return {
      command: 'deploy-communal-proxy',
...
function formatSubscriptionDurationLabel(seconds, days) {
  if (Number.isFinite(Number(days)) && Number(days) > 0) {
    return `${Number(days)} day${Number(days) === 1 ? '' : 's'}`;
  }

  if (Number.isFinite(Number(seconds)) && Number(seconds) > 0) {
    const derivedDays = Number(seconds) / (24 * 60 * 60);
    if (Number.isInteger(derivedDays)) {
...
function buildPurchaseQuoteSummary(purchasePlan) {
  const chosen = purchasePlan.chosenSubscriptionOption;
  const deviationPercent = purchasePlan.chosenQueueEntry?.deviationThresholdPercent
    ?? chosen?.decodedUpdateParameters?.deviationThresholdPercent
    ?? null;
  const durationSeconds = purchasePlan.chosenQueueEntry?.duration ?? chosen?.duration ?? null;
  const durationDays = purchasePlan.chosenQueueEntry?.durationDays ?? chosen?.durationDays ?? null;
  const price = purchasePlan.chosenQueueEntry?.price ?? chosen?.price ?? null;
  const sponsorWallet = purchasePlan.chosenQueueEntry?.sponsorWallet
    || purchasePlan.queueSubscriptionModel?.sponsorWallet
    || purchasePlan.partialArgumentBundle?.sponsorWallet
    || null;
  const classificationState = purchasePlan.fundingExecutionClassification?.state || null;

  return {
    flow: {
      stage: 'quote',
      nextRecommendedCommand: classificationState === 'executable'
        ? 'prepare-buy-subscription'
        : (classificationState === 'not-needed' ? 'resolve' : 'browser-plan'),
      executionClassification: classificationState,
    },
    feed: purchasePlan.dapiName,
    chain: {
      chainId: purchasePlan.chain?.chainId || null,
      requested: purchasePlan.chain?.requested || null,
      marketChainSlug: purchasePlan.chain?.marketChainSlug || null,
    },
    deviationThresholdPercent: deviationPercent,
    duration: {
      seconds: durationSeconds,
      days: durationDays,
      label: formatSubscriptionDurationLabel(durationSeconds, durationDays),
    },
    price,
    sponsorWallet,
    communalProxy: purchasePlan.communalProxy || null,
    targetContract: purchasePlan.targetContract || null,
    pricingFileUrl: purchasePlan.pricingFile?.url || null,
    pricingRootMatchesOnChain: purchasePlan.pricingFile?.matchesOnChainRegisteredRoot ?? null,
    selectedQueuePriority: purchasePlan.chosenQueueEntry?.queuePriority ?? null,
    availableExecutionModes: purchasePlan.availableExecutionModes || [],
    warnings: [
      purchasePlan.fundingExecutionClassification?.reason,
      ...(purchasePlan.blockers || []),
    ].filter(Boolean),
  };
}

function buildExecutionPreviewSummary(preparedPlan) {
  const quoteSummary = buildPurchaseQuoteSummary(preparedPlan);

  return {
    ...quoteSummary,
    flow: {
      stage: 'prepare',
      nextRecommendedCommand: 'execute-buy-subscription',
      executionClassification: preparedPlan.fundingExecutionClassification?.state || null,
      preparedButNotTransmitted: true,
    },
    selectedExecutionMode: preparedPlan.selectedExecutionMode || null,
    availableExecutionModes: preparedPlan.availableExecutionModes || [],
    sendTransactionSupported: preparedPlan.sendTransactionSupported,
    exactTransactionExecutionSupported: preparedPlan.exactTransactionExecutionSupported,
  };
}

function buildExecutionResultSummary(executionResult) {
  const quoteSummary = buildPurchaseQuoteSummary(executionResult);
  const executionSummary = executionResult.executionSummary || {};

  return {
    ...quoteSummary,
    flow: {
      stage: 'execute',
      nextRecommendedCommand: executionSummary.completed ? 'resolve' : null,
      executionClassification: executionResult.fundingExecutionClassification?.state || null,
    },
    selectedExecutionMode: executionResult.selectedExecutionMode || null,
    availableExecutionModes: executionResult.availableExecutionModes || [],
    dryRun: executionSummary.dryRun ?? null,
    attempted: executionSummary.attempted ?? null,
    submitted: executionSummary.submitted ?? null,
    completed: executionSummary.completed ?? null,
    signerAddress: executionSummary.signerAddress || null,
    txHash: executionSummary.txHash || null,
    failureReason: executionSummary.failureReason || null,
    communalProxyDeployment: executionResult.communalProxyDeployment
      ? {
          command: executionResult.communalProxyDeployment.command,
          sendTransactionSupported: executionResult.communalProxyDeployment.sendTransactionSupported,
          executionSummary: executionResult.communalProxyDeployment.executionSummary,
        }
      : null,
  };
}

async function quoteSubscription(options) {
  const purchasePlan = await purchaseInputs(options);

  return {
    command: 'quote-subscription',
    quoteSummary: buildPurchaseQuoteSummary(purchasePlan),
    chain: purchasePlan.chain,
    dapiName: purchasePlan.dapiName,
    dapiNameBytes32: purchasePlan.dapiNameBytes32,
    communalProxy: purchasePlan.communalProxy,
...
async function purchaseInputs(options) {
  ...
  purchasePlan.quoteSummary = buildPurchaseQuoteSummary(purchasePlan);
  return purchasePlan;
}
...
async function executeBuySubscription(options) {
  ...
  const submit = parseBooleanOption(options.submit, false);
  const deployCommunalProxyAfterBuy = options.deployCommunalProxy === undefined
    ? submit
    : parseBooleanOption(options.deployCommunalProxy, false);
  ...
  if (classificationState && classificationState !== 'executable' && classificationState !== 'not-needed') {
    const blockedExecutionResult = {
      ...
      queueSubscriptionModel: prepared.queueSubscriptionModel,
      chosenQueueEntry: prepared.chosenQueueEntry,
    ...
    blockedExecutionResult.purchaseSummary = buildExecutionResultSummary(blockedExecutionResult);
  ...
  executionResult.purchaseSummary = buildExecutionResultSummary(executionResult);
...
async function prepareContractCall(options) {
  ...
  preparedPlan.purchaseSummary = buildExecutionPreviewSummary(preparedPlan);
  return preparedPlan;
}
...
async function runCli(argv = process.argv.slice(2)) {
  ...
  if (command === 'quote-subscription') {
    const result = await quoteSubscription(args);
    outputResult(command, result, args);
    return;
  }
