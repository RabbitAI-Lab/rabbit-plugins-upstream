...
4. **输出结果**：Excel 导出 + Top10 预览 + NEXT_STEPS_JSON
...
1. **用户说"找商品"**：用默认条件或用户指定条件运行脚本第一轮，输出 Excel
...
7. **禁止生成 HTML 报告**，所有结果只输出 Excel 数据
...
| 每轮 Excel | `<session>/data/scout_round{N}_new_products.xlsx` | 当轮新商品 |
| 全量 Excel | `<session>/data/scout_all_unique_products.xlsx` | 所有唯一 ASIN（`--export-all`） |
