...
```
https://api.maton.ai/basecamp/{resource}.json
```
...
Manage your Basecamp OAuth connections at `https://api.maton.ai`.
...
python <<'EOF'
import urllib.request, os, json
req = urllib.request.Request('https://api.maton.ai/connections?app=basecamp&status=ACTIVE')
req.add_header('Authorization', f'Bearer {os.environ["MATON_API_KEY"]}')
print(json.dumps(json.load(urllib.request.urlopen(req)), indent=2))
EOF
```
...
const response = await fetch(
  'https://api.maton.ai/basecamp/projects.json',
  {
    headers: {
...
- Correct: `https://api.maton.ai/basecamp/projects.json`
- Incorrect: `https://api.maton.ai/projects.json`
