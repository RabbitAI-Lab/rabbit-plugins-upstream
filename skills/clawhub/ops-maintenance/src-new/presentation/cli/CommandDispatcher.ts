...
import type { OpsAction } from '../../config/schemas'
...
export class CommandParser {
   ...
   * 解析剩余参数
   */
  private parseRest(rest: string[]): Partial<ParsedCommand> {
   ...
   */
  private parseTarget(targetStr: string): ParsedCommand['target'] {
  ...
  getHelp(): string {
    ...
    lines.push('| `email-config` | 邮件配置测试 | 发送测试邮件')
