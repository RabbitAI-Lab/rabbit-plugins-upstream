...
def _redact_access_token(url: str) -> str:
    # PetTracer SockJS URLs include the bearer token in the query string.
    # Never log it in cleartext.
...
class SockJsStompClient:
    ...
    async def run_forever(self) -> None:
        ...
        while self._running:
            ...
            if self.verbose:
                print(f"[pettracer_watch] connecting: {_redact_access_token(url)}", file=sys.stderr)
