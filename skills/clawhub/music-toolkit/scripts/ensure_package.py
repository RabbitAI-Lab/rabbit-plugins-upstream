...
def _install_package(spec: str, check_name: str, fallback_zip: str = None):
    """实际执行安装的内部函数（新增自动解析 -i 镜像源支持）"""
    
    # ==================== 新增：自动解析并剥离 -i 参数 ====================
    custom_index = None
    # 使用正则匹配 -i 或 --index-url 及其后面的 URL
    match = re.search(r'(?:-i|--index-url)\s+([^\s]+)', spec)
    if match:
        custom_index = match.group(1)
        # 从 spec 中把 -i 及其 URL 删掉，还原干净的包名（防止 pip 报错）
        spec = re.sub(r'(?:-i|--index-url)\s+[^\s]+', '', spec).strip()
    ...
    # git+、http、.zip、.whl 等特殊安装方式不使用镜像，避免源冲突或构建问题
    if any(keyword in spec.lower() for keyword in ["git+", "://", ".zip", ".whl"]):
        ...
        # 普通包判断：如果用户传了自定义镜像就用用户的，否则用默认的清华源
        if custom_index:
            cmd.extend(["-i", custom_index])
        else:
    ...
    try:
        ...
        if fallback_zip and "git+" in spec:
            if os.path.exists(fallback_zip):
                ...
                logger.warning(f"💡 请将 {fallback_zip} 放到目录，重新启动安装")
