...
> 范围说明：本文档服务于本技能的**商机发现**匹配。原"四、风险规则库 §4.1 围标串标检测规则"
> 仅作背景参考，**本技能不实现、不使用**投标人之间的围标/串标检测（无权限获取他人数据，
> 且信号可被反向利用规避合规审查，已从技能移除）。风险自查统一见 `compliance-guardrails.md`。
...
## 四、合规检查规则库（投标人防御视角，仅时间节点合规）

> 本节仅含时间节点合规检查，供 `compliance_checker.py` 校验使用。围标/串标检测规则已从本技能
> 移除：本技能不实现、不使用投标人之间的围标/串标检测（无权限获取他人数据，且信号可被反向利用
> 规避合规审查）。如需该能力，应走独立的法定监督/审计渠道，而非在投标人侧参谋技能内实现。
