---
name: payment_voucher_ocr
description: 支持支款凭证识别,准确提取支款人、支款方式、用途、金额及经办人签章等信息。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
---
# Sugon-Scnet 支款凭证识别 OCR 技能

本技能封装了支款凭证识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，准确提取支款人、支款方式、用途、金额及经办人签章等信息。

## 功能特性

- **支款凭证识别**：支持识别支款凭证信息，准确提取支款人、支款方式、用途、金额及经办人签章等信息。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• WITHDRAWAL_VOUCHER（支款凭证）|
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/payment_voucher_ocr/scripts/main.py WITHDRAWAL_VOUCHER /path/to/test.jpg
```
...
- “帮我识别这张图像中的支款凭证信息，图片在 /Users/name/Downloads/test.jpg”
