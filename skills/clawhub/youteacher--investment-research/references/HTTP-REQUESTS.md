...
只在两个来源达到 `succeeded`（成功）或有真实证据的 `partial`（部分成功）后，用真实任务 ID 构造本地请求：
...
四个操作通常同步。若返回 HTTP `202`，读取 `task_id` 并查询相同操作：
...
GET 只需 Bearer 鉴权。`queued`（已排队）、`processing`（处理中）从约 2 秒开始指数退避到最长约 30 秒，并限制
总次数与总时长；`succeeded`（成功）、`partial`（部分成功）、`failed`（失败）、`cancelled`（已取消）为终态。达到上限只报告
`task_id` 和当前状态，不新建 POST。

读取 `result.version` 与操作对应结构化字段；`artifacts[]` 只作为平台返回元数据，
