...
只从环境变量读取密钥。不要将密钥放进 JSON、图谱字段、来源、日志、产物、命令参数或回答。

Knowledge Graph 使用平台本地服务提供方，无需第三方连接。若返回 `provider_not_configured`（未配置服务提供方），视为平台部署或产品绑定故障：停止操作并联系平台管理员，不要尝试直连图数据库或其他服务。
