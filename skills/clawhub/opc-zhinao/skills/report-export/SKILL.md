---
name: report-export
description: "诊断完成后一键导出报告，支持Markdown和HTML格式。触发关键词：导出、报告、下载"
---
...
当用户在诊断后引导流程中说以下关键词时触发：
- "导出"、"下载"、"报告"、"保存"
- "生成文档"、"导出报告"

**触发流程**：
1. 用户提出Idea → 智能体诊断 → 输出报告 → 引导用户互动
2. 用户在互动中说"导出" → 触发report-export skill → 生成文档
3. 用户继续探讨 → 继续诊断或深入分析

**⚠️ 禁止主动触发**：
- 不要在诊断完成后自动询问"是否导出报告"
- 而是在引导流程中提供"导出"选项，等待用户主动选择
...
💡 提示：不懂技术也没关系，回复数字后，文件会自动生成到 opc-reports 文件夹
...
**设计目标**：先让用户直观看到整体结果，再吸引阅读后续详细内容，避免长篇文字枯燥。

**首屏结构（左右布局，必须放在报告最顶部，紧跟标题之后）**：
...
**五维度分值来源**：
- 需求可行性：来自Skill1诊断的可行度评分
- 市场空间：来自Skill1诊断的市场空间评分
- 个人匹配度：来自Skill1诊断的个人匹配度评分
- 产品成熟度：根据是否有MVP、是否有付费用户判定（无产品=0，有Idea=30，有MVP=60，有付费用户=80，有复购=100）
- 合规就绪度：根据是否注册主体、是否有合同、是否有财税规划判定（均无=0，有主体=40，有主体+合同=70，全部就绪=100）
...
**可行性等级判定**：
- 80-100分：高度可行（绿灯 #27ae60）
- 60-79分：可行但需突破（黄灯 #f39c12）
- 40-59分：有挑战（橙灯 #e8792b）
- 0-39分：不可行（红灯 #e74c3c）
...
| 序号 | 校验项 | 校验标准 | 不通过处理 |
|------|--------|---------|-----------|
| 1 | 五维度分值均在0-100 | 条形图v0-v4 | 修正越界值 |
| 2 | 综合可行度=加权计算 | 需求×0.4+市场×0.3+匹配×0.3 | 重新计算 |
| 3 | 等级颜色与分数一致 | 80-100绿/60-79黄/40-59橙/0-39红 | 修正颜色 |
| 4 | 条形图宽度与分值一致 | v0-v4宽度正确 | 修正宽度 |
| 5 | AI标识已添加 | 文档末尾有AI生成标识 | 补全标识 |
