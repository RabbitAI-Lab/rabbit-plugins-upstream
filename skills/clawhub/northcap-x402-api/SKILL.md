...
metadata: {"clawbot": {"requires": {"python3": true, "network": ["https://show-zum-anyway-sanyo.trycloudflare.com"], "env": ["X402_API_KEY"]}}}
---

# Northcap Crypto Signals API (x402)

> ⚠️ **WARNING — PAID API CALL:** This skill calls a paid external API. Each call costs money (x402, USDC on Ethereum) and sends your API key (`X402_API_KEY`) plus your request to `https://show-zum-anyway-sanyo.trycloudflare.com`. Only run when you consciously accept the charge. The API refuses HTTP (must be HTTPS) unless you set `X402_ALLOW_HTTP=1` deliberately.
...
## API details
- **Base URL**: `https://show-zum-anyway-sanyo.trycloudflare.com`
