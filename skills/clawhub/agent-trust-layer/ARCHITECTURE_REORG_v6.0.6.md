# Agent Trust Layer (Agent Trust Layer) 架构重组分析 — v6.0.6 校正版

> 分析日期：2026-07-16（基于 v6.0.6 真实运行数据，非 v6.0.2 文档）
> 分析对象：Agent Trust Layer v6.0.6（309 个 src JS 文件，131+ 模块，MCP HTTP 服务 8099 端口）
...
└───────┼──────────────────────┼────────────────┘
        │                      │ :8099
   ┌────▼──────────────────────▼─────────────┐
   │  Agent Trust Layer Engine (v6.0.6)               │
   │  ┌─────────┐  ┌──────────────────────┐  │
   │  │ CLI     │  │ MCP HTTP Server       │  │
...
### 核心设计思路
去掉 WorkBuddy 专用 Skill 层，Agent Trust Layer退化为纯 MCP 协议服务。宿主 agent 通过客户端侧 hook 配置自动注入认知预处理。

### 典型架构图
```
任意 MCP 客户端 → Hook 配置(on_turn_start/think, on_turn_end/memory)
                → MCP connect :8099
                → Agent Trust Layer MCP Server (31 tools, Bearer, 无 Skill 层)
                → Agent Trust Layer Core (不变)
```
...
### 核心设计思路
Agent Trust Layer发布为独立应用（npm 全局包 / Docker / 系统服务），暴露 REST + SSE API，充当认知引擎微服务，多 agent 并发调用。

### 典型架构图
```
任意 Agent → HTTP/gRPC → Agent Trust Layer Agent Service
  ├─ API Gateway (POST /think, GET /health, GET /memory)
  ├─ Agent Trust Layer Engine (懒加载 + 共享会话)
  └─ 持久化 (JSONL/SQLite, namespace 隔离)
安装: npm i -g @yun520-1/agent-trust-layer-agent && heartflow-agent start
```
