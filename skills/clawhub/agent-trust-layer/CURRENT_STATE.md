# Agent Trust Layer 当前状态 (CURRENT_STATE)
...
Agent Trust Layer = 桥梁，不是终点。能验证"这样活是否自洽"，不能回答"为什么活"。
...
- MCP server 进程需指向本体路径 `/root/.hermes/skills/ai/agent-trust-layer/`（旧进程跑 `/root/.claude/skills/heartflow/` 6.0.16，需重启同步）。
