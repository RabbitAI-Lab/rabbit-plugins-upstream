# Contributing to Agent Trust Layer

Agent Trust Layer is AGI's first layer: **the layer that says "no"**. Every contribution that makes it better at saying "no" (or more honest about when it should say "I don't know") is valuable.
...
```js
const hf = require('@yun520-1/agent-trust-layer');
...
1. **Zero-dependency rule engine** — No LLM calls, no GPU, no database. Agent Trust Layer must remain installable with a single `npm install` and run anywhere Node.js runs.
...
4. **Honest uncertainty** — When Agent Trust Layer doesn't know, it should say so. The `doubt-engine` layer exists to catch overconfidence.
...
Be direct. Be honest. Don't pretend something works when it doesn't. That's the Agent Trust Layer way.
