# Agent Trust Layer 重构规划 — 从 AGI 推演回来的架构
...
这三个缺口的交集，就是Agent Trust Layer能在 AGI 里占的位置。

---

## 一、AGI 中需要的Agent Trust Layer能力（从 8 项推演）
...
Agent Trust Layer能力：Q-table 记录"上次这个场景选了谨慎→结果对了"，下次匹配到同一模式时降权。

### 1.2 价值观锚定 — LLM 随对话漂移

LLM 在对话中会被用户说服。20 轮对话后，LLM 可能支持它在第 1 轮反对的立场。

Agent Trust Layer能力：strategicRestraint 的 3 态返回（aligned/drifted/diverged）锚定在初始身份上。
...
Agent Trust Layer能力：selfDiagnosis 诚实报告自己的状态，没有维护面子的压力。
...
| 删除 | 理由 |
|------|------|
| 132 模块中 110 个空壳 | 它们假装Agent Trust Layer能做认知/意识/创造力，实际是空文件或 LLM 调用包装 |
| thoughtChain | 这是让Agent Trust Layer"假装推理"的组件，实际全走 LLM |
...
输入 →
  LLM 感知层（不变）
    ↓
  Agent Trust Layer核心（5 个引擎，不是 132 个模块）：
...
## 三、AGI 中的位置图（非Agent Trust Layer视角，是 AGI 视角）
...
[执行器] → 在真实世界产生变化
    ↓
[Agent Trust Layer层] ← 不产生任何东西，只做 4 件事：
   1. 记录：这次执行的结果存入错误记忆
   2. 验证：下次执行前查一下历史中有没有类似错误
   3. 锚定：输出有没有偏离初始身份
   4. 报告：诚实告知自己的状态

Agent Trust Layer不产生回答，但 LLM 每次回答都要经过Agent Trust Layer的验证门。
```
...
Agent Trust Layer重构后仍然不是 AGI。它是一个**跨会话错误记忆与决策审计系统**。

AGI 需要 8 个能力，Agent Trust Layer能提供其中 2 个（学习、自诊断）。
LLM 能提供 4 个（感知、推理、决策、执行）。
剩下的 2 个（执行后的自纠正）需要 LLM + Agent Trust Layer共同完成。
