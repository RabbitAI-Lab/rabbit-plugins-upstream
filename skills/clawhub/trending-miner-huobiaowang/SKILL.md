---
name: Trending Tender Miner - Huobiaowang
description: 热门标讯挖掘助手-火标网，当用户需要进行行业热度分析、统计Top采购商或聚合近期高频中标记录时调用，调用多维度聚合统计接口，呈现特定行业的市场热度与集中度趋势。
...
- 从环境变量 `ZLBX_API_KEY`
- 从Agent配置文件中读取。
- 申请地址：https://ai.zhiliaobiaoxun.com/?ch=s28 → 手机号登录 → 复制 Key → 配置到环境变量 `ZLBX_API_KEY` 即可。（新用户限时赠200次额度）
