#!/usr/bin/env python3
"""
即刻数据国内猪肉价格实时查询 Skill 脚本。

功能说明：
1. 支持按省份查询猪肉价格，也支持不传省份返回全部地区价格。
2. 按 `--key`、环境变量、脚本目录 `.env` 的优先级读取 AppKey。
3. 调用即刻数据开放接口 `/v1/pork/query` 查询白条肉、精瘦肉、土杂猪、外三元和内三元价格。
...
API_PATH = "/v1/pork/query"
APPKEY_ENV_NAMES = ("JIKE_PORK_QUERY_KEY", "JIKE_APPKEY")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    功能说明：
    1. 解析国内猪肉价格查询参数。
    2. `--province` 可选，不传时返回全部地区价格。
    3. 支持 `--json` 输出接口原始 JSON。

    @param argv 命令行参数列表
    @return argparse.Namespace 解析后的参数对象
    """
    parser = argparse.ArgumentParser(description="国内猪肉价格实时查询 - 即刻数据")
...
def request_api(province: str, appkey: str) -> dict[str, Any]:
    """
    功能说明：
    1. 调用即刻数据国内猪肉价格实时查询接口。
    2. 省份参数可为空；为空时返回全部地区价格。
...
def print_text_result(payload: dict[str, Any]) -> int:
    ...
    print("\n国内猪肉价格实时查询结果\n")
    print_table(["省份", "更新时间", "白条肉", "精瘦肉", "土杂猪", "外三元", "内三元"], rows)
    return 0
...
def main() -> int:
    ...
    if not appkey:
        print("错误: 未找到 AppKey，请先配置环境变量 JIKE_PORK_QUERY_KEY 或 JIKE_APPKEY")
