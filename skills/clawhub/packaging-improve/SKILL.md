---
name: amazon-packaging-improvement
description: >
  从 Amazon 评论中的破损、缺件、开箱和收纳反馈提炼包装改进任务与证据。仅用于包装问题分析；不用于物流索赔、供应商下单或库存执行。
  Requires an ARI API key (ari_live_*).
slug: packaging-improve
displayName: Amazon 包装改进建议
version: 1.4.7
summary: 从评论中的运输和开箱问题提出包装改进
license: MIT
---
...
- 本入口只执行 `product/packaging`，输出模板固定为 `ops_product`。
