...
- `favoriteChange`: `command` 仅允许 `del` 或 `move=<目标目录>`。`del` 省略 `dict` 会从所有目录删除，必须显式确认；移动省略 `dict` 时源目录是“未分类”。
...
| `favoriteAdd` | `0` 成功、`1` 已存在、`9` 失败 |
| `favoriteChange` | `0` 成功、`9` 关键词不存在 |
...
curl --max-time 120 -X POST "${LINKFOX_TOOL_GATEWAY}/sorftime/walmart/keywordResearch" \
