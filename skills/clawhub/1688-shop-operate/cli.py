...
from _const import ENV_FILE, LOG_FILE
...
def cmd_authorize(args: list[str]) -> int:
    ...
    from _const import CLIENT_ID, ENV_CLIENT_ID
    from secure_store import load_metadata
    final_client_id = client_id or load_metadata(ENV_CLIENT_ID, ENV_FILE) or CLIENT_ID
    mod = _import_authorize()
    return mod.run_oauth_mode(scope, final_client_id, timeout, ENV_FILE)


def _run_authorize(scope: str) -> int:
    """内部辅助：直接调用 authorize 模块发起 OAuth 授权。"""
    from _const import CLIENT_ID, ENV_CLIENT_ID
    from secure_store import load_metadata
    client_id = load_metadata(ENV_CLIENT_ID, ENV_FILE) or CLIENT_ID
    mod = _import_authorize()
    return mod.run_oauth_mode(scope, client_id, 300, ENV_FILE)
...
def cmd_get_ak(args: list[str]) -> int:
    """通过 browser_use 获取 1688 AK（Access Key）"""
    timeout = 300
    for i, arg in enumerate(args):
    ...
    mod = _import_authorize()
    return mod.run_ak_mode(timeout, ENV_FILE)
