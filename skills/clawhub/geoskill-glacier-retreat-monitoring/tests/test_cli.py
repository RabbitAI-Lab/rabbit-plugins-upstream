"""CLI argument parsing tests for glacier-retreat-monitoring."""
import os
import subprocess
import sys

sys.path.insert(0, os.path.dirname(__file__))
from conftest import mod as gr, SCRIPT
...
class TestCLIBasics:
    ...
    def test_bad_ndates_exit_2(self):
        r = run_cli(["--bbox", "86", "28", "87", "29", "--synthetic",
                     "--n-dates", "zz"])
        assert r.returncode == 2
    ...
    def test_synthetic_runs(self, tmp_path):
        ...
        assert os.path.exists(os.path.join(out, "glacier_boundaries.geojson"))
        assert os.path.exists(os.path.join(out, "glacier_retreat.json"))
