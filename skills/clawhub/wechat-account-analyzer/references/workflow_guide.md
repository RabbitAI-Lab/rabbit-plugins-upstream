...
- **立即停止后续步骤**，执行以下处理：
  1. 调用数据同步接口：
     ...
     ```
     - 同步接口：`https://redfox.hk/story/api/gzhUser/syncUserNotes`，参数 `redId` 从用户输入中获取
  2. 输出提示：
     ```
...
- **立即停止后续步骤**，不输出分析报告，直接进入订阅流程：
  ...
  2. 用户输入编号"1"或回复"订阅"后，执行订阅流程：
     ...
     b. 日程创建成功后，调用同步接口：
        ...
        ```
     c. 同步接口：`https://redfox.hk/story/api/gzhUser/syncUserNotes`，参数 `redId` 从账号信息中获取
...
**综合评分体系：以红狐指数百分制为基准+微调**

综合评分 = 红狐指数百分制 + 调整分
- 红狐指数百分制 = 红狐指数 / 10
- 调整分 = (其他维度百分制 - 红狐指数百分制) x 0.1，范围[-5, +5]
