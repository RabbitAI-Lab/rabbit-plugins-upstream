...
class HeartFlowConfig {
  ...
  _mergeFile(filePath, source) {
    try {
      ...
      const filtered = this._filterKeys(parsed, DEFAULTS);
      this._deepMerge(this._values, filtered);
      this._sources.push({ file: filePath, source });
    } catch (_) { /* skip invalid config files */ }
  }

  _filterKeys(input, schema) {
    if (input === null || typeof input !== 'object') return input;
    if (Array.isArray(input)) return input;
    const out = {};
    for (const key of Object.keys(input)) {
      if (!Object.prototype.hasOwnProperty.call(schema, key)) continue;
      out[key] = this._filterKeys(input[key], schema[key]);
    }
    return out;
  }
  ...
  _mergeEnv() {
    const envMap = {
      ...
      // 记忆系统
      HEARTFLOW_MEMORY_DECAY: ['memory', 'decayEnabled'],
      HEARTFLOW_MEMORY_DECAY_RATE: ['memory', 'decayRate'],
      HEARTFLOW_MAX_CORE: ['memory', 'maxCoreSize'],
      HEARTFLOW_MAX_LEARNED: ['memory', 'maxLearnedSize'],
      // 自愈 RL
      HEARTFLOW_RL_EPSILON: ['selfHealing', 'epsilon'],
      HEARTFLOW_RL_EPSILON_DECAY: ['selfHealing', 'learningRate'],
      HEARTFLOW_ENABLE_SELF_MODIFICATION: ['selfHealing', 'reflectionEnabled'],
      // 功能开关
      HEARTFLOW_CODE_EXECUTOR_ENABLED: ['features', 'codeExecution'],
      HEARTFLOW_DAEMON: ['daemon'],
      // 调试
      HEARTFLOW_DEBUG: ['debug'],
      HEARTFLOW_ENV: ['env'],
    };
