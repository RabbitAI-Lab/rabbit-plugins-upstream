...
class MetaEngine {
  ...
  /**
   * 自我编辑 - 更新技能描述
   * 安全: 只允许 ~/.heartflow/skills/ 目录下的文件
   * [A01] 安全修复: 使用 fs.realpathSync 替代 path.resolve，防止符号链接绕过路径检查
   */
  async selfEdit(skillPath, updates) {
    try {
      // Use module-level path (avoid redundant require)
      const homedir = require('os').homedir();
      const allowedDir = path.resolve(process.env.HF_SKILLS_DIR || path.join(homedir, '.heartflow', 'skills')) + path.sep;

      // 使用 fs.realpathSync 解析真实路径，消除所有符号链接（防止通过符号链接逃逸）
      ...
      let content = await fsPromises.readFile(realPath, 'utf8');
      ...
      await atomicWrite(realPath, content);
