...
function bootCheck(silent = false) {
  ...
  if (!silent) {
    ...
    console.log(`  Health: ${health.score}/100 (${health.grade}) — ${suggestions.length} fixable issue(s)`);
    ...
    if (versionCheck.consistent) {
      console.log(`  ✓ Version consistent: ${versionCheck.expectedVersion}`);
    } else {
    ...
    for (const [dim, info] of Object.entries(health.breakdown)) {
      console.log(`    ${dim}: ${info.earned}/${info.max} — ${info.detail}`);
    }
