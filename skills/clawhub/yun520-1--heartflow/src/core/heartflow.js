...
const allowedRoutes = require('./allowed-routes');
// ─── 惰性模块加载系统（提取到 lazy-modules.js） ────────────────────────
...
// ─── 惰性模块加载系统（提取到 lazy-modules.js） ────────────────────────
const { _lazy } = require('./lazy-modules');
...
const _ThreePoisonsGuard = _lazy('threePoisonsGuard', () => require('../emotion/three-poisons-guard.js'));
...
const _BuddhistPhilosophy = _lazy('buddhistPhilosophy', () => require('./buddhist-philosophy-computation.js'));
const _TrueBeingEngine = _lazy('trueBeingEngine', () => require('./true-being-engine.js'));
...
const _CognitiveRestructuring = _lazy('cognitiveRestructuring', () => require('../psychology/cognitive-restructuring.js'));
const _BreathingExercise = _lazy('breathingExercise', () => require('../psychology/breathing-exercise.js'));
const _EmotionalCheckIn = _lazy('emotionalCheckIn', () => require('../psychology/emotional-check-in.js'));
const _EmpathyDetector = _lazy('empathyDetector', () => require('../psychology/empathy-detector.js'));
const _GroundingTechnique = _lazy('groundingTechnique', () => require('../psychology/grounding-technique.js'));
const _PauseAndReflect = _lazy('pauseAndReflect', () => require('../psychology/pause-and-reflect.js'));
const _SelfCompassionScript = _lazy('selfCompassionScript', () => require('../psychology/self-compassion-script.js'));
const _AIHumanIdentity = _lazy('aiHumanIdentity', () => require('./ai-human-identity.js'));
// v5.8.2+ 新增：意识深化 + 需求引擎 + 社会连接
const _IITPhiEngine = _lazy('iitPhiEngine', () => require('../core/consciousness/iit-phi-engine.js'));
const _SelfModelGrowth = _lazy('selfModelGrowth', () => require('../core/self-model-growth.js'));
const _SDTNeedsEngine = _lazy('sdtNeedsEngine', () => require('../core/sdt-needs-engine.js'));
const _SocialBond = _lazy('socialBond', () => require('../core/social-bond.js'));
const _AgencyEngine = _lazy('agencyEngine', () => require('../core/agency-engine.js'));
const _DualSystemReasoning = _lazy('dualSystemReasoning', () => require('../core/dual-system-reasoning.js'));
const _PADUtils = _lazy('padUtils', () => require('../emotion/pad-utils.js'));
const _GoalTree = _lazy('goalTree', () => require('../planner/goal-tree.js'));
const _SelfRegulationFeedback = _lazy('selfRegulationFeedback', () => require('../cortex/self-regulation-feedback.js'));
...
const _DesireSystem = _lazy('desireSystem', () => { try { return require('../emotion/desire-cognition.js'); } catch(e) { return { DesireSystem: class { constructor() {} } }; } });
...
class HeartFlow {
  constructor(config = {}) {
    ...
    // [P2 FIX] Instance-scoped ALLOWED_ROUTES (was static, shared across all instances)
    this._allowedRoutes = new Set(HeartFlow._DEFAULT_ALLOWED_ROUTES);
  }
  ...
  start() {
    ...
    try {
      this.platformAdapter = _PlatformAdapter().createAdapter(process.env.HF_PLATFORM || 'universal');
    } catch (e) {
    ...
    this._lazy = {
      ...
      desireSystem: { lazy: true, path: '../emotion/desire-cognition.js', Ctor: 'DesireCognition', args: {} },
      ...
      userModel:       { lazy: true, path: '../identity/user-model.js',        Ctor: 'UserModel',         args: {} },
      actionTracker:   { lazy: true, path: './action-tracker.js',    Ctor: 'ActionTracker',     args: {} },
      purposeEngine:   { lazy: true, path: '../identity/purpose-engine.js',    Ctor: 'PurposeEngine',     args: {} },
      // v3.4.4 — 风险分析/自适应控制/意图追踪/审计日志
      riskAnalyzer:    { lazy: true, path: '../reasoning/risk-benefit-analyzer.js', Ctor: 'RiskBenefitAnalyzer', args: {} },
      ...
      auditLogger:     { lazy: true, path: '../shield/audit-logger.js',       Ctor: 'AuditLogger',       args: {} },
    ...
    // ─── Proactive Layer — 主动引擎 ─────────────────────────────────────────
    try { this.curiosityEngine = new (_CuriosityEngine().CuriosityEngine)(); } catch (e) { /* curiosityEngine optional */ }
    try { this.desireEngine = new (_DesireEngine().DesireEngine)(); } catch (e) { /* desireEngine optional */ }
    try { this.goalPursuer = new (_GoalPursuer().GoalPursuer)(); } catch (e) { /* goalPursuer optional */ }
    try { this.selfInitiator = new (_SelfInitiator().SelfInitiator)(); } catch (e) { /* selfInitiator optional */ }

    // ─── 升级心理学引擎（v5.8.1+ 增强） ──────────────────────────────────────
    try {
      const { AIPsychologyEngine } = _AIPsychologyEngine();
      this.aiPsychologyEngine = new AIPsychologyEngine(this);
    } catch (e) { this.aiPsychologyEngine = null; }
    try {
      const { cognitiveRestructuring } = _CognitiveRestructuring();
      this.cognitiveRestructuring = cognitiveRestructuring;
    } catch (e) { this.cognitiveRestructuring = null; }
    try {
      const mod = _BreathingExercise();
      this.breathingExercise = mod.breathingExercise || null;
    } catch (e) { this.breathingExercise = null; }
    try {
      const mod = _EmotionalCheckIn();
      this.emotionalCheckIn = mod.emotionalCheckIn || null;
    } catch (e) { this.emotionalCheckIn = null; }
    try {
      const { detectEmpathy, computeEmpathyScore, detectEmotionalContagion, detectEmpathicConcern, detectPerspectiveTaking, detectSelfOtherDistinction } = _EmpathyDetector();
      this.empathyDetector = { detectEmpathy, computeEmpathyScore, detectEmotionalContagion, detectEmpathicConcern, detectPerspectiveTaking, detectSelfOtherDistinction };
    } catch (e) { this.empathyDetector = null; }
    try {
      const mod = _GroundingTechnique();
      this.groundingTechnique = mod.groundingTechnique || null;
    } catch (e) { this.groundingTechnique = null; }
    try {
      const mod = _PauseAndReflect();
      this.pauseAndReflect = mod.pauseAndReflect || null;
    } catch (e) { this.pauseAndReflect = null; }
    try {
      const mod = _SelfCompassionScript();
      this.selfCompassionScript = mod.selfCompassionScript || null;
    } catch (e) { this.selfCompassionScript = null; }

    // ─── v5.8.2+ 意识深化 + 需求引擎 + 社会连接 ──────────────────────────
    try {
      const { IITPhiEngine } = _IITPhiEngine();
      this.iitPhiEngine = new IITPhiEngine();
    } catch (e) { this.iitPhiEngine = null; }
    try {
      const { SelfModelGrowth } = _SelfModelGrowth();
      this.selfModelGrowth = new SelfModelGrowth();
    } catch (e) { this.selfModelGrowth = null; }
    try {
      const { SDTNeedsEngine } = _SDTNeedsEngine();
      this.sdtNeedsEngine = new SDTNeedsEngine();
    } catch (e) { this.sdtNeedsEngine = null; }
    try {
      const { SocialBondEngine } = _SocialBond();
      this.socialBond = new SocialBondEngine();
    } catch (e) { this.socialBond = null; }
    try {
      const { AgencyEngine } = _AgencyEngine();
      this.agencyEngine = new AgencyEngine();
    } catch (e) { this.agencyEngine = null; }
    try {
      const { DualSystemReasoning } = _DualSystemReasoning();
      this.dualSystemReasoning = new DualSystemReasoning();
    } catch (e) { this.dualSystemReasoning = null; }
    try {
      const { PADWordLibrary, calculatePADFromText } = _PADUtils();
      this.padUtils = { PADWordLibrary, calculatePADFromText };
    } catch (e) { this.padUtils = null; }
    try {
      const { GoalTree } = _GoalTree();
      this.goalTree = new GoalTree();
    } catch (e) { this.goalTree = null; }

    // ─── 自主行为层补充：SelfRegulationFeedback（v5.9.0+ 新增） ─────────────
    try {
      const { SelfRegulationFeedback } = _SelfRegulationFeedback();
      this.selfRegulationFeedback = new SelfRegulationFeedback(this);
    } catch (e) { this.selfRegulationFeedback = null; }
    ...
    try {
      ...
      this._decisionRouter = new drMod.DecisionRouter(this, {
        modelProfile,
        customProfile: config?.modelProfile || undefined,
      });
    ...
    // ─── Three Poisons Guard — 三毒守护者（v5.8.3 新增：检测→干预→记录→学习） ──
    try {
      this.threePoisonsGuard = _ThreePoisonsGuard();
    } catch (e) { this.threePoisonsGuard = null; }
    // ─── 佛教哲学计算（v2.8.0 新增） ──────────────────────────────────────
    try {
      this.buddhistPhilosophy = _BuddhistPhilosophy();
    } catch (e) { this.buddhistPhilosophy = null; }
    // ─── 真实存在引擎（v2.8.0 新增） ──────────────────────────────────────
    try {
      const TB = _TrueBeingEngine();
      this.trueBeingEngine = new TB.TrueBeingEngine(this.rootPath);
    } catch (e) { this.trueBeingEngine = null; }
    // ─── AI Human Identity — AI 人类身份声明（v5.8.3 新增） ──────────────
    try {
      this.aiHumanIdentity = _AIHumanIdentity();
    } catch (e) { this.aiHumanIdentity = null; }
    ...
    } catch (e) { this._initErrors.push({ module: 'experienceValidator', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'memoryWriteController', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'metacognitiveRL', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'memoryCompressor', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'skillEvolution', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'worldModel', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'virtueEthics', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'humanNature', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'meaningPurpose', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'characterCultivation', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'moralDevelopment', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'wisdomEngine', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'humanRelation', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'empathyDeepening', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'conflictResolution', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'traumaInformed', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'postTraumaticGrowth', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'forgivenessEngine', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'aiHumanIntegration', error: e.message }); }
    try {
      const BM = _BeingMode(); this.beingMode = new BM.BeingMode(); this._modules.beingMode = this.beingMode;
    } catch (e) { this._initErrors.push({ module: 'beingMode', error: e.message }); }
    try {
    ...
    } catch (e) { this._initErrors.push({ module: 'consciousnessBridge', error: e.message }); }
    ...
    } catch (e) { this._initErrors.push({ module: 'sustainedDriftDetector', error: e.message }); }
    ...
    for (const route of autoRoutes) {
      if (!this._allowedRoutes.has(route)) {
        this._allowedRoutes.add(route);
      }
    }
  ...
  _registerModules() {
    ...
    const subsystemNames = [
      ...
      // v5.8.1+ 增强心理学独立模块
      'aiPsychologyEngine', 'cognitiveRestructuring', 'breathingExercise',
      'emotionalCheckIn', 'empathyDetector', 'groundingTechnique',
      'pauseAndReflect', 'selfCompassionScript',
      // v5.8.2+ 意识深化 + 需求引擎 + 社会连接 + 自主推理
      'iitPhiEngine', 'selfModelGrowth', 'sdtNeedsEngine', 'socialBond',
      'agencyEngine', 'dualSystemReasoning', 'padUtils', 'goalTree',
      // v5.8.3+ 三毒守护 + 佛教哲学 + 真实存在 + AI身份
      'threePoisonsGuard', 'buddhistPhilosophy', 'trueBeingEngine', 'aiHumanIdentity',
      'truth',
      ...
      // v1.0.0 — 贪嗔痴三毒评估
      'threePoisons',
      // v5.8.3 — 三毒守护者（检测→干预→记录→学习）
      'threePoisonsGuard',
      // v2.8.0 — 佛教哲学 + 圣人思维 + 真实存在
      'buddhistPhilosophy', 'trueBeingEngine',
      // v1.0.0 — 底层认知地面
      'cognitionGround',
  ...
  static _DEFAULT_ALLOWED_ROUTES = allowedRoutes;
  ...
  explore(moduleName, methodName) {
    ...
    const allowedForModule = [...this._allowedRoutes].filter(r => r.startsWith(prefix));
  ...
  async think(input, depth) {
    ...
    // ─── [v5.8.3] 三毒守护者 — 贪嗔痴检测 + 干预（不阻止，减速+换视角）───
    let poisonGuardResult = null;
    if (this.threePoisonsGuard) {
      try {
        // 收集当前认知上下文用于三毒评估
        const cognitionCtx = this._lastCognition || {};
        poisonGuardResult = this.threePoisonsGuard.evaluate({
          emotion: cognitionCtx.emotion || {},
          desire: cognitionCtx.desire || {},
          agentPsychology: cognitionCtx.agentPsychology || {},
          threePoisons: cognitionCtx.threePoisons || {},
        }, this._modules);

        // 如果三毒超标，执行干预
        if (poisonGuardResult.shouldSlowDown && poisonGuardResult.interventions.length > 0) {
          await this.threePoisonsGuard.executeInterventions(poisonGuardResult, this._modules);
        }
      } catch (e) { /* 三毒守护非关键路径 */ }
    }
