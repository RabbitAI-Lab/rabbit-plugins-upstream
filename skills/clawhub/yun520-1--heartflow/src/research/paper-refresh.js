  ...
  async importPapers(jsonString) {
    try {
      ...
      if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
        return { added: 0, skipped: 0, errors: 1, error: 'Expected JSON object with "papers" array or a top-level array' };
      }
      const papers = Array.isArray(raw) ? raw : (Array.isArray(raw.papers) ? raw.papers : []);
      if (papers.length > 1000) {
        return { added: 0, skipped: 0, errors: 1, error: 'Maximum 1000 papers per import' };
      }
      // Validate each paper has required fields
      const validated = papers.filter(p => {
        if (!p || typeof p !== 'object') return false;
        if (typeof p.id !== 'string' || p.id.length > 200) return false;
        if (typeof p.title !== 'string' || p.title.length > 1000) return false;
        if (p.authors && typeof p.authors !== 'string') return false;
        if (p.tags && !Array.isArray(p.tags)) return false;
        return true;
      });
      return await this.bulkAddPapers(validated);
    } catch (e) {
