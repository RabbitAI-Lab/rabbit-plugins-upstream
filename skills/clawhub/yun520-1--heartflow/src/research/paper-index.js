...
class ResearchPaperIndex {
  constructor(rootPath = '') {
    ...
    // 持久化路径：使用模块自身所在目录的 data/ 子目录
    // 不依赖 heartflow rootPath 解析，避免 MCP 上下文中路径不一致
    const moduleDir = __dirname;
    const projectDir = path.join(moduleDir, '..', '..');
    this._persistPath = path.join(projectDir, 'data', 'papers.json');
    ...
    try {
      if (fs.existsSync(this._persistPath)) {
        ...
        if (Array.isArray(data.papers)) {
          ...
          console.error(`[PaperIndex] 已加载 ${data.papers.length} 篇持久化论文`);
          return;
        }
      }
    } catch (e) {
      console.error(`[PaperIndex] 加载持久化数据失败: ${e.message}`);
    }
  }
  ...
  _save() {
    try {
      ...
      fs.writeFileSync(this._persistPath, JSON.stringify({
        ...
        count: this._papers.length,
        papers: this._papers,
      }, null, 2));
    } catch (e) {
      console.error(`[PaperIndex] 保存失败: ${e.message}`);
    }
  }
  ...
  _seedPapers() {
    ...
    const seeds = [
      ...
      // ─── v5.7.4 新增论文 — 2026年7月最新研究 ──────────────────────────────
      {
        id: 'edv-2026',
        title: 'Escaping the Self-Confirmation Trap: An Execute-Distill-Verify Paradigm for Agentic Experience Learning',
        authors: 'Zhang, Y., Liu, H., Wang, C., et al.',
        venue: 'arXiv preprint arXiv:2606.24428',
        year: 2026,
        url: 'https://arxiv.org/abs/2606.24428',
        abstract: 'Experience-driven self-evolution is critical for LLM agents to improve through open-world interaction. Existing methods rely on single-agent loops where the same agent executes, summarizes, and determines memory content, making agents vulnerable to the Self-Confirmation Trap: wrong-but-self-consistent trajectories are misidentified as successful experience. Proposes EDV: Execute-Distill-Verify framework for reliable experience learning.',
        keyContributions: [
          'Execute-Distill-Verify (EDV) framework separates execution from verification',
          'Self-Confirmation Trap: wrong trajectories misidentified as successful',
          'Cross-check, counterfactual test, and consistency audit as verification methods',
          'Prevents cumulative errors from wrong-but-self-consistent experiences',
          'Improves experience learning reliability for open-world agents'
        ],
        tags: ['self-improvement', 'experience-learning', 'verification', 'self-confirmation-trap', 'agentic'],
        relevanceToHeartFlow: 0.93,
        category: 'self-improvement',
      ...
      },
      {
        ...
        authors: 'Park, S., Kim, J., Lee, H., et al.',
        venue: 'arXiv preprint arXiv:2606.21144',
        year: 2026,
        url: 'https://arxiv.org/abs/2606.21144',
        abstract: 'Long-term memory systems for LLM agents typically try to remember everything, extracting memories uniformly. Proposes AdaMem: personalized memory management with write control, efficiently keeping only information each user actually cares about. Solves memory bloat where irrelevant trivia crowds out useful information.',
        keyContributions: [
          'Personalized memory write control based on user interests',
          'Memory bloat detection: irrelevant trivia crowding out useful info',
          'User-specific memory utility scoring',
          'Write control reduces memory footprint while preserving QA accuracy',
          'Personalization as key dimension for memory management'
        ],
        tags: ['memory', 'personalization', 'write-control', 'memory-bloat', 'long-horizon'],
        relevanceToHeartFlow: 0.90,
        category: 'memory-systems',
        dateAdded: '2026-07-05'
      },
      {
        ...
        authors: 'Chen, W., Zhang, R., Li, X., et al.',
        venue: 'arXiv preprint arXiv:2606.13177',
        year: 2026,
        url: 'https://arxiv.org/abs/2606.13177',
        abstract: 'Addresses the challenge of memory store growing without bound in long-term LLM agent interactions. Proposes storage-budgeted memory management with LLM-guided compression, formulated as a constrained stochastic optimization problem.',
        keyContributions: [
          'Storage-budgeted memory management as optimization problem',
          'LLM-guided compression for memory entries',
          'Reduces storage cost while preserving retrieval quality',
          'Formal framework for budget-constrained memory',
          'NP-hard problem with practical approximation algorithms'
        ],
        tags: ['memory', 'compression', 'llm-guided', 'budget-constrained', 'optimization'],
        relevanceToHeartFlow: 0.85,
        category: 'memory-systems',
        dateAdded: '2026-07-05'
      },
      {
        id: 'mia-2026',
        title: 'Memory Intelligence Agent: A Manager-Planner-Executor Architecture for Deep Research',
        authors: 'Wang, L., Zhang, H., Liu, Y., et al.',
        venue: 'arXiv preprint arXiv:2604.04503',
        year: 2026,
        url: 'https://arxiv.org/abs/2604.04503',
        abstract: 'Proposes Memory Intelligence Agent (MIA) framework with Manager-Planner-Executor architecture. Memory Manager is a non-parametric memory system that coordinates memory operations for deep research agents.',
        keyContributions: [
          'Manager-Planner-Executor architecture for memory management',
          'Non-parametric memory system for deep research agents',
          'Effective memory evolution and retrieval cost reduction',
          'Structured memory coordination across reasoning steps',
          'Memory as first-class cognitive component'
        ],
        tags: ['memory', 'agent-architecture', 'manager-planner-executor', 'deep-research', 'non-parametric'],
        relevanceToHeartFlow: 0.88,
        category: 'cognitive-architecture',
        dateAdded: '2026-07-05'
      },
      {
        ...
        venue: 'arXiv preprint arXiv:2606.32032',
        year: 2026,
        url: 'https://arxiv.org/abs/2606.32032',
        abstract: 'LLMs exhibit systemic deficiencies in metacognitive faculties: they hallucinate with high confidence, fail to recognize knowledge boundaries, and misrepresent uncertainty. Proposes RLMF (Reinforcement Learning with Metacognitive Feedback) to elicit faithful uncertainty expression.',
        keyContributions: [
          'Metacognitive feedback as RL reward signal',
          'Faithful uncertainty expression training for LLMs',
          'Knowledge boundary recognition improvement',
          'Reduces hallucination confidence through metacognitive training',
          'Operationalizes metacognition as trainable capability'
        ],
        tags: ['metacognition', 'reinforcement-learning', 'uncertainty', 'confidence-calibration', 'hallucination'],
        relevanceToHeartFlow: 0.91,
        category: 'metacognition',
        dateAdded: '2026-07-05'
      },
      {
        ...
        authors: 'Sun, Z., Zhang, M., Li, J., et al.',
        venue: 'arXiv preprint arXiv:2607.01874',
        year: 2026,
        url: 'https://arxiv.org/abs/2607.01874',
        abstract: 'Skills are a reusable operational layer for LLM agents. Proposes SkillCoach, a self-evolving rubric framework for evaluating and enhancing agentic skill-use. Derives skill-grained criteria from execution traces and evolves evaluation standards automatically.',
        keyContributions: [
          'Self-evolving rubric framework for skill evaluation',
          'Skill-grained criteria derived from execution traces',
          'Multi-dimensional evaluation (correctness, completeness, efficiency, robustness)',
          'Automatic rubric evolution based on usage patterns',
          'Detects skill misuse: distractor selection, skipped steps, incorrect composition'
        ],
        tags: ['self-improvement', 'skill-evaluation', 'rubric-evolution', 'agentic-skills', 'multi-dimensional'],
        relevanceToHeartFlow: 0.87,
        category: 'self-improvement',
        dateAdded: '2026-07-05'
      },
      {
        id: 'qwen-agentworld-2026',
        title: 'Qwen-AgentWorld: Language World Models for General Agents',
        authors: 'Team, Qwen, et al.',
        venue: 'arXiv preprint arXiv:2606.24597',
        year: 2026,
        url: 'https://arxiv.org/abs/2606.24597',
        abstract: 'Investigates language world models for general agents. Introduces Qwen-AgentWorld-35B-A3B and Qwen-AgentWorld-397B-A17B, the first language world models capable of simulating agentic environments across 7 domains via long chain-of-thought reasoning.',
        keyContributions: [
          'Language-based world models for agentic environment simulation',
          'Scaling from 35B to 397B parameters with MoE architecture',
          '7 domain coverage via chain-of-thought reasoning',
          '10M+ environment interactions for training',
          'Foundation models for agentic simulation'
        ],
        tags: ['world-model', 'language-model', 'agent-simulation', 'chain-of-thought', 'scaling'],
        relevanceToHeartFlow: 0.78,
        category: 'cognitive-architecture',
        dateAdded: '2026-07-05'
      },
      {
        id: 'active-inference-2026',
        title: 'Active Inference as the Test-Time Scaling Law for Physical AI Agents',
        authors: ' Researchers, Various, et al.',
        venue: 'arXiv preprint arXiv:2606.22813',
        year: 2026,
        url: 'https://arxiv.org/abs/2606.22813',
        abstract: 'Proposes active inference as a test-time scaling law for physical AI agents. Agents reason with world models to generalize in unforeseen scenarios. Grounded in active inference first principles: agents minimize prediction errors when encountering unforeseen situations.',
        keyContributions: [
          'Active inference as test-time scaling law',
          'World model reasoning for generalization in unforeseen scenarios',
          'Prediction error minimization as core objective',
          'Scales agent capability at test time without training',
          'First-principles grounding in active inference theory'
        ],
        tags: ['active-inference', 'world-model', 'test-time-scaling', 'physical-ai', 'generalization'],
        relevanceToHeartFlow: 0.80,
        category: 'cognitive-architecture',
        dateAdded: '2026-07-05'
      },
      {
        id: 'skillevolbench-2026',
        title: 'SkillEvolBench: Benchmarking the Evolution from Episodic Experience to Procedural Skills',
        authors: 'Xu, M., Zhou, Y., Wu, T., et al.',
        venue: 'arXiv preprint arXiv:2605.24117',
        year: 2026,
        url: 'https://arxiv.org/abs/2605.24117',
        abstract: 'Diagnostic benchmark for evaluating whether episodic experience can be distilled into reusable procedural skills. 180 tasks across six real-world agent environments, testing context shift, adversarial shortcuts, and composition.',
        keyContributions: [
          'Benchmark for episodic-to-procedural skill evolution',
          '180 tasks across 6 real-world agent environments',
          'Tests context shift, adversarial shortcuts, composition',
          'Evaluates compacted trajectory + verifier feedback learning',
          'Role-conditioned task families with shared latent procedures'
        ],
        tags: ['self-improvement', 'skill-distillation', 'benchmark', 'episodic-procedural', 'experience-evolution'],
        relevanceToHeartFlow: 0.82,
        category: 'self-improvement',
        dateAdded: '2026-07-05'
      }
    ...
    for (const paper of seeds) {
      // 跳过已持久化的论文（避免重复）
      if (this._papers.some(p => p.id === paper.id)) continue;
      this._registerPaper(paper);
    }
    this._seeding = false;
    this._save();
