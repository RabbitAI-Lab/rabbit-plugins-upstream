...
class MemoryAdapter {
  ...
  retrieve(query, layer = 'all', limit = 10) {
    ...
    // [AUDIT-FIX] layer='all' 时也传递 limit 参数，防止返回全部结果
  ...
  // [AUDIT-FIX] 补全 heartflow.js 调用的兼容 API
  addLearned(key, value, tags = []) {
    return this.learn(key, value, tags);
  }

  addEphemeral(key, value, ttlMs = 3600000) {
    return this.remember(key, value, ttlMs);
  }
  ...
  _topicSimilarity(topicA, topicB) {
    ...
    // [AUDIT-FIX] 缓存 bigram 计算结果，避免重复计算
    const cacheKey = a < b ? `${a}|||${b}` : `${b}|||${a}`;
    if (!this._topicSimCache) this._topicSimCache = new Map();
    if (this._topicSimCache.has(cacheKey)) return this._topicSimCache.get(cacheKey);
    ...
    const result = overlap / Math.max(gramsA.length, gramsB.length);
    // 限制缓存大小（最多 500 条）
    if (this._topicSimCache.size > 500) this._topicSimCache.clear();
    this._topicSimCache.set(cacheKey, result);
    return result;
  }
