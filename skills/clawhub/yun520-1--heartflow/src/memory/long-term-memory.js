...
class LongTermMemory {
  ...
  search(query, options = {}) {
    ...
    // [AUDIT-FIX] 批量预加载记忆记录，避免 N+1 文件读写
    // 先收集需要的 ID，一次性读取所有文件
    const neededIds = [];
    for (const memMeta of this.index.memories) {
      if (tags.length > 0 && !tags.every(t => memMeta.tags.includes(t))) continue;
      if (type && memMeta.type !== type) continue;
      neededIds.push(memMeta.id);
    }

    // 批量读取（一次性加载所有需要的记录）
    const records = this._batchGet(neededIds);
    ...
    for (const memMeta of this.index.memories) {
      ...
      // 文本搜索（简单匹配）
      const record = records[memMeta.id] || this.get(memMeta.id);
      if (!record) continue;
  ...
  /**
   * [AUDIT-FIX] 批量获取记忆记录，避免 N+1 文件读写
   */
  _batchGet(ids) {
    const records = {};
    for (const id of ids) {
    ...
    // 批量写回（减少 writeFileSync 调用）
    for (const id of ids) {
      if (records[id]) {
        try {
          fs.writeFileSync(path.join(this.storagePath, `${id}.json`), JSON.stringify(records[id]));
        } catch (e) { /* non-fatal */ }
      }
    }
    return records;
