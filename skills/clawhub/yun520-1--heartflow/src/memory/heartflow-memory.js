...
class HeartFlowMemory {
  constructor(rootPath) {
    ...
    // [AUDIT-FIX] 搜索索引 — 避免 search/searchByTag 全表扫描 (O(N) → O(1))
    this._tagIndex = new Map();       // lowercaseTag -> Set<{key, tier}>
    this._wordIndex = new Map();      // lowercaseWord -> Set<{key, tier}>
  ...
  _loadJson(filePath) {
    try {
      ...
      try { fs.renameSync(filePath, filePath + '.bak.' + Date.now()); } catch (e2) {
        console.error(`[HeartFlow Memory]  corrupted file backup failed: ${filePath}: ${e2.message}`);
      }
    }
    return {};
  }
  ...
  // [AUDIT-FIX] 搜索索引重建 — 为 search/searchByTag 提供 O(1) 查找
  _rebuildIndex() {
    this._tagIndex.clear();
    this._wordIndex.clear();
    const tiers = [
      { entries: this.core, tier: 'CORE' },
      { entries: this.learned, tier: 'LEARNED' },
      { entries: this.ephemeral, tier: 'EPHEMERAL' },
    ];
    for (const { entries, tier } of tiers) {
      for (const [key, v] of Object.entries(entries)) {
        const valStr = this._safeString(v.value).toLowerCase();
        // Tag index
        for (const tag of (v.tags || [])) {
          const lt = tag.toLowerCase();
          if (!this._tagIndex.has(lt)) this._tagIndex.set(lt, []);
          this._tagIndex.get(lt).push({ key, tier });
        }
        // Word index (simple tokenization)
        const words = valStr.split(/[\s,，。.！？、；;:：""''【】《》（）()\[\]{}]+/);
        for (const w of words) {
          if (w.length < 2) continue;
          if (!this._wordIndex.has(w)) this._wordIndex.set(w, []);
          this._wordIndex.get(w).push({ key, tier });
        }
      }
    }
  ...
  search(query) {
    ...
    // Use word index for O(1) lookup per word
    const words = q.split(/[\s,，。.！？、；;:：""''【】《》（）()\[\]{}]+/).filter(w => w.length >= 2);
    if (words.length === 0) {
      // Fallback: return all if no searchable words
      for (const [k, v] of Object.entries(this.core)) { results.push({ key: k, tier: 'CORE', ...v }); }
      for (const [k, v] of Object.entries(this.learned)) { results.push({ key: k, tier: 'LEARNED', ...v }); }
      for (const [k, v] of Object.entries(this.ephemeral)) { results.push({ key: k, tier: 'EPHEMERAL', ...v }); }
      return results;
    }

    // Collect candidates from word index
    const candidates = new Map(); // key -> {key, tier, score}
    for (const word of words) {
      const hits = this._wordIndex.get(word);
      if (!hits) continue;
      for (const hit of hits) {
        const id = `${hit.tier}:${hit.key}`;
        if (seen.has(id)) {
          candidates.get(id).score += 1;
        } else {
          seen.add(id);
          const tierEntries = hit.tier === 'CORE' ? this.core : hit.tier === 'LEARNED' ? this.learned : this.ephemeral;
          const v = tierEntries[hit.key];
          candidates.set(id, { key: hit.key, tier: hit.tier, score: 1, ...v });
        }
      }
    }
      ...
      try {
        ...
        for (let i = Math.max(0, lines.length - 10); i < lines.length; i++) {
          try {
            ...
            const d = raw;
            const eventText = typeof d.eventText === 'string' ? d.eventText : '';
            const narrative = typeof d.narrative === 'string' ? d.narrative : '';
            const ts = typeof d.ts === 'number' ? d.ts : 0;
            const eventType = typeof d.eventType === 'string' ? d.eventType : 'unknown';
            blocks.push({
              content: `梦: ${eventText || narrative.substring(0, 100)}`,
              layer: 'LEARNED',
              timestamp: ts,
              tags: ['dream', eventType],
              id: `dream:${i}`,
              isToday: ts >= todayTs,
            });
          } catch (e) {
            console.error(`[HeartFlow Memory]  dream entry #${i} parse failed: ${e.message}`);
          }
        }
      } catch (e) {
        console.error(`[HeartFlow Memory]  dream consolidation failed: ${e.message}`);
      }
    }
  ...
  searchByTag(tag) {
    ...
    const hits = this._tagIndex.get(q) || [];
    return hits.map(hit => {
      const tierEntries = hit.tier === 'CORE' ? this.core : hit.tier === 'LEARNED' ? this.learned : this.ephemeral;
      const v = tierEntries[hit.key];
      return { key: hit.key, tier: hit.tier, ...v };
    });
  }
  ...
  getTotalSize() {
    ...
    for (const f of [this.corePath, this.learnedPath, this.ephemeralPath, this.dreamPath]) {
      try { total += fs.statSync(f).size; } catch (e) {
        console.error(`[HeartFlow Memory]  stat failed for ${f}: ${e.message}`);
      }
    }
    return total;
  }
