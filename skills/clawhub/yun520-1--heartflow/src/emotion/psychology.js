...
function detectDINKFears(text) {
  ...
  return {
    fears: detected,
    fearCount: count,
    coreStance: '引擎不审判选择。引擎只问：你准备好了吗？',
    insight: count >= 2
...
// ========================================
// TopicScope 话题隔离（引用 identity/topic-scope.js，消除重复实现）
// ========================================
let _TopicScope = null;

function _getTopicScope() {
  if (!_TopicScope) {
    try {
      _TopicScope = require('../identity/topic-scope.js');
    } catch (e) {
      return null;
    }
  }
  return new _TopicScope();
}
...
const _TOPIC_CENTROIDS = {
  ...
  '引擎开发': ['引擎','heartflow','heart-logic','版本','升级','修复','bug','代码','模块','引擎','启动','skill','hermes'],
