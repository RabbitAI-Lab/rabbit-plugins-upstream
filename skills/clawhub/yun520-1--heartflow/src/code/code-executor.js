...
// [AUDIT-FIX] 安全 shell 命令解析器
// 将 shell 命令字符串拆分为 command + args 数组，供 execFileSync 使用（不经过 shell）。
// 这防止了 shell 注入：; | $() ` 等元字符不会被解释。
function _parseShellCommand(code) {
  ...
  while (i < trimmed.length) {
    ...
    if (ch === ' ' || ch === '\t') {
      if (current) {
        if (!result.command) result.command = current;
        else result.args.push(current);
        current = '';
      }
      i++;
      continue;
...
class CodeExecutor {
  ...
  _executeShell(code, opts) {
    ...
    try {
      // [AUDIT-FIX] 使用 execFileSync + 参数数组，防止 shell 注入
      // execSync(code, { shell: '/bin/bash' }) 会将整个 code 字符串传给 bash，
      // 导致 shell 元字符（; | $() `）被解释执行。
      // execFileSync(command, args) 不经过 shell，参数逐个传递。
      const { execFileSync } = require('child_process');
      const parsed = _parseShellCommand(code);
      const result = execFileSync(parsed.command, parsed.args, {
        timeout,
        encoding: 'utf-8',
        maxBuffer: MAX_OUTPUT_LIMIT,
        shell: false  // [AUDIT-FIX] 明确禁用 shell，防止注入
      });
