...
class AutoCompactionEngine {
  ...
  compact(messages, options = {}) {
    ...
    // [AUDIT-FIX] 防止数组无限增长，上限 100 条
    if (this.stats.compressionRatios.length > 100) this.stats.compressionRatios.shift();
    if (this.stats.compressionEfficiency.length > 100) this.stats.compressionEfficiency.shift();
