...
let _qMeta = {};
let _qMetaCacheDirty = true;

function _loadQMeta() {
  if (!_qMetaCacheDirty) return;
  try {
    ...
    _qMetaCacheDirty = false;
  } catch (e) {
    _qMeta = {};
  }
}

function _invalidateQMetaCache() {
  _qMetaCacheDirty = true;
}
...
class HealingMemoryRL {
  ...
  updateFromRepair(errorPattern, strategy, success) {
    ...
    _debouncedSave(this);
    // 每50次update触发自动清理
    this._autoCleanupCounter++;
    if (this._autoCleanupCounter % 50 === 0) {
      this.autoCleanupRL().catch(e => process.stderr.write('[self-healing-rl] autoCleanupRL error: ' + e.message + '\n'));
    }
  }
  ...
  letGoOf(errorPattern) {
    ...
    // 容量上限 200
    if (this._letGoLog.length > 200) {
      this._letGoLog = this._letGoLog.slice(-200);
    }
    return {
      result: true,
      pattern: errorPattern.slice(0, 50),
