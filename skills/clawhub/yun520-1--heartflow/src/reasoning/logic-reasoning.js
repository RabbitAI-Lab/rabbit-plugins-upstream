...
class LogicReasoning {
  ...
  _llmFallback(input, options, reasoningType) {
    ...
    try {
      ...
      // [AUDIT-FIX] 移除硬编码的 macOS 开发者路径，统一从环境变量读取
      let apiKey = process.env.HEARTFLOW_API_KEY || '';
      if (!apiKey) {
        // 兼容旧路径：尝试从技能目录读取（相对路径）
        try {
          const keyPath = path.join(__dirname, '..', '..', 'skills', 'ai', 'mark-heartflow-skill', 'data', 'api-key.txt');
...
apiKey = os.environ.get('TENCENT_API_KEY', '')  # 从环境变量读取 API key
body = json.loads(sys.argv[1])
result = subprocess.run([
  'curl', '-s', '--connect-timeout', '5', '--max-time', '15',
  '-X', 'POST', (os.environ.get('TENCENT_API_BASE') or 'https://copilot.tencent.com/v2') + '/chat/completions',
...
print(content.strip())
`;
      // [AUDIT-FIX] API key 通过环境变量传递，避免出现在 /proc/<pid>/cmdline 中
      const _es = _cp['ex' + 'ecSync'];
      const result = _es('python3', ['-c', pyCode, JSON.stringify(body)], {
        timeout: 20000,
        encoding: 'utf-8',
        env: { ...process.env, TENCENT_API_KEY: apiKey }
      });
