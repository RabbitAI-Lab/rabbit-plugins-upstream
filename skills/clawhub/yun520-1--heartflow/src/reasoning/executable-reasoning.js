...
/**
 * Dangerous code patterns that are never allowed in thought node execution.
 * These patterns indicate attempts to escape the reasoning sandbox.
 */
const BLOCKED_PATTERNS = [
  /\brequire\s*\(/,
  /\bimport\s+/,        // dynamic import()
  /\bexport\s+/,        // re-exporting
  /\bprocess\s*\./,     // access to process object
  /\bglobal\s*\./,      // access to global object
  /\bglobalThis\s*\./,  // access to globalThis
  /\b__proto__\b/,
  /\bconstructor\s*\(/,
  /\bprototype\s*\./,
  /\bfunction\s*\*\s*\(/, // generator functions
  /\basync\s+function\s*\*\s*\(/, // async generators
  /\bwhile\s*\(\s*true\s*\)/,     // infinite loops
  /\bfor\s*\(\s*;\s*;\s*\)/,      // infinite loops
];
...
class ExecutableReasoning {
  /**
   * Validate that code body does not contain dangerous patterns.
   * @param {string} body - The JavaScript source to validate.
   * @throws {Error} If dangerous patterns are detected.
   */
  static validateCode(body) {
    for (const pattern of BLOCKED_PATTERNS) {
      if (pattern.test(body)) {
        throw new Error(`ExecutableReasoning: blocked dangerous pattern in thought node code: ${pattern.source}`);
      }
    }
  }
  /**
  ...
  generateCode(thoughtNode) {
    ...
    // Validate thought node code for dangerous patterns
    if (typeof thoughtNode.code === 'string') {
      ExecutableReasoning.validateCode(thoughtNode.code);
    }
  ...
  execute(codeOrResult, overrideContext = {}) {
    ...
    try {
      // Validate code before execution to prevent sandbox escape
      ExecutableReasoning.validateCode(body);
