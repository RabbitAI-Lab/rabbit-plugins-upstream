...
// 无子命令时默认 status
if (!cmd || cmd.startsWith('-')) cmd = 'status';
...
function createEngine(opts = {}) {
  ...
  const engine = new HeartFlow({
    rootPath: hfDir,
    dataDir: opts.dataDir || path.join(hfDir, 'data'),
...
// ─── 配置命令 ──────────────────────────────────────────────
function configCmd(sub) {
  const { HeartFlowConfig, load, getInstance } = require(path.join(hfDir, 'src/core/config.js'));
  const cfg = load(hfDir);

  switch (sub) {
    case 'list': {
      const s = cfg.summary();
      console.log('━'.repeat(50));
      console.log(`HeartFlow 配置 (v${s.version})`);
      console.log('━'.repeat(50));
      console.log(`  项目根目录: ${s.rootPath}`);
      console.log(`  数据目录:   ${s.dataDir}`);
      console.log('');
      console.log('  MCP 服务器:');
      console.log(`    端口:     ${s.mcp.port}`);
      console.log(`    监听:     ${s.mcp.host}`);
      console.log(`    会话超时: ${(s.mcp.sessionMaxAge / 1000 / 60).toFixed(0)} 分钟`);
      console.log('');
      console.log('  功能开关:');
      ...
      for (const src of s.sources) {
        const label = src.source === 'project' ? `项目 config.json (${src.file})`
                    : src.source === 'user' ? `用户 config.json (${src.file})`
                    : `环境变量 ${src.env || ''}`;
        console.log(`    · ${label}`);
      }
      console.log('');
    ...
    case 'get': {
      ...
      if (!key) { console.error('用法: node bin/cli.js config get <key>'); process.exit(1); }
      const val = cfg.get(key, '(未设置)');
    ...
    case 'set': {
      ...
      if (!key || rawVal === undefined) { console.error('用法: node bin/cli.js config set <key> <value>'); process.exit(1); }
      let val = rawVal;
      if (rawVal === 'true') val = true;
      else if (rawVal === 'false') val = false;
      else if (/^\d+$/.test(rawVal)) val = parseInt(rawVal, 10);

      cfg.saveUserConfig({ [key]: val });
      console.log(`✅ 已设置 ${key} = ${val}`);
      console.log(`   （已保存到 ~/.heartflow/config.json）`);
      break;
    }

    default:
      console.log(`HeartFlow 配置命令
Usage: node bin/cli.js config <subcommand>
Subcommands:
  list          显示全部配置及来源
  get <key>     读取单个配置值
  set <key> <value>  设置配置（持久化到用户目录）

示例:
  node bin/cli.js config list
  node bin/cli.js config get mcp.port
  node bin/cli.js config set mcp.port 8100`);
  }
  process.exit(0);
}
...
async function setupWizard() {
  ...
  console.log('');
  console.log('╔══════════════════════════════════════════════════════╗');
  console.log('║       HeartFlow 心虫 — 配置向导 (可选)              ║');
  console.log('╠══════════════════════════════════════════════════════╣');
  console.log('║  引擎已配置了合理的默认值，所有选项都可跳过。       ║');
  console.log('║  按 Enter 使用默认值，或输入新值覆盖。               ║');
  console.log('╚══════════════════════════════════════════════════════╝');
  console.log('');
  ...
  // 1. MCP 端口
  const defaultPort = String(cfg.get('mcp.port'));
  const portAnswer = await question(`  MCP 服务器端口 [${defaultPort}]: `);
  if (portAnswer.trim()) updates['mcp.port'] = parseInt(portAnswer.trim(), 10);

  // 3. 日志级别
  const logLevels = ['silent', 'error', 'warn', 'info', 'debug'];
  const currentLog = cfg.get('logLevel');
  console.log(`  日志级别: ${logLevels.map(l => `${l}${l === currentLog ? '(当前)' : ''}`).join(' ')} [${currentLog}]:`);
  const logAnswer = await question('  > ');
  if (logAnswer.trim() && logLevels.includes(logAnswer.trim())) {
    updates['logLevel'] = logAnswer.trim();
  }

  // 4. 功能开关
  const features = cfg.get('features', {});
  for (const [key, def] of Object.entries(features)) {
    const yn = await question(`  启用 ${key}? (Y/n) [${def ? 'Y' : 'n'}]: `);
    if (yn.trim().toLowerCase() === 'n') {
      updates[`features.${key}`] = false;
    } else if (yn.trim().toLowerCase() === 'y') {
      updates[`features.${key}`] = true;
    }
  }
  ...
  if (Object.keys(updates).length === 0) {
    console.log('  未修改任何配置，使用默认值。');
    process.exit(0);
  }

  const saved = cfg.saveUserConfig(updates);
  console.log('');
  console.log('✅ 配置已保存到 ~/.heartflow/config.json:');
  for (const [k, v] of Object.entries(saved)) {
    console.log(`  ${k} = ${typeof v === 'object' ? JSON.stringify(v) : v}`);
  }
  console.log('');
  console.log('  修改立即生效（无需重启）。');
  process.exit(0);
}
...
function formatStatus(engine) {
  ...
  lines.push(`  平台: ${engine.platformAdapter?.platform || 'universal'}`);
...
switch (cmd) {
  ...
  case 'status': {
    const { load, getInstance } = require(path.join(hfDir, 'src/core/config.js'));
    const cfg = load(hfDir);
    let engine = null;
    try {
      engine = createEngine({ silent: true });
      const version = cfg.get('version') || require(path.join(hfDir, 'package.json')).version || 'unknown';
      const result = {
        version,
        platform: engine.platformAdapter?.platform || 'universal',
...
Commands:
  ...
  config list      显示全部配置及来源
  config get <k>  读取配置值
  config set <k> <v>  设置配置
  setup            配置向导（可选，首次运行推荐）
  chat             启动交互式控制台（支持 think() 和斜杠命令）
  --chat <msg>     单次执行 think() 后退出
  help             显示此帮助信息`);
    process.exit(0);
    break;
  ...
  case 'config': {
    ...
    if (!sub || sub === '--help' || sub === '-h') {
      console.log(`HeartFlow 配置管理
Usage: node cli.js config <subcommand>
Subcommands:
  list             显示全部配置及来源
  get <key>        读取单个配置值
  set <key> <value>  设置配置（持久化）
...
示例:
  ...
  case 'daemon': {
    ...
    const validSubs = ['start', 'stop', 'restart', 'status'];
    if (!validSubs.includes(sub)) {
      console.error(`用法: node bin/cli.js daemon <${validSubs.join('|')}>`);
      process.exit(1);
    }
    try {
      const out = execSync(`node ${path.join(hfDir, 'bin', 'daemon.js')} ${sub}`, {
        encoding: 'utf-8',
        stdio: 'pipe',
      });
      console.log(out);
