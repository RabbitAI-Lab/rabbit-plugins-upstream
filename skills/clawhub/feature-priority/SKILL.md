---
name: amazon-feature-priority-planner
description: >
  结合 Amazon 评论频次、体验影响和商品信息，为产品特性改进提供证据化优先级。仅用于产品特性排序；不用于销量预测、预算审批或自动执行采购。
  Requires an ARI API key (ari_live_*).
slug: feature-priority
displayName: Amazon 产品特性优先级
version: 1.4.7
summary: 按评论频次和影响排序产品改进特性
license: MIT
---
...
- 本入口只执行 `product/priorities`，输出模板固定为 `ops_product`。
