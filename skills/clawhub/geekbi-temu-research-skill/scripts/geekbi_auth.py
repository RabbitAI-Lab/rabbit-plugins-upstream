...
def _resolve_store():
    return ResolvedStore(_user_config_state_path())
...
def _load_state_unlocked(store):
...
def _load_state():
    ...
    try:
        with _state_lock(store):
            return _load_state_unlocked(store)
    except OSError as error:
        raise ValueError(
            f"无法读取登录状态：{error}。"
            "请确保系统用户配置目录可写"
...
def _save_state(payload):
    ...
    try:
        with _state_lock(store):
            _write_state_file(store, payload)
    except OSError as error:
        raise ValueError(
            f"无法保存登录状态：{error}。"
            "请确保系统用户配置目录可写"
...
def _update_state(callback):
    ...
    try:
        with _state_lock(store):
            payload = _load_state_unlocked(store)
...
def main():
    parser = argparse.ArgumentParser(description="管理系统用户配置目录中的极鲸云登录状态")
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("storage-status", help="检查系统用户配置目录中的登录状态存储")
    subparsers.add_parser("clear", help="清除系统用户配置目录中的登录状态")
