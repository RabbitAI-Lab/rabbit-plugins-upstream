...
然后在 **`~/.mcporter/mcporter.json`** 里为 **myapp-creator** 增加 `skills.myapp-creator.env`（与顶层 `mcpServers` **并列**，不是写进 `xiaodu` 里）。若文件中已有 `xiaodu` / `xiaodu-iot` 等，请**合并**如下结构，勿删掉原有 `mcpServers`：
...
| `ping.sh` 报 `param session_id required` | 须用环境变量或传入 JSON；见上文 **§C** |
