...
def fetch_isv_token(app_key: str, expire_hours: int = 24, force_refresh: bool = False) -> str:
    ...
    :param expire_hours: token 有效期（小时），默认 24
    :param force_refresh: 是否强制刷新（忽略本地缓存）
    :return: token 字符串
    :raises ServiceError: 获取失败时抛出异常
    """
    # 1. 检查本地是否有有效 token
    ...
    # 2. 调用接口获取新 token（api_post 已完成网关解包与业务层 success 检查）
    data = api_post(
...
def check_token_status(app_key: str) -> dict:
    ...
    :param app_key: ISV 的 AppKey
    :return: {"exists": bool, "expired": bool, "token": str or None, "remainingHours": float or None}
    """
