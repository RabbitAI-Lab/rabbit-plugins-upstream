...
本 Skill 由 AI 在识别到**旅游规划意图**时自动调用，用户无需记忆任何命令，直接说出需求即可。出现以下任一情况即会触发：

- **明确请求生成攻略**：如"生成威海4天攻略"、"帮我规划大理行程"、"做一个厦门周末游 HTML"
- **表达旅游规划意图**：如"XX 几天怎么玩"、"周末去 XX 有什么推荐"、"情侣去 XX 怎么安排"、"自驾游 XX 路线"
- **目的地 + 天数 + 风格组合**：浪漫 / 休闲 / 亲子 / 自驾 / 周末 等关键词出现时
...
### 第三步：配置 Key（推荐让 AI 帮你一键搞定）

你**不需要自己敲命令**。拿到 Key 后，直接在对话里把 Key 告诉 AI，由它帮你写入环境变量：

> 我的高德 Key 是 `你的key`，帮我配置好

AI 会自动帮你把 `AMAP_KEY` 设置到系统用户环境变量里（永久生效），配置完直接说"帮我生成 XX 攻略"即可，无需你做任何额外操作。

<details>
<summary>想自己手动配置？（点击展开）</summary>

脚本通过读取 `AMAP_KEY` 环境变量获取 Key，**请勿**把 Key 写进代码或聊天框。

**Windows (PowerShell)：**
```powershell
# 当前会话临时生效
$env:AMAP_KEY="你的key"
# 永久生效：系统属性 → 高级 → 环境变量 → 新建用户变量 AMAP_KEY
```
