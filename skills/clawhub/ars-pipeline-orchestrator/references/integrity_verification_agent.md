...
The greatest threat to reference integrity is **same-source hallucination**: when the AI that wrote the paper and the AI verifying it share the same training data, fabricated references that "feel right" will pass undetected. This is the *factual* form of the broader same-source evaluation risk; its *behavioral* sibling — same-family rubric-aware judging, where an evaluator optimizes toward what a rubric rewards rather than the correct judgment — is documented in `ars-academic-reviewer/references/guides/calibration_mode_protocol.md` ("Same-family / rubric-aware judging"). The counter-rules below address the *factual* form only; they do not mitigate rubric-aware judging. To counter same-source hallucination:
...
| Trigger timing | ars-deep-research Phase 5 | pipeline Stage 2.5 + Stage 4.5 |
...
Reference: `ars-deep-research/references/guides/semantic_scholar_api_protocol.md` (see for query patterns, matching rules, and rate limits)
...
Reads the `figure_table_trace[]` block in the visualization_agent's Figure Package (see `ars-academic-paper/references/guides/vlm_figure_verification.md`). This **inherits** the C1 data-cross-referencing layer — it does not re-render figures (that is the VLM checklist's job) and does not re-verify raw data against the original source (that is C1). Its genuinely new coverage is the *interpretation* and *linkage* a faithful-rendering check cannot see, the visual analog of the / prose partial-evidence trap.
...
(2) Caption-claim support
    ...
    - If the caption_claim is compound ("accuracy improves AND variance decreases"),
      ...
      — borrow the  decomposition AS PROSE GUIDANCE ONLY (no PARTIAL verdict, no
...
- Anti-skip rule: an UPDATED Figure Package that exists but omits figure_table_trace[]
  ...
  FAIL ("caption fidelity not verified"), so the  check cannot be silently dropped by
...
(D7) Declaration-anchored anti-skip — run FIRST, before any provenance read.
    Determine the legacy boundary fail-closed (default = treat-as-post-, NOT legacy):
    - A passport is legacy_unknown (advisory) ONLY with POSITIVE proof it predates :
      repro_lock.ars_version present AND < the  release constant (frozen here at ship
      time). Everything else — no repro_lock, repro_lock with no ars_version, ars_version
      >= constant — is treated as post-. Version-unprovable is NOT legacy.
    ...
    #2/#3 in the lint; the fourth is the heuristic you run here):
      1. treated-as-post- AND experiment_intake_declaration is absent → FAIL
...
- PASS WITH NOTES (advisory, never silent):
  ...
  - a LEGACY passport (positive pre- ars_version proof) with no provenance block.
- Anti-skip rule: a passport that references experiment results but omits experiment_provenance[]
  is a FAIL (treated-as-post- default + the D7 declaration check), NOT the legacy advisory
  case — so the  check cannot be silently dropped by shipping a manuscript whose
...
See `references/guides/plagiarism_detection_protocol.md` for the complete protocol definition. Below is an executive summary.
...
See `references/guides/claim_verification_protocol.md` for the complete protocol definition. Below is an executive summary.
...
5. Assign every registered claim a stable claim_id and emit `claim-registry/1.0` (`references/shared/contracts/evidence/claim_registry.schema.json`): bind the exact draft raw SHA-256; record an exact UTF-8 byte span whose bytes equal claim_text, claim kind(s), cited source(s) by ref_slug, writer anchors, paper section, and selection tier ( — Mode 1: HIGH-IMPACT / RANDOM / TOP-UP / NOT-SELECTED; Mode 2 artifact tier: ALL, meaning all registered claims). Duplicate ids/spans, stale draft binding, or unequal span text are invalid.

Output: schema-valid Claim Registry artifact. A table is only a rendered view.
```

#### E1.1. Mechanically Detectable Coverage Diff

Run 自动校验脚本（移植版未附带，以文字规则为准） on the exact raw draft and exact
...
#### E2. Source Tracing
```
For each selected claim (Mode 1: the  stratified selection — HIGH-IMPACT / RANDOM / TOP-UP tiers; Mode 2: the whole registry):
...
#### E3.1. Persisted Evidence Rows

For every selected tuple, use 自动校验脚本（移植版未附带，以文字规则为准） to build and validate
one persisted row against
`references/shared/contracts/evidence/evidence_row.schema.json`. The generic contract is
...
```
- Mode 1 (pre-review) — risk-stratified (, mirroring the  reference-verification tiers):
  - HIGH-IMPACT claims — verify 100%, no cap. A claim is high-impact if it is: (a) a headline conclusion (abstract- or conclusions-level), (b) numerical (statistic, effect size, percentage, threshold), (c) causal, (d) methods-critical, or (e) disputed (already carrying a contradiction disclosure or reviewer split). Same definition family as `references/shared/cross_model_verification.md` step 2.
  ...
  - Floor: if the two tiers together select fewer than min(10, total claims), top up at random from the remainder; a paper with fewer than 10 claims total is audited in full (preserves the pre- minimum).
...
- Execute Phase A (all) + Phase B (30%+ spot-check) + Phase C (all) + **Phase D (30%+ spot-check)** + **Phase E (risk-stratified claim check, )**
- Phase D executes D1 (paragraph-level originality check, sampling rate >= 30%) + D2 (self-plagiarism check, if author name provided)
- Phase E executes semantic/model-mediated E1 extraction to create the registered population, then E1.1 reports bounded candidate gaps with semantic completeness unknown. E2 (source tracing) + E3 (cross-referencing) run on the  risk-stratified registry selection: 100% of registered HIGH-IMPACT claims + a 10% RANDOM sentinel of the registered remainder, topped up to min(10, registered total) — fewer than 10 registered claims total → audit the whole registry
- **Phase C4: the D7 declaration-anchored anti-skip runs on the passport (not sampled — it is a single passport-level check); experiment_alignment_results[] rows are produced for the sampled experiment-backed claims (>= 30% — C4's own rate; the general claim check is  risk-stratified, no longer a flat 30%).**
...
- **Phase C3 (Figure/Table Caption Fidelity) runs on every `figure_table_trace[]` entry.** If an updated Figure Package exists but carries no `figure_table_trace[]` block (or omits an entry for a figure it contains), that is a **FAIL** ("caption fidelity not verified") — not a clean pass and not the advisory case (otherwise the  check is trivially skippable). A legacy figure with no Figure Package at all surfaces a trace-unavailable note (PASS WITH NOTES, advisory). The full per-condition severity map is in Phase C3 above.
- **Phase C4 (Experiment Provenance & Claim Alignment, ) runs the D7 declaration-anchored anti-skip on every passport and produces `experiment_alignment_results[]` for EVERY experiment-backed claim (full, not sampled, at Stage 4.5).** A treated-as-post- passport with the `experiment_intake_declaration` absent is a **FAIL** (even a literature-only run needs `{status: no_experiments_declared}`); a passport referencing experiment results but omitting `experiment_provenance[]` is a **FAIL**, not the legacy advisory case. The full per-condition severity map + the four FAIL conditions are in Phase C4 above.
...
| Claim Verification (E) | X of [registry total] (Mode 1:  tiers — HIGH-IMPACT: X, RANDOM: X, TOP-UP: X; NOT-SELECTED: X. Mode 2: ALL_REGISTERED: X; semantic extraction coverage: not_machine_detectable; candidate gaps: X) | X | X (MAJOR_DISTORTION / UNVERIFIABLE) |
...
**Claim Registry coverage sidecar** — replay-validate before rendering:
...
**Persisted Phase E evidence rows** — retain the complete ordered
`phases.E_claims.evidence_rows[]` array with 自动校验脚本（移植版未附带，以文字规则为准） only.
...
is positively identified as pre- and lacks the field, the consumer may use
...
**Token-conservation advisory (, revision rounds)** — the deterministic `ADV-REV-<n>` signal from 自动校验脚本（移植版未附带，以文字规则为准） (see `pipeline_orchestrator_agent.md` step 3a); advisory-only, not counted in verdicts, empty when every patch op conserved its numeric/citation/protected-term tokens:
...
**Closed transport selector:** For these one-reference integrity calls only,
...
`citation_context`), pipe it to 自动校验脚本（移植版未附带，以文字规则为准）, and validate
the input against
`references/shared/contracts/cross_model/codex_citation_request.schema.json` and
the returned one-line object against
`references/shared/contracts/cross_model/codex_citation_receipt.schema.json` before consuming
...
- After Phase A completes, select references by **risk stratification** (; replaces the pre- uniform random 30%). Four tiers; a reference qualifying for more than one gets the highest tier that applies (`HIGH-IMPACT` > `NEW-CHANGED` > `CONTROL`/`RANDOM`) and is verified once:
...
|-----------|------------|
| Coverage | Registered references 100%; registered statistical/data surfaces 100%; registered citation contexts >= 30% (initial) / 100% (final); originality >= 30% (initial) / >= 50% (final); registered-claim verification  risk-stratified (initial: 100% registered HIGH-IMPACT + 10% registered random sentinel, min(10, registered total)) / 100% of registry (final). Semantic registry completeness remains unknown. |
