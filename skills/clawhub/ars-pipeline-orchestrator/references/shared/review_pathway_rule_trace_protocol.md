# Review-pathway rule trace protocol

This protocol governs the candidate-only, deterministic rule trace built by
自动校验脚本（移植版未附带，以文字规则为准）。 The trace displays selected-profile
...
`unresolved_predicates`. This does not open or change 's
...
The validator replays , rebuilds the request coverage and complete expected
...
After replay, call 自动校验脚本（移植版未附带，以文字规则为准） with the one explicit
...
The trace never supplies an action assignment. For action planning, the normal
 gate and exact actor/consumer-scope rules still apply independently. A
...
`ars-deep-research/references/guides/irb_decision_tree.md` is a derived navigation aid. Its
