# Cross-document Consistency Advisory Protocol (``)
...
- `preregistration-artifact/1.0`, the persistent ars-deep-research handoff sidecar;
...
/; this family adds no consent/protocol role, enum, observation, or
consumer.
...
Before the ars-deep-research handoff, a shell-capable orchestrator invokes:

```bash
python 自动校验脚本（移植版未附带，以文字规则为准） \
...
`ars-deep-research/references/templates/preregistration_template.md` is guidance only and can
...
`references/shared/claim_strength_ladder.md` is semantic caller guidance only.
...
`NO_LISTED_INCONSISTENCY_LOCATED`. Conversely, a 
...
verdict, gate result, convergence decision, or `audit_artifact_entry`.
 ...
 never invokes the legacy runner, consumes an old audit output, or writes
...
## Pipeline coexistence with 

 tortured-phrase screening and  run inside the **single existing**
mandatory Stage-5 entry checkpoint after the same exact Stage 4.5 PASS. The
orchestrator runs  first and  second and surfaces two independent
...
Neither carrier consumes, suppresses, aggregates, or changes the other. A 
loaded-snapshot failure may still write its schema-valid degraded artifact and
exit 1; the checkpoint preserves and validates it. A  contract/runtime
...
integrity review, obtain a new exact Stage 4.5 PASS, and rerun  then 
...
On  failure, no schema-valid output is created or replaced. Only a closed
