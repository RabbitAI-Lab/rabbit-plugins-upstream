# Deterministic Submission-Packet Manifest Protocol
...
Accept the  authority inputs only as a complete context, registry, and
...
is canonical-hashed only to verify its  pointer and digest. Those rows concern
semantic consent or information coverage and belong to . Do not interpret or
...
- copy  context and registry pointers and the resolved digest exactly; and
...
seven-fact order. Never use  `trace_order` as a semantic sort key.

Do not compute a raw context digest.  uses a semantic digest that excludes
...
- applicability unknown: never crosses the  gate.
...
Rendered output includes these four fixed  status lines:
...
##  handoff
 ...
 consumes only a manifest that first passes
...
自动校验脚本（移植版未附带，以文字规则为准）, and its closed carrier/protocol are
`references/shared/contracts/human_subjects/content_coverage_advisory.schema.json` and
`references/shared/authority_content_coverage_advisory_protocol.md`.
