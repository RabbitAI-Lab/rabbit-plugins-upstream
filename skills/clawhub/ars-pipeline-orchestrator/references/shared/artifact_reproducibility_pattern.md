...
repro_lock:
  ...
  model:
    ...
    id: "<model-id-at-run-time>"           # model identifier at run time, e.g. the inherited agent session model (not weight hash)
...
`references/shared/handoff_schemas.md`). A missing key fails the lint. A null value passes with a
