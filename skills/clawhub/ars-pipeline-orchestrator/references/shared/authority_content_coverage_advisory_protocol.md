# Authority-profile Content-Coverage Advisory Protocol
...
- `references/shared/submission_packet_manifest_protocol.md`
- `references/shared/evidence_row_protocol.md`
- 对应设计文档（移植版未附带）
...
`render_advisory` in 自动校验脚本（移植版未附带，以文字规则为准）。 The CLI
...
into a  `entries[].status`, packet observation, readiness, authorization,
...
Group  entries by their exact `requirement_ref`; do not merge parallel
...
- every matching  entry as a `deterministic_entry_refs[]` row containing its
  complete `evidence_ref`, responsibility, status copied under
  `deterministic_status`, reason codes, and matched artifact ids.

An applicability-false  `excluded_requirements` row is copied into the final
...
Packet prose enters only through an explicit in-memory map keyed by exact 
...
digest and byte size are copied from the matching  packet observation and
...
Quotes use 's strict one-time percent decoding: malformed escapes and invalid
...
`references/shared/contracts/human_subjects/content_coverage_advisory.schema.json#/$defs/advisory_draft`,
...
`context.confirmed_at`:  deliberately excludes it from the semantic context
...
The  resolved/downstream gate and the  capability envelope must be open.
...
institutional or funder overlay selection:  retains the selected base
requirement entries while reporting `OVERLAY_SELECTION_NOT_PROVIDED`. For each
such requirement,  emits `not_checked`,
...
Validation replays  and every source-bound evidence row against the same
