...
applies_to: "references/shared/compliance_agent.md"
---
...
**Used by**: `compliance_agent` (Task 8), `ars-pipeline-orchestrator/SKILL.md` Stage 2.5 / 4.5.
...
|---|---|---|
| `[MATERIAL GAP]` | `gaps[].reason`, `principle_evidence[]`, `evidence[]` | The item cannot be verified because the underlying material (passport field, manuscript section, supplementary file) is missing. Apply [Anti-Leakage Protocol](ars-academic-paper/references/guides/anti_leakage_protocol.md) — do not hallucinate. |
...
### Advisory adjudication-activity receipt
...
the complete qualifying-override predicate in the frozen  spec: a
...
- `<item_title>`: look up the short title from `references/shared/prisma_trAIce_protocol.md` (e.g. `M4` → "Input data").
...
When invoked from `ars-academic-paper full` (primary research standalone), no pipeline state machine is available. Compliance_agent:
