...
**Canonical location**: `references/shared/collaboration_depth_rubric.md` in `Imbad0202/academic-research-skills`. External consumers should reference by stable URL; do not vendor (bump the `rubric_version` field on any modification).
...
This rubric operationalizes their framework as a post-hoc observer signal for the agent runtime agents. It is used by `ars-pipeline-orchestrator/references/collaboration_depth_agent.md` to score dialogue logs and produce **advisory, non-blocking** feedback to the user on how their collaboration pattern is shaping learning depth.
...
- Replace AI self-reflection protocols (e.g., `ars-pipeline-orchestrator` Stage 6's six-dimension Collaboration Quality Evaluation, which is AI-about-itself; this rubric is AI-about-user-collaboration).
