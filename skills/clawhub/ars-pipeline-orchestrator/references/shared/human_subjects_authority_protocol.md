# Human-Subjects Authority Selection and Resolution Protocol
...
-  consumes a serialized authority result only after
  ...
  to .
-  consumes authority rows only through a  manifest that has first been
  ...
  through `references/shared/authority_content_coverage_advisory_protocol.md`.
...
-  remains valid in `artifact_agnostic` mode and no profile may rewrite its
  source correspondence or committee authority.
-  consumes every selected-profile requirement scoped to `pathway_trace`
  only after exact  replay, then projects exact fact occurrences,
  authoritative-decision-maker role ids, and authority anchors under
  `references/shared/review_pathway_rule_trace_protocol.md`. Its caller-owned
...
-  owns correction and migration of mixed-jurisdiction reference prose.

 itself changes no user-facing output. It emits no verdict, pathway,
readiness, authorization, conformance result, or timeline. The  independent
status grammar and fixed non-authorization footer remain unchanged.

##  capability-envelope facts
...
or non-matching value makes  return `APPLICABILITY_UNRESOLVED`; it never
means that an authority is legally inapplicable. The complete  contract is
