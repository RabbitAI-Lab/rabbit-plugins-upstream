...
```
Entering Stage 1 (RESEARCH) -- using ars-deep-research socratic mode

Loading ars-deep-research SKILL.md...
...
```
Entering Stage 2 (WRITE) -- using ars-academic-paper plan mode

Loading ars-academic-paper SKILL.md...
handoff materials: RQ Brief + Bibliography + Synthesis
intake_agent auto-detected ars-deep-research output, skipping redundant Phase 0 questions
...
ars-academic-paper draft_writer_agent writing based on Chapter Plan...
...
```
Entering Stage 3 (REVIEW) -- using ars-academic-reviewer full mode

Loading ars-academic-reviewer SKILL.md...
...
I'll pass the Revision Roadmap to ars-academic-paper revision mode,
...
```
Entering Stage 4 (REVISE) -- using ars-academic-paper revision mode

Loading ars-academic-paper SKILL.md (revision mode)...
...
- Apply report sidecar (`<output>.apply-report.json`,  — retained beside the Revised Draft; transferred to Stage 3' as untouched-block evidence)
...
Loading ars-academic-reviewer SKILL.md (re-review mode)...
Passing Revised Draft + Response to Reviewers + Editorial Decision Letter ( Judge Record input) + Round-1 Revision Roadmap + apply report(s) + Round-1 Reviewer Configuration Cards (yardstick continuity)...
...
```
Entering Stage 5 (FINALIZE) -- using ars-academic-paper format-convert mode

Loading ars-academic-paper SKILL.md (format-convert mode)...
...
- Revise again --> just say "revise the paper" (triggers ars-academic-paper revision mode)
