...
- [`references/shared/contracts/passport/literature_corpus_entry.schema.json`](references/shared/contracts/passport/literature_corpus_entry.schema.json)
- [`references/shared/contracts/passport/rejection_log.schema.json`](references/shared/contracts/passport/rejection_log.schema.json)
...
The original ARS repository provides three Python reference adapters in the original repository's adapter examples (not shipped in the portable edition). Users are expected to write their own adapters for non-reference sources. The three reference adapters are starting points, not production tools.
...
Refer to the [`literature_corpus_entry` schema](references/shared/contracts/passport/literature_corpus_entry.schema.json) for the authoritative contract. The field tables below are generated from that schema and must not drift.
...
| `bibliographic_integrity_signals` | array |  additive v1 carrier. |
| `contamination_signal_omissions` | object |  Part A reason-provenance for degraded-lookup omissions. |
| `contamination_signals` | object | v3.7.3 + v3.9.0 + v3.11 contaminated-source advisory field (spec v3.7.3 §3.2 + v3.9.0 §3.4–§3.5 + v3.11  Delta 1). |
| `contamination_signals_backfilled_at` | string | v3.7.3 backfill provenance (issue ). |
...
1. Validate against the schema: `python 自动校验脚本（移植版未附带，以文字规则为准） --passport <your_passport.yaml> --rejection-log <your_rejection_log.yaml>`.
...
The three reference adapters have pytest coverage under the original adapter test examples (not shipped) — copying that pattern for your own adapter is a good starting point.

## 10. Relationship to other ARS artifacts

- [`references/shared/handoff_schemas.md`](references/shared/handoff_schemas.md) Schema 9: the `literature_corpus[]` field lives inside the Material Passport.
- [`ars-pipeline-orchestrator/references/guides/passport_as_reset_boundary.md`](passport_as_reset_boundary.md): `literature_corpus[]` is consumed across reset boundaries like any other passport field.
