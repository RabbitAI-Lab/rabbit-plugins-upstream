...
Guidelines for coordinating multi-person academic research teams using the pipeline. The skill runs as a single-user agent session; this protocol documents the **human coordination layer** that wraps around pipeline executions.
...
| **Materials** | RQ Brief, Bibliography, Synthesis Report (conforming to Schemas 1-3 in `references/shared/handoff_schemas.md`) |
...
| **Who hands off** | Lead Author |
| **Who receives** | Review Coordinator |
| **Materials** | Revised Draft + hard-required Original pre-revision Draft + immutable Round-1 Revision Roadmap + exact author-adjudication sidecar + fully replayed Revision-Evidence Bundle + Response to Reviewers + Editorial Decision Letter + Round-1 findings + exact ordered patch/report pairs matching the bundle + Round-1 Reviewer Configuration Cards. This is the default current  1.1 re-review transfer; missing original/roadmap/author/bundle fails closed. A user-requested fresh full review at 3' is a separate branch with Revised Draft + available context. |
...
- Version labels must match the Material Passport `version_label` field (Schema 9 in `references/shared/handoff_schemas.md`)
...
**Deadline**: [date]
**Return Format**: [Use Schema 6 format from references/shared/handoff_schemas.md]
```
...
Person A (Research Lead + Lead Author):
  - Runs Stage 1 (RESEARCH) with their agent session
  - Runs Stage 2 (WRITE) with their agent session
...
Materials: conform to schemas in references/shared/handoff_schemas.md
```
...
- The skill runs as a single-user agent session; this protocol documents the HUMAN coordination layer
- Each team member runs their own pipeline stages independently in separate agent sessions
- Handoff materials (conforming to schemas in `references/shared/handoff_schemas.md`) ensure consistency across sessions
