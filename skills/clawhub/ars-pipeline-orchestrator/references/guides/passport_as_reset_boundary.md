...
4. **Emit human instruction.** In the same checkpoint notification, include a `### Resume Instruction` subsection with:
   ...
   - The exact resume command the user pastes into a fresh session:
     ...
     ```
   - A one-line note that the next stage should be invoked in a fresh session to realize the token-savings intent.
...
- Does not claim specific token savings numbers. Empirical measurement goes in 性能说明文档（移植版未附带） only after real runs.

## Related references

- [`references/shared/handoff_schemas.md`](references/shared/handoff_schemas.md) — Schema 9 definition
- [`ars-pipeline-orchestrator/references/pipeline_orchestrator_agent.md`](references/pipeline_orchestrator_agent.md) — orchestrator integration
- [`ars-pipeline-orchestrator/references/guides/pipeline_state_machine.md`](pipeline_state_machine.md) — state transitions
- [性能说明文档（移植版未附带）](性能说明文档（移植版未附带）) — long-running session guidance
