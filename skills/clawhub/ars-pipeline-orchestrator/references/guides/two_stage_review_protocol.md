...
See `ars-academic-reviewer/SKILL.md` for review process details.
...
- **Input**: Revised draft + hard-required original (pre-revision) draft ( current 1.1 §3.1 Phase 2A comparison base; absence is `manifest_incomplete`, because the required bundle already carries these bytes) + Response to Reviewers + original Revision Roadmap + exact author-adjudication sidecar + fully replayed Revision-Evidence Bundle + Editorial Decision Letter ( — its Review Panel Provenance block feeds the Judge Record) + Round-1 review findings (Schema 6 reports —  §4 level-3 criterion layer) + the exact ordered apply report/patch pairs projected by the bundle (/ §11) + Round-1 Reviewer Configuration Cards (yardstick continuity — field_analyst NOT re-run; `ars-academic-reviewer/references/guides/re_review_mode_protocol.md` § Yardstick Continuity). This input set describes re-review mode, the default Stage 3' (dispatched under the  three-gate contract; legacy single-pass only behind the `ARS_RE_REVIEW_LEGACY` option); a user-requested fresh full review at 3' instead (mid-entry quick→full path) takes the revised draft + available context only and runs full mode, field_analyst included
- **Mode**: `ars-academic-reviewer` re-review mode (default); the user-requested fresh full review at 3' dispatches full mode instead
...
See `ars-academic-reviewer/SKILL.md` Re-Review Mode for verification review process.
