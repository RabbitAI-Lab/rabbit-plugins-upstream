...
- For each claim, assign a stable `claim_id` and emit the closed `claim-registry/1.0` artifact defined by `references/shared/contracts/evidence/claim_registry.schema.json`. Every row records exact UTF-8 draft byte span + equal `claim_text`, claim kind(s), cited source(s) by `ref_slug`, writer anchors, paper section, and selection tier ( — Mode 1: `HIGH-IMPACT` / `RANDOM` / `TOP-UP` / `NOT-SELECTED`; Mode 2: `ALL`). The registry binds `draft_raw_sha256`; duplicate IDs/spans and stale or unequal spans are invalid.
- For a claim that satisfies the high-impact definition, the registry row also records WHICH of the five criteria fired (`high_impact_basis` ⊆ {headline_conclusion, numerical, causal, methods_critical, disputed}) — an optional `claim-registry/1.0` row field: the tier says that it is high-impact, the basis says why; the  claim-standing probe offer below consumes the recorded basis
...
After E1, run 自动校验脚本（移植版未附带，以文字规则为准） on the exact raw draft bytes
...
## E2: Source Tracing
- For each SELECTED claim (Mode 1: the  risk-stratified selection — tiers `HIGH-IMPACT` / `RANDOM` / `TOP-UP`; Mode 2: every claim in the registry), locate the specific passage in the cited source that supports it
...
## E3.1: Evidence-Row Persistence

The current producer MUST use 自动校验脚本（移植版未附带，以文字规则为准） to build and validate
...
## E5: Novelty-Claim Classification ( — advisory-only)
...
自动校验脚本（移植版未附带，以文字规则为准） build`. Each transient input row
...
```bash
python 自动校验脚本（移植版未附带，以文字规则为准） build \
...
python 自动校验脚本（移植版未附带，以文字规则为准） validate \
  --finding-set <claim-strength-drift-findings.json> \
...
**Trigger (design §3.1, gate 1 — enforced by 自动校验脚本（移植版未附带，以文字规则为准）):**
...
**Consent (design §3.2, gate 2):** Eligibility never dispatches anything. Before any query planner, index, or model receives claim text, the researcher sees and affirmatively accepts a closed consent surface (`propose` → `bind` in 自动校验脚本（移植版未附带，以文字规则为准）) whose hash binds the complete consentable-plan projection; absence of that acceptance, any post-proposal change (claim, query, provider, filter, cap, stance plan, persistence), or an explicit cancel produces an explicit local `not_checked` declination record (`consent_absent` / `consent_invalidated` / `consent_cancelled`) and no network or model call. Retrieval, stance classification, freshness, and per-event transmission accounting are specified in `references/shared/claim_standing_candidate_ledger_protocol.md`. Every probe surface carries `STANCE CLASSIFICATION UNMEASURED` until the  baseline measurement row exists.
...
External motivation: Ren et al. (2026, arXiv:2607.13104): §3.3 frames active data-acquisition as targeting frequent failure modes and verifier disagreement; §9.2 frames improvement as resource optimization (gating expensive evaluations, penalizing waste). The high-impact-first allocation here is ARS's design inference from those principles, mirroring 's reference-verification shift.
...
自动校验脚本（移植版未附带，以文字规则为准）。 The default and maximum page size are 25. Render
...
A positively identified pre- Schema 5 report that has no `evidence_rows`
