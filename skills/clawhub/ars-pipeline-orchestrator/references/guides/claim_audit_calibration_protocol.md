...
1. **Gold set**: 25 tuples (17 alignment incl. 5  partial-support fixtures + 8 constraint per the canonical shipped fixture at transport test fixtures (not shipped in the portable edition)). Per-tuple shape and the four ingestion-gate rules (alignment-judgment coverage, ≥3 NOT_VIOLATED floor, required constraint fields, tuple_kind validity) are spelled out in **Gold tuple schema** below + spec §7.7 rules (a)-(d). The shipped fixture is the canonical example.
...
```jsonc
{
  ...
  "manifest_fixture_path": "transport test fixtures (not shipped in the portable edition)"     // OR this
}
```
...
The shape is the canonical contract 自动校验脚本（移植版未附带，以文字规则为准） (T-C1 / T-C2) pins against. Adding fields is non-breaking; removing or renaming is a spec bump.

**Partial-support subset (`partial_support`, ).** Gold tuples tagged `expected_prompt_verdict=PARTIAL` carry `expected_judgment=UNSUPPORTED` (the B1 normalization), so a judge that **stops decomposing and emits a bare `UNSUPPORTED`** matches the aggregate gate and hides the exact regression  closes. The subset metric counts a partial fixture as passed ONLY when the judge emits `UNSUPPORTED` AND a well-formed true-partial `sub_claim_breakdown` (≥2 items, ≥1 SUPPORTED AND ≥1 non-SUPPORTED — the INV-19 definition). `miss_rate = misses / n_partial`; a bare-`UNSUPPORTED` judge registers `miss_rate=1.0` here while the aggregate FNR stays green. The block is additive — a gold set with no partial fixtures yields `{"miss_rate": 0.0, "n_partial": 0}`.
...
- Replace the post-calibration ramp-on plan recorded in `ars-pipeline-orchestrator/SKILL.md` mode flags. v3.8.0 ships the audit agent dispatch as opt-in default-OFF; T-C1 passing on the canonical gold set is necessary but not sufficient evidence for default-ON.
...
**Partial-evidence regression IS caught.** One failure the aggregate gate alone would NOT catch: a judge that regresses to emitting bare `UNSUPPORTED` on compound claims instead of decomposing them. Because partial fixtures carry `expected_judgment=UNSUPPORTED`, the aggregate FNR stays green on that regression. The `partial_support` subset metric exists precisely to surface it — `miss_rate > 0` means the judge stopped producing inspectable `sub_claim_breakdown` decompositions even though it got the headline label right. Treat a non-zero `partial_support.miss_rate` as a judge-quality regression on the §F.3.2 partial-evidence trap, distinct from an aggregate FNR/FPR threshold breach.
...
|  clearance | Calibration does not interact with mark-read state; HIGH-WARN-CLAIM-NOT-SUPPORTED stays non-clearable regardless of calibration outcome. |
...
- **Activation**: opt-in. The audit agent ships with default thresholds and the canonical fixture; operators run the calibration checker (not shipped; rules apply) as a CI gate. Re-calibration with a domain-specific gold set is the operator's call, not auto-triggered.
...
The CI-equivalent invocation lives in `.github/workflows/spec-consistency.yml` (the v3.8 `` unittest step). Both paths exercise T-C1 (threshold gates), T-C2 (per-class FNR/FPR reporting), and T-C3 (gold-set shape integrity).

---

## References

- Spec: 对应设计文档（移植版未附带） §7.7 (test contract) + §1 deliverable 7 (this doc) + §9 (acceptance criteria) + §13 step 10 (implementation order).
- Issue: [academic-research-skills ](https://github.com/Imbad0202/academic-research-skills/issues/103) acceptance criterion (FNR < 0.15 + FPR < 0.10 gates).
- Parent agent: `ars-pipeline-orchestrator/references/claim_ref_alignment_audit_agent.md` — dispatches calibration mode and consumes the report's thresholds block. The agent prompt and this doc form a two-way pair following the v3.6.5 protocol-doc convention (`literature_corpus_consumers.md` ↔ `bibliography_agent.md`).
- Reviewer calibration baseline: `ars-academic-reviewer/references/guides/calibration_mode_protocol.md` (same vocabulary, different unit of analysis).
