...
2. Verification scope:
   ...
   - Phase E: risk-stratified claim verification: 100% of HIGH-IMPACT claims (headline / numerical / causal / methods-critical / disputed) + 10% random sentinel of the remainder (rounded up, min 3 / max 10; remainder <3 → all of it), topped up to min(10, total claims) — bounds per `claim_verification_protocol.md` § Sampling Strategy
   - Phase E additionally emits the scope-conformance advisory and the novelty-claim classification — both advisory-only, never gate; advisory rows are not issues and may remain open on PASS — see `claim_verification_protocol.md` § E4-E5
3. Result handling:
...
2. Verification scope:
   ...
   - Phase E additionally emits the scope-conformance advisory and the novelty-claim classification — both advisory-only, never gate; advisory rows are not issues, stay outside the zero-issues PASS count, and may remain open — see `claim_verification_protocol.md` § E4-E5
...
## Tortured-Phrase Advisory Boundary
...
explicit `not_checked` artifact. The  path
...
## Cross-Document Consistency Advisory Boundary

 is not a sixth integrity phase. After the same exact Stage 4.5 PASS, it runs
second, after , inside the one existing mandatory Stage-5 entry checkpoint.
Both bind the identical accepted draft: 's
`input_binding.artifact.artifact_id/artifact_sha256` must equal 's
...
Before observations are consumed, the  finalizer replay-validates the exact
...
duplicate, or agreement/clean meaning. A  contract failure writes no carrier
...
formatter/Stage-5 routing. A later manuscript revision stales both  and 
and must re-enter integrity before both rerun in fixed order.

## Criterion Trajectory Tracking

Reference: `ars-pipeline-orchestrator/references/guides/score_trajectory_protocol.md`
