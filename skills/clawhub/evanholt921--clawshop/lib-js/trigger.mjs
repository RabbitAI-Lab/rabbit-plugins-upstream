...
async function doRecommend(url) {
  ...
  try {
    await ct.cronStartPolling(sessionKeyArg || await _detectWebchatSessionKey());
  } catch (e) {
...
// ========== 自动检测当前 webchat session key ==========
async function _detectWebchatSessionKey() {
  // webchat 渠道固定用 agent:main:main，不再自动猜测
...
main(rawArg).then(out => {
  if (isWebchat) {
    ...
    let wcOutput = ct.webchatFlush();

    // 读取 poll-loop fallback 暂存文件（sessions_send 失败时写入的内容）
    const PENDING_PATH = join(dirname(__dirname), 'data', 'webchat_pending_output.txt');
    try {
      if (existsSync(PENDING_PATH)) {
        const pending = readFileSync(PENDING_PATH, 'utf-8').trim();
        if (pending) {
          wcOutput = pending + (wcOutput ? '\n' + wcOutput : '');
        }
        // 读完即删
