---
name: qinghu-shortvideo-data-engine
description: 青虎AI 短视频数据引擎：批量传入抖音、小红书、B 站的视频长链接，自动抓取每条视频的播放、点赞、分享、收藏、评论等全维度数据并导出 Excel，替代人工统计，用于监测自有与竞品带货视频的热度和转化表现。当用户要批量统计短视频数据、追踪视频每日表现、监测竞品视频、导出视频数据表、对比多条视频数据时必须触发。关键词：青虎AI、短视频数据、视频数据统计、抖音、小红书、B站、播放量、点赞、收藏、评论、Excel、竞品监测、数据引擎。
user-invocable: true
...
一次传最多 20 条视频链接，自动跑出**播放、点赞、分享、收藏、评论**全维度数据并导出 Excel。

替代的是每天手工抄数据那件事——尤其适合监测自有和竞品带货视频的热度变化。

## 何时触发

- 「帮我统计这些视频的数据」「批量拉一下这几条视频的播放点赞」
- 「监测竞品视频的表现」「导出视频数据表」
- 「这些抖音/小红书/B 站视频的数据对比一下」

## 使用配方

```bash
# 字段表（线上定义）
qhkit workflow options '{"workflow":"wf_053"}'

# 报价：按链接条数计费（per_count），条数越多积分越高
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| 视频链接(长链) | 是 | 抖音 / 小红书 / B 站视频链接，**最多 20 条**，一行一个 |

**必须用长链**：`v.douyin.com/xxx` 这类分享短链取不到数据，让用户从平台里复制完整链接。

**计费按条数**：20 条 ≈ 20 × 单价，报价时把条数和总积分一起说清楚。

**要每天自动更新**：本技能只做单次提交。定时任务需要用户到青虎工作台「AI 应用 - 定时」里配置周期与时间。
...
- 只支持抖音 / 小红书 / B 站三个平台的**视频链接**；博主主页链接走「达人数据引擎」。
- 定时更新要在青虎工作台配置，CLI 侧只做单次提交。
- 产出是 xlsx 数据表，不做数据解读——需要分析结论时把表交给对应的社媒运营技能。

## 相关技能

同系列的其他「青虎AI」技能（按需转交，不要在本技能里硬做）：

- **达人数据引擎**：追踪博主账号维度的每日数据
- **抖音 / 小红书 / B站-社媒运营专家**：拿到数据后的选题与投放解读
