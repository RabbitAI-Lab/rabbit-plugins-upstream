...
2. **写页前只需读两份契约**:`reference/design-principles.md`(设计)+ `reference/html-spec.md`(转换)。
   ...
   → **再选原型**(查 `reference/page-archetypes.md` 的索引跳到目标原型,别通读 1000+ 行)
   → **起手用现成件**:`assets/snippets/INDEX.md` 找片段,或 `assets/slide-template{,-flex,-layout}.html` 三套样板页(方式 A/B/C)
   → 容器内部写法查 `reference/layout-recipes.md`;要图标查 `assets/icons.md`(stroke 显式 hex,**禁 currentColor**)
...
```
把每页截成 PNG(slides/.preview/),**逐张看图**,按六条自查:
...
7. **限定词**(2026-08-06 第六轮起,有深化层时):对照 `deck-insights.md` 的洞察清单 ——
   本页有没有"反向验证=降级"的论断?有则必须带限定词(我们认为/有待验证/预估…),
   并确认该页已在 `slides.config.json` 的 `hedgePages` 里。**把假设写成结论是 AI PPT 最典型的病**
发现问题**就地修复并重截**,直到七关全过,再进 Step 4。
...
| 文件 | 什么时候需要 |
|---|---|
| `reference/page-archetypes.md` | 选页面形态时 —— **查索引直接跳到目标原型,别通读**(1000+ 行) |
...
| `reference/content-deepening.md` | 需要四步追问/多角色/委员会(素材×深度矩阵定做什么) |
| `reference/narrative-skeletons.md` | 定全篇叙事结构 |
| `reference/theme-presets.md` / `brand-styles.md` | 选色板或蒸馏品牌风 |
| `assets/icons.md` | 要图标时复制(stroke 显式 hex,**禁 currentColor**) |
| `assets/snippets/INDEX.md` | 找现成片段(28 个,含参数表与使用限制) |
