...
- `design.formProfile` 由 brief 视觉形式偏好映射(克制型→text/平衡型→balanced/丰富型→rich);
  配置后 validate 追加 deck 级形式检查(纯文字页占比/同形式连排,见 design-principles 第五章)
...
2. **先读设计契约,再按二级流程选页面形态**:
   - `reference/design-principles.md` — 设计总纲(版面分区/字号阶/scale-to-fill/去 AI 味 12 条反模式/**视觉形式**),**写页前必读**
   - **先定形式**(brief 大纲表"形式"列:文字/图示/图表/图片/混合)→ **再选原型**:`reference/page-archetypes.md`(28 个原型;图示需求查组 6 · 原型 23-30)
   - `assets/icons.md` — **40 枚内联 SVG 图标**(并列要点/色带/对比卡配图示用;stroke 必须显式 hex,禁 currentColor)
   - `assets/slide-template.html`(方式 A 要点列表)/ `slide-template-flex.html`(B 图标网格)/ `slide-template-layout.html`(C 大数字带)
   - `assets/snippets/`(片段库:议程/分隔/大字观点/引用/编辑双栏/出血大图 + 页头/卡片/KPI/流程/时间线/对比/代码/表格/图标网格等,INDEX.md 有参数表与使用限制)
...
```
把每页截成 PNG(slides/.preview/),**逐张看图**,按五条自查:
...
- `reference/design-principles.md` — **设计总纲(版面分区/字号阶/scale-to-fill/去 AI 味 12 条/视觉形式;写页前必读)**
- `reference/page-archetypes.md` — **28 个页面原型库(写页首选起点;组 6 = 图示原型 23-30)**
- `reference/narrative-skeletons.md` — 4 套叙事骨架(脚手架非模具;形式+原型双列推荐)
...
- `assets/icons.md` — **40 枚内联 SVG 图标库(复制即用;stroke 显式 hex,禁 currentColor)**
- `assets/deck-brief.template.md` — 需求确认卡模板
- `assets/slide-template{,-flex,-layout}.html` — 三套样板页(方式 A/B/C,各演示一个原型)
- `assets/snippets/` — 28 个片段(6 原型直出 + 8 图示构件 + 基础构件;INDEX.md 索引与使用限制)
