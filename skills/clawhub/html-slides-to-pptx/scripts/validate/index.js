...
async function run(args) {
  ...
  // 缺省/text = 休眠(旧项目零新增 WARN);balanced/rich 才启用。口径见 design-principles 第五章。
  const FORM_LIMITS = { balanced: { ratio: 0.5, streak: 3 }, rich: { ratio: 0.3, streak: 2 } };
  ...
  if (formLimit && pageStats.length) {
    ...
    if (content.length >= 4 && ratio > formLimit.ratio + 1e-9)
      deckIssues.push({
