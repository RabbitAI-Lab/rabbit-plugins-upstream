...
(() => {
  ...
  ns.primitives.textLeaf = {
    ...
    emit(el, cs, rect, collector, cfg) {
      ...
      const isSingleLine = rect.h <= oneLineH * cfg.singleLineFactor;
      ...
      // 列表项(2026-07-27 P1):LI 直接含文字 → 首个实质 run 挂原生 bullet
      // UL → 圆点(U+2022);OL → 阿拉伯数字;嵌套层数 → 缩进。文字位置近似(详见 html-spec)
      if (el.tagName === "LI") {
        const isOl = el.parentElement && el.parentElement.tagName === "OL";
        const bullet = isOl
          ? { type: "number", numberType: "arabicPeriod", indent: 14 }
          : { type: "bullet", characterCode: "2022", indent: 14 };
