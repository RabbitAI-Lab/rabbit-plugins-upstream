...
| ②色板与角色 | theme.css 26 变量值 | **核心映射**:品牌主色→`--lenovo-red` 组;品牌深色面→`--deep-navy` 组;文字色阶→`--text-*`(rgba 先混合);承载→`--off-white/--card-bg`;边框→`--border-*`;强调→`--accent-orange`;深底文字→`--on-navy-*`(与品牌深底混合);`--signal-*` 默认保留跨预设一致(仅 figma 类多彩品牌允许替换,且须人工审对比度);`--code-*` 全品牌一致 |
