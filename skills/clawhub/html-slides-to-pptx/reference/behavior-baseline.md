...
4. **validate 设计检查(tier-gated,旧页零新增 WARN)**:`slides.config.json` 配 `design.tier` 才启用;未配置时 validate 行为与旧版逐字一致(39 文件 / 0 ERROR / 44 WARN 不变)。新检查:文字适配(仅固定高度盒;溢出余量 = max(8, 字号×0.12),CJK 度量差豁免;欠载按内容底边推算)、字号下限(绝对 14px;>20 字正文启发;底部注释区 top≥840 且 ≥ 档内注释下限豁免)、画布填充(页脚区 top≥980 不计;airyPages 配置 + 深底/超大字启发豁免)、对齐(边缘离群簇/等距网格突变/自带视觉子级溢出)。
   > ⚠️ **2026-08-06 起此条的"未配置即休眠"已反转**:缺省档即 `presentation`+`balanced`(默认开启),
   > 旧夹具页改为显式 `design:{tier:"",formProfile:""}` 豁免。详见 [H14](#h14--设计检查默认开启--满填判据重写2026-08-06-第四轮-p2p3)。
...
### H13 · 夹具 theme.css 少一个右花括号,14 个设计令牌全程未定义(2026-08-06 第四轮 P1)

`scripts/test/fixtures/assets/theme.css` 里 `.big-num` 的 `line-height: 1` 之后**漏了 `}`**,
其后整段分区/字号令牌(`--zone-*` / `--text-*`,14 个)被吞进 `.big-num` 规则体,
`:root` 上**全部 UNDEFINED**;`--on-navy-text/-sub/-dim`、`--accent-orange`、`.num` 则从未写入过。
后果两条,且都被三层 golden 当"正确"锁死:

1. **深底页文字不可见**:97/103/117 页用 `color:var(--on-navy-text)`,变量缺失 → 回退继承的
   `--text-primary #1F1F1F`,近黑压 `--deep-navy #0A2E5C`,**对比度 1.22:1**。
   97 页整页标题"给知识加上保质期"在 PPTX 里等于消失,而 L1 基线正锁着 `color:"1F1F1F"`。
2. **55 处 `class="num"` 静默失效**:`.num` 规则不存在 → 数字回退中文字体,
   经 H1 FONT_MAP(Inter→Arial)本该出 Arial 的地方全出 Microsoft YaHei。

**修复**:补花括号、令牌移回 `:root`、补齐 `--on-navy-*`/`--accent-orange`/`.num`。
刻意**保留**夹具自有的 `--font-cn`(Microsoft YaHei 打头)与 `.slide-container{background:var(--white)}`,
避免无关基线漂移。基线重建后差异仅三类,逐条复核过:
`fontFace` YaHei→Inter(129 处 XML typeface)、`1F1F1F`→`FFFFFF`/`D9E2F0`/`A9BCD6`/`F39800`(8 处)、
数字字形度量变化带来的小几何位移(30 处 off/ext + 106 页 4 处 gridCol)。
**全 44 页基元类型序列零漂移**,确认无结构性变化。

**根因教训**:三层 golden 锚定的是"渲染结果与上次一致",不是"渲染结果正确"。
一个从未正确过的样式,基线只会把错误固化。故本轮新增**对比度检查**作为独立判据
(`layout-checks.js` 规则 5,tier 配置后启用),它不看基线、只看"文字与底色比值"。
自证:把 `--on-navy-*` 改回 `#1F1F1F` 复现缺陷,规则立刻在 97 页报 2 处 ERROR(1.22:1)。

**对比度检查口径**(经全 44 页实测标定,标定后老页命中 0 处、validate 基线零 diff):
- 底色回溯两条线索取更近者:祖先链(**必须穿过 tr/table**,H12 同源)+ `elementsFromPoint`
  几何叠压命中(绝对定位文字压兄弟形状的写法)。**祖先链非画布时优先**——
  `elementsFromPoint` 结构上不返回 `<tr>`,若让它无条件覆盖,106 页深蓝表头会被画布色冲掉成假阳性。
- 渐变/图片底跳过(单色比值无意义,且这类区域走截图路径)。
- 豁免:字号 ≥120px 的巨型数字(幽灵水印手法,01/03/21/97 页均为同色系叠压);
  纯符号且 ≤2 字符(`|` 分隔线、`■` 图例色块本就该淡,不是正文)。
- 双档:<1.3 → ERROR(确定不可见,97 页实测 1.22);1.3–1.6 → WARN(可疑弱对比)。

### H14 · 设计检查默认开启 + 满填判据重写(2026-08-06 第四轮 P2/P3)

**改动一:`design.tier`/`formProfile` 缺省值 `""` → `presentation`/`balanced`。**
此前"缺省休眠"是为了新项目零门槛,实测代价是**新 deck 可以 0 ERROR 通过却小字、单调、上重下空** ——
最该被检查的恰是新写的页。旧夹具页改由 `scripts/test/fixtures/slides/slides.config.json` 里
显式 `design:{tier:"",formProfile:""}` 豁免(deepMerge 支持部分覆盖,只写这两键即可)。
**别删那段豁免** —— 删了 `golden/validate-baseline.txt` 立刻漂。

**改动二:满填判据从"最低边"扩到四条并行。**
原判据只看内容带最低边 ≥847px,一根 1px 细线 + 16px 脚注就能满足 ——
96/104/105/106 四页源码里甚至写着注释"底部口径行(内容带拉满)",实测墨迹只占 29-52%、
中间挖着 220-330px 空洞,却全部"通过"。新增三条(全 WARN,与原判据并行):
① 墨迹行覆盖率 <55%;② 内容带内部 ≥200px 连续无墨;③ 墨迹面积任一半 ≥88%。
判据②比"底部空"更普适:105-timeline 内容压在**下**半截(topShare 0.196),
单看"上重下空"会漏。墨迹口径 = 直接文字 / 图表图形表格图片 / 不透明底或渐变 / 有边框,整页底不计。

**改动三:新增字号层级(规则 15)与结构色面下限(规则 16)。**
- 规则 15:页标题/正文 <1.6 或页内字号档数 <3 → WARN。正文样本**限内容区 top≥320** ——
  否则长页标题自身 >20 字会被当成"正文",比值恒为 1.00(106-table-focus 踩过,它页内只有一个 56px 元素)。
- 规则 16(仅 balanced/rich):内容页无结构色面(≥1.5 万 px² 的非画布色底/渐变)且色面 <8% → WARN。
  实测 96-107 段 **10 页色面恰好 0%**(纯白底+黑字+细线),而 110-117 段是 11-72% —— 这是"素雅单调"的量化根因。

**改动四(P5):96-107 段样张按上述判据重排**,并非只改基线。
9 页(96/98/99/100/104/105/106/107/111)墨迹 29-52% → 58-97%、色面 0% → 13.9-45%;
细线凑底边的写法全部删除,改为承载信息的色带/深色面板/浅底色块。
基线已重建(L1 9 页 + L2 9 个 slide XML),失配项与改动页**一一对应,无附带漂移**。
产出 PPTX 抽查确认深色面板上是白/浅蓝/橙字(即 H13 的失效模式真的修好了,不是被基线固化)。

**改动五(P4):片段/模板/原型库字号上调。**
正文 22→26px(表格 24px)、子说明 18/20→22px,共 32+16 处;刻意不动徽章、代码块、
定高盒内文字、kicker(角色本就该小,且放大会触发溢出检查)。
`assets/theme.css` 与 16 套预设的 `--text-body: 24px` 本就正确,未改;preset-parity 仍全绿。

### H15 · 限定词落地检查(2026-08-06 第六轮 P4)

**结论**:新增 `layout-checks` 规则 6 —— `design.hedgePages` 列出的页,正文必须出现限定词
(`hedgeWords` 缺省 7 词:我们认为/有待验证/预估/初步/假设/尚需/待确认),缺失报 **WARN**。

**为什么**:`content-deepening.md` 第一章 Step 4 一直要求"反向验证通不过的论断降级为观点,
标注'我们认为'",但此前 Step 3.5 六条自查、validate 全部规则、design-principles 都**没有承接**——
降级只存在于措辞层,页面写完后无法核验哪些断言曾被降级。这类"规范写了不被执行"的缺陷
与 H14 的成因同型(第五轮 P4 已实证两例:`--text-*` 零引用、正文钉在破档线)。

**口径**:
- 只查内容区(`top < footerZoneTop` 980),`<template data-slide-notes>` 内的备注**不计**(不显示给观众)
- 只查"该标的地方标了没有",**不判断内容真伪** —— 真伪是用户在 Q5 的判断,机器不越位
- `hedgePages` 缺省 `[]` → 旧夹具与任何未登记页**零命中**(基线零 diff 已实测)

**自证**(FEATURE-COVERAGE 六表已登记):构造两页同内容,一页把"复合增长率 34%,由政企替换需求驱动"
改为"我们认为复合增长率约 34%,该口径有待验证" → 前者 WARN、后者静默;
把该页从 `hedgePages` 移除 → WARN 消失。

### H16 · 文字盒按 content box 定位;单行判定扣对称 padding(2026-08-07)

**症状**:胶囊标签(`padding:3px 16px` + `border:1px` + 无 `text-align`)在浏览器与
`preview.js` 截图里**完全正常**,只有 PPTX 里文字跑到胶囊左上角并溢出。

**两个独立成因**(缺一不可,修一个只好一半):

1. **纵向** —— `isSingleLine` 用 **border box** 高比行高:`line-height:1.5×14px = 21px` 内容
   + 上下 padding 6px + 上下边框 2px = 盒高 **29px**,而 `29 ≤ 21×1.3(27.3)` 为 **false**
   → 判成多行 → `valign:"top"` + `wrap:"square"` → 顶对齐。
   修法:按**内容高**判定(`rect.h − padding − border`),content=21 ≤ 27.3 → 正确判单行。
2. **横向** —— 文字盒取 border box,而 PPTX `lIns=0` 让文字从 border box 左边起排;
   浏览器从 **content box** 起排 → 整体左偏「左 padding + 左边框」(实测 17px)。
   修法:文字盒横向内缩到 content box。

**两个必须的守卫**(都是实测踩出来的):

- **纵向只在上下对称时扣除**。`valign:"middle"` 是在 border box 内居中,只有对称 padding 才与
  浏览器等价。`08.html` 的 `padding-top:25px`(盒高 47.4 / 行高 22.4)浏览器把文字压在下半部,
  若判单行居中会**上移 12.5px**。第一版无此守卫,golden 失配 5 项且弄坏了这类页。
- **内缩必须新建 rect 对象**。`walk.js` 把同一个 `rect` 引用先传给 shape 基元、再传给 text,
  原地修改会连带改坏**已 push 的形状几何**。
- 居中 + 左右对称 padding 时内缩**视觉等价**,跳过以免无谓基线扰动。

**基线影响**:重建 5 项(`07-journey-map` 四个 8px 对称 padding 徽章 top→middle;
`data-limit-boundary` 两个左对齐 20px padding 块横向内缩 +21px;`chip-market-share` 为浮点噪声)。
逐项对照浏览器 content box 复核过,全部是**修正**——这些页此前一直错位,只是盒子大看不出来,
56px 宽的胶囊才把它暴露。

**preview 盲区(重要)**:`preview.js` 走浏览器渲染、天然是 content box,
**结构上不可能发现这类缺陷** —— Step 3.5 的七关目检对它完全无效,只能靠 XML 断言
(`generation-checks.js` E1-E5)或人工开 PPTX。这与 `data-chart` 在预览里不渲染同类:
凡是浏览器里没有对应物、纯由 pptxgenjs 在 PPTX 端生成或解释的东西,目检都覆盖不到。
