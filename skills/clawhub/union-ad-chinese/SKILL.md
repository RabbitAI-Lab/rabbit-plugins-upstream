---
name: union-ad-chinese
description: "UnionSkill国潮中国风广告PPT。图像型工作流：内容分析→大纲确认→逐页生图→PPTX装配。适合文化展示、政府汇报、高端制造。"
...
> 生图Prompt必须包含：16:9横版、中国红（#8B0000），金色元素，云纹/剪纸装饰、无页码标注、中文标题清晰可读
...
```bash
python3 ~/.openclaw/skills/ppt-generator/scripts/union_pptx_assembler.py <图片目录> --style chinese-traditional --topic "PPT标题"
```
