...
from _auth import get_auth_headers, build_auth_headers_with_ak
...
def _do_post(path: str, headers: dict, body_str: str, timeout: int = 30):
    """执行 POST 请求并处理响应（内部公共逻辑）"""
    url = f"{BASE_URL}{path}"
    try:
...
def api_post(path: str, body: dict = None, timeout: int = 30):
    """POST 请求（自动签名 + 重试 + 错误映射），使用当前 skill 自身的 AK"""
    body_str = json.dumps(body or {}, ensure_ascii=False)
    headers = get_auth_headers("POST", path, body_str)
    if not headers:
        raise AuthError("AK 未配置")
    return _do_post(path, headers, body_str, timeout)

@_with_retry()
def api_post_with_ak(path: str, raw_ak: str, body: dict = None, timeout: int = 30):
    """POST 请求（使用指定的 AK 签名），用于多店铺场景"""
    body_str = json.dumps(body or {}, ensure_ascii=False)
    headers = build_auth_headers_with_ak("POST", path, body_str, raw_ak)
    if not headers:
        raise AuthError(f"传入的 AK 无效")
    return _do_post(path, headers, body_str, timeout)
