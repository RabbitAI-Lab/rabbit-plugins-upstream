...
SKILL_NAME = "1688-multi-shop-compare"
...
def build_signature(method, uri, body, content_type, ak_id, ak_secret):
...
def get_auth_headers(method: str, uri: str, body: str = "") -> Optional[Dict[str, str]]:
    ...
    if not ak_id or not ak_secret:
        return None
    return build_signature(method=method, uri=uri, body=body,
                           content_type="application/json", ak_id=ak_id, ak_secret=ak_secret)

def build_auth_headers_with_ak(method: str, uri: str, body: str, raw_ak: str) -> Optional[Dict[str, str]]:
    """使用指定的原始 AK 构建认证头（多店铺场景）"""
