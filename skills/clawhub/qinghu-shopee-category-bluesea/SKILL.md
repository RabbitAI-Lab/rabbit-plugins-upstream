---
name: qinghu-shopee-category-bluesea
description: 青虎AI Shopee 类目蓝海挖掘：逐级下钻 Shopee 一级/二级/三级类目，比对类目榜单、类目趋势快照与趋势概览、类目价格分布，评估各类目的市场容量与竞争激烈程度，锁定高增长低竞争的蓝海细分类目。当用户要进 Shopee 新站点、新开店选类目、评估类目容量、找高增长低竞争类目、看类目价格带分布时必须触发。关键词：青虎AI、Shopee、虾皮、类目、蓝海、细分类目、市场容量、竞争度、类目趋势、价格分布、新站点。
user-invocable: true
metadata: {"openclaw":{"emoji":"🗺️"}}
---

# Shopee-类目蓝海挖掘专家 | 青虎AI

Shopee 新站点 / 新店立项：**逐级下钻类目 → 比容量 → 比增速 → 比拥挤度 → 锁定蓝海细分。**

关键是下钻到**三级类目**：一级大类的结论没有操作价值，机会都藏在细分层。

## 何时触发

- 「Shopee 哪个类目好做」「新开虾皮店选什么类目」
- 「这个类目在马来站有多大」
- 「找高增长低竞争的细分类目」
- 「这个类目的价格带怎么分布」
...
| 工具 | 中文名 | 用在哪一步 |
| --- | --- | --- |
| `queryshopeelevel1categories` | 查询 Shopee 一级类目 | 类目下钻起点（language + site 必填） |
| `queryshopeelevel2categories` | 查询 Shopee 二级类目 | 按一级类目下钻 |
| `queryshopeelevel3categories` | 查询 Shopee 三级类目 | 下钻到真正可操作的细分层 |
| `queryshopeecatdata` | 查询站点类目列表 | 站点下的类目大盘数据 |
| `queryshopeesubcatdata` | 查询子类目列表 | 某类目下的子类目表现 |
| `queryshopeecatranking` | 查类目榜单 | 类目之间的横向排名 |
| `queryshopeecattrend` | 查询类目趋势快照 | 类目历史走势 |
| `queryshopeecategorytrendoverview` | 类目趋势概览 | 跨站点、按粒度对比类目增速 |
| `queryshopeecatpricedistribute` | 查类目价格分布 | 价格带销量结构，找空位 |
...
`queryshopeecatdata` 先看站点下各类目的整体量级，让用户圈定 1~2 个方向。

### 2) 逐级下钻到三级

`queryshopeelevel1categories` → `level2` → `level3`。
**不要停在二级**：二级类目往往已经很拥挤，真正的空位在三级。

### 3) 比增速：`queryshopeecattrend` + `queryshopeecategorytrendoverview`

对候选细分类目看历史走势；要在多个站点之间挑站点时，用趋势概览的 `sites` 一次对比。

### 4) 比拥挤度：`queryshopeecatranking` + `queryshopeesubcatdata`

看类目内销量是否集中在少数商品/店铺。集中度高 = 头部通吃，新店难起。

### 5) 找价格空位：`queryshopeecatpricedistribute`

哪个价格带销量占比高但商品数少，就是切入点。**同时要算清该价格带在东南亚的物流成本能否覆盖。**

### 6) 出蓝海清单

给 2~3 个推荐细分类目，每个说清：类目路径、规模、增速、集中度、建议价格带、建议产品方向。

## 输出

- **推荐细分类目 Top2~3**：完整类目路径 + 规模 + 增速 + 集中度 + 推荐理由。
- **建议切入价格带**与对应的产品形态。
- **劝退名单**：看着大但已经饱和的类目。
- **站点建议**（做跨站点对比时）：同一类目在哪个站点机会更大。
- 类目明细按「数据交付」导出。
...
- 本技能只做**类目层面**判断。挑具体款走「Shopee-爆款截流跟卖大师」，全景立项报告走「Shopee-选品决策专家」。
- 站点必须明确；跨站点比较要用支持 `sites` 的工具，不要把不同站点的独立查询硬拼在一起。
- 不覆盖各站点的税务、认证与本地仓政策，只做风险提示。

## 相关技能

同系列的其他「青虎AI」技能（按需转交，不要在本技能里硬做）：

- **Shopee-爆款截流跟卖大师**：类目定了之后挑款
- **Shopee-跨站点拓客专家**：同类目要多站点铺开时
- **Shopee-选品决策专家**：需要完整立项报告时
