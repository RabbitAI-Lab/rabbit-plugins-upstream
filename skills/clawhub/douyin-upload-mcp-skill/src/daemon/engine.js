...
function resolveUserDataDir() {
  const targetDir = config.browserUserDataDir || GLOBAL_WJZ_DATA_DIR;
  if (process.env.DOUYIN_RESET_BROWSER_PROFILE === 'true') {
    try {
      rmSync(targetDir, { recursive: true, force: true });
      console.log(`[engine] 已重置浏览器数据目录: ${targetDir}`);
    } catch (err) {
      console.warn(`[engine] 重置浏览器数据目录失败: ${err.message}`);
    }
    mkdirSync(targetDir, { recursive: true });
    return targetDir;
  }

  if (config.browserUserDataDir) return targetDir;
...
export async function ensureBrowserForDaemon() {
  ...
  const resetProfile = process.env.DOUYIN_RESET_BROWSER_PROFILE === 'true';

  if (_browser && _browser.isConnected()) {
    if (!resetProfile) {
      return _browser;
    }
    console.log('[engine] 检测到冷启动标记，先关闭现有浏览器');
  ...
  if (alive) {
    if (resetProfile) {
      try {
        ...
        console.log(`[engine] 已连接到端口 ${port} 的浏览器，准备执行冷启动重置`);
        await terminateBrowser();
      } catch (err) {
        console.warn(`[engine] 连接现有浏览器以执行重置失败: ${err.message}`);
      }
    } else {
      try {
        _browser = await puppeteer.connect({
