...
- **Batch Processing**: Process every video in a directory in one run, with per-video failure isolation
- **Auto Categorization**: Group reports by sub-folder or keyword rules, published under per-category Wiki nodes
...
# Optional: keyword-based categories (override folder-based grouping)
python src/main.py path/to/videos/ --batch --user-id <uuid> --space-id <space-id> \
  --category-rules '{"webinar": "events", "tutorial": "learning"}'
```

Batch flags:
- `--batch` treat the path as a directory
- `--no-recursive` only scan the top level
- `--category-rules` JSON map of path-keyword -> category (takes priority over the sub-folder name)
- `--stop-on-error` abort on the first failing video (default: continue and report failures)
