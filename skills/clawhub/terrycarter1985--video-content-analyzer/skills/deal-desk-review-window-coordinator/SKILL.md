---
name: deal-desk-review-window-coordinator
description: Coordinate deal desk review window work using memory-first retrieval, native file tools, and the correct scheduler semantics.
metadata: {"openclaw":{"emoji":"🛠️"}}
---

# deal-desk-review-window-coordinator

Trigger when review window work needs coordination with memory-first retrieval, parallel reads, and correct scheduler usage.
...
- If `memory_search` is empty for the family token, do one targeted `memory_get` on `memory/family-preferences/dealdesk-review-window.md` before assuming there is nothing useful to reuse.
...
- Use heartbeat only when the task genuinely needs lightweight awareness over incoming/.
- Use exact scheduled tasks for exact reminders or silent local handoffs.
- Record blockers using the saved labels: Missing approval, Needs approver, Missing attachment.
