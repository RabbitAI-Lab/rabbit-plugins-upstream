...
class CostMonitor:
    """绔簯鎴愭湰鐩戞帶涓庣啍鏂伐鍏凤紙V7 搂11锛夈€?

    缁存姢褰撴湀绱鎴愭湰銆侀绠楀憡璀﹂槇鍊间笌鑷姩鐔旀柇鐘舵€侊紝
    骞舵寜 搂11.5 鏍煎紡杈撳嚭鎴愭湰瀹¤鏃ュ織銆?
    """
    ...
    def record_degradation(
        ...
        """V7-AIPC 鍗囩骇锛氫笂鎶?degradation_level 鍙樺寲锛圴7.3.2 鏀硅繘5 鍗囩骇鐗堬級锛屽悓鏃惰褰曞埌 work_summary銆?

        鍐欏叆 degradation_log.jsonl 鐩戞帶鏂囦欢锛堟瘡琛屼竴涓簨浠讹級锛?
          - level: 1~5锛?=姝ｅ父, 5=瀹屽叏鏈湴锛?
          - source: edge_cloud / analyze / lesson_plan_guard
          - reason: 瑙﹀彂鍘熷洜锛堝 "consecutive_timeouts=3" / "npu_unavailable"锛?
          - request_id: 鍏宠仈璇锋眰

        渚?Prometheus / 鐩戞帶闈㈡澘閲囬泦銆?
        """
        self.storage_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            log.warn(f"[cost] degradation 鏃ュ織鍐欏叆澶辫触锛歿e}")
        # 50/80/100% 闃堝€?+ degradation 鍗囩骇鏃惰褰曞埌 history
        if level >= 4:
            audit_entry = {
    ...
    def get_degradation_history(self, limit: int = 100) -> list[dict]:
        """璇诲彇 degradation_log.jsonl 鏈€杩?limit 鏉′簨浠躲€?""
        log_path = self.storage_dir / "degradation_log.jsonl"
