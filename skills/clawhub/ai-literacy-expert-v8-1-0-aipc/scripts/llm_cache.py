"""
llm_cache.py - V7-AIPC LLM 鎺ㄧ悊缂撳瓨锛圴7.3.2 鏀硅繘4 鍗囩骇鐗堬級锛? 澶?TTL + 涓?work_summary 鑱斿姩璁板綍鍛戒腑鐜?

璁捐鐩爣锛?
  - 鐩稿悓 abstract_data 澶氭鎺ㄧ悊鏃跺懡涓湰鍦扮紦瀛橈紝閬垮厤閲嶅鏈湴鎺ㄧ悊
  - SQLite 瀛樺偍锛岄浂渚濊禆锛堟爣鍑嗗簱 sqlite3锛?
  - TTL 榛樿 7 澶╋紙涓?V7 鍗忚鏁版嵁淇濈暀鏈熶竴鑷达級
  - 璺ㄨ繘绋嬪畨鍏紙澶?worker 鍚屾椂璇诲啓锛?
  - 缂撳瓨閿?= SHA256(abstract_data + decision_type + model_id)

CLI:
    python llm_cache.py --stats          # 缂撳瓨缁熻
    python llm_cache.py --clear         # 娓呯┖缂撳瓨
    python llm_cache.py --prune 7       # 娓呯悊 7 澶╁墠鏉＄洰
"""
...
class LLMCache:
    ...
    def __init__(
        ...
        self._init_db()
        log.info(f"[llm_cache] 鍒濆鍖? db={self.db_path} ttl={ttl_seconds}s")
    ...
    def make_key(abstract_data: dict, decision_type: str, model_id: str) -> tuple[str, str]:
        """鐢熸垚缂撳瓨閿?= (cache_key, abstract_hash)銆?
        cache_key 鐢ㄤ簬绮剧‘鏌ユ壘锛宎bstract_hash 鐢ㄤ簬缁熻鍛戒腑鐜囥€?
        """
        # 鎺掑簭閿繚璇?dict 椤哄簭鏃犲叧
    ...
    def get(self, abstract_data: dict, decision_type: str, model_id: str) -> Optional[dict]:
        """鏌ヨ缂撳瓨銆傚懡涓繑鍥?result锛屾湭鍛戒腑杩斿洖 None銆?""
    ...
    def clear(self) -> int:
        """娓呯┖鍏ㄩ儴缂撳瓨锛岃繑鍥炴竻鐞嗘潯鏁般€?""
        with self._conn() as conn:
            cur = conn.execute("DELETE FROM cache")
            return cur.rowcount

    def stats(self) -> dict:
        """杩斿洖缂撳瓨缁熻锛氭潯鐩暟銆佹€?hit_count銆佸钩鍧囨垚鏈瓑銆?""
        with self._conn() as conn:
            row = conn.execute("""
                SELECT
                    COUNT(*) AS total,
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="LLM 鎺ㄧ悊缂撳瓨绠＄悊")
    parser.add_argument("--stats", action="store_true", help="鏄剧ず缁熻")
    parser.add_argument("--clear", action="store_true", help="娓呯┖鍏ㄩ儴")
    parser.add_argument("--prune", type=int, metavar="DAYS", help="娓呯悊 N 澶╁墠鐨勬潯鐩?)
    ...
    if args.stats:
        ...
        print(f"娓呯悊 {n} 涓秴杩?{args.prune} 澶╃殑鏉＄洰")
    else:
        parser.print_help()
