...
def render_markdown_table(records: list[WorkRecord]) -> str:
    ...
    for i, r in enumerate(records, 1):
        local = r.local
        cloud = r.cloud
        local_model = f"{local.get('model', 'N/A').split('/')[-1]} ({local.get('device', '?')})" if local.get("used") else "—"
        local_lat = f"{local.get('latency_ms', 0):.0f}ms" if local.get("used") else "—"
        cloud_model = f"{cloud.get('model', 'N/A')}" if cloud.get("used") else "—"
        cloud_cost = f"${cloud.get('cost_usd', 0):.4f}" if cloud.get("used") else "—"
        saved = f"${r.cost.get('saved_usd', 0):.4f}" if r.cost else "—"
        zup = "✅" if r.privacy.get("zero_upload_proof", False) else "❌"
        md.append(
