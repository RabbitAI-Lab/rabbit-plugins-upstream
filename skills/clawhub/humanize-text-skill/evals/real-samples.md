<!-- Migrated from shuorenhua/evals/real-samples.md. SF/SNF cases validate the bilingual subtraction engine. -->

# Real Sample Eval Pack

> Added in v1.7.2 (first 12 cases), expanded to 14 in v1.7.3, 18 in v1.8.0, and 19 in v1.8.5. This file fills the gap left by `benchmark.md`: benchmark cases are synthetic, structurally clean, and dominated by obvious failure modes. This pack focuses on passages that may not look blatantly wrong until you try to send them. The real acceptance bar is whether the rewrite is ready to ship.

## About sources

> **⚠️ The first batch is high-fidelity synthetic data** (v1.7.2).
>
> The samples were not invented out of thin air, but they are also not direct copies of real user posts. The method was:
>
> 1. Observe the AI habits and sentence shapes Chinese users complain about most.
> 2. Abstract the typical structures, failure-mode combinations, and scene distribution.
> 3. Build each sample from those observations without pointing at any real person, project, or account.
>
> The reason for this compromise is straightforward: copying real posts into a public repository without permission creates attribution and compliance problems, while purely intuition-driven synthetic samples drift away from the real distribution. Observation plus synthesis is the safest middle ground for now.
>
> Later versions should add a submission flow and permission template so clearly authorized real samples can be added to this file.

## High-frequency AI pattern distribution
...
## Community observation: why the “catch-you” style reads as AI immediately
...
The most useful thing about this feedback is that the community is not reacting to one hot phrase. It is reacting to a full writing pattern where stance comes before information, promises outweigh facts, and emotional judgment replaces an answer. That is why the maintenance strategy here is not to chase every trending phrase one by one, but to:
...
Reference observations (public discussions, used only for pattern extraction and not transcribed directly as samples):
...
| 数量 | 75 条，持续扩充 | 19 条，质量优先 |

## Three-axis scoring

Score the **original** sample from 1 to 5 on the following three axes (5 is best). Score it again after rewriting, and check whether the total improves and whether "ready to send" rises from <=2 to >=4.
...
**Suggested workflow**: score the original first to anchor the baseline, then run the rewrite and rescore it on the same three axes. `Ready to send` is the final outcome metric; if `Fidelity` drops below 4, the rewrite counts as a regression even if `Naturalness` is perfect.

### Extra scoring for long-form in-place

When a long-form sample is handled in `in-place` scope, add one more axis beyond the three above: `length and rhythm`.

| 维度 | 1 分 | 3 分 | 5 分 |
|------|------|------|------|
| **长度节奏** | 明显缩水，删掉承担转场或停顿的句子 | 字数大致保住，但有几处节奏被压平 | 字数、句数、段落顺序和关键转场基本保留，只做句内去 AI 味 |

This axis is only for long-form length-preservation scenarios and does not need to be scored for every sample.
...
**Original score**：自然 1 / 保真 3 / 可直接发 1
...
**Original score**：自然 1 / 保真 2 / 可直接发 1
...
**Original score**：自然 1 / 保真 3 / 可直接发 2
...
**Original score**：自然 1 / 保真 4 / 可直接发 1
...
**Original score**：自然 2 / 保真 3 / 可直接发 2
...
**Original score**：自然 1 / 保真 4 / 可直接发 2
...
>
> 现在覆盖 210+ 中文词条，75 条 benchmark。如果你也在写 AI 会帮你起草的文档、release note、周报，欢迎试试，也欢迎提 bad case。
...
**Original score**：自然 1 / 保真 4 / 可直接发 1（这消息发出去关系立刻紧张）
...
**Original score**：自然 3 / 保真 4 / 可直接发 3 / 长度节奏 2
