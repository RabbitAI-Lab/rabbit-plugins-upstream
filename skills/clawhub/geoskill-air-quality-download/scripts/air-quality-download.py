...
# Shared Nominatim resolver (used by all subcommands)
NOMINATIM_ENDPOINTS = [
...
def _resolve_place(place: str) -> dict:
    """Resolve a place name via Nominatim; returns lat/lon/display_name/osm_id."""
    import re
    ...
    for endpoint in NOMINATIM_ENDPOINTS:
        try:
            r = _req.get(
                f"{endpoint}/search",
                params={"q": normalised, "format": "jsonv2", "limit": 1, "addressdetails": 1},
                headers={"User-Agent": "air-quality-download/0.2.0",
    ...
    raise RuntimeError(f"All Nominatim endpoints failed for {place!r}: {last_err}")


def _apply_place(args):
    """If --place is set, resolve it and patch args.lat / args.lon in place."""
    if getattr(args, "place", None):
        info = _resolve_place(args.place)
        args.lat = info["lat"]
        args.lon = info["lon"]
        print(f"Resolved {args.place!r} → {info['display_name']}  ({info['lat']:.4f}, {info['lon']:.4f})")
        return info
    return None


def _write_qa(path, place_info, pollutants, **extra):
    import json as _json
    qa = {
        "skill": "air-quality-download",
        "place": place_info,
...
def cmd_current(args):
    ...
    if args.place and (args.lat is None or args.lon is None):
        try:
            place_info = _apply_place(args)
    ...
    if args.lat is None or args.lon is None:
        print("ERROR: provide either --place or both --lat/--lon.")
        sys.exit(1)
    ...
    if getattr(args, "qa", False):
        _write_qa(output_path, place_info, pollutants,
                  subcommand="current", sample=records[:1])
...
def cmd_historical(args):
    ...
    if getattr(args, "qa", False):
        _write_qa(output_path, place_info, pollutants,
                  subcommand="historical", start=args.start, end=args.end,
                  aggregate=args.aggregate, record_count=len(output_records))
...
def cmd_forecast(args):
    ...
    if getattr(args, "qa", False):
        _write_qa(output_path, place_info, pollutants,
                  subcommand="forecast", days=args.days, sample=records[:1])
...
Available pollutants: pm2_5, pm10, ozone, nitrogen_dioxide, sulphur_dioxide, carbon_monoxide
    ...
    current_parser.add_argument("--place", help="Place name (e.g. '北京市朝阳区'); resolved via Nominatim")
    ...
    current_parser.add_argument("--qa", action="store_true",
    ...
    hist_parser.add_argument("--place", help="Place name; resolved via Nominatim")
    hist_parser.add_argument("--lat", type=float, help="Latitude")
    hist_parser.add_argument("--lon", type=float, help="Longitude")
    ...
    hist_parser.add_argument("--qa", action="store_true", help="Write a QA summary JSON")
    hist_parser.set_defaults(func=cmd_historical)

    # --- forecast ---
    forecast_parser = subparsers.add_parser("forecast", help="Get air quality forecast")
    forecast_parser.add_argument("--place", help="Place name; resolved via Nominatim")
    forecast_parser.add_argument("--lat", type=float, help="Latitude")
    forecast_parser.add_argument("--lon", type=float, help="Longitude")
    ...
    forecast_parser.add_argument("--qa", action="store_true", help="Write a QA summary JSON")
