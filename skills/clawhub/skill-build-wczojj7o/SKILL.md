...
description: 从本地视频文件中提取语音文案/字幕。跨平台支持macOS/Windows/Linux，完全离线运行。使用whisper模型进行高精度语音识别，支持多语言字幕提取和OCR文字识别。触发词：视频文案提取、提取字幕、语音转文字、视频转文字、提取文案。
version: 1.0.3
metadata:
  openclaw:
    requires: {}
...
> 📌 **核心能力**：一行命令 → 同时输出 `.txt`（纯文案）+ `.srt`（带时间戳字幕）+ `.wav`（音频），无需额外配置。
