...
These rules are the lowest-level execution contract for every Google KeyAPI REST workflow. Every scenario, routing path, and endpoint choice must comply with them.

## API-Only Policy

- Use KeyAPI REST API execution for live lookup, ranking, analysis, search, comparison, and reporting.
- Do not browse, inspect, scrape, or navigate Google web pages as a substitute for API data.
