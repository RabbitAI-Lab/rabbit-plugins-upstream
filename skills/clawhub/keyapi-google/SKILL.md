---
name: keyapi-google
description: Search and extract Google data through the KeyAPI REST API using live official docs. Use for web search, images, videos, places, maps, reviews, news, shopping, Lens lookups, scholar, patents, autocomplete, and webpage extraction.
---

# Google KeyAPI Skill

Turn natural-language Google requests into documentation-guided KeyAPI REST workflows. Start from the user's business goal, map it to the closest scenario, verify the current endpoint contract in the latest KeyAPI docs, then execute the request with a helper script when available or with direct REST when it is not.
...
For Google data lookup, ranking, analysis, search, comparison, or reporting, use KeyAPI API execution and official KeyAPI docs. Do not use platform web pages as a substitute for API results.
...
7. Apply `references/google-rules.md` for Google-specific identifiers, pagination, and reporting rules.
...
- Entity: the Google object being searched, analyzed, compared, ranked, or monitored
- Scope: query, result surface, country, language, location, time filter, page depth, and source type
- Metric or sort: relevance, freshness, location proximity, rating, review count, result type, source authority
...
- For Google reporting, state the Google surface used, such as web, image, video, places, news, shopping, scholar, patents, Lens, or webpage extraction.
...
```bash
node scripts/keyapi-api.mjs --path /v1/google/... --query-param example=value
```

Execute query parameters from a JSON file:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --query-file query.json
```

Preview a large response without saving:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --query-param example=value --stdout preview
```

Write a complete response to a temporary file for internal analysis:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --query-param example=value --output-file .tmp-keyapi-google-response.json
```

Save or export a complete response for the user:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --query-param example=value --output-file response.json
```

Execute a documented POST endpoint:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --method POST --body '{"example":"value"}'
```

Execute a documented POST endpoint with a large JSON body:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --method POST --body-file request.json
```

Execute an image JSON endpoint from a local file:

```bash
node scripts/keyapi-api.mjs --path /v1/google/... --method POST --query-param example=value --image-file ./image.jpg --image-field image_base64
```
...
- `references/google-rules.md`
- `references/google-search-rules.md`
- `references/google-webpage-rules.md`
- `references/google-visual-rules.md`
- `references/google-local-rules.md`
- `references/google-vertical-rules.md`
