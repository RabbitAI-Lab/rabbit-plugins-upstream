...
def determine_prize_level(front_hits, back_hits):
    ...
    **单一权威源**: 本函数不再手写 if/elif 判定, 一律委托
    ...
    Returns:
        (等级名称, 预计奖金) — 奖金口径取自 dlt_power_engine 当前生效档位
        (默认=奖池<8亿保底档, 保守口径; 可用 set_pool_mode(True) 切上浮档)
    """
    try:
    ...
    if pay == 0:
        ...
        # 档位标注必须与实际金额同源, 否则会出现"5元(奖池≥8亿)"这类自相矛盾
        try:
            from dlt_power_engine import pool_mode_label
            tier = pool_mode_label()
        except Exception:
            tier = ''
        money = f'{pay}元({tier})' if tier else f'{pay}元'
    return name, money
...
def verify_prediction(pred_file, actual_front, actual_back):
    ...
    # 验证胆拖方案
    # 兼容两种 dantuo 结构: 直接 {dan,tuo,back} 或 {standard:{dan,tuo,back}, optimized, ...}
    dantuo = pred.get('dantuo', {})
    if dantuo:
        if isinstance(dantuo, dict) and isinstance(dantuo.get('standard'), dict):
            _std = dantuo['standard']
        else:
            _std = dantuo
        dan = set(_std.get('dan', []))
        tuo = set(_std.get('tuo', []))
        dt_back = set(_std.get('back', []))
