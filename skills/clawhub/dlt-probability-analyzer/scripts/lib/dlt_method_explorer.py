...
# 固定奖(三~七等)单一权威源, 杜绝本模块写死奖金表造成漂移
try:
    from dlt_power_engine import PRIZE_PAYOUT
except Exception:
    PRIZE_PAYOUT = {
        (5, 0): 5000, (4, 2): 5000, (4, 1): 300, (3, 2): 150, (4, 0): 150,
        (3, 1): 15, (2, 2): 15, (3, 0): 5, (2, 1): 5, (1, 2): 5, (0, 2): 5,
    }


def _prize(fh, bh):
    """按 2026 新规奖级映射返回派彩(元)。无中奖=0。

    固定奖(三~七等)委托单一权威源 dlt_power_engine.PRIZE_PAYOUT,
    避免写死"上浮档"导致奖池<8亿时虚高 20%~40% (2026-08-29 修复)。
    一/二等奖为浮动奖, 权威表存的是封顶代表值, 直接用于期望收益会严重
    高估, 故此处保留现实估计值(不套用封顶值)。
    """
    if fh == 5 and bh == 2: return 10_000_000   # 浮动奖, 代表性估计
    if fh == 5 and bh == 1: return 500_000      # 浮动奖, 现实估计(非封顶500万)
    if (fh == 5 and bh == 0) or (fh == 4 and bh == 2): return PRIZE_PAYOUT[(5, 0)]
    if fh == 4 and bh == 1: return PRIZE_PAYOUT[(4, 1)]
    if (fh == 3 and bh == 2) or (fh == 4 and bh == 0): return PRIZE_PAYOUT[(3, 2)]
    if (fh == 3 and bh == 1) or (fh == 2 and bh == 2): return PRIZE_PAYOUT[(2, 2)]
    if (fh == 3 and bh == 0) or (fh == 2 and bh == 1) or (fh == 1 and bh == 2) or (fh == 0 and bh == 2): return PRIZE_PAYOUT[(2, 1)]
    return 0
