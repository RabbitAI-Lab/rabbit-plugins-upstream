...
def kelly_criterion():
    ...
    for front_match in range(6):
        for back_match in range(3):
            ...
            # 固定奖(三~七等)一律委托单一权威源 dlt_power_engine.PRIZE_PAYOUT,
            # 杜绝"写死上浮档→奖池<8亿时虚高 20%~40%"的漂移(2026-08-29 修复)。
            # 一/二等奖为浮动奖: 权威表存的是封顶代表值(1000万/500万), 直接用于
            # 期望收益会严重高估(单注 EV 可虚高近 3 倍), 故此处保留现实估计值
            # (与 dlt_cost_effectiveness 的"真实返奖率"口径一致), 不套用封顶值。
            if front_match == 5 and back_match == 2:
                prize = 10_000_000   # 浮动奖, 代表性估计(非派彩保证)
                name = "一等奖"
            elif front_match == 5 and back_match == 1:
                prize = 200_000      # 浮动奖, 现实估计(权威表封顶500万, 不用于EV)
                name = "二等奖"
            elif (front_match == 5 and back_match == 0) or (front_match == 4 and back_match == 2):
                prize = PRIZE_PAYOUT[(5, 0)]
                name = "三等奖"
            elif front_match == 4 and back_match == 1:
                prize = PRIZE_PAYOUT[(4, 1)]
                name = "四等奖"
            elif (front_match == 3 and back_match == 2) or (front_match == 4 and back_match == 0):
                prize = PRIZE_PAYOUT[(3, 2)]
                name = "五等奖"
            elif (front_match == 3 and back_match == 1) or (front_match == 2 and back_match == 2):
                prize = PRIZE_PAYOUT[(2, 2)]
                name = "六等奖"
