# -*- coding: utf-8 -*-
"""
大乐透开奖序列 — NIST SP 800-22 密码学级随机性检验 (dlt_nist_sts.py)
========================================================================

将真实开奖号码编码为二进制序列, 跑 NIST SP 800-22 标准随机性测试套件
(密码学级), 作为既有 χ² 结构检验 (dlt_randomness_test.py) 的"灵敏度升级"。
...
实现说明 (纯标准库, 无 numpy/scipy, 可在排程任务稳定跑):
  1. Monobit / Block Frequency / Runs / Longest Run / Approximate Entropy /
     Cumulative Sums / Binary Matrix Rank / Maurer's Universal:
  ...
  3. igamc (正则化不完全 Gamma Q(a,x)): 自实现 (级数 + 连分式, Numerical Recipes),
     用于 Block Frequency / Longest Run / Approximate Entropy / Binary Matrix Rank
     的 p-value。

编码方案 (关键, 见下方 encode_draws 说明):
  每球取 1 个奇偶比特, 先全部前区后全部后区 (扁平化, 去除"每期 5+2=7 位帧"的周期)。
  这样可避免两类误报假象: (i) "一球→多位"带来的同球位内强依赖; (ii) 7 位抽屉帧的
  1/7 频谱峰。位流整体 1 比例≈0.51 (由前区奇偶 18/35 决定), 这一轻微边际偏差与
...
自检 (self_test): 用 random.getrandbits 生成真随机 i.i.d. 位流, 验证 (a) FFT 与朴素
  DFT 一致;   (b) 全部 (10 项) 在大量随机序列上的通过率落入 NIST 推荐置信区间 [0.90, 1.06]
  (α=0.01, 20 序列) —— 证明算法实现正确 (不会把随机判成非随机, 反之亦然)。

用法:
  python dlt_nist_sts.py            # 在真实开奖上跑 NIST 套件并打印诚实报告
  python dlt_nist_sts.py --selftest # 跑算法实现自检 (不改任何数据)
"""
import json
import math
import os
...
def _cumsum_p(bits):
    """Cumulative Sums (累积和, NIST §2.4) 核心: 对给定位序列算 p-value。

    X_i = 2*bit_i - 1, 运行和 S_k = Σ_{i=1..k} X_i, z = max_k |S_k|。
    采用 NIST 公布的闭式 (依赖 z, 否则 p 将退化为常数而与数据无关):
        p = 1 - Σ_{k=-(n/2)+1}^{(n/2)-1} [ Φ((4k+1)z/√n) - Φ((4k-1)z/√n) ]
    其中 Φ 为标准正态 CDF = 0.5*erfc(-x/√2)。
    """
    n = len(bits)
    if n < 2:
        return None
    ...
    if z == 0:
        return 1.0
    srt = 1.0 / math.sqrt(n)
    start = int(math.floor(-n / 2.0 + 1.0))
    end = int(math.floor(n / 2.0))
    ...
    for k in range(start, end + 1):
        total += phi((4.0 * k + 1.0) * z * srt) - phi((4.0 * k - 1.0) * z * srt)
    p = 1.0 - total
    if p < 0.0:
        p = 0.0
    elif p > 1.0:
...
def cumulative_sums_forward(bits):
    """2.4 Cumulative Sums (正向, NIST §2.4)。"""
    return _cumsum_p(bits)


def cumulative_sums_reverse(bits):
    """2.4 Cumulative Sums (反向, NIST §2.4) — 对位序列逆序后检验。"""
    return _cumsum_p(list(reversed(bits)))


def _gf2_rank(matrix):
    """求 GF(2) 上矩阵秩 (高斯消元, 矩阵为 M 行, 每行 M 个 0/1 列表)。"""
    m = len(matrix)
    n = len(matrix[0])
    rank = 0
    row = 0
...
def binary_matrix_rank(bits, R=32):
    """2.7 Binary Matrix Rank (矩阵秩, NIST §2.7)。

    将位流切成连续不重叠的 R×R 矩阵, GF(2) 高斯消元求秩。统计
    满秩(F_32)/秩31(F_31)/秩≤30(F_30) 三类计数, 与 NIST 公布的 32×32
    理论概率 p_32=0.2888, p_31=0.5776, p_30=0.1336 做 χ² 检验。
    矩阵数过少 (这里要求 ≥38) 则返回 None。
    """
    n = len(bits)
    cells = R * R
    N = n // cells
    if N < 38:
        return None
    F = [0, 0, 0]  # rank 32, rank 31, rank <=30
    idx = 0
    for _ in range(N):
        mat = [list(bits[idx + r * R:idx + r * R + R]) for r in range(R)]
        idx += cells
        r = _gf2_rank(mat)
        if r == R:
            F[0] += 1
    ...
    chi = 0.0
    for i in range(3):
        chi += (F[i] - N * p[i]) ** 2 / (N * p[i])
    return igamc(1.0, chi / 2.0)  # df = 3-1 = 2 -> a = 1


def maurer_universal(bits, L=7, Q=1280):
    """2.9 Maurer's Universal (通用统计, NIST §2.9)。

    参数 L=7, 初始化块 Q=1280, 统计块 K = (total_bits//L) - Q。
    需要 total_bits ≥ (Q+K)*L 且 K ≥ 900, 否则返回 None。
    先以前 Q*L 位构建 L 位模式的"上一次出现位置"查表, 再对随后 K 块计算
    该模式距上次出现的间隔 (gap), 取 log2(gap) 的均值; 用 NIST 公布 L=7
    常数 (μ=6.1962507, 方差=4.150) 算 z 分数:
        f = |mean - μ| * √K / √方差 ; p = erfc(f/√2)。
    """
    n = len(bits)
    total_blocks = n // L
    if total_blocks < Q + 900:
        return None
    K = total_blocks - Q
    T = [-1] * (1 << L)  # 上一次出现位置查表
    for i in range(Q):
    ...
    c = s / K
    MU = 6.1962507
    SIGMA = math.sqrt(4.150)  # 方差=4.150 (NIST 表, L=7) 取标准差
    f = abs(c - MU) * math.sqrt(K) / SIGMA
    return math.erfc(f / math.sqrt(2.0))
...
def encode_draws(draws):
    ...
    设计取舍 (诚实, 见模块头说明):
      ...
      * 前区奇偶 P(1)=18/35≈0.514 (因 1..35 含 18 个奇数), 后区奇偶 P(1)=6/12=0.5;
        故位流整体 1 比例≈0.51 —— 这一轻微边际偏差正是后续 NIST '边际敏感'项
...
def bit_bias(bits):
    """实测 1 比例 (本编码理论≈0.51, 由前区奇偶 18/35 决定)。"""
    if not bits:
        return None
...
def run_nist_suite(bits, verbose=False):
    """对位流跑 10 项 NIST 检验 (6 原版 + 4 扩展), 返回结果列表。"""
    n = len(bits)
    M = max(20, n // 100)
    ...
    results = [
        ...
        # Cumulative Sums: 取正向/反向中更保守 (更小) 的 p 作为有效下界
        _mk('Cumulative Sums (累积和)',
            min(cumulative_sums_forward(bits), cumulative_sums_reverse(bits)),
            '边际平衡敏感', 'min(forward,reverse)'),
        _mk('Binary Matrix Rank (矩阵秩)', binary_matrix_rank(bits), '结构敏感',
            '32x32'),
        _mk("Maurer's Universal (通用统计)", maurer_universal(bits), '结构/模式敏感',
            'L=7,Q=1280'),
    ]
    if verbose:
...
def print_nist_results(results, bias, n_draws=None, n_bits=None):
    ...
    供 run_nist_on_draws 与 dlt_randomness_test.randomness_battery 共用。
    ...
    print("位流实测 1 比例 = %.4f  (前区奇偶 18/35≈0.514 决定, 后区=0.5)" % bias)
    print("=" * 80)
    print("-" * 80)
    ...
    print("      (2) 位编码固有特性 (前区奇偶边际≈0.514 / 无放回抽样 / 前-后区阶跃)。")
