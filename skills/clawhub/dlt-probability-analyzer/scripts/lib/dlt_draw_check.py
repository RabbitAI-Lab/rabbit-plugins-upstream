# -*- coding: utf-8 -*-
"""
大乐透 · 开奖核对 + 随机基线实证模块 (V3 强化版, 2026-08-04)

为什么存在这个模块?
  用户看到"推荐号码只中七等奖"会很失望, 误以为系统不行。
  但大乐透是独立随机事件, 任何方法(含本系统)的期望都与随机机选相同。
  本模块的核心价值: 用真实开奖数据 + 蒙特卡洛随机基线, 实证
  "我的成绩 ≈ 随机水平", 把每次'没中'变成系统诚实性的证据, 而非失败。

功能:
  1. 加载某期预测 JSON, 展开全部实际投注注数(含后区复式/胆拖)
  2. 逐注比对真实开奖, 算命中数 / 奖级 / 理论奖金(用 dlt_power_engine 单一奖金源)
  3. 蒙特卡洛模拟: 同等注数纯随机机选 2 万次, 得到'随机基线'分布
  4. 对比: 我的实际最佳成绩落在随机分布的哪个百分位 -> 证明无优势
  5. 生成自包含 HTML 核对报告, 落盘到真实用户桌面, 打印 REPORT_DESKTOP_PATH

用法:
  python dlt_draw_check.py --period 26087 --front 5,10,16,24,27 --back 4,10
  python dlt_draw_check.py --period 26087   # 若 dlt_results.json 有记录则自动取
"""
import json
import os
import sys
...
try:
    ...
    # 兜底(正常情况下不会用到)。必须与权威源保底档一致, 否则奖池<8亿时虚高 20%~40%。
    PRIZE_PAYOUT = {
        (5, 2): 10_000_000, (5, 1): 5_000_000, (5, 0): 5000, (4, 2): 5000,
        (4, 1): 300, (3, 2): 150, (4, 0): 150, (3, 1): 15, (2, 2): 15,
        (3, 0): 5, (2, 1): 5, (1, 2): 5, (0, 2): 5,
    }

try:
    from dlt_huiniao_api import fetch_huiniao_dlt
except Exception:
    fetch_huiniao_dlt = None

PRIZE_RANK = {
    '一等奖': 7, '二等奖': 6, '三等奖': 5, '四等奖': 4,
    '五等奖': 3, '六等奖': 2, '七等奖': 1, '未中奖': 0,
}
RANK_TO_NAME = {v: k for k, v in PRIZE_RANK.items()}

# 本模块目录(lib/): 所有相对路径数据文件读取锚定到此处, 与调用方 cwd 解耦。
# 排程 bat 以 cd %~dp0 (Root) 启动本脚本, cwd=Root, 但 dlt_results.json 等数据在 lib/;
...
def expand_tickets(pred):
    """把预测展开成全部实际投注注 (前区5固定, 后区复式展开)

    Returns: list of dict {front:set(5), back:set(2), tag:str}
    """
    tickets = []
    for g in pred.get('groups', []):
        ...
        if len(b) <= 2:
            ...
            backs = [c for c in itertools.combinations(b, 2)]
        for bb in backs:
            tickets.append({'front': set(f), 'back': set(bb),
                            'tag': f"组-{g.get('name','?')}"})
    # 胆拖: 采用"优化"方案(结构完整, 含 back_dan/back_tuo), 标准方案为其替代选项不重复计
    # 玩法为 1胆N拖: 后区 = back_dan 固定 + 每个 back_tuo 配对 -> back_combos 注 (非全复式)
    ...
    if opt:
        ...
        bd = opt.get('back_dan', [])
        bt = opt.get('back_tuo', [])
        # 若标准方案无 back_dan, 则回退: 取 back 列表末位作胆, 其余作拖(仅用于展开, 注数对齐 total_bets)
        ...
        if k <= len(tuo) and k >= 0 and bd:
            for comb in itertools.combinations(tuo, k):
                ...
                for tb in bt:
                    tickets.append({'front': fset, 'back': set([bd[0], tb]),
                                    'tag': '胆拖-优化'})
    return tickets
...
def random_baseline(num_bets, n_sim=20000, seed=20260804):
    ...
    for _ in range(n_sim):
        ...
        for _ in range(num_bets):
            ...
            fh = _hypergeom_sample(rng, 35, 5, 5)
            bh = _hypergeom_sample(rng, 12, 2, 2)
...
def find_result_source(period):
    """尝试从本地结果文件取开奖号。
    注意: 权威历史库在 dlt_history.json(列表[{period,date,front,back,open_time}]),
    必须纳入, 否则本地历史已含某期开奖却因只查 dlt_results/dlt_winner_stats 而静默'未找到'。
    """
    for fn in ['dlt_results.json', 'dlt_winner_stats.json', 'dlt_history.json']:
    ...
    for r in rows:
        if r['pay'] > 0 or r['tag'].startswith('组'):
            ...
            detail_rows += (f"<tr class='{cls}'><td>{r['tag']}</td>"
                            f"<td>{r['fh']}/5</td><td>{r['bh']}/2</td>"
                            f"<td>{r['prize']}</td><td>{r['pay']}元</td></tr>")

    html = f"""<!DOCTYPE html><html lang='zh-CN'><head><meta charset='utf-8'>
<meta name='viewport' content='width=device-width,initial-scale=1'>
<title>大乐透{period}期 开奖核对报告</title>
<style>
...
<h1>大乐透 {period} 期 · 开奖核对报告</h1>
<div class='card'>
  <div>本期开奖号码</div>
  <div class='win'>前区 {win_front} ｜ 后区 {win_back}</div>
...
<div class='card'>
  <h3>一、我的推荐逐注核对</h3>
  <table><tr><th>组合</th><th>前区命中</th><th>后区命中</th><th>奖级</th><th>奖金</th></tr>
  {detail_rows}</table>
</div>
...
<div class='card'>
  <div class='verdict'>
    ...
    <p>④ <strong>大乐透每期开奖是独立随机事件。</strong>中国体彩官方、数学与统计学界共识：
       任何选号法（含本系统、AI、专家、走势图）<b>期望都与随机机选完全相同</b>，
       不存在可稳定提升中选可能的方法。本期结果正是这一真理的又一次实证。</p>
    <p>⑤ 本系统定位为<strong>反割韭菜 / 理性购彩教育工具</strong>，不是"中奖不实技法"。
...
def export_to_desktop(html, period, out_dir=None):
    """落盘到真实用户桌面(排除 SYSTEM 虚拟桌面)。

    out_dir 用于测试/自动化: 指定则直接写入该目录, 跳过桌面探测,
    避免冒烟/CI 把报告写到真实用户桌面污染环境。
    """
    fn = f"大乐透{period}期_开奖核对报告.html"
...
def fetch_result_online(period):
    """在线拉取某期开奖结果(复用 dlt_huiniao_api, 含 front/back)"""
    if fetch_huiniao_dlt is None:
        return None, None
    try:
        data = fetch_huiniao_dlt(page=1, limit=30)
        for d in data:
...
def verify_period(period, win_front, win_back, sim=20000, out_dir=None):
    """对单期执行完整核对, 返回桌面报告路径"""
    pred_file = f'dlt_prediction_{period}_v8.json'
    if not os.path.exists(pred_file):
        alt = f'dlt_prediction_{period}.json'
    ...
    print(f"[核对] 开奖 前区{win_front} 后区{win_back}")
    ...
    path = export_to_desktop(html, period, out_dir=out_dir)
...
def main():
    ...
    ap.add_argument('--front', help='开奖前区, 逗号分隔, 如 5,10,16,24,27')
    ap.add_argument('--back', help='开奖后区, 逗号分隔, 如 4,10')
    ap.add_argument('--sim', type=int, default=20000)
    ap.add_argument('--out-dir',
                    help='报告输出目录(测试/自动化用, 默认真实桌面)')
    ...
    if args.auto:
        import glob
        files = sorted(glob.glob('dlt_prediction_*_v8.json'), reverse=True)
        done = set()
        for f in files:
            parts = f.split('_')
            ...
            if wf and wb:
                ...
                verify_period(period, wf, wb, sim=args.sim, out_dir=args.out_dir)
                return
        # 终极自愈: 本地均无结果时, 尝试在线刷新历史库(dlt_history.json)后再核对一次。
        # 解决"排程在开奖后跑但API数据滞后→静默未找到→对比报告缺失"的复发问题。
        try:
            from dlt_huiniao_api import fetch_latest_huiniao, merge_huiniao_with_existing
            if os.path.exists('dlt_history.json'):
                hist = json.load(open('dlt_history.json', encoding='utf-8'))
                fresh = fetch_latest_huiniao(limit=30)
                merged = merge_huiniao_with_existing(hist, fresh)
                if len(merged) > len(hist):
                    json.dump(merged, open('dlt_history.json', 'w', encoding='utf-8'),
                    ...
                    for f in files:
                        ...
                        if wf and wb:
                            ...
                            verify_period(pp, wf, wb, sim=args.sim, out_dir=args.out_dir)
                            return
        except Exception as e:
    ...
    verify_period(args.period, win_front, win_back, sim=args.sim, out_dir=args.out_dir)
