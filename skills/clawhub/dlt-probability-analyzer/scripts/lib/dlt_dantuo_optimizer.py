# -*- coding: utf-8 -*-
"""
大乐透胆拖组合优化引擎 (V8.9.7 新增, 用户需求: 加强胆拖分析 + 性价比最高组合)

设计目标 (用户原话):
  "加强胆拖号码的分析能力... 在报告中给我输出, 性价比最高的胆拖组合,
   胆拖组合的方式不固定, 只要保证 号码最可靠 / 成本最低 / 中奖概率最高 这三个指标的最完美组合"

实现要点:
  1. 形态不固定: 枚举 前区1~3胆 × 多种拖数, 后区0~1胆 × 多种拖数 (在预算上限内)
  2. 号码最可靠: 用模型综合评分 combined_score / back_scored 选胆(评分最高)与拖(次高)
  3. 成本最低: 注数 = C(拖,5-胆) × C(后拖,2-后胆), 成本 = 注数×2/×3
  4. 中奖概率最高(诚实重定义): 用"模型评分加权蒙特卡洛"估算该结构
     至少中任一固定奖 / 至少中五等奖及以上的概率 —— 这是胆拖真正的"结构化中奖覆盖"
     能力, 而非单注一等奖概率(恒 1/21,425,712, 任何结构都不改变)
  5. 多目标加权综合: 在预算内选 准度↑ + 成本↓ + 中奖覆盖↑ 的 Pareto 最优, 输出性价比最高

数学诚实声明:
  - 任何单注一等奖概率恒 1/21,425,712, 胆拖不改变它
  - 胆拖的本质是"在既定预算下优化投入结构", 不是让期望转正
  - MC 概率是基于模型对号码置信度的加权估计, 非官方随机概率; 真实随机下所有结构等价

带 main() 守卫: 独立运行 `python dlt_dantuo_optimizer.py` 可打印 26087 期最优方案用于自检。
"""
import math
import random
import itertools
...
try:
    from dlt_common import passes_filters
...
# ---------------------------------------------------------------------------
# 奖级映射: 前区命中 f, 后区命中 b -> 奖级(1一等..7七等, 0不中)
# 官方中奖条件(奖池≥8亿档, 当前持续触发):
#   一等5+2  二等5+1  三等5+0或4+2  四等4+1  五等3+2或4+0
...
def front_back_to_prize(f, b):
    ...
    if f == 5 and b == 1:
        return 2
    if (f == 5 and b == 0) or (f == 4 and b == 2):
        return 3
    if f == 4 and b == 1:
        return 4
    if (f == 3 and b == 2) or (f == 4 and b == 0):
        return 5
    if (f == 3 and b == 1) or (f == 2 and b == 2):
        return 6
    if (f == 3 and b == 0) or (f == 2 and b == 1) or (f == 1 and b == 2) or (f == 0 and b == 2):
        return 7
    return 0
...
def estimate_win_prob(dan, tuo, back_dan, back_tuo, combined_score, back_scored,
    ...
    Wf = {n: combined_score.get(n, 1e-9) for n in range(1, 36)}
    Wb = {n: back_scored.get(n, 1e-9) for n in range(1, 13)}
    dan_s, tuo_s = set(dan), set(tuo)
    bd_s, bt_s = set(back_dan), set(back_tuo)
    any_win = 0
    five_plus = 0
    for _ in range(n):
        W = set(_weighted_sample_topk(Wf, 5, rng))
        Wb2 = set(_weighted_sample_topk(Wb, 2, rng))
        hD = len(dan_s & W)
        hT = len(tuo_s & W)
        hBd = len(bd_s & Wb2)
        hBt = len(bt_s & Wb2)
        mf = hD + min(5 - df, hT)
        mb = hBd + min(2 - dfb, hBt)
...
# ---------------------------------------------------------------------------
# 容错保底表: 假设"我们选的 df+dtf 个前区号中, 共有 m 个命中当期开奖"(m=0..5),
# 取最坏分布(命中号尽量落在拖码而非胆码), 给出该胆拖票"仍能达到的前区最高命中"
# 再配后区最坏(0命中)映射奖级, 直观展示"胆码全落空时还剩什么"。
# ---------------------------------------------------------------------------
def tolerance_table(dan, tuo, back_dan, back_tuo):
    ...
    rows = []
    for m in range(0, 6):
        # 最坏前区: 命中号尽量在拖码, 胆码命中 = max(0, m-dtf)
        hD_min = max(0, m - dtf)
        hT_max = min(dtf, m)
        mf_worst = hD_min + min(5 - df, hT_max)
        # 后区最坏: 0 命中 -> max_back = 0
        mb_worst = 0
...
def _perturb(dist, period_seed, salt, tag):
    ...
    与 dlt_auto._perturb 同原理, 此处自包含以避免跨模块循环导入。
    """
...
def optimize_dantuo(combined_score, back_scored, prev_front, passes_filters_fn=passes_filters,
    ...
    """
    combined_score: dict 前区号码->模型综合评分 (CDM+马尔可夫+频率+遗漏加权)
    back_scored:    dict 后区号码->模型评分
    prev_front:     上期前区5号码(list), 用于9项过滤器
    ...
    """
    # V8.9.8 跨期变化修复: 胆拖此前纯按评分排序, 后区拖码每期几乎不变,
        ...
        for dtf in range(5 - df, 5 - df + 9):
            ...
            for dan in itertools.combinations(dan_pool, df):
                ...
                for tuo in itertools.combinations(avail, dtf):
                    ok = True
                    for sub in itertools.combinations(tuo, 5 - df):
                    ...
                    for dfb in (0, 1):
                        for dtb in range(2 - dfb, 2 - dfb + 7):
                            if dtb > 11:
                                continue
                            ...
                            if len(bt) < dtb:
                                continue
                            front_combos = math.comb(dtf, 5 - df)
                            back_combos = math.comb(dtb, 2 - dfb)
                            total = front_combos * back_combos
                            # V4 纠偏: 退化单注胆拖剔除 (前区或后区仅1注=普通单注, 非真正胆拖展开)
                            #   典型: 3胆2拖(dtf=2→C(2,2)=1)/1胆4拖(dtf=4→C(4,4)=1)/2胆3拖(dtf=3→C(3,3)=1)
                            #   以及后区 1胆1拖(dtb=1→C(1,1)=1)/0胆2拖(dtb=2→C(2,2)=1)
                            if front_combos < 2 or back_combos < 2:
                                continue
                            ...
                            raw.append({
                                'form': f'{df}胆{dtf}拖+后{dfb}胆{dtb}拖',
    ...
    # 兼容 dlt_auto.generate_report 的 dantuo['standard'] 结构, 额外带优化字段
    best_out = {
        ...
        'cost_basic': best['cost'], 'cost_extra': best['total_bets'] * 3,
...
HONESTY = (
    "【数学诚实声明】任何单注一等奖概率恒为 1/21,425,712, 胆拖不改变它。"
...
if __name__ == '__main__':
    ...
    # 加载历史 + 模型评分 (复用 dlt_auto 的计算)
    from dlt_auto import compute_models, _back_score

    hist_path = 'dlt_history.json'
    if not os.path.exists(hist_path):
        print('✗ 缺少 dlt_history.json')
        raise SystemExit(1)
    with open(hist_path, 'r', encoding='utf-8') as f:
        draws = json.load(f)
    models = compute_models(draws)
    prev_front = draws[-1]['front']
    # 后区评分需从 models 真实键重算 (back_dist 是 generate_predictions 内局部变量, 不在 models)
    ...
    if b is None:
        ...
        print(f"  拖码(前区): {b['tuo']}")
        print(f"  后区: {b['back']}")
        print(f"  注数: {b['total_bets']} | 成本: {b['cost_basic']}元(基本)/{b['cost_extra']}元(含追加)")
        ...
        print('  容错保底表(前区命中m个, 最坏情形):')
        for row in b['tolerance_table']:
            print(f"    m={row['m']} -> 前区最坏命中{row['front_hit_worst']} -> {row['desc']}")
        print('  候选 top3:')
