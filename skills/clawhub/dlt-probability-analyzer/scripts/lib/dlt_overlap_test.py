# -*- coding: utf-8 -*-
"""
重叠假设实证检验 (dlt_overlap_test.py)
====================================================================

用户观察: 每一期中一等奖的 5+2 号码, 都会和「同比那期+同比最近三期 +
...
def main():
    ...
    actual_front = []   # 实际号码与并集的前区重合数
    actual_back = []
    union_front_size = []   # 并集覆盖了多少个不同前区号码 (1-35)
    union_back_size = []
    n_valid = 0

    R = 1500  # 每个目标的随机对照抽样数
    rand_front = []   # 随机号码与"实际并集"的前区重合数 (对照)
    rand_back = []

    randref_front = []  # 用"8期随机历史"并集与"实际号码"的前区重合 (对照构造特殊性)
    randref_back = []
    ...
    print(f"  前区并集平均覆盖 {mean(union_front_size):.1f} / 35 个不同号码 "
          f"({mean(union_front_size)/35*100:.1f}%)")
    print(f"  后区并集平均覆盖 {mean(union_back_size):.1f} / 12 个不同号码 "
          f"({mean(union_back_size)/12*100:.1f}%)")
    print()
    print("【重合数分布 (前区5球 / 后区2球)】")
    print(f"  实际当期: 前区均={mean(actual_front):.2f} | {dist(actual_front)}")
    print(f"  随机对照: 前区均={mean(rand_front):.2f} | {dist(rand_front)}")
    print(f"  实际当期: 后区均={mean(actual_back):.2f} | {dist(actual_back)}")
    print(f"  随机对照: 后区均={mean(rand_back):.2f} | {dist(rand_back)}")
    print()
    print("【'至少重合几个'的比例】")
    print(f"  前区≥1: 实际={frac_ge(actual_front,1)*100:.2f}%  随机={frac_ge(rand_front,1)*100:.2f}%")
    print(f"  前区≥2: 实际={frac_ge(actual_front,2)*100:.2f}%  随机={frac_ge(rand_front,2)*100:.2f}%")
    print(f"  前区≥3: 实际={frac_ge(actual_front,3)*100:.2f}%  随机={frac_ge(rand_front,3)*100:.2f}%")
    print(f"  后区≥1: 实际={frac_ge(actual_back,1)*100:.2f}%  随机={frac_ge(rand_back,1)*100:.2f}%")
    print()
    print("【用户构造是否特殊? (8期随机历史并集 vs 实际号码)】")
    print(f"  随机参考构造前区均重合={mean(randref_front):.2f}  vs 用户构造实际={mean(actual_front):.2f}")
    print(f"  随机参考构造后区均重合={mean(randref_back):.2f}  vs 用户构造实际={mean(actual_back):.2f}")
    print()
    ...
    if abs(df) < 0.15 and abs(db) < 0.08:
        verdict = ("结论: 实际重合数与纯随机对照几乎一致 → 该现象是集合大小的必然结果"
                   "(并集已覆盖约85%的前区号码, 任意新开奖都极可能落入其中), "
                   "不含预测力, no_edge 不变。")
    else:
        verdict = (f"结论: 实际重合相对随机有偏差(前区+{df:.2f}/后区+{db:.2f}球), "
                   f"需进一步排查是否为真实弱相关。")
    print("判定:", verdict)
    print()
    print("补充: 即使'总重合几个'成立, 并集覆盖~30/35前区号码, 无法缩小到具体号码;")
    print("      用它预测下一期 = 从几乎全部号码里盲选, 一等奖概率仍恒为 1/21,425,712。")
