# -*- coding: utf-8 -*-
"""
大乐透系统 — 一键永久自检 / 回归护栏 (dlt_healthcheck_all.py)
====================================================================

元问题:
    过去每个版本都"自称稳定", 但下次审计又发现关键 BUG (期号进年、后区公式漂移、
    硬编码写死)。根因是"没有永久的回归护栏" —— 改完即忘, 下次再犯。

本脚本作为系统唯一入口前的"预检", 每次运行都强制校验:
  1. 全部 .py 语法可编译 (改坏一个文件立即发现)
  2. dlt_common 自检 (PRIMES/AC/过滤器/后区评分 单元断言)
  3. 跨文件常量一致性: 活动脚本内联 PRIMES / 前区权重 必须与 dlt_common 完全一致
     —— 从机制上阻止"公式漂移"类 BUG 复发
  4. 期号滚动自检 (含 26156->27001 等年终边界)
  5. V8.4 三方交叉验证 52 项 (预测 vs JSON vs HTML)
  6. ECI 回测可运行且不崩溃
  7. 关键产物存在性 (dlt_history.json / dlt_valid_combos.json)
 ...
 12. 版本标识一致性: 当前版本(README 版本章节取最大值)必须与 dlt_run_v8.bat 脚本头
     声明的"大乐透Vx.y.z智能预测"以及 WorkBuddy 自动化条目名完全一致 ——
     防止"代码升到 V8.9.x 但系统任务/自动化名仍停在 V8.8"这类版本标签漂移复发
     (历史上真实发生过, 被用户发现)。
 13. 方法发现+证伪引擎闸门: 常驻运行 dlt_method_explorer.py (系统性生成一批频率类候选
 ...
 16. 核心函数 属性/差分/变质 测试: 对 dlt_common 纯函数做 5000 随机样本不变量校验
 ...
 18. 模型+任务+程序 三体协同一致性自检: 验证三层咬合无漂移——执行器(Windows schtasks
     DLT_V8_Smart 已启用/SYSTEM/目标bat)↔程序单入口(dlt_run_v8.bat→dlt_smart.py --force)↔
 ...
 20. 根离线数据 vs SKILL 副本离线数据 同步自检: 代码(.py)与预测产物(JSON/HTML)已分别由
     ...
     dlt_history.json 在每次开奖后被重下载/重生成, SKILL 副本的离线兜底数据需手动同步,
     历史上真实发生(根最新期=26086 而 SKILL=26087, 预测链静默用了旧数据)。本闸门取根与
     SKILL 副本 dlt_history.json 各自最新期号与整条记录(前区/后区/日期)做深度比对, 副本
     不能落后、同最新期则开奖号必须一致, 把"离线数据未跟随同步"的隐性回归永久纳入护栏。

退出码 0 = 全绿, 非0 = 存在回归, 自动化任务应据此拒绝交付。

 23. 预测追踪自证闸门: 常驻运行 dlt_track_record_selfcheck.py, 把系统自身累积的
     真实推荐战绩(dlt_performance.json)与理论超几何分布/蒙特卡洛随机基线做显著性比对,
     断言 no_edge_confirmed(系统推荐与纯随机无显著差异)或样本不足; 一旦 empirical 命中
     显著偏离随机(no_edge 翻转)即告警复核 —— 把"用自身历史自证无优势"化为系统级护栏。
 24. 自查自愈增强闸门: 常驻运行 dlt_self_integrity.diagnose(), 综合校验 代码完整性哈希
     (篡改检测+备份引导恢复) / 数据新鲜度(过期自动刷新) / 运行一致性(抓"预测了却未核对"
     的静默漏洞), 把"自主发现问题并力所能及地自愈"化为系统级护栏。

用法:
  python dlt_healthcheck_all.py
"""
import sys
import os
import io
...
# 父目录: 在 lib/ 子目录布局下, README.md / dlt_run_v8.bat 等"顶层"文件位于 lib/ 的父级
...
def _pick_runtime_file(name):
    """定位由 .bat 在"自身所在目录"写出的运行态文件(日志/告警)。

    dlt_run_v8.bat 用 `cd /d "%~dp0"` 切到自己所在目录再写文件, 该目录在 Root 部署下是
    工程顶层、在 SKILL 包下是 scripts/ —— 都是 lib/ 的**父级**(SCRIPTS_DIR), 而非 WORK_DIR。
    历史 BUG: 第22项曾用 WORK_DIR 拼接, 永远找不到 → 日志中断/未处理告警两项静默失效。
    两处都存在时返回 mtime 最新者; 都不存在返回 None。
    """
    found = []
    for d in (SCRIPTS_DIR, WORK_DIR):
...
# 动态解析, 不写死用户名: 优先环境变量 DLT_PROJECT_ROOT, 否则从本文件位置推导项目根。
...
def _detect_project_root():
    ...
    优先级: ① 环境变量 DLT_PROJECT_ROOT; ② 本文件位置推导:
      在 Root/lib 运行时 dirname(lib)=Root 顶层(含 lib/dlt_smart.py);
      在 SKILL/scripts/lib 运行时 dirname(lib)=scripts(含 lib/dlt_smart.py)——
...
def _candidate_skill_dirs():
    ...
    cands = [os.path.join(os.path.expanduser("~/.workbuddy/skills/dlt-probability-analyzer/scripts"), "lib"),
             os.path.expandvars(r"%USERPROFILE%\.workbuddy\skills\dlt-probability-analyzer\scripts\lib")]
    for p in _iter_real_user_profiles():
        base = os.path.join(p, ".workbuddy", "skills", "dlt-probability-analyzer", "scripts")
...
def prelaunch_heavy_scripts():
    ...
    heavy = [
        "dlt_cross_validate_v84_final.py",
        "dlt_eci_backtest.py",
        "dlt_method_explorer.py",
        "dlt_randomness_test.py",
        "check_undefined_names.py",
        "dlt_hypothesis_test.py",
        "dlt_symmetry_test.py",
        "dlt_visual_pattern_test.py",
        "dlt_overlap_test.py",
        "dlt_winner_stats.py",
    ]
    launched = []
...
# ------------------------------------------------------------
def check_common():
    step("2. dlt_common 单元自检")
    try:
        import dlt_common
        dlt_common._self_test()
        check("dlt_common._self_test()", True,
              f"PRIMES={len(dlt_common.PRIMES)} FRONT_WEIGHTS={dlt_common.FRONT_WEIGHTS}")
    except Exception as e:
        check("dlt_common._self_test()", False, str(e))
...
def check_constants_consistency():
    ...
    import dlt_common
    target_files = ["dlt_auto.py", "dlt_enhance.py", "dlt_smart.py",
                    "dlt_cross_validate_v84_final.py",
                    "dlt_result_verify.py", "dlt_eci_backtest.py", "dlt_ml_models.py",
                    "dlt_data_recovery.py", "dlt_expert_scraper.py"]
    issues = []
    for fname in target_files:
        ...
        if m:
            ...
            if nums != dlt_common.PRIMES:
                issues.append(f"{fname}: PRIMES={sorted(nums)} != 标准{sorted(dlt_common.PRIMES)}")
        # 前区权重一致性
        for w in dlt_common.FRONT_WEIGHTS:
            if f"{w}" not in src and f"{w:.2f}" not in src:
                # 仅报告明显缺失 (宽松, 避免误报)
                pass
        # 是否仍出现被禁的 (1-eci_s) 后区公式 (漂移标志)
        if "(1 - eci_s)" in src or "(1-eci_s)" in src:
            issues.append(f"{fname}: 仍含被禁后区公式 (1-eci_s), 应改用 dlt_common.back_score")
        # 是否仍写死 26085 作为预测目标 (动态化标志)
        if "dlt_prediction_26085" in src and fname in ("dlt_auto.py", "dlt_enhance.py",
                                                        "dlt_cross_validate_v84_final.py", "dlt_smart.py"):
            issues.append(f"{fname}: 仍写死 dlt_prediction_26085 (应动态)")
    check("活动脚本常量与 dlt_common 一致", len(issues) == 0,
...
def check_period():
    ...
    try:
        ...
        cases = [
            (26084, "2026-07-27", "26085"),
            (26085, "2026-07-29", "26086"),
            (26156, "2026-12-30", "27001"),   # 年终->进年
            (27001, "2027-01-02", "27002"),
            (28158, "2028-12-29", "29001"),   # 闰年边界
        ]
        bad = []
...
def check_cross_validate():
    step("5. V8.4 三方交叉验证 (52 项)")
    # 交叉验证是「预测流水线产物的校验闸门」: 需要 dlt_prediction_<期>_v8.json 与
    # 报告 HTML 作为比对基准。若本环境尚未运行预测流水线(无产物), 则无内容可校验。
    # 此情形属构建期/首跑前, 跳过(非正确性失败); 生产环境(排程运行后)会自动校验。
    import glob as _glob
    _has_pred = bool(_glob.glob(os.path.join(WORK_DIR, "dlt_prediction_*_v8.json")))
    _has_html = bool(_glob.glob(os.path.join(WORK_DIR, "*_V85_增强版.html")) +
                    _glob.glob(os.path.join(WORK_DIR, "*预测报告_V8_全面修复.html")))
    if not _has_pred and not _has_html:
        ...
        return
    try:
        r = _wait_prelaunched("dlt_cross_validate_v84_final.py", 300)
        ...
        if m_pct:
            ...
            # 兜底: 交叉验证脚本 check() 打印的是 "✓"(U+2713), 非 "✅"(U+2705), 故统计 ✓
            n_pass = out.count("✓")
...
def check_eci():
    step("6. ECI 回测可运行")
    try:
        r = _wait_prelaunched("dlt_eci_backtest.py", 200)
...
def check_artifacts():
    step("7. 关键产物存在性")
    must = ["dlt_history.json", "dlt_valid_combos.json", "dlt_common.py", "dlt_period.py"]
    miss = [f for f in must if not os.path.exists(os.path.join(WORK_DIR, f))]
    check("关键产物齐全", len(miss) == 0,
          "缺失: " + ", ".join(miss) if miss else "dlt_history/valid_combos/common/period 均存在")
...
def check_data_dr():
    ...
    ok = True
    detail = []
    if not getattr(dlt_auto, 'DATA_SOURCES', []):
        ok = False
        detail.append("DATA_SOURCES 为空")
    else:
        detail.append(f"优先级 {[s[0] for s in dlt_auto.DATA_SOURCES]}")
    if not os.path.exists(os.path.join(WORK_DIR, 'dlt_data_recovery.py')):
        ok = False
        detail.append("dlt_data_recovery.py 缺失")
    try:
        with open(os.path.join(WORK_DIR, 'dlt_history.json'), encoding='utf-8') as f:
            hist = json.load(f)
        if len(hist) < 2000:
            ok = False
            detail.append(f"dlt_history 仅{len(hist)}期")
        else:
            detail.append(f"dlt_history {len(hist)}期")
    except Exception as e:
        ok = False
        detail.append(f"dlt_history 读取失败: {e}")
    # 最后成功源记录 (若存在应标记 ok)
    rec = os.path.join(WORK_DIR, 'dlt_data_source.json')
...
def check_power_engine():
    ...
    try:
        import dlt_power_engine
        dlt_power_engine._self_test()
        # 确认三大能力函数齐全
        need = ["walk_forward_backtest", "monte_carlo_cost", "assert_improvement", "honesty_guardrail"]
        miss = [f for f in need if not hasattr(dlt_power_engine, f)]
        ok = len(miss) == 0
...
def check_ml_selfcheck():
    ...
    try:
        import dlt_ml_selfcheck
        # 确认核心函数齐全且可被调用 (不要求"有优势", 只要求模块可复现诚实结论)
        need = ["walk_forward", "summarize", "main"]
        miss = [f for f in need if not hasattr(dlt_ml_selfcheck, f)]
        ok = len(miss) == 0
        check("ML自评模块健康", ok,
              "OK" if ok else "缺失: " + ", ".join(miss))
    except Exception as e:
...
def check_antifraud_gate():
    ...
    all_ok = True
    msgs = []
    # 各脚本超时: dlt_winner_stats.py 做2904期置换检验(B=2000)计算量大,
    # 重型护栏连跑(交叉验证+方法引擎+本闸门)CPU满载时偶发>120s, 单独放宽到300s
    timeout_map = {spec[0]: 300 if spec[0] == 'dlt_winner_stats.py' else 120
                   for spec in specs}
    for fname, must_have, must_not in specs:
        ...
        if missing:
            all_ok = False
            msgs.append(f"{fname}: 缺失诚实结论标记 {missing}")
...
def check_method_explorer_gate():
    ...
    fp = os.path.join(WORK_DIR, "dlt_method_explorer.py")
    if not os.path.exists(fp):
        check("方法发现引擎闸门", False, "dlt_method_explorer.py 缺失")
        return
    try:
        r = _wait_prelaunched("dlt_method_explorer.py", 300)
        out = (r.stdout + r.stderr)
        rc = r.returncode
    except Exception as e:
    ...
    jp = os.path.join(WORK_DIR, "dlt_method_explorer.json")
    try:
        data = json.load(open(jp, encoding='utf-8'))
    except Exception as e:
        check("方法发现引擎闸门", False, f"读取 dlt_method_explorer.json 失败: {e}")
        return
    ...
    # 双保险: 禁止出现"有真实边缘/方法有正收益可能/显著有效方法"类正面断言标签,
    # 防止引擎被离群值骗过而谎称方法有效。
    # 注意: 用词须精确, 不能匹配结论里的"不有正收益可能"(诚实声明), 故不用子串"有正收益可能"。
    forbidden = ["有真实边缘", "方法有正收益可能", "显著有效方法", "能稳定正收益", "具备预测力"]
...
def check_version_consistency():
    ...
    # (b) dlt_run_v8.bat 脚本头必须声明同一版本 ("大乐透Vx.y.z智能预测")
    bat = None
    for _d in (SCRIPTS_DIR, WORK_DIR):
        _p = os.path.join(_d, "dlt_run_v8.bat")
        if os.path.exists(_p):
            bat = _p
            break
    if bat:
        bsrc = open(bat, encoding="utf-8").read()
        m = re.search(r'大乐透V(\d+\.\d+\.\d+)智能预测', bsrc)
        if not m:
            issues.append("dlt_run_v8.bat 未声明 '大乐透Vx.y.z智能预测'")
        elif m.group(1) != cur:
            issues.append(f"dlt_run_v8.bat 声明版本 {m.group(1)} != 当前 {cur}")
    else:
        # 跨平台 skill 包按设计不含 .bat(发布包排除可执行脚本), 缺失属环境不适用, 软跳过而非硬失败
        print("  ⚠️ 跳过: dlt_run_v8.bat 不在本部署(跨平台 skill 包按设计不含 .bat), 版本/三体一致性仅校验其余维度")
    ...
    try:
        ...
        if db:
            ...
            for r in con.execute("SELECT name FROM automations").fetchall():
                nm = r["name"] or ""
                if ("大乐透" in nm) or ("DLT" in nm.upper()):
...
def check_data_freshness():
    ...
    机制(来自软件自查方法论: 数据时效性 assert + 汇总统计一致性):
      - 完整性: dlt_history.json 条数充足(>=2700) 且 最新期号在合理区间
      - 不来自未来: 最新开奖日期 <= 今天
    ...
    step("15. 数据时效性与完整性自检")
    import datetime
    fp = os.path.join(WORK_DIR, "dlt_history.json")
    if not os.path.exists(fp):
        check("数据时效性与完整性", False, "dlt_history.json 缺失")
        return
    try:
    ...
    if not isinstance(hist, list) or not hist:
        check("数据时效性与完整性", False, "dlt_history.json 结构异常")
        return
    n = len(hist)
    periods = []
    for d in hist:
...
def check_report_sections():
    ...
    enh = glob.glob(os.path.join(WORK_DIR, "大乐透*_V85_增强版.html"))
    base = glob.glob(os.path.join(WORK_DIR, "大乐透*预测报告_V8_全面修复.html"))
    target = None
    enhanced = False
    if enh:
...
def check_property_tests():
    ...
    调用 dlt_common.property_checks(): 对纯函数做 5000 随机样本的不变量校验
...
def check_randomness():
    """17. 开奖序列随机性检验电池 + NIST SP 800-22 密码学级灵敏度升级

    对真实历史开奖序列跑 10 项卡方检验(频率/奇偶/和值/012路/连号),
    实证开奖高度随机、无 exploitable 模式, 直接支撑诚实框架(no_edge)。
    另挂 NIST SP 800-22 标准套件(密码学级)作"位级"灵敏度压力测试:
    若检出超出已知良性偏差的可利用结构才报警(ANOMALY), 其个别项"未通过"是编码/偏差假象, 不纳入硬闸门。
    硬闸门 = 卡方无灾难性异常(GROSS); 可检出的极小偏差标记为 KNOWN_BIAS(已知, 非异常)。
    """
    step("17. 开奖序列随机性检验电池")
    fp = os.path.join(WORK_DIR, "dlt_randomness_test.py")
    if not os.path.exists(fp):
        check("开奖序列随机性检验电池", False, "dlt_randomness_test.py 缺失")
        return
    try:
        r = _wait_prelaunched("dlt_randomness_test.py", 120)
        rc = r.returncode
        out = (r.stdout + r.stderr).strip()
    except Exception as e:
    ...
    check("开奖序列随机性检验电池", True,
          f"10项卡方 + NIST SP 800-22 密码学级灵敏度升级: 无灾难性异常, "
          f"{n_bias}项已知极小偏差(非异常,已证伪有正收益可能性); NIST 0 项 ANOMALY")
...
def check_three_body_sync():
    ...
    验证三层不是各跑各的, 而是账实相符、互相咬合:
      (A) 执行器 Windows schtasks DLT_V8_Smart: 已启用 / 以 SYSTEM 运行 / 目标=本目录 dlt_run_v8.bat
      (B) 程序入口 dlt_run_v8.bat: 单入口必须=dlt_smart.py --force (与护栏校验的同一流水线)
      (C) WorkBuddy 自动化数据库:
            - id=v8 (同名展示自动化) 必须为 PAUSED, 否则会和 Windows 排程双触发
    ...
    try:
        ...
        if genuinely_missing:
            issues.append("schtasks 未找到任务 DLT_V8_Smart(可能名称不同)")
        else:
            ...
            if m:
                target = m.group(1)
            if not enabled:
                issues.append("DLT_V8_Smart 未启用(排程停了)")
            if not system:
                issues.append(f"DLT_V8_Smart 非 SYSTEM 运行(实际: {userid})")
            if "dlt_run_v8.bat" not in target:
                issues.append(f"DLT_V8_Smart 目标不是 dlt_run_v8.bat: {target!r}")
    except Exception as e:
        issues.append(f"排程查询不可达(非Windows/受限), 跳过排程配置校验: {e}")
    ...
    if bat:
        ...
        if "dlt_smart.py" not in btxt or "--force" not in btxt:
            issues.append("dlt_run_v8.bat 未以 dlt_smart.py --force 为单入口(流水线漂移)")
    else:
        # 跨平台 skill 包按设计不含 .bat(发布包排除可执行脚本), 缺失属环境不适用, 软跳过而非硬失败
        print("  ⚠️ 跳过: dlt_run_v8.bat 不在本部署(跨平台 skill 包按设计不含 .bat), 版本/三体一致性仅校验其余维度")
    ...
    try:
        ...
        if db:
            ...
            cur = c.cursor()
            cur.execute("SELECT id, status FROM automations WHERE (id='v8' OR id='automation-1785599490412') AND (deleted_at IS NULL OR deleted_at=0)")
            st = {r[0]: r[1] for r in cur.fetchall()}
            c.close()
            if st.get("v8") != "PAUSED":
                issues.append(f"WorkBuddy v8 自动化状态={st.get('v8')} (须 PAUSED 防与排程双触发)")
            if st.get("automation-1785599490412") != "ACTIVE":
                issues.append(f"看门狗 automation-1785599490412 状态={st.get('automation-1785599490412')} (须 ACTIVE)")
        else:
            issues.append("workbuddy.db 不存在(无法校验自动化状态)")
    except Exception as e:
        issues.append(f"自动化数据库校验失败: {e}")
    ...
    if issues:
        ...
        check("三体协同一致性", True,
              "排程(SYSTEM/启用/bat)↔程序(单入口dlt_smart.py)↔自动化(v8暂停防双触发+看门狗激活)↔模型产物新鲜 四层咬合无漂移")
...
def check_root_skill_product_sync():
    ...
    back_combos 根=21 副本=7)。本闸门把此漂移永久纳入护栏:
      ...
      1) 取根目录最新一期 dlt_prediction_*_v8.json, 解析其周期号
    ...
    root_preds = sorted(glob.glob(os.path.join(WORK_DIR, "dlt_prediction_*_v8.json")),
                        key=os.path.getmtime)
    if not root_preds:
        check("根↔SKILL产物同步", True, "根目录无预测产物(首跑前, 跳过)")
        return
    latest_root = root_preds[-1]
    m = re.search(r'dlt_prediction_(\d+)_v8\.json', os.path.basename(latest_root))
    ...
    miss = []
    for pat in (f"大乐透{root_period or ''}*预测报告_V8_全面修复.html",
                f"大乐透{root_period or ''}*_V85_增强版.html"):
...
def check_root_skill_data_sync():
    ...
    但第三种漂移是**捆绑离线数据本身不同步**: 根目录 dlt_history.json 在每次开奖后被
    ...
    预测链静默用了旧数据。本闸门把"离线数据时效性 + 根↔副本一致"永久纳入护栏:
      ...
      1) 取根 dlt_history.json 最新期号与整条记录(含开奖号码)
      2) 若 SKILL 副本不可达→软跳过(非致命, 跨平台语境无 skill)
      3) 副本 dlt_history.json 必须存在且 最新期号 >= 根最新期号(副本不能"落后")
      4) 若两者最新期号一致, 深度比较该期整条记录(前区/后区/日期)必须完全相同
    ...
    root_hist = os.path.join(WORK_DIR, "dlt_history.json")
    if not os.path.exists(root_hist):
        check("根↔SKILL离线数据同步", True, "根 dlt_history.json 缺失(首跑前, 跳过)")
        return
    ...
    skill_hist = os.path.join(skill_dir, "dlt_history.json")
    if not os.path.exists(skill_hist):
        check("根↔SKILL离线数据同步", False,
              "SKILL 副本缺失 dlt_history.json(须将根离线数据同步进副本)")
        return
    ...
    if issues:
        check("根↔SKILL离线数据同步", False,
              "离线数据未同步: " + "; ".join(issues) + " → 须将根 dlt_history.json 同步进 SKILL 副本")
        return
    check("根↔SKILL离线数据同步", True,
          f"dlt_history.json 根最新期{rp} 副本最新期{sp} 一致(无数据漂移)")
...
def check_undefined_names_gate():
    ...
    动机(2026-08-04 实战): dlt_smart.py 新增函数里用了 glob, 但该模块的
...
def check_scheduler_runtime_health():
    ...
    动机(2026-08-04 实战, 此前 21 项全绿却漏检的真实生产缺陷):
      ...
      2026/08/03 20:51 那次排程运行**开跑即被杀**, dlt_scheduler_run.log
    ...
    检查:
      ...
      (D) 未处理告警文件 dlt_run_alert.txt 不得比最近产物更新
    ...
    reachable = False
    try:
        r = subprocess.run(["schtasks", "/query", "/tn", "DLT_V8_Smart", "/fo", "list", "/v"],
                           capture_output=True, timeout=30)
        out = (r.stdout or b"").decode("gbk", errors="replace")
        if r.returncode == 0 and ("DLT_V8_Smart" in out or "任务名" in out):
            reachable = True
    ...
    if reachable:
        try:
            rx = subprocess.run(["schtasks", "/query", "/tn", "DLT_V8_Smart", "/xml"],
                                capture_output=True, timeout=30)
            raw = rx.stdout or b""
    ...
    # ---- (C) 上次排程日志"开跑即中断"特征 ----
    # 路径修正(2026-08-07): dlt_run_v8.bat 以 `cd /d "%~dp0"` 在**自身所在目录**(Root 顶层 /
    # SKILL 包 scripts/)写日志与告警, 而本文件位于 lib/ → WORK_DIR 恒差一层, 旧代码在 lib/ 下
    # 找 dlt_scheduler_run.log 永远落空, (C)(D) 两个子检查因此**从未真正生效**(静默漏检)。
    # 现改为 SCRIPTS_DIR 优先、WORK_DIR 兜底, 两处都存在时取 mtime 最新者。
    ...
    # ---- (D) 未处理告警 ----
    # 同 (C): 告警文件由 bat 写在顶层, 须用 SCRIPTS_DIR 查找; 产物则两层都要 glob
    # (lib/ 存最新期, 顶层可能留有历史副本), 取全局最新 mtime 才是正确的比较基准。
    alert = _pick_runtime_file("dlt_run_alert.txt")
    if alert:
        try:
            import glob as _g
            reports = []
            for _d in (WORK_DIR, SCRIPTS_DIR):
                reports += _g.glob(os.path.join(_d, "*增强版*.html"))
                reports += _g.glob(os.path.join(_d, "dlt_prediction_*_v8.json"))
            newest = max((os.path.getmtime(p) for p in reports), default=0)
            if os.path.getmtime(alert) > newest:
                issues.append("dlt_run_alert.txt 比最新产物还新 → 存在未处理的排程失败告警")
            else:
    ...
    if not reachable and not issues:
        print("  ⚠️ 跳过: schtasks 不可达(非 Windows 部署 / 跨平台 skill 包), 排程运行态无法校验")
        return
...
def check_track_record_selfcheck():
    ...
    fp = os.path.join(WORK_DIR, "dlt_track_record_selfcheck.py")
    if not os.path.exists(fp):
        check("预测追踪自证闸门", False, "dlt_track_record_selfcheck.py 缺失")
        return
    try:
        import dlt_track_record_selfcheck as tr
...
def check_self_integrity_diagnose():
    ...
    (`python dlt_self_integrity.py --init`) 而非误阻断交付。
    """
    step("24. 自查自愈增强闸门 (篡改检测/自愈 + 数据过期 + 一致性)")
    fp = os.path.join(WORK_DIR, "dlt_self_integrity.py")
    if not os.path.exists(fp):
        check("自查自愈增强闸门", False, "dlt_self_integrity.py 缺失")
        return
    try:
    ...
    if not integrity.get("ok"):
        if os.path.exists(si.MANIFEST):
            ...
            issues.append("代码完整性基线陈旧(有意升级后未重建) → 请运行 "
                          f"`python dlt_self_integrity.py --init` 重建基线; 疑似差异={integrity.get('tampered')}")
        else:
