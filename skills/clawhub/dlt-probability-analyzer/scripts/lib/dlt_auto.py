...
from dlt_expert_roster import render_expert_views_html, render_expert_baseline_html
...
from dlt_responsible_facts import get_facts as _get_responsible_facts  # 权威事实+2026反诈案例(教育/防骗)
...
def _gen_core(draws, models, valid_combos, expert_picks, salt=0):
    ...
    # ---- 最小成本最大收益覆盖方案 (V8.9.9 新增, 用户需求: 用最小成本获取最大收益, 至少捕获3-7等奖) ----
    # 诚实: 纯组合覆盖优化, 不宣称预测优势(no_edge); 在固定预算内最大化命中六/七等奖概率。
    try:
        from dlt_coverage_optimizer import optimize, honest_ev_note
        _front_pool = sorted(combined_score, key=lambda n: combined_score[n], reverse=True)[:14]
        _back_pool = sorted(back_scored, key=lambda n: back_scored[n], reverse=True)[:12]
        _best, _all = optimize(_front_pool, _back_pool, budget=30, target_tier=7, n_sims=120000)
        if _best:
            dantuo['coverage_plan'] = {
                'name': _best.name,
                'cost': _best.cost,
                'notes': len(_best.tickets),
                'p_seven': _best.p_capture.get(7, 0),
                'p_six': _best.p_capture.get(6, 0),
                'p_any': _best.p_any,
                'front_pool': _front_pool,
                'back_pool': _back_pool,
                'honesty': honest_ev_note(_best),
            }
            print(f"\n  最小成本最大收益方案: {_best.name} = ¥{_best.cost}({len(_best.tickets)}注) "
                  f"P(七等)={_best.p_capture.get(7,0)*100:.1f}% P(六等)={_best.p_capture.get(6,0)*100:.2f}%")
        else:
            dantuo['coverage_plan'] = {'error': 'optimizer 未返回方案'}
    except Exception as e:
        dantuo['coverage_plan'] = {'error': str(e)}
        print(f"\n  ⚠ 覆盖优化引擎异常({e})")
...
def generate_report(draws, models, groups, dantuo, expert_picks, data_issues, back_top4_main):
    ...
    # ===== 2026 真实反诈案例 + 官方反诈铁律（来自 dlt_responsible_facts，教育/防骗用途）=====
    _resp = _get_responsible_facts()
    _scams = _resp.get('scam_alerts', []) or []
    scam_block = "\n".join(
        f"<li><strong>{s.get('title','')}</strong>（损失 {s.get('loss','')}）：{s.get('summary','')}"
        f"<br><span style='color:#9fb4e6;font-size:11px;'>来源：{s.get('source','')}</span></li>"
        for s in _scams
    ) or "<li>（暂无案例数据）</li>"
    _rules = _resp.get('anti_fraud_rules', []) or []
    antifraud_block = "\n".join(f"<li>{r}</li>" for r in _rules) or "<li>（暂无）</li>"
...
<p style="color:#888; font-size:11px; margin-top:8px; line-height:1.6;">{dantuo.get('honesty','')}</p>
    ...
    # ▶ 最小成本最大收益覆盖方案 (V8.9.9 新增, 用户需求)
    _cov = dantuo.get('coverage_plan') or {}
    if _cov and 'error' not in _cov:
        html += f"""
<div class="section" style="border:2px solid #00d4ff;">
<div class="section-title">▶ 最小成本最大收益方案（覆盖优化 · 专攻六/七等奖）</div>
<div class="info">
<h3>ℹ️ 为什么单式几乎中不了低等奖</h3>
<p style="color:#88ccff; font-size:14px; line-height:1.7;">
单式只覆盖 <strong>1 个前区组合 + 1 个后区对</strong>，按现行 7 奖级规则几乎永远凑不出六/七等奖所需的对齐（需前区≤3 且后区≤2）。
本方案用<strong>复式/胆拖的组合覆盖</strong>在固定预算内铺满号码空间，<strong>最大化命中六/七等奖的概率</strong>——
这是成本结构优化，不是预测优势（任何选号法都不优于随机，no_edge 不变；返奖率约51%，期望净亏）。
</p>
</div>
<div class="stat-grid">
<div class="stat-card"><div class="value" style="color:#00d4ff;">¥{_cov.get('cost',0)}</div><div class="label">方案成本（{_cov.get('notes',0)}注）</div></div>
<div class="stat-card"><div class="value" style="color:#00d4ff;">{_cov.get('p_seven',0)*100:.1f}%</div><div class="label">P(命中七等奖)</div></div>
<div class="stat-card"><div class="value" style="color:#00d4ff;">{_cov.get('p_six',0)*100:.2f}%</div><div class="label">P(命中六等奖)</div></div>
<div class="stat-card"><div class="value" style="color:#00d4ff;">{_cov.get('p_any',0)*100:.1f}%</div><div class="label">P(命中任意奖)</div></div>
</div>
<div class="group-card">
<h3>推荐结构: {_cov.get('name','')}</h3>
<p style="color:#aaa; font-size:13px;">前区覆盖池(模型高分前 {len(_cov.get('front_pool',[]))} 码): {_cov.get('front_pool',[])}</p>
<p style="color:#aaa; font-size:13px;">后区覆盖池(模型高分前 {len(_cov.get('back_pool',[]))} 码): {_cov.get('back_pool',[])}</p>
<p style="color:#ff9999; font-size:12px; margin-top:8px; line-height:1.6;">{_cov.get('honesty','')}</p>
</div>
<p style="color:#888; font-size:11px; margin-top:8px;">说明: 概率为蒙特卡洛(约12万次随机开奖)估计；所选号码与随机机选等价，优化仅针对"覆盖结构"。对比: 单式 P(七等奖)≈6.0%，本方案约翻倍。理性投注，量力而行。</p>
</div>
"""
...
<li><strong>🚨 2026年真实反诈案例（均来自警方/官方通报，请引以为戒）</strong>：
<ul style="margin:4px 0 4px 18px; padding-left:6px; line-height:1.7; color:#ffd0d0;">
{scam_block}
</ul>
</li>
<li><strong>🛡️ 官方反诈铁律</strong>：
<ul style="margin:4px 0 4px 18px; padding-left:6px; line-height:1.7; color:#ffe0a0;">
{antifraud_block}
</ul>
</li>
<li><strong>2026规则要点</strong>：奖级已由9个调整为<strong>7个</strong>；一等奖单注2元封顶1000万、3元追加封顶1800万，且<strong>单期一等奖总奖金封顶1亿元</strong>（财政部批复）；8.8亿派奖（26050期起15期）已结束。每注36%纳入公益金。</li>
<li><strong>固定奖分档与本报告口径</strong>：奖池<strong>&lt;8亿</strong>时三至七等奖为<strong>5000 / 300 / 150 / 15 / 5元</strong>；奖池<strong>≥8亿</strong>时自动上调为<strong>6666 / 380 / 200 / 18 / 7元</strong>（上调20%~40%）。历史数据源不提供逐期奖池，无法还原每期档位，因此<strong>本报告的中奖金额与ROI一律按「奖池&lt;8亿保底档」保守计算</strong>——若当期奖池实际≥8亿，三至七等奖实发金额会更高。此举是为了<strong>绝不夸大收益</strong>。</li>
</ul>
