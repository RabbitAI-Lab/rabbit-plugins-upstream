...
# ---- 修复前 BUG(2026-08-29): 上表曾把"上浮档"金额写死为常态值 ----
# 奖池 <8亿 的期次, 三~七等实际只有 5000/300/150/15/5, 写死上浮档会让
# 中奖金额与回测 ROI 被系统性高估 20%~40%。历史数据源不提供每期奖池,
# 无法逐期还原, 故改为双档 + 显式口径切换。
# 口径默认取"保底档"(保守): 绝不夸大收益, 与本系统"负期望/娱乐"定位一致。
PRIZE_PAYOUT_BASE = {           # 奖池 < 8亿 (保底金额)
    (5, 2): 10_000_000,   # 一等奖 (浮动, 代表性固定值)
    (5, 1): 5_000_000,    # 二等奖 (浮动, 代表性固定值, 单注封顶500万)
    (5, 0): 5000,         # 三等奖
    (4, 2): 5000,         # 三等奖 (4+2 合并入三等奖)
    (4, 1): 300,          # 四等奖
    (3, 2): 150,          # 五等奖
    (4, 0): 150,          # 五等奖 (4+0 合并入五等奖)
    (3, 1): 15,           # 六等奖
    (2, 2): 15,           # 六等奖 (2+2 合并入六等奖)
    (3, 0): 5,            # 七等奖
    (2, 1): 5,            # 七等奖
    (1, 2): 5,            # 七等奖
    (0, 2): 5,            # 七等奖
}
PRIZE_PAYOUT_BOOSTED = {        # 奖池 >= 8亿 (上浮档金额)
    (5, 2): 10_000_000,
    (5, 1): 5_000_000,
    (5, 0): 6666,
    (4, 2): 6666,
...
POOL_BOOST_THRESHOLD = 800_000_000  # 8亿元

# 当前生效口径: False=保底档(默认, 保守) / True=上浮档(奖池>=8亿)
_POOL_GE_8YI = False


def set_pool_mode(pool_ge_8yi):
    """切换奖金档位口径。True=上浮档(奖池>=8亿), False=保底档(默认)。

    **原地更新** PRIZE_PAYOUT 的内容(不重新绑定), 这样所有已经
    `from dlt_power_engine import PRIZE_PAYOUT` 的模块持有的仍是同一字典
    对象, 切换后自动同步 —— 若改成重新绑定, 那些模块会停留在旧表上,
    造成"口径已切但判定未变"的静默不一致(与 SSQ 福运奖同源问题)。
    """
    global _POOL_GE_8YI
    _POOL_GE_8YI = bool(pool_ge_8yi)
    target = PRIZE_PAYOUT_BOOSTED if _POOL_GE_8YI else PRIZE_PAYOUT_BASE
    PRIZE_PAYOUT.clear()
    PRIZE_PAYOUT.update(target)
    return PRIZE_PAYOUT


def prize_table(pool_ge_8yi=None):
    """返回指定口径的奖金表; 不传参则用当前全局口径。"""
    if pool_ge_8yi is None:
        pool_ge_8yi = _POOL_GE_8YI
    return PRIZE_PAYOUT_BOOSTED if pool_ge_8yi else PRIZE_PAYOUT_BASE


def pool_mode_label(pool_ge_8yi=None):
    """返回人类可读的口径说明, 供报告/日志标注使用(勿含括号, 避免嵌套)。"""
    if pool_ge_8yi is None:
        pool_ge_8yi = _POOL_GE_8YI
    return ('奖池≥8亿上浮档' if pool_ge_8yi
            else '奖池<8亿保底档·保守口径')


# 向后兼容主入口: 全系统既有 import 均走这里, 默认=保底档(保守口径)。
# 必须是**独立副本**而非 PRIZE_PAYOUT_BASE 的别名, 否则 set_pool_mode
# 的 clear() 会把 PRIZE_PAYOUT_BASE 一起清空。
PRIZE_PAYOUT = dict(PRIZE_PAYOUT_BASE)
PRIZE_NAME = {
    (5, 2): '一等奖', (5, 1): '二等奖',
    (5, 0): '三等奖', (4, 2): '三等奖',
    (4, 1): '四等奖',
