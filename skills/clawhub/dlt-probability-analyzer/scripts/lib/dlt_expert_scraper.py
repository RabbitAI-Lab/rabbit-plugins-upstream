"""
大乐透专家推荐自动抓取 / 解析模块  (V8.7 升级)

设计目标：把"专家推荐"从「手动改 JSON」变成「自动化、可校验」的环节。
提供三种获取途径，按优先级自动组合：
  1. 通用文本解析 (parse_experts_from_text)：从任意粘贴/搜索结果文本中抽取
     "前区 XX XX XX XX XX + 后区 XX XX" 或 "XX XX XX XX XX + XX XX" 形态，
     自动命名专家。这是最稳的方式——自动化任务把 WebSearch 结果写入
     dlt_expert_input.txt，本脚本即可解析并更新 dlt_expert_picks.json。
  2. 站点直抓 (--live)：尝试新浪/中彩网/头条等 HTTP 源（best-effort，失败不致命）。
  3. 本地输入文件 (--from-file PATH)：解析指定文本文件。

用法：
  python dlt_expert_scraper.py --auto            # 自动：尝试live + 读取 dlt_expert_input.txt
  python dlt_expert_scraper.py --from-file txt  # 解析指定文本文件并更新JSON
  python dlt_expert_scraper.py --live            # 仅尝试HTTP直抓
"""
import re
import json
import sys
...
from dlt_huiniao_api import safe_urlopen  # 协议白名单校验(防 file:// / MITM, 专家源可能为外部URL)

try:
    from dlt_expert_roster import EXPERTS as _ROSTER, DATA_SOURCES as _DATA_SOURCES, meta_for
...
def parse_experts_from_text(text, require_named=False):
    ...
    支持的形态（不区分全角/半角、空格/逗号/加号）：
      前区 05 08 11 24 31 + 后区 03 10
      前区: 05 08 11 24 31  后区: 03 10
      05 08 11 24 31 + 03 10
      张三：前区 05 08 11 24 31 + 后区 03 10
    ...
    NOT_NAME = {'前区', '后区', '单注', '推荐', '预测', '分析', '大乐透',
    ...
    for blk in blocks:
        ...
        if not blk:
            continue
        # 跳过注释行: 以 # 开头的行是人工说明, 不得当作专家推荐解析
        # (否则注释里的日期/网址数字会被误当成推荐号, 生成伪专家)
        if blk.startswith('#'):
            continue

        front, back = [], []
        # 形态1: 显式 "前区 ... 后区 ..."
        m_exp = re.search(r'前区[：:\s]*([\d\s、,，+]+?)\s*后区[：:\s]*([\d\s、,，+]+)', blk)
        if m_exp:
        ...
        # 专家名：前缀 "XXX：" 且非关键词
        name = None
        m_name = re.match(r'^([\u4e00-\u9fa5A-Za-z0-9·]{1,12})[：:\s]', blk)
        if m_name:
...
def fetch_from_file(filepath):
    """读取本地文本文件并解析为专家列表（跳过 # 注释行，避免模板示例被误解析）"""
    try:
...
# 公开可访问的"大乐透专家推荐/号码分析"页面(非付费、best-effort)。
# 说明: 多数专家站点为 JS 渲染或付费墙, 定时任务里的纯 HTTP 抓取只能尽力而为;
# 最稳的"实时"路径是智能体(本系统宿主)用 WebSearch 抓取后写入 dlt_expert_input.txt。
# 注意: 仅保留"专家推荐/分析"类资讯页。datachart.500.com/dlt/ 是历史开奖数据页,
# 含大量 "XX XX XX XX XX + XX XX" 形态的历史号码, 会被通用解析器误当成专家推荐,
# 故不纳入 HTTP 直抓源(开奖数据由 dlt_huiniao_api 负责, 不在此重复)。
# V8.9.8: 新增指向"已知名家榜单页"的源(乐彩原创/彩宝贝十大/彩经网榜), 提升抓取针对性与时效。
LIVE_SOURCES = [
    ('乐彩网-原创分析', 'https://www.17500.cn/arts/'),
    ('彩宝贝-十大专家', 'https://www.78500.cn/dltyuce/'),
    ('彩经网-专家排行榜', 'https://www.cjcp.com.cn/showsortchannel.php?type=dlt&sm=2'),
    ('中彩网-大乐透', 'https://www.zhcw.com/dlt/'),
    ('新浪彩票-大乐透', 'https://sports.sina.com.cn/lottery/'),
    ('今日头条搜索', 'https://so.toutiao.com/search?keyword=' + urllib.parse.quote('大乐透 专家推荐 预测 下期')),
]


def fetch_live_experts():
    """尝试多个公开 HTTP 源(best-effort), 返回专家列表。
    对抓回的页面做标签剥离后用通用解析器抽取 '前区...后区...' 形态。"""
    experts = []
    for name, url in LIVE_SOURCES:
        try:
            ...
            # 防误报: 仅当页面确实含大乐透推荐特征时才解析, 避免随机数字被当成专家
            if not (('前区' in text or '后区' in text or '大乐透' in text)
                    and ('+' in text or '前区' in text or '后区' in text)):
                continue
...
def fetch_search_experts(query='大乐透 专家推荐 预测 下期'):
    ...
    try:
        ...
        if html:
            text = _html_to_text(html)
            if (('前区' in text or '后区' in text or '大乐透' in text) and '+' in text
...
def fetch_all_experts(target_period=None, text_file=None):
    ...
    if text_file and os.path.exists(text_file):
        c = fetch_from_file(text_file)
        input_contributed += len(c)
        all_experts.extend(c)

    # 2) 默认输入文件（自动化任务写入的 WebSearch 结果）
    default_input = 'dlt_expert_input.txt'
    if os.path.exists(default_input):
        c = fetch_from_file(default_input)
        input_contributed += len(c)
        all_experts.extend(c)

    # 3) 仅当本地 input 未贡献任何专家时, 才尝试 HTTP/搜索兜底。
    #    (关键修复: 此前无论 input 是否有数据, 都会把 fetch_search/fetch_live 返回的
    #     历史库/过期专家混入当期 picks, 导致 picks 非确定性 + 混入非当期推荐,
    #     进而使 dlt_auto 生成报告 与 交叉验证独立重算 使用不同专家集合 → 三方不一致。
    #     智能体已用 WebSearch 把当期真实推荐写入 input.txt, 它就是权威源。)
    if input_contributed == 0:
        try:
        ...
        print(f"  ✓ input.txt 已贡献 {input_contributed} 位当期真实专家, 跳过 live/search 兜底(避免混入非当期/历史数据)")
    ...
    return {
        ...
        'input_contributed': input_contributed,
        'experts': valid,
    }
...
def _normalize_expert(e):
    ...
    规范：专家名统一用键 'expert'(与 dlt_auto.py 消费端一致)，前区5个
    [1,35] 去重升序、后区2个 [1,12] 去重升序；不合法返回 None。"""
...
def _source_prio(source):
    """来源可信度优先级：历史精选(curated) > 智能体抓取 > 在线实时爬取 > 兜底。"""
    s = (source or '')
    if s == 'curated':
        return 4
...
def update_expert_picks_json(filepath='dlt_expert_picks.json', scraped_data=None,
                             curated_data=None, max_experts=40, auto_scraped=False):
    """汇总并写回 dlt_expert_picks.json。
    ...
    # V8.9.9 关键修复: 不再继承旧文件中的专家。旧文件专家(含历史 curated 库)并非当期推荐,
    # 继承它们会导致 (1) picks 非确定性(随旧文件内容漂移) (2) 混入非当期/历史专家,
    # 使 dlt_auto 生成报告 与 交叉验证独立重算 使用不同专家集 → 三方不一致。
    # 仅当本次运行确实抓到新专家时以新数据为准; 完全无新数据时才回退旧文件(避免空白)。
    has_new = bool((scraped_data and scraped_data.get('experts')) or
                   (curated_data and curated_data.get('experts')))
    if has_new:
        if scraped_data:
            for e in scraped_data.get('experts', []):
                if e.get('source') == 'http:search':
                    # Bing 搜索摘要解析出的"伪专家"不可靠, 不入 curated 库
                    continue
        ...
        # 完全无新数据: 回退旧文件专家(避免空白), 诚实标注 curated
        for e in existing.get('experts', []):
            n = _normalize_expert(e)
            if n:
                n['source'] = 'curated'   # 历史精选库, 诚实标注来源
                put(n)
    ...
    # V8.9.5 主动体检修复: 此前写回时只保留旧 _meta, 从不吸收最新 target_period,
    ...
    new_meta = dict(existing.get('_meta', {
        'description': '大乐透专家推荐数据 - 历史 curated 专家库汇总(权威名家+野路子高手)',
        'auto_scraped': False,
        'note': '诚信说明: 实时HTTP抓取源(17500.cn等)当前失效(HTTP 404); 智能体WebSearch仅能取得专家【名】与分析口径, 未取到可核实的真实推荐号。专家库为历史curated汇总, auto_scraped=False。模型仅将专家数据用作逆向(避开热门)描述性信号, 数学上不改变中奖概率(no_edge)。',
    }))
    # 诚实闸门: auto_scraped 仅当本次运行确实由智能体经输入文件注入了真实推荐才为 True
    new_meta['auto_scraped'] = auto_scraped
    ...
    # V8.9.8: 常驻专家名录段——即使实时抓取失败, 系统也始终'拥有'权威+野路子专家
...
def _auto_target():
    try:
        from dlt_period import next_period as _np
...
if __name__ == '__main__':
    ...
    # V8.9.8: 打印常驻专家名录(供诊断/展示"系统拥有哪些专家")
    if mode == '--roster':
        try:
            from dlt_expert_roster import catalog_summary
            print("专家名录汇总:", catalog_summary())
            print("运行 `python dlt_expert_roster.py` 查看完整名单与官方数据源。")
        except Exception as e:
            print("名录模块不可用:", e)
        sys.exit(0)
    ...
    if data['source_count'] > 0:
        # 诚实闸门: auto_scraped 仅当本次确实抓到真实推荐(--live 显式直抓, 或智能体经输入文件注入)
        auto_scraped = (mode == '--live') or (data.get('input_contributed', 0) > 0)
        update_expert_picks_json('dlt_expert_picks.json', data, auto_scraped=auto_scraped)
