# -*- coding: utf-8 -*-
"""
中奖人数/销售额 实证检验 (dlt_winner_stats.py)
====================================================================
...
做两套严格检验, 并给出诚实结论:
  ...
  [B] 冷热号 EV 检验 (唯一站得住的角度, 但与"预测"无关)
      人类选号偏好偏差: 生日号(1-31, 尤其1-12)被过度投注 ->
      这些号码若开出, 中奖人数暴涨 -> 浮动奖(一/二等奖)每人分得变少。
      检验: "生日号密集"的期 vs "生日号稀疏"的期, 比较
        一/二等奖注数、单人浮动奖回报, 看分奖效应是否真实。
      结论: 分奖效应真实存在, 但它只影响"万一中奖能分多少",
            不改变中奖概率, 也不改变负期望(EV<0, f*<0)。对"预测号码"零帮助。

数据来源说明: 本文件仅用于抓取"中奖注数/销售额"结构化数据, 与
dlt_expert_scraper.py 的"专家推荐资讯页"用途完全隔离, 不涉及专家误报风险。

注: 反诈骗闸门第5套。被 dlt_healthcheck_all.check_antifraud_gate 调用,
...
CACHE = os.path.join(WORK_DIR, "dlt_winner_stats.json")

# 500 datachart 大乐透历史表 (含 销售额/奖池/一二等奖注数)
HIST_URL = "https://datachart.500.com/dlt/history/newinc/history.php?start={start}&end={end}"
...
def run_tests():
    ...
    for feat_name, feat in [("下期和值(差分)", dsum5), ("下期AC(差分)", dac),
        ...
        for pred_name, pred in [("本期一等奖注数(差分)", dp1c),
            ...
            print(f"    {pred_name} -> {feat_name}: r={real_r:+.4f}  r²={real_r*real_r*100:4.1f}%  "
                  f"p={p:.3f}  {tag}")
    print("    (注: N=2904 时, 即便 r≈0.1~0.17 也会 p<0.001, 但 r²<3% 属可忽略的")
    ...
    # 直接测: 当期前区生日号占比 越高(越像人类爱选的号) -> 一等奖注数 是否越多?
