...
2026-08-04 就因为这个: dlt_smart.py 新增 sync_products_to_peers() 里用了
glob, 而 dlt_smart.py 的 `import glob` 其实写在**另一个函数内部**(局部作用域),
...
用法:
    python check_undefined_names.py            # 检查当前目录 dlt_*.py + run_dlt.py
...
def main(argv):
    ...
    if not files:
        ...
        files = sorted(_glob.glob(os.path.join(here, 'dlt_*.py')))
        for extra in ('run_dlt.py',):
