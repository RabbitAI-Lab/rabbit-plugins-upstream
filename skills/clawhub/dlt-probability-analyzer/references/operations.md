...
```
        ┌─────────────┐
        │  任务体      │  真正执行器: Windows 计划任务 DLT_V8_Smart
        │ (排程/自动化) │  SYSTEM / 每周一三六 20:10 / 无论登录都跑
        └──────┬──────┘
               │ 调用
               ▼
        ┌─────────────┐
        │  程序体      │  dlt_run_v8.bat → dlt_smart.py --force
...
- **WorkBuddy 自动化 `v8` 必须 PAUSED**：其 rrule 同为周一/三/六 20:10，若启用会与排程双触发。PAUSED = 仅展示、非执行器。
- **看门狗自动化**（每日 21:30）巡检 `LastTaskResult`，正常则简短 ✅，异常主动告警（含读最近日志结尾 / `dlt_run_alert.txt`）。
...
| # | 位置 | 内容 |
|---|------|------|
| 1 | `README.md` | 标题 + 版本章节（§20 二级标题 `## N. V8.9.7 ...`，正则 `^##\s*\d+\.\s*V`） |
| 2 | `dlt_run_v8.bat` | 第 3 行 `REM 大乐透V8.9.7智能预测` |
| 3 | WorkBuddy 自动化 `v8` | 名称/id 中的版本标识 |

代码内当前版本声明也须对齐：`dlt_power_engine.py`（打印 + 写入 JSON 的 `version`）、`dlt_method_explorer.py`（打印 + 注释）、`dlt_auto.py`（写入预测 JSON 的 `version` 字段）。

**不要改**：`V8` / `V8.5` / `V8.8新增` 等是项目/模块代号或历史引入标记，属合理层级。

## 3. 22 项自检护栏（dlt_healthcheck_all.py）
...
| 13 | 方法发现+证伪闸门 | 常跑 `dlt_method_explorer.py`，断言 `no_edge_first_prize=True`，头条=一等奖命中 |
| 14 | 报告反遗漏闸门 | 基础 10 项 + 增强 4 项必需板块，缺即失败 |
| 15 | 数据时效性与完整性 | 最新期号 / 滞后天数(≤7) / 无异常断裂 |
| 16 | 核心函数属性测试 | `dlt_common` 5000 样本属性/差分/变质不变量 |
...
| 20 | 根↔SKILL 离线数据同步 | `dlt_history.json` 等离线兜底数据两副本一致，防"skill 跑的是旧数据" |
...
**注意（第 18 项 vs 第 22 项，务必分清）**：第 18 项只查**静态配置**（已启用 / SYSTEM / bat 目标），第 22 项查**动态运行结果**。二者不可互相替代——2026-08-04 实战中第 18 项全绿，而 `LastTaskResult=267014 (SCHED_S_TASK_TERMINATED)` 显示 08/03 20:51 那次排程**开跑即被杀**，`dlt_scheduler_run.log` 只有 4 行、结尾 `^C`，整条预测流水线根本没执行。MEMORY 里写了多次"体检必须动态验证"，但这条铁律此前**从未落进代码**——静态绿 ≠ 真的跑过。
...
```powershell
$t = Get-ScheduledTask -TaskName 'DLT_V8_Smart'
$t.Settings.DisallowStartIfOnBatteries = $false
$t.Settings.StopIfGoingOnBatteries     = $false
$t.Settings.IdleSettings.StopOnIdleEnd = $false
$t.Settings.ExecutionTimeLimit         = 'PT2H'
Set-ScheduledTask -TaskName 'DLT_V8_Smart' -Settings $t.Settings
```

若未提权，`Set-ScheduledTask` 会抛 `CimException: 拒绝访问`（任务以 SYSTEM 身份运行）。
**没有 Windows 排程部署的用户**（例如只用 `run_dlt.py` 手动跑）不受影响：`schtasks` 不可达时第 22 项自动软跳过。

**告警落盘铁律**：看门狗自动化的判定结论常停留在 `PENDING_REVIEW`/未读状态——"发现了却没人看见"等于没告警（08/03 那次看门狗其实已察觉 `^C`，但用户从未看到）。故看门狗被要求每次把结论写入 `dlt_watchdog_status.txt`（首行 `OK`/`ALERT`），异常时另写 `dlt_run_alert.txt`，由第 22 项(D) 兜底暴露。
...
- **胆拖组合**（用户最看重）：当前 `dlt_auto.py` 接 `dlt_dantuo_optimizer.optimize_dantuo`，1胆5拖+后1胆7拖=35注 形态。**曾因 `--skip-download` 绕过增强层 + `find_valid_dantuo` 固定取前 4 拖码 bug 导致胆拖为空**，已修复（改枚举组合）。
- **各号码频率参考**（含最热/最冷组合，标注概率恒等）。
- **预测能力诚实说明** + **最终结论**（含娱乐/量力而行）。

## 5. Windows 计划任务配置（dlt_run_v8.bat + schtasks）

> ⚠️ 发布包**不含** `dlt_run_v8.bat`（SkillHub 等平台禁用 `.bat` 等可执行脚本）。下面的 bat 仅适用于**本机 Windows 部署**：若你从本仓库/工作区拿到了 bat，或自行创建（内容只需 `python run_dlt.py`），则可按下方 schtasks 命令挂定时任务；跨平台或纯发布包用户请直接 `python run_dlt.py`（已校验为单入口，功能等价）。

- 任务名 `DLT_V8_Smart`：`schtasks /create /tn DLT_V8_Smart /tr "绝对路径\dlt_run_v8.bat" /sc weekly /d MON,WED,SAT /st 20:10 /ru SYSTEM /rl highest`
...
1. **明天要投注前**：手动 `python run_dlt.py`（= 排程真实链路）预演，确认 EXIT=0、报告含胆拖。
2. **日常自检**：`python run_dlt.py --healthcheck` → 全部 ✅（EXIT=0）。
3. **扩充候选法**：在 `dlt_method_explorer.py` 的 `METHODS` 加新方法，跑 `--explore` 看头条是否翻转（翻转让即告警复核）。
...
- 排程疑似漏跑：看门狗 21:30 会告警；或 `schtasks /query /tn DLT_V8_Smart /fo list /v`（GBK 输出）查 LastTaskResult。
