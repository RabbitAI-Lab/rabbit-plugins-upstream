...
- For a one-off artifact or result, use the registered `gpt-4o-image` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi gpt-4o-image <operation> --help
curl --fail --location https://runapi.ai/docs/api/gpt-4o-image/<operation>.md --output contract.md
```
...
```shell
runapi gpt-4o-image <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service gpt-4o-image --action <operation> > result.json
```
...
- SDK integration: https://github.com/runapi-ai/gpt-4o-image-sdk
