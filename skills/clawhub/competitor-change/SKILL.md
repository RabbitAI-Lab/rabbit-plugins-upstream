---
name: amazon-competitor-change-monitor
description: >
  基于已授权竞品的商品快照、确定性 Diff 和已有评论计数，按已支持的周或日周期摘要可观察变化。仅用于确定性竞品监控；
  不用于小时级监控或实时市场数据，不自动调用付费 LLM，也不替代销量、库存、广告、订单或真实退货率数据。
  Requires an ARI API key (ari_live_*).
slug: competitor-change
displayName: Amazon 竞品变化监控
version: 1.4.3
summary: 按已支持周期摘要竞品商品快照变化
license: MIT
---
# Amazon 竞品变化监控
...
- 本入口只执行 `watch/competitor`，输出模板固定为 `watch_digest`。
