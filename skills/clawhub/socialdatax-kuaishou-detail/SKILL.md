...
source_skill: "socialdatax-kuaishou-detail"
...
npx -y socialdatax-skills@latest kuaishou detail \
  ...
  --source-platform clawhub --source-skill socialdatax-kuaishou-detail
...
- `--source-client socialdatax-skills --source-platform clawhub --source-skill socialdatax-kuaishou-detail`: usage attribution for this Agent Skill; keep these values unchanged when running examples from this Skill.
...
This skill is read-only for SocialDataX detail requests. It uses `SOCIALDATAX_API_KEY` from the user's environment at runtime. Generated Skill files do not contain API keys. It does not read local browser data or perform login, posting, liking, commenting, or account changes. The optional Kuaishou local save command writes only to the requested local `--output` path or `--output-dir` directory and does not require `SOCIALDATAX_API_KEY`.
