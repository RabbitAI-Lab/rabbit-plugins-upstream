...
def main():
    ...
    while i < len(sys.argv):
        ...
        if arg == "--level" and i + 1 < len(sys.argv):
            installation_level = sys.argv[i + 1]
            i += 2
        elif arg == "--help":
            print("用法: python query_app_list.py [选项]")
            print("\n选项:")
            print("  --level <量级>  安装量级: 安装量级100万以下、100万-1000万、1000万-5000万、5000万-1亿、1亿以上")
            print("\n示例:")
            print("  python query_app_list.py")
            print("  python query_app_list.py --level '1000万-5000万'")
            sys.exit(0)
        else:
            i += 1
    ...
    if installation_level is not None:
        request_body["installationLevel"] = installation_level
    
    # 调用API
    call_api("/audience/rule/app/list", request_body)
