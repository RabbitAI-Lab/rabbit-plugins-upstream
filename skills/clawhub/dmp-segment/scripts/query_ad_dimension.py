...
def main():
    if len(sys.argv) < 3:
        ...
        print("用法: python query_ad_dimension.py <trackType> <type>")
        print("\n参数说明:")
        print("  trackType: 平台类型 (MOBILE/OTT/PC)")
        print("  type: 维度类型 (活动维度/行业维度/媒体维度)")
        print("\n示例:")
        print("  python query_ad_dimension.py MOBILE 行业维度")
        print("  python query_ad_dimension.py MOBILE 媒体维度")
        sys.exit(1)
    
    track_type = sys.argv[1]
    dimension_type = sys.argv[2]
    
    # 验证平台类型
    valid_track_types = ["MOBILE", "OTT", "PC"]
    if track_type not in valid_track_types:
        print(f"[PARAM_ERROR] trackType必须是 {'/'.join(valid_track_types)} 之一")
        sys.exit(1)
    
    # 验证维度类型
    valid_dimension_types = ["活动维度", "行业维度", "媒体维度"]
    if dimension_type not in valid_dimension_types:
        print(f"[PARAM_ERROR] type必须是 {'/'.join(valid_dimension_types)} 之一")
        sys.exit(1)
    ...
    # 调用API（使用GET方法）
    call_api("/audience/rule/dictionary/search", request_body, method="GET")
