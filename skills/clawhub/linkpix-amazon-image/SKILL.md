---
name: linkpix-amazon-image
description: "帮助亚马逊卖家、跨境电商美工、外贸团队通过青虎AI完成“亚马逊商品图”：支持生成亚马逊标准纯白底图、主副图（附场景图）、A+页面图、对比图、卖点图、多语言图。可调用GPT Image 2、Seedream 5.0等大模型，严格把控合规性、高清尺寸和原生质感，适配美国、欧洲、日本等不同站点的审美要求。适用于Amazon、Amazon Prime、Amazon Vine等平台视觉营销。Use this skill for 亚马逊商品图, 白底图, 主副图, A+页面, 卖点图, 多语言图片, 跨境电商, GPT Image 2, Seedream 5.0, AIGC图像生成。通过青虎AI统一接入，支持素材上传、实时模型配置、任务轮询和结果下载。 当用户要求做亚马逊 商品图、主图套图、详情图、活动图生成或该平台的主图/套图/详情图/活动图时必须触发。关键词：LinkPix、qhkit、青虎、亚马逊商品图、Amazon主图、白底图、主副图、A+页面、Vine、Prime、多语言、跨境电商。"
user-invocable: true
...
帮助亚马逊卖家、跨境电商美工、外贸团队通过青虎AI完成“亚马逊商品图”：支持生成亚马逊标准纯白底图、主副图（附场景图）、A+页面图、对比图、卖点图、多语言图。可调用GPT Image 2、Seedream 5.0等大模型，严格把控合规性、高清尺寸和原生质感，适配美国、欧洲、日本等不同站点的审美要求。适用于Amazon、Amazon Prime、Amazon Vine等平台视觉营销。Use this skill for 亚马逊商品图, 白底图, 主副图, A+页面, 卖点图, 多语言图片, 跨境电商, GPT Image 2, Seedream 5.0, AIGC图像生成。通过青虎AI统一接入，支持素材上传、实时模型配置、任务轮询和结果下载。

一张产品图覆盖该平台的主图套图、详情页和活动海报：套图走 `套图模式`（`platform` 默认 **Amazon**），详情走 `电商详情图`，活动图走自定义生图。对外文案里的 GPT Image 2 / Nano Banana 2 / Seedream 5.0 / Grok Image 只是检索名，真正提交的 `modelLabel` 必须来自 `options`。

## 何时触发

- 「做亚马逊的商品图 / 主图 / 套图 / 详情图 / 活动图 / 海报」
...
qhkit image generate '{"modelLabel":"套图模式","uploadedImages":["./商品图.jpg"],"customCopy":"限时5折","platform":"Amazon","imageCount":6}'
qhkit image estimate '{"modelLabel":"套图模式","uploadedImages":["./商品图.jpg"],"platform":"Amazon","imageCount":6}'
...
qhkit image generate '{"modelLabel":"智慧模型","uploadedImages":["./商品图.jpg"],"prompt":"将产品变为营销活动图，活动内容如下：【限时折扣】，适配Amazon点击率构图"}'
```

平台适配要点：Amazon 合规：主图纯白底、无logo/无促销字；副图/A+ 再放场景与卖点。站点语言用用户指定（英语/德语/日语等）。

- `platform` 候选以 options 为准，本技能默认按 **Amazon** 传。
