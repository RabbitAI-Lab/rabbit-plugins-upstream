...
// Reddit fetcher — resolve the reddit-readonly skill portably (Hermes / OpenClaw / sibling)
const REDDIT_SKILL = [
  join(SKILL_DIR, '..', '..', 'reddit-readonly', 'scripts', 'reddit-readonly.mjs'),
  join(process.env.HOME || '', '.hermes', 'skills', 'reddit-readonly', 'scripts', 'reddit-readonly.mjs'),
  join(process.env.HOME || '', '.openclaw', 'workspace', 'skills', 'reddit-readonly', 'scripts', 'reddit-readonly.mjs')
].find(p => existsSync(p)) || '';

async function fetchReddit(subreddit, limit = 10) {
  try {
    if (!REDDIT_SKILL) return { ok: false, error: 'reddit-readonly skill not found locally' };
...
function weeklyReport() {
  ...
  let report = `📈 DAILY-TIPS-OMNI - Weekly Report
...
if (command === 'tips') {
  ...
  console.log('Usage:');
  console.log('  node daily-tips-omni.mjs tips      # Daily tip');
  console.log('  node daily-tips-omni.mjs save <id>  # Save a tip');
  console.log('  node daily-tips-omni.mjs skip <topic> # Skip topic');
  console.log('  node daily-tips-omni.mjs search <query> # Search');
  console.log('  node daily-tips-omni.mjs weekly     # Weekly report');
  console.log('  node daily-tips-omni.mjs all        # List all tips');
}
