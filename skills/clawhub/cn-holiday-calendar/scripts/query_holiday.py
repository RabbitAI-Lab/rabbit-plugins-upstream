...
except Exception:  # pragma: no cover - optional dependency may not exist in every env
    Lunar = None
...
def refresh_year(year: int, search_word: str | None = None) -> dict[str, Any]:
    cached = load_year_cache(year)
    if cached is not None:
        return cached

    if search_word is None:
        search_word = f"{year} 节假日安排"
...
def _jieqi_snapshot(obj: Any) -> dict[str, str] | None:
    if obj is None:
        return None
    name = _call_noarg(obj, "getName")
    solar = _call_noarg(_call_noarg(obj, "getSolar"), "toYmdHms")
    result: dict[str, str] = {}
    if name:
        result["name"] = str(name)
    if solar:
        result["solar"] = str(solar)
...
def _collect_public_getters(obj: Any) -> dict[str, Any]:
    result: dict[str, Any] = {}
    for name in dir(obj):
        if not name.startswith("get"):
            continue
        if name in {"getPrev", "getNext"}:
            continue
        try:
            attr = getattr(obj, name)
        except Exception:
            continue
        if not callable(attr) or not _callable_takes_no_required_args(attr):
            continue
        try:
            value = attr()
        except Exception:
...
def build_lunar_profile(dt: datetime) -> dict[str, Any] | None:
    if Lunar is None:
        return None
    ...
    profile: dict[str, Any] = {
        ...
        "current_jieqi": _jieqi_snapshot(_call_noarg(lunar, "getCurrentJieQi")),
        "prev_jieqi": _jieqi_snapshot(_call_noarg(lunar, "getPrevJieQi")),
        "next_jieqi": _jieqi_snapshot(_call_noarg(lunar, "getNextJieQi")),
        "ganzhi": {
            "year": _call_noarg(lunar, "getYearInGanZhi"),
            "month": _call_noarg(lunar, "getMonthInGanZhi"),
            "day": _call_noarg(lunar, "getDayInGanZhi"),
            "time": _call_noarg(lunar, "getTimeInGanZhi"),
        },
        "shengxiao": {
            "year": _call_noarg(lunar, "getYearShengXiao"),
            "month": _call_noarg(lunar, "getMonthShengXiao"),
            "day": _call_noarg(lunar, "getDayShengXiao"),
            "time": _call_noarg(lunar, "getTimeShengXiao"),
        },
    }
    for key, getter_name in (("day_yi", "getDayYi"), ("day_ji", "getDayJi")):
        value = _call_noarg(lunar, getter_name)
        if value is not None:
            profile[key] = value
    profile["all"] = _collect_public_getters(lunar)
    return profile
...
def evaluate_date(date_str: str, cache: dict[str, Any] | None, detail: bool = False) -> dict[str, Any]:
    ...
    lunar_profile = build_lunar_profile(dt)
    if lunar_profile is not None:
        result["lunar"] = lunar_profile["lunar"]
        result["jieqi"] = lunar_profile["jieqi"] or None
        result["solar_term"] = lunar_profile["jieqi"] or None
        if detail:
            result["calendar"] = lunar_profile
    return result


def month_rows(year: int, month: int, cache: dict[str, Any] | None, detail: bool = False) -> list[dict[str, Any]]:
    last_day = calendar.monthrange(year, month)[1]
    rows = []
    for day in range(1, last_day + 1):
        rows.append(evaluate_date(f"{year:04d}-{month:02d}-{day:02d}", cache, detail=detail))
    return rows


def print_date(date_str: str, cache: dict[str, Any] | None, detail: bool = False) -> None:
    print(json.dumps(evaluate_date(date_str, cache, detail=detail), ensure_ascii=False, indent=2))


def print_month(year: int, month: int, cache: dict[str, Any] | None, detail: bool = False) -> None:
    rows = month_rows(year, month, cache, detail=detail)
    if detail:
        print(json.dumps(rows, ensure_ascii=False, indent=2))
        return
    print("| date | weekday | workday | kind | holiday | lunar | jieqi |")
    print("| --- | --- | --- | --- | --- | --- | --- |")
    for row in rows:
        print(
            f"| {row['date']} | {row['weekday']} | {'yes' if row['workday'] else 'no'} | "
            f"{row['kind']} | {row['holiday'] or ''} | {row.get('lunar') or ''} | {row.get('jieqi') or ''} |"
        )
...
def main() -> None:
    ...
    month_parser.add_argument("--detail", action="store_true", help="Print expanded lunar calendar data")

    date_parser = sub.add_parser("date", help="Print a single date from cache")
    date_parser.add_argument("value", help="YYYY-MM-DD")
    date_parser.add_argument("--detail", action="store_true", help="Print expanded lunar calendar data")
    ...
    if args.command == "month":
        ...
        print_month(year, month, load_or_refresh_year_cache(year), detail=bool(args.detail))
        return
    if args.command == "date":
        dt = parse_date(args.value)
        print_date(args.value, load_or_refresh_year_cache(dt.year), detail=bool(args.detail))
        return
