...
面向**机器人（工业机器人/服务机器人/移动机器人/ROS）**领域的 Agent Skill：从需求文档/描述输出**标准化、可追溯、可直接导入用例管理平台**的 Excel 测试用例。
...
- **标准合规门禁（V1.1.0）**：国标/行标/企标三级体系，**试验项级覆盖率 100% 硬性验收线**（GB 11291.1/.2、GB/T 36008、GB/T 12642、GB/T 36530、GB 5226.1）
- **隐含项兜底**：奇异位形、指令冲突、掉电抱闸、感知失效降级等需求未明示项自动覆盖或标注 `[隐含项]`
- **测试级别/回归硬性要求**：每模块 ≥2 级别、全 4 级别出现、每模块 ≥1 条回归用例
- **专项设备**：实物样机、Gazebo 仿真、半实物安全 PLC、激光跟踪仪、老化台架
...
├── references/
│   ├── domain-knowledge.md     # 机器人知识库（系统构成/运动/感知/安全 + 标准-试验项矩阵）
│   └── format-spec.md          # 固定格式规范（15 列/编号/Excel 输出，生成时必读）
└── examples/
    ├── requirements.md         # 真实案例需求输入（协作机械臂）
    └── testcases.xlsx          # 对应输出 Excel（36 条，双 sheet）
```
...
3. 技能按五阶段流程生成 `{项目名}_机器人测试用例_{YYYYMMDD}.xlsx`

> 生成 Excel 需要 **Python 3 + openpyxl**（见 frontmatter `compatibility`）。

## 输出规格（速览）

- 15 列：用例编号 / 业务域模块 / 优先级 / 测试维度 / 用例类型 / 设计方法 / 测试场景 / 测试点 / 操作步骤 / 测试数据 / 前置条件 / 需求来源 / **控制参数** / 测试环境 / 测试级别
- 编号规则 `TC-{模块前缀}-NNN`（12 个机器人模块前缀，如 MC/ARM/SAFE/HMI）
- 覆盖统计：需求覆盖率 100% · 模块×维度矩阵 · 试验项级标准覆盖率 · 级别/回归清单

## 示例

`examples/` 内含真实案例：协作机械臂运动与安全需求 → 36 条测试用例 Excel（已通过官方校验脚本 10 项断言）。
