...
面向**工业/协作机器人（六轴机械臂、SCARA、协作臂）**领域的 Agent Skill：从需求文档/描述输出**标准化、可追溯、可直接导入用例管理平台**的 Excel 测试用例。
...
- **标准合规门禁（V1.2.0）**：国标/行标/企标三级体系，**试验项级覆盖率 100% 硬性验收线**（GB 11291.1、GB/T 36008、GB/T 12642、GB 5226.1）
- **隐含项兜底**：奇异位形、指令冲突、掉电抱闸、感知失效降级等需求未明示项自动覆盖或标注 `[隐含项]`
- **测试级别/回归硬性要求**：每模块 ≥2 级别、全 4 级别出现、每模块 ≥1 条回归用例
- **专项设备**：实物样机、离线编程/物理仿真、半实物安全 PLC、激光跟踪仪、老化台架
...
- 编号规则 `TC-{模块前缀}-NNN`（9 个机器人模块前缀，如 MC/ARM/SAFE/HMI）
