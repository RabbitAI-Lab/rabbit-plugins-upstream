...
export function buildPairRoutes(cfg) {
    ...
    async function handleFinish(req, res) {
        ...
        res.setHeader('Content-Security-Policy', "default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src data:; connect-src 'self'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'");
    ...
    async function handleStart(req, res) {
        ...
        const transitioned = await transitionPairSession(cfg.sessionsPath, sid, 'device_connected', now);
        if (!transitioned) {
            sendJson(res, 404, { error: 'not_found' });
            return;
        }
    ...
    async function handleRespond(req, res) {
        ...
        if (!consumed.ok) {
            const status = consumed.error === 'not_found' ? 404 :
...
function validateRespondBody(body) {
    if (typeof body !== 'object' || body === null)
        return { ok: false, error: 'body_not_object' };
    const r = body;
    if (r.v !== 1)
        return { ok: false, error: 'unsupported_version' };
    if (typeof r.sid !== 'string' || !/^[0-9a-f]{32}$/.test(r.sid))
    ...
    if (typeof r.ct !== 'string' || !isB64url(r.ct))
    ...
    if (ctLen < 17 || ctLen > 2048)
        return { ok: false, error: 'bad_ct_length' };
    return {
        ok: true,
        value: {
            v: r.v,
