 ...
 */
function resolveCoreCodec() {
    if (_coreCodec !== undefined)
        return _coreCodec;
    try {
        const core = requireWasm('@totalreclaw/core');
        if (typeof core.encodeEmbeddingCanonical === 'function' &&
 ...
 */
function f16LeBytesToNumber(lo, hi) {
...
export function decodeEmbeddingLocal(payload) {
    ...
    if (trimmed.startsWith('[')) {
        ...
        try {
            ...
            throw new Error(`invalid JSON embedding array: ${e instanceof Error ? e.message : String(e)}`);
        }
    ...
    throw new Error(`decoded embedding buffer has length ${buf.length} (expected ${EMBEDDING_DIMS * 2} for f16, or a multiple of 4 for f32)`);
}
/**
 ...
 */
export function decodeEmbeddingWithCore(payload, core) {
    if (core)
        return Array.from(core.decodeEmbeddingUniversal(payload));
 ...
 */
export function decodeEmbeddingUniversal(payload) {
 ...
 */
export function decodeEmbeddingFromHex(hexBlob, encryptionKey) {
 ...
 */
export function encodeEmbeddingPayloadWithCore(embedding, core) {
    if (core)
        return core.encodeEmbeddingCanonical(embedding);
 ...
 */
export function encodeEmbeddingPayload(embedding) {
