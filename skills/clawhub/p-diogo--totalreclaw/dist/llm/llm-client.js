 ...
 */
export function selectCheapestConfiguredModel(configuredModels) {
    if (!configuredModels || configuredModels.length === 0)
        return null;
    const cheap = configuredModels.filter((m) => m && typeof m.id === 'string' && CHEAP_INDICATOR_RE.test(m.id));
    if (cheap.length === 0)
        return null;
 ...
 */
export function resolveExtractionModel(provider, opts) {
...
export function initLLMClient(options) {
    ...
    if (llmOverride && typeof llmOverride === 'object') {
        ...
        if (provider && apiKey) {
            ...
            if (resolved) {
                ...
                _logger?.info?.(`TotalReclaw extraction LLM: resolved ${provider}/${resolved.config.model} (plugin config override; model: ${resolved.reason})`);
                return;
            }
        }
    }
    ...
    if (openclawProviders) {
        if (providerFromPrimary) {
            ...
            if (ocProvider?.apiKey) {
                ...
                if (resolved) {
                    _cachedConfig = resolved.config;
                    _logger?.info?.(`TotalReclaw extraction LLM: resolved ${providerFromPrimary}/${resolved.config.model} (OpenClaw provider config; model: ${resolved.reason})`);
                    return;
                }
            }
        }
        for (const [providerName, providerConfig] of Object.entries(openclawProviders)) {
            ...
            if (resolved) {
                _cachedConfig = resolved.config;
                _logger?.info?.(`TotalReclaw extraction LLM: resolved ${provider}/${resolved.config.model} (OpenClaw provider config; model: ${resolved.reason})`);
                return;
            }
        }
    }
    ...
    if (authProfileKeys && authProfileKeys.length > 0) {
        if (providerFromPrimary) {
            ...
            if (hit) {
                ...
                if (resolved) {
                    _cachedConfig = resolved.config;
                    _logger?.info?.(`TotalReclaw extraction LLM: resolved ${providerFromPrimary}/${resolved.config.model} (auth-profiles.json; model: ${resolved.reason})`);
                    return;
                }
            }
        }
        ...
        for (const entry of ordered) {
            ...
            if (resolved) {
                _cachedConfig = resolved.config;
                _logger?.info?.(`TotalReclaw extraction LLM: resolved ${entry.provider}/${resolved.config.model} (auth-profiles.json; model: ${resolved.reason})`);
                return;
            }
        }
    }
    ...
    if (providerFromPrimary) {
        ...
        if (keyNames) {
            ...
            if (apiKey) {
                ...
                if (resolved) {
                    _cachedConfig = resolved.config;
                    _logger?.info?.(`TotalReclaw extraction LLM: resolved ${providerFromPrimary}/${resolved.config.model} (env var; model: ${resolved.reason})`);
                    return;
                }
            }
        }
    ...
    for (const [provider, keyName] of envFallback) {
        ...
        if (resolved) {
            _cachedConfig = resolved.config;
            _logger?.info?.(`TotalReclaw extraction LLM: resolved ${provider}/${resolved.config.model} (env var; model: ${resolved.reason})`);
            return;
        }
    }
    // ---------------------------------------------------------------------
 ...
 */
export function parseRetryAfter(header, opts = {}) {
    if (header == null)
        return null;
 ...
 */
function stampHttpError(err, status, retryAfterHeader) {
    const e = err;
    if (status !== undefined)
        e.httpStatus = status;
 ...
 */
function wrapLlmError(err) {
    const msg = err instanceof Error ? err.message : String(err);
    const wrapped = new Error(`LLM call failed: ${msg}`);
    if (err && typeof err === 'object') {
        const src = err;
        if (src.httpStatus !== undefined)
            wrapped.httpStatus = src.httpStatus;
        if (src.retryAfterHeader !== undefined)
            wrapped.retryAfterHeader = src.retryAfterHeader;
    }
    return wrapped;
}
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
            ...
            if (retryAfterMs !== null && retryAfterMs > capMs) {
                logger?.warn?.(`chatCompletion: Retry-After ${retryAfterMs}ms exceeds ${capMs}ms ceiling; surfacing outage after ${attempt} attempts (will re-queue): ${msg.slice(0, 160)}`);
                throw new LLMUpstreamOutageError(`LLM upstream outage (Retry-After ${retryAfterMs}ms > ${capMs}ms ceiling after ${attempt} attempts): ${msg.slice(0, 200)}`, attempt, lastStatus);
            }
            ...
            if (attempt === 1) {
                logger?.info?.(`chatCompletion: retrying after transient failure (attempt ${attempt}/${attempts}, wait ${waitLabel}): ${msg.slice(0, 160)}`);
            }
            else {
                logger?.debug?.(`chatCompletion: retry attempt ${attempt}/${attempts} (wait ${waitLabel}): ${msg.slice(0, 160)}`);
            }
...
async function chatCompletionOpenAI(config, messages, maxTokens, temperature, timeoutMs) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw stampHttpError(new Error(`LLM API ${res.status}: ${text.slice(0, 200)}`), res.status, res.headers.get('retry-after'));
        }
...
async function chatCompletionAnthropic(config, messages, maxTokens, temperature, timeoutMs) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw stampHttpError(new Error(`Anthropic API ${res.status}: ${text.slice(0, 200)}`), res.status, res.headers.get('retry-after'));
        }
        const json = (await res.json());
