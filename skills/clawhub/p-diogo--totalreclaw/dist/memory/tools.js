...
// ---------------------------------------------------------------------------
import { parseEntity, VALID_MEMORY_TYPES, VALID_MEMORY_SCOPES, } from '../extraction/extractor.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
function readStringEnum(params, key, allowed) {
    const v = params[key];
 ...
 */
export function createMemorySaveTool(store) {
    return {
        name: 'memory_save',
        label: 'Memory Save',
        description: 'Store ONE explicitly-remembered fact to the user’s encrypted TotalReclaw ' +
        ...
        execute: async (_toolCallId, params) => {
            ...
            const input = { text };
            const type = readStringEnum(raw, 'type', VALID_MEMORY_TYPES);
            if (type)
                input.type = type;
            // importance must be a positive integer within the 1-10 band.
            const importanceRaw = readPositiveIntegerParam(raw, 'importance');
            if (importanceRaw !== undefined && importanceRaw <= 10)
                input.importance = importanceRaw;
            const scope = readStringEnum(raw, 'scope', VALID_MEMORY_SCOPES);
            if (scope)
                input.scope = scope;
            const reasoning = readStringParam(raw, 'reasoning');
            if (reasoning)
                input.reasoning = reasoning.slice(0, 256);
