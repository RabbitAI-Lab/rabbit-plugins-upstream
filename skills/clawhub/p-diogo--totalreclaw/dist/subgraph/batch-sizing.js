 ...
 */
export function estimatePayloadBytes(fs) {
 ...
 */
export function groupPayloadsBySize(payloads, maxCount, maxBytes, sizeOf) {
    const groups = [];
    let group = [];
    let groupBytes = 0;
 ...
 */
export function isSimRevertError(err) {
    ...
    if (/\baa25\b|invalid account nonce/.test(msgL))
        return false;
 ...
 */
export async function groupAndStoreAdaptive(payloads, storeFn, maxCount, maxBytes, sizeOf) {
...
async function storeGroupAdaptive(storeFn, group) {
    try {
