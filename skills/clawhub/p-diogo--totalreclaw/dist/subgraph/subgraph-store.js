...
import { MAX_BATCH_BYTES, groupAndStoreAdaptive, } from './batch-sizing.js';
 ...
 * trip bundler serde).
 */
function reassertInitCodeAfterSponsorship(op, factory, factoryData) {
    if (factory) {
        if (op.factory !== factory)
            op.factory = factory;
        if (op.factoryData !== factoryData)
    ...
    return withSenderLock(sender, async () => {
        ...
        const storeFn = async (group) => {
            ...
            if (!r.success) {
                ...
                throw new Error(`Group of ${group.length} mined but receipt.success=false (tx=${(r.txHash || '').slice(0, 10)}…)`);
            }
            return r;
        };
        const { results, errors } = await groupAndStoreAdaptive(protobufPayloads, storeFn, maxCount, MAX_BATCH_BYTES, b => b.length);
