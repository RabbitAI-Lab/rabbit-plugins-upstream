...
def run_airbnb_listing_details_scraper_task(api_key, listing_id_or_url='5476930', check_in='', check_out='', adults='1', currency='USD'):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python airbnb_listing_details_scraper_api.py <listing_id_or_url> [check_in] [check_out] [adults] [currency]", flush=True)
        sys.exit(1)
    ...
    listing_id_or_url = sys.argv[1] if len(sys.argv) > 1 else '5476930'
    check_in = sys.argv[2] if len(sys.argv) > 2 else ''
    check_out = sys.argv[3] if len(sys.argv) > 3 else ''
    adults = sys.argv[4] if len(sys.argv) > 4 else '1'
    currency = sys.argv[5] if len(sys.argv) > 5 else 'USD'

    result = run_airbnb_listing_details_scraper_task(api_key, listing_id_or_url, check_in, check_out, adults, currency)
    if result:
