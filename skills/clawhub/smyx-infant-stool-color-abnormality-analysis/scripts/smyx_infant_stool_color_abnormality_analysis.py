...
def main():
    parser = argparse.ArgumentParser(description="婴儿大便颜色识别（陶土色/血便）工具")
    parser.add_argument("--input", help="本地婴儿尿不湿/排泄物高清图像文件路径")
    parser.add_argument("--url", help="网络婴儿尿不湿/排泄物高清图像的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="类别标识：婴儿健康筛查场景默认 other")
    parser.add_argument("--open-id", required=False, help=argparse.SUPPRESS)
    parser.add_argument("--list", action='store_true', help="显示婴儿大便颜色异常分析列表清单")
    ...
    try:
        ...
        print("🔍 正在识别婴儿大便颜色（正常/陶土色/血便等）并评估风险等级，请稍候...")
