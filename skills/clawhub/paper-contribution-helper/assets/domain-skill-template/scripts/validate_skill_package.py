...
REQUIRED = [
    ...
    "references/anonymous_casebook.json",
...
def nonempty_resource(path: Path) -> bool:
...
def main() -> None:
    ...
    report = {
        ...
        "base_knowledge_available": nonempty_resource(skill_dir / "references" / "anonymous_casebook.json")
        and nonempty_resource(skill_dir / "references" / "per_pdf_report_synthesis.md")
        and nonempty_resource(skill_dir / "references" / "llm_deep_read_synthesis.md")
        if (skill_dir / "references" / "anonymous_casebook.json").exists()
