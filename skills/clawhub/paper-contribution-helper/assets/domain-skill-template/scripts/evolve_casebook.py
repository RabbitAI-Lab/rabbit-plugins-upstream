...
def main() -> None:
    ...
    overlay_path = overlay_dir / "evolution" / "evolving_casebook.json"
    ...
    if overlay_path.exists():
        try:
            cards = json.loads(overlay_path.read_text(encoding="utf-8", errors="replace") or "[]")
        except Exception:
            cards = []
    else:
        cards = []
    if not isinstance(cards, list):
        cards = []
    cards.append(card)
    overlay_path.write_text(json.dumps(cards, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
