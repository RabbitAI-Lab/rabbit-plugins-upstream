...
- For ClawHub distribution, ship only text-based files accepted by the registry: `SKILL.md`, Markdown/YAML/JSON/JSONL/templates, Python scripts, and license text.
- Exclude platform-specific UI metadata such as `agents/openai.yaml`.
- Exclude `__pycache__`, `.pyc`, `.pyo`, temporary build folders, source caches, old PDFs, binary media, archives, and raw forum dumps.
- Keep the package MIT-0 compatible and do not add conflicting license terms in `SKILL.md`.
