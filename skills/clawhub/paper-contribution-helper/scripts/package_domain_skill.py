...
REQUIRED_INITIALIZED_RESOURCES = ["anonymous_casebook.json", "per_pdf_report_synthesis.md", "llm_deep_read_synthesis.md"]
...
def copy_template_tree(target: Path, variables: dict[str, str]) -> None:
    ...
    for source in root.rglob("*"):
        ...
        target_rel = Path("SKILL.md") if rel.as_posix() == "SKILL.template.md" else rel
        content = read_text(source)
        if rel.as_posix() == "SKILL.template.md":
