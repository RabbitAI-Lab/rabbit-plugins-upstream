#!/usr/bin/env python3
"""
EchoTik TikTok Seller Product List - LinkFox Skill
调用 /echotik/listSellerProduct 接口。

Usage:
  python echotik_list_seller_product.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_list_seller_product.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/listSellerProduct"
SLUG = "linkfox-echotik-list-seller-product"
...
def main():
    ...
    if not argv:
        print(
            "Usage: echotik_list_seller_product.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
