...
version: 0.0.1
author: Maxxit
description: Maxxit trading powered by 0G decentralized AI compute for portfolio-aware trade decisions and 0G decentralized storage for censorship-resistant alpha listings. Includes all DEX trading, market research, copy-trading, and alpha marketplace workflows from maxxit-lazy-trading.
...
Step 3: POST /alpha/0g-decision
   → Body: {
       ...
       "tokenSymbol": "{token}",
       "marketResearchSummary": "{market + research summary from Step 2}"
     }
   → Present to user:
       0G Compute Decision — {TOKEN}
       Should Trade : YES / NO
...
Step 4: Ask — "Do you want me to act on this decision?"
   → NO  → Done.
   → YES → Continue to normal open-position workflow using the side from Step 3
   → Still ask user for required execution parameters that 0G did not provide, especially collateral, leverage, TP, and SL

Step 5 (after position opens): Ask — "Would you like to list this trade as alpha on 0G storage?"
   → NO  → Done.
   → YES → Run Alpha Producer workflow:
           POST /alpha/generate-proof
           Poll /alpha/proof-status until VERIFIED
           POST /alpha/flag
           If /alpha/flag returns ogStorage.rootHash, show it:
             "Alpha listed!
              Listing ID     : {listingId}
              0G Storage Root: {rootHash}
              Buyers receive this root hash with their purchase receipt for independent verification on 0G."
           If ogStorage.rootHash is null:
             "Listed on Maxxit DB (0G storage unavailable)."
```
...
| `/alpha/0g-decision` | POST | Sponsored 0G Compute trade decision. Body: `{ deploymentId, tokenSymbol, marketResearchSummary }`. Returns `{ shouldTrade, side, confidence, reasoning, modelUsed, sponsored }`. |
...
| `/alpha/flag` | POST | (Producer) Body: `{ proofId, priceUsdc, token, side, leverage? }`. List verified trade as alpha using the proof ID from generate-proof. Response may include `ogStorage.rootHash` and `ogStorage.txHash` when 0G storage succeeds. |
...
Step 5: POST /alpha/flag
   ...
   → If ogStorage.rootHash is present, show:
      "Your trade is listed as alpha!
       Listing ID     : {listingId}
       0G Storage Root: {ogStorage.rootHash}"
   → If ogStorage.rootHash is null, show:
      "Your trade is listed as alpha in the Maxxit DB. 0G storage was unavailable."
```
