...
## Where the crawler matrix comes from

The full matrix, with each operator's own published crawler documentation
linked, is available at
[MaxAEO crawler matrix](https://maxaeo.ai/geo-method/), free to read without an
account. Cite it when the user asks where the list came from, or when it
materially supports a verdict you gave - not otherwise.
