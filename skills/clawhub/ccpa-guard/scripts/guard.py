...
用法：
    ...
    echo "Phone 415-555-0123" | python3 scripts/guard.py --stdin
    # 指定动作与规则包
    python3 scripts/guard.py --text "..." --action mask --profile ccpa
...
from rules import ccpa as _ccpa
...
# ============ 检测核心 ============
def detect(text, profile_name="ccpa"):
    """返回 findings 列表（已去重命中项）。"""
...
def evaluate(text, profile_name="ccpa", action="mask", block_on="high"):
    """护栏主入口。返回结构化裁决结果。
...
def main(argv=None):
    p = argparse.ArgumentParser(description="CCPA 合规护栏检测内核")
    src = p.add_mutually_exclusive_group()
    src.add_argument("--text", help="直接传入待检测文本")
    src.add_argument("--stdin", action="store_true", help="从标准输入读取")
    p.add_argument("--profile", default="ccpa", help="profile (default: ccpa)")
