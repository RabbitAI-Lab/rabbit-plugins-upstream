...
- For a one-off artifact or result, use the registered `topaz` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi topaz --help
runapi topaz <operation> --help
curl --fail --location https://runapi.ai/docs/api/topaz/<operation>.md --output contract.md
```
...
```shell
runapi topaz <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service topaz --action <operation> > result.json
```
