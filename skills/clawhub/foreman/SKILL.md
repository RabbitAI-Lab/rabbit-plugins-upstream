...
- **`foreman-cworker.sh` passes `$DEEPSEEK_API_KEY` into a container** as that worker's own API credential. That is the entire point of the caged worker: it receives one key and one mounted worktree, and nothing else — not your home directory, not your other repos, not your primary agent login. The key is read from your environment at call time and is never stored in this skill.
