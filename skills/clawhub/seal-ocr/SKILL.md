---
name: seal_ocr
description: 精准识别图像中的印章信息，返回印章坐标、印章形状、印章颜色、印章类型、印章内文字内容。
version: 1.0.0
author: SCNet
license: MIT
tags:
...
本技能封装了印章识别的 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取印章里核心信息。

## 功能特性

- **印章识别**：精准识别图像中的印章信息，返回印章坐标、印章形状、印章颜色、印章类型、印章内文字内容。。
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| ocrType | string | 是 | 识别类型枚举。必须为以下之一：<br>• SEAL_CHARACTER_RECOGNITION（印章文字识别） |
| filePath | string | 是 | 待识别图片的本地绝对路径。支持 jpg、png、pdf 等常见格式。 |

### 命令行调用示例

```bash
   python .claude/skills/seal_ocr/scripts/main.py SEAL_CHARACTER_RECOGNITION /path/to/test.jpg
```
...
- “帮我识别这张图像中的印章信息，图片在 /Users/name/Downloads/test.jpg”
