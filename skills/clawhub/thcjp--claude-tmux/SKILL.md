...
| 错误现象 | 可能原因 | 诊断步骤 | 解决方案 |
|:-------|:-------|:-------|:-------|
| 无法启动tmux会话 | tmux未安装或配置错误 | 检查tmux是否安装，并确认配置文件 | 安装tmux或修复配置文件 |
| 会话列表为空 | tmux会话未启动或已关闭 | 检查tmux会话状态 | 启动tmux会话或重新连接 |
| 命令执行失败 | 权限不足或命令错误 | 检查命令权限和语法 | 获取必要权限或修正命令 |
| 网络连接问题 | 网络不稳定或代理设置错误 | 检查网络连接和代理设置 | 修复网络连接或调整代理设置 |
| API Key错误 | API Key配置错误或已过期 | 检查API Key配置和有效期 | 重新配置API Key或更新过期Key |
...
| 网络攻击 | 高 | 使用防火墙和入侵检测系统 | 定期检查网络流量和安全事件 |
...
## 核心特性
- **自动化执行**: 纯指令型tmux助手,所宣即所做,会话管理利器。This skill is an instruction-only tm
