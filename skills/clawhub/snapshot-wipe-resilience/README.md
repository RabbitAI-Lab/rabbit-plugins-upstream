...
**Public tags:** #productivity, #workspace-recovery, #snapshot, #resilience, #security
...
```bash
npx --yes clawhub@latest install @orionshaowswmw/snapshot-wipe-resilience
```

Initialize and review the signed manifest, run check/doctor in dry-run mode first, then execute only verified restore recipes; verify peer fingerprints before encrypted sync.
...
**Artifact SHA-256 (TREE-SHA256-v1):** `a37d5bb21689195428b859a5c143d2cdaeb421c8184b4ef70b2581aaa8e1a9a0`
...
# 🧬 Snapshot-Wipe Resilience  <sub>v1.4.1</sub>
...
export DEBUG_LOG=/tmp/snapshot-wipe-resilience-debug.log
```

### Run Diagnostics
```bash
python3 scripts/debugger.py diagnose snapshot-wipe-resilience
```

### View Logs
```bash
tail -f /tmp/snapshot-wipe-resilience-debug.log
```
