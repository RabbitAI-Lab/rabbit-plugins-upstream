#!/usr/bin/env python3
"""
swr — Snapshot-Wipe Resilience (v1.5.6)
...
the manifest carries a valid local HMAC signature, a valid ML-DSA-87 signature
from a trusted signer, or the operator approves its exact digest.
...
# Object-id keyed, process-local provenance. It is never serialized into a
# manifest. Mutation commands may auto-sign only a manifest that was trusted
# when loaded; an untrusted/unsigned input is written unsigned until the user
# approves its exact digest with `swr sign --approve-digest`.
_LOADED_TRUST = {}
...
def bad_manifest(message):
    """Fail closed for malformed or resource-exhausting manifests."""
    eprint(f"bad manifest: {message}")
    raise SystemExit(EXIT_BADMANIFEST)


def secure_dir(path):
    """Create an SWR-owned directory with private permissions."""
...
def atomic_json(path, value, mode=0o600):
    """Write JSON atomically, privately, and durably where supported."""
...
def safe_id(value):
    """Entry IDs are labels, never path fragments or control strings."""
    return (isinstance(value, str) and 1 <= len(value) <= 128 and
            value not in (".", "..") and "/" not in value and "\\" not in value and
            all(ord(c) >= 32 and ord(c) != 127 for c in value))


def safe_relative(value):
    """Sentinel paths are relative members, not arbitrary host paths."""
...
def validate_manifest(man):
    """Validate shape and bounded fields before any verifier or recipe runs."""
    if not isinstance(man, dict):
        bad_manifest("top-level value must be an object")
    v = man.get("version", 1)
    if isinstance(v, bool) or not isinstance(v, int) or v < 1:
        bad_manifest("version must be a positive integer")
    if v > SCHEMA_VERSION:
        bad_manifest(f"schema v{v} is newer than supported v{SCHEMA_VERSION}")
    ws = man.get("workspace")
    if not isinstance(ws, str) or not ws:
        bad_manifest("workspace must be a non-empty path")
    if os.path.realpath(ws) == os.path.sep:
        bad_manifest("workspace '/' is too broad; choose a dedicated directory")
    entries = man.get("entries")
    if not isinstance(entries, list) or len(entries) > MAX_ENTRIES:
        bad_manifest(f"entries must be a list of at most {MAX_ENTRIES:,} items")
    ids = set()
    for i, e in enumerate(entries):
        if not isinstance(e, dict):
            bad_manifest(f"entry {i} is not an object")
        ident = e.get("id")
        if not safe_id(ident):
            bad_manifest(f"entry {i} has an invalid id")
        if ident in ids:
            bad_manifest(f"duplicate entry id {ident!r}")
        ids.add(ident)
        if not isinstance(e.get("path"), str) or not e["path"]:
            bad_manifest(f"entry {ident!r} has no valid path")
        if e.get("kind", "file") not in {"file", "blob", "tree"}:
            bad_manifest(f"entry {ident!r} has an unsupported kind")
        tier = e.get("tier", 5)
        if isinstance(tier, bool) or not isinstance(tier, int) or not 0 <= tier <= 9:
            bad_manifest(f"entry {ident!r} has an invalid tier")
        recipe = e.get("restore")
        if recipe is not None and (not isinstance(recipe, str) or
                                   len(recipe.encode()) > MAX_RECIPE_BYTES):
            bad_manifest(f"entry {ident!r} has an oversized or invalid restore recipe")
        mode = e.get("mode")
        if mode is not None and (not isinstance(mode, str) or
                                 not mode.isdigit() or len(mode) not in (3, 4) or
                                 any(c not in "01234567" for c in mode)):
            bad_manifest(f"entry {ident!r} has an invalid mode")
        for field in ("bytes", "files_at_capture", "min_files"):
            value = e.get(field)
            if value is not None and (isinstance(value, bool) or not isinstance(value, int) or value < 0):
                bad_manifest(f"entry {ident!r} has an invalid {field}")
        for field in ("sentinels", "ignore_dirs", "needs"):
            if field in e:
                vals = e[field]
                if not isinstance(vals, list) or any(not isinstance(x, str) for x in vals):
                    bad_manifest(f"entry {ident!r} has invalid {field}")
        if any(not safe_relative(x) for x in e.get("sentinels", [])):
            bad_manifest(f"entry {ident!r} has an escaping sentinel")
        if any(not safe_relative(x) for x in e.get("ignore_dirs", [])):
            bad_manifest(f"entry {ident!r} has an invalid ignore_dirs member")
        for field in ("sha256", "headtail", "merkle", "tree_sha256"):
            value = e.get(field)
            if value is not None and (not isinstance(value, str) or len(value) != 64 or
                                      any(c not in "0123456789abcdefABCDEF" for c in value)):
                bad_manifest(f"entry {ident!r} has an invalid {field}")
        if "escrow" in e:
            if (not isinstance(e["escrow"], str) or
                    len(e["escrow"]) > 4 * ((ESCROW_MAX + 1023) // 1024) * 1024):
                bad_manifest(f"entry {ident!r} has oversized escrow")
            eb = e.get("escrow_bytes")
            if isinstance(eb, bool) or not isinstance(eb, int) or not 0 <= eb <= ESCROW_DECOMP_MAX:
                bad_manifest(f"entry {ident!r} has invalid escrow_bytes")
        for timeout_field in ("timeout_s", "smoke_timeout"):
            timeout = e.get(timeout_field)
            if timeout is not None and (isinstance(timeout, bool) or not isinstance(timeout, int)
                                        or not 1 <= timeout <= 24 * 3600):
                bad_manifest(f"entry {ident!r} has invalid {timeout_field}")
    for e in entries:
        for dep in e.get("needs", []):
            if dep == e["id"]:
                bad_manifest(f"entry {e['id']!r} depends on itself")
            if dep not in ids:
                bad_manifest(f"entry {e['id']!r} depends on unknown id {dep!r}")
    return man
...
def get_key(create=True):
    ...
    if os.path.exists(KEY_FILE):
        try:
            if os.path.islink(KEY_FILE):
                bad_manifest("signing key must not be a symlink")
            st = os.stat(KEY_FILE)
            if not stat.S_ISREG(st.st_mode) or st.st_uid != os.getuid():
                bad_manifest("signing key is not a regular file owned by this user")
            if stat.S_IMODE(st.st_mode) & 0o077:
                bad_manifest("signing key permissions are too broad; use chmod 600")
            key = open(KEY_FILE, "rb").read().strip()
        except OSError as e:
            bad_manifest(f"cannot read signing key: {e}")
        if len(key) != 64 or any(c not in b"0123456789abcdef" for c in key):
            bad_manifest("signing key is not the expected 32-byte hex value")
        return key
    if not create:
        return None
    ...
    try:
        ...
        # Another process won the race; load and validate its key.
        return get_key(create=False)
    except OSError as e:
        bad_manifest(f"cannot create signing key: {e}")
    return k
...
def verify_sig(man):
    ...
    if not isinstance(s, dict) or not isinstance(s.get("value"), str):
        return "invalid", "malformed HMAC signature"
    got = s["value"]
    ...
    for body in forms:
        if hmac.compare_digest(
            ...
            if isinstance(at, bool) or not isinstance(at, int):
                return "invalid", "malformed HMAC timestamp"
            return "valid", f"signed {max(0, int(time.time()) - at)}s ago"
...
def verify_pksig(man):
    ...
    if not isinstance(ps, dict) or ps.get("alg") != "ML-DSA-87":
        return "invalid", "unsupported or malformed _pksig algorithm", None
    try:
        pub = base64.b64decode(ps["pub"], validate=True)
        sigv = base64.b64decode(ps["value"], validate=True)
    except Exception:
        return "invalid", "malformed _pksig", None
    if not pub or not sigv:
        return "invalid", "empty _pksig key or signature", None
...
def migrate(man):
    ...
    if v < SCHEMA_VERSION:
        ...
        if not isinstance(entries, list):
            bad_manifest("entries must be a list")
        for e in entries:
            if not isinstance(e, dict):
                bad_manifest("entries must contain objects")
            e.setdefault("needs", [])
...
def load(mpath, require=True):
    ...
    try:
        if os.path.getsize(mpath) > MAX_MANIFEST_BYTES:
            bad_manifest(f"file exceeds {MAX_MANIFEST_BYTES // (1024 * 1024)} MiB")
        ...
        eprint(f"cannot read manifest {mpath}: {e}")
        sys.exit(EXIT_BADMANIFEST)
    _reject_reserved(man, mpath)
    man = validate_manifest(migrate(man))
    _LOADED_TRUST[id(man)] = {
        "trusted": (verify_sig(man)[0] == "valid" or
                    verify_pksig(man)[0] == "valid")
    }
    return man


def save(mpath, man, do_sign=True, allow_untrusted_sign=False):
    provenance = _LOADED_TRUST.pop(id(man), None)
    man.pop("_presig_body", None)
    man.pop("_migrated_from", None)
    validate_manifest(man)
    if do_sign and provenance and not provenance["trusted"] and not allow_untrusted_sign:
        eprint("note: refusing to sign an untrusted or unsigned loaded manifest; "
               "writing it unsigned. Review it, then approve its exact digest "
               "with `swr sign --approve-digest DIGEST`.")
        man.pop("_sig", None)
        man.pop("_pksig", None)
        do_sign = False
    if do_sign:
...
def tree_fingerprint(root, ignore_dirs=()):
    """Legacy fast fingerprint over (relpath, size, mode), not file content."""
    parts, n = [], 0
...
def content_tree_hash(root, ignore_dirs=()):
    """Full-content tree digest; opt-in because it reads every file."""
    leaves, n = [], 0
    for dp, dns, fns in os.walk(root):
        dns[:] = sorted(d for d in dns if d not in ignore_dirs)
        for fn in sorted(fns):
            ...
            try:
                ...
                if stat.S_ISREG(st.st_mode):
                    ...
                    digest = hashlib.sha256(os.readlink(fp).encode()).hexdigest()
            except OSError:
                continue
            leaves.append(f"{rel}\0{st.st_size}\0{stat.S_IMODE(st.st_mode):o}\0{digest}")
            n += 1
    return hashlib.sha256("\n".join(leaves).encode()).hexdigest(), n


# Backward-compatible name for callers that imported the pre-1.5 helper.
merkle_tree = tree_fingerprint


def tree_cache_key(root, ignore_dirs=()):
    """Per-file (size, mtime_ns) fingerprint so tree results can be cached.
...
def save_cache(c):
    ...
    try:
        atomic_json(CACHE_FILE, c, 0o600)
    except OSError as e:
...
def smoke_result(man, e, allow_smoke):
    """Return a status tuple for an optional smoke command, or None."""
    if not e.get("smoke"):
        return None
    if not allow_smoke:
        return OK, "verified (smoke skipped; use --run-smoke on a trusted manifest)"
...
def verify(man, e, cache=None, use_cache=True, allow_smoke=False):
    ...
    if kind == "tree":
        ...
        tree_key = e.get("tree_sha256") or e.get("merkle")
        if tree_key:
            ...
            if not isinstance(tc, dict):
                tc = None
            tc_age = time.time() - tc.get("t", 0) if isinstance(tc, dict) and isinstance(tc.get("t", 0), (int, float)) else CACHE_TTL + 1
            if (tk and tc and tc.get("key") == tk and tc.get("root") == tree_key
                    and 0 <= tc_age < CACHE_TTL):
                label = "content tree hash" if e.get("tree_sha256") else "legacy tree fingerprint"
                result = smoke_result(man, e, allow_smoke)
                if result:
                    return result
                return OK, f"{tc.get('n', '?')} files, {label} ok (cached)"
            if e.get("tree_sha256"):
                root, n = content_tree_hash(path, ign)
                label = "content tree hash"
            else:
                root, n = tree_fingerprint(path, ign)
                label = "legacy tree fingerprint"
            if n == 0:
                return EMPTY, "directory present but contains 0 files"
            if root != tree_key:
                return CORRUPT, f"{label} drift ({n} files, root {root[:10]}…)"
    ...
    if not isinstance(cached, dict):
        cached = None
    cache_age = (time.time() - cached.get("t", 0)
                 if isinstance(cached, dict) and isinstance(cached.get("t", 0), (int, float))
                 else CACHE_TTL + 1)
    fresh = bool(ck and cached and cached.get("key") == ck and cached.get("ok")
                 and 0 <= cache_age < CACHE_TTL)
...
def run_check(man, only=None, jobs=8, use_cache=True, allow_smoke=False):
    ...
    if jobs > 1 and len(ents) > 2:
        with futures.ThreadPoolExecutor(max_workers=jobs) as ex:
            fut = {ex.submit(verify, man, e, cache, use_cache, allow_smoke): e
                   for e in ents}
            res = {}
        ...
        rows = [(e, *verify(man, e, cache, use_cache, allow_smoke)) for e in ents]
    if use_cache:
        save_cache(cache)
    return rows
...
def record(man, rows):
    """Best-effort, explicitly local history; never leaves the host."""
    try:
        secure_dir(SWR_DIR)
        ...
        eprint(f"note: local history not written ({e.strerror})")
...
def _rotate_history():
    ...
    try:
        ...
        if len(lines) <= HISTORY_MAX_LINES:
            return
        atomic_json_lines = lines[-HISTORY_TRIM_TO:]
        fd, tmp = tempfile.mkstemp(prefix=".history.", dir=SWR_DIR, text=True)
        try:
            os.fchmod(fd, 0o600)
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                f.writelines(atomic_json_lines)
                f.flush()
                os.fsync(f.fileno())
...
def history():
    ...
    try:
        ...
        eprint(f"note: local history not readable ({e.strerror})")
    return out
...
def survival(hist):
    tot, ok = {}, {}
    for h in hist:
        if not isinstance(h, dict) or not isinstance(h.get("s"), dict):
            continue
...
def _has_symlink_component(man, path):
    """Reject symlinked escrow sources, including symlinked parents."""
    ws = os.path.abspath(os.path.expanduser(man.get("workspace", HOME)))
...
def escrow_blob(path, max_bytes=ESCROW_MAX):
    """Read one regular, non-followed file with a hard pre-compression cap."""
    import zlib
    limit = min(int(max_bytes), ESCROW_DECOMP_MAX)
    if limit < 0:
        raise ValueError("negative escrow limit")
    flags = os.O_RDONLY
    ...
    try:
        ...
        if st.st_size > limit:
            raise ValueError(f"source is {st.st_size:,}B, over the {limit:,}B limit")
        chunks, total = [], 0
        while True:
            ...
            total += len(chunk)
            if total > limit:
                raise ValueError(f"source grew beyond the {limit:,}B limit")
        data = b"".join(chunks)
        return base64.b64encode(zlib.compress(data, 9)).decode(), total
    finally:
        os.close(fd)
...
def quarantine(man, e):
    """Preserve a damaged path without removing it before its recipe runs."""
    if not contained(man, e, "quarantine"):
        return None
    p = abspath(man, e["path"])
    if not os.path.lexists(p):
        return "(nothing present to quarantine)"
    secure_dir(QUARANTINE)
    stamp = f"{int(time.time())}.{os.getpid()}.{secrets.token_hex(4)}"
    dst = os.path.join(QUARANTINE, f"{e['id']}.{stamp}")
    try:
        if os.path.islink(p):
...
def restore_entry(man, e, dry=False, timeout=None, quar=False):
    ...
    if cmd and cmd.startswith("swr-escrow:"):
        ...
        try:
            ...
            fd, tmp = tempfile.mkstemp(prefix="." + os.path.basename(p) + ".swr-",
                                       dir=parent)
            os.fchmod(fd, 0o600)
            with os.fdopen(fd, "wb") as f:
                f.write(data)
    ...
    if quar:
        q = quarantine(man, e)
        if not q:
            print("     quarantine failed; refusing to run the recipe")
            return False
        if q != "(nothing present to quarantine)":
            print(f"     preserved old copy -> {q}")
...
def plan(man, rows):
    """Topological order over `needs`, refusing dependency cycles."""
    bad = {e["id"]: e for e, s, _ in rows if s in BAD}
    ordered, seen, cycles = [], set(), set()

    def visit(eid, stack):
        if eid in seen:
            return
        if eid in stack:
            cycles.update(stack[stack.index(eid):])
            return
        stack.append(eid)
        e = bad.get(eid)
        if e:
            for dep in e.get("needs", []):
                if dep in bad:
                    visit(dep, stack)
            if eid not in seen and eid not in cycles:
                ordered.append(e)
                seen.add(eid)
        stack.pop()
...
def progress_path(man):
    ...
    key = hashlib.sha256(
        (man.get("workspace", HOME) + "|" + manifest_digest(man)).encode()
    ).hexdigest()[:16]
    return os.path.join(PROGRESS_DIR, f"{key}.json")


def do_restore(man, rows, a):
    ordered, cycles = plan(man, rows)
    if cycles:
        names = ", ".join(sorted(cycles))
        eprint(f"refusing restore: dependency cycle involving {names}")
        return False, sorted(cycles)
    if not ordered:
        return True, []
    prog = {}
    ...
    if getattr(a, "resume", False) and os.path.exists(pfile):
        try:
            ...
            elif (cand.get("workspace") != man.get("workspace") or
                  cand.get("digest") != manifest_digest(man)):
                print(f"  {C['d']}(resume state is for another manifest){C['r']}")
            else:
                prog = cand
        except Exception:
            prog = {}
    ...
    for i, e in enumerate(ordered, 1):
        ...
        if not a.dry_run:
            try:
                ...
                atomic_json(pfile, {"done": sorted(done), "t": int(time.time()),
                                     "workspace": man.get("workspace"),
                                     "digest": manifest_digest(man)}, 0o600)
...
def guard_signature(man, a=None):
    ...
    eprint("  If you authored this manifest locally, review its exact digest, then run:")
    eprint(f"    swr sign --approve-digest {manifest_digest(man)}")
...
def cmd_check(man, mpath, a):
    if getattr(a, "run_smoke", False) and not guard_signature(man, a):
        return EXIT_UNREPAIRABLE
    rows = run_check(man, a.only, a.jobs, not a.no_cache,
                     allow_smoke=getattr(a, "run_smoke", False))
    if getattr(a, "record", False) and not a.no_record:
        record(man, rows)
    if a.json:
...
def cmd_doctor(man, mpath, a):
    ...
    if a.dry_run:
        return EXIT_DAMAGED
    print("\n== re-check ==")
    rows2 = run_check(man, a.only, a.jobs, use_cache=False, allow_smoke=True)
...
def cmd_audit(man, mpath, a):
    ...
    if not os.path.isdir(ws):
        eprint(f"workspace is absent: {ws}")
        return EXIT_DAMAGED
    ...
    for name in sorted(os.listdir(ws)):
        ...
        try:
            if os.path.islink(p):
                sz, kind = os.lstat(p).st_size, "symlink"
            elif os.path.isdir(p):
                sz, kind = (sum(os.path.getsize(os.path.join(dp, f))
                                 for dp, _, fs in os.walk(p) for f in fs), "dir")
            else:
                sz, kind = os.path.getsize(p), "file"
        except OSError:
            sz, kind = 0, "unreadable"
        found.append((sz, name, kind))
...
def cmd_canary(man, mpath, a):
    ...
    if a.action == "plant":
        ...
        for name, p in spots.items():
            ...
            if os.path.islink(parent) or os.path.islink(p):
                eprint(f"refusing canary path through symlink: {p}")
...
def cmd_retighten(man, mpath, a):
    ...
    changed = []
    for e in man["entries"]:
        if e.get("kind") != "tree" or e.get("merkle") or e.get("tree_sha256"):
            continue
        p = abspath(man, e["path"])
...
def cmd_trust(man, mpath, a):
    ...
    if a.action == "add":
        ...
        atomic_json(TRUST_FILE, trust, 0o600)
...
def cmd_escrow(man, mpath, a):
    ...
    for e in man["entries"]:
        ...
        if e.get("kind") != "file":
            continue
        p = abspath(man, e["path"])
        if not contained(man, e, "read escrow from"):
            continue
        if _has_symlink_component(man, p):
            eprint(f"  refusing to escrow symlinked path: {e['path']!r}")
            continue
        try:
            encoded, sz = escrow_blob(p, min(a.max_bytes, ESCROW_DECOMP_MAX))
        except (OSError, ValueError) as ex:
            eprint(f"  refusing to escrow {e['path']!r}: {ex}")
            continue
...
def cmd_sign(man, mpath, a):
    provenance = _LOADED_TRUST.get(id(man), {})
    allow = False
    if provenance and not provenance.get("trusted"):
        digest = manifest_digest(man)
        if getattr(a, "approve_digest", None) != digest:
            eprint("refusing to sign an untrusted or unsigned manifest without "
                   "an exact-digest approval")
            eprint(f"review it, then re-run:  swr sign --approve-digest {digest}")
            return EXIT_UNREPAIRABLE
        allow = True
    save(mpath, man, allow_untrusted_sign=allow)
...
def cmd_selftest(man, mpath, a):
    """Exercise restore recipes twice as an idempotence check; guarded."""
    ...
    if not targets:
        ...
        print("selftest did not exercise a restore recipe; use --allow-empty only for a new manifest")
        return EXIT_OK if getattr(a, "allow_empty", False) else EXIT_UNREPAIRABLE
    bad = 0
    for e in targets:
        ...
        if e["restore"].startswith("swr-escrow:"):
            ...
            continue
        ok2 = _run()
        st2, _ = verify(man, e, None, False, allow_smoke=True)
        ok3 = _run()
        st3, _ = verify(man, e, None, False, allow_smoke=True)
...
# the user's own channel (env var, secret manager, or bounded `swr escrow`).
# Escrow is compressed/Base64 inline storage, not encryption; encrypt the
# manifest during paste sync. This is the same rule the redactor enforces.
AGENT_PRESETS = {
    "arena": [
...
def _preset_entries(man, preset):
    ...
    bounds — the user is expected to run `retighten`/`--content-hash` on valuable
    ...
    for ident, rel, kind, tier, sentinels, recipe in AGENT_PRESETS.get(preset, []):
        ...
        ap = os.path.join(ws, rel) if not os.path.isabs(rel) else os.path.abspath(rel)
        if not os.path.exists(ap):
            continue
        # Recipes run with the manifest workspace as cwd and use only relative
        # paths.  Still reject symlinked entries whose real target escapes the
        # workspace (a symlink must not smuggle an external path past a lexical
        # check).
...
def cmd_preset(man, mpath, a):
    ...
    if any(e["kind"] == "tree" for e in ents):
        print("next: `swr retighten` to tighten preset trees, `swr add --content-hash` for valuable ones")
    return EXIT_OK
...
def cmd_models(man, mpath, a):
    ...
    if not found:
        print(f"no untracked model files > {min_mb} MB"
...
def cmd_export(man, mpath, a):
    """Export the restore runbook in portable shell, Markdown, or JSON forms.
    ...
    script, Markdown, or JSON that a compatible shell, agent, or human can read
    directly. Escrow recipes (`swr-escrow:`) are exported as comments: they
    need the encrypted manifest itself, which paste-sync carries. Shell output
    is opt-in and requires a trusted manifest unless the operator explicitly
    overrides both safeguards after review.
    """
    if man is None:
        eprint("no manifest — run `swr init` first")
        return EXIT_BADMANIFEST
    sig_ok = verify_sig(man)[0] == "valid" or verify_pksig(man)[0] == "valid"
    ...
    fmt = a.format
    if fmt == "sh":
        if not a.allow_shell_export:
            eprint("refusing shell runbook export without --allow-shell-export")
            return EXIT_UNREPAIRABLE
        if not sig_ok and not a.allow_untrusted_shell_export:
            eprint("refusing shell export for an untrusted manifest; use "
                   "--allow-untrusted-shell-export only after review")
            return EXIT_UNREPAIRABLE
    if fmt == "json":
...
def capture(man, path, kind, ident=None, restore=None, tier=None,
    ...
    if kind == "tree":
        ...
        if os.path.isdir(ap):
            if merkle:
                root, n = content_tree_hash(ap)
                e["tree_sha256"], e["files_at_capture"] = root, n
            else:
        ...
        # Blob metadata is derived only from the local file.  Network probing
        # during manifest authoring would add SSRF and availability risk.
...
def cmd_init(mpath, a):
    ...
    if not os.path.isdir(ws):
        eprint(f"workspace does not exist or is not a directory: {ws}")
        return EXIT_BADMANIFEST
    ...
    print("next: `swr add` your trees with --sentinel/--content-hash, then `swr escrow`")
    return EXIT_OK
...
def cmd_autopilot(man, mpath, a):
    ...
    ws = os.path.abspath(os.path.expanduser(a.workspace))
    if man is not None:
        ws = os.path.abspath(os.path.expanduser(man.get("workspace", ws)))
    print("== autopilot ==")
    if not os.path.isdir(ws):
        eprint(f"workspace does not exist or is not a directory: {ws}")
        return EXIT_BADMANIFEST
    if man is None:
        rc = cmd_init(mpath, argparse.Namespace(workspace=ws))
        if rc != EXIT_OK:
            return rc
        man = load(mpath)
    n0 = len(man["entries"])
...
def main():
    ...
    p.add_argument("--record", action="store_true",
                   help="explicitly append local-only observations for `stats`")
    p.add_argument("--no-record", action="store_true",
                   help="compatibility flag; local observations are off by default")
    ...
    q.add_argument("--format", choices=["sh", "md", "json"], default="md")
    q.add_argument("--only", nargs="*")
    q.add_argument("--allow-shell-export", action="store_true",
                   help="explicitly permit executable shell runbook output")
    q.add_argument("--allow-untrusted-shell-export", action="store_true",
                   help="also permit shell output for an untrusted manifest after review")
    ...
    q.add_argument("--content-hash", "--merkle", dest="merkle", action="store_true",
                   help="full-content tree hash (reads every file; --merkle is a legacy alias)")

    for n in ("check", "restore", "doctor", "why", "stats", "sign", "verify"):
        q = s.add_parser(n)
        q.add_argument("--only", nargs="*")
        if n == "check":
            q.add_argument("--run-smoke", action="store_true",
                           help="execute recorded smoke commands (requires a trusted manifest)")
        ...
        if n == "sign":
            q.add_argument("--approve-digest", metavar="DIGEST",
                           help="approve signing this exact reviewed manifest")
    ...
    q.add_argument("--allow-empty", action="store_true",
                   help="return success without exercising a recipe (not a coverage result)")
    q.add_argument("--i-trust-this-manifest", nargs="?", const=True,
    ...
    if a.cmd == "trust":
        with Lock(not a.no_lock):
            return cmd_trust(load(a.manifest, require=False), a.manifest, a)
    if a.cmd == "attacktest":
        with Lock(not a.no_lock):
            return cmd_attacktest(load(a.manifest, require=False) or {}, a.manifest, a)
    ...
    needs_lock = a.cmd in ("check", "restore", "doctor", "selftest", "retighten",
                           "escrow", "sign", "add", "models", "preset", "export-recipes")
