#!/usr/bin/env python3
"""
swr_paste — off-box manifest sync for snapshot-wipe-resilience (v1.5.6).
...
SECURITY MODEL
  ...
  * Redaction (secrets -> ${ENV_VAR}) protects unencrypted pushes. Inline
    escrow is never allowed in an unencrypted push: it is compressed/base64,
    not confidential, and opaque encoding must not bypass redaction.
...
UA = "swr-snapshot-wipe-resilience/1.5.6"
...
# ───────────────────────────────────────────────────────────────── transport
# Pulling arbitrary URLs would turn an agent recovery helper into an SSRF
# primitive.  Restrict transport to the hosts this file actually implements,
# use HTTPS, reject non-default ports, and do not follow redirects.
ALLOWED_PASTE_HOSTS = frozenset({"pastebin.com", "dpaste.com", "paste.rs"})
...
def _validate_url(url):
    try:
        ...
        raise RuntimeError("invalid URL") from None
    if parsed.scheme != "https" or not host:
        raise RuntimeError("only HTTPS paste URLs are allowed")
    if host not in ALLOWED_PASTE_HOSTS:
        raise RuntimeError("URL host is not an approved paste backend")
    if port not in (None, 443):
        raise RuntimeError("non-default HTTPS ports are refused")
    if parsed.username or parsed.password:
        raise RuntimeError("URLs containing embedded credentials are refused")
    return url
...
def _open(req, retries=3):
    ...
    for i in range(retries + 1):
        try:
            with _SAFE_OPENER.open(req, timeout=45) as r:
                # Redirects are disabled above; retain this check as a defense
                # in depth if the opener implementation changes.
                _validate_url(r.geturl())
                length = r.headers.get("Content-Length")
                if length and length.isdigit() and int(length) > PAYLOAD_MAX:
                    raise RuntimeError(f"response exceeds {PAYLOAD_MAX:,}B limit")
                data = r.read(PAYLOAD_MAX + 1)
...
def qr(data):
    if shutil.which("qrencode"):
        try:
            r = subprocess.run(["qrencode", "-t", "UTF8", data],
                               capture_output=True, text=True,
                               timeout=15)
            return _terminal_safe(r.stdout)
        except Exception:
            pass
    return None
...
def wrap(text, compress, encrypt_to, pq_to=None, no_sign=False):
    ...
    if pq_to:
        ...
        env = _pq._encrypt_envelope(body.encode(), peer, sign=not no_sign)
...
def _bounded_decompress(b64):
    """Decompress with input, output, and stream-integrity caps."""
    if not isinstance(b64, str) or len(b64) > (PAYLOAD_MAX * 4 // 3 + 4096):
        raise RuntimeError("compressed payload is too large")
    try:
        raw = base64.b64decode(b64, validate=True)
    except Exception:
        raise RuntimeError("compressed payload is not valid base64") from None
    d = zlib.decompressobj()
    try:
        out = d.decompress(raw, PAYLOAD_MAX + 1)
    except zlib.error:
        raise RuntimeError("compressed payload is not a valid zlib stream") from None
    if len(out) > PAYLOAD_MAX or d.unconsumed_tail or not d.eof or d.unused_data:
        raise RuntimeError(f"payload expands beyond {PAYLOAD_MAX:,}B or has a "
                           f"truncated/trailing stream — refusing")
    try:
        return out.decode()
    except UnicodeDecodeError:
        raise RuntimeError("decompressed payload is not UTF-8") from None


def _safe_age_identity(identity):
    """Allow age keys only from the private SWR state tree."""
    if not isinstance(identity, str) or not identity:
        raise RuntimeError("age identity path is empty")
    path = os.path.abspath(os.path.expanduser(identity))
    root = os.path.realpath(SWR_DIR)
    if os.path.commonpath([root, os.path.realpath(path)]) != root:
        raise RuntimeError("age identity must be inside the private ~/.swr tree")
    if os.path.islink(path) or not os.path.isfile(path):
        raise RuntimeError("age identity must be a regular non-symlink file")
    try:
        mode = os.stat(path).st_mode
    except OSError:
        raise RuntimeError("cannot access age identity") from None
    if mode & 0o077:
        raise RuntimeError("age identity permissions must be private (mode 600)")
    return path
...
def unwrap(raw, identity=None, expect_from=None):
    ...
    if o.get("enc") == "swr-hybrid-v1":
        if _pq is None:
            raise RuntimeError("payload is hybrid-PQ encrypted but swr_crypto.py "
                               "is not importable")
        pt, hdr, sender = _pq._decrypt_envelope(json.loads(o["body"]), expect_from)
        inner = pt.decode()
        if o.get("z"):
    ...
    if o.get("enc") == "age":
        ...
        idpath = _safe_age_identity(identity) if identity else None
        cmd = ["age", "-d"] + (["-i", idpath] if idpath else [])
        r = subprocess.run(cmd, input=o["body"].encode(),
                           capture_output=True, timeout=60)
        if r.returncode:
            raise RuntimeError("age decrypt failed")
...
def cmd_push(a):
    ...
    try:
        if os.path.getsize(a.manifest) > PAYLOAD_MAX:
            sys.exit(f"manifest exceeds {PAYLOAD_MAX:,}B")
    ...
    if not isinstance(man, dict):
        sys.exit("manifest must be a JSON object")
    ...
    if (not a.pq_to and not a.encrypt_to and not a.plaintext_ok
        ...
        if has_id:
            try:
                ident = _pq.load_identity()
            except SystemExit:
                ident = None
            if ident:
                self_peer = os.path.join(_pq.PEERS_DIR, "self.json")
                os.makedirs(_pq.PEERS_DIR, mode=0o700, exist_ok=True)
                os.chmod(_pq.PEERS_DIR, 0o700)
                if not os.path.exists(self_peer):
                    with open(self_peer, "w") as f:
                        json.dump(ident, f, indent=2)
                    os.chmod(self_peer, 0o600)
                a.pq_to = "self"
            eprint("note: encrypting to your own identity by default (hybrid PQ). "
                   "Use --plaintext-ok to publish unencrypted, or --pq-to <peer>.")

    has_escrow = any(
        isinstance(entry, dict) and isinstance(entry.get("escrow"), str)
        and bool(entry.get("escrow"))
        for entry in (man.get("entries") or [])
    )
    if has_escrow and not (a.pq_to or a.encrypt_to):
        sys.exit("manifest contains inline escrow; encrypted transport is required "
                 "(--pq-to or --encrypt-to). --plaintext-ok cannot override this")

    if not a.pq_to and not a.encrypt_to and not a.plaintext_ok:
        sys.exit("no encryption recipient is available; use --pq-to/--encrypt-to "
                 "or explicitly acknowledge cleartext with --plaintext-ok")
    ...
    # Encryption protects transport content, but it is not permission to ship
    # a secret-bearing manifest silently. Scan first; the default is to refuse
    # recognized secret-shaped values unless the operator explicitly accepts
    # that risk. Redacting a signed body would invalidate its signature, so the
    # safe default is review/decline rather than an invisible rewrite.
    encrypted = bool(a.pq_to or a.encrypt_to)
    if encrypted and not a.force_redact:
        _, encrypted_found, encrypted_susp = redact(raw, a.paranoid)
        if (encrypted_found or (a.strict and encrypted_susp)) \
                and not a.allow_encrypted_secrets:
            sys.exit("encrypted manifest contains secret-shaped values; remove "
                     "them or re-run with --allow-encrypted-secrets after review "
                     "(use --force-redact only when you accept signature loss)")
        if encrypted_found or encrypted_susp:
            eprint("warning: encrypted upload includes secret-shaped manifest "
                   "content by explicit operator approval")
        a.no_redact = True
    ...
    _atomic_text(PIN_FILE, json.dumps(pin, indent=2) + "\n")
...
def cmd_pull(a):
    try:
        ...
        sys.exit(f"invalid paste URL: {e}")
    try:
        raw = fetch(a.url)
    except RuntimeError as e:
        msg = str(e)
    ...
    if not isinstance(man, dict) or not isinstance(man.get("entries", []), list):
        sys.exit("fetched JSON is not an SWR manifest object")

    dest = a.out or a.manifest
    if os.path.exists(dest) and not a.force:
        sys.exit(f"{dest} exists — use --force to overwrite")
    _atomic_text(dest, json.dumps(man, indent=2) + "\n")
...
def cmd_status(a):
    ...
    try:
        ...
        eprint(f"remote pin is not valid JSON: {e}")
        return 1
    if not isinstance(pin, dict):
        eprint("remote pin is not an object")
        return 1
    ...
    ok = 0
    replicas = pin.get("replicas", [])
    if not isinstance(replicas, list):
        eprint("remote pin replicas are malformed")
        return 1
    for r in replicas:
        if not isinstance(r, dict) or not isinstance(r.get("raw"), str):
            print("  malformed replica record")
            continue
        try:
            ...
            print(f"  {r.get('backend', '?'):<9} {'hash OK' if good else 'HASH DRIFT'}  {r['raw']}")
            ok += good
        except Exception as e:
            print(f"  {r.get('backend', '?'):<9} unreachable: {e}")
...
def main():
    ...
    p.add_argument("--identity", help="private age identity under ~/.swr for decryption")
    s = p.add_subparsers(dest="cmd", required=True)

    q = s.add_parser("push")
    q.add_argument("--backend", choices=list(BACKENDS))
    q.add_argument("--mirror", action="store_true", help="push to ALL backends")
    q.add_argument("--expire", choices=list(EXPIRY_SECONDS), default="1W",
                   help="10M 1H 1D 1W 2W 1M N")
    ...
    q.add_argument("--allow-encrypted-secrets", action="store_true",
                   help="explicitly permit recognized secret-shaped values in an encrypted upload")
    q.add_argument("--plaintext-ok", action="store_true",
                   help="publish WITHOUT encryption (redaction only; escrow still requires encryption)")
