...
6. **字段语义** — 趋势由页面箭头颜色判定：红色=上升（up）、绿色=下降（down）；`is_new` 表示新上榜；`vendor` 为模型厂商（JSON 中为页面原始 slug，文本表格中映射为中文展示名：中国厂商中文名、海外厂商英文品牌）
...
> `search` 命令返回结构类似：`{source, data_source, keyword, matches[], status}`，
