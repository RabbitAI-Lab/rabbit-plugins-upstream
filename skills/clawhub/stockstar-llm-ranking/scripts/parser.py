...
页脚数据来源：<div class="data_source"><p>数据来源：openrouter</p></div>

核心业务流程：
    parse_ranking_html → 提取日榜 + 周榜两个列表 + 数据来源
"""
...
def parse_ranking_html(html):
    ...
    """
    result = {"data_source": _extract_data_source(html)}
    for pid in LIST_IDS:
...
def _extract_data_source(html):
    ...
    m = re.search(r'<div class="data_source">\s*<p>(.*?)</p>', html, re.DOTALL)
