...
# API Configuration
TEMPLATE_ID = '113862426446792103'
API_BASE_URL = "https://api.browseract.com/v3/bots"
API_KEY_URL = 'https://www.browseract.com/reception/integrations?co-from=amazon-best-sellers-scraper'


def run_amazon_best_sellers_scraper_task(api_key, marketplace_category='United States - Electronics', count='20'):
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python amazon_best_sellers_scraper_api.py <marketplace_category> [count]", flush=True)
        sys.exit(1)
    ...
    marketplace_category = sys.argv[1] if len(sys.argv) > 1 else 'United States - Electronics'
    count = sys.argv[2] if len(sys.argv) > 2 else '20'

    result = run_amazon_best_sellers_scraper_task(api_key, marketplace_category, count)
    if result:
