...
# === Skill 固定配置（由构建器生成） ===
SKILL_CONFIG = json.loads('{"example": "电影感城市夜景，镜头缓慢推进，灯光倒映在湿润路面", "keywords": "Happy Horse、HappyHorse、Happyhorse、快乐马、通义万相视频、文生视频、图生视频、参考生视频、AI视频、视频生成、广告、TVC、带货、种草、短剧、漫剧", "models": {"edit": "public_model_happyhorse_video_edit", "i2v": "public_model_happyhorse_i2v", "r2v": "public_model_happyhorse_r2v", "t2v": "public_model_happyhorse_t2v"}, "name": "happy-horse", "rule": "optional", "search": "覆盖 Happy Horse、HappyHorse、Happyhorse、快乐马、通义万相视频 的裸接口与全部公开模式", "title": "Happy Horse 视频生成"}')

def _normalize_model_text(value):
    return "".join(ch.lower() for ch in str(value) if ch.isalnum())

def _resolve_video_model(client, mode, requested_model_id=None):
    model_ref = SKILL_CONFIG["models"][mode]
    if isinstance(model_ref, str):
        if requested_model_id and requested_model_id != model_ref:
            raise SystemExit(
                f"当前 Skill 的 {mode} 模式固定为 {model_ref}，不接受其他 --model-id"
            )
        return model_ref

    if model_ref.get("resolve") != "runtime_exact":
        raise SystemExit(f"无法识别的模型选择配置：{model_ref}")

    models = client.list_models("VIDEO")
    family_markers = [_normalize_model_text(x) for x in model_ref["model_markers"]]
    mode_markers = [_normalize_model_text(x) for x in model_ref["mode_markers"]]

    def matches(entry):
        searchable = " ".join(str(entry.get(key, "")) for key in (
            "publicModelId", "displayName", "name", "description"
        ))
        normalized = _normalize_model_text(searchable)
        return (
            any(marker in normalized for marker in family_markers)
            and any(marker in normalized for marker in mode_markers)
        )

    candidates = [entry for entry in models if matches(entry)]
    if requested_model_id:
        exact = [entry for entry in candidates if entry.get("publicModelId") == requested_model_id]
        if len(exact) == 1:
            return requested_model_id
        raise SystemExit(
            f"--model-id {requested_model_id} 不属于 {model_ref['label']} 的 {mode} 实时候选"
        )

    if len(candidates) == 1:
        selected = candidates[0].get("publicModelId")
        if selected:
            print(f"实时发现模型：{model_ref['label']} {mode} → {selected}")
            return selected

    if not candidates:
        raise SystemExit(
            f"AI Hive 实时模型列表中未找到 {model_ref['label']} 的 {mode} 模式。"
            "本 Skill 不会自动回退到其他版本；请确认后台已上架该模式。"
        )

    choices = ", ".join(
        str(entry.get("publicModelId", "?")) for entry in candidates[:10]
    )
    raise SystemExit(
        f"找到多个 {model_ref['label']} {mode} 候选：{choices}。"
        "请用 --model-id 指定其中一个。"
    )
...
def _validate_video_inputs(args, rule):
    ...
    if rule == "none" and any((has_image, has_video, has_audio, has_first, has_last)):
        raise SystemExit("此文生视频 Skill 不接受媒体输入")
    if rule == "first" and not has_first:
        raise SystemExit("图生视频必须提供 --first-frame")
    if rule == "first_last" and not (has_first and has_last):
        raise SystemExit("首尾帧生视频必须同时提供 --first-frame 和 --last-frame")
    if rule == "reference" and not any((has_image, has_video, has_audio)):
        raise SystemExit("参考生视频至少需要 --image/--video/--audio 中的一种素材")
    if rule == "image" and not has_image:
        raise SystemExit("参考图生视频必须提供 --image")
    if rule in ("video", "edit", "extend") and not has_video:
        raise SystemExit("此 Skill 必须提供 --video")
    if rule == "audio" and not has_audio:
        raise SystemExit("参考音频生视频必须提供 --audio")
...
def skill_generate(client, args):
    ...
    model_id = _resolve_video_model(client, mode, args.model_id)
...
def build_skill_parser():
    ...
    p.add_argument("--model-id", help="仅实时发现出现多个同版本候选时使用")
