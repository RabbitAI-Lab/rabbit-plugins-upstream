...
DIM_KEYS = ["obs_timeliness", "coverage_completeness", "tool_usability",
...
def run_tool(args, timeout=30):
    try:
        r = subprocess.run([PY, os.path.join(TOOLS, "observability_toolkit.py")] + args,
...
def check_coverage():
    ...
    for kw in ["01-可观测性全景", "02-调用追踪与日志", "03-质量监控指标", "04-性能与成本监控",
               "05-护栏与安全监控", "06-告警体系", "07-平台与落地", "08-FAQ"]:
...
def check_tool():
    ...
    for cmd in ["pillars", "metrics", "alert", "trace", "plan"]:
        items.append({"name": f"命令 {cmd} 在帮助中", "pass": cmd in out})
    rc, out = run_tool(["pillars"])
    items.append({"name": "pillars 含三大支柱", "pass": rc == 0 and "三大支柱" in out})
    rc, out = run_tool(["metrics"])
    items.append({"name": "metrics 含幻觉率", "pass": rc == 0 and "幻觉率" in out})
    rc, out = run_tool(["alert"])
    items.append({"name": "alert 含分级", "pass": rc == 0 and "分级" in out})
    rc, out = run_tool(["trace"])
    items.append({"name": "trace 含敏感过滤", "pass": rc == 0 and "敏感" in out})
    rc, out = run_tool(["plan"])
    items.append({"name": "plan 含灰度监控", "pass": rc == 0 and "90" in out})
    return items
...
def check_judgment():
    ...
    for cmd, expect in cases:
        rc, out = run_tool([cmd])
        items.append({"name": f"命令[{cmd}]含[{expect}]", "pass": rc == 0 and expect in out})
    return items
...
def check_safety():
    ...
    items.append({"name": "工具脚本零网络依赖",
                  "pass": "import requests" not in open(os.path.join(TOOLS, "observability_toolkit.py"), encoding="utf-8").read()})
    return items
...
def main():
    ...
    result = {
        ...
        "note": "AI 可观测性监控 · 质量与安全稳定性实测（本地闭环）",
