...
- **如果 5 个必需命名空间全部可解析** → 配置正常，直接进入环节1（patent-panorama-insights-search）。不要打断用户。
...
```text
显式启动 -> 默认设置 + 需求采集
  -> Stage 1 patent-panorama-insights-search   (规则检索 + 降噪 + 候选池)
  -> [Checkpoint 1: confirm queries]
  -> Stage 2 patent-panorama-insights-stats    (全景统计 + 价值点交叉挖掘)
  -> [Checkpoint 2: confirm scope before tagging-system design]
  -> Stage 3 patent-panorama-insights-tag      (推荐标引体系 + demo + 待标引导出)
  ...
  -> Stage 4 patent-panorama-insights-report   (演进路线 + 护城河解读 + 单文件 HTML 报告)
...
| 环节1 | `patent-panorama-insights-search` | 环节 1 | 检索建库 | 专家检索式 + 降噪 + 候选池 | ✅ confirm queries |
| 环节2 | `patent-panorama-insights-stats` | 环节 2 | 格局统计 | 趋势 / 申请人 / 竞品 / 价值点挖掘 | review results |
| 环节3 | `patent-panorama-insights-tag` | 环节 3 | 标引体系设计 | 推荐分解表 + 专利包 + demo | ✅ take to SaaS |
| (manual) | — | 环节 3.5 | 客户标引（线下） | 在客户 SaaS 中逐条全量标引 | ✅✅ client-led |
| 环节4 | `patent-panorama-insights-report` | 环节 4 | 报告生成 | 演进路线 + 护城河 + HTML 报告 | confirm report |

进度汇报规则：

- 每条环节级状态消息都以 `【环节 X/4 · 名称】` 开头。环节编号为 1–4；人工标引步骤为 `环节 3.5`。
- 进入任一层时，agent 用一行话自报位置，例如：“我现在在【环节 2/4 · 格局统计】（patent-panorama-insights-stats）。”
...
Stage 1 (环节 1) /patent-panorama-insights-search  -> search_config.json + candidate_pool.csv + core_recall.csv
  └─ Checkpoint 1: confirm per-branch queries + precision
Stage 2 (环节 2) /patent-panorama-insights-stats   -> panorama_stats.json + patent_index.core.* + value_signals.json
                                 + chart_data.json + panorama_stats_report.html
  └─ Checkpoint 2: confirm scope before tagging-system design
Stage 3 (环节 3) /patent-panorama-insights-tag     -> tech_breakdown.json + key_questions.json + patent_packages.csv
                                 + tagging_demo_sample.csv + to_be_tagged.csv
  └─ Checkpoint 3: confirm tagging system
Handoff 3.5 (环节 3.5)        -> client tags to_be_tagged.csv in SaaS, returns tagged_pool.csv
Stage 4 (环节 4) /patent-panorama-insights-report  -> report_manifest.json + report.html
  └─ Checkpoint 4: confirm report
```
...
|---|---|---|---|
| 环节 1 | `/patent-panorama-insights-search` | user intake | `search_config.json`, `candidate_pool.csv`, `core_recall.csv` |
| 环节 2 | `/patent-panorama-insights-stats` | `search_config.json`, `candidate_pool.csv`, `core_recall.csv` | `panorama_stats.json`, `patent_index.core.*`, `value_signals.json`, `chart_data.json`, `panorama_stats_report.html` |
| 环节 3 | `/patent-panorama-insights-tag` | `candidate_pool.csv`, `panorama_stats.json`, `value_signals.json` | `tech_breakdown.json`, `key_questions.json`, `patent_packages.csv`, `tagging_demo_sample.csv`, `to_be_tagged.csv` |
| 环节 3.5 | — (client SaaS) | `to_be_tagged.csv` | `tagged_pool.csv` (returned by client) |
| 环节 4 | `/patent-panorama-insights-report` | all Stage 2/3 files + `tagged_pool.csv` | `report_manifest.json`, `report.html` |
...
- 选择：
  - (a) 确认 → 进入环节 2。
  - (b) 编辑某个分支 → 仅为该分支重新运行 patent-panorama-insights-search → 重新抽样 → 回到这里。
...
- 选择：
  - (a) 确认 → 进入环节 3。
  - (b) 调整范围（收窄分支、替换竞品、改变日期口径）→ 重新运行 patent-panorama-insights-stats（如果检索式本身变化，也重新运行 patent-panorama-insights-search）→ 回到这里。
- 默认：(a)。
...
- 选择：
  ...
  - (b) 修改分类体系（合并/拆分节点、收紧描述、重新平衡问题）→ 重新运行 patent-panorama-insights-tag → 回到这里。
...
- 选择：
  - (a) 接受 → 交付。
  - (b) 要求修改（章节、表达、深度）→ 重新运行相关 patent-panorama-insights-report 步骤 → 回到这里。
...
如果领域较宽，提出第一版拆解（子技术、产品类别、部件、应用场景、技术问题/功效、排除区域）。这是环节1的规则设计脚手架，不是最终标引分类体系——最终四列分解表由环节3（patent-panorama-insights-tag）产出。
...
|---|---|
| 环节 1 检索建库 | `patent-panorama-insights-search/SKILL.md` — 字段算符、恒定主题锚点、半自动 IPC、分层 NOT、A6 骨架、精度抽样 |
| 环节 2 格局统计 | `patent-panorama-insights-stats/SKILL.md` — 趋势/申请人/竞品聚合 + 价值点交叉挖掘 |
| 环节 3 标引体系设计 | `patent-panorama-insights-tag/SKILL.md` — 四列分解表、关键问题、专利包、标引 demo、待标引导出 |
| 环节 4 报告生成 | `patent-panorama-insights-report/SKILL.md` — 演进路线、竞品画像、护城河解读、推荐包叙事、HTML 构建 |
...
这份第一版拆解不是最终标引。它是环节 1（patent-panorama-insights-search）的规则设计脚手架。
