...
**v5.2 新增合同台账与履约提醒、档案库全文检索、风险趋势对比、类案要点库与判决倾向参考**：审查完成的合同自动沉淀到台账，到期前 30/7/1 天生成待办清单；历史合同库建本地倒排索引，支持全文检索与多维过滤；多合同聚合视图展示风险趋势变化；内置 30 个最高法指导案例要点，按争议条款类型输出法院倾向摘要。

**v5.0 完成从"审查工具"到"决策辅助"的升级**：内置法条引用溯源（民法典合同编 60 条 + 公司法 40 条 + 司法解释 30 条），每个审查结论附带具体法律条文引用，增强专业可信度；新增合同谈判辅助，基于多轮版本对比自动识别必争/可让步条款，生成谈判准备文档；新增中英文双语合同对照审查，段落级对齐并检测版本不一致。

**v4.0 完成从「找问题」到「给方案」的升级**：内置 275 条标准条款库，风险点自动匹配推荐替换文本，一键生成带红色修订标记的合同修订稿；同时提供医疗、建筑工程、跨境电商、互联网软件四大行业共 244 条专项合规规则。

### 🆕 v5.2 新特性（2026-07-29）

- **📊 合同台账与履约提醒**：审查完成的合同自动抽取关键信息（合同号、对方主体、金额、付款节点、到期日）回填 SQLite 台账；提醒引擎通过 Windows 计划任务每日扫描，到期前 30/7/1 天生成待办清单；可选企微 webhook 推送（用户自配 URL）
- **🔍 档案库全文检索**：对历史合同库建本地倒排索引（char/bigram 分词 + BM25 召回），支持按条款类型、风险等级、对方主体、日期范围多维过滤，命中高亮显示；统一检索引擎合并法条检索与条款匹配，共享分词与召回基础设施
- **📈 风险趋势对比**：多合同聚合视图——风险类型频次按月分布、红级条款占比变化趋势、同一对方主体历史风险复发标记；支持 matplotlib 出图（无依赖时自动降级为数据表）
- **⚖️ 类案要点库与判决倾向参考**：内置 30 个最高法指导案例要点（覆盖买卖/借款/公司/建设工程/知识产权/劳动等 20 个类别），按争议条款类型输出「同类条款争议中法院倾向」摘要；支持用户导入自整理类案 JSON 增库；数据 freshness 明确标注（静态公开案例库，非实时判例）
- **🔄 统一检索引擎**：合并 legal_retriever（法条检索）与 clause_matcher（条款匹配）为 unified_retriever.py，共享 char/bigram 分词、TF-IDF/BM25 打分基础设施，支持法条索引 + 条款索引双索引统一搜索

### 🆕 v5.1 新特性（2026-07-29）

- **📖 指导案例引用溯源**：为每个审查结论附带最高人民法院指导性案例引用（"参见指导案例 17 号「产品责任纠纷—欺诈行为的惩罚性赔偿」"），增强说服力
- **📚 指导案例数据库（30 个）**：覆盖买卖合同、服务合同、借款合同、公司法、网络服务、建设工程、破产法、保险合同、赠与合同、股权转让、保理合同、反垄断、房地产合同、执行程序、知识产权、劳动合同、环境侵权、合同效力、个人信息、技术合同 20 个类别
- **🔄 月度更新提醒**：每月检查指导案例数据库是否需要更新，提示用户关注最高法新发布的指导性案例
- **💰 关键金额校验**：金额大小写一致性验证（自动转换中文大写与阿拉伯数字进行比对），勾稽关系自动验证（单价 × 数量 = 总价等），金额识别错误率 <0.5%
...
- **⚖️ 法条引用溯源**：为每个审查结论附带具体法律条文引用（"依据《民法典》第 584 条，违约金过高可请求人民法院减少"），增强专业可信度
- **📖 法条数据库（130 条）**：民法典合同编核心条文 60 条、公司法核心条文 40 条、司法解释精选 30 条，结构化可检索
- **🔄 季度更新提醒**：每 3 个月检查法条数据库是否需要更新，提示用户关注法律法规变化
- **🤝 合同谈判辅助**：分析多轮修改差异，识别对方"必争条款"（始终不改的）和"可让步条款"（逐步退让的），生成谈判准备文档
- **📋 谈判策略库**：13 项条款策略（违约金/知识产权/付款节点/竞业限制/工程价款等），给出"守住/让步/交换"建议
- **🌐 中英文双语合同对照审查**：段落级对齐算法，检测"中文有但英文没有""措辞矛盾"等不一致
- **📚 中英术语对照表**：200+ 组高频法律/公司/劳动/建设工程/知识产权术语
...
| `--legal-base` | **v5.0** 启用法条引用溯源，为每个风险点匹配相关法律条文 | 关闭 |
| `--guiding-cases` | **v5.1** 启用指导案例引用溯源，为风险点匹配最高法指导案例 | 关闭 |
| `--negotiate` | **v5.0** 启用谈判辅助，指定历史合同版本目录（目录下按时间顺序放置多个版本文件） | 不启用 |
| `--align` | **v5.0** 启用中英双语对照审查，指定另一语言版本路径（与当前版本进行段落对齐） | 不启用 |
...
| `--archive-search` | **v5.2** 档案库全文检索，指定搜索关键词 | — |
| `--archive-filter` | **v5.2** 档案库检索过滤条件（JSON 格式：clause_type/risk_level/counterparty/date_from/date_to） | — |
| `--risk-trend` | **v5.2** 风险趋势对比分析，指定分析月数（默认 12） | — |
| `--case-law` | **v5.2** 启用类案要点库，为风险点补充判决倾向参考 | 关闭 |
| `--case-search` | **v5.2** 类案要点库搜索，指定搜索关键词 | — |
| `--case-import` | **v5.2** 导入用户自整理案例 JSON 文件 | — |
...
- `references/legal_basis/civil_code_contract.yaml` — **v5.0** 民法典合同编核心条文 60 条
- `references/legal_basis/company_law.yaml` — **v5.0** 公司法核心条文 40 条
- `references/legal_basis/judicial_interpretations.yaml` — **v5.0** 司法解释精选 30 条
- `references/legal_basis/index.json` — **v5.0** 法条数据库索引（136 条）
- `references/negotiation_strategies.yaml` — **v5.0** 谈判策略库（13 项条款策略）
- `references/bilingual_glossary.yaml` — **v5.0** 中英法律术语对照表（200+ 组）
- `references/ja_legal_terms.yaml` — **v5.1** 日语法律术语对照表（200+ 组）
- `references/ko_legal_terms.yaml` — **v5.1** 韩语法律术语对照表（200+ 组）
- `references/guiding_cases/cases.json` — **v5.1** 指导案例数据库（30 个民商事合同类指导案例）
...
- `scripts/updater.py` — **v3.0** 更新检测提醒 + **v5.0** 法条数据库季度更新提醒 + **v5.1** 指导案例数据库月度更新提醒
- `scripts/clause_matcher.py` — **v4.0** 条款匹配引擎（风险点 → 条款库 → 推荐替换文本）
- `scripts/contract_reviser.py` — **v4.0** 一键修订稿生成（红色删除线 + 绿色下划线 + 批注）
- `scripts/legal_retriever.py` — **v5.0** 法条检索引擎（关键词自动匹配法条、法条全文展开查看、季度更新提醒）
- `scripts/negotiation_analyzer.py` — **v5.0** 合同谈判辅助引擎（多轮修改差异分析、必争/可让步条款识别、谈判文档生成）
- `scripts/bilingual_aligner.py` — **v5.0/v5.1** 多语种合同对齐引擎（段落对齐算法、版本不一致检测、优先级标注、术语表懒加载、日/韩扩展）
- `scripts/case_retriever.py` — **v5.1** 指导案例检索引擎（关键词+标签自动匹配案例、案例全文展开查看、月度更新提醒）
- `scripts/amount_validator.py` — **v5.1** 关键金额校验引擎（金额大小写一致性验证、勾稽关系自动验证、错误率 <0.5%）
- `scripts/contract_ledger.py` — **v5.2** 合同台账与履约提醒引擎（SQLite 台账、自动抽取回填、到期提醒、计划任务注册、企微推送）
- `scripts/archive_search.py` — **v5.2** 档案库全文检索引擎（char/bigram 倒排索引、BM25 召回、多维过滤、命中高亮）
