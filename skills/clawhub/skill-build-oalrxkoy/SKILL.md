---
name: opc-skill-content-discover
description: MCP Skill: read OPC discover / AI羊毛 articles for solo entrepreneurs
version: 0.1.0
metadata:
  openclaw:
    requires: {}
---
# SKILL.md — OPC 发现 / AI 羊毛读取

> 一句话：把 OPC 的发现 / AI 羊毛装进你的 AI 助手，对话式查询，无需手动翻网页。
...
OPC（一Person 公司 / One Person Company）工具站维护着公开的发现内容：

- **发现 / AI 羊毛**：深度种草、工具上手体验、一人公司效率兵器谱

本 Skill 是一个 **MCP（Model Context Protocol）Server**，让 Claude Desktop / Cursor / Cline 等 AI 助手在你的对话中直接读取发现内容，按你的需求筛选后返回。
...
```json
{
  "mcpServers": {
    "opc-discover": {
      "command": "npx",
      "args": ["-y", "@opc/skill-content-discover"],
      "env": {
...
AI 会自动调用 `opc_list_discover`，把筛选后的结果返回给你。

## 工具清单

| 工具名 | 功能 | 示例问题 |
|--------|------|---------|
| `opc_list_discover` | 读取发现 / AI 羊毛列表 | 「最近有什么 AI 羊毛工具？」「效率工具推荐」 |
...
| 返回 | 含义 |
|------|------|
| `❌ 获取发现失败：unauthenticated` | 缺少或错误的 API Key |
| `❌ 获取发现失败：invalid_key` | Key 被吊销 |
| `❌ 获取发现失败：forbidden` | Key 权限不足（需 read 权限） |
| `❌ 获取发现失败：internal_error` | 服务器异常 |
