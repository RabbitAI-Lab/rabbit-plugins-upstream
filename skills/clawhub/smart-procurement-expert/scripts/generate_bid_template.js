/**
 * 智慧招采专家 · 深度节点 B — 可配置化标书生成引擎
 *
 * 接收配置对象，生成格式正确的投标文件 docx。
 * 支持：四种编号体系、动态章节列表、自定义项目编号、封面【正本】、X/100 页脚。
 * v1.0.2 升级：章节「初稿」模式——chapters[].body 可注入起草正文（骨架→章节初稿），
 *            chapters[].chart 可注入图表占位说明（可视化增强）。
 *
 * 使用方式:
...
/**
 * 图表占位块（可视化增强）。
 * desc 可为字符串（占位标题）或 { title, hint } 对象。
 * 渲染为灰色底纹居中提示段落，引导用户在对应位置插入真实图表。
 */
function buildChartPlaceholder(desc) {
  const title = (desc && typeof desc === 'object') ? (desc.title || '图表占位') : String(desc || '图表占位');
  const hint = (desc && typeof desc === 'object' && desc.hint) ? `（${desc.hint}）` : '';
  return new Paragraph({
    spacing: { line: LINE_BODY, before: 120, after: 120 },
    shading: { type: ShadingType.CLEAR, fill: 'F2F2F2' },
    alignment: AlignmentType.CENTER,
    children: [new TextRun({
      text: `【图表占位 · ${title}】${hint}`,
      font: 'SimSun', size: 20, bold: true, color: '808080'
    })]
  });
}

/** 渲染章节初稿正文（body）。支持 string / string[] / {text,bold,size,font}[] */
function renderChapterBody(ch) {
  ...
  for (const para of paras) {
    if (typeof para === 'string') {
      if (para.trim() === '') out.push(emptyPara());
      else out.push(bodyPara(para));
    } else if (para && typeof para === 'object') {
      out.push(bodyPara(para.text || '', { bold: para.bold, size: para.size, font: para.font }));
    }
  }
  return out;
}
...
async function generateBidDocument(config) {
  ...
  for (const ch of numberedChapters) {
    ...
    // 章节初稿模式：若提供 body 则渲染起草正文，否则保留占位（骨架模式向后兼容）
    const hasBody = ch.body && (Array.isArray(ch.body) ? ch.body.length : String(ch.body).trim().length);
    if (hasBody) {
      renderChapterBody(ch).forEach(p => bodyChildren.push(p));
    } else {
      if (ch.level >= 4) {
    ...
    // 图表占位（可视化增强）
    if (ch.chart) {
      const charts = Array.isArray(ch.chart) ? ch.chart : [ch.chart];
      charts.forEach(cdesc => bodyChildren.push(buildChartPlaceholder(cdesc)));
    }
  }
