...
2. **Copy config from skill package:**
   ...
   CONFIG_PATH="$(find . -path "*/skills/relationship-buddy/config/relationship-config.json" -type f -print -quit 2>/dev/null)"
   SKILL_DIR=""
...
4. **Copy scripts (optional — for contact import):**
   ...
   done < <(find . -path "*/skills/relationship-buddy/scripts/*" -type f 2>/dev/null)
