...
if [ -z "$SKILL_CONFIG" ]; then
  SKILL_CONFIG=$(find ./skills ./security-team -path '*/security-team/config/security-config.json' -type f 2>/dev/null | grep '/security-team/config/security-config.json$' | head -1)
fi
...
# Fallback: constrained search under expected skill roots only
if [ -z "$SKILL_DIR" ]; then
  SKILL_SCAN=$(find ./skills ./security-team -path '*/security-team/scripts/security-scan.sh' -type f 2>/dev/null | grep '/security-team/scripts/security-scan.sh$' | head -1)
