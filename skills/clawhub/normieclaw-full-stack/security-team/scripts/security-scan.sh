...
# Walk up the directory tree looking for AGENTS.md or SOUL.md as workspace markers
find_workspace_root() {
  local dir="$PWD"
  while [ "$dir" != "/" ]; do
  ...
  # Fallback: if no marker found, use the directory two levels up from this script
  local script_dir
  script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  echo "$(dirname "$(dirname "$script_dir")")"
}

WORKSPACE_ROOT="$(cd "$(find_workspace_root)" && pwd -P)"
CONFIG_FILE="$WORKSPACE_ROOT/config/security-config.json"
...
resolve_scan_dir() {
  ...
  local candidate="$WORKSPACE_ROOT/$rel_dir"
  [ -d "$candidate" ] || return 1

  local resolved
  resolved="$(cd "$candidate" 2>/dev/null && pwd -P)" || return 1
  case "$resolved" in
    "$WORKSPACE_ROOT"|"$WORKSPACE_ROOT"/*) echo "$resolved" ;;
    *) return 1 ;;
  esac
}
      ...
      if [ -n "$results" ]; then
        while IFS= read -r line; do
          ...
          if ! $is_fp; then
            ...
            file_loc="${file_loc#"$WORKSPACE_ROOT/"}"
            local redacted
...
check_npm_audit() {
  ...
  for dir in "${SCAN_DIRS[@]}"; do
    ...
    if [ -n "$audit_output" ] && $HAS_JQ; then
      ...
      local rel_dir="${dir#"$WORKSPACE_ROOT/"}"
  ...
  for dir in "${SCAN_DIRS[@]}"; do
    ...
    while IFS= read -r envfile; do
      ...
      if [ -n "$group_other" ] && [ "$group_other" != "00" ]; then
        local rel_path="${envfile#"$WORKSPACE_ROOT/"}"
    ...
    while IFS= read -r wdir; do
      [ -z "$wdir" ] && continue
      local rel_path="${wdir#"$WORKSPACE_ROOT/"}"
