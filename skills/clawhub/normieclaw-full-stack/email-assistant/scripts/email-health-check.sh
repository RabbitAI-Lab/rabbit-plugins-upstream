...
# Walk up from script location to find workspace root (contains AGENTS.md or SOUL.md)
find_workspace_root() {
  local dir
...
if [ -z "$WORKSPACE_ROOT" ]; then
  echo "⚠️  Could not find workspace root (no AGENTS.md or SOUL.md found)."
...
DATA_DIR=""
if [ -n "$WORKSPACE_ROOT" ] && [ -d "$WORKSPACE_ROOT/email-assistant" ]; then
  DATA_DIR="$WORKSPACE_ROOT/email-assistant"
