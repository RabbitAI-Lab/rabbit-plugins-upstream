"""Run registered TikTok Shop ERP logistics APIs via /tiktokShop/developerProxy."""
...
from _logistics_endpoints import LOGISTICS_ENDPOINTS, RESERVED_PARAM_KEYS
from _shop_logistics_common import (
...
def resolve_shop_cipher(params: dict, open_id: str) -> str:
    ...
    print(
        ...
        '  Example: {"api":"get_warehouse_list","openId":"...","order_ids":["..."],'
        '"shop_cipher":"GCP_..."}',
        file=sys.stderr,
    )
    sys.exit(1)
...
def run_logistics_api(api_name: str, params: dict, caller: Optional[str] = None) -> dict:
    if api_name not in LOGISTICS_ENDPOINTS:
        print(
            f"Unknown api: {api_name}. Valid: {', '.join(sorted(LOGISTICS_ENDPOINTS))}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_logistics_proxy(params: dict, caller: str = "logistics_proxy.py") -> dict:
    ...
    shop_cipher = None
    needs_cipher = str(path).lstrip("/").startswith("logistics/")
    if needs_cipher:
        shop_cipher = resolve_shop_cipher(params, open_id)
