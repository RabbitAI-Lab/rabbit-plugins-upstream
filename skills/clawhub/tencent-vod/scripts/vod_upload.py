...
try:
    ...
    print(f"错误：请先安装依赖: python3 -m pip install tencentcloud-sdk-python")
    sys.exit(1)
...
try:
    ...
    print("错误：请先安装依赖: python3 -m pip install vod-python-sdk")
    sys.exit(1)
...
def upload_media(args):
    ...
    # 获取密钥（复用 get_credential，含 dotenv 兜底加载）
    cred = get_credential()
...
示例:
  ...
  # 上传并指定媒体名称、分类
  python3 vod_upload.py upload --file /path/to/video.mp4 --media-name "我的视频" --class-id 100
  ...
  # URL 拉取上传并等待完成
  python3 vod_upload.py pull --url "https://example.com/video.mp4"  # 默认等待完成
  ...
  python3 vod_upload.py upload --file /path/to/video.mp4 --dry-run
    ...
    upload_parser.add_argument('--region', default=os.getenv('TENCENTCLOUD_REGION', 'ap-guangzhou'), help='地域，默认 ap-guangzhou')
    ...
    pull_parser.add_argument('--region', default=os.getenv('TENCENTCLOUD_REGION', 'ap-guangzhou'), help='地域，默认 ap-guangzhou')
