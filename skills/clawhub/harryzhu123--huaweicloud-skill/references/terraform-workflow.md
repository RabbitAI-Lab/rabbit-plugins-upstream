...
ClawHub packages Terraform examples as inspectable `*.tf.txt` files because
the registry does not currently accept the `.tf` extension. Treat their
contents as Terraform source and remove only the final `.txt` suffix when
materializing a runnable working copy.
