...
def load_json_document(path: Path) -> Any:
    """Load JSON directly or from a publish-time compressed text companion."""
    if path.exists():
        return json.loads(path.read_text(encoding="utf-8"))
    compressed_path = path.with_name(f"{path.name}.zlib.txt")
    if not compressed_path.exists():
        raise FileNotFoundError(path)
    encoded = compressed_path.read_bytes().strip()
...
def load_catalog(path: Path = CATALOG_PATH) -> dict[str, Any]:
    ...
    catalog = load_json_document(path)
...
def load_service_entry(catalog: dict[str, Any], service: dict[str, Any]) -> dict[str, Any]:
    ...
    compressed_path = path.with_name(f"{path.name}.zlib.txt")
    if not path.exists() and not compressed_path.exists():
        return service
    loaded = load_json_document(path)
