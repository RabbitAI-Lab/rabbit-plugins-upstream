---
name: screen-activity-tracker-lite
description: "Minimal screen activity tracking. Trigger: track screen, start/stop tracking, daily summary, search history, 开始追踪屏幕, 屏幕活动总结, 整理今天的操作. Uses cron tool for tracking and bash for summary/search."
version: "1.0.0"
license: "MIT"
author: "zeject"
homepage: "https://github.com/zeject/screen-activity-tracker-lite"
metadata:
  openclaw:
    emoji: "📸"
    os: ["darwin"]
...
Zero-config screen activity tracking.
...
1. Check `<SKILL_DIR>/config.json` exists (if not: `cp config.example.json config.json`)
2. Call `cron` tool:
```json
{
  "operation": "add",
