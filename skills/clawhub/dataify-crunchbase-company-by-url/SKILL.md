...
description: "Collect structured Crunchbase company profiles from one or more known company URLs. Do not use for general web research or LinkedIn company URLs."
---

# Dataify Builder Skill

Use this skill to prepare Dataify builder requests for the scraper family rooted at `crunchbase_company_by-url` on `crunchbase.com`.


## Quick Start

**Input:** a Crunchbase company URL.

```bash
python3 scripts/build-dataify-request.py --tool-sign crunchbase_company_by-url --params-json '[{"url":"https://www.crunchbase.com/organization/openai"}]'
```
