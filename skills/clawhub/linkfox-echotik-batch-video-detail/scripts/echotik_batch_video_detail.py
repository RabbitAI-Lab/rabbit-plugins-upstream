#!/usr/bin/env python3
"""
EchoTik TikTok Batch Video Detail - LinkFox Skill
调用 /echotik/batchVideoDetail 接口。

Usage:
  python echotik_batch_video_detail.py '<JSON parameters>'           # 自动：小结果全量；大结果写文件+摘要
  python echotik_batch_video_detail.py '<JSON parameters>' --inline  # 强制全量打印到 stdout
...
API_PATH = "/echotik/batchVideoDetail"
SLUG = "linkfox-echotik-batch-video-detail"
...
def main():
    # cp936(GBK) Windows 控制台下，TikTok 视频标题常含 emoji，会导致 print/落盘时
    # UnicodeEncodeError。统一将 stdout 切到 utf-8（落盘写入见下方 encoding="utf-8"）。
    try:
    ...
    if not argv:
        print(
            "Usage: echotik_batch_video_detail.py '<JSON parameters>' [--inline]",
            file=sys.stderr,
        )
        sys.exit(1)
