...
from diagnosis_sufficiency_review import DEFAULT_LLM_BASE, DEFAULT_LLM_MODEL, review_diagnosis_sufficiency_payload
...
def save_prepared(payload: dict[str, Any], output_json: str, input_path: Path) -> None:
    save_dir = Path(output_json).parent if output_json else ICD_DRG_DIR / "runs" / "diagnosis-sufficiency-review"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="诊断依据充分性审核统一入口：支持 pdf/doc/docx/xls/xlsx/csv/txt/json 输入。")
    ...
    parser.add_argument("--diagnosis", action="append", default=[], help="待审核诊断，格式 role|code|name 或 code|name；可重复。")
    parser.add_argument("--diagnoses-json", default="", help="待审核诊断 JSON 字符串或文件路径。")
...
def main() -> int:
    ...
    try:
        ...
        diagnoses = merge_candidate_args(args.diagnoses_json, args.diagnosis, default_role="other")
        if diagnoses:
        ...
        response = review_diagnosis_sufficiency_payload(payload)
