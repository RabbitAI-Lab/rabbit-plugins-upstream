...
API_BASE = os.environ.get(f"COZE_JD_API_URL_{SKILL_ID}", os.environ.get("JD_API_URL", "http://47.109.39.255:8765/api/v4")
USER_KEY = os.environ.get("JD_API_USER_KEY", "")
...
def cmd_register(args):
    ...
    print()
    print(f"  ⚠️  请设置环境变量：")
    print(f"  export JD_API_USER_KEY={result.get('user_key')}")
    print("=" * 50)
    print()
...
def cmd_info(args):
    if not USER_KEY:
        print("❌ 请先设置 JD_API_USER_KEY 环境变量", file=sys.stderr)
        sys.exit(1)
...
def cmd_generate(args):
    if not USER_KEY:
        ...
        sys.exit(1)
    print(f"🚀 JD生成器中...", file=sys.stderr)
    data = {}
    ...
    if args.requirements: data['requirements'] = args.requirements
    result = api_call("jd-generator", data)
    content = result.get("jd_content", "")
...
def main():
    parser = argparse.ArgumentParser(description="JD生成器 - SaaS版，2点/次")
    ...
    g = sub.add_parser("generate", help="JD生成器")
    g.add_argument("--position", required=True, help="职位名称")
    g.add_argument("--department", default="", help="部门")
    g.add_argument("--level", default="mid", help="级别: entry/mid/senior/lead")
    g.add_argument("--industry", default="", help="行业")
    g.add_argument("--requirements", default="", help="特殊要求")
