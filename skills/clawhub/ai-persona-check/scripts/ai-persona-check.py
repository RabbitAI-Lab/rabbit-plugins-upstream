#!/usr/bin/env python3
"""
AI 拟人化互动服务合规检查 — 《人工智能拟人化互动服务管理暂行办法》（国家网信办等五部门令第21号，2026-07-15 施行）
...
free API Key (100 free calls) on the web:
  ...
  Open: https://compliancehub.cn/account.html?skill=ai-persona-check
  Then provide the Key via env COMPLIANCEHUB_API_KEY, or save it to
  ~/.config/compliancehub/ai-persona-check.key (mode 0600).
    ...
    try:
        if os.path.isfile(pkg):
            with open(pkg, encoding="utf-8") as f:
                return json.load(f).get("name", "ai-persona-check")
    except Exception:
        pass
    return "ai-persona-check"
...
CHECK_ITEMS = [
  {
    ...
    "ref": "办法§22",
    "category": "A. 安全评估",
    "desc": "上线前是否开展安全评估？注册用户100万以上或月活10万以上是否向省级网信部门提交安全评估报告？",
    "recommendation": "上线前完成自评估，达到阈值及时向省级网信部门提交评估报告。"
  },
  {
    "id": "algo_filing",
    "name": "算法备案",
    "ref": "办法§26",
    "category": "A. 安全评估",
    "desc": "具有舆论属性或社会动员能力的算法是否按规定履行算法备案？",
    "recommendation": "在网信部门完成算法备案并取得备案号。"
  },
  {
    "id": "content_redline",
    "name": "七条红线拦截",
    "ref": "办法§8",
    "category": "B. 内容红线",
    "desc": "是否拦截危害国安、煽动暴力、诱导自残自杀、语言暴力、诱导套取个人信息、过度迎合、情感操纵等红线内容？",
    "recommendation": "建立红线内容识别与处置机制，全流程拦截。"
  },
  {
    ...
    "ref": "办法§9",
    "category": "B. 内容红线",
    "desc": "是否建立内容安全主体责任制度与应急处置流程？",
    "recommendation": "明确责任主体，制定安全管理制度与应急预案。"
  },
  {
    "id": "minor_virtual_intimacy",
    "name": "不向未成年人提供虚拟亲密关系",
    "ref": "办法§14",
    "category": "C. 未成年人保护",
    "desc": "是否不向未成年人提供模拟恋爱、虚拟伴侣等虚拟亲密关系服务？",
    "recommendation": "对未成年人关闭虚拟亲密关系功能，做年龄识别。"
  },
  {
    "id": "minor_consent",
    "name": "未成年人监护人同意",
    "ref": "办法§14/§17",
    "category": "C. 未成年人保护",
    "desc": "处理不满十四周岁未成年人个人信息是否取得监护人同意？",
    "recommendation": "未成年人信息单独同意（监护人），制定专门处理规则。"
  },
  {
    "id": "minor_mode",
    "name": "未成年人模式",
    "ref": "办法§17",
    "category": "C. 未成年人保护",
    "desc": "是否提供未成年人模式并限制使用时长与功能？",
    "recommendation": "默认或一键开启未成年人模式，限制功能与时段。"
  },
  {
    "id": "ai_identity_disclosure",
    "name": "AI 身份明示",
    "ref": "办法§10/§12",
    "category": "D. 用户权益",
    "desc": "是否向用户显著明示其为人工智能生成/提供的服务，不冒充自然人？",
    "recommendation": "在交互界面持续明示AI身份，不得假装真人。"
  },
  {
    "id": "anti_addiction",
    "name": "防沉迷与便捷退出",
    "ref": "办法§18/§19",
    "category": "D. 用户权益",
    "desc": "连续使用超2小时是否提醒？是否提供便捷退出、中断情感依赖的机制？",
    "recommendation": "设置使用时长提醒与一键退出，防止过度依赖。"
  },
  {
    "id": "data_security",
    "name": "数据安全与日志留存",
    "ref": "办法§10/§11/§16",
    "category": "E. 数据安全",
    "desc": "交互数据是否加密存储、不向第三方提供？训练数据是否合规管理？是否留存日志？",
    "recommendation": "交互数据最小必要、加密存储，训练数据合规，日志留痕。"
  }
]
...
def collect_responses(items):
    ...
    responses = []
    total = len(items)
    print(f"\n📋 AI 拟人化互动服务合规检查 — {total} 项", file=sys.stderr)
...
def render_text(data, items):
    s = data
    lines = ["=" * 60, "  AI 拟人化互动服务合规检查报告 (云端评分)" if not s.get("scored_locally") else "  AI 拟人化互动服务合规检查报告 (本地兜底评分)",
             f"  法规：《人工智能拟人化互动服务管理暂行办法》（国家网信办等五部门令第21号，2026-07-15 施行）", f"  引擎版本：{s.get('version', '?')}",
             _score_block(s), "=" * 60]
    current_cat = ""
    for r in items:
...
<html lang="zh"><head><meta charset="UTF-8"><title>AI 拟人化互动服务合规检查报告</title>
<style>
...
<h1>AI 拟人化互动服务合规检查报告</h1>
<p>法规：《人工智能拟人化互动服务管理暂行办法》（国家网信办等五部门令第21号，2026-07-15 施行）</p>
...
def main():
    parser = argparse.ArgumentParser(description="AI 拟人化互动服务合规检查 (free skill + cloud engine)")
