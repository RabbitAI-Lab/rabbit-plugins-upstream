---
name: amazon-rating-analysis
display_name: 亚马逊星级评分分析 · 评分结构诊断
description: >
  亚马逊星级评分分析 Skill：拆解一个 ASIN 的评分结构——各星级占比、评分随时间的变化、
  差评集中在哪个时间段，判断是批次问题还是长期硬伤，辅助定位评分下滑的原因。
  Use when the user asks about star rating breakdown, rating distribution,
  review score analysis, 星级分析、评分分析、评分分布、评分下滑、星级占比、
  评分诊断。Requires an ARI API key (ari_live_*).
author: ARI (funewa)
agent_created: true
slug: rating-analysis
displayName: 亚马逊星级评分分析 · 评分结构诊断
version: 1.4.7
summary: 评分结构与下滑归因：星级占比 + 时间分布
license: MIT
---
