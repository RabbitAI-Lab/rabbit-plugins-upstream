...
/**
 * Shopify 分类采集器 - 最终版本
 * 自动处理 Ajax 懒加载下拉菜单，从导航结构提取真实层级
 * 
 * 功能：
 * - 自动识别一级/二级分类导航结构
 * - 处理需要鼠标悬停才能加载的 Ajax 下拉菜单
 * - 一级分类单独一个单元格，二级分类单独一个单元格
 * - 根据实际层级分栏，有多少层分多少层
 * - 输出清晰的中文表头
 * 
 * 使用方法：
 *   node collect.js <website-url> [output-directory]
 * 
 * 示例：
 *   node collect.js https://shop.futvortexstore.com/
 *   node collect.js https://shop.futvortexstore.com/ C:\workspace\caiji
 * 
 * 输出格式：
...
async function collectCategories(startUrl, outputDir = 'C:\\workspace\\caiji') {
  ...
  try {
    ...
    // 分析导航结构，获取完整的分类层级
    console.log('🔍 分析导航结构（处理 Ajax 懒加载下拉菜单）...');
  ...
  for (const li of level1Items) {
    ...
    if (hasChildren) {
      ...
      if (subMenu) {
        // 等待懒加载完成，等待至少有一个链接出现
        try {
          await subMenu.waitForSelector('a', { timeout: 15000 });
          await page.waitForTimeout(1500); // 等待全部加载
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(`
📦 Shopify 分类采集器 (Category Collector) - 最终版

功能：
✅ 自动识别导航结构，提取真实分类层级
✅ 处理 Ajax 懒加载下拉菜单（需要悬停触发加载）
✅ 一级分类单独一个单元格，二级分类单独一个单元格
✅ 根据页面实际结构，有多少层分多少层
✅ 清晰的中文表头，方便 Excel 打开
...
输出格式示例：
  | 完整链接 | URL 路径 slug | 一级分类 | 二级分类 | 实际层级深度 |
  | https://shop.futvortexstore.com/collections/liverpool | premier-league/liverpool | Premier League | Liverpool | 2 |
    `);
    process.exit(0);
  }
