...
# 3. 成片：脚本 + 商品图；要模仿运镜时把原片直链当参考视频（仅 supportsReferenceVideo:true 的模型支持，先 options 查）
...
- **`uploadedVideo` 可传数组**：用户想同时对标多条爆款（融合运镜）时把多个直链一起传，上限见 `options` 的 `models` 里 `maxReferenceVideos`，CLI 提交前校验。
- 想连原片的 BGM/口播一起对标时，用 `uploadedAudio` 传参考音频（本地文件 ≤50MB 自动上传），仅 `options` 的 `models` 里 `supportsReferenceAudio: true` 的模型支持。
