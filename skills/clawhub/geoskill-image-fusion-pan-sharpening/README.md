# Image Fusion and Pansharpening (geoskill-image-fusion-pan-sharpening)

> Brovey/IHS pansharpening that fuses multispectral and panchromatic bands to produce higher spatial resolution imagery

---

## 1. Overview

Fuses a high spatial resolution panchromatic band (PAN) with lower resolution multispectral (MS) data to produce imagery with both high spatial resolution and multispectral information. Two classic methods are implemented: the **Brovey** transform (fused_b = MS_b↑ × PAN / Σ(MS↑), which preserves band proportions so that the fused bands sum to PAN) and **IHS** (which replaces the intensity component with PAN, then applies the inverse transform fused_b = MS_b↑ + (PAN − I), injecting spatial detail into each band through the intensity difference). The multispectral data is first upsampled to PAN resolution by bicubic interpolation before fusion. Typical applications: Landsat-8/9 OLI (30 m MS + 15 m PAN), Sentinel-2 (10/20/60 m bands fused to 10 m), the GaoFen and Ziyuan series, WorldView/GeoEye, etc. Brovey suits scenes with fewer bands and low noise; IHS is simple to implement and preserves hue well.
...
python geoskill-image-fusion-pan-sharpening.py --bbox 116 39 117 40 --synthetic --output-dir ./out
```
