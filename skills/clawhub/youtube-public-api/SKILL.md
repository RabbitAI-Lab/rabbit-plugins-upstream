...
description: Access information: Accountless requests use x402 v2 pay-per-request settlement in USDC; no account or API key required on this path. Read-only YouTube public-data agent skill for video search, video/channel/playlist metadata, public comments, related videos, and transcripts, normalized to structured JSON.
version: 1.0.4
...
**Access information:** Accountless requests use x402 v2 pay-per-request settlement in USDC; no ReplyNodes account or API key is required on this path.
