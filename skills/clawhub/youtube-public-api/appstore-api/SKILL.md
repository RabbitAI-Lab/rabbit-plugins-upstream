...
description: Access information: Accountless requests use x402 v2 pay-per-request settlement in USDC; no ReplyNodes account or API key is required on this path. Read-only, normalized public-data reads of App Store applications: app lookup by track ID or bundle ID, bounded term search, and related-app listings. Bearer workspace-key access is also supported as an alternative, with transparent bounded pages, normalized errors, and an explicit unsupported-capability matrix. Public reads only: no purchase, review, account, or other mutation exists.
version: 1.1.0
contract_version: v1
mode: readonly
...
Read-only, normalized public-data reads of App Store applications through the ReplyNodes fetcher domain: app lookup by track id or bundle id, bounded term search, and related-app listings. Bearer workspace-key authentication is also supported as an alternative, with transparent bounded pages, normalized errors, and an explicit unsupported-capability matrix. Public reads only: no purchase, review, account, or other mutation exists, and no platform login material is involved.
