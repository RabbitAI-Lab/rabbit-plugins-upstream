...
如果需要基于真人照片生成换发型后的效果图，使用配套脚本 [`scripts/generate_hairstyle.py`](scripts/generate_hairstyle.py)。
...
```bash
python3 scripts/generate_hairstyle.py <图片路径> <输出路径> [提示词]
```
...
```bash
python3 scripts/generate_hairstyle.py ~/photo.png ~/Desktop/效果图.png \
  "Change only the hairstyle to a short textured fringe covering the forehead. Keep everything else exactly the same."
```

#### 提示词技巧

脚本使用 `google/nano-banana-2/edit` 模型，提示词需要：
- **明确指定只改头发**：`"Change only the hairstyle..."`
- **强调保留其他部分**：`"Keep the face, glasses, clothing, and background exactly as they are"`
- **详细描述新发型**：长度、渐变类型、刘海、纹理等

**推荐提示词模板**：
```
Change only the hairstyle. The person now has [detailed hairstyle description]. 
Keep the face, skin tone, expression, clothing, and background exactly as they are. 
Only the hair changes.
```

#### 注意事项

- 效果图是 AI 生成的参考，实际效果可能因发质、理发师技术等因素有所不同
- 模型可能会连带改变背景或衣服颜色，重点是观察发型方向
- 生成时间约 30-60 秒
