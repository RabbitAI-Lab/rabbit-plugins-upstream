...
class HeixiangFusion:
    ...
    def _divine_qimen(self, y, m, d, h, mi, question):
        ...
        try:
            ...
            text = self.duanju_engine.duanju_text(pan, question)
    ...
    def _fuse_by_type(self, question, liuyao, qimen):
        ...
        # 事业（在寻物之前检查——"找工作"的"找"不应触发寻物）
        if any(kw in question for kw in ['事业', '工作', '面试', 'offer', '职场', '升职', '跳槽', '转行', '发展', '老板', '同事']):
            pass  # 事业判断由综合结论覆盖
        ...
        # 财运
        if any(kw in question for kw in ['财', '钱', '赚', '投资', '股票']):
            qm_cai = self._qimen_cai_judge(qimen)
            ly_cai = self._liuyao_cai_judge(liuyao)
            
            if qm_cai and ly_cai and qm_cai['倾向'] == ly_cai['倾向']:
                agree.append({
                    '维度': '财运',
                    '结论': f"两法一致：**{qm_cai['倾向']}**",
                    '置信度': '高',
                })
            elif qm_cai and ly_cai:
                differ.append({
                    '维度': '财运',
                    '六爻': ly_cai['倾向'],
                    '奇门': qm_cai['倾向'],
                    '说明': '一法看财运向上，一法看需谨慎。综合来看宜谨慎投资，小步试探。',
                })
    ...
    def _qimen_kaishi_judge(self, qimen):
        return None  # 由综合结论替代
    
    def _liuyao_kaishi_judge(self, liuyao):
        return None  # 由综合结论替代
    
    def _qimen_cai_judge(self, qimen):
        return None
    
    def _liuyao_cai_judge(self, liuyao):
        return None
