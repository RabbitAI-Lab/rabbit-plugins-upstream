...
**处理**: 解析读取页面内容并追加块的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回读取页面内容并追加块的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 创建页面并设置父级与标题
创建页面并设置父级与标题

**处理**: 解析创建页面并设置父级与标题的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回创建页面并设置父级与标题的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 查询数据库并过滤排序
查询数据库并过滤排序

**处理**: 解析查询数据库并过滤排序的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回查询数据库并过滤排序的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 创建和更新数据库行
创建和更新数据库行

**处理**: 解析创建和更新数据库行的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回创建和更新数据库行的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作

### 检查数据库架构
检查数据库架构

**处理**: 解析检查数据库架构的输入参数,完成核心逻辑,返回结构化响应.
**输出**: 返回检查数据库架构的响应数据,包含状态码、结果和日志.
- 执行此能力时使用`input_params`参数,支持创建/查询/导出操作
...
**能力覆盖范围**：本skill的核心能力覆盖以下场景关键词：Notion、API、操作页面与数据库、支持读取、更新与架构管理、知识管理领域的专、辅助工具、提供核心基础功能等。这些关键词对应description中声明的使用场景,均已在上述能力点中提供对应的操作支持.
## 使用场景
...
| 参数名 | 类型 | 必填 | 说明 |
|:-----|:-----|:-----|:-----|
| input | string | 是 | Notion 技能基础版处理的输入数据或指令 |
| options | object | 否 | 附加配置选项,如模式选择、格式偏好等 |
| callback_url | string | 否 | 异步处理完成后的回调通知URL |

```bash
notion-cli page get <page_id>
notion-cli block append <page_id> --markdown "## 新章节
这是追加的内容"
```
...
```bash
notion-cli db query <database_id> --filter '{"property":"Status","select":{"equals":"Active"}}' --sort '[{"property":"Date","direction":"descending"}]'
```

## 不适用场景

以下场景Notion 技能基础版不适合处理：
...
A: 通过 NOTION_PROFILE 环境变量切换 personal/work 等配置文件.
### Q: 可以修改数据库架构吗?

A: 可以,但必须先检查差异并获得明确确认,架构变更不可逆.
### Q: 如何升级到 PRO 版本?
...
```json
{
  "success": true,
  "data": {
    "result": "Notion 技能基础版处理结果",
    "execution_time": "0.5s",
    "metadata": {
      "version": "1.0",
      "processor": "notion skill"
    }
  },
