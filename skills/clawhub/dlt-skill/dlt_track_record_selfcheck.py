# -*- coding: utf-8 -*-
"""
大乐透 — 预测追踪自证引擎 (dlt_track_record_selfcheck.py)
===========================================================

这是"自主学习 + 自主检查"能力的旗舰模块。它把系统**自己累积的真实战绩**
(dlt_result_verify.py 维护的 dlt_performance.json: 每一期系统推荐了什么号、
...
方法:
  1. 读取 dlt_performance.json, 提取每一期每个整数分组(单式组)的
     前区命中数 front_hits ∈[0,5] 与总命中数 total_hits ∈[0,7]。
  2. 计算经验分布(均值 + 直方图)。
  3. 理论基线:
       (a) 超几何分布: 单组 5/35 前区 vs 实际 5/35 → Hypergeometric(N=35,K=5,n=5),
           期望 = 25/35 ≈ 0.7143; 后区 2/12 vs 2/12 → H(N=12,K=2,n=2), 期望 ≈ 0.3333。
           总命中 = 前区 ⊗ 后区(独立) 卷积。
       (b) 蒙特卡洛基线: 对每一期真实开奖, 生成与当日分组数相同的纯随机 5+2 选号,
           在**同一批真实开奖结果**上计分 —— 这是"纯随机在完全相同的实际结果上会得多少分"
           的最公平对照。
  4. 显著性检验:
       (a) 前区命中 z 检验: 经验均值 vs 理论期望 (用超几何方差)。
       (b) 前区 / 总命中 卡方拟合优度: 经验直方图 vs 理论 pmf。
  5. 结论:
...
用法:
  python dlt_track_record_selfcheck.py            # 跑自证, 打印诚实报告 + 写 JSON
  (被 dlt_healthcheck_all.py #23 自动调用)
"""
import json
import math
import os
...
PERF_FILE = os.path.join(WORK_DIR, "dlt_performance.json")
REPORT_FILE = os.path.join(WORK_DIR, "dlt_track_record_selfcheck.json")
...
# 大乐透参数
FRONT_N, FRONT_K, FRONT_n = 35, 5, 5     # 前区 35 选 5
BACK_N, BACK_K, BACK_n = 12, 2, 2        # 后区 12 选 2
TICKET_COST = 2
...
def load_track_record():
    """读取 dlt_performance.json, 返回 [(front_hits, back_hits, total_hits), ...] 仅整数分组。"""
    ...
    for rec in recs:
        ...
        for r in rec.get("results", []):
            if not isinstance(r.get("group"), int):
                continue  # 仅取整数单式分组(独立 5+2 选号), 胆拖单独处理
            fh = r.get("front_hits")
            bh = r.get("back_hits")
            th = r.get("total_hits")
...
def monte_carlo_baseline(records, trials_seed=MC_TRIALS_SEED):
    """对每一期真实开奖, 生成与该期分组数相同的纯随机 5+2 选号并计分, 返回 (mean_front, mean_total, n)。"""
...
# 统计检验
# ============================================================
def _z_test_2sided(emp_mean,理论_mean, sd, n):
    """双侧 z 检验, 返回 p-value (用正态近似)。"""
    if n < 2 or sd <= 0:
        return None
    se = sd / math.sqrt(n)
    z = abs(emp_mean - 理论_mean) / se
...
def _igamc(a, x):
    """正则化不完全 Gamma Q(a,x) (与 dlt_nist_sts 同源算法, 这里独立实现以保证本模块自包含)。"""
    if x <= 0.0 or a <= 0.0:
        return 1.0
    if x < a + 1.0:
        ap = a
...
def run_selfcheck(verbose=True):
    ...
    if not meta.get("exists"):
        return {"status": "no_data", "detail": "dlt_performance.json 不存在(系统尚未累积任何验证记录)"}
    if meta.get("error"):
...
def _print_report(out):
    print("=" * 72)
    print("大乐透 预测追踪自证引擎 (系统用自身累积战绩自证无优势)")
    print("=" * 72)
    ...
    print(f"  前区命中均值  经验={e['front_mean']:.3f}  理论随机={t['front_mean']:.3f}  "
          f"蒙特卡洛={m['front_mean']}")
    print(f"  总命中均值    经验={e['total_mean']:.3f}  理论随机={t['total_mean']:.3f}  "
          f"蒙特卡洛={m['total_mean']}")
    print(f"  显著性: 前区z p={x['front_z_pvalue']}  前区χ² p={x['front_chi_pvalue']}  "
