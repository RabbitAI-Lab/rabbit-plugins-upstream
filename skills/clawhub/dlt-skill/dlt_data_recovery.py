"""
大乐透数据容灾 / 恢复运维工具  (V8.7 新增)

解决"无独立备份源切换开关"短板：把多源下载做成可观测、可强制切换的运维能力。
依赖 dlt_auto 的 download_data / persist_history / validate_data / DATA_SOURCES。

用法：
  python dlt_data_recovery.py status            # 查看当前数据状态 + 最后成功源
  python dlt_data_recovery.py list              # 列出可用数据源及优先级
  python dlt_data_recovery.py force huiniao     # 强制从指定源重新下载并覆盖合并
  python dlt_data_recovery.py force 500         # 强制从500彩票网恢复
  python dlt_data_recovery.py verify            # 完整性自检（期数/格式/期号连续性）
"""
import sys
import json
import os
...
def cmd_status():
    ...
    try:
        with open('dlt_history.json', 'r', encoding='utf-8') as f:
            hist = json.load(f)
        print(f"  dlt_history.json: {len(hist)} 期")
        if hist:
            print(f"    最早: {hist[0]['period']} ({hist[0].get('date','')})")
            print(f"    最新: {hist[-1]['period']} ({hist[-1].get('date','')})")
    except Exception as e:
        print(f"  dlt_history.json: 读取失败 ({e})")
    # 最后成功源
    try:
        with open('dlt_data_source.json', 'r', encoding='utf-8') as f:
            src = json.load(f)
        print(f"  最后成功源: {src.get('source')} | 期数: {src.get('count')} | 时间: {src.get('timestamp')}")
    except Exception:
        print("  最后成功源: 无记录 (dlt_data_source.json 不存在)")
    # 优先级
...
def cmd_verify():
    ...
    try:
        ...
        print(f"✗ 无法读取 dlt_history.json: {e}")
        return 1
    issues = []
    total = len(hist)
    print(f"  总期数: {total}")
