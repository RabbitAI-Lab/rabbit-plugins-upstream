...
诚信说明(重要)：
  检验(B)中"方法选择器"前区命中球比随机高约 +0.035 球。这**不是**逆向工程成功的
...
def prize(fh, bh):
    tiers = {(5,2):1,(5,1):2,(5,0):3,(4,2):4,(4,1):5,(3,2):6,(4,0):7,(3,1):8,(2,2):9,(3,0):10,(1,2):11,(2,1):12,(0,2):13}
    return tiers.get((fh, bh), 0)
...
def main():
    ...
    # 随机期望: 前区 5*(5/35)=0.714 ; 后区 2*(2/12)=0.333
    exp_cf = 5*5/35
    exp_cb = 2*2/12
    ...
    print("="*64)
    print(f"  前区相邻重叠均值 = {mean_cf:.3f}  | 随机期望 = {exp_cf:.3f}  | 差 = {mean_cf-exp_cf:+.3f}")
    print(f"  后区相邻重叠均值 = {mean_cb:.3f}  | 随机期望 = {exp_cb:.3f}  | 差 = {mean_cb-exp_cb:+.3f}")
    ...
    print(f"  [方法选择器] 前区命中均值 = {m_sel_hit:.3f}  | 任意奖级期率 = {m_sel_prize*100:.1f}%")
    print(f"  [随机对照]   前区命中均值 = {m_rnd_hit:.3f}  | 任意奖级期率 = {m_rnd_prize*100:.1f}%")
    print(f"  [随机期望]   前区命中均值 = {exp_cf:.3f}")
    print()
    print("  各独立方法的前区命中均值 / 任意奖级期率:")
    for m in METHODS:
    ...
    print(f"  方法选择器 vs 随机 前区命中 = {sel_vs_rand:+.3f} 球")
