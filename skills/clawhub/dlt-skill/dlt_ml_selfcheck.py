# -*- coding: utf-8 -*-
"""
大乐透 ML 模型样本外自评 (V8.9 新增)

目的: 科学严谨地回答"ML 模型到底有没有用", 而不是空口声称"增加了分析视角多样性"。
方法: 在历史上逐期做 walk-forward —
  用 [0, k) 期训练 → 预测第 k 期前区 (取模型 Top5) → 与真实开奖核对命中数
  累计统计 ML 三模型(加权频率/随机森林/遗传算法)的前区命中分布, 并与纯随机基线对比。
...
用法:
  python dlt_ml_selfcheck.py            # 样本外回测(末200期)
  python dlt_ml_selfcheck.py --full    # 全量历史回测
"""
import sys
import io
import json
...
from dlt_ml_models import generate_ml_prediction
from dlt_auto import load_valid_combos, passes_filters
from dlt_period import next_period as next_period_func
...
def walk_forward(draws, start=50, end=None, valid_combos=None):
    """逐期样本外回测 ML 模型前区命中。返回每模型的命中计数累加。"""
    if end is None:
        end = len(draws)
    ...
    for k in range(start, end):
        ...
        # 随机基线: 从1-35随机取5个(不限过滤, 公平比较)
        rand = sorted(rng.sample(range(1, 36), 5))
...
def main():
    ...
    draws = json.load(open('dlt_history.json', encoding='utf-8'))
    ...
    print("\n  前区命中数分布 (越高越好; 随机期望≈5×5/35≈0.714):")
    ...
    if best_mean <= rs['mean'] + 0.05:
        print(f"  ✅ 诚实结论: ML 模型前区命中均值({best_mean:.3f}) ≈ 随机基线({rs['mean']:.3f}),")
    ...
    with open('dlt_ml_selfcheck.json', 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    print(f"  ✓ 自评报告已存: dlt_ml_selfcheck.json")
