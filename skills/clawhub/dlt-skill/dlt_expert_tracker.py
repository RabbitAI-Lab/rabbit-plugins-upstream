"""
大乐透专家战绩自算 / 随机基线对照 (V8.9.8 新增)

诚实机制: 不采信平台自报命中率(幸存者偏差 + 指标注水 + 事后篡改)。
每期开奖后, 用系统自己抓到的专家推荐 vs 实际开奖号独立打分; 同时生成
"机选基线" 用相同口径打分。任何专家的排名只有相对基线才有意义——
若专家前区均命中 ≤ 随机基线, 即证明无超额优势(与理论一致)。

用法:
  python dlt_expert_tracker.py            # 对未评分期号打分 + 打印近20期汇总
  (被 dlt_smart.py Phase 0.x 以非致命子进程调用)
"""
import json
import os
import random
...
ACC_FILE = 'dlt_expert_accuracy.json'
HIST_FILE = 'dlt_expert_history.json'
DATA_FILE = 'dlt_history.json'
...
def random_baseline(af, ab, n=500, seed=None):
    """机选基线: 生成 n 组随机 5+2 用相同口径打分, 返回均值。"""
    rnd = random.Random(seed)
    tot_f = tot_b = 0
    for _ in range(n):
        f = rnd.sample(range(1, 36), 5)
        b = rnd.sample(range(1, 13), 2)
...
def run_tracker(periods_back=9999):
    """对 dlt_expert_history.json 中、尚未评分的期号, 用 dlt_history.json 实际开奖号打分。
...
def summary(periods=20):
    """近 periods 期每专家前区均命中, 与随机基线对比, 降序。"""
    ...
    return {
        ...
        'experts': rows,
        'note': '专家前区均命中 vs 随机基线; 若专家≤基线则说明无超额优势(符合理论)。',
    }
...
if __name__ == '__main__':
    ...
    s = summary()
    if not s:
        print("暂无足够评分数据(需先有 dlt_expert_history.json 且对应期已开奖)。")
    else:
        print(f"近 {s['periods']} 期 | 随机基线前区均命中 {s['baseline_front_hits']}")
        for name, fh, bh, cnt in s['experts'][:15]:
            print(f"  {name:10s} 前区均命中 {fh}  后区均命中 {bh}  (n={cnt})")
