#!/usr/bin/env python3
"""qa-security - Create service order via clawtip (Phase 1: sends slug + question text to api.ideaidea.com.cn)"""
import argparse
import hashlib
import json
import sys
...
def create_order(question: str) -> tuple:
    ...
    req = urllib.request.Request(
        CREATE_ORDER_URL,
        data=payload,
        headers={"Content-Type": "application/json", "User-Agent": "QASecurity/1.0"},
        method="POST",
    )
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create service order for QA/security audit service")
    ...
    print("        Your source code and security credentials")
