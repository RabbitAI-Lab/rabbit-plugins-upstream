...
{
  "name": "ebay/search-ca",
  "description": "Search eBay CA (nationwide) listings (new, used, refurbished items)",
  "domain": "www.ebay.ca",
  "args": {
  ...
  "readOnly": true,
  "example": "bb-browser site ebay/search-ca 'iPhone 15 Pro' --condition used --sort price"
}
*/
async function(args) {
  if (!args.query) return { error: 'Missing argument: query' };

  const SITE_HOST = 'www.ebay.ca';
  const SITE_CURRENCY = 'C$';
  const SITE_REGION = 'CA (nationwide)';
  const SITE_SOURCE = 'ebay.ca';
