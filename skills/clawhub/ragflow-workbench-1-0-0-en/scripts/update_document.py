...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("document_id", nargs="?", help="Document ID")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Resolve dataset_id and document_id
        dataset_id = args.dataset_id
        document_id = args.document_id
        
        if dataset_id and not document_id:
            # Shift dataset_id to document_id and use default for dataset_id
            document_id = dataset_id
            dataset_id = None
            
        resolved_dataset_id = require_dataset_id(dataset_id)
        if not document_id:
            raise ConfigError("document_id is required.")
        payload = _build_payload(args)
        response = request_json(
            _build_url(base_url, resolved_dataset_id, document_id),
            api_key,
            method="PUT",
