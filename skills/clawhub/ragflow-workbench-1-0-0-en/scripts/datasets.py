...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    info_parser.add_argument("dataset_id", nargs="?", help="Dataset ID (uses default if omitted)")
...
def create_dataset(args: argparse.Namespace, *, base_url: str, api_key: str) -> dict[str, Any]:
    ...
    if dataset_id and dataset_name:
        record_dataset(dataset_id, dataset_name)
...
def _format_list(payload: dict[str, Any]) -> str:
    ...
    for dataset in payload["datasets"]:
        ...
        is_default = ds_id == default_id
        name_line = f"- {dataset.get('name') or 'unknown'}"
        if is_default:
            name_line += " (DEFAULT)"
        lines.extend(
            [
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "info":
            ...
            payload = dataset_info(dataset_id, base_url=base_url, api_key=api_key)
