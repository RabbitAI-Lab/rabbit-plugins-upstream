    ...
    if payload["document_ids"]:
        lines.extend(
            [
                "",
                "Next:",
                f"uv run python scripts/parse.py {payload['dataset_id']} {' '.join(payload['document_ids'])}",
            ]
        )
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Resolve dataset_id and files
        dataset_id = args.dataset_id
        files = args.files
        
        # Handle case where dataset_id is omitted and shifted to files
        if dataset_id and not files and os.path.exists(dataset_id):
            files = [dataset_id]
            dataset_id = None
        ...
        if args.command == "delete":
            payload = delete_documents(resolved_dataset_id, args.ids, base_url=base_url, api_key=api_key)
            print(format_json(payload) if args.json_output else _format_delete_text(payload))
            return 0

        if not files:
            raise ConfigError("No files specified for upload.")
