...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Resolve dataset_id and potentially shift from doc_ids if needed
        # But parse_status usually takes dataset_id as first arg.
        # If user provides only doc_ids, they might be in dataset_id position.
        
        dataset_id = args.dataset_id
        if dataset_id and not args.doc_ids and "," in dataset_id:
            # Heuristic: if it looks like multiple IDs, it's probably doc_ids
            target_ids = parse_doc_ids(dataset_id)
            dataset_id = None
