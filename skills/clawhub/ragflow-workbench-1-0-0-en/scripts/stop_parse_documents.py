...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if dataset_id and not document_ids:
            # Shift dataset_id to document_ids and use default for dataset_id
            document_ids = [dataset_id]
            dataset_id = None
            
        resolved_dataset_id = require_dataset_id(dataset_id)
        if not document_ids:
            raise ConfigError("No document IDs specified to stop parsing.")

        phase = "stop_parse"
        stop_request = stop_parse(resolved_dataset_id, document_ids, base_url=base_url, api_key=api_key)
        phase = "status"
