...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("dataset_id", nargs="?", help="Dataset ID (uses default if omitted)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # Resolve dataset_id and document_ids
        dataset_id = args.dataset_id
        document_ids = args.document_ids
        
        # Heuristic: if dataset_id looks like a document ID (often similar format) 
        # but document_ids is empty, and we have a default dataset, 
        # we might be misinterpreting. 
        # But for parse, document_ids are required.
        
        if dataset_id and not document_ids:
            # Check if dataset_id is actually a document_id
            # This is hard to tell, so we'll assume if only one arg is provided,
            # it's a document_id and we use default dataset_id.
            document_ids = [dataset_id]
            dataset_id = None
            
        resolved_dataset_id = require_dataset_id(dataset_id)
        if not document_ids:
            raise ConfigError("No document IDs specified for parsing.")

        parse_request = start_parse(resolved_dataset_id, document_ids, base_url=base_url, api_key=api_key)
