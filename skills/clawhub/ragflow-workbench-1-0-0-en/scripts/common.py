...
DATASETS_RECORD_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)), "datasets.json")
HTTP_TIMEOUT = 30
...
def load_datasets_record() -> dict[str, Any]:
    if not os.path.exists(DATASETS_RECORD_FILE):
        return {"datasets": [], "default_dataset_id": None}
    try:
        with open(DATASETS_RECORD_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return {"datasets": [], "default_dataset_id": None}


def save_datasets_record(record: dict[str, Any]) -> None:
    try:
        with open(DATASETS_RECORD_FILE, "w", encoding="utf-8") as f:
            json.dump(record, f, ensure_ascii=False, indent=2)
    except Exception as exc:
        print(f"Warning: Failed to save datasets record: {exc}", file=sys.stderr)


def record_dataset(dataset_id: str, name: str) -> None:
    record = load_datasets_record()
    ...
    for ds in datasets:
        if ds.get("id") == dataset_id:
            ds["name"] = name
            save_datasets_record(record)
            return

    datasets.append({"id": dataset_id, "name": name})
    record["datasets"] = datasets
    
    # Set default if it's the first one
    if not record.get("default_dataset_id"):
        record["default_dataset_id"] = dataset_id
...
def get_default_dataset_id() -> str | None:
    record = load_datasets_record()
    return record.get("default_dataset_id")


def require_dataset_id(dataset_id: str | None) -> str:
    ...
    raise ConfigError("Dataset ID is required, and no default dataset is set. Please create a dataset first or specify the ID.")
