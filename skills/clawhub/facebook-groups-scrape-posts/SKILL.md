...
permissions:
  ...
  credentials_access: facebook_session  # reads DTSGInitialData.token, LSD.token, CurrentUserInitialData.USER_ID from logged-in session; tokens are used solely to authenticate requests to facebook.com and are never transmitted to any third-party endpoint
---
...
>
> **Token usage**: `DTSGInitialData.token` and `LSD.token` are read from the current page's JS module registry solely to authenticate requests to `facebook.com/api/graphql/` on behalf of the logged-in user. These tokens are never transmitted to any third-party endpoint.
>
> **Eval scope**: All JS executed via `browser-act eval` is statically generated from `scripts/scrape-posts.py`. No user-supplied strings are interpolated into eval payloads at runtime.
...
**Large-count confirmation**: when `--count > 50`, confirm with the user before starting pagination — e.g., "This will paginate up to N pages to collect {count} posts. Proceed?" — to prevent unintended large-scale scraping.
