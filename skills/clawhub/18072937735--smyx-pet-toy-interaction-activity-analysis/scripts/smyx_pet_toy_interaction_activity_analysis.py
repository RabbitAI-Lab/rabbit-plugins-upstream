...
def main():
    parser = argparse.ArgumentParser(description="宠物玩具互动活跃度分析工具")
    parser.add_argument("--input", help="本地玩具区域视频文件路径")
    parser.add_argument("--url", help="网络玩具区域视频MP4的URL地址")
    parser.add_argument("--pet-type", choices=["cat", "dog", "other"], default=ConstantEnum.DEFAULT__PET_TYPE,
                        help="宠物类型：cat(猫), dog(狗), other(其他)，默认 cat")
    parser.add_argument("--open-id", required=True, help="当前用户的OpenID/UserId/用户名/手机号")
    parser.add_argument("--list", action='store_true', help="显示宠物玩具互动历史分析列表清单")
