...
version: "1.0.0"
displayName: "报表配置基础版"
summary: "自定义报表基础能力，支,提供核心能力"
summary_zh: "自定义报表基础能力，支,提供核心能力"
license: "MIT"
description: "|- 自定义报表基础能力，兼容单一数据源接入、基础字段映射 与标准格式导出. 核心能力:.该技能适用于相关开发场景,包含结构化的工作流程和配置指引.经过深度差异化处置,针对用户反馈和使用痛点进行了改进,提升了实用性和可操作性.该技能适用于相关开发场景,包含结构化的工作流程和配置指引.经过深度差异化处置,针对用户反馈和使用痛点进行了改进,提升了实用性和可操作性."

  - 单一数据源的基础报表配置

  - 字段映射与基础聚合（求和、计数、平均值）

  - 标准 Markdown / JSON / CSV 格式导出
  ...
  差异化:基础版聚焦单一数据源的报表产出，去除了高级版的多源合并、复杂调度与可视化图表能力.
tags:
  - Other
  - 工具
  - 效率
...
- 报表元信息记录：记录数据源、生成时间、记录数等基础元信息 关键词: report
#
## 快速开始
...
| 网络错误 | 远程数据源连接超时 | 检查网络连接和配置后重试，建议优先使用本地数据源 |
