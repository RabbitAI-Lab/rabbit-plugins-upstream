...
async function processAgentMessage(params: {
  ...
  if (useDynamicAgent) {
    const dmOrGroup: "dm" | "group" = isGroup ? "group" : "dm";
    const targetAgentId = generateAgentId(dmOrGroup, peerId, agent.accountId);
    route.agentId = targetAgentId;
    route.sessionKey = `agent:${targetAgentId}:wecom:${agent.accountId}:${dmOrGroup}:${peerId}`;
    // 异步注册（cfg.agents.list + workspace/agentDir + cfg.bindings）不阻塞 dispatch
    ensureDynamicAgentListed({
