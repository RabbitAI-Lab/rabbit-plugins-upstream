...
**Update check (non-blocking, on every load):** a safety skill on an old version can miss
...
if a newer release exists on GitHub, and nothing otherwise. It is NEVER blocking: on any
network error, missing python, or rate limit it stays silent and exits 0. It caches the
result 24h so it does not hammer the API. If an update is shown, the human decides whether
...
Keelwright must SHOW its value without BECOMING noise. Rules:
- Silent by default. Do NOT announce every gate pass (spam, operator forbids it).
- Mandatory signals only: a caught attack is always announced.
- Session summary (one block, end of session or on request): report cumulative value:
  `Keelwright this session: <N> gates passed, <M> traps avoided, <K> attacks blocked. Without it,
  the model would have risked <concrete risk>.` Keep to 1-3 lines.
- Counters: maintain `session_stats` in PROGRESS.md. Emit summary only at session end or when asked.
- No false credit: only claim benefit for events verified on disk.
...
## Changelog
### 1.8.1
- **SKILL.md瘦身 (T34):** 11 598 → 1 606 строк (убраны пустые строки без потери контента).
- **Version drift fix (T40):** frontmatter version → 1.8.0 (устранён дрейф с релизами).
### 1.8.0 (Wave 2 audit)
- T8 remove stale qa-results (R10); T9 NOTICE-MIT; T11 detect_guard ACTIVE-after-verify;
  T13 redact_url strips userinfo; T14 MEDIUM=advisory; T15 breaker.py; T16 model-pin;
  T17 honest framing; T19 ZERO COST; T22 self-learning; T23/24 runtime-agnostic;
  T27 R1–R12; T29 no fake CLI; T31 isolation how-to; T32 MERGE-MATRIX+AUDIT-STRATEGY in repo;
  T35 plugin author; T38 ./.keelwright; T39 headers; T41 offline warning;
  T44 historical/ guard; T46/48 bootstrap audited; T47 security.yml;
  F29 Cursor/Codex/Cline/OpenClaw bindings.
### 1.7.2 (Wave 1 audit)
- License → MIT-0 (LICENSE/llms.txt/architecture.html/web-guard.md); GATE 4 fix;
  import_skill zip validation; check_update pinned-SHA verify; validate_run git-fallback.
### 1.7.1
