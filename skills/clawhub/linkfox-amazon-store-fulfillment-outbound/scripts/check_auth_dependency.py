#!/usr/bin/env python3
"""
Dependency Check - linkfox-amazon-store-fulfillment-outbound
======================================================
...
注意:
    ...
    真正能否调用取决于已授权的 `sellerId` + `region`；业务脚本直接调用
    `/spApi/developerProxy`，由服务端解析并在需要时刷新 token。
...
def main() -> None:
    ...
    payload = {
        ...
        "reason": (
            f"linkfox-amazon-store-fulfillment-outbound 依赖 `{REQUIRED_SKILL}`，"
