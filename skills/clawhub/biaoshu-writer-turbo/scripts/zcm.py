...
凭证读取优先级：环境变量 > skill 内 config.json（旧版 ~/.zcm/config.json 仍可读，兼容已有安装）。
...
转述给用户时按顺序完整给出，链接原样显示完整 URL：
  ...
  第 1 步 · 注册拿 Key（只需手机号）
    ...
    3) 点左侧菜单『Skill 接入 → 获取 APP Key』，在面板里查看/复制 App Key（首次打开自动生成，形如 bk_live_xxxxx）
...
凭证安全：App Key 是账户的完整凭证，全程由用户本人写入本地文件——
  不要在对话里索取、让用户粘贴或复述 Key。Key 泄露或想作废时，随时到官网
  左侧菜单『Skill 接入 → 获取 APP Key』重置，旧 Key 立即失效。
"""
