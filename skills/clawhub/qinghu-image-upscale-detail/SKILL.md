---
name: qinghu-image-upscale-detail
description: 青虎AI 超清修复强化细节质感：采用分块放大算法对图片做超清修复与放大，完整留存原图原有细节不篡改，适配商品图、人像、景物等全品类图像优化。当用户要放大图片、图片太糊、要高清化、要补细节、印刷需要高分辨率、老图翻新时必须触发。关键词：青虎AI、超清修复、图片放大、高清化、分块放大、细节增强、商品图优化、人像、景物、印刷。
user-invocable: true
...
图片**放大 + 补细节**，用的是分块放大算法：**只放大不篡改**，原图有什么就还原什么。

和「去 AI 感」类应用的区别：这个不改风格、不改质感，只是把图做大做清楚。适合商品图、人像、景物全品类。固定计费（快照 2 积分/次）。
...
```bash
# 字段表
qhkit workflow options '{"workflow":"wf_050"}'

# 报价（固定计费）
qhkit workflow estimate '{"workflow":"wf_050","fields":{"上传图片":"./素材/商品图.jpg"}}'

# 提交
qhkit workflow generate '{"workflow":"wf_050","fields":{"上传图片":"./素材/商品图.jpg"}}'
...
| 诉求 | 用哪个 |
| --- | --- |
| 只要更大更清楚，**内容一点都不能变** | 本应用（超清修复强化细节质感） |
| 图片有 AI 油腻感，要更写实 | 图片高清写实去AI感 |
| 模特图皮肤假 | 模特图去AI感超写实 |
...
- 只做放大与细节修复，**不改风格、不改内容**；要改质感走「图片高清写实去AI感」或「模特图去AI感超写实」。
- 一次一张，批量要逐张提交。
- 视频超清走「商品视频画质超清提升」。
