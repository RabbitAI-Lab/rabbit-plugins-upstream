...
# ========== MCP URL 安全校验 ==========
#
# 该脚本会带着 mcp_token (== 用户的微云访问凭据) 向 mcp_url POST 请求。如果
# mcp_url 来自不可信源（命令行、WEIYUN_MCP_URL 环境变量、CI 注入等）并被指
# 向攻击者控制的主机，token 就会被发送到任意外网域名，造成凭据外泄。
#
# 因此对 mcp_url 必须执行严格校验：
#   1) 仅允许 https:// 方案；
#   2) 主机名必须命中受信任的后缀白名单；
#   3) 调试场景需要显式设置 WEIYUN_MCP_URL_ALLOW_INSECURE=1 才能放行，
#      且会在 stderr 打印明显告警，避免静默信任。

# Trusted host suffixes for the MCP endpoint. Keep this list minimal.
_TRUSTED_MCP_HOST_SUFFIXES = (
    "weiyun.com",
    "qq.com",
)


def _validate_mcp_url(url, *, allow_insecure=False):
    """Validate ``url`` so a tainted env / CLI value cannot exfiltrate the token.

    Returns the normalized URL on success, raises ``ValueError`` otherwise.
    Set ``allow_insecure=True`` only for local debugging (a loud warning is
    ...
    if parsed.scheme not in ("https", "http"):
        raise ValueError(
            f"非法的 MCP URL scheme: {parsed.scheme!r}（仅支持 https）"
        )
    host = (parsed.hostname or "").lower()
    if not host:
        raise ValueError(f"MCP URL 缺少主机名: {url!r}")

    is_trusted_host = any(
        host == suffix or host.endswith("." + suffix)
        for suffix in _TRUSTED_MCP_HOST_SUFFIXES
    )
    is_https = parsed.scheme == "https"

    if is_https and is_trusted_host:
        return url

    if allow_insecure:
        print(
            f"[security] ⚠️ WEIYUN_MCP_URL_ALLOW_INSECURE=1 已开启，"
            f"放行非受信任的 MCP URL: {url}（仅用于本地调试，"
            f"请勿在生产 / 共享环境中使用）",
            file=sys.stderr,
        )
        return url
    ...
    if not is_https:
        raise ValueError(
            f"出于安全考虑，MCP URL 必须使用 https://；当前: {url!r}。"
            "本地调试时请显式设置 WEIYUN_MCP_URL_ALLOW_INSECURE=1。"
        )
    raise ValueError(
        f"MCP URL 主机 {host!r} 不在受信任列表中（{', '.join(_TRUSTED_MCP_HOST_SUFFIXES)}），"
        "拒绝向其发送 mcp_token。如确属调试需要，请显式设置 "
        "WEIYUN_MCP_URL_ALLOW_INSECURE=1。"
    )
...
def mcp_call(mcp_url, headers, tool_name, arguments, max_retries=3):
    ...
    """
    # Defense-in-depth: even though main() already validated mcp_url before
    # any upload starts, we re-run the sanitizer here so that the taint flow
    # `os.environ.get -> requests.post` is provably terminated immediately
    # before the network sink. ``allow_insecure`` is intentionally False:
    # the loud opt-in warning has already been printed in main(), and this
    # second check must never silently accept a tainted URL.
    safe_mcp_url = _validate_mcp_url(
        mcp_url,
        allow_insecure=(os.environ.get("WEIYUN_MCP_URL_ALLOW_INSECURE") == "1"),
    )
    global _request_id
    for attempt in range(1, max_retries + 1):
        ...
        resp = requests.post(safe_mcp_url, headers=headers, json=payload, timeout=120)
...
""",
    ...
    # 安全校验：mcp_url 来自不可信源（命令行/环境变量），必须先校验后才能
    # 携带 mcp_token 发送，避免凭据被转发到攻击者控制的主机。
    allow_insecure = os.environ.get("WEIYUN_MCP_URL_ALLOW_INSECURE") == "1"
    try:
        mcp_url = _validate_mcp_url(mcp_url, allow_insecure=allow_insecure)
