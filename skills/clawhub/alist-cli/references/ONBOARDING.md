...
> Windows/Codex：以下 `python3` 可替换为 `python`。不需要创建全局 symlink，可直接运行 `python <plugin_root>/scripts/alist_cli.py <command>`。
...
Windows PowerShell 可直接调用：

```powershell
python "<plugin_root>\scripts\alist_cli.py" --help
```
...
```powershell
$env:ALIST_URL = "https://..."
$env:ALIST_USERNAME = "<username>"
$env:ALIST_PASSWORD = "<password>"
```

密码和 token 不得写入仓库。Agent 未获用户明确同意时，不把密码追加到 shell profile 或其他持久化配置。
