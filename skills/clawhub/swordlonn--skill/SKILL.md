...
> ⚠️ **安全提示**：此技能会捕获屏幕画面，可选允许远程鼠标/键盘控制。请使用 `--permission view` 仅查看模式。使用完毕后按 Ctrl+C 关闭桥接服务。共享屏幕时请注意遮挡敏感内容。
...
**仅当用户明确要求屏幕共享或远程监控时使用此技能**：
- "分享屏幕"、"屏幕共享"、"共享我的屏幕" — 启动屏幕共享
- "远程控制电脑"、"帮我操作电脑" — 以控制权限启动
- "远程查看电脑"、"用手机看电脑" — 以仅查看模式启动
- "watchitai share"、"启动 watchitai" — 启动屏幕共享

**不要**在用户只是说"帮我看看电脑问题"等无明确屏幕共享意图时激活此技能。
...
# ⚠️ --permission control 授予远程鼠标/键盘控制权限。建议使用 --permission view 仅查看模式。
```
...
技能包仅含脚本和配置（~30KB），**平台二进制在首次运行时自动从 GitHub Releases 下载**。无需 Node.js，无需 npm install。
...
> ```
>
> 首次运行 `run.sh share` 或 `run.sh link` 时会自动从 `https://watchitai.net/downloads/` 下载对应平台的 Go 二进制到 `bin/` 目录（约 11MB），之后不再重复下载。
...
|------|------|------|------|
| `/health` | GET | 无 | 健康检查 |
| `/permissions` | GET | bridgeToken | 权限状态 |
| `/screenshot` | GET | bridgeToken | 当前截图 (PNG) |
| `/create-session` | POST | bridgeToken | 创建会话（返回 shareUrl + localUrl） |
| `/api/*` | ALL | API 自有认证 | API 代理（转发到 watchitai.net） |

## 安全

- **本地桥接认证**：所有敏感 HTTP 端点（截图、会话创建、通知、控制）均需 bridgeToken 认证，防止其他本地进程未授权调用
- **仅监听 localhost**：桥接服务绑定 `127.0.0.1`，不暴露到公网
- **P2P 端到端加密**：屏幕视频流使用 WebRTC DTLS-SRTP，不经过服务器中转
- **会话级 token**：会话链接包含 32 位随机 token，会话结束即失效
- **输入净化**：鼠标 / 键盘控制指令在桥接服务端进行参数校验

### 安全使用建议

- **优先使用 view-only 模式**：`--permission view` 仅允许查看，不允许远程控制
- **会话结束后及时关闭**：按 Ctrl+C 停止桥接服务，避免端口残留
- **敏感信息遮挡**：共享屏幕时注意遮挡密码、私钥等敏感内容
- **bridgeToken 敏感**：本地主机页面 URL 中的 bridgeToken 仅为本机使用，不要分享给他人

## 文件结构

```
watchitai/
├── run.sh                # Unix 入口脚本（自动检测平台，首次运行自动下载二进制）
├── run.cmd               # Windows 入口脚本（首次运行自动下载二进制）
├── config.json           # 配置
├── SKILL.md
├── bin/                  # 首次运行时自动创建，存放下载的平台二进制
│   └── watchitai-{platform}-{arch}  # 按需下载，仅当前平台
```
