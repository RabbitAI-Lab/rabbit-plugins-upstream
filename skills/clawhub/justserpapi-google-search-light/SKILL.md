---
name: Google SERP Search Light API
description: Call GET /api/v1/google/search/light for Google SERP Search Light through Just Serp API with query.
author: Just Serp API
homepage: https://justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_search_light&utm_content=project_link
metadata: {"openclaw":{"homepage":"https://justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_search_light&utm_content=project_link","primaryEnv":"JUST_SERP_API_KEY","requires":{"bins":["node"],"env":["JUST_SERP_API_KEY"]},"skillKey":"justserpapi_google_search_light"}}
---

# Google SERP Search Light

Use this focused Just Serp API skill for Google SERP Search Light. It targets `GET /api/v1/google/search/light`. Required inputs are `query`. OpenAPI describes it as: Get Google light Search SERP data, including essential result data, for high-volume monitoring and fast rank checks.

## Endpoint Scope

- Group key: `google`
- Endpoint key: `search/light`
- Group family: Google SERP
- Skill slug: `justserpapi-google-search-light`

| Operation | Version | Method | Path | OpenAPI summary |
| --- | --- | --- | --- | --- |
| `searchLight` | `v1` | `GET` | `/api/v1/google/search/light` | Light Search |
...
Use `searchLight` for the documented `v1` endpoint. There are no alternate versions grouped in this skill.

## Run This Endpoint

Supported operation IDs in this skill: `searchLight`.

```bash
node {baseDir}/bin/run.mjs --operation "searchLight" --api-key "$JUST_SERP_API_KEY" --params-json '{"query":"<query>"}'
```
...
- Project site: [Just Serp API](https://justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_search_light&utm_content=project_link).
- Authentication details: [Just Serp API Docs](https://docs.justserpapi.com/?utm_source=clawhub.ai&utm_medium=referral&utm_campaign=justserpapi_google_search_light&utm_content=project_link).

## Output Focus

- State the operation ID and endpoint path used, for example `searchLight` on `/api/v1/google/search/light`.
- Echo the required lookup scope (`query`) before summarizing results.
- Prioritize fields that support this endpoint purpose: Get Google light Search SERP data, including essential result data, for high-volume monitoring and fast rank checks.
