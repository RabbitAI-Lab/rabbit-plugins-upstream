...
~~~text
审查主 ASIN 与竞品商品页的字段完整度和证据一致性。商品 B0XXXXXXXX（美国站），竞品 B0YYYYYYYY（美国站）。
~~~

示例 ASIN 是占位符。AI 会识别商品、站点和目标，无需填写接口参数。
客户端未选中时，在问题前加“使用 $amazon-competitor-page-audit”。
...
固定工作流：`page_compare/page`，输出 `ops_page_compare`。
