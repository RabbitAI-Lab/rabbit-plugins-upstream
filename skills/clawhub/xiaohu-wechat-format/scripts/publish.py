...
def load_config():
    """Load config.json, falling back to safe formatting defaults.

    Publishing still requires users to copy config.example.json to config.json
    and fill wechat.app_id/app_secret, but argparse --help and dry inspection
    should not crash on a fresh install.
    """
...
def detect_image_format(image_path):
    """Return actual image format from magic bytes: jpeg/png/gif/webp/unknown."""
...
def ensure_wechat_image(image_path):
    """Return (path, content_type, cleanup_path) suitable for WeChat upload.

    WeChat article images are often downloaded with a .png/.jpg suffix even
    when the real bytes are WebP. Uploading those files fails with errcode
    40137 (invalid image format). Detect the real format before upload and
    convert WebP to JPEG when Pillow is available.
    """
    image_path = str(image_path)
    actual = detect_image_format(image_path)
    suffix_by_format = {"jpeg": ".jpg", "png": ".png", "gif": ".gif"}
    ctype_by_format = {
        "jpeg": "image/jpeg",
        "png": "image/png",
        "gif": "image/gif",
    ...
    if actual in suffix_by_format:
        expected_suffix = suffix_by_format[actual]
        if Path(image_path).suffix.lower() in {expected_suffix, ".jpeg" if actual == "jpeg" else expected_suffix}:
            return image_path, ctype_by_format[actual], None
        tmp = tempfile.NamedTemporaryFile(suffix=expected_suffix, delete=False)
        tmp.close()
        shutil.copyfile(image_path, tmp.name)
        return tmp.name, ctype_by_format[actual], tmp.name
    ...
    if actual == "webp":
        try:
            ...
            raise RuntimeError(
                "图片实际格式是 WebP，微信素材接口不接受。请安装 Pillow "
                "或先转成 JPEG：uv run --with pillow ..."
            ) from e
        tmp = tempfile.NamedTemporaryFile(suffix=".jpg", delete=False)
        tmp.close()
        with Image.open(image_path) as im:
            im.convert("RGB").save(tmp.name, "JPEG", quality=92, optimize=True)
        return tmp.name, "image/jpeg", tmp.name
...
def upload_thumb_image(token, image_path):
    ...
    upload_path, content_type, cleanup_path = ensure_wechat_image(image_path)
...
def upload_content_image(token, image_path, max_retries=3):
    ...
    try:
        ...
        print(f"  ✗ 图片格式处理失败 - {os.path.basename(image_path)}: {e}")
        return None
