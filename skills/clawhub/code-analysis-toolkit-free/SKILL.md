...
description: "个人Git历史自查工具,生成提交模式、代码风格、质量指标的描述性报告"
license: Proprietary
...
**处理**: 按照skill规范执行使用政策(必须遵守)操作,遵循单一意图原则。
**输出**: 返回使用政策(必须遵守)的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行六大分析维度操作,遵循单一意图原则。
**输出**: 返回六大分析维度的执行结果,包含操作状态和输出数据。
...
**处理**: 按照skill规范执行自我反思叙事操作,遵循单一意图原则。
**输出**: 返回自我反思叙事的执行结果,包含操作状态和输出数据。
