#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""病历实体抽取与结构化 — 独立 skill，可单独拷贝发布；不依赖其他 skill 或 _shared。"""
...
DEFAULT_SYSTEM_PROMPT = (
    "你是专业医学大模型。请严格按用户消息中的题目与输出格式要求作答，"
    "不要输出与解题无关的寒暄；不要编造题干未给出的检验数值、量表结果或隐私信息。"
)
...
def collect_jobs(args: argparse.Namespace) -> List[Tuple[int, Dict[str, Any], str, str, Optional[str]]]:
    """返回 (record_index, meta, question, input_mode, input_path)。"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="病历实体抽取与结构化：由调用方传入题目，直连内部医疗大模型。")
    p.add_argument("--question", default="", help="题目全文（与 --input、stdin 三选一）。")
    p.add_argument("--input", default="", help="题目来源文件：.jsonl（按 --index/--batch/--match-id）、.json（单条含 question），或任意文本文件；传 - 则从 stdin 读 JSON/文本。")
