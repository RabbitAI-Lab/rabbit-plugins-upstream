...
> 前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=clawhub) 注册获取 API Key。

---

## 后续使用

可前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=clawhub) 注册账号获取自己的 API Key，三种配置方式任选其一：
...
A：需要。前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=clawhub) 注册获取自己的 API Key，通过环境变量 `REDFOX_API_KEY` 或 `--api-key` 参数配置。
...
如有其他问题，可前往 [redfox.hk](https://redfox.hk/settings/api-keys?source=clawhub) 查看平台文档或联系客服。
