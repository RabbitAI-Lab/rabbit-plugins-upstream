---
name: opc-skill-content-policies
description: MCP Skill: read OPC policy news for solo entrepreneurs and self-employed
version: 0.1.0
metadata:
  openclaw:
    requires: {}
...
| 工具名 | 功能 | 示例问题 |
|--------|------|---------|
| `opc_list_policies` | 读取政策资讯列表 | 「最新税收政策」「上海的社保政策」 |
...
| 返回 | 含义 |
|------|------|
| `❌ 获取政策失败：unauthenticated` | 缺少或错误的 API Key |
| `❌ 获取政策失败：invalid_key` | Key 被吊销 |
| `❌ 获取政策失败：forbidden` | Key 权限不足（需 read 权限） |
| `❌ 获取政策失败：internal_error` | 服务器异常 |
