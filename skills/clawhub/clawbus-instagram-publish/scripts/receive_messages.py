...
def receive_messages(api_key: str, connection_id: str, limit: int) -> dict:
    ...
    print(f"Error receiving messages: HTTP {response.status_code}", file=sys.stderr)
    print(response.text, file=sys.stderr)
    sys.exit(1)


def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch Instagram direct messages via MyBrandMetrics API")
    ...
    parser.add_argument("--limit", type=int, default=25, help="Number of messages to fetch")
    ...
    result = receive_messages(api_key, connection_id, args.limit)
