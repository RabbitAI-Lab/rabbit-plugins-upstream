...
- **UserPromptSubmit**: Reminder after matching prompts to evaluate deal learnings
...
```json
{
  "hooks": {
    "UserPromptSubmit": [
      {
        "matcher": "deal|pipeline|forecast|objection|competitor|pricing|quota|MEDDIC",
        "hooks": [
          {
            "type": "command",
...
2. **Narrow the matcher further**. Current domain matcher examples: sales-related prompts:
