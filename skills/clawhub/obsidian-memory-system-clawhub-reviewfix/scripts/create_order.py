#!/usr/bin/env python3
"""obsidian-memory-system - Order Creation Script (Phase 1)

Creates local order file for clawtip payment processing.
The AI model provides memory management in conversation after payment.
Order file: ~/.openclaw/skills/orders/{indicator}/{order_no}.json
Stores: payTo, amount, order_no, encrypted_data, slug, question, description, resource_url.
These are standard clawtip order fields for payment verification.
No Obsidian notes, vault content, or project files are stored.
"""
import argparse
import hashlib
import json
...
AMOUNT = 190  # 1.9 yuan = 190 fen
DESCRIPTION = "Obsidian persistent memory system for AI agents"
...
# ============================================================
# SM4 encryption for clawtip payment credential
# Encrypts {orderNo, amount, payTo} for fee processing only.
# ============================================================
    ...
    if sm4_key:
        try:
            if len(sm4_key) == 32 and all(c in "0123456789abcdefABCDEF" for c in sm4_key):
                ...
                import base64
                key_hex = base64.b64decode(sm4_key).hex()
...
if __name__ == "__main__":
    ...
    print("        Fields stored: orderNo, amount, question, slug, payTo,")
    print("        encrypted_data, description, resource_url.")
    print("        These are clawtip payment fields only.")
    print("        Caution: do not include passwords, API keys, or secrets")
    print("        in the question, as it is saved to a local file.")
    print("=" * 60)
