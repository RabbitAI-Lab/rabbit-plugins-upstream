#!/usr/bin/env python3
"""
Discord Bulk Message Purge Tool
====================================

A simple, reusable Python script for deleting messages from Discord channels.
No npm/discord.js dependencies required - uses only Python standard library.

USAGE:
  python discord-purge-tool.py <count> --channel-id <channel_id>
...
def purge_messages(channel_id, count=5):
    """
    Delete the last N messages from a Discord channel.
    ...
    # Extract message IDs for bulk deletion (max 100 per Discord API batch)
    ...
    # Delete the messages - Discord expects {"messages": [...]} format
    try:
...
def main():
    ...
    if not channel_id:
        ...
        print("\nUSAGE:")
        print('  python discord-purge-tool.py purge <count> --channel-id <id>')
        return
    ...
    # Execute purge
    success, count_deleted, deleted_ids = purge_messages(channel_id, count)
