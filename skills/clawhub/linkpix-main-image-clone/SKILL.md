...
「图1 爆款参考 + 图2 自己商品」双参考图复刻构图与视觉风格：底层是 `qhkit image` 自定义生图。
...
走官方「主图复刻」通道（`image-batch`，独立 SKU `IMAGE_SET_REPLICATION`，与 LinkPix 工作台主图复刻卡片同链路）：参考图只用于 AI 分析风格版式，出图只用你的产品图，天然不会把参考图里的商品或品牌元素带出来。

```bash
# 参考图（1–10 张，每张出一张）+ 我的产品图（1–6 张）
qhkit image-batch generate '{"mode":"主图复刻","referenceImages":["./爆款参考图.jpg"],"productImages":["./我的商品图.jpg"]}'
# 逐张指定复刻要求：元素写成 {url, hint}
qhkit image-batch generate '{"mode":"主图复刻","referenceImages":[{"url":"./爆款参考图.jpg","hint":"只借鉴构图与配色，文案换成「买一送一」"}],"productImages":["./我的商品图.jpg"]}'
# 去掉参考图上的文字
qhkit image-batch generate '{"mode":"主图复刻","referenceImages":["./爆款参考图.jpg"],"productImages":["./我的商品图.jpg"],"removeText":true}'
# 报价（同参数换 estimate）
qhkit image-batch estimate '{"mode":"主图复刻","referenceImages":["./爆款参考图.jpg"],"productImages":["./我的商品图.jpg"]}'
```

- **出图张数 = 参考图张数**（上限 10 张），没有 `imageCount` 参数；`productImages` 1–6 张，是同一个产品的多个角度，不会按产品图数量翻倍出图。
- **当前仅支持 1:1**，尺寸参数在这个 mode 下不生效。
- 参考图里有文案版式时：要保留版式但换成自己的文案 → 写进该张的 `hint`；要整体去掉参考图文字 → 传 `"removeText":true`。
- 需要多模型对比、或参考图与产品图必须放进同一条提示词描述时，才退回 `qhkit image generate` 双图 + 提示词的老写法（此时务必写明「不要出现图1中的商品、品牌名、logo 或水印」）。
...
- 复刻**详情页**走「电商商品详情页复刻助手 | LinkPix」；复刻**视频**走「AI爆款视频复刻 | LinkPix」。
- 直接照搬他人图片素材（不换商品）不属于本技能，涉及侵权风险应提醒用户。
